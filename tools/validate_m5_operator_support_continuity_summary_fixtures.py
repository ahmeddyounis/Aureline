#!/usr/bin/env python3
"""Validate the operator/support continuity summary fixtures against their schema.

The fixtures under ``fixtures/continuity/operator_support_summary_cases`` and the
canonical artifact under ``artifacts/m5/continuity`` are emitted by ``cargo run
-p aureline-continuity --example
dump_m5_operator_support_continuity_summary_fixtures``. They are page records, a
summary record, and a support-export record; every file must validate against
...
def main() -> int:
    ...
    fixture_dir = repo_root / "fixtures/continuity/operator_support_summary_cases"
    artifact = repo_root / "artifacts/m5/continuity/operator_support_continuity_summary.json"
    ...
    if not fixture_dir.exists():
        print(
            f"Missing fixtures directory: {fixture_dir.relative_to(repo_root)}",
            file=sys.stderr,
        )
        return 2
    ...
    print(
        "OK: validated "
        f"{len(fixture_paths)} operator/support continuity fixture(s) against "
