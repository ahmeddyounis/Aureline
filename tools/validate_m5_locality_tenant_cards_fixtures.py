#!/usr/bin/env python3
"""Validate the locality-descriptor and tenant-card fixtures against their schema.

The fixtures under ``fixtures/continuity/locality_tenant_cases`` are emitted by
``cargo run -p aureline-continuity --example
dump_m5_locality_tenant_cards_fixtures``. They are card-page records, a summary
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas/continuity/locality_descriptor.schema.json"
    fixture_dir = repo_root / "fixtures/continuity/locality_tenant_cases"
    ...
    print(
        "OK: validated "
        f"{len(fixture_paths)} locality-descriptor and tenant-card fixture(s) against "
