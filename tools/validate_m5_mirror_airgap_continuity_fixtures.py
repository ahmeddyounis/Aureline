#!/usr/bin/env python3
"""Validate the mirror-only and air-gapped continuity fixtures against their schema.

The fixtures under ``fixtures/continuity/mirror_airgap_cases`` are emitted by
``cargo run -p aureline-continuity --example
dump_m5_mirror_airgap_continuity_fixtures``. They are page records, a summary
record, an offline-continuity registry record, and a support-export record, plus
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas/continuity/mirror_airgap_packet.schema.json"
    fixture_dir = repo_root / "fixtures/continuity/mirror_airgap_cases"
    artifact_dir = repo_root / "artifacts/m5/continuity/mirror_airgap"
    ...
    print(
        "OK: validated "
        f"{len(fixture_paths)} mirror/air-gap continuity fixture(s) against "
