#!/usr/bin/env python3
"""Validate the restore-from-backup review continuity fixtures against their schema.

The fixtures under ``fixtures/continuity/post_restore_narrowing`` are emitted by
``cargo run -p aureline-continuity --example
dump_m5_restore_from_backup_review_fixtures``. They are page records, a summary
record, a restore-review registry record, and a support-export record, plus
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas/continuity/restore_identity_summary.schema.json"
    fixture_dir = repo_root / "fixtures/continuity/post_restore_narrowing"
    artifact_dir = repo_root / "artifacts/m5/continuity/restore_reviews"
    ...
    print(
        "OK: validated "
        f"{len(fixture_paths)} restore-from-backup review fixture(s) against "
