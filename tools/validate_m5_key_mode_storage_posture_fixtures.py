#!/usr/bin/env python3
"""Validate the key-mode and storage-posture fixtures against their schema.

The fixtures under ``fixtures/continuity/key_mode_failure_cases`` are emitted by
``cargo run -p aureline-continuity --example
dump_m5_key_mode_storage_posture_fixtures``. They are page records, a summary
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas/continuity/key_mode_descriptor.schema.json"
    fixture_dir = repo_root / "fixtures/continuity/key_mode_failure_cases"
    ...
    print(
        "OK: validated "
        f"{len(fixture_paths)} key-mode and storage-posture fixture(s) against "
