#!/usr/bin/env python3
"""Validate the control-plane-versus-data-plane outage fixtures against their schema.

The fixtures under ``fixtures/continuity/outage_taxonomy`` are emitted by
``cargo run -p aureline-continuity --example
dump_m5_control_plane_vs_data_plane_outage_fixtures``. They are page records, a
...
def main() -> int:
    ...
    schema_path = repo_root / "schemas/continuity/control_vs_data_plane_packet.schema.json"
    fixture_dir = repo_root / "fixtures/continuity/outage_taxonomy"
    ...
    print(
        "OK: validated "
        f"{len(fixture_paths)} control-plane-versus-data-plane outage fixture(s) against "
