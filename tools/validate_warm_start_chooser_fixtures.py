...
def main() -> int:
    ...
    schema_path = repo_root / "schemas/entry/freshness_revalidation.schema.json"
    fixture_dir = repo_root / "fixtures/entry/warm_start_cases"
    ...
    print(
        "OK: validated "
        f"{len(fixture_paths)} warm-start chooser fixture(s) against {schema_path.relative_to(repo_root)}"
    )
    return 0
