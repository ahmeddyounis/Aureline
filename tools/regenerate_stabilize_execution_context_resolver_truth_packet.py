#!/usr/bin/env python3
"""Regenerate the M4 stabilize-execution-context-resolver truth packet artifact and fixture corpus.

Mirrors the Rust unit-test sample input in
crates/aureline-runtime/src/stabilize_execution_context_resolver/mod.rs. The
...
crates/aureline-runtime/tests/stabilize_execution_context_resolver_truth_packet.rs.
...
DOC_REF = "docs/runtime/m4/stabilize-execution-context-resolver.md"
FIXTURE_DIR = "fixtures/runtime/m4/stabilize_execution_context_resolver"
ARTIFACT_PATH = "artifacts/runtime/m4/stabilize_execution_context_resolver_truth_packet.json"
SCHEMA_REF = "schemas/runtime/stabilize_execution_context_resolver_truth.schema.json"
...
def base_row(row_id, lane, row_class):
    return {
        ...
        "resolver_state_class": "not_applicable",
...
def quality_row(lane, prefix):
    row = base_row(
        f"row:{prefix}:quality", lane, "execution_context_resolution_quality"
    )
...
def state_rows(lane, prefix):
    rows = []
    for state in RESOLVER_STATES:
        row = base_row(
            f"row:{prefix}:state:{state}", lane, "state_admission"
        )
        row["resolver_state_class"] = state
        row["evidence_class"] = "failure_recovery_drill_evidence"
        row["downgrade_automation_class"] = "auto_narrow_on_state_admission_gap"
        row["disclosure_ref"] = f"{DOC_REF}#auto_narrow_on_state_admission_gap"
        rows.append(row)
    return rows


def target_admission_row(lane, prefix):
    row = base_row(
        f"row:{prefix}:target_admission", lane, "target_admission"
    )
    row["evidence_class"] = "automated_functional_evidence"
    row["downgrade_automation_class"] = "auto_narrow_on_target_unreachable"
    row["disclosure_ref"] = f"{DOC_REF}#auto_narrow_on_target_unreachable"
    return row


def restore_rerun_row(lane, prefix):
    row = base_row(
        f"row:{prefix}:restore_rerun_honesty", lane, "restore_rerun_honesty"
    )
    row["evidence_class"] = "failure_recovery_drill_evidence"
...
def capability_skew_row(lane, prefix):
    row = base_row(
        f"row:{prefix}:capability_skew_admission", lane, "capability_skew_admission"
    )
    row["evidence_class"] = "conformance_suite_evidence"
    row["downgrade_automation_class"] = "auto_narrow_on_helper_skew"
    row["disclosure_ref"] = f"{DOC_REF}#auto_narrow_on_helper_skew"
    return row
...
def lane_rows(lane, prefix):
    ...
    rows.extend(state_rows(lane, prefix))
    rows.append(target_admission_row(lane, prefix))
    rows.append(restore_rerun_row(lane, prefix))
    rows.append(capability_skew_row(lane, prefix))
...
def projection(surface, packet_id, suffix=""):
    return {
        ...
        "stabilize_execution_context_resolver_packet_id_ref": packet_id,
...
def packet_artifact():
    packet_id = "packet:m4:stabilize_execution_context_resolver:stable"
    workflow_id = (
        "workflow.runtime.stabilize_execution_context_resolver.stable"
    )
    base = baseline_input(packet_id, workflow_id)
    base.update(
        {
            "record_kind": "stabilize_execution_context_resolver_truth_stable_packet",
            "schema_version": 1,
...
def expectations_from_input(input_obj, promotion_state, finding_count, expected_findings=None):
    ...
    surface_binding_tokens = sorted({row["surface_binding_class"] for row in rows})
    resolver_state_tokens = sorted({row["resolver_state_class"] for row in rows})
    ...
    expect = {
        ...
        "surface_binding_tokens": surface_binding_tokens,
        "resolver_state_tokens": resolver_state_tokens,
...
def baseline_case():
    pid = "packet:m4:stabilize_execution_context_resolver:baseline_stable"
    wid = "workflow.runtime.stabilize_execution_context_resolver.baseline_stable"
    inp = baseline_input(pid, wid)
    return {
        "record_kind": "stabilize_execution_context_resolver_truth_stable_case",
        "schema_version": 1,
        ...
        "scenario": (
            ...
            "execution_context_resolution_quality row at launch_stable plus "
            "the full ten-surface binding coverage (terminal, task, test, "
            "debug, request_workspace, artifact, ai_tool, cli_headless, "
            "docs_help, support_export), the full eight-state admission "
            "coverage (wrong_target, blocked_activator, "
            "stale_capsule_or_prebuild, unsupported_skew, reconnect_required, "
            "restore_no_rerun, degraded_helper, route_drift), a "
            "target_admission row, a restore_rerun_honesty row attesting "
            "no-silent-rerun, a capability_skew_admission row, and a "
            "lineage_admission row binding execution_context_id; every row "
            "binds support, known limit, downgrade automation, and evidence "
            "classes; narrowed rows carry their disclosure refs; and all "
            "eight required consumer projections preserve the packet "
            "verbatim."
        ),
        "input": inp,
...
def unbound_evidence_case():
    pid = "packet:m4:stabilize_execution_context_resolver:launch_stable_with_unbound_evidence"
    wid = (
        "workflow.runtime.stabilize_execution_context_resolver."
        "launch_stable_with_unbound_evidence"
    )
    inp = baseline_input(pid, wid)
    # Demote the local lane quality row's evidence class.
    for row in inp["rows"]:
    ...
    return {
        ...
        "scenario": (
            "The local lane's execution_context_resolution_quality row claims "
...
def missing_state_admission_case():
    pid = (
        "packet:m4:stabilize_execution_context_resolver:"
        "missing_state_admission_for_launch_stable"
    )
    wid = (
        "workflow.runtime.stabilize_execution_context_resolver."
        "missing_state_admission_for_launch_stable"
    ...
    inp = baseline_input(pid, wid)
    # Drop the restore_no_rerun state admission for the local lane.
    inp["rows"] = [
        row
        for row in inp["rows"]
        if not (
            row["row_class"] == "state_admission"
            and row["resolver_state_class"] == "restore_no_rerun"
    ...
    return {
        "record_kind": "stabilize_execution_context_resolver_truth_stable_case",
        "schema_version": 1,
        "case_name": "missing_state_admission_for_launch_stable_blocks_stable",
        "scenario": (
            "The local lane claims launch_stable but drops its "
            "restore_no_rerun state_admission row; the packet blocks the "
            "stable claim because every launch_stable lane must admit all "
            "eight structured resolver states "
            "(wrong_target, blocked_activator, stale_capsule_or_prebuild, "
            "unsupported_skew, reconnect_required, restore_no_rerun, "
            "degraded_helper, route_drift)."
        ),
        "input": inp,
        "expect": expectations_from_input(
            inp,
            "blocks_stable",
            1,
            expected_findings=["missing_resolver_state_coverage"],
        ),
    }
...
def narrowed_row_missing_disclosure_ref_case():
    ...
    for row in inp["rows"]:
        if row["row_id"] == "row:local:quality":
            ...
            break
    # Removing the quality row launch_stable claim means none of the lane's
    # required rows are checked for coverage, but the narrowed row itself
    # still needs a disclosure ref.
    return {
        "record_kind": "stabilize_execution_context_resolver_truth_stable_case",
        "schema_version": 1,
        "case_name": "narrowed_row_missing_disclosure_ref_blocks_stable",
        "scenario": (
            "The local lane's execution_context_resolution_quality row "
            "narrows to launch_stable_below but drops its disclosure ref; the "
            "packet blocks the stable claim because every row narrowed below "
...
def projection_collapses_resolver_state_vocabulary_case():
    pid = (
        "packet:m4:stabilize_execution_context_resolver:"
        "projection_collapses_resolver_state_vocabulary"
    )
    wid = (
        "workflow.runtime.stabilize_execution_context_resolver."
        "projection_collapses_resolver_state_vocabulary"
    ...
    for proj in inp["consumer_projections"]:
        if proj["consumer_surface"] == "help_about":
            proj["preserves_resolver_state_vocabulary"] = False
            break
    return {
        "record_kind": "stabilize_execution_context_resolver_truth_stable_case",
        "schema_version": 1,
        "case_name": "projection_collapses_resolver_state_vocabulary_blocks_stable",
        "scenario": (
            "The help_about consumer projection drops the resolver-state "
            "vocabulary; the packet blocks the stable claim because a "
            "downstream surface that collapses the eight structured resolver "
            "states (wrong_target, blocked_activator, "
            "stale_capsule_or_prebuild, unsupported_skew, reconnect_required, "
            "restore_no_rerun, degraded_helper, route_drift) cannot preserve "
            "execution-context truth verbatim."
        ),
        "input": inp,
        "expect": expectations_from_input(
            inp,
...
def raw_source_material_case():
    pid = (
        "packet:m4:stabilize_execution_context_resolver:raw_source_material"
    )
    wid = (
        "workflow.runtime.stabilize_execution_context_resolver."
        "raw_source_material"
    ...
    return {
        ...
        "scenario": (
            "The local lane's execution_context_resolution_quality row admits "
            "raw command lines, raw process environment bytes, or raw capsule "
            "bodies past the boundary; the packet blocks the stable claim "
            "because raw runtime material, secrets, and ambient credentials "
            "must never leak through the execution-context boundary."
        ),
        "input": inp,
        "expect": expectations_from_input(
            inp,
...
def main():
    ...
    cases = [
        ...
        missing_state_admission_case(),
        narrowed_row_missing_disclosure_ref_case(),
        projection_collapses_resolver_state_vocabulary_case(),
    ...
    with open(full_readme, "w", encoding="utf-8") as fh:
        fh.write(
            ...
            "Fixture corpus for the M4 stable execution-context resolver "
            "truth packet (`schemas/runtime/"
            "stabilize_execution_context_resolver_truth.schema.json`).\n\n"
            "Each fixture is a "
            "`StabilizeExecutionContextResolverTruthPacketInput` with an "
            "`expect` block that pins the materialized packet's promotion "
            "state, finding count, lane and row-class token sets, "
            "support-class, surface-binding, resolver-state, known-limit, "
            ...
            "stabilize_execution_context_resolver_truth_packet.rs` load each "
            "case and assert that "
            "`StabilizeExecutionContextResolverTruthPacket::materialize` "
            "agrees.\n\n"
            "Cases:\n\n"
            "- `baseline_stable.json` — All four execution-context lanes "
            "(local, remote_helper, container, managed) carry one "
            "`execution_context_resolution_quality` row at `launch_stable` "
            "plus the full ten-surface binding coverage, the full "
            "eight-state admission coverage, target_admission, "
            "restore_rerun_honesty, capability_skew_admission, and "
            "lineage_admission rows binding `execution_context_id`. All "
            "eight required consumer projections preserve the packet "
            "verbatim.\n"
            "- "
            "`launch_stable_with_unbound_evidence_blocks_stable.json` — "
            "The local lane's quality row claims `launch_stable` while its "
            "evidence class is `evidence_unbound`; the packet blocks the "
            "stable claim.\n"
            "- "
            "`missing_state_admission_for_launch_stable_blocks_stable.json` "
            "— The local lane claims `launch_stable` but the "
            "`restore_no_rerun` state admission is missing; the packet "
            ...
            "blocks the stable claim.\n"
            "- "
            "`projection_collapses_resolver_state_vocabulary_blocks_stable.json` "
            "— The `help_about` consumer projection drops the "
            "resolver-state vocabulary; the packet blocks the stable claim.\n"
            "- `raw_source_material_blocks_stable.json` — The local "
            "lane's quality row admits raw command lines, env bytes, or "
