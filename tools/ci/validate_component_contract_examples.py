#!/usr/bin/env python3
"""Validate component-contract example packets without external deps."""
...
SCHEMA_PATH = REPO_ROOT / "schemas/design/component_contract.schema.json"
EXAMPLE_DIR = REPO_ROOT / "fixtures/design/component_contract_examples"
    ...
    if schema_type == "integer":
        ...
        if "minimum" in schema:
            expect(
                ...
                f"{where}: expected minimum {schema['minimum']}, got {instance}",
            )
        return
...
def main() -> int:
    ...
    if not EXAMPLE_PATHS:
        print("no component-contract examples found", file=sys.stderr)
        return 1
