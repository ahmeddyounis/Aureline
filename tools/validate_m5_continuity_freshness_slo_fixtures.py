#!/usr/bin/env python3
"""Validate the continuity freshness-SLO dashboard fixtures against their schema.

The fixtures under ``fixtures/continuity/stale_evidence_cases`` and the canonical
dashboard artifact under ``artifacts/m5/continuity`` are emitted by ``cargo run
-p aureline-continuity --example dump_m5_continuity_freshness_slo_fixtures``. They
are dashboard records, a summary record, and a support-export record; every file
...
def main() -> int:
    ...
    print(
        "OK: validated "
        f"{len(fixture_paths)} continuity freshness-SLO fixture(s) against "
