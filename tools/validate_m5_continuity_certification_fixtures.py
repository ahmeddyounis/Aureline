#!/usr/bin/env python3
"""Validate the continuity certification fixtures against their schema.

The fixtures under ``fixtures/continuity/certification_cases`` and the canonical
certified-row registry under ``artifacts/m5/continuity/certification`` are emitted
by ``cargo run -p aureline-continuity --example
dump_m5_continuity_certification_fixtures``. They are report records, a summary
record, and a support-export record; every file must validate against
...
def main() -> int:
    ...
    schema_path = (
        repo_root / "schemas/continuity/continuity_certification_report.schema.json"
    )
    fixture_dir = repo_root / "fixtures/continuity/certification_cases"
    artifact_dir = repo_root / "artifacts/m5/continuity/certification"
    ...
    for artifact in ("certified_rows.json", "certification_support_export.json"):
        candidate = artifact_dir / artifact
        if candidate.exists():
    ...
    print(
        "OK: validated "
        f"{len(fixture_paths)} continuity certification fixture(s) against "
