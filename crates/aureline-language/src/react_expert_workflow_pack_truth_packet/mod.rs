//! Stable React Expert workflow pack truth packet.
//!
//! This module is the language-owned contract that pins how the React
...
//! source bodies, secrets, ambient credentials, or provider payloads. A
...
/// Stable record-kind tag for [`ReactExpertWorkflowPackTruthPacket`].
pub const REACT_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND: &str =
    "react_expert_workflow_pack_truth_stable_packet";

/// Stable record-kind tag for [`ReactExpertWorkflowPackTruthSupportExport`].
pub const REACT_EXPERT_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND: &str =
    "react_expert_workflow_pack_truth_support_export";

/// Integer schema version for the workflow-pack truth packet.
pub const REACT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the boundary schema.
pub const REACT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_REF: &str =
    "schemas/language/react_expert_workflow_pack_truth.schema.json";

/// Repo-relative path of the reviewer contract doc.
pub const REACT_EXPERT_WORKFLOW_PACK_TRUTH_DOC_REF: &str =
...
pub const REACT_EXPERT_WORKFLOW_PACK_TRUTH_ARTIFACT_DOC_REF: &str =
    "artifacts/language/m4/stabilize-the-react-expert-workflow-pack-across-create.md";

/// Repo-relative path of the protected fixture corpus directory.
pub const REACT_EXPERT_WORKFLOW_PACK_TRUTH_FIXTURE_DIR: &str =
    "fixtures/language/m4/react_expert_workflow_pack_truth_packet";

/// Repo-relative path of the checked-in stable packet.
pub const REACT_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_ARTIFACT_REF: &str =
...
pub enum WorkflowPackClass {
    /// React Expert replacement-grade workflow pack.
    ReactExpertWorkflowPack,
}

impl WorkflowPackClass {
    /// Every required workflow pack, in declaration order.
    pub const REQUIRED: [Self; 1] = [Self::ReactExpertWorkflowPack];

    /// Stable token used in fixtures, schemas, and support exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::ReactExpertWorkflowPack => "react_expert_workflow_pack",
        }
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ReactExpertWorkflowPackConsumerProjection {
    ...
    pub react_expert_workflow_pack_packet_id_ref: String,
...
impl ReactExpertWorkflowPackConsumerProjection {
    fn preserves_truth_for(&self, packet_id: &str) -> bool {
        self.react_expert_workflow_pack_packet_id_ref == packet_id
...
/// Constructor input for [`ReactExpertWorkflowPackTruthPacket::materialize`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ReactExpertWorkflowPackTruthPacketInput {
    /// Stable packet id.
    ...
    #[serde(default)]
    pub consumer_projections: Vec<ReactExpertWorkflowPackConsumerProjection>,
    /// Source contracts (docs/schema/fixtures) consumed by the packet.
    #[serde(default)]
    pub source_contract_refs: Vec<String>,
}

/// Language-owned packet certifying the React Expert workflow pack on
/// the stable lane across create, open, run, test, debug, rename, and
/// review loops.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ReactExpertWorkflowPackTruthPacket {
    /// Stable record kind.
...
impl ReactExpertWorkflowPackTruthPacket {
    /// Materializes a packet and records derived validation findings.
    pub fn materialize(input: ReactExpertWorkflowPackTruthPacketInput) -> Self {
        let mut packet = Self {
            record_kind: REACT_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND.to_owned(),
            schema_version: REACT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION,
    ...
    ) -> ReactExpertWorkflowPackTruthSupportExport {
        ReactExpertWorkflowPackTruthSupportExport {
            record_kind: REACT_EXPERT_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND.to_owned(),
            schema_version: REACT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION,
            export_id: export_id.into(),
            react_expert_workflow_pack_packet_id_ref: self.packet_id.clone(),
            exported_at: exported_at.into(),
            raw_private_material_excluded: true,
            ambient_authority_excluded: true,
            react_expert_workflow_pack_packet: self.clone(),
        }
    }
    ...
    fn derived_findings(&self, include_record_fields: bool) -> Vec<ValidationFinding> {
        ...
        if include_record_fields
            && self.record_kind != REACT_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongRecordKind,
                FindingSeverity::Blocker,
                "react expert workflow pack packet has the wrong record kind",
            ));
        }
        if include_record_fields
            && self.schema_version != REACT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongSchemaVersion,
                FindingSeverity::Blocker,
                "react expert workflow pack packet has the wrong schema version",
            ));
        }
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ReactExpertWorkflowPackTruthSupportExport {
    /// Stable record kind.
    pub record_kind: String,
    /// Schema version.
    pub schema_version: u32,
    ...
    pub react_expert_workflow_pack_packet: ReactExpertWorkflowPackTruthPacket,
}

impl ReactExpertWorkflowPackTruthSupportExport {
    /// Returns true when the export preserves the same packet id safely.
    pub fn is_export_safe(&self) -> bool {
        self.record_kind == REACT_EXPERT_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND
            && self.schema_version == REACT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION
            && self.react_expert_workflow_pack_packet_id_ref
                == self.react_expert_workflow_pack_packet.packet_id
            && self.raw_private_material_excluded
            && self.ambient_authority_excluded
            && self.react_expert_workflow_pack_packet.validate().is_empty()
    }
}

/// Errors emitted when reading the checked-in stable workflow-pack packet.
#[derive(Debug)]
pub enum ReactExpertWorkflowPackTruthArtifactError {
...
impl fmt::Display for ReactExpertWorkflowPackTruthArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Packet(error) => write!(
                formatter,
                "react expert workflow pack packet parse failed: {error}"
            ),
            Self::Validation(findings) => {
                ...
                write!(
                    formatter,
                    "react expert workflow pack packet failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for ReactExpertWorkflowPackTruthArtifactError {}

/// Returns the checked-in stable React Expert workflow pack truth packet.
///
/// # Errors
///
/// Returns an artifact error if the checked-in packet does not parse or validate.
pub fn current_stable_react_expert_workflow_pack_truth_packet(
) -> Result<ReactExpertWorkflowPackTruthPacket, ReactExpertWorkflowPackTruthArtifactError> {
    let packet: ReactExpertWorkflowPackTruthPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../artifacts/language/m4/react_expert_workflow_pack_truth_packet.json"
    )))
    .map_err(ReactExpertWorkflowPackTruthArtifactError::Packet)?;
    let findings = packet.validate();
    if findings.is_empty() {
        Ok(packet)
    } else {
        Err(ReactExpertWorkflowPackTruthArtifactError::Validation(
            findings,
        ))
    }
}
...
mod tests {
    ...
    fn pack_evidence_ref() -> String {
        REACT_EXPERT_WORKFLOW_PACK_TRUTH_DOC_REF.to_owned()
    }
    ...
    fn qualification_row(row_id: &str, pack: WorkflowPackClass) -> WorkflowPackRow {
        WorkflowPackRow {
            ...
            disclosure_ref: Some(format!(
                ...
                REACT_EXPERT_WORKFLOW_PACK_TRUTH_DOC_REF
            )),
    ...
    fn projection(surface: ConsumerSurface) -> ReactExpertWorkflowPackConsumerProjection {
    ...
    fn sample_input() -> ReactExpertWorkflowPackTruthPacketInput {
        let rows = pack_rows(WorkflowPackClass::ReactExpertWorkflowPack, "react_expert");
        ReactExpertWorkflowPackTruthPacketInput {
            packet_id: "packet:m4:react_expert_workflow_pack".to_owned(),
            workflow_or_surface_id: "workflow.language.react_expert_workflow_pack".to_owned(),
    ...
    fn baseline_materialization_is_stable() {
        let packet = ReactExpertWorkflowPackTruthPacket::materialize(sample_input());
        assert_eq!(
        ...
        assert!(packet
            .support_export(
                "support:m4:react_expert_workflow_pack",
    ...
    fn expert_grade_with_unbound_evidence_blocks() {
        ...
        let packet = ReactExpertWorkflowPackTruthPacket::materialize(input);
