//! Stable Go daily-driver quality truth packet for the M4 stable
//! lane.
//!
//! This module is the language-owned contract that pins how Go
...
//!
//! 1. The **Go modules and workspaces truth** — every row that
//!    crosses the Go module / workspace boundary (single-module
//!    `go.mod` / `go.sum`, multi-module `go.work` / `go.work.sum`,
//!    `GO111MODULE` module-mode vs `GOPATH` legacy mode,
//!    `GOMODCACHE` / `GOPROXY` / `GOPRIVATE` resolution, and
//!    `replace` / `exclude` / `retract` directives) binds a dedicated
//!    `module_workspace_row` and a disclosure ref so the daily-driver
//!    row never confuses one module layout for another.
//! 2. The **Go test runner and debugger parity** — every row that
//!    certifies the run/test/debug step on Go archetype repos binds
//!    a dedicated `test_runner_row` (`go test`, `go test -race`,
//!    `go test -cover`, `gotestsum`, `testify` table tests) and a
//!    `debugger_row` (Delve `dlv debug` / `dlv attach` / `dlv test`,
//!    DAP integration, and headless `dlv --headless` recovery) so
//!    that run/test/debug rows cannot inherit an adjacent test-runner
//!    or debugger row's evidence.
//! 3. The **Go symbol and refactor parity** — every row that
//!    certifies symbol navigation or refactor on Go archetype repos
//!    binds a dedicated `symbol_refactor_row` (`gopls` LSP symbol /
//!    rename / extract, `gorename`, `gofmt`, `goimports`, and
//!    `golangci-lint` review surface) so a beta-grade capability
//!    sample cannot masquerade as a replacement-grade Go daily
//!    driver.
//!
...
/// Stable record-kind tag for [`GoDailyDriverQualityTruthPacket`].
pub const GO_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND: &str =
    "go_daily_driver_quality_truth_stable_packet";

/// Stable record-kind tag for [`GoDailyDriverQualityTruthSupportExport`].
pub const GO_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND: &str =
    "go_daily_driver_quality_truth_support_export";

/// Integer schema version for the Go daily-driver quality packet.
pub const GO_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the boundary schema.
pub const GO_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_REF: &str =
    "schemas/language/go_daily_driver_quality_truth.schema.json";

/// Repo-relative path of the reviewer contract doc.
pub const GO_DAILY_DRIVER_QUALITY_TRUTH_DOC_REF: &str =
...
pub const GO_DAILY_DRIVER_QUALITY_TRUTH_ARTIFACT_DOC_REF: &str =
    "artifacts/language/m4/stabilize-go-daily-driver-quality-with-modules-workspaces.md";

/// Repo-relative path of the protected fixture corpus directory.
pub const GO_DAILY_DRIVER_QUALITY_TRUTH_FIXTURE_DIR: &str =
    "fixtures/language/m4/go_daily_driver_quality_truth_packet";

/// Repo-relative path of the checked-in stable packet.
pub const GO_DAILY_DRIVER_QUALITY_TRUTH_PACKET_ARTIFACT_REF: &str =
...
pub enum DailyDriverRowClass {
    ...
    /// Go modules / workspaces evidence row on a lane.
    ModuleWorkspaceRow,
    /// Go test-runner (`go test` / `gotestsum` / `testify`) evidence row.
    TestRunnerRow,
    /// Go debugger (Delve / `dlv` / DAP) evidence row.
    DebuggerRow,
    /// Go symbol / refactor (`gopls` / `gorename` / `gofmt` /
    /// `goimports`) evidence row.
    SymbolRefactorRow,
...
impl DailyDriverRowClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ModuleWorkspaceRow => "module_workspace_row",
            Self::TestRunnerRow => "test_runner_row",
            Self::DebuggerRow => "debugger_row",
            Self::SymbolRefactorRow => "symbol_refactor_row",
...
pub enum EvidenceClass {
    ...
    BenchmarkEvidence,
    /// The row is backed by a Go modules / workspaces capture.
    ModuleWorkspaceEvidence,
    /// The row is backed by a Go test-runner capture (`go test` /
    /// `gotestsum` / `testify`).
    TestRunnerEvidence,
    /// The row is backed by a Go debugger capture (Delve / `dlv`).
    DebuggerEvidence,
    /// The row is backed by a Go symbol / refactor capture (`gopls` /
    /// `gorename` / `gofmt` / `goimports`).
    SymbolRefactorEvidence,
...
impl EvidenceClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ModuleWorkspaceEvidence => "module_workspace_evidence",
            Self::TestRunnerEvidence => "test_runner_evidence",
            Self::DebuggerEvidence => "debugger_evidence",
            Self::SymbolRefactorEvidence => "symbol_refactor_evidence",
...
pub enum KnownLimitClass {
    ...
    MigrationSubsetOnly,
    /// The row only certifies a subset of the Go modules / workspaces
    /// surface.
    ModuleWorkspaceSubsetOnly,
    /// The row only certifies a subset of the Go test-runner surface.
    TestRunnerSubsetOnly,
    /// The row only certifies a subset of the Go debugger surface.
    DebuggerSubsetOnly,
    /// The row only certifies a subset of the Go symbol / refactor
    /// surface.
    SymbolRefactorSubsetOnly,
...
impl KnownLimitClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ModuleWorkspaceSubsetOnly => "module_workspace_subset_only",
            Self::TestRunnerSubsetOnly => "test_runner_subset_only",
            Self::DebuggerSubsetOnly => "debugger_subset_only",
            Self::SymbolRefactorSubsetOnly => "symbol_refactor_subset_only",
...
pub enum DowngradeAutomationClass {
    ...
    /// Automatically narrow when the Go modules / workspaces surface is unproven.
    AutoNarrowOnUnprovenModuleWorkspace,
    /// Automatically narrow when the Go test-runner surface is unproven.
    AutoNarrowOnUnprovenTestRunner,
    /// Automatically narrow when the Go debugger surface is unproven.
    AutoNarrowOnUnprovenDebugger,
    /// Automatically narrow when the Go symbol / refactor surface is unproven.
    AutoNarrowOnUnprovenSymbolRefactor,
...
impl DowngradeAutomationClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::AutoNarrowOnUnprovenModuleWorkspace => "auto_narrow_on_unproven_module_workspace",
            ...
            Self::AutoNarrowOnUnprovenSymbolRefactor => "auto_narrow_on_unproven_symbol_refactor",
...
/// Closed validation-finding vocabulary for the Go daily-driver packet.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Consumer surface that must inherit the Go daily-driver packet
/// verbatim.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// One Go daily-driver quality row.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoDailyDriverQualityRow {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoDailyDriverQualityConsumerProjection {
    ...
    pub go_daily_driver_quality_packet_id_ref: String,
...
impl GoDailyDriverQualityConsumerProjection {
    fn preserves_truth_for(&self, packet_id: &str) -> bool {
        self.go_daily_driver_quality_packet_id_ref == packet_id
...
/// Constructor input for [`GoDailyDriverQualityTruthPacket::materialize`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoDailyDriverQualityTruthPacketInput {
    /// Stable packet id.
    ...
    /// Daily-driver rows.
    #[serde(default)]
    pub rows: Vec<GoDailyDriverQualityRow>,
    /// Consumer projections preserving this packet.
    #[serde(default)]
    pub consumer_projections: Vec<GoDailyDriverQualityConsumerProjection>,
    /// Source contracts (docs/schema/fixtures) consumed by the packet.
    #[serde(default)]
    pub source_contract_refs: Vec<String>,
}

/// Language-owned packet certifying Go replacement-grade daily-driver
/// quality on the M4 stable lane with explicit Go modules / workspaces,
/// test-runner (`go test` / `gotestsum`), debugger (Delve / `dlv`), and
/// symbol / refactor (`gopls` / `gorename` / `gofmt` / `goimports`)
/// truth.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoDailyDriverQualityTruthPacket {
    /// Stable record kind.
...
impl GoDailyDriverQualityTruthPacket {
    /// Materializes a packet and records derived validation findings.
    pub fn materialize(input: GoDailyDriverQualityTruthPacketInput) -> Self {
        let mut packet = Self {
            record_kind: GO_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND.to_owned(),
            schema_version: GO_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION,
    ...
    ) -> GoDailyDriverQualityTruthSupportExport {
        GoDailyDriverQualityTruthSupportExport {
            record_kind: GO_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND.to_owned(),
            schema_version: GO_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION,
            export_id: export_id.into(),
            go_daily_driver_quality_packet_id_ref: self.packet_id.clone(),
            exported_at: exported_at.into(),
            raw_private_material_excluded: true,
            ambient_authority_excluded: true,
            go_daily_driver_quality_packet: self.clone(),
        }
    }
    ...
    fn derived_findings(&self, include_record_fields: bool) -> Vec<ValidationFinding> {
        ...
        if include_record_fields
            && self.record_kind != GO_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongRecordKind,
                FindingSeverity::Blocker,
                "go daily-driver quality packet has the wrong record kind",
            ));
        }
        if include_record_fields
            && self.schema_version != GO_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongSchemaVersion,
                FindingSeverity::Blocker,
                "go daily-driver quality packet has the wrong schema version",
            ));
        }
        ...
        for projection in &self.consumer_projections {
            if !projection.preserves_truth_for(&self.packet_id) {
                findings.push(ValidationFinding::new(
                    ...
                    FindingSeverity::Blocker,
                    format!(
                        "projection {} does not preserve go daily-driver quality truth",
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoDailyDriverQualityTruthSupportExport {
    /// Stable record kind.
    pub record_kind: String,
    /// Schema version.
    pub schema_version: u32,
    ...
    pub go_daily_driver_quality_packet: GoDailyDriverQualityTruthPacket,
}

impl GoDailyDriverQualityTruthSupportExport {
    /// Returns true when the export preserves the same packet id safely.
    pub fn is_export_safe(&self) -> bool {
        self.record_kind == GO_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND
            && self.schema_version == GO_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION
            && self.go_daily_driver_quality_packet_id_ref
...
/// Errors emitted when reading the checked-in stable Go daily-driver packet.
#[derive(Debug)]
pub enum GoDailyDriverQualityTruthArtifactError {
...
impl fmt::Display for GoDailyDriverQualityTruthArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Packet(error) => write!(
                formatter,
                "go daily-driver quality packet parse failed: {error}"
            ),
            Self::Validation(findings) => {
                ...
                write!(
                    formatter,
                    "go daily-driver quality packet failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for GoDailyDriverQualityTruthArtifactError {}

/// Returns the checked-in stable Go daily-driver quality truth packet.
///
/// # Errors
///
/// Returns an artifact error if the checked-in packet does not parse or validate.
pub fn current_stable_go_daily_driver_quality_truth_packet(
) -> Result<GoDailyDriverQualityTruthPacket, GoDailyDriverQualityTruthArtifactError> {
    let packet: GoDailyDriverQualityTruthPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../artifacts/language/m4/go_daily_driver_quality_truth_packet.json"
    )))
    .map_err(GoDailyDriverQualityTruthArtifactError::Packet)?;
...
mod tests {
    ...
    fn lane_evidence_ref() -> String {
        GO_DAILY_DRIVER_QUALITY_TRUTH_DOC_REF.to_owned()
    }

    fn quality_row(row_id: &str, lane: LanguageLaneClass) -> GoDailyDriverQualityRow {
        GoDailyDriverQualityRow {
            ...
            disclosure_ref: Some(format!(
                ...
                GO_DAILY_DRIVER_QUALITY_TRUTH_DOC_REF
            )),
    ...
    fn projection(surface: ConsumerSurface) -> GoDailyDriverQualityConsumerProjection {
    ...
    fn lane_rows(lane: LanguageLaneClass, prefix: &str) -> Vec<GoDailyDriverQualityRow> {
    ...
    fn sample_input() -> GoDailyDriverQualityTruthPacketInput {
        let rows = lane_rows(LanguageLaneClass::GoDailyDriverLane, "go");
        GoDailyDriverQualityTruthPacketInput {
            packet_id: "packet:m4:go_daily_driver_quality".to_owned(),
            workflow_or_surface_id: "workflow.language.go_daily_driver_quality".to_owned(),
    ...
    fn closed_tokens_are_pinned() {
        assert_eq!(
            LanguageLaneClass::GoDailyDriverLane.as_str(),
        ...
        assert_eq!(
            EvidenceClass::ModuleWorkspaceEvidence.as_str(),
        ...
        assert_eq!(
            KnownLimitClass::ModuleWorkspaceSubsetOnly.as_str(),
        ...
        assert_eq!(
            KnownLimitClass::SymbolRefactorSubsetOnly.as_str(),
            "symbol_refactor_subset_only"
        );
        assert_eq!(KnownLimitClass::LimitUnbound.as_str(), "limit_unbound");
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenModuleWorkspace.as_str(),
        ...
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenSymbolRefactor.as_str(),
    ...
    fn baseline_materialization_is_stable() {
        let packet = GoDailyDriverQualityTruthPacket::materialize(sample_input());
        assert_eq!(
