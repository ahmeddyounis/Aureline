//! Stable Node and Express Advanced workflow pack truth packet.
//!
//! This module is the language-owned contract that pins how the Node
//! and Express Advanced workflow pack stays one boundary truth across
...
//! the create, open, run, test, debug, rename, and review workflow
//! loops, the Node/Express server project model (CommonJS vs ESM
//! module system; Express router, middleware, and controller
//! boundary), and the launch-profile parity surface (dev / start /
//! debug / test launch profiles plus node --inspect, nodemon, and
//! ts-node-dev hot-reload surfaces). A row that claims `expert_grade`
//! while leaving its known limit or downgrade automation unbound is
//! refused; the validator narrows below expert grade instead of
//! inheriting an adjacent certified row, and the downgrade-automation
//! vocabulary is preserved verbatim so a beta-grade capability sample
//! cannot masquerade as a replacement-grade workflow pack.
//!
...
/// Stable record-kind tag for [`NodeAndExpressAdvancedWorkflowPackTruthPacket`].
pub const NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND: &str =
    "node_and_express_advanced_workflow_pack_truth_stable_packet";

/// Stable record-kind tag for [`NodeAndExpressAdvancedWorkflowPackTruthSupportExport`].
pub const NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND: &str =
    "node_and_express_advanced_workflow_pack_truth_support_export";

/// Integer schema version for the workflow-pack truth packet.
pub const NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the boundary schema.
pub const NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_SCHEMA_REF: &str =
...
pub const NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_DOC_REF: &str =
...
pub const NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_ARTIFACT_DOC_REF: &str =
    "artifacts/language/m4/stabilize-the-node-and-express-advanced-workflow-pack.md";

/// Repo-relative path of the protected fixture corpus directory.
pub const NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_FIXTURE_DIR: &str =
...
pub const NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_PACKET_ARTIFACT_REF: &str =
...
pub enum WorkflowPackClass {
    /// Node and Express Advanced replacement-grade workflow pack.
    NodeAndExpressAdvancedWorkflowPack,
}

impl WorkflowPackClass {
    /// Every required workflow pack, in declaration order.
    pub const REQUIRED: [Self; 1] = [Self::NodeAndExpressAdvancedWorkflowPack];

    /// Stable token used in fixtures, schemas, and support exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::NodeAndExpressAdvancedWorkflowPack => "node_and_express_advanced_workflow_pack",
        }
    }
}
...
pub enum WorkflowPackRowClass {
    ...
    ArchetypeRepoRow,
    /// Node/Express server project-model evidence row.
    ServerProjectModelRow,
    /// Launch-profile parity (dev / start / debug / test launch
    /// profiles) evidence row.
    LaunchProfileParityRow,
...
impl WorkflowPackRowClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ServerProjectModelRow => "server_project_model_row",
            Self::LaunchProfileParityRow => "launch_profile_parity_row",
...
pub enum EvidenceClass {
    ...
    BenchmarkEvidence,
    /// The row is backed by a Node/Express server project-model capture.
    ServerProjectModelEvidence,
    /// The row is backed by a launch-profile parity capture.
    LaunchProfileParityEvidence,
...
impl EvidenceClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ServerProjectModelEvidence => "server_project_model_evidence",
            Self::LaunchProfileParityEvidence => "launch_profile_parity_evidence",
...
pub enum KnownLimitClass {
    ...
    ArchetypeSubsetOnly,
    /// The row only certifies a migration subset (e.g., Express 4 → Express 5 only).
    MigrationSubsetOnly,
    /// The row only certifies a subset of the Node/Express server project model.
    ServerProjectModelSubsetOnly,
    /// The row only certifies a subset of the launch-profile parity surface.
    LaunchProfileParitySubsetOnly,
...
impl KnownLimitClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ServerProjectModelSubsetOnly => "server_project_model_subset_only",
            Self::LaunchProfileParitySubsetOnly => "launch_profile_parity_subset_only",
...
pub enum DowngradeAutomationClass {
    ...
    /// Automatically narrow when the Node/Express server project model is unproven.
    AutoNarrowOnUnprovenServerProjectModel,
    /// Automatically narrow when the launch-profile parity surface is unproven.
    AutoNarrowOnUnprovenLaunchProfileParity,
...
impl DowngradeAutomationClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::AutoNarrowOnUnprovenServerProjectModel => {
                "auto_narrow_on_unproven_server_project_model"
            }
            Self::AutoNarrowOnUnprovenLaunchProfileParity => {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NodeAndExpressAdvancedWorkflowPackConsumerProjection {
    ...
    pub node_and_express_advanced_workflow_pack_packet_id_ref: String,
...
impl NodeAndExpressAdvancedWorkflowPackConsumerProjection {
    fn preserves_truth_for(&self, packet_id: &str) -> bool {
        self.node_and_express_advanced_workflow_pack_packet_id_ref == packet_id
...
/// Constructor input for [`NodeAndExpressAdvancedWorkflowPackTruthPacket::materialize`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NodeAndExpressAdvancedWorkflowPackTruthPacketInput {
    /// Stable packet id.
    ...
    #[serde(default)]
    pub consumer_projections: Vec<NodeAndExpressAdvancedWorkflowPackConsumerProjection>,
    /// Source contracts (docs/schema/fixtures) consumed by the packet.
    #[serde(default)]
    pub source_contract_refs: Vec<String>,
}

/// Language-owned packet certifying the Node and Express Advanced
/// workflow pack on the stable lane across the create, open, run,
/// test, debug, rename, and review loops, with explicit Node/Express
/// server project-model and launch-profile parity rows.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NodeAndExpressAdvancedWorkflowPackTruthPacket {
    /// Stable record kind.
...
impl NodeAndExpressAdvancedWorkflowPackTruthPacket {
    /// Materializes a packet and records derived validation findings.
    pub fn materialize(input: NodeAndExpressAdvancedWorkflowPackTruthPacketInput) -> Self {
        let mut packet = Self {
            record_kind: NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND
                .to_owned(),
    ...
    pub fn support_export(
        ...
        NodeAndExpressAdvancedWorkflowPackTruthSupportExport {
            ...
            node_and_express_advanced_workflow_pack_packet: self.clone(),
        }
    }
    ...
    fn derived_findings(&self, include_record_fields: bool) -> Vec<ValidationFinding> {
        ...
        if include_record_fields
            && self.record_kind != NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongRecordKind,
                FindingSeverity::Blocker,
                "node and express advanced workflow pack packet has the wrong record kind",
            ));
        }
        if include_record_fields
            && self.schema_version != NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION
        {
            findings.push(ValidationFinding::new(
                ...
                FindingSeverity::Blocker,
                "node and express advanced workflow pack packet has the wrong schema version",
            ));
        }
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NodeAndExpressAdvancedWorkflowPackTruthSupportExport {
    /// Stable record kind.
    pub record_kind: String,
    /// Schema version.
    pub schema_version: u32,
...
#[derive(Debug)]
pub enum NodeAndExpressAdvancedWorkflowPackTruthArtifactError {
...
impl fmt::Display for NodeAndExpressAdvancedWorkflowPackTruthArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Packet(error) => write!(
                formatter,
                "node and express advanced workflow pack packet parse failed: {error}"
            ),
            Self::Validation(findings) => {
                ...
                write!(
                    formatter,
                    "node and express advanced workflow pack packet failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for NodeAndExpressAdvancedWorkflowPackTruthArtifactError {}

/// Returns the checked-in stable Node and Express Advanced workflow pack truth packet.
///
/// # Errors
///
/// Returns an artifact error if the checked-in packet does not parse or validate.
pub fn current_stable_node_and_express_advanced_workflow_pack_truth_packet() -> Result<
    NodeAndExpressAdvancedWorkflowPackTruthPacket,
    NodeAndExpressAdvancedWorkflowPackTruthArtifactError,
> {
    let packet: NodeAndExpressAdvancedWorkflowPackTruthPacket =
        serde_json::from_str(include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/language/m4/node_and_express_advanced_workflow_pack_truth_packet.json"
        )))
        .map_err(NodeAndExpressAdvancedWorkflowPackTruthArtifactError::Packet)?;
    let findings = packet.validate();
    if findings.is_empty() {
        Ok(packet)
    } else {
        Err(NodeAndExpressAdvancedWorkflowPackTruthArtifactError::Validation(findings))
    }
}
...
mod tests {
    ...
    fn pack_evidence_ref() -> String {
        NODE_AND_EXPRESS_ADVANCED_WORKFLOW_PACK_TRUTH_DOC_REF.to_owned()
    }
    ...
    fn projection(
        ...
        NodeAndExpressAdvancedWorkflowPackConsumerProjection {
            ...
            node_and_express_advanced_workflow_pack_packet_id_ref:
                "packet:m4:node_and_express_advanced_workflow_pack".to_owned(),
    ...
    fn sample_input() -> NodeAndExpressAdvancedWorkflowPackTruthPacketInput {
        let rows = pack_rows(
            WorkflowPackClass::NodeAndExpressAdvancedWorkflowPack,
            "node_express_advanced",
        );
    ...
    fn closed_tokens_are_pinned() {
        assert_eq!(
            WorkflowPackClass::NodeAndExpressAdvancedWorkflowPack.as_str(),
            "node_and_express_advanced_workflow_pack"
        );
        assert_eq!(
            WorkflowPackRowClass::ServerProjectModelRow.as_str(),
            "server_project_model_row"
        );
        assert_eq!(
            WorkflowPackRowClass::LaunchProfileParityRow.as_str(),
        ...
        assert_eq!(
            EvidenceClass::ServerProjectModelEvidence.as_str(),
            "server_project_model_evidence"
        );
        assert_eq!(
            EvidenceClass::LaunchProfileParityEvidence.as_str(),
            "launch_profile_parity_evidence"
        );
        assert_eq!(EvidenceClass::EvidenceUnbound.as_str(), "evidence_unbound");
        assert_eq!(
            KnownLimitClass::ServerProjectModelSubsetOnly.as_str(),
            "server_project_model_subset_only"
        );
        assert_eq!(
            KnownLimitClass::LaunchProfileParitySubsetOnly.as_str(),
            "launch_profile_parity_subset_only"
        );
        assert_eq!(KnownLimitClass::LimitUnbound.as_str(), "limit_unbound");
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenServerProjectModel.as_str(),
            "auto_narrow_on_unproven_server_project_model"
        );
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenLaunchProfileParity.as_str(),
            "auto_narrow_on_unproven_launch_profile_parity"
        );
        assert_eq!(
    ...
    fn baseline_materialization_is_stable() {
        ...
        assert!(packet
            .support_export(
                "support:m4:node_and_express_advanced_workflow_pack",
    ...
    fn expert_grade_with_unbound_evidence_blocks() {
        ...
        let packet = NodeAndExpressAdvancedWorkflowPackTruthPacket::materialize(input);
