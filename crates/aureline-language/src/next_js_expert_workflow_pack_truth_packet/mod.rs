//! Stable Next.js Expert workflow pack truth packet.
//!
//! This module is the language-owned contract that pins how the Next.js
//! Expert workflow pack stays one boundary truth across the editor
...
//!
//! The packet pins the Next.js Expert workflow pack across the
//! `app/router` workflow loops, the server-component vs. client-component
//! boundary, and the deployment / build-time / request-time environment
//! truth. A row that claims `expert_grade` while leaving its known limit
//! or downgrade automation unbound is refused; the validator narrows
//! below expert grade instead of inheriting an adjacent certified row,
//! and the downgrade-automation vocabulary is preserved verbatim so a
//! beta-grade capability sample cannot masquerade as a replacement-grade
//! workflow pack.
//!
...
/// Stable record-kind tag for [`NextJsExpertWorkflowPackTruthPacket`].
pub const NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND: &str =
    "next_js_expert_workflow_pack_truth_stable_packet";

/// Stable record-kind tag for [`NextJsExpertWorkflowPackTruthSupportExport`].
pub const NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND: &str =
    "next_js_expert_workflow_pack_truth_support_export";

/// Integer schema version for the workflow-pack truth packet.
pub const NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the boundary schema.
pub const NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_REF: &str =
    "schemas/language/next_js_expert_workflow_pack_truth.schema.json";

/// Repo-relative path of the reviewer contract doc.
pub const NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_DOC_REF: &str =
...
pub const NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_ARTIFACT_DOC_REF: &str =
    "artifacts/language/m4/stabilize-the-next-js-expert-workflow-pack-with.md";

/// Repo-relative path of the protected fixture corpus directory.
pub const NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_FIXTURE_DIR: &str =
    "fixtures/language/m4/next_js_expert_workflow_pack_truth_packet";

/// Repo-relative path of the checked-in stable packet.
pub const NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_ARTIFACT_REF: &str =
...
pub enum WorkflowPackClass {
    /// Next.js Expert replacement-grade workflow pack.
    NextJsExpertWorkflowPack,
}

impl WorkflowPackClass {
    /// Every required workflow pack, in declaration order.
    pub const REQUIRED: [Self; 1] = [Self::NextJsExpertWorkflowPack];

    /// Stable token used in fixtures, schemas, and support exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::NextJsExpertWorkflowPack => "next_js_expert_workflow_pack",
        }
    }
}
...
pub enum WorkflowPackRowClass {
    ...
    ArchetypeRepoRow,
    /// Server-component vs. client-component boundary evidence row.
    ServerClientBoundaryRow,
    /// Environment-truth (build-time, request-time, deployment) row.
    EnvironmentTruthRow,
...
impl WorkflowPackRowClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ServerClientBoundaryRow => "server_client_boundary_row",
            Self::EnvironmentTruthRow => "environment_truth_row",
...
pub enum EvidenceClass {
    ...
    BenchmarkEvidence,
    /// The row is backed by a server-component / client-component boundary capture.
    ServerClientBoundaryEvidence,
    /// The row is backed by an environment-truth (build/request/deploy) capture.
    EnvironmentTruthEvidence,
...
impl EvidenceClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ServerClientBoundaryEvidence => "server_client_boundary_evidence",
            Self::EnvironmentTruthEvidence => "environment_truth_evidence",
...
pub enum KnownLimitClass {
    ...
    ArchetypeSubsetOnly,
    /// The row only certifies a migration subset (e.g., one router mode).
    MigrationSubsetOnly,
    /// The row only certifies a subset of the server/client boundary.
    ServerClientBoundarySubsetOnly,
    /// The row only certifies a subset of the environment-truth surface.
    EnvironmentTruthSubsetOnly,
...
impl KnownLimitClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ServerClientBoundarySubsetOnly => "server_client_boundary_subset_only",
            Self::EnvironmentTruthSubsetOnly => "environment_truth_subset_only",
...
pub enum DowngradeAutomationClass {
    ...
    /// Automatically narrow when the server/client boundary is unproven.
    AutoNarrowOnUnprovenServerClientBoundary,
    /// Automatically narrow when the environment-truth surface is unproven.
    AutoNarrowOnUnprovenEnvironmentTruth,
...
impl DowngradeAutomationClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::AutoNarrowOnUnprovenServerClientBoundary => {
                "auto_narrow_on_unproven_server_client_boundary"
            }
            Self::AutoNarrowOnUnprovenEnvironmentTruth => {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NextJsExpertWorkflowPackConsumerProjection {
    ...
    pub next_js_expert_workflow_pack_packet_id_ref: String,
...
impl NextJsExpertWorkflowPackConsumerProjection {
    fn preserves_truth_for(&self, packet_id: &str) -> bool {
        self.next_js_expert_workflow_pack_packet_id_ref == packet_id
...
/// Constructor input for [`NextJsExpertWorkflowPackTruthPacket::materialize`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NextJsExpertWorkflowPackTruthPacketInput {
    /// Stable packet id.
    ...
    #[serde(default)]
    pub consumer_projections: Vec<NextJsExpertWorkflowPackConsumerProjection>,
    /// Source contracts (docs/schema/fixtures) consumed by the packet.
    #[serde(default)]
    pub source_contract_refs: Vec<String>,
}

/// Language-owned packet certifying the Next.js Expert workflow pack on
/// the stable lane across the create, open, run, test, debug, rename,
/// and review loops, with explicit server/client boundary and
/// environment-truth rows.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NextJsExpertWorkflowPackTruthPacket {
    /// Stable record kind.
...
impl NextJsExpertWorkflowPackTruthPacket {
    /// Materializes a packet and records derived validation findings.
    pub fn materialize(input: NextJsExpertWorkflowPackTruthPacketInput) -> Self {
        let mut packet = Self {
            record_kind: NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND.to_owned(),
            schema_version: NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION,
    ...
    pub fn support_export(
        ...
        NextJsExpertWorkflowPackTruthSupportExport {
            record_kind: NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND.to_owned(),
            schema_version: NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION,
            export_id: export_id.into(),
            next_js_expert_workflow_pack_packet_id_ref: self.packet_id.clone(),
            exported_at: exported_at.into(),
            raw_private_material_excluded: true,
            ambient_authority_excluded: true,
            next_js_expert_workflow_pack_packet: self.clone(),
        }
    }
    ...
    fn derived_findings(&self, include_record_fields: bool) -> Vec<ValidationFinding> {
        ...
        if include_record_fields
            && self.record_kind != NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongRecordKind,
                FindingSeverity::Blocker,
                "next.js expert workflow pack packet has the wrong record kind",
            ));
        }
        if include_record_fields
            && self.schema_version != NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongSchemaVersion,
                FindingSeverity::Blocker,
                "next.js expert workflow pack packet has the wrong schema version",
            ));
        }
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NextJsExpertWorkflowPackTruthSupportExport {
    /// Stable record kind.
    pub record_kind: String,
    /// Schema version.
    pub schema_version: u32,
    ...
    pub next_js_expert_workflow_pack_packet: NextJsExpertWorkflowPackTruthPacket,
}

impl NextJsExpertWorkflowPackTruthSupportExport {
    /// Returns true when the export preserves the same packet id safely.
    pub fn is_export_safe(&self) -> bool {
        self.record_kind == NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND
            && self.schema_version == NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION
            && self.next_js_expert_workflow_pack_packet_id_ref
...
#[derive(Debug)]
pub enum NextJsExpertWorkflowPackTruthArtifactError {
...
impl fmt::Display for NextJsExpertWorkflowPackTruthArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Packet(error) => write!(
                formatter,
                "next.js expert workflow pack packet parse failed: {error}"
            ),
            Self::Validation(findings) => {
                ...
                write!(
                    formatter,
                    "next.js expert workflow pack packet failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for NextJsExpertWorkflowPackTruthArtifactError {}

/// Returns the checked-in stable Next.js Expert workflow pack truth packet.
///
/// # Errors
///
/// Returns an artifact error if the checked-in packet does not parse or validate.
pub fn current_stable_next_js_expert_workflow_pack_truth_packet(
) -> Result<NextJsExpertWorkflowPackTruthPacket, NextJsExpertWorkflowPackTruthArtifactError> {
    let packet: NextJsExpertWorkflowPackTruthPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../artifacts/language/m4/next_js_expert_workflow_pack_truth_packet.json"
    )))
    .map_err(NextJsExpertWorkflowPackTruthArtifactError::Packet)?;
    let findings = packet.validate();
    if findings.is_empty() {
        Ok(packet)
    } else {
        Err(NextJsExpertWorkflowPackTruthArtifactError::Validation(
            findings,
        ))
    }
}
...
mod tests {
    ...
    fn pack_evidence_ref() -> String {
        NEXT_JS_EXPERT_WORKFLOW_PACK_TRUTH_DOC_REF.to_owned()
    }
    ...
    fn projection(surface: ConsumerSurface) -> NextJsExpertWorkflowPackConsumerProjection {
    ...
    fn closed_tokens_are_pinned() {
        assert_eq!(
            WorkflowPackClass::NextJsExpertWorkflowPack.as_str(),
            "next_js_expert_workflow_pack"
        );
        assert_eq!(
            WorkflowPackRowClass::ServerClientBoundaryRow.as_str(),
            "server_client_boundary_row"
        );
        assert_eq!(
            WorkflowPackRowClass::EnvironmentTruthRow.as_str(),
        ...
        assert_eq!(
            EvidenceClass::ServerClientBoundaryEvidence.as_str(),
            "server_client_boundary_evidence"
        );
        assert_eq!(
            EvidenceClass::EnvironmentTruthEvidence.as_str(),
            "environment_truth_evidence"
        );
        assert_eq!(EvidenceClass::EvidenceUnbound.as_str(), "evidence_unbound");
        assert_eq!(
            KnownLimitClass::ServerClientBoundarySubsetOnly.as_str(),
            "server_client_boundary_subset_only"
        );
        assert_eq!(
            KnownLimitClass::EnvironmentTruthSubsetOnly.as_str(),
            "environment_truth_subset_only"
        );
        assert_eq!(KnownLimitClass::LimitUnbound.as_str(), "limit_unbound");
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenServerClientBoundary.as_str(),
            "auto_narrow_on_unproven_server_client_boundary"
        );
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenEnvironmentTruth.as_str(),
            "auto_narrow_on_unproven_environment_truth"
        );
        assert_eq!(
    ...
    fn baseline_materialization_is_stable() {
        let packet = NextJsExpertWorkflowPackTruthPacket::materialize(sample_input());
        assert_eq!(
        ...
        assert!(packet
            .support_export(
                "support:m4:next_js_expert_workflow_pack",
    ...
    fn expert_grade_with_unbound_evidence_blocks() {
        ...
        let packet = NextJsExpertWorkflowPackTruthPacket::materialize(input);
