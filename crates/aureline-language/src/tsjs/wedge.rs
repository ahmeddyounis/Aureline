...
fn included_scope_refs(context: &super::records::TsJsWorkspaceContext) -> Vec<String> {
    let mut refs = vec![
        context.covered_scope_ref.clone(),
...
fn refactor_scope_candidates(symbol: &TsJsSymbolSeed) -> Vec<RefactorScopeCandidate> {
    symbol
        .reference_occurrences()
        .filter(|occurrence| occurrence.rename_writable_authored_candidate())
        .map(|occurrence| {
