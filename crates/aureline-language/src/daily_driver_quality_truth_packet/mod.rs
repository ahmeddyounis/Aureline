//! Stable daily-driver quality truth packet for the M4 stable lane.
//!
//! This module is the language-owned contract that pins how TypeScript,
//! JavaScript, HTML, and CSS replacement-grade daily-driver quality stays
//! one boundary truth across the editor language pack, framework pack
//! panel, language settings/help, CLI/headless inspector, support export,
...
//! and `daily_driver_confidence_class` plus an `evidence_refs` array and a
//! `disclosure_ref` whenever the row is narrowed below replacement grade,
...
//! bodies, secrets, ambient credentials, or provider payloads. A row that
//! claims `replacement_grade` while leaving its known limit or downgrade
//! automation unbound is refused; the validator narrows below replacement
//! grade instead of inheriting an adjacent certified row, and the
//! downgrade-automation vocabulary is preserved verbatim so a beta-grade
//! capability sample cannot masquerade as a replacement-grade daily
//! driver.
...
/// Stable record-kind tag for [`DailyDriverQualityTruthPacket`].
pub const DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND: &str =
    "daily_driver_quality_truth_stable_packet";

/// Stable record-kind tag for [`DailyDriverQualityTruthSupportExport`].
pub const DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND: &str =
    "daily_driver_quality_truth_support_export";

/// Integer schema version for the daily-driver quality packet.
pub const DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the boundary schema.
pub const DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_REF: &str =
    "schemas/language/daily_driver_quality_truth.schema.json";

/// Repo-relative path of the reviewer doc.
pub const DAILY_DRIVER_QUALITY_TRUTH_DOC_REF: &str =
...
pub enum LanguageLaneClass {
    /// TypeScript replacement-grade daily-driver lane.
    TypescriptDailyDriverLane,
    /// JavaScript replacement-grade daily-driver lane.
    JavascriptDailyDriverLane,
    /// HTML replacement-grade daily-driver lane.
    HtmlDailyDriverLane,
    /// CSS replacement-grade daily-driver lane.
    CssDailyDriverLane,
}
...
impl LanguageLaneClass {
    ...
    pub const REQUIRED: [Self; 4] = [
        Self::TypescriptDailyDriverLane,
        Self::JavascriptDailyDriverLane,
        Self::HtmlDailyDriverLane,
        Self::CssDailyDriverLane,
    ];

    /// Stable token used in fixtures, schemas, and support exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::TypescriptDailyDriverLane => "typescript_daily_driver_lane",
            Self::JavascriptDailyDriverLane => "javascript_daily_driver_lane",
            Self::HtmlDailyDriverLane => "html_daily_driver_lane",
            Self::CssDailyDriverLane => "css_daily_driver_lane",
        }
    }
}
...
/// `replacement_grade` while its known limit or downgrade automation is
/// unbound; the validator demotes it instead of inheriting an adjacent
/// replacement-grade row.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Closed daily-loop step vocabulary. The full daily loop MUST be covered
/// for each lane that claims `replacement_grade`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Closed validation-finding vocabulary for the daily-driver packet.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Consumer surface that must inherit the daily-driver packet verbatim.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// One daily-driver quality row.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DailyDriverQualityRow {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DailyDriverQualityConsumerProjection {
    ...
    pub daily_driver_quality_packet_id_ref: String,
...
impl DailyDriverQualityConsumerProjection {
    fn preserves_truth_for(&self, packet_id: &str) -> bool {
        self.daily_driver_quality_packet_id_ref == packet_id
...
/// Constructor input for [`DailyDriverQualityTruthPacket::materialize`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DailyDriverQualityTruthPacketInput {
    /// Stable packet id.
    ...
    /// Daily-driver rows.
    #[serde(default)]
    pub rows: Vec<DailyDriverQualityRow>,
    /// Consumer projections preserving this packet.
    #[serde(default)]
    pub consumer_projections: Vec<DailyDriverQualityConsumerProjection>,
    /// Source contracts (docs/schema/fixtures) consumed by the packet.
    #[serde(default)]
    pub source_contract_refs: Vec<String>,
}

/// Language-owned packet certifying TypeScript, JavaScript, HTML, and CSS
/// replacement-grade daily-driver quality for the M4 stable lane.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DailyDriverQualityTruthPacket {
    /// Stable record kind.
...
impl DailyDriverQualityTruthPacket {
    /// Materializes a packet and records derived validation findings.
    pub fn materialize(input: DailyDriverQualityTruthPacketInput) -> Self {
        let mut packet = Self {
            record_kind: DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND.to_owned(),
            schema_version: DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION,
    ...
    ) -> DailyDriverQualityTruthSupportExport {
        DailyDriverQualityTruthSupportExport {
            record_kind: DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND.to_owned(),
            schema_version: DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION,
            export_id: export_id.into(),
            daily_driver_quality_packet_id_ref: self.packet_id.clone(),
            exported_at: exported_at.into(),
            raw_private_material_excluded: true,
            ambient_authority_excluded: true,
            daily_driver_quality_packet: self.clone(),
        }
    }
    ...
    fn derived_findings(&self, include_record_fields: bool) -> Vec<ValidationFinding> {
        ...
        if include_record_fields
            && self.record_kind != DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongRecordKind,
                FindingSeverity::Blocker,
                "daily-driver quality packet has the wrong record kind",
            ));
        }
        if include_record_fields && self.schema_version != DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongSchemaVersion,
                FindingSeverity::Blocker,
                "daily-driver quality packet has the wrong schema version",
            ));
        }
        ...
        for projection in &self.consumer_projections {
            if !projection.preserves_truth_for(&self.packet_id) {
                findings.push(ValidationFinding::new(
                    ...
                    FindingSeverity::Blocker,
                    format!(
                        "projection {} does not preserve daily-driver quality truth",
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DailyDriverQualityTruthSupportExport {
    /// Stable record kind.
    pub record_kind: String,
    /// Schema version.
    pub schema_version: u32,
    ...
    pub daily_driver_quality_packet: DailyDriverQualityTruthPacket,
}

impl DailyDriverQualityTruthSupportExport {
...
/// Errors emitted when reading the checked-in stable daily-driver packet.
#[derive(Debug)]
pub enum DailyDriverQualityTruthArtifactError {
...
impl fmt::Display for DailyDriverQualityTruthArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Packet(error) => write!(
                formatter,
                "daily-driver quality packet parse failed: {error}"
            ),
            Self::Validation(findings) => {
                ...
                write!(
                    formatter,
                    "daily-driver quality packet failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for DailyDriverQualityTruthArtifactError {}

/// Returns the checked-in stable daily-driver quality truth packet.
///
/// # Errors
///
/// Returns an artifact error if the checked-in packet does not parse or validate.
pub fn current_stable_daily_driver_quality_truth_packet(
) -> Result<DailyDriverQualityTruthPacket, DailyDriverQualityTruthArtifactError> {
    let packet: DailyDriverQualityTruthPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../artifacts/language/m4/daily_driver_quality_truth_packet.json"
    )))
    .map_err(DailyDriverQualityTruthArtifactError::Packet)?;
    let findings = packet.validate();
    if findings.is_empty() {
        Ok(packet)
    } else {
        Err(DailyDriverQualityTruthArtifactError::Validation(findings))
    }
}
...
mod tests {
    ...
    fn quality_row(
        ...
        DailyDriverQualityRow {
            ...
            disclosure_ref: Some(format!(
                ...
                DAILY_DRIVER_QUALITY_TRUTH_DOC_REF
            )),
    ...
    fn projection(surface: ConsumerSurface) -> DailyDriverQualityConsumerProjection {
        DailyDriverQualityConsumerProjection {
            consumer_surface: surface,
            projection_ref: format!("projection:{}", surface.as_str()),
            daily_driver_quality_packet_id_ref: "packet:m4:daily_driver_quality".to_owned(),
    ...
    fn lane_rows(lane: LanguageLaneClass, prefix: &str) -> Vec<DailyDriverQualityRow> {
    ...
    fn sample_input() -> DailyDriverQualityTruthPacketInput {
        ...
        DailyDriverQualityTruthPacketInput {
            packet_id: "packet:m4:daily_driver_quality".to_owned(),
            workflow_or_surface_id: "workflow.language.daily_driver_quality".to_owned(),
    ...
    fn baseline_materialization_is_stable() {
        let packet = DailyDriverQualityTruthPacket::materialize(sample_input());
        assert_eq!(
    ...
    fn daily_loop_step_not_applicable_on_loop_row_blocks() {
        ...
        for row in &mut input.rows {
            if matches!(row.row_class, DailyDriverRowClass::DailyLoopStep)
                && row.lane_class == LanguageLaneClass::TypescriptDailyDriverLane
    ...
    #[test]
    fn narrowed_below_replacement_with_disclosure_keeps_stable() {
        let mut input = sample_input();
        // HTML lane downgrades to below-replacement with disclosure; remove its
        // loop-step rows since they are no longer required when no
        // replacement_grade quality row is present.
        input.rows.retain(|row| {
            !(row.lane_class == LanguageLaneClass::HtmlDailyDriverLane
                && matches!(row.row_class, DailyDriverRowClass::DailyLoopStep))
        });
        for row in &mut input.rows {
            if row.lane_class == LanguageLaneClass::HtmlDailyDriverLane
                && matches!(row.row_class, DailyDriverRowClass::DailyDriverQuality)
            {
                row.support_class = SupportClass::DailyDriverBelowReplacement;
