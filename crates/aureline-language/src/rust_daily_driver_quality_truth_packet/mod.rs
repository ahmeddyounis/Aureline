//! Stable Rust daily-driver quality truth packet for the M4 stable
//! lane.
//!
//! This module is the language-owned contract that pins how Rust
//! replacement-grade daily-driver quality stays one boundary truth
//! across the editor language pack, framework pack panel, language
...
//! Every row binds a closed `language_lane_class`,
...
//!
//! 1. The **Cargo workspaces truth** — every row that crosses the
//!    Cargo workspace boundary (single-package `Cargo.toml` /
...
//!    `CARGO_REGISTRIES_*` / `CARGO_NET_OFFLINE` resolution, and
//!    `[patch]` / `[replace]` / `[profile.*]` directives plus
//!    dev-dependencies / build-dependencies / optional features)
//!    binds a dedicated `cargo_workspace_row` and a disclosure ref so
//!    the daily-driver row never confuses one workspace layout for
//!    another.
//! 2. The **clippy and rustfmt parity** — every row that certifies
//!    the review step or edit-time lint/format on Rust archetype
//!    repos binds a dedicated `lint_format_row` (`cargo clippy` /
//!    `cargo clippy --fix`, `cargo fmt` / `cargo fmt --check`, custom
//!    `clippy.toml` / `rustfmt.toml` configuration, and editor
//!    format-on-save behaviour) so a beta-grade capability sample
//!    cannot masquerade as a replacement-grade Rust review surface.
//! 3. The **cargo test runner and debugger parity** — every row that
//!    certifies the run/test/debug step on Rust archetype repos binds
//!    a dedicated `test_runner_row` (`cargo test`, `cargo test --doc`,
//!    `cargo nextest`, `cargo bench`) and a `debugger_row`
//!    (`rust-lldb` / `rust-gdb`, CodeLLDB / `lldb-dap` DAP integration,
//!    and `RUST_BACKTRACE` recovery) so that run/test/debug rows
//!    cannot inherit an adjacent test-runner or debugger row's
//!    evidence.
//! 4. The **rust-analyzer large-workspace indexing parity** — every
//!    row that certifies symbol navigation, completion, or refactor on
//!    Rust archetype repos binds a dedicated `workspace_index_row`
//!    (`rust-analyzer` LSP symbol / rename / extract, proc-macro
//!    expansion, build-script execution, save-analysis / on-the-fly
//!    indexing, and metadata fetch budget) so that large-workspace
//!    indexing posture is never inferred from a tiny single-crate
//!    sample.
//!
...
/// Stable record-kind tag for [`RustDailyDriverQualityTruthPacket`].
pub const RUST_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND: &str =
    "rust_daily_driver_quality_truth_stable_packet";

/// Stable record-kind tag for [`RustDailyDriverQualityTruthSupportExport`].
pub const RUST_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND: &str =
    "rust_daily_driver_quality_truth_support_export";

/// Integer schema version for the Rust daily-driver quality packet.
pub const RUST_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the boundary schema.
pub const RUST_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_REF: &str =
    "schemas/language/rust_daily_driver_quality_truth.schema.json";

/// Repo-relative path of the reviewer contract doc.
pub const RUST_DAILY_DRIVER_QUALITY_TRUTH_DOC_REF: &str =
...
pub const RUST_DAILY_DRIVER_QUALITY_TRUTH_ARTIFACT_DOC_REF: &str =
    "artifacts/language/m4/stabilize-rust-daily-driver-quality-with-cargo-workspaces.md";

/// Repo-relative path of the protected fixture corpus directory.
pub const RUST_DAILY_DRIVER_QUALITY_TRUTH_FIXTURE_DIR: &str =
    "fixtures/language/m4/rust_daily_driver_quality_truth_packet";

/// Repo-relative path of the checked-in stable packet.
pub const RUST_DAILY_DRIVER_QUALITY_TRUTH_PACKET_ARTIFACT_REF: &str =
...
pub enum LanguageLaneClass {
    /// Rust replacement-grade daily-driver lane.
    RustDailyDriverLane,
}

impl LanguageLaneClass {
    /// Every required language lane, in declaration order.
    pub const REQUIRED: [Self; 1] = [Self::RustDailyDriverLane];

    /// Stable token used in fixtures, schemas, and support exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::RustDailyDriverLane => "rust_daily_driver_lane",
        }
    }
}
...
pub enum DailyDriverRowClass {
    ...
    /// Cargo workspace / `Cargo.toml` / `rust-toolchain.toml` evidence row.
    CargoWorkspaceRow,
    /// `cargo clippy` / `cargo fmt` lint and format evidence row.
    LintFormatRow,
    /// Rust test-runner (`cargo test` / `cargo nextest` / `cargo bench`) evidence row.
    TestRunnerRow,
    /// Rust debugger (`rust-lldb` / `rust-gdb` / CodeLLDB DAP) evidence row.
    DebuggerRow,
    /// rust-analyzer large-workspace indexing evidence row.
    WorkspaceIndexRow,
...
impl DailyDriverRowClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CargoWorkspaceRow => "cargo_workspace_row",
            Self::LintFormatRow => "lint_format_row",
            Self::TestRunnerRow => "test_runner_row",
            Self::DebuggerRow => "debugger_row",
            Self::WorkspaceIndexRow => "workspace_index_row",
...
pub enum EvidenceClass {
    ...
    BenchmarkEvidence,
    /// The row is backed by a Cargo workspace / toolchain capture.
    CargoWorkspaceEvidence,
    /// The row is backed by a `cargo clippy` / `cargo fmt` capture.
    LintFormatEvidence,
    /// The row is backed by a Rust test-runner capture (`cargo test` /
    /// `cargo nextest` / `cargo bench`).
    TestRunnerEvidence,
    /// The row is backed by a Rust debugger capture (`rust-lldb` /
    /// `rust-gdb` / CodeLLDB).
    DebuggerEvidence,
    /// The row is backed by a rust-analyzer large-workspace indexing
    /// capture.
    WorkspaceIndexEvidence,
...
impl EvidenceClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CargoWorkspaceEvidence => "cargo_workspace_evidence",
            Self::LintFormatEvidence => "lint_format_evidence",
            Self::TestRunnerEvidence => "test_runner_evidence",
            Self::DebuggerEvidence => "debugger_evidence",
            Self::WorkspaceIndexEvidence => "workspace_index_evidence",
...
pub enum KnownLimitClass {
    ...
    FrameworkSubsetOnly,
    /// The row only certifies a language subset (e.g., one edition).
    LanguageSubsetOnly,
    /// The row only certifies an archetype subset (specific repos).
    ArchetypeSubsetOnly,
    /// The row only certifies a migration subset.
    MigrationSubsetOnly,
    /// The row only certifies a subset of the Cargo workspace surface.
    CargoWorkspaceSubsetOnly,
    /// The row only certifies a subset of the clippy / rustfmt surface.
    LintFormatSubsetOnly,
    /// The row only certifies a subset of the cargo test-runner surface.
    TestRunnerSubsetOnly,
    /// The row only certifies a subset of the Rust debugger surface.
    DebuggerSubsetOnly,
    /// The row only certifies a subset of the rust-analyzer indexing
    /// surface.
    WorkspaceIndexSubsetOnly,
...
impl KnownLimitClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CargoWorkspaceSubsetOnly => "cargo_workspace_subset_only",
            Self::LintFormatSubsetOnly => "lint_format_subset_only",
            Self::TestRunnerSubsetOnly => "test_runner_subset_only",
            Self::DebuggerSubsetOnly => "debugger_subset_only",
            Self::WorkspaceIndexSubsetOnly => "workspace_index_subset_only",
...
pub enum DowngradeAutomationClass {
    ...
    /// Automatically narrow when the Cargo workspace surface is unproven.
    AutoNarrowOnUnprovenCargoWorkspace,
    /// Automatically narrow when the clippy / rustfmt surface is unproven.
    AutoNarrowOnUnprovenLintFormat,
    /// Automatically narrow when the cargo test-runner surface is unproven.
    AutoNarrowOnUnprovenTestRunner,
    /// Automatically narrow when the Rust debugger surface is unproven.
    AutoNarrowOnUnprovenDebugger,
    /// Automatically narrow when the rust-analyzer indexing surface is
    /// unproven.
    AutoNarrowOnUnprovenWorkspaceIndex,
...
impl DowngradeAutomationClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::AutoNarrowOnUnprovenCargoWorkspace => "auto_narrow_on_unproven_cargo_workspace",
            Self::AutoNarrowOnUnprovenLintFormat => "auto_narrow_on_unproven_lint_format",
            Self::AutoNarrowOnUnprovenTestRunner => "auto_narrow_on_unproven_test_runner",
            Self::AutoNarrowOnUnprovenDebugger => "auto_narrow_on_unproven_debugger",
            Self::AutoNarrowOnUnprovenWorkspaceIndex => "auto_narrow_on_unproven_workspace_index",
...
/// Closed validation-finding vocabulary for the Rust daily-driver packet.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Consumer surface that must inherit the Rust daily-driver packet
/// verbatim.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// One Rust daily-driver quality row.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RustDailyDriverQualityRow {
...
impl RustDailyDriverQualityRow {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RustDailyDriverQualityConsumerProjection {
    ...
    pub rust_daily_driver_quality_packet_id_ref: String,
...
impl RustDailyDriverQualityConsumerProjection {
    fn preserves_truth_for(&self, packet_id: &str) -> bool {
        self.rust_daily_driver_quality_packet_id_ref == packet_id
...
/// Constructor input for [`RustDailyDriverQualityTruthPacket::materialize`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RustDailyDriverQualityTruthPacketInput {
    /// Stable packet id.
    ...
    /// Daily-driver rows.
    #[serde(default)]
    pub rows: Vec<RustDailyDriverQualityRow>,
    /// Consumer projections preserving this packet.
    #[serde(default)]
    pub consumer_projections: Vec<RustDailyDriverQualityConsumerProjection>,
    /// Source contracts (docs/schema/fixtures) consumed by the packet.
    #[serde(default)]
    pub source_contract_refs: Vec<String>,
}

/// Language-owned packet certifying Rust replacement-grade daily-driver
/// quality on the M4 stable lane with explicit Cargo workspace, clippy
/// / rustfmt lint-format, test-runner (`cargo test` / `cargo nextest`),
/// debugger (`rust-lldb` / `rust-gdb` / CodeLLDB), and rust-analyzer
/// large-workspace indexing truth.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RustDailyDriverQualityTruthPacket {
    /// Stable record kind.
...
impl RustDailyDriverQualityTruthPacket {
    /// Materializes a packet and records derived validation findings.
    pub fn materialize(input: RustDailyDriverQualityTruthPacketInput) -> Self {
        let mut packet = Self {
            record_kind: RUST_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND.to_owned(),
            schema_version: RUST_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION,
    ...
    ) -> RustDailyDriverQualityTruthSupportExport {
        RustDailyDriverQualityTruthSupportExport {
            record_kind: RUST_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND.to_owned(),
            schema_version: RUST_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION,
            export_id: export_id.into(),
            rust_daily_driver_quality_packet_id_ref: self.packet_id.clone(),
            exported_at: exported_at.into(),
            raw_private_material_excluded: true,
            ambient_authority_excluded: true,
            rust_daily_driver_quality_packet: self.clone(),
        }
    }
    ...
    fn derived_findings(&self, include_record_fields: bool) -> Vec<ValidationFinding> {
        ...
        if include_record_fields
            && self.record_kind != RUST_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongRecordKind,
                FindingSeverity::Blocker,
                "rust daily-driver quality packet has the wrong record kind",
            ));
        }
        if include_record_fields
            && self.schema_version != RUST_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongSchemaVersion,
                FindingSeverity::Blocker,
                "rust daily-driver quality packet has the wrong schema version",
            ));
        }
        ...
        for projection in &self.consumer_projections {
            if !projection.preserves_truth_for(&self.packet_id) {
                findings.push(ValidationFinding::new(
                    ...
                    FindingSeverity::Blocker,
                    format!(
                        "projection {} does not preserve rust daily-driver quality truth",
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct RustDailyDriverQualityTruthSupportExport {
    /// Stable record kind.
    pub record_kind: String,
    /// Schema version.
    pub schema_version: u32,
    ...
    pub rust_daily_driver_quality_packet: RustDailyDriverQualityTruthPacket,
}

impl RustDailyDriverQualityTruthSupportExport {
    /// Returns true when the export preserves the same packet id safely.
    pub fn is_export_safe(&self) -> bool {
        self.record_kind == RUST_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND
            && self.schema_version == RUST_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION
            && self.rust_daily_driver_quality_packet_id_ref
...
/// Errors emitted when reading the checked-in stable Rust daily-driver packet.
#[derive(Debug)]
pub enum RustDailyDriverQualityTruthArtifactError {
...
impl fmt::Display for RustDailyDriverQualityTruthArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Packet(error) => write!(
                formatter,
                "rust daily-driver quality packet parse failed: {error}"
            ),
            Self::Validation(findings) => {
                ...
                write!(
                    formatter,
                    "rust daily-driver quality packet failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for RustDailyDriverQualityTruthArtifactError {}

/// Returns the checked-in stable Rust daily-driver quality truth packet.
///
/// # Errors
///
/// Returns an artifact error if the checked-in packet does not parse or validate.
pub fn current_stable_rust_daily_driver_quality_truth_packet(
) -> Result<RustDailyDriverQualityTruthPacket, RustDailyDriverQualityTruthArtifactError> {
    let packet: RustDailyDriverQualityTruthPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../artifacts/language/m4/rust_daily_driver_quality_truth_packet.json"
    )))
    .map_err(RustDailyDriverQualityTruthArtifactError::Packet)?;
    let findings = packet.validate();
    if findings.is_empty() {
        Ok(packet)
    } else {
        Err(RustDailyDriverQualityTruthArtifactError::Validation(
            findings,
        ))
    }
}
...
mod tests {
    ...
    fn lane_evidence_ref() -> String {
        RUST_DAILY_DRIVER_QUALITY_TRUTH_DOC_REF.to_owned()
    }

    fn quality_row(row_id: &str, lane: LanguageLaneClass) -> RustDailyDriverQualityRow {
        RustDailyDriverQualityRow {
            ...
            disclosure_ref: Some(format!(
                ...
                RUST_DAILY_DRIVER_QUALITY_TRUTH_DOC_REF
            )),
    ...
    fn projection(surface: ConsumerSurface) -> RustDailyDriverQualityConsumerProjection {
    ...
    fn lane_rows(lane: LanguageLaneClass, prefix: &str) -> Vec<RustDailyDriverQualityRow> {
    ...
    fn sample_input() -> RustDailyDriverQualityTruthPacketInput {
        let rows = lane_rows(LanguageLaneClass::RustDailyDriverLane, "rust");
        RustDailyDriverQualityTruthPacketInput {
            packet_id: "packet:m4:rust_daily_driver_quality".to_owned(),
            workflow_or_surface_id: "workflow.language.rust_daily_driver_quality".to_owned(),
    ...
    fn closed_tokens_are_pinned() {
        assert_eq!(
            LanguageLaneClass::RustDailyDriverLane.as_str(),
            "rust_daily_driver_lane"
        );
        assert_eq!(
            DailyDriverRowClass::CargoWorkspaceRow.as_str(),
            "cargo_workspace_row"
        );
        assert_eq!(
        ...
        assert_eq!(
            EvidenceClass::CargoWorkspaceEvidence.as_str(),
            "cargo_workspace_evidence"
        );
        assert_eq!(
            EvidenceClass::LintFormatEvidence.as_str(),
        ...
        assert_eq!(
            KnownLimitClass::CargoWorkspaceSubsetOnly.as_str(),
            "cargo_workspace_subset_only"
        );
        assert_eq!(
            KnownLimitClass::LintFormatSubsetOnly.as_str(),
            "lint_format_subset_only"
        );
        assert_eq!(
            KnownLimitClass::TestRunnerSubsetOnly.as_str(),
            "test_runner_subset_only"
        );
        assert_eq!(
            KnownLimitClass::DebuggerSubsetOnly.as_str(),
            "debugger_subset_only"
        );
        assert_eq!(
            KnownLimitClass::WorkspaceIndexSubsetOnly.as_str(),
            "workspace_index_subset_only"
        );
        assert_eq!(KnownLimitClass::LimitUnbound.as_str(), "limit_unbound");
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenCargoWorkspace.as_str(),
            "auto_narrow_on_unproven_cargo_workspace"
        );
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenLintFormat.as_str(),
            "auto_narrow_on_unproven_lint_format"
        );
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenTestRunner.as_str(),
            "auto_narrow_on_unproven_test_runner"
        );
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenDebugger.as_str(),
            "auto_narrow_on_unproven_debugger"
        );
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenWorkspaceIndex.as_str(),
    ...
    fn baseline_materialization_is_stable() {
        let packet = RustDailyDriverQualityTruthPacket::materialize(sample_input());
        assert_eq!(
        ...
        assert!(packet
            .support_export(
                "support:m4:rust_daily_driver_quality",
