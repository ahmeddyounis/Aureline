//! Stable Vue Advanced workflow pack truth packet.
//!
//! This module is the language-owned contract that pins how the Vue
//! Advanced workflow pack stays one boundary truth across the editor
//! framework pack panel, workflow companion, framework settings/help,
//! CLI/headless inspector, support export, release proof index,
//! Help/About proof card, and the conformance dashboard. Every row binds
//! a closed `workflow_pack_class`, `workflow_pack_row_class`,
//! `support_class`, `workflow_loop_class`, `evidence_class`,
//! `known_limit_class`, `downgrade_automation_class`, and
//! `workflow_pack_confidence_class` plus an `evidence_refs` array and a
//! `disclosure_ref` whenever the row is narrowed below `expert_grade`,
//! declares a non-`none_declared` known limit, or binds a non-`none`
//! downgrade automation.
//!
//! The packet pins the Vue Advanced workflow pack across the create,
//! open, run, test, debug, rename, and review workflow loops, the Vue
//! single-file-component project model (Composition API and Options
//! API), and the Vite / Webpack / Vue CLI build-tool parity surface. A
//! row that claims `expert_grade` while leaving its known limit or
//! downgrade automation unbound is refused; the validator narrows below
//! expert grade instead of inheriting an adjacent certified row, and
//! the downgrade-automation vocabulary is preserved verbatim so a
...
/// Stable record-kind tag for [`VueAdvancedWorkflowPackTruthPacket`].
pub const VUE_ADVANCED_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND: &str =
    "vue_advanced_workflow_pack_truth_stable_packet";

/// Stable record-kind tag for [`VueAdvancedWorkflowPackTruthSupportExport`].
pub const VUE_ADVANCED_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND: &str =
    "vue_advanced_workflow_pack_truth_support_export";

/// Integer schema version for the workflow-pack truth packet.
pub const VUE_ADVANCED_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the boundary schema.
pub const VUE_ADVANCED_WORKFLOW_PACK_TRUTH_SCHEMA_REF: &str =
    "schemas/language/vue_advanced_workflow_pack_truth.schema.json";

/// Repo-relative path of the reviewer contract doc.
pub const VUE_ADVANCED_WORKFLOW_PACK_TRUTH_DOC_REF: &str =
    "docs/languages/m4/stabilize-the-vue-advanced-workflow-pack-with-project.md";

/// Repo-relative path of the human-readable reviewer artifact.
pub const VUE_ADVANCED_WORKFLOW_PACK_TRUTH_ARTIFACT_DOC_REF: &str =
    "artifacts/language/m4/stabilize-the-vue-advanced-workflow-pack-with-project.md";

/// Repo-relative path of the protected fixture corpus directory.
pub const VUE_ADVANCED_WORKFLOW_PACK_TRUTH_FIXTURE_DIR: &str =
    "fixtures/language/m4/vue_advanced_workflow_pack_truth_packet";

/// Repo-relative path of the checked-in stable packet.
pub const VUE_ADVANCED_WORKFLOW_PACK_TRUTH_PACKET_ARTIFACT_REF: &str =
    "artifacts/language/m4/vue_advanced_workflow_pack_truth_packet.json";
...
pub enum WorkflowPackClass {
    /// Vue Advanced replacement-grade workflow pack.
    VueAdvancedWorkflowPack,
}

impl WorkflowPackClass {
    /// Every required workflow pack, in declaration order.
    pub const REQUIRED: [Self; 1] = [Self::VueAdvancedWorkflowPack];

    /// Stable token used in fixtures, schemas, and support exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::VueAdvancedWorkflowPack => "vue_advanced_workflow_pack",
        }
    }
}
...
pub enum WorkflowPackRowClass {
    ...
    ArchetypeRepoRow,
    /// Vue single-file-component project-model evidence row.
    ProjectModelRow,
    /// Build-tool parity (Vite / Webpack / Vue CLI) evidence row.
    BuildToolParityRow,
...
impl WorkflowPackRowClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::BuildToolParityRow => "build_tool_parity_row",
...
pub enum EvidenceClass {
    ...
    BenchmarkEvidence,
    /// The row is backed by a Vue SFC project-model capture.
    ProjectModelEvidence,
    /// The row is backed by a Vite / Webpack / Vue CLI build-tool parity capture.
    BuildToolParityEvidence,
...
impl EvidenceClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::BuildToolParityEvidence => "build_tool_parity_evidence",
...
pub enum KnownLimitClass {
    ...
    ArchetypeSubsetOnly,
    /// The row only certifies a migration subset (e.g., Vue 2 → Vue 3 only).
    MigrationSubsetOnly,
    /// The row only certifies a subset of the Vue SFC project model.
    ProjectModelSubsetOnly,
    /// The row only certifies a subset of the build-tool parity surface.
    BuildToolParitySubsetOnly,
...
impl KnownLimitClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::BuildToolParitySubsetOnly => "build_tool_parity_subset_only",
...
pub enum DowngradeAutomationClass {
    ...
    /// Automatically narrow when the Vue SFC project model is unproven.
    AutoNarrowOnUnprovenProjectModel,
    /// Automatically narrow when the build-tool parity surface is unproven.
    AutoNarrowOnUnprovenBuildToolParity,
...
impl DowngradeAutomationClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::AutoNarrowOnUnprovenBuildToolParity => {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct VueAdvancedWorkflowPackConsumerProjection {
    ...
    pub vue_advanced_workflow_pack_packet_id_ref: String,
...
impl VueAdvancedWorkflowPackConsumerProjection {
    fn preserves_truth_for(&self, packet_id: &str) -> bool {
        self.vue_advanced_workflow_pack_packet_id_ref == packet_id
...
/// Constructor input for [`VueAdvancedWorkflowPackTruthPacket::materialize`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct VueAdvancedWorkflowPackTruthPacketInput {
    /// Stable packet id.
    ...
    #[serde(default)]
    pub consumer_projections: Vec<VueAdvancedWorkflowPackConsumerProjection>,
    /// Source contracts (docs/schema/fixtures) consumed by the packet.
    #[serde(default)]
    pub source_contract_refs: Vec<String>,
}

/// Language-owned packet certifying the Vue Advanced workflow pack on
/// the stable lane across the create, open, run, test, debug, rename,
/// and review loops, with explicit Vue SFC project-model and
/// build-tool parity rows.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct VueAdvancedWorkflowPackTruthPacket {
    /// Stable record kind.
...
impl VueAdvancedWorkflowPackTruthPacket {
    /// Materializes a packet and records derived validation findings.
    pub fn materialize(input: VueAdvancedWorkflowPackTruthPacketInput) -> Self {
        let mut packet = Self {
            record_kind: VUE_ADVANCED_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND.to_owned(),
            schema_version: VUE_ADVANCED_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION,
    ...
    pub fn support_export(
        ...
        VueAdvancedWorkflowPackTruthSupportExport {
            record_kind: VUE_ADVANCED_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND.to_owned(),
            schema_version: VUE_ADVANCED_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION,
            export_id: export_id.into(),
            vue_advanced_workflow_pack_packet_id_ref: self.packet_id.clone(),
            exported_at: exported_at.into(),
            raw_private_material_excluded: true,
            ambient_authority_excluded: true,
            vue_advanced_workflow_pack_packet: self.clone(),
        }
    }
    ...
    fn derived_findings(&self, include_record_fields: bool) -> Vec<ValidationFinding> {
        ...
        if include_record_fields
            && self.record_kind != VUE_ADVANCED_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongRecordKind,
                FindingSeverity::Blocker,
                "vue advanced workflow pack packet has the wrong record kind",
            ));
        }
        if include_record_fields
            && self.schema_version != VUE_ADVANCED_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongSchemaVersion,
                FindingSeverity::Blocker,
                "vue advanced workflow pack packet has the wrong schema version",
            ));
        }
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct VueAdvancedWorkflowPackTruthSupportExport {
    /// Stable record kind.
    pub record_kind: String,
    /// Schema version.
    pub schema_version: u32,
    ...
    pub vue_advanced_workflow_pack_packet: VueAdvancedWorkflowPackTruthPacket,
}

impl VueAdvancedWorkflowPackTruthSupportExport {
    /// Returns true when the export preserves the same packet id safely.
    pub fn is_export_safe(&self) -> bool {
        self.record_kind == VUE_ADVANCED_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND
            && self.schema_version == VUE_ADVANCED_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION
            && self.vue_advanced_workflow_pack_packet_id_ref
                == self.vue_advanced_workflow_pack_packet.packet_id
            && self.raw_private_material_excluded
            && self.ambient_authority_excluded
            && self.vue_advanced_workflow_pack_packet.validate().is_empty()
    }
}

/// Errors emitted when reading the checked-in stable workflow-pack packet.
#[derive(Debug)]
pub enum VueAdvancedWorkflowPackTruthArtifactError {
...
impl fmt::Display for VueAdvancedWorkflowPackTruthArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Packet(error) => write!(
                formatter,
                "vue advanced workflow pack packet parse failed: {error}"
            ),
            Self::Validation(findings) => {
                ...
                write!(
                    formatter,
                    "vue advanced workflow pack packet failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for VueAdvancedWorkflowPackTruthArtifactError {}

/// Returns the checked-in stable Vue Advanced workflow pack truth packet.
///
/// # Errors
///
/// Returns an artifact error if the checked-in packet does not parse or validate.
pub fn current_stable_vue_advanced_workflow_pack_truth_packet(
) -> Result<VueAdvancedWorkflowPackTruthPacket, VueAdvancedWorkflowPackTruthArtifactError> {
    let packet: VueAdvancedWorkflowPackTruthPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../artifacts/language/m4/vue_advanced_workflow_pack_truth_packet.json"
    )))
    .map_err(VueAdvancedWorkflowPackTruthArtifactError::Packet)?;
    let findings = packet.validate();
    if findings.is_empty() {
        Ok(packet)
    } else {
        Err(VueAdvancedWorkflowPackTruthArtifactError::Validation(
            findings,
        ))
    }
}
...
mod tests {
    ...
    fn pack_evidence_ref() -> String {
        VUE_ADVANCED_WORKFLOW_PACK_TRUTH_DOC_REF.to_owned()
    }
    ...
    fn qualification_row(row_id: &str, pack: WorkflowPackClass) -> WorkflowPackRow {
        WorkflowPackRow {
            ...
            disclosure_ref: Some(format!(
                ...
                VUE_ADVANCED_WORKFLOW_PACK_TRUTH_DOC_REF
            )),
    ...
    fn projection(surface: ConsumerSurface) -> VueAdvancedWorkflowPackConsumerProjection {
    ...
    fn sample_input() -> VueAdvancedWorkflowPackTruthPacketInput {
        let rows = pack_rows(WorkflowPackClass::VueAdvancedWorkflowPack, "vue_advanced");
        VueAdvancedWorkflowPackTruthPacketInput {
            packet_id: "packet:m4:vue_advanced_workflow_pack".to_owned(),
            workflow_or_surface_id: "workflow.language.vue_advanced_workflow_pack".to_owned(),
    ...
    fn closed_tokens_are_pinned() {
        assert_eq!(
            WorkflowPackClass::VueAdvancedWorkflowPack.as_str(),
            "vue_advanced_workflow_pack"
        );
        assert_eq!(
        ...
        assert_eq!(
            WorkflowPackRowClass::BuildToolParityRow.as_str(),
            "build_tool_parity_row"
        );
        ...
        assert_eq!(
            EvidenceClass::BuildToolParityEvidence.as_str(),
            "build_tool_parity_evidence"
        );
        ...
        assert_eq!(
            KnownLimitClass::BuildToolParitySubsetOnly.as_str(),
            "build_tool_parity_subset_only"
        );
        ...
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenBuildToolParity.as_str(),
            "auto_narrow_on_unproven_build_tool_parity"
        );
        assert_eq!(
    ...
    fn baseline_materialization_is_stable() {
        let packet = VueAdvancedWorkflowPackTruthPacket::materialize(sample_input());
        assert_eq!(
        ...
        assert!(packet
            .support_export(
                "support:m4:vue_advanced_workflow_pack",
    ...
    fn expert_grade_with_unbound_evidence_blocks() {
        ...
        let packet = VueAdvancedWorkflowPackTruthPacket::materialize(input);
