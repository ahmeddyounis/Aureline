//! Stable Java and Kotlin daily-driver quality truth packet for the
//! M4 stable lane.
//!
//! This module is the language-owned contract that pins how Java and
//! Kotlin replacement-grade daily-driver quality stays one boundary
...
//!    proxy configuration) binds a dedicated `build_workspace_row` and
//!    a disclosure ref so the daily-driver row never confuses one
...
//!    `application.properties` / `application.yml` profile selection)
...
//!    call-hierarchy, and find-references; source-roots /
//!    classpath / module-path / kotlin source-set resolution; package
//!    rename across both `java/` and `kotlin/` source roots; JavaDoc
//!    and KDoc cross-link navigation) so large-workspace rename and
//!    navigation posture is never inferred from a tiny single-module
//!    sample.
//!
...
/// Stable record-kind tag for [`JavaAndKotlinDailyDriverQualityTruthPacket`].
pub const JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND: &str =
    "java_and_kotlin_daily_driver_quality_truth_stable_packet";

/// Stable record-kind tag for [`JavaAndKotlinDailyDriverQualityTruthSupportExport`].
pub const JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND: &str =
    "java_and_kotlin_daily_driver_quality_truth_support_export";

/// Integer schema version for the Java and Kotlin daily-driver quality packet.
pub const JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the boundary schema.
pub const JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_REF: &str =
    "schemas/language/java_and_kotlin_daily_driver_quality_truth.schema.json";

/// Repo-relative path of the reviewer contract doc.
pub const JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_DOC_REF: &str =
...
pub const JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_ARTIFACT_DOC_REF: &str =
    "artifacts/language/m4/stabilize-java-and-kotlin-daily-driver-quality-with.md";

/// Repo-relative path of the protected fixture corpus directory.
pub const JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_FIXTURE_DIR: &str =
    "fixtures/language/m4/java_and_kotlin_daily_driver_quality_truth_packet";

/// Repo-relative path of the checked-in stable packet.
pub const JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_PACKET_ARTIFACT_REF: &str =
...
pub enum LanguageLaneClass {
    /// Java and Kotlin replacement-grade daily-driver lane.
    JavaAndKotlinDailyDriverLane,
}

impl LanguageLaneClass {
    /// Every required language lane, in declaration order.
    pub const REQUIRED: [Self; 1] = [Self::JavaAndKotlinDailyDriverLane];

    /// Stable token used in fixtures, schemas, and support exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::JavaAndKotlinDailyDriverLane => "java_and_kotlin_daily_driver_lane",
        }
    }
}
...
pub enum DailyDriverRowClass {
    ...
    /// Gradle / Maven build-workspace evidence row.
    BuildWorkspaceRow,
    /// Spring Boot run / debug / test evidence row.
    SpringBootRunTestDebugRow,
    /// `eclipse.jdt.ls` / Kotlin LSP rename / navigation evidence row.
    RenameNavigationRow,
...
impl DailyDriverRowClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SpringBootRunTestDebugRow => "spring_boot_run_test_debug_row",
            Self::RenameNavigationRow => "rename_navigation_row",
...
pub enum EvidenceClass {
    ...
    BenchmarkEvidence,
    /// The row is backed by a Gradle / Maven build-workspace capture.
    BuildWorkspaceEvidence,
    /// The row is backed by a Spring Boot run / debug / test capture.
    SpringBootRunTestDebugEvidence,
    /// The row is backed by a Java / Kotlin rename and navigation capture.
    RenameNavigationEvidence,
...
impl EvidenceClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SpringBootRunTestDebugEvidence => "spring_boot_run_test_debug_evidence",
            Self::RenameNavigationEvidence => "rename_navigation_evidence",
...
pub enum KnownLimitClass {
    ...
    FrameworkSubsetOnly,
    /// The row only certifies a language subset (e.g., Java without Kotlin).
    LanguageSubsetOnly,
    /// The row only certifies an archetype subset (specific repos).
    ArchetypeSubsetOnly,
    /// The row only certifies a migration subset.
    MigrationSubsetOnly,
    /// The row only certifies a subset of the Gradle / Maven build-workspace surface.
    BuildWorkspaceSubsetOnly,
    /// The row only certifies a subset of the Spring Boot run / debug / test surface.
    SpringBootRunTestDebugSubsetOnly,
    /// The row only certifies a subset of the rename / navigation surface.
    RenameNavigationSubsetOnly,
...
impl KnownLimitClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SpringBootRunTestDebugSubsetOnly => "spring_boot_run_test_debug_subset_only",
            Self::RenameNavigationSubsetOnly => "rename_navigation_subset_only",
...
pub enum DowngradeAutomationClass {
    ...
    /// Automatically narrow when the Gradle / Maven build-workspace surface is unproven.
    AutoNarrowOnUnprovenBuildWorkspace,
    /// Automatically narrow when the Spring Boot run / debug / test surface is unproven.
    AutoNarrowOnUnprovenSpringBootRunTestDebug,
    /// Automatically narrow when the rename / navigation surface is unproven.
    AutoNarrowOnUnprovenRenameNavigation,
...
impl DowngradeAutomationClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::AutoNarrowOnUnprovenSpringBootRunTestDebug => {
                "auto_narrow_on_unproven_spring_boot_run_test_debug"
            }
            Self::AutoNarrowOnUnprovenRenameNavigation => {
...
/// Closed validation-finding vocabulary for the Java and Kotlin
/// daily-driver packet.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Consumer surface that must inherit the Java and Kotlin daily-driver
/// packet verbatim.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// One Java and Kotlin daily-driver quality row.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct JavaAndKotlinDailyDriverQualityRow {
...
impl JavaAndKotlinDailyDriverQualityRow {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct JavaAndKotlinDailyDriverQualityConsumerProjection {
    ...
    pub java_and_kotlin_daily_driver_quality_packet_id_ref: String,
...
impl JavaAndKotlinDailyDriverQualityConsumerProjection {
    fn preserves_truth_for(&self, packet_id: &str) -> bool {
        self.java_and_kotlin_daily_driver_quality_packet_id_ref == packet_id
...
/// [`JavaAndKotlinDailyDriverQualityTruthPacket::materialize`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct JavaAndKotlinDailyDriverQualityTruthPacketInput {
    /// Stable packet id.
    ...
    /// Daily-driver rows.
    #[serde(default)]
    pub rows: Vec<JavaAndKotlinDailyDriverQualityRow>,
    /// Consumer projections preserving this packet.
    #[serde(default)]
    pub consumer_projections: Vec<JavaAndKotlinDailyDriverQualityConsumerProjection>,
    /// Source contracts (docs/schema/fixtures) consumed by the packet.
    #[serde(default)]
    pub source_contract_refs: Vec<String>,
}

/// Language-owned packet certifying Java and Kotlin replacement-grade
/// daily-driver quality on the M4 stable lane with explicit Gradle /
/// Maven build-workspace, Spring Boot run / debug / test, and
/// rename / navigation truth.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct JavaAndKotlinDailyDriverQualityTruthPacket {
    /// Stable record kind.
...
impl JavaAndKotlinDailyDriverQualityTruthPacket {
    /// Materializes a packet and records derived validation findings.
    pub fn materialize(input: JavaAndKotlinDailyDriverQualityTruthPacketInput) -> Self {
        let mut packet = Self {
            record_kind: JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND.to_owned(),
            schema_version: JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION,
    ...
    pub fn support_export(
        ...
        JavaAndKotlinDailyDriverQualityTruthSupportExport {
            record_kind: JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND
                .to_owned(),
            schema_version: JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION,
            export_id: export_id.into(),
            java_and_kotlin_daily_driver_quality_packet_id_ref: self.packet_id.clone(),
            exported_at: exported_at.into(),
            raw_private_material_excluded: true,
            ambient_authority_excluded: true,
            java_and_kotlin_daily_driver_quality_packet: self.clone(),
        }
    }
    ...
    fn derived_findings(&self, include_record_fields: bool) -> Vec<ValidationFinding> {
        ...
        if include_record_fields
            && self.record_kind != JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongRecordKind,
                FindingSeverity::Blocker,
                "java and kotlin daily-driver quality packet has the wrong record kind",
            ));
        }
        if include_record_fields
            && self.schema_version != JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongSchemaVersion,
                FindingSeverity::Blocker,
                "java and kotlin daily-driver quality packet has the wrong schema version",
            ));
        }
        ...
        for projection in &self.consumer_projections {
            if !projection.preserves_truth_for(&self.packet_id) {
                findings.push(ValidationFinding::new(
                    ...
                    FindingSeverity::Blocker,
                    format!(
                        "projection {} does not preserve java and kotlin daily-driver quality truth",
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct JavaAndKotlinDailyDriverQualityTruthSupportExport {
    /// Stable record kind.
    pub record_kind: String,
    /// Schema version.
    pub schema_version: u32,
    ...
    pub java_and_kotlin_daily_driver_quality_packet: JavaAndKotlinDailyDriverQualityTruthPacket,
}

impl JavaAndKotlinDailyDriverQualityTruthSupportExport {
    /// Returns true when the export preserves the same packet id safely.
    pub fn is_export_safe(&self) -> bool {
        self.record_kind == JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND
            && self.schema_version == JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION
            && self.java_and_kotlin_daily_driver_quality_packet_id_ref
                == self.java_and_kotlin_daily_driver_quality_packet.packet_id
            && self.raw_private_material_excluded
            && self.ambient_authority_excluded
            && self
                .java_and_kotlin_daily_driver_quality_packet
                .validate()
                .is_empty()
    }
}

/// Errors emitted when reading the checked-in stable Java and Kotlin
/// daily-driver packet.
#[derive(Debug)]
pub enum JavaAndKotlinDailyDriverQualityTruthArtifactError {
...
impl fmt::Display for JavaAndKotlinDailyDriverQualityTruthArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Packet(error) => write!(
                formatter,
                "java and kotlin daily-driver quality packet parse failed: {error}"
            ),
            Self::Validation(findings) => {
                ...
                write!(
                    formatter,
                    "java and kotlin daily-driver quality packet failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for JavaAndKotlinDailyDriverQualityTruthArtifactError {}

/// Returns the checked-in stable Java and Kotlin daily-driver quality
/// truth packet.
///
/// # Errors
///
...
) -> Result<JavaAndKotlinDailyDriverQualityTruthPacket, JavaAndKotlinDailyDriverQualityTruthArtifactError>
{
    ...
    if findings.is_empty() {
        ...
        Err(JavaAndKotlinDailyDriverQualityTruthArtifactError::Validation(findings))
    }
}
...
mod tests {
    ...
    fn lane_evidence_ref() -> String {
        JAVA_AND_KOTLIN_DAILY_DRIVER_QUALITY_TRUTH_DOC_REF.to_owned()
    }

    fn quality_row(row_id: &str, lane: LanguageLaneClass) -> JavaAndKotlinDailyDriverQualityRow {
        JavaAndKotlinDailyDriverQualityRow {
    ...
    fn projection(
        ...
        JavaAndKotlinDailyDriverQualityConsumerProjection {
            ...
            java_and_kotlin_daily_driver_quality_packet_id_ref:
                "packet:m4:java_and_kotlin_daily_driver_quality".to_owned(),
    ...
    fn sample_input() -> JavaAndKotlinDailyDriverQualityTruthPacketInput {
        let rows = lane_rows(
            LanguageLaneClass::JavaAndKotlinDailyDriverLane,
            "java_kotlin",
        );
    ...
    fn closed_tokens_are_pinned() {
        assert_eq!(
            LanguageLaneClass::JavaAndKotlinDailyDriverLane.as_str(),
        ...
        assert_eq!(
            DailyDriverRowClass::SpringBootRunTestDebugRow.as_str(),
            "spring_boot_run_test_debug_row"
        );
        assert_eq!(
            DailyDriverRowClass::RenameNavigationRow.as_str(),
        ...
        assert_eq!(
            EvidenceClass::SpringBootRunTestDebugEvidence.as_str(),
            "spring_boot_run_test_debug_evidence"
        );
        assert_eq!(
            EvidenceClass::RenameNavigationEvidence.as_str(),
        ...
        assert_eq!(
            KnownLimitClass::SpringBootRunTestDebugSubsetOnly.as_str(),
            "spring_boot_run_test_debug_subset_only"
        );
        assert_eq!(
            KnownLimitClass::RenameNavigationSubsetOnly.as_str(),
        ...
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenSpringBootRunTestDebug.as_str(),
            "auto_narrow_on_unproven_spring_boot_run_test_debug"
        );
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenRenameNavigation.as_str(),
            "auto_narrow_on_unproven_rename_navigation"
        );
        assert_eq!(
    ...
    fn baseline_materialization_is_stable() {
        ...
        assert!(packet
            .support_export(
                "support:m4:java_and_kotlin_daily_driver_quality",
    ...
    fn replacement_grade_with_unbound_evidence_blocks() {
        ...
        let packet = JavaAndKotlinDailyDriverQualityTruthPacket::materialize(input);
