//! Stable Spring Boot Expert workflow pack truth packet.
//!
//! This module is the language-owned contract that pins how the
//! Spring Boot Expert workflow pack stays one boundary truth across
//! the editor framework pack panel, workflow companion, framework
...
//!
//! The packet pins the Spring Boot Expert workflow pack across the
//! create, open, run, test, debug, rename, and review workflow loops,
//! the Spring Boot target discovery surface (Gradle `./gradlew
//! bootRun`, `./gradlew bootBuildImage`, `./gradlew bootJar`,
//! `./gradlew bootWar`, Maven `mvn spring-boot:run`,
//! `mvn spring-boot:build-image`, `mvn spring-boot:repackage`, custom
//! Gradle/Maven targets, JUnit 5 / JUnit 4 / Spock / Kotest test
//! tasks, and JDWP / JPDA attach and listen debug targets discovered
//! from `build.gradle(.kts)` / `pom.xml` / `settings.gradle(.kts)`),
//! the Spring Boot config/property awareness surface
...
//! resolution), and the Spring Boot review-safe refactor surface
//! (refactors that respect Spring IoC/DI, `@Component` /
//! `@Service` / `@Repository` / `@Controller` /
//! `@RestController` / `@Configuration` bean lifecycle,
//! `@Autowired` / constructor-injection / `@Qualifier` resolution,
//! Spring AOP advice targets, bean rename across `@Bean` / `@Bean(name)`
//! / `@Component("name")` references, and rename-preview gating on
//! beans whose binding cannot be statically proven). A row that
//! claims `expert_grade` while leaving its known limit or downgrade
//! automation unbound is refused; the validator narrows below expert
//! grade instead of inheriting an adjacent certified row, and the
//! downgrade-automation vocabulary is preserved verbatim so a
...
/// Stable record-kind tag for [`SpringBootExpertWorkflowPackTruthPacket`].
pub const SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND: &str =
    "spring_boot_expert_workflow_pack_truth_stable_packet";

/// Stable record-kind tag for [`SpringBootExpertWorkflowPackTruthSupportExport`].
pub const SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND: &str =
    "spring_boot_expert_workflow_pack_truth_support_export";

/// Integer schema version for the workflow-pack truth packet.
pub const SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the boundary schema.
pub const SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_REF: &str =
    "schemas/language/spring_boot_expert_workflow_pack_truth.schema.json";

/// Repo-relative path of the reviewer contract doc.
pub const SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_DOC_REF: &str =
...
pub const SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_ARTIFACT_DOC_REF: &str =
    "artifacts/language/m4/stabilize-the-spring-boot-expert-workflow-pack-with.md";

/// Repo-relative path of the protected fixture corpus directory.
pub const SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_FIXTURE_DIR: &str =
    "fixtures/language/m4/spring_boot_expert_workflow_pack_truth_packet";

/// Repo-relative path of the checked-in stable packet.
pub const SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_ARTIFACT_REF: &str =
...
pub enum WorkflowPackClass {
    /// Spring Boot Expert replacement-grade workflow pack.
    SpringBootExpertWorkflowPack,
}

impl WorkflowPackClass {
    /// Every required workflow pack, in declaration order.
    pub const REQUIRED: [Self; 1] = [Self::SpringBootExpertWorkflowPack];

    /// Stable token used in fixtures, schemas, and support exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::SpringBootExpertWorkflowPack => "spring_boot_expert_workflow_pack",
        }
    }
}
...
pub enum WorkflowPackRowClass {
    ...
    ArchetypeRepoRow,
    /// Spring Boot target discovery evidence row (`bootRun`,
    /// `bootBuildImage`, `bootJar`, `bootWar`, `mvn spring-boot:run`,
    /// custom Gradle/Maven targets, JUnit/Spock/Kotest test tasks,
    /// JDWP/JPDA debug targets).
    TargetDiscoveryRow,
    /// Spring Boot config/property awareness evidence row
    ...
    /// auto-configuration imports resolution).
    ConfigPropertyAwarenessRow,
    /// Spring Boot review-safe refactor evidence row (refactors that
    /// respect Spring IoC/DI bean lifecycle, `@Component` /
    /// `@Service` / `@Repository` / `@Controller` /
    /// `@RestController` / `@Configuration`, `@Autowired` /
    /// constructor-injection / `@Qualifier` resolution, AOP advice
    /// targets, bean rename across `@Bean(name)` references, and
    /// rename-preview gating on beans whose binding cannot be
    /// statically proven).
    ReviewSafeRefactorRow,
...
impl WorkflowPackRowClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::TargetDiscoveryRow => "target_discovery_row",
            Self::ConfigPropertyAwarenessRow => "config_property_awareness_row",
            Self::ReviewSafeRefactorRow => "review_safe_refactor_row",
...
pub enum EvidenceClass {
    ...
    BenchmarkEvidence,
    /// The row is backed by a Spring Boot target discovery capture
    /// (bootRun / bootBuildImage / bootJar / bootWar /
    /// spring-boot:run / JUnit/Spock/Kotest tasks / JDWP debug
    /// targets discovered from build files).
    TargetDiscoveryEvidence,
    /// The row is backed by a Spring Boot config/property awareness
    /// capture (`application.properties`/`application.yml`,
    /// profile-specific config, `@ConfigurationProperties`,
    /// `@Value`, `@ConditionalOnProperty`, `@Profile`,
    /// `spring.config.import`, auto-configuration imports).
    ConfigPropertyAwarenessEvidence,
    /// The row is backed by a Spring Boot review-safe refactor
    /// capture (DI/IoC-aware rename, `@Component` /`@Service` /
    /// `@Repository` / `@Controller` / `@Configuration` lifecycle
    /// safety, `@Autowired` / constructor-injection / `@Qualifier`
    /// resolution, AOP advice targets, bean rename across
    /// `@Bean(name)` references).
    ReviewSafeRefactorEvidence,
...
impl EvidenceClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::TargetDiscoveryEvidence => "target_discovery_evidence",
            Self::ConfigPropertyAwarenessEvidence => "config_property_awareness_evidence",
            Self::ReviewSafeRefactorEvidence => "review_safe_refactor_evidence",
...
pub enum KnownLimitClass {
    ...
    FrameworkSubsetOnly,
    /// The row only certifies a language subset (e.g., Java only or Kotlin only).
    LanguageSubsetOnly,
    /// The row only certifies an archetype subset (specific repos).
    ArchetypeSubsetOnly,
    /// The row only certifies a migration subset (e.g., Spring Boot 2.x → 3.x only).
    MigrationSubsetOnly,
    /// The row only certifies a subset of the Spring Boot target discovery surface.
    TargetDiscoverySubsetOnly,
    /// The row only certifies a subset of the Spring Boot config/property awareness surface.
    ConfigPropertySubsetOnly,
    /// The row only certifies a subset of the Spring Boot review-safe refactor surface.
    ReviewSafeRefactorSubsetOnly,
...
impl KnownLimitClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::TargetDiscoverySubsetOnly => "target_discovery_subset_only",
            Self::ConfigPropertySubsetOnly => "config_property_subset_only",
            Self::ReviewSafeRefactorSubsetOnly => "review_safe_refactor_subset_only",
...
pub enum DowngradeAutomationClass {
    ...
    /// Automatically narrow when the Spring Boot target discovery surface is unproven.
    AutoNarrowOnUnprovenTargetDiscovery,
    /// Automatically narrow when the Spring Boot config/property awareness surface is unproven.
    AutoNarrowOnUnprovenConfigPropertyAwareness,
    /// Automatically narrow when the Spring Boot review-safe refactor surface is unproven.
    AutoNarrowOnUnprovenReviewSafeRefactor,
...
impl DowngradeAutomationClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::AutoNarrowOnUnprovenReviewSafeRefactor => {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct SpringBootExpertWorkflowPackConsumerProjection {
    ...
    pub spring_boot_expert_workflow_pack_packet_id_ref: String,
...
impl SpringBootExpertWorkflowPackConsumerProjection {
    fn preserves_truth_for(&self, packet_id: &str) -> bool {
        self.spring_boot_expert_workflow_pack_packet_id_ref == packet_id
...
/// Constructor input for [`SpringBootExpertWorkflowPackTruthPacket::materialize`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct SpringBootExpertWorkflowPackTruthPacketInput {
    /// Stable packet id.
    ...
    #[serde(default)]
    pub consumer_projections: Vec<SpringBootExpertWorkflowPackConsumerProjection>,
    /// Source contracts (docs/schema/fixtures) consumed by the packet.
    #[serde(default)]
    pub source_contract_refs: Vec<String>,
}

/// Language-owned packet certifying the Spring Boot Expert workflow
/// pack on the stable lane across the create, open, run, test, debug,
/// rename, and review loops, with explicit Spring Boot target
/// discovery, config/property awareness, and review-safe refactor
/// rows.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct SpringBootExpertWorkflowPackTruthPacket {
    /// Stable record kind.
...
impl SpringBootExpertWorkflowPackTruthPacket {
    /// Materializes a packet and records derived validation findings.
    pub fn materialize(input: SpringBootExpertWorkflowPackTruthPacketInput) -> Self {
        let mut packet = Self {
            record_kind: SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND.to_owned(),
            schema_version: SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION,
    ...
    pub fn support_export(
        ...
        SpringBootExpertWorkflowPackTruthSupportExport {
            record_kind: SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND
                .to_owned(),
            schema_version: SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION,
            export_id: export_id.into(),
            spring_boot_expert_workflow_pack_packet_id_ref: self.packet_id.clone(),
            exported_at: exported_at.into(),
            raw_private_material_excluded: true,
            ambient_authority_excluded: true,
            spring_boot_expert_workflow_pack_packet: self.clone(),
        }
    }
    ...
    fn derived_findings(&self, include_record_fields: bool) -> Vec<ValidationFinding> {
        ...
        if include_record_fields
            && self.record_kind != SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_PACKET_RECORD_KIND
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongRecordKind,
                FindingSeverity::Blocker,
                "spring boot expert workflow pack packet has the wrong record kind",
            ));
        }
        if include_record_fields
            && self.schema_version != SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongSchemaVersion,
                FindingSeverity::Blocker,
                "spring boot expert workflow pack packet has the wrong schema version",
            ));
        }
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct SpringBootExpertWorkflowPackTruthSupportExport {
    /// Stable record kind.
    pub record_kind: String,
    /// Schema version.
    pub schema_version: u32,
    ...
    pub spring_boot_expert_workflow_pack_packet: SpringBootExpertWorkflowPackTruthPacket,
}

impl SpringBootExpertWorkflowPackTruthSupportExport {
    /// Returns true when the export preserves the same packet id safely.
    pub fn is_export_safe(&self) -> bool {
        self.record_kind == SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_SUPPORT_EXPORT_RECORD_KIND
            && self.schema_version == SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_SCHEMA_VERSION
            && self.spring_boot_expert_workflow_pack_packet_id_ref
                == self.spring_boot_expert_workflow_pack_packet.packet_id
            && self.raw_private_material_excluded
            && self.ambient_authority_excluded
            && self
                .spring_boot_expert_workflow_pack_packet
                .validate()
                .is_empty()
    }
}
...
#[derive(Debug)]
pub enum SpringBootExpertWorkflowPackTruthArtifactError {
...
impl fmt::Display for SpringBootExpertWorkflowPackTruthArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Packet(error) => write!(
                formatter,
                "spring boot expert workflow pack packet parse failed: {error}"
            ),
            Self::Validation(findings) => {
                ...
                write!(
                    formatter,
                    "spring boot expert workflow pack packet failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for SpringBootExpertWorkflowPackTruthArtifactError {}

/// Returns the checked-in stable Spring Boot Expert workflow pack truth packet.
///
/// # Errors
///
...
> {
    let packet: SpringBootExpertWorkflowPackTruthPacket =
        ...
        .map_err(SpringBootExpertWorkflowPackTruthArtifactError::Packet)?;
    let findings = packet.validate();
    if findings.is_empty() {
        Ok(packet)
    } else {
        Err(SpringBootExpertWorkflowPackTruthArtifactError::Validation(
            findings,
        ))
    }
}
...
mod tests {
    ...
    fn pack_evidence_ref() -> String {
        SPRING_BOOT_EXPERT_WORKFLOW_PACK_TRUTH_DOC_REF.to_owned()
    }
    ...
    fn projection(surface: ConsumerSurface) -> SpringBootExpertWorkflowPackConsumerProjection {
        SpringBootExpertWorkflowPackConsumerProjection {
            consumer_surface: surface,
            projection_ref: format!("projection:{}", surface.as_str()),
            spring_boot_expert_workflow_pack_packet_id_ref:
                "packet:m4:spring_boot_expert_workflow_pack".to_owned(),
    ...
    fn sample_input() -> SpringBootExpertWorkflowPackTruthPacketInput {
        let rows = pack_rows(
            WorkflowPackClass::SpringBootExpertWorkflowPack,
            "spring_boot_expert",
        );
        SpringBootExpertWorkflowPackTruthPacketInput {
            packet_id: "packet:m4:spring_boot_expert_workflow_pack".to_owned(),
            workflow_or_surface_id: "workflow.language.spring_boot_expert_workflow_pack".to_owned(),
    ...
    fn closed_tokens_are_pinned() {
        assert_eq!(
            WorkflowPackClass::SpringBootExpertWorkflowPack.as_str(),
            "spring_boot_expert_workflow_pack"
        );
        assert_eq!(
            WorkflowPackRowClass::TargetDiscoveryRow.as_str(),
            "target_discovery_row"
        );
        assert_eq!(
            WorkflowPackRowClass::ConfigPropertyAwarenessRow.as_str(),
            "config_property_awareness_row"
        );
        assert_eq!(
            WorkflowPackRowClass::ReviewSafeRefactorRow.as_str(),
        ...
        assert_eq!(
            EvidenceClass::TargetDiscoveryEvidence.as_str(),
            "target_discovery_evidence"
        );
        assert_eq!(
            EvidenceClass::ConfigPropertyAwarenessEvidence.as_str(),
            "config_property_awareness_evidence"
        );
        assert_eq!(
            EvidenceClass::ReviewSafeRefactorEvidence.as_str(),
            "review_safe_refactor_evidence"
        );
        assert_eq!(EvidenceClass::EvidenceUnbound.as_str(), "evidence_unbound");
        assert_eq!(
            KnownLimitClass::TargetDiscoverySubsetOnly.as_str(),
            "target_discovery_subset_only"
        );
        assert_eq!(
            KnownLimitClass::ConfigPropertySubsetOnly.as_str(),
            "config_property_subset_only"
        );
        assert_eq!(
            KnownLimitClass::ReviewSafeRefactorSubsetOnly.as_str(),
            "review_safe_refactor_subset_only"
        );
        assert_eq!(KnownLimitClass::LimitUnbound.as_str(), "limit_unbound");
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenTargetDiscovery.as_str(),
            "auto_narrow_on_unproven_target_discovery"
        );
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenConfigPropertyAwareness.as_str(),
            "auto_narrow_on_unproven_config_property_awareness"
        );
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenReviewSafeRefactor.as_str(),
            "auto_narrow_on_unproven_review_safe_refactor"
        );
        assert_eq!(
    ...
    fn baseline_materialization_is_stable() {
        let packet = SpringBootExpertWorkflowPackTruthPacket::materialize(sample_input());
        assert_eq!(
        ...
        assert!(packet
            .support_export(
                "support:m4:spring_boot_expert_workflow_pack",
    ...
    fn expert_grade_with_unbound_evidence_blocks() {
        ...
        let packet = SpringBootExpertWorkflowPackTruthPacket::materialize(input);
