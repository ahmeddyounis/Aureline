//! Stable Python daily-driver quality truth packet for the M4 stable
//! lane.
//!
//! This module is the language-owned contract that pins how Python
...
//!    active `python_path`) binds a dedicated `interpreter_selection_row`
//!    and a disclosure ref so the daily-driver row never confuses one
...
//!    capability sample cannot masquerade as a replacement-grade Python
//!    daily driver.
...
//!    `python -m pdb` / `pytest --pdb` recovery) so that
//!    run/test/debug rows cannot inherit an adjacent test-runner or
...
/// Stable record-kind tag for [`PythonDailyDriverQualityTruthPacket`].
pub const PYTHON_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND: &str =
    "python_daily_driver_quality_truth_stable_packet";

/// Stable record-kind tag for [`PythonDailyDriverQualityTruthSupportExport`].
pub const PYTHON_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND: &str =
    "python_daily_driver_quality_truth_support_export";

/// Integer schema version for the Python daily-driver quality packet.
pub const PYTHON_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the boundary schema.
pub const PYTHON_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_REF: &str =
    "schemas/language/python_daily_driver_quality_truth.schema.json";

/// Repo-relative path of the reviewer contract doc.
pub const PYTHON_DAILY_DRIVER_QUALITY_TRUTH_DOC_REF: &str =
...
pub const PYTHON_DAILY_DRIVER_QUALITY_TRUTH_ARTIFACT_DOC_REF: &str =
    "artifacts/language/m4/stabilize-python-daily-driver-quality-with-interpreter-venv.md";

/// Repo-relative path of the protected fixture corpus directory.
pub const PYTHON_DAILY_DRIVER_QUALITY_TRUTH_FIXTURE_DIR: &str =
    "fixtures/language/m4/python_daily_driver_quality_truth_packet";

/// Repo-relative path of the checked-in stable packet.
pub const PYTHON_DAILY_DRIVER_QUALITY_TRUTH_PACKET_ARTIFACT_REF: &str =
...
pub enum LanguageLaneClass {
    /// Python replacement-grade daily-driver lane.
    PythonDailyDriverLane,
}

impl LanguageLaneClass {
    /// Every required language lane, in declaration order.
    pub const REQUIRED: [Self; 1] = [Self::PythonDailyDriverLane];

    /// Stable token used in fixtures, schemas, and support exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::PythonDailyDriverLane => "python_daily_driver_lane",
        }
    }
}
...
pub enum DailyDriverRowClass {
    ...
    /// Python interpreter selection evidence row on a lane.
    InterpreterSelectionRow,
    /// Python environment-manager (venv / uv / Poetry / pipenv / conda)
    /// evidence row on a lane.
    EnvironmentManagerRow,
    /// Python test-runner (pytest / unittest / tox) evidence row.
    TestRunnerRow,
    /// Python debugger (debugpy / pdb) evidence row.
    DebuggerRow,
...
impl DailyDriverRowClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::InterpreterSelectionRow => "interpreter_selection_row",
            Self::EnvironmentManagerRow => "environment_manager_row",
...
pub enum EvidenceClass {
    ...
    BenchmarkEvidence,
    /// The row is backed by a Python interpreter selection capture.
    InterpreterSelectionEvidence,
    /// The row is backed by an environment-manager capture (venv / uv
    /// / Poetry / pipenv / conda).
    EnvironmentManagerEvidence,
    /// The row is backed by a Python test-runner capture (pytest /
    /// unittest / tox).
    TestRunnerEvidence,
    /// The row is backed by a Python debugger capture (debugpy / pdb).
    DebuggerEvidence,
...
impl EvidenceClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::InterpreterSelectionEvidence => "interpreter_selection_evidence",
            Self::EnvironmentManagerEvidence => "environment_manager_evidence",
...
pub enum KnownLimitClass {
    ...
    FrameworkSubsetOnly,
    /// The row only certifies a language subset (e.g., one syntax mode).
    LanguageSubsetOnly,
    /// The row only certifies an archetype subset (specific repos).
    ArchetypeSubsetOnly,
    /// The row only certifies a migration subset.
    MigrationSubsetOnly,
    /// The row only certifies a subset of the Python interpreter
    /// selection surface.
    InterpreterSubsetOnly,
    /// The row only certifies a subset of the environment-manager surface.
    EnvironmentManagerSubsetOnly,
    /// The row only certifies a subset of the Python test-runner surface.
    TestRunnerSubsetOnly,
    /// The row only certifies a subset of the Python debugger surface.
    DebuggerSubsetOnly,
...
impl KnownLimitClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::InterpreterSubsetOnly => "interpreter_subset_only",
            Self::EnvironmentManagerSubsetOnly => "environment_manager_subset_only",
...
pub enum DowngradeAutomationClass {
    ...
    /// Automatically narrow when the Python interpreter selection is unproven.
    AutoNarrowOnUnprovenInterpreter,
    /// Automatically narrow when the environment-manager surface is unproven.
    AutoNarrowOnUnprovenEnvironmentManager,
    /// Automatically narrow when the Python test-runner surface is unproven.
    AutoNarrowOnUnprovenTestRunner,
    /// Automatically narrow when the Python debugger surface is unproven.
...
impl DowngradeAutomationClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::AutoNarrowOnUnprovenInterpreter => "auto_narrow_on_unproven_interpreter",
            Self::AutoNarrowOnUnprovenEnvironmentManager => {
...
/// Closed validation-finding vocabulary for the Python daily-driver packet.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Consumer surface that must inherit the Python daily-driver packet
/// verbatim.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// One Python daily-driver quality row.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PythonDailyDriverQualityRow {
...
impl PythonDailyDriverQualityRow {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PythonDailyDriverQualityConsumerProjection {
    ...
    pub python_daily_driver_quality_packet_id_ref: String,
...
impl PythonDailyDriverQualityConsumerProjection {
    fn preserves_truth_for(&self, packet_id: &str) -> bool {
        self.python_daily_driver_quality_packet_id_ref == packet_id
...
/// Constructor input for [`PythonDailyDriverQualityTruthPacket::materialize`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PythonDailyDriverQualityTruthPacketInput {
    /// Stable packet id.
    ...
    /// Daily-driver rows.
    #[serde(default)]
    pub rows: Vec<PythonDailyDriverQualityRow>,
    /// Consumer projections preserving this packet.
    #[serde(default)]
    pub consumer_projections: Vec<PythonDailyDriverQualityConsumerProjection>,
    /// Source contracts (docs/schema/fixtures) consumed by the packet.
    #[serde(default)]
    pub source_contract_refs: Vec<String>,
}

/// Language-owned packet certifying Python replacement-grade
/// daily-driver quality on the M4 stable lane with explicit interpreter
/// selection, environment-manager (venv / uv / Poetry), test-runner
/// (pytest), and debugger truth.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PythonDailyDriverQualityTruthPacket {
    /// Stable record kind.
...
impl PythonDailyDriverQualityTruthPacket {
    /// Materializes a packet and records derived validation findings.
    pub fn materialize(input: PythonDailyDriverQualityTruthPacketInput) -> Self {
        let mut packet = Self {
            record_kind: PYTHON_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND.to_owned(),
            schema_version: PYTHON_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION,
    ...
    ) -> PythonDailyDriverQualityTruthSupportExport {
        PythonDailyDriverQualityTruthSupportExport {
            record_kind: PYTHON_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND.to_owned(),
            schema_version: PYTHON_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION,
            export_id: export_id.into(),
            python_daily_driver_quality_packet_id_ref: self.packet_id.clone(),
            exported_at: exported_at.into(),
            raw_private_material_excluded: true,
            ambient_authority_excluded: true,
            python_daily_driver_quality_packet: self.clone(),
        }
    }
    ...
    fn derived_findings(&self, include_record_fields: bool) -> Vec<ValidationFinding> {
        ...
        if include_record_fields
            && self.record_kind != PYTHON_DAILY_DRIVER_QUALITY_TRUTH_PACKET_RECORD_KIND
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongRecordKind,
                FindingSeverity::Blocker,
                "python daily-driver quality packet has the wrong record kind",
            ));
        }
        if include_record_fields
            && self.schema_version != PYTHON_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongSchemaVersion,
                FindingSeverity::Blocker,
                "python daily-driver quality packet has the wrong schema version",
            ));
        }
        ...
        for projection in &self.consumer_projections {
            if !projection.preserves_truth_for(&self.packet_id) {
                findings.push(ValidationFinding::new(
                    ...
                    FindingSeverity::Blocker,
                    format!(
                        "projection {} does not preserve python daily-driver quality truth",
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct PythonDailyDriverQualityTruthSupportExport {
    /// Stable record kind.
    pub record_kind: String,
    /// Schema version.
    pub schema_version: u32,
    ...
    pub python_daily_driver_quality_packet: PythonDailyDriverQualityTruthPacket,
}

impl PythonDailyDriverQualityTruthSupportExport {
    /// Returns true when the export preserves the same packet id safely.
    pub fn is_export_safe(&self) -> bool {
        self.record_kind == PYTHON_DAILY_DRIVER_QUALITY_TRUTH_SUPPORT_EXPORT_RECORD_KIND
            && self.schema_version == PYTHON_DAILY_DRIVER_QUALITY_TRUTH_SCHEMA_VERSION
            && self.python_daily_driver_quality_packet_id_ref
                == self.python_daily_driver_quality_packet.packet_id
            && self.raw_private_material_excluded
            && self.ambient_authority_excluded
            && self
                .python_daily_driver_quality_packet
                .validate()
                .is_empty()
    }
}

/// Errors emitted when reading the checked-in stable Python daily-driver packet.
#[derive(Debug)]
pub enum PythonDailyDriverQualityTruthArtifactError {
...
impl fmt::Display for PythonDailyDriverQualityTruthArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Packet(error) => write!(
                formatter,
                "python daily-driver quality packet parse failed: {error}"
            ),
            Self::Validation(findings) => {
                ...
                write!(
                    formatter,
                    "python daily-driver quality packet failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for PythonDailyDriverQualityTruthArtifactError {}

/// Returns the checked-in stable Python daily-driver quality truth packet.
///
/// # Errors
///
/// Returns an artifact error if the checked-in packet does not parse or validate.
pub fn current_stable_python_daily_driver_quality_truth_packet(
) -> Result<PythonDailyDriverQualityTruthPacket, PythonDailyDriverQualityTruthArtifactError> {
    let packet: PythonDailyDriverQualityTruthPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../artifacts/language/m4/python_daily_driver_quality_truth_packet.json"
    )))
    .map_err(PythonDailyDriverQualityTruthArtifactError::Packet)?;
    let findings = packet.validate();
    if findings.is_empty() {
        Ok(packet)
    } else {
        Err(PythonDailyDriverQualityTruthArtifactError::Validation(
            findings,
        ))
    }
}
...
mod tests {
    ...
    fn lane_evidence_ref() -> String {
        PYTHON_DAILY_DRIVER_QUALITY_TRUTH_DOC_REF.to_owned()
    }

    fn quality_row(row_id: &str, lane: LanguageLaneClass) -> PythonDailyDriverQualityRow {
        PythonDailyDriverQualityRow {
            ...
            disclosure_ref: Some(format!(
                ...
                PYTHON_DAILY_DRIVER_QUALITY_TRUTH_DOC_REF
            )),
    ...
    fn projection(surface: ConsumerSurface) -> PythonDailyDriverQualityConsumerProjection {
    ...
    fn lane_rows(lane: LanguageLaneClass, prefix: &str) -> Vec<PythonDailyDriverQualityRow> {
    ...
    fn sample_input() -> PythonDailyDriverQualityTruthPacketInput {
        let rows = lane_rows(LanguageLaneClass::PythonDailyDriverLane, "python");
        PythonDailyDriverQualityTruthPacketInput {
            packet_id: "packet:m4:python_daily_driver_quality".to_owned(),
            workflow_or_surface_id: "workflow.language.python_daily_driver_quality".to_owned(),
    ...
    fn closed_tokens_are_pinned() {
        assert_eq!(
            LanguageLaneClass::PythonDailyDriverLane.as_str(),
            "python_daily_driver_lane"
        );
        assert_eq!(
            DailyDriverRowClass::InterpreterSelectionRow.as_str(),
            "interpreter_selection_row"
        );
        assert_eq!(
        ...
        assert_eq!(
            EvidenceClass::InterpreterSelectionEvidence.as_str(),
            "interpreter_selection_evidence"
        );
        assert_eq!(
            EvidenceClass::EnvironmentManagerEvidence.as_str(),
            "environment_manager_evidence"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "interpreter_subset_only"
        );
        assert_eq!(
            KnownLimitClass::EnvironmentManagerSubsetOnly.as_str(),
            "environment_manager_subset_only"
        );
        assert_eq!(
        ...
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenInterpreter.as_str(),
            "auto_narrow_on_unproven_interpreter"
        );
        assert_eq!(
            DowngradeAutomationClass::AutoNarrowOnUnprovenEnvironmentManager.as_str(),
            "auto_narrow_on_unproven_environment_manager"
        );
        assert_eq!(
    ...
    fn baseline_materialization_is_stable() {
        let packet = PythonDailyDriverQualityTruthPacket::materialize(sample_input());
        assert_eq!(
        ...
        assert!(packet
            .support_export(
                "support:m4:python_daily_driver_quality",
