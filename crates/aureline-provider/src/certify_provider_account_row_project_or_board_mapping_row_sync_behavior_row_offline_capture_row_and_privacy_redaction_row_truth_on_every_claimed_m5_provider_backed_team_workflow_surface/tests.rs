//! Tests for the M05-923 provider-account / offline-capture component surface certification
//! capstone.

use super::*;

fn packet() -> ProviderSurfaceCertificationPacket {
    seeded_m5_provider_account_offline_capture_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, PROVIDER_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, PROVIDER_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5ProviderBackedTeamWorkflowCertifiedSurface::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            ProviderCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, ProviderAxisCertificationState::Certified);
...
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, PROVIDER_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(row.canonical_bundle_ref, PROVIDER_CERT_CANONICAL_BUNDLE_REF);
    }
...
fn the_four_spec_auto_narrow_conditions_are_each_certified() {
    // The four spec narrowing conditions: limited effective write scope, stale session,
    // policy-blocked mapping, and local-only capture packet — must each be a certified yellow
    // surface.
    let p = packet();
    let certified: BTreeSet<M5ProviderComponentClaim> = p
        .rows
        .iter()
        .filter(|r| r.derived_status == ProviderSurfaceClaimStatus::Yellow)
        .map(|r| r.certified_claim)
        .collect();
    assert!(certified.contains(&M5ProviderComponentClaim::LimitedScopeProjection));
    assert!(certified.contains(&M5ProviderComponentClaim::StaleSessionProjection));
    assert!(certified.contains(&M5ProviderComponentClaim::PolicyBlockedMapping));
    assert!(certified.contains(&M5ProviderComponentClaim::LocalOnlyPacket));
}

#[test]
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5ProviderBackedTeamWorkflowCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
...
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = ProviderCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), ProviderCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in ProviderCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == ProviderCertificationAxis::CliExport
        );
...
fn provider_claim_ladder_is_strictly_ordered() {
    let ranks: Vec<u8> = M5ProviderComponentClaim::ALL
        .iter()
        .map(|c| c.capability_rank())
        .collect();
    assert_eq!(ranks, vec![5, 4, 3, 2, 1, 0]);
    let tokens: BTreeSet<&str> = M5ProviderComponentClaim::ALL
        .iter()
        .map(|c| c.as_str())
        .collect();
    assert_eq!(tokens.len(), M5ProviderComponentClaim::ALL.len());
}
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5ProviderBackedTeamWorkflowCertifiedSurface::WorkItemDetail)
        .expect("work-item-detail row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ProviderCertificationAxis::Visual {
            outcome.state = ProviderAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("connection state silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, ProviderSurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, ProviderCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5ProviderBackedTeamWorkflowCertifiedSurface::SupportExport)
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ProviderCertificationAxis::DegradedState {
            *outcome = ProviderAxisOutcome {
                axis: ProviderCertificationAxis::DegradedState,
                state: ProviderAxisCertificationState::DisclosedNarrowed,
                parity_note: "redaction boundary lagging".to_owned(),
                narrowing_reason: Some(
                    "the redaction class was not restated for this export".to_owned(),
                ),
                downgrade_trigger: Some(M5ProviderDowngradeTrigger::RedactionClassUnstated),
            };
        }
    }
    // Claim stays ReviewableProjection == certified ReviewableProjection, no auto-narrow.
    assert_eq!(row.derive_status(), ProviderSurfaceClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ProviderCertificationAxis::CliExport {
            outcome.state = ProviderAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5ProviderDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), ProviderSurfaceClaimStatus::Red);
}
...
fn dropped_lineage_blocks_the_surface() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ProviderCertificationViolation::LineageDropped { .. })));
}
...
fn narrowed_row_dropping_lineage_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == ProviderSurfaceClaimStatus::Yellow)
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(ProviderClaimAutoNarrow {
        binding_axis: ProviderCertificationAxis::DegradedState,
        from_claim: M5ProviderComponentClaim::ProviderCommitted,
        to_claim: M5ProviderComponentClaim::ProviderCommitted,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5ProviderComponentClaim::LocalOnlyPacket;
...
fn certified_claim_above_claim_blocks() {
    ...
    row.certified_claim = M5ProviderComponentClaim::ProviderCommitted;
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ProviderCertificationViolation::CertifiedClaimExceedsClaim { .. }
    )));
}
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5ProviderBackedTeamWorkflowCertifiedSurface::ProviderSettings)
        .expect("provider-settings row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = ProviderCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), ProviderSurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ProviderCertificationAxis::CliExport {
            outcome.state = ProviderAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the settings drill".to_owned());
            outcome.downgrade_trigger = Some(M5ProviderDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = ProviderCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), ProviderSurfaceClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "limited scope".to_owned();
    }
    assert_eq!(row.derive_status(), ProviderSurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(ProviderCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5ProviderBackedTeamWorkflowCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, ProviderCertificationViolation::SurfaceCoverageIncomplete)));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the PrivacyRedactionRow family from every row that carries it; coverage must fail
    // (only the support-export row consumes it).
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5ProviderAccountOfflineComponentFamily::PrivacyRedactionRow);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ProviderCertificationViolation::FamilyCoverageIncomplete)));
}

#[test]
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = ProviderSurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ProviderCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ProviderCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ProviderCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ProviderCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != ProviderCertificationAxis::ProviderBoundaryProvenance);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ProviderCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ProviderCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ProviderCertificationViolation::RawProviderMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: ProviderSurfaceCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_provider_account_offline_capture_component_certification_export()
...
/// `GEN_PROVIDER_CERT_ARTIFACTS=1 cargo test -p aureline-provider \
///  certify_provider_account_row...::tests::generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_PROVIDER_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_provider_account_offline_capture_component_certification_packet();
    ...
    let art = Path::new(manifest).join(
        "../../artifacts/release/m5-provider-account-offline-capture-component-certification",
    );
    ...
    let fixtures = Path::new(manifest)
        .join("../../fixtures/ui/m5-provider-account-offline-capture-component-certification");
