...
use aureline_provider::{
    audit_work_item_sync_beta_page, seeded_work_item_sync_beta_page, WorkItemSyncBetaSupportExport,
};
...
fn main() {
    ...
    let mode = match args.first().map(String::as_str) {
        ...
        Some(other) => {
            ...
            eprintln!(
                "usage: aureline_provider_work_item_sync_beta [page|validate|support-export]"
            );
            process::exit(2);
        }
    };
    ...
    match mode {
        ...
        Mode::SupportExport => {
            let export = WorkItemSyncBetaSupportExport::from_page(
                "work-item-sync-beta:support-export:001",
