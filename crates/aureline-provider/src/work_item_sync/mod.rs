...
fn seeded_publish_reviews() -> Vec<PublishReviewRecord> {
    vec![
        ...
        publish_review(
            "work_item_sync:publish_review:open-in-provider-comment",
            "work_items:detail:cached-stale",
            None,
            Some("work_item_sync:comment:queued"),
            Some("work_items:transition_packet:open-in-provider"),
            PublishReviewActionClass::CreateProviderComment,
            PublishReviewSourceClass::LocalDraftAuthored,
            PublishReviewActorScopeClass::HumanActorWritesProvider,
            ...
            vec![
                side_effect(
                    "fanout:open-in-provider-comment:provider",
                    PublishReviewSideEffectClass::ProviderCommentMutationFanout,
                    WorkItemMutationMode::OpenInProvider,
                    Some("providers:browser_handoff:issue:242"),
                    "Provider comment is completed through the typed browser handoff.",
                ),
                side_effect(
                    "fanout:open-in-provider-comment:notification",
                    PublishReviewSideEffectClass::NotificationEmissionFanout,
                    WorkItemMutationMode::OpenInProvider,
                    Some("providers:browser_handoff:issue:242"),
                    "Provider notifications occur only after the browser-mediated completion path.",
                ),
            ],
            None,
            Some("providers:browser_handoff:issue:242"),
            None,
            None,
            "Open-in-provider comment publish-review row preserves the local draft and typed browser handoff continuity.",
        ),
        publish_review(
