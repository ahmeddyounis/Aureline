//! Protected tests binding the typed generated-artifact lineage register to the
...
//! lineage-cell counts, and the packet-freshness counts; the negative cases
...
//! not label its artifact as generated, a held surface without verified
//! downgrade automation, a surface missing a dimension, and a promotion verdict
//! that disagrees with the firing rules all fail validation.

use std::path::{Path, PathBuf};

use aureline_release::ship_generated_artifact_lineage_surfaces_for_scaffolded_ai_generated_notebook_derived_and_preview_derived_outputs::{
    current_generated_artifact_lineage_register, GeneratedArtifactLineageRegister, GeneratorKind,
    LineageDimension, LineageRegisterViolation, LineageState, NarrowingReason,
    GENERATED_ARTIFACT_LINEAGE_RECORD_KIND, GENERATED_ARTIFACT_LINEAGE_SCHEMA_VERSION,
};
...
const CAPTURE_JSON: &str = include_str!(concat!(
    ...
    "/../../artifacts/release/captures/ship_generated_artifact_lineage_surfaces_for_scaffolded_ai_generated_notebook_derived_and_preview_derived_outputs_validation_capture.json"
));

fn register() -> GeneratedArtifactLineageRegister {
    current_generated_artifact_lineage_register()
...
#[test]
fn covers_every_generator_kind_and_dimension() {
    let r = register();
...
fn model_matches_frozen_validation_capture() {
    ...
    assert_eq!(
        summary["entries_traced"].as_u64().unwrap() as usize,
        r.rows_published_stable().len(),
        "capture traced count must match the model"
    );
    assert_eq!(
    ...
    assert_eq!(
        summary["entries_unlabeled"].as_u64().unwrap() as usize,
        computed.entries_unlabeled,
        "capture unlabeled count must match the model"
    );
    assert_eq!(
    ...
    assert_eq!(
        summary["total_lineage_cells"].as_u64().unwrap() as usize,
        computed.total_lineage_cells,
...
fn register_shows_a_surface_on_waiver() {
    ...
    let on_waiver = r
        .rows
        .iter()
        .find(|row| row.lineage_state == LineageState::OnWaiver)
...
#[test]
fn register_narrows_an_unlabeled_generated_artifact() {
    let r = register();
    let unlabeled = r.rows.iter().find(|row| !row.lineage.generated_labeled);
    let unlabeled = unlabeled.expect("the register must show an unlabeled generated artifact");
    assert!(
        !unlabeled.publishes_stable(),
        "an artifact not labeled as generated must not publish a Stable claim"
    );
}
...
fn narrowing_surface_that_does_not_narrow_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            LineageRegisterViolation::PublishedLabelNotNarrowed { .. }
        )),
...
fn backed_surface_with_active_gap_fails() {
    ...
    row.active_narrowing_reasons
        .push(NarrowingReason::LineageDimensionFailed);
    r.summary = r.computed_summary();
...
#[test]
fn backed_surface_without_disclosure_fails() {
    let mut r = register();
    let row = r
        .rows
        .iter_mut()
    ...
    assert!(
        ...
        "a backed surface must label its artifact as generated"
    );
}
...
fn backed_surface_on_a_breached_packet_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, LineageRegisterViolation::HeldOnStalePacket { .. })),
...
fn backed_surface_without_verified_downgrade_automation_fails() {
    ...
    assert!(
        ...
        "a backed surface must ride defined+verified rollback/downgrade automation"
    );
}
...
fn checked_in_fixtures_are_rejected_by_the_model() {
    let fixtures_dir = repo_root().join(
        "fixtures/release/m5/ship_generated_artifact_lineage_surfaces_for_scaffolded_ai_generated_notebook_derived_and_preview_derived_outputs",
    );
    ...
    for case in cases {
        ...
        let candidate: GeneratedArtifactLineageRegister =
