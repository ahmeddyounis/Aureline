//! Protected tests binding the typed M5 dependency/quality parity matrix to the
...
//! that a lane which fails to narrow, a held row with an active gap, a row
//! carried wider than its public claim's ceiling, and a promotion verdict that
//! disagrees with the firing rules all fail validation.

use std::path::{Path, PathBuf};

use aureline_release::freeze_the_m5_dependency_intelligence_package_health_and_code_quality_parity_matrix::{
    current_m5_dependency_quality_parity_matrix, DqGapReason, DqLaneKind, DqParityMatrix,
    DqParityMatrixViolation, DqScorecardState,
...
const CAPTURE_JSON: &str = include_str!(concat!(
    ...
    "/../../artifacts/release/captures/freeze_the_m5_dependency_intelligence_package_health_and_code_quality_parity_matrix_validation_capture.json"
));

fn matrix() -> DqParityMatrix {
    current_m5_dependency_quality_parity_matrix().expect("checked-in matrix parses into the model")
}
...
fn matrix_shows_a_row_on_waiver() {
    ...
    let on_waiver = m
        .rows
        .iter()
        .find(|row| row.scorecard_state == DqScorecardState::OnWaiver)
...
#[test]
fn every_row_names_an_explicit_downgrade_rule() {
    let m = matrix();
    for row in &m.rows {
        assert!(
            !row.compatibility_downgrade_rule.trim().is_empty(),
            "row {} must name an explicit compatibility/downgrade rule",
            row.entry_id
        );
    }
}
...
fn backed_row_on_a_breached_packet_fails() {
    ...
    assert!(
        m.validate()
            .iter()
            .any(|v| matches!(v, DqParityMatrixViolation::HeldOnStalePacket { .. })),
...
fn checked_in_fixtures_are_rejected_by_the_model() {
    let fixtures_dir = repo_root().join(
        "fixtures/release/m5/freeze_the_m5_dependency_intelligence_package_health_and_code_quality_parity_matrix",
    );
