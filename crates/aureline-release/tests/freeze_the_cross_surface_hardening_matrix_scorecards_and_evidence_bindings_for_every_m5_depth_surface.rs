//! Protected tests binding the typed cross-surface hardening register to the
...
use aureline_release::freeze_the_cross_surface_hardening_matrix_scorecards_and_evidence_bindings_for_every_m5_depth_surface::{
    current_m5_cross_surface_register, M5CrossSurfaceRegister, M5CrossSurfaceViolation,
    NarrowingReason, ReadinessDimension, SurfaceKind, SurfaceState,
    M5_CROSS_SURFACE_RECORD_KIND, M5_CROSS_SURFACE_SCHEMA_VERSION,
};
...
const CAPTURE_JSON: &str = include_str!(concat!(
    ...
    "/../../artifacts/release/captures/freeze_the_cross_surface_hardening_matrix_scorecards_and_evidence_bindings_for_every_m5_depth_surface_validation_capture.json"
));

fn register() -> M5CrossSurfaceRegister {
    current_m5_cross_surface_register().expect("checked-in register parses into the model")
}
...
fn checked_in_register_parses_and_validates() {
    let r = register();
    assert_eq!(r.schema_version, M5_CROSS_SURFACE_SCHEMA_VERSION);
    assert_eq!(r.record_kind, M5_CROSS_SURFACE_RECORD_KIND);
...
fn model_matches_frozen_validation_capture() {
    ...
    assert_eq!(
        summary["entries_hardened"].as_u64().unwrap() as usize,
        r.rows_published_stable().len(),
        "capture hardened count must match the model"
    );
    assert_eq!(
...
fn narrowing_surface_that_does_not_narrow_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, M5CrossSurfaceViolation::PublishedLabelNotNarrowed { .. })),
...
fn backed_surface_with_active_gap_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, M5CrossSurfaceViolation::HeldWithActiveGap { .. })),
...
fn backed_surface_without_redaction_disclosure_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            M5CrossSurfaceViolation::HeldWithoutRedactionDisclosure { .. }
        )),
...
fn backed_surface_on_a_breached_packet_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, M5CrossSurfaceViolation::HeldOnStalePacket { .. })),
...
fn backed_surface_without_verified_downgrade_automation_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            M5CrossSurfaceViolation::HeldWithoutDowngradeAutomation { .. }
        )),
...
fn promotion_proceed_while_a_rule_fires_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            M5CrossSurfaceViolation::PromotionDecisionInconsistent { .. }
        )),
...
fn checked_in_fixtures_are_rejected_by_the_model() {
    let fixtures_dir = repo_root().join(
        "fixtures/release/m5/freeze_the_cross_surface_hardening_matrix_scorecards_and_evidence_bindings_for_every_m5_depth_surface",
    );
    ...
    for case in cases {
        ...
        let candidate: M5CrossSurfaceRegister =
