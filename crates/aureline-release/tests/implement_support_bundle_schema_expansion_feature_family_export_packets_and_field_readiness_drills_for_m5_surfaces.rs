//! Protected tests binding the typed field-readiness register to the checked-in
...
use aureline_release::implement_support_bundle_schema_expansion_feature_family_export_packets_and_field_readiness_drills_for_m5_surfaces::{
    current_field_readiness_register, FieldReadinessRegister, FieldReadinessViolation, NarrowingReason,
    ReadinessDimension, SurfaceKind, SurfaceState, FIELD_READINESS_RECORD_KIND,
...
const CAPTURE_JSON: &str = include_str!(concat!(
    ...
    "/../../artifacts/release/captures/implement_support_bundle_schema_expansion_feature_family_export_packets_and_field_readiness_drills_for_m5_surfaces_validation_capture.json"
));

fn register() -> FieldReadinessRegister {
    current_field_readiness_register().expect("checked-in register parses into the model")
}
...
fn checked_in_register_parses_and_validates() {
    let r = register();
    assert_eq!(r.schema_version, FIELD_READINESS_SCHEMA_VERSION);
    assert_eq!(r.record_kind, FIELD_READINESS_RECORD_KIND);
...
fn narrowing_surface_that_does_not_narrow_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, FieldReadinessViolation::PublishedLabelNotNarrowed { .. })),
...
fn backed_surface_with_active_gap_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, FieldReadinessViolation::HeldWithActiveGap { .. })),
...
fn backed_surface_without_redaction_disclosure_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            FieldReadinessViolation::HeldWithoutRedactionDisclosure { .. }
        )),
...
fn backed_surface_on_a_breached_packet_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, FieldReadinessViolation::HeldOnStalePacket { .. })),
...
fn backed_surface_without_verified_downgrade_automation_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            FieldReadinessViolation::HeldWithoutDowngradeAutomation { .. }
        )),
...
fn promotion_proceed_while_a_rule_fires_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            FieldReadinessViolation::PromotionDecisionInconsistent { .. }
        )),
...
fn checked_in_fixtures_are_rejected_by_the_model() {
    ...
    for case in cases {
        ...
        let candidate: FieldReadinessRegister =
