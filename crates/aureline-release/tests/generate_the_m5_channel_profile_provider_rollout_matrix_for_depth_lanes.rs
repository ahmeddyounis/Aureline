//! Protected tests binding the typed M5 channel/profile/provider rollout matrix
//! to the checked-in artifact, the frozen CI validation capture, and the
//! negative fixtures.
//!
//! The positive case is the checked-in matrix; the capture cross-check proves
//! the typed model and the CI gate agree on the rollout verdict, the lane-kind
//! coverage counts, the packet-freshness counts, and the active gap-reason
//! counts; the negative cases mutate a parsed copy and the checked-in fixtures
//! to prove that a tuple which fails to narrow, a held row with an active gap,
//! a row carried wider than its public claim's ceiling, and a rollout verdict
//! that disagrees with the firing rules all fail validation.

use std::path::{Path, PathBuf};

use aureline_release::generate_the_m5_channel_profile_provider_rollout_matrix_for_depth_lanes::{
    current_m5_channel_profile_provider_rollout_matrix, M5ChannelProfileProviderRolloutMatrix,
    M5RolloutMatrixViolation, RolloutState,
...
const CAPTURE_JSON: &str = include_str!(concat!(
    ...
    "/../../artifacts/release/captures/generate_the_m5_channel_profile_provider_rollout_matrix_for_depth_lanes_validation_capture.json"
));

fn matrix() -> M5ChannelProfileProviderRolloutMatrix {
    current_m5_channel_profile_provider_rollout_matrix()
        .expect("checked-in matrix parses into the model")
}
...
fn model_matches_frozen_validation_capture() {
    ...
    let captured_decision = capture["rollout"]["decision"].as_str().unwrap();
    assert_eq!(
        captured_decision,
        m.rollout.decision.as_str(),
        "capture rollout decision must match the model"
    );
...
fn narrowing_row_that_does_not_narrow_fails() {
    ...
    let row = m
        ...
        .expect("matrix has a pending-evidence row under a stable ceiling");
...
fn backed_row_with_active_gap_fails() {
    let mut m = matrix();
    use aureline_release::generate_the_m5_channel_profile_provider_rollout_matrix_for_depth_lanes::RolloutGapReason;
    let row = m
        .rows
        .iter_mut()
...
fn backed_row_on_a_breached_packet_fails() {
    ...
    assert!(
        m.validate()
            .iter()
            .any(|v| matches!(v, M5RolloutMatrixViolation::HeldOnStalePacket { .. })),
        "a backed row may not ride a packet outside its freshness SLO"
    );
}

#[test]
fn rollout_proceed_while_a_rule_fires_fails() {
    let mut m = matrix();
    ...
    assert!(
        ...
        "rollout must not proceed while a blocking rule fires"
    );
}
...
fn checked_in_fixtures_are_rejected_by_the_model() {
    let fixtures_dir = repo_root()
        .join("fixtures/release/m5/generate_the_m5_channel_profile_provider_rollout_matrix_for_depth_lanes");
    ...
    for case in cases {
        ...
        let candidate: M5ChannelProfileProviderRolloutMatrix =
