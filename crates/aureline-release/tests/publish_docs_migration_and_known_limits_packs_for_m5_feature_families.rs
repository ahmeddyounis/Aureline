...
use aureline_release::publish_docs_migration_and_known_limits_packs_for_m5_feature_families::{
    current_publication_pack_register, NarrowingReason, PublicationPackRegister,
    PublicationPackViolation, ReadinessDimension, SurfaceKind, SurfaceState,
...
const CAPTURE_JSON: &str = include_str!(concat!(
    ...
    "/../../artifacts/release/captures/publish_docs_migration_and_known_limits_packs_for_m5_feature_families_validation_capture.json"
));

fn register() -> PublicationPackRegister {
    current_publication_pack_register().expect("checked-in register parses into the model")
}
...
fn checked_in_register_parses_and_validates() {
    let r = register();
    assert_eq!(r.schema_version, PUBLICATION_PACK_SCHEMA_VERSION);
    assert_eq!(r.record_kind, PUBLICATION_PACK_RECORD_KIND);
...
fn narrowing_surface_that_does_not_narrow_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            PublicationPackViolation::PublishedLabelNotNarrowed { .. }
        )),
...
fn backed_surface_with_active_gap_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, PublicationPackViolation::HeldWithActiveGap { .. })),
...
fn backed_surface_without_redaction_disclosure_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            PublicationPackViolation::HeldWithoutRedactionDisclosure { .. }
        )),
...
fn backed_surface_on_a_breached_packet_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, PublicationPackViolation::HeldOnStalePacket { .. })),
...
fn backed_surface_without_verified_downgrade_automation_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            PublicationPackViolation::HeldWithoutDowngradeAutomation { .. }
        )),
...
fn promotion_proceed_while_a_rule_fires_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            PublicationPackViolation::PromotionDecisionInconsistent { .. }
        )),
...
fn checked_in_fixtures_are_rejected_by_the_model() {
    ...
    for case in cases {
        ...
        let candidate: PublicationPackRegister =
