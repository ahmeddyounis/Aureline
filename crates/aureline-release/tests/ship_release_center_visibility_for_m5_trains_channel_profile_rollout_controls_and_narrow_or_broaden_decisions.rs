//! Protected tests binding the typed release-center visibility register to the
...
use aureline_release::ship_release_center_visibility_for_m5_trains_channel_profile_rollout_controls_and_narrow_or_broaden_decisions::{
    current_release_visibility_register, NarrowingReason, ReadinessDimension,
    ReleaseVisibilityRegister, ReleaseVisibilityViolation, SurfaceKind, SurfaceState,
...
const CAPTURE_JSON: &str = include_str!(concat!(
    ...
    "/../../artifacts/release/captures/ship_release_center_visibility_for_m5_trains_channel_profile_rollout_controls_and_narrow_or_broaden_decisions_validation_capture.json"
));

fn register() -> ReleaseVisibilityRegister {
    current_release_visibility_register().expect("checked-in register parses into the model")
}
...
fn checked_in_register_parses_and_validates() {
    let r = register();
    assert_eq!(r.schema_version, RELEASE_VISIBILITY_SCHEMA_VERSION);
    assert_eq!(r.record_kind, RELEASE_VISIBILITY_RECORD_KIND);
...
fn narrowing_surface_that_does_not_narrow_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            ReleaseVisibilityViolation::PublishedLabelNotNarrowed { .. }
        )),
...
fn backed_surface_with_active_gap_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, ReleaseVisibilityViolation::HeldWithActiveGap { .. })),
...
fn backed_surface_without_redaction_disclosure_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            ReleaseVisibilityViolation::HeldWithoutRedactionDisclosure { .. }
        )),
...
fn backed_surface_on_a_breached_packet_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, ReleaseVisibilityViolation::HeldOnStalePacket { .. })),
...
fn backed_surface_without_verified_downgrade_automation_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            ReleaseVisibilityViolation::HeldWithoutDowngradeAutomation { .. }
        )),
...
fn promotion_proceed_while_a_rule_fires_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            ReleaseVisibilityViolation::PromotionDecisionInconsistent { .. }
        )),
...
fn checked_in_fixtures_are_rejected_by_the_model() {
    ...
    for case in cases {
        ...
        let candidate: ReleaseVisibilityRegister =
