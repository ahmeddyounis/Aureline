//! Protected tests binding the typed maintenance-truth register to the checked-in
...
//! maintenance-cell counts, and the packet-freshness counts; the negative cases
...
//! disclose its Help/About truth, a held lane without verified downgrade
...
use aureline_release::add_backport_and_hotfix_rules_proof_freshness_automation_and_help_about_truth_updates_for_m5_lanes::{
    current_maintenance_truth_register, LaneKind, MaintenanceDimension, MaintenanceTruthRegister,
    MaintenanceTruthViolation, NarrowingReason, LaneState,
...
const CAPTURE_JSON: &str = include_str!(concat!(
    ...
    "/../../artifacts/release/captures/add_backport_and_hotfix_rules_proof_freshness_automation_and_help_about_truth_updates_for_m5_lanes_validation_capture.json"
));

fn register() -> MaintenanceTruthRegister {
    current_maintenance_truth_register().expect("checked-in register parses into the model")
}
...
#[test]
fn covers_every_lane_kind_and_dimension() {
    let r = register();
...
fn model_matches_frozen_validation_capture() {
    ...
    assert_eq!(
        summary["entries_truth_undisclosed"].as_u64().unwrap() as usize,
        computed.entries_truth_undisclosed,
        "capture truth-undisclosed count must match the model"
    );
    assert_eq!(
    ...
    assert_eq!(
        summary["total_maintenance_cells"].as_u64().unwrap() as usize,
        computed.total_maintenance_cells,
...
fn register_shows_a_lane_on_waiver() {
    ...
    let on_waiver = r
        .rows
        .iter()
        .find(|row| row.lane_state == LaneState::OnWaiver)
...
#[test]
fn register_narrows_a_lane_without_truth_disclosure() {
    let r = register();
    let undisclosed = r.rows.iter().find(|row| !row.disclosure.truth_disclosed);
    let undisclosed = undisclosed.expect("the register must show a lane without truth disclosure");
    assert!(
        !undisclosed.publishes_stable(),
        "a lane that does not disclose its Help/About truth must not publish a Stable claim"
    );
}
...
fn narrowing_lane_that_does_not_narrow_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            MaintenanceTruthViolation::PublishedLabelNotNarrowed { .. }
        )),
...
fn backed_lane_with_active_gap_fails() {
    ...
    row.active_narrowing_reasons
        .push(NarrowingReason::MaintenanceDimensionFailed);
    r.summary = r.computed_summary();
...
#[test]
fn backed_lane_without_truth_disclosure_fails() {
    let mut r = register();
    let row = r
        .rows
        .iter_mut()
    ...
    assert!(
        ...
        "a backed lane must disclose its Help/About truth"
    );
}
...
fn backed_lane_on_a_breached_packet_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, MaintenanceTruthViolation::HeldOnStalePacket { .. })),
...
fn checked_in_fixtures_are_rejected_by_the_model() {
    let fixtures_dir = repo_root().join(
        "fixtures/release/m5/add_backport_and_hotfix_rules_proof_freshness_automation_and_help_about_truth_updates_for_m5_lanes",
    );
    ...
    for case in cases {
        ...
        let candidate: MaintenanceTruthRegister =
