//! Protected tests binding the typed community locale-pack governance register to
...
//! governance-cell counts, and the packet-freshness counts; the negative cases
...
//! disclose fallback, a held lane without verified downgrade automation, a lane
//! missing a dimension, and a promotion verdict that disagrees with the firing
//! rules all fail validation.

use std::path::{Path, PathBuf};

use aureline_release::add_community_locale_pack_lifecycle_translation_governance_and_parity_audits_for_new_m5_surfaces::{
    current_locale_pack_governance_register, GovernanceDimension, LocalePackGovernanceRegister,
    LocalePackRegisterViolation, NarrowingReason, PackChannel, PackState,
    LOCALE_PACK_GOVERNANCE_RECORD_KIND, LOCALE_PACK_GOVERNANCE_SCHEMA_VERSION,
};
...
const CAPTURE_JSON: &str = include_str!(concat!(
    ...
    "/../../artifacts/release/captures/add_community_locale_pack_lifecycle_translation_governance_and_parity_audits_for_new_m5_surfaces_validation_capture.json"
));

fn register() -> LocalePackGovernanceRegister {
    current_locale_pack_governance_register().expect("checked-in register parses into the model")
}
...
#[test]
fn covers_every_pack_channel_and_dimension() {
    let r = register();
...
fn model_matches_frozen_validation_capture() {
    ...
    assert_eq!(
        summary["entries_fallback_undisclosed"].as_u64().unwrap() as usize,
        computed.entries_fallback_undisclosed,
        "capture fallback-undisclosed count must match the model"
    );
    assert_eq!(
    ...
    assert_eq!(
        summary["total_governance_cells"].as_u64().unwrap() as usize,
        computed.total_governance_cells,
...
fn register_shows_a_lane_on_waiver() {
    ...
    let on_waiver = r
        .rows
        .iter()
        .find(|row| row.pack_state == PackState::OnWaiver)
...
#[test]
fn register_narrows_a_lane_without_fallback_disclosure() {
    let r = register();
    let undisclosed = r.rows.iter().find(|row| !row.governance.fallback_disclosed);
    let undisclosed =
        undisclosed.expect("the register must show a lane without fallback disclosure");
    assert!(
        !undisclosed.publishes_stable(),
        "a lane that does not disclose fallback must not publish a Stable claim"
    );
}
...
fn narrowing_lane_that_does_not_narrow_fails() {
    ...
    assert!(
        r.validate().iter().any(|v| matches!(
            v,
            LocalePackRegisterViolation::PublishedLabelNotNarrowed { .. }
        )),
...
fn backed_lane_with_active_gap_fails() {
    ...
    row.active_narrowing_reasons
        .push(NarrowingReason::GovernanceDimensionFailed);
    r.summary = r.computed_summary();
...
#[test]
fn backed_lane_without_fallback_disclosure_fails() {
    let mut r = register();
    let row = r
        .rows
        .iter_mut()
    ...
    assert!(
        ...
        "a backed lane must disclose untranslated-string fallback"
    );
}
...
fn backed_lane_on_a_breached_packet_fails() {
    ...
    assert!(
        r.validate()
            .iter()
            .any(|v| matches!(v, LocalePackRegisterViolation::HeldOnStalePacket { .. })),
...
fn backed_lane_without_verified_downgrade_automation_fails() {
    ...
    assert!(
        ...
        "a backed lane must ride defined+verified rollback/downgrade automation"
    );
}
...
fn checked_in_fixtures_are_rejected_by_the_model() {
    ...
    for case in cases {
        ...
        let candidate: LocalePackGovernanceRegister =
