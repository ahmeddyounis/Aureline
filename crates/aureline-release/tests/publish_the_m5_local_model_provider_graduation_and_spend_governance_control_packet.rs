//! Protected tests binding the typed M5 control-packet register to the
...
use aureline_release::publish_the_m5_local_model_provider_graduation_and_spend_governance_control_packet::{
    current_m5_control_packet_register, ControlPacketGapReason, ControlPacketLaneState,
    M5ControlPacketLaneKind, M5ControlPacketRegisterViolation, M5ControlPacketRegister,
...
const CAPTURE_JSON: &str = include_str!(concat!(
    ...
    "/../../artifacts/release/captures/publish_the_m5_local_model_provider_graduation_and_spend_governance_control_packet_validation_capture.json"
));

fn register() -> M5ControlPacketRegister {
    current_m5_control_packet_register().expect("checked-in register parses into the model")
}
...
fn register_shows_a_row_on_waiver() {
    ...
    let on_waiver = reg
        .rows
        .iter()
        .find(|row| row.lane_state == ControlPacketLaneState::OnWaiver)
...
fn backed_row_on_a_breached_packet_fails() {
    ...
    assert!(
        reg.validate().iter().any(|v| matches!(
            v,
            M5ControlPacketRegisterViolation::HeldOnStalePacket { .. }
        )),
...
fn checked_in_fixtures_are_rejected_by_the_model() {
    let fixtures_dir = repo_root()
        .join("fixtures/release/m5/publish_the_m5_local_model_provider_graduation_and_spend_governance_control_packet");
    ...
    for case in cases {
        ...
        let candidate: M5ControlPacketRegister =
