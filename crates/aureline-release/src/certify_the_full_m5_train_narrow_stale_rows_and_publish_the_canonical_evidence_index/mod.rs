//! Typed M5 certification-train register binding the feature-family
//! certifications, qualification packets, compatibility reports, and the
//! canonical evidence index across the M5 train to per-surface readiness
//! scorecards, support-posture disclosure, and downgrade automation.
//!
//! This module freezes the canonical control surface for certifying the full M5
//! feature train and publishing the canonical M5 evidence index: the
//! feature-family certifications, the qualification packets, the compatibility
//! reports, and the evidence index that ties them together across the feature
//! families (notebooks, DB/API, profiler, AI, frameworks, companion, sync, and
...
//! the *certification surface* every M5 train row exposes — the feature-family
//! certification, the qualification packet, the compatibility report, and the
//! evidence index. Each [`M5CertTrainSurface`] binds one surface to:
//!
//! - the stable claim it backs ([`M5CertTrainSurface::claim_ref`],
//!   [`M5CertTrainSurface::claim_label`]),
//! - a readiness scorecard ([`M5CertTrainSurface::scorecard`]) of one
//!   [`ReadinessCell`] per [`ReadinessDimension`], so scorecard completeness,
//!   qualification currency, compatibility currency, Help/About truth, proof
//!   freshness, and locale parity are each an explicit, inspectable grade,
//! - the support posture it discloses ([`M5CertTrainSurface::disclosure`]):
...
//!   operator,
//! - an owner manifest ([`M5CertTrainSurface::owner_signoff`]) recording who
//!   signed the claim,
//! - an explicit downgrade/rollback automation record
//!   ([`M5CertTrainSurface::downgrade_automation`]) binding the surface to a
...
//!   ([`M5CertTrainSurface::published_label`]),
...
//! it. The [`M5CertTrainStopRule`] set names the closed conditions that gate
//! promotion — one per [`NarrowingReason`] — and
//! [`M5CertTrainRegister::promotion`] records the proceed/hold verdict.
//!
...
//! train surface without a cargo build in CI.
//!
...
pub const M5_CERT_TRAIN_SCHEMA_VERSION: u32 = 1;

/// Stable record-kind tag for the register.
pub const M5_CERT_TRAIN_RECORD_KIND: &str =
    "certify_the_full_m5_train_narrow_stale_rows_and_publish_the_canonical_evidence_index";

/// Repo-relative path to the checked-in register.
pub const M5_CERT_TRAIN_PATH: &str =
    "artifacts/release/m5/certify_the_full_m5_train_narrow_stale_rows_and_publish_the_canonical_evidence_index.json";

/// Embedded checked-in register JSON.
pub const M5_CERT_TRAIN_JSON: &str = include_str!(concat!(
...
pub enum SurfaceKind {
    /// A certified M5 feature family in the train.
    FeatureFamily,
    /// A qualification packet for an M5 feature family.
    QualificationPacket,
    /// A compatibility report for an M5 feature family.
    CompatibilityReport,
    /// The canonical evidence index tying the train together across families.
    EvidenceIndex,
}
...
impl SurfaceKind {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::QualificationPacket => "qualification_packet",
...
pub enum ReadinessDimension {
    /// The feature scorecard is complete across every claimed dimension.
    ScorecardComplete,
    /// The qualification packet is current and complete.
    QualificationCurrent,
    /// The compatibility report is current for the supported windows.
    CompatibilityCurrent,
    /// Help/About and About-box truth matches the certified claim.
    HelpAboutTruth,
...
impl ReadinessDimension {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::ScorecardComplete => "scorecard_complete",
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct M5CertTrainStopRule {
    /// Stable rule id.
    pub rule_id: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CertTrainExportRow {
    /// Stable surface id.
    pub entry_id: String,
    /// The surface kind this surface governs.
    pub surface_kind: SurfaceKind,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CertTrainExportProjection {
    /// Register identifier.
    ...
    /// Export rows.
    pub rows: Vec<M5CertTrainExportRow>,
}

/// The typed publication-pack register.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct M5CertTrainRegister {
...
impl M5CertTrainRegister {
    ...
    pub fn rows_narrowed(&self) -> Vec<&M5CertTrainSurface> {
        self.rows
            .iter()
            .filter(|row| !row.publishes_stable())
            .collect()
    ...
    pub fn release_blocking_rows(&self) -> Vec<&M5CertTrainSurface> {
        self.rows
            .iter()
    ...
    pub fn rows_for_kind(&self, kind: SurfaceKind) -> Vec<&M5CertTrainSurface> {
        self.rows
            .iter()
    ...
    pub fn stop_rule_fires(&self, rule: &M5CertTrainStopRule) -> bool {
    ...
    pub fn computed_summary(&self) -> M5CertTrainSummary {
        ...
        let release_blocking: Vec<&M5CertTrainSurface> = self.release_blocking_rows();
        M5CertTrainSummary {
            ...
            release_blocking_narrowed: release_blocking
                ...
                .filter(|row| !row.publishes_stable())
                .count(),
            feature_family_entries: kind(SurfaceKind::FeatureFamily),
            qualification_packet_entries: kind(SurfaceKind::QualificationPacket),
            compatibility_report_entries: kind(SurfaceKind::CompatibilityReport),
            evidence_index_entries: kind(SurfaceKind::EvidenceIndex),
    ...
    pub fn support_export_projection(&self) -> M5CertTrainExportProjection {
        M5CertTrainExportProjection {
    ...
    pub fn validate(&self) -> Vec<M5CertTrainViolation> {
        ...
        for row in &self.rows {
            if !seen.insert(row.entry_id.clone()) {
                violations.push(M5CertTrainViolation::DuplicateEntryId {
                    entry_id: row.entry_id.clone(),
                });
            }
        ...
        if self.rows.is_empty() {
            violations.push(M5CertTrainViolation::EmptyRegister);
        }
        ...
        if self.summary != self.computed_summary() {
            violations.push(M5CertTrainViolation::SummaryMismatch);
        }

        violations
    }

    fn validate_envelope(&self, violations: &mut Vec<M5CertTrainViolation>) {
        if self.schema_version != M5_CERT_TRAIN_SCHEMA_VERSION {
            violations.push(M5CertTrainViolation::UnsupportedSchemaVersion {
                actual: self.schema_version,
            });
        }
        if self.record_kind != M5_CERT_TRAIN_RECORD_KIND {
            violations.push(M5CertTrainViolation::UnsupportedRecordKind {
        ...
        for (field, value) in [
            ...
            if value.trim().is_empty() {
                violations.push(M5CertTrainViolation::EmptyField {
        ...
        if self.lifecycle_labels != StableClaimLevel::ALL.to_vec() {
            violations.push(M5CertTrainViolation::ClosedVocabularyMismatch {
    ...
    fn validate_stop_rules(&self, violations: &mut Vec<M5CertTrainViolation>) {
        if self.stop_rules.is_empty() {
            violations.push(M5CertTrainViolation::NoStopRules);
        }
        let mut seen = BTreeSet::new();
        let mut covered = BTreeSet::new();
        for rule in &self.stop_rules {
            if !seen.insert(rule.rule_id.clone()) {
                violations.push(M5CertTrainViolation::DuplicateStopRuleId {
            ...
            if rule.applies_to_labels.is_empty() {
                violations.push(M5CertTrainViolation::StopRuleWithoutLabels {
                    rule_id: rule.rule_id.clone(),
                });
            }
        ...
        for reason in NarrowingReason::ALL {
            if !covered.contains(&reason) {
                violations.push(M5CertTrainViolation::NarrowingReasonWithoutStopRule { reason });
            }
        }
    }

    fn validate_row(&self, row: &M5CertTrainSurface, violations: &mut Vec<M5CertTrainViolation>) {
        for (field, value) in [
        ...
        if row.published_label.rank() > row.claim_label.rank() {
            violations.push(M5CertTrainViolation::PublishedWiderThanClaim {
                entry_id: row.entry_id.clone(),
        ...
        if !row.proof_packet.freshness_slo.window_is_consistent() {
            violations.push(M5CertTrainViolation::FreshnessSloInconsistent {
                entry_id: row.entry_id.clone(),
            });
        }
        ...
        if !row.claim_holds_stable() {
            if row.holds_label() {
                violations.push(M5CertTrainViolation::HeldOnNarrowedClaim {
                    entry_id: row.entry_id.clone(),
                    claim: row.claim_label,
                });
            }
            if row.active_narrowing_reasons.is_empty() {
                violations.push(M5CertTrainViolation::NarrowingWithoutReason {
                    entry_id: row.entry_id.clone(),
        ...
        if row.holds_label() {
            ...
            if row.published_label != row.claim_label {
                violations.push(M5CertTrainViolation::HeldLabelNotEqualClaim {
                    entry_id: row.entry_id.clone(),
                    claim: row.claim_label,
                    published: row.published_label,
                });
            ...
            if !row.active_narrowing_reasons.is_empty() {
                violations.push(M5CertTrainViolation::HeldWithActiveGap {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !row.disclosure.redaction_disclosed {
                violations.push(M5CertTrainViolation::HeldWithoutRedactionDisclosure {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !row.proof_packet.has_capture() {
                violations.push(M5CertTrainViolation::HeldWithoutFreshPacket {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !slo_state.is_within_slo() {
                violations.push(M5CertTrainViolation::HeldOnStalePacket {
                    entry_id: row.entry_id.clone(),
                    slo_state,
                });
            }
            if !(row.owner_signoff.signed_off && row.owner_signoff.signed_at.is_some()) {
                violations.push(M5CertTrainViolation::HeldWithoutSignoff {
                    entry_id: row.entry_id.clone(),
                });
            }
            ...
            {
                violations.push(M5CertTrainViolation::HeldWithoutDowngradeAutomation {
                    entry_id: row.entry_id.clone(),
            ...
            if row.publishes_stable() {
                violations.push(M5CertTrainViolation::PublishedLabelNotNarrowed {
                    entry_id: row.entry_id.clone(),
                    state: row.surface_state,
                    published: row.published_label,
                });
            ...
            {
                violations.push(M5CertTrainViolation::BreachedPacketWithoutReason {
                    entry_id: row.entry_id.clone(),
                });
            }
            ...
            {
                violations.push(M5CertTrainViolation::MissingPacketWithoutReason {
                    entry_id: row.entry_id.clone(),
                });
            }
        }
    ...
    fn validate_scorecard(
        &self,
        row: &M5CertTrainSurface,
        violations: &mut Vec<M5CertTrainViolation>,
    ) {
        let mut seen: BTreeSet<ReadinessDimension> = BTreeSet::new();
        for cell in &row.scorecard {
            if !seen.insert(cell.dimension) {
                violations.push(M5CertTrainViolation::DuplicateDimension {
                    entry_id: row.entry_id.clone(),
            ...
            if cell.grade != DimensionGrade::Missing && cell.evidence_ref.trim().is_empty() {
                violations.push(M5CertTrainViolation::CellEvidenceMissing {
                    entry_id: row.entry_id.clone(),
                    dimension: cell.dimension,
                });
            }
            ...
            if cell.grade == DimensionGrade::Waived && row.waiver.is_none() {
                violations.push(M5CertTrainViolation::WaivedCellWithoutWaiver {
                    entry_id: row.entry_id.clone(),
                    dimension: cell.dimension,
                });
            }
            ...
            if !cell.grade.holds() {
                if let Some(reason) = cell.dimension.reason_for_grade(cell.grade) {
                    if !row.has_active_reason(reason) {
                        violations.push(M5CertTrainViolation::CellReasonNotActive {
                            entry_id: row.entry_id.clone(),
                            dimension: cell.dimension,
                            reason,
                        });
        ...
        for dimension in ReadinessDimension::ALL {
            if !seen.contains(&dimension) {
                violations.push(M5CertTrainViolation::ReadinessIncompleteCoverage {
                    entry_id: row.entry_id.clone(),
                    dimension,
                });
            }
    ...
    fn validate_downgrade_automation(
        ...
        if automation.target_floor.is_at_or_above_cutline() {
            violations.push(M5CertTrainViolation::AutomationFloorNotBelowCutline {
                entry_id: row.entry_id.clone(),
        ...
        {
            violations.push(M5CertTrainViolation::AutomationStateWithoutReason {
                entry_id: row.entry_id.clone(),
        ...
        if !automation.rollback_verified
            ...
            && !row.has_active_reason(NarrowingReason::DowngradeAutomationUndefined)
        {
            violations.push(M5CertTrainViolation::RollbackUnverifiedWithoutReason {
                entry_id: row.entry_id.clone(),
            });
        }
    }
    ...
    fn validate_state_reason_coherence(
        ...
        let push_incoherent = |violations: &mut Vec<M5CertTrainViolation>,
                               expected: NarrowingReason| {
            violations.push(M5CertTrainViolation::StateReasonIncoherent {
                entry_id: row.entry_id.clone(),
                state: row.surface_state,
        ...
        match row.surface_state {
            ...
            SurfaceState::OnWaiver => {
                if row
                    ...
                    .unwrap_or(true)
                {
                    violations.push(M5CertTrainViolation::WaiverStateWithoutWaiver {
                        entry_id: row.entry_id.clone(),
                        state: row.surface_state,
                    });
                }
    ...
    fn validate_coverage(&self, violations: &mut Vec<M5CertTrainViolation>) {
        ...
        for declared in &self.release_blocking_surface_refs {
            if !covered.contains(declared) {
                violations.push(M5CertTrainViolation::ReleaseBlockingSurfaceUncovered {
        ...
        for row in &self.rows {
            ...
            {
                violations.push(M5CertTrainViolation::ReleaseBlockingRowNotDeclared {
                    entry_id: row.entry_id.clone(),
                });
            }
        }
    ...
    fn validate_promotion(&self, violations: &mut Vec<M5CertTrainViolation>) {
        ...
        if self.promotion.decision != computed {
            violations.push(M5CertTrainViolation::PromotionDecisionInconsistent {
                declared: self.promotion.decision,
                computed,
            });
        }
        if self.promotion.blocking_rule_ids != self.computed_blocking_rule_ids() {
            violations.push(M5CertTrainViolation::PromotionBlockingSetMismatch {
...
impl fmt::Display for M5CertTrainViolation {
...
impl Error for M5CertTrainViolation {}
...
/// [`M5CertTrainRegister`].
pub fn current_m5_cert_train_register() -> Result<M5CertTrainRegister, serde_json::Error> {
    serde_json::from_str(M5_CERT_TRAIN_JSON)
}
