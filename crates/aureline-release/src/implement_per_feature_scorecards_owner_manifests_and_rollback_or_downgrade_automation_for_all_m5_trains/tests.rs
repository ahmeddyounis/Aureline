...
fn register() -> TrainScorecardRegister {
    current_m5_train_scorecard_register().expect("register parses")
}
...
#[test]
fn every_scorecard_covers_every_axis() {
    let r = register();
    for row in &r.rows {
...
#[test]
fn every_held_scorecard_has_verified_downgrade_automation() {
    let r = register();
    for row in r.rows_published_stable() {
        assert!(
            row.downgrade_automation.state.holds() && row.downgrade_automation.rollback_verified,
            "held scorecard {} must carry defined+verified downgrade automation",
            row.entry_id
        );
    }
}
...
#[test]
fn summary_counts_match_scorecards() {
    let r = register();
    assert_eq!(r.summary, r.computed_summary());
    assert_eq!(
        r.summary.entries_qualified + r.summary.entries_narrowed,
        r.rows.len()
    );
}
...
#[test]
fn validate_flags_a_held_scorecard_with_active_gap() {
    let mut r = register();
    let row = r
        .rows
        .iter_mut()
        .find(|row| row.publishes_stable())
        .expect("a held scorecard exists");
...
#[test]
fn validate_flags_a_missing_axis_cell() {
    let mut r = register();
    r.rows[0]
        .scorecard
        .retain(|cell| cell.axis != ScorecardAxis::Localization);
...
#[test]
fn validate_flags_a_held_scorecard_without_downgrade_automation() {
    let mut r = register();
    let row = r
        .rows
        .iter_mut()
...
#[test]
fn export_projection_mirrors_scorecards() {
    let r = register();
