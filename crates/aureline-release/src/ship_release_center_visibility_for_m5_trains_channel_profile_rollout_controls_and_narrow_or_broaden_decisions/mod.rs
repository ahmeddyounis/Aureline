//! Typed release-center visibility register binding the M5 release-center view,
//! channel/profile rollout controls, and narrow-or-broaden decisions to
//! per-surface readiness scorecards, rollout-posture disclosure, and downgrade
//! automation.
//!
//! This module freezes the canonical control surface for the M5 release-center
//! visibility lanes: the release-center view of the feature trains, the channel
//! rollout controls, the profile rollout controls, and the narrow-or-broaden
//! decision surface those controls drive. Where the depth-claim manifest speaks
//! for the *depth claim* each feature family publishes and the field-readiness
//! register speaks for the *field-readiness surface* every surface kind exposes,
//! this register speaks for the *release-center visibility surface* every surface
//! kind exposes — the release-center-view surface, the channel-rollout-control
//! surface, the profile-rollout-control surface, and the narrow-or-broaden-decision
//! surface. Each [`ReleaseVisibilitySurface`] binds one surface to:
//!
//! - the stable claim it backs ([`ReleaseVisibilitySurface::claim_ref`],
//!   [`ReleaseVisibilitySurface::claim_label`]),
//! - a readiness scorecard ([`ReleaseVisibilitySurface::scorecard`]) of one
//!   [`ReadinessCell`] per [`ReadinessDimension`], so visibility coverage, rollout
//!   control safety, decision auditability, redaction safety, proof freshness, and
//!   docs truth are each an explicit, inspectable grade,
//! - the rollout posture it discloses ([`ReleaseVisibilitySurface::disclosure`]):
...
//!   operator,
//! - an owner manifest ([`ReleaseVisibilitySurface::owner_signoff`]) recording who
//!   signed the claim,
//! - an explicit downgrade/rollback automation record
//!   ([`ReleaseVisibilitySurface::downgrade_automation`]) binding the surface to a
...
//!   ([`ReleaseVisibilitySurface::published_label`]),
...
//! it. The [`ReleaseVisibilityStopRule`] set names the closed conditions that gate
//! promotion — one per [`NarrowingReason`] — and
//! [`ReleaseVisibilityRegister::promotion`] records the proceed/hold verdict.
//!
//! The register is checked in at
//! `artifacts/release/m5/ship_release_center_visibility_for_m5_trains_channel_profile_rollout_controls_and_narrow_or_broaden_decisions.json`
//! and embedded here, so this typed consumer and the CI gate agree on every
//! release-center visibility surface without a cargo build in CI.
//!
//! The model is metadata-only: every field is a typed state or an opaque ref. It
//! carries no rollout payloads, release-center bodies, signatures, or credential
//! material.
...
pub const RELEASE_VISIBILITY_SCHEMA_VERSION: u32 = 1;

/// Stable record-kind tag for the register.
pub const RELEASE_VISIBILITY_RECORD_KIND: &str =
    "ship_release_center_visibility_for_m5_trains_channel_profile_rollout_controls_and_narrow_or_broaden_decisions";

/// Repo-relative path to the checked-in register.
pub const RELEASE_VISIBILITY_PATH: &str =
    "artifacts/release/m5/ship_release_center_visibility_for_m5_trains_channel_profile_rollout_controls_and_narrow_or_broaden_decisions.json";

/// Embedded checked-in register JSON.
pub const RELEASE_VISIBILITY_JSON: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../artifacts/release/m5/ship_release_center_visibility_for_m5_trains_channel_profile_rollout_controls_and_narrow_or_broaden_decisions.json"
));

/// Surface kind a release-center visibility surface governs.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SurfaceKind {
    /// The release-center view of the M5 feature trains.
    ReleaseCenterView,
    /// A channel rollout-control surface.
    ChannelRolloutControl,
    /// A profile rollout-control surface.
    ProfileRolloutControl,
    /// A narrow-or-broaden decision surface.
    NarrowBroadenDecision,
}

impl SurfaceKind {
    /// Every kind, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::ReleaseCenterView,
        Self::ChannelRolloutControl,
        Self::ProfileRolloutControl,
        Self::NarrowBroadenDecision,
    ];

    /// Stable token recorded in the register.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::ReleaseCenterView => "release_center_view",
            Self::ChannelRolloutControl => "channel_rollout_control",
            Self::ProfileRolloutControl => "profile_rollout_control",
            Self::NarrowBroadenDecision => "narrow_broaden_decision",
        }
    }
}
...
pub enum ReadinessDimension {
    /// The release-center view covers every claimed M5 train.
    VisibilityCoverage,
    /// The channel/profile rollout controls are guarded and reversible.
    RolloutControlSafety,
    /// The narrow-or-broaden decisions are recorded and auditable.
    DecisionAuditability,
    /// Redaction safety: no credential bodies or raw provider payloads leak.
    RedactionSafety,
    /// The proof-freshness automation is wired and exercised.
    ProofFreshness,
    /// The docs/help/About truth matches the shipped behavior.
    DocsTruth,
}
...
impl ReadinessDimension {
    ...
    pub const ALL: [Self; 6] = [
        Self::VisibilityCoverage,
        Self::RolloutControlSafety,
        Self::DecisionAuditability,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::VisibilityCoverage => "visibility_coverage",
            Self::RolloutControlSafety => "rollout_control_safety",
            Self::DecisionAuditability => "decision_auditability",
...
/// Trust tier of the maintainer behind a release-center visibility surface.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TrustTier {
...
/// Overall readiness/lifecycle state a release-center visibility surface earned.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SurfaceState {
    /// Every dimension passes, the rollout posture is disclosed, the owner
...
/// Closed reason a release-center visibility surface claim narrows or a stop rule
/// fires.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// The disclosed rollout posture of a release-center visibility surface.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SupportDisclosure {
    /// Stable ref to the support window the surface commits to.
    pub support_window_ref: String,
    /// Stable ref to the release-center/rollout/decision policy the surface rides.
...
/// One release-center visibility stop rule.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ReleaseVisibilityStopRule {
    /// Stable rule id.
    pub rule_id: String,
...
/// One release-center visibility surface.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ReleaseVisibilitySurface {
    /// Stable surface id.
    pub entry_id: String,
    /// Human-readable title.
    pub title: String,
    ...
    /// The disclosed rollout posture of the surface.
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ReleaseVisibilitySummary {
    /// Total number of release-center visibility surfaces.
    ...
    /// Release-center-view surfaces.
    pub release_center_view_entries: usize,
    /// Channel-rollout-control surfaces.
    pub channel_rollout_control_entries: usize,
    /// Profile-rollout-control surfaces.
    pub profile_rollout_control_entries: usize,
    /// Narrow-or-broaden-decision surfaces.
    pub narrow_broaden_decision_entries: usize,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ReleaseVisibilityExportRow {
    /// Stable surface id.
    pub entry_id: String,
    /// The surface kind this surface governs.
    pub surface_kind: SurfaceKind,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ReleaseVisibilityExportProjection {
    /// Register identifier.
    ...
    /// Export rows.
    pub rows: Vec<ReleaseVisibilityExportRow>,
}

/// The typed release-center visibility register.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ReleaseVisibilityRegister {
    ...
    /// Stop rules.
    pub stop_rules: Vec<ReleaseVisibilityStopRule>,
    /// Release-center visibility surfaces.
    pub rows: Vec<ReleaseVisibilitySurface>,
    /// Recorded promotion verdict.
    pub promotion: PromotionDecisionRecord,
    /// Summary counts.
    pub summary: ReleaseVisibilitySummary,
}

impl ReleaseVisibilityRegister {
    /// Returns the surface registered for `entry_id`.
    pub fn row(&self, entry_id: &str) -> Option<&ReleaseVisibilitySurface> {
        self.rows.iter().find(|row| row.entry_id == entry_id)
    }

    /// Returns the surfaces publishing a label at or above the cutline.
    pub fn rows_published_stable(&self) -> Vec<&ReleaseVisibilitySurface> {
        self.rows
            .iter()
    ...
    pub fn rows_narrowed(&self) -> Vec<&ReleaseVisibilitySurface> {
        self.rows
            .iter()
            .filter(|row| !row.publishes_stable())
            .collect()
    ...
    pub fn release_blocking_rows(&self) -> Vec<&ReleaseVisibilitySurface> {
        self.rows
            .iter()
    ...
    pub fn rows_for_kind(&self, kind: SurfaceKind) -> Vec<&ReleaseVisibilitySurface> {
        self.rows
            .iter()
    ...
    pub fn stop_rule_fires(&self, rule: &ReleaseVisibilityStopRule) -> bool {
    ...
    pub fn computed_summary(&self) -> ReleaseVisibilitySummary {
        ...
        let release_blocking: Vec<&ReleaseVisibilitySurface> = self.release_blocking_rows();
        ReleaseVisibilitySummary {
            ...
            release_blocking_narrowed: release_blocking
                ...
                .filter(|row| !row.publishes_stable())
                .count(),
            release_center_view_entries: kind(SurfaceKind::ReleaseCenterView),
            channel_rollout_control_entries: kind(SurfaceKind::ChannelRolloutControl),
            profile_rollout_control_entries: kind(SurfaceKind::ProfileRolloutControl),
            narrow_broaden_decision_entries: kind(SurfaceKind::NarrowBroadenDecision),
    ...
    pub fn support_export_projection(&self) -> ReleaseVisibilityExportProjection {
        ReleaseVisibilityExportProjection {
            ...
            rows: self
                .rows
                .iter()
                .map(|row| ReleaseVisibilityExportRow {
    ...
    pub fn validate(&self) -> Vec<ReleaseVisibilityViolation> {
        ...
        for row in &self.rows {
            if !seen.insert(row.entry_id.clone()) {
                violations.push(ReleaseVisibilityViolation::DuplicateEntryId {
                    entry_id: row.entry_id.clone(),
                });
            }
        ...
        if self.rows.is_empty() {
            violations.push(ReleaseVisibilityViolation::EmptyRegister);
        }
        ...
        if self.summary != self.computed_summary() {
            violations.push(ReleaseVisibilityViolation::SummaryMismatch);
        }

        violations
    }

    fn validate_envelope(&self, violations: &mut Vec<ReleaseVisibilityViolation>) {
        if self.schema_version != RELEASE_VISIBILITY_SCHEMA_VERSION {
            violations.push(ReleaseVisibilityViolation::UnsupportedSchemaVersion {
                actual: self.schema_version,
            });
        }
        if self.record_kind != RELEASE_VISIBILITY_RECORD_KIND {
            violations.push(ReleaseVisibilityViolation::UnsupportedRecordKind {
        ...
        for (field, value) in [
            ...
            if value.trim().is_empty() {
                violations.push(ReleaseVisibilityViolation::EmptyField {
        ...
        if self.lifecycle_labels != StableClaimLevel::ALL.to_vec() {
            violations.push(ReleaseVisibilityViolation::ClosedVocabularyMismatch {
    ...
    fn validate_stop_rules(&self, violations: &mut Vec<ReleaseVisibilityViolation>) {
        if self.stop_rules.is_empty() {
            violations.push(ReleaseVisibilityViolation::NoStopRules);
        }
        let mut seen = BTreeSet::new();
        let mut covered = BTreeSet::new();
        for rule in &self.stop_rules {
            if !seen.insert(rule.rule_id.clone()) {
                violations.push(ReleaseVisibilityViolation::DuplicateStopRuleId {
            ...
            if rule.applies_to_labels.is_empty() {
                violations.push(ReleaseVisibilityViolation::StopRuleWithoutLabels {
                    rule_id: rule.rule_id.clone(),
                });
            }
        ...
        for reason in NarrowingReason::ALL {
            if !covered.contains(&reason) {
                violations
                    .push(ReleaseVisibilityViolation::NarrowingReasonWithoutStopRule { reason });
            }
        }
    }
    ...
    fn validate_row(
        &self,
        row: &ReleaseVisibilitySurface,
        violations: &mut Vec<ReleaseVisibilityViolation>,
    ) {
        for (field, value) in [
        ...
        if row.published_label.rank() > row.claim_label.rank() {
            violations.push(ReleaseVisibilityViolation::PublishedWiderThanClaim {
                entry_id: row.entry_id.clone(),
        ...
        if !row.proof_packet.freshness_slo.window_is_consistent() {
            violations.push(ReleaseVisibilityViolation::FreshnessSloInconsistent {
                entry_id: row.entry_id.clone(),
            });
        }
        ...
        if !row.claim_holds_stable() {
            if row.holds_label() {
                violations.push(ReleaseVisibilityViolation::HeldOnNarrowedClaim {
                    entry_id: row.entry_id.clone(),
                    claim: row.claim_label,
                });
            }
            if row.active_narrowing_reasons.is_empty() {
                violations.push(ReleaseVisibilityViolation::NarrowingWithoutReason {
                    entry_id: row.entry_id.clone(),
        ...
        if row.holds_label() {
            ...
            if row.published_label != row.claim_label {
                violations.push(ReleaseVisibilityViolation::HeldLabelNotEqualClaim {
                    entry_id: row.entry_id.clone(),
                    claim: row.claim_label,
                    published: row.published_label,
                });
            ...
            if !row.active_narrowing_reasons.is_empty() {
                violations.push(ReleaseVisibilityViolation::HeldWithActiveGap {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !row.disclosure.redaction_disclosed {
                violations.push(ReleaseVisibilityViolation::HeldWithoutRedactionDisclosure {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !row.proof_packet.has_capture() {
                violations.push(ReleaseVisibilityViolation::HeldWithoutFreshPacket {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !slo_state.is_within_slo() {
                violations.push(ReleaseVisibilityViolation::HeldOnStalePacket {
                    entry_id: row.entry_id.clone(),
                    slo_state,
                });
            }
            if !(row.owner_signoff.signed_off && row.owner_signoff.signed_at.is_some()) {
                violations.push(ReleaseVisibilityViolation::HeldWithoutSignoff {
                    entry_id: row.entry_id.clone(),
                });
            }
            ...
            {
                violations.push(ReleaseVisibilityViolation::HeldWithoutDowngradeAutomation {
                    entry_id: row.entry_id.clone(),
            ...
            if row.publishes_stable() {
                violations.push(ReleaseVisibilityViolation::PublishedLabelNotNarrowed {
                    entry_id: row.entry_id.clone(),
                    state: row.surface_state,
                    published: row.published_label,
                });
            ...
            {
                violations.push(ReleaseVisibilityViolation::BreachedPacketWithoutReason {
                    entry_id: row.entry_id.clone(),
                });
            }
            ...
            {
                violations.push(ReleaseVisibilityViolation::MissingPacketWithoutReason {
                    entry_id: row.entry_id.clone(),
                });
            }
        }
    ...
    fn validate_scorecard(
        ...
        for cell in &row.scorecard {
            if !seen.insert(cell.dimension) {
                violations.push(ReleaseVisibilityViolation::DuplicateDimension {
                    entry_id: row.entry_id.clone(),
            ...
            if cell.grade != DimensionGrade::Missing && cell.evidence_ref.trim().is_empty() {
                violations.push(ReleaseVisibilityViolation::CellEvidenceMissing {
                    entry_id: row.entry_id.clone(),
                    dimension: cell.dimension,
                });
            }
            ...
            if cell.grade == DimensionGrade::Waived && row.waiver.is_none() {
                violations.push(ReleaseVisibilityViolation::WaivedCellWithoutWaiver {
                    entry_id: row.entry_id.clone(),
                    dimension: cell.dimension,
                });
            }
            ...
            if !cell.grade.holds() {
                if let Some(reason) = cell.dimension.reason_for_grade(cell.grade) {
                    if !row.has_active_reason(reason) {
                        violations.push(ReleaseVisibilityViolation::CellReasonNotActive {
                            entry_id: row.entry_id.clone(),
                            dimension: cell.dimension,
                            reason,
                        });
        ...
        for dimension in ReadinessDimension::ALL {
            if !seen.contains(&dimension) {
                violations.push(ReleaseVisibilityViolation::ReadinessIncompleteCoverage {
                    entry_id: row.entry_id.clone(),
                    dimension,
                });
            }
    ...
    fn validate_downgrade_automation(
        ...
        if automation.target_floor.is_at_or_above_cutline() {
            violations.push(ReleaseVisibilityViolation::AutomationFloorNotBelowCutline {
                entry_id: row.entry_id.clone(),
        ...
        {
            violations.push(ReleaseVisibilityViolation::AutomationStateWithoutReason {
                entry_id: row.entry_id.clone(),
        ...
        {
            violations.push(
                ReleaseVisibilityViolation::RollbackUnverifiedWithoutReason {
                    entry_id: row.entry_id.clone(),
                },
            );
        }
    ...
    fn validate_state_reason_coherence(
        ...
        let push_incoherent = |violations: &mut Vec<ReleaseVisibilityViolation>,
                               expected: NarrowingReason| {
            violations.push(ReleaseVisibilityViolation::StateReasonIncoherent {
                entry_id: row.entry_id.clone(),
                state: row.surface_state,
        ...
        match row.surface_state {
            ...
            SurfaceState::OnWaiver => {
                if row
                    ...
                    .unwrap_or(true)
                {
                    violations.push(ReleaseVisibilityViolation::WaiverStateWithoutWaiver {
                        entry_id: row.entry_id.clone(),
                        state: row.surface_state,
                    });
                }
    ...
    fn validate_coverage(&self, violations: &mut Vec<ReleaseVisibilityViolation>) {
        ...
        for declared in &self.release_blocking_surface_refs {
            if !covered.contains(declared) {
                violations.push(
                    ReleaseVisibilityViolation::ReleaseBlockingSurfaceUncovered {
        ...
        for row in &self.rows {
            ...
            {
                violations.push(ReleaseVisibilityViolation::ReleaseBlockingRowNotDeclared {
                    entry_id: row.entry_id.clone(),
                });
            }
        }
    ...
    fn validate_promotion(&self, violations: &mut Vec<ReleaseVisibilityViolation>) {
        ...
        if self.promotion.decision != computed {
            violations.push(ReleaseVisibilityViolation::PromotionDecisionInconsistent {
                declared: self.promotion.decision,
                computed,
            });
        }
        if self.promotion.blocking_rule_ids != self.computed_blocking_rule_ids() {
            violations.push(ReleaseVisibilityViolation::PromotionBlockingSetMismatch {
...
/// A validation violation for the release-center visibility register.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ReleaseVisibilityViolation {
...
impl fmt::Display for ReleaseVisibilityViolation {
...
impl Error for ReleaseVisibilityViolation {}

/// Loads the embedded release-center visibility register.
///
/// # Errors
///
/// Returns a JSON parse error when the checked-in register no longer matches
/// [`ReleaseVisibilityRegister`].
pub fn current_release_visibility_register() -> Result<ReleaseVisibilityRegister, serde_json::Error>
{
    serde_json::from_str(RELEASE_VISIBILITY_JSON)
}
