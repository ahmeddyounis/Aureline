...
fn register() -> ReleaseVisibilityRegister {
    current_release_visibility_register().expect("register parses")
}
...
fn validate_flags_a_missing_dimension_cell() {
    ...
    r.rows[0]
        .scorecard
        .retain(|cell| cell.dimension != ReadinessDimension::DocsTruth);
    assert!(r.validate().iter().any(|v| matches!(
        v,
        ReleaseVisibilityViolation::ReadinessIncompleteCoverage { .. }
    )));
}
