//! Typed field-readiness register binding the M5 support-bundle schema
//! expansion, feature-family export packets, and field-readiness drills to
//! per-surface readiness scorecards, support-posture disclosure, and downgrade
//! automation.
//!
//! This module freezes the canonical control surface for the M5 field-readiness
//! lanes: the expanded support-bundle schema, the per-feature-family export
//! packets, the field-readiness drills, and the operator escalation runbook those
//! surfaces lean on. Where the depth-claim manifest speaks for the *depth claim*
//! each feature family publishes and the maintenance-truth register speaks for the
//! *maintenance-truth lane* every lane kind exposes, this register speaks for the
//! *field-readiness surface* every surface kind exposes — the support-bundle-schema
//! surface, the feature-family-export surface, the field-readiness-drill surface,
//! and the escalation-runbook surface. Each [`FieldReadinessSurface`] binds one
//! surface to:
//!
//! - the stable claim it backs ([`FieldReadinessSurface::claim_ref`],
//!   [`FieldReadinessSurface::claim_label`]),
//! - a readiness scorecard ([`FieldReadinessSurface::scorecard`]) of one
//!   [`ReadinessCell`] per [`ReadinessDimension`], so schema coverage, export
//!   completeness, drill execution, redaction safety, proof freshness, and docs
//!   truth are each an explicit, inspectable grade,
//! - the support posture it discloses ([`FieldReadinessSurface::disclosure`]): the
//!   support window, the policy ref, the maintainer [`TrustTier`], the scope refs,
//!   and whether the redaction/provenance posture is disclosed to the operator,
//! - an owner manifest ([`FieldReadinessSurface::owner_signoff`]) recording who
//!   signed the claim,
//! - an explicit downgrade/rollback automation record
//!   ([`FieldReadinessSurface::downgrade_automation`]) binding the surface to a
...
//!   ([`FieldReadinessSurface::published_label`]),
...
//! it. The [`FieldReadinessStopRule`] set names the closed conditions that gate
//! promotion — one per [`NarrowingReason`] — and
//! [`FieldReadinessRegister::promotion`] records the proceed/hold verdict.
//!
//! The register is checked in at
//! `artifacts/release/m5/implement_support_bundle_schema_expansion_feature_family_export_packets_and_field_readiness_drills_for_m5_surfaces.json`
//! and embedded here, so this typed consumer and the CI gate agree on every
//! field-readiness surface without a cargo build in CI.
//!
//! The model is metadata-only: every field is a typed state or an opaque ref. It
//! carries no support-bundle bodies, export payloads, signatures, or credential
//! material.
...
pub const FIELD_READINESS_SCHEMA_VERSION: u32 = 1;

/// Stable record-kind tag for the register.
pub const FIELD_READINESS_RECORD_KIND: &str =
    "implement_support_bundle_schema_expansion_feature_family_export_packets_and_field_readiness_drills_for_m5_surfaces";

/// Repo-relative path to the checked-in register.
pub const FIELD_READINESS_PATH: &str =
    "artifacts/release/m5/implement_support_bundle_schema_expansion_feature_family_export_packets_and_field_readiness_drills_for_m5_surfaces.json";

/// Embedded checked-in register JSON.
pub const FIELD_READINESS_JSON: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../artifacts/release/m5/implement_support_bundle_schema_expansion_feature_family_export_packets_and_field_readiness_drills_for_m5_surfaces.json"
));

/// Surface kind a field-readiness surface governs.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SurfaceKind {
    /// The support-bundle schema expansion.
    SupportBundleSchema,
    /// A feature-family export packet surface.
    FeatureFamilyExport,
    /// A field-readiness drill surface.
    FieldReadinessDrill,
    /// An operator escalation runbook surface.
    EscalationRunbook,
}
...
impl SurfaceKind {
    ...
    pub const ALL: [Self; 4] = [
        Self::SupportBundleSchema,
        Self::FeatureFamilyExport,
        Self::FieldReadinessDrill,
        Self::EscalationRunbook,
    ];

    /// Stable token recorded in the register.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::SupportBundleSchema => "support_bundle_schema",
            Self::FeatureFamilyExport => "feature_family_export",
            Self::FieldReadinessDrill => "field_readiness_drill",
            Self::EscalationRunbook => "escalation_runbook",
        }
    }
}
...
pub enum ReadinessDimension {
    /// The support-bundle schema covers the surface's fields.
    SchemaCoverage,
    /// The feature-family export packet is complete.
    ExportCompleteness,
    /// The field-readiness drill was executed and captured.
    DrillExecution,
...
impl ReadinessDimension {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::SchemaCoverage => "schema_coverage",
            Self::ExportCompleteness => "export_completeness",
            Self::DrillExecution => "drill_execution",
...
/// Trust tier of the maintainer behind a field-readiness surface.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TrustTier {
...
/// Overall readiness/lifecycle state a field-readiness surface earned.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// Closed reason a field-readiness surface claim narrows or a stop rule fires.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// The disclosed support posture of a field-readiness surface.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SupportDisclosure {
    /// Stable ref to the support window the surface commits to.
    pub support_window_ref: String,
    /// Stable ref to the support-bundle/export/drill policy the surface rides.
...
/// One field-readiness stop rule.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct FieldReadinessStopRule {
    /// Stable rule id.
    pub rule_id: String,
...
/// One field-readiness surface.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct FieldReadinessSurface {
    /// Stable surface id.
    pub entry_id: String,
    /// Human-readable title.
    pub title: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct FieldReadinessSummary {
    /// Total number of field-readiness surfaces.
    ...
    /// Support-bundle-schema surfaces.
    pub support_bundle_schema_entries: usize,
    /// Feature-family-export surfaces.
    pub feature_family_export_entries: usize,
    /// Field-readiness-drill surfaces.
    pub field_readiness_drill_entries: usize,
    /// Escalation-runbook surfaces.
    pub escalation_runbook_entries: usize,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FieldReadinessExportRow {
    /// Stable surface id.
    pub entry_id: String,
    /// The surface kind this surface governs.
    pub surface_kind: SurfaceKind,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FieldReadinessExportProjection {
    /// Register identifier.
    ...
    /// Export rows.
    pub rows: Vec<FieldReadinessExportRow>,
}

/// The typed field-readiness register.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct FieldReadinessRegister {
    ...
    /// Stop rules.
    pub stop_rules: Vec<FieldReadinessStopRule>,
    /// Field-readiness surfaces.
    pub rows: Vec<FieldReadinessSurface>,
    /// Recorded promotion verdict.
    pub promotion: PromotionDecisionRecord,
    /// Summary counts.
    pub summary: FieldReadinessSummary,
}

impl FieldReadinessRegister {
    /// Returns the surface registered for `entry_id`.
    pub fn row(&self, entry_id: &str) -> Option<&FieldReadinessSurface> {
        self.rows.iter().find(|row| row.entry_id == entry_id)
    }

    /// Returns the surfaces publishing a label at or above the cutline.
    pub fn rows_published_stable(&self) -> Vec<&FieldReadinessSurface> {
        self.rows
            .iter()
    ...
    pub fn rows_narrowed(&self) -> Vec<&FieldReadinessSurface> {
        self.rows
            .iter()
            .filter(|row| !row.publishes_stable())
            .collect()
    ...
    pub fn release_blocking_rows(&self) -> Vec<&FieldReadinessSurface> {
        self.rows
            .iter()
    ...
    pub fn rows_for_kind(&self, kind: SurfaceKind) -> Vec<&FieldReadinessSurface> {
        self.rows
            .iter()
    ...
    pub fn stop_rule_fires(&self, rule: &FieldReadinessStopRule) -> bool {
    ...
    pub fn computed_summary(&self) -> FieldReadinessSummary {
        ...
        let release_blocking: Vec<&FieldReadinessSurface> = self.release_blocking_rows();
        FieldReadinessSummary {
            ...
            release_blocking_narrowed: release_blocking
                ...
                .filter(|row| !row.publishes_stable())
                .count(),
            support_bundle_schema_entries: kind(SurfaceKind::SupportBundleSchema),
            feature_family_export_entries: kind(SurfaceKind::FeatureFamilyExport),
            field_readiness_drill_entries: kind(SurfaceKind::FieldReadinessDrill),
            escalation_runbook_entries: kind(SurfaceKind::EscalationRunbook),
    ...
    pub fn support_export_projection(&self) -> FieldReadinessExportProjection {
        FieldReadinessExportProjection {
            ...
            rows: self
                .rows
                .iter()
                .map(|row| FieldReadinessExportRow {
    ...
    pub fn validate(&self) -> Vec<FieldReadinessViolation> {
        ...
        for row in &self.rows {
            if !seen.insert(row.entry_id.clone()) {
                violations.push(FieldReadinessViolation::DuplicateEntryId {
                    entry_id: row.entry_id.clone(),
                });
            }
        ...
        if self.rows.is_empty() {
            violations.push(FieldReadinessViolation::EmptyRegister);
        }
        ...
        if self.summary != self.computed_summary() {
            violations.push(FieldReadinessViolation::SummaryMismatch);
        }

        violations
    }

    fn validate_envelope(&self, violations: &mut Vec<FieldReadinessViolation>) {
        if self.schema_version != FIELD_READINESS_SCHEMA_VERSION {
            violations.push(FieldReadinessViolation::UnsupportedSchemaVersion {
                actual: self.schema_version,
            });
        }
        if self.record_kind != FIELD_READINESS_RECORD_KIND {
            violations.push(FieldReadinessViolation::UnsupportedRecordKind {
        ...
        for (field, value) in [
            ...
            if value.trim().is_empty() {
                violations.push(FieldReadinessViolation::EmptyField {
        ...
        if self.lifecycle_labels != StableClaimLevel::ALL.to_vec() {
            violations.push(FieldReadinessViolation::ClosedVocabularyMismatch {
    ...
    fn validate_stop_rules(&self, violations: &mut Vec<FieldReadinessViolation>) {
        if self.stop_rules.is_empty() {
            violations.push(FieldReadinessViolation::NoStopRules);
        }
        let mut seen = BTreeSet::new();
        let mut covered = BTreeSet::new();
        for rule in &self.stop_rules {
            if !seen.insert(rule.rule_id.clone()) {
                violations.push(FieldReadinessViolation::DuplicateStopRuleId {
            ...
            if rule.applies_to_labels.is_empty() {
                violations.push(FieldReadinessViolation::StopRuleWithoutLabels {
                    rule_id: rule.rule_id.clone(),
                });
            }
        ...
        for reason in NarrowingReason::ALL {
            if !covered.contains(&reason) {
                violations.push(FieldReadinessViolation::NarrowingReasonWithoutStopRule { reason });
            }
        }
    }
    ...
    fn validate_row(
        &self,
        row: &FieldReadinessSurface,
        violations: &mut Vec<FieldReadinessViolation>,
    ) {
        for (field, value) in [
        ...
        if row.published_label.rank() > row.claim_label.rank() {
            violations.push(FieldReadinessViolation::PublishedWiderThanClaim {
                entry_id: row.entry_id.clone(),
        ...
        if !row.proof_packet.freshness_slo.window_is_consistent() {
            violations.push(FieldReadinessViolation::FreshnessSloInconsistent {
                entry_id: row.entry_id.clone(),
            });
        }
        ...
        if !row.claim_holds_stable() {
            if row.holds_label() {
                violations.push(FieldReadinessViolation::HeldOnNarrowedClaim {
                    entry_id: row.entry_id.clone(),
                    claim: row.claim_label,
                });
            }
            if row.active_narrowing_reasons.is_empty() {
                violations.push(FieldReadinessViolation::NarrowingWithoutReason {
                    entry_id: row.entry_id.clone(),
        ...
        if row.holds_label() {
            ...
            if row.published_label != row.claim_label {
                violations.push(FieldReadinessViolation::HeldLabelNotEqualClaim {
                    entry_id: row.entry_id.clone(),
                    claim: row.claim_label,
                    published: row.published_label,
                });
            ...
            if !row.active_narrowing_reasons.is_empty() {
                violations.push(FieldReadinessViolation::HeldWithActiveGap {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !row.disclosure.redaction_disclosed {
                violations.push(FieldReadinessViolation::HeldWithoutRedactionDisclosure {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !row.proof_packet.has_capture() {
                violations.push(FieldReadinessViolation::HeldWithoutFreshPacket {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !slo_state.is_within_slo() {
                violations.push(FieldReadinessViolation::HeldOnStalePacket {
                    entry_id: row.entry_id.clone(),
                    slo_state,
                });
            }
            if !(row.owner_signoff.signed_off && row.owner_signoff.signed_at.is_some()) {
                violations.push(FieldReadinessViolation::HeldWithoutSignoff {
                    entry_id: row.entry_id.clone(),
                });
            }
            ...
            {
                violations.push(FieldReadinessViolation::HeldWithoutDowngradeAutomation {
                    entry_id: row.entry_id.clone(),
            ...
            if row.publishes_stable() {
                violations.push(FieldReadinessViolation::PublishedLabelNotNarrowed {
                    entry_id: row.entry_id.clone(),
                    state: row.surface_state,
                    published: row.published_label,
                });
            ...
            {
                violations.push(FieldReadinessViolation::BreachedPacketWithoutReason {
                    entry_id: row.entry_id.clone(),
                });
            }
            ...
            {
                violations.push(FieldReadinessViolation::MissingPacketWithoutReason {
                    entry_id: row.entry_id.clone(),
                });
            }
        }
    ...
    fn validate_scorecard(
        ...
        for cell in &row.scorecard {
            if !seen.insert(cell.dimension) {
                violations.push(FieldReadinessViolation::DuplicateDimension {
                    entry_id: row.entry_id.clone(),
            ...
            if cell.grade != DimensionGrade::Missing && cell.evidence_ref.trim().is_empty() {
                violations.push(FieldReadinessViolation::CellEvidenceMissing {
                    entry_id: row.entry_id.clone(),
                    dimension: cell.dimension,
                });
            }
            ...
            if cell.grade == DimensionGrade::Waived && row.waiver.is_none() {
                violations.push(FieldReadinessViolation::WaivedCellWithoutWaiver {
                    entry_id: row.entry_id.clone(),
                    dimension: cell.dimension,
                });
            }
            ...
            if !cell.grade.holds() {
                if let Some(reason) = cell.dimension.reason_for_grade(cell.grade) {
                    if !row.has_active_reason(reason) {
                        violations.push(FieldReadinessViolation::CellReasonNotActive {
                            entry_id: row.entry_id.clone(),
                            dimension: cell.dimension,
                            reason,
                        });
        ...
        for dimension in ReadinessDimension::ALL {
            if !seen.contains(&dimension) {
                violations.push(FieldReadinessViolation::ReadinessIncompleteCoverage {
                    entry_id: row.entry_id.clone(),
                    dimension,
                });
            }
    ...
    fn validate_downgrade_automation(
        ...
        if automation.target_floor.is_at_or_above_cutline() {
            violations.push(FieldReadinessViolation::AutomationFloorNotBelowCutline {
                entry_id: row.entry_id.clone(),
        ...
        {
            violations.push(FieldReadinessViolation::AutomationStateWithoutReason {
                entry_id: row.entry_id.clone(),
        ...
        if !automation.rollback_verified
            ...
            && !row.has_active_reason(NarrowingReason::DowngradeAutomationUndefined)
        {
            violations.push(FieldReadinessViolation::RollbackUnverifiedWithoutReason {
                entry_id: row.entry_id.clone(),
            });
        }
    }
    ...
    fn validate_state_reason_coherence(
        ...
        let push_incoherent = |violations: &mut Vec<FieldReadinessViolation>,
                               expected: NarrowingReason| {
            violations.push(FieldReadinessViolation::StateReasonIncoherent {
                entry_id: row.entry_id.clone(),
                state: row.surface_state,
        ...
        match row.surface_state {
            ...
            SurfaceState::OnWaiver => {
                if row
                    ...
                    .unwrap_or(true)
                {
                    violations.push(FieldReadinessViolation::WaiverStateWithoutWaiver {
                        entry_id: row.entry_id.clone(),
                        state: row.surface_state,
                    });
                }
    ...
    fn validate_coverage(&self, violations: &mut Vec<FieldReadinessViolation>) {
        ...
        for declared in &self.release_blocking_surface_refs {
            if !covered.contains(declared) {
                violations.push(FieldReadinessViolation::ReleaseBlockingSurfaceUncovered {
        ...
        for row in &self.rows {
            ...
            {
                violations.push(FieldReadinessViolation::ReleaseBlockingRowNotDeclared {
                    entry_id: row.entry_id.clone(),
                });
            }
        }
    ...
    fn validate_promotion(&self, violations: &mut Vec<FieldReadinessViolation>) {
        ...
        if self.promotion.decision != computed {
            violations.push(FieldReadinessViolation::PromotionDecisionInconsistent {
                declared: self.promotion.decision,
                computed,
            });
        }
        if self.promotion.blocking_rule_ids != self.computed_blocking_rule_ids() {
            violations.push(FieldReadinessViolation::PromotionBlockingSetMismatch {
...
/// A validation violation for the field-readiness register.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum FieldReadinessViolation {
...
impl fmt::Display for FieldReadinessViolation {
...
impl Error for FieldReadinessViolation {}

/// Loads the embedded field-readiness register.
///
/// # Errors
///
/// Returns a JSON parse error when the checked-in register no longer matches
/// [`FieldReadinessRegister`].
pub fn current_field_readiness_register() -> Result<FieldReadinessRegister, serde_json::Error> {
    serde_json::from_str(FIELD_READINESS_JSON)
}
