...
fn register() -> PublicationPackRegister {
    current_publication_pack_register().expect("register parses")
}
...
fn validate_flags_a_missing_dimension_cell() {
    ...
    r.rows[0]
        .scorecard
        .retain(|cell| cell.dimension != ReadinessDimension::LocaleParity);
    assert!(r.validate().iter().any(|v| matches!(
        v,
        PublicationPackViolation::ReadinessIncompleteCoverage { .. }
    )));
}
