...
fn register() -> FeatureTrainCompatibilityRegister {
    current_feature_train_compatibility_register().expect("register parses")
}
...
#[test]
fn every_held_lane_discloses_eol_and_verified_freeze() {
    let r = register();
    for row in r.rows_published_stable() {
        assert!(
            row.support_window.eol_disclosed,
            "held lane {} must disclose its end-of-support window",
            row.entry_id
        );
        assert!(
            row.change_freeze.state.holds() && row.change_freeze.freeze_verified,
            "held lane {} must carry defined+verified change-freeze guidance",
            row.entry_id
        );
    }
}
...
fn validate_flags_a_held_lane_with_active_gap() {
    ...
    row.active_narrowing_reasons
        .push(NarrowingReason::FreezePlanUnverified);
...
#[test]
fn validate_flags_a_held_lane_without_eol_disclosure() {
    let mut r = register();
    let row = r
        .rows
        .iter_mut()
...
fn validate_flags_a_missing_dimension_cell() {
    ...
    r.rows[0]
        .scorecard
        .retain(|cell| cell.dimension != CompatibilityDimension::ChangeFreezeAdherence);
    assert!(r.validate().iter().any(|v| matches!(
        v,
...
#[test]
fn validate_flags_a_held_lane_without_change_freeze() {
    let mut r = register();
    let row = r
        .rows
        .iter_mut()
...
fn export_projection_mirrors_lanes() {
    ...
    for (row, proj) in r.rows.iter().zip(&projection.rows) {
        ...
        assert_eq!(row.support_window.trust_tier, proj.trust_tier);
        assert_eq!(row.support_window.eol_disclosed, proj.eol_disclosed);
        assert_eq!(row.change_freeze.state, proj.freeze_state);
    }
}
