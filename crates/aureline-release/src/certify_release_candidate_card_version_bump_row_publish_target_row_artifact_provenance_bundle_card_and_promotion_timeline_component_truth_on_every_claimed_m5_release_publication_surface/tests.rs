//! Tests for the M05-867 release-publication component surface certification
//! capstone.

use super::*;

fn packet() -> PublicationSurfaceCertificationPacket {
    seeded_m5_release_publication_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, PUBLICATION_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, PUBLICATION_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5ReleasePublicationCertifiedSurface::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            PublicationCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, PublicationAxisCertificationState::Certified);
...
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5ReleasePublicationCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
...
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = PublicationCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), PublicationCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in PublicationCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == PublicationCertificationAxis::CliExport
        );
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5ReleasePublicationCertifiedSurface::ReleaseCenter)
        .expect("release-center row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == PublicationCertificationAxis::Visual {
            outcome.state = PublicationAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason =
                Some("candidate blocker freshness silently stale".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, PublicationSurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, PublicationCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5ReleasePublicationCertifiedSurface::AboutHelp)
        .expect("about-help row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == PublicationCertificationAxis::DegradedState {
            *outcome = PublicationAxisOutcome {
                axis: PublicationCertificationAxis::DegradedState,
                state: PublicationAxisCertificationState::DisclosedNarrowed,
                parity_note: "provenance freshness lagging".to_owned(),
                narrowing_reason: Some("provenance evidence is partial for this build".to_owned()),
                downgrade_trigger: Some(M5ReleaseCenterDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays Supported == certified Supported, no claim_auto_narrow.
    assert_eq!(row.derive_status(), PublicationSurfaceClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == PublicationCertificationAxis::CliExport {
            outcome.state = PublicationAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5ReleaseCenterDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), PublicationSurfaceClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(PublicationClaimAutoNarrow {
        binding_axis: PublicationCertificationAxis::DegradedState,
        from_claim: M5PublicationSupportClaim::Certified,
        to_claim: M5PublicationSupportClaim::Certified,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5PublicationSupportClaim::Degraded;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("about-help row exists");
    let row = &mut p.rows[idx];
    row.certified_claim = M5PublicationSupportClaim::Certified; // claimed is Supported
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5ReleasePublicationCertifiedSurface::UpdateCenter)
        .expect("update-center row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = PublicationCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), PublicationSurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == PublicationCertificationAxis::CliExport {
            outcome.state = PublicationAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the update view".to_owned());
            outcome.downgrade_trigger = Some(M5ReleaseCenterDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = PublicationCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), PublicationSurfaceClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == PublicationSurfaceClaimStatus::Yellow)
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(PublicationCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5ReleasePublicationCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        PublicationCertificationViolation::SurfaceCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the version-bump family from every row that carries it; coverage must
    // fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5ReleaseCenterComponentFamily::VersionBumpRow);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        PublicationCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = PublicationSurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        PublicationCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, PublicationCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, PublicationCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != PublicationCertificationAxis::RollbackRevocation);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        PublicationCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, PublicationCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        PublicationCertificationViolation::RawReleaseMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: PublicationSurfaceCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_release_publication_component_certification_export().expect("export is valid");
    assert_eq!(
...
///  certify_release_candidate_card...::tests::generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_PUBLICATION_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_release_publication_component_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-release-publication-component-certification-proof");
    ...
    let fixtures = Path::new(manifest)
        .join("../../fixtures/ui/m5-release-publication-component-certification");
    ...
    fs::write(
        ...
        "# M5 release-publication component surface certification fixtures\n\n\
         Mirror of `artifacts/release/m5-release-publication-component-certification-proof/`.\n\
         Regenerate with `GEN_PUBLICATION_CERT_ARTIFACTS=1 cargo test -p aureline-release \
         certify_release_candidate_card...::tests::generate_artifacts`.\n",
    )
