//! Tests for the M05-947 badge-family surface certification capstone.

use super::*;

fn packet() -> BadgeSurfaceCertificationPacket {
    seeded_m5_badge_family_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, BADGE_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, BADGE_CERT_SCHEMA_VERSION);
}
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(row.axis_outcomes.len(), BadgeCertificationAxis::ALL.len());
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, BadgeAxisCertificationState::Certified);
...
#[test]
fn every_row_preserves_badge_meaning() {
    let p = packet();
    for row in &p.rows {
        assert!(
            row.preserves_badge_meaning(),
            "row {} dropped badge meaning",
            row.row_id
        );
    }
    assert!(p.summary.all_rows_preserve_badge_meaning);
}

#[test]
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, BADGE_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(row.canonical_bundle_ref, BADGE_CERT_CANONICAL_BUNDLE_REF);
    }
...
/// AC2 / track theme: badge axes are certified independently, so a Certified support
/// class on one surface never forces evidence freshness to read as fresh elsewhere. The
/// evidence-freshness family appears on both a green surface (marketplace) and a yellow
/// surface (diagnostics), proving no cross-axis implication.
#[test]
fn certified_support_class_never_implies_fresh_across_surfaces() {
    let p = packet();
    let marketplace = p
        .rows
        .iter()
        .find(|r| r.surface == M5BadgeCertifiedSurface::Marketplace)
        .expect("marketplace row");
    assert_eq!(marketplace.derived_status, BadgeSurfaceClaimStatus::Green);
    assert!(marketplace
        .consumed_families
        .contains(&M5BadgeFamily::SupportClass));

    // A surface carrying the freshness axis can narrow independently while the
    // support-class surface stays green — no axis implies another.
    let narrowed_freshness_surface = p.rows.iter().any(|r| {
        r.derived_status == BadgeSurfaceClaimStatus::Yellow
            && r.consumed_families
                .contains(&M5BadgeFamily::EvidenceFreshness)
    });
    assert!(
        narrowed_freshness_surface,
        "expected a freshness-bearing surface to narrow independently of the certified support class"
    );
}

#[test]
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5BadgeCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5BadgeCertifiedSurface::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = BadgeCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), BadgeCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in BadgeCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == BadgeCertificationAxis::CliExport
        );
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5BadgeCertifiedSurface::Marketplace)
        .expect("marketplace row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BadgeCertificationAxis::Visual {
            outcome.state = BadgeAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("support-class value silently unstated".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, BadgeSurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, BadgeCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5BadgeCertifiedSurface::HelpAbout)
        .expect("help-about row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BadgeCertificationAxis::DegradedState {
            *outcome = BadgeAxisOutcome {
                axis: BadgeCertificationAxis::DegradedState,
                state: BadgeAxisCertificationState::DisclosedNarrowed,
                parity_note: "lifecycle freshness lagging".to_owned(),
                narrowing_reason: Some("lifecycle evidence is partial for this build".to_owned()),
                downgrade_trigger: Some(M5BadgeDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays Supported == certified Supported, no claim_auto_narrow.
    assert_eq!(row.derive_status(), BadgeSurfaceClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BadgeCertificationAxis::CliExport {
            outcome.state = BadgeAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5BadgeDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), BadgeSurfaceClaimStatus::Red);
}
...
#[test]
fn dropped_badge_meaning_blocks_the_surface() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.badge_meaning_preserved = false;
    assert_eq!(row.derive_status(), BadgeSurfaceClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, BadgeCertificationViolation::BadgeMeaningDropped { .. })));
    assert!(!p.summary.all_rows_preserve_badge_meaning);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(BadgeClaimAutoNarrow {
        binding_axis: BadgeCertificationAxis::DegradedState,
        from_claim: M5BadgeSupportClaim::FullClaim,
        to_claim: M5BadgeSupportClaim::FullClaim,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5BadgeSupportClaim::Limited;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("help-about row exists");
    let row = &mut p.rows[idx];
    row.certified_claim = M5BadgeSupportClaim::FullClaim; // claimed is Supported
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        BadgeCertificationViolation::CertifiedClaimExceedsClaim { .. }
    )));
}
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5BadgeCertifiedSurface::Onboarding)
        .expect("onboarding row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = BadgeCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), BadgeSurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BadgeCertificationAxis::CliExport {
            outcome.state = BadgeAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the onboarding view".to_owned());
            outcome.downgrade_trigger = Some(M5BadgeDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = BadgeCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), BadgeSurfaceClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == BadgeSurfaceClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "limited".to_owned();
    }
    assert_eq!(row.derive_status(), BadgeSurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(BadgeCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5BadgeCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, BadgeCertificationViolation::SurfaceCoverageIncomplete)));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the deployment-scope family from every row that carries it; coverage must
    // fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5BadgeFamily::DeploymentScope);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BadgeCertificationViolation::FamilyCoverageIncomplete)));
}

#[test]
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = BadgeSurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, BadgeCertificationViolation::StatusDerivationStale { .. })));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        BadgeCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BadgeCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BadgeCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != BadgeCertificationAxis::AxisSeparation);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        BadgeCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BadgeCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BadgeCertificationViolation::RawBadgeMaterialInExport)));
}
...
fn export_json_round_trips() {
    ...
    let back: BadgeSurfaceCertificationPacket = serde_json::from_str(&json).expect("round trips");
    assert_eq!(p, back);
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_badge_family_certification_export().expect("export is valid");
    assert_eq!(
...
/// `GEN_BADGE_CERT_ARTIFACTS=1 cargo test -p aureline-release \
///  certify_support_class_evidence_freshness...::tests::generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_BADGE_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_badge_family_certification_packet();
    ...
    let art =
        Path::new(manifest).join("../../artifacts/release/m5-badge-family-certification-proof");
    ...
    let fixtures = Path::new(manifest).join("../../fixtures/ui/m5-badge-family-certification");
    ...
    fs::write(
        ...
        "# M5 badge-family surface certification fixtures\n\n\
         Mirror of `artifacts/release/m5-badge-family-certification-proof/`.\n\
         Regenerate with `GEN_BADGE_CERT_ARTIFACTS=1 cargo test -p aureline-release \
         certify_support_class_evidence_freshness...::tests::generate_artifacts`.\n",
    )
