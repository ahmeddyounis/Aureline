//! Typed M5 cross-surface hardening register binding every M5 depth surface —
//! notebooks, data/API, profiler, docs/browser, pipeline, preview,
//! framework-packs, companion, sync, and offboarding — to one readiness
//! scorecard of learnability, visual-system parity, durable-attention, and
//! embedded-boundary obligations, the disclosed support posture, an owner
//! manifest, and downgrade automation.
//!
//! This module freezes the canonical cross-surface hardening control plane for
//! M5: it maps every marketed M5 depth family to its onboarding or migration
//! packet, its design-token / component-state parity audit, its durable-attention
//! obligations, its embedded-boundary audit, its exact-target reopen expectations,
//! and its qualification packet, so no notebook, data/API, profiler, docs/browser,
//! pipeline, preview, framework-pack, companion, sync, or offboarding surface can
//! quietly broaden while learnability, token/state, durable-attention, or
//! embedded-boundary debt is outstanding. Where the depth-claim manifest speaks
//! for the *depth claim* each feature family publishes and the certification train
//! speaks for the *certification surface* each train row exposes, this register
//! speaks for the *cross-surface hardening posture* every M5 depth surface
//! exposes. Each [`M5CrossSurfaceRow`] binds one surface to:
//!
//! - the stable claim it backs ([`M5CrossSurfaceRow::claim_ref`],
//!   [`M5CrossSurfaceRow::claim_label`]),
//! - a readiness scorecard ([`M5CrossSurfaceRow::scorecard`]) of one
//!   [`ReadinessCell`] per [`ReadinessDimension`], so onboarding/migration truth,
//!   token/component-state parity, durable-attention semantics, embedded-boundary
//!   integrity, exact-target reopen expectations, and qualification currency are
//!   each an explicit, inspectable grade,
//! - the support posture it discloses ([`M5CrossSurfaceRow::disclosure`]):
...
//!   operator,
//! - an owner manifest ([`M5CrossSurfaceRow::owner_signoff`]) recording who
//!   signed the claim,
//! - an explicit downgrade/rollback automation record
//!   ([`M5CrossSurfaceRow::downgrade_automation`]) binding the surface to a
...
//!   ([`M5CrossSurfaceRow::published_label`]),
...
//! it. The [`M5CrossSurfaceStopRule`] set names the closed conditions that gate
//! promotion — one per [`NarrowingReason`] — and
//! [`M5CrossSurfaceRegister::promotion`] records the proceed/hold verdict.
//!
//! The register is checked in at
//! `artifacts/release/m5/freeze_the_cross_surface_hardening_matrix_scorecards_and_evidence_bindings_for_every_m5_depth_surface.json`
//! and embedded here, so this typed consumer and the CI gate agree on every M5
//! depth surface without a cargo build in CI. The [`M5CrossSurfaceRegister::support_export_projection`]
//! method is the export consumer release, Help/About, and support surfaces render
//! instead of cloning status text.
//!
//! The model is metadata-only: every field is a typed state or an opaque ref. It
//! carries no surface payloads, rendered bodies, signatures, or credential
//! material.
...
pub const M5_CROSS_SURFACE_SCHEMA_VERSION: u32 = 1;

/// Stable record-kind tag for the register.
pub const M5_CROSS_SURFACE_RECORD_KIND: &str =
    "freeze_the_cross_surface_hardening_matrix_scorecards_and_evidence_bindings_for_every_m5_depth_surface";

/// Repo-relative path to the checked-in register.
pub const M5_CROSS_SURFACE_PATH: &str =
    "artifacts/release/m5/freeze_the_cross_surface_hardening_matrix_scorecards_and_evidence_bindings_for_every_m5_depth_surface.json";

/// Embedded checked-in register JSON.
pub const M5_CROSS_SURFACE_JSON: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../artifacts/release/m5/freeze_the_cross_surface_hardening_matrix_scorecards_and_evidence_bindings_for_every_m5_depth_surface.json"
));

/// M5 depth-surface family a cross-surface hardening row governs.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SurfaceKind {
    /// The notebook authoring/review depth surface.
    Notebook,
    /// The data/API explorer depth surface.
    DataApi,
    /// The profiler / trace-replay depth surface.
    Profiler,
    /// The docs/browser semantic-recall depth surface.
    DocsBrowser,
    /// The pipeline / merge-queue depth surface.
    Pipeline,
    /// The preview / designer-publish depth surface.
    Preview,
    /// The framework-pack / template-registry depth surface.
    FrameworkPack,
    /// The browser/mobile companion depth surface.
    Companion,
    /// The cross-device sync depth surface.
    Sync,
    /// The storage / export / offboarding depth surface.
    Offboarding,
}
...
/// One dimension of the cross-surface hardening readiness scorecard.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ReadinessDimension {
    /// Onboarding or migration truth is current: the surface teaches its setup
    /// with no hidden setup work.
    OnboardingTruth,
    /// Design-token and component-state parity holds: no theme-only state meaning.
    TokenStateParity,
    /// Durable-attention semantics hold: no toast-only long-running truth.
    DurableAttention,
    /// The embedded-boundary audit passes: no embedded high-risk approval.
    EmbeddedBoundary,
    /// Exact-target reopen expectations hold: the surface reopens to its precise
    /// prior target.
    ReopenExactTarget,
    /// The qualification packet is current and complete.
...
impl ReadinessDimension {
    ...
    pub const ALL: [Self; 6] = [
        Self::OnboardingTruth,
        Self::TokenStateParity,
        Self::DurableAttention,
        Self::EmbeddedBoundary,
        Self::ReopenExactTarget,
        Self::QualificationCurrent,
    ];

    /// Stable token recorded in the register.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::OnboardingTruth => "onboarding_truth",
            Self::TokenStateParity => "token_state_parity",
            Self::DurableAttention => "durable_attention",
            Self::EmbeddedBoundary => "embedded_boundary",
            Self::ReopenExactTarget => "reopen_exact_target",
            Self::QualificationCurrent => "qualification_current",
        }
    }
...
/// Trust tier of the maintainer behind a depth surface.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TrustTier {
...
/// Overall readiness/lifecycle state a depth surface earned.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
impl SurfaceState {
    ...
    pub const fn holds_label(self) -> bool {
        matches!(self, Self::Hardened | Self::OnWaiver)
    }
...
/// Closed reason a depth surface claim narrows or a stop rule fires.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// The disclosed support posture of a depth surface.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SupportDisclosure {
    /// Stable ref to the support window the surface commits to.
    pub support_window_ref: String,
    /// Stable ref to the hardening/qualification publication policy the surface rides.
...
/// One cross-surface hardening stop rule.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct M5CrossSurfaceStopRule {
    /// Stable rule id.
    pub rule_id: String,
...
/// One M5 depth surface in the cross-surface hardening matrix.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct M5CrossSurfaceRow {
    /// Stable surface id.
    pub entry_id: String,
    /// Human-readable title.
    pub title: String,
    /// The M5 depth-surface family this row governs.
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct M5CrossSurfaceSummary {
    /// Total number of depth surfaces.
    pub total_entries: usize,
    /// Distinct claims covered.
    pub total_claims: usize,
    /// Surfaces publishing a label at or above the cutline.
    pub entries_hardened: usize,
    ...
    pub release_blocking_hardened: usize,
    /// Release-blocking surfaces narrowed below the cutline.
    pub release_blocking_narrowed: usize,
    /// Notebook surfaces.
    pub notebook_entries: usize,
    /// Data/API surfaces.
    pub data_api_entries: usize,
    /// Profiler surfaces.
    pub profiler_entries: usize,
    /// Docs/browser surfaces.
    pub docs_browser_entries: usize,
    /// Pipeline surfaces.
    pub pipeline_entries: usize,
    /// Preview surfaces.
    pub preview_entries: usize,
    /// Framework-pack surfaces.
    pub framework_pack_entries: usize,
    /// Companion surfaces.
    pub companion_entries: usize,
    /// Sync surfaces.
    pub sync_entries: usize,
    /// Offboarding surfaces.
    pub offboarding_entries: usize,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CrossSurfaceExportRow {
    /// Stable surface id.
    pub entry_id: String,
    /// The M5 depth-surface family this row governs.
    pub surface_kind: SurfaceKind,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CrossSurfaceExportProjection {
    /// Register identifier.
    ...
    /// Export rows.
    pub rows: Vec<M5CrossSurfaceExportRow>,
}

/// The typed cross-surface hardening register.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct M5CrossSurfaceRegister {
    ...
    /// Stop rules.
    pub stop_rules: Vec<M5CrossSurfaceStopRule>,
    /// Depth surfaces.
    pub rows: Vec<M5CrossSurfaceRow>,
    /// Recorded promotion verdict.
    pub promotion: PromotionDecisionRecord,
    /// Summary counts.
    pub summary: M5CrossSurfaceSummary,
}

impl M5CrossSurfaceRegister {
    /// Returns the surface registered for `entry_id`.
    pub fn row(&self, entry_id: &str) -> Option<&M5CrossSurfaceRow> {
        self.rows.iter().find(|row| row.entry_id == entry_id)
    }

    /// Returns the surfaces publishing a label at or above the cutline.
    pub fn rows_published_stable(&self) -> Vec<&M5CrossSurfaceRow> {
        self.rows
            .iter()
    ...
    pub fn rows_narrowed(&self) -> Vec<&M5CrossSurfaceRow> {
        self.rows
            .iter()
            .filter(|row| !row.publishes_stable())
            .collect()
    ...
    pub fn release_blocking_rows(&self) -> Vec<&M5CrossSurfaceRow> {
        self.rows
            .iter()
    ...
    pub fn rows_for_kind(&self, kind: SurfaceKind) -> Vec<&M5CrossSurfaceRow> {
        self.rows
            .iter()
    ...
    pub fn stop_rule_fires(&self, rule: &M5CrossSurfaceStopRule) -> bool {
    ...
    pub fn computed_summary(&self) -> M5CrossSurfaceSummary {
        ...
        let release_blocking: Vec<&M5CrossSurfaceRow> = self.release_blocking_rows();
        M5CrossSurfaceSummary {
            total_entries: self.rows.len(),
            total_claims: self.claims().len(),
            entries_hardened: self
                .rows
                .iter()
                .filter(|row| row.publishes_stable())
                .count(),
            ...
            release_blocking_hardened: release_blocking
                .iter()
                .filter(|row| row.publishes_stable())
                .count(),
            release_blocking_narrowed: release_blocking
                ...
                .filter(|row| !row.publishes_stable())
                .count(),
            notebook_entries: kind(SurfaceKind::Notebook),
            data_api_entries: kind(SurfaceKind::DataApi),
            profiler_entries: kind(SurfaceKind::Profiler),
            docs_browser_entries: kind(SurfaceKind::DocsBrowser),
            pipeline_entries: kind(SurfaceKind::Pipeline),
            preview_entries: kind(SurfaceKind::Preview),
            framework_pack_entries: kind(SurfaceKind::FrameworkPack),
            companion_entries: kind(SurfaceKind::Companion),
            sync_entries: kind(SurfaceKind::Sync),
            offboarding_entries: kind(SurfaceKind::Offboarding),
    ...
    pub fn support_export_projection(&self) -> M5CrossSurfaceExportProjection {
        M5CrossSurfaceExportProjection {
            ...
            rows: self
                .rows
                .iter()
                .map(|row| M5CrossSurfaceExportRow {
    ...
    pub fn validate(&self) -> Vec<M5CrossSurfaceViolation> {
        ...
        for row in &self.rows {
            if !seen.insert(row.entry_id.clone()) {
                violations.push(M5CrossSurfaceViolation::DuplicateEntryId {
                    entry_id: row.entry_id.clone(),
                });
            }
        ...
        if self.rows.is_empty() {
            violations.push(M5CrossSurfaceViolation::EmptyRegister);
        }
        ...
        if self.summary != self.computed_summary() {
            violations.push(M5CrossSurfaceViolation::SummaryMismatch);
        }

        violations
    }

    fn validate_envelope(&self, violations: &mut Vec<M5CrossSurfaceViolation>) {
        if self.schema_version != M5_CROSS_SURFACE_SCHEMA_VERSION {
            violations.push(M5CrossSurfaceViolation::UnsupportedSchemaVersion {
                actual: self.schema_version,
            });
        }
        if self.record_kind != M5_CROSS_SURFACE_RECORD_KIND {
            violations.push(M5CrossSurfaceViolation::UnsupportedRecordKind {
        ...
        for (field, value) in [
            ...
            if value.trim().is_empty() {
                violations.push(M5CrossSurfaceViolation::EmptyField {
        ...
        if self.lifecycle_labels != StableClaimLevel::ALL.to_vec() {
            violations.push(M5CrossSurfaceViolation::ClosedVocabularyMismatch {
    ...
    fn validate_stop_rules(&self, violations: &mut Vec<M5CrossSurfaceViolation>) {
        if self.stop_rules.is_empty() {
            violations.push(M5CrossSurfaceViolation::NoStopRules);
        }
        let mut seen = BTreeSet::new();
        let mut covered = BTreeSet::new();
        for rule in &self.stop_rules {
            if !seen.insert(rule.rule_id.clone()) {
                violations.push(M5CrossSurfaceViolation::DuplicateStopRuleId {
            ...
            if rule.applies_to_labels.is_empty() {
                violations.push(M5CrossSurfaceViolation::StopRuleWithoutLabels {
                    rule_id: rule.rule_id.clone(),
                });
            }
        ...
        for reason in NarrowingReason::ALL {
            if !covered.contains(&reason) {
                violations.push(M5CrossSurfaceViolation::NarrowingReasonWithoutStopRule { reason });
            }
        }
    }

    fn validate_row(&self, row: &M5CrossSurfaceRow, violations: &mut Vec<M5CrossSurfaceViolation>) {
        for (field, value) in [
        ...
        if row.published_label.rank() > row.claim_label.rank() {
            violations.push(M5CrossSurfaceViolation::PublishedWiderThanClaim {
                entry_id: row.entry_id.clone(),
        ...
        if !row.proof_packet.freshness_slo.window_is_consistent() {
            violations.push(M5CrossSurfaceViolation::FreshnessSloInconsistent {
                entry_id: row.entry_id.clone(),
            });
        }
        ...
        if !row.claim_holds_stable() {
            if row.holds_label() {
                violations.push(M5CrossSurfaceViolation::HeldOnNarrowedClaim {
                    entry_id: row.entry_id.clone(),
                    claim: row.claim_label,
                });
            }
            if row.active_narrowing_reasons.is_empty() {
                violations.push(M5CrossSurfaceViolation::NarrowingWithoutReason {
                    entry_id: row.entry_id.clone(),
        ...
        if row.holds_label() {
            ...
            if row.published_label != row.claim_label {
                violations.push(M5CrossSurfaceViolation::HeldLabelNotEqualClaim {
                    entry_id: row.entry_id.clone(),
                    claim: row.claim_label,
                    published: row.published_label,
                });
            ...
            if !row.active_narrowing_reasons.is_empty() {
                violations.push(M5CrossSurfaceViolation::HeldWithActiveGap {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !row.disclosure.redaction_disclosed {
                violations.push(M5CrossSurfaceViolation::HeldWithoutRedactionDisclosure {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !row.proof_packet.has_capture() {
                violations.push(M5CrossSurfaceViolation::HeldWithoutFreshPacket {
                    entry_id: row.entry_id.clone(),
                });
            }
            if !slo_state.is_within_slo() {
                violations.push(M5CrossSurfaceViolation::HeldOnStalePacket {
                    entry_id: row.entry_id.clone(),
                    slo_state,
                });
            }
            if !(row.owner_signoff.signed_off && row.owner_signoff.signed_at.is_some()) {
                violations.push(M5CrossSurfaceViolation::HeldWithoutSignoff {
                    entry_id: row.entry_id.clone(),
                });
            }
            ...
            {
                violations.push(M5CrossSurfaceViolation::HeldWithoutDowngradeAutomation {
                    entry_id: row.entry_id.clone(),
            ...
            if row.publishes_stable() {
                violations.push(M5CrossSurfaceViolation::PublishedLabelNotNarrowed {
                    entry_id: row.entry_id.clone(),
                    state: row.surface_state,
                    published: row.published_label,
                });
            ...
            {
                violations.push(M5CrossSurfaceViolation::BreachedPacketWithoutReason {
                    entry_id: row.entry_id.clone(),
                });
            }
            ...
            {
                violations.push(M5CrossSurfaceViolation::MissingPacketWithoutReason {
                    entry_id: row.entry_id.clone(),
                });
            }
        }
    ...
    fn validate_scorecard(
        &self,
        row: &M5CrossSurfaceRow,
        violations: &mut Vec<M5CrossSurfaceViolation>,
    ) {
        let mut seen: BTreeSet<ReadinessDimension> = BTreeSet::new();
        for cell in &row.scorecard {
            if !seen.insert(cell.dimension) {
                violations.push(M5CrossSurfaceViolation::DuplicateDimension {
                    entry_id: row.entry_id.clone(),
            ...
            if cell.grade != DimensionGrade::Missing && cell.evidence_ref.trim().is_empty() {
                violations.push(M5CrossSurfaceViolation::CellEvidenceMissing {
                    entry_id: row.entry_id.clone(),
                    dimension: cell.dimension,
                });
            }
            ...
            if cell.grade == DimensionGrade::Waived && row.waiver.is_none() {
                violations.push(M5CrossSurfaceViolation::WaivedCellWithoutWaiver {
                    entry_id: row.entry_id.clone(),
                    dimension: cell.dimension,
                });
            }
            ...
            if !cell.grade.holds() {
                if let Some(reason) = cell.dimension.reason_for_grade(cell.grade) {
                    if !row.has_active_reason(reason) {
                        violations.push(M5CrossSurfaceViolation::CellReasonNotActive {
                            entry_id: row.entry_id.clone(),
                            dimension: cell.dimension,
                            reason,
                        });
        ...
        for dimension in ReadinessDimension::ALL {
            if !seen.contains(&dimension) {
                violations.push(M5CrossSurfaceViolation::ReadinessIncompleteCoverage {
                    entry_id: row.entry_id.clone(),
                    dimension,
                });
            }
    ...
    fn validate_downgrade_automation(
        ...
        if automation.target_floor.is_at_or_above_cutline() {
            violations.push(M5CrossSurfaceViolation::AutomationFloorNotBelowCutline {
                entry_id: row.entry_id.clone(),
        ...
        {
            violations.push(M5CrossSurfaceViolation::AutomationStateWithoutReason {
                entry_id: row.entry_id.clone(),
        ...
        if !automation.rollback_verified
            ...
            && !row.has_active_reason(NarrowingReason::DowngradeAutomationUndefined)
        {
            violations.push(M5CrossSurfaceViolation::RollbackUnverifiedWithoutReason {
                entry_id: row.entry_id.clone(),
            });
        }
    }
    ...
    fn validate_state_reason_coherence(
        ...
        let push_incoherent = |violations: &mut Vec<M5CrossSurfaceViolation>,
                               expected: NarrowingReason| {
            violations.push(M5CrossSurfaceViolation::StateReasonIncoherent {
                entry_id: row.entry_id.clone(),
                state: row.surface_state,
        ...
        match row.surface_state {
            ...
            SurfaceState::OnWaiver => {
                if row
                    ...
                    .unwrap_or(true)
                {
                    violations.push(M5CrossSurfaceViolation::WaiverStateWithoutWaiver {
                        entry_id: row.entry_id.clone(),
                        state: row.surface_state,
                    });
                }
            }
            SurfaceState::Hardened => {}
        }
    }

    fn validate_coverage(&self, violations: &mut Vec<M5CrossSurfaceViolation>) {
        ...
        for declared in &self.release_blocking_surface_refs {
            if !covered.contains(declared) {
                violations.push(M5CrossSurfaceViolation::ReleaseBlockingSurfaceUncovered {
        ...
        for row in &self.rows {
            ...
            {
                violations.push(M5CrossSurfaceViolation::ReleaseBlockingRowNotDeclared {
                    entry_id: row.entry_id.clone(),
                });
            }
        }
    ...
    fn validate_promotion(&self, violations: &mut Vec<M5CrossSurfaceViolation>) {
        ...
        if self.promotion.decision != computed {
            violations.push(M5CrossSurfaceViolation::PromotionDecisionInconsistent {
                declared: self.promotion.decision,
                computed,
            });
        }
        if self.promotion.blocking_rule_ids != self.computed_blocking_rule_ids() {
            violations.push(M5CrossSurfaceViolation::PromotionBlockingSetMismatch {
...
/// A validation violation for the cross-surface hardening register.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum M5CrossSurfaceViolation {
...
impl fmt::Display for M5CrossSurfaceViolation {
...
impl Error for M5CrossSurfaceViolation {}

/// Loads the embedded cross-surface hardening register.
///
/// # Errors
///
/// Returns a JSON parse error when the checked-in register no longer matches
/// [`M5CrossSurfaceRegister`].
pub fn current_m5_cross_surface_register() -> Result<M5CrossSurfaceRegister, serde_json::Error> {
    serde_json::from_str(M5_CROSS_SURFACE_JSON)
}
