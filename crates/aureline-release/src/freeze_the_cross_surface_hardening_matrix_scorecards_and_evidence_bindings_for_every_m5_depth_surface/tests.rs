...
fn register() -> M5CrossSurfaceRegister {
    current_m5_cross_surface_register().expect("register parses")
}
...
#[test]
fn every_held_surface_discloses_redaction_and_verified_automation() {
    let r = register();
    for row in r.rows_published_stable() {
        assert!(
            row.disclosure.redaction_disclosed,
            "held surface {} must disclose its redaction posture",
            row.entry_id
        );
        assert!(
...
fn summary_counts_match_surfaces() {
    ...
    assert_eq!(
        r.summary.entries_hardened + r.summary.entries_narrowed,
        r.rows.len()
    );
}
...
#[test]
fn validate_flags_a_held_surface_without_redaction_disclosure() {
    let mut r = register();
    let row = r
        .rows
        .iter_mut()
...
fn validate_flags_a_missing_dimension_cell() {
    ...
    r.rows[0]
        .scorecard
        .retain(|cell| cell.dimension != ReadinessDimension::QualificationCurrent);
    assert!(r.validate().iter().any(|v| matches!(
        v,
        M5CrossSurfaceViolation::ReadinessIncompleteCoverage { .. }
    )));
}
...
fn export_projection_mirrors_surfaces() {
    ...
    for (row, proj) in r.rows.iter().zip(&projection.rows) {
        ...
        assert_eq!(row.disclosure.redaction_disclosed, proj.redaction_disclosed);
