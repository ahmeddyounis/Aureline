//! Harden the release artifact graph with one-build identity, provenance, SBOM,
...
//! answer the question this module answers: **for each artifact family in the
//! release graph — one-build identity, provenance, SBOM, notices, attestation,
//! and mirror parity — is the family current, grounded in a signed or governed
//! packet, and narrowed below the cutline the moment its backing thins out?**
//! This module is the **release artifact graph hardening register**. For every
//! artifact family it records one row that binds the family to the
//! [`stable_claim_manifest`](crate::stable_claim_manifest) entry whose lifecycle
//! label it backs, the proof packet that grounds it, and the waiver (if any)
//! holding it provisionally.
//!
//! Each [`ArtifactFamilyRow`] is one `(artifact family, public claim)` binding.
//! It:
//!
//! - names the artifact family it governs ([`ArtifactFamilyRow::family_kind`],
//!   [`ArtifactFamilyRow::subject_ref`], [`ArtifactFamilyRow::subject_summary`])
//!   and whether that family is part of the release-blocking set
//!   ([`ArtifactFamilyRow::release_blocking`]);
//! - pins the proof packet ([`ProofPacket`]) with its packet-freshness SLO and
//!   the evidence refs that ground the family's claim;
//! - names the [`stable_claim_manifest`](crate::stable_claim_manifest) entry
//!   whose public claim it backs ([`ArtifactFamilyRow::claim_ref`]) and the
//!   canonical lifecycle label that entry publishes
//!   ([`ArtifactFamilyRow::claim_label`]);
//! - records the family state earned ([`ArtifactFamilyState`]), the active gap
//!   reasons ([`ArtifactFamilyGapReason`]), and the effective label after
//!   narrowing ([`ArtifactFamilyRow::effective_label`]);
//! - carries an owner sign-off and optional waiver so a provisionally held
//!   family can still block promotion if the waiver expires or sign-off is
//!   missing.
//!
//! The [`ArtifactFamilyRule`] set names the closed conditions that gate
//! publication, and [`HardenReleaseArtifactGraph::publication`] records the
...
//! logical invariants that hold regardless of the clock — narrowing
//! consistency, the no-widening rule, packet/state coherence, owner sign-off on
//! current rows, rule wiring, and the publication verdict.
...
pub const HARDEN_RELEASE_ARTIFACT_GRAPH_SCHEMA_VERSION: u32 = 1;

/// Stable record-kind tag for the artifact.
pub const HARDEN_RELEASE_ARTIFACT_GRAPH_RECORD_KIND: &str = "harden_release_artifact_graph";

/// Repo-relative path to the checked-in artifact.
pub const HARDEN_RELEASE_ARTIFACT_GRAPH_PATH: &str =
    "artifacts/release/harden_the_release_artifact_graph_with_one_build_identity_provenance_sbom_notices_attestation_and_mirror_parity.json";

/// Embedded checked-in artifact JSON.
pub const HARDEN_RELEASE_ARTIFACT_GRAPH_JSON: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../artifacts/release/harden_the_release_artifact_graph_with_one_build_identity_provenance_sbom_notices_attestation_and_mirror_parity.json"
));

/// Artifact family kind governed by this register.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ArtifactFamilyKind {
    /// Exact one-build identity across binaries, docs/help truth, compatibility
    /// packets, symbols, SBOM/attestation data, and rollback manifests.
    OneBuildIdentity,
    /// Build provenance capture and attestation.
    Provenance,
    /// Software bill of materials.
    Sbom,
    /// Third-party and human-readable notices.
    Notices,
    /// Signed attestation produced by the hardened signing boundary.
    Attestation,
...
impl ArtifactFamilyKind {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::OneBuildIdentity => "one_build_identity",
            Self::Provenance => "provenance",
            Self::Sbom => "sbom",
            Self::Notices => "notices",
            Self::Attestation => "attestation",
            Self::MirrorParity => "mirror_parity",
        }
    }
}

/// Family state a row earned for the public claim it backs.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ArtifactFamilyState {
    /// The family is current: a captured, within-SLO proof packet and complete
    /// evidence back the public claim at its full canonical lifecycle label.
    Current,
    /// The family carries the claim's full label only because an active,
...
impl ArtifactFamilyState {
    ...
    /// Whether the state lets the family carry the public claim at its label.
    pub const fn holds_label(self) -> bool {
        matches!(self, Self::Current | Self::OnWaiver)
    }

    /// Whether the state forces the family below the public claim label.
    pub const fn forces_narrowing(self) -> bool {
        !self.holds_label()
    }
}

/// Closed reason a family narrows or a rule fires.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ArtifactFamilyGapReason {
    ...
    EvidenceIncomplete,
    /// The artifact is unverified or incomplete (SBOM placeholder, notice delta
    /// pending, attestation unchecked, provenance gap, mirror parity mismatch).
    ArtifactUnverified,
    /// A waiver the row relied on has expired.
    WaiverExpired,
    /// The required row owner sign-off is missing.
    OwnerSignoffMissing,
    /// The one-build identity does not match across all artifact families.
    OneBuildIdentityMismatch,
}

impl ArtifactFamilyGapReason {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::OneBuildIdentityMismatch => "one_build_identity_mismatch",
        }
    }
}
...
pub enum ArtifactFamilyAction {
    ...
    RefreshPacket,
    /// Recapture the family evidence.
    RecaptureEvidence,
    /// Obtain the required owner sign-off.
    RequestOwnerSignoff,
}

impl ArtifactFamilyAction {
...
/// Publication verdict for the release artifact graph lane.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum PublicationDecision {
    /// The artifact graph line may publish.
    Proceed,
...
/// One artifact family row in the release artifact graph register.
...
pub struct ArtifactFamilyRow {
    ...
    pub family_kind: ArtifactFamilyKind,
    ...
    /// Whether this family is part of the release-blocking set.
    ...
    /// Family state earned.
    pub family_state: ArtifactFamilyState,
    ...
    #[serde(default)]
    pub active_gap_reasons: Vec<ArtifactFamilyGapReason>,
...
impl ArtifactFamilyRow {
    ...
    pub fn has_active_reason(&self, reason: ArtifactFamilyGapReason) -> bool {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ArtifactFamilyRule {
    /// Stable rule id.
    pub rule_id: String,
    /// Human-readable title.
    pub title: String,
    /// The gap reason whose presence on a claimed row fires this rule.
    pub trigger_reason: ArtifactFamilyGapReason,
    /// Family states this rule watches.
    pub applies_to_states: Vec<ArtifactFamilyState>,
    /// Default action prescribed when the rule fires.
    pub default_action: ArtifactFamilyAction,
...
/// The recorded publication verdict for the release artifact graph lane.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HardenReleaseArtifactGraphSummary {
    /// Total number of artifact family rows.
...
/// The typed release artifact graph artifact.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct HardenReleaseArtifactGraph {
...
impl HardenReleaseArtifactGraph {
    ...
    pub fn computed_blocking_row_ids(&self) -> Vec<String> {
        let blocking_triggers: BTreeSet<ArtifactFamilyGapReason> = self
            .rules
            .iter()
            .filter(|rule| rule.blocks_publication && self.rule_fires(rule))
    ...
    pub fn computed_summary(&self) -> HardenReleaseArtifactGraphSummary {
        HardenReleaseArtifactGraphSummary {
            ...
            rows_on_active_waiver: self
                .rows
                .iter()
                .filter(|r| r.family_state == ArtifactFamilyState::OnWaiver)
                .count(),
    ...
    pub fn support_export_projection(&self) -> HardenReleaseArtifactGraphExportProjection {
        HardenReleaseArtifactGraphExportProjection {
            ...
            rows: self
                .rows
                .iter()
                .map(|row| ArtifactFamilyExportRow {
    ...
    pub fn validate(&self) -> Vec<HardenReleaseArtifactGraphViolation> {
        ...
        for row in &self.rows {
            if !seen.insert(row.entry_id.clone()) {
                violations.push(HardenReleaseArtifactGraphViolation::DuplicateRowId {
        ...
        if self.rows.is_empty() {
            violations.push(HardenReleaseArtifactGraphViolation::EmptyRows);
        }

        // Every family kind must appear at least once.
        let present_kinds: BTreeSet<ArtifactFamilyKind> =
            self.rows.iter().map(|r| r.family_kind).collect();
        for kind in ArtifactFamilyKind::ALL {
            if !present_kinds.contains(&kind) {
                violations.push(HardenReleaseArtifactGraphViolation::FamilyKindMissing { kind });
            }
        }
        ...
        if self.summary != self.computed_summary() {
            violations.push(HardenReleaseArtifactGraphViolation::SummaryMismatch);
        }

        violations
    }

    fn validate_envelope(&self, violations: &mut Vec<HardenReleaseArtifactGraphViolation>) {
        if self.schema_version != HARDEN_RELEASE_ARTIFACT_GRAPH_SCHEMA_VERSION {
            violations.push(
                HardenReleaseArtifactGraphViolation::UnsupportedSchemaVersion {
                    actual: self.schema_version,
                },
            );
        }
        if self.record_kind != HARDEN_RELEASE_ARTIFACT_GRAPH_RECORD_KIND {
            violations.push(HardenReleaseArtifactGraphViolation::UnsupportedRecordKind {
        ...
        for (field, value) in [
            ...
            if value.trim().is_empty() {
                violations.push(HardenReleaseArtifactGraphViolation::EmptyField {
                    id: "<artifact>".to_owned(),
                    field_name: field,
                });
            }
        }
        if self.family_kinds != ArtifactFamilyKind::ALL.to_vec() {
            violations.push(
                HardenReleaseArtifactGraphViolation::ClosedVocabularyMismatch {
                    field: "family_kinds",
                },
            );
        }
        if self.family_states != ArtifactFamilyState::ALL.to_vec() {
            violations.push(
                HardenReleaseArtifactGraphViolation::ClosedVocabularyMismatch {
                    field: "family_states",
                },
        ...
        if self.gap_reasons != ArtifactFamilyGapReason::ALL.to_vec() {
            violations.push(
                HardenReleaseArtifactGraphViolation::ClosedVocabularyMismatch {
                    field: "gap_reasons",
                },
        ...
        if self.family_actions != ArtifactFamilyAction::ALL.to_vec() {
            violations.push(
                HardenReleaseArtifactGraphViolation::ClosedVocabularyMismatch {
    ...
    fn validate_rules(&self, violations: &mut Vec<HardenReleaseArtifactGraphViolation>) {
        if self.rules.is_empty() {
            violations.push(HardenReleaseArtifactGraphViolation::NoRules);
        }
        let mut seen = BTreeSet::new();
        let mut covered = BTreeSet::new();
        for rule in &self.rules {
            if !seen.insert(rule.rule_id.clone()) {
                violations.push(HardenReleaseArtifactGraphViolation::DuplicateRuleId {
            ...
            if rule.applies_to_states.is_empty() {
                violations.push(HardenReleaseArtifactGraphViolation::RuleWithoutStates {
                    rule_id: rule.rule_id.clone(),
                });
            }
        ...
        for reason in ArtifactFamilyGapReason::ALL {
            if !covered.contains(&reason) {
                violations
                    .push(HardenReleaseArtifactGraphViolation::GapReasonWithoutRule { reason });
            }
        }
    }
    ...
    fn validate_row(
        &self,
        row: &ArtifactFamilyRow,
        violations: &mut Vec<HardenReleaseArtifactGraphViolation>,
    ) {
        for (field, value) in [
        ...
        if row.family_state.forces_narrowing() {
            ...
            if row.active_gap_reasons.is_empty() {
                violations.push(
                    HardenReleaseArtifactGraphViolation::NarrowingWithoutReason {
                        row_id: row.entry_id.clone(),
                        state: row.family_state,
                    },
                );
        ...
        if row.family_state.holds_label() {
            if !row.active_gap_reasons.is_empty() {
                violations.push(HardenReleaseArtifactGraphViolation::HeldRowWithActiveGap {
                    row_id: row.entry_id.clone(),
                });
            }
            if !(row.owner_signoff.signed_off && row.owner_signoff.signed_at.is_some()) {
                violations.push(HardenReleaseArtifactGraphViolation::HeldRowWithoutSignoff {
                    row_id: row.entry_id.clone(),
                });
            }
        }
    ...
    fn validate_row_state_reason_coherence(
        ...
        let push_incoherent = |violations: &mut Vec<HardenReleaseArtifactGraphViolation>,
                               expected: ArtifactFamilyGapReason| {
            violations.push(HardenReleaseArtifactGraphViolation::StateReasonIncoherent {
                row_id: row.entry_id.clone(),
                state: row.family_state,
        ...
        match row.family_state {
            ArtifactFamilyState::NarrowedStale => {
                if !row.has_active_reason(ArtifactFamilyGapReason::PacketFreshnessBreached) {
                    push_incoherent(violations, ArtifactFamilyGapReason::PacketFreshnessBreached);
                }
            }
            ArtifactFamilyState::NarrowedMissing => {
                if !row.has_active_reason(ArtifactFamilyGapReason::PacketMissing) {
                    push_incoherent(violations, ArtifactFamilyGapReason::PacketMissing);
                }
            }
            ArtifactFamilyState::NarrowedUnbacked => {
                const ALLOWED: [ArtifactFamilyGapReason; 4] = [
                    ArtifactFamilyGapReason::EvidenceIncomplete,
                    ArtifactFamilyGapReason::ArtifactUnverified,
                    ArtifactFamilyGapReason::OwnerSignoffMissing,
                    ArtifactFamilyGapReason::OneBuildIdentityMismatch,
                ];
                if !ALLOWED.iter().any(|r| row.has_active_reason(*r)) {
                    push_incoherent(violations, ArtifactFamilyGapReason::EvidenceIncomplete);
                }
            }
            ArtifactFamilyState::NarrowedWaiverExpired => {
                if !row.has_active_reason(ArtifactFamilyGapReason::WaiverExpired) {
                    push_incoherent(violations, ArtifactFamilyGapReason::WaiverExpired);
                }
                if row.waiver.is_none() {
                    violations.push(
                        HardenReleaseArtifactGraphViolation::WaiverStateWithoutWaiver {
                            row_id: row.entry_id.clone(),
                            state: row.family_state,
                        },
                    );
            ...
            ArtifactFamilyState::Current => {}
        }
    }

    fn validate_publication(&self, violations: &mut Vec<HardenReleaseArtifactGraphViolation>) {
        ...
        if self.publication.blocking_rule_ids != self.computed_blocking_rule_ids() {
            violations.push(
                HardenReleaseArtifactGraphViolation::PublicationBlockingSetMismatch {
...
/// A redaction-safe export row projected from an artifact family row.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ArtifactFamilyExportRow {
    /// Stable row id.
    pub entry_id: String,
    /// The artifact family kind.
    pub family_kind: ArtifactFamilyKind,
    /// Canonical lifecycle label the claim entry publishes.
    pub claim_label: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct HardenReleaseArtifactGraphExportProjection {
    ...
    /// Projected rows.
    pub rows: Vec<ArtifactFamilyExportRow>,
}

/// A validation violation for the release artifact graph artifact.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum HardenReleaseArtifactGraphViolation {
    ...
    GapReasonWithoutRule {
        /// Uncovered reason.
        reason: ArtifactFamilyGapReason,
    },
    ...
    StateReasonIncoherent {
        ...
        expected_reason: ArtifactFamilyGapReason,
    },
...
impl fmt::Display for HardenReleaseArtifactGraphViolation {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::EmptyRows => write!(f, "artifact has no artifact family rows"),
...
impl Error for HardenReleaseArtifactGraphViolation {}

/// Loads the embedded release artifact graph artifact.
///
/// # Errors
///
/// Returns a JSON parse error when the checked-in artifact no longer matches
/// [`HardenReleaseArtifactGraph`].
pub fn current_harden_release_artifact_graph(
) -> Result<HardenReleaseArtifactGraph, serde_json::Error> {
    serde_json::from_str(HARDEN_RELEASE_ARTIFACT_GRAPH_JSON)
}
...
mod tests {
    ...
    fn artifact() -> HardenReleaseArtifactGraph {
        current_harden_release_artifact_graph().expect("artifact parses")
    }
    ...
    fn every_family_kind_is_present() {
        let artifact = artifact();
        let present: BTreeSet<ArtifactFamilyKind> =
    ...
    fn every_gap_reason_has_a_rule() {
        let artifact = artifact();
        let covered: BTreeSet<ArtifactFamilyGapReason> = artifact
            .rules
            .iter()
            .map(|rule| rule.trigger_reason)
            .collect();
    ...
    fn validate_flags_a_held_row_with_active_gap() {
        ...
        assert!(artifact
            .validate()
            .contains(&HardenReleaseArtifactGraphViolation::HeldRowWithActiveGap { row_id }));
    }
