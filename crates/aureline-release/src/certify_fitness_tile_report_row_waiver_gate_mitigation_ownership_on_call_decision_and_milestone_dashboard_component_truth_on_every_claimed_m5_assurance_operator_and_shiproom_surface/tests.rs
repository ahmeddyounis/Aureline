//! Tests for the M05-1059 governance-dashboard component surface certification
//! capstone.

use super::*;

fn packet() -> GovernanceSurfaceCertificationPacket {
    seeded_m5_governance_dashboard_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, GOVERNANCE_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, GOVERNANCE_CERT_SCHEMA_VERSION);
}
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            GovernanceCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, GovernanceAxisCertificationState::Certified);
...
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, GOVERNANCE_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(
...
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5GovernanceCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5GovernanceCertifiedSurface::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = GovernanceCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), GovernanceCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in GovernanceCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == GovernanceCertificationAxis::CliExport
        );
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5GovernanceCertifiedSurface::AssuranceCenter)
        .expect("assurance-center row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == GovernanceCertificationAxis::Visual {
            outcome.state = GovernanceAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("evidence freshness silently stale".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, GovernanceSurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, GovernanceCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5GovernanceCertifiedSurface::ReleaseCenter)
        .expect("release-center row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == GovernanceCertificationAxis::DegradedState {
            *outcome = GovernanceAxisOutcome {
                axis: GovernanceCertificationAxis::DegradedState,
                state: GovernanceAxisCertificationState::DisclosedNarrowed,
                parity_note: "gate evidence freshness lagging".to_owned(),
                narrowing_reason: Some("gate evidence is partial for this decision".to_owned()),
                downgrade_trigger: Some(M5GovernanceDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays GovernedPass == certified GovernedPass, no claim_auto_narrow.
    assert_eq!(row.derive_status(), GovernanceSurfaceClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == GovernanceCertificationAxis::CliExport {
            outcome.state = GovernanceAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5GovernanceDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), GovernanceSurfaceClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(GovernanceClaimAutoNarrow {
        binding_axis: GovernanceCertificationAxis::DegradedState,
        from_claim: M5GovernanceSupportClaim::GovernedPass,
        to_claim: M5GovernanceSupportClaim::GovernedPass,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5GovernanceSupportClaim::Degraded;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5GovernanceCertifiedSurface::SupportExport)
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    row.certified_claim = M5GovernanceSupportClaim::GovernedPass; // claimed is GovernedResolved
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5GovernanceCertifiedSurface::OperatorOverview)
        .expect("operator-overview row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = GovernanceCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), GovernanceSurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == GovernanceCertificationAxis::CliExport {
            outcome.state = GovernanceAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the operator view".to_owned());
            outcome.downgrade_trigger = Some(M5GovernanceDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = GovernanceCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), GovernanceSurfaceClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == GovernanceSurfaceClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "waived".to_owned();
    }
    assert_eq!(row.derive_status(), GovernanceSurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(GovernanceCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5GovernanceCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        GovernanceCertificationViolation::SurfaceCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the governance-report-row family from every row that carries it;
    // coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5GovernanceDashboardComponentFamily::GovernanceReportRow);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        GovernanceCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = GovernanceSurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        GovernanceCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, GovernanceCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, GovernanceCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != GovernanceCertificationAxis::GovernanceTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        GovernanceCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, GovernanceCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        GovernanceCertificationViolation::RawGovernanceMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: GovernanceSurfaceCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_governance_dashboard_component_certification_export().expect("export is valid");
    assert_eq!(
...
///  certify_fitness_tile...::tests::generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_GOVERNANCE_DASHBOARD_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_governance_dashboard_component_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-governance-dashboard-component-certification-proof");
    ...
    let fixtures = Path::new(manifest)
        .join("../../fixtures/ui/m5-governance-dashboard-component-certification");
    ...
    fs::write(
        ...
        "# M5 governance-dashboard component surface certification fixtures\n\n\
         Mirror of `artifacts/release/m5-governance-dashboard-component-certification-proof/`.\n\
         Regenerate with `GEN_GOVERNANCE_DASHBOARD_CERT_ARTIFACTS=1 cargo test -p aureline-release \
         certify_fitness_tile...::tests::generate_artifacts`.\n",
    )
