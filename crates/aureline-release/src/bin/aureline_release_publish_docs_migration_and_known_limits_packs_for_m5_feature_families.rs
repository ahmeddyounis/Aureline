...
use aureline_release::publish_docs_migration_and_known_limits_packs_for_m5_feature_families::current_publication_pack_register;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        None | Some("register") => {
            let reg = current_publication_pack_register()?;
