//! Headless emitter for the feature-train compatibility register, provider-family
//! support windows, and change-freeze guidance.
//!
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        None | Some("register") => {
            let reg = current_feature_train_compatibility_register()?;
