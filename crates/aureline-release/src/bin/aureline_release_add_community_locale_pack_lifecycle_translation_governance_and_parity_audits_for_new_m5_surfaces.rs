//! Headless emitter for the community locale-pack governance register,
//! translation-governance disclosure, and rollback/downgrade automation.
//!
...
use aureline_release::add_community_locale_pack_lifecycle_translation_governance_and_parity_audits_for_new_m5_surfaces::
    current_locale_pack_governance_register;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        None | Some("register") => {
            let reg = current_locale_pack_governance_register()?;
