//! Headless emitter for the M5 per-train scorecard register, owner manifests, and
//! rollback/downgrade automation.
//!
...
use aureline_release::implement_per_feature_scorecards_owner_manifests_and_rollback_or_downgrade_automation_for_all_m5_trains::
    current_m5_train_scorecard_register;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        None | Some("register") => {
            let reg = current_m5_train_scorecard_register()?;
