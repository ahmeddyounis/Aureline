//! Headless emitter for the M5 publication-pack register, rollout
...
use aureline_release::certify_the_full_m5_train_narrow_stale_rows_and_publish_the_canonical_evidence_index::current_m5_cert_train_register;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        None | Some("register") => {
            let reg = current_m5_cert_train_register()?;
