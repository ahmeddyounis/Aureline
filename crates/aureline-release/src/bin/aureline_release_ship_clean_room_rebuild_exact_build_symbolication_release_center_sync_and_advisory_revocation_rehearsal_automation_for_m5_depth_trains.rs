//! Headless emitter for the M5 rehearsal-automation register.
//!
//! Mints the on-disk fixtures under
//! `fixtures/release/m5/ship_clean_room_rebuild_exact_build_symbolication_release_center_sync_and_advisory_revocation_rehearsal_automation_for_m5_depth_trains/`
//! so the release center, shiproom dashboards, the evidence index, and support
//! export all quote the same record renders as the in-code corpus.
//!
//! Subcommands:
//!
//! ```sh
//! # Print the full register as JSON.
//! cargo run -q -p aureline-release \
//!   --bin aureline_release_ship_clean_room_rebuild_exact -- register
//!
//! # Print the support-export projection as JSON.
//! cargo run -q -p aureline-release \
//!   --bin aureline_release_ship_clean_room_rebuild_exact -- support-export
//!
//! # Print the rehearsal-expiry feed that dashboards and the evidence index ingest.
//! cargo run -q -p aureline-release \
//!   --bin aureline_release_ship_clean_room_rebuild_exact -- expiry-feed
//!
//! # Refresh the on-disk fixtures.
//! cargo run -q -p aureline-release \
//!   --bin aureline_release_ship_clean_room_rebuild_exact -- emit-fixtures \
//!   fixtures/release/m5/ship_clean_room_rebuild_exact_build_symbolication_release_center_sync_and_advisory_revocation_rehearsal_automation_for_m5_depth_trains
//! ```

use std::path::PathBuf;

use aureline_release::ship_clean_room_rebuild_exact_build_symbolication_release_center_sync_and_advisory_revocation_rehearsal_automation_for_m5_depth_trains::
    current_m5_rehearsal_automation_register;

fn main() {
    if let Err(err) = run() {
        eprintln!("{err}");
        std::process::exit(2);
    }
}

fn run() -> Result<(), Box<dyn std::error::Error>> {
    let args: Vec<String> = std::env::args().skip(1).collect();
    match args.first().map(String::as_str) {
        None | Some("register") => {
            let reg = current_m5_rehearsal_automation_register()?;
            println!("{}", serde_json::to_string_pretty(&reg)?);
            Ok(())
        }
        Some("support-export") => {
            let reg = current_m5_rehearsal_automation_register()?;
            let projection = reg.support_export_projection();
            println!("{}", serde_json::to_string_pretty(&projection)?);
            Ok(())
        }
        Some("expiry-feed") => {
            let reg = current_m5_rehearsal_automation_register()?;
            let feed = reg.rehearsal_expiry_feed();
            println!("{}", serde_json::to_string_pretty(&feed)?);
            Ok(())
        }
        Some("validate") => {
            let reg = current_m5_rehearsal_automation_register()?;
            let violations = reg.validate();
            if violations.is_empty() {
                println!("validate: clean (no violations)");
            } else {
                println!("validate: {} violation(s)", violations.len());
                for v in &violations {
                    println!("- {}", v);
                }
                std::process::exit(1);
            }
            Ok(())
        }
        Some("emit-fixtures") => {
            let dir = args
                .get(1)
                .ok_or("emit-fixtures requires a target directory argument")?;
            emit_fixtures(dir)
        }
        Some(cmd) => Err(format!("unknown subcommand: {cmd}").into()),
    }
}

fn emit_fixtures(dir: &str) -> Result<(), Box<dyn std::error::Error>> {
    let base = PathBuf::from(dir);
    std::fs::create_dir_all(&base)?;

    let reg = current_m5_rehearsal_automation_register()?;
    let violations = reg.validate();
    if !violations.is_empty() {
        return Err(format!("register has {} violation(s)", violations.len()).into());
    }

    std::fs::write(
        base.join("register.json"),
        serde_json::to_string_pretty(&reg)?,
    )?;
    std::fs::write(
        base.join("support_export_projection.json"),
        serde_json::to_string_pretty(&reg.support_export_projection())?,
    )?;
    std::fs::write(
        base.join("rehearsal_expiry_feed.json"),
        serde_json::to_string_pretty(&reg.rehearsal_expiry_feed())?,
    )?;

    println!("emitted {} fixtures to {}", 3, base.display());
    Ok(())
}
