//! Headless emitter for the M5 release-center visibility register, rollout
//! posture, and downgrade automation.
//!
...
use aureline_release::ship_release_center_visibility_for_m5_trains_channel_profile_rollout_controls_and_narrow_or_broaden_decisions::
    current_release_visibility_register;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        None | Some("register") => {
            let reg = current_release_visibility_register()?;
