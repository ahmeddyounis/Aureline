//! Headless emitter for the M5 depth-train automation register.
//!
...
use aureline_release::generate_m5_proof_freshness_backport_and_evidence_expiry_automation_for_depth_trains::
    current_m5_depth_train_automation_register;
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        None | Some("register") => {
            let reg = current_m5_depth_train_automation_register()?;
