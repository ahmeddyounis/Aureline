//! Headless emitter for the M5 portable-state export/import review corpus.

use std::path::PathBuf;

use aureline_settings::m5_portable_state_review::{
    portable_state_review_corpus, PortableStateReviewScenario,
};
...
fn print_all() {
    let scenarios = portable_state_review_corpus();
...
fn print_plaintext() {
    for (idx, scenario) in portable_state_review_corpus().iter().enumerate() {
        if idx > 0 {
            println!();
        }
...
fn print_index() {
    println!(
        "{:<52} {:<10} {:<16} schema_match",
        "scenario_id", "direction", "readiness"
    );
    for scenario in portable_state_review_corpus() {
        ...
        println!(
            ...
            format!("{:?}", record.qualification.readiness),
            record.qualification.schema_versions_match,
        );
    }
}
...
fn render_json(scenario: &PortableStateReviewScenario) -> String {
