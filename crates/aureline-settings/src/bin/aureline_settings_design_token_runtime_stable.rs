//! Headless emitter for the stable design-token runtime certification corpus.
//!
//! Mints the on-disk fixtures under
//! `fixtures/ux/m4/certify-the-design-token-runtime-across-dark-light/` so the
//! settings appearance panel, command palette, status bar, diagnostics packet,
//! support export, Help/About, and reviewer-facing reports all quote the same
...
//!   --bin aureline_settings_design_token_runtime_stable -- scenario <scenario_id>
//!
//! # Print every scenario, separated by --- markers.
//! cargo run -q -p aureline-settings \
//!   --bin aureline_settings_design_token_runtime_stable -- all
//!
//! # Plaintext truth block (per-scenario, suitable for support export).
//! cargo run -q -p aureline-settings \
//!   --bin aureline_settings_design_token_runtime_stable -- plaintext
//!
...
use aureline_settings::design_token_runtime_stable::{
    design_token_runtime_corpus, DesignTokenRuntimeScenario,
};
...
fn print_all() {
    let scenarios = design_token_runtime_corpus();
...
fn print_scenario(id: &str) -> Result<(), Box<dyn std::error::Error>> {
    let scenario = design_token_runtime_corpus()
        .into_iter()
...
fn print_plaintext() {
    for (idx, scenario) in design_token_runtime_corpus().iter().enumerate() {
        if idx > 0 {
            println!();
        }
...
fn render_json(scenario: &DesignTokenRuntimeScenario) -> String {
