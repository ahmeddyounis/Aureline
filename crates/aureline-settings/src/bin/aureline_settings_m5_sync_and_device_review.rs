//! Headless emitter for the M5 sync-and-device review corpus.

use std::path::PathBuf;

use aureline_settings::m5_sync_and_device_review::{
    m5_sync_and_device_review_corpus, M5SyncAndDeviceReviewScenario,
};
...
fn print_all() {
    let scenarios = m5_sync_and_device_review_corpus();
...
fn print_plaintext() {
    for (idx, scenario) in m5_sync_and_device_review_corpus().iter().enumerate() {
        if idx > 0 {
            println!();
        }
...
fn print_index() {
    println!(
        "{:<56} {:<30} {:<10} ceiling",
        "record_id", "claim", "qualifies"
    );
    for scenario in m5_sync_and_device_review_corpus() {
        let record = scenario.record();
        let claim = format!("{:?}", record.trust_qualification.claim_class);
        println!(
            "{:<56} {:<30} {:<10} {}",
            record.record_id,
            claim,
            record.trust_qualification.qualifies_fully_synced,
            record.trust_qualification.effective_trust_ceiling.as_str(),
        );
    }
}
...
fn render_json(scenario: &M5SyncAndDeviceReviewScenario) -> String {
