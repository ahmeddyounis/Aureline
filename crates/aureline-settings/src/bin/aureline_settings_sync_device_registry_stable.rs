//! Headless emitter for the stable sync / device-registry certification corpus.
//!
//! Mints the on-disk fixtures under
//! `fixtures/ux/m4/ship-sync-device-registry-conflict-review-and-support/` so the
//! desktop device-and-sync surface, command palette, status bar, support export,
//! Help/About, admin device-registry view, and reviewer-facing reports all quote
...
//!   --bin aureline_settings_sync_device_registry_stable -- scenario <scenario_id>
//!
//! # Print every scenario, separated by --- markers.
//! cargo run -q -p aureline-settings \
//!   --bin aureline_settings_sync_device_registry_stable -- all
//!
//! # Plaintext truth block (per-scenario, suitable for support export).
//! cargo run -q -p aureline-settings \
//!   --bin aureline_settings_sync_device_registry_stable -- plaintext
//!
...
use aureline_settings::sync_device_registry_stable::{
    sync_device_registry_corpus, SyncDeviceRegistryScenario,
};
...
fn print_all() {
    let scenarios = sync_device_registry_corpus();
...
fn print_scenario(id: &str) -> Result<(), Box<dyn std::error::Error>> {
    let scenario = sync_device_registry_corpus()
        .into_iter()
...
fn print_plaintext() {
    for (idx, scenario) in sync_device_registry_corpus().iter().enumerate() {
        if idx > 0 {
            println!();
        }
...
fn render_json(scenario: &SyncDeviceRegistryScenario) -> String {
