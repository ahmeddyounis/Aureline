//! Headless emitter for the stable appearance-session finalization certification
//! corpus.
//!
//! Mints the on-disk fixtures under
//! `fixtures/ux/m4/finalize-appearance-session-theme-packages-token-overlays/`
//! so the settings appearance panel, command palette, status bar, diagnostics
//! packet, support export, Help/About, and reviewer-facing reports all quote the
//! same record renders as the in-code corpus. The fixture-replay test in
...
//!   --bin aureline_settings_finalize_appearance_session_theme_packages_token_overlays -- scenario <scenario_id>
//!
//! # Print every scenario, separated by --- markers.
//! cargo run -q -p aureline-settings \
//!   --bin aureline_settings_finalize_appearance_session_theme_packages_token_overlays -- all
//!
//! # Plaintext truth block (per-scenario, suitable for support export).
//! cargo run -q -p aureline-settings \
//!   --bin aureline_settings_finalize_appearance_session_theme_packages_token_overlays -- plaintext
//!
//! # Stable corpus index — scenario id, posture, claim, surface marker.
//! cargo run -q -p aureline-settings \
//!   --bin aureline_settings_finalize_appearance_session_theme_packages_token_overlays -- index
//!
...
use aureline_settings::finalize_appearance_session_theme_packages_token_overlays::{
    appearance_session_finalization_corpus, AppearanceSessionFinalizationScenario,
};
...
fn print_all() {
    let scenarios = appearance_session_finalization_corpus();
...
fn print_scenario(id: &str) -> Result<(), Box<dyn std::error::Error>> {
    let scenario = appearance_session_finalization_corpus()
        .into_iter()
...
fn print_plaintext() {
    for (idx, scenario) in appearance_session_finalization_corpus().iter().enumerate() {
        if idx > 0 {
            println!();
        }
...
fn render_json(scenario: &AppearanceSessionFinalizationScenario) -> String {
