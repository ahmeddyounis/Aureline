...
use aureline_content_safety::{RepresentationLabelsBetaCase, RepresentationLabelsBetaPacket};

fn main() {
    let args = std::env::args().collect::<Vec<_>>();
    if args.len() != 2 {
        eprintln!("usage: representation_labels_beta <case.json>");
        std::process::exit(2);
    }
    ...
    let case: RepresentationLabelsBetaCase = match serde_json::from_str(&raw) {
        Ok(case) => case,
        Err(err) => {
