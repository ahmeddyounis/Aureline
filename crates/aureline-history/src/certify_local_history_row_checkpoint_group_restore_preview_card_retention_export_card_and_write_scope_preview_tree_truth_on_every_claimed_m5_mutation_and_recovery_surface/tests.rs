//! Tests for the M05-899 local-history / write-scope component surface certification
//! capstone.

use super::*;

fn packet() -> HistorySurfaceCertificationPacket {
    seeded_m5_local_history_write_scope_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, HISTORY_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, HISTORY_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5LocalHistoryWriteScopeCertifiedSurface::ALL {
...
#[test]
fn every_surface_preserves_history() {
    let p = packet();
    for row in &p.rows {
        assert!(
            row.preserves_history_integrity(),
            "row {} erases history",
            row.row_id
        );
        assert!(row.history_preserved);
    }
    assert!(p.summary.all_history_preserved);
}
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(row.axis_outcomes.len(), HistoryCertificationAxis::ALL.len());
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, HistoryAxisCertificationState::Certified);
...
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, HISTORY_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(row.canonical_bundle_ref, HISTORY_CERT_CANONICAL_BUNDLE_REF);
    }
...
fn yellow_rows_narrow_their_claim_and_bind_to_a_narrowed_axis() {
        ...
        .filter(|r| r.derived_status == HistorySurfaceClaimStatus::Yellow)
    {
        ...
        assert!(narrow.preserves_history_integrity);
    }
...
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5LocalHistoryWriteScopeCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
...
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = HistoryCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), HistoryCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in HistoryCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == HistoryCertificationAxis::CliExport
        );
...
fn support_claim_ladder_is_strictly_ordered() {
    let ranks: Vec<u8> = M5HistorySupportClaim::ALL
        .iter()
        .map(|c| c.capability_rank())
        .collect();
    assert_eq!(ranks, vec![5, 4, 3, 2, 1, 0]);
    let tokens: BTreeSet<&str> = M5HistorySupportClaim::ALL
        .iter()
        .map(|c| c.as_str())
        .collect();
    assert_eq!(tokens.len(), M5HistorySupportClaim::ALL.len());
}
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5LocalHistoryWriteScopeCertifiedSurface::EditorRenameRefactor)
        .expect("editor-rename-refactor row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == HistoryCertificationAxis::Visual {
            outcome.state = HistoryAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("write scope silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, HistorySurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, HistoryCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5LocalHistoryWriteScopeCertifiedSurface::SupportExport)
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == HistoryCertificationAxis::DegradedState {
            *outcome = HistoryAxisOutcome {
                axis: HistoryCertificationAxis::DegradedState,
                state: HistoryAxisCertificationState::DisclosedNarrowed,
                parity_note: "capture fidelity lagging".to_owned(),
                narrowing_reason: Some("only metadata was captured for this history".to_owned()),
                downgrade_trigger: Some(M5HistoryDowngradeTrigger::CaptureFidelityMasked),
            };
        }
    }
    // Claim stays ReviewableHistory == certified ReviewableHistory, no auto-narrow.
    assert_eq!(row.derive_status(), HistorySurfaceClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == HistoryCertificationAxis::CliExport {
            outcome.state = HistoryAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5HistoryDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), HistorySurfaceClaimStatus::Red);
}
...
#[test]
fn erased_history_blocks_the_surface() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.history_preserved = false;
    assert!(!row.preserves_history_integrity());
    assert_eq!(row.derive_status(), HistorySurfaceClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, HistoryCertificationViolation::HistoryErased { .. })));
}

#[test]
fn narrowed_row_erasing_history_blocks() {
    let mut p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == HistorySurfaceClaimStatus::Yellow)
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(HistoryClaimAutoNarrow {
        binding_axis: HistoryCertificationAxis::DegradedState,
        from_claim: M5HistorySupportClaim::RestorableCheckpoint,
        to_claim: M5HistorySupportClaim::RestorableCheckpoint,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5HistorySupportClaim::MetadataOnlyHistory;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableHistory
    row.certified_claim = M5HistorySupportClaim::RestorableCheckpoint;
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        HistoryCertificationViolation::CertifiedClaimExceedsClaim { .. }
    )));
}
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5LocalHistoryWriteScopeCertifiedSurface::ImportMigration)
        .expect("import-migration row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = HistoryCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), HistorySurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == HistoryCertificationAxis::CliExport {
            outcome.state = HistoryAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the import view".to_owned());
            outcome.downgrade_trigger = Some(M5HistoryDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = HistoryCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), HistorySurfaceClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "metadata only".to_owned();
    }
    assert_eq!(row.derive_status(), HistorySurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(HistoryCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5LocalHistoryWriteScopeCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, HistoryCertificationViolation::SurfaceCoverageIncomplete)));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the RetentionExportCard family from every row that carries it; coverage must
    // fail (only the support-export row consumes it).
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5LocalHistoryWriteScopeComponentFamily::RetentionExportCard);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, HistoryCertificationViolation::FamilyCoverageIncomplete)));
}

#[test]
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = HistorySurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        HistoryCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        HistoryCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, HistoryCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, HistoryCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != HistoryCertificationAxis::MutationAndRecoveryProvenance);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        HistoryCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, HistoryCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, HistoryCertificationViolation::RawHistoryMaterialInExport)));
}
...
fn export_json_round_trips() {
    ...
    let back: HistorySurfaceCertificationPacket = serde_json::from_str(&json).expect("round trips");
    assert_eq!(p, back);
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_local_history_write_scope_component_certification_export()
...
/// `GEN_HISTORY_CERT_ARTIFACTS=1 cargo test -p aureline-history \
///  certify_local_history_row...::tests::generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_HISTORY_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_local_history_write_scope_component_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-local-history-write-scope-component-certification");
    ...
    let fixtures = Path::new(manifest)
        .join("../../fixtures/ui/m5-local-history-write-scope-component-certification");
