...
fn main() {
    ...
    println!("cargo:rerun-if-changed={}", repo_root.join("rust-toolchain.toml").display());
    ...
    let commit_short = git(repo_root, &["rev-parse", "--short=12", "HEAD"]).unwrap_or_else(|| "unknown".to_owned());
    ...
    let toolchain_channel = read_quoted_key(repo_root.join("rust-toolchain.toml"), "channel").unwrap_or_else(|| "unknown".to_owned());

    let rustc_cmd = env::var("RUSTC").unwrap_or_else(|_| "rustc".to_owned());
    let cargo_cmd = env::var("CARGO").unwrap_or_else(|_| "cargo".to_owned());

    let rustc_version = run_stdout_trimmed(repo_root, &rustc_cmd, &["--version"]).unwrap_or_else(|| "unknown".to_owned());
    let cargo_version = run_stdout_trimmed(repo_root, &cargo_cmd, &["--version"]).unwrap_or_else(|| "unknown".to_owned());
    ...
    let source_date_epoch = env::var("SOURCE_DATE_EPOCH")
        ...
        .or_else(|| git(repo_root, &["log", "-1", "--pretty=%ct"]).and_then(|s| s.parse::<i64>().ok()))
        .unwrap_or(0);
    ...
    emit_env("AURELINE_BUILD_IDENTITY_DIRTY", if dirty { "true" } else { "false" });
    emit_env("AURELINE_BUILD_IDENTITY_TOOLCHAIN_CHANNEL", &toolchain_channel);
    ...
    emit_env("AURELINE_BUILD_IDENTITY_WORKSPACE_VERSION", &workspace_version);
    emit_env("AURELINE_BUILD_IDENTITY_SOURCE_DATE_EPOCH", &source_date_epoch.to_string());
    emit_env("AURELINE_BUILD_IDENTITY_BUILD_TIMESTAMP_UTC", &build_timestamp_utc);
}
...
fn run_stdout_trimmed(repo_root: &Path, cmd: &str, args: &[&str]) -> Option<String> {
    let output = Command::new(cmd).args(args).current_dir(repo_root).output().ok()?;
