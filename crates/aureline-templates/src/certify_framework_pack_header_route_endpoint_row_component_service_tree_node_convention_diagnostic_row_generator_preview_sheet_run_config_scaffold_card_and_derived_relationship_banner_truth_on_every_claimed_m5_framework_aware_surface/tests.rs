//! Tests for the M05-1043 framework component surface certification capstone.

use super::*;

fn packet() -> FrameworkSurfaceCertificationPacket {
    seeded_m5_framework_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, FRAMEWORK_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, FRAMEWORK_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5FrameworkCertifiedSurface::ALL {
...
#[test]
fn packet_has_three_green_and_five_yellow_and_no_red() {
    let p = packet();
    assert_eq!(p.summary.green_row_count, 3);
...
fn every_surface_preserves_source_and_recovery() {
    ...
    for row in &p.rows {
        assert!(
            ...
            "row {} drops proving-source/recovery",
            row.row_id
        );
        assert!(row.proving_source_and_recovery_preserved);
    }
    assert!(p.summary.all_proving_source_and_recovery_preserved);
}

#[test]
fn no_surface_lets_heuristic_read_exact_or_hides_write_boundary() {
    let p = packet();
    for row in &p.rows {
        assert!(
            !row.lets_heuristic_masquerade_as_exact,
            "row {} lets a heuristic route masquerade as exact",
            row.row_id
        );
        assert!(
            !row.implies_no_op_write_or_hides_execution_boundary,
            "row {} implies a no-op write or hides the execution boundary",
            row.row_id
        );
    }
    assert!(p.summary.no_surface_lets_heuristic_masquerade_as_exact);
    assert!(
        p.summary
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            FrameworkCertificationAxis::ALL.len()
        );
    }
...
fn export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("export axis");
        assert_eq!(export.state, FrameworkAxisCertificationState::Certified);
...
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, FRAMEWORK_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(
...
#[test]
fn the_five_spec_auto_narrow_conditions_are_each_certified() {
    // The five spec narrowing conditions: an unverified pack health, an unprovable supported version
    // range, a missing proving-source linkage, a heuristically-inferred relationship, and a partial
    // generator-effect truth — must each be a certified yellow surface.
    let p = packet();
    let certified: BTreeSet<M5FrameworkComponentClaim> = p
        .rows
        .iter()
        .filter(|r| r.derived_status == FrameworkSurfaceClaimStatus::Yellow)
        .map(|r| r.certified_claim)
        .collect();
    assert!(certified.contains(&M5FrameworkComponentClaim::UnverifiedPackProjection));
    assert!(certified.contains(&M5FrameworkComponentClaim::UnprovenVersionRangeProjection));
    assert!(certified.contains(&M5FrameworkComponentClaim::UnlinkedSourceProjection));
    assert!(certified.contains(&M5FrameworkComponentClaim::HeuristicInferenceProjection));
    assert!(certified.contains(&M5FrameworkComponentClaim::PartialGeneratorEffectProjection));
}

#[test]
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5FrameworkCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5FrameworkCertifiedSurface::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = FrameworkCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), FrameworkCertificationAxis::ALL.len());
}

#[test]
fn only_export_axis_is_always_on() {
    for axis in FrameworkCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == FrameworkCertificationAxis::Export
        );
...
#[test]
fn exactness_claim_ladder_is_strictly_ordered() {
    let ranks: Vec<u8> = M5FrameworkComponentClaim::ALL
        .iter()
        .map(|c| c.capability_rank())
        .collect();
    assert_eq!(ranks, vec![5, 4, 3, 2, 1, 0]);
    let tokens: BTreeSet<&str> = M5FrameworkComponentClaim::ALL
        .iter()
        .map(|c| c.as_str())
        .collect();
    assert_eq!(tokens.len(), M5FrameworkComponentClaim::ALL.len());
}
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5FrameworkCertifiedSurface::RunConfigCenter)
        .expect("run-config-center row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == FrameworkCertificationAxis::Visual {
            outcome.state = FrameworkAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("pack identity silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, FrameworkSurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, FrameworkCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5FrameworkCertifiedSurface::SupportExport)
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == FrameworkCertificationAxis::DegradedState {
            *outcome = FrameworkAxisOutcome {
                axis: FrameworkCertificationAxis::DegradedState,
                state: FrameworkAxisCertificationState::DisclosedNarrowed,
                parity_note: "pack health lagging".to_owned(),
                narrowing_reason: Some(
                    "the pack health was not restated for this export".to_owned(),
                ),
                downgrade_trigger: Some(M5FrameworkDowngradeTrigger::SupportClassUnstated),
            };
        }
    }
    // Claim stays ExactFrameworkTruth == certified ExactFrameworkTruth, no auto-narrow.
    assert_eq!(row.derive_status(), FrameworkSurfaceClaimStatus::Red);
}
...
fn export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == FrameworkCertificationAxis::Export {
            outcome.state = FrameworkAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5FrameworkDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), FrameworkSurfaceClaimStatus::Red);
}
...
fn dropped_source_and_recovery_blocks_the_surface() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.proving_source_and_recovery_preserved = false;
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        FrameworkCertificationViolation::SourceOrRecoveryDropped { .. }
    )));
}

#[test]
fn heuristic_masquerading_as_exact_blocks_the_surface() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.lets_heuristic_masquerade_as_exact = true;
    assert_eq!(row.derive_status(), FrameworkSurfaceClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
#[test]
fn implying_no_op_write_or_hiding_boundary_blocks_the_surface() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.implies_no_op_write_or_hides_execution_boundary = true;
    assert_eq!(row.derive_status(), FrameworkSurfaceClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn narrowed_row_dropping_source_and_recovery_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == FrameworkSurfaceClaimStatus::Yellow)
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(FrameworkClaimAutoNarrow {
        binding_axis: FrameworkCertificationAxis::DegradedState,
        from_claim: M5FrameworkComponentClaim::ExactFrameworkTruth,
        to_claim: M5FrameworkComponentClaim::ExactFrameworkTruth,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5FrameworkComponentClaim::PartialGeneratorEffectProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    row.claimed_claim = M5FrameworkComponentClaim::UnverifiedPackProjection;
    row.certified_claim = M5FrameworkComponentClaim::ExactFrameworkTruth;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5FrameworkCertifiedSurface::FrameworkPackCenter)
        .expect("framework-pack-center row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = FrameworkCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), FrameworkSurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == FrameworkCertificationAxis::Export {
            outcome.state = FrameworkAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the framework pack center".to_owned());
            outcome.downgrade_trigger = Some(M5FrameworkDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = FrameworkCertificationAxis::Export;
    }
    assert_eq!(row.derive_status(), FrameworkSurfaceClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "heuristic".to_owned();
    }
    assert_eq!(row.derive_status(), FrameworkSurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(FrameworkCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5FrameworkCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        FrameworkCertificationViolation::SurfaceCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the ConventionDiagnosticRow family from every row that carries it; coverage must fail.
    // The convention-diagnostics row consumes only that family, so it also goes red (empty families),
    // but the family-coverage violation must surface.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5FrameworkComponentFamily::ConventionDiagnosticRow);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, FrameworkCertificationViolation::FamilyCoverageIncomplete)));
}

#[test]
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = FrameworkSurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        FrameworkCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        FrameworkCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, FrameworkCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, FrameworkCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != FrameworkCertificationAxis::SourceLinkageAndExecutionBoundary);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        FrameworkCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, FrameworkCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        FrameworkCertificationViolation::RawFrameworkMaterialInExport
    )));
}

#[test]
fn seeded_packet_carries_no_forbidden_material() {
    // The governed framework vocabulary carries only typed class tokens, opaque refs, booleans, and
    // redacted labels; the seeded packet must stay clean.
    let p = packet();
...
fn export_json_round_trips() {
    ...
    let back: FrameworkSurfaceCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_framework_component_certification_export().expect("export is valid");
    assert_eq!(
...
/// `GEN_FRAMEWORK_CERT_ARTIFACTS=1 cargo test -p aureline-templates \
///  certify_framework...::tests::generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_FRAMEWORK_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_framework_component_certification_packet();
    ...
    let art =
        Path::new(manifest).join("../../artifacts/release/m5-framework-component-certification");
    ...
    let fixtures =
        Path::new(manifest).join("../../fixtures/ui/m5-framework-component-certification");
