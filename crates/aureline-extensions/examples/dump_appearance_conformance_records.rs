//! Dump extension appearance-conformance records.
//!
...
//! cargo run -q -p aureline-extensions --example dump_appearance_conformance_records -- inputs
...
use aureline_extensions::{
    project_appearance_conformance_support_export, seeded_appearance_conformance_inputs,
    seeded_appearance_conformance_packet, validate_appearance_conformance_packet,
};
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("inputs") => print_json(&seeded_appearance_conformance_inputs())?,
        ...
        Some("validate") => match validate_appearance_conformance_packet(&packet) {
            Ok(()) => println!("ok"),
            Err(defects) => {
