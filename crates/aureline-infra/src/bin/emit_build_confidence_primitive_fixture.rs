//! Emits the canonical M5 build / run confidence primitive as `support`, `csv`,
//! or `summary`. The `support` output is the byte-for-byte `include_str!`
//! canonical checked in under
//! `artifacts/release/m5-build-confidence-primitive-proof/`.

fn main() {
    let mode = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "support".to_owned());
    let packet = aureline_infra::seeded_m5_build_confidence_packet();
    match mode.as_str() {
