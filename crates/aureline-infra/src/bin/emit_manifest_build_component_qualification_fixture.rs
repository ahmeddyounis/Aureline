//! Emits the canonical M05-819 manifest / build component qualification packet as
//! `support`, `csv`, or `summary`. The `support` output is the byte-for-byte
//! `include_str!` canonical checked in under
//! `artifacts/release/m5-manifest-build-component-qualification-proof/`.

fn main() {
    let mode = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "support".to_owned());
    let packet = aureline_infra::seeded_m5_manifest_build_component_qualification_packet();
    match mode.as_str() {
