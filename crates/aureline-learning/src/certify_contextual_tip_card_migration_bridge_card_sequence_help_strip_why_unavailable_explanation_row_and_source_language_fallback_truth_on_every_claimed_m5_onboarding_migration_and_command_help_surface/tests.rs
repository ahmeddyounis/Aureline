//! Tests for the M05-931 contextual-teaching component surface certification capstone.

use super::*;

fn packet() -> TeachingSurfaceCertificationPacket {
    seeded_m5_contextual_teaching_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, TEACHING_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, TEACHING_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5ContextualTeachingHelpCertifiedSurface::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            TeachingCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, TeachingAxisCertificationState::Certified);
...
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, TEACHING_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(row.canonical_bundle_ref, TEACHING_CERT_CANONICAL_BUNDLE_REF);
    }
...
fn the_four_spec_auto_narrow_conditions_are_each_certified() {
    // The four spec narrowing conditions: snoozed tip, partial bridge, unsupported sequence, and
    // stale localized fallback — must each be a certified yellow surface.
    let p = packet();
    let certified: BTreeSet<M5TeachingComponentClaim> = p
        .rows
        .iter()
        .filter(|r| r.derived_status == TeachingSurfaceClaimStatus::Yellow)
        .map(|r| r.certified_claim)
        .collect();
    assert!(certified.contains(&M5TeachingComponentClaim::SnoozedTipProjection));
    assert!(certified.contains(&M5TeachingComponentClaim::PartialBridgeProjection));
    assert!(certified.contains(&M5TeachingComponentClaim::UnsupportedSequenceProjection));
    assert!(certified.contains(&M5TeachingComponentClaim::StaleFallbackProjection));
}

#[test]
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5ContextualTeachingHelpCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
...
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = TeachingCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), TeachingCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in TeachingCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == TeachingCertificationAxis::CliExport
        );
...
#[test]
fn teaching_claim_ladder_is_strictly_ordered() {
    let ranks: Vec<u8> = M5TeachingComponentClaim::ALL
        .iter()
        .map(|c| c.capability_rank())
        .collect();
    assert_eq!(ranks, vec![5, 4, 3, 2, 1, 0]);
    let tokens: BTreeSet<&str> = M5TeachingComponentClaim::ALL
        .iter()
        .map(|c| c.as_str())
        .collect();
    assert_eq!(tokens.len(), M5TeachingComponentClaim::ALL.len());
}
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5ContextualTeachingHelpCertifiedSurface::FirstRunOnboarding)
        .expect("first-run-onboarding row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == TeachingCertificationAxis::Visual {
            outcome.state = TeachingAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("command binding silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, TeachingSurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, TeachingCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5ContextualTeachingHelpCertifiedSurface::SupportExport)
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == TeachingCertificationAxis::DegradedState {
            *outcome = TeachingAxisOutcome {
                axis: TeachingCertificationAxis::DegradedState,
                state: TeachingAxisCertificationState::DisclosedNarrowed,
                parity_note: "source-language citation lagging".to_owned(),
                narrowing_reason: Some(
                    "the canonical citation was not restated for this export".to_owned(),
                ),
                downgrade_trigger: Some(M5TeachingDowngradeTrigger::CitationSevered),
            };
        }
    }
    // Claim stays ReviewableGuidance == certified ReviewableGuidance, no auto-narrow.
    assert_eq!(row.derive_status(), TeachingSurfaceClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == TeachingCertificationAxis::CliExport {
            outcome.state = TeachingAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5TeachingDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), TeachingSurfaceClaimStatus::Red);
}
...
fn dropped_lineage_blocks_the_surface() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, TeachingCertificationViolation::LineageDropped { .. })));
}
...
fn narrowed_row_dropping_lineage_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == TeachingSurfaceClaimStatus::Yellow)
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(TeachingClaimAutoNarrow {
        binding_axis: TeachingCertificationAxis::DegradedState,
        from_claim: M5TeachingComponentClaim::ExactTeaching,
        to_claim: M5TeachingComponentClaim::ExactTeaching,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5TeachingComponentClaim::StaleFallbackProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    row.certified_claim = M5TeachingComponentClaim::ExactTeaching;
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        TeachingCertificationViolation::CertifiedClaimExceedsClaim { .. }
    )));
}
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.surface == M5ContextualTeachingHelpCertifiedSurface::MigrationImporterReview
        })
        .expect("migration-importer-review row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = TeachingCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), TeachingSurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == TeachingCertificationAxis::CliExport {
            outcome.state = TeachingAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the importer review".to_owned());
            outcome.downgrade_trigger = Some(M5TeachingDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = TeachingCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), TeachingSurfaceClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "partial".to_owned();
    }
    assert_eq!(row.derive_status(), TeachingSurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(TeachingCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5ContextualTeachingHelpCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, TeachingCertificationViolation::SurfaceCoverageIncomplete)));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the WhyUnavailableExplanationRow family from every row that carries it; coverage must
    // fail (only the command-palette-docs row consumes it, and it retains another family so it
    // does not itself go red).
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5ContextualTeachingComponentFamily::WhyUnavailableExplanationRow);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, TeachingCertificationViolation::FamilyCoverageIncomplete)));
}

#[test]
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = TeachingSurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        TeachingCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        TeachingCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, TeachingCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, TeachingCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != TeachingCertificationAxis::TeachingBoundaryProvenance);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        TeachingCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, TeachingCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        TeachingCertificationViolation::RawTeachingMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: TeachingSurfaceCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_contextual_teaching_component_certification_export().expect("export is valid");
    assert_eq!(
...
/// `GEN_TEACHING_CERT_ARTIFACTS=1 cargo test -p aureline-learning \
///  certify_contextual_tip_card...::tests::generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_TEACHING_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_contextual_teaching_component_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-contextual-teaching-component-certification");
    ...
    let fixtures = Path::new(manifest)
        .join("../../fixtures/ui/m5-contextual-teaching-component-certification");
