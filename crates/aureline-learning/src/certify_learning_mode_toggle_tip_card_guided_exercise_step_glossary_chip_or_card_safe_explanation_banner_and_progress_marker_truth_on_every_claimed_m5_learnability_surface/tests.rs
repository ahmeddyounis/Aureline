//! Tests for the M05-1011 learning component surface certification capstone.

use super::*;

fn packet() -> LearningSurfaceCertificationPacket {
    seeded_m5_learning_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, LEARNING_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, LEARNING_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5LearningCertifiedSurface::ALL {
...
#[test]
fn no_surface_widens_authority() {
    let p = packet();
    for row in &p.rows {
        assert!(
            !row.widens_learning_authority,
            "row {} widens authority",
            row.row_id
        );
    }
    assert!(p.summary.no_surface_widens_authority);
}
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            LearningCertificationAxis::ALL.len()
        );
    }
...
fn export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("export axis");
        assert_eq!(export.state, LearningAxisCertificationState::Certified);
...
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, LEARNING_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(row.canonical_bundle_ref, LEARNING_CERT_CANONICAL_BUNDLE_REF);
    }
...
fn the_four_spec_auto_narrow_conditions_are_each_certified() {
    // The four spec narrowing conditions: stale exercise pack, uncited glossary, unprovable
    // explain-versus-do boundary, and blocked progress portability — must each be a certified
    // yellow surface.
    let p = packet();
    let certified: BTreeSet<M5LearningComponentClaim> = p
        .rows
        .iter()
        .filter(|r| r.derived_status == LearningSurfaceClaimStatus::Yellow)
        .map(|r| r.certified_claim)
        .collect();
    assert!(certified.contains(&M5LearningComponentClaim::StalePackProjection));
    assert!(certified.contains(&M5LearningComponentClaim::UncitedGlossaryProjection));
    assert!(certified.contains(&M5LearningComponentClaim::UnprovableBoundaryProjection));
    assert!(certified.contains(&M5LearningComponentClaim::BlockedProgressProjection));
}

#[test]
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5LearningCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5LearningCertifiedSurface::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = LearningCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), LearningCertificationAxis::ALL.len());
}

#[test]
fn only_export_axis_is_always_on() {
    for axis in LearningCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == LearningCertificationAxis::Export
        );
...
#[test]
fn learning_claim_ladder_is_strictly_ordered() {
    let ranks: Vec<u8> = M5LearningComponentClaim::ALL
        .iter()
        .map(|c| c.capability_rank())
        .collect();
    assert_eq!(ranks, vec![7, 6, 5, 4, 3, 2, 1, 0]);
    let tokens: BTreeSet<&str> = M5LearningComponentClaim::ALL
        .iter()
        .map(|c| c.as_str())
        .collect();
    assert_eq!(tokens.len(), M5LearningComponentClaim::ALL.len());
}
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5LearningCertifiedSurface::FirstRunOnboarding)
        .expect("first-run-onboarding row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == LearningCertificationAxis::Visual {
            outcome.state = LearningAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("cited source silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, LearningSurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, LearningCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5LearningCertifiedSurface::SupportExport)
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == LearningCertificationAxis::DegradedState {
            *outcome = LearningAxisOutcome {
                axis: LearningCertificationAxis::DegradedState,
                state: LearningAxisCertificationState::DisclosedNarrowed,
                parity_note: "progress ownership lagging".to_owned(),
                narrowing_reason: Some(
                    "the progress ownership was not restated for this export".to_owned(),
                ),
                downgrade_trigger: Some(M5LearningDowngradeTrigger::ProgressOwnershipUnstated),
            };
        }
    }
    // Claim stays ReviewableGuidance == certified ReviewableGuidance, no auto-narrow.
    assert_eq!(row.derive_status(), LearningSurfaceClaimStatus::Red);
}
...
fn export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == LearningCertificationAxis::Export {
            outcome.state = LearningAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5LearningDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), LearningSurfaceClaimStatus::Red);
}
...
fn dropped_lineage_blocks_the_surface() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, LearningCertificationViolation::LineageDropped { .. })));
}

#[test]
fn widening_authority_blocks_the_surface() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.widens_learning_authority = true;
    assert_eq!(row.derive_status(), LearningSurfaceClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        LearningCertificationViolation::LearningAuthorityWidened { .. }
    )));
    assert!(!p.summary.no_surface_widens_authority);
}
...
fn narrowed_row_dropping_lineage_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == LearningSurfaceClaimStatus::Yellow)
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(LearningClaimAutoNarrow {
        binding_axis: LearningCertificationAxis::DegradedState,
        from_claim: M5LearningComponentClaim::ExactLearning,
        to_claim: M5LearningComponentClaim::ExactLearning,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5LearningComponentClaim::BlockedProgressProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableGuidance
    row.certified_claim = M5LearningComponentClaim::ExactLearning;
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        LearningCertificationViolation::CertifiedClaimExceedsClaim { .. }
    )));
}
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5LearningCertifiedSurface::ContextualHelp)
        .expect("contextual-help row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = LearningCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), LearningSurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == LearningCertificationAxis::Export {
            outcome.state = LearningAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the contextual help".to_owned());
            outcome.downgrade_trigger = Some(M5LearningDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = LearningCertificationAxis::Export;
    }
    assert_eq!(row.derive_status(), LearningSurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(LearningCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5LearningCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, LearningCertificationViolation::SurfaceCoverageIncomplete)));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the GuidedExerciseStep family from every row that carries it; coverage must fail (only
    // the guided-exercise-practice row consumes it, so removing it drops the family). That row then
    // has no families and goes red, but the family-coverage violation must also surface.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5LearningComponentFamily::GuidedExerciseStep);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, LearningCertificationViolation::FamilyCoverageIncomplete)));
}

#[test]
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = LearningSurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        LearningCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        LearningCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, LearningCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, LearningCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != LearningCertificationAxis::LearningBoundaryProvenance);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        LearningCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, LearningCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        LearningCertificationViolation::RawLearningMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: LearningSurfaceCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_learning_component_certification_export().expect("export is valid");
    assert_eq!(
...
/// `GEN_LEARNING_CERT_ARTIFACTS=1 cargo test -p aureline-learning \
///  certify_learning_mode_toggle...::tests::generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_LEARNING_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_learning_component_certification_packet();
    ...
    let art =
        Path::new(manifest).join("../../artifacts/release/m5-learning-component-certification");
    ...
    let fixtures =
        Path::new(manifest).join("../../fixtures/ui/m5-learning-component-certification");
