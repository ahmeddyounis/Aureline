//! Tests for the M05-891 prompt-composer component surface certification capstone.

use super::*;

fn packet() -> ComposerSurfaceCertificationPacket {
    seeded_m5_prompt_composer_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, COMPOSER_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, COMPOSER_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5PromptComposerCertifiedSurface::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            ComposerCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, ComposerAxisCertificationState::Certified);
...
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, COMPOSER_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(row.canonical_bundle_ref, COMPOSER_CERT_CANONICAL_BUNDLE_REF);
    }
...
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5PromptComposerCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5PromptComposerCertifiedSurface::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = ComposerCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), ComposerCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in ComposerCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == ComposerCertificationAxis::CliExport
        );
...
fn support_claim_ladder_is_strictly_ordered() {
    let ranks: Vec<u8> = M5ComposerSupportClaim::ALL
        .iter()
        .map(|c| c.capability_rank())
        .collect();
    assert_eq!(ranks, vec![5, 4, 3, 2, 1, 0]);
    let tokens: BTreeSet<&str> = M5ComposerSupportClaim::ALL
        .iter()
        .map(|c| c.as_str())
        .collect();
    assert_eq!(tokens.len(), M5ComposerSupportClaim::ALL.len());
}
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5PromptComposerCertifiedSurface::InlineComposer)
        .expect("inline-composer row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ComposerCertificationAxis::Visual {
            outcome.state = ComposerAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("composer mode silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, ComposerSurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, ComposerCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5PromptComposerCertifiedSurface::AssistantPanel)
        .expect("assistant-panel row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ComposerCertificationAxis::DegradedState {
            *outcome = ComposerAxisOutcome {
                axis: ComposerCertificationAxis::DegradedState,
                state: ComposerAxisCertificationState::DisclosedNarrowed,
                parity_note: "route read lagging".to_owned(),
                narrowing_reason: Some("the draft is held offline pending route access".to_owned()),
                downgrade_trigger: Some(M5ComposerDowngradeTrigger::DraftLocalityMasked),
            };
        }
    }
    // Claim stays ReviewableComposition == certified ReviewableComposition, no auto-narrow.
    assert_eq!(row.derive_status(), ComposerSurfaceClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ComposerCertificationAxis::CliExport {
            outcome.state = ComposerAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5ComposerDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), ComposerSurfaceClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(ComposerClaimAutoNarrow {
        binding_axis: ComposerCertificationAxis::DegradedState,
        from_claim: M5ComposerSupportClaim::ReadyToSend,
        to_claim: M5ComposerSupportClaim::ReadyToSend,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5ComposerSupportClaim::LocalOnlyComposition;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("assistant-panel row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableComposition
    row.certified_claim = M5ComposerSupportClaim::ReadyToSend;
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ComposerCertificationViolation::CertifiedClaimExceedsClaim { .. }
    )));
}
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5PromptComposerCertifiedSurface::CliHeadless)
        .expect("cli-headless row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = ComposerCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), ComposerSurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = ComposerCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), ComposerSurfaceClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == ComposerSurfaceClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "unresolved".to_owned();
    }
    assert_eq!(row.derive_status(), ComposerSurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(ComposerCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5PromptComposerCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, ComposerCertificationViolation::SurfaceCoverageIncomplete)));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the MentionResolver family from every row that carries it; coverage must fail
    // (only the cli-headless row consumes it).
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5PromptComposerComponentFamily::MentionResolver);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ComposerCertificationViolation::FamilyCoverageIncomplete)));
}

#[test]
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = ComposerSurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ComposerCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ComposerCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ComposerCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ComposerCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != ComposerCertificationAxis::CompositionAndSendProvenance);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ComposerCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ComposerCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ComposerCertificationViolation::RawComposerMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: ComposerSurfaceCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_prompt_composer_component_certification_export().expect("export is valid");
    assert_eq!(
...
///  certify_prompt_composer_header...::tests::generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_COMPOSER_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_prompt_composer_component_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/ai/m5/m5-prompt-composer-component-certification");
    ...
    let fixtures =
        Path::new(manifest).join("../../fixtures/ai/m5/m5-prompt-composer-component-certification");
    ...
    fs::write(
        ...
        "# M5 prompt-composer component surface certification fixtures\n\n\
         Mirror of `artifacts/ai/m5/m5-prompt-composer-component-certification/`.\n\
         Regenerate with `GEN_COMPOSER_CERT_ARTIFACTS=1 cargo test -p aureline-ai \
         certify_prompt_composer_header...::tests::generate_artifacts`.\n",
    )
