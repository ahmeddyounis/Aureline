//! Headless emitter for the mutation / generated-artifact lineage
//! record.
//!
//! Reads a JSON envelope carrying the mutation / generated-artifact
//! lineage inputs (and optional inspection hooks) and prints the
//! governed, export-safe lineage record. This is the CLI / replay
//! surface that proves the same projection the workspace mutation
//! lineage status surface consumes.
//!
...
//!   "inspection_hooks": [ ...MutationAndGeneratedArtifactInspectionHook... ]
//! }
//! ```
//!
...
struct LineageEmitterEnvelope {
    ...
    inputs: MutationAndGeneratedArtifactInputs,
    #[serde(default)]
    inspection_hooks: Option<Vec<MutationAndGeneratedArtifactInspectionHook>>,
}
...
fn main() {
    ...
    for arg in std::env::args().skip(1) {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                eprintln!(
                    "usage: aureline_mutation_and_generated_artifact_lineage [--lines] [envelope.json]\n\
    ...
    let hooks = envelope
        .inspection_hooks
        .unwrap_or_else(default_mutation_and_generated_artifact_inspection_hooks);

    let record: MutationAndGeneratedArtifactLineageRecord =
        project_mutation_and_generated_artifact_lineage_with_hooks(
            envelope.posture_id,
            &envelope.inputs,
            hooks,
    ...
    if want_lines {
        for line in mutation_and_generated_artifact_lineage_lines(&record) {
            println!("{line}");
        }
    } else {
        println!(
