//! Headless emitter for the schema-migration and repair lineage
//! record.
//!
//! Reads a JSON envelope carrying the schema-migration and repair
...
//! schema-migration status surface consumes.
//!
...
//!   "inspection_hooks": [ ...SchemaMigrationInspectionHook... ]
//! }
//! ```
//!
...
struct LineageEmitterEnvelope {
    ...
    inputs: SchemaMigrationAndRepairInputs,
    #[serde(default)]
    inspection_hooks: Option<Vec<SchemaMigrationInspectionHook>>,
}
...
fn main() {
    ...
    for arg in std::env::args().skip(1) {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                eprintln!(
                    "usage: aureline_schema_migration_and_repair_lineage [--lines] [envelope.json]\n\
    ...
    let hooks = envelope
        .inspection_hooks
        .unwrap_or_else(default_schema_migration_inspection_hooks);

    let record: SchemaMigrationAndRepairLineageRecord =
        project_schema_migration_and_repair_lineage_with_hooks(
            envelope.posture_id,
            &envelope.inputs,
            hooks,
    ...
    if want_lines {
        for line in schema_migration_and_repair_lineage_lines(&record) {
            println!("{line}");
        }
    } else {
        println!(
