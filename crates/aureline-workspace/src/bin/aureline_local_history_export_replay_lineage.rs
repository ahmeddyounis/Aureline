//! Headless emitter for the local-history export/replay lineage
//! record.
//!
//! Reads a JSON envelope carrying the local-history export/replay
//! lineage inputs (and optional inspection hooks) and prints the
//! governed, export-safe lineage record. This is the CLI / replay
//! surface that proves the same projection the workspace
//! local-history status surface consumes.
//!
...
//!   "inspection_hooks": [ ...LocalHistoryExportReplayInspectionHook... ]
//! }
//! ```
//!
...
struct LineageEmitterEnvelope {
    ...
    inputs: LocalHistoryExportReplayInputs,
    #[serde(default)]
    inspection_hooks: Option<Vec<LocalHistoryExportReplayInspectionHook>>,
}
...
fn main() {
    ...
    for arg in std::env::args().skip(1) {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                eprintln!(
                    "usage: aureline_local_history_export_replay_lineage [--lines] [envelope.json]\n\
    ...
    let hooks = envelope
        .inspection_hooks
        .unwrap_or_else(default_local_history_export_replay_inspection_hooks);

    let record: LocalHistoryExportReplayLineageRecord =
    ...
    if want_lines {
        for line in local_history_export_replay_lineage_lines(&record) {
            println!("{line}");
        }
    } else {
        println!(
