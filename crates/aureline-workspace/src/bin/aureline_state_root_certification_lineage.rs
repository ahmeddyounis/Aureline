//! Headless emitter for the state-root certification lineage record.
//!
//! Reads a JSON envelope carrying the state-root certification
//! lineage inputs (and optional inspection hooks) and prints the
//! governed, export-safe lineage record. This is the CLI / replay
//! surface that proves the same projection the workspace state-root
//! audit status surface consumes.
//!
...
//!   "inspection_hooks": [ ...StateRootInspectionHook... ]
//! }
//! ```
//!
...
struct LineageEmitterEnvelope {
    ...
    inputs: StateRootCertificationInputs,
    #[serde(default)]
    inspection_hooks: Option<Vec<StateRootInspectionHook>>,
}
...
fn main() {
    ...
    for arg in std::env::args().skip(1) {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                eprintln!(
                    "usage: aureline_state_root_certification_lineage [--lines] [envelope.json]\n\
    ...
    let hooks = envelope
        .inspection_hooks
        .unwrap_or_else(default_state_root_inspection_hooks);

    let record: StateRootCertificationLineageRecord =
    ...
    if want_lines {
        for line in state_root_certification_lineage_lines(&record) {
            println!("{line}");
        }
    } else {
        println!(
