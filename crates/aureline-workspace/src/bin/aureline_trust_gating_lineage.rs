//! Headless emitter for the trust-gating lineage record.
//!
//! Reads a JSON envelope carrying the trust-gating inputs (and optional
//! inspection hooks) and prints the governed, export-safe trust-gating
//! lineage record. This is the CLI / replay surface that proves the
//! same projection the workspace trust-gating status surface consumes.
//!
...
//!   "inspection_hooks": [ ...TrustInspectionHook... ]
//! }
//! ```
//!
...
use aureline_workspace::{
    ...
    trust_gating_lineage_lines, TrustGatingInputs, TrustGatingLineageRecord, TrustInspectionHook,
};
...
struct LineageEmitterEnvelope {
    ...
    inputs: TrustGatingInputs,
    #[serde(default)]
    inspection_hooks: Option<Vec<TrustInspectionHook>>,
}
...
fn main() {
    ...
    for arg in std::env::args().skip(1) {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                eprintln!(
                    "usage: aureline_trust_gating_lineage [--lines] [envelope.json]\n\
    ...
    let hooks = envelope
        .inspection_hooks
        .unwrap_or_else(default_trust_gating_inspection_hooks);

    let record: TrustGatingLineageRecord =
        project_trust_gating_lineage_with_hooks(envelope.posture_id, &envelope.inputs, hooks);

    if want_lines {
        for line in trust_gating_lineage_lines(&record) {
            println!("{line}");
        }
    } else {
        println!(
