//! Replay gate for the trust-gating lineage record.
//!
//! Each fixture under `fixtures/workspace/m4/trust_gating_lineage/` carries
//! the posture input (a [`TrustGatingInputs`] envelope plus the inspection-
//! hook set) and the expected projected lineage record. This gate
//! re-projects each input and asserts the result equals the checked-in
//! `expected`, so the projection cannot drift from the canonical
//! checked-in record without failing CI. It also proves every fixture
//! stays support-export safe and that the corpus covers Stable plus the
//! three controlled workspace-trust postures (Trusted / Restricted /
...
use aureline_workspace::{
    project_trust_gating_lineage_with_hooks, trust_gating_lineage_lines, TrustGatingInputs,
    TrustGatingLineageRecord, TrustInspectionHook, WorkspaceTrustPosture,
...
struct LineageFixture {
    ...
    expected: TrustGatingLineageRecord,
}
...
fn projection_replays_each_fixture_exactly() {
    for (name, fixture) in load_fixtures() {
        let projected = project_trust_gating_lineage_with_hooks(
        ...
        let roundtrip: TrustGatingLineageRecord =
...
fn every_fixture_is_support_export_safe_and_well_formed() {
    for (name, fixture) in load_fixtures() {
        ...
        assert!(
            ...
            "fixture {name} must render gate decision truth"
        );
        assert!(
            lines
                .iter()
...
#[test]
fn corpus_covers_all_controlled_workspace_trust_postures() {
    let fixtures = load_fixtures();
    let observed: BTreeSet<&'static str> = fixtures
        .iter()
        .map(|(_, fixture)| fixture.expected.workspace_trust_posture.as_str())
        .collect();
    for required in [
        ...
        assert!(
            ...
            "corpus must include a {} workspace trust posture",
            required.as_str()
        );
    }
}
