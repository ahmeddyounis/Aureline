...
fn load_fixture(name: &str) -> MigrationWizardImportFidelityPacket {
    ...
    serde_json::from_str(&payload).unwrap_or_else(|err| {
        panic!("fixture {} must parse as MigrationWizardImportFidelityPacket: {err}", path.display())
    })
}
...
fn vs_code_settings_keybindings_exact_fixture_validates() {
    ...
    assert_eq!(packet.record_kind, "workspace_migration_wizard_import_fidelity_packet");
    assert_eq!(packet.schema_version, 1);
    assert_eq!(packet.fidelity_record.source_editor, "vs_code_code_oss");
    assert_eq!(packet.fidelity_record.overall_outcome, ImportOutcomeLabel::Exact);
...
fn jetbrains_partial_with_diagnostics_fixture_validates() {
    ...
    assert_eq!(packet.fidelity_record.overall_outcome, ImportOutcomeLabel::Partial);
    ...
    assert_eq!(diagnostic.suggested_action.as_deref(), Some("use_native_alternative"));
...
fn vim_modal_editing_translated_fixture_validates() {
    ...
    assert_eq!(packet.fidelity_record.overall_outcome, ImportOutcomeLabel::Translated);
...
fn projection_rejects_unknown_source_editor() {
    ...
    assert!(
        result.is_err(),
        "unknown source editor must be rejected"
    );
}
...
fn projection_rejects_unknown_import_target() {
    ...
    assert!(
        result.is_err(),
        "unknown import target must be rejected"
    );
}
...
fn projection_rejects_empty_consumer_surfaces() {
    ...
    assert!(
        result.is_err(),
        "empty consumer surfaces must be rejected"
    );
}
