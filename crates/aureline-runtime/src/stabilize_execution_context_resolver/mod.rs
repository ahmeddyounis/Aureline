//! Execution-context resolver stabilization truth packet for the M4
//! stable lane.
//!
//! This module pins how the local, remote/helper, container, and
//! managed execution-context lanes stay one boundary truth across the
//! editor run surface, terminal pane, task panel, CLI/headless
//! inspector, support export, release proof index, Help/About proof
//! card, and the conformance dashboard. The four execution lanes are
//! certified at the M4 launch-stable grade. Surfaces MUST NOT mint
//! local copies, fork their own runtime semantics, or paraphrase
//! execution-context posture; they read this packet verbatim.
//!
//! The packet refuses to certify a launch-stable lane whose
//! `execution_context_resolution_quality` row cannot prove:
//!
//! - the same execution-context fields and explanation paths cross
//!   every run-capable surface (terminal, task, test, debug,
//!   request_workspace, artifact, AI tool, CLI/headless, docs/help,
//!   support/export),
//! - requested-vs-materialized target identity is admitted with a
//!   structured target admission, not surface-local guesswork,
//! - the eight structured resolver states (wrong_target,
//!   blocked_activator, stale_capsule_or_prebuild, unsupported_skew,
//!   reconnect_required, restore_no_rerun, degraded_helper,
//!   route_drift) each have an admission row instead of generic launch
//!   failure copy,
...
//!   target,
//! - capability skew and mixed-version posture are admitted so a
//!   remote/helper attach cannot silently assume features outside the
//!   published window.
//!
//! Every row binds a closed `execution_lane_class`,
//! `execution_context_row_class`, `support_class`,
//! `surface_binding_class`, `resolver_state_class`,
//! `evidence_class`, `known_limit_class`, `downgrade_automation_class`,
//! and `execution_context_confidence_class` plus an `evidence_refs`
...
/// Stable record-kind tag for [`StabilizeExecutionContextResolverTruthPacket`].
pub const STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_PACKET_RECORD_KIND: &str =
    "stabilize_execution_context_resolver_truth_stable_packet";

/// Stable record-kind tag for [`StabilizeExecutionContextResolverTruthSupportExport`].
pub const STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_SUPPORT_EXPORT_RECORD_KIND: &str =
    "stabilize_execution_context_resolver_truth_support_export";

/// Integer schema version for the stabilize-execution-context-resolver truth packet.
pub const STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the boundary schema.
pub const STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_SCHEMA_REF: &str =
    "schemas/runtime/stabilize_execution_context_resolver_truth.schema.json";

/// Repo-relative path of the reviewer contract doc.
pub const STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_DOC_REF: &str =
    "docs/runtime/m4/stabilize-execution-context-resolver.md";

/// Repo-relative path of the human-readable reviewer artifact.
pub const STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_ARTIFACT_DOC_REF: &str =
    "artifacts/runtime/m4/stabilize-execution-context-resolver.md";

/// Repo-relative path of the protected fixture corpus directory.
pub const STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_FIXTURE_DIR: &str =
    "fixtures/runtime/m4/stabilize_execution_context_resolver";

/// Repo-relative path of the checked-in stable packet.
pub const STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_PACKET_ARTIFACT_REF: &str =
    "artifacts/runtime/m4/stabilize_execution_context_resolver_truth_packet.json";
...
/// Closed execution-context row vocabulary the packet certifies.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ExecutionContextRowClass {
    /// The lane's headline execution-context-resolution qualification row.
    ExecutionContextResolutionQuality,
    /// A row binding one run-capable surface (terminal, task, test,
    /// debug, request_workspace, artifact, AI tool, CLI/headless,
    /// docs/help, support/export) to the shared execution-context
    /// object.
    SurfaceBinding,
    /// A row admitting requested-vs-materialized target identity for
    /// the lane.
    TargetAdmission,
    /// A row admitting one of the eight structured resolver states.
    StateAdmission,
    /// A row certifying restore/reconnect honesty (metadata restore
    /// without silent rerun, reattach, or drifted-target reuse).
    RestoreRerunHonesty,
    /// A row admitting helper capability negotiation / mixed-version
    /// skew posture.
    CapabilitySkewAdmission,
...
impl ExecutionContextRowClass {
    /// Stable token used in fixtures, schemas, and support exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::ExecutionContextResolutionQuality => "execution_context_resolution_quality",
            Self::SurfaceBinding => "surface_binding",
            Self::TargetAdmission => "target_admission",
            Self::StateAdmission => "state_admission",
            Self::RestoreRerunHonesty => "restore_rerun_honesty",
            Self::CapabilitySkewAdmission => "capability_skew_admission",
    ...
    /// True when this row class requires a bound resolver-state token.
    pub const fn requires_resolver_state(self) -> bool {
        matches!(self, Self::StateAdmission)
    }
}

/// Closed support-class vocabulary applied to an execution-context row.
...
pub enum SurfaceBindingClass {
    ...
    Artifact,
    /// The row certifies the AI tool-call binding.
    AiTool,
...
/// Closed resolver-state vocabulary. Every lane claiming
/// `launch_stable` MUST publish a `state_admission` row for each
/// structured resolver state so the surface never falls back to
/// generic launch-failure copy.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ResolverStateClass {
    /// Requested target disagrees with materialized target.
    WrongTarget,
    /// Activator (env-manager, container, devcontainer build, etc.) is
    /// blocked by trust, policy, or capability.
    BlockedActivator,
    /// Cached capsule or prebuild is stale / drifted.
    StaleCapsuleOrPrebuild,
    /// Mixed-version skew falls outside the published window.
    UnsupportedSkew,
    /// Reattach / reconnect is required before the surface may dispatch.
    ReconnectRequired,
    /// Restore brought metadata back without rerunning tasks, reattaching
    /// debuggers, or reusing a drifted target.
    RestoreNoRerun,
    /// Helper / remote agent reports degraded capabilities.
    DegradedHelper,
    /// Route / tunnel posture drifted relative to the stored binding.
    RouteDrift,
    /// The row is not bound to a resolver state (non-state-admission row classes).
    NotApplicable,
}

impl ResolverStateClass {
    /// Every certified resolver state in declaration order. A
    /// `launch_stable` lane MUST cover every state.
    pub const REQUIRED_FOR_LAUNCH_STABLE: [Self; 8] = [
        Self::WrongTarget,
        Self::BlockedActivator,
        Self::StaleCapsuleOrPrebuild,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::StaleCapsuleOrPrebuild => "stale_capsule_or_prebuild",
...
/// Closed known-limit vocabulary attached to an execution-context row.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum KnownLimitClass {
    ...
    ManagedSubsetOnly,
    /// The lane only certifies a subset of the eight required resolver states.
    StateAdmissionSubsetOnly,
    /// The lane only certifies a subset of the ten required surface bindings.
    SurfaceBindingSubsetOnly,
    /// The lane only certifies a subset of the capability skew window.
    CapabilitySkewSubsetOnly,
...
impl KnownLimitClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::StateAdmissionSubsetOnly => "state_admission_subset_only",
            Self::SurfaceBindingSubsetOnly => "surface_binding_subset_only",
            Self::CapabilitySkewSubsetOnly => "capability_skew_subset_only",
...
/// Closed downgrade-automation vocabulary attached to an execution-context row.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum DowngradeAutomationClass {
    ...
    /// Automatically narrow when the environment capsule drifts.
    AutoNarrowOnCapsuleDrift,
    /// Automatically narrow when the route/tunnel posture drifts.
    AutoNarrowOnRouteDrift,
    /// Automatically narrow when any of the eight resolver states is unbound.
    AutoNarrowOnStateAdmissionGap,
...
impl DowngradeAutomationClass {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::AutoNarrowOnCapsuleDrift => "auto_narrow_on_capsule_drift",
            Self::AutoNarrowOnRouteDrift => "auto_narrow_on_route_drift",
            Self::AutoNarrowOnStateAdmissionGap => "auto_narrow_on_state_admission_gap",
...
/// Closed confidence-class vocabulary for an execution-context row.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ExecutionContextConfidenceClass {
...
impl ExecutionContextConfidenceClass {
    /// Stable token used in fixtures, schemas, and support exports.
    pub const fn as_str(self) -> &'static str {
        match self {
...
/// Closed validation-finding vocabulary for the execution-context packet.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum FindingKind {
    ...
    /// A lane claiming launch_stable is missing a required resolver state admission.
    MissingResolverStateCoverage,
    /// A lane claiming launch_stable is missing the required target admission row.
    ...
    /// A lane claiming launch_stable is missing the required capability-skew row.
    MissingCapabilitySkewAdmission,
    ...
    /// A state-admission row drops its resolver state binding.
    ResolverStateNotApplicable,
    /// A non-state-admission row binds a resolver state it cannot certify.
    ResolverStateNotPermittedOnRowClass,
    ...
    /// A consumer projection remints or drops execution-context truth.
    ...
    /// A projection collapses the resolver-state vocabulary.
    ResolverStateVocabularyCollapsed,
...
impl FindingKind {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::MissingResolverStateCoverage => "missing_resolver_state_coverage",
            Self::MissingTargetAdmission => "missing_target_admission",
            Self::MissingRestoreRerunHonesty => "missing_restore_rerun_honesty",
            Self::MissingCapabilitySkewAdmission => "missing_capability_skew_admission",
            ...
            Self::ResolverStateVocabularyCollapsed => "resolver_state_vocabulary_collapsed",
...
/// Consumer surface that must inherit the execution-context packet verbatim.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// One execution-context truth row.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StabilizeExecutionContextResolverRow {
    /// Stable row id within the packet.
    pub row_id: String,
    /// Execution-context lane this row certifies.
    pub lane_class: ExecutionLaneClass,
    /// Execution-context row class.
    pub row_class: ExecutionContextRowClass,
    ...
    /// Resolver state admitted by the row (or `not_applicable`).
    pub resolver_state_class: ResolverStateClass,
    ...
    pub confidence_class: ExecutionContextConfidenceClass,
...
impl StabilizeExecutionContextResolverRow {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StabilizeExecutionContextResolverConsumerProjection {
    /// Consumer surface class.
    pub consumer_surface: ConsumerSurface,
    /// Stable projection ref.
    pub projection_ref: String,
    /// Execution-context packet id consumed by the projection.
    pub stabilize_execution_context_resolver_packet_id_ref: String,
    ...
    /// True when the resolver-state vocabulary is preserved verbatim.
    pub preserves_resolver_state_vocabulary: bool,
...
impl StabilizeExecutionContextResolverConsumerProjection {
    fn preserves_truth_for(&self, packet_id: &str) -> bool {
        self.stabilize_execution_context_resolver_packet_id_ref == packet_id
            ...
            && self.preserves_resolver_state_vocabulary
...
/// [`StabilizeExecutionContextResolverTruthPacket::materialize`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StabilizeExecutionContextResolverTruthPacketInput {
    /// Stable packet id.
    ...
    #[serde(default)]
    pub covered_lanes: Vec<ExecutionLaneClass>,
    /// Execution-context rows.
    #[serde(default)]
    pub rows: Vec<StabilizeExecutionContextResolverRow>,
    /// Consumer projections preserving this packet.
    #[serde(default)]
    pub consumer_projections: Vec<StabilizeExecutionContextResolverConsumerProjection>,
...
/// and managed execution-context resolution at the M4 launch-stable
/// grade.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StabilizeExecutionContextResolverTruthPacket {
    /// Stable record kind.
...
impl StabilizeExecutionContextResolverTruthPacket {
    /// Materializes a packet and records derived validation findings.
    pub fn materialize(input: StabilizeExecutionContextResolverTruthPacketInput) -> Self {
        let mut packet = Self {
            record_kind: STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_PACKET_RECORD_KIND.to_owned(),
            schema_version: STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_SCHEMA_VERSION,
    ...
    /// Re-validates the packet against stable execution-context invariants.
    ...
    pub fn lane_tokens(&self) -> Vec<&'static str> {
        ...
        set.into_iter().map(ExecutionLaneClass::as_str).collect()
    }
    ...
    pub fn row_class_tokens(&self) -> Vec<&'static str> {
        ...
        set.into_iter()
            .map(ExecutionContextRowClass::as_str)
            .collect()
    }
    ...
    /// Returns the unique resolver-state tokens observed across rows.
    pub fn resolver_state_tokens(&self) -> Vec<&'static str> {
        let mut set = BTreeSet::new();
        for row in &self.rows {
            set.insert(row.resolver_state_class);
        }
        set.into_iter().map(ResolverStateClass::as_str).collect()
    }
    ...
    pub fn support_export(
        ...
        StabilizeExecutionContextResolverTruthSupportExport {
            record_kind: STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_SUPPORT_EXPORT_RECORD_KIND
                .to_owned(),
            schema_version: STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_SCHEMA_VERSION,
            export_id: export_id.into(),
            stabilize_execution_context_resolver_packet_id_ref: self.packet_id.clone(),
            exported_at: exported_at.into(),
            raw_private_material_excluded: true,
            ambient_authority_excluded: true,
            stabilize_execution_context_resolver_packet: self.clone(),
        }
    }
    ...
    fn derived_findings(&self, include_record_fields: bool) -> Vec<ValidationFinding> {
        ...
        if include_record_fields
            && self.record_kind != STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_PACKET_RECORD_KIND
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongRecordKind,
                FindingSeverity::Blocker,
                "stabilize-execution-context packet has the wrong record kind",
            ));
        }
        if include_record_fields
            && self.schema_version != STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_SCHEMA_VERSION
        {
            findings.push(ValidationFinding::new(
                FindingKind::WrongSchemaVersion,
                FindingSeverity::Blocker,
                "stabilize-execution-context packet has the wrong schema version",
            ));
        }
        ...
        for row in &self.rows {
            ...
            if row.row_class.requires_resolver_state()
                && matches!(row.resolver_state_class, ResolverStateClass::NotApplicable)
            {
                findings.push(ValidationFinding::new(
                    FindingKind::ResolverStateNotApplicable,
                    FindingSeverity::Blocker,
                    format!(
                        "row {} is a state_admission but has no bound resolver state",
                        row.row_id
                    ),
                ));
            }
            ...
            {
                findings.push(ValidationFinding::new(
                    FindingKind::ResolverStateNotPermittedOnRowClass,
                    FindingSeverity::Blocker,
                    format!(
                        "row {} has row class {} but binds resolver state {}; only state_admission rows may bind a state",
                        row.row_id,
                        row.row_class.as_str(),
                        row.resolver_state_class.as_str()
                    ),
                ));
            }

            if matches!(row.row_class, ExecutionContextRowClass::LineageAdmission)
                && row
            ...
            if matches!(row.row_class, ExecutionContextRowClass::RestoreRerunHonesty)
            ...
            if matches!(
                ...
                ExecutionContextConfidenceClass::LowConfidence
        ...
        for lane in &self.covered_lanes {
            let lane_claims_launch = self.rows.iter().any(|row| {
                row.lane_class == *lane
                    && matches!(
                        row.row_class,
                        ExecutionContextRowClass::ExecutionContextResolutionQuality
                    )
                    && matches!(row.support_class, SupportClass::LaunchStable)
            });
            ...
            for surface in SurfaceBindingClass::REQUIRED_FOR_LAUNCH_STABLE {
                let covered = self.rows.iter().any(|row| {
                    row.lane_class == *lane
                        && matches!(row.row_class, ExecutionContextRowClass::SurfaceBinding)
            ...
            for state in ResolverStateClass::REQUIRED_FOR_LAUNCH_STABLE {
                let covered = self.rows.iter().any(|row| {
                    row.lane_class == *lane
                        && matches!(row.row_class, ExecutionContextRowClass::StateAdmission)
                        && row.resolver_state_class == state
                });
                if !covered {
                    findings.push(ValidationFinding::new(
                        FindingKind::MissingResolverStateCoverage,
                        FindingSeverity::Blocker,
                        format!(
                            "lane {} claims launch_stable but has no state_admission row for {}",
                            lane.as_str(),
                            state.as_str()
                        ),
                    ));
            ...
            let has_target_admission = self.rows.iter().any(|row| {
                row.lane_class == *lane
                    && matches!(row.row_class, ExecutionContextRowClass::TargetAdmission)
            });
            ...
            let has_restore_rerun = self.rows.iter().any(|row| {
                row.lane_class == *lane
                    && matches!(row.row_class, ExecutionContextRowClass::RestoreRerunHonesty)
                    && row.restore_preserves_no_rerun
            });
            ...
            let has_capability_skew = self.rows.iter().any(|row| {
                row.lane_class == *lane
                    && matches!(
                        row.row_class,
                        ExecutionContextRowClass::CapabilitySkewAdmission
                    )
            });
            if !has_capability_skew {
                findings.push(ValidationFinding::new(
                    FindingKind::MissingCapabilitySkewAdmission,
                    FindingSeverity::Blocker,
                    format!(
                        "lane {} claims launch_stable but has no capability_skew_admission row",
                        lane.as_str()
                    ),
                ));
            }
            ...
            let has_lineage = self.rows.iter().any(|row| {
                row.lane_class == *lane
                    && matches!(row.row_class, ExecutionContextRowClass::LineageAdmission)
                    && row
                        .execution_context_id_binding
                        .as_deref()
                        .map(str::trim)
        ...
        for projection in &self.consumer_projections {
            if !projection.preserves_truth_for(&self.packet_id) {
                findings.push(ValidationFinding::new(
                    ...
                    FindingSeverity::Blocker,
                    format!(
                        "projection {} does not preserve execution-context truth",
            ...
            if !projection.preserves_resolver_state_vocabulary {
                findings.push(ValidationFinding::new(
                    FindingKind::ResolverStateVocabularyCollapsed,
                    FindingSeverity::Blocker,
                    format!(
                        "projection {} collapses the resolver-state vocabulary",
                        projection.projection_ref
                    ),
                ));
            }
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct StabilizeExecutionContextResolverTruthSupportExport {
    /// Stable record kind.
    pub record_kind: String,
    /// Schema version.
    pub schema_version: u32,
    ...
    pub stabilize_execution_context_resolver_packet: StabilizeExecutionContextResolverTruthPacket,
}

impl StabilizeExecutionContextResolverTruthSupportExport {
    /// Returns true when the export preserves the same packet id safely.
    pub fn is_export_safe(&self) -> bool {
        self.record_kind == STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_SUPPORT_EXPORT_RECORD_KIND
            && self.schema_version == STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_SCHEMA_VERSION
            && self.stabilize_execution_context_resolver_packet_id_ref
                == self.stabilize_execution_context_resolver_packet.packet_id
            && self.raw_private_material_excluded
            && self.ambient_authority_excluded
            && self
                .stabilize_execution_context_resolver_packet
                .validate()
                .is_empty()
    }
}

/// Errors emitted when reading the checked-in stable execution-context packet.
#[derive(Debug)]
pub enum StabilizeExecutionContextResolverTruthArtifactError {
...
impl fmt::Display for StabilizeExecutionContextResolverTruthArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Packet(error) => write!(
                formatter,
                "stabilize-execution-context packet parse failed: {error}"
            ),
            Self::Validation(findings) => {
                ...
                write!(
                    formatter,
                    "stabilize-execution-context packet failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for StabilizeExecutionContextResolverTruthArtifactError {}

/// Returns the checked-in stable execution-context truth packet.
///
/// # Errors
///
...
) -> Result<StabilizeExecutionContextResolverTruthPacket, StabilizeExecutionContextResolverTruthArtifactError>
{
    let packet: StabilizeExecutionContextResolverTruthPacket =
        ...
        .map_err(StabilizeExecutionContextResolverTruthArtifactError::Packet)?;
    let findings = packet.validate();
    if findings.is_empty() {
        Ok(packet)
    } else {
        Err(StabilizeExecutionContextResolverTruthArtifactError::Validation(findings))
    }
}
...
mod tests {
    ...
    fn doc_ref() -> String {
        STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_DOC_REF.to_owned()
    }

    fn fixture_ref() -> String {
        STABILIZE_EXECUTION_CONTEXT_RESOLVER_TRUTH_FIXTURE_DIR.to_owned()
    }

    fn quality_row(prefix: &str, lane: ExecutionLaneClass) -> StabilizeExecutionContextResolverRow {
        StabilizeExecutionContextResolverRow {
            row_id: format!("row:{prefix}:quality"),
            lane_class: lane,
            row_class: ExecutionContextRowClass::ExecutionContextResolutionQuality,
            support_class: SupportClass::LaunchStable,
            surface_binding_class: SurfaceBindingClass::NotApplicable,
            resolver_state_class: ResolverStateClass::NotApplicable,
            ...
            confidence_class: ExecutionContextConfidenceClass::HighConfidence,
    ...
    fn surface_row(
        ...
        StabilizeExecutionContextResolverRow {
            ...
            lane_class: lane,
            row_class: ExecutionContextRowClass::SurfaceBinding,
            support_class: SupportClass::LaunchStable,
    ...
    fn state_row(
        prefix: &str,
        lane: ExecutionLaneClass,
        state: ResolverStateClass,
    ) -> StabilizeExecutionContextResolverRow {
        StabilizeExecutionContextResolverRow {
            row_id: format!("row:{prefix}:state:{}", state.as_str()),
            lane_class: lane,
            row_class: ExecutionContextRowClass::StateAdmission,
            support_class: SupportClass::LaunchStable,
            surface_binding_class: SurfaceBindingClass::NotApplicable,
            resolver_state_class: state,
            evidence_class: EvidenceClass::FailureRecoveryDrillEvidence,
            known_limit_class: KnownLimitClass::NoneDeclared,
            downgrade_automation_class: DowngradeAutomationClass::AutoNarrowOnStateAdmissionGap,
            confidence_class: ExecutionContextConfidenceClass::HighConfidence,
            evidence_refs: vec![fixture_ref()],
            disclosure_ref: Some(format!("{}#auto_narrow_on_state_admission_gap", doc_ref())),
            execution_context_id_binding: None,
            restore_preserves_no_rerun: false,
            raw_source_material_excluded: true,
            secrets_excluded: true,
    ...
    fn target_admission_row(
        ...
        StabilizeExecutionContextResolverRow {
            ...
            lane_class: lane,
            row_class: ExecutionContextRowClass::TargetAdmission,
            support_class: SupportClass::LaunchStable,
            surface_binding_class: SurfaceBindingClass::NotApplicable,
            resolver_state_class: ResolverStateClass::NotApplicable,
    ...
    fn restore_rerun_row(
        ...
        StabilizeExecutionContextResolverRow {
            ...
            lane_class: lane,
            row_class: ExecutionContextRowClass::RestoreRerunHonesty,
            support_class: SupportClass::LaunchStable,
            surface_binding_class: SurfaceBindingClass::NotApplicable,
            resolver_state_class: ResolverStateClass::NotApplicable,
            evidence_class: EvidenceClass::FailureRecoveryDrillEvidence,
    ...
    fn capability_skew_row(
        prefix: &str,
        lane: ExecutionLaneClass,
    ) -> StabilizeExecutionContextResolverRow {
        StabilizeExecutionContextResolverRow {
            row_id: format!("row:{prefix}:capability_skew_admission"),
            lane_class: lane,
            row_class: ExecutionContextRowClass::CapabilitySkewAdmission,
            support_class: SupportClass::LaunchStable,
            surface_binding_class: SurfaceBindingClass::NotApplicable,
            resolver_state_class: ResolverStateClass::NotApplicable,
            evidence_class: EvidenceClass::ConformanceSuiteEvidence,
            known_limit_class: KnownLimitClass::NoneDeclared,
            downgrade_automation_class: DowngradeAutomationClass::AutoNarrowOnHelperSkew,
            confidence_class: ExecutionContextConfidenceClass::HighConfidence,
            evidence_refs: vec![fixture_ref()],
            disclosure_ref: Some(format!("{}#auto_narrow_on_helper_skew", doc_ref())),
            execution_context_id_binding: None,
            restore_preserves_no_rerun: false,
            raw_source_material_excluded: true,
            secrets_excluded: true,
    ...
    fn lineage_row(
        ...
        StabilizeExecutionContextResolverRow {
            ...
            lane_class: lane,
            row_class: ExecutionContextRowClass::LineageAdmission,
            support_class: SupportClass::LaunchStable,
            surface_binding_class: SurfaceBindingClass::NotApplicable,
            resolver_state_class: ResolverStateClass::NotApplicable,
            evidence_class: EvidenceClass::AutomatedFunctionalEvidence,
    ...
    fn projection(
        ...
        StabilizeExecutionContextResolverConsumerProjection {
            ...
            stabilize_execution_context_resolver_packet_id_ref:
                "packet:m4:stabilize_execution_context_resolver".to_owned(),
            ...
            preserves_resolver_state_vocabulary: true,
    ...
    fn lane_rows(
        lane: ExecutionLaneClass,
        prefix: &str,
    ) -> Vec<StabilizeExecutionContextResolverRow> {
        let mut out = vec![quality_row(prefix, lane)];
        for surface in SurfaceBindingClass::REQUIRED_FOR_LAUNCH_STABLE {
            out.push(surface_row(prefix, lane, surface));
        }
        for state in ResolverStateClass::REQUIRED_FOR_LAUNCH_STABLE {
            out.push(state_row(prefix, lane, state));
        }
        out.push(target_admission_row(prefix, lane));
        out.push(restore_rerun_row(prefix, lane));
        out.push(capability_skew_row(prefix, lane));
        out.push(lineage_row(prefix, lane));
        out
    }

    fn sample_input() -> StabilizeExecutionContextResolverTruthPacketInput {
        let mut rows = Vec::new();
        rows.extend(lane_rows(ExecutionLaneClass::LocalLane, "local"));
        rows.extend(lane_rows(ExecutionLaneClass::RemoteHelperLane, "remote"));
        rows.extend(lane_rows(ExecutionLaneClass::ContainerLane, "container"));
        rows.extend(lane_rows(ExecutionLaneClass::ManagedLane, "managed"));
        StabilizeExecutionContextResolverTruthPacketInput {
            packet_id: "packet:m4:stabilize_execution_context_resolver".to_owned(),
            workflow_or_surface_id: "workflow.runtime.stabilize_execution_context_resolver"
                .to_owned(),
            generated_at: "2026-05-26T12:00:00Z".to_owned(),
            covered_lanes: ExecutionLaneClass::REQUIRED.to_vec(),
            rows,
    ...
    fn closed_tokens_are_pinned() {
        assert_eq!(ExecutionLaneClass::LocalLane.as_str(), "local_lane");
        assert_eq!(
            ExecutionLaneClass::RemoteHelperLane.as_str(),
            "remote_helper_lane"
        );
        assert_eq!(ExecutionLaneClass::ContainerLane.as_str(), "container_lane");
        assert_eq!(ExecutionLaneClass::ManagedLane.as_str(), "managed_lane");
        assert_eq!(
            ExecutionContextRowClass::ExecutionContextResolutionQuality.as_str(),
            "execution_context_resolution_quality"
        );
        ...
        assert_eq!(
            FindingKind::RestoreRerunHonestyAdmitsSilentRerun.as_str(),
            "restore_rerun_honesty_admits_silent_rerun"
        );
    }
    ...
    fn baseline_materialization_is_stable() {
        ...
        assert!(packet
            .support_export(
                "support:m4:stabilize_execution_context_resolver",
    ...
    #[test]
    fn missing_surface_binding_for_launch_stable_blocks() {
        let mut input = sample_input();
        input.rows.retain(|row| {
            !(matches!(row.row_class, ExecutionContextRowClass::SurfaceBinding)
                && row.surface_binding_class == SurfaceBindingClass::SupportExport
                && row.lane_class == ExecutionLaneClass::LocalLane)
        });
        let packet = StabilizeExecutionContextResolverTruthPacket::materialize(input);
        assert_eq!(packet.promotion_state, PromotionState::BlocksStable);
        assert!(packet
            .validation_findings
            .iter()
            .any(|finding| finding.finding_kind == FindingKind::MissingSurfaceBindingCoverage));
    }

    #[test]
    fn missing_state_admission_for_launch_stable_blocks() {
        let mut input = sample_input();
        input.rows.retain(|row| {
            !(matches!(row.row_class, ExecutionContextRowClass::StateAdmission)
                && row.resolver_state_class == ResolverStateClass::RestoreNoRerun
                && row.lane_class == ExecutionLaneClass::LocalLane)
        });
        let packet = StabilizeExecutionContextResolverTruthPacket::materialize(input);
        assert_eq!(packet.promotion_state, PromotionState::BlocksStable);
        assert!(packet
            .validation_findings
            .iter()
            .any(|finding| finding.finding_kind == FindingKind::MissingResolverStateCoverage));
    }
    ...
    #[test]
    fn restore_rerun_admitting_silent_rerun_blocks() {
        let mut input = sample_input();
        for row in &mut input.rows {
            if matches!(row.row_class, ExecutionContextRowClass::RestoreRerunHonesty)
                && row.lane_class == ExecutionLaneClass::LocalLane
        ...
        assert_eq!(packet.promotion_state, PromotionState::BlocksStable);
        assert!(packet.validation_findings.iter().any(|finding| {
            finding.finding_kind == FindingKind::RestoreRerunHonestyAdmitsSilentRerun
        }));
    }
    ...
    fn collapsed_resolver_state_vocabulary_blocks() {
        ...
        assert!(packet
            .validation_findings
            .iter()
            .any(|finding| finding.finding_kind == FindingKind::ResolverStateVocabularyCollapsed));
    }
