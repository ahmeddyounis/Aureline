//! Tests for the M05-1035 test-intelligence component surface certification capstone.

use super::*;

fn packet() -> TestIntelSurfaceCertificationPacket {
    seeded_m5_test_intelligence_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, TEST_INTEL_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, TEST_INTEL_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5TestIntelligenceCertifiedSurface::ALL {
...
#[test]
fn every_surface_preserves_evidence_continuity() {
    let p = packet();
    for row in &p.rows {
        assert!(
            row.preserves_evidence_continuity(),
            "row {} drops evidence continuity",
            row.row_id
        );
        assert!(row.evidence_continuity_preserved);
    }
    assert!(p.summary.all_evidence_continuity_preserved);
}
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            TestIntelCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, TestIntelAxisCertificationState::Certified);
...
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, TEST_INTEL_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(
...
fn yellow_rows_narrow_their_claim_and_bind_to_a_narrowed_axis() {
        ...
        .filter(|r| r.derived_status == TestIntelSurfaceClaimStatus::Yellow)
    {
        ...
        assert!(narrow.preserves_evidence_continuity);
    }
...
fn the_five_spec_auto_narrow_conditions_are_each_certified() {
    // The five spec narrowing conditions: imported / stale provenance, partial branch / condition
    // coverage, an insufficient flaky evidence window, an unverified snapshot baseline, and an
    // unproven sandbox validation — must each be a certified yellow surface.
    let p = packet();
    let certified: BTreeSet<M5IntelComponentClaim> = p
        .rows
        .iter()
        .filter(|r| r.derived_status == TestIntelSurfaceClaimStatus::Yellow)
        .map(|r| r.certified_claim)
        .collect();
    assert!(certified.contains(&M5IntelComponentClaim::ImportedOrStaleEvidence));
    assert!(certified.contains(&M5IntelComponentClaim::PartialConditionEvidence));
    assert!(certified.contains(&M5IntelComponentClaim::UnconfirmedFlakyEvidence));
    assert!(certified.contains(&M5IntelComponentClaim::UnverifiedBaselineEvidence));
    assert!(certified.contains(&M5IntelComponentClaim::UnvalidatedGeneratedEvidence));
}

#[test]
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5TestIntelligenceCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5TestIntelligenceCertifiedSurface::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = TestIntelCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), TestIntelCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in TestIntelCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == TestIntelCertificationAxis::CliExport
        );
...
#[test]
fn evidence_claim_ladder_is_strictly_ordered() {
    let ranks: Vec<u8> = M5IntelComponentClaim::ALL
        .iter()
        .map(|c| c.capability_rank())
        .collect();
    assert_eq!(ranks, vec![6, 5, 4, 3, 2, 1, 0]);
    let tokens: BTreeSet<&str> = M5IntelComponentClaim::ALL
        .iter()
        .map(|c| c.as_str())
        .collect();
    assert_eq!(tokens.len(), M5IntelComponentClaim::ALL.len());
}
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5TestIntelligenceCertifiedSurface::EditorGutterOverlay)
        .expect("editor-gutter-overlay row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == TestIntelCertificationAxis::Visual {
            outcome.state = TestIntelAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason =
                Some("line-versus-branch dimension silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, TestIntelSurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, TestIntelCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5TestIntelligenceCertifiedSurface::CliExport)
        .expect("cli-export row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == TestIntelCertificationAxis::DegradedState {
            *outcome = TestIntelCertAxisOutcome {
                axis: TestIntelCertificationAxis::DegradedState,
                state: TestIntelAxisCertificationState::DisclosedNarrowed,
                parity_note: "imported evidence on this surface".to_owned(),
                narrowing_reason: Some(
                    "the merged coverage was imported rather than a fresh local run".to_owned(),
                ),
    ...
    // Claim stays ReviewableEvidence == certified ReviewableEvidence, no auto-narrow.
    assert_eq!(row.derive_status(), TestIntelSurfaceClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == TestIntelCertificationAxis::CliExport {
            outcome.state = TestIntelAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5TestIntelligenceDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), TestIntelSurfaceClaimStatus::Red);
}
...
#[test]
fn dropped_evidence_continuity_blocks_the_surface() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.evidence_continuity_preserved = false;
    assert!(!row.preserves_evidence_continuity());
    assert_eq!(row.derive_status(), TestIntelSurfaceClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        TestIntelCertificationViolation::EvidenceContinuityDropped { .. }
    )));
}

#[test]
fn narrowed_row_dropping_continuity_blocks() {
    let mut p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == TestIntelSurfaceClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.preserves_evidence_continuity = false;
    }
    assert!(!row.preserves_evidence_continuity());
    assert_eq!(row.derive_status(), TestIntelSurfaceClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(TestIntelClaimAutoNarrow {
        binding_axis: TestIntelCertificationAxis::DegradedState,
        from_claim: M5IntelComponentClaim::VerifiedCurrentEvidence,
        to_claim: M5IntelComponentClaim::VerifiedCurrentEvidence,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5IntelComponentClaim::ImportedOrStaleEvidence;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("cli-export row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableEvidence
    row.certified_claim = M5IntelComponentClaim::VerifiedCurrentEvidence;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5TestIntelligenceCertifiedSurface::CoverageReportView)
        .expect("coverage-report-view row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = TestIntelCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), TestIntelSurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == TestIntelCertificationAxis::CliExport {
            outcome.state = TestIntelAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the coverage-report view".to_owned());
            outcome.downgrade_trigger = Some(M5TestIntelligenceDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = TestIntelCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), TestIntelSurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(TestIntelCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5TestIntelligenceCertifiedSurface::CliExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        TestIntelCertificationViolation::SurfaceCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the CoverageOverlayMarker family from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5TestIntelligenceComponentFamily::CoverageOverlayMarker);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, TestIntelCertificationViolation::FamilyCoverageIncomplete)));
}

#[test]
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = TestIntelSurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        TestIntelCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        TestIntelCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, TestIntelCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, TestIntelCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != TestIntelCertificationAxis::EvidenceProvenanceAndAssumptionBoundary);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        TestIntelCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, TestIntelCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, TestIntelCertificationViolation::RawTestMaterialInExport)));
}
...
fn export_json_round_trips() {
    ...
    let back: TestIntelSurfaceCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_test_intelligence_component_certification_export().expect("export is valid");
    assert_eq!(
...
///  certify_coverage_summary_bar...::tests::generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_TEST_INTEL_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_test_intelligence_component_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-test-intelligence-component-certification");
    ...
    let fixtures =
        Path::new(manifest).join("../../fixtures/ui/m5-test-intelligence-component-certification");
