...
use aureline_editor::{
    orientation_aids_stability_corpus, OrientationAidsStabilityPacket,
};
...
fn every_fixture_is_contract_valid() {
    ...
    for scenario in &corpus {
        ...
        assert_eq!(
            packet.orientation_aid_state.surface_class,
            scenario.expected_surface_class,
        ...
        assert_eq!(
            packet.orientation_aid_state.multi_cursor.caret_count,
            scenario.expected_caret_count,
...
fn multi_cursor_count_is_visible_for_every_scenario() {
    ...
    for scenario in &corpus {
        ...
        assert!(
            packet.orientation_aid_state.multi_cursor_attribution_is_visible(),
...
fn fold_summaries_preserve_hidden_critical_state() {
    ...
    for scenario in &corpus {
        let packet = scenario.packet();
        assert!(
            packet.orientation_aid_state.fold_summaries_preserve_hidden_state(),
...
fn breadcrumbs_preserve_continuity() {
    ...
    for scenario in &corpus {
        let packet = scenario.packet();
        assert!(
            packet.orientation_aid_state.breadcrumb_preserves_continuity(),
...
fn degraded_aids_have_alternate_paths() {
    ...
    for scenario in &corpus {
        let packet = scenario.packet();
        assert!(
            packet.orientation_aid_state.degraded_aids_have_alternate_paths(),
...
fn marker_vocabulary_is_consistent() {
    ...
    for scenario in &corpus {
        let packet = scenario.packet();
        assert!(
            packet.orientation_aid_state.marker_vocabulary_is_consistent(),
...
fn degraded_mode_labeling_is_explicit() {
    ...
    for scenario in &corpus {
        let packet = scenario.packet();
        assert!(
            packet.orientation_aid_state.degraded_mode_labeling_is_explicit(),
...
fn load_manifest() -> Manifest {
    let path = repo_root()
        .join("fixtures/editor/m4/stabilize-orientation-aids-breadcrumbs-folds-minimap/manifest.json");
    let raw = fs::read_to_string(&path).unwrap_or_else(|err| panic!("read {path:?}: {err}"));
    serde_json::from_str(&raw).unwrap_or_else(|err| panic!("parse {path:?}: {err}"))
}
