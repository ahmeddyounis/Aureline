//! Tests for the M05-1123 editor-inline component surface certification capstone.

use super::*;

fn packet() -> EditorInlineProfileCertificationPacket {
    seeded_m5_editor_inline_component_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, EDITOR_INLINE_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, EDITOR_INLINE_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5EditorInlineCertifiedProfile::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            EditorInlineCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, EditorInlineAxisCertificationState::Certified);
...
#[test]
fn only_the_live_profile_certifies_a_trusted_result_claim() {
    for row in &packet().rows {
...
fn a_narrowed_profile_never_keeps_a_trusted_claim() {
    for row in &packet().rows {
        if row.is_claim_narrowed() {
            assert!(
                !row.certified_claim.asserts_trusted_inline_result(),
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5EditorInlineCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5EditorInlineCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = EditorInlineCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), EditorInlineCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in EditorInlineCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == EditorInlineCertificationAxis::CliExport
        );
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5EditorInlineCertifiedProfile::LiveTrustedInlineSurface)
        .expect("live-trusted-inline-surface row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EditorInlineCertificationAxis::Visual {
            outcome.state = EditorInlineAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("inline state silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, EditorInlineProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, EditorInlineCertificationViolation::ProfileBlocked { .. })));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5EditorInlineCertifiedProfile::ReviewableInlineStructure)
        .expect("reviewable-inline-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails.encodes_state_by_color_alone = true;
    assert_eq!(row.derive_status(), EditorInlineProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        EditorInlineCertificationViolation::GuardrailViolated { .. }
    )));
}

#[test]
fn non_live_profile_claiming_trusted_result_blocks() {
    let mut p = packet();
    let idx = p
        .rows
        .iter()
        ...
        .expect("reviewable-inline-structure row exists");
    let row = &mut p.rows[idx];
    // Claim a trusted result on a reviewable profile — the claimed ceiling is raised too so the
    // block is the non-live rule, not a certified-exceeds-claim strengthening.
    row.claimed_claim = M5EditorInlineComponentClaim::TrustedInlineResult;
    row.certified_claim = M5EditorInlineComponentClaim::TrustedInlineResult;
    assert_eq!(row.derive_status(), EditorInlineProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EditorInlineCertificationAxis::DegradedState {
            *outcome = EditorInlineAxisOutcome {
                axis: EditorInlineCertificationAxis::DegradedState,
                state: EditorInlineAxisCertificationState::DisclosedNarrowed,
                parity_note: "the anchor durability is lagging".to_owned(),
                narrowing_reason: Some(
                    "the anchor durability is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5EditorInlineDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableInlineResult == certified, no claim_auto_narrow.
    assert_eq!(row.derive_status(), EditorInlineProfileClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EditorInlineCertificationAxis::CliExport {
            outcome.state = EditorInlineAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5EditorInlineDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), EditorInlineProfileClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(EditorInlineClaimAutoNarrow {
        binding_axis: EditorInlineCertificationAxis::DegradedState,
        from_claim: M5EditorInlineComponentClaim::TrustedInlineResult,
        to_claim: M5EditorInlineComponentClaim::TrustedInlineResult,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5EditorInlineComponentClaim::AnchorUnverifiedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-inline-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableInlineResult; certify a stronger trusted result.
    row.certified_claim = M5EditorInlineComponentClaim::TrustedInlineResult;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5EditorInlineCertifiedProfile::DriftedAnchorSurface)
        .expect("drifted-anchor-surface row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = EditorInlineCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), EditorInlineProfileClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EditorInlineCertificationAxis::CliExport {
            outcome.state = EditorInlineAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the drifted anchor".to_owned());
            outcome.downgrade_trigger = Some(M5EditorInlineDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = EditorInlineCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), EditorInlineProfileClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == EditorInlineProfileClaimStatus::Yellow)
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(EditorInlineCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5EditorInlineDowngradeTrigger::ProofStale,
    );
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.profile != M5EditorInlineCertifiedProfile::PartialEvidenceTimeline);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        EditorInlineCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the diagnostic-decoration family from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5EditorInlineComponentFamily::DiagnosticDecoration);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        EditorInlineCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = EditorInlineProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        EditorInlineCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, EditorInlineCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, EditorInlineCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != EditorInlineCertificationAxis::InlineComponentTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        EditorInlineCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, EditorInlineCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        EditorInlineCertificationViolation::RawInlineMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: EditorInlineProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_editor_inline_component_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn generate_artifacts() {
    if std::env::var("GEN_EDITOR_INLINE_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_editor_inline_component_surface_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-editor-inline-component-surface-certification-proof");
    ...
    let fixtures = Path::new(manifest)
        .join("../../fixtures/ui/m5-editor-inline-component-surface-certification");
    ...
    fs::write(
        ...
        "# M5 editor-inline component surface certification fixtures\n\n\
         Mirror of `artifacts/release/m5-editor-inline-component-surface-certification-proof/`.\n\
         Regenerate with `GEN_EDITOR_INLINE_CERT_ARTIFACTS=1 cargo test -p aureline-editor \
         m5_editor_inline_component_surface_certification::tests::generate_artifacts`.\n",
    )
