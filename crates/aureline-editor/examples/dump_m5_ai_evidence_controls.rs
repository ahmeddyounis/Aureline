//! Headless emitter for the M5 AI-message-card / evidence-timeline controls packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-ai-message-card-evidence-timeline-controls-proof/`, its matrix CSV, the
...
//! cargo run -p aureline-editor --example dump_m5_ai_evidence_controls -- validate
//! ```

use aureline_editor::m5_ai_message_card_and_evidence_timeline_source_confidence_and_evidence_lineage::{
    seeded_m5_ai_evidence_controls, seeded_m5_ai_evidence_controls_ai_ui_beta_narrowed,
    seeded_m5_ai_evidence_controls_support_export_preview_narrowed, M5AiEvidenceControlsPacket,
};
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("report") => {
            print!(
                "{}",
                seeded_m5_ai_evidence_controls().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!("{}", seeded_m5_ai_evidence_controls().render_matrix_csv());
        }
        Some("fixture-ai-ui-beta-narrowed") => {
            let packet = seeded_m5_ai_evidence_controls_ai_ui_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-support-export-preview-narrowed") => {
            let packet = seeded_m5_ai_evidence_controls_support_export_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("validate") => {
            for packet in [
                ...
                seeded_m5_ai_evidence_controls_ai_ui_beta_narrowed(),
...
fn assert_valid(packet: &M5AiEvidenceControlsPacket) -> Result<(), Box<dyn std::error::Error>> {
