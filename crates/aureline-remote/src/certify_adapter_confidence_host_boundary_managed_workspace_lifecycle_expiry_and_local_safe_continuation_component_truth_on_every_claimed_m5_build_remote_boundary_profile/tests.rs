//! Tests for the M05-1083 build/remote-boundary component profile certification
//! capstone.

use super::*;

fn packet() -> BuildRemoteProfileCertificationPacket {
    seeded_m5_build_remote_boundary_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, BUILD_REMOTE_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, BUILD_REMOTE_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5BuildRemoteCertifiedProfile::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            BuildRemoteCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, BuildRemoteAxisCertificationState::Certified);
...
#[test]
fn every_row_holds_all_three_guardrails() {
    let p = packet();
    for row in &p.rows {
        assert!(
...
fn every_row_cites_the_one_canonical_bundle() {
    ...
    assert_eq!(
        ...
        BUILD_REMOTE_CERT_CANONICAL_BUNDLE_REF
    );
    for row in &p.rows {
        assert_eq!(
...
#[test]
fn only_local_profile_certifies_a_live_full_truth_claim() {
    for row in &packet().rows {
        if row.certified_claim.asserts_live_truth() {
            assert!(
                row.profile.is_local_first_party(),
                "non-local profile {} certifies a live full-truth claim",
                row.row_id
            );
        }
    }
...
#[test]
fn a_stale_unverified_or_unsupported_profile_never_keeps_a_fresh_first_party_claim() {
    // Every narrowed row narrows away from FullTruth (the only claim that asserts live,
    // current, fresh first-party-local truth). No profile keeps FullTruth while degraded.
    for row in &packet().rows {
        if row.is_claim_narrowed() {
            assert!(
                !row.certified_claim.asserts_live_truth(),
                "narrowed row {} still certifies a fresh first-party live claim",
                row.row_id
            );
        }
    }
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5BuildRemoteCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5BuildRemoteCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = BuildRemoteCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), BuildRemoteCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in BuildRemoteCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == BuildRemoteCertificationAxis::CliExport
        );
...
#[test]
fn only_local_execution_is_first_party() {
    for profile in M5BuildRemoteCertifiedProfile::ALL {
        assert_eq!(
            profile.is_local_first_party(),
            profile == M5BuildRemoteCertifiedProfile::LocalExecution
        );
    }
}
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5BuildRemoteCertifiedProfile::LocalExecution)
        .expect("local-execution row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BuildRemoteCertificationAxis::Visual {
            outcome.state = BuildRemoteAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("host ownership silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, BuildRemoteProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildRemoteCertificationViolation::ProfileBlocked { .. })));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5BuildRemoteCertifiedProfile::RebuildRecreate)
        .expect("rebuild-recreate row exists");
    let row = &mut p.rows[idx];
    row.guardrails
        .implies_exact_continuity_after_material_change = true;
    assert_eq!(row.derive_status(), BuildRemoteProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteCertificationViolation::GuardrailViolated { .. }
    )));
}
...
fn non_local_profile_claiming_live_truth_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5BuildRemoteCertifiedProfile::SshExecution)
        .expect("ssh-execution row exists");
    let row = &mut p.rows[idx];
    // Claim a live first-party full-truth on a non-local profile.
    row.claimed_claim = M5BuildRemoteAccessClaim::FullTruth;
    row.certified_claim = M5BuildRemoteAccessClaim::FullTruth;
    assert_eq!(row.derive_status(), BuildRemoteProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5BuildRemoteCertifiedProfile::ContainerExecution)
        .expect("container-execution row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BuildRemoteCertificationAxis::DegradedState {
            *outcome = BuildRemoteAxisOutcome {
                axis: BuildRemoteCertificationAxis::DegradedState,
                state: BuildRemoteAxisCertificationState::DisclosedNarrowed,
                parity_note: "the adapter reading is lagging".to_owned(),
                narrowing_reason: Some("the live proof is partial for this profile".to_owned()),
                downgrade_trigger: Some(M5BuildRemoteDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ResolvedTruth == certified ResolvedTruth, no claim_auto_narrow.
    assert_eq!(row.derive_status(), BuildRemoteProfileClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BuildRemoteCertificationAxis::CliExport {
            outcome.state = BuildRemoteAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5BuildRemoteDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), BuildRemoteProfileClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(BuildRemoteClaimAutoNarrow {
        binding_axis: BuildRemoteCertificationAxis::DegradedState,
        from_claim: M5BuildRemoteAccessClaim::FullTruth,
        to_claim: M5BuildRemoteAccessClaim::FullTruth,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5BuildRemoteAccessClaim::Degraded;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("container-execution row exists");
    let row = &mut p.rows[idx];
    row.certified_claim = M5BuildRemoteAccessClaim::FullTruth; // claimed is ResolvedTruth
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5BuildRemoteCertifiedProfile::ManagedWorkspace)
        .expect("managed-workspace row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = BuildRemoteCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), BuildRemoteProfileClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BuildRemoteCertificationAxis::CliExport {
            outcome.state = BuildRemoteAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the workspace".to_owned());
            outcome.downgrade_trigger = Some(M5BuildRemoteDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = BuildRemoteCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), BuildRemoteProfileClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == BuildRemoteProfileClaimStatus::Yellow)
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(BuildRemoteCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5BuildRemoteDowngradeTrigger::ProofStale,
    );
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.profile != M5BuildRemoteCertifiedProfile::ExpiryLocalSafe);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the local-safe-continuation-card family from every row that carries it;
    // coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5BuildRemoteBoundaryComponentFamily::LocalSafeContinuationCard);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = BuildRemoteProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildRemoteCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildRemoteCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != BuildRemoteCertificationAxis::BoundaryTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildRemoteCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteCertificationViolation::RawRemoteMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: BuildRemoteProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_build_remote_boundary_component_certification_export().expect("export is valid");
    assert_eq!(
...
fn generate_artifacts() {
    if std::env::var("GEN_BUILD_REMOTE_BOUNDARY_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_build_remote_boundary_component_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-build-remote-boundary-component-certification-proof");
    ...
    let fixtures = Path::new(manifest)
        .join("../../fixtures/ui/m5-build-remote-boundary-component-certification");
    ...
    fs::write(
        ...
        "# M5 build/remote-boundary component profile certification fixtures\n\n\
         Mirror of `artifacts/release/m5-build-remote-boundary-component-certification-proof/`.\n\
         Regenerate with `GEN_BUILD_REMOTE_BOUNDARY_CERT_ARTIFACTS=1 cargo test -p aureline-remote \
         --lib certify_adapter_confidence...::tests::generate_artifacts`.\n",
    )
