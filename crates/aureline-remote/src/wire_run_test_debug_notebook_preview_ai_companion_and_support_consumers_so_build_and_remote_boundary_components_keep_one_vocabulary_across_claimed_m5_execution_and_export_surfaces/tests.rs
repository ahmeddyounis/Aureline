//! Tests for the M05-1082 build/remote-boundary component consumer adoption lane.

use super::*;

fn packet() -> BuildRemoteConsumerPacket {
    seeded_m5_build_remote_boundary_component_consumers_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, BUILD_REMOTE_CONSUMER_RECORD_KIND);
    assert_eq!(p.schema_version, BUILD_REMOTE_CONSUMER_SCHEMA_VERSION);
}

#[test]
fn all_six_consumer_classes_present() {
    let p = packet();
    assert!(p.summary.run_test_debug_consumer_present);
    assert!(p.summary.notebook_consumer_present);
    assert!(p.summary.preview_consumer_present);
    assert!(p.summary.ai_tool_routing_consumer_present);
    assert!(p.summary.companion_handoff_consumer_present);
...
fn every_controls_lane_is_exercised() {
    let p = packet();
    let lanes: BTreeSet<M5BuildRemoteControlsLane> =
        p.rows.iter().map(|r| r.controls_lane).collect();
    for lane in M5BuildRemoteControlsLane::ALL {
        assert!(
            lanes.contains(&lane),
...
fn same_family_shares_one_controls_lane_across_classes() {
    // The host-boundary strip appears on the run/test/debug surface and the
    // support export; every one must resolve to the same controls lane.
    let p = packet();
    let lanes: BTreeSet<M5BuildRemoteControlsLane> = p
        .rows
        .iter()
        .filter(|r| r.component_family == M5BuildRemoteBoundaryComponentFamily::HostBoundaryStrip)
        .map(|r| r.controls_lane)
        .collect();
    assert_eq!(lanes.len(), 1, "host-boundary-strip lane forked: {lanes:?}");
}
...
#[test]
fn host_boundary_strip_is_reused_across_multiple_classes() {
    let p = packet();
    ...
    assert!(
        classes.len() >= 2,
        "host-boundary strip should be adopted by >= 2 classes, saw {classes:?}"
    );
}
...
fn canonical_family_schema_refs_are_per_family_and_distinct() {
    let refs: BTreeSet<&str> = M5BuildRemoteBoundaryComponentFamily::ALL
        .iter()
        .map(|f| canonical_family_schema_ref_for(*f))
        .collect();
    assert_eq!(refs.len(), M5BuildRemoteBoundaryComponentFamily::ALL.len());
}

#[test]
fn controls_lane_refs_are_per_lane_and_distinct() {
    let schema_refs: BTreeSet<&str> = M5BuildRemoteControlsLane::ALL
        .iter()
        .map(|l| l.canonical_schema_ref())
        .collect();
    let artifact_refs: BTreeSet<&str> = M5BuildRemoteControlsLane::ALL
        .iter()
        .map(|l| l.canonical_artifact_ref())
        .collect();
    assert_eq!(schema_refs.len(), M5BuildRemoteControlsLane::ALL.len());
    assert_eq!(artifact_refs.len(), M5BuildRemoteControlsLane::ALL.len());
}
...
#[test]
fn boundary_crossing_rows_keep_host_and_continuity_where_declared() {
    // Boundary-crossing classes must never render an anonymous execution surface:
    // their rows preserve the family primary label and at least one of the
    // host-boundary, execution-origin, or continuity truth so the user can tell
    // where the work ran and whether it kept exact continuity.
    let p = packet();
    let crossing: Vec<&BuildRemoteConsumerRow> = p
        .rows
        .iter()
    ...
    for row in crossing {
        ...
        assert!(
            has("host_boundary") || has("execution_origin") || has("continuity"),
            "boundary-crossing row {} names no host boundary, execution origin, or continuity",
            row.row_id
        );
    }
}
...
#[test]
fn run_test_debug_and_support_export_references_present() {
    let p = packet();
    assert!(p.has_run_test_debug_reference());
    assert!(p.has_support_export_reference());
    assert!(p.summary.run_test_debug_reference_present);
...
fn missing_consumer_class_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.consumer_class != ConsumerClass::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildRemoteConsumerViolation::MissingConsumerClass { .. })));
}
...
fn dropped_family_is_rejected() {
    let mut p = packet();
    p.rows.retain(|r| {
        r.component_family != M5BuildRemoteBoundaryComponentFamily::LocalSafeContinuationCard
    });
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteConsumerViolation::MissingFamilyCoverage { .. }
    )));
}
...
fn forked_controls_lane_is_rejected() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.component_family == M5BuildRemoteBoundaryComponentFamily::HostBoundaryStrip)
        .expect("a host-boundary-strip row exists");
    p.rows[idx].controls_lane = M5BuildRemoteControlsLane::AdapterDiscovery;
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteConsumerViolation::NonCanonicalControlsLane { .. }
    )));
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteConsumerViolation::ControlsLaneForkedAcrossSurfaces
    )));
}
...
fn dropping_primary_boundary_label_is_rejected() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.component_family == M5BuildRemoteBoundaryComponentFamily::ExecutionOriginReceiptRow
        })
        .expect("an execution-origin-receipt-row row exists");
    // execution_origin is the receipt row's primary boundary label.
    p.rows[idx]
        .preserved_label_families
        .retain(|f| f != "execution_origin");
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteConsumerViolation::PrimaryBoundaryTruthDropped { .. }
    )));
}
...
fn missing_boundary_state_ref_breaks_reconstruction() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteConsumerViolation::StateNotReconstructable { .. }
    )));
}
...
fn renamed_label_parity_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildRemoteConsumerViolation::LabelParityBroken { .. })));
}
...
fn non_canonical_family_schema_ref_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildRemoteConsumerViolation::NotCanonicalFamily { .. })));
}
...
fn narrowed_without_banner_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteConsumerViolation::NarrowedWithoutDisclosure { .. }
    )));
}
...
fn generic_banner_label_is_rejected() {
    ...
    if let Some(banner) = p.rows[idx].reduced_capability_banner.as_mut() {
        banner.visible_label = "Expired".to_owned();
    }
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn set_guardrail_is_rejected() {
    let mut p = packet();
    p.rows[0].implies_exact_continuity_after_material_change = true;
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildRemoteConsumerViolation::GuardrailViolated { .. })));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildRemoteConsumerViolation::RawBoundaryMaterialInExport { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildRemoteConsumerViolation::SummaryMismatch)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildRemoteConsumerViolation::DuplicateId { .. })));
}
...
fn export_json_round_trips() {
    ...
    let back: BuildRemoteConsumerPacket = serde_json::from_str(&json).expect("round trips");
    assert_eq!(p, back);
...
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5BuildRemoteConsumerSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5BuildRemoteConsumerSurface::ALL.len());
}
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_build_remote_boundary_component_consumers_export()
...
/// `GEN_BUILD_REMOTE_BOUNDARY_CONSUMER_ARTIFACTS=1 cargo test -p aureline-remote generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_BUILD_REMOTE_BOUNDARY_CONSUMER_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_build_remote_boundary_component_consumers_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-build-remote-boundary-component-consumer-proof");
    ...
    let fixtures =
        Path::new(manifest).join("../../fixtures/ui/m5-build-remote-boundary-component-consumers");
    ...
    fs::write(
        ...
        "# M5 build/remote-boundary component consumer fixtures\n\n\
         Mirror of `artifacts/release/m5-build-remote-boundary-component-consumer-proof/`. Proves\n\
         the run/test/debug, notebook, preview, AI tool-routing, companion-handoff, and\n\
         support/export + release-packet consumers all reuse the same frozen build/remote-boundary\n\
         component families and vocabulary.\n\
         Regenerate with `GEN_BUILD_REMOTE_BOUNDARY_CONSUMER_ARTIFACTS=1 cargo test -p aureline-remote generate_artifacts`.\n",
    )
