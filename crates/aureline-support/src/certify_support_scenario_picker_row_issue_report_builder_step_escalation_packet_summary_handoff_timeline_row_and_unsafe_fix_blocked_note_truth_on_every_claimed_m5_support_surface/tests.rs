//! Tests for the M05-907 support-intake / escalation component surface certification
//! capstone.

use super::*;

fn packet() -> SupportSurfaceCertificationPacket {
    seeded_m5_support_intake_escalation_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, SUPPORT_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, SUPPORT_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5SupportIntakeEscalationCertifiedSurface::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(row.axis_outcomes.len(), SupportCertificationAxis::ALL.len());
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, SupportAxisCertificationState::Certified);
...
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, SUPPORT_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(row.canonical_bundle_ref, SUPPORT_CERT_CANONICAL_BUNDLE_REF);
    }
...
fn the_four_spec_auto_narrow_conditions_are_each_certified() {
    // The four spec narrowing conditions: uncertain scenario, omitted evidence, local-only
    // destination, and policy-blocked repair — must each be a certified yellow surface.
    let p = packet();
    let certified: BTreeSet<M5SupportIntakeClaim> = p
        .rows
        .iter()
        .filter(|r| r.derived_status == SupportSurfaceClaimStatus::Yellow)
        .map(|r| r.certified_claim)
        .collect();
    assert!(certified.contains(&M5SupportIntakeClaim::UnclassifiedScenario));
    assert!(certified.contains(&M5SupportIntakeClaim::EvidenceIncompleteCase));
    assert!(certified.contains(&M5SupportIntakeClaim::LocalOnlyDiagnosis));
    assert!(certified.contains(&M5SupportIntakeClaim::PolicyBlockedRepair));
}

#[test]
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5SupportIntakeEscalationCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
...
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = SupportCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), SupportCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in SupportCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == SupportCertificationAxis::CliExport
        );
...
fn support_claim_ladder_is_strictly_ordered() {
    let ranks: Vec<u8> = M5SupportIntakeClaim::ALL
        .iter()
        .map(|c| c.capability_rank())
        .collect();
    assert_eq!(ranks, vec![5, 4, 3, 2, 1, 0]);
    let tokens: BTreeSet<&str> = M5SupportIntakeClaim::ALL
        .iter()
        .map(|c| c.as_str())
        .collect();
    assert_eq!(tokens.len(), M5SupportIntakeClaim::ALL.len());
}
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5SupportIntakeEscalationCertifiedSurface::DoctorResults)
        .expect("doctor-results row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == SupportCertificationAxis::Visual {
            outcome.state = SupportAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("scenario code silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, SupportSurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, SupportCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5SupportIntakeEscalationCertifiedSurface::SupportExport)
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == SupportCertificationAxis::DegradedState {
            *outcome = SupportAxisOutcome {
                axis: SupportCertificationAxis::DegradedState,
                state: SupportAxisCertificationState::DisclosedNarrowed,
                parity_note: "evidence completeness lagging".to_owned(),
                narrowing_reason: Some("one evidence class was omitted from this case".to_owned()),
                downgrade_trigger: Some(M5SupportDowngradeTrigger::EvidenceClassMasked),
            };
        }
    }
    // Claim stays ReviewableCase == certified ReviewableCase, no auto-narrow.
    assert_eq!(row.derive_status(), SupportSurfaceClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == SupportCertificationAxis::CliExport {
            outcome.state = SupportAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5SupportDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), SupportSurfaceClaimStatus::Red);
}
...
fn dropped_lineage_blocks_the_surface() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, SupportCertificationViolation::LineageDropped { .. })));
}
...
fn narrowed_row_dropping_lineage_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == SupportSurfaceClaimStatus::Yellow)
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(SupportClaimAutoNarrow {
        binding_axis: SupportCertificationAxis::DegradedState,
        from_claim: M5SupportIntakeClaim::ReadyToEscalate,
        to_claim: M5SupportIntakeClaim::ReadyToEscalate,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5SupportIntakeClaim::LocalOnlyDiagnosis;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableCase
    row.certified_claim = M5SupportIntakeClaim::ReadyToEscalate;
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        SupportCertificationViolation::CertifiedClaimExceedsClaim { .. }
    )));
}
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5SupportIntakeEscalationCertifiedSurface::ExtensionBisect)
        .expect("extension-bisect row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = SupportCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), SupportSurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == SupportCertificationAxis::CliExport {
            outcome.state = SupportAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the bisect view".to_owned());
            outcome.downgrade_trigger = Some(M5SupportDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = SupportCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), SupportSurfaceClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "unclassified".to_owned();
    }
    assert_eq!(row.derive_status(), SupportSurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(SupportCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5SupportIntakeEscalationCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, SupportCertificationViolation::SurfaceCoverageIncomplete)));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the UnsafeFixBlockedNote family from every row that carries it; coverage must fail
    // (only the cli-headless row consumes it).
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5SupportIntakeEscalationComponentFamily::UnsafeFixBlockedNote);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, SupportCertificationViolation::FamilyCoverageIncomplete)));
}

#[test]
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = SupportSurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        SupportCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        SupportCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, SupportCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, SupportCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != SupportCertificationAxis::SupportIntakeAndEscalationProvenance);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        SupportCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, SupportCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, SupportCertificationViolation::RawSupportMaterialInExport)));
}
...
fn export_json_round_trips() {
    ...
    let back: SupportSurfaceCertificationPacket = serde_json::from_str(&json).expect("round trips");
    assert_eq!(p, back);
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_support_intake_escalation_component_certification_export()
...
/// `GEN_SUPPORT_CERT_ARTIFACTS=1 cargo test -p aureline-support \
///  certify_support_scenario_picker_row...::tests::generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_SUPPORT_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_support_intake_escalation_component_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-support-intake-escalation-component-certification");
    ...
    let fixtures = Path::new(manifest)
        .join("../../fixtures/ui/m5-support-intake-escalation-component-certification");
