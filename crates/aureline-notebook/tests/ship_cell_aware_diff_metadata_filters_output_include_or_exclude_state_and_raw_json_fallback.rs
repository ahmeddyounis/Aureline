...
use aureline_notebook::{
    ...
    NotebookDiffOutputChangeClass, NotebookDiffOutputSummary, NotebookDiffReviewSession,
    NotebookMetadataFilterState, NotebookOutputIncludeState, NotebookRawJsonFallback,
    RawJsonFallbackReason, NOTEBOOK_DIFF_PACKET_RECORD_KIND, NOTEBOOK_DIFF_SCHEMA_VERSION,
};
...
fn manifest_vocabularies_are_complete() {
    ...
    let expected_diff_modes: Vec<String> = NotebookDiffMode::ALL
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    assert_eq!(manifest.diff_modes, expected_diff_modes);

    let expected_cell_change_classes: Vec<String> = NotebookDiffCellChangeClass::ALL
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    assert_eq!(manifest.cell_change_classes, expected_cell_change_classes);

    let expected_output_change_classes: Vec<String> = NotebookDiffOutputChangeClass::ALL
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    assert_eq!(
        manifest.output_change_classes,
        expected_output_change_classes
    );

    let expected_metadata_filter_states: Vec<String> = NotebookMetadataFilterState::ALL
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    assert_eq!(
        manifest.metadata_filter_states,
        expected_metadata_filter_states
    );

    let expected_output_include_states: Vec<String> = NotebookOutputIncludeState::ALL
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    assert_eq!(
        manifest.output_include_states,
        expected_output_include_states
    );

    let expected_raw_json_fallback_reasons: Vec<String> = RawJsonFallbackReason::ALL
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    assert_eq!(
        manifest.raw_json_fallback_reasons,
        expected_raw_json_fallback_reasons
    );
...
fn fixture_cell_aware_review_validates() {
    ...
    if let Some(session) = case.notebook_diff_review_session {
        assert!(
            session.validate().is_empty(),
            "session findings: {:?}",
            session.validate()
        );
        assert_eq!(session.diff_mode, NotebookDiffMode::CellAware);
        assert_eq!(
            session.output_include_state,
            NotebookOutputIncludeState::Included
        );
        assert!(session.stable_cell_anchors);
    }
    if let Some(filter) = case.notebook_diff_metadata_filter {
        assert!(
            filter.validate().is_empty(),
            "filter findings: {:?}",
            filter.validate()
        );
        assert_eq!(
            filter.metadata_filter_state,
            NotebookMetadataFilterState::AllVisible
        );
        assert!(filter.unknown_namespaces_preserved_on_save);
    }
    if let Some(output) = case.notebook_diff_output_summary {
        assert!(
            output.validate().is_empty(),
            "output findings: {:?}",
            output.validate()
        );
        assert_eq!(
            output.output_include_state,
            NotebookOutputIncludeState::Included
        );
    }
    if let Some(change) = case.notebook_diff_cell_change {
        assert!(
            change.validate().is_empty(),
            "change findings: {:?}",
            change.validate()
        );
        assert_eq!(
            change.cell_change_class,
            NotebookDiffCellChangeClass::SourceChanged
        );
    }
}
...
fn fixture_raw_json_fallback_review_validates() {
    ...
    if let Some(session) = case.notebook_diff_review_session {
        ...
        assert_eq!(
            session.output_include_state,
            NotebookOutputIncludeState::Excluded
        );
        assert!(!session.stable_cell_anchors);
    ...
    if let Some(fallback) = case.notebook_raw_json_fallback {
        assert!(
            fallback.validate().is_empty(),
            "fallback findings: {:?}",
            fallback.validate()
        );
        assert_eq!(
            fallback.fallback_reason,
            RawJsonFallbackReason::UnsupportedVersion
        );
...
fn fixture_metadata_filter_unknown_hidden_validates() {
    ...
    if let Some(filter) = case.notebook_diff_metadata_filter {
        ...
        assert_eq!(
            filter.metadata_filter_state,
            NotebookMetadataFilterState::UnknownHidden
        );
        assert!(filter.unknown_namespaces_preserved_on_save);
    }
    if let Some(session) = case.notebook_diff_review_session {
...
fn fixture_output_collapsed_review_validates() {
    ...
    if let Some(output) = case.notebook_diff_output_summary {
        ...
        assert_eq!(
            output.output_include_state,
            NotebookOutputIncludeState::Collapsed
        );
        assert_eq!(
            output.output_change_class,
            NotebookDiffOutputChangeClass::OutputAdded
        );
    }
    if let Some(session) = case.notebook_diff_review_session {
        assert!(
...
fn packet_vocabularies_cover_all_variants() {
    ...
    let expected_diff_modes: Vec<String> = NotebookDiffMode::ALL
        ...
        .map(|v| v.as_str().to_string())
        .collect();
    let actual_diff_modes: Vec<String> = packet
        .diff_modes
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    ...
    let expected_cell_change_classes: Vec<String> = NotebookDiffCellChangeClass::ALL
        ...
        .map(|v| v.as_str().to_string())
        .collect();
    let actual_cell_change_classes: Vec<String> = packet
        .cell_change_classes
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    ...
    let expected_output_change_classes: Vec<String> = NotebookDiffOutputChangeClass::ALL
        ...
        .map(|v| v.as_str().to_string())
        .collect();
    let actual_output_change_classes: Vec<String> = packet
        .output_change_classes
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    ...
    let expected_metadata_filter_states: Vec<String> = NotebookMetadataFilterState::ALL
        ...
        .map(|v| v.as_str().to_string())
        .collect();
    let actual_metadata_filter_states: Vec<String> = packet
        .metadata_filter_states
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    assert_eq!(
        actual_metadata_filter_states,
        expected_metadata_filter_states
    );
    ...
    let expected_output_include_states: Vec<String> = NotebookOutputIncludeState::ALL
        ...
        .map(|v| v.as_str().to_string())
        .collect();
    let actual_output_include_states: Vec<String> = packet
        .output_include_states
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    ...
    let expected_raw_json_fallback_reasons: Vec<String> = RawJsonFallbackReason::ALL
        ...
        .map(|v| v.as_str().to_string())
        .collect();
    let actual_raw_json_fallback_reasons: Vec<String> = packet
        .raw_json_fallback_reasons
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    assert_eq!(
        actual_raw_json_fallback_reasons,
        expected_raw_json_fallback_reasons
    );
    ...
    let expected_merge_resolution_classes: Vec<String> = NotebookDiffMergeResolutionClass::ALL
        ...
        .map(|v| v.as_str().to_string())
        .collect();
    let actual_merge_resolution_classes: Vec<String> = packet
        .merge_resolution_classes
        .iter()
        .map(|v| v.as_str().to_string())
        .collect();
    assert_eq!(
        actual_merge_resolution_classes,
        expected_merge_resolution_classes
    );
}
