//! Tests for the M05-1090 notebook document / kernel / output component consumer
//! adoption lane.

use super::*;

fn packet() -> NotebookConsumerPacket {
    seeded_m5_notebook_kernel_output_component_consumers_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, NOTEBOOK_CONSUMER_RECORD_KIND);
    assert_eq!(p.schema_version, NOTEBOOK_CONSUMER_SCHEMA_VERSION);
}

#[test]
fn all_six_consumer_classes_present() {
    let p = packet();
    assert!(p.summary.notebook_editor_consumer_present);
    assert!(p.summary.diff_review_consumer_present);
    assert!(p.summary.debug_consumer_present);
    assert!(p.summary.ai_context_consumer_present);
    assert!(p.summary.cli_consumer_present);
...
fn every_controls_lane_is_exercised() {
    let p = packet();
    let lanes: BTreeSet<M5NotebookControlsLane> = p.rows.iter().map(|r| r.controls_lane).collect();
    for lane in M5NotebookControlsLane::ALL {
        assert!(
            lanes.contains(&lane),
...
fn same_family_shares_one_controls_lane_across_classes() {
    // The output-trust banner appears on the notebook editor, diff/review, and CLI
    // surfaces; every one must resolve to the same controls lane.
    let p = packet();
    let lanes: BTreeSet<M5NotebookControlsLane> = p
        .rows
        .iter()
        .filter(|r| r.component_family == M5NotebookKernelOutputComponentFamily::OutputTrustBanner)
        .map(|r| r.controls_lane)
        .collect();
    assert_eq!(lanes.len(), 1, "output-trust-banner lane forked: {lanes:?}");
}
...
#[test]
fn output_trust_banner_is_reused_across_multiple_classes() {
    let p = packet();
    ...
    assert!(
        classes.len() >= 2,
        "output-trust banner should be adopted by >= 2 classes, saw {classes:?}"
    );
}
...
fn canonical_family_schema_refs_are_per_family_and_distinct() {
    let refs: BTreeSet<&str> = M5NotebookKernelOutputComponentFamily::ALL
        .iter()
        .map(|f| canonical_family_schema_ref_for(*f))
        .collect();
    assert_eq!(refs.len(), M5NotebookKernelOutputComponentFamily::ALL.len());
}

#[test]
fn controls_lane_refs_are_per_lane_and_distinct() {
    let schema_refs: BTreeSet<&str> = M5NotebookControlsLane::ALL
        .iter()
        .map(|l| l.canonical_schema_ref())
        .collect();
    let artifact_refs: BTreeSet<&str> = M5NotebookControlsLane::ALL
        .iter()
        .map(|l| l.canonical_artifact_ref())
        .collect();
    assert_eq!(schema_refs.len(), M5NotebookControlsLane::ALL.len());
    assert_eq!(artifact_refs.len(), M5NotebookControlsLane::ALL.len());
}
...
#[test]
fn disposition_coverage_is_complete() {
    let p = packet();
    let covered = p.covered_dispositions();
    ...
    assert!(p.summary.disposition_coverage_complete);
    assert_eq!(
        p.summary.disposition_count,
        M5NotebookKernelOutputDisposition::ALL.len()
    );
}

#[test]
fn every_row_preserves_its_family_primary_notebook_label() {
    let p = packet();
    for row in &p.rows {
        assert!(
            row.preserves_primary_notebook_truth(),
            "row {} drops its family's primary notebook label {}",
            row.row_id,
            family_primary_label(row.component_family)
        );
    }
    assert!(p.summary.all_runtime_rows_preserve_primary_truth);
}

#[test]
fn runtime_bearing_rows_keep_a_live_truth_label() {
    // Runtime-bearing classes must never render an anonymous live surface: their
    // rows preserve the family primary label and at least one of the kernel-state,
    // output-trust, or recovery-continuity truth so the user can tell whether the
    // kernel is live, the output is fresh, and continuity survived recovery.
    let p = packet();
    let runtime: Vec<&NotebookConsumerRow> = p
        .rows
        .iter()
        .filter(|r| r.consumer_class.is_runtime_bearing())
        .collect();
    assert!(!runtime.is_empty());
    for row in runtime {
        assert!(row.preserves_primary_notebook_truth());
        let has = |f: &str| row.preserved_label_families.iter().any(|v| v == f);
        assert!(
            has("kernel_state") || has("output_trust") || has("recovery_continuity"),
            "runtime-bearing row {} names no kernel state, output trust, or recovery continuity",
            row.row_id
        );
    }
}
...
fn every_row_supports_state_reconstruction() {
    ...
    for row in &p.rows {
        ...
        assert!(row.notebook_state_ref.starts_with("notebook-state:"));
    }
...
#[test]
fn editor_and_support_export_references_present() {
    let p = packet();
    assert!(p.has_editor_reference());
    assert!(p.has_support_export_reference());
    assert!(p.summary.editor_reference_present);
...
#[test]
fn disposition_vocab_is_canonical_on_every_row() {
    for row in &packet().rows {
        for token in &row.notebook_disposition_vocab {
            assert!(
                is_canonical_notebook_disposition(token),
                "row {} carries non-canonical disposition token {token}",
                row.row_id
            );
        }
    }
...
fn missing_consumer_class_is_rejected() {
    let mut p = packet();
    p.rows.retain(|r| r.consumer_class != ConsumerClass::Cli);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, NotebookConsumerViolation::MissingConsumerClass { .. })));
}
...
fn dropped_family_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.component_family != M5NotebookKernelOutputComponentFamily::KernelPickerRow);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, NotebookConsumerViolation::MissingFamilyCoverage { .. })));
}
...
fn forked_controls_lane_is_rejected() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.component_family == M5NotebookKernelOutputComponentFamily::OutputTrustBanner
        })
        .expect("an output-trust-banner row exists");
    p.rows[idx].controls_lane = M5NotebookControlsLane::DocumentKernel;
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        NotebookConsumerViolation::NonCanonicalControlsLane { .. }
    )));
    assert!(violations.iter().any(|v| matches!(
        v,
        NotebookConsumerViolation::ControlsLaneForkedAcrossSurfaces
    )));
}

#[test]
fn dropping_primary_notebook_label_is_rejected() {
    let mut p = packet();
    let idx = p
        .rows
        .iter()
        ...
        .expect("a kernel-recovery-card row exists");
    // recovery_continuity is the recovery card's primary notebook label.
    p.rows[idx]
        .preserved_label_families
        .retain(|f| f != "recovery_continuity");
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        NotebookConsumerViolation::PrimaryNotebookTruthDropped { .. }
    )));
}

#[test]
fn missing_notebook_state_ref_breaks_reconstruction() {
    let mut p = packet();
    p.rows[0].notebook_state_ref = String::new();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, NotebookConsumerViolation::StateNotReconstructable { .. })));
}
...
fn renamed_label_parity_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, NotebookConsumerViolation::LabelParityBroken { .. })));
}
...
fn non_canonical_family_schema_ref_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, NotebookConsumerViolation::NotCanonicalFamily { .. })));
}
...
fn narrowed_without_banner_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        NotebookConsumerViolation::NarrowedWithoutDisclosure { .. }
    )));
}
...
fn generic_banner_label_is_rejected() {
    ...
    if let Some(banner) = p.rows[idx].reduced_capability_banner.as_mut() {
        banner.visible_label = "Stale".to_owned();
    }
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn set_guardrail_is_rejected() {
    let mut p = packet();
    p.rows[0].presents_stale_output_as_live = true;
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, NotebookConsumerViolation::GuardrailViolated { .. })));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        NotebookConsumerViolation::RawNotebookMaterialInExport { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, NotebookConsumerViolation::SummaryMismatch)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, NotebookConsumerViolation::DuplicateId { .. })));
}
...
fn export_json_round_trips() {
    ...
    let back: NotebookConsumerPacket = serde_json::from_str(&json).expect("round trips");
    assert_eq!(p, back);
...
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5NotebookKernelOutputConsumerSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
        tokens.len(),
        M5NotebookKernelOutputConsumerSurface::ALL.len()
    );
}
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_notebook_kernel_output_component_consumers_export()
...
/// `GEN_NOTEBOOK_KERNEL_OUTPUT_CONSUMER_ARTIFACTS=1 cargo test -p aureline-notebook generate_artifacts`.
#[test]
fn generate_artifacts() {
    if std::env::var("GEN_NOTEBOOK_KERNEL_OUTPUT_CONSUMER_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_notebook_kernel_output_component_consumers_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-notebook-kernel-output-component-consumer-proof");
    ...
    let fixtures =
        Path::new(manifest).join("../../fixtures/ui/m5-notebook-kernel-output-component-consumers");
    ...
    fs::write(
        ...
        "# M5 notebook-kernel-output component consumer fixtures\n\n\
         Mirror of `artifacts/release/m5-notebook-kernel-output-component-consumer-proof/`. Proves\n\
         the notebook editor, diff/review, debug, AI-context, CLI/headless, and support/export +\n\
         release-packet consumers all reuse the same frozen notebook document / kernel / output\n\
         component families and vocabulary.\n\
         Regenerate with `GEN_NOTEBOOK_KERNEL_OUTPUT_CONSUMER_ARTIFACTS=1 cargo test -p aureline-notebook generate_artifacts`.\n",
    )
