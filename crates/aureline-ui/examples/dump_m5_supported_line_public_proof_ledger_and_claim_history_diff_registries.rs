//! Headless emitter for the M5 line-public_proof_ledger and line-downgrade-packet registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-supported-line-public-proof-ledger-and-claim-history-diff-registries-proof/`, its matrix CSV,
//! the Markdown summary, and the narrowed fixtures under
//! `fixtures/release/m5-supported-line-public-proof-ledger-and-claim-history-diff-registries/`.
//!
//! ```text
//! cargo run -p aureline-ui --example dump_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries -- support-export
//! cargo run -p aureline-ui --example dump_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries -- report
//! cargo run -p aureline-ui --example dump_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries -- csv
//! cargo run -p aureline-ui --example dump_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries -- public-proof-ledger-table
//! cargo run -p aureline-ui --example dump_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries -- fixture-public-proof-ledger-beta-narrowed
//! cargo run -p aureline-ui --example dump_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries -- fixture-claim-history-diff-preview-narrowed
//! cargo run -p aureline-ui --example dump_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries -- validate
//! ```

use aureline_ui::m5_supported_line_public_proof_ledger_and_claim_history_diff_registries::{
    seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries,
    seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries_claim_history_diff_preview_narrowed,
    seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries_public_proof_ledger_beta_narrowed,
    M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacket,
};

fn main() {
    if let Err(err) = run() {
        eprintln!("{err}");
        std::process::exit(2);
    }
}

fn run() -> Result<(), Box<dyn std::error::Error>> {
    let args: Vec<String> = std::env::args().skip(1).collect();
    match args.first().map(String::as_str) {
        Some("support-export") | None => {
            let packet =
                seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("report") => {
            print!(
                "{}",
                seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries()
                    .render_markdown_summary()
            );
        }
        Some("csv") => {
            print!(
                "{}",
                seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries()
                    .render_matrix_csv()
            );
        }
        Some("public-proof-ledger-table") => {
            print!(
                "{}",
                seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries()
                    .render_public_proof_ledger_table()
            );
        }
        Some("fixture-public-proof-ledger-beta-narrowed") => {
            let packet =
                seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries_public_proof_ledger_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-claim-history-diff-preview-narrowed") => {
            let packet =
                seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries_claim_history_diff_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("validate") => {
            for packet in [
                seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries(),
                seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries_public_proof_ledger_beta_narrowed(),
                seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries_claim_history_diff_preview_narrowed(),
            ] {
                assert_valid(&packet)?;
            }
            println!("ok");
        }
        Some(other) => {
            return Err(format!("unknown subcommand: {other}").into());
        }
    }
    Ok(())
}

fn assert_valid(
    packet: &M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacket,
) -> Result<(), Box<dyn std::error::Error>> {
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(())
    } else {
        let tokens: Vec<&str> = violations.iter().map(|v| v.as_str()).collect();
        Err(format!("registries packet failed validation: {}", tokens.join(",")).into())
    }
}
