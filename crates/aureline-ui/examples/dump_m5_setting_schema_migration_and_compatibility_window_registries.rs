//! Headless emitter for the M5 schema-migration and compatibility-window registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-setting-schema-migration-and-compatibility-window-registries-proof/`, its matrix CSV,
...
//! cargo run -p aureline-ui --example dump_m5_setting_schema_migration_and_compatibility_window_registries -- validate
//! ```

use aureline_ui::m5_setting_schema_migration_and_compatibility_window_registries::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("fixture-schema-migration-beta-narrowed") => {
            let packet =
                seeded_m5_setting_schema_migration_and_compatibility_window_registries_schema_migration_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-compatibility-window-preview-narrowed") => {
            let packet =
                seeded_m5_setting_schema_migration_and_compatibility_window_registries_compatibility_window_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
