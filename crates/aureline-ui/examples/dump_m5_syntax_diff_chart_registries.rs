//! Headless emitter for the M5 syntax-, diff-, and chart-token registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-syntax-diff-and-chart-token-registries-proof/`, its matrix CSV, the Markdown
...
//! cargo run -p aureline-ui --example dump_m5_syntax_diff_chart_registries -- validate
//! ```

use aureline_ui::m5_syntax_diff_and_chart_token_registries::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("report") => {
            print!(
                "{}",
                seeded_m5_syntax_diff_chart_registries().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!(
                "{}",
                seeded_m5_syntax_diff_chart_registries().render_matrix_csv()
            );
        }
        Some("fixture-editor-ui-beta-narrowed") => {
            let packet = seeded_m5_syntax_diff_chart_registries_editor_ui_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-data-ui-preview-narrowed") => {
            let packet = seeded_m5_syntax_diff_chart_registries_data_ui_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("validate") => {
