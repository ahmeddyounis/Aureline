//! Headless emitter for the M5 archived-snapshot-viewer consumer packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/support/m5-archived-snapshot-viewer-consumers/`, its matrix CSV, the Markdown summary, and
//! the narrowed fixtures under `fixtures/recovery/m5-archived-snapshot-viewer-consumers/`.
//!
//! ```text
...
use aureline_ui::m5_archived_snapshot_viewer_and_analysis_only_banner_consumers::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        Some("support-export") | None => {
            let packet = seeded_m5_archived_snapshot_viewer_consumers();
            assert_valid(&packet)?;
        ...
        Some("report") => {
            print!(
                "{}",
                seeded_m5_archived_snapshot_viewer_consumers().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!(
                "{}",
                seeded_m5_archived_snapshot_viewer_consumers().render_matrix_csv()
            );
        }
        Some("fixture-metadata-only-narrowed") => {
            let packet = seeded_m5_archived_snapshot_viewer_consumers_metadata_only_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-imported-offline-narrowed") => {
            let packet = seeded_m5_archived_snapshot_viewer_consumers_imported_offline_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("validate") => {
...
fn assert_valid(
    ...
    if violations.is_empty() {
        ...
        Err(format!(
            "archived-snapshot-viewer consumer packet failed validation: {}",
            tokens.join(",")
        )
        .into())
    }
