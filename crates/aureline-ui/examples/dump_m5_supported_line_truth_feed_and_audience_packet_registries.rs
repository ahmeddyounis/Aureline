//! Headless emitter for the M5 supported-line truth-feed and audience-packet registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-supported-line-truth-feed-and-audience-packet-registries-proof/`, its matrix CSV,
...
//! cargo run -p aureline-ui --example dump_m5_supported_line_truth_feed_and_audience_packet_registries -- validate
//! ```

use aureline_ui::m5_supported_line_truth_feed_and_audience_packet_registries::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("truth-feed-table") => {
            print!(
                "{}",
                seeded_m5_supported_line_truth_feed_and_audience_packet_registries()
                    .render_truth_feed_table()
            );
        }
        Some("fixture-truth-feed-beta-narrowed") => {
            let packet =
                seeded_m5_supported_line_truth_feed_and_audience_packet_registries_truth_feed_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-audience-packet-preview-narrowed") => {
            let packet =
                seeded_m5_supported_line_truth_feed_and_audience_packet_registries_audience_packet_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
