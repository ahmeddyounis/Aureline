//! Headless emitter for the M5 repository-bootstrap shared-consumer parity packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-repository-bootstrap-shared-consumers-proof/`, its matrix CSV, the Markdown
//! summary, and the narrowed fixtures under `fixtures/workspaces/m5-repository-bootstrap-shared-consumers/`.
//!
//! ```text
...
//! cargo run -p aureline-ui --example dump_m5_repository_bootstrap_shared_consumers -- validate
//! ```

use aureline_ui::m5_repository_bootstrap_shared_consumers_one_registry_across_surfaces::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        Some("support-export") | None => {
            let packet = seeded_m5_repository_bootstrap_shared_consumers();
            assert_valid(&packet)?;
        ...
        Some("report") => {
            print!(
                "{}",
                seeded_m5_repository_bootstrap_shared_consumers().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!(
                "{}",
                seeded_m5_repository_bootstrap_shared_consumers().render_matrix_csv()
            );
        }
        Some("fixture-compact-remote-narrowed") => {
            let packet = seeded_m5_repository_bootstrap_shared_consumers_compact_remote_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
...
fn assert_valid(
    ...
    if violations.is_empty() {
        ...
        Err(format!(
            "repository-bootstrap shared-consumer packet failed validation: {}",
            tokens.join(",")
        )
        .into())
    }
