//! Headless emitter for the M5 live-target-handoff packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/support/m5-live-target-handoff/`, its matrix CSV, the Markdown summary, and the narrowed
//! fixtures under `fixtures/recovery/m5-live-target-handoff/`.
//!
//! ```text
//! cargo run -p aureline-ui --example dump_m5_live_target_handoff_packet_and_route_validation -- support-export
//! cargo run -p aureline-ui --example dump_m5_live_target_handoff_packet_and_route_validation -- report
//! cargo run -p aureline-ui --example dump_m5_live_target_handoff_packet_and_route_validation -- csv
//! cargo run -p aureline-ui --example dump_m5_live_target_handoff_packet_and_route_validation -- fixture-blocked-target-narrowed
//! cargo run -p aureline-ui --example dump_m5_live_target_handoff_packet_and_route_validation -- fixture-needs-prerequisite-narrowed
//! cargo run -p aureline-ui --example dump_m5_live_target_handoff_packet_and_route_validation -- validate
//! ```

use aureline_ui::m5_live_target_handoff_packet_and_route_validation::{
    seeded_m5_live_target_handoff, seeded_m5_live_target_handoff_blocked_target_narrowed,
    seeded_m5_live_target_handoff_needs_prerequisite_narrowed, M5LiveTargetHandoffPacket,
};

fn main() {
    if let Err(err) = run() {
        eprintln!("{err}");
        std::process::exit(2);
    }
}

fn run() -> Result<(), Box<dyn std::error::Error>> {
    let args: Vec<String> = std::env::args().skip(1).collect();
    match args.first().map(String::as_str) {
        Some("support-export") | None => {
            let packet = seeded_m5_live_target_handoff();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("report") => {
            print!(
                "{}",
                seeded_m5_live_target_handoff().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!("{}", seeded_m5_live_target_handoff().render_matrix_csv());
        }
        Some("fixture-blocked-target-narrowed") => {
            let packet = seeded_m5_live_target_handoff_blocked_target_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-needs-prerequisite-narrowed") => {
            let packet = seeded_m5_live_target_handoff_needs_prerequisite_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("validate") => {
            for packet in [
                seeded_m5_live_target_handoff(),
                seeded_m5_live_target_handoff_blocked_target_narrowed(),
                seeded_m5_live_target_handoff_needs_prerequisite_narrowed(),
            ] {
                assert_valid(&packet)?;
            }
            println!("ok");
        }
        Some(other) => {
            return Err(format!("unknown subcommand: {other}").into());
        }
    }
    Ok(())
}

fn assert_valid(packet: &M5LiveTargetHandoffPacket) -> Result<(), Box<dyn std::error::Error>> {
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(())
    } else {
        let tokens: Vec<&str> = violations.iter().map(|v| v.as_str()).collect();
        Err(format!("handoff packet failed validation: {}", tokens.join(",")).into())
    }
}
