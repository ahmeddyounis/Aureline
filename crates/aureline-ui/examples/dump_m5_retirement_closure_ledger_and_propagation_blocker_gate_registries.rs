//! Headless emitter for the M5 retirement-closure-ledger and propagation-blocker-gate registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-retirement-closure-ledger-and-propagation-blocker-gate-registries-proof/`, its matrix CSV,
...
//! cargo run -p aureline-ui --example dump_m5_retirement_closure_ledger_and_propagation_blocker_gate_registries -- validate
//! ```

use aureline_ui::m5_retirement_closure_ledger_and_propagation_blocker_gate_registries::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("retirement-closure-ledger-table") => {
            print!(
                "{}",
                seeded_m5_retirement_closure_ledger_and_propagation_blocker_gate_registries()
                    .render_retirement_closure_ledger_table()
            );
        }
        Some("fixture-retirement-closure-ledger-beta-narrowed") => {
            let packet =
                seeded_m5_retirement_closure_ledger_and_propagation_blocker_gate_registries_retirement_closure_ledger_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-propagation-blocker-gate-preview-narrowed") => {
            let packet =
                seeded_m5_retirement_closure_ledger_and_propagation_blocker_gate_registries_propagation_blocker_gate_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
