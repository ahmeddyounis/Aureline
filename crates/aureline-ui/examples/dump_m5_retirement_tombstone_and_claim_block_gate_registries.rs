//! Headless emitter for the M5 retirement-tombstone and claim-block-gate registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-retirement-tombstone-and-claim-block-gate-registries-proof/`, its matrix CSV,
...
//! cargo run -p aureline-ui --example dump_m5_retirement_tombstone_and_claim_block_gate_registries -- validate
//! ```

use aureline_ui::m5_retirement_tombstone_and_claim_block_gate_registries::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("retirement-tombstone-table") => {
            print!(
                "{}",
                seeded_m5_retirement_tombstone_and_claim_block_gate_registries()
                    .render_retirement_tombstone_table()
            );
        }
        Some("fixture-retirement-tombstone-beta-narrowed") => {
            let packet =
                seeded_m5_retirement_tombstone_and_claim_block_gate_registries_retirement_tombstone_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-claim-block-gate-preview-narrowed") => {
            let packet =
                seeded_m5_retirement_tombstone_and_claim_block_gate_registries_claim_block_gate_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
