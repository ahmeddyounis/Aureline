//! Headless emitter for the M5 dialog / consequence controls packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-dialog-sheet-and-consequence-block-controls-proof/`, its matrix CSV, the
...
use aureline_ui::m5_dialog_sheet_and_consequence_block_rationale_scope_and_rollback_continuity::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("report") => {
            print!(
                "{}",
                seeded_m5_dialog_consequence_controls().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!(
                "{}",
                seeded_m5_dialog_consequence_controls().render_matrix_csv()
            );
        }
        Some("fixture-review-ui-beta-narrowed") => {
            let packet = seeded_m5_dialog_consequence_controls_review_ui_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-updates-ui-preview-narrowed") => {
            let packet = seeded_m5_dialog_consequence_controls_updates_ui_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("validate") => {
