//! Headless emitter for the M5 supported-line retention-policy and stale-escalation registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-supported-line-retention-policy-and-stale-escalation-registries-proof/`, its matrix CSV,
...
//! cargo run -p aureline-ui --example dump_m5_supported_line_retention_policy_and_stale_escalation_registries -- validate
//! ```

use aureline_ui::m5_supported_line_retention_policy_and_stale_escalation_registries::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("retention-policy-table") => {
            print!(
                "{}",
                seeded_m5_supported_line_retention_policy_and_stale_escalation_registries()
                    .render_retention_policy_table()
            );
        }
        Some("fixture-retention-policy-beta-narrowed") => {
            let packet =
                seeded_m5_supported_line_retention_policy_and_stale_escalation_registries_retention_policy_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-stale-escalation-preview-narrowed") => {
            let packet =
                seeded_m5_supported_line_retention_policy_and_stale_escalation_registries_stale_escalation_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
