//! Headless emitter for the M5 last-supported-snapshot and archive-export-gate registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-last-supported-snapshot-and-archive-export-gate-registries-proof/`, its matrix CSV,
...
//! cargo run -p aureline-ui --example dump_m5_last_supported_snapshot_and_archive_export_gate_registries -- validate
//! ```

use aureline_ui::m5_last_supported_snapshot_and_archive_export_gate_registries::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("last-supported-snapshot-table") => {
            print!(
                "{}",
                seeded_m5_last_supported_snapshot_and_archive_export_gate_registries()
                    .render_last_supported_snapshot_table()
            );
        }
        Some("fixture-last-supported-snapshot-beta-narrowed") => {
            let packet =
                seeded_m5_last_supported_snapshot_and_archive_export_gate_registries_last_supported_snapshot_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-archive-export-gate-preview-narrowed") => {
            let packet =
                seeded_m5_last_supported_snapshot_and_archive_export_gate_registries_archive_export_gate_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
