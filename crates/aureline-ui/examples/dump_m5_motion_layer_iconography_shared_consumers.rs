//! Headless emitter for the M5 visual-interaction shared-consumer parity packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-motion-layer-iconography-shared-consumers-proof/`, its matrix CSV, the
...
//! cargo run -p aureline-ui --example dump_m5_motion_layer_iconography_shared_consumers -- validate
//! ```

use aureline_ui::m5_motion_layer_iconography_shared_consumers_one_grammar_across_surfaces::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        Some("support-export") | None => {
            let packet = seeded_m5_motion_layer_iconography_shared_consumers();
            assert_valid(&packet)?;
        ...
        Some("report") => {
            print!(
                "{}",
                seeded_m5_motion_layer_iconography_shared_consumers().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!(
                "{}",
                seeded_m5_motion_layer_iconography_shared_consumers().render_matrix_csv()
            );
        }
...
fn assert_valid(
    ...
    if violations.is_empty() {
        ...
        Err(format!(
            "visual-interaction shared-consumer packet failed validation: {}",
            tokens.join(",")
        )
        .into())
    }
