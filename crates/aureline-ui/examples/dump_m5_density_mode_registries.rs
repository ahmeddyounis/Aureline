//! Headless emitter for the M5 density-mode registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-density-mode-registries-proof/`, its matrix CSV, the Markdown summary, and the
//! narrowed fixtures under `fixtures/ui/m5-density-mode-registries/`.
//!
//! ```text
...
use aureline_ui::m5_density_mode_registries::{
    seeded_m5_density_mode_registries, seeded_m5_density_mode_registries_editor_ui_beta_narrowed,
    seeded_m5_density_mode_registries_settings_ui_preview_narrowed, M5DensityModeRegistriesPacket,
};
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("report") => {
            print!(
                "{}",
                seeded_m5_density_mode_registries().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!(
                "{}",
                seeded_m5_density_mode_registries().render_matrix_csv()
            );
        }
        Some("fixture-editor-ui-beta-narrowed") => {
            let packet = seeded_m5_density_mode_registries_editor_ui_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-settings-ui-preview-narrowed") => {
            let packet = seeded_m5_density_mode_registries_settings_ui_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("validate") => {
...
fn assert_valid(packet: &M5DensityModeRegistriesPacket) -> Result<(), Box<dyn std::error::Error>> {
