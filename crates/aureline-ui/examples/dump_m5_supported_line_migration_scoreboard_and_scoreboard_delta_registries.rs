//! Headless emitter for the M5 line-migration_scoreboard and line-downgrade-packet registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-supported-line-migration-scoreboard-and-scoreboard-delta-registries-proof/`, its matrix CSV,
...
//! cargo run -p aureline-ui --example dump_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries -- validate
//! ```

use aureline_ui::m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("migration-scoreboard-table") => {
            print!(
                "{}",
                seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries()
                    .render_migration_scoreboard_table()
            );
        }
        Some("fixture-migration-scoreboard-beta-narrowed") => {
            let packet =
                seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries_migration_scoreboard_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-scoreboard-delta-preview-narrowed") => {
            let packet =
                seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries_scoreboard_delta_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
