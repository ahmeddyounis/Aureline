//! Headless emitter for the M5 typography-scale, font-stack, and text-overflow registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-typography-scale-font-stack-and-overflow-registries-proof/`, its matrix CSV, the
...
//! cargo run -p aureline-ui --example dump_m5_typography_overflow_registries -- validate
//! ```

use aureline_ui::m5_typography_scale_font_stack_and_overflow_registries::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("report") => {
            print!(
                "{}",
                seeded_m5_typography_overflow_registries().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!(
                "{}",
                seeded_m5_typography_overflow_registries().render_matrix_csv()
            );
        }
        Some("fixture-editor-ui-beta-narrowed") => {
            let packet = seeded_m5_typography_overflow_registries_editor_ui_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-data-ui-preview-narrowed") => {
            let packet = seeded_m5_typography_overflow_registries_data_ui_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("validate") => {
