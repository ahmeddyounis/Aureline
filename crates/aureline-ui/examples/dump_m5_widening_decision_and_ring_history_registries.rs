//! Headless emitter for the M5 widening-decision and ring-history registries packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-widening-decision-and-ring-history-registries-proof/`, its matrix CSV,
//! the Markdown summary, and the narrowed fixtures under
//! `fixtures/release/m5-widening-decision-and-ring-history-registries/`.
//!
//! ```text
//! cargo run -p aureline-ui --example dump_m5_widening_decision_and_ring_history_registries -- support-export
//! cargo run -p aureline-ui --example dump_m5_widening_decision_and_ring_history_registries -- report
//! cargo run -p aureline-ui --example dump_m5_widening_decision_and_ring_history_registries -- csv
//! cargo run -p aureline-ui --example dump_m5_widening_decision_and_ring_history_registries -- widening-decision-table
//! cargo run -p aureline-ui --example dump_m5_widening_decision_and_ring_history_registries -- fixture-widening-decision-beta-narrowed
//! cargo run -p aureline-ui --example dump_m5_widening_decision_and_ring_history_registries -- fixture-ring-history-preview-narrowed
//! cargo run -p aureline-ui --example dump_m5_widening_decision_and_ring_history_registries -- validate
//! ```

use aureline_ui::m5_widening_decision_and_ring_history_registries::{
    seeded_m5_widening_decision_and_ring_history_registries,
    seeded_m5_widening_decision_and_ring_history_registries_ring_history_preview_narrowed,
    seeded_m5_widening_decision_and_ring_history_registries_widening_decision_beta_narrowed,
    M5WideningDecisionRingHistoryRegistriesPacket,
};

fn main() {
    if let Err(err) = run() {
        eprintln!("{err}");
        std::process::exit(2);
    }
}

fn run() -> Result<(), Box<dyn std::error::Error>> {
    let args: Vec<String> = std::env::args().skip(1).collect();
    match args.first().map(String::as_str) {
        Some("support-export") | None => {
            let packet = seeded_m5_widening_decision_and_ring_history_registries();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("report") => {
            print!(
                "{}",
                seeded_m5_widening_decision_and_ring_history_registries().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!(
                "{}",
                seeded_m5_widening_decision_and_ring_history_registries().render_matrix_csv()
            );
        }
        Some("widening-decision-table") => {
            print!(
                "{}",
                seeded_m5_widening_decision_and_ring_history_registries()
                    .render_widening_decision_table()
            );
        }
        Some("fixture-widening-decision-beta-narrowed") => {
            let packet =
                seeded_m5_widening_decision_and_ring_history_registries_widening_decision_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-ring-history-preview-narrowed") => {
            let packet =
                seeded_m5_widening_decision_and_ring_history_registries_ring_history_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("validate") => {
            for packet in [
                seeded_m5_widening_decision_and_ring_history_registries(),
                seeded_m5_widening_decision_and_ring_history_registries_widening_decision_beta_narrowed(),
                seeded_m5_widening_decision_and_ring_history_registries_ring_history_preview_narrowed(),
            ] {
                assert_valid(&packet)?;
            }
            println!("ok");
        }
        Some(other) => {
            return Err(format!("unknown subcommand: {other}").into());
        }
    }
    Ok(())
}

fn assert_valid(
    packet: &M5WideningDecisionRingHistoryRegistriesPacket,
) -> Result<(), Box<dyn std::error::Error>> {
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(())
    } else {
        let tokens: Vec<&str> = violations.iter().map(|v| v.as_str()).collect();
        Err(format!("registries packet failed validation: {}", tokens.join(",")).into())
    }
}
