...
fn clean_review_pack_record_input() -> M5ShareEligibilityDowngradeRecordEntryResolutionInput {
    M5ShareEligibilityDowngradeRecordEntryResolutionInput {
        ...
        report_section: M5ShareEligibilityDowngradeRecordKind::ChangedFilesScope,
        surface_context: M5ShareEligibilityDowngradeRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5ShareEligibilityDowngradeRecordResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5ShareEligibilityDowngradeResultEntryResolutionInput {
    M5ShareEligibilityDowngradeResultEntryResolutionInput {
        ...
        semantic_role: M5CollaborationStateRole::AuthorityModelDisclosure,
        comparison_scope: M5ShareEligibilityDowngradeResultScope::EvaluatedScopeBinding,
        surface_context: M5ShareEligibilityDowngradeRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5ShareEligibilityDowngradeRecordResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_share_eligibility_and_downgrade_state_registries();
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(
        ...
        M5ShareEligibilityDowngradeRecordNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5ShareEligibilityDowngradeRecordKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5ShareEligibilityDowngradeRecordKind::FullTreeScope;
    ...
    let mut input = clean_review_pack_record_input();
    input.surface_context = M5ShareEligibilityDowngradeRecordSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        ...
        M5ShareEligibilityDowngradeRecordResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input).unwrap_err(),
        M5ShareEligibilityDowngradeRecordResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5ShareEligibilityDowngradeRecordKind::PackScopeUnclassified,
        true,
        false,
        true
    ));
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5ShareEligibilityDowngradeResultScope::ChangeObjectResultUnclassified;
    assert_eq!(
...
fn vocabulary_set_is_canonical() {
    assert!(seeded_m5_share_eligibility_and_downgrade_state_registries()
        .vocabulary_set
        .matches_canonical());
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_SAMPLED_PRESENCE_CURSORS_SELECTIONS_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_HIGHER_RISK_CONTROL_PLANE_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5ShareEligibilityDowngradeRecordAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5ShareEligibilityDowngradeRecordExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::ExamplesMissing));
}
...
fn review_pack_record_not_proven_when_incomplete_example_removed() {
    ...
    for row in &mut packet.registry_rows {
        row.review_pack_record_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
                    M5ShareEligibilityDowngradeRecordEntryDegradeReason::CohortDescriptorObjectIncomplete,
                )
        });
    }
    assert!(packet.validate().contains(
...
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_share_eligibility_and_downgrade_state_registries().export_safe_json();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_share_eligibility_and_downgrade_state_registries_export()
        .expect(
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5ShareEligibilityDowngradeRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-share-eligibility-and-downgrade-state-registries/eligibility_descriptor_beta_narrowed.json"
    )))
    ...
    let preview: M5ShareEligibilityDowngradeRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-share-eligibility-and-downgrade-state-registries/continuity_disposition_preview_narrowed.json"
    )))
