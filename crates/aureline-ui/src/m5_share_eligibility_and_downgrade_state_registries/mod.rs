//! Implemented M5 share-eligibility and downgrade-state registries.
//!
//! The frozen [collaboration-state authority matrix][matrix] names Aureline's governed shared-object classes and
//! locks their authority, convergence, and export vocabulary. Building on the sampled-presence shared-object
//! descriptor domain (which classes a session may share, observe, or must keep local) and the higher-risk
//! control-plane degradation-banner domain it freezes, this module keeps *unsupported object classes* — unshared
//! local buffers, unsupported large-file modes, binary assets, declared generated outputs, and policy-limited
//! artifacts — from implying CRDT or full shared-edit support when search, AI context, review panes, docs links, and
//! support packets, or a user inviting participants or exporting session artifacts, reach for them: whenever a surface
//! considers a candidate shared object, the object class, the eligibility class it earns (local-only, view-only,
//! comment-only, unsupported-share, or full-shared-edit), the authority model that eligibility implies, and whether
//! it may be shared, observed, or must stay local are recorded first, and a limited-eligibility object may be observed
//! or commented on but never presents itself as a full shared-edit CRDT replica without an eligibility class and
//! authority model. So the shared editor replica view, the presence / cursor layer, the comment / annotation /
//! review-pin layer, the collaboration degradation banner, and the support / export and help / docs surfaces inherit
//! one canonical share-eligibility descriptor rather than a hand-authored parallel prose that has to be kept
//! consistent: it turns the *share-eligibility* grammar (one durable append-only record per candidate shared object —
//! one typed field per record section: the object class, the eligibility class it earns, the authority model that
//! eligibility implies, and whether it may be shared, observed, or must stay local, each bound to the object it never
//! upgrades to CRDT or full shared-edit by default) and the *share-continuity-disposition* grammar (the disposition a
//! user is offered before invite, share, or export continues — continue-local-only, share-view-only, share-comment-only,
//! block-unsupported-share, or proceed-full-shared-edit — plus its actor / time provenance, its eligibility facts, and
//! its shared-versus-local lineage) into registry resolvers that produce export-safe, honest projections. Every
//! candidate shared object then resolves to one typed share-eligibility descriptor — the fields it
//! publishes, the object class, the eligibility class, the authority model, and whether it may stay local, all
//! attributable so an unsupported object class is never silently promoted to full shared editing — and to one
//! share-continuity-disposition descriptor — the disposition, its actor / time provenance, and its shared-versus-local
//! lineage — that the search, AI, review-pin, degradation-banner, and support surfaces can inspect before continuing to
//! local-only, view-only, comment-only, block, or full shared editing, so a descriptor that cannot keep an object's
//! true eligibility distinct from full-shared-edit capability degrades honestly instead of
//! letting an unsupported object class read as a shareable CRDT replica when it lacks an eligibility class or authority
//! model.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable share-eligibility descriptor per candidate object, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [object class][M5ShareEligibilityDowngradeRecordKind],
//!   a disclosure role, covers every [resolution form][M5ShareEligibilityDowngradeRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (the
//!   object class, the eligibility class and authority model, whether it may stay local, and the object it is
//!   bound to), keeps its lineage
//!   attributable, and keeps the object it never upgrades to full shared-edit bound to the record; otherwise it degrades.
//! * **Never let an unsupported object class imply full shared-edit support; keep the eligibility class and authority model first.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a record whose
//!   eligibility-first ordering is not held (an unsupported object class presenting itself as full-shared-edit capable while
//!   its eligibility class or authority model was dropped) so it degrades to
//!   [`M5ShareEligibilityDowngradeRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and any surface — a search hit, an AI context window, or a docs link — that
//!   would present an unsupported class as shareable degrades the same way — the structured blocker reason an
//!   unsupported-class-promoted-to-shareable attempt must surface.
//! * **Offer a reviewable share-continuity disposition with actor / time provenance, instead of a silent full-share promotion.**
//!   [`resolve_review_pack_result_entry`] names a classified
//!   [disposition binding][M5ShareEligibilityDowngradeResultScope]
//!   (continue-local-only binding, share-view-only binding, or block-unsupported-share binding), requires the full
//!   object-class / eligibility / disposition / actor-time-provenance / binding / active-reason
//!   record, covers every resolution form, and degrades to
//!   [`M5ShareEligibilityDowngradeResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the disposition would promote an unsupported object class to full shared editing without an eligibility class and authority model, act without actor / time provenance, or
//!   drop the eligibility lineage, so a disposition can never read as trustworthy when it has quietly promoted
//!   a limited-eligibility object to full shared editing or dropped its eligibility.
//!
...
//! export surfaces can never fork their own share-eligibility meaning. Raw secrets, raw command text, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_share_eligibility_and_downgrade_state_registries,
    seeded_m5_share_eligibility_and_downgrade_state_registries_review_pack_record_beta_narrowed,
    seeded_m5_share_eligibility_and_downgrade_state_registries_review_pack_result_preview_narrowed,
    M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_PACKET_ID,
};
...
use crate::m5_collaboration_replica_shared_object_authority_anchor_drift_convergence_and_session_archive_matrix::{
    ...
    M5_SAMPLED_PRESENCE_CURSORS_SELECTIONS_DOMAIN_SCHEMA_REF, M5_HIGHER_RISK_CONTROL_PLANE_DOMAIN_SCHEMA_REF,
    M5_COLLABORATION_STATE_MATRIX_DOC_REF, M5_COLLABORATION_STATE_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5ShareEligibilityDowngradeRecordAndResultRegistriesPacket`].
pub const M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_share_eligibility_and_downgrade_state_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_share_eligibility_and_downgrade_state_registries.md";

/// Repo-relative path of the checked support-export artifact.
pub const M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-share-eligibility-and-downgrade-state-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-share-eligibility-and-downgrade-state-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-share-eligibility-and-downgrade-state-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-share-eligibility-and-downgrade-state-registries";
...
pub type M5ShareEligibilityDowngradeRecordAndResultRegistriesConsumerSurface =
...
pub enum M5ShareEligibilityDowngradeRecordResolutionForm {
...
impl M5ShareEligibilityDowngradeRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ShareEligibilityDowngradeRecordKind {
...
impl M5ShareEligibilityDowngradeRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ShareEligibilityDowngradeResultScope {
...
impl M5ShareEligibilityDowngradeResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ShareEligibilityDowngradeRecordSurfaceContext {
...
impl M5ShareEligibilityDowngradeRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ShareEligibilityDowngradeRecordAnatomyPart {
...
impl M5ShareEligibilityDowngradeRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ShareEligibilityDowngradeRecordNextAction {
...
impl M5ShareEligibilityDowngradeRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ShareEligibilityDowngradeRecordExportField {
...
impl M5ShareEligibilityDowngradeRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ShareEligibilityDowngradeRecordEntryDegradeReason {
...
impl M5ShareEligibilityDowngradeRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5ShareEligibilityDowngradeRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5ShareEligibilityDowngradeRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5ShareEligibilityDowngradeRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5ShareEligibilityDowngradeRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => {
                M5ShareEligibilityDowngradeRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ShareEligibilityDowngradeResultEntryDegradeReason {
...
impl M5ShareEligibilityDowngradeResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
...
pub struct M5ShareEligibilityDowngradeRecordEntryResolutionInput {
    ...
    pub report_section: M5ShareEligibilityDowngradeRecordKind,
    /// The render / surface context.
    pub surface_context: M5ShareEligibilityDowngradeRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ShareEligibilityDowngradeRecordResolutionForm>,
...
pub struct M5ResolvedShareEligibilityDowngradeRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ShareEligibilityDowngradeRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ShareEligibilityDowngradeRecordNextAction,
...
impl M5ResolvedShareEligibilityDowngradeRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ShareEligibilityDowngradeResultEntryResolutionInput {
    ...
    pub comparison_scope: M5ShareEligibilityDowngradeResultScope,
    /// The render / surface context.
    pub surface_context: M5ShareEligibilityDowngradeRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ShareEligibilityDowngradeRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedShareEligibilityDowngradeResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ShareEligibilityDowngradeResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ShareEligibilityDowngradeRecordNextAction,
...
impl M5ResolvedShareEligibilityDowngradeResultEntry {
...
pub enum M5ShareEligibilityDowngradeRecordResolutionError {
...
impl M5ShareEligibilityDowngradeRecordResolutionError {
...
impl fmt::Display for M5ShareEligibilityDowngradeRecordResolutionError {
...
impl Error for M5ShareEligibilityDowngradeRecordResolutionError {}

fn form_tokens(forms: &[M5ShareEligibilityDowngradeRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5ShareEligibilityDowngradeRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5ShareEligibilityDowngradeRecordResolutionForm> =
        forms.iter().copied().collect();
    M5ShareEligibilityDowngradeRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5ShareEligibilityDowngradeRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5ShareEligibilityDowngradeResultScope,
...
pub fn resolve_review_pack_record_entry(
    input: M5ShareEligibilityDowngradeRecordEntryResolutionInput,
) -> Result<
    M5ResolvedShareEligibilityDowngradeRecordEntry,
    M5ShareEligibilityDowngradeRecordResolutionError,
> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ShareEligibilityDowngradeRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5ShareEligibilityDowngradeRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ShareEligibilityDowngradeRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ShareEligibilityDowngradeRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5ShareEligibilityDowngradeRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5ShareEligibilityDowngradeRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5ShareEligibilityDowngradeRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5ShareEligibilityDowngradeRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5ShareEligibilityDowngradeRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5ShareEligibilityDowngradeRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5ShareEligibilityDowngradeRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5ShareEligibilityDowngradeRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedShareEligibilityDowngradeRecordEntry {
...
pub fn resolve_review_pack_result_entry(
    input: M5ShareEligibilityDowngradeResultEntryResolutionInput,
) -> Result<
    M5ResolvedShareEligibilityDowngradeResultEntry,
    M5ShareEligibilityDowngradeRecordResolutionError,
> {
    if input.entry_id.trim().is_empty() {
        return Err(
            M5ShareEligibilityDowngradeRecordResolutionError::EmptyCohortEvidencePacketEntryId,
        );
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ShareEligibilityDowngradeResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ShareEligibilityDowngradeResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5ShareEligibilityDowngradeResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5ShareEligibilityDowngradeResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5ShareEligibilityDowngradeResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5ShareEligibilityDowngradeResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5ShareEligibilityDowngradeRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedShareEligibilityDowngradeResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ShareEligibilityDowngradeRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5ShareEligibilityDowngradeRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5ShareEligibilityDowngradeRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5ShareEligibilityDowngradeRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedShareEligibilityDowngradeRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedShareEligibilityDowngradeResultEntry>,
...
impl M5ShareEligibilityDowngradeRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5ShareEligibilityDowngradeRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5ShareEligibilityDowngradeRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5ShareEligibilityDowngradeRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5ShareEligibilityDowngradeRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedShareEligibilityDowngradeRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedShareEligibilityDowngradeResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ShareEligibilityDowngradeRecordAndResultRegistriesVocabularySet {
...
impl M5ShareEligibilityDowngradeRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5CollaborationStateRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5ShareEligibilityDowngradeRecordResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5ShareEligibilityDowngradeRecordKind::ALL, |v| {
                v.as_str()
            }),
            comparison_scopes: tokens(&M5ShareEligibilityDowngradeResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5ShareEligibilityDowngradeRecordSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            review_pack_record_degrade_reasons: tokens(
                &M5ShareEligibilityDowngradeRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5ShareEligibilityDowngradeResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5ShareEligibilityDowngradeRecordAnatomyPart::ALL, |v| {
                v.as_str()
            }),
            next_actions: tokens(&M5ShareEligibilityDowngradeRecordNextAction::ALL, |v| {
                v.as_str()
            }),
            export_fields: tokens(&M5ShareEligibilityDowngradeRecordExportField::ALL, |v| {
                v.as_str()
            }),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ShareEligibilityDowngradeRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ShareEligibilityDowngradeRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ShareEligibilityDowngradeRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ShareEligibilityDowngradeRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5ShareEligibilityDowngradeRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ShareEligibilityDowngradeRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5ShareEligibilityDowngradeRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5ShareEligibilityDowngradeRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5ShareEligibilityDowngradeRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5ShareEligibilityDowngradeRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5ShareEligibilityDowngradeRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5ShareEligibilityDowngradeRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ShareEligibilityDowngradeRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_SCHEMA_VERSION`].
...
impl M5ShareEligibilityDowngradeRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5ShareEligibilityDowngradeRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_RECORD_KIND {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::WrongRecordKind,
            );
        }
        if self.schema_version != M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_SCHEMA_VERSION
        {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::MissingIdentity,
            );
        }
        ...
        if !self.vocabulary_set.matches_canonical() {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::VocabularySetDrift,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::RawMaterialInExport,
            );
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Share-Eligibility and Downgrade-State Registries\n\n");
...
#[derive(Debug)]
pub enum M5ShareEligibilityDowngradeRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5ShareEligibilityDowngradeRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5ShareEligibilityDowngradeRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5ShareEligibilityDowngradeRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation {
...
impl M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_share_eligibility_and_downgrade_state_registries_export() -> Result<
    M5ShareEligibilityDowngradeRecordAndResultRegistriesPacket,
    M5ShareEligibilityDowngradeRecordAndResultRegistriesArtifactError,
> {
    let packet: M5ShareEligibilityDowngradeRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-share-eligibility-and-downgrade-state-registries-proof/support_export.json"
        )
    ))
    .map_err(M5ShareEligibilityDowngradeRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(
            M5ShareEligibilityDowngradeRecordAndResultRegistriesArtifactError::Validation(
                violations,
            ),
        )
    }
...
fn validate_source_contracts(
    packet: &M5ShareEligibilityDowngradeRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation>,
) {
    let refs: BTreeSet<&str> = packet
        ...
        .collect();
    for required in [
        M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_SCHEMA_REF,
        M5_SHARE_ELIGIBILITY_AND_DOWNGRADE_STATE_REGISTRIES_DOC_REF,
        ...
        if !refs.contains(required) {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations
            .push(M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_SAMPLED_PRESENCE_CURSORS_SELECTIONS_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_HIGHER_RISK_CONTROL_PLANE_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::ExamplesMissing,
            );
        }
        if !row.examples_are_honest() {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::DishonestExample,
            );
        }
        if !row.honours_invariants() {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5ShareEligibilityDowngradeRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5ShareEligibilityDowngradeRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    let unbound_degrades = review_pack_records().any(|ex| {
        ex.degrade_reason
            == Some(
                M5ShareEligibilityDowngradeRecordEntryDegradeReason::DescriptorNotBoundToRegistry,
            )
    });
    ...
    {
        violations.push(
            M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ShareEligibilityDowngradeResultScope::CANONICAL_SCOPES
        .iter()
    ...
    let form_incomplete_degrades = downgrade().any(|ex| {
        ex.degrade_reason
            == Some(
                M5ShareEligibilityDowngradeResultEntryDegradeReason::EvidenceFormCoverageIncomplete,
            )
    });
    ...
    {
        violations.push(
            M5ShareEligibilityDowngradeRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
