...
fn seed() -> M5BuildLaneTrustSharedConsumersPacket {
    seeded_m5_build_lane_trust_shared_consumers()
}

fn violations_of(packet: &M5BuildLaneTrustSharedConsumersPacket) -> Vec<&'static str> {
...
fn seed_validates_clean() {
    ...
    assert_eq!(
        packet.record_kind,
        M5_BUILD_LANE_TRUST_SHARED_CONSUMERS_RECORD_KIND
    );
...
fn every_family_is_adopted_by_two_or_more_consumers() {
    ...
    assert_eq!(family_consumers.len(), 4, "all four families adopted");
...
fn every_consumer_surface_and_representation_is_exercised() {
    ...
    let representations: BTreeSet<_> = packet
        ...
        .collect();
    for representation in BuildLaneTrustRepresentation::ALL {
        assert!(
        ...
        match profile_facets.get(binding.build_profile_id.as_str()) {
            ...
            Some(existing) => assert_eq!(
                **existing, binding.state_facets,
                "grammar drift on {}",
                binding.build_profile_id
            ),
        }
    }
    // The four profiles each fan out to more than one consumer.
    assert_eq!(profile_facets.len(), 4);
}

#[test]
fn every_build_lane_trust_role_word_is_a_frozen_role_token() {
    let packet = seed();
    for binding in &packet.consumer_bindings {
        assert!(
            binding
        ...
        assert!(binding.state_facets.replay_continuity_satisfied());
    }
}
    ...
    for binding in &packet.consumer_bindings {
        ...
        if binding.is_narrowed() {
            assert_eq!(
                ...
                BuildLaneTrustParityState::FacetsDisclosedNarrowed
            );
            let note = binding
                .narrow_note
                .as_ref()
            ...
            assert_eq!(
                binding.parity_state,
                BuildLaneTrustParityState::FacetsPreserved
            );
            assert!(binding.narrow_note.is_none());
        }
        if matches!(
            binding.representation,
            BuildLaneTrustRepresentation::RemoteProjected
        ) {
            assert!(!binding.remote_source_note.trim().is_empty());
        }
        if matches!(
            binding.representation,
            BuildLaneTrustRepresentation::ExportedRedacted
        ) {
...
fn disclosure_resolver_matches_representation() {
    assert!(
        !resolve_build_lane_trust_render_disclosure(BuildLaneTrustRepresentation::DesktopFull)
            .needs_narrow_note
    );
    let compact =
        resolve_build_lane_trust_render_disclosure(BuildLaneTrustRepresentation::CompactNarrowed);
    assert_eq!(
        compact.narrow_reason,
        Some(BuildLaneTrustNarrowReason::CompactionNarrowed)
    );
    assert!(compact.needs_narrow_note);
    assert!(!compact.needs_remote_source_note);
    let remote =
        resolve_build_lane_trust_render_disclosure(BuildLaneTrustRepresentation::RemoteProjected);
    assert!(remote.needs_remote_source_note);
    let exported =
        resolve_build_lane_trust_render_disclosure(BuildLaneTrustRepresentation::ExportedRedacted);
...
fn grammar_drift_is_rejected() {
    ...
    let target = packet
        .consumer_bindings
        .iter()
        .position(|b| b.binding_id == "bltsc-protected-merge-shiproom")
        .unwrap();
    packet.consumer_bindings[target]
        .state_facets
        .build_lane_trust_role_word = "credential_boundary".to_owned();
    assert!(violations_of(&packet).contains(&"build_lane_trust_grammar_drift_across_surfaces"));
}

#[test]
fn build_lane_trust_role_word_outside_vocabulary_is_rejected() {
    let mut packet = seed();
    packet.consumer_bindings[0]
        .state_facets
        .build_lane_trust_role_word = "totally_made_up".to_owned();
    let tokens = violations_of(&packet);
    assert!(tokens.contains(&"build_lane_trust_role_word_outside_vocabulary"));
    // Rewording one surface also trips drift, which is expected and fine.
}

#[test]
fn dropped_replay_continuity_on_trust_role_is_rejected() {
    let mut packet = seed();
    // bltsc-protected-merge-release-center carries the publication_authority role, which must always
    // keep a real replay continuity.
    let target = packet
        .consumer_bindings
        .iter()
        .position(|b| b.binding_id == "bltsc-protected-merge-release-center")
        .unwrap();
    packet.consumer_bindings[target]
        .state_facets
        .replay_continuity_word = "treated_a_remote_cache_hit_as_reproducibility_proof".to_owned();
    assert!(violations_of(&packet).contains(&"replay_continuity_missing_for_trust_role"));
}

#[test]
fn reuse_below_two_consumers_is_rejected() {
    let mut packet = seed();
    // Drop every emergency-hotfix binding except one, leaving the family with one consumer.
    let mut kept_one = false;
    packet.consumer_bindings.retain(|b| {
        if b.family == M5BuildLaneFamily::EmergencyHotfix {
            if kept_one {
                return false;
            }
            kept_one = true;
...
fn missing_canonical_reference_on_export_is_rejected() {
    ...
    packet.consumer_bindings[target].source_contract_refs =
        vec![M5_BUILD_LANE_TRUST_MATRIX_DOC_REF.to_owned()];
...
fn unexpected_narrow_note_on_full_binding_is_rejected() {
    ...
    let target = packet
        ...
        .position(|b| !b.is_narrowed())
        .unwrap();
    packet.consumer_bindings[target].narrow_note = Some(BuildLaneTrustNarrowNote {
        reason: BuildLaneTrustNarrowReason::CompactionNarrowed,
        preserved_grammar_note: "x".to_owned(),
        next_action: BuildLaneTrustNarrowNextAction::ExpandInDesktop,
...
fn each_guardrail_is_enforced() {
    let cases: [(fn(&mut BuildLaneTrustConsumerBinding), &str); 5] = [
        ...
        (
            |b| b.treats_remote_cache_hits_as_reproducibility_proof = true,
            "treats_remote_cache_hits_as_reproducibility_proof",
        ),
        (
            |b| b.lets_docs_schema_sbom_or_symbol_sidecars_drift_from_binary_build_identity = true,
            "lets_docs_schema_sbom_or_symbol_sidecars_drift_from_binary_build_identity",
        ),
        (
            |b| {
                b.overclaims_clean_room_parity_when_only_partial_artifact_classes_were_rebuilt = true
            },
            "overclaims_clean_room_parity_when_only_partial_artifact_classes_were_rebuilt",
        ),
        (
            |b| {
                b.hides_non_hermetic_inputs_cache_poisoning_or_unreplayable_artifacts_behind_green_publication_rows = true
            },
...
fn family_coverage_gap_is_rejected() {
    ...
    packet
        .consumer_bindings
        .retain(|b| b.family != M5BuildLaneFamily::EmergencyHotfix);
...
fn missing_source_contract_is_rejected() {
    ...
    packet
        ...
        .retain(|r| r != M5_BUILD_LANE_TRUST_SHARED_CONSUMERS_DOC_REF);
...
fn csv_has_a_row_per_binding() {
    ...
    assert!(lines[0]
        .starts_with("family,consumer,representation,build_lane_trust_role_word,parity_state"));
}
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_build_lane_trust_shared_consumers_export()
        .expect("checked M5 build-lane-trust shared-consumer export validates");
    assert_eq!(
        from_disk.packet_id,
        M5_BUILD_LANE_TRUST_SHARED_CONSUMERS_PACKET_ID
    );
...
fn narrowed_fixture_variants_validate_and_keep_coverage() {
    let compact = seeded_m5_build_lane_trust_shared_consumers_compact_remote_narrowed();
    assert!(
    ...
    let exported = seeded_m5_build_lane_trust_shared_consumers_exported_redaction_narrowed();
    assert!(
...
fn checked_narrowed_fixtures_validate_and_match_builders() {
    let compact: M5BuildLaneTrustSharedConsumersPacket =
        serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-build-lane-trust-shared-consumers/compact_remote_narrowed.json"
    )))
    ...
    let exported: M5BuildLaneTrustSharedConsumersPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-build-lane-trust-shared-consumers/exported_redaction_narrowed.json"
    )))
