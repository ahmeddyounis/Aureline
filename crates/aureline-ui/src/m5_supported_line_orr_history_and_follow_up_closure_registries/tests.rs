...
fn clean_orr_history_event_input() -> M5OrrHistoryEventEntryResolutionInput {
    M5OrrHistoryEventEntryResolutionInput {
        entry_id: "orr_history_event:test".to_owned(),
        line_binding_id: "launch.line.core-team-canary".to_owned(),
        token_name: "line.orr_history_event.core_team_canary".to_owned(),
        semantic_role: M5SupportedLineTransparencyRole::FreshnessWindow,
        report_section: M5OrrHistoryEventKind::OrrPacketArchive,
        surface_context: M5SupportedLineOrrHistoryEventSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SupportedLineOrrHistoryEventResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5FollowUpClosureEntryResolutionInput {
    M5FollowUpClosureEntryResolutionInput {
        ...
        semantic_role: M5SupportedLineTransparencyRole::FreshnessWindow,
        comparison_scope: M5FollowUpClosureScope::UnclosedActionItem,
        surface_context: M5SupportedLineOrrHistoryEventSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SupportedLineOrrHistoryEventResolutionForm::ALL.to_vec(),
...
#[test]
fn orr_history_event_clean_names_meaning_and_is_bound() {
    let resolved = resolve_orr_history_event_entry(clean_orr_history_event_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.orr_history_event_resolves_across_lines);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.orr_history_event_object_complete);
    ...
    assert_eq!(resolved.report_section, "orr_packet_archive");
    assert_eq!(
        resolved.canonical_report_section_mode,
        "orr_packet_archive_mode"
    );
    assert_eq!(resolved.surface_context, "shiproom_surface");
    assert_eq!(
        resolved.next_action,
        M5SupportedLineOrrHistoryEventNextAction::ExpandCohortMeaning
    );
}

#[test]
fn orr_history_event_token_unstated_degrades() {
    let mut input = clean_orr_history_event_input();
    input.token_name = "   ".to_owned();
    assert_eq!(
        resolve_orr_history_event_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn orr_history_event_unbound_and_unclassified_degrade() {
    let mut input = clean_orr_history_event_input();
    ...
    let mut input = clean_orr_history_event_input();
    input.report_section = M5OrrHistoryEventKind::EventClassUnclassified;
    assert_eq!(
        resolve_orr_history_event_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn orr_history_event_object_incomplete_and_widen_fold_and_form_degrade() {
    // An unstated bundle IDs field leaves the resolved object incomplete.
    let mut input = clean_orr_history_event_input();
    input.bundle_ids = "  ".to_owned();
    let resolved = resolve_orr_history_event_entry(input).unwrap();
    assert!(!resolved.orr_history_event_object_complete);
    assert_eq!(
    ...
    assert_eq!(
        resolve_orr_history_event_entry(input).unwrap().degrade_reason,
...
#[test]
fn orr_history_event_public_facing_and_surface_and_proof_degrade() {
    // A public-facing line running support language ahead of proof first fails the widen-boundary fold.
    let mut input = clean_orr_history_event_input();
    input.report_section = M5OrrHistoryEventKind::GoNoGoDecision;
    ...
    let mut input = clean_orr_history_event_input();
    input.surface_context = M5SupportedLineOrrHistoryEventSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_orr_history_event_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn orr_history_event_empty_id_and_forbidden_material_error() {
    let mut input = clean_orr_history_event_input();
    input.entry_id = "".to_owned();
    assert_eq!(
        resolve_orr_history_event_entry(input).unwrap_err(),
        M5SupportedLineOrrHistoryEventResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_orr_history_event_entry(input).unwrap_err(),
        M5SupportedLineOrrHistoryEventResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5OrrHistoryEventKind::EventClassUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn orr_history_event_object_is_complete_requires_all_fields() {
    assert!(orr_history_event_object_is_complete(
        M5OrrHistoryEventKind::OrrPacketArchive,
...
fn downgrade_clean_stays_honest() {
    let resolved = resolve_follow_up_closure_entry(clean_downgrade_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.comparison_safe_on_every_line);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.provides_complete_follow_up_closure);
    assert!(resolved.follow_up_closure_stays_honest);
    assert_eq!(resolved.comparison_scope, "unclosed_action_item");
    assert_eq!(resolved.surface_context, "shiproom_surface");
}
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let resolved = resolve_follow_up_closure_entry(input).unwrap();
    assert!(!resolved.provides_complete_follow_up_closure);
    assert_eq!(
        resolved.degrade_reason,
    ...
    let mut input = clean_downgrade_input();
    input.keeps_follow_up_closure_visible = false;
    assert_eq!(
        resolve_follow_up_closure_entry(input).unwrap().degrade_reason,
        Some(M5FollowUpClosureEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    );
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5FollowUpClosureScope::ClosureScopeUnclassified;
    assert_eq!(
        resolve_follow_up_closure_entry(input)
            .unwrap()
            .degrade_reason,
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    input.entry_id = "  ".to_owned();
    assert_eq!(
        resolve_follow_up_closure_entry(input).unwrap_err(),
...
fn downgrade_bound_support_and_flagged_gap_stay_clean() {
    ...
    assert!(resolve_follow_up_closure_entry(input).unwrap().is_clean());
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_SUPPORTED_LINE_ORR_HISTORY_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_FOLLOW_UP_CLOSURE_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5SupportedLineOrrHistoryEventFollowUpClosureRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5SupportedLineOrrHistoryEventAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5SupportedLineOrrHistoryEventExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    let mut packet = seeded_m5_supported_line_orr_history_and_follow_up_closure_registries();
    packet.registry_rows[0].follow_up_closure_entries.clear();
    assert!(packet.validate().contains(
...
fn dishonest_clean_example_fails() {
    let mut packet = seeded_m5_supported_line_orr_history_and_follow_up_closure_registries();
    // Force a clean orr_history_event entry to also read as object-incomplete — the packet must reject it.
    let row = &mut packet.registry_rows[0];
    row.orr_history_event_entries[0].degrade_reason = None;
    row.orr_history_event_entries[0].orr_history_event_object_complete = false;
    assert!(packet.validate().contains(
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        match mutate {
            ...
            _ => row.collapses_distinct_follow_up_closure_classes_into_one_lane = true,
        }
        assert!(packet.validate().contains(
...
#[test]
fn orr_history_event_not_proven_when_incomplete_example_removed() {
    let mut packet = seeded_m5_supported_line_orr_history_and_follow_up_closure_registries();
    for row in &mut packet.registry_rows {
        row.orr_history_event_entries.retain(|ex| {
            ex.degrade_reason
                != Some(M5OrrHistoryEventEntryDegradeReason::CohortDescriptorObjectIncomplete)
        });
    }
...
#[test]
fn orr_history_event_not_proven_when_surface_collapses() {
    let mut packet = seeded_m5_supported_line_orr_history_and_follow_up_closure_registries();
    // Drop every clean executive-steering-surface orr_history_event so the first-consumer surfaces no longer include it.
    for row in &mut packet.registry_rows {
        row.orr_history_event_entries
...
#[test]
fn follow_up_closure_integrity_not_proven_when_support_ahead_example_removed() {
    let mut packet = seeded_m5_supported_line_orr_history_and_follow_up_closure_registries();
    for row in &mut packet.registry_rows {
        row.follow_up_closure_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
...
#[test]
fn follow_up_closure_integrity_not_proven_when_scope_dropped() {
    let mut packet = seeded_m5_supported_line_orr_history_and_follow_up_closure_registries();
    // Drop every clean go-no-go-signoff downgrade so the coverage no longer includes it.
    for row in &mut packet.registry_rows {
        row.follow_up_closure_entries.retain(|ex| {
            !(ex.is_clean() && ex.comparison_scope == "unreconstructable_line_history")
        });
    }
    assert!(packet.validate().contains(
        &M5SupportedLineOrrHistoryEventFollowUpClosureRegistriesViolation::CohortEvidenceIntegrityNotProven
...
fn export_carries_no_forbidden_raw_material() {
    let json =
        seeded_m5_supported_line_orr_history_and_follow_up_closure_registries().export_safe_json();
...
#[test]
fn orr_history_event_table_lists_only_clean_orr_history_events() {
    let packet = seeded_m5_supported_line_orr_history_and_follow_up_closure_registries();
    let table = packet.render_orr_history_event_table();
    // The clean canary and migration orr_history_events are rendered from the registry.
    assert!(table.contains("orr_packet_archive_mode"));
    assert!(table.contains("freeze_exception_mode"));
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk =
        current_stable_m5_supported_line_orr_history_and_follow_up_closure_registries_export()
            .expect(
            "checked M5 line-orr_history_event / line-downgrade-packet registries export validates",
        );
    assert_eq!(
        from_disk.packet_id,
        M5_SUPPORTED_LINE_ORR_HISTORY_EVENT_FOLLOW_UP_CLOSURE_REGISTRIES_PACKET_ID
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5SupportedLineOrrHistoryEventFollowUpClosureRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-supported-line-orr-history-and-follow-up-closure-registries/orr_history_event_beta_narrowed.json"
    )))
    .expect("line-orr_history_event fixture parses");
    ...
    let preview: M5SupportedLineOrrHistoryEventFollowUpClosureRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-supported-line-orr-history-and-follow-up-closure-registries/follow_up_closure_preview_narrowed.json"
    )))
