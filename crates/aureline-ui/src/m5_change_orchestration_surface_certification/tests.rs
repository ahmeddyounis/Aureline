//! Tests for the M05-1303 change-orchestration surface certification capstone.

use super::*;

fn packet() -> ChangeOrchestrationProfileCertificationPacket {
    seeded_m5_change_orchestration_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, CHANGE_ORCHESTRATION_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, CHANGE_ORCHESTRATION_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5ChangeOrchestrationCertifiedProfile::ALL {
...
fn every_frozen_family_is_certified_on_some_profile() {
    ...
    for family in M5ChangeOrchestrationObject::ALL {
        assert!(
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            ChangeOrchestrationCertificationAxis::ALL.len()
        );
    }
...
fn all_eight_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5ChangeOrchestrationCertClaim> =
        p.rows.iter().map(|r| r.certified_claim).collect();
    for claim in M5ChangeOrchestrationCertClaim::ALL {
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5ChangeOrchestrationCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
...
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = ChangeOrchestrationCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), ChangeOrchestrationCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in ChangeOrchestrationCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == ChangeOrchestrationCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5ChangeOrchestrationCertifiedProfile::StaleValidationShelfProfile
        })
        .expect("local-only-resolution-authority-profile row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(ChangeOrchestrationCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(ChangeOrchestrationCertificationAxis::Localization)
    );
}
...
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    ...
    for row in &packet().rows {
        let outcome = row
            .axis(ChangeOrchestrationCertificationAxis::HighContrast)
...
fn the_six_yellow_rows_bind_six_distinct_axes_and_triggers() {
    ...
    let p = packet();
    let axes: BTreeSet<ChangeOrchestrationCertificationAxis> = p
        .rows
        .iter()
        .filter_map(|r| r.claim_auto_narrow.as_ref().map(|n| n.binding_axis))
        .collect();
    assert_eq!(axes.len(), 6, "yellow rows must bind six distinct axes");
    let triggers: std::collections::HashSet<M5ChangeOrchestrationDowngradeTrigger> = p
        .rows
        .iter()
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5ChangeOrchestrationCertifiedProfile::CertifiedChangeOrchestrationLane
        })
        .expect("certified-change-orchestration-lane row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ChangeOrchestrationCertificationAxis::Visual {
            outcome.state = ChangeOrchestrationAxisCertificationState::UndisclosedDrift;
    ...
    assert_eq!(
        row.derived_status,
        ChangeOrchestrationProfileClaimStatus::Red
    );
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ChangeOrchestrationCertificationViolation::ProfileBlocked { .. }
    )));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5ChangeOrchestrationCertifiedProfile::ReviewableChangeOrchestrationRecordStructure
        })
        .expect("reviewable-change-orchestration-record-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails
        .infers_stack_membership_from_branch_names_alone = true;
    assert_eq!(
        row.derive_status(),
        ChangeOrchestrationProfileClaimStatus::Red
    );
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5ChangeOrchestrationCertClaim::CertifiedChangeOrchestrationTruth;
    row.certified_claim = M5ChangeOrchestrationCertClaim::CertifiedChangeOrchestrationTruth;
    assert_eq!(
        row.derive_status(),
        ChangeOrchestrationProfileClaimStatus::Red
    );
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ChangeOrchestrationCertificationAxis::DegradedState {
            *outcome = ChangeOrchestrationAxisOutcome {
                axis: ChangeOrchestrationCertificationAxis::DegradedState,
    ...
    // Claim stays ReviewableChangeOrchestrationRecord == certified, no claim_auto_narrow.
    assert_eq!(
        row.derive_status(),
        ChangeOrchestrationProfileClaimStatus::Red
    );
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ChangeOrchestrationCertificationAxis::CliExport {
            outcome.state = ChangeOrchestrationAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger =
                Some(M5ChangeOrchestrationDowngradeTrigger::WorktreeBindingUnstated);
        }
    }
    assert_eq!(
        row.derive_status(),
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(ChangeOrchestrationClaimAutoNarrow {
        binding_axis: ChangeOrchestrationCertificationAxis::DegradedState,
        from_claim: M5ChangeOrchestrationCertClaim::CertifiedChangeOrchestrationTruth,
        to_claim: M5ChangeOrchestrationCertClaim::CertifiedChangeOrchestrationTruth,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5ChangeOrchestrationCertClaim::WorktreeBindingUnverifiedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-change-orchestration-record-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableChangeOrchestrationRecord; certify a stronger certified operating line.
    row.certified_claim = M5ChangeOrchestrationCertClaim::CertifiedChangeOrchestrationTruth;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5ChangeOrchestrationCertifiedProfile::UnboundWorktreeBindingProfile
        })
        .expect("local-only-or-reconcile-commit-state-profile row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = ChangeOrchestrationCertificationAxis::Visual;
    }
    assert_eq!(
        row.derive_status(),
        ChangeOrchestrationProfileClaimStatus::Red
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = ChangeOrchestrationCertificationAxis::CliExport;
    }
    assert_eq!(
        row.derive_status(),
        ChangeOrchestrationProfileClaimStatus::Red
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == ChangeOrchestrationProfileClaimStatus::Yellow)
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(ChangeOrchestrationCertificationAxis::Visual);
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows.retain(|r| {
        r.profile != M5ChangeOrchestrationCertifiedProfile::AmbientBranchLandingProfile
    });
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ChangeOrchestrationCertificationViolation::ProfileCoverageIncomplete
    )));
}
...
fn missing_family_coverage_is_rejected() {
    ...
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5ChangeOrchestrationObject::PatchStackQueue);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ChangeOrchestrationCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = ChangeOrchestrationProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ChangeOrchestrationCertificationViolation::WrongCanonicalBundle
    )));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ChangeOrchestrationCertificationViolation::DuplicateId { .. }
    )));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != ChangeOrchestrationCertificationAxis::ChangeOrchestrationTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ChangeOrchestrationCertificationViolation::SummaryMismatch
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: ChangeOrchestrationProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_change_orchestration_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-change-orchestration-surface-certification/support_export.json"
    ));
    let packet_from_fixture: ChangeOrchestrationProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-change-orchestration-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_CHANGE_ORCHESTRATION_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures from the
// seed builder. Off by default so a normal `cargo test` never mutates the tree.
#[test]
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_CHANGE_ORCHESTRATION_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_change_orchestration_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/release/m5-change-orchestration-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-change-orchestration-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/teamwork/m5-change-orchestration-surface-certification");
