//! Implemented M5 comment-annotation-review-pin anchor-history and rebind-review registries.
//!
//! The frozen [collaboration-state authority matrix][matrix] names Aureline's governed shared-object classes and
//! locks their authority, convergence, and export vocabulary. Building on the server-ordered comments / annotations
//! / review-pins anchor-history domain (the append-only drift ledger a pin never silently overwrites) and the
//! presenter / follow convergence-state domain it freezes, this module makes collaboration *anchors* as honest and
//! reviewable as other Aureline evidence objects: when a comment, annotation, or review pin has to move — because
//! the buffer was edited, a generated output refreshed, or an imported snapshot transitioned — its textual anchor,
//! semantic anchor, and revision-pair lineage are recorded first, and it lands in an explicit drift / unresolved /
//! rebound state rather than silently jumping to a new location. So the shared editor replica view, the presence /
//! cursor layer, the comment / annotation / review-pin layer, the collaboration degradation banner, and the
//! support / export and help / docs surfaces inherit one canonical anchor-history record rather than a hand-authored
//! parallel prose that has to be kept consistent: it turns the *anchor-history* grammar (one durable append-only
//! record per anchor — one typed field per record section: the comment / annotation / review-pin identity, the
//! textual and semantic anchors, the revision-pair lineage across which the anchor moved, the drift / unresolved /
//! rebound state it lands in, and its export posture, each bound to the append-only history it never rewrites) and
//! the *rebind-review* grammar (the rebind decision offered on a drifted anchor — keep-textual, keep-semantic,
//! manual-rebind, assisted-rebind, or leave-unresolved — plus its actor / time provenance, its convergence and
//! merge-drift posture, and its policy-labeled export lineage) into registry resolvers that produce export-safe,
//! honest projections. Every drift then resolves to one typed anchor-history record — the fields it publishes,
//! the identity of the comment / annotation / review pin, the textual and semantic anchors and revision-pair
//! lineage, the drift / unresolved / rebound state, and its export posture, all attributable so an anchor is never
//! rebound implicitly — and to one rebind-review disposition — the decision, its actor / time provenance, and its
//! export lineage — that the shared editor, presence, review-pin, degradation-banner, and support surfaces can
//! inspect before rejoining, exporting, or discarding, so a rebind that cannot bind its anchor history degrades
//! honestly instead of letting a drift read as clean when a pin silently jumped without a recorded, reviewable
//! decision.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable anchor-history record per drift, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [drift / rebound state][M5AnchorHistoryRebindRecordKind],
//!   a disclosure role, covers every [resolution form][M5AnchorHistoryRebindRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (the
//!   comment / annotation / review-pin identity, the textual and semantic anchors, the revision-pair lineage, the
//!   drift / unresolved / rebound state, and the export posture bound to the anchor identity), keeps its lineage
//!   attributable, and keeps the append-only history it never rewrites bound to the record; otherwise it degrades.
//! * **Never silently rebind an anchor; preserve append-only drift history first on any move.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a record whose
//!   append-only-history preservation is not held (a drift presenting itself as clean while a comment or pin jumped
//!   without recorded drift history) so it degrades to
//!   [`M5AnchorHistoryRebindRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and any move — buffer edit, generated-output refresh, or imported-snapshot transition — that
//!   would rebind a pin without drift history degrades the same way — the structured blocker reason a
//!   silent-jump attempt must surface.
//! * **Offer a reviewable rebind decision with actor / time provenance, instead of a silent auto-rebind.**
//!   [`resolve_review_pack_result_entry`] names a classified
//!   [rebind binding][M5AnchorHistoryRebindResultScope]
//!   (semantic-rebind binding, textual-rebind binding, or leave-unresolved binding), requires the full
//!   anchor-identity / rebind-decision / actor-time-provenance / binding / active-reason
//!   record, covers every resolution form, and degrades to
//!   [`M5AnchorHistoryRebindResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the rebind would move an anchor without review, act without actor / time provenance, or let an
//!   export drop its redaction lineage, so a rebind can never read as trustworthy when it has quietly moved
//!   the anchor or dropped its provenance.
//!
...
//! export surfaces can never fork their own anchor-history / rebind meaning. Raw secrets, raw command text, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    ...
    seeded_m5_comment_annotation_review_pin_anchor_history_and_rebind_review_registries_review_pack_record_beta_narrowed,
    seeded_m5_comment_annotation_review_pin_anchor_history_and_rebind_review_registries_review_pack_result_preview_narrowed,
    M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_PACKET_ID,
};
...
use crate::m5_collaboration_replica_shared_object_authority_anchor_drift_convergence_and_session_archive_matrix::{
    ...
    M5_SERVER_ORDERED_COMMENTS_ANNOTATIONS_REVIEW_PINS_DOMAIN_SCHEMA_REF, M5_PRESENTER_FOLLOW_STATE_DOMAIN_SCHEMA_REF,
    M5_COLLABORATION_STATE_MATRIX_DOC_REF, M5_COLLABORATION_STATE_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5AnchorHistoryRebindRecordAndResultRegistriesPacket`].
pub const M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_comment_annotation_review_pin_anchor_history_and_rebind_review_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_SCHEMA_VERSION:
    u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_comment_annotation_review_pin_anchor_history_and_rebind_review_registries.md";

/// Repo-relative path of the checked support-export artifact.
pub const M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-comment-annotation-review-pin-anchor-history-and-rebind-review-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-comment-annotation-review-pin-anchor-history-and-rebind-review-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-comment-annotation-review-pin-anchor-history-and-rebind-review-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5AnchorHistoryRebindRecordAndResultRegistriesConsumerSurface =
...
pub enum M5AnchorHistoryRebindRecordResolutionForm {
...
impl M5AnchorHistoryRebindRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AnchorHistoryRebindRecordKind {
...
impl M5AnchorHistoryRebindRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AnchorHistoryRebindResultScope {
...
impl M5AnchorHistoryRebindResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AnchorHistoryRebindRecordSurfaceContext {
...
impl M5AnchorHistoryRebindRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AnchorHistoryRebindRecordAnatomyPart {
...
impl M5AnchorHistoryRebindRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AnchorHistoryRebindRecordNextAction {
...
impl M5AnchorHistoryRebindRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AnchorHistoryRebindRecordExportField {
...
impl M5AnchorHistoryRebindRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AnchorHistoryRebindRecordEntryDegradeReason {
...
impl M5AnchorHistoryRebindRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5AnchorHistoryRebindRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5AnchorHistoryRebindRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5AnchorHistoryRebindRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5AnchorHistoryRebindRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5AnchorHistoryRebindRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AnchorHistoryRebindResultEntryDegradeReason {
...
impl M5AnchorHistoryRebindResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5AnchorHistoryRebindRecordNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5AnchorHistoryRebindRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5AnchorHistoryRebindRecordEntryResolutionInput {
    ...
    pub report_section: M5AnchorHistoryRebindRecordKind,
    /// The render / surface context.
    pub surface_context: M5AnchorHistoryRebindRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5AnchorHistoryRebindRecordResolutionForm>,
...
pub struct M5ResolvedAnchorHistoryRebindRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5AnchorHistoryRebindRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5AnchorHistoryRebindRecordNextAction,
...
impl M5ResolvedAnchorHistoryRebindRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5AnchorHistoryRebindResultEntryResolutionInput {
    ...
    pub comparison_scope: M5AnchorHistoryRebindResultScope,
    /// The render / surface context.
    pub surface_context: M5AnchorHistoryRebindRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5AnchorHistoryRebindRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedAnchorHistoryRebindResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5AnchorHistoryRebindResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5AnchorHistoryRebindRecordNextAction,
...
impl M5ResolvedAnchorHistoryRebindResultEntry {
...
pub enum M5AnchorHistoryRebindRecordResolutionError {
...
impl M5AnchorHistoryRebindRecordResolutionError {
...
impl fmt::Display for M5AnchorHistoryRebindRecordResolutionError {
...
impl Error for M5AnchorHistoryRebindRecordResolutionError {}

fn form_tokens(forms: &[M5AnchorHistoryRebindRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5AnchorHistoryRebindRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5AnchorHistoryRebindRecordResolutionForm> =
        forms.iter().copied().collect();
    M5AnchorHistoryRebindRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5AnchorHistoryRebindRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5AnchorHistoryRebindResultScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedAnchorHistoryRebindRecordEntry, M5AnchorHistoryRebindRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5AnchorHistoryRebindRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5AnchorHistoryRebindRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5AnchorHistoryRebindRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5AnchorHistoryRebindRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5AnchorHistoryRebindRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5AnchorHistoryRebindRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5AnchorHistoryRebindRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5AnchorHistoryRebindRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5AnchorHistoryRebindRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5AnchorHistoryRebindRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5AnchorHistoryRebindRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5AnchorHistoryRebindRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedAnchorHistoryRebindRecordEntry {
...
pub fn resolve_review_pack_result_entry(
...
) -> Result<M5ResolvedAnchorHistoryRebindResultEntry, M5AnchorHistoryRebindRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5AnchorHistoryRebindRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5AnchorHistoryRebindResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5AnchorHistoryRebindResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5AnchorHistoryRebindResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5AnchorHistoryRebindResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5AnchorHistoryRebindResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5AnchorHistoryRebindResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5AnchorHistoryRebindRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedAnchorHistoryRebindResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AnchorHistoryRebindRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5AnchorHistoryRebindRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5AnchorHistoryRebindRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5AnchorHistoryRebindRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedAnchorHistoryRebindRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedAnchorHistoryRebindResultEntry>,
...
impl M5AnchorHistoryRebindRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5AnchorHistoryRebindRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5AnchorHistoryRebindRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5AnchorHistoryRebindRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5AnchorHistoryRebindRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedAnchorHistoryRebindRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedAnchorHistoryRebindResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AnchorHistoryRebindRecordAndResultRegistriesVocabularySet {
...
impl M5AnchorHistoryRebindRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5CollaborationStateRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5AnchorHistoryRebindRecordResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5AnchorHistoryRebindRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5AnchorHistoryRebindResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5AnchorHistoryRebindRecordSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            review_pack_record_degrade_reasons: tokens(
                &M5AnchorHistoryRebindRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5AnchorHistoryRebindResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5AnchorHistoryRebindRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5AnchorHistoryRebindRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5AnchorHistoryRebindRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AnchorHistoryRebindRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AnchorHistoryRebindRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AnchorHistoryRebindRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AnchorHistoryRebindRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5AnchorHistoryRebindRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5AnchorHistoryRebindRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5AnchorHistoryRebindRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5AnchorHistoryRebindRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5AnchorHistoryRebindRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5AnchorHistoryRebindRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5AnchorHistoryRebindRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5AnchorHistoryRebindRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AnchorHistoryRebindRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_SCHEMA_VERSION`].
...
impl M5AnchorHistoryRebindRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5AnchorHistoryRebindRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind:
                M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_RECORD_KIND
                    .to_owned(),
            schema_version:
                M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5AnchorHistoryRebindRecordAndResultRegistriesViolation> {
        ...
        {
            violations
                .push(M5AnchorHistoryRebindRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_COMMENT_ANNOTATION_REVIEW_PIN_ANCHOR_HISTORY_AND_REBIND_REVIEW_REGISTRIES_SCHEMA_VERSION
        {
            violations
                .push(M5AnchorHistoryRebindRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations
                .push(M5AnchorHistoryRebindRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations
                .push(M5AnchorHistoryRebindRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations
                .push(M5AnchorHistoryRebindRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
...
#[derive(Debug)]
pub enum M5AnchorHistoryRebindRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5AnchorHistoryRebindRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5AnchorHistoryRebindRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5AnchorHistoryRebindRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5AnchorHistoryRebindRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5AnchorHistoryRebindRecordAndResultRegistriesViolation {
...
impl M5AnchorHistoryRebindRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_comment_annotation_review_pin_anchor_history_and_rebind_review_registries_export(
) -> Result<
    M5AnchorHistoryRebindRecordAndResultRegistriesPacket,
    M5AnchorHistoryRebindRecordAndResultRegistriesArtifactError,
> {
    let packet: M5AnchorHistoryRebindRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-comment-annotation-review-pin-anchor-history-and-rebind-review-registries-proof/support_export.json"
        )
    ))
    .map_err(M5AnchorHistoryRebindRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5AnchorHistoryRebindRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5AnchorHistoryRebindRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5AnchorHistoryRebindRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5AnchorHistoryRebindRecordAndResultRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5AnchorHistoryRebindRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5AnchorHistoryRebindRecordAndResultRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5AnchorHistoryRebindRecordAndResultRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5AnchorHistoryRebindRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_SERVER_ORDERED_COMMENTS_ANNOTATIONS_REVIEW_PINS_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_PRESENTER_FOLLOW_STATE_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5AnchorHistoryRebindRecordAndResultRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations
                .push(M5AnchorHistoryRebindRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations
                .push(M5AnchorHistoryRebindRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(
                M5AnchorHistoryRebindRecordAndResultRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5AnchorHistoryRebindRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5AnchorHistoryRebindRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5AnchorHistoryRebindRecordAndResultRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5AnchorHistoryRebindRecordAndResultRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5AnchorHistoryRebindRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5AnchorHistoryRebindRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5AnchorHistoryRebindRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5AnchorHistoryRebindRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5AnchorHistoryRebindResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(
            M5AnchorHistoryRebindRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
