//! Tests for the M05-1139 decision-feedback component surface certification capstone.

use super::*;

fn packet() -> DecisionFeedbackProfileCertificationPacket {
    seeded_m5_decision_feedback_component_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, DECISION_FEEDBACK_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, DECISION_FEEDBACK_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5DecisionFeedbackCertifiedProfile::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            DecisionFeedbackCertificationAxis::ALL.len()
        );
    }
...
fn all_eight_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5DecisionFeedbackA11yClaim> =
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5DecisionFeedbackCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5DecisionFeedbackCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = DecisionFeedbackCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), DecisionFeedbackCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in DecisionFeedbackCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == DecisionFeedbackCertificationAxis::CliExport
        );
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5DecisionFeedbackCertifiedProfile::LiveTrustedDecisionSurface)
        .expect("live-trusted-decision-surface row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == DecisionFeedbackCertificationAxis::Visual {
            outcome.state = DecisionFeedbackAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("decision state silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, DecisionFeedbackProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        DecisionFeedbackCertificationViolation::ProfileBlocked { .. }
    )));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5DecisionFeedbackCertifiedProfile::ReviewableDecisionStructure)
        .expect("reviewable-decision-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails.relies_on_color_alone_for_meaning = true;
    assert_eq!(row.derive_status(), DecisionFeedbackProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        DecisionFeedbackCertificationViolation::GuardrailViolated { .. }
    )));
}
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5DecisionFeedbackA11yClaim::TrustedDecisionSurface;
    row.certified_claim = M5DecisionFeedbackA11yClaim::TrustedDecisionSurface;
    assert_eq!(row.derive_status(), DecisionFeedbackProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == DecisionFeedbackCertificationAxis::DegradedState {
            *outcome = DecisionFeedbackAxisOutcome {
                axis: DecisionFeedbackCertificationAxis::DegradedState,
                state: DecisionFeedbackAxisCertificationState::DisclosedNarrowed,
                parity_note: "the severity evidence is lagging".to_owned(),
                narrowing_reason: Some(
                    "the severity evidence is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5DecisionFeedbackDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableDecisionSurface == certified, no claim_auto_narrow.
    assert_eq!(row.derive_status(), DecisionFeedbackProfileClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == DecisionFeedbackCertificationAxis::CliExport {
            outcome.state = DecisionFeedbackAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5DecisionFeedbackDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), DecisionFeedbackProfileClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(DecisionFeedbackClaimAutoNarrow {
        binding_axis: DecisionFeedbackCertificationAxis::DegradedState,
        from_claim: M5DecisionFeedbackA11yClaim::TrustedDecisionSurface,
        to_claim: M5DecisionFeedbackA11yClaim::TrustedDecisionSurface,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5DecisionFeedbackA11yClaim::SeverityUnverifiedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-decision-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableDecisionSurface; certify a stronger trusted decision surface.
    row.certified_claim = M5DecisionFeedbackA11yClaim::TrustedDecisionSurface;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5DecisionFeedbackCertifiedProfile::StaleSeverityBadgeSurface)
        .expect("stale-severity-badge-surface row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = DecisionFeedbackCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), DecisionFeedbackProfileClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == DecisionFeedbackCertificationAxis::CliExport {
            outcome.state = DecisionFeedbackAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the stale severity badge".to_owned());
            outcome.downgrade_trigger = Some(M5DecisionFeedbackDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = DecisionFeedbackCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), DecisionFeedbackProfileClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == DecisionFeedbackProfileClaimStatus::Yellow)
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(DecisionFeedbackCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5DecisionFeedbackDowngradeTrigger::ProofStale,
    );
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows.retain(|r| {
        r.profile != M5DecisionFeedbackCertifiedProfile::PartialRecoveryConsequenceSurface
    });
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        DecisionFeedbackCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the dialog-sheet family from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5DecisionFeedbackFamily::DialogSheet);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        DecisionFeedbackCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = DecisionFeedbackProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        DecisionFeedbackCertificationViolation::WrongCanonicalBundle
    )));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        DecisionFeedbackCertificationViolation::DuplicateId { .. }
    )));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != DecisionFeedbackCertificationAxis::DecisionFeedbackComponentTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, DecisionFeedbackCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        DecisionFeedbackCertificationViolation::RawComponentMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: DecisionFeedbackProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_decision_feedback_component_surface_certification_export()
...
fn generate_artifacts() {
    if std::env::var("GEN_DECISION_FEEDBACK_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_decision_feedback_component_surface_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-decision-feedback-component-surface-certification-proof");
    ...
    let fixtures = Path::new(manifest)
        .join("../../fixtures/ui/m5-decision-feedback-component-surface-certification");
    ...
    fs::write(
        ...
        "# M5 decision-feedback component surface certification fixtures\n\n\
         Mirror of `artifacts/release/m5-decision-feedback-component-surface-certification-proof/`.\n\
         Regenerate with `GEN_DECISION_FEEDBACK_CERT_ARTIFACTS=1 cargo test -p aureline-ui \
         m5_decision_feedback_component_surface_certification::tests::generate_artifacts`.\n",
    )
