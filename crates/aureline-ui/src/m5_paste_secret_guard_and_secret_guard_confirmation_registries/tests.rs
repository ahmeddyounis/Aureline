...
fn clean_review_pack_record_input() -> M5PasteSecretGuardRecordEntryResolutionInput {
    M5PasteSecretGuardRecordEntryResolutionInput {
        ...
        semantic_role: M5CollaborationControlRole::ControlAuthorityDisclosure,
        report_section: M5PasteSecretGuardRecordKind::ChangedFilesScope,
        surface_context: M5PasteSecretGuardRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5PasteSecretGuardRecordResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5PasteSecretGuardResultEntryResolutionInput {
    M5PasteSecretGuardResultEntryResolutionInput {
        ...
        semantic_role: M5CollaborationControlRole::ControlAuthorityDisclosure,
        comparison_scope: M5PasteSecretGuardResultScope::EvaluatedScopeBinding,
        surface_context: M5PasteSecretGuardRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5PasteSecretGuardRecordResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_paste_secret_guard_and_secret_guard_confirmation_registries();
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(resolved.semantic_role, "control_authority_disclosure");
    ...
    assert_eq!(
        ...
        M5PasteSecretGuardRecordNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5PasteSecretGuardRecordKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_object_incomplete_and_widen_fold_and_form_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.resolution_form_coverage = vec![M5PasteSecretGuardRecordResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5PasteSecretGuardRecordKind::FullTreeScope;
    ...
    let mut input = clean_review_pack_record_input();
    input.surface_context = M5PasteSecretGuardRecordSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        ...
        M5PasteSecretGuardRecordResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input).unwrap_err(),
        M5PasteSecretGuardRecordResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5PasteSecretGuardRecordKind::PackScopeUnclassified,
        true,
        false,
        true
    ));
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5PasteSecretGuardResultScope::ChangeObjectResultUnclassified;
    assert_eq!(
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    assert_eq!(
        ...
        M5PasteSecretGuardRecordResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PasteSecretGuardRecordAndResultRegistriesViolation::VocabularySetDrift));
}
...
fn missing_source_contracts_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PasteSecretGuardRecordAndResultRegistriesViolation::MissingSourceContracts));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_PASTE_SECRET_GUARD_LANDED_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5PasteSecretGuardRecordAndResultRegistriesViolation::DomainSchemaRefMissing));

    let mut packet = seeded_m5_paste_secret_guard_and_secret_guard_confirmation_registries();
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_SECRET_GUARD_CONFIRMATION_DOMAIN_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5PasteSecretGuardRecordAndResultRegistriesViolation::DomainSchemaRefMissing));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5PasteSecretGuardRecordAnatomyPart::Identity);
    assert!(packet
        .validate()
        .contains(&M5PasteSecretGuardRecordAndResultRegistriesViolation::MandatoryAnatomyMissing));
}
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5PasteSecretGuardRecordExportField::CohortArchetypes);
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PasteSecretGuardRecordAndResultRegistriesViolation::ExamplesMissing));
}
...
fn dishonest_clean_example_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PasteSecretGuardRecordAndResultRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        assert!(packet
            .validate()
            .contains(&M5PasteSecretGuardRecordAndResultRegistriesViolation::RowInvariantViolated));
    }
}
...
fn review_pack_record_not_proven_when_incomplete_example_removed() {
    ...
    for row in &mut packet.registry_rows {
        row.review_pack_record_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
                    M5PasteSecretGuardRecordEntryDegradeReason::CohortDescriptorObjectIncomplete,
                )
        });
    }
    assert!(packet.validate().contains(
...
fn proof_freshness_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PasteSecretGuardRecordAndResultRegistriesViolation::ProofFreshnessIncomplete));
}
...
fn release_posture_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PasteSecretGuardRecordAndResultRegistriesViolation::ReleasePostureIncomplete));
}
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PasteSecretGuardRecordAndResultRegistriesViolation::RawMaterialInExport));
}

#[test]
fn export_carries_no_forbidden_raw_material() {
    let json =
        seeded_m5_paste_secret_guard_and_secret_guard_confirmation_registries().export_safe_json();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_paste_secret_guard_and_secret_guard_confirmation_registries_export()
        .expect(
...
fn narrowed_variants_validate_and_keep_rows_visible() {
    ...
    let row = beta
        ...
        .iter()
        .find(|r| {
            r.consumer_surface == M5CollaborationControlConsumerSurface::SharedTerminalDebugView
        })
        .unwrap();
    assert_eq!(
        row.qualification,
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5PasteSecretGuardRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-paste-secret-guard-and-secret-guard-confirmation-registries/paste_secret_guard_beta_narrowed.json"
    )))
    ...
    let preview: M5PasteSecretGuardRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-paste-secret-guard-and-secret-guard-confirmation-registries/secret_guard_confirmation_preview_narrowed.json"
    )))
...
fn implemented_lines_is_all_six_constrained_file_state_object_classes() {
    assert_eq!(IMPLEMENTED_LINES, M5CollaborationControlObject::ALL);
