//! Implemented M5 paste-secret-guard and secret-guard-confirmation registries.
//!
//! The frozen [collaboration-control component matrix][matrix] names Aureline's governed sensitive-collaboration
//! object classes and locks their controlled vocabulary. This module makes the explicit paste / secret guard
//! operable — as a canonical record — and adds the secret-guard confirmation that names whether a risky action is
//! allowed-with-confirm, requires step-up, is denied, or blocked, so the desktop shared-terminal / debug surface,
//! the companion-follow flow, and the support / export and help / docs surfaces inherit one canonical guard
//! descriptor rather than a hand-authored parallel prose that has to be kept consistent: it turns the
//! *paste-secret-guard* grammar (one durable guard record per high-risk action — one typed field per
//! record section: the risky-action class (high-risk paste, terminal broadcast, clipboard bridge, debug-evaluate,
//! environment-variable reveal, or variable-body reveal), the disclosed scope, target, and reason, the step-up /
//! confirm posture required, and the visible guardrail badge kept distinct from ordinary presence
//! and follow state — each bound to one session / target scope) and the *secret-guard-confirmation* grammar (the
//! confirmation a participant reads before any risky paste or reveal commits, and the fresh visible guard event any
//! risky action must raise — a disclose, an allowed-with-confirm, a step-up, a deny, or a
//! blocked event that must not copy a raw secret body into logs or exports) into registry
//! resolvers that produce export-safe, honest projections. Every high-risk action then resolves to one typed
//! paste-secret guard — its risky-action class, its disclosed scope, target, and reason, its step-up posture and
//! visible guardrail, all attributable to the session so a risky reveal never commits silently — and to one
//! secret-guard-confirmation object — the resolved action identity, the disclosed scope and target, the guard
//! posture, and the confirm / deny / blocked outcome — that the shared terminal / debug, companion, and support surfaces
//! can inspect without manual reconstruction, so no risky paste or reveal ever proceeds without disclosing scope,
//! target, and reason and the guard history stays visible and exportable, and a guard that cannot bind a field to its
//! session / target scope degrades honestly instead of letting a risky reveal read as an implicit approval when it is not.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable paste-secret guard per high-risk action, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [scope selector][M5PasteSecretGuardRecordKind],
//!   a disclosure role, covers every [resolution form][M5PasteSecretGuardRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (disclosed scope
//!   and target context, risky-action class, reason and step-up posture, and the visible-guardrail
//!   binding bound to the session identity), keeps its guard lineage attributable to the session, and
//!   keeps a step-up-posture field bound to its guard state; otherwise it degrades.
//! * **Keep the secret-guard confirmation honest so a risky reveal never commits silently rather than a
//!   hand-authored parallel prose staying consistent.** [`line_preserves_rollback_and_diagnostics_before_widening`]
//!   rejects a guard whose visible-guardrail binding is not preserved (a risky action presenting itself as
//!   safe from presence alone) so it degrades to
//!   [`M5PasteSecretGuardRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a confirmation whose language outruns its guard state degrades the same
//!   way — the structured blocker reason a reveal-without-guard attempt must surface.
//! * **Turn a disclose, an allowed-with-confirm, a step-up, a deny, or a blocked outcome into a visible, typed guard event
//!   instead of a silent reveal.** [`resolve_review_pack_result_entry`] names a classified
//!   [guard binding][M5PasteSecretGuardResultScope]
//!   (disclose binding, allowed-with-confirm binding, or deny / blocked binding), requires the full
//!   action-identity / affected-guard-field / previous-versus-current-guard-state / binding / active-reason
//!   guard object, covers every resolution form, and degrades to
//!   [`M5PasteSecretGuardResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the change would present a blocked reveal as allowed, hide a raw secret body on a sensitive surface, or let a
//!   declined or blocked event drop its attribution, so a guard event can never read as trustworthy when
//!   it has quietly dropped the reason a fresh visible guard event was required.
//!
...
//! can never fork their own paste-secret-guard meaning. Raw secrets, raw command text, raw variable bodies, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_paste_secret_guard_and_secret_guard_confirmation_registries,
    seeded_m5_paste_secret_guard_and_secret_guard_confirmation_registries_review_pack_record_beta_narrowed,
    seeded_m5_paste_secret_guard_and_secret_guard_confirmation_registries_review_pack_result_preview_narrowed,
    M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_PACKET_ID,
};
...
use crate::m5_shared_terminal_debug_control_grant_presenter_consent_retention_and_session_restore_view_matrix::{
    ...
    M5_PASTE_SECRET_GUARD_LANDED_SCHEMA_REF, M5_COLLABORATION_CONTROL_MATRIX_DOC_REF,
    M5_COLLABORATION_CONTROL_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5PasteSecretGuardRecordAndResultRegistriesPacket`].
pub const M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_paste_secret_guard_and_secret_guard_confirmation_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_paste_secret_guard_and_secret_guard_confirmation_registries.md";

/// Repo-relative path of the secret-guard-confirmation domain schema this lane binds its B object to (the
/// machine-readable confirmation or deny outcome of a guarded reveal, its audit-safe attribution, and the guard
/// state it projects — allowed-with-confirm, step-up required, denied, or blocked — kept distinct from any raw
/// secret body). Minted by this lane; the confirmation and the paste / secret guard never fork their guard
/// vocabulary, and no raw secret, command text, variable body, or clipboard content crosses this boundary.
pub const M5_SECRET_GUARD_CONFIRMATION_DOMAIN_SCHEMA_REF: &str =
    "schemas/ui/m5-secret-guard-confirmation.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-paste-secret-guard-and-secret-guard-confirmation-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-paste-secret-guard-and-secret-guard-confirmation-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-paste-secret-guard-and-secret-guard-confirmation-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-paste-secret-guard-and-secret-guard-confirmation-registries";
...
pub type M5PasteSecretGuardRecordAndResultRegistriesConsumerSurface =
...
pub enum M5PasteSecretGuardRecordResolutionForm {
...
impl M5PasteSecretGuardRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PasteSecretGuardRecordKind {
...
impl M5PasteSecretGuardRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PasteSecretGuardResultScope {
...
impl M5PasteSecretGuardResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PasteSecretGuardRecordSurfaceContext {
...
impl M5PasteSecretGuardRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PasteSecretGuardRecordAnatomyPart {
...
impl M5PasteSecretGuardRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PasteSecretGuardRecordNextAction {
...
impl M5PasteSecretGuardRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PasteSecretGuardRecordExportField {
...
impl M5PasteSecretGuardRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PasteSecretGuardRecordEntryDegradeReason {
...
impl M5PasteSecretGuardRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5PasteSecretGuardRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5PasteSecretGuardRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5PasteSecretGuardRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5PasteSecretGuardRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5PasteSecretGuardRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PasteSecretGuardResultEntryDegradeReason {
...
impl M5PasteSecretGuardResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5PasteSecretGuardRecordNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5PasteSecretGuardRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5PasteSecretGuardRecordEntryResolutionInput {
    ...
    pub report_section: M5PasteSecretGuardRecordKind,
    /// The render / surface context.
    pub surface_context: M5PasteSecretGuardRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5PasteSecretGuardRecordResolutionForm>,
...
pub struct M5ResolvedPasteSecretGuardRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5PasteSecretGuardRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5PasteSecretGuardRecordNextAction,
...
impl M5ResolvedPasteSecretGuardRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5PasteSecretGuardResultEntryResolutionInput {
    ...
    pub comparison_scope: M5PasteSecretGuardResultScope,
    /// The render / surface context.
    pub surface_context: M5PasteSecretGuardRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5PasteSecretGuardRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedPasteSecretGuardResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5PasteSecretGuardResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5PasteSecretGuardRecordNextAction,
...
impl M5ResolvedPasteSecretGuardResultEntry {
...
pub enum M5PasteSecretGuardRecordResolutionError {
...
impl M5PasteSecretGuardRecordResolutionError {
...
impl fmt::Display for M5PasteSecretGuardRecordResolutionError {
...
impl Error for M5PasteSecretGuardRecordResolutionError {}

fn form_tokens(forms: &[M5PasteSecretGuardRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5PasteSecretGuardRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5PasteSecretGuardRecordResolutionForm> = forms.iter().copied().collect();
    M5PasteSecretGuardRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5PasteSecretGuardRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5PasteSecretGuardResultScope,
...
pub fn resolve_review_pack_record_entry(
    input: M5PasteSecretGuardRecordEntryResolutionInput,
) -> Result<M5ResolvedPasteSecretGuardRecordEntry, M5PasteSecretGuardRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5PasteSecretGuardRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5PasteSecretGuardRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5PasteSecretGuardRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5PasteSecretGuardRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5PasteSecretGuardRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5PasteSecretGuardRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5PasteSecretGuardRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5PasteSecretGuardRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5PasteSecretGuardRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5PasteSecretGuardRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5PasteSecretGuardRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5PasteSecretGuardRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedPasteSecretGuardRecordEntry {
...
pub fn resolve_review_pack_result_entry(
    input: M5PasteSecretGuardResultEntryResolutionInput,
) -> Result<M5ResolvedPasteSecretGuardResultEntry, M5PasteSecretGuardRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5PasteSecretGuardRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5PasteSecretGuardResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5PasteSecretGuardResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5PasteSecretGuardResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5PasteSecretGuardResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5PasteSecretGuardResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5PasteSecretGuardResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5PasteSecretGuardRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedPasteSecretGuardResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PasteSecretGuardRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5PasteSecretGuardRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5PasteSecretGuardRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5PasteSecretGuardRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedPasteSecretGuardRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedPasteSecretGuardResultEntry>,
...
impl M5PasteSecretGuardRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5PasteSecretGuardRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5PasteSecretGuardRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5PasteSecretGuardRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5PasteSecretGuardRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedPasteSecretGuardRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedPasteSecretGuardResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PasteSecretGuardRecordAndResultRegistriesVocabularySet {
...
impl M5PasteSecretGuardRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5CollaborationControlRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5PasteSecretGuardRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5PasteSecretGuardRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5PasteSecretGuardResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5PasteSecretGuardRecordSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5PasteSecretGuardRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5PasteSecretGuardResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5PasteSecretGuardRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5PasteSecretGuardRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5PasteSecretGuardRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PasteSecretGuardRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PasteSecretGuardRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PasteSecretGuardRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PasteSecretGuardRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5PasteSecretGuardRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5PasteSecretGuardRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5PasteSecretGuardRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5PasteSecretGuardRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5PasteSecretGuardRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5PasteSecretGuardRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5PasteSecretGuardRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5PasteSecretGuardRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PasteSecretGuardRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_SCHEMA_VERSION`].
...
impl M5PasteSecretGuardRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5PasteSecretGuardRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version:
                M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5PasteSecretGuardRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind
            != M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_RECORD_KIND
        {
            violations.push(M5PasteSecretGuardRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_SCHEMA_VERSION
        {
            violations
                .push(M5PasteSecretGuardRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5PasteSecretGuardRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations
                .push(M5PasteSecretGuardRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations
                .push(M5PasteSecretGuardRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Paste-Secret-Guard and Secret-Guard-Confirmation Registries\n\n");
...
#[derive(Debug)]
pub enum M5PasteSecretGuardRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5PasteSecretGuardRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5PasteSecretGuardRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5PasteSecretGuardRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5PasteSecretGuardRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5PasteSecretGuardRecordAndResultRegistriesViolation {
...
impl M5PasteSecretGuardRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_paste_secret_guard_and_secret_guard_confirmation_registries_export(
) -> Result<
    M5PasteSecretGuardRecordAndResultRegistriesPacket,
    M5PasteSecretGuardRecordAndResultRegistriesArtifactError,
> {
    let packet: M5PasteSecretGuardRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-paste-secret-guard-and-secret-guard-confirmation-registries-proof/support_export.json"
        )
    ))
    .map_err(M5PasteSecretGuardRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5PasteSecretGuardRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5PasteSecretGuardRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5PasteSecretGuardRecordAndResultRegistriesViolation>,
) {
    let refs: BTreeSet<&str> = packet
        ...
        .collect();
    for required in [
        M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_SCHEMA_REF,
        M5_PASTE_SECRET_GUARD_AND_SECRET_GUARD_CONFIRMATION_REGISTRIES_DOC_REF,
        M5_COLLABORATION_CONTROL_MATRIX_SCHEMA_REF,
        M5_COLLABORATION_CONTROL_MATRIX_DOC_REF,
        M5_PASTE_SECRET_GUARD_LANDED_SCHEMA_REF,
        M5_SECRET_GUARD_CONFIRMATION_DOMAIN_SCHEMA_REF,
    ] {
        if !refs.contains(required) {
            violations
                .push(M5PasteSecretGuardRecordAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5PasteSecretGuardRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations
                .push(M5PasteSecretGuardRecordAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5PasteSecretGuardRecordAndResultRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5PasteSecretGuardRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_PASTE_SECRET_GUARD_LANDED_SCHEMA_REF)
            || !refs.contains(M5_SECRET_GUARD_CONFIRMATION_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5PasteSecretGuardRecordAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5PasteSecretGuardRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5PasteSecretGuardRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5PasteSecretGuardRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5PasteSecretGuardRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5PasteSecretGuardRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5PasteSecretGuardRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5PasteSecretGuardRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5PasteSecretGuardRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5PasteSecretGuardRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5PasteSecretGuardRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5PasteSecretGuardRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5PasteSecretGuardResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(
            M5PasteSecretGuardRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
