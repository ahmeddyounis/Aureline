...
fn clean_refresh_policy_input() -> M5RefreshPolicyEntryResolutionInput {
    M5RefreshPolicyEntryResolutionInput {
        entry_id: "refresh_policy:test".to_owned(),
        line_binding_id: "launch.line.core-team-canary".to_owned(),
        token_name: "line.refresh_policy.core_team_canary".to_owned(),
        semantic_role: M5StableLineProtectionRole::SupportWindow,
        refresh_surface: M5RefreshSurfaceKind::CertifiedArchetypeReport,
        surface_context: M5StableLineRefreshSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5StableLineRefreshResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5ClaimDowngradeEntryResolutionInput {
    M5ClaimDowngradeEntryResolutionInput {
        entry_id: "downgrade:test".to_owned(),
        downgrade_ref: "launch.line.core-team-canary".to_owned(),
        token_name: "line.downgrade.core_team_canary".to_owned(),
        semantic_role: M5StableLineProtectionRole::SupportWindow,
        downgrade_scope: M5ClaimDowngradeScope::RetestPendingDowngrade,
        surface_context: M5StableLineRefreshSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5StableLineRefreshResolutionForm::ALL.to_vec(),
...
#[test]
fn refresh_policy_clean_names_meaning_and_is_bound() {
    let resolved = resolve_refresh_policy_entry(clean_refresh_policy_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.refresh_policy_resolves_across_lines);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.refresh_policy_object_complete);
    assert!(resolved.bound_to_registry);
    assert!(resolved.refresh_surface_is_classified);
    assert!(resolved.rollback_and_diagnostics_bounded);
    assert_eq!(resolved.semantic_role, "support_window");
    assert_eq!(resolved.refresh_surface, "certified_archetype_report");
    assert_eq!(
        resolved.canonical_refresh_surface_mode,
        "certified_archetype_report_mode"
    );
    assert_eq!(resolved.surface_context, "shiproom_surface");
    assert_eq!(
        resolved.next_action,
        M5StableLineRefreshNextAction::ExpandCohortMeaning
    );
}

#[test]
fn refresh_policy_token_unstated_degrades() {
    let mut input = clean_refresh_policy_input();
    input.token_name = "   ".to_owned();
    assert_eq!(
        resolve_refresh_policy_entry(input).unwrap().degrade_reason,
        Some(M5RefreshPolicyEntryDegradeReason::DescriptorTokenUnstated)
    );
}

#[test]
fn refresh_policy_unbound_and_unclassified_degrade() {
    let mut input = clean_refresh_policy_input();
    ...
    let mut input = clean_refresh_policy_input();
    input.refresh_surface = M5RefreshSurfaceKind::SurfaceUnclassified;
    assert_eq!(
        resolve_refresh_policy_entry(input).unwrap().degrade_reason,
...
#[test]
fn refresh_policy_object_incomplete_and_widen_fold_and_form_degrade() {
    // An unstated bundle IDs field leaves the resolved object incomplete.
    let mut input = clean_refresh_policy_input();
    input.bundle_ids = "  ".to_owned();
    let resolved = resolve_refresh_policy_entry(input).unwrap();
    assert!(!resolved.refresh_policy_object_complete);
    assert_eq!(
    ...
    let mut input = clean_refresh_policy_input();
    input.resolution_form_coverage = vec![M5StableLineRefreshResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_refresh_policy_entry(input).unwrap().degrade_reason,
...
#[test]
fn refresh_policy_public_facing_and_surface_and_proof_degrade() {
    // A public-facing line running support language ahead of proof first fails the widen-boundary fold.
    let mut input = clean_refresh_policy_input();
    input.refresh_surface = M5RefreshSurfaceKind::PublicProofSurface;
    ...
    let mut input = clean_refresh_policy_input();
    input.surface_context = M5StableLineRefreshSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_refresh_policy_entry(input).unwrap().degrade_reason,
...
#[test]
fn refresh_policy_empty_id_and_forbidden_material_error() {
    let mut input = clean_refresh_policy_input();
    input.entry_id = "".to_owned();
    assert_eq!(
        resolve_refresh_policy_entry(input).unwrap_err(),
        M5StableLineRefreshResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_refresh_policy_entry(input).unwrap_err(),
        M5StableLineRefreshResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5RefreshSurfaceKind::SurfaceUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn refresh_policy_object_is_complete_requires_all_fields() {
    assert!(refresh_policy_object_is_complete(
        M5RefreshSurfaceKind::CertifiedArchetypeReport,
...
fn downgrade_clean_stays_honest() {
    let resolved = resolve_claim_downgrade_entry(clean_downgrade_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.downgrade_safe_on_every_line);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.provides_complete_claim_downgrade);
    assert!(resolved.claim_downgrade_stays_honest);
    assert_eq!(resolved.downgrade_scope, "retest_pending_downgrade");
    assert_eq!(resolved.surface_context, "shiproom_surface");
}
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let resolved = resolve_claim_downgrade_entry(input).unwrap();
    assert!(!resolved.provides_complete_claim_downgrade);
    assert_eq!(
        resolved.degrade_reason,
    ...
    let mut input = clean_downgrade_input();
    input.keeps_claim_downgrade_visible = false;
    assert_eq!(
        resolve_claim_downgrade_entry(input).unwrap().degrade_reason,
        Some(M5ClaimDowngradeEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    );
    ...
    let mut input = clean_downgrade_input();
    input.downgrade_scope = M5ClaimDowngradeScope::ScopeUnclassified;
    assert_eq!(
        resolve_claim_downgrade_entry(input).unwrap().degrade_reason,
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    input.entry_id = "  ".to_owned();
    assert_eq!(
        resolve_claim_downgrade_entry(input).unwrap_err(),
        M5StableLineRefreshResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn downgrade_bound_support_and_flagged_gap_stay_clean() {
    ...
    assert!(resolve_claim_downgrade_entry(input).unwrap().is_clean());
...
fn vocabulary_set_drift_fails() {
    ...
    packet.vocabulary_set.refresh_surface_kinds.pop();
    assert!(packet
        .validate()
        .contains(&M5StableLineRefreshPolicyClaimDowngradeRegistriesViolation::VocabularySetDrift));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_STABLE_LINE_REFRESH_POLICY_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_CLAIM_DOWNGRADE_PACKET_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5StableLineRefreshPolicyClaimDowngradeRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5StableLineRefreshAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5StableLineRefreshExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    let mut packet = seeded_m5_stable_line_refresh_policy_and_claim_downgrade_registries();
    packet.registry_rows[0].claim_downgrade_entries.clear();
    assert!(packet
        .validate()
        .contains(&M5StableLineRefreshPolicyClaimDowngradeRegistriesViolation::ExamplesMissing));
}

#[test]
fn dishonest_clean_example_fails() {
    let mut packet = seeded_m5_stable_line_refresh_policy_and_claim_downgrade_registries();
    // Force a clean refresh_policy entry to also read as object-incomplete — the packet must reject it.
    let row = &mut packet.registry_rows[0];
    row.refresh_policy_entries[0].degrade_reason = None;
    row.refresh_policy_entries[0].refresh_policy_object_complete = false;
    assert!(packet
        .validate()
        .contains(&M5StableLineRefreshPolicyClaimDowngradeRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        match mutate {
            ...
            _ => row.collapses_distinct_claim_downgrade_classes_into_one_lane = true,
        }
        assert!(packet.validate().contains(
...
#[test]
fn refresh_policy_not_proven_when_incomplete_example_removed() {
    let mut packet = seeded_m5_stable_line_refresh_policy_and_claim_downgrade_registries();
    for row in &mut packet.registry_rows {
        row.refresh_policy_entries.retain(|ex| {
            ex.degrade_reason
                != Some(M5RefreshPolicyEntryDegradeReason::CohortDescriptorObjectIncomplete)
        });
    }
...
#[test]
fn refresh_policy_not_proven_when_surface_collapses() {
    let mut packet = seeded_m5_stable_line_refresh_policy_and_claim_downgrade_registries();
    // Drop every clean executive-steering-surface refresh_policy so the first-consumer surfaces no longer include it.
    for row in &mut packet.registry_rows {
        row.refresh_policy_entries
...
#[test]
fn claim_downgrade_integrity_not_proven_when_support_ahead_example_removed() {
    let mut packet = seeded_m5_stable_line_refresh_policy_and_claim_downgrade_registries();
    for row in &mut packet.registry_rows {
        row.claim_downgrade_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
...
#[test]
fn claim_downgrade_integrity_not_proven_when_scope_dropped() {
    let mut packet = seeded_m5_stable_line_refresh_policy_and_claim_downgrade_registries();
    // Drop every clean go-no-go-signoff downgrade so the coverage no longer includes it.
    for row in &mut packet.registry_rows {
        row.claim_downgrade_entries
            .retain(|ex| !(ex.is_clean() && ex.downgrade_scope == "narrowed_support_downgrade"));
    }
    assert!(packet.validate().contains(
        &M5StableLineRefreshPolicyClaimDowngradeRegistriesViolation::CohortEvidenceIntegrityNotProven
    ));
...
fn export_carries_no_forbidden_raw_material() {
    let json =
        seeded_m5_stable_line_refresh_policy_and_claim_downgrade_registries().export_safe_json();
...
#[test]
fn refresh_policy_table_lists_only_clean_refresh_policys() {
    let packet = seeded_m5_stable_line_refresh_policy_and_claim_downgrade_registries();
    let table = packet.render_refresh_policy_table();
    // The clean canary and migration refresh_policys are rendered from the registry.
    assert!(table.contains("certified_archetype_report_mode"));
    assert!(table.contains("compatibility_packet_mode"));
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk =
        current_stable_m5_stable_line_refresh_policy_and_claim_downgrade_registries_export()
            .expect(
            "checked M5 line-refresh_policy / line-downgrade-packet registries export validates",
        );
    assert_eq!(
        from_disk.packet_id,
        M5_STABLE_LINE_REFRESH_POLICY_CLAIM_DOWNGRADE_REGISTRIES_PACKET_ID
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5StableLineRefreshPolicyClaimDowngradeRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-stable-line-refresh-policy-and-claim-downgrade-registries/refresh_policy_beta_narrowed.json"
    )))
    .expect("line-refresh_policy fixture parses");
    ...
    let preview: M5StableLineRefreshPolicyClaimDowngradeRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-stable-line-refresh-policy-and-claim-downgrade-registries/claim_downgrade_preview_narrowed.json"
    )))
