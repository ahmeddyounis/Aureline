//! Implemented M5 review-template-packet and template-publish-attribution registries.
//!
//! The frozen [review-pack evaluator matrix][matrix] names Aureline's governed review-pack object classes and
//! locks their controlled vocabulary. This module makes the matrix's `review_template_packet` class operable so review,
//! AI-review, provider-handoff, and support / export surfaces inherit one canonical comment / summary template
//! descriptor rather than a hand-authored parallel prose that has to be kept consistent: it turns the
//! *review-template-packet* grammar (one typed comment / summary template packet per review — its pack-authored
//! rationale blocks, its checklist text, and its bundle manifests, all carrying a template version and a pack digest
//! and bound to the same active review-pack version / content digest as human, local, and CI review) and the
//! *template-publish-attribution* grammar (the binding an attribution sits in — a field-provenance binding, a
//! template-version-and-digest binding, or a destination-and-redaction binding) into registry resolvers that produce
//! export-safe, honest projections. Every review then resolves to one typed review-template-packet object — the
//! rationale blocks, checklist text, and bundle manifests it carries, its template version, and the pack digest it is
//! bound to, all attributable to the governing pack so a template version or pack digest is never flattened into
//! generic review text on local draft, publish-now, open-in-provider, or export — and to one
//! template-publish-attribution object — the field provenance (pack-authored, generated, user-edited, omitted, or
//! redacted), the template version and pack digest, and the destination / redaction state a publish or export must
//! show first — that the review, AI-review, support, and provider-handoff surfaces can inspect without manual
//! reconstruction, so a user-edited or redacted field stays visibly separate from pack-authored template content
//! instead of reading as clean pack authorship, and a packet that cannot name the template version and pack digest it
//! is bound to degrades honestly instead of publishing template-driven content under an undisclosed template version.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Preserve pack-authored rationale blocks, checklist text, bundle manifests, the template version, and the pack
//!   digest across local draft, publish-now, open-in-provider, and export.** [`resolve_review_template_packet_entry`]
//!   refuses to read as a clean, registry-bound template entry unless it names a canonical registry token, a
//!   classified [template-content class][M5ReviewTemplatePacketKind], a disclosure role, covers every
//!   [resolution form][M5ReviewTemplatePacketResolutionForm] (the canonical object, the accessible summary, and the
//!   audit record), publishes every field (the rationale blocks, checklist text, and bundle manifests bound to the
//!   template version / pack digest), keeps its authorship provenance attributable to the pack, and keeps a
//!   user-edited or redacted field bound to its disclosure state; otherwise it degrades.
//! * **Preview exactly which template text will publish or export, including what fields are generated, user-edited,
//!   omitted, or redacted.** [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a template entry
//!   whose template-version / authorship-provenance join is not preserved (a user-edited or redacted field presenting
//!   itself as pack-authored template content without one) so it degrades to
//!   [`M5ReviewTemplatePacketEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`] — the
//!   structured blocker reason a template-flattening-without-attribution attempt must surface.
//! * **Carry template attribution into browser / provider handoff, support packets, and reopened draft-only review
//!   state so authorship and pack provenance stay inspectable.** [`resolve_template_publish_attribution_entry`] names a
//!   classified [attribution binding][M5TemplatePublishAttributionScope] (field-provenance binding,
//!   template-version-and-digest binding, or destination-and-redaction binding), requires the full field-provenance /
//!   template-version-and-digest / destination-and-redaction attribution object, covers every resolution form, and
//!   degrades to
//!   [`M5TemplatePublishAttributionEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the attribution would send template-driven review content without showing destination, template source, and
//!   redaction state, hide the user-edited / redacted label, or let user edits masquerade as pack-authored content, so
//!   an attribution can never read as trustworthy when it has quietly dropped the reason a field is not pack-authored,
//!   published-safe content.
//!
...
use crate::m5_review_pack_evaluator_matrix::{
    ...
    M5_REVIEW_PACK_MATRIX_SCHEMA_REF, M5_REVIEW_TEMPLATE_PACKET_DOMAIN_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5ReviewTemplatePacketAndPublishAttributionRegistriesPacket`].
pub const M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_review_template_packet_and_publish_attribution_registries";

/// Schema version for M5 line-review_template_packet / line-downgrade-packet registry records.
pub const M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_DOC_REF: &str =
    "docs/review/m5_review_template_packet_and_publish_attribution_registries.md";

/// Repo-relative path of the canonical template-publish-attribution domain schema minted by this lane (the typed
/// record of exactly which template text a publish or export will send — whether each field is pack-authored,
/// generated, user-edited, omitted, or redacted, the template version and pack digest it is bound to, and the
/// destination / template source / redaction state a publish or export must show first). Declared locally so the
/// row-level domain-schema-ref check stays self-contained.
pub const M5_TEMPLATE_PUBLISH_ATTRIBUTION_DOMAIN_SCHEMA_REF: &str =
    "schemas/review/m5-template-publish-attribution.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/review/m5-review-template-packet-and-publish-attribution-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_CSV_REF: &str =
    "artifacts/review/m5-review-template-packet-and-publish-attribution-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_REPORT_REF: &str =
    "artifacts/review/m5-review-template-packet-and-publish-attribution-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5ReviewTemplatePacketAndPublishAttributionRegistriesConsumerSurface =
    M5ReviewPackConsumerSurface;

/// One of the three resolution forms every line-review_template_packet or line-downgrade-packet entry must hold across so
...
/// line-review_template_packet and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewTemplatePacketResolutionForm {
    /// The canonical resolved line-review_template_packet / line-downgrade-packet object.
    CanonicalObject,
...
impl M5ReviewTemplatePacketResolutionForm {
...
/// Controlled manifest field a review-template-packet entry publishes for one retiring object-class identity, so every
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewTemplatePacketKind {
    ...
    TemplatePacketUnclassified,
}

impl M5ReviewTemplatePacketKind {
    /// Every public-proof-ledger kind, in declaration order.
    pub const ALL: [Self; 7] = [
        Self::RationaleBlock,
        Self::ChecklistText,
        Self::BundleManifest,
        Self::UserEditedField,
        Self::RedactedField,
        Self::SummaryText,
        Self::TemplatePacketUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::RationaleBlock => "rationale_block",
            Self::ChecklistText => "checklist_text",
            Self::BundleManifest => "bundle_manifest",
            Self::UserEditedField => "user_edited_field",
            Self::RedactedField => "redacted_field",
            Self::SummaryText => "summary_text",
            Self::TemplatePacketUnclassified => "template_packet_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::TemplatePacketUnclassified)
    }

    /// The canonical mode for this public-proof-ledger kind.
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::RationaleBlock => "rationale_block_mode",
            Self::ChecklistText => "checklist_text_mode",
            Self::BundleManifest => "bundle_manifest_mode",
            Self::UserEditedField => "user_edited_field_mode",
            Self::RedactedField => "redacted_field_mode",
            Self::SummaryText => "summary_text_mode",
            Self::TemplatePacketUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(self, Self::UserEditedField | Self::RedactedField)
    }
}

/// Controlled template-publish-attribution scope a review-template-packet change sits in, so a changed cutoff date or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5TemplatePublishAttributionScope {
    ...
    TemplateVersionAndDigestBinding,
    /// Retention-state-change: a descriptor changed its retention / removal (expiry / cleanup) state.
    DestinationAndRedactionBinding,
    /// The template-publish-attribution scope is unclassified, which is disallowed.
    TemplatePublishAttributionUnclassified,
}

impl M5TemplatePublishAttributionScope {
    /// Every comparison scope, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::FieldProvenanceBinding,
        Self::TemplateVersionAndDigestBinding,
        Self::DestinationAndRedactionBinding,
        Self::TemplatePublishAttributionUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::FieldProvenanceBinding => "field_provenance_binding",
            Self::TemplateVersionAndDigestBinding => "template_version_and_digest_binding",
            Self::DestinationAndRedactionBinding => "destination_and_redaction_binding",
            Self::TemplatePublishAttributionUnclassified => {
                "template_publish_attribution_unclassified"
            }
        }
    }
    ...
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::TemplatePublishAttributionUnclassified)
    }
}

/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-review_template_packet or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewTemplatePacketSurfaceContext {
...
impl M5ReviewTemplatePacketSurfaceContext {
...
/// One mandatory rendered part a line-review_template_packet or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewTemplatePacketAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-review_template_packet entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-review_template_packet entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-review_template_packet
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line review_template_packet or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5ReviewTemplatePacketAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "template_publish_attribution_fields",
...
/// line review_template_packet, a line-downgrade packet, or a degraded line-review_template_packet / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewTemplatePacketNextAction {
    /// Expand the resolved line review_template_packet's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5ReviewTemplatePacketNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewTemplatePacketExportField {
...
impl M5ReviewTemplatePacketExportField {
...
/// Reason a line-review_template_packet entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewTemplatePacketEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the review_template_packet means.
    DescriptorTokenUnstated,
    /// The render / surface context cannot currently be resolved.
    SurfaceContextUnresolved,
    /// The line journey is unclassified (not in the resolved taxonomy).
    CohortTemplatePacketUnclassified,
    ...
    /// The resolved line-review_template_packet object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5ReviewTemplatePacketEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 9] = [
        Self::DescriptorTokenUnstated,
        Self::SurfaceContextUnresolved,
        Self::CohortTemplatePacketUnclassified,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "review_template_packet_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortTemplatePacketUnclassified => "line_template_packet_unclassified",
            Self::DescriptorNotBoundToRegistry => "review_template_packet_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "review_template_packet_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "review_template_packet_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5ReviewTemplatePacketNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5ReviewTemplatePacketNextAction::TraceCanonicalRegistry
            }
            Self::CohortTemplatePacketUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5ReviewTemplatePacketNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5ReviewTemplatePacketNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5ReviewTemplatePacketNextAction::ReviewBlockedOrDegraded,
        }
    }
    ...
    pub const fn downgrade_trigger(self) -> M5ReviewPackDowngradeTrigger {
        match self {
            ...
            Self::CohortTemplatePacketUnclassified | Self::CohortDescriptorObjectIncomplete => {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5TemplatePublishAttributionEntryDegradeReason {
...
impl M5TemplatePublishAttributionEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::EvidenceScopeUnclassified => "comparison_template_publish_attribution_unclassified",
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                "template_publish_attribution_runs_support_ahead_of_proof_or_drops_template_publish_attribution"
            }
    ...
    pub const fn next_action(self) -> M5ReviewTemplatePacketNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5ReviewTemplatePacketNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5ReviewTemplatePacketNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5ReviewTemplatePacketNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_review_template_packet_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ReviewTemplatePacketEntryResolutionInput {
    /// Stable identity of the line-review_template_packet-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this review_template_packet binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.review_template_packet.redacted_field`); empty means unstated.
    ...
    pub report_section: M5ReviewTemplatePacketKind,
    /// The render / surface context.
    pub surface_context: M5ReviewTemplatePacketSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ReviewTemplatePacketResolutionForm>,
    ...
    /// True when the behavior traces to the line-review_template_packet registry (never a hand-copied constant).
...
/// Resolved, export-safe line-review_template_packet-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedReviewTemplatePacketEntry {
    /// Stable identity of the line-review_template_packet-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this review_template_packet binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-review_template_packet object publishes every required field.
    pub review_template_packet_object_complete: bool,
    /// Whether the entry traces to the line-review_template_packet registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5ReviewTemplatePacketEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ReviewTemplatePacketNextAction,
    /// Whether the review_template_packet resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub review_template_packet_resolves_across_lines: bool,
}

impl M5ResolvedReviewTemplatePacketEntry {
    /// Whether this line-review_template_packet entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_template_publish_attribution_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5TemplatePublishAttributionEntryResolutionInput {
    ...
    pub comparison_scope: M5TemplatePublishAttributionScope,
    /// The render / surface context.
    pub surface_context: M5ReviewTemplatePacketSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ReviewTemplatePacketResolutionForm>,
    ...
    pub keeps_template_publish_attribution_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedTemplatePublishAttributionEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub template_publish_attribution_stays_honest: bool,
    ...
    pub provides_complete_template_publish_attribution: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5TemplatePublishAttributionEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ReviewTemplatePacketNextAction,
...
impl M5ResolvedTemplatePublishAttributionEntry {
...
pub enum M5ReviewTemplatePacketResolutionError {
    /// The line-review_template_packet-entry id was empty.
...
impl M5ReviewTemplatePacketResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_review_template_packet_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_template_publish_attribution_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5ReviewTemplatePacketResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-review_template_packet / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5ReviewTemplatePacketResolutionError {}

fn form_tokens(forms: &[M5ReviewTemplatePacketResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5ReviewTemplatePacketResolutionForm]) -> bool {
    let present: BTreeSet<M5ReviewTemplatePacketResolutionForm> = forms.iter().copied().collect();
    M5ReviewTemplatePacketResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-review_template_packet object publishes every required field: classified line journey,
...
pub fn review_template_packet_object_is_complete(
    journey: M5ReviewTemplatePacketKind,
...
/// Whether the line review_template_packet keeps a line from widening without preserving its rollback and diagnostics
...
pub fn template_publish_attribution_stays_honest(
    scope: M5TemplatePublishAttributionScope,
    comparison_is_truthful: bool,
    keeps_template_publish_attribution_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_template_publish_attribution_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-review_template_packet-registry entry so it stays bound to the line-review_template_packet registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete review_template_packet object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_review_template_packet_entry(
    input: M5ReviewTemplatePacketEntryResolutionInput,
) -> Result<M5ResolvedReviewTemplatePacketEntry, M5ReviewTemplatePacketResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ReviewTemplatePacketResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5ReviewTemplatePacketResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = review_template_packet_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        ...
        Some(M5ReviewTemplatePacketEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    ...
    let next_action = match degrade_reason {
        ...
        None => M5ReviewTemplatePacketNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedReviewTemplatePacketEntry {
        ...
        review_template_packet_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        review_template_packet_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_template_publish_attribution_entry(
    input: M5TemplatePublishAttributionEntryResolutionInput,
) -> Result<M5ResolvedTemplatePublishAttributionEntry, M5ReviewTemplatePacketResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ReviewTemplatePacketResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = template_publish_attribution_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_template_publish_attribution_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5TemplatePublishAttributionEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        ...
        Some(M5TemplatePublishAttributionEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5ReviewTemplatePacketNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedTemplatePublishAttributionEntry {
        entry_id: input.entry_id,
        ...
        keeps_template_publish_attribution_visible: input
            .keeps_template_publish_attribution_visible,
        ...
        template_publish_attribution_stays_honest: record_stays_honest,
        provides_complete_template_publish_attribution: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-review_template_packet and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewTemplatePacketAndPublishAttributionRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5ReviewTemplatePacketAndPublishAttributionRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5ReviewTemplatePacketAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5ReviewTemplatePacketExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5ReviewPackDowngradeTrigger>,
    /// Resolved line-review_template_packet-registry examples.
    pub review_template_packet_entries: Vec<M5ResolvedReviewTemplatePacketEntry>,
    /// Resolved line-downgrade-packet examples.
    pub template_publish_attribution_entries: Vec<M5ResolvedTemplatePublishAttributionEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-review_template_packet and
    ...
    pub collapses_distinct_template_publish_attribution_classes_into_one_lane: bool,
}

impl M5ReviewTemplatePacketAndPublishAttributionRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5ReviewTemplatePacketAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5ReviewTemplatePacketAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5ReviewTemplatePacketExportField> =
            self.export_fields.iter().copied().collect();
        M5ReviewTemplatePacketExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_template_publish_attribution_classes_into_one_lane
    }

    /// True when a clean line-review_template_packet entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete review_template_packet object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn review_template_packet_is_honest(ex: &M5ResolvedReviewTemplatePacketEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.review_template_packet_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedTemplatePublishAttributionEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_template_publish_attribution
                && ex.template_publish_attribution_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.review_template_packet_entries
            .iter()
            .all(Self::review_template_packet_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewTemplatePacketAndPublishAttributionRegistriesVocabularySet {
    ...
    /// Cohort-review_template_packet-entry degrade-reason tokens.
    pub review_template_packet_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub template_publish_attribution_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5ReviewTemplatePacketAndPublishAttributionRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ReviewPackRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5ReviewTemplatePacketResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5ReviewTemplatePacketKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5TemplatePublishAttributionScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5ReviewTemplatePacketSurfaceContext::ALL, |v| v.as_str()),
            review_template_packet_degrade_reasons: tokens(
                &M5ReviewTemplatePacketEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            template_publish_attribution_degrade_reasons: tokens(
                &M5TemplatePublishAttributionEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5ReviewTemplatePacketAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5ReviewTemplatePacketNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5ReviewTemplatePacketExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewTemplatePacketAndPublishAttributionRegistriesGovernanceReview {
    /// The review_template_packet registry names a canonical token, semantic role, and line journey for every entry.
    pub review_template_packet_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-review_template_packet object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_review_template_packet_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved review_template_packet.
    ...
    pub template_publish_attribution_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-review_template_packet and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-review_template_packet and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub review_template_packet_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewTemplatePacketAndPublishAttributionRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-review_template_packet registry.
    ...
    /// Behavior traces back to the canonical line-review_template_packet and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-review_template_packet / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewTemplatePacketAndPublishAttributionRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewTemplatePacketAndPublishAttributionRegistriesReleasePosture {
...
/// Constructor input for [`M5ReviewTemplatePacketAndPublishAttributionRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ReviewTemplatePacketAndPublishAttributionRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5ReviewTemplatePacketAndPublishAttributionRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5ReviewTemplatePacketAndPublishAttributionRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5ReviewTemplatePacketAndPublishAttributionRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection:
        M5ReviewTemplatePacketAndPublishAttributionRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5ReviewTemplatePacketAndPublishAttributionRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5ReviewTemplatePacketAndPublishAttributionRegistriesReleasePosture,
...
/// Export-safe M5 line-review_template_packet and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewTemplatePacketAndPublishAttributionRegistriesPacket {
    /// Record kind; must equal [`M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_SCHEMA_VERSION`].
...
impl M5ReviewTemplatePacketAndPublishAttributionRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5ReviewTemplatePacketAndPublishAttributionRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version:
                M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind
            != M5_REVIEW_TEMPLATE_PACKET_AND_PUBLISH_ATTRIBUTION_REGISTRIES_RECORD_KIND
        {
            violations.push(
                M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::WrongRecordKind,
            );
        }
        ...
        {
            violations.push(
                M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations.push(
                M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::MissingIdentity,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-review_template_packet / line-downgrade-packet registries packet serializes",
        )) {
            violations.push(
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,review_template_packet_entries,template_publish_attribution_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            let degrades: Vec<&str> = row
                ...
                .chain(
                    row.template_publish_attribution_entries
                        .iter()
                        .filter_map(|ex| ex.degrade_reason.map(|r| r.as_str())),
                )
                .collect();
            out.push_str(&format!(
                ...
                row.review_template_packet_entries.len(),
                row.template_publish_attribution_entries.len(),
                degrades.join("|"),
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Review-Template-Packet and Template-Publish-Attribution Registries\n\n");
        ...
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                "  - Template-packet entries: {} / Publish-attribution entries: {}\n",
                row.review_template_packet_entries.len(),
                row.template_publish_attribution_entries.len()
            ));
        }
    ...
    /// registry-bound line-review_template_packet entries are listed.
    pub fn render_review_template_packet_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.review_template_packet_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5ReviewTemplatePacketAndPublishAttributionRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation>),
}

impl fmt::Display for M5ReviewTemplatePacketAndPublishAttributionRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-review_template_packet / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-review_template_packet / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5ReviewTemplatePacketAndPublishAttributionRegistriesArtifactError {}

/// Validation failures emitted by [`M5ReviewTemplatePacketAndPublishAttributionRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation {
    ...
    /// A registry row does not point at both the line-review_template_packet and line-downgrade-packet domain schemas.
    ...
    /// Cohort-review_template_packet-resolution is not proven: clean review_template_packet entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean review_template_packet entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded review_template_packet entry is present, or a clean review_template_packet entry is unbounded
    /// or unbound.
...
impl M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "review_template_packet_resolution_not_proven"
            }
            ...
            Self::CohortEvidenceIntegrityNotProven => {
                "template_publish_attribution_integrity_not_proven"
            }
...
pub fn current_stable_m5_review_template_packet_and_publish_attribution_registries_export(
) -> Result<
    M5ReviewTemplatePacketAndPublishAttributionRegistriesPacket,
    M5ReviewTemplatePacketAndPublishAttributionRegistriesArtifactError,
> {
    let packet: M5ReviewTemplatePacketAndPublishAttributionRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/review/m5-review-template-packet-and-publish-attribution-registries-proof/support_export.json"
        )
    ))
    .map_err(M5ReviewTemplatePacketAndPublishAttributionRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(
            M5ReviewTemplatePacketAndPublishAttributionRegistriesArtifactError::Validation(
                violations,
            ),
        )
    }
...
fn validate_source_contracts(
    ...
    violations: &mut Vec<M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations
            .push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations
                .push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::MandatoryExportFieldMissing);
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_REVIEW_TEMPLATE_PACKET_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_TEMPLATE_PUBLISH_ATTRIBUTION_DOMAIN_SCHEMA_REF)
        {
            violations.push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_template_packet_entries.is_empty()
            || row.template_publish_attribution_entries.is_empty()
        {
            violations.push(
        ...
        if !row.honours_invariants() {
            violations.push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.review_template_packet_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_review_template_packet_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.template_publish_attribution_keeps_proof_visible_and_binds_support_language,
        ...
        review.review_template_packet_or_downgrade_drift_caught_before_release,
        review.every_row_declares_mandatory_anatomy,
        review.reuses_frozen_matrix_vocabulary,
    ] {
        if !ok {
            violations.push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::ConsumerProjectionIncomplete);
            return;
        }
    }
}
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let review_template_packets = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.review_template_packet_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.template_publish_attribution_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean review_template_packet entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean review_template_packet entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = review_template_packets()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = review_template_packets()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5ReviewTemplatePacketKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5ReviewTemplatePacketSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = review_template_packets().any(|ex| {
        ex.degrade_reason
            == Some(M5ReviewTemplatePacketEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete = !review_template_packets()
        .any(|ex| ex.is_clean() && !ex.review_template_packet_object_complete);
    if !(journeys_covered
    ...
    {
        violations
            .push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::CohortDescriptorResolutionNotProven);
    }

    // AC2: line packets preserve rollback and diagnostics posture before widening. A widen-without-rollback
    // example degrades, an unbound example degrades, at least one clean bounded review_template_packet entry is present, and
    // no clean review_template_packet entry is unbounded or unbound.
    let widen_fold_degrades = review_template_packets().any(|ex| {
        ex.degrade_reason
            == Some(
                M5ReviewTemplatePacketEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = review_template_packets().any(|ex| {
        ex.degrade_reason
            == Some(M5ReviewTemplatePacketEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_review_template_packet =
        review_template_packets().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound =
        !review_template_packets().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded =
        !review_template_packets().any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_review_template_packet
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5TemplatePublishAttributionScope::CANONICAL_SCOPES
        .iter()
    ...
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_template_publish_attribution);
    ...
    {
        violations
            .push(M5ReviewTemplatePacketAndPublishAttributionRegistriesViolation::CohortEvidenceIntegrityNotProven);
    }
}
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-review_template_packet registry
