//! Implemented M5 review-pack-record and review-pack-result registries.
//!
//! The frozen [review-pack evaluator matrix][matrix] names Aureline's governed review-pack object classes and
//! locks their controlled vocabulary. This module makes the first two of those classes — a declarative review-pack
//! record and the evaluator result packet it produces — operable so review, AI-review, provider-handoff, and
//! support / export surfaces inherit one canonical review-pack descriptor rather than a hand-authored parallel prose
//! that has to be kept consistent: it turns the *review-pack-record* grammar (one machine-readable record per
//! repo-defined review pack — one typed field per record section: the pack version and content digest, the scope
//! selector, the target diff identity, the worktree / base revision, and the evaluator outcome — each bound to one
//! pack identity with its evaluator lineage) and the *review-pack-result* grammar (the evaluator binding a result
//! sits in — an evaluated-scope binding, a pack-version-and-digest binding, or a divergence-label binding) into
//! registry resolvers that produce export-safe, honest projections. Every review pack then resolves to one typed
//! review-pack-record object — the fields it publishes, its evaluator lineage, its pack version / digest, and its
//! scope selector, all attributable to the pack so a local parity estimate never reads as provider-authoritative
//! mergeability — and to one review-pack-result object — the resolved pack identity, the affected evaluator field,
//! the previous-versus-current result-state reference, the binding state, and the active divergence reason
//! (partial-scope, slice-omitted, stale-pack, ci-only, or provider-unavailable) — that the review, AI-review,
//! support, and provider-handoff surfaces can inspect without manual reconstruction, so a changed evaluated scope,
//! pack digest, or divergence label stays visible as a typed evaluator event instead of a silent mutation, and a
//! record that cannot bind a field to its pack identity degrades honestly instead of leaving a stale pack to read
//! as a fresh, authoritative review result.
//!
...
//!   unless it names a canonical registry token, a classified [scope selector][M5ReviewPackRecordKind],
//!   a disclosure role, covers every [resolution form][M5ReviewPackRecordResolutionForm] (the
...
//!   [`M5ReviewPackRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
...
//!   [evaluator binding][M5ReviewPackResultScope]
...
//!   [`M5ReviewPackResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
...
pub use seed::{
    seeded_m5_review_pack_record_and_result_registries,
    seeded_m5_review_pack_record_and_result_registries_review_pack_record_beta_narrowed,
    seeded_m5_review_pack_record_and_result_registries_review_pack_result_preview_narrowed,
    M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5ReviewPackRecordAndResultRegistriesPacket`].
pub const M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_review_pack_record_and_result_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_SCHEMA_REF: &str =
    "schemas/review/m5-review-pack-record-and-result-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_DOC_REF: &str =
    "docs/review/m5_review_pack_record_and_result_registries.md";
...
pub const M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/review/m5-review-pack-record-and-result-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_CSV_REF: &str =
    "artifacts/review/m5-review-pack-record-and-result-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_REPORT_REF: &str =
    "artifacts/review/m5-review-pack-record-and-result-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/review/m5-review-pack-record-and-result-registries";
...
pub type M5ReviewPackRecordAndResultRegistriesConsumerSurface = M5ReviewPackConsumerSurface;
...
pub enum M5ReviewPackRecordResolutionForm {
...
impl M5ReviewPackRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackResultScope {
    ...
    ReviewPackResultUnclassified,
}

impl M5ReviewPackResultScope {
    ...
    pub const ALL: [Self; 4] = [
        ...
        Self::ReviewPackResultUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ReviewPackResultUnclassified => "review_pack_result_unclassified",
        }
    }

    /// Whether the comparison scope is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::ReviewPackResultUnclassified)
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackRecordSurfaceContext {
...
impl M5ReviewPackRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackRecordAnatomyPart {
...
impl M5ReviewPackRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackRecordNextAction {
...
impl M5ReviewPackRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackRecordExportField {
...
impl M5ReviewPackRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackRecordEntryDegradeReason {
...
impl M5ReviewPackRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5ReviewPackRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5ReviewPackRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5ReviewPackRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5ReviewPackRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5ReviewPackRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackResultEntryDegradeReason {
...
impl M5ReviewPackResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5ReviewPackRecordNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5ReviewPackRecordNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5ReviewPackRecordNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5ReviewPackRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5ReviewPackRecordEntryResolutionInput {
    ...
    pub report_section: M5ReviewPackRecordKind,
    /// The render / surface context.
    pub surface_context: M5ReviewPackRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ReviewPackRecordResolutionForm>,
...
pub struct M5ResolvedReviewPackRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ReviewPackRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ReviewPackRecordNextAction,
...
impl M5ResolvedReviewPackRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ReviewPackResultEntryResolutionInput {
    ...
    pub comparison_scope: M5ReviewPackResultScope,
    /// The render / surface context.
    pub surface_context: M5ReviewPackRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ReviewPackRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedReviewPackResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ReviewPackResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ReviewPackRecordNextAction,
...
impl M5ResolvedReviewPackResultEntry {
...
pub enum M5ReviewPackRecordResolutionError {
...
impl M5ReviewPackRecordResolutionError {
...
impl fmt::Display for M5ReviewPackRecordResolutionError {
...
impl Error for M5ReviewPackRecordResolutionError {}

fn form_tokens(forms: &[M5ReviewPackRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5ReviewPackRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5ReviewPackRecordResolutionForm> = forms.iter().copied().collect();
    M5ReviewPackRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5ReviewPackRecordKind,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedReviewPackRecordEntry, M5ReviewPackRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ReviewPackRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5ReviewPackRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ReviewPackRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ReviewPackRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5ReviewPackRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5ReviewPackRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5ReviewPackRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5ReviewPackRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5ReviewPackRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5ReviewPackRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5ReviewPackRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5ReviewPackRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedReviewPackRecordEntry {
...
pub fn resolve_review_pack_result_entry(
    input: M5ReviewPackResultEntryResolutionInput,
) -> Result<M5ResolvedReviewPackResultEntry, M5ReviewPackRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ReviewPackRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ReviewPackResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ReviewPackResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5ReviewPackResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5ReviewPackResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5ReviewPackResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5ReviewPackResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5ReviewPackRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedReviewPackResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5ReviewPackRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5ReviewPackRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5ReviewPackRecordExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5ReviewPackDowngradeTrigger>,
    /// Resolved line-review_pack_record-registry examples.
    pub review_pack_record_entries: Vec<M5ResolvedReviewPackRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedReviewPackResultEntry>,
...
impl M5ReviewPackRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5ReviewPackRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5ReviewPackRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5ReviewPackRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5ReviewPackRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedReviewPackRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedReviewPackResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackRecordAndResultRegistriesVocabularySet {
...
impl M5ReviewPackRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ReviewPackRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5ReviewPackRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5ReviewPackRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ReviewPackResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5ReviewPackRecordSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5ReviewPackRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5ReviewPackResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5ReviewPackRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5ReviewPackRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5ReviewPackRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5ReviewPackRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ReviewPackRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5ReviewPackRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5ReviewPackRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5ReviewPackRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5ReviewPackRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5ReviewPackRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5ReviewPackRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION`].
...
impl M5ReviewPackRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5ReviewPackRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5ReviewPackRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND {
            violations.push(M5ReviewPackRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_REVIEW_PACK_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5ReviewPackRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5ReviewPackRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5ReviewPackRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5ReviewPackRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Review-Pack-Record and Review-Pack-Result Registries\n\n");
...
#[derive(Debug)]
pub enum M5ReviewPackRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5ReviewPackRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5ReviewPackRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5ReviewPackRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5ReviewPackRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5ReviewPackRecordAndResultRegistriesViolation {
...
impl M5ReviewPackRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_review_pack_record_and_result_registries_export() -> Result<
    M5ReviewPackRecordAndResultRegistriesPacket,
    M5ReviewPackRecordAndResultRegistriesArtifactError,
> {
    let packet: M5ReviewPackRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/review/m5-review-pack-record-and-result-registries-proof/support_export.json"
        )
    ))
    .map_err(M5ReviewPackRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5ReviewPackRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5ReviewPackRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5ReviewPackRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(M5ReviewPackRecordAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5ReviewPackRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(M5ReviewPackRecordAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5ReviewPackRecordAndResultRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations
                .push(M5ReviewPackRecordAndResultRegistriesViolation::MandatoryExportFieldMissing);
        }
        ...
        {
            violations.push(M5ReviewPackRecordAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5ReviewPackRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5ReviewPackRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5ReviewPackRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5ReviewPackRecordAndResultRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5ReviewPackRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete);
            return;
        }
    }
}
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5ReviewPackRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5ReviewPackRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5ReviewPackRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5ReviewPackRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5ReviewPackRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5ReviewPackRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ReviewPackResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations
            .push(M5ReviewPackRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven);
    }
}
