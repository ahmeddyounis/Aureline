//! Implemented M5 retirement-impact-report and impact-blocker-gate registries.
//!
//! The frozen [retired-state matrix][matrix] names Aureline's governed terminal-lifecycle object classes and locks
//! their controlled vocabulary. This module makes retirement safe by proving who and what still depends on a
//! retiring surface — a command / deep link or registry-visible package — before Aureline closes the support window,
//! so review packets, support exports, and public-proof consumers inherit one canonical retirement blast-radius
//! report rather than a hand-authored parallel prose that has to be kept consistent: it turns the
//! *retirement-impact-report* grammar (one export-safe impact report per retirement candidate — one classified
//! dependency finding per detected dependency (a workflow bundle, migration pack, command / deep link, CLI alias,
//! SDK contract row, saved artifact, profile, recipe, marketplace entry, mirror, or managed / new-tenant offering),
//! typed as blocking, migration-required, historical-only, mirror-only, tenant-gated, or informational with an
//! owning team and joined to the current compatibility / public-proof state and the successor path or manual
//! fallback) and the *impact-blocker-gate* grammar (the gate scope a blocker sits in — a bundle, a tenant, or a
//! schema / public artifact still pointing at the candidate) into registry resolvers that produce export-safe,
//! honest projections. Every retirement candidate then resolves to one typed retirement-impact-report object — the
//! findings it publishes, its exact-build joins, its successor path, and its rollback / export route, all preserved
//! before a claim widens so a retiring surface never keeps support language ahead of its closed support note and a
//! current rollback / export route — and to one impact-blocker-gate object — the resolved object identity, the
//! affected dependency finding, the candidate-versus-dependent reference, the gate-scope state, and the active gate
//! reason — that the review-packet, support-export, and public-proof surfaces can inspect without manual
//! reconstruction, so a live dependency keeps the candidate's blocker list visible instead of letting closure run
//! over a dangling reference, and an impact report that cannot bind a finding to its exact build degrades honestly
//! instead of leaving a retiring surface to read as safe to close.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable retirement manifest per retiring class, carrying every manifest field.**
//!   [`resolve_retirement_impact_report_entry`] refuses to read as a clean, registry-bound manifest entry
//!   unless it names a canonical registry token, a classified [manifest field][M5RetirementImpactReportKind], a
//!   retirement role, covers every [resolution form][M5RetirementImpactReportResolutionForm] (the
...
//!   [`resolve_impact_blocker_gate_entry`] names a classified [diff scope][M5ImpactBlockerGateScope]
...
/// Stable record-kind tag carried by [`M5RetirementImpactReportBlockerGateRegistriesPacket`].
pub const M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_retirement_impact_report_and_blocker_gate_packet_registries";

/// Schema version for M5 line-retirement_impact_report / line-downgrade-packet registry records.
pub const M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_DOC_REF: &str =
    "docs/release/m5_retirement_impact_report_and_blocker_gate_registries.md";
...
pub const M5_IMPACT_BLOCKER_GATE_DOMAIN_SCHEMA_REF: &str =
    "schemas/program/m5-retirement-impact-blocker-gate.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-retirement-impact-report-and-blocker-gate-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-retirement-impact-report-and-blocker-gate-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-retirement-impact-report-and-blocker-gate-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5RetirementImpactReportBlockerGateRegistriesConsumerSurface =
    M5RetiredStateConsumerSurface;

/// One of the three resolution forms every line-retirement_impact_report or line-downgrade-packet entry must hold across so
...
/// line-retirement_impact_report and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementImpactReportResolutionForm {
    /// The canonical resolved line-retirement_impact_report / line-downgrade-packet object.
    CanonicalObject,
...
impl M5RetirementImpactReportResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementImpactReportKind {
    ...
    ImpactClassUnclassified,
}

impl M5RetirementImpactReportKind {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::TenantGated => "tenant_gated",
            Self::Informational => "informational",
            Self::ImpactClassUnclassified => "impact_class_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::ImpactClassUnclassified)
    }
    ...
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            ...
            Self::MigrationRequired => "migration_required_mode",
            Self::HistoricalOnly => "historical_only_mode",
            Self::MirrorOnly => "mirror_only_mode",
            Self::TenantGated => "tenant_gated_mode",
            Self::Informational => "informational_mode",
            Self::ImpactClassUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(self, Self::MirrorOnly | Self::TenantGated)
    }
}
...
impl M5ImpactBlockerGateScope {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::BundleStillPointsAtCandidate => "bundle_still_points_at_candidate",
            Self::TenantStillPointsAtCandidate => "tenant_still_points_at_candidate",
            Self::SchemaOrPublicArtifactStillPointsAtCandidate => {
                "schema_or_public_artifact_still_points_at_candidate"
            }
            Self::BlockerScopeUnclassified => "blocker_scope_unclassified",
        }
    }

    /// Whether the comparison scope is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::BlockerScopeUnclassified)
    }
}

/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-retirement_impact_report or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementImpactReportSurfaceContext {
...
impl M5RetirementImpactReportSurfaceContext {
...
/// One mandatory rendered part a line-retirement_impact_report or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementImpactReportAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-retirement_impact_report entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-retirement_impact_report entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-retirement_impact_report
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line retirement_impact_report or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5RetirementImpactReportAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "impact_blocker_gate_fields",
...
/// line retirement_impact_report, a line-downgrade packet, or a degraded line-retirement_impact_report / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementImpactReportNextAction {
    /// Expand the resolved line retirement_impact_report's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5RetirementImpactReportNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementImpactReportExportField {
...
impl M5RetirementImpactReportExportField {
...
/// Reason a line-retirement_impact_report entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementImpactReportEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the retirement_impact_report means.
    ...
    /// The resolved line-retirement_impact_report object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5RetirementImpactReportEntryDegradeReason {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "retirement_impact_report_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortManifestFieldUnclassified => "line_impact_class_unclassified",
            Self::DescriptorNotBoundToRegistry => "retirement_impact_report_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "retirement_impact_report_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "retirement_impact_report_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5RetirementImpactReportNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5RetirementImpactReportNextAction::TraceCanonicalRegistry
            }
            Self::CohortManifestFieldUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5RetirementImpactReportNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5RetirementImpactReportNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5RetirementImpactReportNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ImpactBlockerGateEntryDegradeReason {
...
impl M5ImpactBlockerGateEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::EvidenceScopeUnclassified => "comparison_blocker_scope_unclassified",
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                "impact_blocker_gate_runs_support_ahead_of_proof_or_drops_impact_blocker_gate"
            }
    ...
    pub const fn next_action(self) -> M5RetirementImpactReportNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5RetirementImpactReportNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_retirement_impact_report_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5RetirementImpactReportEntryResolutionInput {
    /// Stable identity of the line-retirement_impact_report-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this retirement_impact_report binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.retirement_impact_report.tenant_gated`); empty means unstated.
    ...
    pub report_section: M5RetirementImpactReportKind,
    /// The render / surface context.
    pub surface_context: M5RetirementImpactReportSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5RetirementImpactReportResolutionForm>,
    ...
    /// True when the behavior traces to the line-retirement_impact_report registry (never a hand-copied constant).
...
/// Resolved, export-safe line-retirement_impact_report-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedRetirementImpactReportEntry {
    /// Stable identity of the line-retirement_impact_report-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this retirement_impact_report binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-retirement_impact_report object publishes every required field.
    pub retirement_impact_report_object_complete: bool,
    /// Whether the entry traces to the line-retirement_impact_report registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5RetirementImpactReportEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5RetirementImpactReportNextAction,
    /// Whether the retirement_impact_report resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub retirement_impact_report_resolves_across_lines: bool,
}

impl M5ResolvedRetirementImpactReportEntry {
    /// Whether this line-retirement_impact_report entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_impact_blocker_gate_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ImpactBlockerGateEntryResolutionInput {
    ...
    pub comparison_scope: M5ImpactBlockerGateScope,
    /// The render / surface context.
    pub surface_context: M5RetirementImpactReportSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5RetirementImpactReportResolutionForm>,
    ...
    pub keeps_impact_blocker_gate_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedImpactBlockerGateEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub impact_blocker_gate_stays_honest: bool,
    ...
    pub provides_complete_impact_blocker_gate: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5ImpactBlockerGateEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5RetirementImpactReportNextAction,
...
impl M5ResolvedImpactBlockerGateEntry {
...
pub enum M5RetirementImpactReportResolutionError {
    /// The line-retirement_impact_report-entry id was empty.
...
impl M5RetirementImpactReportResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_retirement_impact_report_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_impact_blocker_gate_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5RetirementImpactReportResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-retirement_impact_report / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5RetirementImpactReportResolutionError {}

fn form_tokens(forms: &[M5RetirementImpactReportResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5RetirementImpactReportResolutionForm]) -> bool {
    let present: BTreeSet<M5RetirementImpactReportResolutionForm> = forms.iter().copied().collect();
    M5RetirementImpactReportResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-retirement_impact_report object publishes every required field: classified line journey,
...
pub fn retirement_impact_report_object_is_complete(
    journey: M5RetirementImpactReportKind,
...
/// Whether the line retirement_impact_report keeps a line from widening without preserving its rollback and diagnostics
...
pub fn impact_blocker_gate_stays_honest(
    scope: M5ImpactBlockerGateScope,
    comparison_is_truthful: bool,
    keeps_impact_blocker_gate_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_impact_blocker_gate_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-retirement_impact_report-registry entry so it stays bound to the line-retirement_impact_report registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete retirement_impact_report object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_retirement_impact_report_entry(
    input: M5RetirementImpactReportEntryResolutionInput,
) -> Result<M5ResolvedRetirementImpactReportEntry, M5RetirementImpactReportResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5RetirementImpactReportResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5RetirementImpactReportResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = retirement_impact_report_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        ...
        Some(
            M5RetirementImpactReportEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    ...
    let next_action = match degrade_reason {
        ...
        None => M5RetirementImpactReportNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedRetirementImpactReportEntry {
        ...
        retirement_impact_report_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        retirement_impact_report_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_impact_blocker_gate_entry(
    input: M5ImpactBlockerGateEntryResolutionInput,
) -> Result<M5ResolvedImpactBlockerGateEntry, M5RetirementImpactReportResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5RetirementImpactReportResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = impact_blocker_gate_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_impact_blocker_gate_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ImpactBlockerGateEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        ...
        Some(M5ImpactBlockerGateEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5RetirementImpactReportNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedImpactBlockerGateEntry {
        entry_id: input.entry_id,
        ...
        keeps_impact_blocker_gate_visible: input.keeps_impact_blocker_gate_visible,
        ...
        impact_blocker_gate_stays_honest: record_stays_honest,
        provides_complete_impact_blocker_gate: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-retirement_impact_report and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementImpactReportBlockerGateRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5RetirementImpactReportBlockerGateRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5RetirementImpactReportAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5RetirementImpactReportExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5RetiredStateDowngradeTrigger>,
    /// Resolved line-retirement_impact_report-registry examples.
    pub retirement_impact_report_entries: Vec<M5ResolvedRetirementImpactReportEntry>,
    /// Resolved line-downgrade-packet examples.
    pub impact_blocker_gate_entries: Vec<M5ResolvedImpactBlockerGateEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-retirement_impact_report and
    ...
    pub collapses_distinct_impact_blocker_gate_classes_into_one_lane: bool,
}

impl M5RetirementImpactReportBlockerGateRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5RetirementImpactReportAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5RetirementImpactReportAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5RetirementImpactReportExportField> =
            self.export_fields.iter().copied().collect();
        M5RetirementImpactReportExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_impact_blocker_gate_classes_into_one_lane
    }

    /// True when a clean line-retirement_impact_report entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete retirement_impact_report object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn retirement_impact_report_is_honest(ex: &M5ResolvedRetirementImpactReportEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.retirement_impact_report_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedImpactBlockerGateEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_impact_blocker_gate
                && ex.impact_blocker_gate_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.retirement_impact_report_entries
            .iter()
            .all(Self::retirement_impact_report_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementImpactReportBlockerGateRegistriesVocabularySet {
    ...
    /// Cohort-retirement_impact_report-entry degrade-reason tokens.
    pub retirement_impact_report_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub impact_blocker_gate_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5RetirementImpactReportBlockerGateRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5RetiredStateRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5RetirementImpactReportResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5RetirementImpactReportKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ImpactBlockerGateScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5RetirementImpactReportSurfaceContext::ALL, |v| v.as_str()),
            retirement_impact_report_degrade_reasons: tokens(
                &M5RetirementImpactReportEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            impact_blocker_gate_degrade_reasons: tokens(
                &M5ImpactBlockerGateEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5RetirementImpactReportAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5RetirementImpactReportNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5RetirementImpactReportExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementImpactReportBlockerGateRegistriesGovernanceReview {
    /// The retirement_impact_report registry names a canonical token, semantic role, and line journey for every entry.
    pub retirement_impact_report_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-retirement_impact_report object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_retirement_impact_report_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved retirement_impact_report.
    ...
    pub impact_blocker_gate_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-retirement_impact_report and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-retirement_impact_report and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub retirement_impact_report_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementImpactReportBlockerGateRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-retirement_impact_report registry.
    ...
    /// Behavior traces back to the canonical line-retirement_impact_report and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-retirement_impact_report / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementImpactReportBlockerGateRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementImpactReportBlockerGateRegistriesReleasePosture {
...
/// Constructor input for [`M5RetirementImpactReportBlockerGateRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5RetirementImpactReportBlockerGateRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5RetirementImpactReportBlockerGateRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5RetirementImpactReportBlockerGateRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5RetirementImpactReportBlockerGateRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5RetirementImpactReportBlockerGateRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5RetirementImpactReportBlockerGateRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5RetirementImpactReportBlockerGateRegistriesReleasePosture,
...
/// Export-safe M5 line-retirement_impact_report and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementImpactReportBlockerGateRegistriesPacket {
    /// Record kind; must equal [`M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_SCHEMA_VERSION`].
...
impl M5RetirementImpactReportBlockerGateRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5RetirementImpactReportBlockerGateRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5RetirementImpactReportBlockerGateRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_RECORD_KIND {
            violations
                .push(M5RetirementImpactReportBlockerGateRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_RETIREMENT_IMPACT_REPORT_BLOCKER_GATE_REGISTRIES_SCHEMA_VERSION
        {
            violations
                .push(M5RetirementImpactReportBlockerGateRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations
                .push(M5RetirementImpactReportBlockerGateRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations
                .push(M5RetirementImpactReportBlockerGateRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-retirement_impact_report / line-downgrade-packet registries packet serializes",
        )) {
            violations
                .push(M5RetirementImpactReportBlockerGateRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,retirement_impact_report_entries,impact_blocker_gate_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            let degrades: Vec<&str> = row
                ...
                .chain(
                    row.impact_blocker_gate_entries
                        .iter()
                        .filter_map(|ex| ex.degrade_reason.map(|r| r.as_str())),
                )
                .collect();
            out.push_str(&format!(
                ...
                row.retirement_impact_report_entries.len(),
                row.impact_blocker_gate_entries.len(),
                degrades.join("|"),
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Retirement-Impact-Report and Impact-Blocker-Gate Registries\n\n");
    ...
    /// registry-bound line-retirement_impact_report entries are listed.
    pub fn render_retirement_impact_report_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.retirement_impact_report_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5RetirementImpactReportBlockerGateRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5RetirementImpactReportBlockerGateRegistriesViolation>),
}

impl fmt::Display for M5RetirementImpactReportBlockerGateRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-retirement_impact_report / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-retirement_impact_report / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5RetirementImpactReportBlockerGateRegistriesArtifactError {}

/// Validation failures emitted by [`M5RetirementImpactReportBlockerGateRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5RetirementImpactReportBlockerGateRegistriesViolation {
    ...
    /// A registry row does not point at both the line-retirement_impact_report and line-downgrade-packet domain schemas.
    ...
    /// Cohort-retirement_impact_report-resolution is not proven: clean retirement_impact_report entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean retirement_impact_report entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded retirement_impact_report entry is present, or a clean retirement_impact_report entry is unbounded
    /// or unbound.
...
impl M5RetirementImpactReportBlockerGateRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "retirement_impact_report_resolution_not_proven"
            }
            Self::RollbackAndDiagnosticsPreservationNotProven => {
                "rollback_and_diagnostics_preservation_not_proven"
            }
            Self::CohortEvidenceIntegrityNotProven => "impact_blocker_gate_integrity_not_proven",
...
pub fn current_stable_m5_retirement_impact_report_and_blocker_gate_registries_export() -> Result<
    M5RetirementImpactReportBlockerGateRegistriesPacket,
    M5RetirementImpactReportBlockerGateRegistriesArtifactError,
> {
    let packet: M5RetirementImpactReportBlockerGateRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-retirement-impact-report-and-blocker-gate-registries-proof/support_export.json"
        )
    ))
    .map_err(M5RetirementImpactReportBlockerGateRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5RetirementImpactReportBlockerGateRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5RetirementImpactReportBlockerGateRegistriesPacket,
    violations: &mut Vec<M5RetirementImpactReportBlockerGateRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5RetirementImpactReportBlockerGateRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5RetirementImpactReportBlockerGateRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5RetirementImpactReportBlockerGateRegistriesViolation::RegistryRowIncomplete,
            );
        }
        ...
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_RETIREMENT_IMPACT_REPORT_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_IMPACT_BLOCKER_GATE_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5RetirementImpactReportBlockerGateRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.retirement_impact_report_entries.is_empty()
            || row.impact_blocker_gate_entries.is_empty()
        {
            violations
                .push(M5RetirementImpactReportBlockerGateRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations
                .push(M5RetirementImpactReportBlockerGateRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5RetirementImpactReportBlockerGateRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.retirement_impact_report_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_retirement_impact_report_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.impact_blocker_gate_keeps_proof_visible_and_binds_support_language,
        ...
        review.retirement_impact_report_or_downgrade_drift_caught_before_release,
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5RetirementImpactReportBlockerGateRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5RetirementImpactReportBlockerGateRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let retirement_impact_reports = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.retirement_impact_report_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.impact_blocker_gate_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean retirement_impact_report entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean retirement_impact_report entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = retirement_impact_reports()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = retirement_impact_reports()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5RetirementImpactReportKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5RetirementImpactReportSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = retirement_impact_reports().any(|ex| {
        ex.degrade_reason
            == Some(M5RetirementImpactReportEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete = !retirement_impact_reports()
        .any(|ex| ex.is_clean() && !ex.retirement_impact_report_object_complete);
    if !(journeys_covered
    ...
    // example degrades, an unbound example degrades, at least one clean bounded retirement_impact_report entry is present, and
    // no clean retirement_impact_report entry is unbounded or unbound.
    let widen_fold_degrades = retirement_impact_reports().any(|ex| {
        ex.degrade_reason
            == Some(
                M5RetirementImpactReportEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = retirement_impact_reports().any(|ex| {
        ex.degrade_reason
            == Some(M5RetirementImpactReportEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_retirement_impact_report =
        retirement_impact_reports().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound =
        !retirement_impact_reports().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded = !retirement_impact_reports()
        .any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_retirement_impact_report
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ImpactBlockerGateScope::CANONICAL_SCOPES
        .iter()
    ...
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_impact_blocker_gate);
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-retirement_impact_report registry
