//! Implemented M5 ownership-signal-row and owner-conflict registries.
//!
//! The frozen [review-pack evaluator matrix][matrix] names Aureline's governed review-pack object classes and
//! locks their controlled vocabulary. This module makes the matrix's `ownership_signal` class operable so review
//! lists, review detail, merge-readiness, AI-review, browser handoff, and support / export surfaces inherit one
//! canonical ownership model rather than a hand-authored parallel prose that has to be kept consistent: it turns
//! the *ownership-signal-row* grammar (one machine-readable row per owned slice, carrying the owner source class —
//! a CODEOWNERS repo rule, a graph-overlay maintainer, a provider-suggested reviewer, an enforced review-gate
//! owner, an advisory area owner, or a fallback default owner — and the advisory-versus-enforced owner authority,
//! kept mechanically distinct so an advisory owner is never flattened into an enforced merge gate) and the
//! *owner-conflict* reconciliation grammar (the binding a reconciliation sits in — an owner-authority binding, an
//! owner-source-provenance binding, or an owner-conflict-rationale binding) into registry resolvers that produce
//! export-safe, honest projections. Every owned slice then resolves to one typed ownership-signal row — its owner
//! source class, its owner authority, and its reviewer rationale, all attributable to the source so a repo rule,
//! a graph overlay, and a provider suggestion stay distinguishable — and to one owner-conflict reconciliation —
//! the reconciliation binding, the winning-versus-advisory relationship, and the rationale — that the review,
//! AI-review, support, and provider-handoff surfaces can inspect without manual reconstruction, so a disagreement
//! between a CODEOWNERS repo rule, a graph-derived maintainer, and a provider suggestion stays a visible, explained
//! event instead of a silent last-writer-wins collapse, and a row that cannot bind its owner to a classified
//! source class degrades honestly instead of reading as an authoritative owner signal.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable ownership-signal row per owned slice, distinguishing advisory from enforced.**
//!   [`resolve_ownership_signal_row_entry`] refuses to read as a clean, registry-bound row entry
//!   unless it names a canonical registry token, a classified [owner source class][M5OwnershipSignalRowKind],
//!   a disclosure role, covers every [resolution form][M5OwnershipSignalRowResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every field, keeps its owner
//!   provenance attributable to the source, and keeps an advisory owner mechanically distinct from an enforced
//!   owner; otherwise it degrades.
//! * **Preserve owner provenance rather than a hand-authored parallel prose staying consistent.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a row entry whose
//!   owner-authority / owner-provenance join is not preserved (an advisory owner presenting itself as an enforced
//!   gate without one) so it degrades to
//!   [`M5OwnershipSignalRowEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`] —
//!   the structured blocker reason an advisory-shown-as-enforced attempt must surface.
//! * **Turn a disagreement between a repo rule, a graph maintainer, and a provider suggestion into a visible,
//!   explained event instead of a silent collapse.** [`resolve_owner_conflict_entry`] names a classified
//!   [reconciliation binding][M5OwnerConflictScope]
//!   (owner-authority binding, owner-source-provenance binding, or owner-conflict-rationale binding), requires the
//!   full reconciliation object with its winning-versus-advisory relationship and reviewer rationale, covers every
...
//!   when the reconciliation would collapse the conflicting owners into one pill or hide the disagreement, so an
//!   exported result can never read as trustworthy when it has quietly dropped the owner source class or rationale.
//!
...
//! can never fork their own ownership meaning. Raw secret values and private endpoints stay outside the export boundary.
//!
...
use crate::m5_review_pack_evaluator_matrix::{
    ...
    M5ReviewPackRequiredLabel, M5ReviewPackRole, M5_OWNERSHIP_SIGNAL_DOMAIN_SCHEMA_REF,
    M5_REVIEW_PACK_MATRIX_DOC_REF, M5_REVIEW_PACK_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5OwnershipSignalAndConflictRegistriesPacket`].
pub const M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_ownership_signal_and_conflict_packet_registries";

/// Schema version for M5 line-ownership_signal_row / line-downgrade-packet registry records.
pub const M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_SCHEMA_REF: &str =
    "schemas/review/m5-ownership-signal-and-conflict-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_DOC_REF: &str =
    "docs/review/m5_ownership_signal_and_conflict_registries.md";
...
pub const M5_OWNER_CONFLICT_DOMAIN_SCHEMA_REF: &str =
    "schemas/review/m5-owner-conflict.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/review/m5-ownership-signal-and-conflict-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_CSV_REF: &str =
    "artifacts/review/m5-ownership-signal-and-conflict-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_REPORT_REF: &str =
    "artifacts/review/m5-ownership-signal-and-conflict-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5OwnershipSignalAndConflictRegistriesConsumerSurface = M5ReviewPackConsumerSurface;

/// One of the three resolution forms every line-ownership_signal_row or line-downgrade-packet entry must hold across so
...
/// line-ownership_signal_row and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5OwnershipSignalRowResolutionForm {
    /// The canonical resolved line-ownership_signal_row / line-downgrade-packet object.
    CanonicalObject,
...
impl M5OwnershipSignalRowResolutionForm {
...
/// Controlled manifest field a ownership-signal-row entry publishes for one retiring object-class identity, so every
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5OwnershipSignalRowKind {
    ...
    FallbackDefaultOwner,
    /// The manifest field is unclassified, which is disallowed.
    OwnershipSourceUnclassified,
}

impl M5OwnershipSignalRowKind {
    /// Every public-proof-ledger kind, in declaration order.
    pub const ALL: [Self; 7] = [
        Self::CodeownersRuleOwner,
        Self::GraphOverlayMaintainer,
        Self::ProviderSuggestedReviewer,
        Self::EnforcedReviewGateOwner,
        Self::AdvisoryAreaOwner,
        Self::FallbackDefaultOwner,
        Self::OwnershipSourceUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::CodeownersRuleOwner => "codeowners_rule_owner",
            Self::GraphOverlayMaintainer => "graph_overlay_maintainer",
            Self::ProviderSuggestedReviewer => "provider_suggested_reviewer",
            Self::EnforcedReviewGateOwner => "enforced_review_gate_owner",
            Self::AdvisoryAreaOwner => "advisory_area_owner",
            Self::FallbackDefaultOwner => "fallback_default_owner",
            Self::OwnershipSourceUnclassified => "ownership_source_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::OwnershipSourceUnclassified)
    }

    /// The canonical mode for this public-proof-ledger kind.
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::CodeownersRuleOwner => "codeowners_rule_owner_mode",
            Self::GraphOverlayMaintainer => "graph_overlay_maintainer_mode",
            Self::ProviderSuggestedReviewer => "provider_suggested_reviewer_mode",
            Self::EnforcedReviewGateOwner => "enforced_review_gate_owner_mode",
            Self::AdvisoryAreaOwner => "advisory_area_owner_mode",
            Self::FallbackDefaultOwner => "fallback_default_owner_mode",
            Self::OwnershipSourceUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(
            self,
            Self::EnforcedReviewGateOwner | Self::AdvisoryAreaOwner
        )
    }
}

/// Controlled owner-conflict scope a ownership-signal-row change sits in, so a changed cutoff date or
...
pub enum M5OwnerConflictScope {
    ...
    OwnerSourceProvenanceBinding,
    /// Retention-state-change: a descriptor changed its retention / removal (expiry / cleanup) state.
    OwnerConflictRationaleBinding,
    /// The owner-conflict scope is unclassified, which is disallowed.
    OwnerConflictUnclassified,
}

impl M5OwnerConflictScope {
    /// Every comparison scope, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::OwnerAuthorityBinding,
        Self::OwnerSourceProvenanceBinding,
        Self::OwnerConflictRationaleBinding,
        Self::OwnerConflictUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::OwnerAuthorityBinding => "owner_authority_binding",
            Self::OwnerSourceProvenanceBinding => "owner_source_provenance_binding",
            Self::OwnerConflictRationaleBinding => "owner_conflict_rationale_binding",
            Self::OwnerConflictUnclassified => "owner_conflict_unclassified",
        }
    }

    /// Whether the comparison scope is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::OwnerConflictUnclassified)
    }
}

/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-ownership_signal_row or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5OwnershipSignalRowSurfaceContext {
...
impl M5OwnershipSignalRowSurfaceContext {
...
/// One mandatory rendered part a line-ownership_signal_row or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5OwnershipSignalRowAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-ownership_signal_row entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-ownership_signal_row entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-ownership_signal_row
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line ownership_signal_row or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5OwnershipSignalRowAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "owner_conflict_fields",
...
/// line ownership_signal_row, a line-downgrade packet, or a degraded line-ownership_signal_row / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5OwnershipSignalRowNextAction {
    /// Expand the resolved line ownership_signal_row's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5OwnershipSignalRowNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5OwnershipSignalRowExportField {
...
impl M5OwnershipSignalRowExportField {
...
/// Reason a line-ownership_signal_row entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5OwnershipSignalRowEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the ownership_signal_row means.
    DescriptorTokenUnstated,
    /// The render / surface context cannot currently be resolved.
    SurfaceContextUnresolved,
    /// The line journey is unclassified (not in the resolved taxonomy).
    CohortOwnershipSourceUnclassified,
    ...
    /// The resolved line-ownership_signal_row object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5OwnershipSignalRowEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 9] = [
        Self::DescriptorTokenUnstated,
        Self::SurfaceContextUnresolved,
        Self::CohortOwnershipSourceUnclassified,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "ownership_signal_row_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortOwnershipSourceUnclassified => "line_ownership_source_unclassified",
            Self::DescriptorNotBoundToRegistry => "ownership_signal_row_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "ownership_signal_row_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "ownership_signal_row_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5OwnershipSignalRowNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5OwnershipSignalRowNextAction::TraceCanonicalRegistry
            }
            Self::CohortOwnershipSourceUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5OwnershipSignalRowNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5OwnershipSignalRowNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5OwnershipSignalRowNextAction::ReviewBlockedOrDegraded,
        }
    }
    ...
    pub const fn downgrade_trigger(self) -> M5ReviewPackDowngradeTrigger {
        match self {
            ...
            Self::CohortOwnershipSourceUnclassified | Self::CohortDescriptorObjectIncomplete => {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5OwnerConflictEntryDegradeReason {
...
impl M5OwnerConflictEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::EvidenceScopeUnclassified => "comparison_owner_conflict_unclassified",
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                "owner_conflict_runs_support_ahead_of_proof_or_drops_owner_conflict"
            }
    ...
    pub const fn next_action(self) -> M5OwnershipSignalRowNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5OwnershipSignalRowNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5OwnershipSignalRowNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5OwnershipSignalRowNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_ownership_signal_row_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5OwnershipSignalRowEntryResolutionInput {
    /// Stable identity of the line-ownership_signal_row-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this ownership_signal_row binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.ownership_signal_row.advisory_area_owner`); empty means unstated.
    ...
    pub report_section: M5OwnershipSignalRowKind,
    /// The render / surface context.
    pub surface_context: M5OwnershipSignalRowSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5OwnershipSignalRowResolutionForm>,
    ...
    /// True when the behavior traces to the line-ownership_signal_row registry (never a hand-copied constant).
...
/// Resolved, export-safe line-ownership_signal_row-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedOwnershipSignalRowEntry {
    /// Stable identity of the line-ownership_signal_row-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this ownership_signal_row binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-ownership_signal_row object publishes every required field.
    pub ownership_signal_row_object_complete: bool,
    /// Whether the entry traces to the line-ownership_signal_row registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5OwnershipSignalRowEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5OwnershipSignalRowNextAction,
    /// Whether the ownership_signal_row resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub ownership_signal_row_resolves_across_lines: bool,
}

impl M5ResolvedOwnershipSignalRowEntry {
    /// Whether this line-ownership_signal_row entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_owner_conflict_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5OwnerConflictEntryResolutionInput {
    ...
    pub comparison_scope: M5OwnerConflictScope,
    /// The render / surface context.
    pub surface_context: M5OwnershipSignalRowSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5OwnershipSignalRowResolutionForm>,
    ...
    pub keeps_owner_conflict_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedOwnerConflictEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub owner_conflict_stays_honest: bool,
    ...
    pub provides_complete_owner_conflict: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5OwnerConflictEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5OwnershipSignalRowNextAction,
...
impl M5ResolvedOwnerConflictEntry {
...
pub enum M5OwnershipSignalRowResolutionError {
    /// The line-ownership_signal_row-entry id was empty.
...
impl M5OwnershipSignalRowResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_ownership_signal_row_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_owner_conflict_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5OwnershipSignalRowResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-ownership_signal_row / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5OwnershipSignalRowResolutionError {}

fn form_tokens(forms: &[M5OwnershipSignalRowResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5OwnershipSignalRowResolutionForm]) -> bool {
    let present: BTreeSet<M5OwnershipSignalRowResolutionForm> = forms.iter().copied().collect();
    M5OwnershipSignalRowResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-ownership_signal_row object publishes every required field: classified line journey,
...
pub fn ownership_signal_row_object_is_complete(
    journey: M5OwnershipSignalRowKind,
...
/// Whether the line ownership_signal_row keeps a line from widening without preserving its rollback and diagnostics
...
pub fn owner_conflict_stays_honest(
    scope: M5OwnerConflictScope,
    comparison_is_truthful: bool,
    keeps_owner_conflict_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_owner_conflict_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-ownership_signal_row-registry entry so it stays bound to the line-ownership_signal_row registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete ownership_signal_row object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_ownership_signal_row_entry(
    input: M5OwnershipSignalRowEntryResolutionInput,
) -> Result<M5ResolvedOwnershipSignalRowEntry, M5OwnershipSignalRowResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5OwnershipSignalRowResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5OwnershipSignalRowResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = ownership_signal_row_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        ...
        Some(M5OwnershipSignalRowEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    ...
    let next_action = match degrade_reason {
        ...
        None => M5OwnershipSignalRowNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedOwnershipSignalRowEntry {
        ...
        ownership_signal_row_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        ownership_signal_row_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_owner_conflict_entry(
    input: M5OwnerConflictEntryResolutionInput,
) -> Result<M5ResolvedOwnerConflictEntry, M5OwnershipSignalRowResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5OwnershipSignalRowResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = owner_conflict_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_owner_conflict_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5OwnerConflictEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        ...
        Some(M5OwnerConflictEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5OwnershipSignalRowNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedOwnerConflictEntry {
        entry_id: input.entry_id,
        ...
        keeps_owner_conflict_visible: input.keeps_owner_conflict_visible,
        ...
        owner_conflict_stays_honest: record_stays_honest,
        provides_complete_owner_conflict: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-ownership_signal_row and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5OwnershipSignalAndConflictRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5OwnershipSignalAndConflictRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5OwnershipSignalRowAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5OwnershipSignalRowExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5ReviewPackDowngradeTrigger>,
    /// Resolved line-ownership_signal_row-registry examples.
    pub ownership_signal_row_entries: Vec<M5ResolvedOwnershipSignalRowEntry>,
    /// Resolved line-downgrade-packet examples.
    pub owner_conflict_entries: Vec<M5ResolvedOwnerConflictEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-ownership_signal_row and
    ...
    pub collapses_distinct_owner_conflict_classes_into_one_lane: bool,
}

impl M5OwnershipSignalAndConflictRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5OwnershipSignalRowAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5OwnershipSignalRowAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5OwnershipSignalRowExportField> =
            self.export_fields.iter().copied().collect();
        M5OwnershipSignalRowExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_owner_conflict_classes_into_one_lane
    }

    /// True when a clean line-ownership_signal_row entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete ownership_signal_row object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn ownership_signal_row_is_honest(ex: &M5ResolvedOwnershipSignalRowEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.ownership_signal_row_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedOwnerConflictEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_owner_conflict
                && ex.owner_conflict_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.ownership_signal_row_entries
            .iter()
            .all(Self::ownership_signal_row_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5OwnershipSignalAndConflictRegistriesVocabularySet {
    ...
    /// Cohort-ownership_signal_row-entry degrade-reason tokens.
    pub ownership_signal_row_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub owner_conflict_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5OwnershipSignalAndConflictRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ReviewPackRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5OwnershipSignalRowResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5OwnershipSignalRowKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5OwnerConflictScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5OwnershipSignalRowSurfaceContext::ALL, |v| v.as_str()),
            ownership_signal_row_degrade_reasons: tokens(
                &M5OwnershipSignalRowEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            owner_conflict_degrade_reasons: tokens(&M5OwnerConflictEntryDegradeReason::ALL, |v| {
                v.as_str()
            }),
            anatomy_parts: tokens(&M5OwnershipSignalRowAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5OwnershipSignalRowNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5OwnershipSignalRowExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5OwnershipSignalAndConflictRegistriesGovernanceReview {
    /// The ownership_signal_row registry names a canonical token, semantic role, and line journey for every entry.
    pub ownership_signal_row_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-ownership_signal_row object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_ownership_signal_row_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved ownership_signal_row.
    ...
    pub owner_conflict_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-ownership_signal_row and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-ownership_signal_row and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub ownership_signal_row_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5OwnershipSignalAndConflictRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-ownership_signal_row registry.
    ...
    /// Behavior traces back to the canonical line-ownership_signal_row and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-ownership_signal_row / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5OwnershipSignalAndConflictRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5OwnershipSignalAndConflictRegistriesReleasePosture {
...
/// Constructor input for [`M5OwnershipSignalAndConflictRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5OwnershipSignalAndConflictRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5OwnershipSignalAndConflictRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5OwnershipSignalAndConflictRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5OwnershipSignalAndConflictRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5OwnershipSignalAndConflictRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5OwnershipSignalAndConflictRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5OwnershipSignalAndConflictRegistriesReleasePosture,
...
/// Export-safe M5 line-ownership_signal_row and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5OwnershipSignalAndConflictRegistriesPacket {
    /// Record kind; must equal [`M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_SCHEMA_VERSION`].
...
impl M5OwnershipSignalAndConflictRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5OwnershipSignalAndConflictRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5OwnershipSignalAndConflictRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_RECORD_KIND {
            violations.push(M5OwnershipSignalAndConflictRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_OWNERSHIP_SIGNAL_AND_CONFLICT_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5OwnershipSignalAndConflictRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5OwnershipSignalAndConflictRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5OwnershipSignalAndConflictRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-ownership_signal_row / line-downgrade-packet registries packet serializes",
        )) {
            violations.push(M5OwnershipSignalAndConflictRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,ownership_signal_row_entries,owner_conflict_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                ...
                row.ownership_signal_row_entries.len(),
                row.owner_conflict_entries.len(),
                degrades.join("|"),
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Ownership-Signal-Row and Owner-Conflict Registries\n\n");
        ...
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                "  - Ownership-signal-row entries: {} / owner-conflict entries: {}\n",
                row.ownership_signal_row_entries.len(),
                row.owner_conflict_entries.len()
            ));
        }
    ...
    /// registry-bound line-ownership_signal_row entries are listed.
    pub fn render_ownership_signal_row_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.ownership_signal_row_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5OwnershipSignalAndConflictRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5OwnershipSignalAndConflictRegistriesViolation>),
}

impl fmt::Display for M5OwnershipSignalAndConflictRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-ownership_signal_row / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-ownership_signal_row / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5OwnershipSignalAndConflictRegistriesArtifactError {}

/// Validation failures emitted by [`M5OwnershipSignalAndConflictRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5OwnershipSignalAndConflictRegistriesViolation {
    ...
    /// A registry row does not point at both the line-ownership_signal_row and line-downgrade-packet domain schemas.
    ...
    /// Cohort-ownership_signal_row-resolution is not proven: clean ownership_signal_row entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean ownership_signal_row entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded ownership_signal_row entry is present, or a clean ownership_signal_row entry is unbounded
    /// or unbound.
...
impl M5OwnershipSignalAndConflictRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "ownership_signal_row_resolution_not_proven"
            }
            Self::RollbackAndDiagnosticsPreservationNotProven => {
                "rollback_and_diagnostics_preservation_not_proven"
            }
            Self::CohortEvidenceIntegrityNotProven => "owner_conflict_integrity_not_proven",
...
pub fn current_stable_m5_ownership_signal_and_conflict_registries_export() -> Result<
    M5OwnershipSignalAndConflictRegistriesPacket,
    M5OwnershipSignalAndConflictRegistriesArtifactError,
> {
    let packet: M5OwnershipSignalAndConflictRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/review/m5-ownership-signal-and-conflict-registries-proof/support_export.json"
        )
    ))
    .map_err(M5OwnershipSignalAndConflictRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5OwnershipSignalAndConflictRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5OwnershipSignalAndConflictRegistriesPacket,
    violations: &mut Vec<M5OwnershipSignalAndConflictRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations
                .push(M5OwnershipSignalAndConflictRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5OwnershipSignalAndConflictRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(M5OwnershipSignalAndConflictRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5OwnershipSignalAndConflictRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations
                .push(M5OwnershipSignalAndConflictRegistriesViolation::MandatoryExportFieldMissing);
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_OWNERSHIP_SIGNAL_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_OWNER_CONFLICT_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5OwnershipSignalAndConflictRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.ownership_signal_row_entries.is_empty() || row.owner_conflict_entries.is_empty() {
            violations.push(M5OwnershipSignalAndConflictRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5OwnershipSignalAndConflictRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5OwnershipSignalAndConflictRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.ownership_signal_row_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_ownership_signal_row_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.owner_conflict_keeps_proof_visible_and_binds_support_language,
        ...
        review.ownership_signal_row_or_downgrade_drift_caught_before_release,
        review.every_row_declares_mandatory_anatomy,
        review.reuses_frozen_matrix_vocabulary,
    ] {
        if !ok {
            violations
                .push(M5OwnershipSignalAndConflictRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5OwnershipSignalAndConflictRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5OwnershipSignalAndConflictRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5OwnershipSignalAndConflictRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let ownership_signal_rows = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.ownership_signal_row_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.owner_conflict_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean ownership_signal_row entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean ownership_signal_row entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = ownership_signal_rows()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = ownership_signal_rows()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5OwnershipSignalRowKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5OwnershipSignalRowSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = ownership_signal_rows().any(|ex| {
        ex.degrade_reason
            == Some(M5OwnershipSignalRowEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete = !ownership_signal_rows()
        .any(|ex| ex.is_clean() && !ex.ownership_signal_row_object_complete);
    if !(journeys_covered
    ...
    // example degrades, an unbound example degrades, at least one clean bounded ownership_signal_row entry is present, and
    // no clean ownership_signal_row entry is unbounded or unbound.
    let widen_fold_degrades = ownership_signal_rows().any(|ex| {
        ex.degrade_reason
            == Some(
                M5OwnershipSignalRowEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = ownership_signal_rows().any(|ex| {
        ex.degrade_reason
            == Some(M5OwnershipSignalRowEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_ownership_signal_row =
        ownership_signal_rows().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound =
        !ownership_signal_rows().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded =
        !ownership_signal_rows().any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_ownership_signal_row
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5OwnerConflictScope::CANONICAL_SCOPES
        .iter()
    ...
    let form_incomplete_degrades = downgrade().any(|ex| {
        ex.degrade_reason == Some(M5OwnerConflictEntryDegradeReason::EvidenceFormCoverageIncomplete)
    });
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_owner_conflict);
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-ownership_signal_row registry
