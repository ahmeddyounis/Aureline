//! Implemented M5 ai-review-finding-record and scope-source-diff registries.
//!
...
//! be kept consistent: it turns the *ai-review-finding-record* grammar (one machine-readable descriptor per
...
//! its capture-context joins) and the *scope-source-diff* grammar (the diff scope a descriptor change sits in —
//! a producer-build change, a target-link change, or a retention-state change) into registry resolvers that produce
//! export-safe, honest projections. Every preserved object then resolves to one typed ai-review-finding-record
...
//! scope-source-diff object — the resolved object identity, the affected descriptor field, the
...
//!   [`resolve_ai_review_finding_record_entry`] refuses to read as a clean, registry-bound descriptor entry
//!   unless it names a canonical registry token, a classified [descriptor field][M5AiReviewFindingRecordKind],
//!   a historical-reference role, covers every [resolution form][M5AiReviewFindingRecordResolutionForm] (the
...
//!   it degrades to
//!   [`M5AiReviewFindingRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
...
//!   mutation.** [`resolve_scope_source_diff_entry`] names a classified [diff scope][M5ScopeSourceDiffScope]
...
//!   [`M5ScopeSourceDiffEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
...
//!   state.
//!
//! The resolvers reuse the frozen matrix vocabulary directly — the [`M5AiReviewAssistRole`] historical-reference
//! role vocabulary and the [`M5AiReviewAssistConsumerSurface`] consumer-surface taxonomy — so the shell / archive
...
pub use seed::{
    ...
    M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5AiReviewFindingAndScopeSourceRegistriesPacket`].
pub const M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_ai_review_finding_record_and_scope_source_packet_registries";

/// Schema version for M5 line-ai_review_finding_record / line-downgrade-packet registry records.
pub const M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_SCHEMA_REF: &str =
    "schemas/review/m5-ai-review-finding-and-scope-source-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_DOC_REF: &str =
    "docs/review/m5_ai_review_finding_and_scope_source_registries.md";
...
pub const M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/review/m5-ai-review-finding-and-scope-source-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_CSV_REF: &str =
    "artifacts/review/m5-ai-review-finding-and-scope-source-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_REPORT_REF: &str =
    "artifacts/review/m5-ai-review-finding-and-scope-source-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5AiReviewFindingAndScopeSourceRegistriesConsumerSurface = M5AiReviewAssistConsumerSurface;

/// One of the three resolution forms every line-ai_review_finding_record or line-downgrade-packet entry must hold across so
...
/// line-ai_review_finding_record and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiReviewFindingRecordResolutionForm {
    /// The canonical resolved line-ai_review_finding_record / line-downgrade-packet object.
    CanonicalObject,
...
impl M5AiReviewFindingRecordResolutionForm {
...
/// Controlled manifest field a ai-review-finding-record entry publishes for one retiring object-class identity, so every
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiReviewFindingRecordKind {
    ...
    FindingClassUnclassified,
}

impl M5AiReviewFindingRecordKind {
    /// Every public-proof-ledger kind, in declaration order.
    pub const ALL: [Self; 7] = [
        Self::CorrectnessBug,
        Self::SecurityRisk,
        Self::PerformanceConcern,
        Self::MaintainabilitySmell,
        Self::TestGap,
        Self::StyleNit,
        Self::FindingClassUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::CorrectnessBug => "correctness_bug",
            Self::SecurityRisk => "security_risk",
            Self::PerformanceConcern => "performance_concern",
            Self::MaintainabilitySmell => "maintainability_smell",
            Self::TestGap => "test_gap",
            Self::StyleNit => "style_nit",
            Self::FindingClassUnclassified => "finding_class_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::FindingClassUnclassified)
    }

    /// The canonical mode for this public-proof-ledger kind.
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::CorrectnessBug => "correctness_bug_mode",
            Self::SecurityRisk => "security_risk_mode",
            Self::PerformanceConcern => "performance_concern_mode",
            Self::MaintainabilitySmell => "maintainability_smell_mode",
            Self::TestGap => "test_gap_mode",
            Self::StyleNit => "style_nit_mode",
            Self::FindingClassUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(self, Self::MaintainabilitySmell | Self::TestGap)
    }
}

/// Controlled scope-source-diff scope a ai-review-finding-record change sits in, so a changed cutoff date or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ScopeSourceDiffScope {
    /// Producer-build-change: a descriptor changed the producer / build identity it was captured from.
    AnalyzedDiffScopeBinding,
    /// Target-link-change: a descriptor changed the current live-target reference it hands off to.
    RepoInstructionSourceBinding,
    /// Retention-state-change: a descriptor changed its retention / removal (expiry / cleanup) state.
    EvidenceAnchorBinding,
    /// The scope-source-diff scope is unclassified, which is disallowed.
    ScopeSourceUnclassified,
}

impl M5ScopeSourceDiffScope {
    /// Every comparison scope, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::AnalyzedDiffScopeBinding,
        Self::RepoInstructionSourceBinding,
        Self::EvidenceAnchorBinding,
        Self::ScopeSourceUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::AnalyzedDiffScopeBinding => "analyzed_diff_scope_binding",
            Self::RepoInstructionSourceBinding => "repo_instruction_source_binding",
            Self::EvidenceAnchorBinding => "evidence_anchor_binding",
            Self::ScopeSourceUnclassified => "scope_source_unclassified",
        }
    }

    /// Whether the comparison scope is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::ScopeSourceUnclassified)
    }
}

/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-ai_review_finding_record or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiReviewFindingRecordSurfaceContext {
...
impl M5AiReviewFindingRecordSurfaceContext {
...
/// One mandatory rendered part a line-ai_review_finding_record or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiReviewFindingRecordAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-ai_review_finding_record entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-ai_review_finding_record entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-ai_review_finding_record
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line ai_review_finding_record or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5AiReviewFindingRecordAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "scope_source_diff_fields",
...
/// line ai_review_finding_record, a line-downgrade packet, or a degraded line-ai_review_finding_record / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiReviewFindingRecordNextAction {
    /// Expand the resolved line ai_review_finding_record's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5AiReviewFindingRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiReviewFindingRecordExportField {
...
impl M5AiReviewFindingRecordExportField {
...
/// Reason a line-ai_review_finding_record entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiReviewFindingRecordEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the ai_review_finding_record means.
    DescriptorTokenUnstated,
    /// The render / surface context cannot currently be resolved.
    SurfaceContextUnresolved,
    /// The line journey is unclassified (not in the resolved taxonomy).
    CohortFindingClassUnclassified,
    ...
    /// The resolved line-ai_review_finding_record object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5AiReviewFindingRecordEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 9] = [
        Self::DescriptorTokenUnstated,
        Self::SurfaceContextUnresolved,
        Self::CohortFindingClassUnclassified,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "ai_review_finding_record_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortFindingClassUnclassified => "line_finding_class_unclassified",
            Self::DescriptorNotBoundToRegistry => "ai_review_finding_record_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "ai_review_finding_record_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "ai_review_finding_record_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5AiReviewFindingRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5AiReviewFindingRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortFindingClassUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5AiReviewFindingRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5AiReviewFindingRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5AiReviewFindingRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
    ...
    pub const fn downgrade_trigger(self) -> M5AiReviewAssistDowngradeTrigger {
        match self {
            ...
            Self::CohortFindingClassUnclassified | Self::CohortDescriptorObjectIncomplete => {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ScopeSourceDiffEntryDegradeReason {
...
impl M5ScopeSourceDiffEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::EvidenceScopeUnclassified => "comparison_scope_source_unclassified",
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                "scope_source_diff_runs_support_ahead_of_proof_or_drops_scope_source_diff"
            }
    ...
    pub const fn next_action(self) -> M5AiReviewFindingRecordNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5AiReviewFindingRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_ai_review_finding_record_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5AiReviewFindingRecordEntryResolutionInput {
    /// Stable identity of the line-ai_review_finding_record-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this ai_review_finding_record binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.ai_review_finding_record.test_gap`); empty means unstated.
    ...
    pub report_section: M5AiReviewFindingRecordKind,
    /// The render / surface context.
    pub surface_context: M5AiReviewFindingRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5AiReviewFindingRecordResolutionForm>,
    ...
    /// True when the behavior traces to the line-ai_review_finding_record registry (never a hand-copied constant).
...
/// Resolved, export-safe line-ai_review_finding_record-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedAiReviewFindingRecordEntry {
    /// Stable identity of the line-ai_review_finding_record-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this ai_review_finding_record binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-ai_review_finding_record object publishes every required field.
    pub ai_review_finding_record_object_complete: bool,
    /// Whether the entry traces to the line-ai_review_finding_record registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5AiReviewFindingRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5AiReviewFindingRecordNextAction,
    /// Whether the ai_review_finding_record resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub ai_review_finding_record_resolves_across_lines: bool,
}

impl M5ResolvedAiReviewFindingRecordEntry {
    /// Whether this line-ai_review_finding_record entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_scope_source_diff_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ScopeSourceDiffEntryResolutionInput {
    ...
    pub comparison_scope: M5ScopeSourceDiffScope,
    /// The render / surface context.
    pub surface_context: M5AiReviewFindingRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5AiReviewFindingRecordResolutionForm>,
    ...
    pub keeps_scope_source_diff_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedScopeSourceDiffEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub scope_source_diff_stays_honest: bool,
    ...
    pub provides_complete_scope_source_diff: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5ScopeSourceDiffEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5AiReviewFindingRecordNextAction,
...
impl M5ResolvedScopeSourceDiffEntry {
...
pub enum M5AiReviewFindingRecordResolutionError {
    /// The line-ai_review_finding_record-entry id was empty.
...
impl M5AiReviewFindingRecordResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_ai_review_finding_record_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_scope_source_diff_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5AiReviewFindingRecordResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-ai_review_finding_record / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5AiReviewFindingRecordResolutionError {}

fn form_tokens(forms: &[M5AiReviewFindingRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5AiReviewFindingRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5AiReviewFindingRecordResolutionForm> = forms.iter().copied().collect();
    M5AiReviewFindingRecordResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-ai_review_finding_record object publishes every required field: classified line journey,
...
pub fn ai_review_finding_record_object_is_complete(
    journey: M5AiReviewFindingRecordKind,
...
/// Whether the line ai_review_finding_record keeps a line from widening without preserving its rollback and diagnostics
...
pub fn scope_source_diff_stays_honest(
    scope: M5ScopeSourceDiffScope,
    comparison_is_truthful: bool,
    keeps_scope_source_diff_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_scope_source_diff_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-ai_review_finding_record-registry entry so it stays bound to the line-ai_review_finding_record registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete ai_review_finding_record object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_ai_review_finding_record_entry(
    input: M5AiReviewFindingRecordEntryResolutionInput,
) -> Result<M5ResolvedAiReviewFindingRecordEntry, M5AiReviewFindingRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5AiReviewFindingRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5AiReviewFindingRecordResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = ai_review_finding_record_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5AiReviewFindingRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5AiReviewFindingRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5AiReviewFindingRecordEntryDegradeReason::CohortFindingClassUnclassified)
    } else if !input.bound_to_registry {
        Some(M5AiReviewFindingRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5AiReviewFindingRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5AiReviewFindingRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5AiReviewFindingRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(
            M5AiReviewFindingRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    } else if !input.proof_fresh {
        Some(M5AiReviewFindingRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5AiReviewFindingRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedAiReviewFindingRecordEntry {
        ...
        ai_review_finding_record_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        ai_review_finding_record_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_scope_source_diff_entry(
    input: M5ScopeSourceDiffEntryResolutionInput,
) -> Result<M5ResolvedScopeSourceDiffEntry, M5AiReviewFindingRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5AiReviewFindingRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = scope_source_diff_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_scope_source_diff_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ScopeSourceDiffEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ScopeSourceDiffEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5ScopeSourceDiffEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5ScopeSourceDiffEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5ScopeSourceDiffEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5ScopeSourceDiffEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5AiReviewFindingRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedScopeSourceDiffEntry {
        entry_id: input.entry_id,
        ...
        keeps_scope_source_diff_visible: input.keeps_scope_source_diff_visible,
        ...
        scope_source_diff_stays_honest: record_stays_honest,
        provides_complete_scope_source_diff: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-ai_review_finding_record and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewFindingAndScopeSourceRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5AiReviewFindingAndScopeSourceRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5AiReviewFindingRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5AiReviewFindingRecordExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5AiReviewAssistDowngradeTrigger>,
    /// Resolved line-ai_review_finding_record-registry examples.
    pub ai_review_finding_record_entries: Vec<M5ResolvedAiReviewFindingRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub scope_source_diff_entries: Vec<M5ResolvedScopeSourceDiffEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-ai_review_finding_record and
    ...
    pub collapses_distinct_scope_source_diff_classes_into_one_lane: bool,
}

impl M5AiReviewFindingAndScopeSourceRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5AiReviewFindingRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5AiReviewFindingRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5AiReviewFindingRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5AiReviewFindingRecordExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_scope_source_diff_classes_into_one_lane
    }

    /// True when a clean line-ai_review_finding_record entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete ai_review_finding_record object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn ai_review_finding_record_is_honest(ex: &M5ResolvedAiReviewFindingRecordEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.ai_review_finding_record_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedScopeSourceDiffEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_scope_source_diff
                && ex.scope_source_diff_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.ai_review_finding_record_entries
            .iter()
            .all(Self::ai_review_finding_record_is_honest)
            && self
                .scope_source_diff_entries
                .iter()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewFindingAndScopeSourceRegistriesVocabularySet {
    ...
    /// Cohort-ai_review_finding_record-entry degrade-reason tokens.
    pub ai_review_finding_record_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub scope_source_diff_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5AiReviewFindingAndScopeSourceRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5AiReviewAssistRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5AiReviewFindingRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5AiReviewFindingRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ScopeSourceDiffScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5AiReviewFindingRecordSurfaceContext::ALL, |v| v.as_str()),
            ai_review_finding_record_degrade_reasons: tokens(
                &M5AiReviewFindingRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            scope_source_diff_degrade_reasons: tokens(
                &M5ScopeSourceDiffEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5AiReviewFindingRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5AiReviewFindingRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5AiReviewFindingRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewFindingAndScopeSourceRegistriesGovernanceReview {
    /// The ai_review_finding_record registry names a canonical token, semantic role, and line journey for every entry.
    pub ai_review_finding_record_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-ai_review_finding_record object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_ai_review_finding_record_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved ai_review_finding_record.
    ...
    pub scope_source_diff_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-ai_review_finding_record and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-ai_review_finding_record and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub ai_review_finding_record_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewFindingAndScopeSourceRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-ai_review_finding_record registry.
    ...
    /// Behavior traces back to the canonical line-ai_review_finding_record and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-ai_review_finding_record / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewFindingAndScopeSourceRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewFindingAndScopeSourceRegistriesReleasePosture {
...
/// Constructor input for [`M5AiReviewFindingAndScopeSourceRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5AiReviewFindingAndScopeSourceRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5AiReviewFindingAndScopeSourceRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5AiReviewFindingAndScopeSourceRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5AiReviewFindingAndScopeSourceRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5AiReviewFindingAndScopeSourceRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5AiReviewFindingAndScopeSourceRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5AiReviewFindingAndScopeSourceRegistriesReleasePosture,
...
/// Export-safe M5 line-ai_review_finding_record and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewFindingAndScopeSourceRegistriesPacket {
    /// Record kind; must equal [`M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_SCHEMA_VERSION`].
...
impl M5AiReviewFindingAndScopeSourceRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5AiReviewFindingAndScopeSourceRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5AiReviewFindingAndScopeSourceRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_RECORD_KIND {
            violations.push(M5AiReviewFindingAndScopeSourceRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5AiReviewFindingAndScopeSourceRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5AiReviewFindingAndScopeSourceRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5AiReviewFindingAndScopeSourceRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-ai_review_finding_record / line-downgrade-packet registries packet serializes",
        )) {
            violations
                .push(M5AiReviewFindingAndScopeSourceRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,ai_review_finding_record_entries,scope_source_diff_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            let degrades: Vec<&str> = row
                .ai_review_finding_record_entries
                .iter()
                .filter_map(|ex| ex.degrade_reason.map(|r| r.as_str()))
                .chain(
                    row.scope_source_diff_entries
                        .iter()
                        .filter_map(|ex| ex.degrade_reason.map(|r| r.as_str())),
                )
                .collect();
            out.push_str(&format!(
                ...
                row.ai_review_finding_record_entries.len(),
                row.scope_source_diff_entries.len(),
                degrades.join("|"),
    ...
    /// registry-bound line-ai_review_finding_record entries are listed.
    pub fn render_ai_review_finding_record_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.ai_review_finding_record_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5AiReviewFindingAndScopeSourceRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5AiReviewFindingAndScopeSourceRegistriesViolation>),
}

impl fmt::Display for M5AiReviewFindingAndScopeSourceRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-ai_review_finding_record / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-ai_review_finding_record / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5AiReviewFindingAndScopeSourceRegistriesArtifactError {}

/// Validation failures emitted by [`M5AiReviewFindingAndScopeSourceRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5AiReviewFindingAndScopeSourceRegistriesViolation {
    ...
    /// A registry row does not point at both the line-ai_review_finding_record and line-downgrade-packet domain schemas.
    ...
    /// Cohort-ai_review_finding_record-resolution is not proven: clean ai_review_finding_record entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean ai_review_finding_record entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded ai_review_finding_record entry is present, or a clean ai_review_finding_record entry is unbounded
    /// or unbound.
...
impl M5AiReviewFindingAndScopeSourceRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "ai_review_finding_record_resolution_not_proven"
            }
            Self::RollbackAndDiagnosticsPreservationNotProven => {
                "rollback_and_diagnostics_preservation_not_proven"
            }
            Self::CohortEvidenceIntegrityNotProven => "scope_source_diff_integrity_not_proven",
...
pub fn current_stable_m5_ai_review_finding_and_scope_source_registries_export() -> Result<
    M5AiReviewFindingAndScopeSourceRegistriesPacket,
    M5AiReviewFindingAndScopeSourceRegistriesArtifactError,
> {
    let packet: M5AiReviewFindingAndScopeSourceRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/review/m5-ai-review-finding-and-scope-source-registries-proof/support_export.json"
        )
    ))
    .map_err(M5AiReviewFindingAndScopeSourceRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5AiReviewFindingAndScopeSourceRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5AiReviewFindingAndScopeSourceRegistriesPacket,
    violations: &mut Vec<M5AiReviewFindingAndScopeSourceRegistriesViolation>,
) {
    let refs: BTreeSet<&str> = packet
        ...
        .collect();
    for required in [
        M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_SCHEMA_REF,
        M5_AI_REVIEW_FINDING_AND_SCOPE_SOURCE_REGISTRIES_DOC_REF,
        ...
        if !refs.contains(required) {
            violations
                .push(M5AiReviewFindingAndScopeSourceRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5AiReviewFindingAndScopeSourceRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations
                .push(M5AiReviewFindingAndScopeSourceRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5AiReviewFindingAndScopeSourceRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5AiReviewFindingAndScopeSourceRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_AI_REVIEW_FINDING_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_AI_REVIEW_SCOPE_SELECTOR_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5AiReviewFindingAndScopeSourceRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.ai_review_finding_record_entries.is_empty()
            || row.scope_source_diff_entries.is_empty()
        {
            violations.push(M5AiReviewFindingAndScopeSourceRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5AiReviewFindingAndScopeSourceRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5AiReviewFindingAndScopeSourceRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.ai_review_finding_record_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_ai_review_finding_record_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.scope_source_diff_keeps_proof_visible_and_binds_support_language,
        ...
        review.ai_review_finding_record_or_downgrade_drift_caught_before_release,
        review.every_row_declares_mandatory_anatomy,
        review.reuses_frozen_matrix_vocabulary,
    ] {
        if !ok {
            violations.push(
                M5AiReviewFindingAndScopeSourceRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5AiReviewFindingAndScopeSourceRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5AiReviewFindingAndScopeSourceRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5AiReviewFindingAndScopeSourceRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let ai_review_finding_records = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.ai_review_finding_record_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.scope_source_diff_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean ai_review_finding_record entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean ai_review_finding_record entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = ai_review_finding_records()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = ai_review_finding_records()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5AiReviewFindingRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5AiReviewFindingRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = ai_review_finding_records().any(|ex| {
        ex.degrade_reason
            == Some(M5AiReviewFindingRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete = !ai_review_finding_records()
        .any(|ex| ex.is_clean() && !ex.ai_review_finding_record_object_complete);
    if !(journeys_covered
    ...
    {
        violations.push(
            M5AiReviewFindingAndScopeSourceRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }

    // AC2: line packets preserve rollback and diagnostics posture before widening. A widen-without-rollback
    // example degrades, an unbound example degrades, at least one clean bounded ai_review_finding_record entry is present, and
    // no clean ai_review_finding_record entry is unbounded or unbound.
    let widen_fold_degrades = ai_review_finding_records().any(|ex| {
        ex.degrade_reason
            == Some(
                M5AiReviewFindingRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = ai_review_finding_records().any(|ex| {
        ex.degrade_reason
            == Some(M5AiReviewFindingRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_ai_review_finding_record =
        ai_review_finding_records().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound =
        !ai_review_finding_records().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded = !ai_review_finding_records()
        .any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_ai_review_finding_record
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
            M5AiReviewFindingAndScopeSourceRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ScopeSourceDiffScope::CANONICAL_SCOPES
        .iter()
    ...
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_scope_source_diff);
    ...
    {
        violations.push(
            M5AiReviewFindingAndScopeSourceRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-ai_review_finding_record registry
