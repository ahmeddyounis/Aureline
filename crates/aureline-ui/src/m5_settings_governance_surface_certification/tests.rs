//! Tests for the M05-1203 settings-governance surface certification capstone.

use super::*;

fn packet() -> SettingsGovernanceProfileCertificationPacket {
    seeded_m5_settings_governance_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, SETTINGS_GOVERNANCE_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, SETTINGS_GOVERNANCE_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5SettingsGovernanceCertifiedProfile::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            SettingsGovernanceCertificationAxis::ALL.len()
        );
    }
...
fn all_five_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5SettingsGovernanceClaim> =
        p.rows.iter().map(|r| r.certified_claim).collect();
    for claim in M5SettingsGovernanceClaim::ALL {
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5SettingsGovernanceCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
...
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = SettingsGovernanceCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), SettingsGovernanceCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in SettingsGovernanceCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == SettingsGovernanceCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    // The B143 localization axis is present, distinct, and exercised by the
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5SettingsGovernanceCertifiedProfile::DisclosedWriteIntentProfile
        })
        .expect("disclosed-write-intent-profile row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(SettingsGovernanceCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(SettingsGovernanceCertificationAxis::Localization)
    );
}

#[test]
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    // The B143 high-contrast reach axis stays certified across the seed (never a binding narrowed axis here).
    for row in &packet().rows {
        let outcome = row
            .axis(SettingsGovernanceCertificationAxis::HighContrast)
...
fn the_three_yellow_rows_bind_three_distinct_axes_and_triggers() {
    ...
    // per-family concern (write scope / sync overwrite / kill-switch cause).
    let p = packet();
    let axes: BTreeSet<SettingsGovernanceCertificationAxis> = p
        .rows
        .iter()
    ...
    let triggers: std::collections::HashSet<M5SettingsGovernanceDowngradeTrigger> = p
        .rows
        .iter()
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5SettingsGovernanceCertifiedProfile::LiveTrustedSettingsSurface)
        .expect("live-trusted-settings-surface row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == SettingsGovernanceCertificationAxis::Visual {
            outcome.state = SettingsGovernanceAxisCertificationState::UndisclosedDrift;
    ...
    assert_eq!(
        row.derived_status,
        SettingsGovernanceProfileClaimStatus::Red
    );
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        SettingsGovernanceCertificationViolation::ProfileBlocked { .. }
    )));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5SettingsGovernanceCertifiedProfile::ReviewableSettingsStructure
        })
        .expect("reviewable-settings-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails.recycles_a_retired_setting_id = true;
    assert_eq!(
        row.derive_status(),
        SettingsGovernanceProfileClaimStatus::Red
    );
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5SettingsGovernanceClaim::TrustedSettingsSurface;
    row.certified_claim = M5SettingsGovernanceClaim::TrustedSettingsSurface;
    assert_eq!(
        row.derive_status(),
        SettingsGovernanceProfileClaimStatus::Red
    );
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == SettingsGovernanceCertificationAxis::DegradedState {
            *outcome = SettingsGovernanceAxisOutcome {
                axis: SettingsGovernanceCertificationAxis::DegradedState,
                state: SettingsGovernanceAxisCertificationState::DisclosedNarrowed,
                parity_note: "the settings-governance evidence is lagging".to_owned(),
                narrowing_reason: Some(
                    "the settings-governance evidence is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5SettingsGovernanceDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableSettingsSurface == certified, no claim_auto_narrow.
    assert_eq!(
        row.derive_status(),
        SettingsGovernanceProfileClaimStatus::Red
    );
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == SettingsGovernanceCertificationAxis::CliExport {
            outcome.state = SettingsGovernanceAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5SettingsGovernanceDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(
        row.derive_status(),
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(SettingsGovernanceClaimAutoNarrow {
        binding_axis: SettingsGovernanceCertificationAxis::DegradedState,
        from_claim: M5SettingsGovernanceClaim::TrustedSettingsSurface,
        to_claim: M5SettingsGovernanceClaim::TrustedSettingsSurface,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5SettingsGovernanceClaim::WriteIntentDisclosedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-settings-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableSettingsSurface; certify a stronger trusted settings surface.
    row.certified_claim = M5SettingsGovernanceClaim::TrustedSettingsSurface;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = SettingsGovernanceCertificationAxis::Visual;
    }
    assert_eq!(
        row.derive_status(),
        SettingsGovernanceProfileClaimStatus::Red
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = SettingsGovernanceCertificationAxis::CliExport;
    }
    assert_eq!(
        row.derive_status(),
        SettingsGovernanceProfileClaimStatus::Red
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == SettingsGovernanceProfileClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "write intent".to_owned();
    }
    assert_eq!(
        row.derive_status(),
        SettingsGovernanceProfileClaimStatus::Red
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(SettingsGovernanceCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5SettingsGovernanceDowngradeTrigger::ProofStale,
    );
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows.retain(|r| {
        r.profile != M5SettingsGovernanceCertifiedProfile::UnverifiedCapabilityLifecycleProfile
    });
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        SettingsGovernanceCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the resolve-setting family from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5SettingsGovernanceFamily::ResolveSetting);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        SettingsGovernanceCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = SettingsGovernanceProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        SettingsGovernanceCertificationViolation::WrongCanonicalBundle
    )));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        SettingsGovernanceCertificationViolation::DuplicateId { .. }
    )));
}
...
fn axis_coverage_gap_is_rejected() {
    let mut p = packet();
    p.rows[0].axis_outcomes.retain(|o| {
        o.axis != SettingsGovernanceCertificationAxis::SettingsGovernanceComponentTruth
    });
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, SettingsGovernanceCertificationViolation::SummaryMismatch)));
}
...
fn export_json_round_trips() {
    ...
    let back: SettingsGovernanceProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_settings_governance_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/config/m5-settings-governance-surface-certification/support_export.json"
    ));
    let packet_from_fixture: SettingsGovernanceProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/config/m5-settings-governance-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_SETTINGS_GOVERNANCE_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures
...
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_SETTINGS_GOVERNANCE_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_settings_governance_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/release/m5-settings-governance-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-settings-governance-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/config/m5-settings-governance-surface-certification");
