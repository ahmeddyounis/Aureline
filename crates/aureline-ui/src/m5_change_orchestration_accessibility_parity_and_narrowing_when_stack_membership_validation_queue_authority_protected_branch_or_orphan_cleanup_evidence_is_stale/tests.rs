//! Tests for the M05-1302 change-orchestration accessibility parity capstone: the honest auto-narrowing
//! logic, the per-object parity contract, the weakened-never-landed guarantee, no-loss worktree-binding /
//! stack-membership / stack-order / landing-authority / validation-freshness / cleanup-evidence truth
//! integrity, and the checked-in support export / CSV / report.

use super::*;

fn row(id: &str) -> ChangeOrchestrationAccessibilityRow {
    seeded_m5_change_orchestration_accessibility_parity_packet()
        .rows
        .into_iter()
        .find(|r| r.row_id == id)
...
fn seeded_packet_validates_clean() {
    let packet = seeded_m5_change_orchestration_accessibility_parity_packet();
...
fn every_frozen_object_is_certified() {
    ...
    // Eight rows cover the six objects (two certified fully green — the selected-change-bound change object
    // and the evidence-bound worktree-cleanup preview — the other six auto-narrowed yellow across the six
    // narrowing axes: selected-change binding, stack membership, stack order, landing authority, validation
    // freshness, and cleanup evidence).
...
fn every_dimension_is_exercised() {
    ...
    assert_eq!(
        ...
        M5ChangeOrchestrationClaimDimension::ALL.len()
    );
}
...
fn summary_reports_all_truth_and_trusted_honesty() {
    ...
    // Rows 2, 5, and 8 key on structure-heavy objects (worktree-cleanup preview ×2, stack-edit review sheet
    // ×1).
...
fn fresh_record_is_trusted_and_green() {
    let record = row("a11y:change-object-selected-change-bound");
    assert_eq!(
        record.full_ready_claim,
        M5ChangeOrchestrationA11yClaim::TrustedProviderLandedSurface
    );
    assert_eq!(
...
fn stale_pack_record_narrows_and_is_never_trusted() {
    let record = row("a11y:change-object-worktree-binding-unbound");
    assert_eq!(
        record.effective_claim(),
...
fn each_weak_object_narrows_to_its_permitted_ceiling() {
    for (id, expected) in [
        (
            "a11y:change-object-worktree-binding-unbound",
            M5ChangeOrchestrationA11yClaim::SelectedChangeBindingUnverifiedProjection,
        ),
        (
            "a11y:patch-stack-queue-membership-inferred",
            M5ChangeOrchestrationA11yClaim::StackMembershipUnverifiedProjection,
        ),
        (
            "a11y:stack-edit-review-sheet-order-drifted",
            M5ChangeOrchestrationA11yClaim::StackOrderUnverifiedProjection,
        ),
        (
            "a11y:landing-candidate-sheet-queue-authority-unavailable",
            M5ChangeOrchestrationA11yClaim::LandingAuthorityUnverifiedProjection,
        ),
        (
            "a11y:portable-shelf-validation-stale",
            M5ChangeOrchestrationA11yClaim::ValidationFreshnessUnverifiedProjection,
        ),
        (
            "a11y:worktree-cleanup-preview-evidence-partial",
...
fn cannot_be_shown_trusted_flags_every_weak_state() {
    use M5ChangeOrchestrationConditionState as C;
    assert!(C::SelectedChangeBindingUnbound.cannot_be_shown_trusted());
    assert!(C::StackMembershipInferredOrUnverifiable.cannot_be_shown_trusted());
    assert!(C::StackOrderDriftedOrRestackRequired.cannot_be_shown_trusted());
    assert!(C::QueueOrProtectedBranchUnprovable.cannot_be_shown_trusted());
    assert!(C::ValidationOrApprovalStale.cannot_be_shown_trusted());
    assert!(C::CleanupEvidencePartial.cannot_be_shown_trusted());
    assert!(!C::FullyQualified.cannot_be_shown_trusted());
}

#[test]
fn bound_template_packet_is_reviewable_and_green() {
    let template = row("a11y:worktree-cleanup-preview-evidence-bound");
    assert!(template.is_structure_heavy());
    assert!(template.has_non_visual_fallback());
    assert_eq!(
        template.effective_claim(),
        M5ChangeOrchestrationA11yClaim::LocalReviewableSurface
    );
...
fn structure_heavy_evidence_row_binds_a_structured_and_non_visual_path() {
    let evidence = row("a11y:stack-edit-review-sheet-order-drifted");
...
fn local_parity_estimate_never_shown_as_provider_authoritative() {
    let parity = row("a11y:landing-candidate-sheet-queue-authority-unavailable");
...
fn over_asserting_a_weak_state_is_stranded() {
    let mut record = row("a11y:change-object-worktree-binding-unbound");
    ...
    assert_eq!(
        record.status(),
        ChangeOrchestrationAccessibilityStatus::Stranded
    );
}

#[test]
fn a_view_only_hover_trap_is_stranded() {
    let mut template = row("a11y:worktree-cleanup-preview-evidence-bound");
    template.keyboard_reach = ChangeOrchestrationNonVisualReachState::ViewOnlyTrap;
...
fn a_raw_payload_only_export_is_stranded() {
    let mut record = row("a11y:change-object-selected-change-bound");
    record.export_summary = ChangeOrchestrationExportSummaryState::RequiresRawPayload;
...
fn dropping_an_object_reports_missing_coverage() {
    ...
    let rows: Vec<ChangeOrchestrationAccessibilityRow> = packet
        .rows
        .into_iter()
        .filter(|r| r.object != M5ChangeOrchestrationObject::PatchStackQueue)
        .collect();
    let packet =
...
fn forbidden_material_in_export_is_rejected() {
    let mut packet = seeded_m5_change_orchestration_accessibility_parity_packet();
    packet.rows[0]
        .copy_export
        .export_fields
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_m5_change_orchestration_accessibility_parity_export()
        .expect("checked M5 review-pack a11y export validates");
    assert_eq!(from_disk.packet_id, CHANGE_ORCHESTRATION_A11Y_PACKET_ID);
    assert_eq!(
        from_disk,
        seeded_m5_change_orchestration_accessibility_parity_packet(),
...
fn checked_matrix_csv_matches_builder() {
    let expected = seeded_m5_change_orchestration_accessibility_parity_packet().render_matrix_csv();
...
fn checked_report_matches_builder() {
    let expected =
        seeded_m5_change_orchestration_accessibility_parity_packet().render_markdown_summary();
    let on_disk = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-change-orchestration-accessibility-parity/support_export.json"
    ));
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-change-orchestration-accessibility-parity/matrix.csv"
    ));
    assert_eq!(
        fixture_csv,
        seeded_m5_change_orchestration_accessibility_parity_packet().render_matrix_csv()
    );
}

// --- gated artifact regeneration ---
//
// Set `GEN_CHANGE_ORCHESTRATION_A11Y_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures from the
// seed builder. Off by default so a normal `cargo test` never mutates the tree.
#[test]
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_CHANGE_ORCHESTRATION_A11Y_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;
    ...
    let release_dir = repo.join("artifacts/release/m5-change-orchestration-accessibility-parity");
    ...
    fs::write(
        repo.join("artifacts/release/m5-change-orchestration-accessibility-parity.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/teamwork/m5-change-orchestration-accessibility-parity");
