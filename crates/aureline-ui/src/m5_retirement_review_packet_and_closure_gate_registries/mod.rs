//! Implemented M5 retirement-review-packet and support-note-closure-gate registries.
//!
//! The frozen [retired-state matrix][matrix] names Aureline's governed terminal-lifecycle object classes and locks
//! their controlled vocabulary. This module forces one typed retirement review packet before a line or stable-facing
//! surface can move to `Retired`, so retirement stops being an ad hoc decision buried in release notes and becomes a
//! completed, inspectable proof of readiness, historical closure, and user-facing honesty that support, help, docs,
//! marketplace, and procurement surfaces inherit rather than re-synthesizing from free-text release notes: it turns
//! the *retirement-review-packet* grammar (one export-safe review packet per retirement candidate — one classified
//! packet field per joined fact, typed as the exact-build snapshot ref, the final compatibility / public-proof join,
//! the unresolved dependent blocker, the support-note closure status, the migration outcome summary, or the archival
//! signoff ref, with an owning team and joined to the retirement manifest and impact report) and the
//! *support-note-closure-gate* grammar (the readiness-check scope a pre-closure blocker sits in — the review packet is
//! incomplete, an unclosed help / support / partner / procurement / incident surface still points at the candidate, or
//! an outstanding exception would be silently dropped) into registry resolvers that produce export-safe, honest
//! projections. Every retirement candidate then resolves to one typed retirement-review-packet object — the migration
//! outcome it records, its exact-build joins, its support-note closure status, and its archival signoff, all completed
//! before the object flips to Retired so a candidate never reads as ready while support language runs ahead of its
//! closed support note — and every candidate resolves to one typed support-note-closure-gate object — the resolved
//! candidate identity, the affected readiness check, the candidate-versus-successor reference, the gate-scope state,
//! and the active gate reason — that the install / update, marketplace / registry, docs / help, and support /
//! procurement surfaces can inspect without manual reconstruction, so a candidate with an incomplete packet or an
//! unclosed support surface keeps its blocker list visible instead of letting closure run past a silently dropped
//! exception, and a packet that cannot bind a fact to its exact build degrades honestly instead of leaving a retiring
//! surface to read as safe to close.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Join the retirement review packet from the manifest, impact report, compatibility / public-proof state,
//!   migration outcome, archival refs, exact-build snapshot refs, and unresolved dependent blockers.**
//!   [`resolve_retirement_review_packet_entry`] refuses to read as a clean, registry-bound review-packet entry
//!   unless it names a canonical registry token, a classified [packet field][M5RetirementReviewPacketKind], a
//!   retirement role, covers every [resolution form][M5RetirementReviewPacketResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every packet field (exact-build
//!   snapshot ref, final compatibility / public-proof join, unresolved dependent blocker, support-note closure status,
//!   migration outcome summary, and archival signoff ref bound to exact-build joins), completes its packet before the
//!   object flips to Retired, and keeps a public-facing support-note / migration field bound to the closed support
//!   note; otherwise it degrades.
//! * **Record mandatory support-note closure so help, support, partner, procurement, and incident references are
//!   marked updated, archived, redirected, or intentionally closed.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a review-packet entry whose
//!   closure is not completed before final retirement (a candidate flipping to Retired without one) so
//!   it degrades to
//!   [`M5RetirementReviewPacketEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a public-facing support-note / migration field whose language outruns the closed support note degrades the
//!   same way — the structured blocker reason an incomplete-closure attempt must surface.
//! * **Block final retirement unless the packet records who approved it, what evidence was accepted, which surfaces
//!   were closed or redirected, and what exceptions remain.**
//!   [`resolve_closure_gate_entry`] names a classified [gate scope][M5ClosureGateScope]
//!   (incomplete-retirement-review-packet, unclosed-support-note-surface, or silently-dropped-exception), requires the
...
//!   when the gate would keep support language ahead of the closed support note, hide the blocker, or let an
//!   outstanding exception masquerade as covered, so a candidate can never read as safe to close when it has quietly
//!   dropped the reason a retirement is not yet ready.
//!
...
pub use seed::{
    ...
    M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5RetirementReviewPacketClosureGateRegistriesPacket`].
pub const M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_retirement_review_packet_and_closure_gate_packet_registries";

/// Schema version for M5 line-retirement_review_packet / line-downgrade-packet registry records.
pub const M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_DOC_REF: &str =
    "docs/release/m5_retirement_review_packet_and_closure_gate_registries.md";

/// Repo-relative path of the canonical support-note-closure-gate domain schema minted by this lane (the
...
pub const M5_CLOSURE_GATE_DOMAIN_SCHEMA_REF: &str =
    "schemas/program/m5-support-note-closure-gate.schema.json";

/// Repo-relative path of the canonical retirement-review-packet domain schema minted by this lane (the
...
pub const M5_RETIREMENT_REVIEW_PACKET_DOMAIN_SCHEMA_REF: &str =
    "schemas/program/m5-retirement-review-packet.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-retirement-review-packet-and-closure-gate-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-retirement-review-packet-and-closure-gate-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-retirement-review-packet-and-closure-gate-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5RetirementReviewPacketClosureGateRegistriesConsumerSurface =
    M5RetiredStateConsumerSurface;

/// One of the three resolution forms every line-retirement_review_packet or line-downgrade-packet entry must hold across so
...
/// line-retirement_review_packet and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementReviewPacketResolutionForm {
    /// The canonical resolved line-retirement_review_packet / line-downgrade-packet object.
    CanonicalObject,
...
impl M5RetirementReviewPacketResolutionForm {
...
pub enum M5RetirementReviewPacketKind {
    ...
    ReviewPacketFieldUnclassified,
}

impl M5RetirementReviewPacketKind {
    /// Every public-proof-ledger kind, in declaration order.
    pub const ALL: [Self; 7] = [
        Self::ExactBuildSnapshotRef,
        Self::FinalCompatibilityPublicProofJoin,
        Self::UnresolvedDependentBlocker,
        Self::SupportNoteClosureStatus,
        Self::MigrationOutcomeSummary,
        Self::ArchivalSignoffRef,
        Self::ReviewPacketFieldUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::ExactBuildSnapshotRef => "exact_build_snapshot_ref",
            Self::FinalCompatibilityPublicProofJoin => "final_compatibility_public_proof_join",
            Self::UnresolvedDependentBlocker => "unresolved_dependent_blocker",
            Self::SupportNoteClosureStatus => "support_note_closure_status",
            Self::MigrationOutcomeSummary => "migration_outcome_summary",
            Self::ArchivalSignoffRef => "archival_signoff_ref",
            Self::ReviewPacketFieldUnclassified => "review_packet_field_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::ReviewPacketFieldUnclassified)
    }

    /// The canonical mode for this public-proof-ledger kind.
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::ExactBuildSnapshotRef => "exact_build_snapshot_ref_mode",
            Self::FinalCompatibilityPublicProofJoin => "final_compatibility_public_proof_join_mode",
            Self::UnresolvedDependentBlocker => "unresolved_dependent_blocker_mode",
            Self::SupportNoteClosureStatus => "support_note_closure_status_mode",
            Self::MigrationOutcomeSummary => "migration_outcome_summary_mode",
            Self::ArchivalSignoffRef => "archival_signoff_ref_mode",
            Self::ReviewPacketFieldUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(
            self,
            Self::SupportNoteClosureStatus | Self::MigrationOutcomeSummary
        )
    }
}
...
impl M5ClosureGateScope {
    ...
    pub const ALL: [Self; 4] = [
        Self::IncompleteRetirementReviewPacket,
        Self::UnclosedSupportNoteSurface,
        Self::SilentlyDroppedException,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::IncompleteRetirementReviewPacket => "incomplete_retirement_review_packet",
            Self::UnclosedSupportNoteSurface => "unclosed_support_note_surface",
            Self::SilentlyDroppedException => "silently_dropped_exception",
...
/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-retirement_review_packet or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementReviewPacketSurfaceContext {
...
impl M5RetirementReviewPacketSurfaceContext {
...
/// One mandatory rendered part a line-retirement_review_packet or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementReviewPacketAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-retirement_review_packet entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-retirement_review_packet entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-retirement_review_packet
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line retirement_review_packet or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5RetirementReviewPacketAnatomyPart {
...
/// line retirement_review_packet, a line-downgrade packet, or a degraded line-retirement_review_packet / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementReviewPacketNextAction {
    /// Expand the resolved line retirement_review_packet's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5RetirementReviewPacketNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementReviewPacketExportField {
...
impl M5RetirementReviewPacketExportField {
...
/// Reason a line-retirement_review_packet entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementReviewPacketEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the retirement_review_packet means.
    ...
    /// The resolved line-retirement_review_packet object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5RetirementReviewPacketEntryDegradeReason {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "retirement_review_packet_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortManifestFieldUnclassified => "line_review_packet_field_unclassified",
            Self::DescriptorNotBoundToRegistry => "retirement_review_packet_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "retirement_review_packet_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "retirement_review_packet_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5RetirementReviewPacketNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5RetirementReviewPacketNextAction::TraceCanonicalRegistry
            }
            Self::CohortManifestFieldUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5RetirementReviewPacketNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5RetirementReviewPacketNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5RetirementReviewPacketNextAction::ReviewBlockedOrDegraded,
        }
    }
    ...
    pub const fn next_action(self) -> M5RetirementReviewPacketNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5RetirementReviewPacketNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_retirement_review_packet_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5RetirementReviewPacketEntryResolutionInput {
    /// Stable identity of the line-retirement_review_packet-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this retirement_review_packet binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.retirement_review_packet.migration_outcome_summary`); empty means unstated.
    ...
    pub report_section: M5RetirementReviewPacketKind,
    /// The render / surface context.
    pub surface_context: M5RetirementReviewPacketSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5RetirementReviewPacketResolutionForm>,
    ...
    /// True when the behavior traces to the line-retirement_review_packet registry (never a hand-copied constant).
...
/// Resolved, export-safe line-retirement_review_packet-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedRetirementReviewPacketEntry {
    /// Stable identity of the line-retirement_review_packet-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this retirement_review_packet binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-retirement_review_packet object publishes every required field.
    pub retirement_review_packet_object_complete: bool,
    /// Whether the entry traces to the line-retirement_review_packet registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5RetirementReviewPacketEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5RetirementReviewPacketNextAction,
    /// Whether the retirement_review_packet resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub retirement_review_packet_resolves_across_lines: bool,
}

impl M5ResolvedRetirementReviewPacketEntry {
    /// Whether this line-retirement_review_packet entry reads as a clean, registry-bound state.
...
pub enum M5RetirementReviewPacketResolutionError {
    /// The line-retirement_review_packet-entry id was empty.
...
impl M5RetirementReviewPacketResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_retirement_review_packet_entry_id",
...
impl fmt::Display for M5RetirementReviewPacketResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-retirement_review_packet / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5RetirementReviewPacketResolutionError {}

fn form_tokens(forms: &[M5RetirementReviewPacketResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5RetirementReviewPacketResolutionForm]) -> bool {
    let present: BTreeSet<M5RetirementReviewPacketResolutionForm> = forms.iter().copied().collect();
    M5RetirementReviewPacketResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-retirement_review_packet object publishes every required field: classified line journey,
...
pub fn retirement_review_packet_object_is_complete(
    journey: M5RetirementReviewPacketKind,
...
/// Whether the line retirement_review_packet keeps a line from widening without preserving its rollback and diagnostics
...
/// Resolves a line-retirement_review_packet-registry entry so it stays bound to the line-retirement_review_packet registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete retirement_review_packet object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_retirement_review_packet_entry(
    input: M5RetirementReviewPacketEntryResolutionInput,
) -> Result<M5ResolvedRetirementReviewPacketEntry, M5RetirementReviewPacketResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5RetirementReviewPacketResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5RetirementReviewPacketResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = retirement_review_packet_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5RetirementReviewPacketEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5RetirementReviewPacketEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5RetirementReviewPacketEntryDegradeReason::CohortManifestFieldUnclassified)
    } else if !input.bound_to_registry {
        Some(M5RetirementReviewPacketEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5RetirementReviewPacketEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5RetirementReviewPacketEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5RetirementReviewPacketEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(
            M5RetirementReviewPacketEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    } else if !input.proof_fresh {
        Some(M5RetirementReviewPacketEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5RetirementReviewPacketNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedRetirementReviewPacketEntry {
        ...
        retirement_review_packet_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        retirement_review_packet_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
pub fn resolve_closure_gate_entry(
...
) -> Result<M5ResolvedClosureGateEntry, M5RetirementReviewPacketResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5RetirementReviewPacketResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5RetirementReviewPacketNextAction::TraceCanonicalRegistry,
    };
...
/// One registry row: one consumer surface bound to the resolved line-retirement_review_packet and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementReviewPacketClosureGateRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5RetirementReviewPacketClosureGateRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5RetirementReviewPacketAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5RetirementReviewPacketExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5RetiredStateDowngradeTrigger>,
    /// Resolved line-retirement_review_packet-registry examples.
    pub retirement_review_packet_entries: Vec<M5ResolvedRetirementReviewPacketEntry>,
    /// Resolved line-downgrade-packet examples.
    pub closure_gate_entries: Vec<M5ResolvedClosureGateEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-retirement_review_packet and
...
impl M5RetirementReviewPacketClosureGateRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5RetirementReviewPacketAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5RetirementReviewPacketAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5RetirementReviewPacketExportField> =
            self.export_fields.iter().copied().collect();
        M5RetirementReviewPacketExportField::MANDATORY
            .iter()
    ...
    /// True when a clean line-retirement_review_packet entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete retirement_review_packet object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn retirement_review_packet_is_honest(ex: &M5ResolvedRetirementReviewPacketEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.retirement_review_packet_object_complete
    ...
    fn examples_are_honest(&self) -> bool {
        self.retirement_review_packet_entries
            .iter()
            .all(Self::retirement_review_packet_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementReviewPacketClosureGateRegistriesVocabularySet {
    ...
    /// Cohort-retirement_review_packet-entry degrade-reason tokens.
    pub retirement_review_packet_degrade_reasons: Vec<String>,
...
impl M5RetirementReviewPacketClosureGateRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5RetiredStateRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5RetirementReviewPacketResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5RetirementReviewPacketKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ClosureGateScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5RetirementReviewPacketSurfaceContext::ALL, |v| v.as_str()),
            retirement_review_packet_degrade_reasons: tokens(
                &M5RetirementReviewPacketEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            closure_gate_degrade_reasons: tokens(&M5ClosureGateEntryDegradeReason::ALL, |v| {
                v.as_str()
            }),
            anatomy_parts: tokens(&M5RetirementReviewPacketAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5RetirementReviewPacketNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5RetirementReviewPacketExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementReviewPacketClosureGateRegistriesGovernanceReview {
    /// The retirement_review_packet registry names a canonical token, semantic role, and line journey for every entry.
    pub retirement_review_packet_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-retirement_review_packet object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_retirement_review_packet_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved retirement_review_packet.
    ...
    /// Every line-retirement_review_packet and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-retirement_review_packet and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub retirement_review_packet_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementReviewPacketClosureGateRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-retirement_review_packet registry.
    ...
    /// Behavior traces back to the canonical line-retirement_review_packet and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-retirement_review_packet / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementReviewPacketClosureGateRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementReviewPacketClosureGateRegistriesReleasePosture {
...
/// Constructor input for [`M5RetirementReviewPacketClosureGateRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5RetirementReviewPacketClosureGateRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5RetirementReviewPacketClosureGateRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5RetirementReviewPacketClosureGateRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5RetirementReviewPacketClosureGateRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5RetirementReviewPacketClosureGateRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5RetirementReviewPacketClosureGateRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5RetirementReviewPacketClosureGateRegistriesReleasePosture,
...
/// Export-safe M5 line-retirement_review_packet and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementReviewPacketClosureGateRegistriesPacket {
    /// Record kind; must equal [`M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_SCHEMA_VERSION`].
...
impl M5RetirementReviewPacketClosureGateRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5RetirementReviewPacketClosureGateRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5RetirementReviewPacketClosureGateRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_RECORD_KIND {
            violations
                .push(M5RetirementReviewPacketClosureGateRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_SCHEMA_VERSION
        {
            violations
                .push(M5RetirementReviewPacketClosureGateRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations
                .push(M5RetirementReviewPacketClosureGateRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations
                .push(M5RetirementReviewPacketClosureGateRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-retirement_review_packet / line-downgrade-packet registries packet serializes",
        )) {
            violations
                .push(M5RetirementReviewPacketClosureGateRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,retirement_review_packet_entries,closure_gate_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                ...
                row.retirement_review_packet_entries.len(),
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Retirement-Review-Packet and Support-Note-Closure-Gate Registries\n\n");
        ...
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                "  - Review-packet entries: {} / closure-gate entries: {}\n",
                row.retirement_review_packet_entries.len(),
                row.closure_gate_entries.len()
            ));
        }
    ...
    /// registry-bound line-retirement_review_packet entries are listed.
    pub fn render_retirement_review_packet_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.retirement_review_packet_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5RetirementReviewPacketClosureGateRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5RetirementReviewPacketClosureGateRegistriesViolation>),
}

impl fmt::Display for M5RetirementReviewPacketClosureGateRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-retirement_review_packet / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-retirement_review_packet / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5RetirementReviewPacketClosureGateRegistriesArtifactError {}

/// Validation failures emitted by [`M5RetirementReviewPacketClosureGateRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5RetirementReviewPacketClosureGateRegistriesViolation {
    ...
    /// A registry row does not point at both the line-retirement_review_packet and line-downgrade-packet domain schemas.
    ...
    /// Cohort-retirement_review_packet-resolution is not proven: clean retirement_review_packet entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean retirement_review_packet entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded retirement_review_packet entry is present, or a clean retirement_review_packet entry is unbounded
    /// or unbound.
...
impl M5RetirementReviewPacketClosureGateRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "retirement_review_packet_resolution_not_proven"
            }
...
pub fn current_stable_m5_retirement_review_packet_and_closure_gate_registries_export() -> Result<
    M5RetirementReviewPacketClosureGateRegistriesPacket,
    M5RetirementReviewPacketClosureGateRegistriesArtifactError,
> {
    let packet: M5RetirementReviewPacketClosureGateRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-retirement-review-packet-and-closure-gate-registries-proof/support_export.json"
        )
    ))
    .map_err(M5RetirementReviewPacketClosureGateRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5RetirementReviewPacketClosureGateRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5RetirementReviewPacketClosureGateRegistriesPacket,
    violations: &mut Vec<M5RetirementReviewPacketClosureGateRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5RetirementReviewPacketClosureGateRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5RetirementReviewPacketClosureGateRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5RetirementReviewPacketClosureGateRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5RetirementReviewPacketClosureGateRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5RetirementReviewPacketClosureGateRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_RETIREMENT_REVIEW_PACKET_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_CLOSURE_GATE_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5RetirementReviewPacketClosureGateRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.retirement_review_packet_entries.is_empty() || row.closure_gate_entries.is_empty() {
            violations
                .push(M5RetirementReviewPacketClosureGateRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations
                .push(M5RetirementReviewPacketClosureGateRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5RetirementReviewPacketClosureGateRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.retirement_review_packet_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_retirement_review_packet_from_shared_registry,
        ...
        review.retirement_review_packet_or_downgrade_drift_caught_before_release,
        review.every_row_declares_mandatory_anatomy,
        review.reuses_frozen_matrix_vocabulary,
    ] {
        if !ok {
            violations.push(
                M5RetirementReviewPacketClosureGateRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5RetirementReviewPacketClosureGateRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5RetirementReviewPacketClosureGateRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5RetirementReviewPacketClosureGateRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let retirement_review_packets = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.retirement_review_packet_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
    ...
    // Clean retirement_review_packet entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean retirement_review_packet entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = retirement_review_packets()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = retirement_review_packets()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5RetirementReviewPacketKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5RetirementReviewPacketSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = retirement_review_packets().any(|ex| {
        ex.degrade_reason
            == Some(M5RetirementReviewPacketEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete = !retirement_review_packets()
        .any(|ex| ex.is_clean() && !ex.retirement_review_packet_object_complete);
    if !(journeys_covered
    ...
    {
        violations.push(
            M5RetirementReviewPacketClosureGateRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }

    // AC2: line packets preserve rollback and diagnostics posture before widening. A widen-without-rollback
    // example degrades, an unbound example degrades, at least one clean bounded retirement_review_packet entry is present, and
    // no clean retirement_review_packet entry is unbounded or unbound.
    let widen_fold_degrades = retirement_review_packets().any(|ex| {
        ex.degrade_reason
            == Some(
                M5RetirementReviewPacketEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = retirement_review_packets().any(|ex| {
        ex.degrade_reason
            == Some(M5RetirementReviewPacketEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_retirement_review_packet =
        retirement_review_packets().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound =
        !retirement_review_packets().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded = !retirement_review_packets()
        .any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_retirement_review_packet
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
            M5RetirementReviewPacketClosureGateRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    {
        violations.push(
            M5RetirementReviewPacketClosureGateRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-retirement_review_packet registry
