...
fn clean_retirement_review_packet_input() -> M5RetirementReviewPacketEntryResolutionInput {
    M5RetirementReviewPacketEntryResolutionInput {
        entry_id: "retirement_review_packet:test".to_owned(),
        line_binding_id: "launch.line.core-team-canary".to_owned(),
        token_name: "line.retirement_review_packet.core_team_canary".to_owned(),
        semantic_role: M5RetiredStateRole::LastSupportedPin,
        report_section: M5RetirementReviewPacketKind::ExactBuildSnapshotRef,
        surface_context: M5RetirementReviewPacketSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5RetirementReviewPacketResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5ClosureGateEntryResolutionInput {
    M5ClosureGateEntryResolutionInput {
        ...
        semantic_role: M5RetiredStateRole::LastSupportedPin,
        comparison_scope: M5ClosureGateScope::IncompleteRetirementReviewPacket,
        surface_context: M5RetirementReviewPacketSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5RetirementReviewPacketResolutionForm::ALL.to_vec(),
...
#[test]
fn retirement_review_packet_clean_names_meaning_and_is_bound() {
    let resolved =
        resolve_retirement_review_packet_entry(clean_retirement_review_packet_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.retirement_review_packet_resolves_across_lines);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.retirement_review_packet_object_complete);
    ...
    assert_eq!(resolved.report_section, "exact_build_snapshot_ref");
    assert_eq!(
        resolved.canonical_report_section_mode,
        "exact_build_snapshot_ref_mode"
    );
    assert_eq!(resolved.surface_context, "shiproom_surface");
    assert_eq!(
        resolved.next_action,
        M5RetirementReviewPacketNextAction::ExpandCohortMeaning
    );
}

#[test]
fn retirement_review_packet_token_unstated_degrades() {
    let mut input = clean_retirement_review_packet_input();
...
#[test]
fn retirement_review_packet_unbound_and_unclassified_degrade() {
    let mut input = clean_retirement_review_packet_input();
    ...
    let mut input = clean_retirement_review_packet_input();
    input.report_section = M5RetirementReviewPacketKind::ReviewPacketFieldUnclassified;
    assert_eq!(
        resolve_retirement_review_packet_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn retirement_review_packet_object_incomplete_and_widen_fold_and_form_degrade() {
    // An unstated bundle IDs field leaves the resolved object incomplete.
    let mut input = clean_retirement_review_packet_input();
    input.bundle_ids = "  ".to_owned();
    let resolved = resolve_retirement_review_packet_entry(input).unwrap();
    assert!(!resolved.retirement_review_packet_object_complete);
    assert_eq!(
    ...
    assert_eq!(
        resolve_retirement_review_packet_entry(input).unwrap().degrade_reason,
        Some(M5RetirementReviewPacketEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    );

    let mut input = clean_retirement_review_packet_input();
    input.resolution_form_coverage = vec![M5RetirementReviewPacketResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_retirement_review_packet_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn retirement_review_packet_public_facing_and_surface_and_proof_degrade() {
    // A public-facing line running support language ahead of proof first fails the widen-boundary fold.
    let mut input = clean_retirement_review_packet_input();
    input.report_section = M5RetirementReviewPacketKind::MigrationOutcomeSummary;
    ...
    let mut input = clean_retirement_review_packet_input();
    input.surface_context = M5RetirementReviewPacketSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_retirement_review_packet_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn retirement_review_packet_empty_id_and_forbidden_material_error() {
    let mut input = clean_retirement_review_packet_input();
    input.entry_id = "".to_owned();
    assert_eq!(
        resolve_retirement_review_packet_entry(input).unwrap_err(),
        M5RetirementReviewPacketResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_retirement_review_packet_entry(input).unwrap_err(),
        M5RetirementReviewPacketResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5RetirementReviewPacketKind::ReviewPacketFieldUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn retirement_review_packet_object_is_complete_requires_all_fields() {
    assert!(retirement_review_packet_object_is_complete(
        M5RetirementReviewPacketKind::ExactBuildSnapshotRef,
...
fn downgrade_clean_stays_honest() {
    ...
    assert_eq!(
        ...
        "incomplete_retirement_review_packet"
    );
    assert_eq!(resolved.surface_context, "shiproom_surface");
}
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    assert_eq!(
        ...
        M5RetirementReviewPacketResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RetirementReviewPacketClosureGateRegistriesViolation::VocabularySetDrift));
}
...
fn missing_source_contracts_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RetirementReviewPacketClosureGateRegistriesViolation::MissingSourceContracts));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_RETIREMENT_REVIEW_PACKET_DOMAIN_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5RetirementReviewPacketClosureGateRegistriesViolation::DomainSchemaRefMissing));
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5RetirementReviewPacketAnatomyPart::Identity);
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5RetirementReviewPacketExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RetirementReviewPacketClosureGateRegistriesViolation::ExamplesMissing));
}

#[test]
fn dishonest_clean_example_fails() {
    let mut packet = seeded_m5_retirement_review_packet_and_closure_gate_registries();
    // Force a clean retirement_review_packet entry to also read as object-incomplete — the packet must reject it.
    let row = &mut packet.registry_rows[0];
    row.retirement_review_packet_entries[0].degrade_reason = None;
    row.retirement_review_packet_entries[0].retirement_review_packet_object_complete = false;
    assert!(packet
        .validate()
        .contains(&M5RetirementReviewPacketClosureGateRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        assert!(packet.validate().contains(
            &M5RetirementReviewPacketClosureGateRegistriesViolation::RowInvariantViolated
        ));
    }
}

#[test]
fn retirement_review_packet_not_proven_when_incomplete_example_removed() {
    let mut packet = seeded_m5_retirement_review_packet_and_closure_gate_registries();
    for row in &mut packet.registry_rows {
        row.retirement_review_packet_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
                    M5RetirementReviewPacketEntryDegradeReason::CohortDescriptorObjectIncomplete,
                )
        });
    }
    assert!(packet.validate().contains(
...
#[test]
fn retirement_review_packet_not_proven_when_surface_collapses() {
    let mut packet = seeded_m5_retirement_review_packet_and_closure_gate_registries();
    // Drop every clean executive-steering-surface retirement_review_packet so the first-consumer surfaces no longer include it.
    for row in &mut packet.registry_rows {
        row.retirement_review_packet_entries
...
fn closure_gate_integrity_not_proven_when_scope_dropped() {
    ...
    for row in &mut packet.registry_rows {
        row.closure_gate_entries
            .retain(|ex| !(ex.is_clean() && ex.comparison_scope == "silently_dropped_exception"));
    }
    assert!(packet.validate().contains(
        &M5RetirementReviewPacketClosureGateRegistriesViolation::CohortEvidenceIntegrityNotProven
    ));
...
fn proof_freshness_incomplete_fails() {
    ...
    assert!(packet.validate().contains(
        &M5RetirementReviewPacketClosureGateRegistriesViolation::ProofFreshnessIncomplete
    ));
}
...
fn release_posture_incomplete_fails() {
    ...
    assert!(packet.validate().contains(
        &M5RetirementReviewPacketClosureGateRegistriesViolation::ReleasePostureIncomplete
    ));
}
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RetirementReviewPacketClosureGateRegistriesViolation::RawMaterialInExport));
}

#[test]
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_retirement_review_packet_and_closure_gate_registries().export_safe_json();
...
#[test]
fn retirement_review_packet_table_lists_only_clean_retirement_review_packets() {
    let packet = seeded_m5_retirement_review_packet_and_closure_gate_registries();
    let table = packet.render_retirement_review_packet_table();
    // The clean canary and migration retirement_review_packets are rendered from the registry.
    assert!(table.contains("exact_build_snapshot_ref_mode"));
    assert!(table.contains("final_compatibility_public_proof_join_mode"));
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_retirement_review_packet_and_closure_gate_registries_export()
        .expect(
        "checked M5 line-retirement_review_packet / line-downgrade-packet registries export validates",
    );
    assert_eq!(
        from_disk.packet_id,
        M5_RETIREMENT_REVIEW_PACKET_CLOSURE_GATE_REGISTRIES_PACKET_ID
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5RetirementReviewPacketClosureGateRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-retirement-review-packet-and-closure-gate-registries/retirement_review_packet_beta_narrowed.json"
    )))
    .expect("line-retirement_review_packet fixture parses");
    ...
    let preview: M5RetirementReviewPacketClosureGateRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-retirement-review-packet-and-closure-gate-registries/closure_gate_preview_narrowed.json"
    )))
