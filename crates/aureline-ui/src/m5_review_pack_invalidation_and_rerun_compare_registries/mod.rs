//! Implemented M5 review-pack-invalidation-record and review-pack-rerun-compare registries.
//!
//! The frozen [review-pack evaluator matrix][matrix] names Aureline's governed review-pack object classes and
//! locks their controlled vocabulary. This module operates the invalidation / rerun / compare lane over that
//! matrix so local review, hosted review, CI, and AI review never let stale evidence overclaim freshness: it
//! turns the *review-pack-invalidation-record* grammar (one machine-readable record per stale transition —
//! naming the exact invalidation cause: base-revision drift, head-revision drift, worktree-scope drift,
//! review-pack version drift, review-pack content-digest drift, or environment-capsule drift — bound to one
//! pack identity with its evaluator lineage, base / head, worktree scope, and pack version / digest) and the
//! *review-pack-rerun-compare* grammar (the rerun-review and compare action a result binds — the previous
//! evaluator packet, the current base/head-or-pack-revision packet, or the preserved draft / local-evidence
//! binding) into registry resolvers that produce export-safe, honest projections. Every material drift then
//! resolves to one typed review-pack-invalidation-record object — the invalidation cause it names, its evaluator
//! lineage, its pack version / digest, and the base / head / worktree scope it invalidates, all attributable to
//! the pack so a stale local parity estimate never reads as fresh provider-authoritative mergeability — and to
//! one review-pack-rerun-compare object — the resolved pack identity, the affected evaluator field, the
//! previous-versus-current packet reference, the binding state, and the active divergence reason (partial-scope,
//! slice-omitted, stale-pack, ci-only, or provider-unavailable) — that the review, AI-review, support, and
//! provider-handoff surfaces can inspect without manual reconstruction, so a changed base/head, worktree scope,
//! pack version, or environment capsule stays visible as a typed stale transition with a named reason instead of
//! a silent mutation, and a record that cannot name its invalidation cause degrades honestly instead of leaving a
//! stale pack to read as a fresh, authoritative review result.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Invalidate review-pack results when base/head identity, worktree scope, review-pack version, or the
//!   environment capsule changes materially, and expose the exact reason for the stale transition.**
//!   [`resolve_review_pack_invalidation_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [invalidation cause][M5ReviewPackInvalidationRecordKind],
//!   a disclosure role, covers every [resolution form][M5ReviewPackInvalidationRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (pack version
//!   and content digest, the drifted base / head / worktree scope, the target diff identity, and the evaluator
//!   outcome bound to pack-identity joins), keeps its evaluator lineage attributable to the pack, and keeps a
//!   provider-facing field bound to its parity state; otherwise it degrades.
//! * **Implement rerun review and compare actions that let users inspect what changed between the previous
//!   evaluator packet and the current base/head or pack revision, preserving draft-only notes and local evidence
//!   while marking them stale rather than discarding them.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a record entry whose
//!   pack-version / evaluator-lineage join is not preserved (a stale estimate presenting itself as fresh provider
//!   authoritative without one) so it degrades to
//!   [`M5ReviewPackInvalidationRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a provider-facing parity field whose language outruns its evaluator outcome degrades the same
//!   way — the structured blocker reason a fresh-over-stale attempt must surface.
//! * **Never keep queue eligibility, approval validity, or AI policy compliance green after a material
//!   pack / base / environment drift.** [`resolve_review_pack_rerun_compare_entry`] names a classified
//!   [rerun / compare binding][M5ReviewPackRerunCompareScope]
//!   (previous evaluator packet, current base/head-or-pack-revision packet, or preserved draft / local-evidence
//!   binding), requires the full pack-identity / affected-evaluator-field / previous-versus-current-packet /
//!   binding / active-reason compare object, covers every resolution form, and degrades to
//!   [`M5ReviewPackRerunCompareEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the compare would present a stale pack as current, hide the divergence label, drop the preserved draft
//!   or local evidence, or let a scope gap masquerade as full coverage, so a review result can never read as
//!   trustworthy when it has quietly dropped the reason the evaluator could not claim full coverage.
//!
...
pub use seed::{
    seeded_m5_review_pack_invalidation_and_rerun_compare_registries,
    seeded_m5_review_pack_invalidation_and_rerun_compare_registries_review_pack_invalidation_record_beta_narrowed,
    seeded_m5_review_pack_invalidation_and_rerun_compare_registries_review_pack_rerun_compare_preview_narrowed,
    M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_PACKET_ID,
};
...
use crate::m5_review_pack_evaluator_matrix::{
    ...
    M5ReviewPackRequiredLabel, M5ReviewPackRole, M5_REVIEW_PACK_DOMAIN_SCHEMA_REF,
    M5_REVIEW_PACK_MATRIX_DOC_REF, M5_REVIEW_PACK_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5ReviewPackInvalidationAndRerunCompareRegistriesPacket`].
pub const M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_review_pack_invalidation_and_rerun_compare_packet_registries";

/// Schema version for M5 line-review_pack_invalidation_record / line-downgrade-packet registry records.
pub const M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_DOC_REF: &str =
    "docs/review/m5_review_pack_invalidation_and_rerun_compare_registries.md";

/// Repo-relative path of the frozen matrix review-pack-result domain schema reused by this lane for the
/// rerun / compare object (the typed evaluator packet naming which previous-versus-current binding a rerun
/// result sits in and the active divergence reason it carries).
pub const M5_REVIEW_PACK_RESULT_DOMAIN_SCHEMA_REF: &str =
    "schemas/review/m5-review-pack-result.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/review/m5-review-pack-invalidation-and-rerun-compare-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_CSV_REF: &str =
    "artifacts/review/m5-review-pack-invalidation-and-rerun-compare-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_REPORT_REF: &str =
    "artifacts/review/m5-review-pack-invalidation-and-rerun-compare-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/review/m5-review-pack-invalidation-and-rerun-compare-registries";
...
pub type M5ReviewPackInvalidationAndRerunCompareRegistriesConsumerSurface =
    M5ReviewPackConsumerSurface;

/// One of the three resolution forms every line-review_pack_invalidation_record or line-downgrade-packet entry must hold across so
...
/// line-review_pack_invalidation_record and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackInvalidationRecordResolutionForm {
    /// The canonical resolved line-review_pack_invalidation_record / line-downgrade-packet object.
    CanonicalObject,
...
impl M5ReviewPackInvalidationRecordResolutionForm {
...
/// Controlled manifest field a review-pack-invalidation-record entry publishes for one retiring object-class identity, so every
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackInvalidationRecordKind {
    ...
    EnvironmentCapsuleDrift,
    /// The manifest field is unclassified, which is disallowed.
    InvalidationCauseUnclassified,
}

impl M5ReviewPackInvalidationRecordKind {
    /// Every public-proof-ledger kind, in declaration order.
    pub const ALL: [Self; 7] = [
        Self::BaseRevisionDrift,
        Self::HeadRevisionDrift,
        Self::WorktreeScopeDrift,
        Self::PackVersionDrift,
        Self::PackDigestDrift,
        Self::EnvironmentCapsuleDrift,
        Self::InvalidationCauseUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::BaseRevisionDrift => "base_revision_drift",
            Self::HeadRevisionDrift => "head_revision_drift",
            Self::WorktreeScopeDrift => "worktree_scope_drift",
            Self::PackVersionDrift => "pack_version_drift",
            Self::PackDigestDrift => "pack_digest_drift",
            Self::EnvironmentCapsuleDrift => "environment_capsule_drift",
            Self::InvalidationCauseUnclassified => "invalidation_cause_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::InvalidationCauseUnclassified)
    }

    /// The canonical mode for this public-proof-ledger kind.
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::BaseRevisionDrift => "base_revision_drift_mode",
            Self::HeadRevisionDrift => "head_revision_drift_mode",
            Self::WorktreeScopeDrift => "worktree_scope_drift_mode",
            Self::PackVersionDrift => "pack_version_drift_mode",
            Self::PackDigestDrift => "pack_digest_drift_mode",
            Self::EnvironmentCapsuleDrift => "environment_capsule_drift_mode",
            Self::InvalidationCauseUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(self, Self::PackVersionDrift | Self::PackDigestDrift)
    }
}

/// Controlled review-pack-rerun-compare scope a review-pack-invalidation-record change sits in, so a changed cutoff date or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackRerunCompareScope {
    /// Producer-build-change: a descriptor changed the producer / build identity it was captured from.
    PreviousPacketBinding,
    /// Target-link-change: a descriptor changed the current live-target reference it hands off to.
    CurrentPacketBinding,
    /// Retention-state-change: a descriptor changed its retention / removal (expiry / cleanup) state.
    PreservedDraftEvidenceBinding,
    /// The review-pack-rerun-compare scope is unclassified, which is disallowed.
    ReviewPackRerunCompareUnclassified,
}

impl M5ReviewPackRerunCompareScope {
    /// Every comparison scope, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::PreviousPacketBinding,
        Self::CurrentPacketBinding,
        Self::PreservedDraftEvidenceBinding,
        Self::ReviewPackRerunCompareUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::PreviousPacketBinding => "previous_packet_binding",
            Self::CurrentPacketBinding => "current_packet_binding",
            Self::PreservedDraftEvidenceBinding => "preserved_draft_evidence_binding",
            Self::ReviewPackRerunCompareUnclassified => "review_pack_rerun_compare_unclassified",
        }
    }

    /// Whether the comparison scope is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::ReviewPackRerunCompareUnclassified)
    }
}

/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-review_pack_invalidation_record or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackInvalidationRecordSurfaceContext {
...
impl M5ReviewPackInvalidationRecordSurfaceContext {
...
/// One mandatory rendered part a line-review_pack_invalidation_record or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackInvalidationRecordAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-review_pack_invalidation_record entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-review_pack_invalidation_record entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-review_pack_invalidation_record
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line review_pack_invalidation_record or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5ReviewPackInvalidationRecordAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "review_pack_rerun_compare_fields",
...
/// line review_pack_invalidation_record, a line-downgrade packet, or a degraded line-review_pack_invalidation_record / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackInvalidationRecordNextAction {
    /// Expand the resolved line review_pack_invalidation_record's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5ReviewPackInvalidationRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackInvalidationRecordExportField {
...
impl M5ReviewPackInvalidationRecordExportField {
...
/// Reason a line-review_pack_invalidation_record entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackInvalidationRecordEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the review_pack_invalidation_record means.
    DescriptorTokenUnstated,
    /// The render / surface context cannot currently be resolved.
    SurfaceContextUnresolved,
    /// The line journey is unclassified (not in the resolved taxonomy).
    CohortInvalidationCauseUnclassified,
    ...
    /// The resolved line-review_pack_invalidation_record object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5ReviewPackInvalidationRecordEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 9] = [
        Self::DescriptorTokenUnstated,
        Self::SurfaceContextUnresolved,
        Self::CohortInvalidationCauseUnclassified,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "review_pack_invalidation_record_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortInvalidationCauseUnclassified => "line_invalidation_cause_unclassified",
            Self::DescriptorNotBoundToRegistry => "review_pack_invalidation_record_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "review_pack_invalidation_record_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "review_pack_invalidation_record_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5ReviewPackInvalidationRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5ReviewPackInvalidationRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortInvalidationCauseUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5ReviewPackInvalidationRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5ReviewPackInvalidationRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5ReviewPackInvalidationRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
    ...
    pub const fn downgrade_trigger(self) -> M5ReviewPackDowngradeTrigger {
        match self {
            ...
            Self::CohortInvalidationCauseUnclassified | Self::CohortDescriptorObjectIncomplete => {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReviewPackRerunCompareEntryDegradeReason {
...
impl M5ReviewPackRerunCompareEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::EvidenceScopeUnclassified => "comparison_review_pack_rerun_compare_unclassified",
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                "review_pack_rerun_compare_runs_support_ahead_of_proof_or_drops_review_pack_rerun_compare"
            }
    ...
    pub const fn next_action(self) -> M5ReviewPackInvalidationRecordNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5ReviewPackInvalidationRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_review_pack_invalidation_record_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ReviewPackInvalidationRecordEntryResolutionInput {
    /// Stable identity of the line-review_pack_invalidation_record-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this review_pack_invalidation_record binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.review_pack_invalidation_record.pack_digest_drift`); empty means unstated.
    ...
    pub report_section: M5ReviewPackInvalidationRecordKind,
    /// The render / surface context.
    pub surface_context: M5ReviewPackInvalidationRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ReviewPackInvalidationRecordResolutionForm>,
    ...
    /// True when the behavior traces to the line-review_pack_invalidation_record registry (never a hand-copied constant).
...
/// Resolved, export-safe line-review_pack_invalidation_record-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedReviewPackInvalidationRecordEntry {
    /// Stable identity of the line-review_pack_invalidation_record-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this review_pack_invalidation_record binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-review_pack_invalidation_record object publishes every required field.
    pub review_pack_invalidation_record_object_complete: bool,
    /// Whether the entry traces to the line-review_pack_invalidation_record registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5ReviewPackInvalidationRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ReviewPackInvalidationRecordNextAction,
    /// Whether the review_pack_invalidation_record resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub review_pack_invalidation_record_resolves_across_lines: bool,
}

impl M5ResolvedReviewPackInvalidationRecordEntry {
    /// Whether this line-review_pack_invalidation_record entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_review_pack_rerun_compare_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ReviewPackRerunCompareEntryResolutionInput {
    ...
    pub comparison_scope: M5ReviewPackRerunCompareScope,
    /// The render / surface context.
    pub surface_context: M5ReviewPackInvalidationRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ReviewPackInvalidationRecordResolutionForm>,
    ...
    pub keeps_review_pack_rerun_compare_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedReviewPackRerunCompareEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub review_pack_rerun_compare_stays_honest: bool,
    ...
    pub provides_complete_review_pack_rerun_compare: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5ReviewPackRerunCompareEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ReviewPackInvalidationRecordNextAction,
...
impl M5ResolvedReviewPackRerunCompareEntry {
...
pub enum M5ReviewPackInvalidationRecordResolutionError {
    /// The line-review_pack_invalidation_record-entry id was empty.
...
impl M5ReviewPackInvalidationRecordResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_review_pack_invalidation_record_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_review_pack_rerun_compare_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5ReviewPackInvalidationRecordResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-review_pack_invalidation_record / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5ReviewPackInvalidationRecordResolutionError {}

fn form_tokens(forms: &[M5ReviewPackInvalidationRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5ReviewPackInvalidationRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5ReviewPackInvalidationRecordResolutionForm> =
        forms.iter().copied().collect();
    M5ReviewPackInvalidationRecordResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-review_pack_invalidation_record object publishes every required field: classified line journey,
...
pub fn review_pack_invalidation_record_object_is_complete(
    journey: M5ReviewPackInvalidationRecordKind,
...
/// Whether the line review_pack_invalidation_record keeps a line from widening without preserving its rollback and diagnostics
...
pub fn review_pack_rerun_compare_stays_honest(
    scope: M5ReviewPackRerunCompareScope,
    comparison_is_truthful: bool,
    keeps_review_pack_rerun_compare_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_review_pack_rerun_compare_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-review_pack_invalidation_record-registry entry so it stays bound to the line-review_pack_invalidation_record registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete review_pack_invalidation_record object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_review_pack_invalidation_record_entry(
    input: M5ReviewPackInvalidationRecordEntryResolutionInput,
) -> Result<
    M5ResolvedReviewPackInvalidationRecordEntry,
    M5ReviewPackInvalidationRecordResolutionError,
> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ReviewPackInvalidationRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5ReviewPackInvalidationRecordResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = review_pack_invalidation_record_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ReviewPackInvalidationRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ReviewPackInvalidationRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5ReviewPackInvalidationRecordEntryDegradeReason::CohortInvalidationCauseUnclassified)
    } else if !input.bound_to_registry {
        Some(M5ReviewPackInvalidationRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5ReviewPackInvalidationRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5ReviewPackInvalidationRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5ReviewPackInvalidationRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5ReviewPackInvalidationRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5ReviewPackInvalidationRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5ReviewPackInvalidationRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedReviewPackInvalidationRecordEntry {
        ...
        review_pack_invalidation_record_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        review_pack_invalidation_record_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_review_pack_rerun_compare_entry(
    input: M5ReviewPackRerunCompareEntryResolutionInput,
) -> Result<M5ResolvedReviewPackRerunCompareEntry, M5ReviewPackInvalidationRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(
            M5ReviewPackInvalidationRecordResolutionError::EmptyCohortEvidencePacketEntryId,
        );
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = review_pack_rerun_compare_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_review_pack_rerun_compare_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ReviewPackRerunCompareEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ReviewPackRerunCompareEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5ReviewPackRerunCompareEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5ReviewPackRerunCompareEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5ReviewPackRerunCompareEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5ReviewPackRerunCompareEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5ReviewPackInvalidationRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedReviewPackRerunCompareEntry {
        entry_id: input.entry_id,
        ...
        keeps_review_pack_rerun_compare_visible: input.keeps_review_pack_rerun_compare_visible,
        ...
        review_pack_rerun_compare_stays_honest: record_stays_honest,
        provides_complete_review_pack_rerun_compare: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-review_pack_invalidation_record and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackInvalidationAndRerunCompareRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5ReviewPackInvalidationAndRerunCompareRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5ReviewPackInvalidationRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5ReviewPackInvalidationRecordExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5ReviewPackDowngradeTrigger>,
    /// Resolved line-review_pack_invalidation_record-registry examples.
    pub review_pack_invalidation_record_entries: Vec<M5ResolvedReviewPackInvalidationRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_rerun_compare_entries: Vec<M5ResolvedReviewPackRerunCompareEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-review_pack_invalidation_record and
    ...
    pub collapses_distinct_review_pack_rerun_compare_classes_into_one_lane: bool,
}

impl M5ReviewPackInvalidationAndRerunCompareRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5ReviewPackInvalidationRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5ReviewPackInvalidationRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5ReviewPackInvalidationRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5ReviewPackInvalidationRecordExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_review_pack_rerun_compare_classes_into_one_lane
    }

    /// True when a clean line-review_pack_invalidation_record entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete review_pack_invalidation_record object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn review_pack_invalidation_record_is_honest(
        ex: &M5ResolvedReviewPackInvalidationRecordEntry,
    ) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.review_pack_invalidation_record_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedReviewPackRerunCompareEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_review_pack_rerun_compare
                && ex.review_pack_rerun_compare_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.review_pack_invalidation_record_entries
            .iter()
            .all(Self::review_pack_invalidation_record_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackInvalidationAndRerunCompareRegistriesVocabularySet {
    ...
    /// Cohort-review_pack_invalidation_record-entry degrade-reason tokens.
    pub review_pack_invalidation_record_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub review_pack_rerun_compare_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5ReviewPackInvalidationAndRerunCompareRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ReviewPackRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5ReviewPackInvalidationRecordResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5ReviewPackInvalidationRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ReviewPackRerunCompareScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5ReviewPackInvalidationRecordSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            review_pack_invalidation_record_degrade_reasons: tokens(
                &M5ReviewPackInvalidationRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_rerun_compare_degrade_reasons: tokens(
                &M5ReviewPackRerunCompareEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5ReviewPackInvalidationRecordAnatomyPart::ALL, |v| {
                v.as_str()
            }),
            next_actions: tokens(&M5ReviewPackInvalidationRecordNextAction::ALL, |v| {
                v.as_str()
            }),
            export_fields: tokens(&M5ReviewPackInvalidationRecordExportField::ALL, |v| {
                v.as_str()
            }),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackInvalidationAndRerunCompareRegistriesGovernanceReview {
    /// The review_pack_invalidation_record registry names a canonical token, semantic role, and line journey for every entry.
    pub review_pack_invalidation_record_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-review_pack_invalidation_record object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_review_pack_invalidation_record_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved review_pack_invalidation_record.
    ...
    pub review_pack_rerun_compare_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-review_pack_invalidation_record and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-review_pack_invalidation_record and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub review_pack_invalidation_record_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackInvalidationAndRerunCompareRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-review_pack_invalidation_record registry.
    ...
    /// Behavior traces back to the canonical line-review_pack_invalidation_record and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-review_pack_invalidation_record / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackInvalidationAndRerunCompareRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackInvalidationAndRerunCompareRegistriesReleasePosture {
...
/// Constructor input for [`M5ReviewPackInvalidationAndRerunCompareRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ReviewPackInvalidationAndRerunCompareRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5ReviewPackInvalidationAndRerunCompareRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5ReviewPackInvalidationAndRerunCompareRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5ReviewPackInvalidationAndRerunCompareRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5ReviewPackInvalidationAndRerunCompareRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5ReviewPackInvalidationAndRerunCompareRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5ReviewPackInvalidationAndRerunCompareRegistriesReleasePosture,
...
/// Export-safe M5 line-review_pack_invalidation_record and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ReviewPackInvalidationAndRerunCompareRegistriesPacket {
    /// Record kind; must equal [`M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_SCHEMA_VERSION`].
...
impl M5ReviewPackInvalidationAndRerunCompareRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5ReviewPackInvalidationAndRerunCompareRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version: M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5ReviewPackInvalidationAndRerunCompareRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_RECORD_KIND
        {
            violations
                .push(M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_REVIEW_PACK_INVALIDATION_AND_RERUN_COMPARE_REGISTRIES_SCHEMA_VERSION
        {
            violations.push(
                M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations
                .push(M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(
                M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::VocabularySetDrift,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-review_pack_invalidation_record / line-downgrade-packet registries packet serializes",
        )) {
            violations.push(M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,review_pack_invalidation_record_entries,review_pack_rerun_compare_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            let degrades: Vec<&str> = row
                ...
                .chain(
                    row.review_pack_rerun_compare_entries
                        .iter()
                        .filter_map(|ex| ex.degrade_reason.map(|r| r.as_str())),
                )
                .collect();
            out.push_str(&format!(
                ...
                row.review_pack_invalidation_record_entries.len(),
                row.review_pack_rerun_compare_entries.len(),
                degrades.join("|"),
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Review-Pack Invalidation-Record and Rerun-Compare Registries\n\n");
        ...
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                "  - Review-pack-record entries: {} / review-pack-rerun-compare entries: {}\n",
                row.review_pack_invalidation_record_entries.len(),
                row.review_pack_rerun_compare_entries.len()
            ));
        }
    ...
    /// registry-bound line-review_pack_invalidation_record entries are listed.
    pub fn render_review_pack_invalidation_record_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.review_pack_invalidation_record_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5ReviewPackInvalidationAndRerunCompareRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5ReviewPackInvalidationAndRerunCompareRegistriesViolation>),
}

impl fmt::Display for M5ReviewPackInvalidationAndRerunCompareRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-review_pack_invalidation_record / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-review_pack_invalidation_record / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5ReviewPackInvalidationAndRerunCompareRegistriesArtifactError {}

/// Validation failures emitted by [`M5ReviewPackInvalidationAndRerunCompareRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5ReviewPackInvalidationAndRerunCompareRegistriesViolation {
    ...
    /// A registry row does not point at both the line-review_pack_invalidation_record and line-downgrade-packet domain schemas.
    ...
    /// Cohort-review_pack_invalidation_record-resolution is not proven: clean review_pack_invalidation_record entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean review_pack_invalidation_record entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded review_pack_invalidation_record entry is present, or a clean review_pack_invalidation_record entry is unbounded
    /// or unbound.
...
impl M5ReviewPackInvalidationAndRerunCompareRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "review_pack_invalidation_record_resolution_not_proven"
            }
            ...
            Self::CohortEvidenceIntegrityNotProven => {
                "review_pack_rerun_compare_integrity_not_proven"
            }
...
pub fn current_stable_m5_review_pack_invalidation_and_rerun_compare_registries_export() -> Result<
    M5ReviewPackInvalidationAndRerunCompareRegistriesPacket,
    M5ReviewPackInvalidationAndRerunCompareRegistriesArtifactError,
> {
    let packet: M5ReviewPackInvalidationAndRerunCompareRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/review/m5-review-pack-invalidation-and-rerun-compare-registries-proof/support_export.json"
        )
    ))
    .map_err(M5ReviewPackInvalidationAndRerunCompareRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5ReviewPackInvalidationAndRerunCompareRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5ReviewPackInvalidationAndRerunCompareRegistriesPacket,
    violations: &mut Vec<M5ReviewPackInvalidationAndRerunCompareRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations
                .push(M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::MandatoryExportFieldMissing);
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_REVIEW_PACK_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_REVIEW_PACK_RESULT_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.review_pack_invalidation_record_entries.is_empty()
            || row.review_pack_rerun_compare_entries.is_empty()
        {
            violations
                .push(M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations
                .push(M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(
                M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.review_pack_invalidation_record_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_review_pack_invalidation_record_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.review_pack_rerun_compare_keeps_proof_visible_and_binds_support_language,
        ...
        review.review_pack_invalidation_record_or_downgrade_drift_caught_before_release,
        review.every_row_declares_mandatory_anatomy,
        review.reuses_frozen_matrix_vocabulary,
    ] {
        if !ok {
            violations
                .push(M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::ConsumerProjectionIncomplete);
            return;
        }
    }
}
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let review_pack_invalidation_records = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.review_pack_invalidation_record_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.review_pack_rerun_compare_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean review_pack_invalidation_record entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean review_pack_invalidation_record entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = review_pack_invalidation_records()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = review_pack_invalidation_records()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5ReviewPackInvalidationRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5ReviewPackInvalidationRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = review_pack_invalidation_records().any(|ex| {
        ex.degrade_reason
            == Some(
                M5ReviewPackInvalidationRecordEntryDegradeReason::CohortDescriptorObjectIncomplete,
            )
    });
    let no_clean_incomplete = !review_pack_invalidation_records()
        .any(|ex| ex.is_clean() && !ex.review_pack_invalidation_record_object_complete);
    if !(journeys_covered
    ...
    {
        violations.push(
            M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }

    // AC2: line packets preserve rollback and diagnostics posture before widening. A widen-without-rollback
    // example degrades, an unbound example degrades, at least one clean bounded review_pack_invalidation_record entry is present, and
    // no clean review_pack_invalidation_record entry is unbounded or unbound.
    let widen_fold_degrades = review_pack_invalidation_records().any(|ex| {
        ex.degrade_reason
            == Some(
                M5ReviewPackInvalidationRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = review_pack_invalidation_records().any(|ex| {
        ex.degrade_reason
            == Some(M5ReviewPackInvalidationRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_review_pack_invalidation_record = review_pack_invalidation_records()
        .any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound =
        !review_pack_invalidation_records().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded = !review_pack_invalidation_records()
        .any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_review_pack_invalidation_record
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
            M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ReviewPackRerunCompareScope::CANONICAL_SCOPES
        .iter()
    ...
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_review_pack_rerun_compare);
    ...
    {
        violations
            .push(M5ReviewPackInvalidationAndRerunCompareRegistriesViolation::CohortEvidenceIntegrityNotProven);
    }
}
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-review_pack_invalidation_record registry
