//! Tests for the M05-1187 window-restore surface certification capstone.

use super::*;

fn packet() -> WindowRestoreProfileCertificationPacket {
    seeded_m5_window_restore_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, WINDOW_RESTORE_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, WINDOW_RESTORE_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5WindowRestoreCertifiedProfile::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            WindowRestoreCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, WindowRestoreAxisCertificationState::Certified);
...
fn all_five_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5WindowRestoreA11yClaim> =
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5WindowRestoreCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5WindowRestoreCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = WindowRestoreCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), WindowRestoreCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in WindowRestoreCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == WindowRestoreCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    // The B141 localization axis is present, distinct, and exercised by the
    // disclosed-layout-skeleton profile as its binding narrowed axis.
    let p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5WindowRestoreCertifiedProfile::DisclosedLayoutSkeletonProfile)
        .expect("disclosed-layout-skeleton-profile row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(WindowRestoreCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(WindowRestoreCertificationAxis::Localization)
    );
}

#[test]
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    // The B141 high-contrast reach axis stays certified across the seed (never a binding narrowed axis here).
    for row in &packet().rows {
        let outcome = row
            .axis(WindowRestoreCertificationAxis::HighContrast)
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5WindowRestoreCertifiedProfile::LiveTrustedRestoreSurface)
        .expect("live-trusted-restore-surface row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == WindowRestoreCertificationAxis::Visual {
            outcome.state = WindowRestoreAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("workspace authority silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, WindowRestoreProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        WindowRestoreCertificationViolation::ProfileBlocked { .. }
    )));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5WindowRestoreCertifiedProfile::ReviewableRestoreStructure)
        .expect("reviewable-restore-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails
        .reruns_commands_or_reattaches_privileged_sessions_implicitly_during_restore = true;
    assert_eq!(row.derive_status(), WindowRestoreProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        WindowRestoreCertificationViolation::GuardrailViolated { .. }
    )));
}
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5WindowRestoreA11yClaim::TrustedRestoreSurface;
    row.certified_claim = M5WindowRestoreA11yClaim::TrustedRestoreSurface;
    assert_eq!(row.derive_status(), WindowRestoreProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == WindowRestoreCertificationAxis::DegradedState {
            *outcome = WindowRestoreAxisOutcome {
                axis: WindowRestoreCertificationAxis::DegradedState,
                state: WindowRestoreAxisCertificationState::DisclosedNarrowed,
                parity_note: "the window-restore evidence is lagging".to_owned(),
                narrowing_reason: Some(
                    "the window-restore evidence is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5WindowRestoreDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableRestoreSurface == certified, no claim_auto_narrow.
    assert_eq!(row.derive_status(), WindowRestoreProfileClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == WindowRestoreCertificationAxis::CliExport {
            outcome.state = WindowRestoreAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5WindowRestoreDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), WindowRestoreProfileClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(WindowRestoreClaimAutoNarrow {
        binding_axis: WindowRestoreCertificationAxis::DegradedState,
        from_claim: M5WindowRestoreA11yClaim::TrustedRestoreSurface,
        to_claim: M5WindowRestoreA11yClaim::TrustedRestoreSurface,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5WindowRestoreA11yClaim::LayoutSkeletonDisclosedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-restore-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableRestoreSurface; certify a stronger trusted restore surface.
    row.certified_claim = M5WindowRestoreA11yClaim::TrustedRestoreSurface;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = WindowRestoreCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), WindowRestoreProfileClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == WindowRestoreCertificationAxis::CliExport {
            outcome.state = WindowRestoreAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the layout-skeleton surface".to_owned());
            outcome.downgrade_trigger = Some(M5WindowRestoreDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = WindowRestoreCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), WindowRestoreProfileClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == WindowRestoreProfileClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "topology".to_owned();
    }
    assert_eq!(row.derive_status(), WindowRestoreProfileClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(WindowRestoreCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5WindowRestoreDowngradeTrigger::ProofStale,
    );
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.profile != M5WindowRestoreCertifiedProfile::UnverifiedDisplayRecoveryProfile);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        WindowRestoreCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the shared-workspace-authority family from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5WindowRestoreFamily::SharedWorkspaceAuthority);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        WindowRestoreCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = WindowRestoreProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        WindowRestoreCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, WindowRestoreCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, WindowRestoreCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != WindowRestoreCertificationAxis::WindowRestoreComponentTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, WindowRestoreCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        WindowRestoreCertificationViolation::RawWindowRestoreMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: WindowRestoreProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_window_restore_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/ui/m5-window-restore-surface-certification/support_export.json"
    ));
    let packet_from_fixture: WindowRestoreProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/ui/m5-window-restore-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_WINDOW_RESTORE_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures from
...
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_WINDOW_RESTORE_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_window_restore_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/release/m5-window-restore-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-window-restore-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/ui/m5-window-restore-surface-certification");
