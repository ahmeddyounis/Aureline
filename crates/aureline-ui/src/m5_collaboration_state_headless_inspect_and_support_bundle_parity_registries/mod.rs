//! Implemented M5 collaboration-state headless-inspect and support-bundle parity registries.
//!
//! The frozen [collaboration-state authority matrix][matrix] names Aureline's governed shared-object classes and
//! locks their authority, convergence, and export vocabulary. Building on the presenter-follow convergence-state
//! domain (the convergence, awareness, and drift facts a session carries) and the sealed-session compaction-manifest
//! domain it freezes, this module makes *collaboration-state truth inspectable outside the live desktop UI* —
//! collaboration replica descriptors, shared-object records, anchor history, degraded session states, compaction
//! manifests, and archive-finalization descriptors — so search, AI context, review panes, docs links, and
//! support packets, or a user running a CLI export or building a support bundle, reach for them: whenever a headless
//! or support surface inspects a collaboration-state object, the object class, the state class it carries (the
//! convergence / drift / archive facts and the actor / source labels), the authority model those facts imply, and
//! that the CLI / headless projection carries the same state vocabulary the desktop UI shows are projected first, and
//! a headless view carries the same vocabulary the UI shows but never reads as a reduced UI-only summary without a
//! state class and actor / source labels. So the shared editor replica view, the presence / cursor layer, the
//! comment / annotation / review-pin layer, the collaboration degradation banner, and the support / export and help /
//! docs surfaces inherit one canonical headless-inspect descriptor rather than a hand-authored parallel prose that has
//! to be kept consistent: it turns the *headless-inspect* grammar (one durable append-only record per
//! collaboration-state object — one typed field per record section: the object class, the state class it carries, the
//! authority model those facts imply, and that the projection carries the same state vocabulary the UI shows, each
//! bound to the object it never reduces to a UI-only summary by default) and the *support-bundle-export-disposition*
//! grammar (the disposition offered before a collaboration-state fact is emitted into a CLI export or support bundle —
//! include-in-support-bundle, headless-inspect-only, redact-and-export, block-export, or defer — plus its actor / time
//! provenance, its compaction / redaction facts, and its policy-labeled redaction lineage) into registry resolvers
//! that produce export-safe, honest projections. Every collaboration-state object then resolves to one typed
//! headless-inspect descriptor — the fields it publishes, the object class, the state class, the authority model, and
//! that it carries the same vocabulary the UI shows, all attributable so a convergence, drift, or archive fact is
//! never silently reduced to UI-only knowledge — and to one support-bundle-export-disposition descriptor — the
//! disposition, its actor / time provenance, and its policy-labeled redaction lineage — that the search, AI,
//! review-pin, degradation-banner, and support surfaces can inspect before continuing to include-in-support-bundle,
//! headless-inspect-only, redact-and-export, block, or defer, so a descriptor that cannot keep an object's true state
//! vocabulary distinct from a reduced UI-only summary degrades honestly instead of letting a headless or support
//! projection read as a clean object when it lacks a state class or actor / source labels.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable headless-inspect descriptor per collaboration-state object, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [object class][M5CollaborationStateHeadlessInspectParityRecordKind],
//!   a disclosure role, covers every [resolution form][M5CollaborationStateHeadlessInspectParityRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (the
//!   object class, the state class and authority model, that it carries the same vocabulary the UI shows, and the object it is
//!   bound to), keeps its lineage
//!   attributable, and keeps the object it never reduces to a UI-only summary bound to the record; otherwise it degrades.
//! * **Never let a headless or support projection drop the state vocabulary the UI shows; keep the state class and authority model first.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a record whose
//!   parity-first ordering is not held (a headless or support projection presenting itself as a clean object while
//!   its state class or actor / source labels were dropped) so it degrades to
//!   [`M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and any surface — a search hit, an AI context window, or a docs link — that
//!   would present a reduced UI-only projection degrades the same way — the structured blocker reason a
//!   headless-projection-stripped-of-UI-vocabulary attempt must surface.
//! * **Offer a reviewable support-bundle export disposition with actor / time provenance, instead of a silent UI-only reduction.**
//!   [`resolve_review_pack_result_entry`] names a classified
//!   [disposition binding][M5CollaborationStateHeadlessInspectParityResultScope]
//!   (include-in-support-bundle binding, headless-inspect-only binding, or block-export binding), requires the full
...
//!   [`M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the disposition would export an op-log, snapshot, or archive without a policy-labeled redaction and actor lineage, act without actor / time provenance, or
//!   drop the redaction lineage, so a disposition can never read as trustworthy when it has quietly reduced
//!   a collaboration-state fact to UI-only knowledge or dropped its redaction lineage.
//!
//! The resolvers reuse the frozen matrix vocabulary directly — the [`M5CollaborationStateRole`] disclosure
//! role vocabulary and the [`M5CollaborationStateConsumerSurface`] consumer-surface taxonomy — so the shared editor
//! replica view, presence / cursor layer, comment / annotation / review-pin layer, degradation banner, and support /
//! export surfaces can never fork their own collaboration-state meaning. Raw secrets, raw command text, and clipboard contents stay outside the export boundary.
//!
//! [matrix]: crate::m5_collaboration_replica_shared_object_authority_anchor_drift_convergence_and_session_archive_matrix
...
pub use seed::{
    ...
    M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_PACKET_ID,
};
...
use crate::m5_collaboration_replica_shared_object_authority_anchor_drift_convergence_and_session_archive_matrix::{
    M5CollaborationStateAccessibilityRoute, M5CollaborationStateClassificationStage,
    M5CollaborationStateConsumerSurface, M5CollaborationStateDowngradeTrigger,
    M5CollaborationStateObject, M5CollaborationStateQualificationClass,
    M5CollaborationStateRequiredLabel, M5CollaborationStateRole,
    M5_PRESENTER_FOLLOW_STATE_DOMAIN_SCHEMA_REF, M5_SEALED_SESSION_ARCHIVE_DOMAIN_SCHEMA_REF,
    M5_COLLABORATION_STATE_MATRIX_DOC_REF, M5_COLLABORATION_STATE_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesPacket`].
pub const M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_collaboration_state_headless_inspect_and_support_bundle_parity_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_collaboration_state_headless_inspect_and_support_bundle_parity_registries.md";

/// Repo-relative path of the checked support-export artifact.
pub const M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-collaboration-state-headless-inspect-and-support-bundle-parity-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-collaboration-state-headless-inspect-and-support-bundle-parity-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-collaboration-state-headless-inspect-and-support-bundle-parity-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesConsumerSurface =
    M5CollaborationStateConsumerSurface;
...
pub enum M5CollaborationStateHeadlessInspectParityRecordResolutionForm {
...
impl M5CollaborationStateHeadlessInspectParityRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationStateHeadlessInspectParityRecordKind {
...
impl M5CollaborationStateHeadlessInspectParityRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationStateHeadlessInspectParityResultScope {
...
impl M5CollaborationStateHeadlessInspectParityResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationStateHeadlessInspectParityRecordSurfaceContext {
...
impl M5CollaborationStateHeadlessInspectParityRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationStateHeadlessInspectParityRecordAnatomyPart {
...
impl M5CollaborationStateHeadlessInspectParityRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationStateHeadlessInspectParityRecordNextAction {
...
impl M5CollaborationStateHeadlessInspectParityRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationStateHeadlessInspectParityRecordExportField {
...
impl M5CollaborationStateHeadlessInspectParityRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason {
...
impl M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5CollaborationStateHeadlessInspectParityRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5CollaborationStateHeadlessInspectParityRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5CollaborationStateHeadlessInspectParityRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5CollaborationStateHeadlessInspectParityRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => {
                M5CollaborationStateHeadlessInspectParityRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }

    /// The matching downgrade trigger recorded on the frozen matrix.
    pub const fn downgrade_trigger(self) -> M5CollaborationStateDowngradeTrigger {
        match self {
            Self::DescriptorTokenUnstated
            | Self::SurfaceContextUnresolved
            | Self::ResolutionFormCoverageIncomplete
            ...
            Self::CohortPackScopeUnclassified | Self::CohortDescriptorObjectIncomplete => {
                M5CollaborationStateDowngradeTrigger::ConvergenceStateUnstated
            }
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort => {
                M5CollaborationStateDowngradeTrigger::LocalTruthPreservationUnstated
            }
            Self::ProofStale => M5CollaborationStateDowngradeTrigger::CollaborationStateMatrixStale,
        }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason {
...
impl M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn downgrade_trigger(self) -> M5CollaborationStateDowngradeTrigger {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::EvidenceScopeUnclassified => {
                M5CollaborationStateDowngradeTrigger::ExportPostureUnstated
            }
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5CollaborationStateDowngradeTrigger::UnsentLocalEditsDiscardedOnDowngrade
            }
            Self::EvidenceFormCoverageIncomplete => {
                M5CollaborationStateDowngradeTrigger::ProvenanceOrFreshnessUnstated
            }
            Self::ProofStale => M5CollaborationStateDowngradeTrigger::CollaborationStateMatrixStale,
        }
    }
...
pub struct M5CollaborationStateHeadlessInspectParityRecordEntryResolutionInput {
    ...
    pub semantic_role: M5CollaborationStateRole,
    /// The line journey this entry classifies.
    pub report_section: M5CollaborationStateHeadlessInspectParityRecordKind,
    /// The render / surface context.
    pub surface_context: M5CollaborationStateHeadlessInspectParityRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage:
        Vec<M5CollaborationStateHeadlessInspectParityRecordResolutionForm>,
...
pub struct M5ResolvedCollaborationStateHeadlessInspectParityRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5CollaborationStateHeadlessInspectParityRecordNextAction,
...
impl M5ResolvedCollaborationStateHeadlessInspectParityRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5CollaborationStateHeadlessInspectParityResultEntryResolutionInput {
    ...
    pub comparison_scope: M5CollaborationStateHeadlessInspectParityResultScope,
    /// The render / surface context.
    pub surface_context: M5CollaborationStateHeadlessInspectParityRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage:
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedCollaborationStateHeadlessInspectParityResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5CollaborationStateHeadlessInspectParityRecordNextAction,
...
impl M5ResolvedCollaborationStateHeadlessInspectParityResultEntry {
...
pub enum M5CollaborationStateHeadlessInspectParityRecordResolutionError {
...
impl M5CollaborationStateHeadlessInspectParityRecordResolutionError {
...
impl fmt::Display for M5CollaborationStateHeadlessInspectParityRecordResolutionError {
...
impl Error for M5CollaborationStateHeadlessInspectParityRecordResolutionError {}

fn form_tokens(
    forms: &[M5CollaborationStateHeadlessInspectParityRecordResolutionForm],
) -> Vec<String> {
...
fn covers_all_resolution_forms(
    ...
    let present: BTreeSet<M5CollaborationStateHeadlessInspectParityRecordResolutionForm> =
        forms.iter().copied().collect();
    M5CollaborationStateHeadlessInspectParityRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5CollaborationStateHeadlessInspectParityRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5CollaborationStateHeadlessInspectParityResultScope,
...
pub fn resolve_review_pack_record_entry(
    ...
    M5CollaborationStateHeadlessInspectParityRecordResolutionError,
> {
    if input.entry_id.trim().is_empty() {
        return Err(M5CollaborationStateHeadlessInspectParityRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(
            M5CollaborationStateHeadlessInspectParityRecordResolutionError::ForbiddenMaterial,
        );
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5CollaborationStateHeadlessInspectParityRecordNextAction::ExpandCohortMeaning,
    };
...
> {
    if input.entry_id.trim().is_empty() {
        return Err(
            M5CollaborationStateHeadlessInspectParityRecordResolutionError::EmptyCohortEvidencePacketEntryId,
        );
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5CollaborationStateHeadlessInspectParityRecordNextAction::TraceCanonicalRegistry,
    };
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface:
        M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesConsumerSurface,
    ...
    pub capture_lifecycle_stages: Vec<M5CollaborationStateClassificationStage>,
    ...
    pub anatomy_parts: Vec<M5CollaborationStateHeadlessInspectParityRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5CollaborationStateHeadlessInspectParityRecordExportField>,
    ...
    pub review_pack_record_entries:
        Vec<M5ResolvedCollaborationStateHeadlessInspectParityRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries:
        Vec<M5ResolvedCollaborationStateHeadlessInspectParityResultEntry>,
...
impl M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5CollaborationStateHeadlessInspectParityRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5CollaborationStateHeadlessInspectParityRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5CollaborationStateHeadlessInspectParityRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5CollaborationStateHeadlessInspectParityRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(
        ex: &M5ResolvedCollaborationStateHeadlessInspectParityRecordEntry,
    ) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(
        ex: &M5ResolvedCollaborationStateHeadlessInspectParityResultEntry,
    ) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesVocabularySet {
...
impl M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesVocabularySet {
    ...
    pub fn canonical() -> Self {
        Self {
            ...
            report_section_kinds: tokens(
                &M5CollaborationStateHeadlessInspectParityRecordKind::ALL,
                |v| v.as_str(),
            ),
            comparison_scopes: tokens(
                &M5CollaborationStateHeadlessInspectParityResultScope::ALL,
                |v| v.as_str(),
            ),
            surface_contexts: tokens(
                &M5CollaborationStateHeadlessInspectParityRecordSurfaceContext::ALL,
                |v| v.as_str(),
            ),
            review_pack_record_degrade_reasons: tokens(
                &M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(
                &M5CollaborationStateHeadlessInspectParityRecordAnatomyPart::ALL,
                |v| v.as_str(),
            ),
            next_actions: tokens(
                &M5CollaborationStateHeadlessInspectParityRecordNextAction::ALL,
                |v| v.as_str(),
            ),
            export_fields: tokens(
                &M5CollaborationStateHeadlessInspectParityRecordExportField::ALL,
                |v| v.as_str(),
            ),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set:
        M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesVocabularySet,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_SCHEMA_VERSION`].
...
impl M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(
        input: M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesPacketInput,
    ) -> Self {
        Self {
            record_kind: M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(
        &self,
    ) -> Vec<M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_RECORD_KIND {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::WrongRecordKind,
            );
        }
        if self.schema_version != M5_COLLABORATION_STATE_HEADLESS_INSPECT_AND_SUPPORT_BUNDLE_PARITY_REGISTRIES_SCHEMA_VERSION
        {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::MissingIdentity,
            );
        }
        ...
        if !self.vocabulary_set.matches_canonical() {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::VocabularySetDrift,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::RawMaterialInExport,
            );
        }
...
#[derive(Debug)]
pub enum M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation>),
}

impl fmt::Display
    for M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesArtifactError
{
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation {
...
impl M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_collaboration_state_headless_inspect_and_support_bundle_parity_registries_export(
) -> Result<
    M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesPacket,
    M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesArtifactError,
> {
    let packet: M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-collaboration-state-headless-inspect-and-support-bundle-parity-registries-proof/support_export.json"
        )
    ))
    .map_err(M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(
            M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesArtifactError::Validation(
                violations,
            ),
        )
    }
...
fn validate_source_contracts(
    ...
    violations: &mut Vec<
        M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation,
    >,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations
            .push(M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_PRESENTER_FOLLOW_STATE_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_SEALED_SESSION_ARCHIVE_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::ExamplesMissing,
            );
        }
        if !row.examples_are_honest() {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::DishonestExample,
            );
        }
        if !row.honours_invariants() {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5CollaborationStateHeadlessInspectParityRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered =
        M5CollaborationStateHeadlessInspectParityRecordSurfaceContext::FIRST_CONSUMERS
            .iter()
    ...
    {
        violations.push(
            M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    let unbound_degrades = review_pack_records().any(|ex| {
        ex.degrade_reason
            == Some(
                M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::DescriptorNotBoundToRegistry,
            )
    });
    ...
    {
        violations.push(
            M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered =
        M5CollaborationStateHeadlessInspectParityResultScope::CANONICAL_SCOPES
            .iter()
    ...
    let form_incomplete_degrades = downgrade().any(|ex| {
        ex.degrade_reason
            == Some(
                M5CollaborationStateHeadlessInspectParityResultEntryDegradeReason::EvidenceFormCoverageIncomplete,
            )
    });
    ...
    {
        violations.push(
            M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
...
pub const IMPLEMENTED_LINES: [M5CollaborationStateObject; 6] = M5CollaborationStateObject::ALL;
