...
fn clean_review_pack_record_input(
    ...
    M5CollaborationStateHeadlessInspectParityRecordEntryResolutionInput {
        ...
        report_section: M5CollaborationStateHeadlessInspectParityRecordKind::ChangedFilesScope,
        surface_context:
            M5CollaborationStateHeadlessInspectParityRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage:
...
fn clean_downgrade_input() -> M5CollaborationStateHeadlessInspectParityResultEntryResolutionInput {
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(
        ...
        M5CollaborationStateHeadlessInspectParityRecordNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    input.report_section =
        M5CollaborationStateHeadlessInspectParityRecordKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5CollaborationStateHeadlessInspectParityRecordKind::FullTreeScope;
    ...
    input.surface_context =
        M5CollaborationStateHeadlessInspectParityRecordSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        ...
        M5CollaborationStateHeadlessInspectParityRecordResolutionError::EmptyCohortDescriptorEntryId
    );
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5CollaborationStateHeadlessInspectParityRecordAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    let mut packet =
        seeded_m5_collaboration_state_headless_inspect_and_support_bundle_parity_registries();
    packet.registry_rows[0].export_fields.retain(|f| {
        *f != M5CollaborationStateHeadlessInspectParityRecordExportField::CohortArchetypes
    });
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesViolation::ExamplesMissing));
}
...
fn review_pack_record_not_proven_when_incomplete_example_removed() {
    ...
    for row in &mut packet.registry_rows {
        row.review_pack_record_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
                    M5CollaborationStateHeadlessInspectParityRecordEntryDegradeReason::CohortDescriptorObjectIncomplete,
                )
        });
    }
    assert!(packet.validate().contains(
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_collaboration_state_headless_inspect_and_support_bundle_parity_registries_export()
        .expect(
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-collaboration-state-headless-inspect-and-support-bundle-parity-registries/inspect_record_beta_narrowed.json"
    )))
    ...
    let preview: M5CollaborationStateHeadlessInspectParityRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-collaboration-state-headless-inspect-and-support-bundle-parity-registries/support_bundle_preview_narrowed.json"
    )))
