...
fn seed() -> M5CoreControlSharedConsumersPacket {
    seeded_m5_core_action_input_shared_consumers()
}

fn violations_of(packet: &M5CoreControlSharedConsumersPacket) -> Vec<&'static str> {
...
fn seed_validates_clean() {
    ...
    assert_eq!(packet.packet_id, M5_CORE_CONTROL_SHARED_CONSUMERS_PACKET_ID);
    assert_eq!(
        packet.record_kind,
        M5_CORE_CONTROL_SHARED_CONSUMERS_RECORD_KIND
    );
}

#[test]
fn every_control_is_adopted_by_two_or_more_consumers() {
    let packet = seed();
    ...
    assert_eq!(component_consumers.len(), 8, "all eight controls adopted");
    for (component, consumers) in &component_consumers {
        assert!(
            consumers.len() >= 2,
            "control {} only adopted by {} consumers",
            component.as_str(),
            consumers.len()
        );
    }
...
fn every_consumer_surface_and_representation_is_exercised() {
    ...
    let representations: BTreeSet<_> = packet
        ...
        .collect();
    for representation in CoreControlRepresentation::ALL {
        assert!(
    ...
    for binding in &packet.consumer_bindings {
        ...
        if binding.is_narrowed() {
            assert_eq!(
                ...
                CoreControlParityState::FacetsDisclosedNarrowed
            );
            let note = binding
                .narrow_note
                .as_ref()
            ...
            assert_eq!(
                binding.parity_state,
                CoreControlParityState::FacetsPreserved
            );
            assert!(binding.narrow_note.is_none());
        }
        if matches!(
            binding.representation,
            CoreControlRepresentation::RemoteProjected
        ) {
            assert!(!binding.remote_source_note.trim().is_empty());
        }
        if matches!(
            binding.representation,
            CoreControlRepresentation::ExportedRedacted
        ) {
...
fn disclosure_resolver_matches_representation() {
    assert!(
        !resolve_core_control_render_disclosure(CoreControlRepresentation::DesktopFull)
            .needs_narrow_note
    );
    let compact =
        resolve_core_control_render_disclosure(CoreControlRepresentation::CompactNarrowed);
    assert_eq!(
        compact.narrow_reason,
        Some(CoreControlNarrowReason::CompactionNarrowed)
    );
    assert!(compact.needs_narrow_note);
    assert!(!compact.needs_remote_source_note);
    let remote = resolve_core_control_render_disclosure(CoreControlRepresentation::RemoteProjected);
    assert!(remote.needs_remote_source_note);
    let exported =
        resolve_core_control_render_disclosure(CoreControlRepresentation::ExportedRedacted);
...
fn vocabulary_drift_is_rejected() {
    let mut packet = seed();
    // Reword one surface of a two-binding object to a different (still-valid) state word.
    let target = packet
        .consumer_bindings
        .iter()
        .position(|b| b.binding_id == "cc-textfield-forms")
        .unwrap();
    packet.consumer_bindings[target].state_facets.state_word = "loading".to_owned();
...
fn reuse_below_two_consumers_is_rejected() {
    let mut packet = seed();
    // Drop every combobox binding except one, leaving the control with one consumer.
    let mut kept_one = false;
    packet.consumer_bindings.retain(|b| {
        if b.component == M5CoreControlFamily::Combobox {
            if kept_one {
                return false;
            }
            kept_one = true;
    ...
    assert!(violations_of(&packet).contains(&"control_reuse_unproven"));
}
...
fn missing_canonical_reference_on_export_is_rejected() {
    ...
    packet.consumer_bindings[target].source_contract_refs =
        vec![M5_CORE_CONTROL_COMPONENT_SCHEMA_REF.to_owned()];
...
fn unexpected_narrow_note_on_full_binding_is_rejected() {
    ...
    let target = packet
        ...
        .position(|b| !b.is_narrowed())
        .unwrap();
    packet.consumer_bindings[target].narrow_note = Some(CoreControlNarrowNote {
        reason: CoreControlNarrowReason::CompactionNarrowed,
        preserved_vocabulary_note: "x".to_owned(),
        next_action: CoreControlNarrowNextAction::ExpandInDesktop,
...
fn each_guardrail_is_enforced() {
    let cases: [(fn(&mut CoreControlConsumerBinding), &str); 6] = [
        ...
        (
            |b| b.lets_loading_relabel_the_action_or_lose_attribution = true,
            "loading_relabels_or_loses_attribution",
        ),
        (
            |b| b.leaves_icon_only_destructive_action_unlabeled = true,
            "icon_destructive_unlabeled",
        ),
        (
            |b| b.blurs_switch_with_deferred_checkbox = true,
            "switch_blurred_with_deferred_checkbox",
        ),
        (
            |b| b.lets_split_button_default_to_riskier_alternate = true,
            "split_defaults_to_riskier_alternate",
        ),
        (
            |b| b.hides_locked_or_degraded_semantics_behind_generic_disabled = true,
...
#[test]
fn control_coverage_gap_is_rejected() {
    let mut packet = seed();
    packet
        .consumer_bindings
        .retain(|b| b.component != M5CoreControlFamily::SegmentedControl);
...
fn missing_source_contract_is_rejected() {
    ...
    packet
        ...
        .retain(|r| r != M5_CORE_CONTROL_SHARED_CONSUMERS_DOC_REF);
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_core_control_shared_consumers_export()
        .expect("checked M5 core-control shared-consumer export validates");
    assert_eq!(
        from_disk.packet_id,
...
fn narrowed_fixture_variants_validate_and_keep_coverage() {
    let compact = seeded_m5_core_action_input_shared_consumers_compact_remote_narrowed();
    assert!(
    ...
    let exported = seeded_m5_core_action_input_shared_consumers_exported_redaction_narrowed();
    assert!(
...
fn checked_narrowed_fixtures_validate_and_match_builders() {
    let compact: M5CoreControlSharedConsumersPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/ui/m5-core-action-input-shared-consumers/compact_remote_narrowed.json"
    )))
    ...
    let exported: M5CoreControlSharedConsumersPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/ui/m5-core-action-input-shared-consumers/exported_redaction_narrowed.json"
    )))
