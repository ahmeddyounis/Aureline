//! Implemented M5 post-launch correction-report and train-comparison registries.
//!
...
//! LTS-candidate line — and locks their controlled vocabulary. This module makes post-launch learning durable so
//! adoption blockers and field signals tie back to supported-line corrections, freshness gaps, and narrowed claims
//! instead of staying trapped in incident chat or ticket dashboards: it turns the *correction-report* grammar (the
//! post-launch correction report published per release train — one typed section per operating signal: its top
//! adoption blockers, its crash / support signals, its compatibility-report freshness deltas, its bundle drift, its
//! public-truth deltas, and its backport exceptions or deferrals — each linked to its correction packets, its
//! supported-line defect-ledger entries, and its current claim rows) and the *train-comparison* grammar (the
//! comparison scope a supported-line issue sits in across trains — a corrected issue, a remaining narrowed claim,
//! or an open exception still needing explicit closure) into registry resolvers that produce export-safe, honest
//! projections. Every claimed M5 release train then resolves to one typed correction-report object — the section it
//! publishes, its affected rows, the correction packets / defect-ledger entries / claim rows it links, its rollback
//! target, and its correction posture, all preserved before a claim widens so a train never keeps onboarding /
//! migration / support language ahead of its linked correction evidence and a current rollback target — and to one
//! train-comparison object — the resolved line identity, the linked-correction reference, the compared-train
//! reference, the comparison-scope state, the narrowed-claim state, the active comparison reason, and the last
//! revision — that the release / help, support, shiproom, executive-steering, program-governance, and public-proof
//! surfaces can inspect without manual reconstruction, so which supported-line issues were corrected, which
//! narrowed claims remain, and which exceptions still need closure stay visible, an unresolved train correction
//! narrows the related stable/LTS support claim automatically, and a report that cannot link a section to explicit
//! correction evidence degrades honestly instead of leaving an operating signal to read as silently resolved.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Publish one typed correction-report section per operating signal, per release train.**
//!   [`resolve_correction_report_entry`] refuses to read as a clean, registry-bound correction-report entry
//!   unless it names a canonical registry token, a classified [report section][M5CorrectionReportKind], a
//!   stable-line-protection role, covers every [resolution form][M5StableLineCorrectionReportResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every report field (affected line
//!   rows, linked correction packets, defect-ledger entries, and claim rows, freshness delta, rollback /
//!   reversibility target, and owning roster), preserves its rollback posture before a claim widens, and keeps any
//!   section bound to its linked correction evidence or claim-narrowing action; otherwise it degrades.
//! * **Keep support / certification language from running ahead of the linked correction evidence.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a correction-report entry whose
//!   rollback posture is not preserved before widening (a train resuming a wider claim while its correction is
//!   unresolved) so it degrades to
//!   [`M5CorrectionReportEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a public-facing section whose compatibility / known-issues language outruns its linked correction
//!   evidence degrades the same way — the structured blocker reason a widen-over-unresolved-train attempt must
//!   surface.
//! * **Let operators compare trains and tell corrected issues from open exceptions.**
//!   [`resolve_train_comparison_entry`] names a classified [comparison scope][M5TrainComparisonScope]
//!   (corrected-issue, remaining-narrowed-claim, or open-exception-closure), requires the full line-identity /
//!   linked-correction / compared-train / comparison-scope / narrowed-claim / active-reason / last-revision
//!   comparison object, covers every resolution form, and degrades to
//!   [`M5TrainComparisonEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the comparison would keep support language ahead of the linked correction, hide the comparison, or let a
//!   gap masquerade as covered, so a train-comparison can never read as trustworthy when it has quietly dropped the
//!   reason a claim stays narrowed or an exception stays open.
//!
...
pub use seed::{
    seeded_m5_stable_line_correction_report_and_train_comparison_registries,
    seeded_m5_stable_line_correction_report_and_train_comparison_registries_correction_report_beta_narrowed,
    seeded_m5_stable_line_correction_report_and_train_comparison_registries_train_comparison_preview_narrowed,
    M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5StableLineCorrectionReportTrainComparisonRegistriesPacket`].
pub const M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_stable_line_correction_report_and_train_comparison_packet_registries";

/// Schema version for M5 line-correction_report / line-downgrade-packet registry records.
pub const M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_DOC_REF: &str =
    "docs/release/m5_stable_line_correction_report_and_train_comparison_registries.md";
...
pub const M5_TRAIN_COMPARISON_DOMAIN_SCHEMA_REF: &str =
    "schemas/program/m5-train-comparison.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-stable-line-correction-report-and-train-comparison-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-stable-line-correction-report-and-train-comparison-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-stable-line-correction-report-and-train-comparison-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5StableLineCorrectionReportTrainComparisonRegistriesConsumerSurface =
    M5StableLineProtectionConsumerSurface;

/// One of the three resolution forms every line-correction_report or line-downgrade-packet entry must hold across so
...
/// line-correction_report and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StableLineCorrectionReportResolutionForm {
    /// The canonical resolved line-correction_report / line-downgrade-packet object.
    CanonicalObject,
...
impl M5StableLineCorrectionReportResolutionForm {
...
/// Controlled report section a post-launch correction-report entry records, so every release train's operating
/// signal shares one typed registry rather than a hand-copied per-train assumption. Minted by this lane because
/// the frozen matrix names the supported lines but not the concrete sections a post-launch correction report must
/// publish per release train — a top adoption-blocker section, a crash/support-signal section, a
/// compatibility-freshness-delta section, a bundle-drift section, a public-truth-delta section, or a
/// backport-exception (deferral) section. Every classified section carries its canonical mode, and the
/// compatibility-freshness and public-truth-delta sections are public-facing (their status surfaces directly in
/// public compatibility / known-issues / support claim language) so their claim must stay matched to the linked
/// correction evidence and a current rollback target before the line widens.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CorrectionReportKind {
    /// The crash/support-signal section: crash and support field signals observed on the train, each linked to its
    /// correction packet, defect-ledger entry, and claim row.
    CrashSupportSignalSection,
    /// The bundle-drift section: launch-bundle / shipping-line drift found on the train, linked to its correction
    /// and claim evidence.
    BundleDriftSection,
    /// The top adoption-blocker section: the top adoption blockers observed after launch, each traced to a
    /// correction packet or a narrowed claim.
    AdoptionBlockerSection,
    /// The compatibility-freshness-delta section (public-facing; its compatibility / known-issues claim must match
    /// the linked correction evidence and a current rollback target).
    CompatibilityFreshnessSection,
    /// The public-truth-delta section (public-facing; a delta between shipped public claims and field truth, whose
    /// claim must match the linked correction evidence and a current rollback target).
    PublicTruthDeltaSection,
    /// The backport-exception section: backport exceptions or deferrals recorded on the train that still need an
    /// explicit disposition.
    BackportExceptionSection,
    /// The report section is unclassified, which is disallowed.
    ReportSectionUnclassified,
}

impl M5CorrectionReportKind {
    /// Every correction-report kind, in declaration order.
    pub const ALL: [Self; 7] = [
        Self::CrashSupportSignalSection,
        Self::BundleDriftSection,
        Self::AdoptionBlockerSection,
        Self::CompatibilityFreshnessSection,
        Self::PublicTruthDeltaSection,
        Self::BackportExceptionSection,
        Self::ReportSectionUnclassified,
    ];

    /// The six canonical correction-report kinds every claimed M5 supported line records for its bundles.
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::CrashSupportSignalSection => "crash_support_signal_section",
            Self::BundleDriftSection => "bundle_drift_section",
            Self::AdoptionBlockerSection => "adoption_blocker_section",
            Self::CompatibilityFreshnessSection => "compatibility_freshness_section",
            Self::PublicTruthDeltaSection => "public_truth_delta_section",
            Self::BackportExceptionSection => "backport_exception_section",
    ...
    /// The canonical mode for this correction-report kind.
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::CrashSupportSignalSection => "crash_support_signal_section_mode",
            Self::BundleDriftSection => "bundle_drift_section_mode",
            Self::AdoptionBlockerSection => "adoption_blocker_section_mode",
            Self::CompatibilityFreshnessSection => "compatibility_freshness_section_mode",
            Self::PublicTruthDeltaSection => "public_truth_delta_section_mode",
            Self::BackportExceptionSection => "backport_exception_section_mode",
            Self::ReportSectionUnclassified => "",
        }
    }

    /// Whether this correction-report kind is public-facing and so must keep its onboarding / migration /
    /// support claim matched to a current, reversible bundle before the line widens.
    pub const fn is_public_facing_line(self) -> bool {
        matches!(
            self,
            Self::CompatibilityFreshnessSection | Self::PublicTruthDeltaSection
        )
    }
}

/// Controlled train-comparison scope a supported-line issue sits in as operators compare one release train to the
/// next, so cross-train post-launch learning becomes visible program truth and shares one registry rather than a
/// hand-copied per-record assumption. Minted by this lane, tracking whether a supported-line issue has been
/// corrected across trains (corrected-issue: a correction packet has shipped a fix), a public claim is still held
/// narrower on the current train (remaining-narrowed-claim), or a backport exception / deferral is still open
/// (open-exception-closure: it still needs explicit closure). Each scope maps directly to what an operator must be
/// able to distinguish when comparing trains — which issues were corrected, which narrowed claims remain, and
/// which exceptions still need closure.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5TrainComparisonScope {
    /// Corrected-issue: a supported-line issue a correction packet has shipped a fix for on the compared train.
    CorrectedIssue,
    /// Remaining-narrowed-claim: a public claim still held narrower on the current train pending further
    /// correction.
    RemainingNarrowedClaim,
    /// Open-exception-closure: a backport exception or deferral that still needs an explicit closure decision.
    OpenExceptionClosure,
    /// The train-comparison scope is unclassified, which is disallowed.
    ScopeUnclassified,
}

impl M5TrainComparisonScope {
    /// Every comparison scope, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::CorrectedIssue,
        Self::RemainingNarrowedClaim,
        Self::OpenExceptionClosure,
        Self::ScopeUnclassified,
    ];

    /// The three canonical comparison scopes every train-comparison report must stay distinct across.
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::CorrectedIssue => "corrected_issue",
            Self::RemainingNarrowedClaim => "remaining_narrowed_claim",
            Self::OpenExceptionClosure => "open_exception_closure",
...
/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-correction_report or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StableLineCorrectionReportSurfaceContext {
...
impl M5StableLineCorrectionReportSurfaceContext {
...
/// One mandatory rendered part a line-correction_report or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StableLineCorrectionReportAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-correction_report entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-correction_report entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-correction_report
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line correction_report or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5StableLineCorrectionReportAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "train_comparison_fields",
...
/// line correction_report, a line-downgrade packet, or a degraded line-correction_report / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StableLineCorrectionReportNextAction {
    /// Expand the resolved line correction_report's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5StableLineCorrectionReportNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StableLineCorrectionReportExportField {
...
impl M5StableLineCorrectionReportExportField {
...
/// Reason a line-correction_report entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CorrectionReportEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the correction_report means.
    ...
    /// The resolved line-correction_report object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5CorrectionReportEntryDegradeReason {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "correction_report_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortReportSectionUnclassified => "line_report_section_unclassified",
            Self::DescriptorNotBoundToRegistry => "correction_report_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "correction_report_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "correction_report_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5StableLineCorrectionReportNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5StableLineCorrectionReportNextAction::TraceCanonicalRegistry
            }
            Self::CohortReportSectionUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5StableLineCorrectionReportNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5StableLineCorrectionReportNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5StableLineCorrectionReportNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5TrainComparisonEntryDegradeReason {
...
impl M5TrainComparisonEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::EvidenceScopeUnclassified => "comparison_scope_unclassified",
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
    ...
    pub const fn next_action(self) -> M5StableLineCorrectionReportNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5StableLineCorrectionReportNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_correction_report_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5CorrectionReportEntryResolutionInput {
    /// Stable identity of the line-correction_report-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this correction_report binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.correction_report.public_truth_delta_section`); empty means unstated.
    ...
    pub report_section: M5CorrectionReportKind,
    /// The render / surface context.
    pub surface_context: M5StableLineCorrectionReportSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5StableLineCorrectionReportResolutionForm>,
    ...
    /// True when the behavior traces to the line-correction_report registry (never a hand-copied constant).
...
/// Resolved, export-safe line-correction_report-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedCorrectionReportEntry {
    /// Stable identity of the line-correction_report-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this correction_report binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-correction_report object publishes every required field.
    pub correction_report_object_complete: bool,
    /// Whether the entry traces to the line-correction_report registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5CorrectionReportEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5StableLineCorrectionReportNextAction,
    /// Whether the correction_report resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub correction_report_resolves_across_lines: bool,
}

impl M5ResolvedCorrectionReportEntry {
    /// Whether this line-correction_report entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_train_comparison_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5TrainComparisonEntryResolutionInput {
    ...
    pub comparison_scope: M5TrainComparisonScope,
    /// The render / surface context.
    pub surface_context: M5StableLineCorrectionReportSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5StableLineCorrectionReportResolutionForm>,
    ...
    pub keeps_train_comparison_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedTrainComparisonEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub train_comparison_stays_honest: bool,
    ...
    pub provides_complete_train_comparison: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5TrainComparisonEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5StableLineCorrectionReportNextAction,
...
impl M5ResolvedTrainComparisonEntry {
...
pub enum M5StableLineCorrectionReportResolutionError {
    /// The line-correction_report-entry id was empty.
...
impl M5StableLineCorrectionReportResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_correction_report_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_train_comparison_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5StableLineCorrectionReportResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-correction_report / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5StableLineCorrectionReportResolutionError {}

fn form_tokens(forms: &[M5StableLineCorrectionReportResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5StableLineCorrectionReportResolutionForm]) -> bool {
    let present: BTreeSet<M5StableLineCorrectionReportResolutionForm> =
        forms.iter().copied().collect();
    M5StableLineCorrectionReportResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-correction_report object publishes every required field: classified line journey,
...
pub fn correction_report_object_is_complete(
    journey: M5CorrectionReportKind,
...
/// Whether the line correction_report keeps a line from widening without preserving its rollback and diagnostics
...
pub fn train_comparison_stays_honest(
    scope: M5TrainComparisonScope,
    comparison_is_truthful: bool,
    keeps_train_comparison_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_train_comparison_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-correction_report-registry entry so it stays bound to the line-correction_report registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete correction_report object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_correction_report_entry(
    input: M5CorrectionReportEntryResolutionInput,
) -> Result<M5ResolvedCorrectionReportEntry, M5StableLineCorrectionReportResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5StableLineCorrectionReportResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5StableLineCorrectionReportResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = correction_report_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5CorrectionReportEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5CorrectionReportEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5CorrectionReportEntryDegradeReason::CohortReportSectionUnclassified)
    } else if !input.bound_to_registry {
        Some(M5CorrectionReportEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5CorrectionReportEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5CorrectionReportEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5CorrectionReportEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5CorrectionReportEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5CorrectionReportEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5StableLineCorrectionReportNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedCorrectionReportEntry {
        ...
        correction_report_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        correction_report_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_train_comparison_entry(
    input: M5TrainComparisonEntryResolutionInput,
) -> Result<M5ResolvedTrainComparisonEntry, M5StableLineCorrectionReportResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5StableLineCorrectionReportResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = train_comparison_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_train_comparison_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5TrainComparisonEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5TrainComparisonEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5TrainComparisonEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5TrainComparisonEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5TrainComparisonEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5TrainComparisonEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5StableLineCorrectionReportNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedTrainComparisonEntry {
        entry_id: input.entry_id,
        ...
        keeps_train_comparison_visible: input.keeps_train_comparison_visible,
        ...
        train_comparison_stays_honest: record_stays_honest,
        provides_complete_train_comparison: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-correction_report and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StableLineCorrectionReportTrainComparisonRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5StableLineCorrectionReportTrainComparisonRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5StableLineCorrectionReportAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5StableLineCorrectionReportExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5StableLineProtectionDowngradeTrigger>,
    /// Resolved line-correction_report-registry examples.
    pub correction_report_entries: Vec<M5ResolvedCorrectionReportEntry>,
    /// Resolved line-downgrade-packet examples.
    pub train_comparison_entries: Vec<M5ResolvedTrainComparisonEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-correction_report and
    ...
    pub collapses_distinct_train_comparison_classes_into_one_lane: bool,
}

impl M5StableLineCorrectionReportTrainComparisonRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5StableLineCorrectionReportAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5StableLineCorrectionReportAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5StableLineCorrectionReportExportField> =
            self.export_fields.iter().copied().collect();
        M5StableLineCorrectionReportExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_train_comparison_classes_into_one_lane
    }

    /// True when a clean line-correction_report entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete correction_report object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn correction_report_is_honest(ex: &M5ResolvedCorrectionReportEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.correction_report_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedTrainComparisonEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_train_comparison
                && ex.train_comparison_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.correction_report_entries
            .iter()
            .all(Self::correction_report_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StableLineCorrectionReportTrainComparisonRegistriesVocabularySet {
    ...
    /// Cohort-correction_report-entry degrade-reason tokens.
    pub correction_report_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub train_comparison_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5StableLineCorrectionReportTrainComparisonRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5StableLineProtectionRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5StableLineCorrectionReportResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5CorrectionReportKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5TrainComparisonScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5StableLineCorrectionReportSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            correction_report_degrade_reasons: tokens(
                &M5CorrectionReportEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            train_comparison_degrade_reasons: tokens(
                &M5TrainComparisonEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5StableLineCorrectionReportAnatomyPart::ALL, |v| {
                v.as_str()
            }),
            next_actions: tokens(&M5StableLineCorrectionReportNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5StableLineCorrectionReportExportField::ALL, |v| {
                v.as_str()
            }),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StableLineCorrectionReportTrainComparisonRegistriesGovernanceReview {
    /// The correction_report registry names a canonical token, semantic role, and line journey for every entry.
    pub correction_report_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-correction_report object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_correction_report_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved correction_report.
    ...
    pub train_comparison_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-correction_report and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-correction_report and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub correction_report_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StableLineCorrectionReportTrainComparisonRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-correction_report registry.
    ...
    /// Behavior traces back to the canonical line-correction_report and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-correction_report / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StableLineCorrectionReportTrainComparisonRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StableLineCorrectionReportTrainComparisonRegistriesReleasePosture {
...
/// Constructor input for [`M5StableLineCorrectionReportTrainComparisonRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5StableLineCorrectionReportTrainComparisonRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5StableLineCorrectionReportTrainComparisonRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5StableLineCorrectionReportTrainComparisonRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5StableLineCorrectionReportTrainComparisonRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection:
        M5StableLineCorrectionReportTrainComparisonRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5StableLineCorrectionReportTrainComparisonRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5StableLineCorrectionReportTrainComparisonRegistriesReleasePosture,
...
/// Export-safe M5 line-correction_report and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StableLineCorrectionReportTrainComparisonRegistriesPacket {
    /// Record kind; must equal [`M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_SCHEMA_VERSION`].
...
impl M5StableLineCorrectionReportTrainComparisonRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5StableLineCorrectionReportTrainComparisonRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version:
                M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5StableLineCorrectionReportTrainComparisonRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind
            != M5_STABLE_LINE_CORRECTION_REPORT_TRAIN_COMPARISON_REGISTRIES_RECORD_KIND
        {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::WrongRecordKind,
            );
        }
        ...
        {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::MissingIdentity,
            );
        }
        ...
        if !self.vocabulary_set.matches_canonical() {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::VocabularySetDrift,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-correction_report / line-downgrade-packet registries packet serializes",
        )) {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::RawMaterialInExport,
            );
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,correction_report_entries,train_comparison_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                ...
                row.correction_report_entries.len(),
                row.train_comparison_entries.len(),
                degrades.join("|"),
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Stable-Line Correction-Report and Train-Comparison Registries\n\n");
        ...
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                "  - Correction-report entries: {} / train-comparison entries: {}\n",
                row.correction_report_entries.len(),
                row.train_comparison_entries.len()
            ));
        }
    ...
    /// registry-bound line-correction_report entries are listed.
    pub fn render_correction_report_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.correction_report_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5StableLineCorrectionReportTrainComparisonRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5StableLineCorrectionReportTrainComparisonRegistriesViolation>),
}

impl fmt::Display for M5StableLineCorrectionReportTrainComparisonRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-correction_report / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-correction_report / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5StableLineCorrectionReportTrainComparisonRegistriesArtifactError {}

/// Validation failures emitted by [`M5StableLineCorrectionReportTrainComparisonRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5StableLineCorrectionReportTrainComparisonRegistriesViolation {
    ...
    /// A registry row does not point at both the line-correction_report and line-downgrade-packet domain schemas.
    ...
    /// Cohort-correction_report-resolution is not proven: clean correction_report entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean correction_report entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded correction_report entry is present, or a clean correction_report entry is unbounded
    /// or unbound.
...
impl M5StableLineCorrectionReportTrainComparisonRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => "correction_report_resolution_not_proven",
            Self::RollbackAndDiagnosticsPreservationNotProven => {
                "rollback_and_diagnostics_preservation_not_proven"
            }
            Self::CohortEvidenceIntegrityNotProven => "train_comparison_integrity_not_proven",
...
pub fn current_stable_m5_stable_line_correction_report_and_train_comparison_registries_export(
) -> Result<
    M5StableLineCorrectionReportTrainComparisonRegistriesPacket,
    M5StableLineCorrectionReportTrainComparisonRegistriesArtifactError,
> {
    let packet: M5StableLineCorrectionReportTrainComparisonRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-stable-line-correction-report-and-train-comparison-registries-proof/support_export.json"
        )
    ))
    .map_err(M5StableLineCorrectionReportTrainComparisonRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(
            M5StableLineCorrectionReportTrainComparisonRegistriesArtifactError::Validation(
                violations,
            ),
        )
    }
...
fn validate_source_contracts(
    packet: &M5StableLineCorrectionReportTrainComparisonRegistriesPacket,
    violations: &mut Vec<M5StableLineCorrectionReportTrainComparisonRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations
            .push(M5StableLineCorrectionReportTrainComparisonRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        ...
        if !refs.contains(M5_SUPPORTED_LINE_DEFECT_LEDGER_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_TRAIN_COMPARISON_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.correction_report_entries.is_empty() || row.train_comparison_entries.is_empty() {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::ExamplesMissing,
            );
        }
        if !row.examples_are_honest() {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::DishonestExample,
            );
        }
        if !row.honours_invariants() {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.correction_report_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_correction_report_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.train_comparison_keeps_proof_visible_and_binds_support_language,
        ...
        review.correction_report_or_downgrade_drift_caught_before_release,
        review.every_row_declares_mandatory_anatomy,
        review.reuses_frozen_matrix_vocabulary,
    ] {
        if !ok {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5StableLineCorrectionReportTrainComparisonRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5StableLineCorrectionReportTrainComparisonRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5StableLineCorrectionReportTrainComparisonRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let correction_reports = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.correction_report_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.train_comparison_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean correction_report entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean correction_report entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = correction_reports()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = correction_reports()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5CorrectionReportKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5StableLineCorrectionReportSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = correction_reports().any(|ex| {
        ex.degrade_reason
            == Some(M5CorrectionReportEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete =
        !correction_reports().any(|ex| ex.is_clean() && !ex.correction_report_object_complete);
    if !(journeys_covered
    ...
    {
        violations.push(
            M5StableLineCorrectionReportTrainComparisonRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }

    // AC2: line packets preserve rollback and diagnostics posture before widening. A widen-without-rollback
    // example degrades, an unbound example degrades, at least one clean bounded correction_report entry is present, and
    // no clean correction_report entry is unbounded or unbound.
    let widen_fold_degrades = correction_reports().any(|ex| {
        ex.degrade_reason
            == Some(
                M5CorrectionReportEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = correction_reports().any(|ex| {
        ex.degrade_reason
            == Some(M5CorrectionReportEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_correction_report =
        correction_reports().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound = !correction_reports().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded =
        !correction_reports().any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_correction_report
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
            M5StableLineCorrectionReportTrainComparisonRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5TrainComparisonScope::CANONICAL_SCOPES
        .iter()
    ...
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_train_comparison);
    ...
    {
        violations.push(
            M5StableLineCorrectionReportTrainComparisonRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-correction_report registry
