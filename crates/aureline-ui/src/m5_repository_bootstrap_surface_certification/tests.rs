//! Tests for the M05-1195 repository-bootstrap surface certification capstone.

use super::*;

fn packet() -> RepositoryBootstrapProfileCertificationPacket {
    seeded_m5_repository_bootstrap_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, REPOSITORY_BOOTSTRAP_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, REPOSITORY_BOOTSTRAP_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5RepositoryBootstrapCertifiedProfile::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            RepositoryBootstrapCertificationAxis::ALL.len()
        );
    }
...
fn all_five_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5RepositoryBootstrapA11yClaim> =
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5RepositoryBootstrapCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
...
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = RepositoryBootstrapCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), RepositoryBootstrapCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in RepositoryBootstrapCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == RepositoryBootstrapCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    // The B142 localization axis is present, distinct, and exercised by the
    // disclosed-checkout-plan profile as its binding narrowed axis.
    let p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| {
            r.profile == M5RepositoryBootstrapCertifiedProfile::DisclosedCheckoutPlanProfile
        })
        .expect("disclosed-checkout-plan-profile row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(RepositoryBootstrapCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(RepositoryBootstrapCertificationAxis::Localization)
    );
}

#[test]
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    // The B142 high-contrast reach axis stays certified across the seed (never a binding narrowed axis here).
    for row in &packet().rows {
        let outcome = row
            .axis(RepositoryBootstrapCertificationAxis::HighContrast)
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5RepositoryBootstrapCertifiedProfile::LiveTrustedAcquisitionSurface
        })
        .expect("live-trusted-acquisition-surface row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RepositoryBootstrapCertificationAxis::Visual {
            outcome.state = RepositoryBootstrapAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("source locator silently dropped".to_owned());
    ...
    assert_eq!(
        row.derived_status,
        RepositoryBootstrapProfileClaimStatus::Red
    );
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        RepositoryBootstrapCertificationViolation::ProfileBlocked { .. }
    )));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5RepositoryBootstrapCertifiedProfile::ReviewableAcquisitionStructure
        })
        .expect("reviewable-acquisition-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5RepositoryBootstrapA11yClaim::TrustedAcquisitionSurface;
    row.certified_claim = M5RepositoryBootstrapA11yClaim::TrustedAcquisitionSurface;
    assert_eq!(
        row.derive_status(),
        RepositoryBootstrapProfileClaimStatus::Red
    );
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RepositoryBootstrapCertificationAxis::DegradedState {
            *outcome = RepositoryBootstrapAxisOutcome {
                axis: RepositoryBootstrapCertificationAxis::DegradedState,
                state: RepositoryBootstrapAxisCertificationState::DisclosedNarrowed,
                parity_note: "the repository-bootstrap evidence is lagging".to_owned(),
                narrowing_reason: Some(
                    "the repository-bootstrap evidence is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5RepositoryBootstrapDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableAcquisitionSurface == certified, no claim_auto_narrow.
    assert_eq!(
        row.derive_status(),
        RepositoryBootstrapProfileClaimStatus::Red
    );
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RepositoryBootstrapCertificationAxis::CliExport {
            outcome.state = RepositoryBootstrapAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5RepositoryBootstrapDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(
        row.derive_status(),
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(RepositoryBootstrapClaimAutoNarrow {
        binding_axis: RepositoryBootstrapCertificationAxis::DegradedState,
        from_claim: M5RepositoryBootstrapA11yClaim::TrustedAcquisitionSurface,
        to_claim: M5RepositoryBootstrapA11yClaim::TrustedAcquisitionSurface,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5RepositoryBootstrapA11yClaim::CheckoutPlanDisclosedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-acquisition-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableAcquisitionSurface; certify a stronger trusted acquisition surface.
    row.certified_claim = M5RepositoryBootstrapA11yClaim::TrustedAcquisitionSurface;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = RepositoryBootstrapCertificationAxis::Visual;
    }
    assert_eq!(
        row.derive_status(),
        RepositoryBootstrapProfileClaimStatus::Red
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RepositoryBootstrapCertificationAxis::CliExport {
            outcome.state = RepositoryBootstrapAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the checkout-plan surface".to_owned());
            outcome.downgrade_trigger = Some(M5RepositoryBootstrapDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = RepositoryBootstrapCertificationAxis::CliExport;
    }
    assert_eq!(
        row.derive_status(),
        RepositoryBootstrapProfileClaimStatus::Red
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == RepositoryBootstrapProfileClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "checkout plan".to_owned();
    }
    assert_eq!(
        row.derive_status(),
        RepositoryBootstrapProfileClaimStatus::Red
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(RepositoryBootstrapCertificationAxis::Visual);
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows.retain(|r| {
        r.profile != M5RepositoryBootstrapCertifiedProfile::UnverifiedBootstrapEvidenceProfile
    });
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        RepositoryBootstrapCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the open-local family from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5RepositoryBootstrapFamily::OpenLocal);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        RepositoryBootstrapCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = RepositoryBootstrapProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        RepositoryBootstrapCertificationViolation::WrongCanonicalBundle
    )));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        RepositoryBootstrapCertificationViolation::DuplicateId { .. }
    )));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0].axis_outcomes.retain(|o| {
        o.axis != RepositoryBootstrapCertificationAxis::RepositoryBootstrapComponentTruth
    });
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        RepositoryBootstrapCertificationViolation::SummaryMismatch
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: RepositoryBootstrapProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_repository_bootstrap_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/workspaces/m5-repository-bootstrap-surface-certification/support_export.json"
    ));
    let packet_from_fixture: RepositoryBootstrapProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/workspaces/m5-repository-bootstrap-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_REPOSITORY_BOOTSTRAP_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures
...
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_REPOSITORY_BOOTSTRAP_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_repository_bootstrap_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/release/m5-repository-bootstrap-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-repository-bootstrap-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir =
        repo.join("fixtures/workspaces/m5-repository-bootstrap-surface-certification");
