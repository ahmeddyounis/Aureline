//! Tests for the M05-1163 shell-metric-density surface certification capstone.

use super::*;

fn packet() -> ShellGeometryProfileCertificationPacket {
    seeded_m5_shell_metric_density_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, SHELL_METRIC_DENSITY_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, SHELL_METRIC_DENSITY_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5ShellGeometryCertifiedProfile::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            ShellGeometryCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, ShellGeometryAxisCertificationState::Certified);
...
fn all_five_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5ShellGeometryA11yClaim> =
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5ShellGeometryCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5ShellGeometryCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = ShellGeometryCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), ShellGeometryCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in ShellGeometryCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == ShellGeometryCertificationAxis::CliExport
        );
...
#[test]
fn snapped_width_axis_is_a_distinct_reach_axis() {
    // The B138 snapped-width axis is present, distinct, and exercised by the
    // unverified-responsive-geometry profile as its binding narrowed axis.
    let p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| {
            r.profile == M5ShellGeometryCertifiedProfile::UnverifiedResponsiveGeometrySurface
        })
        .expect("unverified-responsive-geometry-surface row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(ShellGeometryCertificationAxis::SnappedWidth)
        .expect("snapped-width axis present");
    assert_eq!(
        outcome.state,
    ...
    assert_eq!(
        ...
        Some(ShellGeometryCertificationAxis::SnappedWidth)
    );
}

#[test]
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    // The B138 high-contrast reach axis replaces the B137 reduced-motion axis and stays certified
    // across the seed (never a binding narrowed axis here).
    for row in &packet().rows {
        let outcome = row
            .axis(ShellGeometryCertificationAxis::HighContrast)
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5ShellGeometryCertifiedProfile::LiveTrustedGeometrySurface)
        .expect("live-trusted-geometry-surface row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ShellGeometryCertificationAxis::Visual {
            outcome.state = ShellGeometryAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("zone metric silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, ShellGeometryProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ShellGeometryCertificationViolation::ProfileBlocked { .. }
    )));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5ShellGeometryCertifiedProfile::ReviewableGeometryStructure)
        .expect("reviewable-geometry-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails
        .lets_an_extension_or_embedded_surface_set_a_private_fracturing_width = true;
    assert_eq!(row.derive_status(), ShellGeometryProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ShellGeometryCertificationViolation::GuardrailViolated { .. }
    )));
}
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5ShellGeometryA11yClaim::TrustedGeometrySurface;
    row.certified_claim = M5ShellGeometryA11yClaim::TrustedGeometrySurface;
    assert_eq!(row.derive_status(), ShellGeometryProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ShellGeometryCertificationAxis::DegradedState {
            *outcome = ShellGeometryAxisOutcome {
                axis: ShellGeometryCertificationAxis::DegradedState,
                state: ShellGeometryAxisCertificationState::DisclosedNarrowed,
                parity_note: "the shell-metric evidence is lagging".to_owned(),
                narrowing_reason: Some(
                    "the shell-metric evidence is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5ShellGeometryDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableGeometrySurface == certified, no claim_auto_narrow.
    assert_eq!(row.derive_status(), ShellGeometryProfileClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ShellGeometryCertificationAxis::CliExport {
            outcome.state = ShellGeometryAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5ShellGeometryDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), ShellGeometryProfileClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(ShellGeometryClaimAutoNarrow {
        binding_axis: ShellGeometryCertificationAxis::DegradedState,
        from_claim: M5ShellGeometryA11yClaim::TrustedGeometrySurface,
        to_claim: M5ShellGeometryA11yClaim::TrustedGeometrySurface,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5ShellGeometryA11yClaim::DensityModeUnverifiedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-geometry-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableGeometrySurface; certify a stronger trusted geometry surface.
    row.certified_claim = M5ShellGeometryA11yClaim::TrustedGeometrySurface;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5ShellGeometryCertifiedProfile::UnverifiedDensityModeSurface)
        .expect("unverified-density-mode-surface row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = ShellGeometryCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), ShellGeometryProfileClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ShellGeometryCertificationAxis::CliExport {
            outcome.state = ShellGeometryAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the density mode surface".to_owned());
            outcome.downgrade_trigger = Some(M5ShellGeometryDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = ShellGeometryCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), ShellGeometryProfileClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == ShellGeometryProfileClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "density".to_owned();
    }
    assert_eq!(row.derive_status(), ShellGeometryProfileClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(ShellGeometryCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5ShellGeometryDowngradeTrigger::ProofStale,
    );
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.profile != M5ShellGeometryCertifiedProfile::DisclosedCollapsePrioritySurface);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ShellGeometryCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the minimum-size family from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5ShellGeometryFamily::MinimumSize);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ShellGeometryCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = ShellGeometryProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ShellGeometryCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ShellGeometryCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ShellGeometryCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != ShellGeometryCertificationAxis::ShellGeometryComponentTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ShellGeometryCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ShellGeometryCertificationViolation::RawGeometryMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: ShellGeometryProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_shell_metric_density_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/ui/m5-shell-metric-density-surface-certification/support_export.json"
    ));
    let packet_from_fixture: ShellGeometryProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/ui/m5-shell-metric-density-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_SHELL_METRIC_DENSITY_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and
...
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_SHELL_METRIC_DENSITY_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_shell_metric_density_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/release/m5-shell-metric-density-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-shell-metric-density-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/ui/m5-shell-metric-density-surface-certification");
