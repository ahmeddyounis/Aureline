//! Implemented M5 change-object-record and selected-change-binding registries.
//!
//! The frozen [change-object / patch-stack / landing matrix][matrix] names Aureline's governed change-orchestration
//! object classes and locks their controlled vocabulary. This module makes the durable change object operable — as a
//! canonical record — and adds the selected-change binding any broad-scope flow must pass, so the Git, patch-stack /
//! queue, review, provider-landing, help / docs, and support / export surfaces inherit one canonical change-object
//! descriptor rather than a hand-authored parallel prose that has to be kept consistent: it turns the
//! *change-object-record* grammar (one durable record per non-trivial multi-file change — one typed field per record
//! section: the stable change identity, whether it is a bounded working-set patch or a side-branch work unit, the
//! selected worktree ID and base commit or dirty-tree fingerprint, the intent class and affected path set, and the
//! validation plan and checkpoint lineage — each bound to one selected worktree / base identity) and the
//! *selected-change-binding* grammar (the binding a broad-scope flow sits behind — a broad-refactor binding, a
//! migration / import binding, a scaffold / update binding, or a provider-backed-mutation binding) into registry
//! resolvers that produce export-safe, honest projections. Every non-trivial multi-file change then resolves to one
//! typed change-object record — the fields it publishes, its intent class, its selected worktree / base identity, and
//! its validation freshness, all attributable to the change so ambient branch state never reads as a reviewed,
//! landing-ready change — and to one selected-change-binding object — the resolved change identity, the gated broad
//! flow, the selected change object and worktree identity, the binding state, and the active landing-state reason
//! (selected change, stale validation, restack required, queue eligible, queue blocked, protected-branch blocked,
//! orphaned, abandoned, exported, or imported / reopened) — that the Git, patch-stack, provider-landing, and support
//! surfaces can inspect without manual reconstruction, so a broad refactor, migration / import, scaffold / update
//! flow, or provider-backed mutation stays bound to an explicit selected change object instead of running against
//! ambient branch state, and a record that cannot bind a field to its selected worktree / base identity degrades
//! honestly instead of letting ambient branch state read as a reviewed landing candidate.
//!
...
//!   unless it names a canonical registry token, a classified [scope selector][M5ChangeObjectRecordKind],
//!   a disclosure role, covers every [resolution form][M5ChangeObjectRecordResolutionForm] (the
...
//!   [`M5ChangeObjectRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
...
//!   [evaluator binding][M5ChangeObjectResultScope]
...
//!   [`M5ChangeObjectResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
...
pub use seed::{
    seeded_m5_change_object_record_and_selected_change_binding_registries,
    seeded_m5_change_object_record_and_selected_change_binding_registries_review_pack_record_beta_narrowed,
    seeded_m5_change_object_record_and_selected_change_binding_registries_review_pack_result_preview_narrowed,
    M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_PACKET_ID,
};
...
use crate::m5_change_object_patch_stack_and_landing_matrix::{
    ...
    M5_CHANGE_OBJECT_DOMAIN_SCHEMA_REF, M5_CHANGE_ORCHESTRATION_MATRIX_DOC_REF,
    M5_CHANGE_ORCHESTRATION_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5ChangeObjectRecordAndResultRegistriesPacket`].
pub const M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_change_object_record_and_selected_change_binding_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_DOC_REF: &str =
    "docs/team-workflows/m5_change_object_record_and_selected_change_binding_registries.md";
...
pub const M5_SELECTED_CHANGE_BINDING_DOMAIN_SCHEMA_REF: &str =
    "schemas/ui/m5-selected-change-binding.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-change-object-record-and-selected-change-binding-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-change-object-record-and-selected-change-binding-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-change-object-record-and-selected-change-binding-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/teamwork/m5-change-object-record-and-selected-change-binding-registries";
...
pub type M5ChangeObjectRecordAndResultRegistriesConsumerSurface =
...
pub enum M5ChangeObjectRecordResolutionForm {
...
impl M5ChangeObjectRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeObjectRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeObjectResultScope {
...
impl M5ChangeObjectResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeObjectRecordSurfaceContext {
...
impl M5ChangeObjectRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeObjectRecordAnatomyPart {
...
impl M5ChangeObjectRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeObjectRecordNextAction {
...
impl M5ChangeObjectRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeObjectRecordExportField {
...
impl M5ChangeObjectRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeObjectRecordEntryDegradeReason {
...
impl M5ChangeObjectRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5ChangeObjectRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5ChangeObjectRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5ChangeObjectRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5ChangeObjectRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5ChangeObjectRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeObjectResultEntryDegradeReason {
...
impl M5ChangeObjectResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5ChangeObjectRecordNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5ChangeObjectRecordNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5ChangeObjectRecordNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5ChangeObjectRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5ChangeObjectRecordEntryResolutionInput {
    ...
    pub report_section: M5ChangeObjectRecordKind,
    /// The render / surface context.
    pub surface_context: M5ChangeObjectRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ChangeObjectRecordResolutionForm>,
...
pub struct M5ResolvedChangeObjectRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ChangeObjectRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ChangeObjectRecordNextAction,
...
impl M5ResolvedChangeObjectRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ChangeObjectResultEntryResolutionInput {
    ...
    pub comparison_scope: M5ChangeObjectResultScope,
    /// The render / surface context.
    pub surface_context: M5ChangeObjectRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ChangeObjectRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedChangeObjectResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ChangeObjectResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ChangeObjectRecordNextAction,
...
impl M5ResolvedChangeObjectResultEntry {
...
pub enum M5ChangeObjectRecordResolutionError {
...
impl M5ChangeObjectRecordResolutionError {
...
impl fmt::Display for M5ChangeObjectRecordResolutionError {
...
impl Error for M5ChangeObjectRecordResolutionError {}

fn form_tokens(forms: &[M5ChangeObjectRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5ChangeObjectRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5ChangeObjectRecordResolutionForm> = forms.iter().copied().collect();
    M5ChangeObjectRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5ChangeObjectRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5ChangeObjectResultScope,
...
pub fn resolve_review_pack_record_entry(
    input: M5ChangeObjectRecordEntryResolutionInput,
) -> Result<M5ResolvedChangeObjectRecordEntry, M5ChangeObjectRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ChangeObjectRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5ChangeObjectRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ChangeObjectRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ChangeObjectRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5ChangeObjectRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5ChangeObjectRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5ChangeObjectRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5ChangeObjectRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5ChangeObjectRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5ChangeObjectRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5ChangeObjectRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5ChangeObjectRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedChangeObjectRecordEntry {
...
pub fn resolve_review_pack_result_entry(
    input: M5ChangeObjectResultEntryResolutionInput,
) -> Result<M5ResolvedChangeObjectResultEntry, M5ChangeObjectRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ChangeObjectRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ChangeObjectResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ChangeObjectResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5ChangeObjectResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5ChangeObjectResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5ChangeObjectResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5ChangeObjectResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5ChangeObjectRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedChangeObjectResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeObjectRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5ChangeObjectRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5ChangeObjectRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5ChangeObjectRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedChangeObjectRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedChangeObjectResultEntry>,
...
impl M5ChangeObjectRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5ChangeObjectRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5ChangeObjectRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5ChangeObjectRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5ChangeObjectRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedChangeObjectRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedChangeObjectResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeObjectRecordAndResultRegistriesVocabularySet {
...
impl M5ChangeObjectRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ChangeOrchestrationRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5ChangeObjectRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5ChangeObjectRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ChangeObjectResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5ChangeObjectRecordSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5ChangeObjectRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5ChangeObjectResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5ChangeObjectRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5ChangeObjectRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5ChangeObjectRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeObjectRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeObjectRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeObjectRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeObjectRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5ChangeObjectRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ChangeObjectRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5ChangeObjectRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5ChangeObjectRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5ChangeObjectRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5ChangeObjectRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5ChangeObjectRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5ChangeObjectRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeObjectRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION`].
...
impl M5ChangeObjectRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5ChangeObjectRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5ChangeObjectRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND {
            violations.push(M5ChangeObjectRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5ChangeObjectRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5ChangeObjectRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5ChangeObjectRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5ChangeObjectRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Change-Object-Record and Selected-Change-Binding Registries\n\n");
...
#[derive(Debug)]
pub enum M5ChangeObjectRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5ChangeObjectRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5ChangeObjectRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5ChangeObjectRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5ChangeObjectRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5ChangeObjectRecordAndResultRegistriesViolation {
...
impl M5ChangeObjectRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_change_object_record_and_selected_change_binding_registries_export(
) -> Result<
    M5ChangeObjectRecordAndResultRegistriesPacket,
    M5ChangeObjectRecordAndResultRegistriesArtifactError,
> {
    let packet: M5ChangeObjectRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-change-object-record-and-selected-change-binding-registries-proof/support_export.json"
        )
    ))
    .map_err(M5ChangeObjectRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5ChangeObjectRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5ChangeObjectRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5ChangeObjectRecordAndResultRegistriesViolation>,
) {
    let refs: BTreeSet<&str> = packet
        ...
        .collect();
    for required in [
        M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_REF,
        M5_CHANGE_OBJECT_RECORD_AND_RESULT_REGISTRIES_DOC_REF,
        M5_CHANGE_ORCHESTRATION_MATRIX_SCHEMA_REF,
        M5_CHANGE_ORCHESTRATION_MATRIX_DOC_REF,
        M5_CHANGE_OBJECT_DOMAIN_SCHEMA_REF,
        M5_SELECTED_CHANGE_BINDING_DOMAIN_SCHEMA_REF,
    ] {
        if !refs.contains(required) {
            violations
                .push(M5ChangeObjectRecordAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5ChangeObjectRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations
                .push(M5ChangeObjectRecordAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5ChangeObjectRecordAndResultRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5ChangeObjectRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_CHANGE_OBJECT_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_SELECTED_CHANGE_BINDING_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5ChangeObjectRecordAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5ChangeObjectRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5ChangeObjectRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5ChangeObjectRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5ChangeObjectRecordAndResultRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5ChangeObjectRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5ChangeObjectRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5ChangeObjectRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5ChangeObjectRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5ChangeObjectRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5ChangeObjectRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5ChangeObjectRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ChangeObjectResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(
            M5ChangeObjectRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
