...
fn clean_required_evidence_check_input() -> M5RequiredEvidenceCheckEntryResolutionInput {
    M5RequiredEvidenceCheckEntryResolutionInput {
        entry_id: "required_evidence_check:test".to_owned(),
        line_binding_id: "launch.line.core-team-canary".to_owned(),
        token_name: "line.required_evidence_check.core_team_canary".to_owned(),
        semantic_role: M5ReviewPackRole::EvaluatorResultClassDisclosure,
        report_section: M5RequiredEvidenceCheckKind::Required,
        surface_context: M5RequiredEvidenceCheckSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5RequiredEvidenceCheckResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5LocalCiParityEntryResolutionInput {
    M5LocalCiParityEntryResolutionInput {
        ...
        semantic_role: M5ReviewPackRole::EvaluatorResultClassDisclosure,
        comparison_scope: M5LocalCiParityScope::LocalParityEstimateBinding,
        surface_context: M5RequiredEvidenceCheckSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5RequiredEvidenceCheckResolutionForm::ALL.to_vec(),
...
#[test]
fn required_evidence_check_clean_names_meaning_and_is_bound() {
    let resolved =
        resolve_required_evidence_check_entry(clean_required_evidence_check_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.required_evidence_check_resolves_across_lines);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.required_evidence_check_object_complete);
    ...
    assert_eq!(resolved.report_section, "required");
    assert_eq!(resolved.canonical_report_section_mode, "required_mode");
    assert_eq!(resolved.surface_context, "shiproom_surface");
    assert_eq!(
        resolved.next_action,
        M5RequiredEvidenceCheckNextAction::ExpandCohortMeaning
    );
}

#[test]
fn required_evidence_check_token_unstated_degrades() {
    let mut input = clean_required_evidence_check_input();
...
#[test]
fn required_evidence_check_unbound_and_unclassified_degrade() {
    let mut input = clean_required_evidence_check_input();
    ...
    let mut input = clean_required_evidence_check_input();
    input.report_section = M5RequiredEvidenceCheckKind::EvidenceStateUnclassified;
    assert_eq!(
        resolve_required_evidence_check_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn required_evidence_check_object_incomplete_and_widen_fold_and_form_degrade() {
    // An unstated bundle IDs field leaves the resolved object incomplete.
    let mut input = clean_required_evidence_check_input();
    input.bundle_ids = "  ".to_owned();
    let resolved = resolve_required_evidence_check_entry(input).unwrap();
    assert!(!resolved.required_evidence_check_object_complete);
    assert_eq!(
    ...
    assert_eq!(
        resolve_required_evidence_check_entry(input).unwrap().degrade_reason,
        Some(M5RequiredEvidenceCheckEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    );

    let mut input = clean_required_evidence_check_input();
    input.resolution_form_coverage = vec![M5RequiredEvidenceCheckResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_required_evidence_check_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn required_evidence_check_public_facing_and_surface_and_proof_degrade() {
    // A public-facing line running support language ahead of proof first fails the widen-boundary fold.
    let mut input = clean_required_evidence_check_input();
    input.report_section = M5RequiredEvidenceCheckKind::CiOnly;
    ...
    let mut input = clean_required_evidence_check_input();
    input.surface_context = M5RequiredEvidenceCheckSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_required_evidence_check_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn required_evidence_check_empty_id_and_forbidden_material_error() {
    let mut input = clean_required_evidence_check_input();
    input.entry_id = "".to_owned();
    assert_eq!(
        resolve_required_evidence_check_entry(input).unwrap_err(),
        M5RequiredEvidenceCheckResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_required_evidence_check_entry(input).unwrap_err(),
        M5RequiredEvidenceCheckResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5RequiredEvidenceCheckKind::EvidenceStateUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn required_evidence_check_object_is_complete_requires_all_fields() {
    assert!(required_evidence_check_object_is_complete(
        M5RequiredEvidenceCheckKind::Required,
...
fn downgrade_clean_stays_honest() {
    let resolved = resolve_local_ci_parity_entry(clean_downgrade_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.comparison_safe_on_every_line);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.provides_complete_local_ci_parity);
    assert!(resolved.local_ci_parity_stays_honest);
    assert_eq!(resolved.comparison_scope, "local_parity_estimate_binding");
    assert_eq!(resolved.surface_context, "shiproom_surface");
}
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let resolved = resolve_local_ci_parity_entry(input).unwrap();
    assert!(!resolved.provides_complete_local_ci_parity);
    assert_eq!(
        resolved.degrade_reason,
    ...
    let mut input = clean_downgrade_input();
    input.keeps_local_ci_parity_visible = false;
    assert_eq!(
        resolve_local_ci_parity_entry(input).unwrap().degrade_reason,
        Some(M5LocalCiParityEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    );
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5LocalCiParityScope::LocalCiParityUnclassified;
    assert_eq!(
        resolve_local_ci_parity_entry(input).unwrap().degrade_reason,
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    input.entry_id = "  ".to_owned();
    assert_eq!(
        resolve_local_ci_parity_entry(input).unwrap_err(),
        M5RequiredEvidenceCheckResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn downgrade_bound_support_and_flagged_gap_stay_clean() {
    ...
    assert!(resolve_local_ci_parity_entry(input).unwrap().is_clean());
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RequiredEvidenceCheckAndLocalCiParityRegistriesViolation::VocabularySetDrift));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_LOCAL_CI_PARITY_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5RequiredEvidenceCheckAndLocalCiParityRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5RequiredEvidenceCheckAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5RequiredEvidenceCheckExportField::CohortArchetypes);
    assert!(packet.validate().contains(
        &M5RequiredEvidenceCheckAndLocalCiParityRegistriesViolation::MandatoryExportFieldMissing
    ));
}
...
fn examples_missing_fails() {
    let mut packet = seeded_m5_required_evidence_check_and_local_ci_parity_registries();
    packet.registry_rows[0].local_ci_parity_entries.clear();
    assert!(packet
        .validate()
        .contains(&M5RequiredEvidenceCheckAndLocalCiParityRegistriesViolation::ExamplesMissing));
}

#[test]
fn dishonest_clean_example_fails() {
    let mut packet = seeded_m5_required_evidence_check_and_local_ci_parity_registries();
    // Force a clean required_evidence_check entry to also read as object-incomplete — the packet must reject it.
    let row = &mut packet.registry_rows[0];
    row.required_evidence_check_entries[0].degrade_reason = None;
    row.required_evidence_check_entries[0].required_evidence_check_object_complete = false;
    assert!(packet
        .validate()
        .contains(&M5RequiredEvidenceCheckAndLocalCiParityRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        match mutate {
            ...
            _ => row.collapses_distinct_local_ci_parity_classes_into_one_lane = true,
        }
        assert!(packet.validate().contains(
...
#[test]
fn required_evidence_check_not_proven_when_incomplete_example_removed() {
    let mut packet = seeded_m5_required_evidence_check_and_local_ci_parity_registries();
    for row in &mut packet.registry_rows {
        row.required_evidence_check_entries.retain(|ex| {
            ex.degrade_reason
                != Some(M5RequiredEvidenceCheckEntryDegradeReason::CohortDescriptorObjectIncomplete)
        });
    }
...
#[test]
fn required_evidence_check_not_proven_when_surface_collapses() {
    let mut packet = seeded_m5_required_evidence_check_and_local_ci_parity_registries();
    // Drop every clean executive-steering-surface required_evidence_check so the first-consumer surfaces no longer include it.
    for row in &mut packet.registry_rows {
        row.required_evidence_check_entries
...
#[test]
fn local_ci_parity_integrity_not_proven_when_support_ahead_example_removed() {
    let mut packet = seeded_m5_required_evidence_check_and_local_ci_parity_registries();
    for row in &mut packet.registry_rows {
        row.local_ci_parity_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
    ...
    assert!(packet.validate().contains(
        &M5RequiredEvidenceCheckAndLocalCiParityRegistriesViolation::CohortEvidenceIntegrityNotProven
    ));
}

#[test]
fn local_ci_parity_integrity_not_proven_when_scope_dropped() {
    let mut packet = seeded_m5_required_evidence_check_and_local_ci_parity_registries();
    // Drop every clean go-no-go-signoff downgrade so the coverage no longer includes it.
    for row in &mut packet.registry_rows {
        row.local_ci_parity_entries.retain(|ex| {
            !(ex.is_clean() && ex.comparison_scope == "capability_difference_binding")
        });
    }
    assert!(packet.validate().contains(
        &M5RequiredEvidenceCheckAndLocalCiParityRegistriesViolation::CohortEvidenceIntegrityNotProven
...
fn governance_review_incomplete_fails() {
    ...
    assert!(packet.validate().contains(
        &M5RequiredEvidenceCheckAndLocalCiParityRegistriesViolation::GovernanceReviewIncomplete
    ));
}
...
fn consumer_projection_incomplete_fails() {
    ...
    assert!(packet.validate().contains(
        &M5RequiredEvidenceCheckAndLocalCiParityRegistriesViolation::ConsumerProjectionIncomplete
    ));
}
...
fn export_carries_no_forbidden_raw_material() {
    let json =
        seeded_m5_required_evidence_check_and_local_ci_parity_registries().export_safe_json();
...
#[test]
fn required_evidence_check_table_lists_only_clean_required_evidence_checks() {
    let packet = seeded_m5_required_evidence_check_and_local_ci_parity_registries();
    let table = packet.render_required_evidence_check_table();
    // The clean canary and migration required_evidence_checks are rendered from the registry.
    assert!(table.contains("required_mode"));
    assert!(table.contains("optional_mode"));
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_required_evidence_check_and_local_ci_parity_registries_export().expect(
        "checked M5 line-required_evidence_check / line-downgrade-packet registries export validates",
    );
    assert_eq!(
        from_disk.packet_id,
        M5_REQUIRED_EVIDENCE_CHECK_AND_LOCAL_CI_PARITY_REGISTRIES_PACKET_ID
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5RequiredEvidenceCheckAndLocalCiParityRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-required-evidence-check-and-local-ci-parity-registries/required_evidence_check_beta_narrowed.json"
    )))
    .expect("line-required_evidence_check fixture parses");
    ...
    let preview: M5RequiredEvidenceCheckAndLocalCiParityRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-required-evidence-check-and-local-ci-parity-registries/local_ci_parity_preview_narrowed.json"
    )))
