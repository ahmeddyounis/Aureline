//! Tests for the M05-1228 stable-line surface certification capstone.

use super::*;

fn packet() -> StableLineProtectionProfileCertificationPacket {
    seeded_m5_stable_line_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, STABLE_LINE_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, STABLE_LINE_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5StableLineProtectionCertifiedProfile::ALL {
...
fn every_frozen_family_is_certified_on_some_profile() {
    ...
    for family in M5StableLineProtectionLine::ALL {
        assert!(
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            StableLineProtectionCertificationAxis::ALL.len()
        );
    }
...
fn a_narrowed_profile_never_keeps_a_trusted_claim() {
    for row in &packet().rows {
        if row.is_claim_narrowed() {
            assert!(
                !row.certified_claim.asserts_certified_operating_line(),
...
fn all_five_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5StableLineProtectionClaim> =
        p.rows.iter().map(|r| r.certified_claim).collect();
    for claim in M5StableLineProtectionClaim::ALL {
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5StableLineProtectionCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
...
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = StableLineProtectionCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), StableLineProtectionCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in StableLineProtectionCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == StableLineProtectionCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    // The B146 localization axis is present, distinct, and exercised by the
    // disclosed-correction-packet profile as its binding narrowed axis.
    let p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| {
            r.profile == M5StableLineProtectionCertifiedProfile::DisclosedCorrectionOwnershipProfile
        })
        .expect("disclosed-correction-packet-profile row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(StableLineProtectionCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(StableLineProtectionCertificationAxis::Localization)
    );
}

#[test]
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    // The B146 high-contrast reach axis stays certified across the seed (never a binding narrowed axis here).
    for row in &packet().rows {
        let outcome = row
            .axis(StableLineProtectionCertificationAxis::HighContrast)
...
fn the_three_yellow_rows_bind_three_distinct_axes_and_triggers() {
    ...
    // per-lane concern (correction-packet scope / bundle currentness / lts-readiness evidence).
    let p = packet();
    let axes: BTreeSet<StableLineProtectionCertificationAxis> = p
        .rows
        .iter()
    ...
    let triggers: std::collections::HashSet<M5StableLineProtectionDowngradeTrigger> = p
        .rows
        .iter()
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5StableLineProtectionCertifiedProfile::LiveSupportedLineOperatingLane
        })
        .expect("live-certified-widening-lane row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == StableLineProtectionCertificationAxis::Visual {
            outcome.state = StableLineProtectionAxisCertificationState::UndisclosedDrift;
    ...
    assert_eq!(
        row.derived_status,
        StableLineProtectionProfileClaimStatus::Red
    );
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        StableLineProtectionCertificationViolation::ProfileBlocked { .. }
    )));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5StableLineProtectionCertifiedProfile::ReviewableStableLineStructure
        })
        .expect("reviewable-stable-line-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5StableLineProtectionClaim::CertifiedOperatingLine;
    row.certified_claim = M5StableLineProtectionClaim::CertifiedOperatingLine;
    assert_eq!(
        row.derive_status(),
        StableLineProtectionProfileClaimStatus::Red
    );
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == StableLineProtectionCertificationAxis::DegradedState {
            *outcome = StableLineProtectionAxisOutcome {
                axis: StableLineProtectionCertificationAxis::DegradedState,
                state: StableLineProtectionAxisCertificationState::DisclosedNarrowed,
                parity_note: "the stable-line evidence is lagging".to_owned(),
                narrowing_reason: Some(
                    "the stable-line evidence is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5StableLineProtectionDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableStableLineSurface == certified, no claim_auto_narrow.
    assert_eq!(
        row.derive_status(),
        StableLineProtectionProfileClaimStatus::Red
    );
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == StableLineProtectionCertificationAxis::CliExport {
            outcome.state = StableLineProtectionAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5StableLineProtectionDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(
        row.derive_status(),
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(StableLineProtectionClaimAutoNarrow {
        binding_axis: StableLineProtectionCertificationAxis::DegradedState,
        from_claim: M5StableLineProtectionClaim::CertifiedOperatingLine,
        to_claim: M5StableLineProtectionClaim::CertifiedOperatingLine,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5StableLineProtectionClaim::CorrectionOwnershipDisclosedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-stable-line-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableStableLineSurface; certify a stronger certified operating line.
    row.certified_claim = M5StableLineProtectionClaim::CertifiedOperatingLine;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = StableLineProtectionCertificationAxis::Visual;
    }
    assert_eq!(
        row.derive_status(),
        StableLineProtectionProfileClaimStatus::Red
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == StableLineProtectionCertificationAxis::CliExport {
            outcome.state = StableLineProtectionAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the correction-packet surface".to_owned());
            outcome.downgrade_trigger = Some(M5StableLineProtectionDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = StableLineProtectionCertificationAxis::CliExport;
    }
    assert_eq!(
        row.derive_status(),
        StableLineProtectionProfileClaimStatus::Red
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == StableLineProtectionProfileClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "support window".to_owned();
    }
    assert_eq!(
        row.derive_status(),
        StableLineProtectionProfileClaimStatus::Red
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(StableLineProtectionCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5StableLineProtectionDowngradeTrigger::ProofStale,
    );
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows.retain(|r| {
        r.profile != M5StableLineProtectionCertifiedProfile::UnverifiedLtsReadinessProfile
    });
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        StableLineProtectionCertificationViolation::ProfileCoverageIncomplete
    )));
}
...
fn missing_family_coverage_is_rejected() {
    ...
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5StableLineProtectionLine::FreshStableLine);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        StableLineProtectionCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = StableLineProtectionProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        StableLineProtectionCertificationViolation::WrongCanonicalBundle
    )));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        StableLineProtectionCertificationViolation::DuplicateId { .. }
    )));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != StableLineProtectionCertificationAxis::StableLineComponentTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        StableLineProtectionCertificationViolation::SummaryMismatch
    )));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        StableLineProtectionCertificationViolation::RawStableLineMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: StableLineProtectionProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_stable_line_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-stable-line-surface-certification/support_export.json"
    ));
    let packet_from_fixture: StableLineProtectionProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-stable-line-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_STABLE_LINE_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures
...
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_STABLE_LINE_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_stable_line_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/release/m5-stable-line-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-stable-line-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/release/m5-stable-line-surface-certification");
