//! Implemented M5 change-intent-record and start-work-sheet registries.
//!
//! The frozen [change-intent lifecycle matrix][matrix] names Aureline's governed change-intent object classes and
//! locks their controlled vocabulary. This module makes the first two of those classes — a durable change-intent
//! record and the start-work sheet that launches work from it — operable so the work-item, start-work, linked-change,
//! provider-handoff, help / docs, and support / export surfaces inherit one canonical change-intent descriptor rather
//! than a hand-authored parallel prose that has to be kept consistent: it turns the *change-intent-record* grammar
//! (one durable record per tracked work item — one typed field per record section: the canonical work-item identity
//! and provider ownership, the linked workspace / root and branch / worktree refs, the optional linked review target
//! and task / test preset refs, the actor lineage, and the local-versus-provider commit state — each bound to one
//! provider ownership with its commit-state lineage) and the *start-work-sheet* grammar (the side-effect disclosure a
//! start-work action sits in — a create-new-linked-branch / worktree disclosure, a link-existing-branch-or-review
//! disclosure, or a provider-link mutation disclosure) into registry resolvers that produce export-safe, honest
//! projections. Every tracked item then resolves to one typed change-intent record — the fields it publishes, its
//! actor lineage, its provider ownership, and its commit state, all attributable to the work item so a local-only
//! draft never reads as a provider-committed update — and to one start-work-sheet object — the resolved work-item
//! identity, the affected side-effect field, the previous-versus-current disclosure-state reference, the binding
//! state, and the active commit-state reason (provider-committed, local-only draft, queued for publish, offline
//! handoff packet, or stale relative to provider) — that the work-item, start-work, provider-handoff, and support
//! surfaces can inspect without manual reconstruction, so a created branch, worktree, review draft, or provider link
//! stays visible as a typed disclosure instead of a silent mutation, and a record that cannot bind a field to its
//! work-item identity degrades honestly instead of leaving a local handoff to read as a committed, authoritative
//! tracked-item update.
//!
...
//!   unless it names a canonical registry token, a classified [scope selector][M5ChangeIntentRecordKind],
//!   a disclosure role, covers every [resolution form][M5ChangeIntentRecordResolutionForm] (the
...
//!   [`M5ChangeIntentRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
...
//!   [evaluator binding][M5ChangeIntentResultScope]
...
//!   [`M5ChangeIntentResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
...
//! role vocabulary and the [`M5ChangeIntentConsumerSurface`] consumer-surface taxonomy — so the review detail,
...
pub use seed::{
    seeded_m5_change_intent_record_and_start_work_registries,
    seeded_m5_change_intent_record_and_start_work_registries_review_pack_record_beta_narrowed,
    seeded_m5_change_intent_record_and_start_work_registries_review_pack_result_preview_narrowed,
    M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5ChangeIntentRecordAndResultRegistriesPacket`].
pub const M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_change_intent_record_and_start_work_sheet_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_REF: &str =
    "schemas/teamwork/m5-change-intent-record-and-start-work-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_DOC_REF: &str =
    "docs/team-workflows/m5_change_intent_record_and_start_work_registries.md";
...
pub const M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-change-intent-record-and-start-work-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-change-intent-record-and-start-work-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-change-intent-record-and-start-work-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/teamwork/m5-change-intent-record-and-start-work-registries";
...
pub type M5ChangeIntentRecordAndResultRegistriesConsumerSurface = M5ChangeIntentConsumerSurface;
...
pub enum M5ChangeIntentRecordResolutionForm {
...
impl M5ChangeIntentRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentResultScope {
...
impl M5ChangeIntentResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentRecordSurfaceContext {
...
impl M5ChangeIntentRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentRecordAnatomyPart {
...
impl M5ChangeIntentRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentRecordNextAction {
...
impl M5ChangeIntentRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentRecordExportField {
...
impl M5ChangeIntentRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentRecordEntryDegradeReason {
...
impl M5ChangeIntentRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5ChangeIntentRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5ChangeIntentRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5ChangeIntentRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5ChangeIntentRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5ChangeIntentRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentResultEntryDegradeReason {
...
impl M5ChangeIntentResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5ChangeIntentRecordNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5ChangeIntentRecordNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5ChangeIntentRecordNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5ChangeIntentRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5ChangeIntentRecordEntryResolutionInput {
    ...
    pub report_section: M5ChangeIntentRecordKind,
    /// The render / surface context.
    pub surface_context: M5ChangeIntentRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ChangeIntentRecordResolutionForm>,
...
pub struct M5ResolvedChangeIntentRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ChangeIntentRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ChangeIntentRecordNextAction,
...
impl M5ResolvedChangeIntentRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ChangeIntentResultEntryResolutionInput {
    ...
    pub comparison_scope: M5ChangeIntentResultScope,
    /// The render / surface context.
    pub surface_context: M5ChangeIntentRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ChangeIntentRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedChangeIntentResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ChangeIntentResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ChangeIntentRecordNextAction,
...
impl M5ResolvedChangeIntentResultEntry {
...
pub enum M5ChangeIntentRecordResolutionError {
...
impl M5ChangeIntentRecordResolutionError {
...
impl fmt::Display for M5ChangeIntentRecordResolutionError {
...
impl Error for M5ChangeIntentRecordResolutionError {}

fn form_tokens(forms: &[M5ChangeIntentRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5ChangeIntentRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5ChangeIntentRecordResolutionForm> = forms.iter().copied().collect();
    M5ChangeIntentRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5ChangeIntentRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5ChangeIntentResultScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedChangeIntentRecordEntry, M5ChangeIntentRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ChangeIntentRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5ChangeIntentRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ChangeIntentRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ChangeIntentRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5ChangeIntentRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5ChangeIntentRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5ChangeIntentRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5ChangeIntentRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5ChangeIntentRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5ChangeIntentRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5ChangeIntentRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5ChangeIntentRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedChangeIntentRecordEntry {
...
pub fn resolve_review_pack_result_entry(
    input: M5ChangeIntentResultEntryResolutionInput,
) -> Result<M5ResolvedChangeIntentResultEntry, M5ChangeIntentRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ChangeIntentRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ChangeIntentResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ChangeIntentResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5ChangeIntentResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5ChangeIntentResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5ChangeIntentResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5ChangeIntentResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5ChangeIntentRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedChangeIntentResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5ChangeIntentRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5ChangeIntentRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5ChangeIntentRecordExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5ChangeIntentDowngradeTrigger>,
    /// Resolved line-review_pack_record-registry examples.
    pub review_pack_record_entries: Vec<M5ResolvedChangeIntentRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedChangeIntentResultEntry>,
...
impl M5ChangeIntentRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5ChangeIntentRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5ChangeIntentRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5ChangeIntentRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5ChangeIntentRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedChangeIntentRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedChangeIntentResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentRecordAndResultRegistriesVocabularySet {
...
impl M5ChangeIntentRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ChangeIntentRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5ChangeIntentRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5ChangeIntentRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ChangeIntentResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5ChangeIntentRecordSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5ChangeIntentRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5ChangeIntentResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5ChangeIntentRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5ChangeIntentRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5ChangeIntentRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5ChangeIntentRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ChangeIntentRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5ChangeIntentRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5ChangeIntentRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5ChangeIntentRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5ChangeIntentRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5ChangeIntentRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5ChangeIntentRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION`].
...
impl M5ChangeIntentRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5ChangeIntentRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5ChangeIntentRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND {
            violations.push(M5ChangeIntentRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_CHANGE_INTENT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5ChangeIntentRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5ChangeIntentRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5ChangeIntentRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5ChangeIntentRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Change-Intent-Record and Start-Work-Sheet Registries\n\n");
...
#[derive(Debug)]
pub enum M5ChangeIntentRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5ChangeIntentRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5ChangeIntentRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5ChangeIntentRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5ChangeIntentRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5ChangeIntentRecordAndResultRegistriesViolation {
...
impl M5ChangeIntentRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_change_intent_record_and_start_work_registries_export() -> Result<
    M5ChangeIntentRecordAndResultRegistriesPacket,
    M5ChangeIntentRecordAndResultRegistriesArtifactError,
> {
    let packet: M5ChangeIntentRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-change-intent-record-and-start-work-registries-proof/support_export.json"
        )
    ))
    .map_err(M5ChangeIntentRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5ChangeIntentRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5ChangeIntentRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5ChangeIntentRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations
                .push(M5ChangeIntentRecordAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5ChangeIntentRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations
                .push(M5ChangeIntentRecordAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5ChangeIntentRecordAndResultRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5ChangeIntentRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        ...
        if !refs.contains(M5_CHANGE_INTENT_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_START_WORK_SHEET_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5ChangeIntentRecordAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5ChangeIntentRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5ChangeIntentRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5ChangeIntentRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5ChangeIntentRecordAndResultRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5ChangeIntentRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5ChangeIntentRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5ChangeIntentRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5ChangeIntentRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5ChangeIntentRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5ChangeIntentRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5ChangeIntentRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ChangeIntentResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(
            M5ChangeIntentRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
