...
fn clean_publish_later_draft_input() -> M5PublishLaterDraftEntryResolutionInput {
    M5PublishLaterDraftEntryResolutionInput {
        entry_id: "publish_later_draft:test".to_owned(),
        line_binding_id: "launch.line.core-team-canary".to_owned(),
        token_name: "line.publish_later_draft.core_team_canary".to_owned(),
        semantic_role: M5AiReviewAssistRole::FindingClassification,
        report_section: M5PublishLaterDraftKind::ProviderWriteMissing,
        surface_context: M5PublishLaterDraftSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5PublishLaterDraftResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5CompareReconcileReviewEntryResolutionInput {
    M5CompareReconcileReviewEntryResolutionInput {
        ...
        semantic_role: M5AiReviewAssistRole::FindingClassification,
        comparison_scope: M5CompareReconcileReviewScope::ReconciledPublishReady,
        surface_context: M5PublishLaterDraftSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5PublishLaterDraftResolutionForm::ALL.to_vec(),
...
#[test]
fn publish_later_draft_clean_names_meaning_and_is_bound() {
    let resolved = resolve_publish_later_draft_entry(clean_publish_later_draft_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.publish_later_draft_resolves_across_lines);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.publish_later_draft_object_complete);
    ...
    assert_eq!(resolved.report_section, "provider_write_missing");
    assert_eq!(
        resolved.canonical_report_section_mode,
        "provider_write_missing_mode"
    );
    assert_eq!(resolved.surface_context, "shiproom_surface");
    assert_eq!(
        resolved.next_action,
        M5PublishLaterDraftNextAction::ExpandCohortMeaning
    );
}

#[test]
fn publish_later_draft_token_unstated_degrades() {
    let mut input = clean_publish_later_draft_input();
    input.token_name = "   ".to_owned();
    assert_eq!(
        resolve_publish_later_draft_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn publish_later_draft_unbound_and_unclassified_degrade() {
    let mut input = clean_publish_later_draft_input();
    ...
    let mut input = clean_publish_later_draft_input();
    input.report_section = M5PublishLaterDraftKind::PublishContinuityUnclassified;
    assert_eq!(
        resolve_publish_later_draft_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn publish_later_draft_object_incomplete_and_widen_fold_and_form_degrade() {
    // An unstated bundle IDs field leaves the resolved object incomplete.
    let mut input = clean_publish_later_draft_input();
    input.bundle_ids = "  ".to_owned();
    let resolved = resolve_publish_later_draft_entry(input).unwrap();
    assert!(!resolved.publish_later_draft_object_complete);
    assert_eq!(
    ...
    assert_eq!(
        resolve_publish_later_draft_entry(input).unwrap().degrade_reason,
        Some(M5PublishLaterDraftEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    );

    let mut input = clean_publish_later_draft_input();
    input.resolution_form_coverage = vec![M5PublishLaterDraftResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_publish_later_draft_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn publish_later_draft_public_facing_and_surface_and_proof_degrade() {
    // A public-facing line running support language ahead of proof first fails the widen-boundary fold.
    let mut input = clean_publish_later_draft_input();
    input.report_section = M5PublishLaterDraftKind::PublishLaterQueued;
    ...
    let mut input = clean_publish_later_draft_input();
    input.surface_context = M5PublishLaterDraftSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_publish_later_draft_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn publish_later_draft_empty_id_and_forbidden_material_error() {
    let mut input = clean_publish_later_draft_input();
    input.entry_id = "".to_owned();
    assert_eq!(
        resolve_publish_later_draft_entry(input).unwrap_err(),
        M5PublishLaterDraftResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_publish_later_draft_entry(input).unwrap_err(),
        M5PublishLaterDraftResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5PublishLaterDraftKind::PublishContinuityUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn publish_later_draft_object_is_complete_requires_all_fields() {
    assert!(publish_later_draft_object_is_complete(
        M5PublishLaterDraftKind::ProviderWriteMissing,
...
fn downgrade_clean_stays_honest() {
    let resolved = resolve_compare_reconcile_review_entry(clean_downgrade_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.comparison_safe_on_every_line);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.provides_complete_compare_reconcile_review);
    assert!(resolved.compare_reconcile_review_stays_honest);
    assert_eq!(resolved.comparison_scope, "reconciled_publish_ready");
    assert_eq!(resolved.surface_context, "shiproom_surface");
}
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let resolved = resolve_compare_reconcile_review_entry(input).unwrap();
    assert!(!resolved.provides_complete_compare_reconcile_review);
    assert_eq!(
        resolved.degrade_reason,
    ...
    let mut input = clean_downgrade_input();
    input.keeps_compare_reconcile_review_visible = false;
    assert_eq!(
        resolve_compare_reconcile_review_entry(input).unwrap().degrade_reason,
        Some(M5CompareReconcileReviewEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    );
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5CompareReconcileReviewScope::ReconcileScopeUnclassified;
    assert_eq!(
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    input.entry_id = "  ".to_owned();
    assert_eq!(
        resolve_compare_reconcile_review_entry(input).unwrap_err(),
        M5PublishLaterDraftResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn downgrade_bound_support_and_flagged_gap_stay_clean() {
    ...
    assert!(resolve_compare_reconcile_review_entry(input)
        .unwrap()
        .is_clean());
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5AiReviewPublishContinuityAndReconcileRegistriesViolation::VocabularySetDrift));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5PublishLaterDraftAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5PublishLaterDraftExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5AiReviewPublishContinuityAndReconcileRegistriesViolation::ExamplesMissing));
}

#[test]
fn dishonest_clean_example_fails() {
    let mut packet = seeded_m5_ai_review_publish_continuity_and_reconcile_registries();
    // Force a clean publish_later_draft entry to also read as object-incomplete — the packet must reject it.
    let row = &mut packet.registry_rows[0];
    row.publish_later_draft_entries[0].degrade_reason = None;
    row.publish_later_draft_entries[0].publish_later_draft_object_complete = false;
    assert!(packet
        .validate()
        .contains(&M5AiReviewPublishContinuityAndReconcileRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        match mutate {
            ...
            _ => row.collapses_distinct_compare_reconcile_review_classes_into_one_lane = true,
        }
        assert!(packet.validate().contains(
...
#[test]
fn publish_later_draft_not_proven_when_incomplete_example_removed() {
    let mut packet = seeded_m5_ai_review_publish_continuity_and_reconcile_registries();
    for row in &mut packet.registry_rows {
        row.publish_later_draft_entries.retain(|ex| {
            ex.degrade_reason
                != Some(M5PublishLaterDraftEntryDegradeReason::CohortDescriptorObjectIncomplete)
        });
    }
...
#[test]
fn publish_later_draft_not_proven_when_surface_collapses() {
    let mut packet = seeded_m5_ai_review_publish_continuity_and_reconcile_registries();
    // Drop every clean executive-steering-surface publish_later_draft so the first-consumer surfaces no longer include it.
    for row in &mut packet.registry_rows {
        row.publish_later_draft_entries
...
#[test]
fn compare_reconcile_review_integrity_not_proven_when_support_ahead_example_removed() {
    let mut packet = seeded_m5_ai_review_publish_continuity_and_reconcile_registries();
    for row in &mut packet.registry_rows {
        row.compare_reconcile_review_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
...
#[test]
fn compare_reconcile_review_integrity_not_proven_when_scope_dropped() {
    let mut packet = seeded_m5_ai_review_publish_continuity_and_reconcile_registries();
    // Drop every clean go-no-go-signoff downgrade so the coverage no longer includes it.
    for row in &mut packet.registry_rows {
        row.compare_reconcile_review_entries
            .retain(|ex| !(ex.is_clean() && ex.comparison_scope == "provider_edit_race_reconcile"));
    }
    assert!(packet.validate().contains(
        &M5AiReviewPublishContinuityAndReconcileRegistriesViolation::CohortEvidenceIntegrityNotProven
    ));
...
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_ai_review_publish_continuity_and_reconcile_registries().export_safe_json();
...
#[test]
fn publish_later_draft_table_lists_only_clean_publish_later_drafts() {
    let packet = seeded_m5_ai_review_publish_continuity_and_reconcile_registries();
    let table = packet.render_publish_later_draft_table();
    // The clean canary and migration publish_later_drafts are rendered from the registry.
    assert!(table.contains("provider_write_missing_mode"));
    assert!(table.contains("kept_local_draft_mode"));
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_ai_review_publish_continuity_and_reconcile_registries_export(
    )
    .expect(
        "checked M5 line-publish_later_draft / line-downgrade-packet registries export validates",
    );
    assert_eq!(
        from_disk.packet_id,
        M5_AI_REVIEW_PUBLISH_CONTINUITY_AND_RECONCILE_REGISTRIES_PACKET_ID
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5AiReviewPublishContinuityAndReconcileRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-ai-review-publish-continuity-and-reconcile-registries/publish_later_draft_beta_narrowed.json"
    )))
    .expect("line-publish_later_draft fixture parses");
    ...
    let preview: M5AiReviewPublishContinuityAndReconcileRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-ai-review-publish-continuity-and-reconcile-registries/compare_reconcile_review_preview_narrowed.json"
    )))
