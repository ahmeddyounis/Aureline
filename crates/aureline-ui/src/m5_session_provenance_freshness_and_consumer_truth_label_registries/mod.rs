//! Implemented M5 session-provenance-freshness and consumer-truth-label registries.
//!
//! The frozen [collaboration-state authority matrix][matrix] names Aureline's governed shared-object classes and
//! locks their authority, convergence, and export vocabulary. Building on the CRDT-backed shared-text domain (the
//! live shared session state that other surfaces read from) and the sealed-session-archive domain it freezes, this
//! module keeps *collaboration-derived context* from masquerading as canonical repository truth when search, AI
//! context, review panes, docs links, and support packets reuse it: whenever a consumer reads collaboration state,
//! the provenance class (live session state, captured snapshot, archived session artifact, or canonical Git / VFS
//! truth), the freshness or archive class, the source session and source link, and the actor provenance are recorded
//! first, and session truth may enrich repo truth but never presents itself as canonical Git / VFS state without an
//! explicit label and source link. So the shared editor replica view, the presence / cursor layer, the comment /
//! annotation / review-pin layer, the collaboration degradation banner, and the support / export and help / docs
//! surfaces inherit one canonical provenance-freshness-label record rather than a hand-authored parallel prose that
//! has to be kept consistent: it turns the *provenance-freshness-label* grammar (one durable append-only record per
//! piece of collaboration-derived context a consumer reads — one typed field per record section: the provenance
//! class of the source, the freshness / archive class it carries, the source session and source link, the actor
//! provenance of the capture, and whether it may enrich but never masquerade as canonical repo truth, each bound to
//! the source it never relabels as canonical by default) and the *consumer-truth-disposition* grammar (the
//! disposition a consumer takes on that context — label-as-session-derived, link-to-source, enrich-repo-truth,
//! block-as-canonical, or defer — plus its actor / time provenance, its provenance / freshness facts, and its
//! canonical-versus-session lineage) into registry resolvers that produce export-safe, honest projections. Every
//! piece of consumed collaboration state then resolves to one typed provenance-freshness-label record — the fields it
//! publishes, the provenance class, the freshness / archive class, the source link, and the actor provenance, all
//! attributable so session state is never silently promoted to canonical truth — and to one consumer-truth-disposition
//! descriptor — the disposition, its actor / time provenance, and its canonical-versus-session lineage — that the
//! search, AI, review-pin, degradation-banner, and support surfaces can inspect before labelling, linking, enriching,
//! or blocking, so a record that cannot keep session truth distinct from canonical truth degrades honestly instead of
//! letting collaboration-derived context read as canonical repository state when it lacks a provenance label or source
//! link.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable provenance-freshness-label record per consumed context, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [provenance class][M5SessionProvenanceFreshnessRecordKind],
//!   a disclosure role, covers every [resolution form][M5SessionProvenanceFreshnessRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (the
//!   provenance class, the freshness / archive class and source link, the actor provenance, and the source it is
//!   bound to), keeps its lineage
//!   attributable, and keeps the source it never relabels as canonical bound to the record; otherwise it degrades.
//! * **Never let session state masquerade as canonical truth; keep the provenance label and source link first.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a record whose
//!   provenance-label-first ordering is not held (collaboration-derived context presenting itself as canonical while
//!   its provenance label or source link was dropped) so it degrades to
//!   [`M5SessionProvenanceFreshnessRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and any consumer — a search hit, an AI context window, or a docs link — that
//!   would present session state as canonical degrades the same way — the structured blocker reason an
//!   unlabelled-canonical-promotion attempt must surface.
//! * **Offer a reviewable consumer disposition with actor / time provenance, instead of a silent canonical promotion.**
//!   [`resolve_review_pack_result_entry`] names a classified
//!   [disposition binding][M5SessionProvenanceFreshnessResultScope]
//!   (label-as-session-derived binding, link-to-source binding, or block-as-canonical binding), requires the full
...
//!   [`M5SessionProvenanceFreshnessResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the disposition would promote session state to canonical without a label and source link, act without actor / time provenance, or
//!   drop the source link, so a disposition can never read as trustworthy when it has quietly promoted
//!   session state to canonical or dropped its provenance.
//!
...
//! export surfaces can never fork their own provenance / freshness meaning. Raw secrets, raw command text, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_session_provenance_freshness_and_consumer_truth_label_registries,
    seeded_m5_session_provenance_freshness_and_consumer_truth_label_registries_review_pack_record_beta_narrowed,
    seeded_m5_session_provenance_freshness_and_consumer_truth_label_registries_review_pack_result_preview_narrowed,
    M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5SessionProvenanceFreshnessRecordAndResultRegistriesPacket`].
pub const M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_session_provenance_freshness_and_consumer_truth_label_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_SCHEMA_VERSION: u32 =
    1;

/// Repo-relative path of the combined registries schema.
pub const M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_session_provenance_freshness_and_consumer_truth_label_registries.md";

/// Repo-relative path of the checked support-export artifact.
pub const M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-session-provenance-freshness-and-consumer-truth-label-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-session-provenance-freshness-and-consumer-truth-label-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-session-provenance-freshness-and-consumer-truth-label-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-session-provenance-freshness-and-consumer-truth-label-registries";
...
pub type M5SessionProvenanceFreshnessRecordAndResultRegistriesConsumerSurface =
...
pub enum M5SessionProvenanceFreshnessRecordResolutionForm {
...
impl M5SessionProvenanceFreshnessRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionProvenanceFreshnessRecordKind {
...
impl M5SessionProvenanceFreshnessRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionProvenanceFreshnessResultScope {
...
impl M5SessionProvenanceFreshnessResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionProvenanceFreshnessRecordSurfaceContext {
...
impl M5SessionProvenanceFreshnessRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionProvenanceFreshnessRecordAnatomyPart {
...
impl M5SessionProvenanceFreshnessRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionProvenanceFreshnessRecordNextAction {
...
impl M5SessionProvenanceFreshnessRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionProvenanceFreshnessRecordExportField {
...
impl M5SessionProvenanceFreshnessRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionProvenanceFreshnessRecordEntryDegradeReason {
...
impl M5SessionProvenanceFreshnessRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5SessionProvenanceFreshnessRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5SessionProvenanceFreshnessRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5SessionProvenanceFreshnessRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5SessionProvenanceFreshnessRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => {
                M5SessionProvenanceFreshnessRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionProvenanceFreshnessResultEntryDegradeReason {
...
impl M5SessionProvenanceFreshnessResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
...
pub struct M5SessionProvenanceFreshnessRecordEntryResolutionInput {
    ...
    pub report_section: M5SessionProvenanceFreshnessRecordKind,
    /// The render / surface context.
    pub surface_context: M5SessionProvenanceFreshnessRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SessionProvenanceFreshnessRecordResolutionForm>,
...
pub struct M5ResolvedSessionProvenanceFreshnessRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SessionProvenanceFreshnessRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SessionProvenanceFreshnessRecordNextAction,
...
impl M5ResolvedSessionProvenanceFreshnessRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SessionProvenanceFreshnessResultEntryResolutionInput {
    ...
    pub comparison_scope: M5SessionProvenanceFreshnessResultScope,
    /// The render / surface context.
    pub surface_context: M5SessionProvenanceFreshnessRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SessionProvenanceFreshnessRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedSessionProvenanceFreshnessResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SessionProvenanceFreshnessResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SessionProvenanceFreshnessRecordNextAction,
...
impl M5ResolvedSessionProvenanceFreshnessResultEntry {
...
pub enum M5SessionProvenanceFreshnessRecordResolutionError {
...
impl M5SessionProvenanceFreshnessRecordResolutionError {
...
impl fmt::Display for M5SessionProvenanceFreshnessRecordResolutionError {
...
impl Error for M5SessionProvenanceFreshnessRecordResolutionError {}

fn form_tokens(forms: &[M5SessionProvenanceFreshnessRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5SessionProvenanceFreshnessRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5SessionProvenanceFreshnessRecordResolutionForm> =
        forms.iter().copied().collect();
    M5SessionProvenanceFreshnessRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5SessionProvenanceFreshnessRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5SessionProvenanceFreshnessResultScope,
...
pub fn resolve_review_pack_record_entry(
    ...
    M5SessionProvenanceFreshnessRecordResolutionError,
> {
    if input.entry_id.trim().is_empty() {
        return Err(
            M5SessionProvenanceFreshnessRecordResolutionError::EmptyCohortDescriptorEntryId,
        );
    }
    ...
    {
        return Err(M5SessionProvenanceFreshnessRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SessionProvenanceFreshnessRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SessionProvenanceFreshnessRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5SessionProvenanceFreshnessRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5SessionProvenanceFreshnessRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5SessionProvenanceFreshnessRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5SessionProvenanceFreshnessRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5SessionProvenanceFreshnessRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5SessionProvenanceFreshnessRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5SessionProvenanceFreshnessRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5SessionProvenanceFreshnessRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedSessionProvenanceFreshnessRecordEntry {
...
> {
    if input.entry_id.trim().is_empty() {
        return Err(
            M5SessionProvenanceFreshnessRecordResolutionError::EmptyCohortEvidencePacketEntryId,
        );
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SessionProvenanceFreshnessResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SessionProvenanceFreshnessResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5SessionProvenanceFreshnessResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5SessionProvenanceFreshnessResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5SessionProvenanceFreshnessResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5SessionProvenanceFreshnessResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5SessionProvenanceFreshnessRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedSessionProvenanceFreshnessResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionProvenanceFreshnessRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5SessionProvenanceFreshnessRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5SessionProvenanceFreshnessRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5SessionProvenanceFreshnessRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedSessionProvenanceFreshnessRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedSessionProvenanceFreshnessResultEntry>,
...
impl M5SessionProvenanceFreshnessRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5SessionProvenanceFreshnessRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5SessionProvenanceFreshnessRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5SessionProvenanceFreshnessRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5SessionProvenanceFreshnessRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedSessionProvenanceFreshnessRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedSessionProvenanceFreshnessResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionProvenanceFreshnessRecordAndResultRegistriesVocabularySet {
...
impl M5SessionProvenanceFreshnessRecordAndResultRegistriesVocabularySet {
    ...
    pub fn canonical() -> Self {
        Self {
            ...
            report_section_kinds: tokens(&M5SessionProvenanceFreshnessRecordKind::ALL, |v| {
                v.as_str()
            }),
            comparison_scopes: tokens(&M5SessionProvenanceFreshnessResultScope::ALL, |v| {
                v.as_str()
            }),
            surface_contexts: tokens(
                &M5SessionProvenanceFreshnessRecordSurfaceContext::ALL,
                |v| v.as_str(),
            ),
            review_pack_record_degrade_reasons: tokens(
                &M5SessionProvenanceFreshnessRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5SessionProvenanceFreshnessResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5SessionProvenanceFreshnessRecordAnatomyPart::ALL, |v| {
                v.as_str()
            }),
            next_actions: tokens(&M5SessionProvenanceFreshnessRecordNextAction::ALL, |v| {
                v.as_str()
            }),
            export_fields: tokens(&M5SessionProvenanceFreshnessRecordExportField::ALL, |v| {
                v.as_str()
            }),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionProvenanceFreshnessRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionProvenanceFreshnessRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionProvenanceFreshnessRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionProvenanceFreshnessRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5SessionProvenanceFreshnessRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SessionProvenanceFreshnessRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5SessionProvenanceFreshnessRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5SessionProvenanceFreshnessRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5SessionProvenanceFreshnessRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection:
        M5SessionProvenanceFreshnessRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5SessionProvenanceFreshnessRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5SessionProvenanceFreshnessRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionProvenanceFreshnessRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_SCHEMA_VERSION`].
...
impl M5SessionProvenanceFreshnessRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5SessionProvenanceFreshnessRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind:
                M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_RECORD_KIND
                    .to_owned(),
            schema_version:
                M5_SESSION_PROVENANCE_FRESHNESS_AND_CONSUMER_TRUTH_LABEL_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation> {
        ...
        {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::WrongRecordKind,
            );
        }
        ...
        {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::MissingIdentity,
            );
        }
        ...
        if !self.vocabulary_set.matches_canonical() {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::VocabularySetDrift,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::RawMaterialInExport,
            );
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Session-Provenance-Freshness and Consumer-Truth-Label Registries\n\n");
...
#[derive(Debug)]
pub enum M5SessionProvenanceFreshnessRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5SessionProvenanceFreshnessRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5SessionProvenanceFreshnessRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5SessionProvenanceFreshnessRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation {
...
impl M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_session_provenance_freshness_and_consumer_truth_label_registries_export(
) -> Result<
    M5SessionProvenanceFreshnessRecordAndResultRegistriesPacket,
    M5SessionProvenanceFreshnessRecordAndResultRegistriesArtifactError,
> {
    let packet: M5SessionProvenanceFreshnessRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-session-provenance-freshness-and-consumer-truth-label-registries-proof/support_export.json"
        )
    ))
    .map_err(M5SessionProvenanceFreshnessRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(
            M5SessionProvenanceFreshnessRecordAndResultRegistriesArtifactError::Validation(
                violations,
            ),
        )
    }
...
fn validate_source_contracts(
    packet: &M5SessionProvenanceFreshnessRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations
            .push(M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        ...
        {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::ExamplesMissing,
            );
        }
        if !row.examples_are_honest() {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::DishonestExample,
            );
        }
        if !row.honours_invariants() {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5SessionProvenanceFreshnessRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5SessionProvenanceFreshnessRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    let unbound_degrades = review_pack_records().any(|ex| {
        ex.degrade_reason
            == Some(
                M5SessionProvenanceFreshnessRecordEntryDegradeReason::DescriptorNotBoundToRegistry,
            )
    });
    ...
    {
        violations.push(
            M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5SessionProvenanceFreshnessResultScope::CANONICAL_SCOPES
        .iter()
    ...
    let form_incomplete_degrades = downgrade().any(|ex| {
        ex.degrade_reason
            == Some(
                M5SessionProvenanceFreshnessResultEntryDegradeReason::EvidenceFormCoverageIncomplete,
            )
    });
    ...
    {
        violations.push(
            M5SessionProvenanceFreshnessRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
