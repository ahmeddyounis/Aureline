...
fn clean_review_pack_record_input() -> M5SessionProvenanceFreshnessRecordEntryResolutionInput {
    M5SessionProvenanceFreshnessRecordEntryResolutionInput {
        ...
        report_section: M5SessionProvenanceFreshnessRecordKind::ChangedFilesScope,
        surface_context: M5SessionProvenanceFreshnessRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SessionProvenanceFreshnessRecordResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5SessionProvenanceFreshnessResultEntryResolutionInput {
    M5SessionProvenanceFreshnessResultEntryResolutionInput {
        ...
        semantic_role: M5CollaborationStateRole::AuthorityModelDisclosure,
        comparison_scope: M5SessionProvenanceFreshnessResultScope::EvaluatedScopeBinding,
        surface_context: M5SessionProvenanceFreshnessRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SessionProvenanceFreshnessRecordResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_session_provenance_freshness_and_consumer_truth_label_registries();
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(
        ...
        M5SessionProvenanceFreshnessRecordNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5SessionProvenanceFreshnessRecordKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_object_incomplete_and_widen_fold_and_form_degrade() {
    ...
    assert_eq!(
        ...
        Some(
            M5SessionProvenanceFreshnessRecordEntryDegradeReason::CohortDescriptorObjectIncomplete
        )
    );
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input)
            ...
            .degrade_reason,
        Some(
            M5SessionProvenanceFreshnessRecordEntryDegradeReason::ResolutionFormCoverageIncomplete
        )
    );
}
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5SessionProvenanceFreshnessRecordKind::FullTreeScope;
    ...
    let mut input = clean_review_pack_record_input();
    input.surface_context = M5SessionProvenanceFreshnessRecordSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input).unwrap_err(),
        M5SessionProvenanceFreshnessRecordResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5SessionProvenanceFreshnessRecordKind::PackScopeUnclassified,
        true,
        false,
        true
    ));
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5SessionProvenanceFreshnessRecordAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5SessionProvenanceFreshnessRecordExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_session_provenance_freshness_and_consumer_truth_label_registries()
        .export_safe_json();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_session_provenance_freshness_and_consumer_truth_label_registries_export()
        .expect(
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5SessionProvenanceFreshnessRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-session-provenance-freshness-and-consumer-truth-label-registries/provenance_label_beta_narrowed.json"
    )))
    ...
    let preview: M5SessionProvenanceFreshnessRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-session-provenance-freshness-and-consumer-truth-label-registries/consumer_disposition_preview_narrowed.json"
    )))
