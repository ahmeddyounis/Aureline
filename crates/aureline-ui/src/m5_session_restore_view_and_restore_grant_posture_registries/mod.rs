//! Implemented M5 session-restore-view and restore-grant-posture registries.
//!
//! The frozen [collaboration-control component matrix][matrix] names Aureline's governed sensitive-collaboration
//! object classes and locks their controlled vocabulary. This module makes the explicit session-restore view
//! operable — as a canonical record — and adds the restore-grant posture that names, by replay-free transcript
//! class and disclosed control outcome, exactly what a reconnect rejoined and whether any authority was
//! re-requested, so the desktop shared-terminal / debug surface, the companion-follow flow, and the
//! support / export and help / docs surfaces inherit one canonical restore descriptor rather than a
//! hand-authored parallel prose that has to be kept consistent: it turns the
//! *session-restore-view* grammar (one durable restore record per reconnect — one typed field per
//! record section: the transcript class (replay-free render summary, metadata restore summary, text / comment
//! timeline summary, or elevated support / regulatory evidence summary), the restore path, the target context,
//! whether live control was re-requested, and the replay-free render summary kept distinct from ordinary
//! presence and follow state — each bound to one session / restore scope) and the *restore-grant-posture* grammar (the
//! view-only-by-default posture a participant reads before any restored session touches control, and the fresh visible authority event any
//! reconnect, control re-request, or reopen must raise — a restore-disclosed-view-only, an observing-view-only, a
//! control-re-request-pending, a control-re-granted, a reopen-target-required, or a replay-blocked-no-rerun
//! outcome that must never replay prior input or carry authority forward) into registry
//! resolvers that produce export-safe, honest projections. Every reconnect then resolves to one typed
//! session-restore view — its transcript class, its restore path and target context, whether control was
//! re-requested, and its replay-free render summary, all attributable to the session so restore never replays
//! input silently — and to one restore-grant-posture object — the resolved restore identity, the transcript
//! class and restore path, the control posture, and the observe / re-request / reopen / replay-blocked outcome — that the shared terminal / debug, companion, and support surfaces
//! can inspect without manual reconstruction, so no restored session ever carries authority forward without disclosing
//! whether the user is observing, requesting control again, or needs to reopen the target, and the restore history stays visible and exportable, and a restore that cannot bind a field to its
//! session / restore scope degrades honestly instead of letting a reconnect read as control-carrying when it is not.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable session-restore view per reconnect, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [transcript class][M5SessionRestoreViewRecordKind],
//!   a disclosure role, covers every [resolution form][M5SessionRestoreViewRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (restore path
//!   and target context, transcript class, whether control was re-requested and the replay-free render summary,
//!   and the visible-badge binding bound to the session identity), keeps its restore lineage attributable to the session, and
//!   keeps a control-re-request field bound to its restore state; otherwise it degrades.
//! * **Keep the restore-grant posture honest so a reconnect never carries authority forward silently rather than a
//!   hand-authored parallel prose staying consistent.** [`line_preserves_rollback_and_diagnostics_before_widening`]
//!   rejects a restore whose view-only default is not preserved (a reconnect presenting itself as
//!   already-control-capable from presence alone) so it degrades to
//!   [`M5SessionRestoreViewRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a posture whose language outruns its restore state degrades the same
//!   way — the structured blocker reason a replay-or-reacquire-without-fresh-grant attempt must surface.
//! * **Turn a restore-disclosed, an observing, a control-re-request, a control-re-grant, a reopen, or a replay-blocked outcome into a visible, typed authority event
//!   instead of a silent input replay.** [`resolve_review_pack_result_entry`] names a classified
//!   [restore binding][M5SessionRestoreViewResultScope]
//!   (transcript-class binding, restore-path binding, or observe / re-request / reopen binding), requires the full
//!   restore-identity / affected-restore-field / previous-versus-current-restore-state / binding / active-reason
//!   posture object, covers every resolution form, and degrades to
//!   [`M5SessionRestoreViewResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the outcome would replay prior input, carry authority forward on a sensitive surface, or let a
//!   reconnect or reopen event drop its attribution, so a restored session can never read as trustworthy when
//!   it has quietly dropped the reason a fresh visible authority event was required.
//!
//! The resolvers reuse the frozen matrix vocabulary directly — the [`M5CollaborationControlRole`] disclosure
//! role vocabulary and the [`M5CollaborationControlConsumerSurface`] consumer-surface taxonomy — so the shared
//! terminal / debug view, companion follow, control-grant prompt, paste / secret guard, and support / export surfaces
//! can never fork their own session-restore meaning. Raw terminal / debug input, prior signals, breakpoint edits, command text, variable bodies, and clipboard contents stay outside the export boundary and are never replayed on restore.
//!
//! [matrix]: crate::m5_shared_terminal_debug_control_grant_presenter_consent_retention_and_session_restore_view_matrix
...
pub use seed::{
    seeded_m5_session_restore_view_and_restore_grant_posture_registries,
    seeded_m5_session_restore_view_and_restore_grant_posture_registries_review_pack_record_beta_narrowed,
    seeded_m5_session_restore_view_and_restore_grant_posture_registries_review_pack_result_preview_narrowed,
    M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_PACKET_ID,
};
...
use crate::m5_shared_terminal_debug_control_grant_presenter_consent_retention_and_session_restore_view_matrix::{
    M5CollaborationControlAccessibilityRoute, M5CollaborationControlClassificationStage,
    M5CollaborationControlConsumerSurface, M5CollaborationControlDowngradeTrigger,
    M5CollaborationControlObject, M5CollaborationControlQualificationClass,
    M5CollaborationControlRequiredLabel, M5CollaborationControlRole,
    M5_SESSION_RESTORE_VIEW_DOMAIN_SCHEMA_REF, M5_COLLABORATION_CONTROL_MATRIX_DOC_REF,
    M5_COLLABORATION_CONTROL_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5SessionRestoreViewRecordAndResultRegistriesPacket`].
pub const M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_session_restore_view_and_restore_grant_posture_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_session_restore_view_and_restore_grant_posture_registries.md";

/// Repo-relative path of the restore-grant-posture domain schema this lane binds its B object to (the
/// view-only-by-default control posture a restored session declares on reconnect, its audit-safe attribution,
/// and the replay-free transcript class and restore outcome it projects — restore disclosed view-only,
/// observing view-only, control re-request pending, control re-granted, reopen target required, or replay
/// blocked with no rerun — kept distinct from any prior terminal / debug input). Minted by this lane; the
/// restore-grant posture and the session-restore view never fork their restore vocabulary, and no prior input,
/// signal, breakpoint edit, command text, variable body, or clipboard content crosses this boundary or is
/// replayed on restore.
pub const M5_RESTORE_GRANT_POSTURE_DOMAIN_SCHEMA_REF: &str =
    "schemas/collaboration/m5-restore-grant-posture.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-session-restore-view-and-restore-grant-posture-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-session-restore-view-and-restore-grant-posture-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-session-restore-view-and-restore-grant-posture-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-session-restore-view-and-restore-grant-posture-registries";
...
pub type M5SessionRestoreViewRecordAndResultRegistriesConsumerSurface =
    M5CollaborationControlConsumerSurface;
...
pub enum M5SessionRestoreViewRecordResolutionForm {
...
impl M5SessionRestoreViewRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionRestoreViewRecordKind {
...
impl M5SessionRestoreViewRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionRestoreViewResultScope {
...
impl M5SessionRestoreViewResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionRestoreViewRecordSurfaceContext {
...
impl M5SessionRestoreViewRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionRestoreViewRecordAnatomyPart {
...
impl M5SessionRestoreViewRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionRestoreViewRecordNextAction {
...
impl M5SessionRestoreViewRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionRestoreViewRecordExportField {
...
impl M5SessionRestoreViewRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionRestoreViewRecordEntryDegradeReason {
...
impl M5SessionRestoreViewRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5SessionRestoreViewRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5SessionRestoreViewRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5SessionRestoreViewRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5SessionRestoreViewRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5SessionRestoreViewRecordNextAction::ReviewBlockedOrDegraded,
        }
    }

    /// The matching downgrade trigger recorded on the frozen matrix.
    pub const fn downgrade_trigger(self) -> M5CollaborationControlDowngradeTrigger {
        match self {
            Self::DescriptorTokenUnstated
            | Self::SurfaceContextUnresolved
            | Self::ResolutionFormCoverageIncomplete
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionRestoreViewResultEntryDegradeReason {
...
impl M5SessionRestoreViewResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5SessionRestoreViewRecordNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5SessionRestoreViewRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
    ...
    pub const fn downgrade_trigger(self) -> M5CollaborationControlDowngradeTrigger {
        match self {
            ...
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5CollaborationControlDowngradeTrigger::MoreThanOneActiveDriverOnASensitiveSurface
            }
            Self::EvidenceFormCoverageIncomplete => {
...
pub struct M5SessionRestoreViewRecordEntryResolutionInput {
    ...
    pub semantic_role: M5CollaborationControlRole,
    /// The line journey this entry classifies.
    pub report_section: M5SessionRestoreViewRecordKind,
    /// The render / surface context.
    pub surface_context: M5SessionRestoreViewRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SessionRestoreViewRecordResolutionForm>,
...
pub struct M5ResolvedSessionRestoreViewRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SessionRestoreViewRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SessionRestoreViewRecordNextAction,
...
impl M5ResolvedSessionRestoreViewRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SessionRestoreViewResultEntryResolutionInput {
    ...
    pub comparison_scope: M5SessionRestoreViewResultScope,
    /// The render / surface context.
    pub surface_context: M5SessionRestoreViewRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SessionRestoreViewRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedSessionRestoreViewResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SessionRestoreViewResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SessionRestoreViewRecordNextAction,
...
impl M5ResolvedSessionRestoreViewResultEntry {
...
pub enum M5SessionRestoreViewRecordResolutionError {
...
impl M5SessionRestoreViewRecordResolutionError {
...
impl fmt::Display for M5SessionRestoreViewRecordResolutionError {
...
impl Error for M5SessionRestoreViewRecordResolutionError {}

fn form_tokens(forms: &[M5SessionRestoreViewRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5SessionRestoreViewRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5SessionRestoreViewRecordResolutionForm> =
        forms.iter().copied().collect();
    M5SessionRestoreViewRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5SessionRestoreViewRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5SessionRestoreViewResultScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedSessionRestoreViewRecordEntry, M5SessionRestoreViewRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5SessionRestoreViewRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5SessionRestoreViewRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SessionRestoreViewRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SessionRestoreViewRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5SessionRestoreViewRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5SessionRestoreViewRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5SessionRestoreViewRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5SessionRestoreViewRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5SessionRestoreViewRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5SessionRestoreViewRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5SessionRestoreViewRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5SessionRestoreViewRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedSessionRestoreViewRecordEntry {
...
pub fn resolve_review_pack_result_entry(
...
) -> Result<M5ResolvedSessionRestoreViewResultEntry, M5SessionRestoreViewRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5SessionRestoreViewRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SessionRestoreViewResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SessionRestoreViewResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5SessionRestoreViewResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5SessionRestoreViewResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5SessionRestoreViewResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5SessionRestoreViewResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5SessionRestoreViewRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedSessionRestoreViewResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionRestoreViewRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5SessionRestoreViewRecordAndResultRegistriesConsumerSurface,
    ...
    pub capture_lifecycle_stages: Vec<M5CollaborationControlClassificationStage>,
    ...
    pub anatomy_parts: Vec<M5SessionRestoreViewRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5SessionRestoreViewRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedSessionRestoreViewRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedSessionRestoreViewResultEntry>,
...
impl M5SessionRestoreViewRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5SessionRestoreViewRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5SessionRestoreViewRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5SessionRestoreViewRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5SessionRestoreViewRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedSessionRestoreViewRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedSessionRestoreViewResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionRestoreViewRecordAndResultRegistriesVocabularySet {
...
impl M5SessionRestoreViewRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5CollaborationControlRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5SessionRestoreViewRecordResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5SessionRestoreViewRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5SessionRestoreViewResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5SessionRestoreViewRecordSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            review_pack_record_degrade_reasons: tokens(
                &M5SessionRestoreViewRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5SessionRestoreViewResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5SessionRestoreViewRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5SessionRestoreViewRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5SessionRestoreViewRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionRestoreViewRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionRestoreViewRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionRestoreViewRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionRestoreViewRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5SessionRestoreViewRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SessionRestoreViewRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5SessionRestoreViewRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5SessionRestoreViewRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5SessionRestoreViewRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5SessionRestoreViewRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5SessionRestoreViewRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5SessionRestoreViewRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionRestoreViewRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_SCHEMA_VERSION`].
...
impl M5SessionRestoreViewRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5SessionRestoreViewRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version:
                M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5SessionRestoreViewRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind
            != M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_RECORD_KIND
        {
            violations
                .push(M5SessionRestoreViewRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_SESSION_RESTORE_VIEW_AND_RESTORE_GRANT_POSTURE_REGISTRIES_SCHEMA_VERSION
        {
            violations
                .push(M5SessionRestoreViewRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations
                .push(M5SessionRestoreViewRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations
                .push(M5SessionRestoreViewRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations
                .push(M5SessionRestoreViewRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Session-Restore-View and Restore-Grant-Posture Registries\n\n");
...
#[derive(Debug)]
pub enum M5SessionRestoreViewRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5SessionRestoreViewRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5SessionRestoreViewRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5SessionRestoreViewRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5SessionRestoreViewRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5SessionRestoreViewRecordAndResultRegistriesViolation {
...
impl M5SessionRestoreViewRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_session_restore_view_and_restore_grant_posture_registries_export(
) -> Result<
    M5SessionRestoreViewRecordAndResultRegistriesPacket,
    M5SessionRestoreViewRecordAndResultRegistriesArtifactError,
> {
    let packet: M5SessionRestoreViewRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-session-restore-view-and-restore-grant-posture-registries-proof/support_export.json"
        )
    ))
    .map_err(M5SessionRestoreViewRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5SessionRestoreViewRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5SessionRestoreViewRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5SessionRestoreViewRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5SessionRestoreViewRecordAndResultRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5SessionRestoreViewRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5SessionRestoreViewRecordAndResultRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5SessionRestoreViewRecordAndResultRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5SessionRestoreViewRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_SESSION_RESTORE_VIEW_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_RESTORE_GRANT_POSTURE_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5SessionRestoreViewRecordAndResultRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations
                .push(M5SessionRestoreViewRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations
                .push(M5SessionRestoreViewRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5SessionRestoreViewRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SessionRestoreViewRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SessionRestoreViewRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5SessionRestoreViewRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5SessionRestoreViewRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5SessionRestoreViewRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5SessionRestoreViewRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5SessionRestoreViewRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5SessionRestoreViewRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5SessionRestoreViewResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(
            M5SessionRestoreViewRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
...
pub const IMPLEMENTED_LINES: [M5CollaborationControlObject; 6] = M5CollaborationControlObject::ALL;
