...
fn seed() -> M5ReviewPackSharedConsumersPacket {
    seeded_m5_review_pack_shared_consumers()
}

fn violations_of(packet: &M5ReviewPackSharedConsumersPacket) -> Vec<&'static str> {
...
fn seed_validates_clean() {
    ...
    assert_eq!(packet.packet_id, M5_REVIEW_PACK_SHARED_CONSUMERS_PACKET_ID);
    assert_eq!(
        packet.record_kind,
        M5_REVIEW_PACK_SHARED_CONSUMERS_RECORD_KIND
    );
...
fn every_consumer_surface_and_representation_is_exercised() {
    ...
    let representations: BTreeSet<_> = packet
        ...
        .collect();
    for representation in ReviewPackSharedRepresentation::ALL {
        assert!(
...
#[test]
fn every_review_pack_role_word_is_a_frozen_role_token() {
    let packet = seed();
    for binding in &packet.consumer_bindings {
        assert!(
            binding.state_facets.review_pack_role_word_in_vocabulary(),
            "role word `{}` on {} is not a frozen role token",
            binding.state_facets.review_pack_role_word,
            binding.binding_id
        );
        assert!(binding.state_facets.all_present());
        assert!(binding.state_facets.pack_freshness_satisfied());
    }
}
    ...
    for binding in &packet.consumer_bindings {
        ...
        if binding.is_narrowed() {
            assert_eq!(
                ...
                ReviewPackSharedVocabularyState::FacetsDisclosedNarrowed
            );
            let note = binding
                .narrow_note
                .as_ref()
            ...
            assert_eq!(
                binding.vocabulary_state,
                ReviewPackSharedVocabularyState::FacetsPreserved
            );
            assert!(binding.narrow_note.is_none());
        }
        if matches!(
            binding.representation,
            ReviewPackSharedRepresentation::RemoteProjected
        ) {
            assert!(!binding.remote_source_note.trim().is_empty());
        }
        if matches!(
            binding.representation,
            ReviewPackSharedRepresentation::ExportedRedacted
        ) {
...
fn disclosure_resolver_matches_representation() {
    assert!(
        !resolve_review_pack_shared_render_disclosure(ReviewPackSharedRepresentation::DesktopFull)
            .needs_narrow_note
    );
    let compact = resolve_review_pack_shared_render_disclosure(
        ReviewPackSharedRepresentation::CompactNarrowed,
    );
    assert_eq!(
        compact.narrow_reason,
        Some(ReviewPackSharedNarrowReason::CompactionNarrowed)
    );
    assert!(compact.needs_narrow_note);
    assert!(!compact.needs_remote_source_note);
    let remote = resolve_review_pack_shared_render_disclosure(
        ReviewPackSharedRepresentation::RemoteProjected,
    );
    assert!(remote.needs_remote_source_note);
    let exported = resolve_review_pack_shared_render_disclosure(
        ReviewPackSharedRepresentation::ExportedRedacted,
    );
...
fn vocabulary_drift_is_rejected() {
    ...
    let target = packet
        .consumer_bindings
        .iter()
        .position(|b| b.binding_id == "rpsc-ownership-review-detail")
        .unwrap();
    packet.consumer_bindings[target]
        .state_facets
        .review_pack_role_word = "template_attribution_disclosure".to_owned();
    assert!(violations_of(&packet).contains(&"review_pack_vocabulary_drift_across_surfaces"));
}

#[test]
fn review_pack_role_word_outside_vocabulary_is_rejected() {
    let mut packet = seed();
    packet.consumer_bindings[0]
        .state_facets
        .review_pack_role_word = "totally_made_up".to_owned();
    let tokens = violations_of(&packet);
    assert!(tokens.contains(&"review_pack_role_word_outside_vocabulary"));
...
fn dropped_pack_freshness_on_gate_role_is_rejected() {
    let mut packet = seed();
    // rpsc-parity-strip carries the local_versus_provider_parity_disclosure gate role, which must always keep
    // a real pack freshness.
    let target = packet
        .consumer_bindings
        .iter()
        .position(|b| b.binding_id == "rpsc-parity-strip")
        .unwrap();
    packet.consumer_bindings[target]
        .state_facets
        .pack_freshness_word = "local_estimate_shown_as_provider_authoritative".to_owned();
    assert!(violations_of(&packet).contains(&"pack_freshness_missing_for_gate_role"));
}

#[test]
fn reuse_below_two_consumers_is_rejected() {
    let mut packet = seed();
    // Drop every review-template-packet binding except one, leaving the object with one consumer.
    let mut kept_one = false;
    packet.consumer_bindings.retain(|b| {
        if b.object == M5ReviewPackObject::ReviewTemplatePacket {
            if kept_one {
                return false;
            }
            kept_one = true;
...
fn missing_canonical_reference_on_export_is_rejected() {
    ...
    packet.consumer_bindings[target].source_contract_refs =
        vec![M5_REVIEW_PACK_MATRIX_DOC_REF.to_owned()];
...
fn unexpected_narrow_note_on_full_binding_is_rejected() {
    ...
    let target = packet
        ...
        .position(|b| !b.is_narrowed())
        .unwrap();
    packet.consumer_bindings[target].narrow_note = Some(ReviewPackSharedNarrowNote {
        reason: ReviewPackSharedNarrowReason::CompactionNarrowed,
        preserved_vocabulary_note: "x".to_owned(),
        next_action: ReviewPackSharedNarrowNextAction::ExpandInDesktop,
...
fn each_guardrail_is_enforced() {
    let cases: [crate::GuardrailCase<ReviewPackSharedConsumerBinding>; 5] = [
        ...
        (
            |b| {
                b.hides_ci_only_not_evaluated_here_or_provider_unavailable_behind_a_green_summary =
                    true
            },
            "hides_ci_only_not_evaluated_here_or_provider_unavailable_behind_a_green_summary",
        ),
        (
            |b| b.flattens_advisory_owner_and_enforced_owner_into_one_owner_pill = true,
            "flattens_advisory_owner_and_enforced_owner_into_one_owner_pill",
        ),
        (
            |b| b.lets_ai_review_run_under_a_different_pack_version_without_disclosure = true,
            "lets_ai_review_run_under_a_different_pack_version_without_disclosure",
        ),
        (
            |b| {
                b.loses_review_pack_version_digest_or_template_attribution_when_exporting_publishing_or_reopening = true
            },
...
fn object_coverage_gap_is_rejected() {
    ...
    packet
        .consumer_bindings
        .retain(|b| b.object != M5ReviewPackObject::ReviewTemplatePacket);
...
fn missing_source_contract_is_rejected() {
    ...
    packet
        ...
        .retain(|r| r != M5_REVIEW_PACK_SHARED_CONSUMERS_DOC_REF);
...
fn csv_has_a_row_per_binding() {
    ...
    assert!(lines[0]
        .starts_with("object,consumer,representation,review_pack_role_word,vocabulary_state"));
}
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_review_pack_shared_consumers_export()
        .expect("checked M5 review-pack shared-consumer export validates");
    assert_eq!(
        from_disk.packet_id,
...
fn narrowed_fixture_variants_validate_and_keep_coverage() {
    let compact = seeded_m5_review_pack_shared_consumers_compact_remote_narrowed();
    assert!(
    ...
    let exported = seeded_m5_review_pack_shared_consumers_exported_redaction_narrowed();
    assert!(
...
fn checked_narrowed_fixtures_validate_and_match_builders() {
    let compact: M5ReviewPackSharedConsumersPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-review-pack-shared-consumers/compact_remote_narrowed.json"
    )))
    ...
    let exported: M5ReviewPackSharedConsumersPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-review-pack-shared-consumers/exported_redaction_narrowed.json"
    )))
