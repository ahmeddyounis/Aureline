//! Tests for the M05-1155 motion-layer-iconography surface certification capstone.

use super::*;

fn packet() -> VisualInteractionProfileCertificationPacket {
    seeded_m5_motion_layer_iconography_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, MOTION_LAYER_ICONOGRAPHY_CERT_RECORD_KIND);
    assert_eq!(
        p.schema_version,
        MOTION_LAYER_ICONOGRAPHY_CERT_SCHEMA_VERSION
    );
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5VisualInteractionCertifiedProfile::ALL {
...
#[test]
fn packet_has_two_green_and_five_yellow_and_no_red() {
    let p = packet();
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            VisualInteractionCertificationAxis::ALL.len()
        );
    }
...
#[test]
fn all_seven_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5VisualInteractionA11yClaim> =
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5VisualInteractionCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5VisualInteractionCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = VisualInteractionCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), VisualInteractionCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in VisualInteractionCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == VisualInteractionCertificationAxis::CliExport
        );
...
#[test]
fn power_thermal_axis_is_a_distinct_reach_axis() {
    // The B137 power-saver / thermal-pressure axis is present, distinct, and exercised by the
    // unconfirmed-reduced-motion profile as its binding narrowed axis.
    let p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| {
            r.profile == M5VisualInteractionCertifiedProfile::UnconfirmedReducedMotionSurface
        })
        .expect("unconfirmed-reduced-motion-surface row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(VisualInteractionCertificationAxis::PowerThermal)
        .expect("power-thermal axis present");
    assert_eq!(
        outcome.state,
    ...
    assert_eq!(
        ...
        Some(VisualInteractionCertificationAxis::PowerThermal)
    );
}
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5VisualInteractionCertifiedProfile::LiveTrustedInteractionSurface
        })
        .expect("live-trusted-interaction-surface row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == VisualInteractionCertificationAxis::Visual {
            outcome.state = VisualInteractionAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("icon meaning silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, VisualInteractionProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        VisualInteractionCertificationViolation::ProfileBlocked { .. }
    )));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5VisualInteractionCertifiedProfile::ReviewableLayerStructure)
        .expect("reviewable-layer-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails.lets_an_overlay_bypass_the_shared_z_order = true;
    assert_eq!(
        row.derive_status(),
        VisualInteractionProfileClaimStatus::Red
    );
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        VisualInteractionCertificationViolation::GuardrailViolated { .. }
    )));
}
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5VisualInteractionA11yClaim::TrustedInteractionSurface;
    row.certified_claim = M5VisualInteractionA11yClaim::TrustedInteractionSurface;
    assert_eq!(
        row.derive_status(),
        VisualInteractionProfileClaimStatus::Red
    );
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == VisualInteractionCertificationAxis::DegradedState {
            *outcome = VisualInteractionAxisOutcome {
                axis: VisualInteractionCertificationAxis::DegradedState,
                state: VisualInteractionAxisCertificationState::DisclosedNarrowed,
                parity_note: "the layer-tier evidence is lagging".to_owned(),
                narrowing_reason: Some(
                    "the layer-tier evidence is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5VisualInteractionDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableInteractionSurface == certified, no claim_auto_narrow.
    assert_eq!(
        row.derive_status(),
        VisualInteractionProfileClaimStatus::Red
    );
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == VisualInteractionCertificationAxis::CliExport {
            outcome.state = VisualInteractionAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5VisualInteractionDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(
        row.derive_status(),
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(VisualInteractionClaimAutoNarrow {
        binding_axis: VisualInteractionCertificationAxis::DegradedState,
        from_claim: M5VisualInteractionA11yClaim::TrustedInteractionSurface,
        to_claim: M5VisualInteractionA11yClaim::TrustedInteractionSurface,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5VisualInteractionA11yClaim::MotionTimingUnverifiedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-layer-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableInteractionSurface; certify a stronger trusted interaction surface.
    row.certified_claim = M5VisualInteractionA11yClaim::TrustedInteractionSurface;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5VisualInteractionCertifiedProfile::StaleMotionTimingSurface)
        .expect("stale-motion-timing-surface row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = VisualInteractionCertificationAxis::Visual;
    }
    assert_eq!(
        row.derive_status(),
        VisualInteractionProfileClaimStatus::Red
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == VisualInteractionCertificationAxis::CliExport {
            outcome.state = VisualInteractionAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the stale motion timing surface".to_owned());
            outcome.downgrade_trigger = Some(M5VisualInteractionDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = VisualInteractionCertificationAxis::CliExport;
    }
    assert_eq!(
        row.derive_status(),
        VisualInteractionProfileClaimStatus::Red
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == VisualInteractionProfileClaimStatus::Yellow)
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(VisualInteractionCertificationAxis::Visual);
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows.retain(|r| {
        r.profile != M5VisualInteractionCertifiedProfile::ImpersonatingIllustrationSurface
    });
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        VisualInteractionCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the iconography family from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5VisualInteractionFamily::Iconography);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        VisualInteractionCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = VisualInteractionProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        VisualInteractionCertificationViolation::WrongCanonicalBundle
    )));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        VisualInteractionCertificationViolation::DuplicateId { .. }
    )));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != VisualInteractionCertificationAxis::VisualInteractionComponentTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, VisualInteractionCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        VisualInteractionCertificationViolation::RawInteractionMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: VisualInteractionProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_motion_layer_iconography_surface_certification_export()
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/ui/m5-motion-layer-iconography-surface-certification/support_export.json"
    ));
    let packet_from_fixture: VisualInteractionProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/ui/m5-motion-layer-iconography-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_MOTION_LAYER_ICONOGRAPHY_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and
...
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_MOTION_LAYER_ICONOGRAPHY_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_motion_layer_iconography_surface_certification_packet();
    assert!(
    ...
    let release_dir =
        repo.join("artifacts/release/m5-motion-layer-iconography-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-motion-layer-iconography-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/ui/m5-motion-layer-iconography-surface-certification");
