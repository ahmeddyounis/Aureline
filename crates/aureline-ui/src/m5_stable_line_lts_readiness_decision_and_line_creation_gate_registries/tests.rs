...
fn clean_lts_readiness_decision_input() -> M5LtsReadinessDecisionEntryResolutionInput {
    M5LtsReadinessDecisionEntryResolutionInput {
        entry_id: "lts_readiness_decision:test".to_owned(),
        line_binding_id: "launch.line.core-team-canary".to_owned(),
        token_name: "line.lts_readiness_decision.core_team_canary".to_owned(),
        semantic_role: M5StableLineProtectionRole::SupportWindow,
        decision_section: M5LtsReadinessDecisionKind::BackportBranchPostureSection,
        surface_context: M5StableLineLtsReadinessDecisionSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5StableLineLtsReadinessDecisionResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5LineCreationGateEntryResolutionInput {
    M5LineCreationGateEntryResolutionInput {
        entry_id: "downgrade:test".to_owned(),
        gate_ref: "launch.line.core-team-canary".to_owned(),
        token_name: "line.downgrade.core_team_canary".to_owned(),
        semantic_role: M5StableLineProtectionRole::SupportWindow,
        gate_scope: M5LineCreationGateScope::LtsCommitted,
        surface_context: M5StableLineLtsReadinessDecisionSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5StableLineLtsReadinessDecisionResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_stable_line_lts_readiness_decision_and_line_creation_gate_registries();
...
#[test]
fn lts_readiness_decision_clean_names_meaning_and_is_bound() {
    let resolved =
        resolve_lts_readiness_decision_entry(clean_lts_readiness_decision_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.lts_readiness_decision_resolves_across_lines);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.lts_readiness_decision_object_complete);
    assert!(resolved.bound_to_registry);
    assert!(resolved.decision_section_is_classified);
    assert!(resolved.rollback_and_diagnostics_bounded);
    assert_eq!(resolved.semantic_role, "support_window");
    assert_eq!(resolved.decision_section, "backport_branch_posture_section");
    assert_eq!(
        resolved.canonical_decision_section_mode,
        "backport_branch_posture_section_mode"
    );
    assert_eq!(resolved.surface_context, "shiproom_surface");
    assert_eq!(
        resolved.next_action,
        M5StableLineLtsReadinessDecisionNextAction::ExpandCohortMeaning
    );
}

#[test]
fn lts_readiness_decision_token_unstated_degrades() {
    let mut input = clean_lts_readiness_decision_input();
...
#[test]
fn lts_readiness_decision_unbound_and_unclassified_degrade() {
    let mut input = clean_lts_readiness_decision_input();
    ...
    let mut input = clean_lts_readiness_decision_input();
    input.decision_section = M5LtsReadinessDecisionKind::DecisionSectionUnclassified;
    assert_eq!(
        resolve_lts_readiness_decision_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn lts_readiness_decision_object_incomplete_and_widen_fold_and_form_degrade() {
    // An unstated bundle IDs field leaves the resolved object incomplete.
    let mut input = clean_lts_readiness_decision_input();
    input.bundle_ids = "  ".to_owned();
    let resolved = resolve_lts_readiness_decision_entry(input).unwrap();
    assert!(!resolved.lts_readiness_decision_object_complete);
    assert_eq!(
    ...
    assert_eq!(
        resolve_lts_readiness_decision_entry(input).unwrap().degrade_reason,
...
#[test]
fn lts_readiness_decision_public_facing_and_surface_and_proof_degrade() {
    // A public-facing line running support language ahead of proof first fails the widen-boundary fold.
    let mut input = clean_lts_readiness_decision_input();
    input.decision_section = M5LtsReadinessDecisionKind::MirrorAirGapProofSection;
    ...
    let mut input = clean_lts_readiness_decision_input();
    input.surface_context = M5StableLineLtsReadinessDecisionSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_lts_readiness_decision_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn lts_readiness_decision_empty_id_and_forbidden_material_error() {
    let mut input = clean_lts_readiness_decision_input();
    input.entry_id = "".to_owned();
    assert_eq!(
        resolve_lts_readiness_decision_entry(input).unwrap_err(),
        M5StableLineLtsReadinessDecisionResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_lts_readiness_decision_entry(input).unwrap_err(),
        M5StableLineLtsReadinessDecisionResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5LtsReadinessDecisionKind::DecisionSectionUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn lts_readiness_decision_object_is_complete_requires_all_fields() {
    assert!(lts_readiness_decision_object_is_complete(
        M5LtsReadinessDecisionKind::BackportBranchPostureSection,
...
fn downgrade_clean_stays_honest() {
    let resolved = resolve_line_creation_gate_entry(clean_downgrade_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.gate_safe_on_every_line);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.provides_complete_line_creation_gate);
    assert!(resolved.line_creation_gate_stays_honest);
    assert_eq!(resolved.gate_scope, "lts_committed");
    assert_eq!(resolved.surface_context, "shiproom_surface");
}
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let resolved = resolve_line_creation_gate_entry(input).unwrap();
    assert!(!resolved.provides_complete_line_creation_gate);
    assert_eq!(
        resolved.degrade_reason,
    ...
    let mut input = clean_downgrade_input();
    input.keeps_line_creation_gate_visible = false;
    assert_eq!(
        resolve_line_creation_gate_entry(input).unwrap().degrade_reason,
        Some(M5LineCreationGateEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    );
    ...
    let mut input = clean_downgrade_input();
    input.gate_scope = M5LineCreationGateScope::ScopeUnclassified;
    assert_eq!(
        resolve_line_creation_gate_entry(input)
            .unwrap()
            .degrade_reason,
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    input.entry_id = "  ".to_owned();
    assert_eq!(
        resolve_line_creation_gate_entry(input).unwrap_err(),
...
fn downgrade_bound_support_and_flagged_gap_stay_clean() {
    ...
    assert!(resolve_line_creation_gate_entry(input).unwrap().is_clean());
...
fn vocabulary_set_drift_fails() {
    let mut packet =
        seeded_m5_stable_line_lts_readiness_decision_and_line_creation_gate_registries();
    packet.vocabulary_set.decision_section_kinds.pop();
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_LTS_READINESS_DECISION_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_LINE_CREATION_GATE_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5StableLineLtsReadinessDecisionLineCreationGateRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5StableLineLtsReadinessDecisionAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5StableLineLtsReadinessDecisionExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    let mut packet =
        seeded_m5_stable_line_lts_readiness_decision_and_line_creation_gate_registries();
    packet.registry_rows[0].line_creation_gate_entries.clear();
    assert!(packet.validate().contains(
...
fn dishonest_clean_example_fails() {
    let mut packet =
        seeded_m5_stable_line_lts_readiness_decision_and_line_creation_gate_registries();
    // Force a clean lts_readiness_decision entry to also read as object-incomplete — the packet must reject it.
    let row = &mut packet.registry_rows[0];
    row.lts_readiness_decision_entries[0].degrade_reason = None;
    row.lts_readiness_decision_entries[0].lts_readiness_decision_object_complete = false;
    assert!(packet.validate().contains(
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        match mutate {
            ...
            _ => row.collapses_distinct_line_creation_gate_classes_into_one_lane = true,
        }
        assert!(packet.validate().contains(
...
#[test]
fn lts_readiness_decision_not_proven_when_incomplete_example_removed() {
    let mut packet =
        seeded_m5_stable_line_lts_readiness_decision_and_line_creation_gate_registries();
    for row in &mut packet.registry_rows {
        row.lts_readiness_decision_entries.retain(|ex| {
            ex.degrade_reason
                != Some(M5LtsReadinessDecisionEntryDegradeReason::CohortDescriptorObjectIncomplete)
        });
    }
...
#[test]
fn lts_readiness_decision_not_proven_when_surface_collapses() {
    let mut packet =
        seeded_m5_stable_line_lts_readiness_decision_and_line_creation_gate_registries();
    // Drop every clean executive-steering-surface lts_readiness_decision so the first-consumer surfaces no longer include it.
    for row in &mut packet.registry_rows {
        row.lts_readiness_decision_entries
...
#[test]
fn line_creation_gate_integrity_not_proven_when_support_ahead_example_removed() {
    let mut packet =
        seeded_m5_stable_line_lts_readiness_decision_and_line_creation_gate_registries();
    for row in &mut packet.registry_rows {
        row.line_creation_gate_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
...
#[test]
fn line_creation_gate_integrity_not_proven_when_scope_dropped() {
    let mut packet =
        seeded_m5_stable_line_lts_readiness_decision_and_line_creation_gate_registries();
    // Drop every clean go-no-go-signoff downgrade so the coverage no longer includes it.
    for row in &mut packet.registry_rows {
        row.line_creation_gate_entries
            .retain(|ex| !(ex.is_clean() && ex.gate_scope == "lts_narrowed_to_stable"));
    }
    assert!(packet.validate().contains(
        &M5StableLineLtsReadinessDecisionLineCreationGateRegistriesViolation::CohortEvidenceIntegrityNotProven
    ));
...
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_stable_line_lts_readiness_decision_and_line_creation_gate_registries()
        .export_safe_json();
...
#[test]
fn lts_readiness_decision_table_lists_only_clean_lts_readiness_decisions() {
    let packet = seeded_m5_stable_line_lts_readiness_decision_and_line_creation_gate_registries();
    let table = packet.render_lts_readiness_decision_table();
    // The clean canary and migration lts_readiness_decisions are rendered from the registry.
    assert!(table.contains("backport_branch_posture_section_mode"));
    assert!(table.contains("correction_line_health_section_mode"));
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk =
        current_stable_m5_stable_line_lts_readiness_decision_and_line_creation_gate_registries_export()
            .expect(
            "checked M5 line-lts_readiness_decision / line-downgrade-packet registries export validates",
        );
    assert_eq!(
        from_disk.packet_id,
        M5_STABLE_LINE_LTS_READINESS_DECISION_LINE_CREATION_GATE_REGISTRIES_PACKET_ID
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5StableLineLtsReadinessDecisionLineCreationGateRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-stable-line-lts-readiness-decision-and-line-creation-gate-registries/lts_readiness_decision_beta_narrowed.json"
    )))
    .expect("line-lts_readiness_decision fixture parses");
    ...
    let preview: M5StableLineLtsReadinessDecisionLineCreationGateRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-stable-line-lts-readiness-decision-and-line-creation-gate-registries/line_creation_gate_preview_narrowed.json"
    )))
