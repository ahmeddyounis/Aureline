...
fn seed() -> M5VisualInteractionSharedConsumersPacket {
    seeded_m5_motion_layer_iconography_shared_consumers()
}

fn violations_of(packet: &M5VisualInteractionSharedConsumersPacket) -> Vec<&'static str> {
...
fn seed_validates_clean() {
    ...
    assert_eq!(
        packet.record_kind,
        M5_MOTION_LAYER_ICONOGRAPHY_SHARED_CONSUMERS_RECORD_KIND
    );
...
fn every_family_is_adopted_by_two_or_more_consumers() {
    ...
    assert_eq!(family_consumers.len(), 7, "all seven families adopted");
...
fn every_consumer_surface_and_representation_is_exercised() {
    ...
    let representations: BTreeSet<_> = packet
        ...
        .collect();
    for representation in VisualInteractionRepresentation::ALL {
        assert!(
        ...
        match object_facets.get(binding.interaction_object_id.as_str()) {
            ...
            Some(existing) => assert_eq!(
                **existing, binding.state_facets,
                "grammar drift on {}",
                binding.interaction_object_id
            ),
        }
    }
    // The seven objects each fan out to more than one consumer.
    assert_eq!(object_facets.len(), 7);
}

#[test]
fn every_interaction_role_word_is_a_frozen_role_token() {
    let packet = seed();
    for binding in &packet.consumer_bindings {
        assert!(
            binding.state_facets.interaction_role_word_in_vocabulary(),
            "role word `{}` on {} is not a frozen role token",
            binding.state_facets.interaction_role_word,
            binding.binding_id
        );
        assert!(binding.state_facets.all_present());
        assert!(binding.state_facets.accessible_fallback_satisfied());
    }
}
    ...
    for binding in &packet.consumer_bindings {
        ...
        if binding.is_narrowed() {
            assert_eq!(
                ...
                VisualInteractionParityState::FacetsDisclosedNarrowed
            );
            let note = binding
                .narrow_note
                .as_ref()
            ...
            assert_eq!(
                binding.parity_state,
                VisualInteractionParityState::FacetsPreserved
            );
            assert!(binding.narrow_note.is_none());
        }
        if matches!(
            binding.representation,
            VisualInteractionRepresentation::RemoteProjected
        ) {
            assert!(!binding.remote_source_note.trim().is_empty());
        }
        if matches!(
            binding.representation,
            VisualInteractionRepresentation::ExportedRedacted
        ) {
...
fn disclosure_resolver_matches_representation() {
    assert!(
        !resolve_visual_interaction_render_disclosure(VisualInteractionRepresentation::DesktopFull)
            .needs_narrow_note
    );
    let compact = resolve_visual_interaction_render_disclosure(
        VisualInteractionRepresentation::CompactNarrowed,
    );
    assert_eq!(
        compact.narrow_reason,
        Some(VisualInteractionNarrowReason::CompactionNarrowed)
    );
    assert!(compact.needs_narrow_note);
    assert!(!compact.needs_remote_source_note);
    let remote = resolve_visual_interaction_render_disclosure(
        VisualInteractionRepresentation::RemoteProjected,
    );
    assert!(remote.needs_remote_source_note);
    let exported = resolve_visual_interaction_render_disclosure(
        VisualInteractionRepresentation::ExportedRedacted,
    );
...
fn grammar_drift_is_rejected() {
    ...
    let target = packet
        .consumer_bindings
        .iter()
        .position(|b| b.binding_id == "mi-scrim-shell")
        .unwrap();
    packet.consumer_bindings[target]
        .state_facets
        .interaction_role_word = "layer".to_owned();
    assert!(violations_of(&packet).contains(&"grammar_drift_across_surfaces"));
}

#[test]
fn interaction_role_word_outside_vocabulary_is_rejected() {
    let mut packet = seed();
    packet.consumer_bindings[0]
        .state_facets
        .interaction_role_word = "totally_made_up".to_owned();
    let tokens = violations_of(&packet);
    assert!(tokens.contains(&"interaction_role_word_outside_vocabulary"));
    // Rewording one surface also trips drift, which is expected and fine.
}

#[test]
fn motion_or_decoration_alone_on_meaning_role_is_rejected() {
    let mut packet = seed();
    // mi-icon-editor carries the icon role, which must always pair an accessible fallback.
    let target = packet
        .consumer_bindings
        .iter()
        .position(|b| b.binding_id == "mi-icon-editor")
        .unwrap();
    packet.consumer_bindings[target]
        .state_facets
        .accessible_fallback_word = "motion_alone".to_owned();
    assert!(violations_of(&packet).contains(&"accessible_fallback_missing_for_meaning_role"));
}

#[test]
fn reuse_below_two_consumers_is_rejected() {
    let mut packet = seed();
    // Drop every iconography binding except one, leaving the family with one consumer.
    let mut kept_one = false;
    packet.consumer_bindings.retain(|b| {
        if b.family == M5VisualInteractionFamily::Iconography {
            if kept_one {
                return false;
            }
            kept_one = true;
...
fn missing_canonical_reference_on_export_is_rejected() {
    ...
    packet.consumer_bindings[target].source_contract_refs =
        vec![M5_MOTION_LAYER_ICONOGRAPHY_MATRIX_DOC_REF.to_owned()];
...
fn unexpected_narrow_note_on_full_binding_is_rejected() {
    ...
    let target = packet
        ...
        .position(|b| !b.is_narrowed())
        .unwrap();
    packet.consumer_bindings[target].narrow_note = Some(VisualInteractionNarrowNote {
        reason: VisualInteractionNarrowReason::CompactionNarrowed,
        preserved_grammar_note: "x".to_owned(),
        next_action: VisualInteractionNarrowNextAction::ExpandInDesktop,
...
fn each_guardrail_is_enforced() {
    let cases: [(fn(&mut VisualInteractionConsumerBinding), &str); 5] = [
        ...
        (
            |b| b.lets_scrim_erase_orientation_or_contrast = true,
            "scrim_erases_orientation_or_contrast",
        ),
        (
            |b| b.lets_overlay_bypass_shared_z_order = true,
            "overlay_bypasses_shared_z_order",
        ),
        (
            |b| b.uses_unlabeled_icon_for_uncommon_or_destructive_action = true,
            "unlabeled_icon_for_uncommon_or_destructive_action",
        ),
        (
            |b| b.lets_illustration_impersonate_operational_or_security_truth = true,
...
fn family_coverage_gap_is_rejected() {
    ...
    packet
        .consumer_bindings
        .retain(|b| b.family != M5VisualInteractionFamily::Illustration);
...
fn missing_source_contract_is_rejected() {
    ...
    packet
        ...
        .retain(|r| r != M5_MOTION_LAYER_ICONOGRAPHY_SHARED_CONSUMERS_DOC_REF);
...
fn csv_has_a_row_per_binding() {
    ...
    assert!(
        lines[0].starts_with("family,consumer,representation,interaction_role_word,parity_state")
    );
}
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_motion_layer_iconography_shared_consumers_export()
        .expect("checked M5 visual-interaction shared-consumer export validates");
    assert_eq!(
        from_disk.packet_id,
        M5_MOTION_LAYER_ICONOGRAPHY_SHARED_CONSUMERS_PACKET_ID
    );
...
fn narrowed_fixture_variants_validate_and_keep_coverage() {
    let compact = seeded_m5_motion_layer_iconography_shared_consumers_compact_remote_narrowed();
    assert!(
...
fn checked_narrowed_fixtures_validate_and_match_builders() {
    let compact: M5VisualInteractionSharedConsumersPacket =
        serde_json::from_str(include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../fixtures/ui/m5-motion-layer-iconography-shared-consumers/compact_remote_narrowed.json"
        )))
    ...
    let exported: M5VisualInteractionSharedConsumersPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../fixtures/ui/m5-motion-layer-iconography-shared-consumers/exported_redaction_narrowed.json"
        )
    ))
