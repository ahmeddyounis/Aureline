...
fn clean_truth_feed_input() -> M5TruthFeedEntryResolutionInput {
    M5TruthFeedEntryResolutionInput {
        entry_id: "truth_feed:test".to_owned(),
        line_binding_id: "launch.line.core-team-canary".to_owned(),
        token_name: "line.truth_feed.core_team_canary".to_owned(),
        semantic_role: M5SupportedLineTransparencyRole::FreshnessWindow,
        report_section: M5TruthFeedKind::PublicProofSummary,
        surface_context: M5SupportedLineTruthFeedSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SupportedLineTruthFeedResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5AudiencePacketEntryResolutionInput {
    M5AudiencePacketEntryResolutionInput {
        ...
        semantic_role: M5SupportedLineTransparencyRole::FreshnessWindow,
        comparison_scope: M5AudiencePacketScope::SupportBundle,
        surface_context: M5SupportedLineTruthFeedSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SupportedLineTruthFeedResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_supported_line_truth_feed_and_audience_packet_registries();
...
#[test]
fn truth_feed_clean_names_meaning_and_is_bound() {
    let resolved = resolve_truth_feed_entry(clean_truth_feed_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.truth_feed_resolves_across_lines);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.truth_feed_object_complete);
    ...
    assert_eq!(resolved.report_section, "public_proof_summary");
    assert_eq!(
        resolved.canonical_report_section_mode,
        "public_proof_summary_mode"
    );
    assert_eq!(resolved.surface_context, "shiproom_surface");
    assert_eq!(
        resolved.next_action,
        M5SupportedLineTruthFeedNextAction::ExpandCohortMeaning
    );
}

#[test]
fn truth_feed_token_unstated_degrades() {
    let mut input = clean_truth_feed_input();
    input.token_name = "   ".to_owned();
    assert_eq!(
        resolve_truth_feed_entry(input).unwrap().degrade_reason,
        Some(M5TruthFeedEntryDegradeReason::DescriptorTokenUnstated)
    );
}

#[test]
fn truth_feed_unbound_and_unclassified_degrade() {
    let mut input = clean_truth_feed_input();
    ...
    let mut input = clean_truth_feed_input();
    input.report_section = M5TruthFeedKind::FeedSectionUnclassified;
    assert_eq!(
        resolve_truth_feed_entry(input).unwrap().degrade_reason,
...
#[test]
fn truth_feed_object_incomplete_and_widen_fold_and_form_degrade() {
    // An unstated bundle IDs field leaves the resolved object incomplete.
    let mut input = clean_truth_feed_input();
    input.bundle_ids = "  ".to_owned();
    let resolved = resolve_truth_feed_entry(input).unwrap();
    assert!(!resolved.truth_feed_object_complete);
    assert_eq!(
    ...
    let mut input = clean_truth_feed_input();
    input.resolution_form_coverage = vec![M5SupportedLineTruthFeedResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_truth_feed_entry(input).unwrap().degrade_reason,
...
#[test]
fn truth_feed_public_facing_and_surface_and_proof_degrade() {
    // A public-facing line running support language ahead of proof first fails the widen-boundary fold.
    let mut input = clean_truth_feed_input();
    input.report_section = M5TruthFeedKind::ClaimHistorySummary;
    ...
    let mut input = clean_truth_feed_input();
    input.surface_context = M5SupportedLineTruthFeedSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_truth_feed_entry(input).unwrap().degrade_reason,
...
#[test]
fn truth_feed_empty_id_and_forbidden_material_error() {
    let mut input = clean_truth_feed_input();
    input.entry_id = "".to_owned();
    assert_eq!(
        resolve_truth_feed_entry(input).unwrap_err(),
        M5SupportedLineTruthFeedResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_truth_feed_entry(input).unwrap_err(),
        M5SupportedLineTruthFeedResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5TruthFeedKind::FeedSectionUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn truth_feed_object_is_complete_requires_all_fields() {
    assert!(truth_feed_object_is_complete(
        M5TruthFeedKind::PublicProofSummary,
...
fn downgrade_clean_stays_honest() {
    let resolved = resolve_audience_packet_entry(clean_downgrade_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.comparison_safe_on_every_line);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.provides_complete_audience_packet);
    assert!(resolved.audience_packet_stays_honest);
    assert_eq!(resolved.comparison_scope, "support_bundle");
    assert_eq!(resolved.surface_context, "shiproom_surface");
}
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let resolved = resolve_audience_packet_entry(input).unwrap();
    assert!(!resolved.provides_complete_audience_packet);
    assert_eq!(
        resolved.degrade_reason,
    ...
    let mut input = clean_downgrade_input();
    input.keeps_audience_packet_visible = false;
    assert_eq!(
        resolve_audience_packet_entry(input).unwrap().degrade_reason,
        Some(M5AudiencePacketEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    );
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5AudiencePacketScope::AudiencePacketScopeUnclassified;
    assert_eq!(
        resolve_audience_packet_entry(input).unwrap().degrade_reason,
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    input.entry_id = "  ".to_owned();
    assert_eq!(
        resolve_audience_packet_entry(input).unwrap_err(),
        M5SupportedLineTruthFeedResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn downgrade_bound_support_and_flagged_gap_stay_clean() {
    ...
    assert!(resolve_audience_packet_entry(input).unwrap().is_clean());
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5SupportedLineTruthFeedAudiencePacketRegistriesViolation::VocabularySetDrift));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_TRUTH_FEED_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_AUDIENCE_PACKET_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5SupportedLineTruthFeedAudiencePacketRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5SupportedLineTruthFeedAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5SupportedLineTruthFeedExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    let mut packet = seeded_m5_supported_line_truth_feed_and_audience_packet_registries();
    packet.registry_rows[0].audience_packet_entries.clear();
    assert!(packet
        .validate()
        .contains(&M5SupportedLineTruthFeedAudiencePacketRegistriesViolation::ExamplesMissing));
}

#[test]
fn dishonest_clean_example_fails() {
    let mut packet = seeded_m5_supported_line_truth_feed_and_audience_packet_registries();
    // Force a clean truth_feed entry to also read as object-incomplete — the packet must reject it.
    let row = &mut packet.registry_rows[0];
    row.truth_feed_entries[0].degrade_reason = None;
    row.truth_feed_entries[0].truth_feed_object_complete = false;
    assert!(packet
        .validate()
        .contains(&M5SupportedLineTruthFeedAudiencePacketRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        match mutate {
            ...
            _ => row.collapses_distinct_audience_packet_classes_into_one_lane = true,
        }
        assert!(packet.validate().contains(
...
#[test]
fn truth_feed_not_proven_when_incomplete_example_removed() {
    let mut packet = seeded_m5_supported_line_truth_feed_and_audience_packet_registries();
    for row in &mut packet.registry_rows {
        row.truth_feed_entries.retain(|ex| {
            ex.degrade_reason
                != Some(M5TruthFeedEntryDegradeReason::CohortDescriptorObjectIncomplete)
        });
    }
...
#[test]
fn truth_feed_not_proven_when_surface_collapses() {
    let mut packet = seeded_m5_supported_line_truth_feed_and_audience_packet_registries();
    // Drop every clean executive-steering-surface truth_feed so the first-consumer surfaces no longer include it.
    for row in &mut packet.registry_rows {
        row.truth_feed_entries
...
#[test]
fn audience_packet_integrity_not_proven_when_support_ahead_example_removed() {
    let mut packet = seeded_m5_supported_line_truth_feed_and_audience_packet_registries();
    for row in &mut packet.registry_rows {
        row.audience_packet_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
...
#[test]
fn audience_packet_integrity_not_proven_when_scope_dropped() {
    let mut packet = seeded_m5_supported_line_truth_feed_and_audience_packet_registries();
    // Drop every clean go-no-go-signoff downgrade so the coverage no longer includes it.
    for row in &mut packet.registry_rows {
        row.audience_packet_entries
            .retain(|ex| !(ex.is_clean() && ex.comparison_scope == "partner_review_bundle"));
    }
    assert!(packet.validate().contains(
        &M5SupportedLineTruthFeedAudiencePacketRegistriesViolation::CohortEvidenceIntegrityNotProven
    ));
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet
        .validate()
        .contains(&M5SupportedLineTruthFeedAudiencePacketRegistriesViolation::RawMaterialInExport));
}

#[test]
fn export_carries_no_forbidden_raw_material() {
    let json =
        seeded_m5_supported_line_truth_feed_and_audience_packet_registries().export_safe_json();
...
#[test]
fn truth_feed_table_lists_only_clean_truth_feeds() {
    let packet = seeded_m5_supported_line_truth_feed_and_audience_packet_registries();
    let table = packet.render_truth_feed_table();
    // The clean canary and migration truth_feeds are rendered from the registry.
    assert!(table.contains("public_proof_summary_mode"));
    assert!(table.contains("migration_scoreboard_summary_mode"));
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk =
        current_stable_m5_supported_line_truth_feed_and_audience_packet_registries_export().expect(
            "checked M5 line-truth_feed / line-downgrade-packet registries export validates",
        );
    assert_eq!(
        from_disk.packet_id,
        M5_SUPPORTED_LINE_TRUTH_FEED_AUDIENCE_PACKET_REGISTRIES_PACKET_ID
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5SupportedLineTruthFeedAudiencePacketRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-supported-line-truth-feed-and-audience-packet-registries/truth_feed_beta_narrowed.json"
    )))
    .expect("line-truth_feed fixture parses");
    ...
    let preview: M5SupportedLineTruthFeedAudiencePacketRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-supported-line-truth-feed-and-audience-packet-registries/audience_packet_preview_narrowed.json"
    )))
