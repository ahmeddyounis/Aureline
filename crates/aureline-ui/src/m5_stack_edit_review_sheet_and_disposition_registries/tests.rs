...
fn clean_review_pack_record_input() -> M5StackEditRecordEntryResolutionInput {
    M5StackEditRecordEntryResolutionInput {
        ...
        report_section: M5StackEditRecordKind::ChangedFilesScope,
        surface_context: M5StackEditRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5StackEditRecordResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5StackEditResultEntryResolutionInput {
    M5StackEditResultEntryResolutionInput {
        ...
        semantic_role: M5ChangeOrchestrationRole::SelectedChangeObjectDisclosure,
        comparison_scope: M5StackEditResultScope::EvaluatedScopeBinding,
        surface_context: M5StackEditRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5StackEditRecordResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_stack_edit_review_sheet_and_disposition_registries();
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(
        ...
        M5StackEditRecordNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5StackEditRecordKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_object_incomplete_and_widen_fold_and_form_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.resolution_form_coverage = vec![M5StackEditRecordResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5StackEditRecordKind::FullTreeScope;
    ...
    let mut input = clean_review_pack_record_input();
    input.surface_context = M5StackEditRecordSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        ...
        M5StackEditRecordResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input).unwrap_err(),
        M5StackEditRecordResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5StackEditRecordKind::PackScopeUnclassified,
        true,
        false,
        true
    ));
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5StackEditResultScope::ChangeObjectResultUnclassified;
    assert_eq!(
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    assert_eq!(
        ...
        M5StackEditRecordResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::VocabularySetDrift));
}
...
fn missing_source_contracts_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::MissingSourceContracts));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_STACK_EDIT_REVIEW_SHEET_DOMAIN_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::DomainSchemaRefMissing));

    let mut packet = seeded_m5_stack_edit_review_sheet_and_disposition_registries();
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_STACK_EDIT_DISPOSITION_DOMAIN_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::DomainSchemaRefMissing));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5StackEditRecordAnatomyPart::Identity);
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::MandatoryAnatomyMissing));
}
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5StackEditRecordExportField::CohortArchetypes);
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::MandatoryExportFieldMissing));
}
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::ExamplesMissing));
}
...
fn dishonest_clean_example_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        assert!(packet
            .validate()
            .contains(&M5StackEditRecordAndResultRegistriesViolation::RowInvariantViolated));
    }
}
...
fn review_pack_result_integrity_not_proven_when_support_ahead_example_removed() {
    ...
    assert!(packet.validate().contains(
        &M5StackEditRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven
    ));
}
...
fn governance_review_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::GovernanceReviewIncomplete));
}
...
fn consumer_projection_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete));
}
...
fn proof_freshness_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::ProofFreshnessIncomplete));
}
...
fn release_posture_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::ReleasePostureIncomplete));
}
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet
        .validate()
        .contains(&M5StackEditRecordAndResultRegistriesViolation::RawMaterialInExport));
}

#[test]
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_stack_edit_review_sheet_and_disposition_registries().export_safe_json();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_stack_edit_review_sheet_and_disposition_registries_export()
        .expect(
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5StackEditRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-stack-edit-review-sheet-and-disposition-registries/stack_edit_beta_narrowed.json"
    )))
    ...
    let preview: M5StackEditRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-stack-edit-review-sheet-and-disposition-registries/disposition_preview_narrowed.json"
    )))
