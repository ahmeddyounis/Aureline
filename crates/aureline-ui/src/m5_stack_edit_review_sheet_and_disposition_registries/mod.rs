//! Implemented M5 stack-edit review sheet and disposition registries.
//!
//! The frozen [change-object / patch-stack / landing matrix][matrix] names Aureline's governed change-orchestration
//! object classes and locks their controlled vocabulary. This module makes the matrix's `stack_edit_review_sheet`
//! object class operable — as a per-operation review sheet and the disposition of the plan it proposes — so the Git,
//! patch-stack / queue, review, provider-landing, help / docs, and support / export surfaces inherit one canonical
//! stack-edit review descriptor rather than a hand-authored parallel prose that has to be kept consistent: it turns
//! the *stack-edit review sheet* grammar (one durable review sheet per proposed reorder, split, squash, or restack —
//! one typed field per section: the original order, the proposed order, the affected parent/child links, the stale
//! validation or approval impact, and the resulting branch/worktree consequences) and the *disposition* grammar (the
//! explicit continue, abort, export, and defer paths kept separate so a proposed re-stack plan is never lost when a
//! provider write, hosted approval, local validation, or policy boundary goes stale or blocks publish) into registry
//! resolvers that produce export-safe, honest projections. Every proposed stack edit then resolves to one typed
//! review sheet — the original and proposed order it discloses, the affected parent/child links, the stale-evidence
//! impact, and the resulting branch/worktree consequences, all attributable to the sheet so a stack edit never
//! executes without a review surface — and to one disposition object — the resolved operation, the source of any
//! staleness that gates a continue, the offered dispositions, and whether the proposed plan stays preserved for retry
//! or export — that the Git, patch-stack, provider-landing, and support surfaces can inspect without manual
//! reconstruction, so a proposed reorder's parent-child consequence, stale-check impact, and target-branch disclosure
//! stay visible metadata, no member is silently reordered, and nothing lands from ambient branch state; a review sheet
//! or disposition that cannot bind a field to its original-versus-proposed ordering degrades honestly instead of
//! letting a local-only continue read as a provider-committed landing.
//!
...
//!   unless it names a canonical registry token, a classified [scope selector][M5StackEditRecordKind],
//!   a disclosure role, covers every [resolution form][M5StackEditRecordResolutionForm] (the
...
//!   [`M5StackEditRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
...
//!   [evaluator binding][M5StackEditResultScope]
...
//!   [`M5StackEditResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
...
pub use seed::{
    seeded_m5_stack_edit_review_sheet_and_disposition_registries,
    seeded_m5_stack_edit_review_sheet_and_disposition_registries_review_pack_record_beta_narrowed,
    seeded_m5_stack_edit_review_sheet_and_disposition_registries_review_pack_result_preview_narrowed,
    M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5StackEditRecordAndResultRegistriesPacket`].
pub const M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_stack_edit_review_sheet_and_disposition_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_DOC_REF: &str =
    "docs/team-workflows/m5_stack_edit_review_sheet_and_disposition_registries.md";
...
pub const M5_STACK_EDIT_DISPOSITION_DOMAIN_SCHEMA_REF: &str =
    "schemas/ui/m5-stack-edit-disposition.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-stack-edit-review-sheet-and-disposition-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-stack-edit-review-sheet-and-disposition-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-stack-edit-review-sheet-and-disposition-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/teamwork/m5-stack-edit-review-sheet-and-disposition-registries";
...
pub type M5StackEditRecordAndResultRegistriesConsumerSurface = M5ChangeOrchestrationConsumerSurface;
...
pub enum M5StackEditRecordResolutionForm {
...
impl M5StackEditRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StackEditRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StackEditResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StackEditRecordSurfaceContext {
...
impl M5StackEditRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StackEditRecordAnatomyPart {
...
impl M5StackEditRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StackEditRecordNextAction {
...
impl M5StackEditRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StackEditRecordExportField {
...
impl M5StackEditRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StackEditRecordEntryDegradeReason {
...
impl M5StackEditRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5StackEditRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5StackEditRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5StackEditRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5StackEditRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5StackEditRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5StackEditResultEntryDegradeReason {
...
impl M5StackEditResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5StackEditRecordNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5StackEditRecordNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5StackEditRecordNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5StackEditRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5StackEditRecordEntryResolutionInput {
    ...
    pub report_section: M5StackEditRecordKind,
    /// The render / surface context.
    pub surface_context: M5StackEditRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5StackEditRecordResolutionForm>,
...
pub struct M5ResolvedStackEditRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5StackEditRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5StackEditRecordNextAction,
...
impl M5ResolvedStackEditRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5StackEditResultEntryResolutionInput {
    ...
    pub comparison_scope: M5StackEditResultScope,
    /// The render / surface context.
    pub surface_context: M5StackEditRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5StackEditRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedStackEditResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5StackEditResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5StackEditRecordNextAction,
...
impl M5ResolvedStackEditResultEntry {
...
pub enum M5StackEditRecordResolutionError {
...
impl M5StackEditRecordResolutionError {
...
impl fmt::Display for M5StackEditRecordResolutionError {
...
impl Error for M5StackEditRecordResolutionError {}

fn form_tokens(forms: &[M5StackEditRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5StackEditRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5StackEditRecordResolutionForm> = forms.iter().copied().collect();
    M5StackEditRecordResolutionForm::ALL
        .iter()
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedStackEditRecordEntry, M5StackEditRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5StackEditRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5StackEditRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5StackEditRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5StackEditRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5StackEditRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5StackEditRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5StackEditRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5StackEditRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5StackEditRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5StackEditRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5StackEditRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5StackEditRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedStackEditRecordEntry {
...
pub fn resolve_review_pack_result_entry(
    input: M5StackEditResultEntryResolutionInput,
) -> Result<M5ResolvedStackEditResultEntry, M5StackEditRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5StackEditRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5StackEditResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5StackEditResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5StackEditResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5StackEditResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5StackEditResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5StackEditResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5StackEditRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedStackEditResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StackEditRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5StackEditRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5StackEditRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5StackEditRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedStackEditRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedStackEditResultEntry>,
...
impl M5StackEditRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5StackEditRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5StackEditRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5StackEditRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5StackEditRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedStackEditRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedStackEditResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StackEditRecordAndResultRegistriesVocabularySet {
...
impl M5StackEditRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ChangeOrchestrationRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5StackEditRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5StackEditRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5StackEditResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5StackEditRecordSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5StackEditRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5StackEditResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5StackEditRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5StackEditRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5StackEditRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StackEditRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StackEditRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StackEditRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StackEditRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5StackEditRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5StackEditRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5StackEditRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5StackEditRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5StackEditRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5StackEditRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5StackEditRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5StackEditRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5StackEditRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION`].
...
impl M5StackEditRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5StackEditRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5StackEditRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND {
            violations.push(M5StackEditRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_STACK_EDIT_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5StackEditRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5StackEditRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5StackEditRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5StackEditRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Stack-Edit-Review-Sheet and Disposition Registries\n\n");
...
#[derive(Debug)]
pub enum M5StackEditRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5StackEditRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5StackEditRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5StackEditRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5StackEditRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5StackEditRecordAndResultRegistriesViolation {
...
impl M5StackEditRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_stack_edit_review_sheet_and_disposition_registries_export() -> Result<
    M5StackEditRecordAndResultRegistriesPacket,
    M5StackEditRecordAndResultRegistriesArtifactError,
> {
    let packet: M5StackEditRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-stack-edit-review-sheet-and-disposition-registries-proof/support_export.json"
        )
    ))
    .map_err(M5StackEditRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5StackEditRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5StackEditRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5StackEditRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(M5StackEditRecordAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5StackEditRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(M5StackEditRecordAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(M5StackEditRecordAndResultRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations
                .push(M5StackEditRecordAndResultRegistriesViolation::MandatoryExportFieldMissing);
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_STACK_EDIT_REVIEW_SHEET_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_STACK_EDIT_DISPOSITION_DOMAIN_SCHEMA_REF)
        {
            violations.push(M5StackEditRecordAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5StackEditRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5StackEditRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5StackEditRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5StackEditRecordAndResultRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5StackEditRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete);
            return;
        }
    }
}
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5StackEditRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5StackEditRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5StackEditRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5StackEditRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5StackEditRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    let unbound_degrades = review_pack_records().any(|ex| {
        ex.degrade_reason == Some(M5StackEditRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    ...
    {
        violations.push(
            M5StackEditRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5StackEditResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations
            .push(M5StackEditRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven);
    }
}
