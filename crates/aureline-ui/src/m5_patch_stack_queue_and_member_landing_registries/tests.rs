...
fn clean_review_pack_record_input() -> M5PatchStackRecordEntryResolutionInput {
    M5PatchStackRecordEntryResolutionInput {
        ...
        report_section: M5PatchStackRecordKind::ChangedFilesScope,
        surface_context: M5PatchStackRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5PatchStackRecordResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5PatchStackResultEntryResolutionInput {
    M5PatchStackResultEntryResolutionInput {
        ...
        semantic_role: M5ChangeOrchestrationRole::SelectedChangeObjectDisclosure,
        comparison_scope: M5PatchStackResultScope::EvaluatedScopeBinding,
        surface_context: M5PatchStackRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5PatchStackRecordResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_patch_stack_queue_and_member_landing_registries();
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(
        ...
        M5PatchStackRecordNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5PatchStackRecordKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_object_incomplete_and_widen_fold_and_form_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.resolution_form_coverage = vec![M5PatchStackRecordResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5PatchStackRecordKind::FullTreeScope;
    ...
    let mut input = clean_review_pack_record_input();
    input.surface_context = M5PatchStackRecordSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        ...
        M5PatchStackRecordResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input).unwrap_err(),
        M5PatchStackRecordResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5PatchStackRecordKind::PackScopeUnclassified,
        true,
        false,
        true
    ));
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5PatchStackResultScope::ChangeObjectResultUnclassified;
    assert_eq!(
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    assert_eq!(
        ...
        M5PatchStackRecordResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn vocabulary_set_is_canonical() {
    assert!(seeded_m5_patch_stack_queue_and_member_landing_registries()
        .vocabulary_set
        .matches_canonical());
}
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::VocabularySetDrift));
}
...
fn missing_source_contracts_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::MissingSourceContracts));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_PATCH_STACK_QUEUE_DOMAIN_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::DomainSchemaRefMissing));

    let mut packet = seeded_m5_patch_stack_queue_and_member_landing_registries();
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_STACK_MEMBER_LANDING_DOMAIN_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::DomainSchemaRefMissing));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5PatchStackRecordAnatomyPart::Identity);
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::MandatoryAnatomyMissing));
}
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5PatchStackRecordExportField::CohortArchetypes);
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::MandatoryExportFieldMissing));
}
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::ExamplesMissing));
}
...
fn dishonest_clean_example_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        assert!(packet
            .validate()
            .contains(&M5PatchStackRecordAndResultRegistriesViolation::RowInvariantViolated));
    }
}
...
fn review_pack_result_integrity_not_proven_when_support_ahead_example_removed() {
    ...
    assert!(packet.validate().contains(
        &M5PatchStackRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven
    ));
}
...
fn governance_review_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::GovernanceReviewIncomplete));
}
...
fn consumer_projection_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete));
}
...
fn proof_freshness_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::ProofFreshnessIncomplete));
}
...
fn release_posture_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::ReleasePostureIncomplete));
}
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet
        .validate()
        .contains(&M5PatchStackRecordAndResultRegistriesViolation::RawMaterialInExport));
}

#[test]
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_patch_stack_queue_and_member_landing_registries().export_safe_json();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_patch_stack_queue_and_member_landing_registries_export()
        .expect(
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5PatchStackRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-patch-stack-queue-and-member-landing-registries/patch_stack_beta_narrowed.json"
    )))
    ...
    let preview: M5PatchStackRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-patch-stack-queue-and-member-landing-registries/member_landing_preview_narrowed.json"
    )))
