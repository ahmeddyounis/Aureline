//! Implemented M5 patch-stack / queue and member-landing registries.
//!
//! The frozen [change-object / patch-stack / landing matrix][matrix] names Aureline's governed change-orchestration
//! object classes and locks their controlled vocabulary. This module makes the matrix's `patch_stack_queue` object
//! class operable — as a durable stack record and ordered member-card view — so the Git, patch-stack / queue, review,
//! provider-landing, help / docs, and support / export surfaces inherit one canonical patch-stack descriptor rather
//! than a hand-authored parallel prose that has to be kept consistent: it turns the *patch-stack* grammar (one durable
//! record per stacked change — one typed field per stack section: the stack ID, the ordered member IDs, the
//! parent/child relation, the landing order, the rebase epoch, the inherited blockers, and the current validation
//! freshness) and the *member-landing* grammar (each stack member card keeping its local stack state, its
//! provider-linked review state, and its queue/landing posture explicitly separate) into registry resolvers that
//! produce export-safe, honest projections. Every stacked change then resolves to one typed patch-stack record — the
//! stack ID and ordered members it publishes, the parent/child relation, the landing order, the rebase epoch, the
//! inherited blockers, and the validation freshness, all attributable to the stack so a branch-naming convention never
//! reads as reviewed stack membership — and to one member-landing object — the resolved member identity, its position
//! in landing order, and the active landing-state reason (selected change, stale validation, restack required, queue
//! eligible, queue blocked, protected-branch blocked, orphaned, abandoned, exported, or imported / reopened) — that
//! the Git, patch-stack, provider-landing, and support surfaces can inspect without manual reconstruction, so which
//! member blocks another, which checks are stale, and which members are ready, blocked, or not yet landing candidates
//! stay visible metadata, no member is silently reordered, and nothing lands from ambient branch state; a stack row or
//! member card that cannot bind a field to its stack ID or ordered-member set degrades honestly instead of letting
//! ambient branch state read as a reviewed landing candidate.
//!
...
//!   unless it names a canonical registry token, a classified [scope selector][M5PatchStackRecordKind],
//!   a disclosure role, covers every [resolution form][M5PatchStackRecordResolutionForm] (the
...
//!   [`M5PatchStackRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
...
//!   [evaluator binding][M5PatchStackResultScope]
...
//!   [`M5PatchStackResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
...
pub use seed::{
    seeded_m5_patch_stack_queue_and_member_landing_registries,
    seeded_m5_patch_stack_queue_and_member_landing_registries_review_pack_record_beta_narrowed,
    seeded_m5_patch_stack_queue_and_member_landing_registries_review_pack_result_preview_narrowed,
    M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5PatchStackRecordAndResultRegistriesPacket`].
pub const M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_patch_stack_queue_and_member_landing_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_SCHEMA_REF: &str =
    "schemas/teamwork/m5-patch-stack-queue-and-member-landing-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_DOC_REF: &str =
    "docs/team-workflows/m5_patch_stack_queue_and_member_landing_registries.md";
...
pub const M5_STACK_MEMBER_LANDING_DOMAIN_SCHEMA_REF: &str =
    "schemas/ui/m5-stack-member-landing.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-patch-stack-queue-and-member-landing-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-patch-stack-queue-and-member-landing-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-patch-stack-queue-and-member-landing-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/teamwork/m5-patch-stack-queue-and-member-landing-registries";
...
pub type M5PatchStackRecordAndResultRegistriesConsumerSurface =
...
pub enum M5PatchStackRecordResolutionForm {
...
impl M5PatchStackRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PatchStackRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PatchStackResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PatchStackRecordSurfaceContext {
...
impl M5PatchStackRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PatchStackRecordAnatomyPart {
...
impl M5PatchStackRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PatchStackRecordNextAction {
...
impl M5PatchStackRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PatchStackRecordExportField {
...
impl M5PatchStackRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PatchStackRecordEntryDegradeReason {
...
impl M5PatchStackRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5PatchStackRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5PatchStackRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5PatchStackRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5PatchStackRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5PatchStackRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PatchStackResultEntryDegradeReason {
...
impl M5PatchStackResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5PatchStackRecordNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5PatchStackRecordNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5PatchStackRecordNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5PatchStackRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5PatchStackRecordEntryResolutionInput {
    ...
    pub report_section: M5PatchStackRecordKind,
    /// The render / surface context.
    pub surface_context: M5PatchStackRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5PatchStackRecordResolutionForm>,
...
pub struct M5ResolvedPatchStackRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5PatchStackRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5PatchStackRecordNextAction,
...
impl M5ResolvedPatchStackRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5PatchStackResultEntryResolutionInput {
    ...
    pub comparison_scope: M5PatchStackResultScope,
    /// The render / surface context.
    pub surface_context: M5PatchStackRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5PatchStackRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedPatchStackResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5PatchStackResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5PatchStackRecordNextAction,
...
impl M5ResolvedPatchStackResultEntry {
...
pub enum M5PatchStackRecordResolutionError {
...
impl M5PatchStackRecordResolutionError {
...
impl fmt::Display for M5PatchStackRecordResolutionError {
...
impl Error for M5PatchStackRecordResolutionError {}

fn form_tokens(forms: &[M5PatchStackRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5PatchStackRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5PatchStackRecordResolutionForm> = forms.iter().copied().collect();
    M5PatchStackRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5PatchStackRecordKind,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedPatchStackRecordEntry, M5PatchStackRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5PatchStackRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5PatchStackRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5PatchStackRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5PatchStackRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5PatchStackRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5PatchStackRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5PatchStackRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5PatchStackRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5PatchStackRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5PatchStackRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5PatchStackRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5PatchStackRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedPatchStackRecordEntry {
...
pub fn resolve_review_pack_result_entry(
    input: M5PatchStackResultEntryResolutionInput,
) -> Result<M5ResolvedPatchStackResultEntry, M5PatchStackRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5PatchStackRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5PatchStackResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5PatchStackResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5PatchStackResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5PatchStackResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5PatchStackResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5PatchStackResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5PatchStackRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedPatchStackResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PatchStackRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5PatchStackRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5PatchStackRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5PatchStackRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedPatchStackRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedPatchStackResultEntry>,
...
impl M5PatchStackRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5PatchStackRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5PatchStackRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5PatchStackRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5PatchStackRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedPatchStackRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedPatchStackResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PatchStackRecordAndResultRegistriesVocabularySet {
...
impl M5PatchStackRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ChangeOrchestrationRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5PatchStackRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5PatchStackRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5PatchStackResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5PatchStackRecordSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5PatchStackRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5PatchStackResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5PatchStackRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5PatchStackRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5PatchStackRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PatchStackRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PatchStackRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PatchStackRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PatchStackRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5PatchStackRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5PatchStackRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5PatchStackRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5PatchStackRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5PatchStackRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5PatchStackRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5PatchStackRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5PatchStackRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PatchStackRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION`].
...
impl M5PatchStackRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5PatchStackRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5PatchStackRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND {
            violations.push(M5PatchStackRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_PATCH_STACK_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5PatchStackRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5PatchStackRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5PatchStackRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5PatchStackRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
...
#[derive(Debug)]
pub enum M5PatchStackRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5PatchStackRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5PatchStackRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5PatchStackRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5PatchStackRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5PatchStackRecordAndResultRegistriesViolation {
...
impl M5PatchStackRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_patch_stack_queue_and_member_landing_registries_export() -> Result<
    M5PatchStackRecordAndResultRegistriesPacket,
    M5PatchStackRecordAndResultRegistriesArtifactError,
> {
    let packet: M5PatchStackRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-patch-stack-queue-and-member-landing-registries-proof/support_export.json"
        )
    ))
    .map_err(M5PatchStackRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5PatchStackRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5PatchStackRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5PatchStackRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(M5PatchStackRecordAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5PatchStackRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(M5PatchStackRecordAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5PatchStackRecordAndResultRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations
                .push(M5PatchStackRecordAndResultRegistriesViolation::MandatoryExportFieldMissing);
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_PATCH_STACK_QUEUE_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_STACK_MEMBER_LANDING_DOMAIN_SCHEMA_REF)
        {
            violations.push(M5PatchStackRecordAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5PatchStackRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5PatchStackRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5PatchStackRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5PatchStackRecordAndResultRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5PatchStackRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete);
            return;
        }
    }
}
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5PatchStackRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5PatchStackRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5PatchStackRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5PatchStackRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5PatchStackRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5PatchStackRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5PatchStackResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations
            .push(M5PatchStackRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven);
    }
}
