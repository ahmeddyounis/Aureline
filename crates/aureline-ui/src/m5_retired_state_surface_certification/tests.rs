//! Tests for the M05-1246 retired-state surface certification capstone.

use super::*;

fn packet() -> RetiredStateProfileCertificationPacket {
    seeded_m5_retired_state_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, RETIRED_STATE_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, RETIRED_STATE_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5RetiredStateCertifiedProfile::ALL {
...
fn every_frozen_family_is_certified_on_some_profile() {
    ...
    for family in M5RetiredStateObject::ALL {
        assert!(
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            RetiredStateCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, RetiredStateAxisCertificationState::Certified);
...
fn a_narrowed_profile_never_keeps_a_trusted_claim() {
    for row in &packet().rows {
        if row.is_claim_narrowed() {
            assert!(
                !row.certified_claim.asserts_certified_retired_closure(),
...
fn all_five_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5RetiredStateClaim> = p.rows.iter().map(|r| r.certified_claim).collect();
    for claim in M5RetiredStateClaim::ALL {
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5RetiredStateCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5RetiredStateCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = RetiredStateCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), RetiredStateCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in RetiredStateCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == RetiredStateCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    // The B148 localization axis is present, distinct, and exercised by the
    // disclosed-archive-partial profile as its binding narrowed axis.
    let p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5RetiredStateCertifiedProfile::DisclosedArchivePartialProfile)
        .expect("disclosed-archive-partial-profile row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(RetiredStateCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(RetiredStateCertificationAxis::Localization)
    );
}

#[test]
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    // The B148 high-contrast reach axis stays certified across the seed (never a binding narrowed axis here).
    for row in &packet().rows {
        let outcome = row
            .axis(RetiredStateCertificationAxis::HighContrast)
            .expect("high-contrast axis present");
        assert_eq!(outcome.state, RetiredStateAxisCertificationState::Certified);
    }
}
...
fn the_three_yellow_rows_bind_three_distinct_axes_and_triggers() {
    ...
    // per-lane concern (archive coverage / deployment-profile propagation / closure-ledger retention).
    let p = packet();
    let axes: BTreeSet<RetiredStateCertificationAxis> = p
        .rows
        .iter()
    ...
    let triggers: std::collections::HashSet<M5RetiredStateDowngradeTrigger> = p
        .rows
        .iter()
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5RetiredStateCertifiedProfile::LiveRetiredStateClosureLane)
        .expect("live-retired-state-closure-lane row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RetiredStateCertificationAxis::Visual {
            outcome.state = RetiredStateAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("setting definition silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, RetiredStateProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, RetiredStateCertificationViolation::ProfileBlocked { .. })));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5RetiredStateCertifiedProfile::ReviewableRetirementRecordStructure
        })
        .expect("reviewable-retirement-record-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails
        .lets_a_retired_surface_disappear_without_tombstone_archival_route_or_successor_pointer =
        true;
    assert_eq!(row.derive_status(), RetiredStateProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        RetiredStateCertificationViolation::GuardrailViolated { .. }
    )));
}
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5RetiredStateClaim::CertifiedRetiredClosure;
    row.certified_claim = M5RetiredStateClaim::CertifiedRetiredClosure;
    assert_eq!(row.derive_status(), RetiredStateProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RetiredStateCertificationAxis::DegradedState {
            *outcome = RetiredStateAxisOutcome {
                axis: RetiredStateCertificationAxis::DegradedState,
                state: RetiredStateAxisCertificationState::DisclosedNarrowed,
                parity_note: "the retired-state propagation is lagging".to_owned(),
                narrowing_reason: Some(
                    "the retired-state propagation is partial for this profile".to_owned(),
                ),
    ...
    // Claim stays ReviewableRetirementRecord == certified, no claim_auto_narrow.
    assert_eq!(row.derive_status(), RetiredStateProfileClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RetiredStateCertificationAxis::CliExport {
            outcome.state = RetiredStateAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger =
                Some(M5RetiredStateDowngradeTrigger::SupportNoteClosureIncomplete);
        }
    }
    assert_eq!(row.derive_status(), RetiredStateProfileClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(RetiredStateClaimAutoNarrow {
        binding_axis: RetiredStateCertificationAxis::DegradedState,
        from_claim: M5RetiredStateClaim::CertifiedRetiredClosure,
        to_claim: M5RetiredStateClaim::CertifiedRetiredClosure,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5RetiredStateClaim::ArchiveDisclosedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-retirement-record-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableRetirementRecord; certify a stronger certified operating line.
    row.certified_claim = M5RetiredStateClaim::CertifiedRetiredClosure;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = RetiredStateCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), RetiredStateProfileClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RetiredStateCertificationAxis::CliExport {
            outcome.state = RetiredStateAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the archive-partial surface".to_owned());
            outcome.downgrade_trigger =
                Some(M5RetiredStateDowngradeTrigger::SupportNoteClosureIncomplete);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = RetiredStateCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), RetiredStateProfileClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == RetiredStateProfileClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "propagation".to_owned();
    }
    assert_eq!(row.derive_status(), RetiredStateProfileClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(RetiredStateCertificationAxis::Visual);
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.profile != M5RetiredStateCertifiedProfile::UnverifiedClosureLedgerProfile);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        RetiredStateCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the certified-archetype line from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5RetiredStateObject::SupportedLine);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        RetiredStateCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = RetiredStateProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        RetiredStateCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, RetiredStateCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, RetiredStateCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != RetiredStateCertificationAxis::RetiredStateTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        RetiredStateCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, RetiredStateCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        RetiredStateCertificationViolation::RawRetiredStateMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: RetiredStateProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_retired_state_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-retired-state-surface-certification/support_export.json"
    ));
    let packet_from_fixture: RetiredStateProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-retired-state-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_RETIRED_STATE_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures
...
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_RETIRED_STATE_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_retired_state_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/release/m5-retired-state-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-retired-state-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/release/m5-retired-state-surface-certification");
