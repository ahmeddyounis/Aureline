//! Canonical seed builders for the M5 line-defect_ledger and line-downgrade-packet registries packet.
//!
//! These builders are the single producer of the checked-in support export and the narrowed fixtures.
//! The headless emitter and the inline tests both call them so the in-code registries, the artifact, and the
//! fixtures never drift. Every resolved example is built by calling the real resolvers so the packet can only
//! carry projections the resolvers actually produce. Clean line-defect_ledger and line-downgrade-packet entries
//! are built so the one typed line-defect_ledger object resolving per line, lines never widening without
...
pub const M5_STABLE_LINE_DEFECT_LEDGER_BACKPORT_DECISION_TIMER_REGISTRIES_PACKET_ID: &str =
    "m5-stable-line-defect-ledger-and-backport-decision-timer-registries:stable:0001";
...
fn defect_ledger(input: M5DefectLedgerEntryResolutionInput) -> M5ResolvedDefectLedgerEntry {
    resolve_defect_ledger_entry(input).expect("seed line-defect_ledger entry resolves")
}

fn downgrade(
    input: M5BackportDecisionTimerEntryResolutionInput,
) -> M5ResolvedBackportDecisionTimerEntry {
    resolve_backport_decision_timer_entry(input).expect("seed line-downgrade-packet entry resolves")
}

fn all_forms() -> Vec<M5StableLineDefectLedgerResolutionForm> {
    M5StableLineDefectLedgerResolutionForm::ALL.to_vec()
}

// -- Clean line-defect_ledger entries (typed object, rollback / diagnostics preserved, bound to the registry) --

#[allow(clippy::too_many_arguments)]
fn clean_defect_ledger_base(
    entry_id: &str,
    line_binding_id: &str,
    token_name: &str,
    semantic_role: M5StableLineProtectionRole,
    ledger_defect: M5DefectLedgerKind,
    surface_context: M5StableLineDefectLedgerSurfaceContext,
...
fn defect_ledger_crash_recovery_defect_clean() -> M5ResolvedDefectLedgerEntry {
    defect_ledger(clean_defect_ledger_base(
        "defect_ledger:shiproom:dogfood-core-team-canary",
        "launch.line.core-team-canary",
        "line.defect_ledger.core_team_canary",
        M5StableLineProtectionRole::SupportWindow,
        M5DefectLedgerKind::CrashRecoveryDefect,
        M5StableLineDefectLedgerSurfaceContext::ShiproomSurface,
...
fn defect_ledger_rollback_update_defect_clean() -> M5ResolvedDefectLedgerEntry {
    defect_ledger(clean_defect_ledger_base(
        "defect_ledger:release-center:migration-alpha",
        "launch.line.migration-alpha",
        "line.defect_ledger.rollback_update_defect",
        M5StableLineProtectionRole::CorrectionOwnership,
        M5DefectLedgerKind::RollbackUpdateDefect,
        M5StableLineDefectLedgerSurfaceContext::ReleaseCenterSurface,
...
fn defect_ledger_support_export_defect_clean() -> M5ResolvedDefectLedgerEntry {
    defect_ledger(clean_defect_ledger_base(
        "defect_ledger:program-governance:extension-author",
        "launch.line.extension-author",
        "line.defect_ledger.support_export_defect",
        M5StableLineProtectionRole::EvidenceRefresh,
        M5DefectLedgerKind::SupportExportDefect,
        M5StableLineDefectLedgerSurfaceContext::ProgramGovernanceSurface,
...
fn defect_ledger_migration_import_defect_clean() -> M5ResolvedDefectLedgerEntry {
    // A design-partner preview line is public-facing and keeps its support language matched to line proof.
    let mut base = clean_defect_ledger_base(
        "defect_ledger:executive-steering:design-partner-preview",
        "launch.line.design-partner-preview",
        "line.defect_ledger.migration_import_defect",
        M5StableLineProtectionRole::SupportWindow,
        M5DefectLedgerKind::MigrationImportDefect,
        M5StableLineDefectLedgerSurfaceContext::ExecutiveSteeringSurface,
...
fn defect_ledger_compatibility_regression_defect_clean() -> M5ResolvedDefectLedgerEntry {
    // A public preview line is public-facing and keeps its support language matched to line proof.
    let mut base = clean_defect_ledger_base(
        "defect_ledger:support:public-preview",
        "launch.line.public-preview",
        "line.defect_ledger.compatibility_regression_defect",
        M5StableLineProtectionRole::CorrectionOwnership,
        M5DefectLedgerKind::CompatibilityRegressionDefect,
        M5StableLineDefectLedgerSurfaceContext::SupportOrExportForm,
...
fn defect_ledger_security_or_data_loss_defect_clean() -> M5ResolvedDefectLedgerEntry {
    defect_ledger(clean_defect_ledger_base(
        "defect_ledger:release-center:certified-journey",
        "launch.line.certified-journey",
        "line.defect_ledger.security_or_data_loss_defect",
        M5StableLineProtectionRole::BackportDecision,
        M5DefectLedgerKind::SecurityOrDataLossDefect,
        M5StableLineDefectLedgerSurfaceContext::ReleaseCenterSurface,
...
// -- Degraded line-defect_ledger entries ---------------------------------------------------------

/// Degraded defect_ledger entry: the resolved defect_ledger object is incomplete — the bundle IDs are unstated.
fn defect_ledger_object_incomplete() -> M5ResolvedDefectLedgerEntry {
    let mut base = clean_defect_ledger_base(
        "defect_ledger:shiproom:incomplete",
        "launch.line.core-team-canary",
        "line.defect_ledger.core_team_canary",
        M5StableLineProtectionRole::SupportWindow,
        M5DefectLedgerKind::CrashRecoveryDefect,
...
/// Degraded defect_ledger entry: the line's rollback and diagnostics posture is not preserved before widening —
...
fn defect_ledger_widen_fold() -> M5ResolvedDefectLedgerEntry {
    let mut base = clean_defect_ledger_base(
        "defect_ledger:release-center:widen-fold",
        "launch.line.migration-alpha",
        "line.defect_ledger.rollback_update_defect",
        M5StableLineProtectionRole::CorrectionOwnership,
        M5DefectLedgerKind::RollbackUpdateDefect,
...
/// Degraded defect_ledger entry: the behavior is a hand-copied per-entry assumption instead of tracing to the
/// registry.
fn defect_ledger_unbound() -> M5ResolvedDefectLedgerEntry {
    let mut base = clean_defect_ledger_base(
        "defect_ledger:executive-steering:unbound",
        "launch.line.certified-journey",
        "line.defect_ledger.security_or_data_loss_defect",
        M5StableLineProtectionRole::BackportDecision,
        M5DefectLedgerKind::SecurityOrDataLossDefect,
...
/// Degraded defect_ledger entry: the canonical registry token name is unstated.
fn defect_ledger_token_unstated() -> M5ResolvedDefectLedgerEntry {
    let mut base = clean_defect_ledger_base(
        "defect_ledger:program-governance:token-unstated",
        "launch.line.extension-author",
        "  ",
        M5StableLineProtectionRole::EvidenceRefresh,
        M5DefectLedgerKind::SupportExportDefect,
...
fn clean_downgrade_base(
    ...
    token_name: &str,
    semantic_role: M5StableLineProtectionRole,
    backport_scope: M5BackportDecisionScope,
    surface_context: M5StableLineDefectLedgerSurfaceContext,
    ...
    M5BackportDecisionTimerEntryResolutionInput {
        entry_id: entry_id.to_owned(),
        backport_ref: backport_ref.to_owned(),
        token_name: token_name.to_owned(),
        semantic_role,
        backport_scope,
        surface_context,
...
fn downgrade_dogfood_ring_clean() -> M5ResolvedBackportDecisionTimerEntry {
    ...
    let mut base = clean_downgrade_base(
        ...
        M5StableLineProtectionRole::SupportWindow,
        M5BackportDecisionScope::MissingBackportDecision,
        M5StableLineDefectLedgerSurfaceContext::ShiproomSurface,
...
fn downgrade_rehearsal_currency_clean() -> M5ResolvedBackportDecisionTimerEntry {
    downgrade(clean_downgrade_base(
        "downgrade:program-governance:rehearsal-currency",
        "launch.line.extension-author",
        "line.downgrade.support_export_defect",
        M5StableLineProtectionRole::EvidenceRefresh,
        M5BackportDecisionScope::OverdueBackportDecision,
        M5StableLineDefectLedgerSurfaceContext::ProgramGovernanceSurface,
...
fn downgrade_go_no_go_signoff_clean() -> M5ResolvedBackportDecisionTimerEntry {
    downgrade(clean_downgrade_base(
        "downgrade:release-center:go-no-go-signoff",
        "launch.line.certified-journey",
        "line.downgrade.security_or_data_loss_defect",
        M5StableLineProtectionRole::BackportDecision,
        M5BackportDecisionScope::NarrowedSupportClaim,
        M5StableLineDefectLedgerSurfaceContext::ReleaseCenterSurface,
...
fn downgrade_support_ahead() -> M5ResolvedBackportDecisionTimerEntry {
    let mut base = clean_downgrade_base(
        "downgrade:shiproom:support-ahead",
        "launch.line.public-preview",
        "line.downgrade.compatibility_regression_defect",
        M5StableLineProtectionRole::CorrectionOwnership,
        M5BackportDecisionScope::MissingBackportDecision,
        M5StableLineDefectLedgerSurfaceContext::ShiproomSurface,
...
/// incomplete.
fn downgrade_form_incomplete() -> M5ResolvedBackportDecisionTimerEntry {
    let mut base = clean_downgrade_base(
    ...
    base.resolution_form_coverage = vec![M5StableLineDefectLedgerResolutionForm::CanonicalObject];
    downgrade(base)
}

/// Degraded downgrade entry: the downgrade scope is unclassified.
fn backport_scope_unclassified() -> M5ResolvedBackportDecisionTimerEntry {
    downgrade(clean_downgrade_base(
        "downgrade:executive-steering:scope-unclassified",
        "launch.line.design-partner-preview",
        "line.downgrade.migration_import_defect",
        M5StableLineProtectionRole::SupportWindow,
        M5BackportDecisionScope::ScopeUnclassified,
        M5StableLineDefectLedgerSurfaceContext::ExecutiveSteeringSurface,
...
#[allow(clippy::too_many_arguments)]
fn base_row(
    consumer_surface: M5StableLineDefectLedgerBackportDecisionTimerRegistriesConsumerSurface,
    owner_role: &str,
    scope_summary: &str,
    proof_ref: &str,
    downgrade_triggers: Vec<M5StableLineProtectionDowngradeTrigger>,
    defect_ledger_entries: Vec<M5ResolvedDefectLedgerEntry>,
    backport_decision_timer_entries: Vec<M5ResolvedBackportDecisionTimerEntry>,
) -> M5StableLineDefectLedgerBackportDecisionTimerRegistriesRow {
    M5StableLineDefectLedgerBackportDecisionTimerRegistriesRow {
        consumer_surface,
        ...
        anatomy_parts: M5StableLineDefectLedgerAnatomyPart::ALL.to_vec(),
        export_fields: M5StableLineDefectLedgerExportField::ALL.to_vec(),
        downgrade_triggers,
        ...
        collapses_distinct_backport_decision_timer_classes_into_one_lane: false,
    }
}

fn registry_rows() -> Vec<M5StableLineDefectLedgerBackportDecisionTimerRegistriesRow> {
    ...
    vec![
        base_row(
            ...
            vec![
                defect_ledger_crash_recovery_defect_clean(),
                defect_ledger_object_incomplete(),
            ],
        ...
        base_row(
            ...
            vec![defect_ledger_rollback_update_defect_clean(), defect_ledger_widen_fold()],
        ...
        base_row(
            ...
            vec![
                defect_ledger_migration_import_defect_clean(),
                defect_ledger_unbound(),
            ],
            vec![backport_scope_unclassified()],
        ),
        base_row(
            C::ProgramGovernance,
            "Program-governance owner",
            "Program governance resolves the support-export defect and the overdue-backport-decision alert bound to the registry; an unstated registry token on a defect-ledger entry is caught before it can drift",
            ...
            vec![
                defect_ledger_support_export_defect_clean(),
                defect_ledger_token_unstated(),
            ],
        ...
        base_row(
            ...
            "Diagnostics renders the same resolved defect-ledger and backport-decision-timer truth the resolvers produced across the canonical, accessible, and audit resolution forms rather than a hand-copied line table; the security-or-data-loss defect and the narrowed-support-claim alert stay inspectable off-renderer",
        ...
        base_row(
            ...
            vec![defect_ledger_compatibility_regression_defect_clean()],
            vec![downgrade_rehearsal_currency_clean()],
        ),
    ]
}

fn governance_review() -> M5StableLineDefectLedgerBackportDecisionTimerRegistriesGovernanceReview {
    M5StableLineDefectLedgerBackportDecisionTimerRegistriesGovernanceReview {
        defect_ledger_registry_names_token_role_and_journey: true,
        line_resolves_to_typed_defect_ledger_from_shared_registry: true,
        repo_bundle_toolchain_and_deployment_rows_published: true,
        lines_cannot_widen_without_rollback_and_diagnostics: true,
        backport_decision_timer_keeps_proof_visible_and_binds_support_language: true,
        ...
        defect_ledger_or_downgrade_drift_caught_before_release: true,
        every_row_declares_mandatory_anatomy: true,
        reuses_frozen_matrix_vocabulary: true,
    }
}

fn consumer_projection() -> M5StableLineDefectLedgerBackportDecisionTimerRegistriesConsumerProjection
{
...
fn proof_freshness() -> M5StableLineDefectLedgerBackportDecisionTimerRegistriesProofFreshness {
    M5StableLineDefectLedgerBackportDecisionTimerRegistriesProofFreshness {
...
fn release_posture() -> M5StableLineDefectLedgerBackportDecisionTimerRegistriesReleasePosture {
    M5StableLineDefectLedgerBackportDecisionTimerRegistriesReleasePosture {
        proof_packet_ref:
            M5_STABLE_LINE_DEFECT_LEDGER_BACKPORT_DECISION_TIMER_REGISTRIES_ARTIFACT_REF.to_owned(),
        line_audit_ref: M5_STABLE_LINE_DEFECT_LEDGER_BACKPORT_DECISION_TIMER_REGISTRIES_REPORT_REF
            .to_owned(),
...
/// Narrowed variant: the shiproom row is held at Beta pending line-defect_ledger parity on every journey; every
/// row stays visible and every example stays honest.
pub fn seeded_m5_stable_line_defect_ledger_and_backport_decision_timer_registries_defect_ledger_beta_narrowed(
) -> M5StableLineDefectLedgerBackportDecisionTimerRegistriesPacket {
    let mut packet = seeded_m5_stable_line_defect_ledger_and_backport_decision_timer_registries();
    packet.packet_id =
        "m5-stable-line-defect-ledger-and-backport-decision-timer-registries:defect-ledger-beta:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
...
pub fn seeded_m5_stable_line_defect_ledger_and_backport_decision_timer_registries_backport_decision_timer_preview_narrowed(
) -> M5StableLineDefectLedgerBackportDecisionTimerRegistriesPacket {
    let mut packet = seeded_m5_stable_line_defect_ledger_and_backport_decision_timer_registries();
    packet.packet_id =
        "m5-stable-line-defect-ledger-and-backport-decision-timer-registries:backport-decision-timer-preview:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
