...
pub const M5_CACHE_INTEGRITY_BYPASS_REGISTRIES_PACKET_ID: &str =
    "m5-remote-cache-integrity-and-cache-bypass-drill-registries:stable:0001";
...
) -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    resolve_remote_cache_integrity_finding_entry(input)
        .expect("seed clean-room-rebuild-lane entry resolves")
}

fn sidecar(input: M5CacheBypassDrillEntryResolutionInput) -> M5ResolvedCacheBypassDrillEntry {
    resolve_cache_bypass_drill_entry(input).expect("seed artifact-diff-packet entry resolves")
}

fn all_forms() -> Vec<M5CacheDisciplineResolutionForm> {
    M5CacheDisciplineResolutionForm::ALL.to_vec()
}
...
fn clean_manifest_base(
    ...
    rebuild_source: M5RemoteCacheOriginKind,
    surface_context: M5CacheDisciplineSurfaceContext,
...
fn manifest_rematerialized_release_center_clean() -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    manifest(clean_manifest_base(
        ...
        M5CacheDisciplineSurfaceContext::ReleaseCenterSurface,
...
fn manifest_verified_cache_shiproom_clean() -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    manifest(clean_manifest_base(
        ...
        M5RemoteCacheOriginKind::RematerializedInputReplay,
        M5CacheDisciplineSurfaceContext::ShiproomSurface,
...
fn manifest_unverified_external_diagnostics_clean() -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    ...
    let mut base = clean_manifest_base(
        ...
        M5CacheDisciplineSurfaceContext::DiagnosticsSurface,
...
fn manifest_non_materialized_provenance_clean() -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    ...
    let mut base = clean_manifest_base(
        ...
        M5RemoteCacheOriginKind::UnreplayableReference,
        M5CacheDisciplineSurfaceContext::ProvenanceSurface,
...
fn manifest_pinned_digest_support_clean() -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    manifest(clean_manifest_base(
        "manifest:support:pinned-digest",
        "release.lane.protected-merge",
        "verified.input.manifest.protected_merge",
        M5BuildLaneTrustRole::HermeticInput,
        M5RemoteCacheOriginKind::PinnedDigestReplay,
        M5CacheDisciplineSurfaceContext::SupportOrExportForm,
...
fn manifest_verified_cache_support_clean() -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    manifest(clean_manifest_base(
...
/// unstated.
fn manifest_object_incomplete() -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    let mut base = clean_manifest_base(
...
/// attempt.
fn manifest_admit_fold() -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    let mut base = clean_manifest_base(
...
/// registry.
fn manifest_unbound() -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    let mut base = clean_manifest_base(
...
fn manifest_form_incomplete() -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    let mut base = clean_manifest_base(
    ...
    base.resolution_form_coverage = vec![M5CacheDisciplineResolutionForm::CanonicalObject];
    manifest(base)
}

/// Degraded manifest entry: the canonical registry token name is unstated.
fn registry_token_unstated() -> M5ResolvedRemoteCacheIntegrityFindingEntry {
    let mut base = clean_manifest_base(
...
fn clean_sidecar_base(
    ...
    token_name: &str,
    semantic_role: M5BuildLaneTrustRole,
    diff_scope: M5CacheBypassDrillScope,
    surface_context: M5CacheDisciplineSurfaceContext,
...
fn sidecar_binary_identity_clean() -> M5ResolvedCacheBypassDrillEntry {
    sidecar(clean_sidecar_base(
        "sidecar:release-center:binary-identity",
        "release.lane.release",
        "sidecar.completeness.manifest.release",
        M5BuildLaneTrustRole::ReproducibilityProof,
        M5CacheBypassDrillScope::ByteIdenticalDiff,
        M5CacheDisciplineSurfaceContext::ReleaseCenterSurface,
...
fn sidecar_hermetic_rebuild_clean() -> M5ResolvedCacheBypassDrillEntry {
    sidecar(clean_sidecar_base(
        "sidecar:diagnostics:hermetic-rebuild",
        "release.lane.emergency-hotfix",
        "sidecar.completeness.manifest.emergency_hotfix",
        M5BuildLaneTrustRole::SupportIdentity,
        M5CacheBypassDrillScope::HermeticRebuildDiff,
        M5CacheDisciplineSurfaceContext::DiagnosticsSurface,
...
fn sidecar_missing_family() -> M5ResolvedCacheBypassDrillEntry {
...
/// incomplete.
fn sidecar_form_incomplete() -> M5ResolvedCacheBypassDrillEntry {
    let mut base = clean_sidecar_base(
...
fn sidecar_scope_unclassified() -> M5ResolvedCacheBypassDrillEntry {
    sidecar(clean_sidecar_base(
        "sidecar:provenance:scope-unclassified",
        "release.lane.emergency-hotfix",
        "sidecar.completeness.manifest.emergency_hotfix",
        M5BuildLaneTrustRole::SupportIdentity,
        M5CacheBypassDrillScope::ScopeUnclassified,
        M5CacheDisciplineSurfaceContext::ProvenanceSurface,
        "build-id.sha256.emergency-hotfix-0007",
        "families.binaries-packages-rollback-metadata-symbols",
        "ledger.all-families-present-emergency-hotfix-0007",
...
#[allow(clippy::too_many_arguments)]
fn base_row(
    consumer_surface: M5CacheIntegrityBypassRegistriesConsumerSurface,
    owner_role: &str,
    scope_summary: &str,
    proof_ref: &str,
    downgrade_triggers: Vec<M5BuildLaneDowngradeTrigger>,
    remote_cache_integrity_finding_entries: Vec<M5ResolvedRemoteCacheIntegrityFindingEntry>,
    cache_bypass_drill_entries: Vec<M5ResolvedCacheBypassDrillEntry>,
) -> M5CacheIntegrityBypassRegistriesRow {
    M5CacheIntegrityBypassRegistriesRow {
        consumer_surface,
        ...
        anatomy_parts: M5CacheDisciplineAnatomyPart::ALL.to_vec(),
        export_fields: M5CacheDisciplineExportField::ALL.to_vec(),
        downgrade_triggers,
...
fn registry_rows() -> Vec<M5CacheIntegrityBypassRegistriesRow> {
...
fn governance_review() -> M5CacheIntegrityBypassRegistriesGovernanceReview {
    M5CacheIntegrityBypassRegistriesGovernanceReview {
        remote_cache_integrity_registry_names_token_role_and_source: true,
...
fn consumer_projection() -> M5CacheIntegrityBypassRegistriesConsumerProjection {
    M5CacheIntegrityBypassRegistriesConsumerProjection {
...
fn proof_freshness() -> M5CacheIntegrityBypassRegistriesProofFreshness {
    M5CacheIntegrityBypassRegistriesProofFreshness {
...
fn release_posture() -> M5CacheIntegrityBypassRegistriesReleasePosture {
    M5CacheIntegrityBypassRegistriesReleasePosture {
        proof_packet_ref: M5_CACHE_INTEGRITY_BYPASS_REGISTRIES_ARTIFACT_REF.to_owned(),
        build_lane_audit_ref: M5_CACHE_INTEGRITY_BYPASS_REGISTRIES_REPORT_REF.to_owned(),
...
pub fn seeded_m5_remote_cache_integrity_and_cache_bypass_drill_registries(
) -> M5CacheIntegrityBypassRegistriesPacket {
    M5CacheIntegrityBypassRegistriesPacket::new(
        M5CacheIntegrityBypassRegistriesPacketInput {
            packet_id: M5_CACHE_INTEGRITY_BYPASS_REGISTRIES_PACKET_ID.to_owned(),
            registries_label:
            ...
            vocabulary_set: M5CacheIntegrityBypassRegistriesVocabularySet::canonical(),
...
pub fn seeded_m5_remote_cache_integrity_and_cache_bypass_drill_registries_hermetic_rebuild_beta_narrowed(
) -> M5CacheIntegrityBypassRegistriesPacket {
    let mut packet = seeded_m5_remote_cache_integrity_and_cache_bypass_drill_registries();
    packet.packet_id =
        "m5-remote-cache-integrity-and-cache-bypass-drill-registries:hermetic-rebuild-beta:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
...
pub fn seeded_m5_remote_cache_integrity_and_cache_bypass_drill_registries_artifact_diff_preview_narrowed(
) -> M5CacheIntegrityBypassRegistriesPacket {
    let mut packet = seeded_m5_remote_cache_integrity_and_cache_bypass_drill_registries();
    packet.packet_id =
        "m5-remote-cache-integrity-and-cache-bypass-drill-registries:artifact-diff-preview:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
