...
pub const M5_CLEAN_ROOM_REBUILD_ARTIFACT_DIFF_REGISTRIES_PACKET_ID: &str =
    "m5-clean-room-rebuild-lane-and-artifact-diff-packet-registries:stable:0001";
...
) -> M5ResolvedCleanRoomRebuildLaneEntry {
    resolve_clean_room_rebuild_lane_entry(input)
        .expect("seed clean-room-rebuild-lane entry resolves")
}

fn sidecar(input: M5ArtifactDiffPacketEntryResolutionInput) -> M5ResolvedArtifactDiffPacketEntry {
    resolve_artifact_diff_packet_entry(input).expect("seed artifact-diff-packet entry resolves")
}

fn all_forms() -> Vec<M5CleanRoomResolutionForm> {
    M5CleanRoomResolutionForm::ALL.to_vec()
}
...
fn clean_manifest_base(
    ...
    rebuild_source: M5CleanRoomRebuildSourceKind,
    surface_context: M5CleanRoomSurfaceContext,
...
fn manifest_rematerialized_release_center_clean() -> M5ResolvedCleanRoomRebuildLaneEntry {
    manifest(clean_manifest_base(
        ...
        M5CleanRoomSurfaceContext::ReleaseCenterSurface,
...
fn manifest_verified_cache_shiproom_clean() -> M5ResolvedCleanRoomRebuildLaneEntry {
    manifest(clean_manifest_base(
        ...
        M5CleanRoomRebuildSourceKind::RematerializedInputReplay,
        M5CleanRoomSurfaceContext::ShiproomSurface,
...
fn manifest_unverified_external_diagnostics_clean() -> M5ResolvedCleanRoomRebuildLaneEntry {
    ...
    let mut base = clean_manifest_base(
        ...
        M5CleanRoomSurfaceContext::DiagnosticsSurface,
...
fn manifest_non_materialized_provenance_clean() -> M5ResolvedCleanRoomRebuildLaneEntry {
    ...
    let mut base = clean_manifest_base(
        ...
        M5CleanRoomRebuildSourceKind::UnreplayableReference,
        M5CleanRoomSurfaceContext::ProvenanceSurface,
...
fn manifest_pinned_digest_support_clean() -> M5ResolvedCleanRoomRebuildLaneEntry {
    manifest(clean_manifest_base(
        "manifest:support:pinned-digest",
        "release.lane.protected-merge",
        "verified.input.manifest.protected_merge",
        M5BuildLaneTrustRole::HermeticInput,
        M5CleanRoomRebuildSourceKind::PinnedDigestReplay,
        M5CleanRoomSurfaceContext::SupportOrExportForm,
...
fn manifest_verified_cache_support_clean() -> M5ResolvedCleanRoomRebuildLaneEntry {
    manifest(clean_manifest_base(
...
/// unstated.
fn manifest_object_incomplete() -> M5ResolvedCleanRoomRebuildLaneEntry {
    let mut base = clean_manifest_base(
...
/// attempt.
fn manifest_admit_fold() -> M5ResolvedCleanRoomRebuildLaneEntry {
    let mut base = clean_manifest_base(
...
/// registry.
fn manifest_unbound() -> M5ResolvedCleanRoomRebuildLaneEntry {
    let mut base = clean_manifest_base(
...
fn manifest_form_incomplete() -> M5ResolvedCleanRoomRebuildLaneEntry {
    let mut base = clean_manifest_base(
    ...
    base.resolution_form_coverage = vec![M5CleanRoomResolutionForm::CanonicalObject];
    manifest(base)
}

/// Degraded manifest entry: the canonical registry token name is unstated.
fn registry_token_unstated() -> M5ResolvedCleanRoomRebuildLaneEntry {
    let mut base = clean_manifest_base(
...
fn clean_sidecar_base(
    ...
    token_name: &str,
    semantic_role: M5BuildLaneTrustRole,
    diff_scope: M5ArtifactDiffScope,
    surface_context: M5CleanRoomSurfaceContext,
...
fn sidecar_binary_identity_clean() -> M5ResolvedArtifactDiffPacketEntry {
    sidecar(clean_sidecar_base(
        "sidecar:release-center:binary-identity",
        "release.lane.release",
        "sidecar.completeness.manifest.release",
        M5BuildLaneTrustRole::ReproducibilityProof,
        M5ArtifactDiffScope::ByteIdenticalDiff,
        M5CleanRoomSurfaceContext::ReleaseCenterSurface,
...
fn sidecar_hermetic_rebuild_clean() -> M5ResolvedArtifactDiffPacketEntry {
    sidecar(clean_sidecar_base(
        "sidecar:diagnostics:hermetic-rebuild",
        "release.lane.emergency-hotfix",
        "sidecar.completeness.manifest.emergency_hotfix",
        M5BuildLaneTrustRole::SupportIdentity,
        M5ArtifactDiffScope::HermeticRebuildDiff,
        M5CleanRoomSurfaceContext::DiagnosticsSurface,
...
fn sidecar_missing_family() -> M5ResolvedArtifactDiffPacketEntry {
...
/// incomplete.
fn sidecar_form_incomplete() -> M5ResolvedArtifactDiffPacketEntry {
    let mut base = clean_sidecar_base(
...
fn sidecar_scope_unclassified() -> M5ResolvedArtifactDiffPacketEntry {
    sidecar(clean_sidecar_base(
        "sidecar:provenance:scope-unclassified",
        "release.lane.emergency-hotfix",
        "sidecar.completeness.manifest.emergency_hotfix",
        M5BuildLaneTrustRole::SupportIdentity,
        M5ArtifactDiffScope::ScopeUnclassified,
        M5CleanRoomSurfaceContext::ProvenanceSurface,
        "build-id.sha256.emergency-hotfix-0007",
        "families.binaries-packages-rollback-metadata-symbols",
        "ledger.all-families-present-emergency-hotfix-0007",
...
#[allow(clippy::too_many_arguments)]
fn base_row(
    consumer_surface: M5CleanRoomRebuildArtifactDiffRegistriesConsumerSurface,
    owner_role: &str,
    scope_summary: &str,
    proof_ref: &str,
    downgrade_triggers: Vec<M5BuildLaneDowngradeTrigger>,
    clean_room_rebuild_lane_entries: Vec<M5ResolvedCleanRoomRebuildLaneEntry>,
    artifact_diff_packet_entries: Vec<M5ResolvedArtifactDiffPacketEntry>,
) -> M5CleanRoomRebuildArtifactDiffRegistriesRow {
    M5CleanRoomRebuildArtifactDiffRegistriesRow {
        consumer_surface,
        ...
        anatomy_parts: M5CleanRoomAnatomyPart::ALL.to_vec(),
        export_fields: M5CleanRoomExportField::ALL.to_vec(),
        downgrade_triggers,
...
fn registry_rows() -> Vec<M5CleanRoomRebuildArtifactDiffRegistriesRow> {
...
fn governance_review() -> M5CleanRoomRebuildArtifactDiffRegistriesGovernanceReview {
    M5CleanRoomRebuildArtifactDiffRegistriesGovernanceReview {
        clean_room_rebuild_registry_names_token_role_and_source: true,
...
fn consumer_projection() -> M5CleanRoomRebuildArtifactDiffRegistriesConsumerProjection {
    M5CleanRoomRebuildArtifactDiffRegistriesConsumerProjection {
...
fn proof_freshness() -> M5CleanRoomRebuildArtifactDiffRegistriesProofFreshness {
    M5CleanRoomRebuildArtifactDiffRegistriesProofFreshness {
...
fn release_posture() -> M5CleanRoomRebuildArtifactDiffRegistriesReleasePosture {
    M5CleanRoomRebuildArtifactDiffRegistriesReleasePosture {
        proof_packet_ref: M5_CLEAN_ROOM_REBUILD_ARTIFACT_DIFF_REGISTRIES_ARTIFACT_REF.to_owned(),
        build_lane_audit_ref: M5_CLEAN_ROOM_REBUILD_ARTIFACT_DIFF_REGISTRIES_REPORT_REF.to_owned(),
...
pub fn seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries(
) -> M5CleanRoomRebuildArtifactDiffRegistriesPacket {
    M5CleanRoomRebuildArtifactDiffRegistriesPacket::new(
        M5CleanRoomRebuildArtifactDiffRegistriesPacketInput {
            packet_id: M5_CLEAN_ROOM_REBUILD_ARTIFACT_DIFF_REGISTRIES_PACKET_ID.to_owned(),
            registries_label:
            ...
            vocabulary_set: M5CleanRoomRebuildArtifactDiffRegistriesVocabularySet::canonical(),
...
pub fn seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries_hermetic_rebuild_beta_narrowed(
) -> M5CleanRoomRebuildArtifactDiffRegistriesPacket {
    let mut packet = seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries();
    packet.packet_id =
        "m5-clean-room-rebuild-lane-and-artifact-diff-packet-registries:hermetic-rebuild-beta:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
...
pub fn seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries_artifact_diff_preview_narrowed(
) -> M5CleanRoomRebuildArtifactDiffRegistriesPacket {
    let mut packet = seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries();
    packet.packet_id =
        "m5-clean-room-rebuild-lane-and-artifact-diff-packet-registries:artifact-diff-preview:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
