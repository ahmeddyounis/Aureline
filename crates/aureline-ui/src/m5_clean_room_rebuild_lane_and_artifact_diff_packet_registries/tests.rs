...
fn clean_manifest_input() -> M5CleanRoomRebuildLaneEntryResolutionInput {
    M5CleanRoomRebuildLaneEntryResolutionInput {
        ...
        rebuild_source: M5CleanRoomRebuildSourceKind::HermeticCleanRoomRebuild,
        surface_context: M5CleanRoomSurfaceContext::ReleaseCenterSurface,
        resolution_form_coverage: M5CleanRoomResolutionForm::ALL.to_vec(),
...
fn clean_sidecar_input() -> M5ArtifactDiffPacketEntryResolutionInput {
    M5ArtifactDiffPacketEntryResolutionInput {
        ...
        semantic_role: M5BuildLaneTrustRole::ReproducibilityProof,
        diff_scope: M5ArtifactDiffScope::ByteIdenticalDiff,
        surface_context: M5CleanRoomSurfaceContext::ReleaseCenterSurface,
        resolution_form_coverage: M5CleanRoomResolutionForm::ALL.to_vec(),
...
fn manifest_clean_names_meaning_and_is_bound() {
    let resolved = resolve_clean_room_rebuild_lane_entry(clean_manifest_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.lane_resolves_across_lanes);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.clean_room_rebuild_lane_object_complete);
    ...
    assert_eq!(
        ...
        M5CleanRoomNextAction::ExpandRebuildMeaning
    );
}
...
fn registry_token_unstated_degrades() {
    ...
    assert_eq!(
        resolve_clean_room_rebuild_lane_entry(input)
            .unwrap()
            .degrade_reason,
...
fn manifest_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_manifest_input();
    input.rebuild_source = M5CleanRoomRebuildSourceKind::SourceUnclassified;
    assert_eq!(
        resolve_clean_room_rebuild_lane_entry(input)
            .unwrap()
            .degrade_reason,
...
fn manifest_object_incomplete_and_admit_fold_and_form_degrade() {
    ...
    let resolved = resolve_clean_room_rebuild_lane_entry(input).unwrap();
    assert!(!resolved.clean_room_rebuild_lane_object_complete);
    assert_eq!(
    ...
    let mut input = clean_manifest_input();
    input.resolution_form_coverage = vec![M5CleanRoomResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_clean_room_rebuild_lane_entry(input)
            .unwrap()
            .degrade_reason,
...
fn manifest_trust_risk_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_manifest_input();
    input.rebuild_source = M5CleanRoomRebuildSourceKind::SharedCacheShortcut;
    ...
    let mut input = clean_manifest_input();
    input.surface_context = M5CleanRoomSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_clean_room_rebuild_lane_entry(input)
            .unwrap()
            .degrade_reason,
...
fn manifest_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        resolve_clean_room_rebuild_lane_entry(input).unwrap_err(),
        M5CleanRoomResolutionError::EmptyCleanRoomRebuildLaneEntryId
    );
    ...
    assert_eq!(
        resolve_clean_room_rebuild_lane_entry(input).unwrap_err(),
        M5CleanRoomResolutionError::ForbiddenMaterial
    );
}
...
fn shared_cache_cannot_authorize_rebuild_rejects_unbounded() {
    assert!(shared_cache_cannot_authorize_rebuild(
        M5CleanRoomRebuildSourceKind::HermeticCleanRoomRebuild,
        true,
        false,
        true
    ));
    ...
    assert!(shared_cache_cannot_authorize_rebuild(
        M5CleanRoomRebuildSourceKind::SharedCacheShortcut,
        true,
        true,
        true
    ));
    ...
    assert!(!shared_cache_cannot_authorize_rebuild(
        M5CleanRoomRebuildSourceKind::SourceUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn clean_room_rebuild_lane_object_is_complete_requires_all_fields() {
    assert!(clean_room_rebuild_lane_object_is_complete(
        M5CleanRoomRebuildSourceKind::HermeticCleanRoomRebuild,
...
fn sidecar_clean_stays_converged() {
    let resolved = resolve_artifact_diff_packet_entry(clean_sidecar_input()).unwrap();
    assert!(resolved.is_clean());
...
fn sidecar_missing_family_and_unclassified_degrade() {
    ...
    let resolved = resolve_artifact_diff_packet_entry(input).unwrap();
    assert!(!resolved.provides_complete_artifact_diff);
    assert_eq!(
        resolved.degrade_reason,
    ...
    assert_eq!(
        resolve_artifact_diff_packet_entry(input).unwrap().degrade_reason,
        Some(M5ArtifactDiffPacketEntryDegradeReason::ArtifactDiffDivergesOrOmitsFamilyOrDriftsBuildIdentity)
    );
    ...
    let mut input = clean_sidecar_input();
    input.diff_scope = M5ArtifactDiffScope::ScopeUnclassified;
    assert_eq!(
        resolve_artifact_diff_packet_entry(input)
            .unwrap()
            .degrade_reason,
...
fn sidecar_form_and_surface_and_id_and_material() {
    ...
    input.entry_id = "  ".to_owned();
    assert_eq!(
        resolve_artifact_diff_packet_entry(input).unwrap_err(),
        M5CleanRoomResolutionError::EmptyArtifactDiffPacketEntryId
    );
...
fn sidecar_flagged_missing_and_mismatched_stay_clean() {
    ...
    assert!(resolve_artifact_diff_packet_entry(input)
        .unwrap()
        .is_clean());
...
fn vocabulary_set_drift_fails() {
    let mut packet = seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries();
    packet.vocabulary_set.rebuild_source_kinds.pop();
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::VocabularySetDrift));
}
...
fn missing_source_contracts_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::MissingSourceContracts));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_CLEAN_ROOM_REBUILD_LANE_DOMAIN_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::DomainSchemaRefMissing));

    let mut packet = seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries();
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_ARTIFACT_DIFF_PACKET_DOMAIN_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::DomainSchemaRefMissing));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5CleanRoomAnatomyPart::Identity);
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::MandatoryAnatomyMissing));
}
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5CleanRoomExportField::RebuildSourceKinds);
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::MandatoryExportFieldMissing));
}

#[test]
fn examples_missing_fails() {
    let mut packet = seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries();
    packet.registry_rows[0].artifact_diff_packet_entries.clear();
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::ExamplesMissing));
}
...
fn dishonest_clean_example_fails() {
    ...
    row.clean_room_rebuild_lane_entries[0].degrade_reason = None;
    row.clean_room_rebuild_lane_entries[0].clean_room_rebuild_lane_object_complete = false;
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        assert!(packet
            .validate()
            .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::RowInvariantViolated));
    }
}

#[test]
fn clean_room_rebuild_lane_not_proven_when_incomplete_example_removed() {
    let mut packet = seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries();
    for row in &mut packet.registry_rows {
        row.clean_room_rebuild_lane_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
                    M5CleanRoomRebuildLaneEntryDegradeReason::CleanRoomRebuildLaneObjectIncomplete,
                )
        });
    }
...
fn artifact_diff_determinism_not_proven_when_missing_example_removed() {
    let mut packet = seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries();
    for row in &mut packet.registry_rows {
        row.artifact_diff_packet_entries.retain(|ex| {
            ex.degrade_reason
                != Some(M5ArtifactDiffPacketEntryDegradeReason::ArtifactDiffDivergesOrOmitsFamilyOrDriftsBuildIdentity)
        });
    }
...
fn governance_review_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::GovernanceReviewIncomplete));
}
...
fn proof_freshness_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::ProofFreshnessIncomplete));
}
...
fn release_posture_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::ReleasePostureIncomplete));
}
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet
        .validate()
        .contains(&M5CleanRoomRebuildArtifactDiffRegistriesViolation::RawMaterialInExport));
}

#[test]
fn export_carries_no_forbidden_raw_material() {
    let json =
        seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries().export_safe_json();
...
#[test]
fn clean_room_rebuild_lane_table_lists_only_clean_lanes() {
    let packet = seeded_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries();
    let table = packet.render_clean_room_rebuild_lane_table();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk =
        current_stable_m5_clean_room_rebuild_lane_and_artifact_diff_packet_registries_export()
            .expect(
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5CleanRoomRebuildArtifactDiffRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-clean-room-rebuild-lane-and-artifact-diff-packet-registries/hermetic_rebuild_beta_narrowed.json"
    )))
    ...
    let preview: M5CleanRoomRebuildArtifactDiffRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-clean-room-rebuild-lane-and-artifact-diff-packet-registries/artifact_diff_preview_narrowed.json"
    )))
