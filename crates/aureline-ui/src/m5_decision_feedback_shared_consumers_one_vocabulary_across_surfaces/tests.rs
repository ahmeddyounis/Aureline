...
fn seed() -> M5DecisionFeedbackSharedConsumersPacket {
    seeded_m5_decision_feedback_shared_consumers()
}

fn violations_of(packet: &M5DecisionFeedbackSharedConsumersPacket) -> Vec<&'static str> {
...
fn seed_validates_clean() {
    ...
    assert_eq!(
        packet.record_kind,
        M5_DECISION_FEEDBACK_SHARED_CONSUMERS_RECORD_KIND
    );
}

#[test]
fn every_primitive_is_adopted_by_two_or_more_consumers() {
    let packet = seed();
    ...
    assert_eq!(component_consumers.len(), 8, "all eight primitives adopted");
    for (component, consumers) in &component_consumers {
        assert!(
            consumers.len() >= 2,
            "primitive {} only adopted by {} consumers",
            component.as_str(),
            consumers.len()
        );
    }
...
fn every_consumer_surface_and_representation_is_exercised() {
    ...
    let representations: BTreeSet<_> = packet
        ...
        .collect();
    for representation in DecisionFeedbackRepresentation::ALL {
        assert!(
...
#[test]
fn every_disposition_word_is_a_frozen_disposition_token() {
    let packet = seed();
    for binding in &packet.consumer_bindings {
        assert!(
            binding.state_facets.disposition_word_in_vocabulary(),
            "disposition word `{}` on {} is not a frozen disposition token",
    ...
    for binding in &packet.consumer_bindings {
        ...
        if binding.is_narrowed() {
            assert_eq!(
                ...
                DecisionFeedbackParityState::FacetsDisclosedNarrowed
            );
            let note = binding
                .narrow_note
                .as_ref()
            ...
            assert_eq!(
                binding.parity_state,
                DecisionFeedbackParityState::FacetsPreserved
            );
            assert!(binding.narrow_note.is_none());
        }
        if matches!(
            binding.representation,
            DecisionFeedbackRepresentation::RemoteProjected
        ) {
            assert!(!binding.remote_source_note.trim().is_empty());
        }
        if matches!(
            binding.representation,
            DecisionFeedbackRepresentation::ExportedRedacted
        ) {
...
fn disclosure_resolver_matches_representation() {
    assert!(
        !resolve_decision_feedback_render_disclosure(DecisionFeedbackRepresentation::DesktopFull)
            .needs_narrow_note
    );
    let compact = resolve_decision_feedback_render_disclosure(
        DecisionFeedbackRepresentation::CompactNarrowed,
    );
    assert_eq!(
        compact.narrow_reason,
        Some(DecisionFeedbackNarrowReason::CompactionNarrowed)
    );
    assert!(compact.needs_narrow_note);
    assert!(!compact.needs_remote_source_note);
    let remote = resolve_decision_feedback_render_disclosure(
        DecisionFeedbackRepresentation::RemoteProjected,
    );
    assert!(remote.needs_remote_source_note);
    let exported = resolve_decision_feedback_render_disclosure(
        DecisionFeedbackRepresentation::ExportedRedacted,
    );
...
fn vocabulary_drift_is_rejected() {
    let mut packet = seed();
    // Reword one surface of a two-binding object to a different (still-valid) disposition word.
    let target = packet
        .consumer_bindings
        .iter()
        .position(|b| b.binding_id == "cc-popover-review")
        .unwrap();
    packet.consumer_bindings[target]
        .state_facets
        .disposition_word = "warning".to_owned();
    assert!(violations_of(&packet).contains(&"vocabulary_drift_across_surfaces"));
}

#[test]
fn disposition_word_outside_vocabulary_is_rejected() {
    let mut packet = seed();
    packet.consumer_bindings[0].state_facets.disposition_word = "totally_made_up".to_owned();
    let tokens = violations_of(&packet);
    assert!(tokens.contains(&"disposition_word_outside_vocabulary"));
...
fn reuse_below_two_consumers_is_rejected() {
    let mut packet = seed();
    // Drop every popover binding except one, leaving the primitive with one consumer.
    let mut kept_one = false;
    packet.consumer_bindings.retain(|b| {
        if b.component == M5DecisionFeedbackFamily::Popover {
            if kept_one {
                return false;
            }
            kept_one = true;
    ...
    assert!(violations_of(&packet).contains(&"primitive_reuse_unproven"));
}
...
fn missing_canonical_reference_on_export_is_rejected() {
    ...
    packet.consumer_bindings[target].source_contract_refs =
        vec![M5_DECISION_FEEDBACK_COMPONENT_SCHEMA_REF.to_owned()];
...
fn unexpected_narrow_note_on_full_binding_is_rejected() {
    ...
    let target = packet
        ...
        .position(|b| !b.is_narrowed())
        .unwrap();
    packet.consumer_bindings[target].narrow_note = Some(DecisionFeedbackNarrowNote {
        reason: DecisionFeedbackNarrowReason::CompactionNarrowed,
        preserved_vocabulary_note: "x".to_owned(),
        next_action: DecisionFeedbackNarrowNextAction::ExpandInDesktop,
...
fn each_guardrail_is_enforced() {
    let cases: [(fn(&mut DecisionFeedbackConsumerBinding), &str); 6] = [
        ...
        (
            |b| b.lets_a_popover_carry_the_only_critical_instruction = true,
            "popover_carries_only_critical_instruction",
        ),
        (
            |b| b.uses_generic_yes_no_confirmation_copy = true,
            "generic_yes_no_confirmation_copy",
        ),
        (
            |b| b.represents_durable_work_as_toast_only_truth = true,
            "durable_work_as_toast_only_truth",
        ),
        (
            |b| b.blanks_a_useful_pane_during_loading = true,
            "useful_pane_blanked_during_loading",
        ),
        (
            |b| b.uses_a_full_screen_spinner_where_partial_capable = true,
...
#[test]
fn primitive_coverage_gap_is_rejected() {
    let mut packet = seed();
    packet
        .consumer_bindings
        .retain(|b| b.component != M5DecisionFeedbackFamily::ConsequenceBlock);
...
fn missing_source_contract_is_rejected() {
    ...
    packet
        ...
        .retain(|r| r != M5_DECISION_FEEDBACK_SHARED_CONSUMERS_DOC_REF);
...
fn csv_has_a_row_per_binding() {
    ...
    assert!(lines[0].starts_with("component,consumer,representation,disposition_word,parity_state"));
}
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_decision_feedback_shared_consumers_export()
        .expect("checked M5 decision-feedback shared-consumer export validates");
    assert_eq!(
        from_disk.packet_id,
        M5_DECISION_FEEDBACK_SHARED_CONSUMERS_PACKET_ID
    );
...
fn narrowed_fixture_variants_validate_and_keep_coverage() {
    let compact = seeded_m5_decision_feedback_shared_consumers_compact_remote_narrowed();
    assert!(
    ...
    let exported = seeded_m5_decision_feedback_shared_consumers_exported_redaction_narrowed();
    assert!(
        exported.validate().is_empty(),
        "{:?}",
        violations_of(&exported)
    );
    assert_eq!(exported.consumer_bindings.len(), 19);
}

#[test]
fn checked_narrowed_fixtures_validate_and_match_builders() {
    let compact: M5DecisionFeedbackSharedConsumersPacket =
        serde_json::from_str(include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../fixtures/ui/m5-decision-feedback-shared-consumers/compact_remote_narrowed.json"
        )))
    ...
    let exported: M5DecisionFeedbackSharedConsumersPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../fixtures/ui/m5-decision-feedback-shared-consumers/exported_redaction_narrowed.json"
        )
    ))
