...
fn clean_publish_to_review_sheet_input() -> M5PublishToReviewSheetEntryResolutionInput {
    M5PublishToReviewSheetEntryResolutionInput {
        entry_id: "publish_to_review_sheet:test".to_owned(),
        line_binding_id: "launch.line.core-team-canary".to_owned(),
        token_name: "line.publish_to_review_sheet.core_team_canary".to_owned(),
        semantic_role: M5AiReviewAssistRole::FindingClassification,
        report_section: M5PublishToReviewSheetKind::LocalDraft,
        surface_context: M5PublishToReviewSheetSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5PublishToReviewSheetResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5PublishScopeDecisionEntryResolutionInput {
    M5PublishScopeDecisionEntryResolutionInput {
        ...
        semantic_role: M5AiReviewAssistRole::FindingClassification,
        comparison_scope: M5PublishScopeDecisionScope::PublishScopeAllowed,
        surface_context: M5PublishToReviewSheetSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5PublishToReviewSheetResolutionForm::ALL.to_vec(),
...
#[test]
fn publish_to_review_sheet_clean_names_meaning_and_is_bound() {
    let resolved =
        resolve_publish_to_review_sheet_entry(clean_publish_to_review_sheet_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.publish_to_review_sheet_resolves_across_lines);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.publish_to_review_sheet_object_complete);
    ...
    assert_eq!(resolved.report_section, "local_draft");
    assert_eq!(resolved.canonical_report_section_mode, "local_draft_mode");
    assert_eq!(resolved.surface_context, "shiproom_surface");
    assert_eq!(
        resolved.next_action,
        M5PublishToReviewSheetNextAction::ExpandCohortMeaning
    );
}

#[test]
fn publish_to_review_sheet_token_unstated_degrades() {
    let mut input = clean_publish_to_review_sheet_input();
...
#[test]
fn publish_to_review_sheet_unbound_and_unclassified_degrade() {
    let mut input = clean_publish_to_review_sheet_input();
    ...
    let mut input = clean_publish_to_review_sheet_input();
    input.report_section = M5PublishToReviewSheetKind::PublishModeUnclassified;
    assert_eq!(
        resolve_publish_to_review_sheet_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn publish_to_review_sheet_object_incomplete_and_widen_fold_and_form_degrade() {
    // An unstated bundle IDs field leaves the resolved object incomplete.
    let mut input = clean_publish_to_review_sheet_input();
    input.bundle_ids = "  ".to_owned();
    let resolved = resolve_publish_to_review_sheet_entry(input).unwrap();
    assert!(!resolved.publish_to_review_sheet_object_complete);
    assert_eq!(
    ...
    assert_eq!(
        resolve_publish_to_review_sheet_entry(input).unwrap().degrade_reason,
        Some(M5PublishToReviewSheetEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    );

    let mut input = clean_publish_to_review_sheet_input();
    input.resolution_form_coverage = vec![M5PublishToReviewSheetResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_publish_to_review_sheet_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn publish_to_review_sheet_public_facing_and_surface_and_proof_degrade() {
    // A public-facing line running support language ahead of proof first fails the widen-boundary fold.
    let mut input = clean_publish_to_review_sheet_input();
    input.report_section = M5PublishToReviewSheetKind::OpenInProvider;
    ...
    let mut input = clean_publish_to_review_sheet_input();
    input.surface_context = M5PublishToReviewSheetSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_publish_to_review_sheet_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn publish_to_review_sheet_empty_id_and_forbidden_material_error() {
    let mut input = clean_publish_to_review_sheet_input();
    input.entry_id = "".to_owned();
    assert_eq!(
        resolve_publish_to_review_sheet_entry(input).unwrap_err(),
        M5PublishToReviewSheetResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_publish_to_review_sheet_entry(input).unwrap_err(),
        M5PublishToReviewSheetResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5PublishToReviewSheetKind::PublishModeUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn publish_to_review_sheet_object_is_complete_requires_all_fields() {
    assert!(publish_to_review_sheet_object_is_complete(
        M5PublishToReviewSheetKind::LocalDraft,
...
fn downgrade_clean_stays_honest() {
    let resolved = resolve_publish_scope_decision_entry(clean_downgrade_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.comparison_safe_on_every_line);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.provides_complete_publish_scope_decision);
    assert!(resolved.publish_scope_decision_stays_honest);
    assert_eq!(resolved.comparison_scope, "publish_scope_allowed");
    assert_eq!(resolved.surface_context, "shiproom_surface");
}
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let resolved = resolve_publish_scope_decision_entry(input).unwrap();
    assert!(!resolved.provides_complete_publish_scope_decision);
    assert_eq!(
        resolved.degrade_reason,
    ...
    let mut input = clean_downgrade_input();
    input.keeps_publish_scope_decision_visible = false;
    assert_eq!(
        resolve_publish_scope_decision_entry(input).unwrap().degrade_reason,
        Some(M5PublishScopeDecisionEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    );
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5PublishScopeDecisionScope::PublishScopeUnclassified;
    assert_eq!(
        resolve_publish_scope_decision_entry(input)
            .unwrap()
            .degrade_reason,
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    input.entry_id = "  ".to_owned();
    assert_eq!(
        resolve_publish_scope_decision_entry(input).unwrap_err(),
        M5PublishToReviewSheetResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn downgrade_bound_support_and_flagged_gap_stay_clean() {
    ...
    assert!(resolve_publish_scope_decision_entry(input)
        .unwrap()
        .is_clean());
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5AiReviewPublishSheetAndScopeDecisionRegistriesViolation::VocabularySetDrift));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_AI_REVIEW_PUBLISH_SHEET_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5PublishToReviewSheetAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5PublishToReviewSheetExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5AiReviewPublishSheetAndScopeDecisionRegistriesViolation::ExamplesMissing));
}

#[test]
fn dishonest_clean_example_fails() {
    let mut packet = seeded_m5_ai_review_publish_sheet_and_scope_decision_registries();
    // Force a clean publish_to_review_sheet entry to also read as object-incomplete — the packet must reject it.
    let row = &mut packet.registry_rows[0];
    row.publish_to_review_sheet_entries[0].degrade_reason = None;
    row.publish_to_review_sheet_entries[0].publish_to_review_sheet_object_complete = false;
    assert!(packet
        .validate()
        .contains(&M5AiReviewPublishSheetAndScopeDecisionRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        match mutate {
            ...
            _ => row.collapses_distinct_publish_scope_decision_classes_into_one_lane = true,
        }
        assert!(packet.validate().contains(
...
#[test]
fn publish_to_review_sheet_not_proven_when_incomplete_example_removed() {
    let mut packet = seeded_m5_ai_review_publish_sheet_and_scope_decision_registries();
    for row in &mut packet.registry_rows {
        row.publish_to_review_sheet_entries.retain(|ex| {
            ex.degrade_reason
                != Some(M5PublishToReviewSheetEntryDegradeReason::CohortDescriptorObjectIncomplete)
        });
    }
...
#[test]
fn publish_to_review_sheet_not_proven_when_surface_collapses() {
    let mut packet = seeded_m5_ai_review_publish_sheet_and_scope_decision_registries();
    // Drop every clean executive-steering-surface publish_to_review_sheet so the first-consumer surfaces no longer include it.
    for row in &mut packet.registry_rows {
        row.publish_to_review_sheet_entries
...
#[test]
fn publish_scope_decision_integrity_not_proven_when_support_ahead_example_removed() {
    let mut packet = seeded_m5_ai_review_publish_sheet_and_scope_decision_registries();
    for row in &mut packet.registry_rows {
        row.publish_scope_decision_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
...
#[test]
fn publish_scope_decision_integrity_not_proven_when_scope_dropped() {
    let mut packet = seeded_m5_ai_review_publish_sheet_and_scope_decision_registries();
    // Drop every clean go-no-go-signoff downgrade so the coverage no longer includes it.
    for row in &mut packet.registry_rows {
        row.publish_scope_decision_entries
            .retain(|ex| !(ex.is_clean() && ex.comparison_scope == "publish_scope_blocked"));
    }
    assert!(packet.validate().contains(
        &M5AiReviewPublishSheetAndScopeDecisionRegistriesViolation::CohortEvidenceIntegrityNotProven
    ));
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet
        .validate()
        .contains(&M5AiReviewPublishSheetAndScopeDecisionRegistriesViolation::RawMaterialInExport));
}

#[test]
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_ai_review_publish_sheet_and_scope_decision_registries().export_safe_json();
...
#[test]
fn publish_to_review_sheet_table_lists_only_clean_publish_to_review_sheets() {
    let packet = seeded_m5_ai_review_publish_sheet_and_scope_decision_registries();
    let table = packet.render_publish_to_review_sheet_table();
    // The clean canary and migration publish_to_review_sheets are rendered from the registry.
    assert!(table.contains("local_draft_mode"));
    assert!(table.contains("publish_now_provider_comment_mode"));
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_ai_review_publish_sheet_and_scope_decision_registries_export()
        .expect(
        "checked M5 line-publish_to_review_sheet / line-downgrade-packet registries export validates",
    );
    assert_eq!(
        from_disk.packet_id,
        M5_AI_REVIEW_PUBLISH_SHEET_AND_SCOPE_DECISION_REGISTRIES_PACKET_ID
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5AiReviewPublishSheetAndScopeDecisionRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-ai-review-publish-sheet-and-scope-decision-registries/publish_to_review_sheet_beta_narrowed.json"
    )))
    .expect("line-publish_to_review_sheet fixture parses");
    ...
    let preview: M5AiReviewPublishSheetAndScopeDecisionRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-ai-review-publish-sheet-and-scope-decision-registries/publish_scope_decision_preview_narrowed.json"
    )))
