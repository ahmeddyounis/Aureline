//! Implemented M5 post-launch public-proof-ledger and claim-history-diff registries.
//!
//! The frozen [supported-line-transparency matrix][matrix] names Aureline's governed post-launch external-proof
//! objects and locks their controlled vocabulary. This module makes that external proof operable so public claims,
//! partner reviews, procurement checks, and support escalations inherit current rather than tribal truth instead of
//! being re-synthesized by hand: it turns the *public-proof-ledger* grammar (the ledger published per active
//! supported line — one typed section per joined proof source: a compatibility report, a benchmark / evidence
//! packet, a support-window statement, a known-limits set, a deprecation report, and a successor report — each
//! bound to one supported-line identity with its freshness state, last-versus-current diff, and the exact
//! evidence-packet refs currently backing its public claims) and the *claim-history-diff* grammar (the diff scope a
//! proof change sits in — a freshness change or move from current to retest-pending, a scope narrowing, or a
//! release-line reassociation) into registry resolvers that produce export-safe, honest projections. Every active
//! stable or LTS-candidate line then resolves to one typed public-proof-ledger object — the proof source it joins,
//! its affected rows, the compatibility / benchmark / support-window / known-limits / deprecation evidence-packet
//! refs it links, its rollback target, and its proof posture, all preserved before a claim widens so a line never
//! keeps onboarding / migration / support language ahead of current public proof and a current rollback target —
//! and to one claim-history-diff object — the resolved line identity, the affected proof-source reference, the
//! previous-versus-current claim-state reference, the diff-scope state, and the active diff reason — that the
//! release / help, About, docs, support, and public-proof surfaces can inspect without manual reconstruction, so
//! current-versus-previous claim-state history stays visible, a stale proof source narrows the related stable/LTS
//! support claim automatically, and a ledger that cannot bind a section to current evidence degrades honestly
//! instead of leaving a claim to read as still current.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Publish one typed public-proof-ledger section per joined proof source, per active supported line.**
//!   [`resolve_public_proof_ledger_entry`] refuses to read as a clean, registry-bound public-proof-ledger entry
//!   unless it names a canonical registry token, a classified [proof source][M5PublicProofLedgerKind], a
//!   transparency role, covers every [resolution form][M5SupportedLinePublicProofLedgerResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every ledger field (affected line
//!   rows, linked compatibility / benchmark / support-window / known-limits / deprecation evidence-packet refs,
//!   freshness state, rollback / reversibility target, and owning roster), preserves its rollback posture before a
//!   claim widens, and keeps any section bound to its current evidence or claim-narrowing action; otherwise it
//!   degrades.
//! * **Keep support / certification language from running ahead of current public proof.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a public-proof-ledger entry whose
//!   rollback posture is not preserved before widening (a line resuming a wider claim on stale proof) so it degrades
//!   to
//!   [`M5PublicProofLedgerEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a public-facing section whose compatibility / known-issues language outruns current proof degrades the same
//!   way — the structured blocker reason a widen-on-stale-proof attempt must surface.
//! * **Turn a proof change into a typed diff event instead of an implicit docs mismatch.**
//!   [`resolve_claim_history_diff_entry`] names a classified [diff scope][M5ClaimHistoryDiffScope]
//!   (freshness-change, scope-narrowing, or release-line-reassociation), requires the full line-identity /
//!   affected-proof-source / previous-versus-current-claim-state / diff-scope / active-reason diff object, covers
//!   every resolution form, and degrades to
//!   [`M5ClaimHistoryDiffEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the diff would keep support language ahead of current proof, hide the diff, or let a gap masquerade as
//!   covered, so a claim-history-diff can never read as trustworthy when it has quietly dropped the reason a claim
//!   changed state.
//!
...
pub use seed::{
    ...
    seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries_claim_history_diff_preview_narrowed,
    seeded_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries_public_proof_ledger_beta_narrowed,
    M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacket`].
pub const M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_supported_line_public_proof_ledger_and_claim_history_diff_packet_registries";

/// Schema version for M5 line-public_proof_ledger / line-downgrade-packet registry records.
pub const M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_SCHEMA_VERSION: u32 =
    1;

/// Repo-relative path of the combined registries schema.
pub const M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_DOC_REF: &str =
    "docs/release/m5_supported_line_public_proof_ledger_and_claim_history_diff_registries.md";
...
pub const M5_CLAIM_HISTORY_DIFF_DOMAIN_SCHEMA_REF: &str =
    "schemas/program/m5-claim-history-diff.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-supported-line-public-proof-ledger-and-claim-history-diff-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-supported-line-public-proof-ledger-and-claim-history-diff-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-supported-line-public-proof-ledger-and-claim-history-diff-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesConsumerSurface =
    M5SupportedLineTransparencyConsumerSurface;

/// One of the three resolution forms every line-public_proof_ledger or line-downgrade-packet entry must hold across so
...
/// line-public_proof_ledger and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SupportedLinePublicProofLedgerResolutionForm {
    /// The canonical resolved line-public_proof_ledger / line-downgrade-packet object.
    CanonicalObject,
...
impl M5SupportedLinePublicProofLedgerResolutionForm {
...
/// Controlled proof source a public-proof-ledger entry joins to one supported-line identity, so every active line's
/// external proof shares one typed registry rather than a hand-copied per-line assumption. Minted by this lane
/// because the frozen matrix names the supported lines but not the concrete proof sources a public-proof ledger
/// joins — a compatibility report, a benchmark / evidence packet, a support-window statement, a known-limits set, a
/// deprecation report, or a successor report. Every classified proof source carries its canonical mode, and the
/// known-limits-set and deprecation-report proof sources are public-facing (their status surfaces directly in
/// public compatibility / known-issues / support claim language) so their claim must stay matched to current public
/// proof and a current rollback target before the line widens.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PublicProofLedgerKind {
    /// The compatibility-report proof source: the compatibility report backing the line's public claims, with its
    /// freshness state and the exact evidence-packet ref currently backing it.
    CompatibilityReportProof,
    /// The benchmark / evidence-packet proof source: the benchmark or evidence packet backing the line's performance
    /// and capability claims, with its freshness state and packet ref.
    BenchmarkPacketProof,
    /// The support-window statement proof source: the support-window statement backing the line's supported-lifetime
    /// claims, with its freshness state and statement ref.
    SupportWindowStatementProof,
    /// The known-limits-set proof source (public-facing; its compatibility / known-issues claim must match current
    /// public proof and a current rollback target).
    KnownLimitsSetProof,
    /// The deprecation-report proof source (public-facing; the deprecation report or successor guidance backing the
    /// line's deprecation claims, whose claim must match current public proof and a current rollback target).
    DeprecationReportProof,
    /// The successor-report proof source: the successor / migration-successor report backing the line's continuity
    /// claims, linked to its evidence-packet ref.
    SuccessorReportProof,
    /// The proof source is unclassified, which is disallowed.
    ProofSourceUnclassified,
}

impl M5PublicProofLedgerKind {
    /// Every public-proof-ledger kind, in declaration order.
    pub const ALL: [Self; 7] = [
        Self::CompatibilityReportProof,
        Self::BenchmarkPacketProof,
        Self::SupportWindowStatementProof,
        Self::KnownLimitsSetProof,
        Self::DeprecationReportProof,
        Self::SuccessorReportProof,
        Self::ProofSourceUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::CompatibilityReportProof => "compatibility_report_proof",
            Self::BenchmarkPacketProof => "benchmark_packet_proof",
            Self::SupportWindowStatementProof => "support_window_statement_proof",
            Self::KnownLimitsSetProof => "known_limits_set_proof",
            Self::DeprecationReportProof => "deprecation_report_proof",
            Self::SuccessorReportProof => "successor_report_proof",
            Self::ProofSourceUnclassified => "proof_source_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::ProofSourceUnclassified)
    }

    /// The canonical mode for this public-proof-ledger kind.
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::CompatibilityReportProof => "compatibility_report_proof_mode",
            Self::BenchmarkPacketProof => "benchmark_packet_proof_mode",
            Self::SupportWindowStatementProof => "support_window_statement_proof_mode",
            Self::KnownLimitsSetProof => "known_limits_set_proof_mode",
            Self::DeprecationReportProof => "deprecation_report_proof_mode",
            Self::SuccessorReportProof => "successor_report_proof_mode",
            Self::ProofSourceUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(
            self,
            Self::KnownLimitsSetProof | Self::DeprecationReportProof
        )
    }
}

/// Controlled claim-history-diff scope a supported-line proof change sits in, so a stale or mismatched proof
/// source becomes a typed diff event rather than an implicit docs mismatch and shares one registry rather than a
/// hand-copied per-record assumption. Minted by this lane, tracking whether a proof source changed freshness or
/// moved from current to retest-pending (freshness-change), narrowed the scope it backs (scope-narrowing), or
/// changed the release-line identity it is associated with (release-line-reassociation). Each scope maps directly
/// to the diff events the implementation requirement names — a freshness change, a scope narrowing, or a
/// release-line reassociation — so external claims follow the same currentness rules as internal release control.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ClaimHistoryDiffScope {
    /// Freshness-change: a proof source changed freshness state or moved from current to retest-pending.
    FreshnessChange,
    /// Scope-narrowing: a proof source narrowed the claim scope it backs on the current line.
    ScopeNarrowing,
    /// Release-line-reassociation: a proof source changed the exact build / release-line identity it is bound to.
    ReleaseLineReassociation,
    /// The claim-history-diff scope is unclassified, which is disallowed.
    DiffScopeUnclassified,
}

impl M5ClaimHistoryDiffScope {
    /// Every comparison scope, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::FreshnessChange,
        Self::ScopeNarrowing,
        Self::ReleaseLineReassociation,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::FreshnessChange => "freshness_change",
            Self::ScopeNarrowing => "scope_narrowing",
            Self::ReleaseLineReassociation => "release_line_reassociation",
...
/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-public_proof_ledger or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SupportedLinePublicProofLedgerSurfaceContext {
...
impl M5SupportedLinePublicProofLedgerSurfaceContext {
...
/// One mandatory rendered part a line-public_proof_ledger or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SupportedLinePublicProofLedgerAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-public_proof_ledger entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-public_proof_ledger entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-public_proof_ledger
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line public_proof_ledger or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5SupportedLinePublicProofLedgerAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "claim_history_diff_fields",
...
/// line public_proof_ledger, a line-downgrade packet, or a degraded line-public_proof_ledger / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SupportedLinePublicProofLedgerNextAction {
    /// Expand the resolved line public_proof_ledger's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5SupportedLinePublicProofLedgerNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SupportedLinePublicProofLedgerExportField {
...
impl M5SupportedLinePublicProofLedgerExportField {
...
/// Reason a line-public_proof_ledger entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PublicProofLedgerEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the public_proof_ledger means.
    DescriptorTokenUnstated,
    /// The render / surface context cannot currently be resolved.
    SurfaceContextUnresolved,
    /// The line journey is unclassified (not in the resolved taxonomy).
    CohortProofSourceUnclassified,
    ...
    /// The resolved line-public_proof_ledger object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5PublicProofLedgerEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 9] = [
        Self::DescriptorTokenUnstated,
        Self::SurfaceContextUnresolved,
        Self::CohortProofSourceUnclassified,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "public_proof_ledger_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortProofSourceUnclassified => "line_proof_source_unclassified",
            Self::DescriptorNotBoundToRegistry => "public_proof_ledger_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "public_proof_ledger_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "public_proof_ledger_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5SupportedLinePublicProofLedgerNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5SupportedLinePublicProofLedgerNextAction::TraceCanonicalRegistry
            }
            Self::CohortProofSourceUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5SupportedLinePublicProofLedgerNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5SupportedLinePublicProofLedgerNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => {
                M5SupportedLinePublicProofLedgerNextAction::ReviewBlockedOrDegraded
            }
        }
    }
    ...
    pub const fn downgrade_trigger(self) -> M5SupportedLineTransparencyDowngradeTrigger {
        match self {
            ...
            Self::CohortProofSourceUnclassified | Self::CohortDescriptorObjectIncomplete => {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ClaimHistoryDiffEntryDegradeReason {
...
impl M5ClaimHistoryDiffEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
...
/// Input to [`resolve_public_proof_ledger_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5PublicProofLedgerEntryResolutionInput {
    /// Stable identity of the line-public_proof_ledger-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this public_proof_ledger binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.public_proof_ledger.deprecation_report_proof`); empty means unstated.
    ...
    pub report_section: M5PublicProofLedgerKind,
    /// The render / surface context.
    pub surface_context: M5SupportedLinePublicProofLedgerSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SupportedLinePublicProofLedgerResolutionForm>,
    ...
    /// True when the behavior traces to the line-public_proof_ledger registry (never a hand-copied constant).
...
/// Resolved, export-safe line-public_proof_ledger-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedPublicProofLedgerEntry {
    /// Stable identity of the line-public_proof_ledger-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this public_proof_ledger binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-public_proof_ledger object publishes every required field.
    pub public_proof_ledger_object_complete: bool,
    /// Whether the entry traces to the line-public_proof_ledger registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5PublicProofLedgerEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SupportedLinePublicProofLedgerNextAction,
    /// Whether the public_proof_ledger resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub public_proof_ledger_resolves_across_lines: bool,
}

impl M5ResolvedPublicProofLedgerEntry {
    /// Whether this line-public_proof_ledger entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_claim_history_diff_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ClaimHistoryDiffEntryResolutionInput {
    ...
    pub comparison_scope: M5ClaimHistoryDiffScope,
    /// The render / surface context.
    pub surface_context: M5SupportedLinePublicProofLedgerSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SupportedLinePublicProofLedgerResolutionForm>,
    ...
    pub keeps_claim_history_diff_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedClaimHistoryDiffEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub claim_history_diff_stays_honest: bool,
    ...
    pub provides_complete_claim_history_diff: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5ClaimHistoryDiffEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SupportedLinePublicProofLedgerNextAction,
...
impl M5ResolvedClaimHistoryDiffEntry {
...
pub enum M5SupportedLinePublicProofLedgerResolutionError {
    /// The line-public_proof_ledger-entry id was empty.
...
impl M5SupportedLinePublicProofLedgerResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_public_proof_ledger_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_claim_history_diff_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5SupportedLinePublicProofLedgerResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-public_proof_ledger / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5SupportedLinePublicProofLedgerResolutionError {}

fn form_tokens(forms: &[M5SupportedLinePublicProofLedgerResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5SupportedLinePublicProofLedgerResolutionForm]) -> bool {
    let present: BTreeSet<M5SupportedLinePublicProofLedgerResolutionForm> =
        forms.iter().copied().collect();
    M5SupportedLinePublicProofLedgerResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-public_proof_ledger object publishes every required field: classified line journey,
...
pub fn public_proof_ledger_object_is_complete(
    journey: M5PublicProofLedgerKind,
...
/// Whether the line public_proof_ledger keeps a line from widening without preserving its rollback and diagnostics
...
pub fn claim_history_diff_stays_honest(
    scope: M5ClaimHistoryDiffScope,
    comparison_is_truthful: bool,
    keeps_claim_history_diff_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_claim_history_diff_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-public_proof_ledger-registry entry so it stays bound to the line-public_proof_ledger registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete public_proof_ledger object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_public_proof_ledger_entry(
    input: M5PublicProofLedgerEntryResolutionInput,
) -> Result<M5ResolvedPublicProofLedgerEntry, M5SupportedLinePublicProofLedgerResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5SupportedLinePublicProofLedgerResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5SupportedLinePublicProofLedgerResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = public_proof_ledger_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5PublicProofLedgerEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5PublicProofLedgerEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5PublicProofLedgerEntryDegradeReason::CohortProofSourceUnclassified)
    } else if !input.bound_to_registry {
        Some(M5PublicProofLedgerEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5PublicProofLedgerEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5PublicProofLedgerEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5PublicProofLedgerEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(
            M5PublicProofLedgerEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    } else if !input.proof_fresh {
        Some(M5PublicProofLedgerEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5SupportedLinePublicProofLedgerNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedPublicProofLedgerEntry {
        ...
        public_proof_ledger_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        public_proof_ledger_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_claim_history_diff_entry(
    input: M5ClaimHistoryDiffEntryResolutionInput,
) -> Result<M5ResolvedClaimHistoryDiffEntry, M5SupportedLinePublicProofLedgerResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(
            M5SupportedLinePublicProofLedgerResolutionError::EmptyCohortEvidencePacketEntryId,
        );
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = claim_history_diff_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_claim_history_diff_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ClaimHistoryDiffEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ClaimHistoryDiffEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5ClaimHistoryDiffEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5ClaimHistoryDiffEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5ClaimHistoryDiffEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5ClaimHistoryDiffEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5SupportedLinePublicProofLedgerNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedClaimHistoryDiffEntry {
        entry_id: input.entry_id,
        ...
        keeps_claim_history_diff_visible: input.keeps_claim_history_diff_visible,
        ...
        claim_history_diff_stays_honest: record_stays_honest,
        provides_complete_claim_history_diff: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-public_proof_ledger and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5SupportedLinePublicProofLedgerAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5SupportedLinePublicProofLedgerExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5SupportedLineTransparencyDowngradeTrigger>,
    /// Resolved line-public_proof_ledger-registry examples.
    pub public_proof_ledger_entries: Vec<M5ResolvedPublicProofLedgerEntry>,
    /// Resolved line-downgrade-packet examples.
    pub claim_history_diff_entries: Vec<M5ResolvedClaimHistoryDiffEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-public_proof_ledger and
    ...
    pub collapses_distinct_claim_history_diff_classes_into_one_lane: bool,
}

impl M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5SupportedLinePublicProofLedgerAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5SupportedLinePublicProofLedgerAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5SupportedLinePublicProofLedgerExportField> =
            self.export_fields.iter().copied().collect();
        M5SupportedLinePublicProofLedgerExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_claim_history_diff_classes_into_one_lane
    }

    /// True when a clean line-public_proof_ledger entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete public_proof_ledger object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn public_proof_ledger_is_honest(ex: &M5ResolvedPublicProofLedgerEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.public_proof_ledger_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedClaimHistoryDiffEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_claim_history_diff
                && ex.claim_history_diff_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.public_proof_ledger_entries
            .iter()
            .all(Self::public_proof_ledger_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesVocabularySet {
    ...
    /// Cohort-public_proof_ledger-entry degrade-reason tokens.
    pub public_proof_ledger_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub claim_history_diff_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5SupportedLineTransparencyRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5SupportedLinePublicProofLedgerResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5PublicProofLedgerKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ClaimHistoryDiffScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5SupportedLinePublicProofLedgerSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            public_proof_ledger_degrade_reasons: tokens(
                &M5PublicProofLedgerEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            claim_history_diff_degrade_reasons: tokens(
                &M5ClaimHistoryDiffEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5SupportedLinePublicProofLedgerAnatomyPart::ALL, |v| {
                v.as_str()
            }),
            next_actions: tokens(&M5SupportedLinePublicProofLedgerNextAction::ALL, |v| {
                v.as_str()
            }),
            export_fields: tokens(&M5SupportedLinePublicProofLedgerExportField::ALL, |v| {
                v.as_str()
            }),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesGovernanceReview {
    /// The public_proof_ledger registry names a canonical token, semantic role, and line journey for every entry.
    pub public_proof_ledger_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-public_proof_ledger object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_public_proof_ledger_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved public_proof_ledger.
    ...
    pub claim_history_diff_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-public_proof_ledger and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-public_proof_ledger and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub public_proof_ledger_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-public_proof_ledger registry.
    ...
    /// Behavior traces back to the canonical line-public_proof_ledger and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-public_proof_ledger / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesReleasePosture {
...
/// Constructor input for [`M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesVocabularySet,
    ...
    /// Proof freshness block.
    pub proof_freshness: M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesReleasePosture,
...
/// Export-safe M5 line-public_proof_ledger and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacket {
    /// Record kind; must equal [`M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_SCHEMA_VERSION`].
...
impl M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(
        input: M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacketInput,
    ) -> Self {
        Self {
            record_kind:
                M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_RECORD_KIND
                    .to_owned(),
            schema_version:
                M5_SUPPORTED_LINE_PUBLIC_PROOF_LEDGER_CLAIM_HISTORY_DIFF_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(
        &self,
    ) -> Vec<M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation> {
        ...
        {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::WrongRecordKind,
            );
        }
        ...
        {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::MissingIdentity,
            );
        }
        ...
        if !self.vocabulary_set.matches_canonical() {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::VocabularySetDrift,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-public_proof_ledger / line-downgrade-packet registries packet serializes",
        )) {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::RawMaterialInExport,
            );
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,public_proof_ledger_entries,claim_history_diff_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            let degrades: Vec<&str> = row
                ...
                .chain(
                    row.claim_history_diff_entries
                        .iter()
                        .filter_map(|ex| ex.degrade_reason.map(|r| r.as_str())),
                )
                .collect();
            out.push_str(&format!(
                ...
                row.public_proof_ledger_entries.len(),
                row.claim_history_diff_entries.len(),
                degrades.join("|"),
    ...
    /// registry-bound line-public_proof_ledger entries are listed.
    pub fn render_public_proof_ledger_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.public_proof_ledger_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation>),
}

impl fmt::Display for M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-public_proof_ledger / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-public_proof_ledger / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesArtifactError {}

/// Validation failures emitted by [`M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation {
    ...
    /// A registry row does not point at both the line-public_proof_ledger and line-downgrade-packet domain schemas.
    ...
    /// Cohort-public_proof_ledger-resolution is not proven: clean public_proof_ledger entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean public_proof_ledger entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded public_proof_ledger entry is present, or a clean public_proof_ledger entry is unbounded
    /// or unbound.
...
impl M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "public_proof_ledger_resolution_not_proven"
            }
            Self::RollbackAndDiagnosticsPreservationNotProven => {
                "rollback_and_diagnostics_preservation_not_proven"
            }
            Self::CohortEvidenceIntegrityNotProven => "claim_history_diff_integrity_not_proven",
...
pub fn current_stable_m5_supported_line_public_proof_ledger_and_claim_history_diff_registries_export(
) -> Result<
    M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacket,
    M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesArtifactError,
> {
    let packet: M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-supported-line-public-proof-ledger-and-claim-history-diff-registries-proof/support_export.json"
        )
    ))
    .map_err(M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(
            M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesArtifactError::Validation(
                violations,
            ),
        )
    }
...
fn validate_source_contracts(
    packet: &M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesPacket,
    violations: &mut Vec<M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(
            M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::NoRegistryRows,
        );
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        ...
        if !refs.contains(M5_PUBLIC_PROOF_FRESHNESS_LEDGER_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_CLAIM_HISTORY_DIFF_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.public_proof_ledger_entries.is_empty() || row.claim_history_diff_entries.is_empty() {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::ExamplesMissing,
            );
        }
        if !row.examples_are_honest() {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::DishonestExample,
            );
        }
        if !row.honours_invariants() {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.public_proof_ledger_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_public_proof_ledger_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.claim_history_diff_keeps_proof_visible_and_binds_support_language,
        ...
        review.public_proof_ledger_or_downgrade_drift_caught_before_release,
        review.every_row_declares_mandatory_anatomy,
        review.reuses_frozen_matrix_vocabulary,
    ] {
        if !ok {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let public_proof_ledgers = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.public_proof_ledger_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.claim_history_diff_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean public_proof_ledger entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean public_proof_ledger entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = public_proof_ledgers()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = public_proof_ledgers()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5PublicProofLedgerKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5SupportedLinePublicProofLedgerSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = public_proof_ledgers().any(|ex| {
        ex.degrade_reason
            == Some(M5PublicProofLedgerEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete =
        !public_proof_ledgers().any(|ex| ex.is_clean() && !ex.public_proof_ledger_object_complete);
    if !(journeys_covered
    ...
    {
        violations.push(
            M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }

    // AC2: line packets preserve rollback and diagnostics posture before widening. A widen-without-rollback
    // example degrades, an unbound example degrades, at least one clean bounded public_proof_ledger entry is present, and
    // no clean public_proof_ledger entry is unbounded or unbound.
    let widen_fold_degrades = public_proof_ledgers().any(|ex| {
        ex.degrade_reason
            == Some(
                M5PublicProofLedgerEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = public_proof_ledgers().any(|ex| {
        ex.degrade_reason
            == Some(M5PublicProofLedgerEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_public_proof_ledger =
        public_proof_ledgers().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound = !public_proof_ledgers().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded =
        !public_proof_ledgers().any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_public_proof_ledger
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
            M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ClaimHistoryDiffScope::CANONICAL_SCOPES
        .iter()
    ...
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_claim_history_diff);
    ...
    {
        violations.push(
            M5SupportedLinePublicProofLedgerClaimHistoryDiffRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-public_proof_ledger registry
