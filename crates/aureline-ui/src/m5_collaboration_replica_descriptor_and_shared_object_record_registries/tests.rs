...
fn clean_review_pack_record_input() -> M5CollaborationReplicaRecordEntryResolutionInput {
    M5CollaborationReplicaRecordEntryResolutionInput {
        ...
        report_section: M5CollaborationReplicaRecordKind::ChangedFilesScope,
        surface_context: M5CollaborationReplicaRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5CollaborationReplicaRecordResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5CollaborationReplicaResultEntryResolutionInput {
    M5CollaborationReplicaResultEntryResolutionInput {
        ...
        semantic_role: M5CollaborationStateRole::AuthorityModelDisclosure,
        comparison_scope: M5CollaborationReplicaResultScope::EvaluatedScopeBinding,
        surface_context: M5CollaborationReplicaRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5CollaborationReplicaRecordResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_collaboration_replica_descriptor_and_shared_object_record_registries();
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(
        ...
        M5CollaborationReplicaRecordNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5CollaborationReplicaRecordKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5CollaborationReplicaRecordKind::FullTreeScope;
    ...
    let mut input = clean_review_pack_record_input();
    input.surface_context = M5CollaborationReplicaRecordSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        ...
        M5CollaborationReplicaRecordResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input).unwrap_err(),
        M5CollaborationReplicaRecordResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5CollaborationReplicaRecordKind::PackScopeUnclassified,
        true,
        false,
        true
    ));
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5CollaborationReplicaResultScope::ChangeObjectResultUnclassified;
    assert_eq!(
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    assert_eq!(
        ...
        M5CollaborationReplicaRecordResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5CollaborationReplicaRecordAndResultRegistriesViolation::VocabularySetDrift));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_SHARED_OBJECT_DESCRIPTOR_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5CollaborationReplicaRecordAndResultRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5CollaborationReplicaRecordAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5CollaborationReplicaRecordExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5CollaborationReplicaRecordAndResultRegistriesViolation::ExamplesMissing));
}
...
fn dishonest_clean_example_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5CollaborationReplicaRecordAndResultRegistriesViolation::DishonestExample));
}
...
fn rollback_preservation_not_proven_when_unbound_example_removed() {
    ...
    for row in &mut packet.registry_rows {
        row.review_pack_record_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
                    M5CollaborationReplicaRecordEntryDegradeReason::DescriptorNotBoundToRegistry,
                )
        });
    }
    assert!(packet.validate().contains(
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet
        .validate()
        .contains(&M5CollaborationReplicaRecordAndResultRegistriesViolation::RawMaterialInExport));
}

#[test]
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_collaboration_replica_descriptor_and_shared_object_record_registries()
        .export_safe_json();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_collaboration_replica_descriptor_and_shared_object_record_registries_export()
        .expect(
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5CollaborationReplicaRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-collaboration-replica-descriptor-and-shared-object-record-registries/collaboration_replica_descriptor_beta_narrowed.json"
    )))
    ...
    let preview: M5CollaborationReplicaRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-collaboration-replica-descriptor-and-shared-object-record-registries/shared_object_record_preview_narrowed.json"
    )))
