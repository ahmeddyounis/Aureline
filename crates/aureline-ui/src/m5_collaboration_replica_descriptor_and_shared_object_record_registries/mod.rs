//! Implemented M5 collaboration-replica-descriptor and shared-object-record registries.
//!
//! The frozen [collaboration-state authority matrix][matrix] names Aureline's governed shared-object classes and
//! locks their authority, convergence, and export vocabulary. This module makes the durable collaboration replica
//! descriptor operable — as a canonical record — and adds the per-class shared-object record every consumer must
//! inspect, so the shared editor replica view, the presence / cursor layer, the comment / annotation / review-pin
//! layer, the collaboration degradation banner, and the support / export and help / docs surfaces inherit one
//! canonical replica descriptor rather than a hand-authored parallel prose that has to be kept consistent: it turns
//! the *collaboration-replica-descriptor* grammar (one durable record per shared object entering share mode — one
//! typed field per record section: the workspace-root identity, the buffer / object identity, the revision and
//! session epoch, the trust and policy epoch, and the export posture — each bound to the local canonical truth it
//! never replaces) and the *shared-object-record* grammar (the authority model each object holds — CRDT-convergent
//! shared editable text, sampled presence / cursor / selection samples, server-ordered comments / annotations /
//! review pins, presenter / follow state, linked higher-risk control objects, or immutable sealed-archive evidence —
//! plus its convergence and merge-drift posture, its append-only anchor-drift history, and its policy-labeled export
//! lineage) into registry resolvers that produce export-safe, honest projections. Every shared object then resolves
//! to one typed collaboration replica descriptor — the fields it publishes, its workspace-root and buffer / object
//! identity, its revision / session and trust / policy epoch, and its export posture, all attributable to the object
//! so a replica never overwrites local buffer, VFS, or Git canonical truth — and to one shared-object record — the
//! declared authority model, the convergence state, the anchor-drift history, and the export lineage — that the
//! shared editor, presence, review-pin, degradation-banner, and support surfaces can inspect without manual
//! reconstruction, so entering share mode creates an explicit replica record rather than replacing local buffer
//! authority outright, and a descriptor that cannot bind a field to its buffer identity degrades honestly instead of
//! letting a replica read as the canonical buffer when it is not.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable collaboration replica descriptor per shared object, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [object class][M5CollaborationReplicaRecordKind],
//!   a disclosure role, covers every [resolution form][M5CollaborationReplicaRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (workspace-root
//!   identity, buffer / object identity, revision / session epoch, trust / policy epoch, and the export posture
//!   bound to the replica identity), keeps its lineage attributable to the object, and keeps the local canonical
//!   truth it never replaces bound to the descriptor; otherwise it degrades.
//! * **Never replace local buffer authority; preserve local unsent work first on any downgrade.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a descriptor whose
//!   local-truth preservation is not held (a replica presenting itself as authoritative over the local buffer) so it
//!   degrades to
//!   [`M5CollaborationReplicaRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a shared object that would discard unsent local edits on a permission or relay downgrade degrades the same
//!   way — the structured blocker reason a replica-overwrites-local-truth attempt must surface.
//! * **Declare which authority model each shared object holds, with append-only anchor drift and policy-labeled
//!   export lineage, instead of a silent convergence claim.** [`resolve_review_pack_result_entry`] names a classified
//!   [authority binding][M5CollaborationReplicaResultScope]
//!   (CRDT-convergent binding, server-ordered binding, or host-authoritative binding), requires the full
//!   object-identity / declared-authority-model / convergence-versus-degraded-state / binding / active-reason
//!   record, covers every resolution form, and degrades to
//!   [`M5CollaborationReplicaResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the record would present a degraded state as converged, rebind an anchor without drift history, or let an
//!   export drop its redaction lineage, so a shared object can never read as trustworthy when it has quietly dropped
//!   the reason its authority model and convergence state had to be declared.
//!
...
//! export surfaces can never fork their own replica-descriptor meaning. Raw secrets, raw command text, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    ...
    seeded_m5_collaboration_replica_descriptor_and_shared_object_record_registries_review_pack_record_beta_narrowed,
    seeded_m5_collaboration_replica_descriptor_and_shared_object_record_registries_review_pack_result_preview_narrowed,
    M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_PACKET_ID,
};
...
use crate::m5_collaboration_replica_shared_object_authority_anchor_drift_convergence_and_session_archive_matrix::{
    ...
    M5_CRDT_BACKED_SHARED_TEXT_DOMAIN_SCHEMA_REF, M5_COLLABORATION_STATE_MATRIX_DOC_REF,
    M5_COLLABORATION_STATE_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5CollaborationReplicaRecordAndResultRegistriesPacket`].
pub const M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_RECORD_KIND:
    &str =
    "implement_m5_collaboration_replica_descriptor_and_shared_object_record_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_SCHEMA_VERSION:
    u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_collaboration_replica_descriptor_and_shared_object_record_registries.md";
...
pub const M5_SHARED_OBJECT_DESCRIPTOR_DOMAIN_SCHEMA_REF: &str =
    "schemas/collaboration/m5-shared-object-descriptor.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-collaboration-replica-descriptor-and-shared-object-record-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-collaboration-replica-descriptor-and-shared-object-record-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-collaboration-replica-descriptor-and-shared-object-record-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-collaboration-replica-descriptor-and-shared-object-record-registries";
...
pub type M5CollaborationReplicaRecordAndResultRegistriesConsumerSurface =
...
pub enum M5CollaborationReplicaRecordResolutionForm {
...
impl M5CollaborationReplicaRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationReplicaRecordKind {
...
impl M5CollaborationReplicaRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationReplicaResultScope {
...
impl M5CollaborationReplicaResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationReplicaRecordSurfaceContext {
...
impl M5CollaborationReplicaRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationReplicaRecordAnatomyPart {
...
impl M5CollaborationReplicaRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationReplicaRecordNextAction {
...
impl M5CollaborationReplicaRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationReplicaRecordExportField {
...
impl M5CollaborationReplicaRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationReplicaRecordEntryDegradeReason {
...
impl M5CollaborationReplicaRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5CollaborationReplicaRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5CollaborationReplicaRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5CollaborationReplicaRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5CollaborationReplicaRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5CollaborationReplicaRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5CollaborationReplicaResultEntryDegradeReason {
...
impl M5CollaborationReplicaResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5CollaborationReplicaRecordNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5CollaborationReplicaRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5CollaborationReplicaRecordEntryResolutionInput {
    ...
    pub report_section: M5CollaborationReplicaRecordKind,
    /// The render / surface context.
    pub surface_context: M5CollaborationReplicaRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5CollaborationReplicaRecordResolutionForm>,
...
pub struct M5ResolvedCollaborationReplicaRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5CollaborationReplicaRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5CollaborationReplicaRecordNextAction,
...
impl M5ResolvedCollaborationReplicaRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5CollaborationReplicaResultEntryResolutionInput {
    ...
    pub comparison_scope: M5CollaborationReplicaResultScope,
    /// The render / surface context.
    pub surface_context: M5CollaborationReplicaRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5CollaborationReplicaRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedCollaborationReplicaResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5CollaborationReplicaResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5CollaborationReplicaRecordNextAction,
...
impl M5ResolvedCollaborationReplicaResultEntry {
...
pub enum M5CollaborationReplicaRecordResolutionError {
...
impl M5CollaborationReplicaRecordResolutionError {
...
impl fmt::Display for M5CollaborationReplicaRecordResolutionError {
...
impl Error for M5CollaborationReplicaRecordResolutionError {}

fn form_tokens(forms: &[M5CollaborationReplicaRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5CollaborationReplicaRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5CollaborationReplicaRecordResolutionForm> =
        forms.iter().copied().collect();
    M5CollaborationReplicaRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5CollaborationReplicaRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5CollaborationReplicaResultScope,
...
) -> Result<M5ResolvedCollaborationReplicaRecordEntry, M5CollaborationReplicaRecordResolutionError>
{
    if input.entry_id.trim().is_empty() {
        return Err(M5CollaborationReplicaRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5CollaborationReplicaRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5CollaborationReplicaRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5CollaborationReplicaRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5CollaborationReplicaRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5CollaborationReplicaRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5CollaborationReplicaRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5CollaborationReplicaRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5CollaborationReplicaRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5CollaborationReplicaRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5CollaborationReplicaRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5CollaborationReplicaRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedCollaborationReplicaRecordEntry {
...
) -> Result<M5ResolvedCollaborationReplicaResultEntry, M5CollaborationReplicaRecordResolutionError>
{
    if input.entry_id.trim().is_empty() {
        return Err(M5CollaborationReplicaRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5CollaborationReplicaResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5CollaborationReplicaResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5CollaborationReplicaResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5CollaborationReplicaResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5CollaborationReplicaResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5CollaborationReplicaResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5CollaborationReplicaRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedCollaborationReplicaResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationReplicaRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5CollaborationReplicaRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5CollaborationReplicaRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5CollaborationReplicaRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedCollaborationReplicaRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedCollaborationReplicaResultEntry>,
...
impl M5CollaborationReplicaRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5CollaborationReplicaRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5CollaborationReplicaRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5CollaborationReplicaRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5CollaborationReplicaRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedCollaborationReplicaRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedCollaborationReplicaResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationReplicaRecordAndResultRegistriesVocabularySet {
...
impl M5CollaborationReplicaRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5CollaborationStateRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5CollaborationReplicaRecordResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5CollaborationReplicaRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5CollaborationReplicaResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5CollaborationReplicaRecordSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            review_pack_record_degrade_reasons: tokens(
                &M5CollaborationReplicaRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5CollaborationReplicaResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5CollaborationReplicaRecordAnatomyPart::ALL, |v| {
                v.as_str()
            }),
            next_actions: tokens(&M5CollaborationReplicaRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5CollaborationReplicaRecordExportField::ALL, |v| {
                v.as_str()
            }),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationReplicaRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationReplicaRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationReplicaRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationReplicaRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5CollaborationReplicaRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5CollaborationReplicaRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5CollaborationReplicaRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5CollaborationReplicaRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5CollaborationReplicaRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5CollaborationReplicaRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5CollaborationReplicaRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5CollaborationReplicaRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5CollaborationReplicaRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_SCHEMA_VERSION`].
...
impl M5CollaborationReplicaRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5CollaborationReplicaRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version:
                M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5CollaborationReplicaRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind
            != M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_RECORD_KIND
        {
            violations
                .push(M5CollaborationReplicaRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_COLLABORATION_REPLICA_DESCRIPTOR_AND_SHARED_OBJECT_RECORD_REGISTRIES_SCHEMA_VERSION
        {
            violations.push(M5CollaborationReplicaRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations
                .push(M5CollaborationReplicaRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations
                .push(M5CollaborationReplicaRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(
                M5CollaborationReplicaRecordAndResultRegistriesViolation::RawMaterialInExport,
            );
        }
...
#[derive(Debug)]
pub enum M5CollaborationReplicaRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5CollaborationReplicaRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5CollaborationReplicaRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5CollaborationReplicaRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5CollaborationReplicaRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5CollaborationReplicaRecordAndResultRegistriesViolation {
...
impl M5CollaborationReplicaRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_collaboration_replica_descriptor_and_shared_object_record_registries_export(
) -> Result<
    M5CollaborationReplicaRecordAndResultRegistriesPacket,
    M5CollaborationReplicaRecordAndResultRegistriesArtifactError,
> {
    let packet: M5CollaborationReplicaRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-collaboration-replica-descriptor-and-shared-object-record-registries-proof/support_export.json"
        )
    ))
    .map_err(M5CollaborationReplicaRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5CollaborationReplicaRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5CollaborationReplicaRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5CollaborationReplicaRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5CollaborationReplicaRecordAndResultRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5CollaborationReplicaRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5CollaborationReplicaRecordAndResultRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5CollaborationReplicaRecordAndResultRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5CollaborationReplicaRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        ...
        if !refs.contains(M5_CRDT_BACKED_SHARED_TEXT_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_SHARED_OBJECT_DESCRIPTOR_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5CollaborationReplicaRecordAndResultRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations
                .push(M5CollaborationReplicaRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations
                .push(M5CollaborationReplicaRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(
                M5CollaborationReplicaRecordAndResultRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5CollaborationReplicaRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5CollaborationReplicaRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5CollaborationReplicaRecordAndResultRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5CollaborationReplicaRecordAndResultRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5CollaborationReplicaRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5CollaborationReplicaRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = review_pack_records().any(|ex| {
        ex.degrade_reason
            == Some(
                M5CollaborationReplicaRecordEntryDegradeReason::CohortDescriptorObjectIncomplete,
            )
    });
    ...
    {
        violations.push(
            M5CollaborationReplicaRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5CollaborationReplicaRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5CollaborationReplicaResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(
            M5CollaborationReplicaRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
