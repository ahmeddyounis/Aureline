...
fn clean_transparency_report_input() -> M5TransparencyReportEntryResolutionInput {
    M5TransparencyReportEntryResolutionInput {
        entry_id: "transparency_report:test".to_owned(),
        line_binding_id: "launch.line.core-team-canary".to_owned(),
        token_name: "line.transparency_report.core_team_canary".to_owned(),
        semantic_role: M5SupportedLineTransparencyRole::FreshnessWindow,
        report_section: M5TransparencyReportKind::CriticalUpstreamStatusSection,
        surface_context: M5SupportedLineTransparencyReportSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SupportedLineTransparencyReportResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5SnapshotDiffEntryResolutionInput {
    M5SnapshotDiffEntryResolutionInput {
        ...
        semantic_role: M5SupportedLineTransparencyRole::FreshnessWindow,
        comparison_scope: M5SnapshotDiffScope::HealthStatusChange,
        surface_context: M5SupportedLineTransparencyReportSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SupportedLineTransparencyReportResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_supported_line_transparency_report_and_snapshot_diff_registries();
...
#[test]
fn transparency_report_clean_names_meaning_and_is_bound() {
    let resolved = resolve_transparency_report_entry(clean_transparency_report_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.transparency_report_resolves_across_lines);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.transparency_report_object_complete);
    ...
    assert_eq!(resolved.report_section, "critical_upstream_status_section");
    assert_eq!(
        resolved.canonical_report_section_mode,
        "critical_upstream_status_section_mode"
    );
    assert_eq!(resolved.surface_context, "shiproom_surface");
    assert_eq!(
        resolved.next_action,
        M5SupportedLineTransparencyReportNextAction::ExpandCohortMeaning
    );
}

#[test]
fn transparency_report_token_unstated_degrades() {
    let mut input = clean_transparency_report_input();
    input.token_name = "   ".to_owned();
    assert_eq!(
        resolve_transparency_report_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn transparency_report_unbound_and_unclassified_degrade() {
    let mut input = clean_transparency_report_input();
    ...
    let mut input = clean_transparency_report_input();
    input.report_section = M5TransparencyReportKind::ReportSectionUnclassified;
    assert_eq!(
        resolve_transparency_report_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn transparency_report_object_incomplete_and_widen_fold_and_form_degrade() {
    // An unstated bundle IDs field leaves the resolved object incomplete.
    let mut input = clean_transparency_report_input();
    input.bundle_ids = "  ".to_owned();
    let resolved = resolve_transparency_report_entry(input).unwrap();
    assert!(!resolved.transparency_report_object_complete);
    assert_eq!(
    ...
    assert_eq!(
        resolve_transparency_report_entry(input).unwrap().degrade_reason,
...
#[test]
fn transparency_report_public_facing_and_surface_and_proof_degrade() {
    // A public-facing line running support language ahead of proof first fails the widen-boundary fold.
    let mut input = clean_transparency_report_input();
    input.report_section = M5TransparencyReportKind::SustainmentSponsorPostureSection;
    ...
    let mut input = clean_transparency_report_input();
    input.surface_context = M5SupportedLineTransparencyReportSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_transparency_report_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn transparency_report_empty_id_and_forbidden_material_error() {
    let mut input = clean_transparency_report_input();
    input.entry_id = "".to_owned();
    assert_eq!(
        resolve_transparency_report_entry(input).unwrap_err(),
        M5SupportedLineTransparencyReportResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_transparency_report_entry(input).unwrap_err(),
        M5SupportedLineTransparencyReportResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5TransparencyReportKind::ReportSectionUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn transparency_report_object_is_complete_requires_all_fields() {
    assert!(transparency_report_object_is_complete(
        M5TransparencyReportKind::CriticalUpstreamStatusSection,
...
fn downgrade_clean_stays_honest() {
    let resolved = resolve_snapshot_diff_entry(clean_downgrade_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.comparison_safe_on_every_line);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.provides_complete_snapshot_diff);
    assert!(resolved.snapshot_diff_stays_honest);
    assert_eq!(resolved.comparison_scope, "health_status_change");
    assert_eq!(resolved.surface_context, "shiproom_surface");
}
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let resolved = resolve_snapshot_diff_entry(input).unwrap();
    assert!(!resolved.provides_complete_snapshot_diff);
    assert_eq!(
        resolved.degrade_reason,
    ...
    let mut input = clean_downgrade_input();
    input.keeps_snapshot_diff_visible = false;
    assert_eq!(
        resolve_snapshot_diff_entry(input).unwrap().degrade_reason,
        Some(M5SnapshotDiffEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    );
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5SnapshotDiffScope::DiffScopeUnclassified;
    assert_eq!(
        resolve_snapshot_diff_entry(input).unwrap().degrade_reason,
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    input.entry_id = "  ".to_owned();
    assert_eq!(
        resolve_snapshot_diff_entry(input).unwrap_err(),
...
fn downgrade_bound_support_and_flagged_gap_stay_clean() {
    ...
    assert!(resolve_snapshot_diff_entry(input).unwrap().is_clean());
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_SNAPSHOT_DIFF_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5SupportedLineTransparencyReportSnapshotDiffRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5SupportedLineTransparencyReportAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5SupportedLineTransparencyReportExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    let mut packet = seeded_m5_supported_line_transparency_report_and_snapshot_diff_registries();
    packet.registry_rows[0].snapshot_diff_entries.clear();
    assert!(packet.validate().contains(
...
fn dishonest_clean_example_fails() {
    let mut packet = seeded_m5_supported_line_transparency_report_and_snapshot_diff_registries();
    // Force a clean transparency_report entry to also read as object-incomplete — the packet must reject it.
    let row = &mut packet.registry_rows[0];
    row.transparency_report_entries[0].degrade_reason = None;
    row.transparency_report_entries[0].transparency_report_object_complete = false;
    assert!(packet.validate().contains(
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        match mutate {
            ...
            _ => row.collapses_distinct_snapshot_diff_classes_into_one_lane = true,
        }
        assert!(packet.validate().contains(
...
#[test]
fn transparency_report_not_proven_when_incomplete_example_removed() {
    let mut packet = seeded_m5_supported_line_transparency_report_and_snapshot_diff_registries();
    for row in &mut packet.registry_rows {
        row.transparency_report_entries.retain(|ex| {
            ex.degrade_reason
                != Some(M5TransparencyReportEntryDegradeReason::CohortDescriptorObjectIncomplete)
        });
    }
...
#[test]
fn transparency_report_not_proven_when_surface_collapses() {
    let mut packet = seeded_m5_supported_line_transparency_report_and_snapshot_diff_registries();
    // Drop every clean executive-steering-surface transparency_report so the first-consumer surfaces no longer include it.
    for row in &mut packet.registry_rows {
        row.transparency_report_entries
...
#[test]
fn snapshot_diff_integrity_not_proven_when_support_ahead_example_removed() {
    let mut packet = seeded_m5_supported_line_transparency_report_and_snapshot_diff_registries();
    for row in &mut packet.registry_rows {
        row.snapshot_diff_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
...
#[test]
fn snapshot_diff_integrity_not_proven_when_scope_dropped() {
    let mut packet = seeded_m5_supported_line_transparency_report_and_snapshot_diff_registries();
    // Drop every clean go-no-go-signoff downgrade so the coverage no longer includes it.
    for row in &mut packet.registry_rows {
        row.snapshot_diff_entries
            .retain(|ex| !(ex.is_clean() && ex.comparison_scope == "red_risk_drift"));
    }
    assert!(packet.validate().contains(
        &M5SupportedLineTransparencyReportSnapshotDiffRegistriesViolation::CohortEvidenceIntegrityNotProven
    ));
...
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_supported_line_transparency_report_and_snapshot_diff_registries()
        .export_safe_json();
...
#[test]
fn transparency_report_table_lists_only_clean_transparency_reports() {
    let packet = seeded_m5_supported_line_transparency_report_and_snapshot_diff_registries();
    let table = packet.render_transparency_report_table();
    // The clean canary and migration transparency_reports are rendered from the registry.
    assert!(table.contains("critical_upstream_status_section_mode"));
    assert!(table.contains("backup_maintainer_coverage_section_mode"));
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk =
        current_stable_m5_supported_line_transparency_report_and_snapshot_diff_registries_export()
            .expect(
            "checked M5 line-transparency_report / line-downgrade-packet registries export validates",
        );
    assert_eq!(
        from_disk.packet_id,
        M5_SUPPORTED_LINE_TRANSPARENCY_REPORT_SNAPSHOT_DIFF_REGISTRIES_PACKET_ID
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5SupportedLineTransparencyReportSnapshotDiffRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-supported-line-transparency-report-and-snapshot-diff-registries/transparency_report_beta_narrowed.json"
    )))
    .expect("line-transparency_report fixture parses");
    ...
    let preview: M5SupportedLineTransparencyReportSnapshotDiffRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-supported-line-transparency-report-and-snapshot-diff-registries/snapshot_diff_preview_narrowed.json"
    )))
