...
fn seed() -> M5ChangeIntentSharedConsumersPacket {
    seeded_m5_change_intent_shared_consumers()
}

fn violations_of(packet: &M5ChangeIntentSharedConsumersPacket) -> Vec<&'static str> {
...
fn seed_validates_clean() {
    ...
    assert_eq!(
        packet.record_kind,
        M5_CHANGE_INTENT_SHARED_CONSUMERS_RECORD_KIND
    );
...
fn every_consumer_surface_and_representation_is_exercised() {
    ...
    let representations: BTreeSet<_> = packet
        ...
        .collect();
    for representation in ChangeIntentSharedRepresentation::ALL {
        assert!(
...
#[test]
fn every_change_intent_role_word_is_a_frozen_role_token() {
    let packet = seed();
    for binding in &packet.consumer_bindings {
        assert!(
            binding.state_facets.change_intent_role_word_in_vocabulary(),
        ...
        assert!(binding.state_facets.relation_source_satisfied());
    }
}
    ...
    for binding in &packet.consumer_bindings {
        ...
        if binding.is_narrowed() {
            assert_eq!(
                ...
                ChangeIntentSharedVocabularyState::FacetsDisclosedNarrowed
            );
            let note = binding
                .narrow_note
                .as_ref()
            ...
            assert_eq!(
                binding.vocabulary_state,
                ChangeIntentSharedVocabularyState::FacetsPreserved
            );
            assert!(binding.narrow_note.is_none());
        }
        if matches!(
            binding.representation,
            ChangeIntentSharedRepresentation::RemoteProjected
        ) {
            assert!(!binding.remote_source_note.trim().is_empty());
        }
        if matches!(
            binding.representation,
            ChangeIntentSharedRepresentation::ExportedRedacted
        ) {
...
fn disclosure_resolver_matches_representation() {
    assert!(
        !resolve_change_intent_shared_render_disclosure(
            ChangeIntentSharedRepresentation::DesktopFull
        )
        .needs_narrow_note
    );
    let compact = resolve_change_intent_shared_render_disclosure(
        ChangeIntentSharedRepresentation::CompactNarrowed,
    );
    assert_eq!(
        compact.narrow_reason,
        Some(ChangeIntentSharedNarrowReason::CompactionNarrowed)
    );
    assert!(compact.needs_narrow_note);
    assert!(!compact.needs_remote_source_note);
    let remote = resolve_change_intent_shared_render_disclosure(
        ChangeIntentSharedRepresentation::RemoteProjected,
    );
    assert!(remote.needs_remote_source_note);
    let exported = resolve_change_intent_shared_render_disclosure(
        ChangeIntentSharedRepresentation::ExportedRedacted,
    );
...
fn vocabulary_drift_is_rejected() {
    ...
    let target = packet
        .consumer_bindings
        .iter()
        .position(|b| b.binding_id == "cisc-startwork-work-item")
        .unwrap();
    packet.consumer_bindings[target]
        .state_facets
        .change_intent_role_word = "final_resolution_authority_disclosure".to_owned();
    assert!(violations_of(&packet).contains(&"change_intent_vocabulary_drift_across_surfaces"));
}

#[test]
fn change_intent_role_word_outside_vocabulary_is_rejected() {
    let mut packet = seed();
    packet.consumer_bindings[0]
        .state_facets
        .change_intent_role_word = "totally_made_up".to_owned();
    let tokens = violations_of(&packet);
    assert!(tokens.contains(&"change_intent_role_word_outside_vocabulary"));
    // Rewording one surface also trips drift, which is expected and fine.
}

#[test]
fn dropped_pack_freshness_on_gate_role_is_rejected() {
    let mut packet = seed();
    // cisc-linked-panel carries the linked_engineering_identity_disclosure gate role, which must always keep
    // a real relation source and never collapse to a masquerade sentinel.
    let target = packet
        .consumer_bindings
        .iter()
        .position(|b| b.binding_id == "cisc-linked-panel")
        .unwrap();
    packet.consumer_bindings[target]
        .state_facets
        .relation_source_word = "local_draft_shown_as_provider_committed".to_owned();
    assert!(violations_of(&packet).contains(&"relation_source_missing_for_gate_role"));
}

#[test]
fn reuse_below_two_consumers_is_rejected() {
    let mut packet = seed();
    // Drop every blocked-escalate-card binding except one, leaving the object with one consumer.
    let mut kept_one = false;
    packet.consumer_bindings.retain(|b| {
        if b.object == M5ChangeIntentObject::BlockedEscalateCard {
            if kept_one {
                return false;
            }
            kept_one = true;
...
fn missing_canonical_reference_on_export_is_rejected() {
    ...
    packet.consumer_bindings[target].source_contract_refs =
        vec![M5_CHANGE_INTENT_MATRIX_DOC_REF.to_owned()];
...
fn unexpected_narrow_note_on_full_binding_is_rejected() {
    ...
    let target = packet
        ...
        .position(|b| !b.is_narrowed())
        .unwrap();
    packet.consumer_bindings[target].narrow_note = Some(ChangeIntentSharedNarrowNote {
        reason: ChangeIntentSharedNarrowReason::CompactionNarrowed,
        preserved_vocabulary_note: "x".to_owned(),
        next_action: ChangeIntentSharedNarrowNextAction::ExpandInDesktop,
...
fn each_guardrail_is_enforced() {
    let cases: [crate::GuardrailCase<ChangeIntentSharedConsumerBinding>; 5] = [
        ...
        (
            |b| {
                b.silently_creates_a_branch_worktree_review_draft_or_provider_link_without_disclosure =
                    true
            },
            "silently_creates_a_branch_worktree_review_draft_or_provider_link_without_disclosure",
        ),
        (
            |b| b.flattens_linked_by_provider_linked_locally_suggested_and_stale_or_broken_into_one_relation_badge = true,
            "flattens_linked_by_provider_linked_locally_suggested_and_stale_or_broken_into_one_relation_badge",
        ),
        (
            |b| b.auto_resolves_tracked_work_while_engineering_blockers_remain_unresolved = true,
            "auto_resolves_tracked_work_while_engineering_blockers_remain_unresolved",
        ),
        (
            |b| {
                b.drops_local_notes_handoff_packets_or_linked_evidence_when_provider_write_fails = true
            },
...
fn object_coverage_gap_is_rejected() {
    ...
    packet
        .consumer_bindings
        .retain(|b| b.object != M5ChangeIntentObject::BlockedEscalateCard);
...
fn missing_source_contract_is_rejected() {
    ...
    packet
        ...
        .retain(|r| r != M5_CHANGE_INTENT_SHARED_CONSUMERS_DOC_REF);
...
fn csv_has_a_row_per_binding() {
    ...
    assert!(lines[0]
        .starts_with("object,consumer,representation,change_intent_role_word,vocabulary_state"));
}
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_change_intent_shared_consumers_export()
        .expect("checked M5 change-intent shared-consumer export validates");
    assert_eq!(
        from_disk.packet_id,
        M5_CHANGE_INTENT_SHARED_CONSUMERS_PACKET_ID
    );
...
fn narrowed_fixture_variants_validate_and_keep_coverage() {
    let compact = seeded_m5_change_intent_shared_consumers_compact_remote_narrowed();
    assert!(
    ...
    let exported = seeded_m5_change_intent_shared_consumers_exported_redaction_narrowed();
    assert!(
...
fn checked_narrowed_fixtures_validate_and_match_builders() {
    let compact: M5ChangeIntentSharedConsumersPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-change-intent-shared-consumers/compact_remote_narrowed.json"
    )))
    ...
    let exported: M5ChangeIntentSharedConsumersPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-change-intent-shared-consumers/exported_redaction_narrowed.json"
    )))
