//! Tests for the M05-1131 core-action-input component surface certification capstone.

use super::*;

fn packet() -> CoreControlProfileCertificationPacket {
    seeded_m5_core_action_input_component_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, CORE_ACTION_INPUT_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, CORE_ACTION_INPUT_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5CoreControlCertifiedProfile::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            CoreControlCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, CoreControlAxisCertificationState::Certified);
...
#[test]
fn only_the_live_profile_certifies_a_trusted_control_claim() {
    for row in &packet().rows {
...
fn a_narrowed_profile_never_keeps_a_trusted_claim() {
    for row in &packet().rows {
        if row.is_claim_narrowed() {
            assert!(
                !row.certified_claim.asserts_trusted_control(),
...
fn all_eight_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5CoreControlClaim> = p.rows.iter().map(|r| r.certified_claim).collect();
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5CoreControlCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5CoreControlCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = CoreControlCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), CoreControlCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in CoreControlCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == CoreControlCertificationAxis::CliExport
        );
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5CoreControlCertifiedProfile::LiveTrustedControlSurface)
        .expect("live-trusted-control-surface row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == CoreControlCertificationAxis::Visual {
            outcome.state = CoreControlAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("control state silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, CoreControlProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, CoreControlCertificationViolation::ProfileBlocked { .. })));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5CoreControlCertifiedProfile::ReviewableControlStructure)
        .expect("reviewable-control-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails.uses_placeholder_as_label = true;
    assert_eq!(row.derive_status(), CoreControlProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        CoreControlCertificationViolation::GuardrailViolated { .. }
    )));
}

#[test]
fn non_live_profile_claiming_trusted_control_blocks() {
    let mut p = packet();
    let idx = p
        .rows
        .iter()
        ...
        .expect("reviewable-control-structure row exists");
    let row = &mut p.rows[idx];
    // Claim a trusted control on a reviewable profile — the claimed ceiling is raised too so the
    // block is the non-live rule, not a certified-exceeds-claim strengthening.
    row.claimed_claim = M5CoreControlClaim::TrustedControl;
    row.certified_claim = M5CoreControlClaim::TrustedControl;
    assert_eq!(row.derive_status(), CoreControlProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == CoreControlCertificationAxis::DegradedState {
            *outcome = CoreControlAxisOutcome {
                axis: CoreControlCertificationAxis::DegradedState,
                state: CoreControlAxisCertificationState::DisclosedNarrowed,
                parity_note: "the command binding is lagging".to_owned(),
                narrowing_reason: Some(
                    "the command binding is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5CoreControlDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableControl == certified, no claim_auto_narrow.
    assert_eq!(row.derive_status(), CoreControlProfileClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == CoreControlCertificationAxis::CliExport {
            outcome.state = CoreControlAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5CoreControlDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), CoreControlProfileClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(CoreControlClaimAutoNarrow {
        binding_axis: CoreControlCertificationAxis::DegradedState,
        from_claim: M5CoreControlClaim::TrustedControl,
        to_claim: M5CoreControlClaim::TrustedControl,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5CoreControlClaim::CommandBindingUnverifiedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-control-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableControl; certify a stronger trusted control.
    row.certified_claim = M5CoreControlClaim::TrustedControl;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5CoreControlCertifiedProfile::UnboundCommandSurface)
        .expect("unbound-command-surface row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = CoreControlCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), CoreControlProfileClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == CoreControlCertificationAxis::CliExport {
            outcome.state = CoreControlAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the unbound command".to_owned());
            outcome.downgrade_trigger = Some(M5CoreControlDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = CoreControlCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), CoreControlProfileClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == CoreControlProfileClaimStatus::Yellow)
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(CoreControlCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5CoreControlDowngradeTrigger::ProofStale,
    );
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.profile != M5CoreControlCertifiedProfile::PartialRetentionSearchField);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        CoreControlCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the button family from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5CoreControlFamily::Button);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        CoreControlCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = CoreControlProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        CoreControlCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, CoreControlCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, CoreControlCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != CoreControlCertificationAxis::ControlComponentTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        CoreControlCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, CoreControlCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        CoreControlCertificationViolation::RawControlMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: CoreControlProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_core_action_input_component_surface_certification_export()
...
fn generate_artifacts() {
    if std::env::var("GEN_CORE_ACTION_INPUT_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_core_action_input_component_surface_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-core-action-input-component-surface-certification-proof");
    ...
    let fixtures = Path::new(manifest)
        .join("../../fixtures/ui/m5-core-action-input-component-surface-certification");
    ...
    fs::write(
        ...
        "# M5 core-action-input component surface certification fixtures\n\n\
         Mirror of `artifacts/release/m5-core-action-input-component-surface-certification-proof/`.\n\
         Regenerate with `GEN_CORE_ACTION_INPUT_CERT_ARTIFACTS=1 cargo test -p aureline-ui \
         m5_core_action_input_component_surface_certification::tests::generate_artifacts`.\n",
    )
