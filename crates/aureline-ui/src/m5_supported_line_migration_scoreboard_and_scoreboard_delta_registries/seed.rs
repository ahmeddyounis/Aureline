//! Canonical seed builders for the M5 migration-scoreboard and scoreboard-delta registries packet.
//!
//! These builders are the single producer of the checked-in support export and the narrowed fixtures.
//! The headless emitter and the inline tests both call them so the in-code registries, the artifact, and the
//! fixtures never drift. Every resolved example is built by calling the real resolvers so the packet can only
//! carry projections the resolvers actually produce. Clean migration-scoreboard and scoreboard-delta entries
//! are built so the one typed migration-scoreboard object resolving per line, lines never widening a replacement-grade
//! claim without preserving their rollback posture, switching language never running ahead of current migration truth,
//! the canonical / accessible / audit resolution forms, and the complete line-identity / affected-scoreboard-section /
//! previous-versus-current-scoreboard / delta-scope / active-reason scoreboard-delta object are proven across the
//! shiproom, release-center, executive-steering, program-governance, diagnostics, and support surfaces without any
//! hand-copied per-line assumption, widen-without-posture, incomplete object, hidden delta, or resolution-form gap.

use super::*;

/// Stable packet id for the canonical registries packet.
pub const M5_SUPPORTED_LINE_MIGRATION_SCOREBOARD_SCOREBOARD_DELTA_REGISTRIES_PACKET_ID: &str =
    "m5-supported-line-migration-scoreboard-and-scoreboard-delta-registries:stable:0001";
...
fn migration_scoreboard(
    input: M5MigrationScoreboardEntryResolutionInput,
) -> M5ResolvedMigrationScoreboardEntry {
    resolve_migration_scoreboard_entry(input)
        .expect("seed line-migration_scoreboard entry resolves")
}

fn downgrade(input: M5ScoreboardDeltaEntryResolutionInput) -> M5ResolvedScoreboardDeltaEntry {
    resolve_scoreboard_delta_entry(input).expect("seed line-downgrade-packet entry resolves")
}

fn all_forms() -> Vec<M5SupportedLineMigrationScoreboardResolutionForm> {
    M5SupportedLineMigrationScoreboardResolutionForm::ALL.to_vec()
}

// -- Clean line-migration_scoreboard entries (typed object, rollback / diagnostics preserved, bound to the registry) --

#[allow(clippy::too_many_arguments)]
fn clean_migration_scoreboard_base(
    entry_id: &str,
    line_binding_id: &str,
    token_name: &str,
    semantic_role: M5SupportedLineTransparencyRole,
    report_section: M5MigrationScoreboardKind,
    surface_context: M5SupportedLineMigrationScoreboardSurfaceContext,
...
) -> M5MigrationScoreboardEntryResolutionInput {
    M5MigrationScoreboardEntryResolutionInput {
...
fn migration_scoreboard_imported_outcome_class_clean() -> M5ResolvedMigrationScoreboardEntry {
    migration_scoreboard(clean_migration_scoreboard_base(
        "migration_scoreboard:shiproom:dogfood-core-team-canary",
        "launch.line.core-team-canary",
        "line.migration_scoreboard.core_team_canary",
        M5SupportedLineTransparencyRole::FreshnessWindow,
        M5MigrationScoreboardKind::ImportedOutcomeClass,
        M5SupportedLineMigrationScoreboardSurfaceContext::ShiproomSurface,
...
fn migration_scoreboard_translated_outcome_class_clean() -> M5ResolvedMigrationScoreboardEntry {
    migration_scoreboard(clean_migration_scoreboard_base(
        "migration_scoreboard:release-center:migration-alpha",
        "launch.line.migration-alpha",
        "line.migration_scoreboard.translated_outcome_class",
        M5SupportedLineTransparencyRole::TransparencyDisclosure,
        M5MigrationScoreboardKind::TranslatedOutcomeClass,
        M5SupportedLineMigrationScoreboardSurfaceContext::ReleaseCenterSurface,
...
fn migration_scoreboard_partial_outcome_class_clean() -> M5ResolvedMigrationScoreboardEntry {
    migration_scoreboard(clean_migration_scoreboard_base(
        "migration_scoreboard:program-governance:extension-author",
        "launch.line.extension-author",
        "line.migration_scoreboard.partial_outcome_class",
        M5SupportedLineTransparencyRole::MigrationScoreboardCurrency,
        M5MigrationScoreboardKind::PartialOutcomeClass,
        M5SupportedLineMigrationScoreboardSurfaceContext::ProgramGovernanceSurface,
...
fn migration_scoreboard_shimmed_outcome_class_clean() -> M5ResolvedMigrationScoreboardEntry {
    // A design-partner preview line is public-facing and keeps its support language matched to line proof.
    let mut base = clean_migration_scoreboard_base(
        "migration_scoreboard:executive-steering:design-partner-preview",
        "launch.line.design-partner-preview",
        "line.migration_scoreboard.shimmed_outcome_class",
        M5SupportedLineTransparencyRole::FreshnessWindow,
        M5MigrationScoreboardKind::ShimmedOutcomeClass,
        M5SupportedLineMigrationScoreboardSurfaceContext::ExecutiveSteeringSurface,
    ...
    migration_scoreboard(base)
}

fn migration_scoreboard_unsupported_outcome_class_clean() -> M5ResolvedMigrationScoreboardEntry {
    // A public preview line is public-facing and keeps its support language matched to line proof.
    let mut base = clean_migration_scoreboard_base(
        "migration_scoreboard:support:public-preview",
        "launch.line.public-preview",
        "line.migration_scoreboard.unsupported_outcome_class",
        M5SupportedLineTransparencyRole::TransparencyDisclosure,
        M5MigrationScoreboardKind::UnsupportedOutcomeClass,
        M5SupportedLineMigrationScoreboardSurfaceContext::SupportOrExportForm,
...
fn migration_scoreboard_rollback_cleanliness_outcome_class_clean(
) -> M5ResolvedMigrationScoreboardEntry {
    migration_scoreboard(clean_migration_scoreboard_base(
        "migration_scoreboard:release-center:certified-journey",
        "launch.line.certified-journey",
        "line.migration_scoreboard.rollback_cleanliness_outcome_class",
        M5SupportedLineTransparencyRole::OrrHistoryRetention,
        M5MigrationScoreboardKind::RollbackCleanlinessOutcomeClass,
        M5SupportedLineMigrationScoreboardSurfaceContext::ReleaseCenterSurface,
...
// -- Degraded line-migration_scoreboard entries ---------------------------------------------------------

/// Degraded migration_scoreboard entry: the resolved migration_scoreboard object is incomplete — the bundle IDs are unstated.
fn migration_scoreboard_object_incomplete() -> M5ResolvedMigrationScoreboardEntry {
    let mut base = clean_migration_scoreboard_base(
        "migration_scoreboard:shiproom:incomplete",
        "launch.line.core-team-canary",
        "line.migration_scoreboard.core_team_canary",
        M5SupportedLineTransparencyRole::FreshnessWindow,
        M5MigrationScoreboardKind::ImportedOutcomeClass,
...
/// Degraded migration_scoreboard entry: the line's rollback and diagnostics posture is not preserved before widening —
...
fn migration_scoreboard_widen_fold() -> M5ResolvedMigrationScoreboardEntry {
    let mut base = clean_migration_scoreboard_base(
        "migration_scoreboard:release-center:widen-fold",
        "launch.line.migration-alpha",
        "line.migration_scoreboard.translated_outcome_class",
        M5SupportedLineTransparencyRole::TransparencyDisclosure,
        M5MigrationScoreboardKind::TranslatedOutcomeClass,
...
/// Degraded migration_scoreboard entry: the behavior is a hand-copied per-entry assumption instead of tracing to the
/// registry.
fn migration_scoreboard_unbound() -> M5ResolvedMigrationScoreboardEntry {
    let mut base = clean_migration_scoreboard_base(
        "migration_scoreboard:executive-steering:unbound",
        "launch.line.certified-journey",
        "line.migration_scoreboard.rollback_cleanliness_outcome_class",
        M5SupportedLineTransparencyRole::OrrHistoryRetention,
        M5MigrationScoreboardKind::RollbackCleanlinessOutcomeClass,
...
/// Degraded migration_scoreboard entry: the canonical registry token name is unstated.
fn migration_scoreboard_token_unstated() -> M5ResolvedMigrationScoreboardEntry {
    let mut base = clean_migration_scoreboard_base(
        "migration_scoreboard:program-governance:token-unstated",
        "launch.line.extension-author",
        "  ",
        M5SupportedLineTransparencyRole::MigrationScoreboardCurrency,
        M5MigrationScoreboardKind::PartialOutcomeClass,
...
fn clean_downgrade_base(
    ...
    token_name: &str,
    semantic_role: M5SupportedLineTransparencyRole,
    comparison_scope: M5ScoreboardDeltaScope,
    surface_context: M5SupportedLineMigrationScoreboardSurfaceContext,
...
) -> M5ScoreboardDeltaEntryResolutionInput {
    M5ScoreboardDeltaEntryResolutionInput {
        entry_id: entry_id.to_owned(),
        ...
        keeps_scoreboard_delta_visible: true,
...
fn downgrade_dogfood_ring_clean() -> M5ResolvedScoreboardDeltaEntry {
    ...
    let mut base = clean_downgrade_base(
        ...
        M5SupportedLineTransparencyRole::FreshnessWindow,
        M5ScoreboardDeltaScope::FieldPainCluster,
        M5SupportedLineMigrationScoreboardSurfaceContext::ShiproomSurface,
...
fn downgrade_rehearsal_currency_clean() -> M5ResolvedScoreboardDeltaEntry {
    downgrade(clean_downgrade_base(
        "downgrade:program-governance:rehearsal-currency",
        "launch.line.extension-author",
        "line.downgrade.partial_outcome_class",
        M5SupportedLineTransparencyRole::MigrationScoreboardCurrency,
        M5ScoreboardDeltaScope::UnsupportedCategoryGrowth,
        M5SupportedLineMigrationScoreboardSurfaceContext::ProgramGovernanceSurface,
...
fn downgrade_go_no_go_signoff_clean() -> M5ResolvedScoreboardDeltaEntry {
    downgrade(clean_downgrade_base(
        "downgrade:release-center:go-no-go-signoff",
        "launch.line.certified-journey",
        "line.downgrade.rollback_cleanliness_outcome_class",
        M5SupportedLineTransparencyRole::OrrHistoryRetention,
        M5ScoreboardDeltaScope::DocsHelpOrRollbackGap,
        M5SupportedLineMigrationScoreboardSurfaceContext::ReleaseCenterSurface,
...
fn downgrade_support_ahead() -> M5ResolvedScoreboardDeltaEntry {
    let mut base = clean_downgrade_base(
        "downgrade:shiproom:support-ahead",
        "launch.line.public-preview",
        "line.downgrade.unsupported_outcome_class",
        M5SupportedLineTransparencyRole::TransparencyDisclosure,
        M5ScoreboardDeltaScope::FieldPainCluster,
        M5SupportedLineMigrationScoreboardSurfaceContext::ShiproomSurface,
...
/// incomplete.
fn downgrade_form_incomplete() -> M5ResolvedScoreboardDeltaEntry {
    let mut base = clean_downgrade_base(
    ...
    base.resolution_form_coverage =
        vec![M5SupportedLineMigrationScoreboardResolutionForm::CanonicalObject];
    downgrade(base)
}

/// Degraded downgrade entry: the downgrade scope is unclassified.
fn comparison_delta_scope_unclassified() -> M5ResolvedScoreboardDeltaEntry {
    downgrade(clean_downgrade_base(
        "downgrade:executive-steering:scope-unclassified",
        "launch.line.design-partner-preview",
        "line.downgrade.shimmed_outcome_class",
        M5SupportedLineTransparencyRole::FreshnessWindow,
        M5ScoreboardDeltaScope::DeltaScopeUnclassified,
        M5SupportedLineMigrationScoreboardSurfaceContext::ExecutiveSteeringSurface,
...
#[allow(clippy::too_many_arguments)]
fn base_row(
    consumer_surface: M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesConsumerSurface,
    owner_role: &str,
    scope_summary: &str,
    proof_ref: &str,
    downgrade_triggers: Vec<M5SupportedLineTransparencyDowngradeTrigger>,
    migration_scoreboard_entries: Vec<M5ResolvedMigrationScoreboardEntry>,
    scoreboard_delta_entries: Vec<M5ResolvedScoreboardDeltaEntry>,
) -> M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesRow {
    M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesRow {
        consumer_surface,
        ...
        anatomy_parts: M5SupportedLineMigrationScoreboardAnatomyPart::ALL.to_vec(),
        export_fields: M5SupportedLineMigrationScoreboardExportField::ALL.to_vec(),
        downgrade_triggers,
        ...
        collapses_distinct_scoreboard_delta_classes_into_one_lane: false,
    }
}

fn registry_rows() -> Vec<M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesRow> {
    ...
    vec![
        base_row(
            ...
            vec![
                migration_scoreboard_imported_outcome_class_clean(),
                migration_scoreboard_object_incomplete(),
            ],
        ...
        base_row(
            ...
            vec![migration_scoreboard_translated_outcome_class_clean(), migration_scoreboard_widen_fold()],
        ...
        base_row(
            ...
            vec![
                migration_scoreboard_shimmed_outcome_class_clean(),
                migration_scoreboard_unbound(),
            ],
            vec![comparison_delta_scope_unclassified()],
        ),
        base_row(
            C::ProgramGovernance,
            "Program-governance owner",
            "Program governance resolves the partial outcome class and the unsupported-category-growth delta bound to the registry; an unstated registry token on a migration-scoreboard entry is caught before it can drift",
            ...
            vec![
                migration_scoreboard_partial_outcome_class_clean(),
                migration_scoreboard_token_unstated(),
            ],
        ...
        base_row(
            ...
            vec![migration_scoreboard_unsupported_outcome_class_clean()],
            vec![downgrade_rehearsal_currency_clean()],
        ),
    ]
}

fn governance_review() -> M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesGovernanceReview
{
    M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesGovernanceReview {
        migration_scoreboard_registry_names_token_role_and_journey: true,
        line_resolves_to_typed_migration_scoreboard_from_shared_registry: true,
        repo_bundle_toolchain_and_deployment_rows_published: true,
        lines_cannot_widen_without_rollback_and_diagnostics: true,
        scoreboard_delta_keeps_proof_visible_and_binds_support_language: true,
        ...
        migration_scoreboard_or_downgrade_drift_caught_before_release: true,
...
fn proof_freshness() -> M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesProofFreshness {
    M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesProofFreshness {
...
fn release_posture() -> M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesReleasePosture {
    M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesReleasePosture {
        proof_packet_ref:
            M5_SUPPORTED_LINE_MIGRATION_SCOREBOARD_SCOREBOARD_DELTA_REGISTRIES_ARTIFACT_REF
                .to_owned(),
        line_audit_ref:
            M5_SUPPORTED_LINE_MIGRATION_SCOREBOARD_SCOREBOARD_DELTA_REGISTRIES_REPORT_REF.to_owned(),
...
/// Narrowed variant: the shiproom row is held at Beta pending migration-scoreboard parity on every outcome class;
/// every row stays visible and every example stays honest.
pub fn seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries_migration_scoreboard_beta_narrowed(
) -> M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesPacket {
    let mut packet =
        seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries();
    packet.packet_id =
        "m5-supported-line-migration-scoreboard-and-scoreboard-delta-registries:migration-scoreboard-beta:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
...
/// Narrowed variant: the release-center row is narrowed to Preview pending scoreboard-delta parity on every
/// delta scope; every row stays visible and every example stays honest.
pub fn seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries_scoreboard_delta_preview_narrowed(
) -> M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesPacket {
    let mut packet =
        seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries();
    packet.packet_id =
        "m5-supported-line-migration-scoreboard-and-scoreboard-delta-registries:scoreboard-delta-preview:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
