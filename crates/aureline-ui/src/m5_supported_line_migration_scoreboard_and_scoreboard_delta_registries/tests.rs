...
fn clean_migration_scoreboard_input() -> M5MigrationScoreboardEntryResolutionInput {
    M5MigrationScoreboardEntryResolutionInput {
        entry_id: "migration_scoreboard:test".to_owned(),
        line_binding_id: "launch.line.core-team-canary".to_owned(),
        token_name: "line.migration_scoreboard.core_team_canary".to_owned(),
        semantic_role: M5SupportedLineTransparencyRole::FreshnessWindow,
        report_section: M5MigrationScoreboardKind::ImportedOutcomeClass,
        surface_context: M5SupportedLineMigrationScoreboardSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SupportedLineMigrationScoreboardResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5ScoreboardDeltaEntryResolutionInput {
    M5ScoreboardDeltaEntryResolutionInput {
        ...
        semantic_role: M5SupportedLineTransparencyRole::FreshnessWindow,
        comparison_scope: M5ScoreboardDeltaScope::FieldPainCluster,
        surface_context: M5SupportedLineMigrationScoreboardSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SupportedLineMigrationScoreboardResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries();
...
#[test]
fn migration_scoreboard_clean_names_meaning_and_is_bound() {
    let resolved = resolve_migration_scoreboard_entry(clean_migration_scoreboard_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.migration_scoreboard_resolves_across_lines);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.migration_scoreboard_object_complete);
    ...
    assert_eq!(resolved.report_section, "imported_outcome_class");
    assert_eq!(
        resolved.canonical_report_section_mode,
        "imported_outcome_class_mode"
    );
    assert_eq!(resolved.surface_context, "shiproom_surface");
    assert_eq!(
        resolved.next_action,
        M5SupportedLineMigrationScoreboardNextAction::ExpandCohortMeaning
    );
}

#[test]
fn migration_scoreboard_token_unstated_degrades() {
    let mut input = clean_migration_scoreboard_input();
...
#[test]
fn migration_scoreboard_unbound_and_unclassified_degrade() {
    let mut input = clean_migration_scoreboard_input();
    ...
    let mut input = clean_migration_scoreboard_input();
    input.report_section = M5MigrationScoreboardKind::OutcomeClassUnclassified;
    assert_eq!(
        resolve_migration_scoreboard_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn migration_scoreboard_object_incomplete_and_widen_fold_and_form_degrade() {
    // An unstated bundle IDs field leaves the resolved object incomplete.
    let mut input = clean_migration_scoreboard_input();
    input.bundle_ids = "  ".to_owned();
    let resolved = resolve_migration_scoreboard_entry(input).unwrap();
    assert!(!resolved.migration_scoreboard_object_complete);
    assert_eq!(
    ...
    assert_eq!(
        resolve_migration_scoreboard_entry(input).unwrap().degrade_reason,
...
#[test]
fn migration_scoreboard_public_facing_and_surface_and_proof_degrade() {
    // A public-facing line running support language ahead of proof first fails the widen-boundary fold.
    let mut input = clean_migration_scoreboard_input();
    input.report_section = M5MigrationScoreboardKind::UnsupportedOutcomeClass;
    ...
    let mut input = clean_migration_scoreboard_input();
    input.surface_context = M5SupportedLineMigrationScoreboardSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_migration_scoreboard_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn migration_scoreboard_empty_id_and_forbidden_material_error() {
    let mut input = clean_migration_scoreboard_input();
    input.entry_id = "".to_owned();
    assert_eq!(
        resolve_migration_scoreboard_entry(input).unwrap_err(),
    ...
    assert_eq!(
        resolve_migration_scoreboard_entry(input).unwrap_err(),
        M5SupportedLineMigrationScoreboardResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5MigrationScoreboardKind::OutcomeClassUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn migration_scoreboard_object_is_complete_requires_all_fields() {
    assert!(migration_scoreboard_object_is_complete(
        M5MigrationScoreboardKind::ImportedOutcomeClass,
...
fn downgrade_clean_stays_honest() {
    let resolved = resolve_scoreboard_delta_entry(clean_downgrade_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.comparison_safe_on_every_line);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.provides_complete_scoreboard_delta);
    assert!(resolved.scoreboard_delta_stays_honest);
    assert_eq!(resolved.comparison_scope, "field_pain_cluster");
    assert_eq!(resolved.surface_context, "shiproom_surface");
}
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let resolved = resolve_scoreboard_delta_entry(input).unwrap();
    assert!(!resolved.provides_complete_scoreboard_delta);
    assert_eq!(
        resolved.degrade_reason,
    ...
    let mut input = clean_downgrade_input();
    input.keeps_scoreboard_delta_visible = false;
    assert_eq!(
        resolve_scoreboard_delta_entry(input).unwrap().degrade_reason,
        Some(M5ScoreboardDeltaEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    );
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5ScoreboardDeltaScope::DeltaScopeUnclassified;
    assert_eq!(
        resolve_scoreboard_delta_entry(input)
            .unwrap()
            .degrade_reason,
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    input.entry_id = "  ".to_owned();
    assert_eq!(
        resolve_scoreboard_delta_entry(input).unwrap_err(),
...
fn downgrade_bound_support_and_flagged_gap_stay_clean() {
    ...
    assert!(resolve_scoreboard_delta_entry(input).unwrap().is_clean());
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_MIGRATION_SCOREBOARD_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_SCOREBOARD_DELTA_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5SupportedLineMigrationScoreboardAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5SupportedLineMigrationScoreboardExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    let mut packet =
        seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries();
    packet.registry_rows[0].scoreboard_delta_entries.clear();
    assert!(packet.validate().contains(
...
fn dishonest_clean_example_fails() {
    let mut packet =
        seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries();
    // Force a clean migration_scoreboard entry to also read as object-incomplete — the packet must reject it.
    let row = &mut packet.registry_rows[0];
    row.migration_scoreboard_entries[0].degrade_reason = None;
    row.migration_scoreboard_entries[0].migration_scoreboard_object_complete = false;
    assert!(packet.validate().contains(
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        match mutate {
            ...
            _ => row.collapses_distinct_scoreboard_delta_classes_into_one_lane = true,
        }
        assert!(packet.validate().contains(
...
#[test]
fn migration_scoreboard_not_proven_when_incomplete_example_removed() {
    let mut packet =
        seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries();
    for row in &mut packet.registry_rows {
        row.migration_scoreboard_entries.retain(|ex| {
            ex.degrade_reason
                != Some(M5MigrationScoreboardEntryDegradeReason::CohortDescriptorObjectIncomplete)
        });
    }
...
#[test]
fn migration_scoreboard_not_proven_when_surface_collapses() {
    let mut packet =
        seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries();
    // Drop every clean executive-steering-surface migration_scoreboard so the first-consumer surfaces no longer include it.
    for row in &mut packet.registry_rows {
        row.migration_scoreboard_entries
...
#[test]
fn scoreboard_delta_integrity_not_proven_when_support_ahead_example_removed() {
    let mut packet =
        seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries();
    for row in &mut packet.registry_rows {
        row.scoreboard_delta_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
...
#[test]
fn scoreboard_delta_integrity_not_proven_when_scope_dropped() {
    let mut packet =
        seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries();
    // Drop every clean go-no-go-signoff downgrade so the coverage no longer includes it.
    for row in &mut packet.registry_rows {
        row.scoreboard_delta_entries
            .retain(|ex| !(ex.is_clean() && ex.comparison_scope == "docs_help_or_rollback_gap"));
    }
    assert!(packet.validate().contains(
        &M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesViolation::CohortEvidenceIntegrityNotProven
    ));
...
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries()
        .export_safe_json();
...
#[test]
fn migration_scoreboard_table_lists_only_clean_migration_scoreboards() {
    let packet = seeded_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries();
    let table = packet.render_migration_scoreboard_table();
    // The clean canary and migration migration_scoreboards are rendered from the registry.
    assert!(table.contains("imported_outcome_class_mode"));
    assert!(table.contains("translated_outcome_class_mode"));
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk =
        current_stable_m5_supported_line_migration_scoreboard_and_scoreboard_delta_registries_export()
            .expect(
            "checked M5 line-migration_scoreboard / line-downgrade-packet registries export validates",
        );
    assert_eq!(
        from_disk.packet_id,
        M5_SUPPORTED_LINE_MIGRATION_SCOREBOARD_SCOREBOARD_DELTA_REGISTRIES_PACKET_ID
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-supported-line-migration-scoreboard-and-scoreboard-delta-registries/migration_scoreboard_beta_narrowed.json"
    )))
    .expect("line-migration_scoreboard fixture parses");
    ...
    let preview: M5SupportedLineMigrationScoreboardScoreboardDeltaRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-supported-line-migration-scoreboard-and-scoreboard-delta-registries/scoreboard_delta_preview_narrowed.json"
    )))
