//! Implemented M5 landing-candidate sheet and authorization registries.
//!
//! The frozen [change-object / patch-stack / landing matrix][matrix] names Aureline's governed change-orchestration
//! object classes and locks their controlled vocabulary. This module makes the matrix's `landing_candidate_sheet`
//! object class operable — as a reviewed landing candidate and the authorization that advances it — so the Git,
//! patch-stack / queue, review, provider-landing, help / docs, and support / export surfaces inherit one canonical
//! landing-candidate descriptor rather than a hand-authored parallel prose that has to be kept consistent: it turns
//! the *landing-candidate sheet* grammar (one durable reviewed candidate per proposed land — one typed field per
//! section: the exact target branch, the merge strategy — a local squash plan, a local rebase plan, a merge-queue
//! enqueue, or a review-ready export bundle — the required checks, the approval state, and the queue eligibility,
//! keeping the provider-authoritative-versus-local-estimate distinction with queue-position ambiguity, stale-base
//! invalidation, and protected-branch blocking visible) and the *authorization* grammar (the explicit advance paths
//! kept separate so a protected branch stays blocked for background agents and broad automation unless the user
//! explicitly advances the reviewed candidate through the correct command path) into registry resolvers that produce
//! export-safe, honest projections. Every proposed land then resolves to one typed landing candidate — the target
//! branch and merge strategy it discloses, the required checks, the approval state, and the queue eligibility, all
//! attributable to the sheet so Aureline only lands from an explicit reviewed landing candidate — and to one
//! authorization object — the resolved landing path, the authority source (provider-authoritative or local estimate),
//! the advancing actor, the protected-branch posture, and whether a reviewed candidate is bound — that the Git,
//! patch-stack, provider-landing, and support surfaces can inspect without manual reconstruction, so a candidate's
//! target, strategy, and queue eligibility stay visible metadata, no member is silently reordered, and nothing lands
//! from ambient branch state; a landing candidate or authorization that cannot bind a field to its reviewed target
//! degrades honestly instead of letting a local estimate read as a provider-authoritative landing.
//!
...
//!   unless it names a canonical registry token, a classified [scope selector][M5LandingCandidateRecordKind],
//!   a disclosure role, covers every [resolution form][M5LandingCandidateRecordResolutionForm] (the
...
//!   [evaluator binding][M5LandingCandidateResultScope]
...
/// Stable record-kind tag carried by [`M5LandingCandidateRecordAndResultRegistriesPacket`].
pub const M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_landing_candidate_sheet_and_authorization_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_DOC_REF: &str =
    "docs/team-workflows/m5_landing_candidate_sheet_and_authorization_registries.md";

/// Repo-relative path of the canonical landing-authorization domain schema minted by this lane (the
/// authorization that advances one reviewed landing candidate: the landing path, the authority source,
/// the advancing actor, the protected-branch posture, and whether a reviewed candidate is bound — so a
/// protected branch stays blocked for background agents and broad automation unless the user explicitly
/// advances the reviewed candidate through the correct command path).
pub const M5_LANDING_AUTHORIZATION_DOMAIN_SCHEMA_REF: &str =
    "schemas/ui/m5-landing-authorization.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-landing-candidate-sheet-and-authorization-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-landing-candidate-sheet-and-authorization-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-landing-candidate-sheet-and-authorization-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/teamwork/m5-landing-candidate-sheet-and-authorization-registries";
...
pub type M5LandingCandidateRecordAndResultRegistriesConsumerSurface =
...
pub enum M5LandingCandidateRecordResolutionForm {
...
impl M5LandingCandidateRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LandingCandidateRecordKind {
...
impl M5LandingCandidateRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LandingCandidateResultScope {
...
impl M5LandingCandidateResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LandingCandidateRecordSurfaceContext {
...
impl M5LandingCandidateRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LandingCandidateRecordAnatomyPart {
...
impl M5LandingCandidateRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LandingCandidateRecordNextAction {
...
impl M5LandingCandidateRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LandingCandidateRecordExportField {
...
impl M5LandingCandidateRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LandingCandidateRecordEntryDegradeReason {
...
impl M5LandingCandidateRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5LandingCandidateRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5LandingCandidateRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5LandingCandidateRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5LandingCandidateRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5LandingCandidateRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LandingCandidateResultEntryDegradeReason {
...
impl M5LandingCandidateResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5LandingCandidateRecordNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5LandingCandidateRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5LandingCandidateRecordEntryResolutionInput {
    ...
    pub report_section: M5LandingCandidateRecordKind,
    /// The render / surface context.
    pub surface_context: M5LandingCandidateRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5LandingCandidateRecordResolutionForm>,
...
pub struct M5ResolvedLandingCandidateRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5LandingCandidateRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5LandingCandidateRecordNextAction,
...
impl M5ResolvedLandingCandidateRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5LandingCandidateResultEntryResolutionInput {
    ...
    pub comparison_scope: M5LandingCandidateResultScope,
    /// The render / surface context.
    pub surface_context: M5LandingCandidateRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5LandingCandidateRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedLandingCandidateResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5LandingCandidateResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5LandingCandidateRecordNextAction,
...
impl M5ResolvedLandingCandidateResultEntry {
...
pub enum M5LandingCandidateRecordResolutionError {
...
impl M5LandingCandidateRecordResolutionError {
...
impl fmt::Display for M5LandingCandidateRecordResolutionError {
...
impl Error for M5LandingCandidateRecordResolutionError {}

fn form_tokens(forms: &[M5LandingCandidateRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5LandingCandidateRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5LandingCandidateRecordResolutionForm> = forms.iter().copied().collect();
    M5LandingCandidateRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5LandingCandidateRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5LandingCandidateResultScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedLandingCandidateRecordEntry, M5LandingCandidateRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5LandingCandidateRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5LandingCandidateRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        ...
        Some(M5LandingCandidateRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    ...
    let next_action = match degrade_reason {
        ...
        None => M5LandingCandidateRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedLandingCandidateRecordEntry {
...
pub fn resolve_review_pack_result_entry(
...
) -> Result<M5ResolvedLandingCandidateResultEntry, M5LandingCandidateRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5LandingCandidateRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5LandingCandidateResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        ...
        Some(M5LandingCandidateResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5LandingCandidateRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedLandingCandidateResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LandingCandidateRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5LandingCandidateRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5LandingCandidateRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5LandingCandidateRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedLandingCandidateRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedLandingCandidateResultEntry>,
...
impl M5LandingCandidateRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5LandingCandidateRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5LandingCandidateRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5LandingCandidateRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5LandingCandidateRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedLandingCandidateRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedLandingCandidateResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LandingCandidateRecordAndResultRegistriesVocabularySet {
...
impl M5LandingCandidateRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ChangeOrchestrationRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5LandingCandidateRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5LandingCandidateRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5LandingCandidateResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5LandingCandidateRecordSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5LandingCandidateRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5LandingCandidateResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5LandingCandidateRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5LandingCandidateRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5LandingCandidateRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LandingCandidateRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LandingCandidateRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LandingCandidateRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LandingCandidateRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5LandingCandidateRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5LandingCandidateRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5LandingCandidateRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5LandingCandidateRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5LandingCandidateRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5LandingCandidateRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5LandingCandidateRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5LandingCandidateRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LandingCandidateRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION`].
...
impl M5LandingCandidateRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5LandingCandidateRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5LandingCandidateRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND {
            violations.push(M5LandingCandidateRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_LANDING_CANDIDATE_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION {
            violations
                .push(M5LandingCandidateRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5LandingCandidateRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations
                .push(M5LandingCandidateRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations
                .push(M5LandingCandidateRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Landing-Candidate-Sheet and Authorization Registries\n\n");
...
#[derive(Debug)]
pub enum M5LandingCandidateRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5LandingCandidateRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5LandingCandidateRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5LandingCandidateRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5LandingCandidateRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5LandingCandidateRecordAndResultRegistriesViolation {
...
impl M5LandingCandidateRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_landing_candidate_sheet_and_authorization_registries_export() -> Result<
    M5LandingCandidateRecordAndResultRegistriesPacket,
    M5LandingCandidateRecordAndResultRegistriesArtifactError,
> {
    let packet: M5LandingCandidateRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-landing-candidate-sheet-and-authorization-registries-proof/support_export.json"
        )
    ))
    .map_err(M5LandingCandidateRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5LandingCandidateRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5LandingCandidateRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5LandingCandidateRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations
                .push(M5LandingCandidateRecordAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5LandingCandidateRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations
                .push(M5LandingCandidateRecordAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5LandingCandidateRecordAndResultRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        ...
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_LANDING_CANDIDATE_SHEET_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_LANDING_AUTHORIZATION_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5LandingCandidateRecordAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5LandingCandidateRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5LandingCandidateRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5LandingCandidateRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5LandingCandidateRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5LandingCandidateRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5LandingCandidateRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5LandingCandidateRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5LandingCandidateResultScope::CANONICAL_SCOPES
        .iter()
