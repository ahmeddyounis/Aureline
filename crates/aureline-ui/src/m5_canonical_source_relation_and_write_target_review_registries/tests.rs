...
fn clean_canonical_source_relation_input() -> M5CanonicalSourceRelationEntryResolutionInput {
    M5CanonicalSourceRelationEntryResolutionInput {
        entry_id: "canonical_source_relation:test".to_owned(),
        line_binding_id: "launch.line.core-team-canary".to_owned(),
        token_name: "line.canonical_source_relation.core_team_canary".to_owned(),
        semantic_role: M5ConstrainedFileStateRole::StateBadgeClassification,
        report_section: M5CanonicalSourceRelationKind::ReadOnlyPathObject,
        surface_context: M5CanonicalSourceRelationSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5CanonicalSourceRelationResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5WriteTargetReviewEntryResolutionInput {
    M5WriteTargetReviewEntryResolutionInput {
        ...
        semantic_role: M5ConstrainedFileStateRole::StateBadgeClassification,
        comparison_scope: M5WriteTargetReviewScope::StateClassChange,
        surface_context: M5CanonicalSourceRelationSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5CanonicalSourceRelationResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_canonical_source_relation_and_write_target_review_registries();
...
#[test]
fn canonical_source_relation_clean_names_meaning_and_is_bound() {
    let resolved =
        resolve_canonical_source_relation_entry(clean_canonical_source_relation_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.canonical_source_relation_resolves_across_lines);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.canonical_source_relation_object_complete);
    ...
    assert_eq!(
        ...
        M5CanonicalSourceRelationNextAction::ExpandCohortMeaning
    );
}

#[test]
fn canonical_source_relation_token_unstated_degrades() {
    let mut input = clean_canonical_source_relation_input();
...
#[test]
fn canonical_source_relation_unbound_and_unclassified_degrade() {
    let mut input = clean_canonical_source_relation_input();
    ...
    let mut input = clean_canonical_source_relation_input();
    input.report_section = M5CanonicalSourceRelationKind::ObjectClassUnclassified;
    assert_eq!(
        resolve_canonical_source_relation_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn canonical_source_relation_object_incomplete_and_widen_fold_and_form_degrade() {
    // An unstated bundle IDs field leaves the resolved object incomplete.
    let mut input = clean_canonical_source_relation_input();
    input.bundle_ids = "  ".to_owned();
    let resolved = resolve_canonical_source_relation_entry(input).unwrap();
    assert!(!resolved.canonical_source_relation_object_complete);
    assert_eq!(
    ...
    assert_eq!(
        resolve_canonical_source_relation_entry(input).unwrap().degrade_reason,
        Some(M5CanonicalSourceRelationEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    );

    let mut input = clean_canonical_source_relation_input();
    input.resolution_form_coverage = vec![M5CanonicalSourceRelationResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_canonical_source_relation_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn canonical_source_relation_public_facing_and_surface_and_proof_degrade() {
    // A public-facing line running support language ahead of proof first fails the widen-boundary fold.
    let mut input = clean_canonical_source_relation_input();
    input.report_section = M5CanonicalSourceRelationKind::ProjectionObject;
    ...
    let mut input = clean_canonical_source_relation_input();
    input.surface_context = M5CanonicalSourceRelationSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_canonical_source_relation_entry(input)
            .unwrap()
            .degrade_reason,
...
#[test]
fn canonical_source_relation_empty_id_and_forbidden_material_error() {
    let mut input = clean_canonical_source_relation_input();
    input.entry_id = "".to_owned();
    assert_eq!(
        resolve_canonical_source_relation_entry(input).unwrap_err(),
        M5CanonicalSourceRelationResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_canonical_source_relation_entry(input).unwrap_err(),
        M5CanonicalSourceRelationResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5CanonicalSourceRelationKind::ObjectClassUnclassified,
        true,
        false,
        true
    ));
...
#[test]
fn canonical_source_relation_object_is_complete_requires_all_fields() {
    assert!(canonical_source_relation_object_is_complete(
        M5CanonicalSourceRelationKind::ReadOnlyPathObject,
...
fn downgrade_clean_stays_honest() {
    let resolved = resolve_write_target_review_entry(clean_downgrade_input()).unwrap();
    assert!(resolved.is_clean());
    assert!(resolved.comparison_safe_on_every_line);
    assert!(resolved.covers_all_resolution_forms);
    assert!(resolved.provides_complete_write_target_review);
    assert!(resolved.write_target_review_stays_honest);
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let resolved = resolve_write_target_review_entry(input).unwrap();
    assert!(!resolved.provides_complete_write_target_review);
    assert_eq!(
        resolved.degrade_reason,
    ...
    let mut input = clean_downgrade_input();
    input.keeps_write_target_review_visible = false;
    assert_eq!(
        resolve_write_target_review_entry(input).unwrap().degrade_reason,
        Some(M5WriteTargetReviewEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    );
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5WriteTargetReviewScope::DiffScopeUnclassified;
    assert_eq!(
        resolve_write_target_review_entry(input)
            .unwrap()
            .degrade_reason,
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    input.entry_id = "  ".to_owned();
    assert_eq!(
        resolve_write_target_review_entry(input).unwrap_err(),
        M5CanonicalSourceRelationResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn downgrade_bound_support_and_flagged_gap_stay_clean() {
    ...
    assert!(resolve_write_target_review_entry(input).unwrap().is_clean());
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_CANONICAL_SOURCE_RELATION_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_WRITE_TARGET_REVIEW_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5CanonicalSourceRelationWriteTargetReviewRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5CanonicalSourceRelationAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5CanonicalSourceRelationExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    let mut packet = seeded_m5_canonical_source_relation_and_write_target_review_registries();
    packet.registry_rows[0].write_target_review_entries.clear();
    assert!(packet
        .validate()
        .contains(&M5CanonicalSourceRelationWriteTargetReviewRegistriesViolation::ExamplesMissing));
}

#[test]
fn dishonest_clean_example_fails() {
    let mut packet = seeded_m5_canonical_source_relation_and_write_target_review_registries();
    // Force a clean canonical_source_relation entry to also read as object-incomplete — the packet must reject it.
    let row = &mut packet.registry_rows[0];
    row.canonical_source_relation_entries[0].degrade_reason = None;
    row.canonical_source_relation_entries[0].canonical_source_relation_object_complete = false;
    assert!(packet.validate().contains(
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        match mutate {
            ...
            _ => row.collapses_distinct_write_target_review_classes_into_one_lane = true,
        }
        assert!(packet.validate().contains(
...
#[test]
fn canonical_source_relation_not_proven_when_incomplete_example_removed() {
    let mut packet = seeded_m5_canonical_source_relation_and_write_target_review_registries();
    for row in &mut packet.registry_rows {
        row.canonical_source_relation_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
                    M5CanonicalSourceRelationEntryDegradeReason::CohortDescriptorObjectIncomplete,
                )
        });
    }
    assert!(packet.validate().contains(
...
#[test]
fn canonical_source_relation_not_proven_when_surface_collapses() {
    let mut packet = seeded_m5_canonical_source_relation_and_write_target_review_registries();
    // Drop every clean executive-steering-surface canonical_source_relation so the first-consumer surfaces no longer include it.
    for row in &mut packet.registry_rows {
        row.canonical_source_relation_entries
...
#[test]
fn write_target_review_integrity_not_proven_when_support_ahead_example_removed() {
    let mut packet = seeded_m5_canonical_source_relation_and_write_target_review_registries();
    for row in &mut packet.registry_rows {
        row.write_target_review_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
...
#[test]
fn write_target_review_integrity_not_proven_when_scope_dropped() {
    let mut packet = seeded_m5_canonical_source_relation_and_write_target_review_registries();
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet.validate().contains(
        &M5CanonicalSourceRelationWriteTargetReviewRegistriesViolation::RawMaterialInExport
    ));
}
...
fn export_carries_no_forbidden_raw_material() {
    let json =
        seeded_m5_canonical_source_relation_and_write_target_review_registries().export_safe_json();
...
#[test]
fn canonical_source_relation_table_lists_only_clean_canonical_source_relations() {
    let packet = seeded_m5_canonical_source_relation_and_write_target_review_registries();
    let table = packet.render_canonical_source_relation_table();
    // The clean canary and migration canonical_source_relations are rendered from the registry.
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_canonical_source_relation_and_write_target_review_registries_export()
        .expect(
        "checked M5 line-canonical_source_relation / line-downgrade-packet registries export validates",
    );
    assert_eq!(
        from_disk.packet_id,
        M5_CANONICAL_SOURCE_RELATION_WRITE_TARGET_REVIEW_REGISTRIES_PACKET_ID
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5CanonicalSourceRelationWriteTargetReviewRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/editor/m5-canonical-source-relation-and-write-target-review-registries/canonical_source_relation_beta_narrowed.json"
    )))
    .expect("line-canonical_source_relation fixture parses");
    ...
    let preview: M5CanonicalSourceRelationWriteTargetReviewRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/editor/m5-canonical-source-relation-and-write-target-review-registries/write_target_review_preview_narrowed.json"
    )))
