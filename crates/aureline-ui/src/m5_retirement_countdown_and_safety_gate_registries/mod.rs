//! Implemented M5 retirement-countdown and safety-gate registries.
//!
//! The frozen [retired-state matrix][matrix] names Aureline's governed terminal-lifecycle object classes and locks
//! their controlled vocabulary. This module turns retirement from a hidden date in release notes into a visible,
//! inspectable countdown with an explicit successor path and safe exit steps, so affected install / update,
//! settings / help, docs, marketplace, and support surfaces inherit one canonical grace-window countdown rather than
//! a hand-authored parallel prose that has to be kept consistent: it turns the
//! *retirement-countdown* grammar (one export-safe grace-window countdown per affected surface — one classified
//! countdown field per published fact, typed as the first-deprecated version, cutoff version / date, remaining
//! overlap window, successor route, fallback action, or no-surprises explanation, with an owning team and joined to
//! the current compatibility / public-proof state and the successor path or manual fallback) and the
//! *pre-retirement-safety-gate* grammar (the readiness-check scope a pre-cutoff blocker sits in — the candidate is
//! missing its declared rollback / export path, missing its declared archive bundle, or missing its successor /
//! fallback route) into registry resolvers that produce export-safe, honest projections. Every affected surface then
//! resolves to one typed retirement-countdown object — the cutoff truth it publishes, its exact-build joins, its
//! successor route, and its rollback / export route, all preserved before a claim widens so a retiring surface never
//! keeps support language ahead of its closed support note and a current rollback / export route — and every
//! retirement candidate resolves to one typed pre-retirement-safety-gate object — the resolved candidate identity,
//! the affected readiness check, the candidate-versus-successor reference, the gate-scope state, and the active gate
//! reason — that the install / update, marketplace / registry, docs / help, and support / procurement surfaces can
//! inspect without manual reconstruction, so a candidate missing a declared safe-exit route keeps its blocker list
//! visible instead of letting closure run past a surprise shutdown, and a countdown that cannot bind a fact to its
//! exact build degrades honestly instead of leaving a retiring surface to read as safe to close.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Expose a retirement countdown on every affected surface, carrying every countdown field.**
//!   [`resolve_retirement_countdown_entry`] refuses to read as a clean, registry-bound countdown entry
//!   unless it names a canonical registry token, a classified [countdown field][M5RetirementCountdownKind], a
//!   retirement role, covers every [resolution form][M5RetirementCountdownResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every countdown field (first-deprecated
//!   version, cutoff version / date, remaining overlap window, successor route, fallback action, and no-surprises
//!   explanation bound to exact-build joins), preserves its rollback / export route before a claim widens, and keeps a
//!   public-facing successor / fallback field bound to the closed support note; otherwise it degrades.
//! * **Provide successor-routing and fallback actions instead of dead-ending the user before a claim widens.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a countdown entry whose
//!   rollback / export route is not preserved before widening (a retiring class resuming a wider claim without one) so
//!   it degrades to
//!   [`M5RetirementCountdownEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a public-facing successor / fallback field whose language outruns the closed support note degrades the same
//!   way — the structured blocker reason a widen-without-route attempt must surface.
//! * **Require rollback / export / archive readiness before a candidate can pass to final closure.**
//!   [`resolve_safety_gate_entry`] names a classified [gate scope][M5SafetyGateScope]
//!   (missing-rollback-or-export-path, missing-archive-bundle, or missing-successor-or-fallback-route), requires the
//!   full candidate-identity / affected-readiness-check / candidate-versus-successor / gate-scope / active-reason gate
//!   object, covers every resolution form, and degrades to
//!   [`M5SafetyGateEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the gate would keep support language ahead of the closed support note, hide the blocker, or let a gap
//!   masquerade as covered, so a candidate can never read as safe to close when it has quietly dropped the reason a
//!   retirement is not yet ready.
//!
...
pub use seed::{
    seeded_m5_retirement_countdown_and_safety_gate_registries,
    seeded_m5_retirement_countdown_and_safety_gate_registries_retirement_countdown_beta_narrowed,
    seeded_m5_retirement_countdown_and_safety_gate_registries_safety_gate_preview_narrowed,
    M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5RetirementCountdownSafetyGateRegistriesPacket`].
pub const M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_retirement_countdown_and_safety_gate_packet_registries";

/// Schema version for M5 line-retirement_countdown / line-downgrade-packet registry records.
pub const M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_SCHEMA_REF: &str =
    "schemas/program/m5-retirement-countdown-and-safety-gate-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_DOC_REF: &str =
    "docs/release/m5_retirement_countdown_and_safety_gate_registries.md";

/// Repo-relative path of the canonical pre-retirement-safety-gate domain schema minted by this lane (the
/// machine-readable pre-cutoff blocker that stops a retirement candidate from passing to final closure while it is
/// still missing its declared rollback / export path, archive bundle, or successor / fallback route).
pub const M5_SAFETY_GATE_DOMAIN_SCHEMA_REF: &str =
    "schemas/program/m5-pre-retirement-safety-gate.schema.json";

/// Repo-relative path of the canonical retirement-countdown domain schema minted by this lane (the
/// machine-readable grace-window countdown a retiring class publishes on affected install / update,
/// settings / help, docs, marketplace, and support surfaces: first-deprecated version, cutoff
/// version / date, successor route, and any remaining overlap window).
pub const M5_RETIREMENT_COUNTDOWN_DOMAIN_SCHEMA_REF: &str =
    "schemas/program/m5-retirement-countdown.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-retirement-countdown-and-safety-gate-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-retirement-countdown-and-safety-gate-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-retirement-countdown-and-safety-gate-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5RetirementCountdownSafetyGateRegistriesConsumerSurface = M5RetiredStateConsumerSurface;

/// One of the three resolution forms every line-retirement_countdown or line-downgrade-packet entry must hold across so
...
/// line-retirement_countdown and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementCountdownResolutionForm {
    /// The canonical resolved line-retirement_countdown / line-downgrade-packet object.
    CanonicalObject,
...
impl M5RetirementCountdownResolutionForm {
...
/// Controlled manifest field a retirement-manifest entry publishes for one retiring object-class identity, so every
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementCountdownKind {
    ...
    NoSurprisesExplanation,
    /// The manifest field is unclassified, which is disallowed.
    CountdownFieldUnclassified,
}

impl M5RetirementCountdownKind {
    /// Every public-proof-ledger kind, in declaration order.
    pub const ALL: [Self; 7] = [
        Self::FirstDeprecatedVersion,
        Self::CutoffVersionOrDate,
        Self::RemainingOverlapWindow,
        Self::SuccessorRoute,
        Self::FallbackAction,
        Self::NoSurprisesExplanation,
        Self::CountdownFieldUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::FirstDeprecatedVersion => "first_deprecated_version",
            Self::CutoffVersionOrDate => "cutoff_version_or_date",
            Self::RemainingOverlapWindow => "remaining_overlap_window",
            Self::SuccessorRoute => "successor_route",
            Self::FallbackAction => "fallback_action",
            Self::NoSurprisesExplanation => "no_surprises_explanation",
            Self::CountdownFieldUnclassified => "countdown_field_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::CountdownFieldUnclassified)
    }

    /// The canonical mode for this public-proof-ledger kind.
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::FirstDeprecatedVersion => "first_deprecated_version_mode",
            Self::CutoffVersionOrDate => "cutoff_version_or_date_mode",
            Self::RemainingOverlapWindow => "remaining_overlap_window_mode",
            Self::SuccessorRoute => "successor_route_mode",
            Self::FallbackAction => "fallback_action_mode",
            Self::NoSurprisesExplanation => "no_surprises_explanation_mode",
            Self::CountdownFieldUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(self, Self::SuccessorRoute | Self::FallbackAction)
    }
}

/// Controlled manifest-change-diff scope a retirement-manifest change sits in, so a changed cutoff date or
/// replacement path becomes a visible, typed diff event rather than a silent mutation and shares one registry rather
/// than a hand-copied per-record assumption. Minted by this lane, tracking whether a manifest changed its cutoff
/// date (cutoff-date-change), changed its successor / replacement path (replacement-path-change), or changed its
/// disable path or export / rollback route (disable-or-export-route-change). Each scope maps directly to the
/// acceptance-criteria change the implementation requirement names — a changed cutoff date, a changed replacement
/// path, or a changed disable / export route — so a retirement never mutates silently.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SafetyGateScope {
    /// Cutoff-date-change: a manifest changed the cutoff date after which the class is retired.
    MissingRollbackOrExportPath,
    /// Replacement-path-change: a manifest changed the successor / replacement reference it routes forward to.
    MissingArchiveBundle,
    /// Disable-or-export-route-change: a manifest changed its disable path or its export / rollback route.
    MissingSuccessorOrFallbackRoute,
    /// The manifest-change-diff scope is unclassified, which is disallowed.
    SafetyGateScopeUnclassified,
}

impl M5SafetyGateScope {
    /// Every comparison scope, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::MissingRollbackOrExportPath,
        Self::MissingArchiveBundle,
        Self::MissingSuccessorOrFallbackRoute,
        Self::SafetyGateScopeUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::MissingRollbackOrExportPath => "missing_rollback_or_export_path",
            Self::MissingArchiveBundle => "missing_archive_bundle",
            Self::MissingSuccessorOrFallbackRoute => "missing_successor_or_fallback_route",
            Self::SafetyGateScopeUnclassified => "safety_gate_scope_unclassified",
        }
    }

    /// Whether the comparison scope is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::SafetyGateScopeUnclassified)
    }
}

/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-retirement_countdown or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementCountdownSurfaceContext {
...
impl M5RetirementCountdownSurfaceContext {
...
/// One mandatory rendered part a line-retirement_countdown or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementCountdownAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-retirement_countdown entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-retirement_countdown entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-retirement_countdown
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line retirement_countdown or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5RetirementCountdownAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "safety_gate_fields",
...
/// line retirement_countdown, a line-downgrade packet, or a degraded line-retirement_countdown / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementCountdownNextAction {
    /// Expand the resolved line retirement_countdown's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5RetirementCountdownNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementCountdownExportField {
...
impl M5RetirementCountdownExportField {
...
/// Reason a line-retirement_countdown entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementCountdownEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the retirement_countdown means.
    ...
    /// The resolved line-retirement_countdown object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5RetirementCountdownEntryDegradeReason {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "retirement_countdown_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortManifestFieldUnclassified => "line_countdown_field_unclassified",
            Self::DescriptorNotBoundToRegistry => "retirement_countdown_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "retirement_countdown_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "retirement_countdown_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5RetirementCountdownNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5RetirementCountdownNextAction::TraceCanonicalRegistry
            }
            Self::CohortManifestFieldUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5RetirementCountdownNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5RetirementCountdownNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5RetirementCountdownNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SafetyGateEntryDegradeReason {
...
impl M5SafetyGateEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::EvidenceScopeUnclassified => "comparison_safety_gate_scope_unclassified",
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                "safety_gate_runs_support_ahead_of_proof_or_drops_safety_gate"
            }
    ...
    pub const fn next_action(self) -> M5RetirementCountdownNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5RetirementCountdownNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5RetirementCountdownNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5RetirementCountdownNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_retirement_countdown_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5RetirementCountdownEntryResolutionInput {
    /// Stable identity of the line-retirement_countdown-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this retirement_countdown binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.retirement_countdown.fallback_action`); empty means unstated.
    ...
    pub report_section: M5RetirementCountdownKind,
    /// The render / surface context.
    pub surface_context: M5RetirementCountdownSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5RetirementCountdownResolutionForm>,
    ...
    /// True when the behavior traces to the line-retirement_countdown registry (never a hand-copied constant).
...
/// Resolved, export-safe line-retirement_countdown-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedRetirementCountdownEntry {
    /// Stable identity of the line-retirement_countdown-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this retirement_countdown binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-retirement_countdown object publishes every required field.
    pub retirement_countdown_object_complete: bool,
    /// Whether the entry traces to the line-retirement_countdown registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5RetirementCountdownEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5RetirementCountdownNextAction,
    /// Whether the retirement_countdown resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub retirement_countdown_resolves_across_lines: bool,
}

impl M5ResolvedRetirementCountdownEntry {
    /// Whether this line-retirement_countdown entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_safety_gate_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SafetyGateEntryResolutionInput {
    ...
    pub comparison_scope: M5SafetyGateScope,
    /// The render / surface context.
    pub surface_context: M5RetirementCountdownSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5RetirementCountdownResolutionForm>,
    ...
    pub keeps_safety_gate_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedSafetyGateEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub safety_gate_stays_honest: bool,
    ...
    pub provides_complete_safety_gate: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5SafetyGateEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5RetirementCountdownNextAction,
...
impl M5ResolvedSafetyGateEntry {
...
pub enum M5RetirementCountdownResolutionError {
    /// The line-retirement_countdown-entry id was empty.
...
impl M5RetirementCountdownResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_retirement_countdown_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_safety_gate_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5RetirementCountdownResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-retirement_countdown / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5RetirementCountdownResolutionError {}

fn form_tokens(forms: &[M5RetirementCountdownResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5RetirementCountdownResolutionForm]) -> bool {
    let present: BTreeSet<M5RetirementCountdownResolutionForm> = forms.iter().copied().collect();
    M5RetirementCountdownResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-retirement_countdown object publishes every required field: classified line journey,
...
pub fn retirement_countdown_object_is_complete(
    journey: M5RetirementCountdownKind,
...
/// Whether the line retirement_countdown keeps a line from widening without preserving its rollback and diagnostics
...
pub fn safety_gate_stays_honest(
    scope: M5SafetyGateScope,
    comparison_is_truthful: bool,
    keeps_safety_gate_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_safety_gate_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-retirement_countdown-registry entry so it stays bound to the line-retirement_countdown registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete retirement_countdown object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_retirement_countdown_entry(
    input: M5RetirementCountdownEntryResolutionInput,
) -> Result<M5ResolvedRetirementCountdownEntry, M5RetirementCountdownResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5RetirementCountdownResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5RetirementCountdownResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = retirement_countdown_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5RetirementCountdownEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5RetirementCountdownEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5RetirementCountdownEntryDegradeReason::CohortManifestFieldUnclassified)
    } else if !input.bound_to_registry {
        Some(M5RetirementCountdownEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5RetirementCountdownEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5RetirementCountdownEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5RetirementCountdownEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(
            M5RetirementCountdownEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    } else if !input.proof_fresh {
        Some(M5RetirementCountdownEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5RetirementCountdownNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedRetirementCountdownEntry {
        ...
        retirement_countdown_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        retirement_countdown_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_safety_gate_entry(
    input: M5SafetyGateEntryResolutionInput,
) -> Result<M5ResolvedSafetyGateEntry, M5RetirementCountdownResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5RetirementCountdownResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = safety_gate_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_safety_gate_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SafetyGateEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SafetyGateEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5SafetyGateEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5SafetyGateEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5SafetyGateEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5SafetyGateEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5RetirementCountdownNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedSafetyGateEntry {
        entry_id: input.entry_id,
        ...
        keeps_safety_gate_visible: input.keeps_safety_gate_visible,
        ...
        safety_gate_stays_honest: record_stays_honest,
        provides_complete_safety_gate: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-retirement_countdown and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementCountdownSafetyGateRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5RetirementCountdownSafetyGateRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5RetirementCountdownAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5RetirementCountdownExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5RetiredStateDowngradeTrigger>,
    /// Resolved line-retirement_countdown-registry examples.
    pub retirement_countdown_entries: Vec<M5ResolvedRetirementCountdownEntry>,
    /// Resolved line-downgrade-packet examples.
    pub safety_gate_entries: Vec<M5ResolvedSafetyGateEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-retirement_countdown and
    ...
    pub collapses_distinct_safety_gate_classes_into_one_lane: bool,
}

impl M5RetirementCountdownSafetyGateRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5RetirementCountdownAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5RetirementCountdownAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5RetirementCountdownExportField> =
            self.export_fields.iter().copied().collect();
        M5RetirementCountdownExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_safety_gate_classes_into_one_lane
    }

    /// True when a clean line-retirement_countdown entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete retirement_countdown object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn retirement_countdown_is_honest(ex: &M5ResolvedRetirementCountdownEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.retirement_countdown_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedSafetyGateEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_safety_gate
                && ex.safety_gate_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.retirement_countdown_entries
            .iter()
            .all(Self::retirement_countdown_is_honest)
            && self
                .safety_gate_entries
                .iter()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementCountdownSafetyGateRegistriesVocabularySet {
    ...
    /// Cohort-retirement_countdown-entry degrade-reason tokens.
    pub retirement_countdown_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub safety_gate_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5RetirementCountdownSafetyGateRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5RetiredStateRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5RetirementCountdownResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5RetirementCountdownKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5SafetyGateScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5RetirementCountdownSurfaceContext::ALL, |v| v.as_str()),
            retirement_countdown_degrade_reasons: tokens(
                &M5RetirementCountdownEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            safety_gate_degrade_reasons: tokens(&M5SafetyGateEntryDegradeReason::ALL, |v| {
                v.as_str()
            }),
            anatomy_parts: tokens(&M5RetirementCountdownAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5RetirementCountdownNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5RetirementCountdownExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementCountdownSafetyGateRegistriesGovernanceReview {
    /// The retirement_countdown registry names a canonical token, semantic role, and line journey for every entry.
    pub retirement_countdown_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-retirement_countdown object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_retirement_countdown_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved retirement_countdown.
    ...
    pub safety_gate_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-retirement_countdown and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-retirement_countdown and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub retirement_countdown_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementCountdownSafetyGateRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-retirement_countdown registry.
    ...
    /// Behavior traces back to the canonical line-retirement_countdown and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-retirement_countdown / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementCountdownSafetyGateRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementCountdownSafetyGateRegistriesReleasePosture {
...
/// Constructor input for [`M5RetirementCountdownSafetyGateRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5RetirementCountdownSafetyGateRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5RetirementCountdownSafetyGateRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5RetirementCountdownSafetyGateRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5RetirementCountdownSafetyGateRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5RetirementCountdownSafetyGateRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5RetirementCountdownSafetyGateRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5RetirementCountdownSafetyGateRegistriesReleasePosture,
...
/// Export-safe M5 line-retirement_countdown and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementCountdownSafetyGateRegistriesPacket {
    /// Record kind; must equal [`M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_SCHEMA_VERSION`].
...
impl M5RetirementCountdownSafetyGateRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5RetirementCountdownSafetyGateRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5RetirementCountdownSafetyGateRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_RECORD_KIND {
            violations.push(M5RetirementCountdownSafetyGateRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_RETIREMENT_COUNTDOWN_SAFETY_GATE_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5RetirementCountdownSafetyGateRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5RetirementCountdownSafetyGateRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5RetirementCountdownSafetyGateRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-retirement_countdown / line-downgrade-packet registries packet serializes",
        )) {
            violations
                .push(M5RetirementCountdownSafetyGateRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,retirement_countdown_entries,safety_gate_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                ...
                row.retirement_countdown_entries.len(),
                row.safety_gate_entries.len(),
                degrades.join("|"),
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Retirement-Countdown and Pre-Retirement-Safety-Gate Registries\n\n");
        ...
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                "  - Countdown entries: {} / safety-gate entries: {}\n",
                row.retirement_countdown_entries.len(),
                row.safety_gate_entries.len()
            ));
        }
    ...
    /// registry-bound line-retirement_countdown entries are listed.
    pub fn render_retirement_countdown_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.retirement_countdown_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5RetirementCountdownSafetyGateRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5RetirementCountdownSafetyGateRegistriesViolation>),
}

impl fmt::Display for M5RetirementCountdownSafetyGateRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-retirement_countdown / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-retirement_countdown / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5RetirementCountdownSafetyGateRegistriesArtifactError {}

/// Validation failures emitted by [`M5RetirementCountdownSafetyGateRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5RetirementCountdownSafetyGateRegistriesViolation {
    ...
    /// A registry row does not point at both the line-retirement_countdown and line-downgrade-packet domain schemas.
    ...
    /// Cohort-retirement_countdown-resolution is not proven: clean retirement_countdown entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean retirement_countdown entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded retirement_countdown entry is present, or a clean retirement_countdown entry is unbounded
    /// or unbound.
...
impl M5RetirementCountdownSafetyGateRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "retirement_countdown_resolution_not_proven"
            }
            Self::RollbackAndDiagnosticsPreservationNotProven => {
                "rollback_and_diagnostics_preservation_not_proven"
            }
            Self::CohortEvidenceIntegrityNotProven => "safety_gate_integrity_not_proven",
...
pub fn current_stable_m5_retirement_countdown_and_safety_gate_registries_export() -> Result<
    M5RetirementCountdownSafetyGateRegistriesPacket,
    M5RetirementCountdownSafetyGateRegistriesArtifactError,
> {
    let packet: M5RetirementCountdownSafetyGateRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-retirement-countdown-and-safety-gate-registries-proof/support_export.json"
        )
    ))
    .map_err(M5RetirementCountdownSafetyGateRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5RetirementCountdownSafetyGateRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5RetirementCountdownSafetyGateRegistriesPacket,
    violations: &mut Vec<M5RetirementCountdownSafetyGateRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations
                .push(M5RetirementCountdownSafetyGateRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5RetirementCountdownSafetyGateRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations
                .push(M5RetirementCountdownSafetyGateRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5RetirementCountdownSafetyGateRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5RetirementCountdownSafetyGateRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_RETIREMENT_COUNTDOWN_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_SAFETY_GATE_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5RetirementCountdownSafetyGateRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.retirement_countdown_entries.is_empty() || row.safety_gate_entries.is_empty() {
            violations.push(M5RetirementCountdownSafetyGateRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5RetirementCountdownSafetyGateRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5RetirementCountdownSafetyGateRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.retirement_countdown_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_retirement_countdown_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.safety_gate_keeps_proof_visible_and_binds_support_language,
        ...
        review.retirement_countdown_or_downgrade_drift_caught_before_release,
        review.every_row_declares_mandatory_anatomy,
        review.reuses_frozen_matrix_vocabulary,
    ] {
        if !ok {
            violations.push(
                M5RetirementCountdownSafetyGateRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5RetirementCountdownSafetyGateRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5RetirementCountdownSafetyGateRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5RetirementCountdownSafetyGateRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let retirement_countdowns = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.retirement_countdown_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.safety_gate_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean retirement_countdown entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean retirement_countdown entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = retirement_countdowns()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = retirement_countdowns()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5RetirementCountdownKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5RetirementCountdownSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = retirement_countdowns().any(|ex| {
        ex.degrade_reason
            == Some(M5RetirementCountdownEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete = !retirement_countdowns()
        .any(|ex| ex.is_clean() && !ex.retirement_countdown_object_complete);
    if !(journeys_covered
    ...
    {
        violations.push(
            M5RetirementCountdownSafetyGateRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }

    // AC2: line packets preserve rollback and diagnostics posture before widening. A widen-without-rollback
    // example degrades, an unbound example degrades, at least one clean bounded retirement_countdown entry is present, and
    // no clean retirement_countdown entry is unbounded or unbound.
    let widen_fold_degrades = retirement_countdowns().any(|ex| {
        ex.degrade_reason
            == Some(
                M5RetirementCountdownEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = retirement_countdowns().any(|ex| {
        ex.degrade_reason
            == Some(M5RetirementCountdownEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_retirement_countdown =
        retirement_countdowns().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound =
        !retirement_countdowns().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded =
        !retirement_countdowns().any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_retirement_countdown
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
            M5RetirementCountdownSafetyGateRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5SafetyGateScope::CANONICAL_SCOPES
        .iter()
    ...
    let form_incomplete_degrades = downgrade().any(|ex| {
        ex.degrade_reason == Some(M5SafetyGateEntryDegradeReason::EvidenceFormCoverageIncomplete)
    });
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_safety_gate);
    ...
    {
        violations.push(
            M5RetirementCountdownSafetyGateRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-retirement_countdown registry
