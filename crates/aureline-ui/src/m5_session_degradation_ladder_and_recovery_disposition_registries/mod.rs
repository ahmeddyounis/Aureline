//! Implemented M5 session-degradation-ladder and recovery-disposition registries.
//!
//! The frozen [collaboration-state authority matrix][matrix] names Aureline's governed shared-object classes and
//! locks their authority, convergence, and export vocabulary. Building on the higher-risk control-plane
//! degradation-banner domain (the separate higher-risk control plane and the convergence / awareness degradation
//! banner it drives) and the presenter / follow convergence-state domain it freezes, this module keeps a *failing*
//! shared session as honest and reviewable as other Aureline evidence objects: when a shared session degrades — a
//! participant lag, shared-degraded, relay-partition, awareness-degraded, or convergence-degraded transition — the
//! degraded state, its sticky banner, its recent-activity rows, and the recovery paths still open are recorded first,
//! and the session degrades in the required order (local editing last) rather than freezing local work or letting
//! remote authority silently resolve divergence. So the shared editor replica view, the presence / cursor layer, the
//! comment / annotation / review-pin layer, the collaboration degradation banner, and the support / export and help /
//! docs surfaces inherit one canonical degradation-ladder record rather than a hand-authored parallel prose that has
//! to be kept consistent: it turns the *degradation-ladder* grammar (one durable append-only record per degraded
//! session — one typed field per record section: the degraded session state, the sticky banner it raises, the
//! recent-activity rows it keeps, the degradation-order rung so local editing degrades last, the actor provenance of
//! the transition, and the recovery paths still available, each bound to the degradation ladder it never silently
//! resolves by remote authority) and the *recovery-disposition* grammar (the recovery path offered on a degraded
//! session — continue-local, retry-shared, retry-convergence, review-and-reconcile, or leave-open — plus its actor /
//! time provenance, its awareness / convergence facts, and its local-edit-first continuity lineage) into registry
//! resolvers that produce export-safe, honest projections. Every degraded session then resolves to one typed
//! degradation-ladder record — the fields it publishes, the degraded state, the sticky banner and recent-activity
//! rows, the degradation-order rung, and the recovery paths still open, all attributable so divergence is never
//! resolved implicitly — and to one recovery-disposition descriptor — the path, its actor / time provenance, and its
//! local-edit-first continuity lineage — that the shared editor, presence, review-pin, degradation-banner, and
//! support surfaces can inspect before continuing local, retrying, or reconciling, so a record that cannot keep local
//! editing first degrades honestly instead of letting a degraded session read as clean when local work was frozen or
//! divergence was silently resolved by remote authority.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable degradation-ladder record per degraded session, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [degraded state][M5SessionDegradationRecoveryRecordKind],
//!   a disclosure role, covers every [resolution form][M5SessionDegradationRecoveryRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (the
//!   degraded state, the sticky banner and recent-activity rows, the degradation-order rung, and the recovery paths
//!   bound to the degraded state), keeps its lineage
//!   attributable, and keeps the degradation ladder it never silently resolves bound to the record; otherwise it degrades.
//! * **Never freeze local editing or silently resolve divergence; keep local editing first on any degradation.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a record whose
//!   local-edit-first ordering is not held (a degraded session presenting itself as clean while local editing was
//!   frozen or divergence was resolved by remote authority) so it degrades to
//!   [`M5SessionDegradationRecoveryRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and any degradation — a relay partition, participant failure, or awareness / convergence loss — that
//!   would freeze local editing degrades the same way — the structured blocker reason a
//!   local-editing-freeze attempt must surface.
//! * **Offer a reviewable recovery path with actor / time provenance, instead of a silent remote resolution.**
//!   [`resolve_review_pack_result_entry`] names a classified
//!   [recovery binding][M5SessionDegradationRecoveryResultScope]
//!   (continue-local binding, retry binding, or review-and-reconcile binding), requires the full
...
//!   [`M5SessionDegradationRecoveryResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the recovery would resolve divergence by remote authority without review, act without actor / time provenance, or
//!   discard unsent local edits, so a recovery can never read as trustworthy when it has quietly resolved
//!   divergence or dropped its provenance.
//!
...
//! export surfaces can never fork their own degradation / recovery meaning. Raw secrets, raw command text, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_session_degradation_ladder_and_recovery_disposition_registries,
    seeded_m5_session_degradation_ladder_and_recovery_disposition_registries_review_pack_record_beta_narrowed,
    seeded_m5_session_degradation_ladder_and_recovery_disposition_registries_review_pack_result_preview_narrowed,
    M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_PACKET_ID,
};
...
use crate::m5_collaboration_replica_shared_object_authority_anchor_drift_convergence_and_session_archive_matrix::{
    ...
    M5_HIGHER_RISK_CONTROL_PLANE_DOMAIN_SCHEMA_REF, M5_PRESENTER_FOLLOW_STATE_DOMAIN_SCHEMA_REF,
    M5_COLLABORATION_STATE_MATRIX_DOC_REF, M5_COLLABORATION_STATE_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5SessionDegradationRecoveryRecordAndResultRegistriesPacket`].
pub const M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_session_degradation_ladder_and_recovery_disposition_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_session_degradation_ladder_and_recovery_disposition_registries.md";

/// Repo-relative path of the checked support-export artifact.
pub const M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-session-degradation-ladder-and-recovery-disposition-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-session-degradation-ladder-and-recovery-disposition-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-session-degradation-ladder-and-recovery-disposition-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-session-degradation-ladder-and-recovery-disposition-registries";
...
pub type M5SessionDegradationRecoveryRecordAndResultRegistriesConsumerSurface =
...
pub enum M5SessionDegradationRecoveryRecordResolutionForm {
...
impl M5SessionDegradationRecoveryRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionDegradationRecoveryRecordKind {
...
impl M5SessionDegradationRecoveryRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionDegradationRecoveryResultScope {
...
impl M5SessionDegradationRecoveryResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionDegradationRecoveryRecordSurfaceContext {
...
impl M5SessionDegradationRecoveryRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionDegradationRecoveryRecordAnatomyPart {
...
impl M5SessionDegradationRecoveryRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionDegradationRecoveryRecordNextAction {
...
impl M5SessionDegradationRecoveryRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionDegradationRecoveryRecordExportField {
...
impl M5SessionDegradationRecoveryRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionDegradationRecoveryRecordEntryDegradeReason {
...
impl M5SessionDegradationRecoveryRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5SessionDegradationRecoveryRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5SessionDegradationRecoveryRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5SessionDegradationRecoveryRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5SessionDegradationRecoveryRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => {
                M5SessionDegradationRecoveryRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionDegradationRecoveryResultEntryDegradeReason {
...
impl M5SessionDegradationRecoveryResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
...
pub struct M5SessionDegradationRecoveryRecordEntryResolutionInput {
    ...
    pub report_section: M5SessionDegradationRecoveryRecordKind,
    /// The render / surface context.
    pub surface_context: M5SessionDegradationRecoveryRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SessionDegradationRecoveryRecordResolutionForm>,
...
pub struct M5ResolvedSessionDegradationRecoveryRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SessionDegradationRecoveryRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SessionDegradationRecoveryRecordNextAction,
...
impl M5ResolvedSessionDegradationRecoveryRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SessionDegradationRecoveryResultEntryResolutionInput {
    ...
    pub comparison_scope: M5SessionDegradationRecoveryResultScope,
    /// The render / surface context.
    pub surface_context: M5SessionDegradationRecoveryRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SessionDegradationRecoveryRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedSessionDegradationRecoveryResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SessionDegradationRecoveryResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SessionDegradationRecoveryRecordNextAction,
...
impl M5ResolvedSessionDegradationRecoveryResultEntry {
...
pub enum M5SessionDegradationRecoveryRecordResolutionError {
...
impl M5SessionDegradationRecoveryRecordResolutionError {
...
impl fmt::Display for M5SessionDegradationRecoveryRecordResolutionError {
...
impl Error for M5SessionDegradationRecoveryRecordResolutionError {}

fn form_tokens(forms: &[M5SessionDegradationRecoveryRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5SessionDegradationRecoveryRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5SessionDegradationRecoveryRecordResolutionForm> =
        forms.iter().copied().collect();
    M5SessionDegradationRecoveryRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5SessionDegradationRecoveryRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5SessionDegradationRecoveryResultScope,
...
pub fn resolve_review_pack_record_entry(
    ...
    M5SessionDegradationRecoveryRecordResolutionError,
> {
    if input.entry_id.trim().is_empty() {
        return Err(
            M5SessionDegradationRecoveryRecordResolutionError::EmptyCohortDescriptorEntryId,
        );
    }
    ...
    {
        return Err(M5SessionDegradationRecoveryRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SessionDegradationRecoveryRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SessionDegradationRecoveryRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5SessionDegradationRecoveryRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5SessionDegradationRecoveryRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5SessionDegradationRecoveryRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5SessionDegradationRecoveryRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5SessionDegradationRecoveryRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5SessionDegradationRecoveryRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5SessionDegradationRecoveryRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5SessionDegradationRecoveryRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedSessionDegradationRecoveryRecordEntry {
...
> {
    if input.entry_id.trim().is_empty() {
        return Err(
            M5SessionDegradationRecoveryRecordResolutionError::EmptyCohortEvidencePacketEntryId,
        );
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SessionDegradationRecoveryResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SessionDegradationRecoveryResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5SessionDegradationRecoveryResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5SessionDegradationRecoveryResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5SessionDegradationRecoveryResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5SessionDegradationRecoveryResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5SessionDegradationRecoveryRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedSessionDegradationRecoveryResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionDegradationRecoveryRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5SessionDegradationRecoveryRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5SessionDegradationRecoveryRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5SessionDegradationRecoveryRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedSessionDegradationRecoveryRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedSessionDegradationRecoveryResultEntry>,
...
impl M5SessionDegradationRecoveryRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5SessionDegradationRecoveryRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5SessionDegradationRecoveryRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5SessionDegradationRecoveryRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5SessionDegradationRecoveryRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedSessionDegradationRecoveryRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedSessionDegradationRecoveryResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionDegradationRecoveryRecordAndResultRegistriesVocabularySet {
...
impl M5SessionDegradationRecoveryRecordAndResultRegistriesVocabularySet {
    ...
    pub fn canonical() -> Self {
        Self {
            ...
            report_section_kinds: tokens(&M5SessionDegradationRecoveryRecordKind::ALL, |v| {
                v.as_str()
            }),
            comparison_scopes: tokens(&M5SessionDegradationRecoveryResultScope::ALL, |v| {
                v.as_str()
            }),
            surface_contexts: tokens(
                &M5SessionDegradationRecoveryRecordSurfaceContext::ALL,
                |v| v.as_str(),
            ),
            review_pack_record_degrade_reasons: tokens(
                &M5SessionDegradationRecoveryRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5SessionDegradationRecoveryResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5SessionDegradationRecoveryRecordAnatomyPart::ALL, |v| {
                v.as_str()
            }),
            next_actions: tokens(&M5SessionDegradationRecoveryRecordNextAction::ALL, |v| {
                v.as_str()
            }),
            export_fields: tokens(&M5SessionDegradationRecoveryRecordExportField::ALL, |v| {
                v.as_str()
            }),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionDegradationRecoveryRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionDegradationRecoveryRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionDegradationRecoveryRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionDegradationRecoveryRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5SessionDegradationRecoveryRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SessionDegradationRecoveryRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5SessionDegradationRecoveryRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5SessionDegradationRecoveryRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5SessionDegradationRecoveryRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection:
        M5SessionDegradationRecoveryRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5SessionDegradationRecoveryRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5SessionDegradationRecoveryRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionDegradationRecoveryRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_SCHEMA_VERSION`].
...
impl M5SessionDegradationRecoveryRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5SessionDegradationRecoveryRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind:
                M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_RECORD_KIND
                    .to_owned(),
            schema_version:
                M5_SESSION_DEGRADATION_LADDER_AND_RECOVERY_DISPOSITION_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5SessionDegradationRecoveryRecordAndResultRegistriesViolation> {
        ...
        {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::WrongRecordKind,
            );
        }
        ...
        {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::MissingIdentity,
            );
        }
        ...
        if !self.vocabulary_set.matches_canonical() {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::VocabularySetDrift,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::RawMaterialInExport,
            );
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Session-Degradation-Ladder and Recovery-Disposition Registries\n\n");
...
#[derive(Debug)]
pub enum M5SessionDegradationRecoveryRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5SessionDegradationRecoveryRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5SessionDegradationRecoveryRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5SessionDegradationRecoveryRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5SessionDegradationRecoveryRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5SessionDegradationRecoveryRecordAndResultRegistriesViolation {
...
impl M5SessionDegradationRecoveryRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_session_degradation_ladder_and_recovery_disposition_registries_export(
) -> Result<
    M5SessionDegradationRecoveryRecordAndResultRegistriesPacket,
    M5SessionDegradationRecoveryRecordAndResultRegistriesArtifactError,
> {
    let packet: M5SessionDegradationRecoveryRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-session-degradation-ladder-and-recovery-disposition-registries-proof/support_export.json"
        )
    ))
    .map_err(M5SessionDegradationRecoveryRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(
            M5SessionDegradationRecoveryRecordAndResultRegistriesArtifactError::Validation(
                violations,
            ),
        )
    }
...
fn validate_source_contracts(
    packet: &M5SessionDegradationRecoveryRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5SessionDegradationRecoveryRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations
            .push(M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_HIGHER_RISK_CONTROL_PLANE_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_PRESENTER_FOLLOW_STATE_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::ExamplesMissing,
            );
        }
        if !row.examples_are_honest() {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::DishonestExample,
            );
        }
        if !row.honours_invariants() {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5SessionDegradationRecoveryRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5SessionDegradationRecoveryRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    let unbound_degrades = review_pack_records().any(|ex| {
        ex.degrade_reason
            == Some(
                M5SessionDegradationRecoveryRecordEntryDegradeReason::DescriptorNotBoundToRegistry,
            )
    });
    ...
    {
        violations.push(
            M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5SessionDegradationRecoveryResultScope::CANONICAL_SCOPES
        .iter()
    ...
    let form_incomplete_degrades = downgrade().any(|ex| {
        ex.degrade_reason
            == Some(
                M5SessionDegradationRecoveryResultEntryDegradeReason::EvidenceFormCoverageIncomplete,
            )
    });
    ...
    {
        violations.push(
            M5SessionDegradationRecoveryRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
