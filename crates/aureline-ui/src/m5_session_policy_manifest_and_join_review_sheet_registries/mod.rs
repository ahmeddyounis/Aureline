//! Implemented M5 session-policy-manifest and join-review-sheet registries.
//!
//! The frozen [collaboration-control component matrix][matrix] names Aureline's governed sensitive-collaboration
//! object classes and locks their controlled vocabulary. This module makes the durable session-policy manifest
//! operable — as a canonical record — and adds the join-review / consent envelope any participant must pass before
//! joining, so the desktop join flow, the shared terminal / debug view, the companion-follow flow, and the support /
//! export and help / docs surfaces inherit one canonical session-policy descriptor rather than a hand-authored
//! parallel prose that has to be kept consistent: it turns the *session-policy-manifest* grammar (one durable record
//! per collaboration session — one typed field per record section: the stable session identity, the session type and
//! tenant / guest policy, the participant list or scope class, the active roles and active badges, the retention
//! envelope and export / delete posture, and the read-only default a session begins in — each bound to one session /
//! tenant scope) and the *join-review / consent-envelope* grammar (the disclosure a participant sits behind before
//! joining, and the fresh visible consent event any change to an already-active session must raise — an external
//! guest, scope widening, a retention-mode change, or a route-share visibility change) into registry resolvers that
//! produce export-safe, honest projections. Every collaboration session then resolves to one typed session-policy
//! manifest — the fields it publishes, its tenant / guest policy, its retention envelope, and its read-only default,
//! all attributable to the session so presence never reads as terminal / debug control — and to one join-review /
//! consent object — the resolved session identity, the disclosed scope, the available authority, and the active
//! consent / retention gate — that the desktop join, shared terminal / debug, companion, and support surfaces can
//! inspect without manual reconstruction, so a sensitive session begins view-first with its retention / guest /
//! export consequences disclosed, and a manifest that cannot bind a field to its session / tenant scope degrades
//! honestly instead of letting a session read as safe to join when it is not.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable session-policy manifest per session, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [scope selector][M5SessionPolicyRecordKind],
//!   a disclosure role, covers every [resolution form][M5SessionPolicyRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (session type
//!   and tenant / guest policy, participant scope, retention envelope, export / delete posture, and the read-only
//!   default bound to the session identity), keeps its disclosure lineage attributable to the session, and keeps a
//!   guest-facing field bound to its consent state; otherwise it degrades.
//! * **Disclose retention / guest / export consequences at join rather than a hand-authored parallel prose staying
//!   consistent.** [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a manifest whose
//!   disclosure join is not preserved (a session presenting itself as safe to join without one) so it degrades to
//!   [`M5SessionPolicyRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a guest-facing disclosure field whose language outruns its consent state degrades the same
//!   way — the structured blocker reason a join-without-disclosure attempt must surface.
//! * **Turn a changed guest scope, retention mode, or route-share visibility into a visible, typed consent event
//!   instead of a silent mutation.** [`resolve_review_pack_result_entry`] names a classified
//!   [consent binding][M5SessionPolicyResultScope]
//!   (guest-scope binding, retention-mode binding, or route-visibility binding), requires the full
//!   session-identity / affected-disclosure-field / previous-versus-current-consent-state / binding / active-reason
//!   consent object, covers every resolution form, and degrades to
//!   [`M5SessionPolicyResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the change would present a stale consent as current, hide the widened scope, or let a retention gap
//!   masquerade as covered, so a session change can never read as trustworthy when it has quietly dropped the reason
//!   a fresh visible consent event was required.
//!
//! The resolvers reuse the frozen matrix vocabulary directly — the [`M5CollaborationControlRole`] disclosure
//! role vocabulary and the [`M5CollaborationControlConsumerSurface`] consumer-surface taxonomy — so the desktop join
//! sheet, shared terminal / debug view, companion follow, retention sheet, and support / export surfaces
//! can never fork their own session-policy meaning. Raw secrets, raw command text, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_session_policy_manifest_and_join_review_sheet_registries,
    seeded_m5_session_policy_manifest_and_join_review_sheet_registries_review_pack_record_beta_narrowed,
    seeded_m5_session_policy_manifest_and_join_review_sheet_registries_review_pack_result_preview_narrowed,
    M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5SessionPolicyRecordAndResultRegistriesPacket`].
pub const M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_session_policy_manifest_and_join_review_sheet_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_session_policy_manifest_and_join_review_sheet_registries.md";
...
pub const M5_COLLABORATION_JOIN_REVIEW_SHEET_DOMAIN_SCHEMA_REF: &str =
    "schemas/ui/m5-collaboration-join-review-sheet.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-session-policy-manifest-and-join-review-sheet-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-session-policy-manifest-and-join-review-sheet-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-session-policy-manifest-and-join-review-sheet-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-session-policy-manifest-and-join-review-sheet-registries";
...
pub type M5SessionPolicyRecordAndResultRegistriesConsumerSurface =
...
pub enum M5SessionPolicyRecordResolutionForm {
...
impl M5SessionPolicyRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionPolicyRecordKind {
...
impl M5SessionPolicyRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionPolicyResultScope {
...
impl M5SessionPolicyResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionPolicyRecordSurfaceContext {
...
impl M5SessionPolicyRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionPolicyRecordAnatomyPart {
...
impl M5SessionPolicyRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionPolicyRecordNextAction {
...
impl M5SessionPolicyRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionPolicyRecordExportField {
...
impl M5SessionPolicyRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionPolicyRecordEntryDegradeReason {
...
impl M5SessionPolicyRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5SessionPolicyRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5SessionPolicyRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5SessionPolicyRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5SessionPolicyRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5SessionPolicyRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionPolicyResultEntryDegradeReason {
...
impl M5SessionPolicyResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5SessionPolicyRecordNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5SessionPolicyRecordNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5SessionPolicyRecordNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5SessionPolicyRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5SessionPolicyRecordEntryResolutionInput {
    ...
    pub report_section: M5SessionPolicyRecordKind,
    /// The render / surface context.
    pub surface_context: M5SessionPolicyRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SessionPolicyRecordResolutionForm>,
...
pub struct M5ResolvedSessionPolicyRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SessionPolicyRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SessionPolicyRecordNextAction,
...
impl M5ResolvedSessionPolicyRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SessionPolicyResultEntryResolutionInput {
    ...
    pub comparison_scope: M5SessionPolicyResultScope,
    /// The render / surface context.
    pub surface_context: M5SessionPolicyRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SessionPolicyRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedSessionPolicyResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SessionPolicyResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SessionPolicyRecordNextAction,
...
impl M5ResolvedSessionPolicyResultEntry {
...
pub enum M5SessionPolicyRecordResolutionError {
...
impl M5SessionPolicyRecordResolutionError {
...
impl fmt::Display for M5SessionPolicyRecordResolutionError {
...
impl Error for M5SessionPolicyRecordResolutionError {}

fn form_tokens(forms: &[M5SessionPolicyRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5SessionPolicyRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5SessionPolicyRecordResolutionForm> = forms.iter().copied().collect();
    M5SessionPolicyRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5SessionPolicyRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5SessionPolicyResultScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedSessionPolicyRecordEntry, M5SessionPolicyRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5SessionPolicyRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5SessionPolicyRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SessionPolicyRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SessionPolicyRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5SessionPolicyRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5SessionPolicyRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5SessionPolicyRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5SessionPolicyRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5SessionPolicyRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5SessionPolicyRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5SessionPolicyRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5SessionPolicyRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedSessionPolicyRecordEntry {
...
pub fn resolve_review_pack_result_entry(
...
) -> Result<M5ResolvedSessionPolicyResultEntry, M5SessionPolicyRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5SessionPolicyRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SessionPolicyResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SessionPolicyResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5SessionPolicyResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5SessionPolicyResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5SessionPolicyResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5SessionPolicyResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5SessionPolicyRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedSessionPolicyResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionPolicyRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5SessionPolicyRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5SessionPolicyRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5SessionPolicyRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedSessionPolicyRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedSessionPolicyResultEntry>,
...
impl M5SessionPolicyRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5SessionPolicyRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5SessionPolicyRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5SessionPolicyRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5SessionPolicyRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedSessionPolicyRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedSessionPolicyResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionPolicyRecordAndResultRegistriesVocabularySet {
...
impl M5SessionPolicyRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5CollaborationControlRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5SessionPolicyRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5SessionPolicyRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5SessionPolicyResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5SessionPolicyRecordSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5SessionPolicyRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5SessionPolicyResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5SessionPolicyRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5SessionPolicyRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5SessionPolicyRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionPolicyRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionPolicyRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionPolicyRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionPolicyRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5SessionPolicyRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SessionPolicyRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5SessionPolicyRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5SessionPolicyRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5SessionPolicyRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5SessionPolicyRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5SessionPolicyRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5SessionPolicyRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionPolicyRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_SCHEMA_VERSION`].
...
impl M5SessionPolicyRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5SessionPolicyRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version:
                M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5SessionPolicyRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind
            != M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_RECORD_KIND
        {
            violations.push(M5SessionPolicyRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_SESSION_POLICY_MANIFEST_AND_JOIN_REVIEW_SHEET_REGISTRIES_SCHEMA_VERSION
        {
            violations.push(M5SessionPolicyRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5SessionPolicyRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5SessionPolicyRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5SessionPolicyRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Session-Policy-Manifest and Join-Review-Sheet Registries\n\n");
...
#[derive(Debug)]
pub enum M5SessionPolicyRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5SessionPolicyRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5SessionPolicyRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5SessionPolicyRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5SessionPolicyRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5SessionPolicyRecordAndResultRegistriesViolation {
...
impl M5SessionPolicyRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_session_policy_manifest_and_join_review_sheet_registries_export() -> Result<
    M5SessionPolicyRecordAndResultRegistriesPacket,
    M5SessionPolicyRecordAndResultRegistriesArtifactError,
> {
    let packet: M5SessionPolicyRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-session-policy-manifest-and-join-review-sheet-registries-proof/support_export.json"
        )
    ))
    .map_err(M5SessionPolicyRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5SessionPolicyRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5SessionPolicyRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5SessionPolicyRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations
                .push(M5SessionPolicyRecordAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5SessionPolicyRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations
                .push(M5SessionPolicyRecordAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5SessionPolicyRecordAndResultRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5SessionPolicyRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        ...
        if !refs.contains(M5_SESSION_RESTORE_VIEW_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_COLLABORATION_JOIN_REVIEW_SHEET_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5SessionPolicyRecordAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5SessionPolicyRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5SessionPolicyRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5SessionPolicyRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SessionPolicyRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SessionPolicyRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5SessionPolicyRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5SessionPolicyRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5SessionPolicyRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5SessionPolicyRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5SessionPolicyRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5SessionPolicyRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5SessionPolicyResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(
            M5SessionPolicyRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
