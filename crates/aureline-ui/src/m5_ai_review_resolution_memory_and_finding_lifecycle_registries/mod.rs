//! Implemented M5 resolution-memory-row and finding-lifecycle-transition registries.
//!
//! The frozen [AI-review-assist matrix][matrix] names Aureline's governed AI-review object classes and locks their
//! controlled vocabulary. This module makes the matrix's `resolution_memory_row` and `ai_review_finding_row`
//! classes operable so review, AI, provider, pending-review, and support / export surfaces inherit one canonical
//! model of *what happened to each AI review finding over time* and *how a published, outdated, or reopened state
//! joins back to its original finding and diff scope* rather than dismissal, suppression, publish, staleness, and
//! reopen decisions disappearing into UI state: it turns the *resolution-memory-row* grammar (one durable row per
//! finding-state transition — one typed field per section: the finding's lifecycle state [open, dismissed,
//! suppressed, published, outdated, reopened], the actor / source and rationale class captured on the transition,
//! the timestamp, the reopen action, and any provider destination or local-draft relation — each bound to one
//! object-class identity, without shaming language or anthropomorphic copy) and the *finding-lifecycle-transition*
//! grammar (the typed lifecycle-transition join that ties a published, outdated, or reopened state back to the
//! original finding and diff scope) into registry resolvers that produce export-safe, honest projections. Every AI
//! review finding then resolves to a readable history — the stable ID it keeps, its dismissed / suppressed state
//! preserving actor / source and rationale class rather than collapsing into a generic hidden state, so a
//! `dismissed` finding never reads the same as a `suppressed` one and a stale finding never keeps looking current
//! after diff or instruction drift — and to one finding-lifecycle-transition object — the published / outdated /
//! reopened state joined back to its originating finding — that the review, AI, provider, and support surfaces can
//! inspect without manual reconstruction, so later review and support exports can reconstruct the full lifecycle,
//! and a row that cannot bind its transition to a classified state degrades honestly instead of reading as a clean,
//! provable history entry. The same finding packet — stable ID, actor / source, rationale class, and destination or
//! local-draft relation — is preserved in local history and support / export so the lifecycle stays available after
//! restart, export, and support capture without implying provider commitment where none exists.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Record one durable resolution-memory row per finding-state transition, carrying every field.**
//!   [`resolve_resolution_memory_row_entry`] refuses to read as a clean, registry-bound entry
//!   unless it names a canonical registry token, a classified [lifecycle state][M5ResolutionMemoryRowKind],
//!   an AI-review-assist role, covers every [resolution form][M5ResolutionMemoryRowResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), records every field (finding state,
//!   actor / source, timestamp, rationale class, reopen action, and any provider destination or local-draft
//!   relation), keeps its history attributable to the finding it records, and keeps the row bound to its finding
//!   identity; otherwise it degrades.
//! * **Keep `dismissed` and `suppressed` states distinct and preserve actor / source and rationale class.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a resolution-memory row whose
//!   actor / source / rationale-class join is not preserved (a transition collapsing into a generic hidden state) so
//!   it degrades to
//!   [`M5ResolutionMemoryRowEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`]
//!   — the structured blocker reason a dismissed / suppressed transition that drops its rationale class must surface.
//! * **Join published, outdated, and reopened states back to the original finding and diff scope.**
//!   [`resolve_finding_lifecycle_transition_entry`] names a classified [lifecycle-transition join][M5FindingLifecycleTransitionScope]
//!   (published-transition-joined, outdated-transition-joined, or reopened-transition-joined), requires the full
//!   finding-identity / history / join object, covers every resolution form, and degrades to
//!   [`M5FindingLifecycleTransitionEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the transition would hide the join back to the original finding, or let a missing scope masquerade as a
//!   committed provider state, so later review and support exports can always reconstruct the full lifecycle.
//!
...
//! AI review panel, resolution memory ledger, provider publish review, and support / export surfaces can never fork
//! their own state or lifecycle meaning. Raw secret values and private endpoints stay outside the export boundary.
//!
...
pub use seed::{
    ...
    M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesPacket`].
pub const M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_ai_review_resolution_memory_and_finding_lifecycle_packet_registries";

/// Schema version for M5 line-resolution_memory_row / line-downgrade-packet registry records.
pub const M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_DOC_REF: &str =
    "docs/review/m5_ai_review_resolution_memory_and_finding_lifecycle_registries.md";

/// Repo-relative path of the matrix AI-review-finding domain schema this lane reuses for the B-registry
/// (the finding-lifecycle transition joins a published / outdated / reopened state back to the original
/// finding so later review and support exports can reconstruct the full lifecycle).
pub const M5_AI_REVIEW_FINDING_DOMAIN_SCHEMA_REF: &str =
    "schemas/review/m5-ai-review-finding.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/review/m5-ai-review-resolution-memory-and-finding-lifecycle-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_CSV_REF: &str =
    "artifacts/review/m5-ai-review-resolution-memory-and-finding-lifecycle-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_REPORT_REF: &str =
    "artifacts/review/m5-ai-review-resolution-memory-and-finding-lifecycle-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesConsumerSurface =
    M5AiReviewAssistConsumerSurface;

/// One of the three resolution forms every line-resolution_memory_row or line-downgrade-packet entry must hold across so
...
/// line-resolution_memory_row and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolutionMemoryRowResolutionForm {
    /// The canonical resolved line-resolution_memory_row / line-downgrade-packet object.
    CanonicalObject,
...
impl M5ResolutionMemoryRowResolutionForm {
...
/// Controlled manifest field a resolution-memory-row entry publishes for one retiring object-class identity, so every
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolutionMemoryRowKind {
    ...
    ResolutionStateUnclassified,
}

impl M5ResolutionMemoryRowKind {
    ...
    pub const ALL: [Self; 7] = [
        ...
        Self::Outdated,
        Self::Reopened,
        Self::ResolutionStateUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ResolutionStateUnclassified => "resolution_state_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::ResolutionStateUnclassified)
    }
    ...
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::Open => "open_mode",
            Self::Dismissed => "dismissed_mode",
            Self::Suppressed => "suppressed_mode",
            Self::Published => "published_mode",
            Self::Outdated => "outdated_mode",
            Self::Reopened => "reopened_mode",
            Self::ResolutionStateUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(self, Self::Published | Self::Outdated)
    }
}

/// Controlled finding-lifecycle-transition scope a resolution-memory-row change sits in, so a changed cutoff date or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5FindingLifecycleTransitionScope {
    /// Producer-build-change: a descriptor changed the producer / build identity it was captured from.
    PublishedTransitionJoined,
    /// Target-link-change: a descriptor changed the current live-target reference it hands off to.
    OutdatedTransitionJoined,
    /// Retention-state-change: a descriptor changed its retention / removal (expiry / cleanup) state.
    ReopenedTransitionJoined,
    /// The finding-lifecycle-transition scope is unclassified, which is disallowed.
    LifecycleTransitionUnclassified,
}

impl M5FindingLifecycleTransitionScope {
    /// Every comparison scope, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::PublishedTransitionJoined,
        Self::OutdatedTransitionJoined,
        Self::ReopenedTransitionJoined,
        Self::LifecycleTransitionUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::PublishedTransitionJoined => "published_transition_joined",
            Self::OutdatedTransitionJoined => "outdated_transition_joined",
            Self::ReopenedTransitionJoined => "reopened_transition_joined",
            Self::LifecycleTransitionUnclassified => "lifecycle_transition_unclassified",
        }
    }

    /// Whether the comparison scope is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::LifecycleTransitionUnclassified)
    }
}

/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-resolution_memory_row or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolutionMemoryRowSurfaceContext {
...
impl M5ResolutionMemoryRowSurfaceContext {
...
/// One mandatory rendered part a line-resolution_memory_row or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolutionMemoryRowAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-resolution_memory_row entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-resolution_memory_row entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-resolution_memory_row
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line resolution_memory_row or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5ResolutionMemoryRowAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "finding_lifecycle_transition_fields",
...
/// line resolution_memory_row, a line-downgrade packet, or a degraded line-resolution_memory_row / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolutionMemoryRowNextAction {
    /// Expand the resolved line resolution_memory_row's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5ResolutionMemoryRowNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolutionMemoryRowExportField {
...
impl M5ResolutionMemoryRowExportField {
...
/// Reason a line-resolution_memory_row entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolutionMemoryRowEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the resolution_memory_row means.
    DescriptorTokenUnstated,
    /// The render / surface context cannot currently be resolved.
    SurfaceContextUnresolved,
    /// The line journey is unclassified (not in the resolved taxonomy).
    CohortResolutionStateUnclassified,
    ...
    /// The resolved line-resolution_memory_row object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5ResolutionMemoryRowEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 9] = [
        Self::DescriptorTokenUnstated,
        Self::SurfaceContextUnresolved,
        Self::CohortResolutionStateUnclassified,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "resolution_memory_row_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortResolutionStateUnclassified => "line_resolution_state_unclassified",
            Self::DescriptorNotBoundToRegistry => "resolution_memory_row_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "resolution_memory_row_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "resolution_memory_row_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5ResolutionMemoryRowNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5ResolutionMemoryRowNextAction::TraceCanonicalRegistry
            }
            Self::CohortResolutionStateUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5ResolutionMemoryRowNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5ResolutionMemoryRowNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5ResolutionMemoryRowNextAction::ReviewBlockedOrDegraded,
        }
    }
    ...
    pub const fn downgrade_trigger(self) -> M5AiReviewAssistDowngradeTrigger {
        match self {
            ...
            Self::CohortResolutionStateUnclassified | Self::CohortDescriptorObjectIncomplete => {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5FindingLifecycleTransitionEntryDegradeReason {
...
impl M5FindingLifecycleTransitionEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::EvidenceScopeUnclassified => "comparison_lifecycle_transition_unclassified",
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                "finding_lifecycle_transition_runs_support_ahead_of_proof_or_drops_finding_lifecycle_transition"
            }
    ...
    pub const fn next_action(self) -> M5ResolutionMemoryRowNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5ResolutionMemoryRowNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5ResolutionMemoryRowNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5ResolutionMemoryRowNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_resolution_memory_row_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ResolutionMemoryRowEntryResolutionInput {
    /// Stable identity of the line-resolution_memory_row-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this resolution_memory_row binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.resolution_memory_row.outdated`); empty means unstated.
    ...
    pub report_section: M5ResolutionMemoryRowKind,
    /// The render / surface context.
    pub surface_context: M5ResolutionMemoryRowSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ResolutionMemoryRowResolutionForm>,
    ...
    /// True when the behavior traces to the line-resolution_memory_row registry (never a hand-copied constant).
...
/// Resolved, export-safe line-resolution_memory_row-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedResolutionMemoryRowEntry {
    /// Stable identity of the line-resolution_memory_row-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this resolution_memory_row binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-resolution_memory_row object publishes every required field.
    pub resolution_memory_row_object_complete: bool,
    /// Whether the entry traces to the line-resolution_memory_row registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5ResolutionMemoryRowEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ResolutionMemoryRowNextAction,
    /// Whether the resolution_memory_row resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub resolution_memory_row_resolves_across_lines: bool,
}

impl M5ResolvedResolutionMemoryRowEntry {
    /// Whether this line-resolution_memory_row entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_finding_lifecycle_transition_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5FindingLifecycleTransitionEntryResolutionInput {
    ...
    pub comparison_scope: M5FindingLifecycleTransitionScope,
    /// The render / surface context.
    pub surface_context: M5ResolutionMemoryRowSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ResolutionMemoryRowResolutionForm>,
    ...
    pub keeps_finding_lifecycle_transition_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedFindingLifecycleTransitionEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub finding_lifecycle_transition_stays_honest: bool,
    ...
    pub provides_complete_finding_lifecycle_transition: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5FindingLifecycleTransitionEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ResolutionMemoryRowNextAction,
...
impl M5ResolvedFindingLifecycleTransitionEntry {
...
pub enum M5ResolutionMemoryRowResolutionError {
    /// The line-resolution_memory_row-entry id was empty.
...
impl M5ResolutionMemoryRowResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_resolution_memory_row_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_finding_lifecycle_transition_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5ResolutionMemoryRowResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-resolution_memory_row / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5ResolutionMemoryRowResolutionError {}

fn form_tokens(forms: &[M5ResolutionMemoryRowResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5ResolutionMemoryRowResolutionForm]) -> bool {
    let present: BTreeSet<M5ResolutionMemoryRowResolutionForm> = forms.iter().copied().collect();
    M5ResolutionMemoryRowResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-resolution_memory_row object publishes every required field: classified line journey,
...
pub fn resolution_memory_row_object_is_complete(
    journey: M5ResolutionMemoryRowKind,
...
/// Whether the line resolution_memory_row keeps a line from widening without preserving its rollback and diagnostics
...
pub fn finding_lifecycle_transition_stays_honest(
    scope: M5FindingLifecycleTransitionScope,
    comparison_is_truthful: bool,
    keeps_finding_lifecycle_transition_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_finding_lifecycle_transition_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-resolution_memory_row-registry entry so it stays bound to the line-resolution_memory_row registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete resolution_memory_row object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_resolution_memory_row_entry(
    input: M5ResolutionMemoryRowEntryResolutionInput,
) -> Result<M5ResolvedResolutionMemoryRowEntry, M5ResolutionMemoryRowResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ResolutionMemoryRowResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5ResolutionMemoryRowResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = resolution_memory_row_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ResolutionMemoryRowEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ResolutionMemoryRowEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5ResolutionMemoryRowEntryDegradeReason::CohortResolutionStateUnclassified)
    } else if !input.bound_to_registry {
        Some(M5ResolutionMemoryRowEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5ResolutionMemoryRowEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5ResolutionMemoryRowEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5ResolutionMemoryRowEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(
            M5ResolutionMemoryRowEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    } else if !input.proof_fresh {
        Some(M5ResolutionMemoryRowEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5ResolutionMemoryRowNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedResolutionMemoryRowEntry {
        ...
        resolution_memory_row_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        resolution_memory_row_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_finding_lifecycle_transition_entry(
    input: M5FindingLifecycleTransitionEntryResolutionInput,
) -> Result<M5ResolvedFindingLifecycleTransitionEntry, M5ResolutionMemoryRowResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ResolutionMemoryRowResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = finding_lifecycle_transition_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_finding_lifecycle_transition_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5FindingLifecycleTransitionEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5FindingLifecycleTransitionEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5FindingLifecycleTransitionEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5FindingLifecycleTransitionEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5FindingLifecycleTransitionEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5FindingLifecycleTransitionEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5ResolutionMemoryRowNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedFindingLifecycleTransitionEntry {
        entry_id: input.entry_id,
        ...
        keeps_finding_lifecycle_transition_visible: input
            .keeps_finding_lifecycle_transition_visible,
        ...
        finding_lifecycle_transition_stays_honest: record_stays_honest,
        provides_complete_finding_lifecycle_transition: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-resolution_memory_row and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5ResolutionMemoryRowAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5ResolutionMemoryRowExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5AiReviewAssistDowngradeTrigger>,
    /// Resolved line-resolution_memory_row-registry examples.
    pub resolution_memory_row_entries: Vec<M5ResolvedResolutionMemoryRowEntry>,
    /// Resolved line-downgrade-packet examples.
    pub finding_lifecycle_transition_entries: Vec<M5ResolvedFindingLifecycleTransitionEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-resolution_memory_row and
    ...
    pub collapses_distinct_finding_lifecycle_transition_classes_into_one_lane: bool,
}

impl M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5ResolutionMemoryRowAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5ResolutionMemoryRowAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5ResolutionMemoryRowExportField> =
            self.export_fields.iter().copied().collect();
        M5ResolutionMemoryRowExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_finding_lifecycle_transition_classes_into_one_lane
    }

    /// True when a clean line-resolution_memory_row entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete resolution_memory_row object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn resolution_memory_row_is_honest(ex: &M5ResolvedResolutionMemoryRowEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.resolution_memory_row_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedFindingLifecycleTransitionEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_finding_lifecycle_transition
                && ex.finding_lifecycle_transition_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.resolution_memory_row_entries
            .iter()
            .all(Self::resolution_memory_row_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesVocabularySet {
    ...
    /// Cohort-resolution_memory_row-entry degrade-reason tokens.
    pub resolution_memory_row_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub finding_lifecycle_transition_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5AiReviewAssistRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5ResolutionMemoryRowResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5ResolutionMemoryRowKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5FindingLifecycleTransitionScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5ResolutionMemoryRowSurfaceContext::ALL, |v| v.as_str()),
            resolution_memory_row_degrade_reasons: tokens(
                &M5ResolutionMemoryRowEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            finding_lifecycle_transition_degrade_reasons: tokens(
                &M5FindingLifecycleTransitionEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5ResolutionMemoryRowAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5ResolutionMemoryRowNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5ResolutionMemoryRowExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesGovernanceReview {
    /// The resolution_memory_row registry names a canonical token, semantic role, and line journey for every entry.
    pub resolution_memory_row_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-resolution_memory_row object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_resolution_memory_row_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved resolution_memory_row.
    ...
    pub finding_lifecycle_transition_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-resolution_memory_row and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-resolution_memory_row and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub resolution_memory_row_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-resolution_memory_row registry.
    ...
    /// Behavior traces back to the canonical line-resolution_memory_row and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-resolution_memory_row / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesReleasePosture {
...
/// Constructor input for [`M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection:
        M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesReleasePosture,
...
/// Export-safe M5 line-resolution_memory_row and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesPacket {
    /// Record kind; must equal [`M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_SCHEMA_VERSION`].
...
impl M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesPacketInput) -> Self {
        Self {
            record_kind:
                M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_RECORD_KIND
                    .to_owned(),
            schema_version:
                M5_AI_REVIEW_RESOLUTION_MEMORY_AND_FINDING_LIFECYCLE_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(
        &self,
    ) -> Vec<M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation> {
        ...
        {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::WrongRecordKind,
            );
        }
        ...
        {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::MissingIdentity,
            );
        }
        ...
        if !self.vocabulary_set.matches_canonical() {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::VocabularySetDrift,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-resolution_memory_row / line-downgrade-packet registries packet serializes",
        )) {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::RawMaterialInExport,
            );
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,resolution_memory_row_entries,finding_lifecycle_transition_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            let degrades: Vec<&str> = row
                ...
                .chain(
                    row.finding_lifecycle_transition_entries
                        .iter()
                        .filter_map(|ex| ex.degrade_reason.map(|r| r.as_str())),
                )
                .collect();
            out.push_str(&format!(
                ...
                row.resolution_memory_row_entries.len(),
                row.finding_lifecycle_transition_entries.len(),
                degrades.join("|"),
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Resolution-Memory-Row and Finding-Lifecycle-Transition Registries\n\n");
    ...
    /// registry-bound line-resolution_memory_row entries are listed.
    pub fn render_resolution_memory_row_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.resolution_memory_row_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation>),
}

impl fmt::Display for M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-resolution_memory_row / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-resolution_memory_row / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesArtifactError {}

/// Validation failures emitted by [`M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation {
    ...
    /// A registry row does not point at both the line-resolution_memory_row and line-downgrade-packet domain schemas.
    ...
    /// Cohort-resolution_memory_row-resolution is not proven: clean resolution_memory_row entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean resolution_memory_row entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded resolution_memory_row entry is present, or a clean resolution_memory_row entry is unbounded
    /// or unbound.
...
impl M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "resolution_memory_row_resolution_not_proven"
            }
            ...
            Self::CohortEvidenceIntegrityNotProven => {
                "finding_lifecycle_transition_integrity_not_proven"
            }
...
pub fn current_stable_m5_ai_review_resolution_memory_and_finding_lifecycle_registries_export(
) -> Result<
    M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesPacket,
    M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesArtifactError,
> {
    let packet: M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/review/m5-ai-review-resolution-memory-and-finding-lifecycle-registries-proof/support_export.json"
        )
    ))
    .map_err(M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(
            M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesArtifactError::Validation(
                violations,
            ),
        )
    }
...
fn validate_source_contracts(
    ...
    violations: &mut Vec<M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations
            .push(M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_AI_REVIEW_RESOLUTION_MEMORY_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_AI_REVIEW_FINDING_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.resolution_memory_row_entries.is_empty()
            || row.finding_lifecycle_transition_entries.is_empty()
        {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::ExamplesMissing,
            );
        }
        if !row.examples_are_honest() {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::DishonestExample,
            );
        }
        if !row.honours_invariants() {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.resolution_memory_row_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_resolution_memory_row_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.finding_lifecycle_transition_keeps_proof_visible_and_binds_support_language,
        ...
        review.resolution_memory_row_or_downgrade_drift_caught_before_release,
        review.every_row_declares_mandatory_anatomy,
        review.reuses_frozen_matrix_vocabulary,
    ] {
        if !ok {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let resolution_memory_rows = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.resolution_memory_row_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.finding_lifecycle_transition_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean resolution_memory_row entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean resolution_memory_row entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = resolution_memory_rows()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = resolution_memory_rows()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5ResolutionMemoryRowKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5ResolutionMemoryRowSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = resolution_memory_rows().any(|ex| {
        ex.degrade_reason
            == Some(M5ResolutionMemoryRowEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete = !resolution_memory_rows()
        .any(|ex| ex.is_clean() && !ex.resolution_memory_row_object_complete);
    if !(journeys_covered
    ...
    {
        violations.push(
            M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }

    // AC2: line packets preserve rollback and diagnostics posture before widening. A widen-without-rollback
    // example degrades, an unbound example degrades, at least one clean bounded resolution_memory_row entry is present, and
    // no clean resolution_memory_row entry is unbounded or unbound.
    let widen_fold_degrades = resolution_memory_rows().any(|ex| {
        ex.degrade_reason
            == Some(
                M5ResolutionMemoryRowEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = resolution_memory_rows().any(|ex| {
        ex.degrade_reason
            == Some(M5ResolutionMemoryRowEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_resolution_memory_row =
        resolution_memory_rows().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound =
        !resolution_memory_rows().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded =
        !resolution_memory_rows().any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_resolution_memory_row
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
            M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5FindingLifecycleTransitionScope::CANONICAL_SCOPES
        .iter()
    ...
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_finding_lifecycle_transition);
    ...
    {
        violations.push(
            M5AiReviewResolutionMemoryAndFindingLifecycleRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-resolution_memory_row registry
