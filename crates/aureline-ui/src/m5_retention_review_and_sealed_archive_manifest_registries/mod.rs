//! Implemented M5 retention-review and sealed-archive-manifest registries.
//!
//! The frozen [collaboration-control component matrix][matrix] names Aureline's governed sensitive-collaboration
//! object classes and locks their controlled vocabulary. This module makes the explicit retention review
//! operable — as a canonical record — and adds the sealed-archive manifest that names, by content-addressed and
//! policy-labeled identity, exactly what a session retained, so the desktop shared-terminal / debug surface,
//! the companion-follow flow, and the support / export and help / docs surfaces inherit one canonical retention
//! descriptor rather than a hand-authored parallel prose that has to be kept consistent: it turns the
//! *retention-review* grammar (one durable retention record per session — one typed field per
//! record section: the retention mode (live-only, metadata audit, replayable text / comment timeline, or
//! elevated support / regulatory evidence), the disclosed retention envelope, the export / delete-right
//! posture, and the visible retention badge kept distinct from ordinary presence
//! and follow state — each bound to one session / retention scope) and the *sealed-archive-manifest* grammar (the
//! content-addressed, policy-labeled manifest a participant reads before any archive is trusted, and the fresh visible consent event any
//! retention change, guest-scope widening, or archive creation must raise — a disclose, a consent-renewal, a
//! guest-scope-widen, an export / delete-right change, or a sealed-archive event that must not copy a raw
//! session body into logs or exports) into registry
//! resolvers that produce export-safe, honest projections. Every session then resolves to one typed
//! retention review — its retention mode, its disclosed retention envelope, its export / delete-right posture and
//! visible badge, all attributable to the session so retention never widens silently — and to one
//! sealed-archive-manifest object — the resolved archive identity, the content-address and policy label, the retention
//! posture, and the export / delete / sealed outcome — that the shared terminal / debug, companion, and support surfaces
//! can inspect without manual reconstruction, so no archive ever reads as a generic "session recording saved" without disclosing
//! what is retained, exportable, and deletable and the retention history stays visible and exportable, and a review that cannot bind a field to its
//! session / retention scope degrades honestly instead of letting a retention change read as pre-approved when it is not.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable retention review per session, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [retention mode][M5RetentionReviewRecordKind],
//!   a disclosure role, covers every [resolution form][M5RetentionReviewRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (disclosed retention envelope
//!   and target context, retention mode, reason and export / delete-right posture, and the visible-badge
//!   binding bound to the session identity), keeps its retention lineage attributable to the session, and
//!   keeps an export / delete-right field bound to its retention state; otherwise it degrades.
//! * **Keep the sealed-archive manifest honest so a retention change never begins silently rather than a
//!   hand-authored parallel prose staying consistent.** [`line_preserves_rollback_and_diagnostics_before_widening`]
//!   rejects a review whose visible-badge binding is not preserved (a retention change presenting itself as
//!   already-consented from presence alone) so it degrades to
//!   [`M5RetentionReviewRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a manifest whose language outruns its retention state degrades the same
//!   way — the structured blocker reason a retain-or-archive-without-consent attempt must surface.
//! * **Turn a disclose, a consent-renewal, a guest-scope-widen, an export / delete-right change, or a sealed-archive outcome into a visible, typed consent event
//!   instead of a silent retention change.** [`resolve_review_pack_result_entry`] names a classified
//!   [archive binding][M5RetentionReviewResultScope]
//!   (content-address binding, policy-label binding, or export / delete / sealed binding), requires the full
//!   archive-identity / affected-retention-field / previous-versus-current-retention-state / binding / active-reason
//!   manifest object, covers every resolution form, and degrades to
//!   [`M5RetentionReviewResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the change would present a sealed archive as generic, hide a policy label on a sensitive surface, or let a
//!   retention-change or guest-scope-widen event drop its attribution, so an archive can never read as trustworthy when
//!   it has quietly dropped the reason a fresh visible consent event was required.
//!
...
//! can never fork their own retention-review meaning. Raw session bodies, raw command text, raw variable bodies, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_retention_review_and_sealed_archive_manifest_registries,
    seeded_m5_retention_review_and_sealed_archive_manifest_registries_review_pack_record_beta_narrowed,
    seeded_m5_retention_review_and_sealed_archive_manifest_registries_review_pack_result_preview_narrowed,
    M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_PACKET_ID,
};
...
use crate::m5_shared_terminal_debug_control_grant_presenter_consent_retention_and_session_restore_view_matrix::{
    ...
    M5_RETENTION_REVIEW_DOMAIN_SCHEMA_REF, M5_COLLABORATION_CONTROL_MATRIX_DOC_REF,
    M5_COLLABORATION_CONTROL_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5RetentionReviewRecordAndResultRegistriesPacket`].
pub const M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_retention_review_and_sealed_archive_manifest_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_retention_review_and_sealed_archive_manifest_registries.md";

/// Repo-relative path of the sealed-archive-manifest domain schema this lane binds its B object to (the
/// content-addressed, policy-labeled manifest describing a retained session archive, its audit-safe attribution,
/// and the retention posture it projects — live-only, metadata audit, replayable text / comment timeline, or
/// elevated support / regulatory evidence — kept distinct from any raw session body). Minted by this lane; the
/// sealed-archive manifest and the retention review never fork their retention vocabulary, and no raw session
/// body, command text, variable body, or clipboard content crosses this boundary.
pub const M5_SEALED_ARCHIVE_MANIFEST_DOMAIN_SCHEMA_REF: &str =
    "schemas/collaboration/m5-sealed-archive-manifest.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-retention-review-and-sealed-archive-manifest-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-retention-review-and-sealed-archive-manifest-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-retention-review-and-sealed-archive-manifest-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-retention-review-and-sealed-archive-manifest-registries";
...
pub type M5RetentionReviewRecordAndResultRegistriesConsumerSurface =
...
pub enum M5RetentionReviewRecordResolutionForm {
...
impl M5RetentionReviewRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetentionReviewRecordKind {
...
impl M5RetentionReviewRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetentionReviewResultScope {
...
impl M5RetentionReviewResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetentionReviewRecordSurfaceContext {
...
impl M5RetentionReviewRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetentionReviewRecordAnatomyPart {
...
impl M5RetentionReviewRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetentionReviewRecordNextAction {
...
impl M5RetentionReviewRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetentionReviewRecordExportField {
...
impl M5RetentionReviewRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetentionReviewRecordEntryDegradeReason {
...
impl M5RetentionReviewRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5RetentionReviewRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5RetentionReviewRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5RetentionReviewRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5RetentionReviewRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5RetentionReviewRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetentionReviewResultEntryDegradeReason {
...
impl M5RetentionReviewResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5RetentionReviewRecordNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5RetentionReviewRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5RetentionReviewRecordEntryResolutionInput {
    ...
    pub report_section: M5RetentionReviewRecordKind,
    /// The render / surface context.
    pub surface_context: M5RetentionReviewRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5RetentionReviewRecordResolutionForm>,
...
pub struct M5ResolvedRetentionReviewRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5RetentionReviewRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5RetentionReviewRecordNextAction,
...
impl M5ResolvedRetentionReviewRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5RetentionReviewResultEntryResolutionInput {
    ...
    pub comparison_scope: M5RetentionReviewResultScope,
    /// The render / surface context.
    pub surface_context: M5RetentionReviewRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5RetentionReviewRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedRetentionReviewResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5RetentionReviewResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5RetentionReviewRecordNextAction,
...
impl M5ResolvedRetentionReviewResultEntry {
...
pub enum M5RetentionReviewRecordResolutionError {
...
impl M5RetentionReviewRecordResolutionError {
...
impl fmt::Display for M5RetentionReviewRecordResolutionError {
...
impl Error for M5RetentionReviewRecordResolutionError {}

fn form_tokens(forms: &[M5RetentionReviewRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5RetentionReviewRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5RetentionReviewRecordResolutionForm> = forms.iter().copied().collect();
    M5RetentionReviewRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5RetentionReviewRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5RetentionReviewResultScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedRetentionReviewRecordEntry, M5RetentionReviewRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5RetentionReviewRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5RetentionReviewRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5RetentionReviewRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5RetentionReviewRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5RetentionReviewRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5RetentionReviewRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5RetentionReviewRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5RetentionReviewRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5RetentionReviewRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5RetentionReviewRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5RetentionReviewRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5RetentionReviewRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedRetentionReviewRecordEntry {
...
pub fn resolve_review_pack_result_entry(
...
) -> Result<M5ResolvedRetentionReviewResultEntry, M5RetentionReviewRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5RetentionReviewRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5RetentionReviewResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5RetentionReviewResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5RetentionReviewResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5RetentionReviewResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5RetentionReviewResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5RetentionReviewResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5RetentionReviewRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedRetentionReviewResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetentionReviewRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5RetentionReviewRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5RetentionReviewRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5RetentionReviewRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedRetentionReviewRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedRetentionReviewResultEntry>,
...
impl M5RetentionReviewRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5RetentionReviewRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5RetentionReviewRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5RetentionReviewRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5RetentionReviewRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedRetentionReviewRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedRetentionReviewResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetentionReviewRecordAndResultRegistriesVocabularySet {
...
impl M5RetentionReviewRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5CollaborationControlRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5RetentionReviewRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5RetentionReviewRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5RetentionReviewResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5RetentionReviewRecordSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5RetentionReviewRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5RetentionReviewResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5RetentionReviewRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5RetentionReviewRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5RetentionReviewRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetentionReviewRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetentionReviewRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetentionReviewRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetentionReviewRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5RetentionReviewRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5RetentionReviewRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5RetentionReviewRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5RetentionReviewRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5RetentionReviewRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5RetentionReviewRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5RetentionReviewRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5RetentionReviewRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetentionReviewRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_SCHEMA_VERSION`].
...
impl M5RetentionReviewRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5RetentionReviewRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version:
                M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5RetentionReviewRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind
            != M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_RECORD_KIND
        {
            violations.push(M5RetentionReviewRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_RETENTION_REVIEW_AND_SEALED_ARCHIVE_MANIFEST_REGISTRIES_SCHEMA_VERSION
        {
            violations
                .push(M5RetentionReviewRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5RetentionReviewRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations
                .push(M5RetentionReviewRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations
                .push(M5RetentionReviewRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Retention-Review and Sealed-Archive-Manifest Registries\n\n");
...
#[derive(Debug)]
pub enum M5RetentionReviewRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5RetentionReviewRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5RetentionReviewRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5RetentionReviewRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5RetentionReviewRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5RetentionReviewRecordAndResultRegistriesViolation {
...
impl M5RetentionReviewRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_retention_review_and_sealed_archive_manifest_registries_export() -> Result<
    M5RetentionReviewRecordAndResultRegistriesPacket,
    M5RetentionReviewRecordAndResultRegistriesArtifactError,
> {
    let packet: M5RetentionReviewRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-retention-review-and-sealed-archive-manifest-registries-proof/support_export.json"
        )
    ))
    .map_err(M5RetentionReviewRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5RetentionReviewRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5RetentionReviewRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5RetentionReviewRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations
                .push(M5RetentionReviewRecordAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5RetentionReviewRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations
                .push(M5RetentionReviewRecordAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5RetentionReviewRecordAndResultRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5RetentionReviewRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_RETENTION_REVIEW_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_SEALED_ARCHIVE_MANIFEST_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5RetentionReviewRecordAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5RetentionReviewRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5RetentionReviewRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5RetentionReviewRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5RetentionReviewRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5RetentionReviewRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5RetentionReviewRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5RetentionReviewRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5RetentionReviewRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5RetentionReviewRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5RetentionReviewRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5RetentionReviewRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5RetentionReviewResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(
            M5RetentionReviewRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
