...
fn clean_review_pack_record_input() -> M5RetentionReviewRecordEntryResolutionInput {
    M5RetentionReviewRecordEntryResolutionInput {
        ...
        report_section: M5RetentionReviewRecordKind::ChangedFilesScope,
        surface_context: M5RetentionReviewRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5RetentionReviewRecordResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5RetentionReviewResultEntryResolutionInput {
    M5RetentionReviewResultEntryResolutionInput {
        ...
        semantic_role: M5CollaborationControlRole::ControlAuthorityDisclosure,
        comparison_scope: M5RetentionReviewResultScope::EvaluatedScopeBinding,
        surface_context: M5RetentionReviewRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5RetentionReviewRecordResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_retention_review_and_sealed_archive_manifest_registries();
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(
        ...
        M5RetentionReviewRecordNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5RetentionReviewRecordKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_object_incomplete_and_widen_fold_and_form_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.resolution_form_coverage = vec![M5RetentionReviewRecordResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5RetentionReviewRecordKind::FullTreeScope;
    ...
    let mut input = clean_review_pack_record_input();
    input.surface_context = M5RetentionReviewRecordSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        ...
        M5RetentionReviewRecordResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input).unwrap_err(),
        M5RetentionReviewRecordResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5RetentionReviewRecordKind::PackScopeUnclassified,
        true,
        false,
        true
    ));
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5RetentionReviewResultScope::ChangeObjectResultUnclassified;
    assert_eq!(
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    assert_eq!(
        ...
        M5RetentionReviewRecordResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RetentionReviewRecordAndResultRegistriesViolation::VocabularySetDrift));
}
...
fn missing_source_contracts_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RetentionReviewRecordAndResultRegistriesViolation::MissingSourceContracts));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_RETENTION_REVIEW_DOMAIN_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5RetentionReviewRecordAndResultRegistriesViolation::DomainSchemaRefMissing));

    let mut packet = seeded_m5_retention_review_and_sealed_archive_manifest_registries();
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_SEALED_ARCHIVE_MANIFEST_DOMAIN_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5RetentionReviewRecordAndResultRegistriesViolation::DomainSchemaRefMissing));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5RetentionReviewRecordAnatomyPart::Identity);
    assert!(packet
        .validate()
        .contains(&M5RetentionReviewRecordAndResultRegistriesViolation::MandatoryAnatomyMissing));
}
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5RetentionReviewRecordExportField::CohortArchetypes);
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RetentionReviewRecordAndResultRegistriesViolation::ExamplesMissing));
}
...
fn dishonest_clean_example_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RetentionReviewRecordAndResultRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        assert!(packet
            .validate()
            .contains(&M5RetentionReviewRecordAndResultRegistriesViolation::RowInvariantViolated));
    }
}
...
fn proof_freshness_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RetentionReviewRecordAndResultRegistriesViolation::ProofFreshnessIncomplete));
}
...
fn release_posture_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RetentionReviewRecordAndResultRegistriesViolation::ReleasePostureIncomplete));
}
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet
        .validate()
        .contains(&M5RetentionReviewRecordAndResultRegistriesViolation::RawMaterialInExport));
}

#[test]
fn export_carries_no_forbidden_raw_material() {
    let json =
        seeded_m5_retention_review_and_sealed_archive_manifest_registries().export_safe_json();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_retention_review_and_sealed_archive_manifest_registries_export()
        .expect(
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5RetentionReviewRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-retention-review-and-sealed-archive-manifest-registries/retention_review_beta_narrowed.json"
    )))
    ...
    let preview: M5RetentionReviewRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-retention-review-and-sealed-archive-manifest-registries/sealed_archive_manifest_preview_narrowed.json"
    )))
