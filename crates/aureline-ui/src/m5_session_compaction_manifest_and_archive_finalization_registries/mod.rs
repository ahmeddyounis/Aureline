//! Implemented M5 session-compaction-manifest and archive-finalization registries.
//!
//! The frozen [collaboration-state authority matrix][matrix] names Aureline's governed shared-object classes and
//! locks their authority, convergence, and export vocabulary. Building on the CRDT-backed shared-text replica-state
//! domain (the CRDT replica of shared session text and the local buffer it never replaces) and the sealed
//! session-archive compaction-manifest domain it freezes, this module makes collaboration *state growth* as honest
//! and reviewable as other Aureline evidence objects: when the CRDT-backed session text has to be compacted — to
//! bound op-log and tombstone growth after a buffer edit, generated output refresh, or imported snapshot transition
//! — its snapshot ID, retained-object references, and tombstone / op-log bounds are recorded first, and it lands in
//! an explicit compacted / retained / tombstoned state rather than reclaiming storage as hidden churn. So the shared
//! editor replica view, the presence / cursor layer, the comment / annotation / review-pin layer, the collaboration
//! degradation banner, and the support / export and help / docs surfaces inherit one canonical
//! session-compaction-manifest record rather than a hand-authored parallel prose that has to be kept consistent: it
//! turns the *compaction-manifest* grammar (one durable append-only record per compaction — one typed field per
//! record section: the snapshot ID, the retained-object references it keeps, the tombstone / op-log bounds it draws,
//! the actor provenance of the compaction, and its export posture, each bound to the compaction lineage it never
//! rewrites) and the *archive-finalization* grammar (the finalization decision offered on a compacted session —
//! finalize-archive, export-snapshot, export-op-log, redact-and-finalize, or leave-open — plus its actor / time
//! provenance, its compaction / redaction / omission facts, and its policy-labeled export lineage) into registry
//! resolvers that produce export-safe, honest projections. Every compaction then resolves to one typed
//! session-compaction-manifest record — the fields it publishes, the snapshot identity, the retained-object
//! references and tombstone / op-log bounds, the compacted / retained / tombstoned state, and its export posture,
//! all attributable so state is never reclaimed implicitly — and to one archive-finalization descriptor — the
//! decision, its actor / time provenance, and its export lineage — that the shared editor, presence, review-pin,
//! degradation-banner, and support surfaces can inspect before rejoining, exporting, or discarding, so a finalization
//! that cannot bind its compaction lineage degrades honestly instead of letting a compaction read as clean when
//! retained state was silently reclaimed without a recorded, inspectable snapshot / manifest lineage.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable session-compaction-manifest record per compaction, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [compacted / retained state][M5SessionCompactionArchiveRecordKind],
//!   a disclosure role, covers every [resolution form][M5SessionCompactionArchiveRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (the
//!   snapshot ID, the retained-object references, the tombstone / op-log bounds, the
//!   compacted / retained / tombstoned state, and the export posture bound to the snapshot identity), keeps its lineage
//!   attributable, and keeps the compaction lineage it never rewrites bound to the record; otherwise it degrades.
//! * **Never reclaim state as hidden churn; preserve inspectable snapshot / manifest lineage first on any compaction.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a record whose
//!   compaction-lineage preservation is not held (a compaction presenting itself as clean while retained state was
//!   reclaimed as hidden storage churn) so it degrades to
//!   [`M5SessionCompactionArchiveRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and any compaction — after a buffer edit, generated-output refresh, or imported-snapshot transition — that
//!   would reclaim state without snapshot / manifest lineage degrades the same way — the structured blocker reason a
//!   hidden-storage-churn attempt must surface.
//! * **Offer a reviewable finalization decision with actor / time provenance, instead of a silent auto-finalize.**
//!   [`resolve_review_pack_result_entry`] names a classified
//!   [finalization binding][M5SessionCompactionArchiveResultScope]
//!   (archive-finalization binding, snapshot-export binding, or leave-open binding), requires the full
//!   snapshot-identity / finalization-decision / actor-time-provenance / binding / active-reason
//!   record, covers every resolution form, and degrades to
//!   [`M5SessionCompactionArchiveResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the finalization would seal or export a session without review, act without actor / time provenance, or let an
//!   export drop its redaction lineage, so a finalization can never read as trustworthy when it has quietly sealed
//!   or exported the session or dropped its provenance.
//!
...
//! export surfaces can never fork their own compaction / finalization meaning. Raw secrets, raw command text, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_session_compaction_manifest_and_archive_finalization_registries,
    seeded_m5_session_compaction_manifest_and_archive_finalization_registries_review_pack_record_beta_narrowed,
    seeded_m5_session_compaction_manifest_and_archive_finalization_registries_review_pack_result_preview_narrowed,
    M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_PACKET_ID,
};
...
use crate::m5_collaboration_replica_shared_object_authority_anchor_drift_convergence_and_session_archive_matrix::{
    ...
    M5_CRDT_BACKED_SHARED_TEXT_DOMAIN_SCHEMA_REF, M5_SEALED_SESSION_ARCHIVE_DOMAIN_SCHEMA_REF,
    M5_COLLABORATION_STATE_MATRIX_DOC_REF, M5_COLLABORATION_STATE_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5SessionCompactionArchiveRecordAndResultRegistriesPacket`].
pub const M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_session_compaction_manifest_and_archive_finalization_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_SCHEMA_VERSION: u32 =
    1;

/// Repo-relative path of the combined registries schema.
pub const M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_session_compaction_manifest_and_archive_finalization_registries.md";

/// Repo-relative path of the checked support-export artifact.
pub const M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-session-compaction-manifest-and-archive-finalization-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-session-compaction-manifest-and-archive-finalization-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-session-compaction-manifest-and-archive-finalization-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-session-compaction-manifest-and-archive-finalization-registries";
...
pub type M5SessionCompactionArchiveRecordAndResultRegistriesConsumerSurface =
...
pub enum M5SessionCompactionArchiveRecordResolutionForm {
...
impl M5SessionCompactionArchiveRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionCompactionArchiveRecordKind {
...
impl M5SessionCompactionArchiveRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionCompactionArchiveResultScope {
...
impl M5SessionCompactionArchiveResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionCompactionArchiveRecordSurfaceContext {
...
impl M5SessionCompactionArchiveRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionCompactionArchiveRecordAnatomyPart {
...
impl M5SessionCompactionArchiveRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionCompactionArchiveRecordNextAction {
...
impl M5SessionCompactionArchiveRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionCompactionArchiveRecordExportField {
...
impl M5SessionCompactionArchiveRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionCompactionArchiveRecordEntryDegradeReason {
...
impl M5SessionCompactionArchiveRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5SessionCompactionArchiveRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5SessionCompactionArchiveRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5SessionCompactionArchiveRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5SessionCompactionArchiveRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => {
                M5SessionCompactionArchiveRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SessionCompactionArchiveResultEntryDegradeReason {
...
impl M5SessionCompactionArchiveResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
...
pub struct M5SessionCompactionArchiveRecordEntryResolutionInput {
    ...
    pub report_section: M5SessionCompactionArchiveRecordKind,
    /// The render / surface context.
    pub surface_context: M5SessionCompactionArchiveRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SessionCompactionArchiveRecordResolutionForm>,
...
pub struct M5ResolvedSessionCompactionArchiveRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SessionCompactionArchiveRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SessionCompactionArchiveRecordNextAction,
...
impl M5ResolvedSessionCompactionArchiveRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SessionCompactionArchiveResultEntryResolutionInput {
    ...
    pub comparison_scope: M5SessionCompactionArchiveResultScope,
    /// The render / surface context.
    pub surface_context: M5SessionCompactionArchiveRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SessionCompactionArchiveRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedSessionCompactionArchiveResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SessionCompactionArchiveResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SessionCompactionArchiveRecordNextAction,
...
impl M5ResolvedSessionCompactionArchiveResultEntry {
...
pub enum M5SessionCompactionArchiveRecordResolutionError {
...
impl M5SessionCompactionArchiveRecordResolutionError {
...
impl fmt::Display for M5SessionCompactionArchiveRecordResolutionError {
...
impl Error for M5SessionCompactionArchiveRecordResolutionError {}

fn form_tokens(forms: &[M5SessionCompactionArchiveRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5SessionCompactionArchiveRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5SessionCompactionArchiveRecordResolutionForm> =
        forms.iter().copied().collect();
    M5SessionCompactionArchiveRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5SessionCompactionArchiveRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5SessionCompactionArchiveResultScope,
...
pub fn resolve_review_pack_record_entry(
    input: M5SessionCompactionArchiveRecordEntryResolutionInput,
) -> Result<
    M5ResolvedSessionCompactionArchiveRecordEntry,
    M5SessionCompactionArchiveRecordResolutionError,
> {
    if input.entry_id.trim().is_empty() {
        return Err(M5SessionCompactionArchiveRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5SessionCompactionArchiveRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SessionCompactionArchiveRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SessionCompactionArchiveRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5SessionCompactionArchiveRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5SessionCompactionArchiveRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5SessionCompactionArchiveRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5SessionCompactionArchiveRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5SessionCompactionArchiveRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5SessionCompactionArchiveRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5SessionCompactionArchiveRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5SessionCompactionArchiveRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedSessionCompactionArchiveRecordEntry {
...
pub fn resolve_review_pack_result_entry(
    input: M5SessionCompactionArchiveResultEntryResolutionInput,
) -> Result<
    M5ResolvedSessionCompactionArchiveResultEntry,
    M5SessionCompactionArchiveRecordResolutionError,
> {
    if input.entry_id.trim().is_empty() {
        return Err(
            M5SessionCompactionArchiveRecordResolutionError::EmptyCohortEvidencePacketEntryId,
        );
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SessionCompactionArchiveResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SessionCompactionArchiveResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5SessionCompactionArchiveResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5SessionCompactionArchiveResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5SessionCompactionArchiveResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5SessionCompactionArchiveResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5SessionCompactionArchiveRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedSessionCompactionArchiveResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionCompactionArchiveRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5SessionCompactionArchiveRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5SessionCompactionArchiveRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5SessionCompactionArchiveRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedSessionCompactionArchiveRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedSessionCompactionArchiveResultEntry>,
...
impl M5SessionCompactionArchiveRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5SessionCompactionArchiveRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5SessionCompactionArchiveRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5SessionCompactionArchiveRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5SessionCompactionArchiveRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedSessionCompactionArchiveRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedSessionCompactionArchiveResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionCompactionArchiveRecordAndResultRegistriesVocabularySet {
...
impl M5SessionCompactionArchiveRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5CollaborationStateRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5SessionCompactionArchiveRecordResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5SessionCompactionArchiveRecordKind::ALL, |v| {
                v.as_str()
            }),
            comparison_scopes: tokens(&M5SessionCompactionArchiveResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5SessionCompactionArchiveRecordSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            review_pack_record_degrade_reasons: tokens(
                &M5SessionCompactionArchiveRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5SessionCompactionArchiveResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5SessionCompactionArchiveRecordAnatomyPart::ALL, |v| {
                v.as_str()
            }),
            next_actions: tokens(&M5SessionCompactionArchiveRecordNextAction::ALL, |v| {
                v.as_str()
            }),
            export_fields: tokens(&M5SessionCompactionArchiveRecordExportField::ALL, |v| {
                v.as_str()
            }),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionCompactionArchiveRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionCompactionArchiveRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionCompactionArchiveRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionCompactionArchiveRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5SessionCompactionArchiveRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SessionCompactionArchiveRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5SessionCompactionArchiveRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5SessionCompactionArchiveRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5SessionCompactionArchiveRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5SessionCompactionArchiveRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5SessionCompactionArchiveRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5SessionCompactionArchiveRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SessionCompactionArchiveRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_SCHEMA_VERSION`].
...
impl M5SessionCompactionArchiveRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5SessionCompactionArchiveRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind:
                M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_RECORD_KIND
                    .to_owned(),
            schema_version:
                M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5SessionCompactionArchiveRecordAndResultRegistriesViolation> {
        ...
        {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::WrongRecordKind,
            );
        }
        ...
        {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::MissingIdentity,
            );
        }
        ...
        if !self.vocabulary_set.matches_canonical() {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::VocabularySetDrift,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::RawMaterialInExport,
            );
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Session-Compaction-Manifest and Archive-Finalization Registries\n\n");
...
#[derive(Debug)]
pub enum M5SessionCompactionArchiveRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5SessionCompactionArchiveRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5SessionCompactionArchiveRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5SessionCompactionArchiveRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5SessionCompactionArchiveRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5SessionCompactionArchiveRecordAndResultRegistriesViolation {
...
impl M5SessionCompactionArchiveRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_session_compaction_manifest_and_archive_finalization_registries_export(
) -> Result<
    M5SessionCompactionArchiveRecordAndResultRegistriesPacket,
    M5SessionCompactionArchiveRecordAndResultRegistriesArtifactError,
> {
    let packet: M5SessionCompactionArchiveRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-session-compaction-manifest-and-archive-finalization-registries-proof/support_export.json"
        )
    ))
    .map_err(M5SessionCompactionArchiveRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(
            M5SessionCompactionArchiveRecordAndResultRegistriesArtifactError::Validation(
                violations,
            ),
        )
    }
...
fn validate_source_contracts(
    packet: &M5SessionCompactionArchiveRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5SessionCompactionArchiveRecordAndResultRegistriesViolation>,
) {
    let refs: BTreeSet<&str> = packet
        ...
        .collect();
    for required in [
        M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_SCHEMA_REF,
        M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_DOC_REF,
        ...
        if !refs.contains(required) {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations
            .push(M5SessionCompactionArchiveRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_CRDT_BACKED_SHARED_TEXT_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_SEALED_SESSION_ARCHIVE_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::ExamplesMissing,
            );
        }
        if !row.examples_are_honest() {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::DishonestExample,
            );
        }
        if !row.honours_invariants() {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SessionCompactionArchiveRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5SessionCompactionArchiveRecordAndResultRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5SessionCompactionArchiveRecordAndResultRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5SessionCompactionArchiveRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5SessionCompactionArchiveRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5SessionCompactionArchiveRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    let unbound_degrades = review_pack_records().any(|ex| {
        ex.degrade_reason
            == Some(
                M5SessionCompactionArchiveRecordEntryDegradeReason::DescriptorNotBoundToRegistry,
            )
    });
    ...
    {
        violations.push(
            M5SessionCompactionArchiveRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5SessionCompactionArchiveResultScope::CANONICAL_SCOPES
        .iter()
    ...
    let form_incomplete_degrades = downgrade().any(|ex| {
        ex.degrade_reason
            == Some(
                M5SessionCompactionArchiveResultEntryDegradeReason::EvidenceFormCoverageIncomplete,
            )
    });
    ...
    {
        violations.push(
            M5SessionCompactionArchiveRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
