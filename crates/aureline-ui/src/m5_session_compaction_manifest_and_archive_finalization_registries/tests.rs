...
fn clean_review_pack_record_input() -> M5SessionCompactionArchiveRecordEntryResolutionInput {
    M5SessionCompactionArchiveRecordEntryResolutionInput {
        ...
        semantic_role: M5CollaborationStateRole::AuthorityModelDisclosure,
        report_section: M5SessionCompactionArchiveRecordKind::ChangedFilesScope,
        surface_context: M5SessionCompactionArchiveRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SessionCompactionArchiveRecordResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5SessionCompactionArchiveResultEntryResolutionInput {
    M5SessionCompactionArchiveResultEntryResolutionInput {
        ...
        semantic_role: M5CollaborationStateRole::AuthorityModelDisclosure,
        comparison_scope: M5SessionCompactionArchiveResultScope::EvaluatedScopeBinding,
        surface_context: M5SessionCompactionArchiveRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SessionCompactionArchiveRecordResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_session_compaction_manifest_and_archive_finalization_registries();
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(resolved.semantic_role, "authority_model_disclosure");
    ...
    assert_eq!(
        ...
        M5SessionCompactionArchiveRecordNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5SessionCompactionArchiveRecordKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5SessionCompactionArchiveRecordKind::FullTreeScope;
    ...
    let mut input = clean_review_pack_record_input();
    input.surface_context = M5SessionCompactionArchiveRecordSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        ...
        M5SessionCompactionArchiveRecordResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input).unwrap_err(),
        M5SessionCompactionArchiveRecordResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5SessionCompactionArchiveRecordKind::PackScopeUnclassified,
        true,
        false,
        true
    ));
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5SessionCompactionArchiveResultScope::ChangeObjectResultUnclassified;
    assert_eq!(
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_CRDT_BACKED_SHARED_TEXT_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_SEALED_SESSION_ARCHIVE_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5SessionCompactionArchiveRecordAndResultRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5SessionCompactionArchiveRecordAnatomyPart::Identity);
    assert!(packet.validate().contains(
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5SessionCompactionArchiveRecordExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5SessionCompactionArchiveRecordAndResultRegistriesViolation::ExamplesMissing));
}
...
fn dishonest_clean_example_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5SessionCompactionArchiveRecordAndResultRegistriesViolation::DishonestExample));
}
...
fn review_pack_record_not_proven_when_incomplete_example_removed() {
    ...
    for row in &mut packet.registry_rows {
        row.review_pack_record_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
                    M5SessionCompactionArchiveRecordEntryDegradeReason::CohortDescriptorObjectIncomplete,
                )
        });
    }
    assert!(packet.validate().contains(
...
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_session_compaction_manifest_and_archive_finalization_registries()
        .export_safe_json();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_session_compaction_manifest_and_archive_finalization_registries_export()
        .expect(
...
fn narrowed_variants_validate_and_keep_rows_visible() {
    ...
    let row = beta
        ...
        .iter()
        .find(|r| {
            r.consumer_surface == M5CollaborationStateConsumerSurface::SharedEditorReplicaView
        })
        .unwrap();
    assert_eq!(
        row.qualification,
    ...
    let row = preview
        .registry_rows
        .iter()
        .find(|r| r.consumer_surface == M5CollaborationStateConsumerSurface::PresenceCursorLayer)
        .unwrap();
    assert_eq!(
        row.qualification,
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5SessionCompactionArchiveRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-session-compaction-manifest-and-archive-finalization-registries/compaction_manifest_beta_narrowed.json"
    )))
    ...
    let preview: M5SessionCompactionArchiveRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-session-compaction-manifest-and-archive-finalization-registries/archive_finalization_preview_narrowed.json"
    )))
...
fn implemented_lines_is_all_six_constrained_file_state_object_classes() {
    assert_eq!(IMPLEMENTED_LINES, M5CollaborationStateObject::ALL);
