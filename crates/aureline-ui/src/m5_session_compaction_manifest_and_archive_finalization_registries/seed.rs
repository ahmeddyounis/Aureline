//! Canonical seed builders for the M5 session-compaction-manifest and archive-finalization registries packet.
//!
//! These builders are the single producer of the checked-in support export and the narrowed fixtures.
//! The headless emitter and the inline tests both call them so the in-code registries, the artifact, and the
//! fixtures never drift. Every resolved example is built by calling the real resolvers so the packet can only
//! carry projections the resolvers actually produce. Clean session-compaction-manifest and
//! session-archive-finalization entries are built so the one typed compaction manifest resolving per compaction,
//! records never letting a compaction read as clean while retained state was reclaimed as hidden storage churn
//! without inspectable snapshot / manifest lineage, finalization descriptors never acting without actor / time
...
//! archive-finalization object are proven across the shared-editor-replica-view, presence / cursor layer,
//! comment / annotation / review-pin layer, collaboration degradation banner, help / docs, and support / export
//! surfaces without any hand-copied per-compaction assumption, hidden-storage-churn-shown-as-clean, incomplete
//! object, provenance-free finalization, or resolution-form gap.

use super::*;

/// Stable packet id for the canonical registries packet.
pub const M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_PACKET_ID: &str =
    "m5-session-compaction-manifest-and-archive-finalization-registries:stable:0001";
...
fn all_forms() -> Vec<M5SessionCompactionArchiveRecordResolutionForm> {
    M5SessionCompactionArchiveRecordResolutionForm::ALL.to_vec()
}
...
fn clean_review_pack_record_base(
    ...
    report_section: M5SessionCompactionArchiveRecordKind,
    surface_context: M5SessionCompactionArchiveRecordSurfaceContext,
...
) -> M5SessionCompactionArchiveRecordEntryResolutionInput {
    M5SessionCompactionArchiveRecordEntryResolutionInput {
...
fn review_pack_record_changed_files_scope_clean() -> M5ResolvedSessionCompactionArchiveRecordEntry {
    review_pack_record(clean_review_pack_record_base(
        ...
        M5SessionCompactionArchiveRecordKind::ChangedFilesScope,
        M5SessionCompactionArchiveRecordSurfaceContext::ShiproomSurface,
...
fn review_pack_record_pull_request_scope_clean() -> M5ResolvedSessionCompactionArchiveRecordEntry {
    review_pack_record(clean_review_pack_record_base(
        ...
        M5SessionCompactionArchiveRecordKind::PullRequestScope,
        M5SessionCompactionArchiveRecordSurfaceContext::ReleaseCenterSurface,
...
fn review_pack_record_base_head_range_scope_clean() -> M5ResolvedSessionCompactionArchiveRecordEntry
{
    review_pack_record(clean_review_pack_record_base(
        ...
        M5SessionCompactionArchiveRecordKind::BaseHeadRangeScope,
        M5SessionCompactionArchiveRecordSurfaceContext::ProgramGovernanceSurface,
...
fn review_pack_record_worktree_uncommitted_scope_clean(
    ...
    let mut base = clean_review_pack_record_base(
        ...
        M5CollaborationStateRole::AuthorityModelDisclosure,
        M5SessionCompactionArchiveRecordKind::WorktreeUncommittedScope,
        M5SessionCompactionArchiveRecordSurfaceContext::ExecutiveSteeringSurface,
...
fn review_pack_record_full_tree_scope_clean() -> M5ResolvedSessionCompactionArchiveRecordEntry {
    ...
    let mut base = clean_review_pack_record_base(
        ...
        M5CollaborationStateRole::LocalTruthPreservationDisclosure,
        M5SessionCompactionArchiveRecordKind::FullTreeScope,
        M5SessionCompactionArchiveRecordSurfaceContext::SupportOrExportForm,
...
fn review_pack_record_saved_pack_snapshot_scope_clean(
    ...
    review_pack_record(clean_review_pack_record_base(
        ...
        M5SessionCompactionArchiveRecordKind::SavedPackSnapshotScope,
        M5SessionCompactionArchiveRecordSurfaceContext::ReleaseCenterSurface,
...
fn review_pack_record_object_incomplete() -> M5ResolvedSessionCompactionArchiveRecordEntry {
    let mut base = clean_review_pack_record_base(
...
fn review_pack_record_widen_fold() -> M5ResolvedSessionCompactionArchiveRecordEntry {
    let mut base = clean_review_pack_record_base(
...
/// registry.
fn review_pack_record_unbound() -> M5ResolvedSessionCompactionArchiveRecordEntry {
    let mut base = clean_review_pack_record_base(
...
fn review_pack_record_token_unstated() -> M5ResolvedSessionCompactionArchiveRecordEntry {
    let mut base = clean_review_pack_record_base(
...
fn clean_downgrade_base(
    ...
    token_name: &str,
    semantic_role: M5CollaborationStateRole,
    comparison_scope: M5SessionCompactionArchiveResultScope,
    surface_context: M5SessionCompactionArchiveRecordSurfaceContext,
...
) -> M5SessionCompactionArchiveResultEntryResolutionInput {
    M5SessionCompactionArchiveResultEntryResolutionInput {
        entry_id: entry_id.to_owned(),
...
fn downgrade_dogfood_ring_clean() -> M5ResolvedSessionCompactionArchiveResultEntry {
    ...
    let mut base = clean_downgrade_base(
        ...
        M5CollaborationStateRole::AuthorityModelDisclosure,
        M5SessionCompactionArchiveResultScope::EvaluatedScopeBinding,
        M5SessionCompactionArchiveRecordSurfaceContext::ShiproomSurface,
...
fn downgrade_rehearsal_currency_clean() -> M5ResolvedSessionCompactionArchiveResultEntry {
    downgrade(clean_downgrade_base(
        "downgrade:program-governance:rehearsal-currency",
        "launch.line.extension-author",
        "line.downgrade.base_head_range_scope",
        M5CollaborationStateRole::MergeAndDriftSemanticsDisclosure,
        M5SessionCompactionArchiveResultScope::PackVersionDigestBinding,
        M5SessionCompactionArchiveRecordSurfaceContext::ProgramGovernanceSurface,
...
fn downgrade_go_no_go_signoff_clean() -> M5ResolvedSessionCompactionArchiveResultEntry {
    downgrade(clean_downgrade_base(
        "downgrade:release-center:go-no-go-signoff",
        "launch.line.certified-journey",
        "line.downgrade.saved_pack_snapshot_scope",
        M5CollaborationStateRole::DowngradeBehaviorDisclosure,
        M5SessionCompactionArchiveResultScope::DivergenceLabelBinding,
        M5SessionCompactionArchiveRecordSurfaceContext::ReleaseCenterSurface,
...
fn downgrade_support_ahead() -> M5ResolvedSessionCompactionArchiveResultEntry {
    let mut base = clean_downgrade_base(
...
/// incomplete.
fn downgrade_form_incomplete() -> M5ResolvedSessionCompactionArchiveResultEntry {
    let mut base = clean_downgrade_base(
    ...
    base.resolution_form_coverage =
        vec![M5SessionCompactionArchiveRecordResolutionForm::CanonicalObject];
    downgrade(base)
}

/// Degraded downgrade entry: the downgrade scope is unclassified.
fn comparison_review_pack_result_unclassified() -> M5ResolvedSessionCompactionArchiveResultEntry {
    downgrade(clean_downgrade_base(
        "downgrade:executive-steering:scope-unclassified",
        "launch.line.design-partner-preview",
        "line.downgrade.worktree_uncommitted_scope",
        M5CollaborationStateRole::AuthorityModelDisclosure,
        M5SessionCompactionArchiveResultScope::ChangeObjectResultUnclassified,
        M5SessionCompactionArchiveRecordSurfaceContext::ExecutiveSteeringSurface,
...
#[allow(clippy::too_many_arguments)]
fn base_row(
    consumer_surface: M5SessionCompactionArchiveRecordAndResultRegistriesConsumerSurface,
    owner_role: &str,
    scope_summary: &str,
    proof_ref: &str,
    downgrade_triggers: Vec<M5CollaborationStateDowngradeTrigger>,
    review_pack_record_entries: Vec<M5ResolvedSessionCompactionArchiveRecordEntry>,
    review_pack_result_entries: Vec<M5ResolvedSessionCompactionArchiveResultEntry>,
) -> M5SessionCompactionArchiveRecordAndResultRegistriesRow {
    M5SessionCompactionArchiveRecordAndResultRegistriesRow {
        consumer_surface,
        ...
        anatomy_parts: M5SessionCompactionArchiveRecordAnatomyPart::ALL.to_vec(),
        export_fields: M5SessionCompactionArchiveRecordExportField::ALL.to_vec(),
        downgrade_triggers,
...
fn registry_rows() -> Vec<M5SessionCompactionArchiveRecordAndResultRegistriesRow> {
    ...
    vec![
        base_row(
            ...
            "session-compaction-archive:m5-shared-editor-replica-view:001",
            vec![
        ...
        base_row(
            ...
            "session-compaction-archive:m5-comment-annotation-review-pin-layer:001",
            vec![
                D::AuthorityModelUnstated,
...
fn governance_review() -> M5SessionCompactionArchiveRecordAndResultRegistriesGovernanceReview {
    M5SessionCompactionArchiveRecordAndResultRegistriesGovernanceReview {
...
fn consumer_projection() -> M5SessionCompactionArchiveRecordAndResultRegistriesConsumerProjection {
    M5SessionCompactionArchiveRecordAndResultRegistriesConsumerProjection {
...
fn proof_freshness() -> M5SessionCompactionArchiveRecordAndResultRegistriesProofFreshness {
    M5SessionCompactionArchiveRecordAndResultRegistriesProofFreshness {
...
fn release_posture() -> M5SessionCompactionArchiveRecordAndResultRegistriesReleasePosture {
    M5SessionCompactionArchiveRecordAndResultRegistriesReleasePosture {
        proof_packet_ref:
            M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_ARTIFACT_REF
                .to_owned(),
        line_audit_ref:
            M5_SESSION_COMPACTION_MANIFEST_AND_ARCHIVE_FINALIZATION_REGISTRIES_REPORT_REF.to_owned(),
...
pub fn seeded_m5_session_compaction_manifest_and_archive_finalization_registries_review_pack_record_beta_narrowed(
) -> M5SessionCompactionArchiveRecordAndResultRegistriesPacket {
    let mut packet = seeded_m5_session_compaction_manifest_and_archive_finalization_registries();
    packet.packet_id =
        "m5-session-compaction-manifest-and-archive-finalization-registries:compaction-manifest-beta:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
...
pub fn seeded_m5_session_compaction_manifest_and_archive_finalization_registries_review_pack_result_preview_narrowed(
) -> M5SessionCompactionArchiveRecordAndResultRegistriesPacket {
    let mut packet = seeded_m5_session_compaction_manifest_and_archive_finalization_registries();
    packet.packet_id =
        "m5-session-compaction-manifest-and-archive-finalization-registries:archive-finalization-preview:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
