//! Tests for the M05-1313 collaboration-control surface certification capstone.

use super::*;

fn packet() -> CollaborationControlProfileCertificationPacket {
    seeded_m5_collaboration_control_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, COLLABORATION_CONTROL_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, COLLABORATION_CONTROL_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5CollaborationControlCertifiedProfile::ALL {
...
fn every_frozen_family_is_certified_on_some_profile() {
    ...
    for family in M5CollaborationControlObject::ALL {
        assert!(
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            CollaborationControlCertificationAxis::ALL.len()
        );
    }
...
fn all_eight_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5CollaborationControlCertClaim> =
        p.rows.iter().map(|r| r.certified_claim).collect();
    for claim in M5CollaborationControlCertClaim::ALL {
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5CollaborationControlCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
...
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = CollaborationControlCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), CollaborationControlCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in CollaborationControlCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == CollaborationControlCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5CollaborationControlCertifiedProfile::StaleRetentionStateProfile
        })
        .expect("stale-retention-state-profile row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(CollaborationControlCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(CollaborationControlCertificationAxis::Localization)
    );
}
...
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    ...
    for row in &packet().rows {
        let outcome = row
            .axis(CollaborationControlCertificationAxis::HighContrast)
...
fn the_six_yellow_rows_bind_six_distinct_axes_and_triggers() {
    ...
    let p = packet();
    let axes: BTreeSet<CollaborationControlCertificationAxis> = p
        .rows
        .iter()
        .filter_map(|r| r.claim_auto_narrow.as_ref().map(|n| n.binding_axis))
        .collect();
    assert_eq!(axes.len(), 6, "yellow rows must bind six distinct axes");
    let triggers: std::collections::HashSet<M5CollaborationControlDowngradeTrigger> = p
        .rows
        .iter()
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5CollaborationControlCertifiedProfile::CertifiedCollaborationControlLane
        })
        .expect("certified-collaboration-control-lane row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == CollaborationControlCertificationAxis::Visual {
            outcome.state = CollaborationControlAxisCertificationState::UndisclosedDrift;
    ...
    assert_eq!(
        row.derived_status,
        CollaborationControlProfileClaimStatus::Red
    );
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        CollaborationControlCertificationViolation::ProfileBlocked { .. }
    )));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5CollaborationControlCertifiedProfile::ReviewableCollaborationControlRecordStructure
        })
        .expect("reviewable-collaboration-control-record-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails
        .acquires_terminal_or_debug_control_from_presence_or_follow_without_grant = true;
    assert_eq!(
        row.derive_status(),
        CollaborationControlProfileClaimStatus::Red
    );
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5CollaborationControlCertClaim::CertifiedCollaborationControlTruth;
    row.certified_claim = M5CollaborationControlCertClaim::CertifiedCollaborationControlTruth;
    assert_eq!(
        row.derive_status(),
        CollaborationControlProfileClaimStatus::Red
    );
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == CollaborationControlCertificationAxis::DegradedState {
            *outcome = CollaborationControlAxisOutcome {
                axis: CollaborationControlCertificationAxis::DegradedState,
    ...
    // Claim stays ReviewableCollaborationControlRecord == certified, no claim_auto_narrow.
    assert_eq!(
        row.derive_status(),
        CollaborationControlProfileClaimStatus::Red
    );
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == CollaborationControlCertificationAxis::CliExport {
            outcome.state = CollaborationControlAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger =
                Some(M5CollaborationControlDowngradeTrigger::ControlAuthorityUnstated);
        }
    }
    assert_eq!(
        row.derive_status(),
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(CollaborationControlClaimAutoNarrow {
        binding_axis: CollaborationControlCertificationAxis::DegradedState,
        from_claim: M5CollaborationControlCertClaim::CertifiedCollaborationControlTruth,
        to_claim: M5CollaborationControlCertClaim::CertifiedCollaborationControlTruth,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5CollaborationControlCertClaim::ControlAuthorityUnverifiedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-collaboration-control-record-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableCollaborationControlRecord; certify a stronger certified operating line.
    row.certified_claim = M5CollaborationControlCertClaim::CertifiedCollaborationControlTruth;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5CollaborationControlCertifiedProfile::UnprovenControlAuthorityProfile
        })
        .expect("unproven-control-authority-profile row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = CollaborationControlCertificationAxis::Visual;
    }
    assert_eq!(
        row.derive_status(),
        CollaborationControlProfileClaimStatus::Red
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = CollaborationControlCertificationAxis::CliExport;
    }
    assert_eq!(
        row.derive_status(),
        CollaborationControlProfileClaimStatus::Red
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == CollaborationControlProfileClaimStatus::Yellow)
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(CollaborationControlCertificationAxis::Visual);
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows.retain(|r| {
        r.profile != M5CollaborationControlCertifiedProfile::UndisclosedConsentScopeProfile
    });
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        CollaborationControlCertificationViolation::ProfileCoverageIncomplete
    )));
}
...
fn missing_family_coverage_is_rejected() {
    ...
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5CollaborationControlObject::ControlGrant);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        CollaborationControlCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = CollaborationControlProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        CollaborationControlCertificationViolation::WrongCanonicalBundle
    )));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        CollaborationControlCertificationViolation::DuplicateId { .. }
    )));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != CollaborationControlCertificationAxis::CollaborationControlTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        CollaborationControlCertificationViolation::SummaryMismatch
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: CollaborationControlProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_collaboration_control_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-collaboration-control-surface-certification/support_export.json"
    ));
    let packet_from_fixture: CollaborationControlProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-collaboration-control-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_COLLABORATION_CONTROL_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures from the
// seed builder. Off by default so a normal `cargo test` never mutates the tree.
#[test]
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_COLLABORATION_CONTROL_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_collaboration_control_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/release/m5-collaboration-control-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-collaboration-control-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir =
        repo.join("fixtures/collaboration/m5-collaboration-control-surface-certification");
