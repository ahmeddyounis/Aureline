//! Implemented M5 blocked-escalate-card and escalation-outcome registries.
//!
//! The frozen [change-intent lifecycle matrix][matrix] names Aureline's governed change-intent object classes and
//! locks their controlled vocabulary. This module makes the `blocked_escalate_card` class operable so the work-item
//! detail, review detail, Git / worktree, provider-handoff, help / docs, and support / export surfaces inherit one
//! canonical blocked-or-escalate descriptor rather than a hand-authored parallel prose that has to be kept
//! consistent: it turns the *blocked-escalate-card* grammar (one reusable card per blocked or escalated tracked work
//! item — naming the blocker class, the missing dependency or approval, the suggested escalation path, the
//! attach-evidence action, and the local note or handoff-packet fallback — so a stalled item surfaces one explicit
//! blocker object instead of generic failure chrome, and users can attach evidence and export or retry from the
//! same blocked state without losing the tracked-item context) and the *escalation-outcome* grammar (the outcome a
//! blocked-or-escalate card resolves toward — escalated to the provider, queued as a local handoff packet, exported
//! locally, blocked by a missing permission, or blocked by unresolved engineering state — kept honest about what the
//! provider has actually accepted, what stays local, and what remains blocked) into registry resolvers that produce
//! export-safe, honest projections. Every blocked or escalated item then resolves to one typed blocked-escalate card
//! — its blocker class, its missing dependency or approval, its suggested escalation path, its attach-evidence
//! action, and its local note or handoff-packet fallback, all attributable to the work item so a local handoff
//! packet never reads as a provider-committed update — and to one escalation-outcome object — the outcome identity,
//! the affected blocker-cause field, the previous-versus-current commit-state reference, the binding state, and the
//! active blocker reason (dependency, approval, provider, policy, or unresolved-engineering cause) — that the
//! work-item, review, provider-handoff, and support surfaces can inspect without manual reconstruction, so a blocked
//! state never implies the provider accepted an escalation it has not when the target is offline, policy-blocked, or
//! only partially writable, and a card that cannot bind an evidence field to its commit state degrades honestly
//! instead of letting a local handoff packet masquerade as a provider-committed escalation.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable blocked-escalate card per blocked work item, carrying every blocker field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound blocked-or-escalate entry
//!   unless it names a canonical registry token, a classified [blocker-cause selector][M5BlockedEscalateCardKind]
//!   (dependency, approval, provider, policy, or unresolved-engineering cause — never one generic warning),
//!   a disclosure role, covers every [resolution form][M5BlockedEscalateCardResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every card field (blocker class,
//!   missing dependency or approval, suggested escalation path, attach-evidence action, and local note or
//!   handoff-packet fallback), keeps its evidence lineage attributable to the tracked item, and
//!   keeps the escalation authority bound to its commit state; otherwise it degrades.
//! * **Preserve attach-evidence / export / retry continuity rather than letting a blocked state drop its blockers or evidence.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a blocked-or-escalate entry that would clear
//!   a blocked state while an engineering blocker remains unresolved or that drops the attach-evidence / export /
//!   retry path, so it degrades to
//!   [`M5BlockedEscalateCardEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a permission-scoped field whose language outruns its commit state degrades the same
//!   way — the structured blocker reason a local-handoff-shown-as-provider escalation must surface.
//! * **Turn escalated-to-provider, queued-as-local-handoff-packet, exported-locally, blocked-by-missing-permission, and
//!   blocked-by-unresolved-engineering-state into a visible, typed escalation-outcome event instead of an implied
//!   provider acceptance.** [`resolve_review_pack_result_entry`] names a classified
//!   [outcome binding][M5BlockedEscalateOutcomeScope]
//!   (blocker-cause binding, commit-state binding, or permission-scope binding), requires the full
//!   outcome-identity / affected-blocker-cause-field / previous-versus-current-commit-state / binding / active-reason
//!   escalation-outcome object, covers every resolution form, and degrades to
//!   [`M5BlockedEscalateOutcomeEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the outcome would present a queued or local handoff packet as provider-accepted, hide the offline /
//!   policy-blocked / partially-writable state, or let an evidence gap masquerade as full coverage, so an
//!   escalation-outcome can never read as accepted when it has quietly dropped the reason the provider could not
...
//! can never fork their own blocker meaning. Raw secret values and private endpoints stay outside the export boundary.
//!
...
pub use seed::{
    ...
    M5_BLOCKED_ESCALATE_CARD_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5BlockedEscalateCardRegistriesPacket`].
pub const M5_BLOCKED_ESCALATE_CARD_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_blocked_escalate_card_and_relation_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_BLOCKED_ESCALATE_CARD_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_BLOCKED_ESCALATE_CARD_REGISTRIES_SCHEMA_REF: &str =
    "schemas/teamwork/m5-blocked-escalate-card-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_BLOCKED_ESCALATE_CARD_REGISTRIES_DOC_REF: &str =
    "docs/team-workflows/m5_blocked_escalate_card_registries.md";
...
pub const M5_BLOCKED_ESCALATE_CARD_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-blocked-escalate-card-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_BLOCKED_ESCALATE_CARD_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-blocked-escalate-card-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_BLOCKED_ESCALATE_CARD_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-blocked-escalate-card-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_BLOCKED_ESCALATE_CARD_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5BlockedEscalateCardRegistriesConsumerSurface = M5ChangeIntentConsumerSurface;
...
pub enum M5BlockedEscalateCardResolutionForm {
...
impl M5BlockedEscalateCardResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5BlockedEscalateCardKind {
...
impl M5BlockedEscalateCardKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5BlockedEscalateOutcomeScope {
...
impl M5BlockedEscalateOutcomeScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5BlockedEscalateCardSurfaceContext {
...
impl M5BlockedEscalateCardSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5BlockedEscalateCardAnatomyPart {
...
impl M5BlockedEscalateCardAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5BlockedEscalateCardNextAction {
...
impl M5BlockedEscalateCardNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5BlockedEscalateCardExportField {
...
impl M5BlockedEscalateCardExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5BlockedEscalateCardEntryDegradeReason {
...
impl M5BlockedEscalateCardEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5BlockedEscalateCardNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5BlockedEscalateCardNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5BlockedEscalateCardNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5BlockedEscalateCardNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5BlockedEscalateCardNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5BlockedEscalateOutcomeEntryDegradeReason {
...
impl M5BlockedEscalateOutcomeEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5BlockedEscalateCardNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5BlockedEscalateCardNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5BlockedEscalateCardNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5BlockedEscalateCardNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5BlockedEscalateCardEntryResolutionInput {
    ...
    pub report_section: M5BlockedEscalateCardKind,
    /// The render / surface context.
    pub surface_context: M5BlockedEscalateCardSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5BlockedEscalateCardResolutionForm>,
...
pub struct M5ResolvedBlockedEscalateCardEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5BlockedEscalateCardEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5BlockedEscalateCardNextAction,
...
impl M5ResolvedBlockedEscalateCardEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5BlockedEscalateOutcomeEntryResolutionInput {
    ...
    pub comparison_scope: M5BlockedEscalateOutcomeScope,
    /// The render / surface context.
    pub surface_context: M5BlockedEscalateCardSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5BlockedEscalateCardResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedBlockedEscalateOutcomeEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5BlockedEscalateOutcomeEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5BlockedEscalateCardNextAction,
...
impl M5ResolvedBlockedEscalateOutcomeEntry {
...
pub enum M5BlockedEscalateCardResolutionError {
...
impl M5BlockedEscalateCardResolutionError {
...
impl fmt::Display for M5BlockedEscalateCardResolutionError {
...
impl Error for M5BlockedEscalateCardResolutionError {}

fn form_tokens(forms: &[M5BlockedEscalateCardResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5BlockedEscalateCardResolutionForm]) -> bool {
    let present: BTreeSet<M5BlockedEscalateCardResolutionForm> = forms.iter().copied().collect();
    M5BlockedEscalateCardResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5BlockedEscalateCardKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5BlockedEscalateOutcomeScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedBlockedEscalateCardEntry, M5BlockedEscalateCardResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5BlockedEscalateCardResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5BlockedEscalateCardResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5BlockedEscalateCardEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5BlockedEscalateCardEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5BlockedEscalateCardEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5BlockedEscalateCardEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5BlockedEscalateCardEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5BlockedEscalateCardEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5BlockedEscalateCardEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(
            M5BlockedEscalateCardEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    } else if !input.proof_fresh {
        Some(M5BlockedEscalateCardEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5BlockedEscalateCardNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedBlockedEscalateCardEntry {
...
pub fn resolve_review_pack_result_entry(
...
) -> Result<M5ResolvedBlockedEscalateOutcomeEntry, M5BlockedEscalateCardResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5BlockedEscalateCardResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5BlockedEscalateOutcomeEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5BlockedEscalateOutcomeEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5BlockedEscalateOutcomeEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5BlockedEscalateOutcomeEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5BlockedEscalateOutcomeEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5BlockedEscalateOutcomeEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5BlockedEscalateCardNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedBlockedEscalateOutcomeEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5BlockedEscalateCardRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5BlockedEscalateCardRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5BlockedEscalateCardAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5BlockedEscalateCardExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5ChangeIntentDowngradeTrigger>,
    /// Resolved line-review_pack_record-registry examples.
    pub review_pack_record_entries: Vec<M5ResolvedBlockedEscalateCardEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedBlockedEscalateOutcomeEntry>,
...
impl M5BlockedEscalateCardRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5BlockedEscalateCardAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5BlockedEscalateCardAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5BlockedEscalateCardExportField> =
            self.export_fields.iter().copied().collect();
        M5BlockedEscalateCardExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedBlockedEscalateCardEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedBlockedEscalateOutcomeEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5BlockedEscalateCardRegistriesVocabularySet {
...
impl M5BlockedEscalateCardRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ChangeIntentRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5BlockedEscalateCardResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5BlockedEscalateCardKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5BlockedEscalateOutcomeScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5BlockedEscalateCardSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5BlockedEscalateCardEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5BlockedEscalateOutcomeEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5BlockedEscalateCardAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5BlockedEscalateCardNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5BlockedEscalateCardExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5BlockedEscalateCardRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5BlockedEscalateCardRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5BlockedEscalateCardRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5BlockedEscalateCardRegistriesReleasePosture {
...
/// Constructor input for [`M5BlockedEscalateCardRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5BlockedEscalateCardRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5BlockedEscalateCardRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5BlockedEscalateCardRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5BlockedEscalateCardRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5BlockedEscalateCardRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5BlockedEscalateCardRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5BlockedEscalateCardRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5BlockedEscalateCardRegistriesPacket {
    /// Record kind; must equal [`M5_BLOCKED_ESCALATE_CARD_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_BLOCKED_ESCALATE_CARD_REGISTRIES_SCHEMA_VERSION`].
...
impl M5BlockedEscalateCardRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5BlockedEscalateCardRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_BLOCKED_ESCALATE_CARD_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_BLOCKED_ESCALATE_CARD_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5BlockedEscalateCardRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_BLOCKED_ESCALATE_CARD_REGISTRIES_RECORD_KIND {
            violations.push(M5BlockedEscalateCardRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_BLOCKED_ESCALATE_CARD_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5BlockedEscalateCardRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5BlockedEscalateCardRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5BlockedEscalateCardRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5BlockedEscalateCardRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Blocked-or-Escalate Card and Escalation-Outcome Registries\n\n");
...
#[derive(Debug)]
pub enum M5BlockedEscalateCardRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5BlockedEscalateCardRegistriesViolation>),
}

impl fmt::Display for M5BlockedEscalateCardRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5BlockedEscalateCardRegistriesArtifactError {}

/// Validation failures emitted by [`M5BlockedEscalateCardRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5BlockedEscalateCardRegistriesViolation {
...
impl M5BlockedEscalateCardRegistriesViolation {
...
pub fn current_stable_m5_blocked_escalate_card_registries_export(
) -> Result<M5BlockedEscalateCardRegistriesPacket, M5BlockedEscalateCardRegistriesArtifactError> {
    let packet: M5BlockedEscalateCardRegistriesPacket =
        serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../artifacts/release/m5-blocked-escalate-card-registries-proof/support_export.json"
    )))
        .map_err(M5BlockedEscalateCardRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5BlockedEscalateCardRegistriesArtifactError::Validation(
            violations,
        ))
    }
}
...
fn validate_source_contracts(
    ...
    violations: &mut Vec<M5BlockedEscalateCardRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(M5BlockedEscalateCardRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5BlockedEscalateCardRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(M5BlockedEscalateCardRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(M5BlockedEscalateCardRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(M5BlockedEscalateCardRegistriesViolation::MandatoryExportFieldMissing);
        }
        ...
        if !refs.contains(M5_CHANGE_INTENT_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_BLOCKED_ESCALATE_CARD_DOMAIN_SCHEMA_REF)
        {
            violations.push(M5BlockedEscalateCardRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5BlockedEscalateCardRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5BlockedEscalateCardRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5BlockedEscalateCardRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(M5BlockedEscalateCardRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(M5BlockedEscalateCardRegistriesViolation::ConsumerProjectionIncomplete);
            return;
        }
    }
}
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5BlockedEscalateCardRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5BlockedEscalateCardRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5BlockedEscalateCardKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5BlockedEscalateCardSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations
            .push(M5BlockedEscalateCardRegistriesViolation::CohortDescriptorResolutionNotProven);
    }
    ...
    {
        violations.push(
            M5BlockedEscalateCardRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5BlockedEscalateOutcomeScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(M5BlockedEscalateCardRegistriesViolation::CohortEvidenceIntegrityNotProven);
    }
}
