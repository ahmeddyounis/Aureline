//! Canonical seed builders for the M5 line-retirement_manifest and line-downgrade-packet registries packet.
//!
//! These builders are the single producer of the checked-in support export and the narrowed fixtures.
//! The headless emitter and the inline tests both call them so the in-code registries, the artifact, and the
//! fixtures never drift. Every resolved example is built by calling the real resolvers so the packet can only
//! carry projections the resolvers actually produce. Clean line-retirement_manifest and line-downgrade-packet entries
//! are built so the one typed line-retirement_manifest object resolving per line, lines never widening without
...
pub const M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_PACKET_ID: &str =
    "m5-retirement-manifest-and-change-diff-registries:stable:0001";
...
) -> M5ResolvedRetirementManifestEntry {
    resolve_retirement_manifest_entry(input).expect("seed line-retirement_manifest entry resolves")
}

fn downgrade(input: M5ManifestChangeDiffEntryResolutionInput) -> M5ResolvedManifestChangeDiffEntry {
    resolve_manifest_change_diff_entry(input).expect("seed line-downgrade-packet entry resolves")
}

fn all_forms() -> Vec<M5RetirementManifestResolutionForm> {
    M5RetirementManifestResolutionForm::ALL.to_vec()
}

// -- Clean line-retirement_manifest entries (typed object, rollback / diagnostics preserved, bound to the registry) --

#[allow(clippy::too_many_arguments)]
fn clean_retirement_manifest_base(
    entry_id: &str,
    line_binding_id: &str,
    token_name: &str,
    semantic_role: M5RetiredStateRole,
    report_section: M5RetirementManifestKind,
    surface_context: M5RetirementManifestSurfaceContext,
...
) -> M5RetirementManifestEntryResolutionInput {
    M5RetirementManifestEntryResolutionInput {
...
fn retirement_manifest_last_supported_version_channel_clean() -> M5ResolvedRetirementManifestEntry {
    retirement_manifest(clean_retirement_manifest_base(
        "retirement_manifest:shiproom:dogfood-core-team-canary",
        "launch.line.core-team-canary",
        "line.retirement_manifest.core_team_canary",
        M5RetiredStateRole::LastSupportedPin,
        M5RetirementManifestKind::LastSupportedVersionChannel,
        M5RetirementManifestSurfaceContext::ShiproomSurface,
...
fn retirement_manifest_retirement_trigger_clean() -> M5ResolvedRetirementManifestEntry {
    retirement_manifest(clean_retirement_manifest_base(
        "retirement_manifest:release-center:migration-alpha",
        "launch.line.migration-alpha",
        "line.retirement_manifest.retirement_trigger",
        M5RetiredStateRole::SuccessorRouting,
        M5RetirementManifestKind::RetirementTrigger,
        M5RetirementManifestSurfaceContext::ReleaseCenterSurface,
...
fn retirement_manifest_cutoff_date_clean() -> M5ResolvedRetirementManifestEntry {
    retirement_manifest(clean_retirement_manifest_base(
        "retirement_manifest:program-governance:extension-author",
        "launch.line.extension-author",
        "line.retirement_manifest.cutoff_date",
        M5RetiredStateRole::DisablePath,
        M5RetirementManifestKind::CutoffDate,
        M5RetirementManifestSurfaceContext::ProgramGovernanceSurface,
...
fn retirement_manifest_successor_reference_clean() -> M5ResolvedRetirementManifestEntry {
    // A design-partner preview line is public-facing and keeps its support language matched to line proof.
    let mut base = clean_retirement_manifest_base(
        "retirement_manifest:executive-steering:design-partner-preview",
        "launch.line.design-partner-preview",
        "line.retirement_manifest.successor_reference",
        M5RetiredStateRole::LastSupportedPin,
        M5RetirementManifestKind::SuccessorReference,
        M5RetirementManifestSurfaceContext::ExecutiveSteeringSurface,
    ...
    retirement_manifest(base)
}

fn retirement_manifest_disable_path_clean() -> M5ResolvedRetirementManifestEntry {
    // A public preview line is public-facing and keeps its support language matched to line proof.
    let mut base = clean_retirement_manifest_base(
        "retirement_manifest:support:public-preview",
        "launch.line.public-preview",
        "line.retirement_manifest.disable_path",
        M5RetiredStateRole::SuccessorRouting,
        M5RetirementManifestKind::DisablePath,
        M5RetirementManifestSurfaceContext::SupportOrExportForm,
...
fn retirement_manifest_export_rollback_route_clean() -> M5ResolvedRetirementManifestEntry {
    retirement_manifest(clean_retirement_manifest_base(
        "retirement_manifest:release-center:certified-journey",
        "launch.line.certified-journey",
        "line.retirement_manifest.export_rollback_route",
        M5RetiredStateRole::SupportNoteClosure,
        M5RetirementManifestKind::ExportRollbackRoute,
        M5RetirementManifestSurfaceContext::ReleaseCenterSurface,
...
// -- Degraded line-retirement_manifest entries ---------------------------------------------------------

/// Degraded retirement_manifest entry: the resolved retirement_manifest object is incomplete — the bundle IDs are unstated.
fn retirement_manifest_object_incomplete() -> M5ResolvedRetirementManifestEntry {
    let mut base = clean_retirement_manifest_base(
        "retirement_manifest:shiproom:incomplete",
        "launch.line.core-team-canary",
        "line.retirement_manifest.core_team_canary",
        M5RetiredStateRole::LastSupportedPin,
        M5RetirementManifestKind::LastSupportedVersionChannel,
...
/// Degraded retirement_manifest entry: the line's rollback and diagnostics posture is not preserved before widening —
...
fn retirement_manifest_widen_fold() -> M5ResolvedRetirementManifestEntry {
    let mut base = clean_retirement_manifest_base(
        "retirement_manifest:release-center:widen-fold",
        "launch.line.migration-alpha",
        "line.retirement_manifest.retirement_trigger",
        M5RetiredStateRole::SuccessorRouting,
        M5RetirementManifestKind::RetirementTrigger,
...
/// Degraded retirement_manifest entry: the behavior is a hand-copied per-entry assumption instead of tracing to the
/// registry.
fn retirement_manifest_unbound() -> M5ResolvedRetirementManifestEntry {
    let mut base = clean_retirement_manifest_base(
        "retirement_manifest:executive-steering:unbound",
        "launch.line.certified-journey",
        "line.retirement_manifest.export_rollback_route",
        M5RetiredStateRole::SupportNoteClosure,
        M5RetirementManifestKind::ExportRollbackRoute,
...
/// Degraded retirement_manifest entry: the canonical registry token name is unstated.
fn retirement_manifest_token_unstated() -> M5ResolvedRetirementManifestEntry {
    let mut base = clean_retirement_manifest_base(
        "retirement_manifest:program-governance:token-unstated",
        "launch.line.extension-author",
        "  ",
        M5RetiredStateRole::DisablePath,
        M5RetirementManifestKind::CutoffDate,
...
fn clean_downgrade_base(
    ...
    token_name: &str,
    semantic_role: M5RetiredStateRole,
    comparison_scope: M5ManifestChangeDiffScope,
    surface_context: M5RetirementManifestSurfaceContext,
...
) -> M5ManifestChangeDiffEntryResolutionInput {
    M5ManifestChangeDiffEntryResolutionInput {
        entry_id: entry_id.to_owned(),
        ...
        keeps_manifest_change_diff_visible: true,
...
fn downgrade_dogfood_ring_clean() -> M5ResolvedManifestChangeDiffEntry {
    ...
    let mut base = clean_downgrade_base(
        ...
        M5RetiredStateRole::LastSupportedPin,
        M5ManifestChangeDiffScope::CutoffDateChange,
        M5RetirementManifestSurfaceContext::ShiproomSurface,
...
fn downgrade_rehearsal_currency_clean() -> M5ResolvedManifestChangeDiffEntry {
    downgrade(clean_downgrade_base(
        "downgrade:program-governance:rehearsal-currency",
        "launch.line.extension-author",
        "line.downgrade.cutoff_date",
        M5RetiredStateRole::DisablePath,
        M5ManifestChangeDiffScope::ReplacementPathChange,
        M5RetirementManifestSurfaceContext::ProgramGovernanceSurface,
...
fn downgrade_go_no_go_signoff_clean() -> M5ResolvedManifestChangeDiffEntry {
    downgrade(clean_downgrade_base(
        "downgrade:release-center:go-no-go-signoff",
        "launch.line.certified-journey",
        "line.downgrade.export_rollback_route",
        M5RetiredStateRole::SupportNoteClosure,
        M5ManifestChangeDiffScope::DisableOrExportRouteChange,
        M5RetirementManifestSurfaceContext::ReleaseCenterSurface,
...
fn downgrade_support_ahead() -> M5ResolvedManifestChangeDiffEntry {
    let mut base = clean_downgrade_base(
...
/// incomplete.
fn downgrade_form_incomplete() -> M5ResolvedManifestChangeDiffEntry {
    let mut base = clean_downgrade_base(
    ...
    base.resolution_form_coverage = vec![M5RetirementManifestResolutionForm::CanonicalObject];
    downgrade(base)
}

/// Degraded downgrade entry: the downgrade scope is unclassified.
fn comparison_diff_scope_unclassified() -> M5ResolvedManifestChangeDiffEntry {
    downgrade(clean_downgrade_base(
        "downgrade:executive-steering:scope-unclassified",
        "launch.line.design-partner-preview",
        "line.downgrade.successor_reference",
        M5RetiredStateRole::LastSupportedPin,
        M5ManifestChangeDiffScope::DiffScopeUnclassified,
        M5RetirementManifestSurfaceContext::ExecutiveSteeringSurface,
...
#[allow(clippy::too_many_arguments)]
fn base_row(
    consumer_surface: M5RetirementManifestChangeDiffRegistriesConsumerSurface,
    owner_role: &str,
    scope_summary: &str,
    proof_ref: &str,
    downgrade_triggers: Vec<M5RetiredStateDowngradeTrigger>,
    retirement_manifest_entries: Vec<M5ResolvedRetirementManifestEntry>,
    manifest_change_diff_entries: Vec<M5ResolvedManifestChangeDiffEntry>,
) -> M5RetirementManifestChangeDiffRegistriesRow {
    M5RetirementManifestChangeDiffRegistriesRow {
        consumer_surface,
        ...
        anatomy_parts: M5RetirementManifestAnatomyPart::ALL.to_vec(),
        export_fields: M5RetirementManifestExportField::ALL.to_vec(),
        downgrade_triggers,
        ...
        collapses_distinct_manifest_change_diff_classes_into_one_lane: false,
    }
}

fn registry_rows() -> Vec<M5RetirementManifestChangeDiffRegistriesRow> {
    ...
    vec![
        base_row(
            ...
            vec![
                retirement_manifest_last_supported_version_channel_clean(),
                retirement_manifest_object_incomplete(),
            ],
        ...
        base_row(
            ...
            vec![retirement_manifest_retirement_trigger_clean(), retirement_manifest_widen_fold()],
            vec![downgrade_go_no_go_signoff_clean(), downgrade_form_incomplete()],
        ),
        base_row(
            C::Support,
            "Support owner",
            "Support resolves the successor-reference field while keeping its public-facing successor / exit claim matched to the closed support note and reports the manifest-change-diff outcome; a manifest entry that is a hand-copied per-entry assumption and a diff on an unclassified diff scope degrade honestly",
            ...
            vec![
                retirement_manifest_successor_reference_clean(),
                retirement_manifest_unbound(),
            ],
        ...
        base_row(
            ...
            vec![
                retirement_manifest_cutoff_date_clean(),
                retirement_manifest_token_unstated(),
            ],
        ...
        base_row(
            ...
            "Install / update renders the same resolved retirement-manifest and manifest-change-diff truth the resolvers produced across the canonical, accessible, and audit resolution forms rather than a hand-copied class table; the export / rollback-route field and the disable-or-export-route-change diff stay inspectable off-renderer so no new install can still select a retired class",
        ...
        base_row(
            ...
            vec![retirement_manifest_disable_path_clean()],
            vec![downgrade_rehearsal_currency_clean()],
        ),
    ]
}

fn governance_review() -> M5RetirementManifestChangeDiffRegistriesGovernanceReview {
    M5RetirementManifestChangeDiffRegistriesGovernanceReview {
        retirement_manifest_registry_names_token_role_and_journey: true,
        line_resolves_to_typed_retirement_manifest_from_shared_registry: true,
        repo_bundle_toolchain_and_deployment_rows_published: true,
        lines_cannot_widen_without_rollback_and_diagnostics: true,
        manifest_change_diff_keeps_proof_visible_and_binds_support_language: true,
        ...
        retirement_manifest_or_downgrade_drift_caught_before_release: true,
        every_row_declares_mandatory_anatomy: true,
        reuses_frozen_matrix_vocabulary: true,
    }
}

fn consumer_projection() -> M5RetirementManifestChangeDiffRegistriesConsumerProjection {
    M5RetirementManifestChangeDiffRegistriesConsumerProjection {
...
fn proof_freshness() -> M5RetirementManifestChangeDiffRegistriesProofFreshness {
    M5RetirementManifestChangeDiffRegistriesProofFreshness {
...
fn release_posture() -> M5RetirementManifestChangeDiffRegistriesReleasePosture {
    M5RetirementManifestChangeDiffRegistriesReleasePosture {
        proof_packet_ref: M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_ARTIFACT_REF.to_owned(),
        line_audit_ref: M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_REPORT_REF.to_owned(),
...
pub fn seeded_m5_retirement_manifest_and_change_diff_registries_retirement_manifest_beta_narrowed(
) -> M5RetirementManifestChangeDiffRegistriesPacket {
    let mut packet = seeded_m5_retirement_manifest_and_change_diff_registries();
    packet.packet_id =
        "m5-retirement-manifest-and-change-diff-registries:public-proof-ledger-beta:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
...
pub fn seeded_m5_retirement_manifest_and_change_diff_registries_manifest_change_diff_preview_narrowed(
) -> M5RetirementManifestChangeDiffRegistriesPacket {
    let mut packet = seeded_m5_retirement_manifest_and_change_diff_registries();
    packet.packet_id =
        "m5-retirement-manifest-and-change-diff-registries:claim-history-diff-preview:0001"
            .to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
