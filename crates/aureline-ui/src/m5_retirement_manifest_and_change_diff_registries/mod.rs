//! Implemented M5 retirement-manifest and manifest-change-diff registries.
//!
//! The frozen [retired-state matrix][matrix] names Aureline's governed terminal-lifecycle object classes and locks
//! their controlled vocabulary. This module makes the retirement of the first two of those classes — a supported
//! line and a stable-facing capability — operable so CLI, docs / help, partner packets, and support bundles inherit
//! one canonical retirement object rather than a hand-authored parallel prose that has to be kept consistent: it
//! turns the *retirement-manifest* grammar (one machine-readable manifest per retiring class — one typed field per
//! manifest section: the last-supported version / channel pinned to an exact build, the retirement trigger, the
//! cutoff date, the successor reference, the disable path, and the export / rollback route — each bound to one
//! object-class identity with its exact-build joins) and the *manifest-change-diff* grammar (the diff scope a
//! manifest change sits in — a cutoff-date change, a replacement-path change, or a disable / export-route change)
//! into registry resolvers that produce export-safe, honest projections. Every retiring class then resolves to one
//! typed retirement-manifest object — the fields it publishes, its exact-build joins, its disable path, and its
//! export / rollback route, all preserved before a claim widens so a retired class never keeps support language
//! ahead of its closed support note and a current rollback / export route — and to one manifest-change-diff object —
//! the resolved object identity, the affected manifest field, the previous-versus-current manifest-state reference,
//! the diff-scope state, and the active diff reason — that the CLI, docs / help, partner-packet, and support-bundle
//! surfaces can inspect without manual reconstruction, so a changed cutoff date or replacement path stays visible as
//! a typed diff instead of a silent mutation, and a manifest that cannot bind a field to its exact build degrades
//! honestly instead of leaving a retired class to read as still supported.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable retirement manifest per retiring class, carrying every manifest field.**
//!   [`resolve_retirement_manifest_entry`] refuses to read as a clean, registry-bound manifest entry
//!   unless it names a canonical registry token, a classified [manifest field][M5RetirementManifestKind], a
//!   retirement role, covers every [resolution form][M5RetirementManifestResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every manifest field (last-supported
//!   version / channel, retirement trigger, cutoff date, successor reference, disable path, and export / rollback
//!   route bound to exact-build joins), preserves its rollback / export route before a claim widens, and keeps a
//!   public-facing field bound to the closed support note; otherwise it degrades.
//! * **Expose successor and rollback / export truth without hand-authored parallel prose staying consistent.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a manifest entry whose
//!   rollback / export route is not preserved before widening (a retired class resuming a wider claim without one) so
//!   it degrades to
//!   [`M5RetirementManifestEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a public-facing successor / disable field whose language outruns the closed support note degrades the same
//!   way — the structured blocker reason a widen-without-route attempt must surface.
//! * **Turn a changed cutoff date or replacement path into a visible, typed diff instead of a silent mutation.**
//!   [`resolve_manifest_change_diff_entry`] names a classified [diff scope][M5ManifestChangeDiffScope]
//!   (cutoff-date-change, replacement-path-change, or disable-or-export-route-change), requires the full
//!   object-identity / affected-manifest-field / previous-versus-current-manifest-state / diff-scope / active-reason
//!   diff object, covers every resolution form, and degrades to
//!   [`M5ManifestChangeDiffEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the diff would keep support language ahead of the closed support note, hide the diff, or let a gap
//!   masquerade as covered, so a manifest change can never read as trustworthy when it has quietly dropped the reason
//!   a retirement changed state.
//!
...
pub use seed::{
    ...
    M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5RetirementManifestChangeDiffRegistriesPacket`].
pub const M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_retirement_manifest_and_change_diff_packet_registries";

/// Schema version for M5 line-retirement_manifest / line-downgrade-packet registry records.
pub const M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_SCHEMA_REF: &str =
    "schemas/program/m5-retirement-manifest-and-change-diff-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_DOC_REF: &str =
    "docs/release/m5_retirement_manifest_and_change_diff_registries.md";
...
pub const M5_MANIFEST_CHANGE_DIFF_DOMAIN_SCHEMA_REF: &str =
    "schemas/program/m5-retirement-manifest-change-diff.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-retirement-manifest-and-change-diff-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-retirement-manifest-and-change-diff-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-retirement-manifest-and-change-diff-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5RetirementManifestChangeDiffRegistriesConsumerSurface = M5RetiredStateConsumerSurface;

/// One of the three resolution forms every line-retirement_manifest or line-downgrade-packet entry must hold across so
...
/// line-retirement_manifest and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementManifestResolutionForm {
    /// The canonical resolved line-retirement_manifest / line-downgrade-packet object.
    CanonicalObject,
...
impl M5RetirementManifestResolutionForm {
...
pub enum M5RetirementManifestKind {
    ...
    ManifestFieldUnclassified,
}

impl M5RetirementManifestKind {
    /// Every public-proof-ledger kind, in declaration order.
    pub const ALL: [Self; 7] = [
        Self::LastSupportedVersionChannel,
        Self::RetirementTrigger,
        Self::CutoffDate,
        Self::SuccessorReference,
        Self::DisablePath,
        Self::ExportRollbackRoute,
        Self::ManifestFieldUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::LastSupportedVersionChannel => "last_supported_version_channel",
            Self::RetirementTrigger => "retirement_trigger",
            Self::CutoffDate => "cutoff_date",
            Self::SuccessorReference => "successor_reference",
            Self::DisablePath => "disable_path",
            Self::ExportRollbackRoute => "export_rollback_route",
            Self::ManifestFieldUnclassified => "manifest_field_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::ManifestFieldUnclassified)
    }

    /// The canonical mode for this public-proof-ledger kind.
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::LastSupportedVersionChannel => "last_supported_version_channel_mode",
            Self::RetirementTrigger => "retirement_trigger_mode",
            Self::CutoffDate => "cutoff_date_mode",
            Self::SuccessorReference => "successor_reference_mode",
            Self::DisablePath => "disable_path_mode",
            Self::ExportRollbackRoute => "export_rollback_route_mode",
            Self::ManifestFieldUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(self, Self::SuccessorReference | Self::DisablePath)
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ManifestChangeDiffScope {
...
impl M5ManifestChangeDiffScope {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::CutoffDateChange => "cutoff_date_change",
            Self::ReplacementPathChange => "replacement_path_change",
            Self::DisableOrExportRouteChange => "disable_or_export_route_change",
...
/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-retirement_manifest or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementManifestSurfaceContext {
...
impl M5RetirementManifestSurfaceContext {
...
/// One mandatory rendered part a line-retirement_manifest or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementManifestAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-retirement_manifest entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-retirement_manifest entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-retirement_manifest
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line retirement_manifest or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5RetirementManifestAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "manifest_change_diff_fields",
...
/// line retirement_manifest, a line-downgrade packet, or a degraded line-retirement_manifest / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementManifestNextAction {
    /// Expand the resolved line retirement_manifest's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5RetirementManifestNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementManifestExportField {
...
impl M5RetirementManifestExportField {
...
/// Reason a line-retirement_manifest entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5RetirementManifestEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the retirement_manifest means.
    ...
    /// The resolved line-retirement_manifest object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5RetirementManifestEntryDegradeReason {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "retirement_manifest_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortManifestFieldUnclassified => "line_manifest_field_unclassified",
            Self::DescriptorNotBoundToRegistry => "retirement_manifest_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "retirement_manifest_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "retirement_manifest_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5RetirementManifestNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5RetirementManifestNextAction::TraceCanonicalRegistry
            }
            Self::CohortManifestFieldUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5RetirementManifestNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5RetirementManifestNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5RetirementManifestNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ManifestChangeDiffEntryDegradeReason {
...
impl M5ManifestChangeDiffEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                "manifest_change_diff_runs_support_ahead_of_proof_or_drops_manifest_change_diff"
            }
    ...
    pub const fn next_action(self) -> M5RetirementManifestNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5RetirementManifestNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5RetirementManifestNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5RetirementManifestNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_retirement_manifest_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5RetirementManifestEntryResolutionInput {
    /// Stable identity of the line-retirement_manifest-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this retirement_manifest binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.retirement_manifest.disable_path`); empty means unstated.
    ...
    pub report_section: M5RetirementManifestKind,
    /// The render / surface context.
    pub surface_context: M5RetirementManifestSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5RetirementManifestResolutionForm>,
    ...
    /// True when the behavior traces to the line-retirement_manifest registry (never a hand-copied constant).
...
/// Resolved, export-safe line-retirement_manifest-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedRetirementManifestEntry {
    /// Stable identity of the line-retirement_manifest-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this retirement_manifest binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-retirement_manifest object publishes every required field.
    pub retirement_manifest_object_complete: bool,
    /// Whether the entry traces to the line-retirement_manifest registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5RetirementManifestEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5RetirementManifestNextAction,
    /// Whether the retirement_manifest resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub retirement_manifest_resolves_across_lines: bool,
}

impl M5ResolvedRetirementManifestEntry {
    /// Whether this line-retirement_manifest entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_manifest_change_diff_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ManifestChangeDiffEntryResolutionInput {
    ...
    pub comparison_scope: M5ManifestChangeDiffScope,
    /// The render / surface context.
    pub surface_context: M5RetirementManifestSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5RetirementManifestResolutionForm>,
    ...
    pub keeps_manifest_change_diff_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedManifestChangeDiffEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub manifest_change_diff_stays_honest: bool,
    ...
    pub provides_complete_manifest_change_diff: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5ManifestChangeDiffEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5RetirementManifestNextAction,
...
impl M5ResolvedManifestChangeDiffEntry {
...
pub enum M5RetirementManifestResolutionError {
    /// The line-retirement_manifest-entry id was empty.
...
impl M5RetirementManifestResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_retirement_manifest_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_manifest_change_diff_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5RetirementManifestResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-retirement_manifest / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5RetirementManifestResolutionError {}

fn form_tokens(forms: &[M5RetirementManifestResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5RetirementManifestResolutionForm]) -> bool {
    let present: BTreeSet<M5RetirementManifestResolutionForm> = forms.iter().copied().collect();
    M5RetirementManifestResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-retirement_manifest object publishes every required field: classified line journey,
...
pub fn retirement_manifest_object_is_complete(
    journey: M5RetirementManifestKind,
...
/// Whether the line retirement_manifest keeps a line from widening without preserving its rollback and diagnostics
...
pub fn manifest_change_diff_stays_honest(
    scope: M5ManifestChangeDiffScope,
    comparison_is_truthful: bool,
    keeps_manifest_change_diff_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_manifest_change_diff_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-retirement_manifest-registry entry so it stays bound to the line-retirement_manifest registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete retirement_manifest object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_retirement_manifest_entry(
    input: M5RetirementManifestEntryResolutionInput,
) -> Result<M5ResolvedRetirementManifestEntry, M5RetirementManifestResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5RetirementManifestResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5RetirementManifestResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = retirement_manifest_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5RetirementManifestEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5RetirementManifestEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5RetirementManifestEntryDegradeReason::CohortManifestFieldUnclassified)
    } else if !input.bound_to_registry {
        Some(M5RetirementManifestEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5RetirementManifestEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5RetirementManifestEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5RetirementManifestEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(
            M5RetirementManifestEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    } else if !input.proof_fresh {
        Some(M5RetirementManifestEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5RetirementManifestNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedRetirementManifestEntry {
        ...
        retirement_manifest_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        retirement_manifest_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_manifest_change_diff_entry(
    input: M5ManifestChangeDiffEntryResolutionInput,
) -> Result<M5ResolvedManifestChangeDiffEntry, M5RetirementManifestResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5RetirementManifestResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = manifest_change_diff_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_manifest_change_diff_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ManifestChangeDiffEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ManifestChangeDiffEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5ManifestChangeDiffEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5ManifestChangeDiffEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5ManifestChangeDiffEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5ManifestChangeDiffEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5RetirementManifestNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedManifestChangeDiffEntry {
        entry_id: input.entry_id,
        ...
        keeps_manifest_change_diff_visible: input.keeps_manifest_change_diff_visible,
        ...
        manifest_change_diff_stays_honest: record_stays_honest,
        provides_complete_manifest_change_diff: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-retirement_manifest and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementManifestChangeDiffRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5RetirementManifestChangeDiffRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5RetirementManifestAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5RetirementManifestExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5RetiredStateDowngradeTrigger>,
    /// Resolved line-retirement_manifest-registry examples.
    pub retirement_manifest_entries: Vec<M5ResolvedRetirementManifestEntry>,
    /// Resolved line-downgrade-packet examples.
    pub manifest_change_diff_entries: Vec<M5ResolvedManifestChangeDiffEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-retirement_manifest and
    ...
    pub collapses_distinct_manifest_change_diff_classes_into_one_lane: bool,
}

impl M5RetirementManifestChangeDiffRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5RetirementManifestAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5RetirementManifestAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5RetirementManifestExportField> =
            self.export_fields.iter().copied().collect();
        M5RetirementManifestExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_manifest_change_diff_classes_into_one_lane
    }

    /// True when a clean line-retirement_manifest entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete retirement_manifest object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn retirement_manifest_is_honest(ex: &M5ResolvedRetirementManifestEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.retirement_manifest_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedManifestChangeDiffEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_manifest_change_diff
                && ex.manifest_change_diff_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.retirement_manifest_entries
            .iter()
            .all(Self::retirement_manifest_is_honest)
            && self
                .manifest_change_diff_entries
                .iter()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementManifestChangeDiffRegistriesVocabularySet {
    ...
    /// Cohort-retirement_manifest-entry degrade-reason tokens.
    pub retirement_manifest_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub manifest_change_diff_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5RetirementManifestChangeDiffRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5RetiredStateRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5RetirementManifestResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5RetirementManifestKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ManifestChangeDiffScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5RetirementManifestSurfaceContext::ALL, |v| v.as_str()),
            retirement_manifest_degrade_reasons: tokens(
                &M5RetirementManifestEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            manifest_change_diff_degrade_reasons: tokens(
                &M5ManifestChangeDiffEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5RetirementManifestAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5RetirementManifestNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5RetirementManifestExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementManifestChangeDiffRegistriesGovernanceReview {
    /// The retirement_manifest registry names a canonical token, semantic role, and line journey for every entry.
    pub retirement_manifest_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-retirement_manifest object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_retirement_manifest_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved retirement_manifest.
    ...
    pub manifest_change_diff_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-retirement_manifest and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-retirement_manifest and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub retirement_manifest_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementManifestChangeDiffRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-retirement_manifest registry.
    ...
    /// Behavior traces back to the canonical line-retirement_manifest and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-retirement_manifest / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementManifestChangeDiffRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementManifestChangeDiffRegistriesReleasePosture {
...
/// Constructor input for [`M5RetirementManifestChangeDiffRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5RetirementManifestChangeDiffRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5RetirementManifestChangeDiffRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5RetirementManifestChangeDiffRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5RetirementManifestChangeDiffRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5RetirementManifestChangeDiffRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5RetirementManifestChangeDiffRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5RetirementManifestChangeDiffRegistriesReleasePosture,
...
/// Export-safe M5 line-retirement_manifest and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5RetirementManifestChangeDiffRegistriesPacket {
    /// Record kind; must equal [`M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION`].
...
impl M5RetirementManifestChangeDiffRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5RetirementManifestChangeDiffRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5RetirementManifestChangeDiffRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_RECORD_KIND {
            violations.push(M5RetirementManifestChangeDiffRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5RetirementManifestChangeDiffRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5RetirementManifestChangeDiffRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5RetirementManifestChangeDiffRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-retirement_manifest / line-downgrade-packet registries packet serializes",
        )) {
            violations.push(M5RetirementManifestChangeDiffRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,retirement_manifest_entries,manifest_change_diff_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            let degrades: Vec<&str> = row
                .retirement_manifest_entries
                .iter()
                .filter_map(|ex| ex.degrade_reason.map(|r| r.as_str()))
                .chain(
                    row.manifest_change_diff_entries
                        .iter()
                        .filter_map(|ex| ex.degrade_reason.map(|r| r.as_str())),
                )
                .collect();
            out.push_str(&format!(
                ...
                row.retirement_manifest_entries.len(),
                row.manifest_change_diff_entries.len(),
                degrades.join("|"),
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Retirement-Manifest and Manifest-Change-Diff Registries\n\n");
    ...
    /// registry-bound line-retirement_manifest entries are listed.
    pub fn render_retirement_manifest_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.retirement_manifest_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5RetirementManifestChangeDiffRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5RetirementManifestChangeDiffRegistriesViolation>),
}

impl fmt::Display for M5RetirementManifestChangeDiffRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-retirement_manifest / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-retirement_manifest / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5RetirementManifestChangeDiffRegistriesArtifactError {}

/// Validation failures emitted by [`M5RetirementManifestChangeDiffRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5RetirementManifestChangeDiffRegistriesViolation {
    ...
    /// A registry row does not point at both the line-retirement_manifest and line-downgrade-packet domain schemas.
    ...
    /// Cohort-retirement_manifest-resolution is not proven: clean retirement_manifest entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean retirement_manifest entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded retirement_manifest entry is present, or a clean retirement_manifest entry is unbounded
    /// or unbound.
...
impl M5RetirementManifestChangeDiffRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "retirement_manifest_resolution_not_proven"
            }
            Self::RollbackAndDiagnosticsPreservationNotProven => {
                "rollback_and_diagnostics_preservation_not_proven"
            }
            Self::CohortEvidenceIntegrityNotProven => "manifest_change_diff_integrity_not_proven",
...
pub fn current_stable_m5_retirement_manifest_and_change_diff_registries_export() -> Result<
    M5RetirementManifestChangeDiffRegistriesPacket,
    M5RetirementManifestChangeDiffRegistriesArtifactError,
> {
    let packet: M5RetirementManifestChangeDiffRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-retirement-manifest-and-change-diff-registries-proof/support_export.json"
        )
    ))
    .map_err(M5RetirementManifestChangeDiffRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5RetirementManifestChangeDiffRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5RetirementManifestChangeDiffRegistriesPacket,
    violations: &mut Vec<M5RetirementManifestChangeDiffRegistriesViolation>,
) {
    let refs: BTreeSet<&str> = packet
        ...
        .collect();
    for required in [
        M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_SCHEMA_REF,
        M5_RETIREMENT_MANIFEST_CHANGE_DIFF_REGISTRIES_DOC_REF,
        M5_RETIRED_STATE_MATRIX_SCHEMA_REF,
        M5_RETIRED_STATE_MATRIX_DOC_REF,
        M5_RETIREMENT_MANIFEST_DOMAIN_SCHEMA_REF,
        M5_MANIFEST_CHANGE_DIFF_DOMAIN_SCHEMA_REF,
    ] {
        if !refs.contains(required) {
            violations
                .push(M5RetirementManifestChangeDiffRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5RetirementManifestChangeDiffRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations
                .push(M5RetirementManifestChangeDiffRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5RetirementManifestChangeDiffRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5RetirementManifestChangeDiffRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_RETIREMENT_MANIFEST_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_MANIFEST_CHANGE_DIFF_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5RetirementManifestChangeDiffRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.retirement_manifest_entries.is_empty() || row.manifest_change_diff_entries.is_empty()
        {
            violations.push(M5RetirementManifestChangeDiffRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5RetirementManifestChangeDiffRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5RetirementManifestChangeDiffRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.retirement_manifest_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_retirement_manifest_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.manifest_change_diff_keeps_proof_visible_and_binds_support_language,
        ...
        review.retirement_manifest_or_downgrade_drift_caught_before_release,
        review.every_row_declares_mandatory_anatomy,
        review.reuses_frozen_matrix_vocabulary,
    ] {
        if !ok {
            violations.push(
                M5RetirementManifestChangeDiffRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5RetirementManifestChangeDiffRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5RetirementManifestChangeDiffRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5RetirementManifestChangeDiffRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let retirement_manifests = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.retirement_manifest_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.manifest_change_diff_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean retirement_manifest entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean retirement_manifest entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = retirement_manifests()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = retirement_manifests()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5RetirementManifestKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5RetirementManifestSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = retirement_manifests().any(|ex| {
        ex.degrade_reason
            == Some(M5RetirementManifestEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete =
        !retirement_manifests().any(|ex| ex.is_clean() && !ex.retirement_manifest_object_complete);
    if !(journeys_covered
    ...
    {
        violations.push(
            M5RetirementManifestChangeDiffRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }

    // AC2: line packets preserve rollback and diagnostics posture before widening. A widen-without-rollback
    // example degrades, an unbound example degrades, at least one clean bounded retirement_manifest entry is present, and
    // no clean retirement_manifest entry is unbounded or unbound.
    let widen_fold_degrades = retirement_manifests().any(|ex| {
        ex.degrade_reason
            == Some(
                M5RetirementManifestEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = retirement_manifests().any(|ex| {
        ex.degrade_reason
            == Some(M5RetirementManifestEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_retirement_manifest =
        retirement_manifests().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound = !retirement_manifests().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded =
        !retirement_manifests().any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_retirement_manifest
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
            M5RetirementManifestChangeDiffRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ManifestChangeDiffScope::CANONICAL_SCOPES
        .iter()
    ...
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_manifest_change_diff);
    ...
    {
        violations.push(
            M5RetirementManifestChangeDiffRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-retirement_manifest registry
