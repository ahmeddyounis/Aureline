//! Tests for the M05-1237 supported-line surface certification capstone.

use super::*;

fn packet() -> SupportedLineTransparencyProfileCertificationPacket {
    seeded_m5_supported_line_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, SUPPORTED_LINE_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, SUPPORTED_LINE_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5SupportedLineTransparencyCertifiedProfile::ALL {
...
fn every_frozen_family_is_certified_on_some_profile() {
    ...
    for family in M5SupportedLineTransparencyObject::ALL {
        assert!(
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            SupportedLineTransparencyCertificationAxis::ALL.len()
        );
    }
...
fn all_five_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5SupportedLineTransparencyClaim> =
        p.rows.iter().map(|r| r.certified_claim).collect();
    for claim in M5SupportedLineTransparencyClaim::ALL {
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5SupportedLineTransparencyCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(
...
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = SupportedLineTransparencyCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(
...
fn only_cli_export_axis_is_always_on() {
    for axis in SupportedLineTransparencyCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == SupportedLineTransparencyCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    // The B147 localization axis is present, distinct, and exercised by the
    // disclosed-correction-archive profile as its binding narrowed axis.
    let p = packet();
    let idx = p
        .rows
        .iter()
    ...
    let row = &p.rows[idx];
    let outcome = row
        .axis(SupportedLineTransparencyCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(SupportedLineTransparencyCertificationAxis::Localization)
    );
}

#[test]
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    // The B147 high-contrast reach axis stays certified across the seed (never a binding narrowed axis here).
    for row in &packet().rows {
        let outcome = row
            .axis(SupportedLineTransparencyCertificationAxis::HighContrast)
...
fn the_three_yellow_rows_bind_three_distinct_axes_and_triggers() {
    ...
    // per-lane concern (correction-archive coverage / migration-scoreboard currency / ORR-history retention).
    let p = packet();
    let axes: BTreeSet<SupportedLineTransparencyCertificationAxis> = p
        .rows
        .iter()
    ...
    let triggers: std::collections::HashSet<M5SupportedLineTransparencyDowngradeTrigger> = p
        .rows
        .iter()
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5SupportedLineTransparencyCertifiedProfile::LiveSupportedLineOperatingLane
        })
        .expect("live-certified-widening-lane row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == SupportedLineTransparencyCertificationAxis::Visual {
            outcome.state = SupportedLineTransparencyAxisCertificationState::UndisclosedDrift;
    ...
    assert_eq!(
        row.derived_status,
        SupportedLineTransparencyProfileClaimStatus::Red
    );
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5SupportedLineTransparencyClaim::CertifiedOperatingLine;
    row.certified_claim = M5SupportedLineTransparencyClaim::CertifiedOperatingLine;
    assert_eq!(
        row.derive_status(),
        SupportedLineTransparencyProfileClaimStatus::Red
    );
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == SupportedLineTransparencyCertificationAxis::DegradedState {
            *outcome = SupportedLineTransparencyAxisOutcome {
                axis: SupportedLineTransparencyCertificationAxis::DegradedState,
                state: SupportedLineTransparencyAxisCertificationState::DisclosedNarrowed,
                parity_note: "the supported-line proof is lagging".to_owned(),
                narrowing_reason: Some(
                    "the supported-line proof is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5SupportedLineTransparencyDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableTransparencySurface == certified, no claim_auto_narrow.
    assert_eq!(
        row.derive_status(),
        SupportedLineTransparencyProfileClaimStatus::Red
    );
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == SupportedLineTransparencyCertificationAxis::CliExport {
            outcome.state = SupportedLineTransparencyAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger =
                Some(M5SupportedLineTransparencyDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(
        row.derive_status(),
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(SupportedLineTransparencyClaimAutoNarrow {
        binding_axis: SupportedLineTransparencyCertificationAxis::DegradedState,
        from_claim: M5SupportedLineTransparencyClaim::CertifiedOperatingLine,
        to_claim: M5SupportedLineTransparencyClaim::CertifiedOperatingLine,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5SupportedLineTransparencyClaim::CorrectionArchiveDisclosedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-stable-line-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableTransparencySurface; certify a stronger certified operating line.
    row.certified_claim = M5SupportedLineTransparencyClaim::CertifiedOperatingLine;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = SupportedLineTransparencyCertificationAxis::Visual;
    }
    assert_eq!(
        row.derive_status(),
        SupportedLineTransparencyProfileClaimStatus::Red
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = SupportedLineTransparencyCertificationAxis::CliExport;
    }
    assert_eq!(
        row.derive_status(),
        SupportedLineTransparencyProfileClaimStatus::Red
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == SupportedLineTransparencyProfileClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "migration scoreboard".to_owned();
    }
    assert_eq!(
        row.derive_status(),
        SupportedLineTransparencyProfileClaimStatus::Red
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(SupportedLineTransparencyCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5SupportedLineTransparencyDowngradeTrigger::ProofStale,
    );
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows.retain(|r| {
        r.profile != M5SupportedLineTransparencyCertifiedProfile::UnverifiedOrrHistoryProfile
    });
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        SupportedLineTransparencyCertificationViolation::ProfileCoverageIncomplete
    )));
}
...
fn missing_family_coverage_is_rejected() {
    ...
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5SupportedLineTransparencyObject::PublicProofLedger);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        SupportedLineTransparencyCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = SupportedLineTransparencyProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        SupportedLineTransparencyCertificationViolation::WrongCanonicalBundle
    )));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != SupportedLineTransparencyCertificationAxis::SupportedLineProofTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        SupportedLineTransparencyCertificationViolation::SummaryMismatch
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: SupportedLineTransparencyProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_supported_line_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-supported-line-surface-certification/support_export.json"
    ));
    let packet_from_fixture: SupportedLineTransparencyProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-supported-line-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_SUPPORTED_LINE_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures
...
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_SUPPORTED_LINE_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_supported_line_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/release/m5-supported-line-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-supported-line-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/release/m5-supported-line-surface-certification");
