//! Implemented M5 last-supported-snapshot and archive-export-gate registries.
//!
//! The frozen [retired-state matrix][matrix] names Aureline's governed terminal-lifecycle object classes and locks
//! their controlled vocabulary. This module ships last-supported snapshot and retirement archive bundles for the
//! first consumer surfaces — release-center, help / docs, support, marketplace / registry, install / update, and
//! partner / procurement — so a retiring M5 line or stable-facing surface leaves a trustworthy last-supported
//! reference that migration, audit, procurement, and support can inspect without keeping the retired surface live
//! forever, rather than destroying its docs, schemas, and evidence at cutoff. It turns the *last-supported-snapshot*
//! grammar (one export-safe snapshot per retiring object — one classified snapshot field per preserved fact, typed
//! as the docs / help truth, the schema / contract set, the known-limits snapshot, the compatibility report, the
//! provenance / SBOM reference, or the support-article links, with an owning team and joined to the exact-build
//! identity, the retirement manifest, and the review packet) and the *archive-export-gate* grammar (the export-safety
//! blocker a bundle is stopped by — a live vendor dependency, an internal-only or secret leak, or a missing binding
//! back to the retirement manifest and review packet) into registry resolvers that produce export-safe, honest
//! projections. Every retiring object then resolves to one typed last-supported-snapshot object — the docs / help
//! truth, schemas / contracts, compatibility report, known limits, provenance / SBOM refs, and support-article links
//! it preserves for the final supported build or line state, joined to its exact build so the historical reference
//! stays inspectable offline — and to one typed archive-export-gate object — the resolved object identity, the
//! affected retirement-manifest reference, the bundle-versus-successor reference, the blocked archive handoff, and
//! the active block reason — that self-hosted, offline, and procurement / support consumers can read without live
//! vendor dependencies, so an archive bundle is never handed off while it carries a live dependency, a leaked secret,
//! or an unbound manifest, and a snapshot that cannot bind a fact to its exact build degrades honestly instead of
//! shipping an untrustworthy historical reference.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Generate last-supported snapshot bundles that capture docs / help truth, schemas / contracts, compatibility
//!   reports, known limits, provenance / SBOM references, and support-article links for the final supported build or
//!   line state.**
//!   [`resolve_last_supported_snapshot_entry`] refuses to read as a clean, registry-bound snapshot entry
//!   unless it names a canonical registry token, a classified [snapshot field][M5LastSupportedSnapshotKind], a
//!   retirement role, covers every [resolution form][M5LastSupportedSnapshotResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every snapshot field (docs / help
//!   truth, schema / contract set, known-limits snapshot, compatibility report, provenance / SBOM reference, and
//!   support-article links bound to exact-build joins), and keeps a public-facing compatibility / support field
//!   matched to the archived successor; otherwise it degrades.
//! * **Keep archive bundles export-safe and mirror-aware so self-hosted, offline, and procurement / support
//!   consumers can inspect the historical reference without live vendor dependencies.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a snapshot entry whose exact-build joins
...
//!   and a public-facing compatibility / support field whose language outruns the archived successor degrades the
//!   same way — the structured blocker reason an untrustworthy historical reference must surface.
//! * **Bind every archive bundle back to the retirement manifest and review packet so the historical pack explains
//!   why the surface retired and what replaced it.**
//!   [`resolve_archive_export_gate_entry`] names a classified [archive-export scope][M5ArchiveExportGateScope]
//!   (live-dependency-present, internal-only-or-secret-leak, or unbound-manifest-or-review-packet), requires the
//!   full bundle-identity / affected-manifest-reference / bundle-versus-successor / block-scope / active-reason gate
...
//!   when the gate would hand off a bundle carrying a live dependency, leak a secret or internal-only detail, or drop
//!   its binding back to the retirement manifest and review packet, so release, support, and procurement consumers
//!   can never ship an untrustworthy or non-reproducible archive.
//!
...
use crate::m5_retired_state_matrix::{
    ...
    M5_LAST_SUPPORTED_SNAPSHOT_DOMAIN_SCHEMA_REF, M5_RETIRED_STATE_MATRIX_DOC_REF,
    M5_RETIRED_STATE_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5LastSupportedSnapshotArchiveExportGateRegistriesPacket`].
pub const M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_last_supported_snapshot_and_archive_export_gate_packet_registries";

/// Schema version for M5 last-supported-snapshot / archive-export-gate registry records.
pub const M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_DOC_REF: &str =
    "docs/release/m5_last_supported_snapshot_and_archive_export_gate_registries.md";

/// Repo-relative path of the canonical archive-export-gate domain schema minted by this lane (the
/// machine-readable gate that stops a last-supported snapshot / retirement archive bundle from being handed off
/// while it carries a live vendor dependency, would leak a secret or internal-only detail, or is not bound back to
/// the retirement manifest and review packet).
pub const M5_ARCHIVE_EXPORT_GATE_DOMAIN_SCHEMA_REF: &str =
    "schemas/program/m5-archive-export-gate.schema.json";

// The canonical last-supported-snapshot domain schema for Registry-A is reused from the frozen retired-state
// matrix (minted there for the bundle and schema-bearing-surface object classes) via
// [`M5_LAST_SUPPORTED_SNAPSHOT_DOMAIN_SCHEMA_REF`], so this lane does not mint a parallel snapshot schema.

/// Repo-relative path of the checked support-export artifact.
pub const M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-last-supported-snapshot-and-archive-export-gate-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-last-supported-snapshot-and-archive-export-gate-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-last-supported-snapshot-and-archive-export-gate-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5LastSupportedSnapshotArchiveExportGateRegistriesConsumerSurface =
    M5RetiredStateConsumerSurface;

/// One of the three resolution forms every line-last_supported_snapshot or line-downgrade-packet entry must hold across so
...
/// line-last_supported_snapshot and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LastSupportedSnapshotResolutionForm {
    /// The canonical resolved line-last_supported_snapshot / line-downgrade-packet object.
    CanonicalObject,
...
impl M5LastSupportedSnapshotResolutionForm {
...
/// Controlled snapshot field a last-supported-snapshot entry captures for one retiring object-class identity, so
/// every retiring object shares one typed registry rather than a hand-copied per-class assumption. Minted by this
/// lane because the frozen matrix names the retired classes but not the concrete facts a last-supported bundle
/// preserves — the docs / help truth, the schema / contract set, the known-limits snapshot, the compatibility
/// report, the provenance / SBOM reference, or the support-article links for the final supported build or line
/// state. Every classified field carries its canonical mode, and the compatibility-report and support-article-links
/// fields are public-facing (they surface directly in the customer / procurement-facing historical bundle) so their
/// claim must stay matched to the archived successor before the bundle is handed off.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LastSupportedSnapshotKind {
    /// The docs-help-truth field: the docs / help content captured for the final supported build so the historical
    /// reference reads as it did at last-supported time.
    DocsHelpTruth,
    /// The schema-contract-set field: the schemas / contracts pinned for the final supported build so consumers can
    /// still validate against what was supported.
    SchemaContractSet,
    /// The known-limits-snapshot field: the known limits / caveats captured at last-supported time so nothing is
    /// silently lost at retirement.
    KnownLimitsSnapshot,
    /// The compatibility-report field (public-facing; the final compatibility state whose claim must match the
    /// archived successor / final supported version).
    CompatibilityReportReference,
    /// The support-article-links field (public-facing; the support / help article links preserved so support can
    /// still answer questions about the retired surface).
    SupportArticleLinks,
    /// The provenance-SBOM-reference field: the provenance / SBOM reference bound to the exact build so the archive
    /// stays auditable.
    ProvenanceSbomReference,
    /// The snapshot field is unclassified, which is disallowed.
    SnapshotFieldUnclassified,
}

impl M5LastSupportedSnapshotKind {
    /// Every public-proof-ledger kind, in declaration order.
    pub const ALL: [Self; 7] = [
        Self::DocsHelpTruth,
        Self::SchemaContractSet,
        Self::KnownLimitsSnapshot,
        Self::CompatibilityReportReference,
        Self::SupportArticleLinks,
        Self::ProvenanceSbomReference,
        Self::SnapshotFieldUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SchemaContractSet => "schema_contract_set",
            Self::KnownLimitsSnapshot => "known_limits_snapshot",
            Self::CompatibilityReportReference => "compatibility_report_reference",
            Self::SupportArticleLinks => "support_article_links",
            Self::ProvenanceSbomReference => "provenance_sbom_reference",
            Self::SnapshotFieldUnclassified => "snapshot_field_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::SnapshotFieldUnclassified)
    }

    /// The canonical mode for this public-proof-ledger kind.
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::DocsHelpTruth => "docs_help_truth_mode",
            Self::SchemaContractSet => "schema_contract_set_mode",
            Self::KnownLimitsSnapshot => "known_limits_snapshot_mode",
            Self::CompatibilityReportReference => "compatibility_report_reference_mode",
            Self::SupportArticleLinks => "support_article_links_mode",
            Self::ProvenanceSbomReference => "provenance_sbom_reference_mode",
            Self::SnapshotFieldUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(
            self,
            Self::CompatibilityReportReference | Self::SupportArticleLinks
        )
    }
}

/// Controlled archive-export scope a last-supported bundle's export-safety gate sits in, so a retirement archive
/// bundle stops being handed off while it is not export-safe rather than silently shipping an untrustworthy or
/// non-reproducible historical reference, and shares one registry rather than a hand-copied per-record assumption.
/// Minted by this lane, tracking whether the blocker is a live vendor dependency (live-dependency-present), an
/// internal-only or secret leak (internal-only-or-secret-leak), or a missing binding back to the retirement manifest
/// and review packet (unbound-manifest-or-review-packet). Each scope maps directly to an acceptance criterion —
/// open without live service dependencies, exclude live secrets / internal-only detail, and explain why the surface
/// retired and what replaced it — so an untrustworthy archive can never be handed off.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ArchiveExportGateScope {
    /// Live-dependency-present: the block that stops a bundle from being handed off while it still carries a live
    /// vendor dependency and cannot be opened offline.
    LiveDependencyPresent,
    /// Internal-only-or-secret-leak: the block that stops a bundle from being handed off while it would leak a live
    /// secret or internal-only detail.
    InternalOnlyOrSecretLeak,
    /// Unbound-manifest-or-review-packet: the block that stops a bundle from being handed off while it is not bound
    /// back to the retirement manifest and review packet that explain why the surface retired and what replaced it.
    UnboundManifestOrReviewPacket,
    /// The archive-export scope is unclassified, which is disallowed.
    ArchiveExportGateScopeUnclassified,
}

impl M5ArchiveExportGateScope {
    /// Every comparison scope, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::LiveDependencyPresent,
        Self::InternalOnlyOrSecretLeak,
        Self::UnboundManifestOrReviewPacket,
        Self::ArchiveExportGateScopeUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::LiveDependencyPresent => "live_dependency_present",
            Self::InternalOnlyOrSecretLeak => "internal_only_or_secret_leak",
            Self::UnboundManifestOrReviewPacket => "unbound_manifest_or_review_packet",
            Self::ArchiveExportGateScopeUnclassified => "archive_export_gate_scope_unclassified",
        }
    }

    /// Whether the comparison scope is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::ArchiveExportGateScopeUnclassified)
    }
}

/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-last_supported_snapshot or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LastSupportedSnapshotSurfaceContext {
...
impl M5LastSupportedSnapshotSurfaceContext {
...
/// One mandatory rendered part a line-last_supported_snapshot or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LastSupportedSnapshotAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-last_supported_snapshot entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-last_supported_snapshot entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-last_supported_snapshot
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line last_supported_snapshot or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5LastSupportedSnapshotAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "archive_export_gate_fields",
...
/// line last_supported_snapshot, a line-downgrade packet, or a degraded line-last_supported_snapshot / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LastSupportedSnapshotNextAction {
    /// Expand the resolved line last_supported_snapshot's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5LastSupportedSnapshotNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LastSupportedSnapshotExportField {
...
impl M5LastSupportedSnapshotExportField {
...
/// Reason a line-last_supported_snapshot entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LastSupportedSnapshotEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the last_supported_snapshot means.
    ...
    /// The resolved line-last_supported_snapshot object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5LastSupportedSnapshotEntryDegradeReason {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "last_supported_snapshot_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortManifestFieldUnclassified => "line_snapshot_field_unclassified",
            Self::DescriptorNotBoundToRegistry => "last_supported_snapshot_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "last_supported_snapshot_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "last_supported_snapshot_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5LastSupportedSnapshotNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5LastSupportedSnapshotNextAction::TraceCanonicalRegistry
            }
            Self::CohortManifestFieldUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5LastSupportedSnapshotNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5LastSupportedSnapshotNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5LastSupportedSnapshotNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ArchiveExportGateEntryDegradeReason {
...
impl M5ArchiveExportGateEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::EvidenceScopeUnclassified => "comparison_archive_export_gate_scope_unclassified",
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                "archive_export_gate_runs_support_ahead_of_proof_or_drops_archive_export_gate"
            }
    ...
    pub const fn next_action(self) -> M5LastSupportedSnapshotNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5LastSupportedSnapshotNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_last_supported_snapshot_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5LastSupportedSnapshotEntryResolutionInput {
    /// Stable identity of the line-last_supported_snapshot-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this last_supported_snapshot binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.last_supported_snapshot.support_article_links`); empty means unstated.
    ...
    pub report_section: M5LastSupportedSnapshotKind,
    /// The render / surface context.
    pub surface_context: M5LastSupportedSnapshotSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5LastSupportedSnapshotResolutionForm>,
    ...
    /// True when the behavior traces to the line-last_supported_snapshot registry (never a hand-copied constant).
...
/// Resolved, export-safe line-last_supported_snapshot-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedLastSupportedSnapshotEntry {
    /// Stable identity of the line-last_supported_snapshot-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this last_supported_snapshot binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-last_supported_snapshot object publishes every required field.
    pub last_supported_snapshot_object_complete: bool,
    /// Whether the entry traces to the line-last_supported_snapshot registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5LastSupportedSnapshotEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5LastSupportedSnapshotNextAction,
    /// Whether the last_supported_snapshot resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub last_supported_snapshot_resolves_across_lines: bool,
}

impl M5ResolvedLastSupportedSnapshotEntry {
    /// Whether this line-last_supported_snapshot entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_archive_export_gate_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ArchiveExportGateEntryResolutionInput {
    ...
    pub comparison_scope: M5ArchiveExportGateScope,
    /// The render / surface context.
    pub surface_context: M5LastSupportedSnapshotSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5LastSupportedSnapshotResolutionForm>,
    ...
    pub keeps_archive_export_gate_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedArchiveExportGateEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub archive_export_gate_stays_honest: bool,
    ...
    pub provides_complete_archive_export_gate: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5ArchiveExportGateEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5LastSupportedSnapshotNextAction,
...
impl M5ResolvedArchiveExportGateEntry {
...
pub enum M5LastSupportedSnapshotResolutionError {
    /// The line-last_supported_snapshot-entry id was empty.
...
impl M5LastSupportedSnapshotResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_last_supported_snapshot_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_archive_export_gate_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5LastSupportedSnapshotResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-last_supported_snapshot / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5LastSupportedSnapshotResolutionError {}

fn form_tokens(forms: &[M5LastSupportedSnapshotResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5LastSupportedSnapshotResolutionForm]) -> bool {
    let present: BTreeSet<M5LastSupportedSnapshotResolutionForm> = forms.iter().copied().collect();
    M5LastSupportedSnapshotResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-last_supported_snapshot object publishes every required field: classified line journey,
...
pub fn last_supported_snapshot_object_is_complete(
    journey: M5LastSupportedSnapshotKind,
...
/// Whether the line last_supported_snapshot keeps a line from widening without preserving its rollback and diagnostics
...
pub fn archive_export_gate_stays_honest(
    scope: M5ArchiveExportGateScope,
    comparison_is_truthful: bool,
    keeps_archive_export_gate_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_archive_export_gate_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-last_supported_snapshot-registry entry so it stays bound to the line-last_supported_snapshot registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete last_supported_snapshot object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_last_supported_snapshot_entry(
    input: M5LastSupportedSnapshotEntryResolutionInput,
) -> Result<M5ResolvedLastSupportedSnapshotEntry, M5LastSupportedSnapshotResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5LastSupportedSnapshotResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5LastSupportedSnapshotResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = last_supported_snapshot_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        ...
        Some(
            M5LastSupportedSnapshotEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    ...
    let next_action = match degrade_reason {
        ...
        None => M5LastSupportedSnapshotNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedLastSupportedSnapshotEntry {
        ...
        last_supported_snapshot_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        last_supported_snapshot_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_archive_export_gate_entry(
    input: M5ArchiveExportGateEntryResolutionInput,
) -> Result<M5ResolvedArchiveExportGateEntry, M5LastSupportedSnapshotResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5LastSupportedSnapshotResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = archive_export_gate_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_archive_export_gate_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ArchiveExportGateEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        ...
        Some(M5ArchiveExportGateEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5LastSupportedSnapshotNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedArchiveExportGateEntry {
        entry_id: input.entry_id,
        ...
        keeps_archive_export_gate_visible: input.keeps_archive_export_gate_visible,
        ...
        archive_export_gate_stays_honest: record_stays_honest,
        provides_complete_archive_export_gate: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-last_supported_snapshot and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LastSupportedSnapshotArchiveExportGateRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5LastSupportedSnapshotArchiveExportGateRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5LastSupportedSnapshotAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5LastSupportedSnapshotExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5RetiredStateDowngradeTrigger>,
    /// Resolved line-last_supported_snapshot-registry examples.
    pub last_supported_snapshot_entries: Vec<M5ResolvedLastSupportedSnapshotEntry>,
    /// Resolved line-downgrade-packet examples.
    pub archive_export_gate_entries: Vec<M5ResolvedArchiveExportGateEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-last_supported_snapshot and
    ...
    pub collapses_distinct_archive_export_gate_classes_into_one_lane: bool,
}

impl M5LastSupportedSnapshotArchiveExportGateRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5LastSupportedSnapshotAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5LastSupportedSnapshotAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5LastSupportedSnapshotExportField> =
            self.export_fields.iter().copied().collect();
        M5LastSupportedSnapshotExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_archive_export_gate_classes_into_one_lane
    }

    /// True when a clean line-last_supported_snapshot entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete last_supported_snapshot object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn last_supported_snapshot_is_honest(ex: &M5ResolvedLastSupportedSnapshotEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.last_supported_snapshot_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedArchiveExportGateEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_archive_export_gate
                && ex.archive_export_gate_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.last_supported_snapshot_entries
            .iter()
            .all(Self::last_supported_snapshot_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LastSupportedSnapshotArchiveExportGateRegistriesVocabularySet {
    ...
    /// Cohort-last_supported_snapshot-entry degrade-reason tokens.
    pub last_supported_snapshot_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub archive_export_gate_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5LastSupportedSnapshotArchiveExportGateRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5RetiredStateRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5LastSupportedSnapshotResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5LastSupportedSnapshotKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ArchiveExportGateScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5LastSupportedSnapshotSurfaceContext::ALL, |v| v.as_str()),
            last_supported_snapshot_degrade_reasons: tokens(
                &M5LastSupportedSnapshotEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            archive_export_gate_degrade_reasons: tokens(
                &M5ArchiveExportGateEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5LastSupportedSnapshotAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5LastSupportedSnapshotNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5LastSupportedSnapshotExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LastSupportedSnapshotArchiveExportGateRegistriesGovernanceReview {
    /// The last_supported_snapshot registry names a canonical token, semantic role, and line journey for every entry.
    pub last_supported_snapshot_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-last_supported_snapshot object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_last_supported_snapshot_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved last_supported_snapshot.
    ...
    pub archive_export_gate_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-last_supported_snapshot and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-last_supported_snapshot and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub last_supported_snapshot_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LastSupportedSnapshotArchiveExportGateRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-last_supported_snapshot registry.
    ...
    /// Behavior traces back to the canonical line-last_supported_snapshot and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-last_supported_snapshot / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LastSupportedSnapshotArchiveExportGateRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LastSupportedSnapshotArchiveExportGateRegistriesReleasePosture {
...
/// Constructor input for [`M5LastSupportedSnapshotArchiveExportGateRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5LastSupportedSnapshotArchiveExportGateRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5LastSupportedSnapshotArchiveExportGateRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5LastSupportedSnapshotArchiveExportGateRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5LastSupportedSnapshotArchiveExportGateRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5LastSupportedSnapshotArchiveExportGateRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5LastSupportedSnapshotArchiveExportGateRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5LastSupportedSnapshotArchiveExportGateRegistriesReleasePosture,
...
/// Export-safe M5 line-last_supported_snapshot and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LastSupportedSnapshotArchiveExportGateRegistriesPacket {
    /// Record kind; must equal [`M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_SCHEMA_VERSION`].
...
impl M5LastSupportedSnapshotArchiveExportGateRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5LastSupportedSnapshotArchiveExportGateRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version:
                M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5LastSupportedSnapshotArchiveExportGateRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_RECORD_KIND
        {
            violations
                .push(M5LastSupportedSnapshotArchiveExportGateRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_LAST_SUPPORTED_SNAPSHOT_ARCHIVE_EXPORT_GATE_REGISTRIES_SCHEMA_VERSION
        {
            violations.push(
                M5LastSupportedSnapshotArchiveExportGateRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations
                .push(M5LastSupportedSnapshotArchiveExportGateRegistriesViolation::MissingIdentity);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-last_supported_snapshot / line-downgrade-packet registries packet serializes",
        )) {
            violations.push(
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,last_supported_snapshot_entries,archive_export_gate_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            let degrades: Vec<&str> = row
                ...
                .chain(
                    row.archive_export_gate_entries
                        .iter()
                        .filter_map(|ex| ex.degrade_reason.map(|r| r.as_str())),
                )
                .collect();
            out.push_str(&format!(
                ...
                row.last_supported_snapshot_entries.len(),
                row.archive_export_gate_entries.len(),
                degrades.join("|"),
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Last-Supported-Snapshot and Archive-Export-Gate Registries\n\n");
        ...
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                "  - Snapshot entries: {} / archive-export-gate entries: {}\n",
                row.last_supported_snapshot_entries.len(),
                row.archive_export_gate_entries.len()
            ));
        }
    ...
    /// registry-bound line-last_supported_snapshot entries are listed.
    pub fn render_last_supported_snapshot_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.last_supported_snapshot_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5LastSupportedSnapshotArchiveExportGateRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5LastSupportedSnapshotArchiveExportGateRegistriesViolation>),
}

impl fmt::Display for M5LastSupportedSnapshotArchiveExportGateRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-last_supported_snapshot / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-last_supported_snapshot / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5LastSupportedSnapshotArchiveExportGateRegistriesArtifactError {}

/// Validation failures emitted by [`M5LastSupportedSnapshotArchiveExportGateRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5LastSupportedSnapshotArchiveExportGateRegistriesViolation {
    ...
    /// A registry row does not point at both the line-last_supported_snapshot and line-downgrade-packet domain schemas.
    ...
    /// Cohort-last_supported_snapshot-resolution is not proven: clean last_supported_snapshot entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean last_supported_snapshot entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded last_supported_snapshot entry is present, or a clean last_supported_snapshot entry is unbounded
    /// or unbound.
...
impl M5LastSupportedSnapshotArchiveExportGateRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "last_supported_snapshot_resolution_not_proven"
            }
            Self::RollbackAndDiagnosticsPreservationNotProven => {
                "rollback_and_diagnostics_preservation_not_proven"
            }
            Self::CohortEvidenceIntegrityNotProven => "archive_export_gate_integrity_not_proven",
...
pub fn current_stable_m5_last_supported_snapshot_and_archive_export_gate_registries_export(
) -> Result<
    M5LastSupportedSnapshotArchiveExportGateRegistriesPacket,
    M5LastSupportedSnapshotArchiveExportGateRegistriesArtifactError,
> {
    let packet: M5LastSupportedSnapshotArchiveExportGateRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-last-supported-snapshot-and-archive-export-gate-registries-proof/support_export.json"
        )
    ))
    .map_err(M5LastSupportedSnapshotArchiveExportGateRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5LastSupportedSnapshotArchiveExportGateRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5LastSupportedSnapshotArchiveExportGateRegistriesPacket,
    violations: &mut Vec<M5LastSupportedSnapshotArchiveExportGateRegistriesViolation>,
) {
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations
            .push(M5LastSupportedSnapshotArchiveExportGateRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5LastSupportedSnapshotArchiveExportGateRegistriesViolation::RegistryRowIncomplete,
            );
        }
        ...
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_LAST_SUPPORTED_SNAPSHOT_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_ARCHIVE_EXPORT_GATE_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5LastSupportedSnapshotArchiveExportGateRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.last_supported_snapshot_entries.is_empty()
            || row.archive_export_gate_entries.is_empty()
        {
            violations
                .push(M5LastSupportedSnapshotArchiveExportGateRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(
                M5LastSupportedSnapshotArchiveExportGateRegistriesViolation::DishonestExample,
            );
        }
...
fn validate_governance_review(
    ...
    for ok in [
        review.last_supported_snapshot_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_last_supported_snapshot_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.archive_export_gate_keeps_proof_visible_and_binds_support_language,
        ...
        review.last_supported_snapshot_or_downgrade_drift_caught_before_release,
...
fn validate_acceptance_criteria(
    ...
    let last_supported_snapshots = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.last_supported_snapshot_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.archive_export_gate_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean last_supported_snapshot entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean last_supported_snapshot entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = last_supported_snapshots()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = last_supported_snapshots()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5LastSupportedSnapshotKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5LastSupportedSnapshotSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = last_supported_snapshots().any(|ex| {
        ex.degrade_reason
            == Some(M5LastSupportedSnapshotEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete = !last_supported_snapshots()
        .any(|ex| ex.is_clean() && !ex.last_supported_snapshot_object_complete);
    if !(journeys_covered
    ...
    // example degrades, an unbound example degrades, at least one clean bounded last_supported_snapshot entry is present, and
    // no clean last_supported_snapshot entry is unbounded or unbound.
    let widen_fold_degrades = last_supported_snapshots().any(|ex| {
        ex.degrade_reason
            == Some(
                M5LastSupportedSnapshotEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = last_supported_snapshots().any(|ex| {
        ex.degrade_reason
            == Some(M5LastSupportedSnapshotEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_last_supported_snapshot =
        last_supported_snapshots().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound =
        !last_supported_snapshots().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded =
        !last_supported_snapshots().any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_last_supported_snapshot
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ArchiveExportGateScope::CANONICAL_SCOPES
        .iter()
    ...
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_archive_export_gate);
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-last_supported_snapshot registry
