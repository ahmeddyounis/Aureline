//! Implemented M5 resolve-or-close-sheet and resolution-outcome registries.
//!
//! The frozen [change-intent lifecycle matrix][matrix] names Aureline's governed change-intent object classes and
//! locks their controlled vocabulary. This module makes the `resolve_close_sheet` class operable so the work-item
//! detail, review detail, Git / worktree, provider-handoff, help / docs, and support / export surfaces inherit one
//! canonical resolve-or-close descriptor rather than a hand-authored parallel prose that has to be kept
//! consistent: it turns the *resolve-or-close-sheet* grammar (one reusable sheet per tracked work item that makes a
//! terminal status transition explicit — the current state, the requested terminal state, the unresolved blockers,
//! the linked evidence, the permission scope, the confirm / reopen / export actions, and the final side-effect
//! preview — so blockers, provider authority, and reopen behaviour are always named before the transition can be
//! confirmed) and the *resolution-outcome* grammar (the terminal outcome a resolve-or-close can take — resolved
//! locally, provider updated, queued for publish, blocked by missing permission, or blocked by unresolved
//! engineering state — kept honest about what the provider has actually accepted, what stays local, and what remains
//! blocked) into registry resolvers that produce export-safe, honest projections. Every tracked item then resolves
//! to one typed resolve-or-close sheet — its current state, its requested terminal state, its unresolved blockers,
//! its linked evidence, its permission scope, and its confirm / reopen / export actions, all attributable to the
//! work item so a local-only resolution never reads as a provider-accepted terminal state — and to one
//! resolution-outcome object — the resolved outcome identity, the affected terminal-state field, the
//! previous-versus-current commit-state reference, the binding state, and the active resolution reason (resolved
//! locally, provider updated, queued for publish, blocked by missing permission, or blocked by unresolved
//! engineering state) — that the work-item, review, provider-handoff, and support surfaces can inspect without
//! manual reconstruction, so a resolution never implies the provider accepted the terminal state when the target is
//! offline, policy-blocked, or only partially writable, and a sheet that cannot bind an evidence field to its commit
//! state degrades honestly instead of letting a local-only resolution masquerade as a provider-updated close.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable resolve-or-close sheet per tracked work item, carrying every terminal-transition field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound resolve-or-close entry
//!   unless it names a canonical registry token, a classified [terminal-state selector][M5ResolveCloseSheetKind],
//!   a disclosure role, covers every [resolution form][M5ResolveCloseSheetResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every sheet field (current state,
//!   requested terminal state, unresolved blockers, linked evidence, permission scope, confirm / reopen / export
//!   actions, and the final side-effect preview), keeps its evidence lineage attributable to the tracked item, and
//!   keeps the final-resolution authority bound to its commit state; otherwise it degrades.
//! * **Preserve reopen / export continuity rather than letting a resolve drop its blockers or evidence.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a resolve-or-close entry that would confirm
//!   a terminal state while an engineering blocker remains unresolved or that drops the reopen / export path, so it
//!   degrades to
//!   [`M5ResolveCloseSheetEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a permission-scoped field whose language outruns its commit state degrades the same
//!   way — the structured blocker reason a local-shown-as-provider close must surface.
//! * **Turn resolved-locally, provider-updated, queued-for-publish, blocked-by-missing-permission, and
//!   blocked-by-unresolved-engineering-state into a visible, typed resolution-outcome event instead of an implied
//!   provider acceptance.** [`resolve_review_pack_result_entry`] names a classified
//!   [outcome binding][M5ResolveCloseOutcomeScope]
//!   (terminal-state binding, commit-state binding, or permission-scope binding), requires the full
//!   outcome-identity / affected-terminal-state-field / previous-versus-current-commit-state / binding / active-reason
//!   resolution-outcome object, covers every resolution form, and degrades to
//!   [`M5ResolveCloseOutcomeEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the outcome would present a queued or local-only resolution as provider-accepted, hide the offline /
//!   policy-blocked / partially-writable state, or let an evidence gap masquerade as full coverage, so a
//!   resolution-outcome can never read as accepted when it has quietly dropped the reason the provider could not
...
//! can never fork their own resolution meaning. Raw secret values and private endpoints stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_resolve_close_sheet_registries,
    seeded_m5_resolve_close_sheet_registries_review_pack_record_beta_narrowed,
    seeded_m5_resolve_close_sheet_registries_review_pack_result_preview_narrowed,
    M5_RESOLVE_CLOSE_SHEET_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5ResolveCloseSheetRegistriesPacket`].
pub const M5_RESOLVE_CLOSE_SHEET_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_resolve_close_sheet_and_relation_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_RESOLVE_CLOSE_SHEET_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_RESOLVE_CLOSE_SHEET_REGISTRIES_SCHEMA_REF: &str =
    "schemas/teamwork/m5-resolve-close-sheet-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_RESOLVE_CLOSE_SHEET_REGISTRIES_DOC_REF: &str =
    "docs/team-workflows/m5_resolve_close_sheet_registries.md";
...
pub const M5_RESOLVE_CLOSE_SHEET_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-resolve-close-sheet-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_RESOLVE_CLOSE_SHEET_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-resolve-close-sheet-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_RESOLVE_CLOSE_SHEET_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-resolve-close-sheet-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_RESOLVE_CLOSE_SHEET_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/teamwork/m5-resolve-close-sheet-registries";
...
pub type M5ResolveCloseSheetRegistriesConsumerSurface = M5ChangeIntentConsumerSurface;
...
pub enum M5ResolveCloseSheetResolutionForm {
...
impl M5ResolveCloseSheetResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolveCloseSheetKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolveCloseOutcomeScope {
...
impl M5ResolveCloseOutcomeScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolveCloseSheetSurfaceContext {
...
impl M5ResolveCloseSheetSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolveCloseSheetAnatomyPart {
...
impl M5ResolveCloseSheetAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolveCloseSheetNextAction {
...
impl M5ResolveCloseSheetNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolveCloseSheetExportField {
...
impl M5ResolveCloseSheetExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolveCloseSheetEntryDegradeReason {
...
impl M5ResolveCloseSheetEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5ResolveCloseSheetNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5ResolveCloseSheetNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5ResolveCloseSheetNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5ResolveCloseSheetNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5ResolveCloseSheetNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ResolveCloseOutcomeEntryDegradeReason {
...
impl M5ResolveCloseOutcomeEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5ResolveCloseSheetNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5ResolveCloseSheetNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5ResolveCloseSheetNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5ResolveCloseSheetNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5ResolveCloseSheetEntryResolutionInput {
    ...
    pub report_section: M5ResolveCloseSheetKind,
    /// The render / surface context.
    pub surface_context: M5ResolveCloseSheetSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ResolveCloseSheetResolutionForm>,
...
pub struct M5ResolvedResolveCloseSheetEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ResolveCloseSheetEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ResolveCloseSheetNextAction,
...
impl M5ResolvedResolveCloseSheetEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ResolveCloseOutcomeEntryResolutionInput {
    ...
    pub comparison_scope: M5ResolveCloseOutcomeScope,
    /// The render / surface context.
    pub surface_context: M5ResolveCloseSheetSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ResolveCloseSheetResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedResolveCloseOutcomeEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ResolveCloseOutcomeEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ResolveCloseSheetNextAction,
...
impl M5ResolvedResolveCloseOutcomeEntry {
...
pub enum M5ResolveCloseSheetResolutionError {
...
impl M5ResolveCloseSheetResolutionError {
...
impl fmt::Display for M5ResolveCloseSheetResolutionError {
...
impl Error for M5ResolveCloseSheetResolutionError {}

fn form_tokens(forms: &[M5ResolveCloseSheetResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5ResolveCloseSheetResolutionForm]) -> bool {
    let present: BTreeSet<M5ResolveCloseSheetResolutionForm> = forms.iter().copied().collect();
    M5ResolveCloseSheetResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5ResolveCloseSheetKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5ResolveCloseOutcomeScope,
...
pub fn resolve_review_pack_record_entry(
    input: M5ResolveCloseSheetEntryResolutionInput,
) -> Result<M5ResolvedResolveCloseSheetEntry, M5ResolveCloseSheetResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ResolveCloseSheetResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5ResolveCloseSheetResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ResolveCloseSheetEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ResolveCloseSheetEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5ResolveCloseSheetEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5ResolveCloseSheetEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5ResolveCloseSheetEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5ResolveCloseSheetEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5ResolveCloseSheetEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(
            M5ResolveCloseSheetEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    } else if !input.proof_fresh {
        Some(M5ResolveCloseSheetEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5ResolveCloseSheetNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedResolveCloseSheetEntry {
...
pub fn resolve_review_pack_result_entry(
    input: M5ResolveCloseOutcomeEntryResolutionInput,
) -> Result<M5ResolvedResolveCloseOutcomeEntry, M5ResolveCloseSheetResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ResolveCloseSheetResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ResolveCloseOutcomeEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ResolveCloseOutcomeEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5ResolveCloseOutcomeEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5ResolveCloseOutcomeEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5ResolveCloseOutcomeEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5ResolveCloseOutcomeEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5ResolveCloseSheetNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedResolveCloseOutcomeEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolveCloseSheetRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5ResolveCloseSheetRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5ResolveCloseSheetAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5ResolveCloseSheetExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5ChangeIntentDowngradeTrigger>,
    /// Resolved line-review_pack_record-registry examples.
    pub review_pack_record_entries: Vec<M5ResolvedResolveCloseSheetEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedResolveCloseOutcomeEntry>,
...
impl M5ResolveCloseSheetRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5ResolveCloseSheetAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5ResolveCloseSheetAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5ResolveCloseSheetExportField> =
            self.export_fields.iter().copied().collect();
        M5ResolveCloseSheetExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedResolveCloseSheetEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedResolveCloseOutcomeEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolveCloseSheetRegistriesVocabularySet {
...
impl M5ResolveCloseSheetRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ChangeIntentRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5ResolveCloseSheetResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5ResolveCloseSheetKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ResolveCloseOutcomeScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5ResolveCloseSheetSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5ResolveCloseSheetEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5ResolveCloseOutcomeEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5ResolveCloseSheetAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5ResolveCloseSheetNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5ResolveCloseSheetExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolveCloseSheetRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolveCloseSheetRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolveCloseSheetRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolveCloseSheetRegistriesReleasePosture {
...
/// Constructor input for [`M5ResolveCloseSheetRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ResolveCloseSheetRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5ResolveCloseSheetRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5ResolveCloseSheetRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5ResolveCloseSheetRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5ResolveCloseSheetRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5ResolveCloseSheetRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5ResolveCloseSheetRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolveCloseSheetRegistriesPacket {
    /// Record kind; must equal [`M5_RESOLVE_CLOSE_SHEET_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_RESOLVE_CLOSE_SHEET_REGISTRIES_SCHEMA_VERSION`].
...
impl M5ResolveCloseSheetRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5ResolveCloseSheetRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_RESOLVE_CLOSE_SHEET_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_RESOLVE_CLOSE_SHEET_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5ResolveCloseSheetRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_RESOLVE_CLOSE_SHEET_REGISTRIES_RECORD_KIND {
            violations.push(M5ResolveCloseSheetRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_RESOLVE_CLOSE_SHEET_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5ResolveCloseSheetRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5ResolveCloseSheetRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5ResolveCloseSheetRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5ResolveCloseSheetRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Resolve-or-Close Sheet and Resolution-Outcome Registries\n\n");
...
#[derive(Debug)]
pub enum M5ResolveCloseSheetRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5ResolveCloseSheetRegistriesViolation>),
}

impl fmt::Display for M5ResolveCloseSheetRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5ResolveCloseSheetRegistriesArtifactError {}

/// Validation failures emitted by [`M5ResolveCloseSheetRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5ResolveCloseSheetRegistriesViolation {
...
impl M5ResolveCloseSheetRegistriesViolation {
...
pub fn current_stable_m5_resolve_close_sheet_registries_export(
) -> Result<M5ResolveCloseSheetRegistriesPacket, M5ResolveCloseSheetRegistriesArtifactError> {
    let packet: M5ResolveCloseSheetRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../artifacts/release/m5-resolve-close-sheet-registries-proof/support_export.json"
    )))
    .map_err(M5ResolveCloseSheetRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5ResolveCloseSheetRegistriesArtifactError::Validation(
            violations,
        ))
    }
}
...
fn validate_source_contracts(
    packet: &M5ResolveCloseSheetRegistriesPacket,
    violations: &mut Vec<M5ResolveCloseSheetRegistriesViolation>,
) {
    let refs: BTreeSet<&str> = packet
        ...
        .collect();
    for required in [
        M5_RESOLVE_CLOSE_SHEET_REGISTRIES_SCHEMA_REF,
        M5_RESOLVE_CLOSE_SHEET_REGISTRIES_DOC_REF,
        M5_CHANGE_INTENT_MATRIX_SCHEMA_REF,
        ...
        if !refs.contains(required) {
            violations.push(M5ResolveCloseSheetRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5ResolveCloseSheetRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(M5ResolveCloseSheetRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(M5ResolveCloseSheetRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(M5ResolveCloseSheetRegistriesViolation::MandatoryExportFieldMissing);
        }
        ...
        if !refs.contains(M5_CHANGE_INTENT_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_RESOLVE_CLOSE_SHEET_DOMAIN_SCHEMA_REF)
        {
            violations.push(M5ResolveCloseSheetRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5ResolveCloseSheetRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5ResolveCloseSheetRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5ResolveCloseSheetRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(M5ResolveCloseSheetRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(M5ResolveCloseSheetRegistriesViolation::ConsumerProjectionIncomplete);
            return;
        }
    }
}
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5ResolveCloseSheetRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5ResolveCloseSheetRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5ResolveCloseSheetKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5ResolveCloseSheetSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations
            .push(M5ResolveCloseSheetRegistriesViolation::CohortDescriptorResolutionNotProven);
    }
    ...
    {
        violations.push(
            M5ResolveCloseSheetRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ResolveCloseOutcomeScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(M5ResolveCloseSheetRegistriesViolation::CohortEvidenceIntegrityNotProven);
    }
}
