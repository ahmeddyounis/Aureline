//! Canonical seed builders for the M5 resolve-or-close-sheet and resolution-outcome registries packet.
//!
//! These builders are the single producer of the checked-in support export and the narrowed fixtures.
//! The headless emitter and the inline tests both call them so the in-code registries, the artifact, and the
//! fixtures never drift. Every resolved example is built by calling the real resolvers so the packet can only
//! carry projections the resolvers actually produce. Clean resolve-or-close-sheet and resolution-outcome entries
//! are built so the one typed resolve-or-close sheet resolving per tracked item, sheets never letting a
//! local-only resolution read as a provider-accepted terminal state, offline / policy-blocked / partially-writable
//! resolution-outcomes never hidden behind a green summary, the canonical / accessible / audit resolution forms, and the
//! complete current-state / requested-terminal-state / unresolved-blockers / linked-evidence / permission-scope / confirm-reopen-export-actions
//! resolution-outcome object are proven across the work-item-detail, review-detail, Git / worktree, resolve-close-sheet,
//! provider-handoff, and support surfaces without any hand-copied per-item assumption, local-shown-as-provider close,
//! incomplete object, hidden offline / policy-blocked state, or resolution-form gap.

use super::*;

/// Stable packet id for the canonical registries packet.
pub const M5_RESOLVE_CLOSE_SHEET_REGISTRIES_PACKET_ID: &str =
    "m5-resolve-close-sheet-registries:stable:0001";
...
) -> M5ResolvedResolveCloseSheetEntry {
...
) -> M5ResolvedResolveCloseOutcomeEntry {
    resolve_review_pack_result_entry(input).expect("seed line-downgrade-packet entry resolves")
}

fn all_forms() -> Vec<M5ResolveCloseSheetResolutionForm> {
    M5ResolveCloseSheetResolutionForm::ALL.to_vec()
}
...
fn clean_review_pack_record_base(
    ...
    report_section: M5ResolveCloseSheetKind,
    surface_context: M5ResolveCloseSheetSurfaceContext,
...
) -> M5ResolveCloseSheetEntryResolutionInput {
    M5ResolveCloseSheetEntryResolutionInput {
...
fn review_pack_record_changed_files_scope_clean() -> M5ResolvedResolveCloseSheetEntry {
    review_pack_record(clean_review_pack_record_base(
        ...
        M5ResolveCloseSheetKind::ChangedFilesScope,
        M5ResolveCloseSheetSurfaceContext::ShiproomSurface,
...
fn review_pack_record_pull_request_scope_clean() -> M5ResolvedResolveCloseSheetEntry {
    review_pack_record(clean_review_pack_record_base(
        ...
        M5ResolveCloseSheetKind::PullRequestScope,
        M5ResolveCloseSheetSurfaceContext::ReleaseCenterSurface,
...
fn review_pack_record_base_head_range_scope_clean() -> M5ResolvedResolveCloseSheetEntry {
    review_pack_record(clean_review_pack_record_base(
        ...
        M5ResolveCloseSheetKind::BaseHeadRangeScope,
        M5ResolveCloseSheetSurfaceContext::ProgramGovernanceSurface,
...
fn review_pack_record_worktree_uncommitted_scope_clean() -> M5ResolvedResolveCloseSheetEntry {
    ...
    let mut base = clean_review_pack_record_base(
        ...
        M5ChangeIntentRole::ProviderOwnershipDisclosure,
        M5ResolveCloseSheetKind::WorktreeUncommittedScope,
        M5ResolveCloseSheetSurfaceContext::ExecutiveSteeringSurface,
...
fn review_pack_record_full_tree_scope_clean() -> M5ResolvedResolveCloseSheetEntry {
    ...
    let mut base = clean_review_pack_record_base(
        ...
        M5ChangeIntentRole::LocalVersusProviderStateDisclosure,
        M5ResolveCloseSheetKind::FullTreeScope,
        M5ResolveCloseSheetSurfaceContext::SupportOrExportForm,
...
fn review_pack_record_saved_pack_snapshot_scope_clean() -> M5ResolvedResolveCloseSheetEntry {
    review_pack_record(clean_review_pack_record_base(
        ...
        M5ResolveCloseSheetKind::SavedPackSnapshotScope,
        M5ResolveCloseSheetSurfaceContext::ReleaseCenterSurface,
...
fn review_pack_record_object_incomplete() -> M5ResolvedResolveCloseSheetEntry {
    let mut base = clean_review_pack_record_base(
...
fn review_pack_record_widen_fold() -> M5ResolvedResolveCloseSheetEntry {
    let mut base = clean_review_pack_record_base(
...
/// registry.
fn review_pack_record_unbound() -> M5ResolvedResolveCloseSheetEntry {
    let mut base = clean_review_pack_record_base(
...
fn review_pack_record_token_unstated() -> M5ResolvedResolveCloseSheetEntry {
    let mut base = clean_review_pack_record_base(
...
fn clean_downgrade_base(
    ...
    token_name: &str,
    semantic_role: M5ChangeIntentRole,
    comparison_scope: M5ResolveCloseOutcomeScope,
    surface_context: M5ResolveCloseSheetSurfaceContext,
...
) -> M5ResolveCloseOutcomeEntryResolutionInput {
    M5ResolveCloseOutcomeEntryResolutionInput {
        entry_id: entry_id.to_owned(),
...
fn downgrade_dogfood_ring_clean() -> M5ResolvedResolveCloseOutcomeEntry {
    ...
    let mut base = clean_downgrade_base(
        ...
        M5ChangeIntentRole::ProviderOwnershipDisclosure,
        M5ResolveCloseOutcomeScope::EvaluatedScopeBinding,
        M5ResolveCloseSheetSurfaceContext::ShiproomSurface,
...
fn downgrade_rehearsal_currency_clean() -> M5ResolvedResolveCloseOutcomeEntry {
    downgrade(clean_downgrade_base(
        "downgrade:program-governance:rehearsal-currency",
        "launch.line.extension-author",
        "line.downgrade.base_head_range_scope",
        M5ChangeIntentRole::LinkedEngineeringIdentityDisclosure,
        M5ResolveCloseOutcomeScope::PackVersionDigestBinding,
        M5ResolveCloseSheetSurfaceContext::ProgramGovernanceSurface,
...
fn downgrade_go_no_go_signoff_clean() -> M5ResolvedResolveCloseOutcomeEntry {
    downgrade(clean_downgrade_base(
        "downgrade:release-center:go-no-go-signoff",
        "launch.line.certified-journey",
        "line.downgrade.saved_pack_snapshot_scope",
        M5ChangeIntentRole::SideEffectDisclosure,
        M5ResolveCloseOutcomeScope::DivergenceLabelBinding,
        M5ResolveCloseSheetSurfaceContext::ReleaseCenterSurface,
...
fn downgrade_support_ahead() -> M5ResolvedResolveCloseOutcomeEntry {
    let mut base = clean_downgrade_base(
...
/// incomplete.
fn downgrade_form_incomplete() -> M5ResolvedResolveCloseOutcomeEntry {
    let mut base = clean_downgrade_base(
    ...
    base.resolution_form_coverage = vec![M5ResolveCloseSheetResolutionForm::CanonicalObject];
    downgrade(base)
}

/// Degraded downgrade entry: the downgrade scope is unclassified.
fn comparison_review_pack_result_unclassified() -> M5ResolvedResolveCloseOutcomeEntry {
    downgrade(clean_downgrade_base(
        "downgrade:executive-steering:scope-unclassified",
        "launch.line.design-partner-preview",
        "line.downgrade.worktree_uncommitted_scope",
        M5ChangeIntentRole::ProviderOwnershipDisclosure,
        M5ResolveCloseOutcomeScope::ChangeIntentResultUnclassified,
        M5ResolveCloseSheetSurfaceContext::ExecutiveSteeringSurface,
...
#[allow(clippy::too_many_arguments)]
fn base_row(
    consumer_surface: M5ResolveCloseSheetRegistriesConsumerSurface,
    owner_role: &str,
    scope_summary: &str,
    proof_ref: &str,
    downgrade_triggers: Vec<M5ChangeIntentDowngradeTrigger>,
    review_pack_record_entries: Vec<M5ResolvedResolveCloseSheetEntry>,
    review_pack_result_entries: Vec<M5ResolvedResolveCloseOutcomeEntry>,
) -> M5ResolveCloseSheetRegistriesRow {
    M5ResolveCloseSheetRegistriesRow {
        consumer_surface,
        ...
        anatomy_parts: M5ResolveCloseSheetAnatomyPart::ALL.to_vec(),
        export_fields: M5ResolveCloseSheetExportField::ALL.to_vec(),
        downgrade_triggers,
...
fn registry_rows() -> Vec<M5ResolveCloseSheetRegistriesRow> {
    ...
    vec![
        base_row(
            ...
            "Work-item detail resolves a tracked work item to one typed resolve-or-close sheet — its current state, requested terminal state, unresolved blockers, linked evidence, permission scope, and confirm / reopen / export actions, with the final side-effect preview — from the shared registry and proves the final-resolution authority for that item; a sheet dropping its unresolved blockers or linked evidence and a resolution-outcome that would let a local-only resolution read as a provider-accepted terminal state degrade honestly instead of implying the provider accepted a terminal state it has not",
            "resolve-close:m5-work-item-detail:001",
            vec![
        ...
        base_row(
            ...
            "Support owner",
            "Support resolves the sheet's commit state while keeping the unresolved blockers / linked evidence / permission scope and the resolved-locally / provider-updated / queued-for-publish attribution bound to the export, and reports the final-resolution authority; a sheet that is a hand-copied per-item assumption and a resolution-outcome on an unclassified binding degrade honestly so the unresolved blockers, linked evidence, and reopen / export continuity are never dropped on export or reopen",
            "resolve-close:m5-support:001",
            vec![
...
fn governance_review() -> M5ResolveCloseSheetRegistriesGovernanceReview {
    M5ResolveCloseSheetRegistriesGovernanceReview {
...
fn consumer_projection() -> M5ResolveCloseSheetRegistriesConsumerProjection {
    M5ResolveCloseSheetRegistriesConsumerProjection {
...
fn proof_freshness() -> M5ResolveCloseSheetRegistriesProofFreshness {
    M5ResolveCloseSheetRegistriesProofFreshness {
...
fn release_posture() -> M5ResolveCloseSheetRegistriesReleasePosture {
    M5ResolveCloseSheetRegistriesReleasePosture {
        proof_packet_ref: M5_RESOLVE_CLOSE_SHEET_REGISTRIES_ARTIFACT_REF.to_owned(),
        line_audit_ref: M5_RESOLVE_CLOSE_SHEET_REGISTRIES_REPORT_REF.to_owned(),
...
/// Narrowed variant: the work-item-detail row is held at Beta pending resolve-close-sheet parity on every
/// commit state; every row stays visible and every example stays honest.
pub fn seeded_m5_resolve_close_sheet_registries_review_pack_record_beta_narrowed(
) -> M5ResolveCloseSheetRegistriesPacket {
    let mut packet = seeded_m5_resolve_close_sheet_registries();
    packet.packet_id = "m5-resolve-close-sheet-registries:resolve-close-sheet-beta:0001".to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
...
/// Narrowed variant: the review-detail row is narrowed to Preview pending resolution-outcome parity on
/// every resolution-mode binding; every row stays visible and every example stays honest.
pub fn seeded_m5_resolve_close_sheet_registries_review_pack_result_preview_narrowed(
) -> M5ResolveCloseSheetRegistriesPacket {
    let mut packet = seeded_m5_resolve_close_sheet_registries();
    packet.packet_id = "m5-resolve-close-sheet-registries:relation-preview:0001".to_owned();
    let row = packet
        .registry_rows
        .iter_mut()
