...
fn clean_review_pack_record_input() -> M5UnsentEditRecoveryRecordEntryResolutionInput {
    M5UnsentEditRecoveryRecordEntryResolutionInput {
        ...
        report_section: M5UnsentEditRecoveryRecordKind::ChangedFilesScope,
        surface_context: M5UnsentEditRecoveryRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5UnsentEditRecoveryRecordResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5UnsentEditRecoveryResultEntryResolutionInput {
    M5UnsentEditRecoveryResultEntryResolutionInput {
        ...
        semantic_role: M5CollaborationStateRole::AuthorityModelDisclosure,
        comparison_scope: M5UnsentEditRecoveryResultScope::EvaluatedScopeBinding,
        surface_context: M5UnsentEditRecoveryRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5UnsentEditRecoveryRecordResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_unsent_local_edit_preservation_and_recovery_disposition_registries();
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(
        ...
        M5UnsentEditRecoveryRecordNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5UnsentEditRecoveryRecordKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_object_incomplete_and_widen_fold_and_form_degrade() {
    ...
    input.resolution_form_coverage =
        vec![M5UnsentEditRecoveryRecordResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5UnsentEditRecoveryRecordKind::FullTreeScope;
    ...
    let mut input = clean_review_pack_record_input();
    input.surface_context = M5UnsentEditRecoveryRecordSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        ...
        M5UnsentEditRecoveryRecordResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input).unwrap_err(),
        M5UnsentEditRecoveryRecordResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5UnsentEditRecoveryRecordKind::PackScopeUnclassified,
        true,
        false,
        true
    ));
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5UnsentEditRecoveryResultScope::ChangeObjectResultUnclassified;
    assert_eq!(
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    assert_eq!(
        ...
        M5UnsentEditRecoveryRecordResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5UnsentEditRecoveryRecordAndResultRegistriesViolation::VocabularySetDrift));
}
...
fn missing_source_contracts_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5UnsentEditRecoveryRecordAndResultRegistriesViolation::MissingSourceContracts));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5UnsentEditRecoveryRecordAndResultRegistriesViolation::DomainSchemaRefMissing));
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5UnsentEditRecoveryRecordAnatomyPart::Identity);
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5UnsentEditRecoveryRecordExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5UnsentEditRecoveryRecordAndResultRegistriesViolation::ExamplesMissing));
}
...
fn dishonest_clean_example_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5UnsentEditRecoveryRecordAndResultRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        assert!(packet.validate().contains(
            &M5UnsentEditRecoveryRecordAndResultRegistriesViolation::RowInvariantViolated
        ));
    }
}
...
fn review_pack_record_not_proven_when_incomplete_example_removed() {
    ...
    for row in &mut packet.registry_rows {
        row.review_pack_record_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
                    M5UnsentEditRecoveryRecordEntryDegradeReason::CohortDescriptorObjectIncomplete,
                )
        });
    }
    assert!(packet.validate().contains(
...
fn proof_freshness_incomplete_fails() {
    ...
    assert!(packet.validate().contains(
        &M5UnsentEditRecoveryRecordAndResultRegistriesViolation::ProofFreshnessIncomplete
    ));
}
...
fn release_posture_incomplete_fails() {
    ...
    assert!(packet.validate().contains(
        &M5UnsentEditRecoveryRecordAndResultRegistriesViolation::ReleasePostureIncomplete
    ));
}
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet
        .validate()
        .contains(&M5UnsentEditRecoveryRecordAndResultRegistriesViolation::RawMaterialInExport));
}

#[test]
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_unsent_local_edit_preservation_and_recovery_disposition_registries()
        .export_safe_json();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_unsent_local_edit_preservation_and_recovery_disposition_registries_export()
        .expect(
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5UnsentEditRecoveryRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-unsent-local-edit-preservation-and-recovery-disposition-registries/unsent_local_edit_preservation_beta_narrowed.json"
    )))
    ...
    let preview: M5UnsentEditRecoveryRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-unsent-local-edit-preservation-and-recovery-disposition-registries/recovery_disposition_preview_narrowed.json"
    )))
