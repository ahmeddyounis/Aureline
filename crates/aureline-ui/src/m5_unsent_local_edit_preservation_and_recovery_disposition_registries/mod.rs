//! Implemented M5 unsent-local-edit-preservation and recovery-disposition registries.
//!
//! The frozen [collaboration-state authority matrix][matrix] names Aureline's governed shared-object classes and
//! locks their authority, convergence, and export vocabulary. Building on the CRDT-backed shared-text replica
//! state (the local buffer a replica never replaces) and the higher-risk control-plane degradation banner it
//! freezes, this module makes collaboration *downgrade* honest: when a shared session narrows — role loss,
//! permission narrowing, explicit leave, host removal, or relay failure — every unsent local shared-text edit is
//! preserved first, as local-only state, reconnect-ready state, or a reviewable patch packet, rather than silently
//! dropped or remotely resolved. So the shared editor replica view, the presence / cursor layer, the comment /
//! annotation / review-pin layer, the collaboration degradation banner, and the support / export and help / docs
//! surfaces inherit one canonical downgrade-recovery record rather than a hand-authored parallel prose that has to
//! be kept consistent: it turns the *unsent-local-edit-preservation* grammar (one durable record per downgrade —
//! one typed field per record section: the workspace-root and buffer / object identity of the local unsent work,
//! the downgrade trigger, the preserved-state class it lands in — local-only, reconnect-ready, or reviewable patch
//! packet — and its export posture, each bound to the local canonical truth it never discards) and the
//! *recovery-disposition* grammar (the next action a user can take on the preserved work — continue-local,
//! reopen-share, export-patch, or discard-with-review — plus its actor / time provenance, its convergence and
//! merge-drift posture, and its policy-labeled export lineage) into registry resolvers that produce export-safe,
//! honest projections. Every downgrade then resolves to one typed preservation record — the fields it publishes,
//! the identity of the local unsent work, the trigger and preserved-state class, and its export posture, all
//! attributable so preserved work is never dropped implicitly — and to one recovery disposition — the action, its
//! actor / time provenance, and its export lineage — that the shared editor, presence, review-pin,
//! degradation-banner, and support surfaces can inspect before rejoining, exporting, or discarding, so a
//! disposition that cannot bind its preserved work degrades honestly instead of letting a downgrade read as clean
//! when unsent local edits were dropped.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable preservation record per downgrade, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [preserved-state class][M5UnsentEditRecoveryRecordKind],
//!   a disclosure role, covers every [resolution form][M5UnsentEditRecoveryRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (the
//!   workspace-root and buffer / object identity of the unsent work, the downgrade trigger, the preserved-state
//!   class, and the export posture bound to the preserved-work identity), keeps its lineage attributable, and keeps
//!   the local canonical truth it never discards bound to the record; otherwise it degrades.
//! * **Never discard unsent local edits; preserve local unsent work first on any downgrade.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a record whose
//!   local-truth preservation is not held (a downgrade presenting itself as clean while unsent local edits are
//!   dropped) so it degrades to
//!   [`M5UnsentEditRecoveryRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and any downgrade — role loss, permission narrowing, explicit leave, host removal, or relay failure — that
//!   would discard unsent local edits degrades the same way — the structured blocker reason an unsent-edits-dropped
//!   attempt must surface.
//! * **Offer a reviewable recovery disposition with actor / time provenance, instead of a silent remote resolve.**
//!   [`resolve_review_pack_result_entry`] names a classified
//!   [disposition binding][M5UnsentEditRecoveryResultScope]
//!   (continue-local binding, export-patch binding, or discard-with-review binding), requires the full
//!   preserved-work-identity / recovery-action / actor-time-provenance / binding / active-reason
//!   record, covers every resolution form, and degrades to
//!   [`M5UnsentEditRecoveryResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the disposition would discard preserved work without review, act without actor / time provenance, or let an
//!   export drop its redaction lineage, so a recovery can never read as trustworthy when it has quietly dropped
//!   the preserved work or its provenance.
//!
...
//! export surfaces can never fork their own downgrade-recovery meaning. Raw secrets, raw command text, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_unsent_local_edit_preservation_and_recovery_disposition_registries,
    seeded_m5_unsent_local_edit_preservation_and_recovery_disposition_registries_review_pack_record_beta_narrowed,
    seeded_m5_unsent_local_edit_preservation_and_recovery_disposition_registries_review_pack_result_preview_narrowed,
    M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_PACKET_ID,
};
...
use crate::m5_collaboration_replica_shared_object_authority_anchor_drift_convergence_and_session_archive_matrix::{
    ...
    M5_CRDT_BACKED_SHARED_TEXT_DOMAIN_SCHEMA_REF, M5_HIGHER_RISK_CONTROL_PLANE_DOMAIN_SCHEMA_REF,
    M5_COLLABORATION_STATE_MATRIX_DOC_REF, M5_COLLABORATION_STATE_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5UnsentEditRecoveryRecordAndResultRegistriesPacket`].
pub const M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_unsent_local_edit_preservation_and_recovery_disposition_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_SCHEMA_VERSION:
    u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_unsent_local_edit_preservation_and_recovery_disposition_registries.md";

/// Repo-relative path of the checked support-export artifact.
pub const M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-unsent-local-edit-preservation-and-recovery-disposition-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-unsent-local-edit-preservation-and-recovery-disposition-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-unsent-local-edit-preservation-and-recovery-disposition-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-unsent-local-edit-preservation-and-recovery-disposition-registries";
...
pub type M5UnsentEditRecoveryRecordAndResultRegistriesConsumerSurface =
...
pub enum M5UnsentEditRecoveryRecordResolutionForm {
...
impl M5UnsentEditRecoveryRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5UnsentEditRecoveryRecordKind {
...
impl M5UnsentEditRecoveryRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5UnsentEditRecoveryResultScope {
...
impl M5UnsentEditRecoveryResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5UnsentEditRecoveryRecordSurfaceContext {
...
impl M5UnsentEditRecoveryRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5UnsentEditRecoveryRecordAnatomyPart {
...
impl M5UnsentEditRecoveryRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5UnsentEditRecoveryRecordNextAction {
...
impl M5UnsentEditRecoveryRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5UnsentEditRecoveryRecordExportField {
...
impl M5UnsentEditRecoveryRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5UnsentEditRecoveryRecordEntryDegradeReason {
...
impl M5UnsentEditRecoveryRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5UnsentEditRecoveryRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5UnsentEditRecoveryRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5UnsentEditRecoveryRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5UnsentEditRecoveryRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5UnsentEditRecoveryRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5UnsentEditRecoveryResultEntryDegradeReason {
...
impl M5UnsentEditRecoveryResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5UnsentEditRecoveryRecordNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5UnsentEditRecoveryRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5UnsentEditRecoveryRecordEntryResolutionInput {
    ...
    pub report_section: M5UnsentEditRecoveryRecordKind,
    /// The render / surface context.
    pub surface_context: M5UnsentEditRecoveryRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5UnsentEditRecoveryRecordResolutionForm>,
...
pub struct M5ResolvedUnsentEditRecoveryRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5UnsentEditRecoveryRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5UnsentEditRecoveryRecordNextAction,
...
impl M5ResolvedUnsentEditRecoveryRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5UnsentEditRecoveryResultEntryResolutionInput {
    ...
    pub comparison_scope: M5UnsentEditRecoveryResultScope,
    /// The render / surface context.
    pub surface_context: M5UnsentEditRecoveryRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5UnsentEditRecoveryRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedUnsentEditRecoveryResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5UnsentEditRecoveryResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5UnsentEditRecoveryRecordNextAction,
...
impl M5ResolvedUnsentEditRecoveryResultEntry {
...
pub enum M5UnsentEditRecoveryRecordResolutionError {
...
impl M5UnsentEditRecoveryRecordResolutionError {
...
impl fmt::Display for M5UnsentEditRecoveryRecordResolutionError {
...
impl Error for M5UnsentEditRecoveryRecordResolutionError {}

fn form_tokens(forms: &[M5UnsentEditRecoveryRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5UnsentEditRecoveryRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5UnsentEditRecoveryRecordResolutionForm> =
        forms.iter().copied().collect();
    M5UnsentEditRecoveryRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5UnsentEditRecoveryRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5UnsentEditRecoveryResultScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedUnsentEditRecoveryRecordEntry, M5UnsentEditRecoveryRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5UnsentEditRecoveryRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5UnsentEditRecoveryRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5UnsentEditRecoveryRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5UnsentEditRecoveryRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5UnsentEditRecoveryRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5UnsentEditRecoveryRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5UnsentEditRecoveryRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5UnsentEditRecoveryRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5UnsentEditRecoveryRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5UnsentEditRecoveryRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5UnsentEditRecoveryRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5UnsentEditRecoveryRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedUnsentEditRecoveryRecordEntry {
...
pub fn resolve_review_pack_result_entry(
...
) -> Result<M5ResolvedUnsentEditRecoveryResultEntry, M5UnsentEditRecoveryRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5UnsentEditRecoveryRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5UnsentEditRecoveryResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5UnsentEditRecoveryResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5UnsentEditRecoveryResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5UnsentEditRecoveryResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5UnsentEditRecoveryResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5UnsentEditRecoveryResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5UnsentEditRecoveryRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedUnsentEditRecoveryResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5UnsentEditRecoveryRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5UnsentEditRecoveryRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5UnsentEditRecoveryRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5UnsentEditRecoveryRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedUnsentEditRecoveryRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedUnsentEditRecoveryResultEntry>,
...
impl M5UnsentEditRecoveryRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5UnsentEditRecoveryRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5UnsentEditRecoveryRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5UnsentEditRecoveryRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5UnsentEditRecoveryRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedUnsentEditRecoveryRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedUnsentEditRecoveryResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5UnsentEditRecoveryRecordAndResultRegistriesVocabularySet {
...
impl M5UnsentEditRecoveryRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5CollaborationStateRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5UnsentEditRecoveryRecordResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5UnsentEditRecoveryRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5UnsentEditRecoveryResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5UnsentEditRecoveryRecordSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            review_pack_record_degrade_reasons: tokens(
                &M5UnsentEditRecoveryRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5UnsentEditRecoveryResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5UnsentEditRecoveryRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5UnsentEditRecoveryRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5UnsentEditRecoveryRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5UnsentEditRecoveryRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5UnsentEditRecoveryRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5UnsentEditRecoveryRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5UnsentEditRecoveryRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5UnsentEditRecoveryRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5UnsentEditRecoveryRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5UnsentEditRecoveryRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5UnsentEditRecoveryRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5UnsentEditRecoveryRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5UnsentEditRecoveryRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5UnsentEditRecoveryRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5UnsentEditRecoveryRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5UnsentEditRecoveryRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_SCHEMA_VERSION`].
...
impl M5UnsentEditRecoveryRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5UnsentEditRecoveryRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind:
                M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_RECORD_KIND
                    .to_owned(),
            schema_version:
                M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5UnsentEditRecoveryRecordAndResultRegistriesViolation> {
        ...
        {
            violations
                .push(M5UnsentEditRecoveryRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_UNSENT_LOCAL_EDIT_PRESERVATION_AND_RECOVERY_DISPOSITION_REGISTRIES_SCHEMA_VERSION
        {
            violations
                .push(M5UnsentEditRecoveryRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations
                .push(M5UnsentEditRecoveryRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations
                .push(M5UnsentEditRecoveryRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations
                .push(M5UnsentEditRecoveryRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Unsent-Local-Edit-Preservation and Recovery-Disposition Registries\n\n");
...
#[derive(Debug)]
pub enum M5UnsentEditRecoveryRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5UnsentEditRecoveryRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5UnsentEditRecoveryRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5UnsentEditRecoveryRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5UnsentEditRecoveryRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5UnsentEditRecoveryRecordAndResultRegistriesViolation {
...
impl M5UnsentEditRecoveryRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_unsent_local_edit_preservation_and_recovery_disposition_registries_export(
) -> Result<
    M5UnsentEditRecoveryRecordAndResultRegistriesPacket,
    M5UnsentEditRecoveryRecordAndResultRegistriesArtifactError,
> {
    let packet: M5UnsentEditRecoveryRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-unsent-local-edit-preservation-and-recovery-disposition-registries-proof/support_export.json"
        )
    ))
    .map_err(M5UnsentEditRecoveryRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5UnsentEditRecoveryRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5UnsentEditRecoveryRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5UnsentEditRecoveryRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5UnsentEditRecoveryRecordAndResultRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5UnsentEditRecoveryRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5UnsentEditRecoveryRecordAndResultRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(
                M5UnsentEditRecoveryRecordAndResultRegistriesViolation::MandatoryAnatomyMissing,
            );
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5UnsentEditRecoveryRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        ...
        {
            violations.push(
                M5UnsentEditRecoveryRecordAndResultRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations
                .push(M5UnsentEditRecoveryRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations
                .push(M5UnsentEditRecoveryRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5UnsentEditRecoveryRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5UnsentEditRecoveryRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5UnsentEditRecoveryRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5UnsentEditRecoveryRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5UnsentEditRecoveryRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5UnsentEditRecoveryRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5UnsentEditRecoveryRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5UnsentEditRecoveryRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5UnsentEditRecoveryRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5UnsentEditRecoveryResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(
            M5UnsentEditRecoveryRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
