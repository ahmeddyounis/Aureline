...
fn clean_review_pack_record_input() -> M5SharedTerminalDebugViewRecordEntryResolutionInput {
    M5SharedTerminalDebugViewRecordEntryResolutionInput {
        ...
        report_section: M5SharedTerminalDebugViewRecordKind::ChangedFilesScope,
        surface_context: M5SharedTerminalDebugViewRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SharedTerminalDebugViewRecordResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5SharedTerminalDebugViewResultEntryResolutionInput {
    M5SharedTerminalDebugViewResultEntryResolutionInput {
        ...
        semantic_role: M5CollaborationControlRole::ControlAuthorityDisclosure,
        comparison_scope: M5SharedTerminalDebugViewResultScope::EvaluatedScopeBinding,
        surface_context: M5SharedTerminalDebugViewRecordSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5SharedTerminalDebugViewRecordResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_shared_terminal_debug_view_and_control_channel_badge_registries();
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(
        ...
        M5SharedTerminalDebugViewRecordNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5SharedTerminalDebugViewRecordKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5SharedTerminalDebugViewRecordKind::FullTreeScope;
    ...
    let mut input = clean_review_pack_record_input();
    input.surface_context = M5SharedTerminalDebugViewRecordSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        ...
        M5SharedTerminalDebugViewRecordResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input).unwrap_err(),
        M5SharedTerminalDebugViewRecordResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5SharedTerminalDebugViewRecordKind::PackScopeUnclassified,
        true,
        false,
        true
    ));
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5SharedTerminalDebugViewResultScope::ChangeObjectResultUnclassified;
    assert_eq!(
...
fn domain_schema_ref_missing_fails() {
    ...
    packet.registry_rows[0]
        ...
        .retain(|r| r != M5_SHARED_TERMINAL_DEBUG_VIEW_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
    ...
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_CONTROL_CHANNEL_BADGE_DOMAIN_SCHEMA_REF);
    assert!(packet.validate().contains(
        &M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::DomainSchemaRefMissing
    ));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5SharedTerminalDebugViewRecordAnatomyPart::Identity);
    assert!(packet.validate().contains(
        &M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::MandatoryAnatomyMissing
    ));
}
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5SharedTerminalDebugViewRecordExportField::CohortArchetypes);
    assert!(packet.validate().contains(
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::ExamplesMissing));
}
...
fn dishonest_clean_example_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::DishonestExample));
}
...
fn rollback_preservation_not_proven_when_unbound_example_removed() {
    ...
    for row in &mut packet.registry_rows {
        row.review_pack_record_entries.retain(|ex| {
            ex.degrade_reason
                != Some(
                    M5SharedTerminalDebugViewRecordEntryDegradeReason::DescriptorNotBoundToRegistry,
                )
        });
    }
    assert!(packet.validate().contains(
...
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_shared_terminal_debug_view_and_control_channel_badge_registries()
        .export_safe_json();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_shared_terminal_debug_view_and_control_channel_badge_registries_export()
        .expect(
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5SharedTerminalDebugViewRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-shared-terminal-debug-view-and-control-channel-badge-registries/shared_terminal_debug_view_beta_narrowed.json"
    )))
    ...
    let preview: M5SharedTerminalDebugViewRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/collaboration/m5-shared-terminal-debug-view-and-control-channel-badge-registries/control_channel_badge_preview_narrowed.json"
    )))
