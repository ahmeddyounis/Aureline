//! Implemented M5 shared-terminal-debug-view and control-channel-badge registries.
//!
//! The frozen [collaboration-control component matrix][matrix] names Aureline's governed sensitive-collaboration
//! object classes and locks their controlled vocabulary. This module makes the read-only shared terminal / debugger
//! view stream operable — as a canonical record — and adds the control-channel badge that names whether input
//! authority is unavailable, requestable, granted, or expired, so the desktop shared-terminal / debug surface, the
//! companion-follow flow, and the support / export and help / docs surfaces inherit one canonical view-stream
//! descriptor rather than a hand-authored parallel prose that has to be kept consistent: it turns the
//! *shared-terminal-debug-view* grammar (one durable view-stream record per sensitive session — one typed field per
//! record section: the stable session start / stop identity, the surface type and target context, the participant
//! scope and observing roles, the command or frame markers the stream carries, the read-only default it begins in,
//! and the control-channel state kept distinct from text / presence channels — each bound to one session / target
//! scope) and the *control-channel-badge* grammar (the badge a viewer reads before assuming any authority, and the
//! fresh visible authority event any change to an already-active session must raise — a control request, a grant to a
//! single driver, an expiry, or a presence reconnect / cursor-follow / companion resume that must not silently
//! upgrade view-only to control-capable) into registry resolvers that produce export-safe, honest projections. Every
//! sensitive session then resolves to one typed shared-terminal-debug view — the frames it streams, its surface type
//! and target context, its observing scope, and its read-only default, all attributable to the session so presence
//! never reads as terminal / debug control — and to one control-channel-badge object — the resolved session identity,
//! the disclosed surface type and target, the available authority, and the active grant / expiry gate — that the
//! shared terminal / debug, companion, and support surfaces can inspect without manual reconstruction, so a viewer
//! observes terminal or debugger state without inheriting input authority or secret visibility, and a view stream
//! that cannot bind a field to its session / target scope degrades honestly instead of letting presence read as
//! control-capable when it is not.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable shared-terminal-debug view stream per session, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [scope selector][M5SharedTerminalDebugViewRecordKind],
//!   a disclosure role, covers every [resolution form][M5SharedTerminalDebugViewRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (surface type
//!   and target context, observing scope, command or frame markers, the control-channel state, and the read-only
//!   default bound to the session start / stop identity), keeps its stream lineage attributable to the session, and
//!   keeps a control-channel field bound to its authority state; otherwise it degrades.
//! * **Keep the control-channel badge honest so a viewer never inherits input authority rather than a hand-authored
//!   parallel prose staying consistent.** [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a
//!   view stream whose control-channel badge is not preserved (a session presenting itself as control-capable from
//!   presence alone) so it degrades to
//!   [`M5SharedTerminalDebugViewRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a control-channel badge whose language outruns its authority state degrades the same
//!   way — the structured blocker reason a control-from-presence attempt must surface.
//! * **Turn a control request, a grant, an expiry, or a presence reconnect into a visible, typed authority event
//!   instead of a silent upgrade.** [`resolve_review_pack_result_entry`] names a classified
//!   [authority binding][M5SharedTerminalDebugViewResultScope]
//!   (control-request binding, grant-to-single-driver binding, or expiry binding), requires the full
//!   session-identity / affected-control-channel-field / previous-versus-current-authority-state / binding / active-reason
//!   authority object, covers every resolution form, and degrades to
//!   [`M5SharedTerminalDebugViewResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the change would present an expired grant as current, hide a second driver on a sensitive surface, or let a
//!   view-only stream masquerade as control-capable, so a session change can never read as trustworthy when it has
//!   quietly dropped the reason a fresh visible authority event was required.
//!
...
//! can never fork their own view-stream meaning. Raw secrets, raw command text, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_shared_terminal_debug_view_and_control_channel_badge_registries,
    seeded_m5_shared_terminal_debug_view_and_control_channel_badge_registries_review_pack_record_beta_narrowed,
    seeded_m5_shared_terminal_debug_view_and_control_channel_badge_registries_review_pack_result_preview_narrowed,
    M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_PACKET_ID,
};
...
use crate::m5_shared_terminal_debug_control_grant_presenter_consent_retention_and_session_restore_view_matrix::{
    ...
    M5_SHARED_TERMINAL_DEBUG_VIEW_DOMAIN_SCHEMA_REF, M5_COLLABORATION_CONTROL_MATRIX_DOC_REF,
    M5_COLLABORATION_CONTROL_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5SharedTerminalDebugViewRecordAndResultRegistriesPacket`].
pub const M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_shared_terminal_debug_view_and_control_channel_badge_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_SCHEMA_VERSION: u32 =
    1;

/// Repo-relative path of the combined registries schema.
pub const M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_shared_terminal_debug_view_and_control_channel_badge_registries.md";

/// Repo-relative path of the control-channel-badge domain schema this lane binds its B object to (the
/// machine-readable control-grant record whose authority state the badge projects — unavailable, requestable,
/// granted to a single driver, or expired — kept distinct from text / presence channels). Minted by the frozen
/// collaboration-control matrix; reused here so the badge and the grant flow never fork their authority vocabulary.
pub const M5_CONTROL_CHANNEL_BADGE_DOMAIN_SCHEMA_REF: &str =
    "schemas/collaboration/m5-control-grant.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-shared-terminal-debug-view-and-control-channel-badge-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-shared-terminal-debug-view-and-control-channel-badge-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-shared-terminal-debug-view-and-control-channel-badge-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-shared-terminal-debug-view-and-control-channel-badge-registries";
...
pub type M5SharedTerminalDebugViewRecordAndResultRegistriesConsumerSurface =
...
pub enum M5SharedTerminalDebugViewRecordResolutionForm {
...
impl M5SharedTerminalDebugViewRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SharedTerminalDebugViewRecordKind {
...
impl M5SharedTerminalDebugViewRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SharedTerminalDebugViewResultScope {
...
impl M5SharedTerminalDebugViewResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SharedTerminalDebugViewRecordSurfaceContext {
...
impl M5SharedTerminalDebugViewRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SharedTerminalDebugViewRecordAnatomyPart {
...
impl M5SharedTerminalDebugViewRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SharedTerminalDebugViewRecordNextAction {
...
impl M5SharedTerminalDebugViewRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SharedTerminalDebugViewRecordExportField {
...
impl M5SharedTerminalDebugViewRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SharedTerminalDebugViewRecordEntryDegradeReason {
...
impl M5SharedTerminalDebugViewRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5SharedTerminalDebugViewRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5SharedTerminalDebugViewRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5SharedTerminalDebugViewRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5SharedTerminalDebugViewRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => {
                M5SharedTerminalDebugViewRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5SharedTerminalDebugViewResultEntryDegradeReason {
...
impl M5SharedTerminalDebugViewResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
...
pub struct M5SharedTerminalDebugViewRecordEntryResolutionInput {
    ...
    pub report_section: M5SharedTerminalDebugViewRecordKind,
    /// The render / surface context.
    pub surface_context: M5SharedTerminalDebugViewRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SharedTerminalDebugViewRecordResolutionForm>,
...
pub struct M5ResolvedSharedTerminalDebugViewRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SharedTerminalDebugViewRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SharedTerminalDebugViewRecordNextAction,
...
impl M5ResolvedSharedTerminalDebugViewRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SharedTerminalDebugViewResultEntryResolutionInput {
    ...
    pub comparison_scope: M5SharedTerminalDebugViewResultScope,
    /// The render / surface context.
    pub surface_context: M5SharedTerminalDebugViewRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5SharedTerminalDebugViewRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedSharedTerminalDebugViewResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5SharedTerminalDebugViewResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5SharedTerminalDebugViewRecordNextAction,
...
impl M5ResolvedSharedTerminalDebugViewResultEntry {
...
pub enum M5SharedTerminalDebugViewRecordResolutionError {
...
impl M5SharedTerminalDebugViewRecordResolutionError {
...
impl fmt::Display for M5SharedTerminalDebugViewRecordResolutionError {
...
impl Error for M5SharedTerminalDebugViewRecordResolutionError {}

fn form_tokens(forms: &[M5SharedTerminalDebugViewRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5SharedTerminalDebugViewRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5SharedTerminalDebugViewRecordResolutionForm> =
        forms.iter().copied().collect();
    M5SharedTerminalDebugViewRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5SharedTerminalDebugViewRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5SharedTerminalDebugViewResultScope,
...
pub fn resolve_review_pack_record_entry(
    ...
    M5SharedTerminalDebugViewRecordResolutionError,
> {
    if input.entry_id.trim().is_empty() {
        return Err(M5SharedTerminalDebugViewRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5SharedTerminalDebugViewRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SharedTerminalDebugViewRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SharedTerminalDebugViewRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5SharedTerminalDebugViewRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5SharedTerminalDebugViewRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5SharedTerminalDebugViewRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5SharedTerminalDebugViewRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5SharedTerminalDebugViewRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5SharedTerminalDebugViewRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5SharedTerminalDebugViewRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5SharedTerminalDebugViewRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedSharedTerminalDebugViewRecordEntry {
...
> {
    if input.entry_id.trim().is_empty() {
        return Err(
            M5SharedTerminalDebugViewRecordResolutionError::EmptyCohortEvidencePacketEntryId,
        );
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5SharedTerminalDebugViewResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5SharedTerminalDebugViewResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5SharedTerminalDebugViewResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5SharedTerminalDebugViewResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5SharedTerminalDebugViewResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5SharedTerminalDebugViewResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5SharedTerminalDebugViewRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedSharedTerminalDebugViewResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SharedTerminalDebugViewRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5SharedTerminalDebugViewRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5SharedTerminalDebugViewRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5SharedTerminalDebugViewRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedSharedTerminalDebugViewRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedSharedTerminalDebugViewResultEntry>,
...
impl M5SharedTerminalDebugViewRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5SharedTerminalDebugViewRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5SharedTerminalDebugViewRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5SharedTerminalDebugViewRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5SharedTerminalDebugViewRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedSharedTerminalDebugViewRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedSharedTerminalDebugViewResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SharedTerminalDebugViewRecordAndResultRegistriesVocabularySet {
...
impl M5SharedTerminalDebugViewRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5CollaborationControlRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5SharedTerminalDebugViewRecordResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5SharedTerminalDebugViewRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5SharedTerminalDebugViewResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5SharedTerminalDebugViewRecordSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            review_pack_record_degrade_reasons: tokens(
                &M5SharedTerminalDebugViewRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5SharedTerminalDebugViewResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5SharedTerminalDebugViewRecordAnatomyPart::ALL, |v| {
                v.as_str()
            }),
            next_actions: tokens(&M5SharedTerminalDebugViewRecordNextAction::ALL, |v| {
                v.as_str()
            }),
            export_fields: tokens(&M5SharedTerminalDebugViewRecordExportField::ALL, |v| {
                v.as_str()
            }),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SharedTerminalDebugViewRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SharedTerminalDebugViewRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SharedTerminalDebugViewRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SharedTerminalDebugViewRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5SharedTerminalDebugViewRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5SharedTerminalDebugViewRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5SharedTerminalDebugViewRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5SharedTerminalDebugViewRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5SharedTerminalDebugViewRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5SharedTerminalDebugViewRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5SharedTerminalDebugViewRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5SharedTerminalDebugViewRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5SharedTerminalDebugViewRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_SCHEMA_VERSION`].
...
impl M5SharedTerminalDebugViewRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5SharedTerminalDebugViewRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind:
                M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_RECORD_KIND
                    .to_owned(),
            schema_version:
                M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5SharedTerminalDebugViewRecordAndResultRegistriesViolation> {
        ...
        {
            violations
                .push(M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_SHARED_TERMINAL_DEBUG_VIEW_AND_CONTROL_CHANNEL_BADGE_REGISTRIES_SCHEMA_VERSION
        {
            violations.push(
                M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations
                .push(M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(
                M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::VocabularySetDrift,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(
                M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::RawMaterialInExport,
            );
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Shared-Terminal-Debug-View and Control-Channel-Badge Registries\n\n");
...
#[derive(Debug)]
pub enum M5SharedTerminalDebugViewRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5SharedTerminalDebugViewRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5SharedTerminalDebugViewRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5SharedTerminalDebugViewRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5SharedTerminalDebugViewRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5SharedTerminalDebugViewRecordAndResultRegistriesViolation {
...
impl M5SharedTerminalDebugViewRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_shared_terminal_debug_view_and_control_channel_badge_registries_export(
) -> Result<
    M5SharedTerminalDebugViewRecordAndResultRegistriesPacket,
    M5SharedTerminalDebugViewRecordAndResultRegistriesArtifactError,
> {
    let packet: M5SharedTerminalDebugViewRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-shared-terminal-debug-view-and-control-channel-badge-registries-proof/support_export.json"
        )
    ))
    .map_err(M5SharedTerminalDebugViewRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5SharedTerminalDebugViewRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5SharedTerminalDebugViewRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5SharedTerminalDebugViewRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(
                M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::MissingSourceContracts,
            );
            return;
        }
    }
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations
            .push(M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_SHARED_TERMINAL_DEBUG_VIEW_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_CONTROL_CHANNEL_BADGE_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations
                .push(M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(
                M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::DishonestExample,
            );
        }
        if !row.honours_invariants() {
            violations.push(
                M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::RowInvariantViolated,
            );
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(
            M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::ProofFreshnessIncomplete,
        );
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(
            M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::ReleasePostureIncomplete,
        );
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5SharedTerminalDebugViewRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5SharedTerminalDebugViewRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = review_pack_records().any(|ex| {
        ex.degrade_reason
            == Some(
                M5SharedTerminalDebugViewRecordEntryDegradeReason::CohortDescriptorObjectIncomplete,
            )
    });
    ...
    {
        violations.push(
            M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5SharedTerminalDebugViewResultScope::CANONICAL_SCOPES
        .iter()
    ...
    let form_incomplete_degrades = downgrade().any(|ex| {
        ex.degrade_reason
            == Some(
                M5SharedTerminalDebugViewResultEntryDegradeReason::EvidenceFormCoverageIncomplete,
            )
    });
    ...
    {
        violations.push(
            M5SharedTerminalDebugViewRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
