//! Tests for the M05-1211 build-lane-trust surface certification capstone.

use super::*;

fn packet() -> BuildLaneTrustProfileCertificationPacket {
    seeded_m5_build_lane_trust_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, BUILD_LANE_TRUST_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, BUILD_LANE_TRUST_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5BuildLaneTrustCertifiedProfile::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            BuildLaneTrustCertificationAxis::ALL.len()
        );
    }
...
fn a_narrowed_profile_never_keeps_a_trusted_claim() {
    for row in &packet().rows {
        if row.is_claim_narrowed() {
            assert!(
                !row.certified_claim.asserts_trusted_lane(),
...
fn all_five_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5BuildLaneTrustClaim> = p.rows.iter().map(|r| r.certified_claim).collect();
    for claim in M5BuildLaneTrustClaim::ALL {
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5BuildLaneTrustCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5BuildLaneTrustCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = BuildLaneTrustCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), BuildLaneTrustCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in BuildLaneTrustCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == BuildLaneTrustCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    // The B144 localization axis is present, distinct, and exercised by the
    // disclosed-write-intent profile as its binding narrowed axis.
    let p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| {
            r.profile == M5BuildLaneTrustCertifiedProfile::DisclosedCacheDisciplineProfile
        })
        .expect("disclosed-cache-discipline-profile row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(BuildLaneTrustCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(BuildLaneTrustCertificationAxis::Localization)
    );
}

#[test]
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    // The B144 high-contrast reach axis stays certified across the seed (never a binding narrowed axis here).
    for row in &packet().rows {
        let outcome = row
            .axis(BuildLaneTrustCertificationAxis::HighContrast)
...
fn the_three_yellow_rows_bind_three_distinct_axes_and_triggers() {
    ...
    // per-lane concern (cache origin trust / clean-room parity / sidecar drift).
    let p = packet();
    let axes: BTreeSet<BuildLaneTrustCertificationAxis> = p
        .rows
        .iter()
    ...
    let triggers: std::collections::HashSet<M5BuildLaneDowngradeTrigger> = p
        .rows
        .iter()
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5BuildLaneTrustCertifiedProfile::LiveExactBuildSupportableLane)
        .expect("live-exact-build-supportable-lane row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BuildLaneTrustCertificationAxis::Visual {
            outcome.state = BuildLaneTrustAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("setting definition silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, BuildLaneTrustProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildLaneTrustCertificationViolation::ProfileBlocked { .. }
    )));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5BuildLaneTrustCertifiedProfile::ReviewableReproducibilityStructure
        })
        .expect("reviewable-reproducibility-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails.pr_caches_publish_release_artifacts = true;
    assert_eq!(row.derive_status(), BuildLaneTrustProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildLaneTrustCertificationViolation::GuardrailViolated { .. }
    )));
}
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5BuildLaneTrustClaim::TrustedExactBuildSupportableLane;
    row.certified_claim = M5BuildLaneTrustClaim::TrustedExactBuildSupportableLane;
    assert_eq!(row.derive_status(), BuildLaneTrustProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BuildLaneTrustCertificationAxis::DegradedState {
            *outcome = BuildLaneTrustAxisOutcome {
                axis: BuildLaneTrustCertificationAxis::DegradedState,
                state: BuildLaneTrustAxisCertificationState::DisclosedNarrowed,
                parity_note: "the build-lane-trust evidence is lagging".to_owned(),
                narrowing_reason: Some(
                    "the build-lane-trust evidence is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5BuildLaneDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableReproducibilitySurface == certified, no claim_auto_narrow.
    assert_eq!(row.derive_status(), BuildLaneTrustProfileClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BuildLaneTrustCertificationAxis::CliExport {
            outcome.state = BuildLaneTrustAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5BuildLaneDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), BuildLaneTrustProfileClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(BuildLaneTrustClaimAutoNarrow {
        binding_axis: BuildLaneTrustCertificationAxis::DegradedState,
        from_claim: M5BuildLaneTrustClaim::TrustedExactBuildSupportableLane,
        to_claim: M5BuildLaneTrustClaim::TrustedExactBuildSupportableLane,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5BuildLaneTrustClaim::CacheDisciplineDisclosedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-reproducibility-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableReproducibilitySurface; certify a stronger trusted exact-build supportable lane.
    row.certified_claim = M5BuildLaneTrustClaim::TrustedExactBuildSupportableLane;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = BuildLaneTrustCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), BuildLaneTrustProfileClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == BuildLaneTrustCertificationAxis::CliExport {
            outcome.state = BuildLaneTrustAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the write-intent surface".to_owned());
            outcome.downgrade_trigger = Some(M5BuildLaneDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = BuildLaneTrustCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), BuildLaneTrustProfileClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == BuildLaneTrustProfileClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "cache posture".to_owned();
    }
    assert_eq!(row.derive_status(), BuildLaneTrustProfileClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(BuildLaneTrustCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5BuildLaneDowngradeTrigger::ProofStale,
    );
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows.retain(|r| {
        r.profile != M5BuildLaneTrustCertifiedProfile::UnverifiedExactBuildSupportabilityProfile
    });
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildLaneTrustCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the release family from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5BuildLaneFamily::Release);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildLaneTrustCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = BuildLaneTrustProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildLaneTrustCertificationViolation::WrongCanonicalBundle
    )));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildLaneTrustCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != BuildLaneTrustCertificationAxis::BuildLaneTrustComponentTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, BuildLaneTrustCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        BuildLaneTrustCertificationViolation::RawBuildLaneTrustMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: BuildLaneTrustProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_build_lane_trust_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-build-lane-trust-surface-certification/support_export.json"
    ));
    let packet_from_fixture: BuildLaneTrustProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-build-lane-trust-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_BUILD_LANE_TRUST_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures
...
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_BUILD_LANE_TRUST_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_build_lane_trust_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/release/m5-build-lane-trust-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-build-lane-trust-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/release/m5-build-lane-trust-surface-certification");
