//! Implemented M5 constrained-state-descriptor and descriptor-change-diff registries.
//!
...
//! be kept consistent: it turns the *constrained-state-descriptor* grammar (one machine-readable descriptor per
...
//! export-safe, honest projections. Every preserved object then resolves to one typed constrained-state-descriptor
...
//!   [`resolve_constrained_state_descriptor_entry`] refuses to read as a clean, registry-bound descriptor entry
//!   unless it names a canonical registry token, a classified [descriptor field][M5ConstrainedStateDescriptorKind],
//!   a historical-reference role, covers every [resolution form][M5ConstrainedStateDescriptorResolutionForm] (the
...
use crate::m5_constrained_file_state_matrix::{
    ...
    M5_CONSTRAINED_FILE_STATE_DOMAIN_SCHEMA_REF, M5_CONSTRAINED_FILE_STATE_MATRIX_DOC_REF,
    M5_CONSTRAINED_FILE_STATE_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5ConstrainedStateDescriptorChangeDiffRegistriesPacket`].
pub const M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_constrained_state_descriptor_and_change_diff_packet_registries";

/// Schema version for M5 line-constrained_state_descriptor / line-downgrade-packet registry records.
pub const M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_REF: &str =
    "schemas/program/m5-constrained-state-descriptor-and-change-diff-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_DOC_REF: &str =
    "docs/support/m5_constrained_state_descriptor_and_change_diff_registries.md";
...
pub const M5_DESCRIPTOR_CHANGE_DIFF_DOMAIN_SCHEMA_REF: &str =
    "schemas/program/m5-constrained-state-change-diff.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/support/m5-constrained-state-descriptor-and-change-diff-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_CSV_REF: &str =
    "artifacts/support/m5-constrained-state-descriptor-and-change-diff-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_REPORT_REF: &str =
    "artifacts/support/m5-constrained-state-descriptor-and-change-diff-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5ConstrainedStateDescriptorChangeDiffRegistriesConsumerSurface =
    M5ConstrainedFileStateConsumerSurface;

/// One of the three resolution forms every line-constrained_state_descriptor or line-downgrade-packet entry must hold across so
...
/// line-constrained_state_descriptor and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ConstrainedStateDescriptorResolutionForm {
    /// The canonical resolved line-constrained_state_descriptor / line-downgrade-packet object.
    CanonicalObject,
...
impl M5ConstrainedStateDescriptorResolutionForm {
...
/// Controlled manifest field a constrained-state-descriptor entry publishes for one retiring object-class identity, so every
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ConstrainedStateDescriptorKind {
...
impl M5ConstrainedStateDescriptorKind {
...
/// Controlled descriptor-change-diff scope a constrained-state-descriptor change sits in, so a changed cutoff date or
...
/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-constrained_state_descriptor or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ConstrainedStateDescriptorSurfaceContext {
...
impl M5ConstrainedStateDescriptorSurfaceContext {
...
/// One mandatory rendered part a line-constrained_state_descriptor or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ConstrainedStateDescriptorAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-constrained_state_descriptor entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-constrained_state_descriptor entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-constrained_state_descriptor
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line constrained_state_descriptor or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5ConstrainedStateDescriptorAnatomyPart {
...
/// line constrained_state_descriptor, a line-downgrade packet, or a degraded line-constrained_state_descriptor / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ConstrainedStateDescriptorNextAction {
    /// Expand the resolved line constrained_state_descriptor's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5ConstrainedStateDescriptorNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ConstrainedStateDescriptorExportField {
...
impl M5ConstrainedStateDescriptorExportField {
...
/// Reason a line-constrained_state_descriptor entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ConstrainedStateDescriptorEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the constrained_state_descriptor means.
    ...
    /// The resolved line-constrained_state_descriptor object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5ConstrainedStateDescriptorEntryDegradeReason {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "constrained_state_descriptor_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortObjectClassUnclassified => "line_object_class_unclassified",
            Self::DescriptorNotBoundToRegistry => "constrained_state_descriptor_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "constrained_state_descriptor_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "constrained_state_descriptor_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5ConstrainedStateDescriptorNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5ConstrainedStateDescriptorNextAction::TraceCanonicalRegistry
            }
            Self::CohortObjectClassUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5ConstrainedStateDescriptorNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5ConstrainedStateDescriptorNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5ConstrainedStateDescriptorNextAction::ReviewBlockedOrDegraded,
        }
    }
    ...
    pub const fn next_action(self) -> M5ConstrainedStateDescriptorNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5ConstrainedStateDescriptorNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_constrained_state_descriptor_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ConstrainedStateDescriptorEntryResolutionInput {
    /// Stable identity of the line-constrained_state_descriptor-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this constrained_state_descriptor binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.constrained_state_descriptor.projection_object`); empty means unstated.
    ...
    pub report_section: M5ConstrainedStateDescriptorKind,
    /// The render / surface context.
    pub surface_context: M5ConstrainedStateDescriptorSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ConstrainedStateDescriptorResolutionForm>,
    ...
    /// True when the behavior traces to the line-constrained_state_descriptor registry (never a hand-copied constant).
...
/// Resolved, export-safe line-constrained_state_descriptor-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedConstrainedStateDescriptorEntry {
    /// Stable identity of the line-constrained_state_descriptor-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this constrained_state_descriptor binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-constrained_state_descriptor object publishes every required field.
    pub constrained_state_descriptor_object_complete: bool,
    /// Whether the entry traces to the line-constrained_state_descriptor registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5ConstrainedStateDescriptorEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ConstrainedStateDescriptorNextAction,
    /// Whether the constrained_state_descriptor resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub constrained_state_descriptor_resolves_across_lines: bool,
}

impl M5ResolvedConstrainedStateDescriptorEntry {
    /// Whether this line-constrained_state_descriptor entry reads as a clean, registry-bound state.
...
pub enum M5ConstrainedStateDescriptorResolutionError {
    /// The line-constrained_state_descriptor-entry id was empty.
...
impl M5ConstrainedStateDescriptorResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_constrained_state_descriptor_entry_id",
...
impl fmt::Display for M5ConstrainedStateDescriptorResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-constrained_state_descriptor / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5ConstrainedStateDescriptorResolutionError {}

fn form_tokens(forms: &[M5ConstrainedStateDescriptorResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5ConstrainedStateDescriptorResolutionForm]) -> bool {
    let present: BTreeSet<M5ConstrainedStateDescriptorResolutionForm> =
        forms.iter().copied().collect();
    M5ConstrainedStateDescriptorResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-constrained_state_descriptor object publishes every required field: classified line journey,
...
pub fn constrained_state_descriptor_object_is_complete(
    journey: M5ConstrainedStateDescriptorKind,
...
/// Whether the line constrained_state_descriptor keeps a line from widening without preserving its rollback and diagnostics
...
/// Resolves a line-constrained_state_descriptor-registry entry so it stays bound to the line-constrained_state_descriptor registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete constrained_state_descriptor object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_constrained_state_descriptor_entry(
    input: M5ConstrainedStateDescriptorEntryResolutionInput,
) -> Result<M5ResolvedConstrainedStateDescriptorEntry, M5ConstrainedStateDescriptorResolutionError>
{
    if input.entry_id.trim().is_empty() {
        return Err(M5ConstrainedStateDescriptorResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5ConstrainedStateDescriptorResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = constrained_state_descriptor_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        ...
        Some(
            M5ConstrainedStateDescriptorEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    ...
    let next_action = match degrade_reason {
        ...
        None => M5ConstrainedStateDescriptorNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedConstrainedStateDescriptorEntry {
        ...
        constrained_state_descriptor_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        constrained_state_descriptor_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
pub fn resolve_descriptor_change_diff_entry(
...
) -> Result<M5ResolvedDescriptorChangeDiffEntry, M5ConstrainedStateDescriptorResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ConstrainedStateDescriptorResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5ConstrainedStateDescriptorNextAction::TraceCanonicalRegistry,
    };
...
/// One registry row: one consumer surface bound to the resolved line-constrained_state_descriptor and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ConstrainedStateDescriptorChangeDiffRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5ConstrainedStateDescriptorChangeDiffRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5ConstrainedStateDescriptorAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5ConstrainedStateDescriptorExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5ConstrainedFileStateDowngradeTrigger>,
    /// Resolved line-constrained_state_descriptor-registry examples.
    pub constrained_state_descriptor_entries: Vec<M5ResolvedConstrainedStateDescriptorEntry>,
    ...
    /// Source contract refs consumed by this row (must include both the line-constrained_state_descriptor and
...
impl M5ConstrainedStateDescriptorChangeDiffRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5ConstrainedStateDescriptorAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5ConstrainedStateDescriptorAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5ConstrainedStateDescriptorExportField> =
            self.export_fields.iter().copied().collect();
        M5ConstrainedStateDescriptorExportField::MANDATORY
            .iter()
    ...
    /// True when a clean line-constrained_state_descriptor entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete constrained_state_descriptor object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn constrained_state_descriptor_is_honest(
        ex: &M5ResolvedConstrainedStateDescriptorEntry,
    ) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.constrained_state_descriptor_object_complete
    ...
    fn examples_are_honest(&self) -> bool {
        self.constrained_state_descriptor_entries
            .iter()
            .all(Self::constrained_state_descriptor_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ConstrainedStateDescriptorChangeDiffRegistriesVocabularySet {
    ...
    /// Cohort-constrained_state_descriptor-entry degrade-reason tokens.
    pub constrained_state_descriptor_degrade_reasons: Vec<String>,
...
impl M5ConstrainedStateDescriptorChangeDiffRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ConstrainedFileStateRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5ConstrainedStateDescriptorResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5ConstrainedStateDescriptorKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5DescriptorChangeDiffScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5ConstrainedStateDescriptorSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            constrained_state_descriptor_degrade_reasons: tokens(
                &M5ConstrainedStateDescriptorEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            ...
            anatomy_parts: tokens(&M5ConstrainedStateDescriptorAnatomyPart::ALL, |v| {
                v.as_str()
            }),
            next_actions: tokens(&M5ConstrainedStateDescriptorNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5ConstrainedStateDescriptorExportField::ALL, |v| {
                v.as_str()
            }),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ConstrainedStateDescriptorChangeDiffRegistriesGovernanceReview {
    /// The constrained_state_descriptor registry names a canonical token, semantic role, and line journey for every entry.
    pub constrained_state_descriptor_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-constrained_state_descriptor object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_constrained_state_descriptor_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved constrained_state_descriptor.
    ...
    /// Every line-constrained_state_descriptor and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-constrained_state_descriptor and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub constrained_state_descriptor_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ConstrainedStateDescriptorChangeDiffRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-constrained_state_descriptor registry.
    ...
    /// Behavior traces back to the canonical line-constrained_state_descriptor and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-constrained_state_descriptor / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ConstrainedStateDescriptorChangeDiffRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ConstrainedStateDescriptorChangeDiffRegistriesReleasePosture {
...
/// Constructor input for [`M5ConstrainedStateDescriptorChangeDiffRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ConstrainedStateDescriptorChangeDiffRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5ConstrainedStateDescriptorChangeDiffRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5ConstrainedStateDescriptorChangeDiffRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5ConstrainedStateDescriptorChangeDiffRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5ConstrainedStateDescriptorChangeDiffRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5ConstrainedStateDescriptorChangeDiffRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5ConstrainedStateDescriptorChangeDiffRegistriesReleasePosture,
...
/// Export-safe M5 line-constrained_state_descriptor and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ConstrainedStateDescriptorChangeDiffRegistriesPacket {
    /// Record kind; must equal [`M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION`].
...
impl M5ConstrainedStateDescriptorChangeDiffRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5ConstrainedStateDescriptorChangeDiffRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version: M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5ConstrainedStateDescriptorChangeDiffRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_RECORD_KIND {
            violations
                .push(M5ConstrainedStateDescriptorChangeDiffRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION
        {
            violations.push(
                M5ConstrainedStateDescriptorChangeDiffRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations
                .push(M5ConstrainedStateDescriptorChangeDiffRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(
                M5ConstrainedStateDescriptorChangeDiffRegistriesViolation::VocabularySetDrift,
            );
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-constrained_state_descriptor / line-downgrade-packet registries packet serializes",
        )) {
            violations.push(M5ConstrainedStateDescriptorChangeDiffRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,constrained_state_descriptor_entries,descriptor_change_diff_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            let degrades: Vec<&str> = row
                .constrained_state_descriptor_entries
                .iter()
            ...
            out.push_str(&format!(
                ...
                row.constrained_state_descriptor_entries.len(),
    ...
    /// registry-bound line-constrained_state_descriptor entries are listed.
    pub fn render_constrained_state_descriptor_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.constrained_state_descriptor_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5ConstrainedStateDescriptorChangeDiffRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5ConstrainedStateDescriptorChangeDiffRegistriesViolation>),
}

impl fmt::Display for M5ConstrainedStateDescriptorChangeDiffRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-constrained_state_descriptor / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-constrained_state_descriptor / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5ConstrainedStateDescriptorChangeDiffRegistriesArtifactError {}

/// Validation failures emitted by [`M5ConstrainedStateDescriptorChangeDiffRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5ConstrainedStateDescriptorChangeDiffRegistriesViolation {
    ...
    /// A registry row does not point at both the line-constrained_state_descriptor and line-downgrade-packet domain schemas.
    ...
    /// Cohort-constrained_state_descriptor-resolution is not proven: clean constrained_state_descriptor entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean constrained_state_descriptor entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded constrained_state_descriptor entry is present, or a clean constrained_state_descriptor entry is unbounded
    /// or unbound.
...
impl M5ConstrainedStateDescriptorChangeDiffRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "constrained_state_descriptor_resolution_not_proven"
            }
...
pub fn current_stable_m5_constrained_state_descriptor_and_change_diff_registries_export() -> Result<
    M5ConstrainedStateDescriptorChangeDiffRegistriesPacket,
    M5ConstrainedStateDescriptorChangeDiffRegistriesArtifactError,
> {
    let packet: M5ConstrainedStateDescriptorChangeDiffRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/support/m5-constrained-state-descriptor-and-change-diff-registries-proof/support_export.json"
        )
    ))
    .map_err(M5ConstrainedStateDescriptorChangeDiffRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5ConstrainedStateDescriptorChangeDiffRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5ConstrainedStateDescriptorChangeDiffRegistriesPacket,
    violations: &mut Vec<M5ConstrainedStateDescriptorChangeDiffRegistriesViolation>,
) {
    let refs: BTreeSet<&str> = packet
        ...
        .collect();
    for required in [
        M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_REF,
        M5_CONSTRAINED_STATE_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_DOC_REF,
        M5_CONSTRAINED_FILE_STATE_MATRIX_SCHEMA_REF,
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5ConstrainedStateDescriptorChangeDiffRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5ConstrainedStateDescriptorChangeDiffRegistriesViolation::RegistryRowIncomplete,
            );
        }
        ...
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_CONSTRAINED_FILE_STATE_DOMAIN_SCHEMA_REF)
        ...
        if row.constrained_state_descriptor_entries.is_empty()
            || row.descriptor_change_diff_entries.is_empty()
        {
            violations
                .push(M5ConstrainedStateDescriptorChangeDiffRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations
                .push(M5ConstrainedStateDescriptorChangeDiffRegistriesViolation::DishonestExample);
        }
...
fn validate_governance_review(
    ...
    for ok in [
        review.constrained_state_descriptor_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_constrained_state_descriptor_from_shared_registry,
        ...
        review.constrained_state_descriptor_or_downgrade_drift_caught_before_release,
...
fn validate_acceptance_criteria(
    ...
    let constrained_state_descriptors = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.constrained_state_descriptor_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
    ...
    // Clean constrained_state_descriptor entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean constrained_state_descriptor entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = constrained_state_descriptors()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = constrained_state_descriptors()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5ConstrainedStateDescriptorKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5ConstrainedStateDescriptorSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = constrained_state_descriptors().any(|ex| {
        ex.degrade_reason
            == Some(
                M5ConstrainedStateDescriptorEntryDegradeReason::CohortDescriptorObjectIncomplete,
            )
    });
    let no_clean_incomplete = !constrained_state_descriptors()
        .any(|ex| ex.is_clean() && !ex.constrained_state_descriptor_object_complete);
    if !(journeys_covered
    ...
    // example degrades, an unbound example degrades, at least one clean bounded constrained_state_descriptor entry is present, and
    // no clean constrained_state_descriptor entry is unbounded or unbound.
    let widen_fold_degrades = constrained_state_descriptors().any(|ex| {
        ex.degrade_reason
            == Some(
                M5ConstrainedStateDescriptorEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = constrained_state_descriptors().any(|ex| {
        ex.degrade_reason
            == Some(M5ConstrainedStateDescriptorEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_constrained_state_descriptor = constrained_state_descriptors()
        .any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound =
        !constrained_state_descriptors().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded = !constrained_state_descriptors()
        .any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_constrained_state_descriptor
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-constrained_state_descriptor registry
