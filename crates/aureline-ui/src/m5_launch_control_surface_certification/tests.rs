//! Tests for the M05-1219 launch-control surface certification capstone.

use super::*;

fn packet() -> LaunchControlProfileCertificationPacket {
    seeded_m5_launch_control_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, LAUNCH_CONTROL_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, LAUNCH_CONTROL_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5LaunchControlCertifiedProfile::ALL {
...
fn every_frozen_family_is_certified_on_some_profile() {
    ...
    for family in M5LaunchControlCohort::ALL {
        assert!(
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            LaunchControlCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, LaunchControlAxisCertificationState::Certified);
...
fn a_narrowed_profile_never_keeps_a_trusted_claim() {
    for row in &packet().rows {
        if row.is_claim_narrowed() {
            assert!(
                !row.certified_claim.asserts_certified_widening_lane(),
...
fn all_five_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5LaunchControlClaim> = p.rows.iter().map(|r| r.certified_claim).collect();
    for claim in M5LaunchControlClaim::ALL {
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5LaunchControlCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5LaunchControlCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = LaunchControlCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), LaunchControlCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in LaunchControlCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == LaunchControlCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    // The B145 localization axis is present, distinct, and exercised by the
    // disclosed-freeze-exception profile as its binding narrowed axis.
    let p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5LaunchControlCertifiedProfile::DisclosedFreezeExceptionProfile)
        .expect("disclosed-freeze-exception-profile row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(LaunchControlCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(LaunchControlCertificationAxis::Localization)
    );
}

#[test]
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    // The B145 high-contrast reach axis stays certified across the seed (never a binding narrowed axis here).
    for row in &packet().rows {
        let outcome = row
            .axis(LaunchControlCertificationAxis::HighContrast)
...
#[test]
fn the_three_yellow_rows_bind_three_distinct_axes_and_triggers() {
    // Each yellow profile discloses via a distinct binding axis and a distinct downgrade trigger, matching the
    // per-lane concern (freeze-exception scope / rehearsal currency / go-no-go evidence).
    let p = packet();
    let axes: BTreeSet<LaunchControlCertificationAxis> = p
        .rows
        .iter()
        .filter_map(|r| r.claim_auto_narrow.as_ref().map(|n| n.binding_axis))
        .collect();
    assert_eq!(axes.len(), 3, "yellow rows must bind three distinct axes");
    let triggers: std::collections::HashSet<M5LaunchControlDowngradeTrigger> = p
        .rows
        .iter()
    ...
    assert_eq!(
        triggers.len(),
        3,
        "yellow rows must carry three distinct downgrade triggers"
    );
}
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5LaunchControlCertifiedProfile::LiveCertifiedWideningLane)
        .expect("live-certified-widening-lane row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == LaunchControlCertificationAxis::Visual {
            outcome.state = LaunchControlAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("setting definition silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, LaunchControlProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        LaunchControlCertificationViolation::ProfileBlocked { .. }
    )));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5LaunchControlCertifiedProfile::ReviewableLaunchControlStructure
        })
        .expect("reviewable-launch-control-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails
        .widens_a_stable_claim_without_current_cohort_and_rehearsal_evidence = true;
    assert_eq!(row.derive_status(), LaunchControlProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        LaunchControlCertificationViolation::GuardrailViolated { .. }
    )));
}
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    // non-live rule, not a certified-exceeds-claim strengthening.
    row.claimed_claim = M5LaunchControlClaim::CertifiedWideningLane;
    row.certified_claim = M5LaunchControlClaim::CertifiedWideningLane;
    assert_eq!(row.derive_status(), LaunchControlProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == LaunchControlCertificationAxis::DegradedState {
            *outcome = LaunchControlAxisOutcome {
                axis: LaunchControlCertificationAxis::DegradedState,
                state: LaunchControlAxisCertificationState::DisclosedNarrowed,
                parity_note: "the launch-control evidence is lagging".to_owned(),
                narrowing_reason: Some(
                    "the launch-control evidence is partial for this profile".to_owned(),
                ),
                downgrade_trigger: Some(M5LaunchControlDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ReviewableLaunchControlSurface == certified, no claim_auto_narrow.
    assert_eq!(row.derive_status(), LaunchControlProfileClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == LaunchControlCertificationAxis::CliExport {
            outcome.state = LaunchControlAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger = Some(M5LaunchControlDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), LaunchControlProfileClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(LaunchControlClaimAutoNarrow {
        binding_axis: LaunchControlCertificationAxis::DegradedState,
        from_claim: M5LaunchControlClaim::CertifiedWideningLane,
        to_claim: M5LaunchControlClaim::CertifiedWideningLane,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5LaunchControlClaim::FreezeExceptionDisclosedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-launch-control-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableLaunchControlSurface; certify a stronger certified widening lane.
    row.certified_claim = M5LaunchControlClaim::CertifiedWideningLane;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = LaunchControlCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), LaunchControlProfileClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == LaunchControlCertificationAxis::CliExport {
            outcome.state = LaunchControlAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the freeze-exception surface".to_owned());
            outcome.downgrade_trigger = Some(M5LaunchControlDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = LaunchControlCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), LaunchControlProfileClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == LaunchControlProfileClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "cohort membership".to_owned();
    }
    assert_eq!(row.derive_status(), LaunchControlProfileClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(LaunchControlCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5LaunchControlDowngradeTrigger::ProofStale,
    );
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.profile != M5LaunchControlCertifiedProfile::UnverifiedRegressionAssetProfile);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        LaunchControlCertificationViolation::ProfileCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the certified-archetype cohort from every row that carries it; coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5LaunchControlCohort::CertifiedArchetype);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        LaunchControlCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = LaunchControlProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        LaunchControlCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, LaunchControlCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, LaunchControlCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != LaunchControlCertificationAxis::LaunchControlComponentTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, LaunchControlCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        LaunchControlCertificationViolation::RawLaunchControlMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: LaunchControlProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_launch_control_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-launch-control-surface-certification/support_export.json"
    ));
    let packet_from_fixture: LaunchControlProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/release/m5-launch-control-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_LAUNCH_CONTROL_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures
...
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_LAUNCH_CONTROL_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_launch_control_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/release/m5-launch-control-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/release/m5-launch-control-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/release/m5-launch-control-surface-certification");
