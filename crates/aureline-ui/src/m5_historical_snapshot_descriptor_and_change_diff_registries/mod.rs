//! Implemented M5 historical-snapshot-descriptor and descriptor-change-diff registries.
//!
...
//! be kept consistent: it turns the *historical-snapshot-descriptor* grammar (one machine-readable descriptor per
...
//! its capture-context joins) and the *descriptor-change-diff* grammar (the diff scope a descriptor change sits in —
//! a producer-build change, a target-link change, or a retention-state change) into registry resolvers that produce
//! export-safe, honest projections. Every preserved object then resolves to one typed historical-snapshot-descriptor
...
//! descriptor-change-diff object — the resolved object identity, the affected descriptor field, the
...
//!   [`resolve_historical_snapshot_descriptor_entry`] refuses to read as a clean, registry-bound descriptor entry
//!   unless it names a canonical registry token, a classified [descriptor field][M5HistoricalSnapshotDescriptorKind],
//!   a historical-reference role, covers every [resolution form][M5HistoricalSnapshotDescriptorResolutionForm] (the
...
//!   mutation.** [`resolve_descriptor_change_diff_entry`] names a classified [diff scope][M5DescriptorChangeDiffScope]
...
//!   state.
//!
//! The resolvers reuse the frozen matrix vocabulary directly — the [`M5HistoricalReferenceRole`] historical-reference
//! role vocabulary and the [`M5HistoricalReferenceConsumerSurface`] consumer-surface taxonomy — so the shell / archive
...
//!
//! [matrix]: crate::m5_historical_reference_matrix
...
use crate::m5_historical_reference_matrix::{
    M5HistoricalReferenceAccessibilityRoute, M5HistoricalReferenceCaptureLifecycleStage,
    M5HistoricalReferenceConsumerSurface, M5HistoricalReferenceDowngradeTrigger,
    M5HistoricalReferenceObject, M5HistoricalReferenceQualificationClass,
    M5HistoricalReferenceRequiredLabel, M5HistoricalReferenceRole,
    M5_HISTORICAL_REFERENCE_MATRIX_DOC_REF, M5_HISTORICAL_REFERENCE_MATRIX_SCHEMA_REF,
    M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_DOMAIN_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5HistoricalSnapshotDescriptorChangeDiffRegistriesPacket`].
pub const M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_historical_snapshot_descriptor_and_change_diff_packet_registries";

/// Schema version for M5 line-historical_snapshot_descriptor / line-downgrade-packet registry records.
pub const M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_DOC_REF: &str =
    "docs/release/m5_historical_snapshot_descriptor_and_change_diff_registries.md";
...
pub const M5_DESCRIPTOR_CHANGE_DIFF_DOMAIN_SCHEMA_REF: &str =
    "schemas/program/m5-descriptor-change-diff.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-historical-snapshot-descriptor-and-change-diff-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-historical-snapshot-descriptor-and-change-diff-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-historical-snapshot-descriptor-and-change-diff-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5HistoricalSnapshotDescriptorChangeDiffRegistriesConsumerSurface =
    M5HistoricalReferenceConsumerSurface;

/// One of the three resolution forms every line-historical_snapshot_descriptor or line-downgrade-packet entry must hold across so
...
/// line-historical_snapshot_descriptor and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5HistoricalSnapshotDescriptorResolutionForm {
    /// The canonical resolved line-historical_snapshot_descriptor / line-downgrade-packet object.
    CanonicalObject,
...
impl M5HistoricalSnapshotDescriptorResolutionForm {
...
/// Controlled manifest field a historical-snapshot-descriptor entry publishes for one retiring object-class identity, so every
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5HistoricalSnapshotDescriptorKind {
...
impl M5HistoricalSnapshotDescriptorKind {
...
/// Controlled descriptor-change-diff scope a historical-snapshot-descriptor change sits in, so a changed cutoff date or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5DescriptorChangeDiffScope {
    ...
    RetentionStateChange,
    /// The descriptor-change-diff scope is unclassified, which is disallowed.
    DiffScopeUnclassified,
}

impl M5DescriptorChangeDiffScope {
    /// Every comparison scope, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::ProducerBuildChange,
        Self::TargetLinkChange,
        Self::RetentionStateChange,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::ProducerBuildChange => "producer_build_change",
            Self::TargetLinkChange => "target_link_change",
            Self::RetentionStateChange => "retention_state_change",
...
/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-historical_snapshot_descriptor or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5HistoricalSnapshotDescriptorSurfaceContext {
...
impl M5HistoricalSnapshotDescriptorSurfaceContext {
...
/// One mandatory rendered part a line-historical_snapshot_descriptor or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5HistoricalSnapshotDescriptorAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-historical_snapshot_descriptor entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-historical_snapshot_descriptor entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-historical_snapshot_descriptor
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line historical_snapshot_descriptor or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5HistoricalSnapshotDescriptorAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "descriptor_change_diff_fields",
...
/// line historical_snapshot_descriptor, a line-downgrade packet, or a degraded line-historical_snapshot_descriptor / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5HistoricalSnapshotDescriptorNextAction {
    /// Expand the resolved line historical_snapshot_descriptor's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
impl M5HistoricalSnapshotDescriptorNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5HistoricalSnapshotDescriptorExportField {
...
impl M5HistoricalSnapshotDescriptorExportField {
...
/// Reason a line-historical_snapshot_descriptor entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5HistoricalSnapshotDescriptorEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the historical_snapshot_descriptor means.
    ...
    /// The resolved line-historical_snapshot_descriptor object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5HistoricalSnapshotDescriptorEntryDegradeReason {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "historical_snapshot_descriptor_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortManifestFieldUnclassified => "line_manifest_field_unclassified",
            Self::DescriptorNotBoundToRegistry => "historical_snapshot_descriptor_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "historical_snapshot_descriptor_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "historical_snapshot_descriptor_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5HistoricalSnapshotDescriptorNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5HistoricalSnapshotDescriptorNextAction::TraceCanonicalRegistry
            }
            Self::CohortManifestFieldUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5HistoricalSnapshotDescriptorNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5HistoricalSnapshotDescriptorNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5HistoricalSnapshotDescriptorNextAction::ReviewBlockedOrDegraded,
        }
    }

    /// The matching downgrade trigger recorded on the frozen matrix.
    pub const fn downgrade_trigger(self) -> M5HistoricalReferenceDowngradeTrigger {
        match self {
            Self::DescriptorTokenUnstated
            | Self::SurfaceContextUnresolved
            | Self::ResolutionFormCoverageIncomplete
            | Self::DescriptorNotBoundToRegistry => {
                M5HistoricalReferenceDowngradeTrigger::EvidenceUnjoinedFromCaptureContext
            }
            Self::CohortManifestFieldUnclassified | Self::CohortDescriptorObjectIncomplete => {
                M5HistoricalReferenceDowngradeTrigger::CaptureTimeMissing
            }
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort => {
                M5HistoricalReferenceDowngradeTrigger::LiveTargetAvailabilityUnstated
            }
            Self::ProofStale => {
                M5HistoricalReferenceDowngradeTrigger::HistoricalReferenceDescriptorStale
            }
        }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5DescriptorChangeDiffEntryDegradeReason {
...
impl M5DescriptorChangeDiffEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                "descriptor_change_diff_runs_support_ahead_of_proof_or_drops_descriptor_change_diff"
            }
    ...
    pub const fn next_action(self) -> M5HistoricalSnapshotDescriptorNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5HistoricalSnapshotDescriptorNextAction::ReviewBlockedOrDegraded
            }
        }
    }
    ...
    pub const fn downgrade_trigger(self) -> M5HistoricalReferenceDowngradeTrigger {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::EvidenceScopeUnclassified => {
                M5HistoricalReferenceDowngradeTrigger::RemovalStateUnstated
            }
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5HistoricalReferenceDowngradeTrigger::ProvenanceUnattributed
            }
            Self::EvidenceFormCoverageIncomplete => {
                M5HistoricalReferenceDowngradeTrigger::SnapshotLabelMissing
            }
            Self::ProofStale => {
                M5HistoricalReferenceDowngradeTrigger::HistoricalReferenceDescriptorStale
            }
...
/// Input to [`resolve_historical_snapshot_descriptor_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5HistoricalSnapshotDescriptorEntryResolutionInput {
    /// Stable identity of the line-historical_snapshot_descriptor-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this historical_snapshot_descriptor binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.historical_snapshot_descriptor.disable_path`); empty means unstated.
    pub token_name: String,
    /// The high-level launch-control role (from the frozen matrix vocabulary).
    pub semantic_role: M5HistoricalReferenceRole,
    /// The line journey this entry classifies.
    pub report_section: M5HistoricalSnapshotDescriptorKind,
    /// The render / surface context.
    pub surface_context: M5HistoricalSnapshotDescriptorSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5HistoricalSnapshotDescriptorResolutionForm>,
    ...
    /// True when the behavior traces to the line-historical_snapshot_descriptor registry (never a hand-copied constant).
...
/// Resolved, export-safe line-historical_snapshot_descriptor-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedHistoricalSnapshotDescriptorEntry {
    /// Stable identity of the line-historical_snapshot_descriptor-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this historical_snapshot_descriptor binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-historical_snapshot_descriptor object publishes every required field.
    pub historical_snapshot_descriptor_object_complete: bool,
    /// Whether the entry traces to the line-historical_snapshot_descriptor registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5HistoricalSnapshotDescriptorEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5HistoricalSnapshotDescriptorNextAction,
    /// Whether the historical_snapshot_descriptor resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub historical_snapshot_descriptor_resolves_across_lines: bool,
}

impl M5ResolvedHistoricalSnapshotDescriptorEntry {
    /// Whether this line-historical_snapshot_descriptor entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_descriptor_change_diff_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5DescriptorChangeDiffEntryResolutionInput {
    ...
    pub comparison_scope: M5DescriptorChangeDiffScope,
    /// The render / surface context.
    pub surface_context: M5HistoricalSnapshotDescriptorSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5HistoricalSnapshotDescriptorResolutionForm>,
    ...
    pub keeps_descriptor_change_diff_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedDescriptorChangeDiffEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub descriptor_change_diff_stays_honest: bool,
    ...
    pub provides_complete_descriptor_change_diff: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5DescriptorChangeDiffEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5HistoricalSnapshotDescriptorNextAction,
...
impl M5ResolvedDescriptorChangeDiffEntry {
...
pub enum M5HistoricalSnapshotDescriptorResolutionError {
    /// The line-historical_snapshot_descriptor-entry id was empty.
...
impl M5HistoricalSnapshotDescriptorResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_historical_snapshot_descriptor_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_descriptor_change_diff_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5HistoricalSnapshotDescriptorResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-historical_snapshot_descriptor / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5HistoricalSnapshotDescriptorResolutionError {}

fn form_tokens(forms: &[M5HistoricalSnapshotDescriptorResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5HistoricalSnapshotDescriptorResolutionForm]) -> bool {
    let present: BTreeSet<M5HistoricalSnapshotDescriptorResolutionForm> =
        forms.iter().copied().collect();
    M5HistoricalSnapshotDescriptorResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-historical_snapshot_descriptor object publishes every required field: classified line journey,
...
pub fn historical_snapshot_descriptor_object_is_complete(
    journey: M5HistoricalSnapshotDescriptorKind,
...
/// Whether the line historical_snapshot_descriptor keeps a line from widening without preserving its rollback and diagnostics
...
pub fn descriptor_change_diff_stays_honest(
    scope: M5DescriptorChangeDiffScope,
    comparison_is_truthful: bool,
    keeps_descriptor_change_diff_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_descriptor_change_diff_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-historical_snapshot_descriptor-registry entry so it stays bound to the line-historical_snapshot_descriptor registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete historical_snapshot_descriptor object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_historical_snapshot_descriptor_entry(
    input: M5HistoricalSnapshotDescriptorEntryResolutionInput,
) -> Result<
    M5ResolvedHistoricalSnapshotDescriptorEntry,
    M5HistoricalSnapshotDescriptorResolutionError,
> {
    if input.entry_id.trim().is_empty() {
        return Err(M5HistoricalSnapshotDescriptorResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5HistoricalSnapshotDescriptorResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = historical_snapshot_descriptor_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        ...
        Some(
            M5HistoricalSnapshotDescriptorEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    ...
    let next_action = match degrade_reason {
        ...
        None => M5HistoricalSnapshotDescriptorNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedHistoricalSnapshotDescriptorEntry {
        ...
        historical_snapshot_descriptor_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        historical_snapshot_descriptor_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_descriptor_change_diff_entry(
    input: M5DescriptorChangeDiffEntryResolutionInput,
) -> Result<M5ResolvedDescriptorChangeDiffEntry, M5HistoricalSnapshotDescriptorResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = descriptor_change_diff_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_descriptor_change_diff_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5DescriptorChangeDiffEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        ...
        Some(M5DescriptorChangeDiffEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5HistoricalSnapshotDescriptorNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedDescriptorChangeDiffEntry {
        entry_id: input.entry_id,
        ...
        keeps_descriptor_change_diff_visible: input.keeps_descriptor_change_diff_visible,
        ...
        descriptor_change_diff_stays_honest: record_stays_honest,
        provides_complete_descriptor_change_diff: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-historical_snapshot_descriptor and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5HistoricalSnapshotDescriptorChangeDiffRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5HistoricalSnapshotDescriptorChangeDiffRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5HistoricalSnapshotDescriptorAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5HistoricalSnapshotDescriptorExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5HistoricalReferenceDowngradeTrigger>,
    /// Resolved line-historical_snapshot_descriptor-registry examples.
    pub historical_snapshot_descriptor_entries: Vec<M5ResolvedHistoricalSnapshotDescriptorEntry>,
    /// Resolved line-downgrade-packet examples.
    pub descriptor_change_diff_entries: Vec<M5ResolvedDescriptorChangeDiffEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-historical_snapshot_descriptor and
    ...
    pub collapses_distinct_descriptor_change_diff_classes_into_one_lane: bool,
}

impl M5HistoricalSnapshotDescriptorChangeDiffRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5HistoricalSnapshotDescriptorAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5HistoricalSnapshotDescriptorAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5HistoricalSnapshotDescriptorExportField> =
            self.export_fields.iter().copied().collect();
        M5HistoricalSnapshotDescriptorExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_descriptor_change_diff_classes_into_one_lane
    }

    /// True when a clean line-historical_snapshot_descriptor entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete historical_snapshot_descriptor object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn historical_snapshot_descriptor_is_honest(
        ex: &M5ResolvedHistoricalSnapshotDescriptorEntry,
    ) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.historical_snapshot_descriptor_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedDescriptorChangeDiffEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_descriptor_change_diff
                && ex.descriptor_change_diff_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.historical_snapshot_descriptor_entries
            .iter()
            .all(Self::historical_snapshot_descriptor_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5HistoricalSnapshotDescriptorChangeDiffRegistriesVocabularySet {
    ...
    /// Cohort-historical_snapshot_descriptor-entry degrade-reason tokens.
    pub historical_snapshot_descriptor_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub descriptor_change_diff_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5HistoricalSnapshotDescriptorChangeDiffRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5HistoricalReferenceRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5HistoricalSnapshotDescriptorResolutionForm::ALL, |v| {
                v.as_str()
            }),
            report_section_kinds: tokens(&M5HistoricalSnapshotDescriptorKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5DescriptorChangeDiffScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5HistoricalSnapshotDescriptorSurfaceContext::ALL, |v| {
                v.as_str()
            }),
            historical_snapshot_descriptor_degrade_reasons: tokens(
                &M5HistoricalSnapshotDescriptorEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            descriptor_change_diff_degrade_reasons: tokens(
                &M5DescriptorChangeDiffEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5HistoricalSnapshotDescriptorAnatomyPart::ALL, |v| {
                v.as_str()
            }),
            next_actions: tokens(&M5HistoricalSnapshotDescriptorNextAction::ALL, |v| {
                v.as_str()
            }),
            export_fields: tokens(&M5HistoricalSnapshotDescriptorExportField::ALL, |v| {
                v.as_str()
            }),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5HistoricalSnapshotDescriptorChangeDiffRegistriesGovernanceReview {
    /// The historical_snapshot_descriptor registry names a canonical token, semantic role, and line journey for every entry.
    pub historical_snapshot_descriptor_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-historical_snapshot_descriptor object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_historical_snapshot_descriptor_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved historical_snapshot_descriptor.
    ...
    pub descriptor_change_diff_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-historical_snapshot_descriptor and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-historical_snapshot_descriptor and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub historical_snapshot_descriptor_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5HistoricalSnapshotDescriptorChangeDiffRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-historical_snapshot_descriptor registry.
    ...
    /// Behavior traces back to the canonical line-historical_snapshot_descriptor and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-historical_snapshot_descriptor / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5HistoricalSnapshotDescriptorChangeDiffRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5HistoricalSnapshotDescriptorChangeDiffRegistriesReleasePosture {
...
/// Constructor input for [`M5HistoricalSnapshotDescriptorChangeDiffRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5HistoricalSnapshotDescriptorChangeDiffRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5HistoricalSnapshotDescriptorChangeDiffRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5HistoricalSnapshotDescriptorChangeDiffRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5HistoricalSnapshotDescriptorChangeDiffRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5HistoricalSnapshotDescriptorChangeDiffRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5HistoricalSnapshotDescriptorChangeDiffRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5HistoricalSnapshotDescriptorChangeDiffRegistriesReleasePosture,
...
/// Export-safe M5 line-historical_snapshot_descriptor and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5HistoricalSnapshotDescriptorChangeDiffRegistriesPacket {
    /// Record kind; must equal [`M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION`].
...
impl M5HistoricalSnapshotDescriptorChangeDiffRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5HistoricalSnapshotDescriptorChangeDiffRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version: M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_RECORD_KIND
        {
            violations
                .push(M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_VERSION
        {
            violations.push(
                M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation::WrongSchemaVersion,
            );
        }
        ...
        {
            violations
                .push(M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation::MissingIdentity);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            "m5 line-historical_snapshot_descriptor / line-downgrade-packet registries packet serializes",
        )) {
            violations.push(M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,historical_snapshot_descriptor_entries,descriptor_change_diff_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            let degrades: Vec<&str> = row
                .historical_snapshot_descriptor_entries
                .iter()
            ...
            out.push_str(&format!(
                ...
                row.historical_snapshot_descriptor_entries.len(),
                row.descriptor_change_diff_entries.len(),
                degrades.join("|"),
    ...
    /// registry-bound line-historical_snapshot_descriptor entries are listed.
    pub fn render_historical_snapshot_descriptor_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.historical_snapshot_descriptor_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5HistoricalSnapshotDescriptorChangeDiffRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation>),
}

impl fmt::Display for M5HistoricalSnapshotDescriptorChangeDiffRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-historical_snapshot_descriptor / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-historical_snapshot_descriptor / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5HistoricalSnapshotDescriptorChangeDiffRegistriesArtifactError {}

/// Validation failures emitted by [`M5HistoricalSnapshotDescriptorChangeDiffRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation {
    ...
    /// A registry row does not point at both the line-historical_snapshot_descriptor and line-downgrade-packet domain schemas.
    ...
    /// Cohort-historical_snapshot_descriptor-resolution is not proven: clean historical_snapshot_descriptor entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean historical_snapshot_descriptor entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded historical_snapshot_descriptor entry is present, or a clean historical_snapshot_descriptor entry is unbounded
    /// or unbound.
...
impl M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => {
                "historical_snapshot_descriptor_resolution_not_proven"
            }
            Self::RollbackAndDiagnosticsPreservationNotProven => {
                "rollback_and_diagnostics_preservation_not_proven"
            }
            Self::CohortEvidenceIntegrityNotProven => "descriptor_change_diff_integrity_not_proven",
...
pub fn current_stable_m5_historical_snapshot_descriptor_and_change_diff_registries_export(
) -> Result<
    M5HistoricalSnapshotDescriptorChangeDiffRegistriesPacket,
    M5HistoricalSnapshotDescriptorChangeDiffRegistriesArtifactError,
> {
    let packet: M5HistoricalSnapshotDescriptorChangeDiffRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-historical-snapshot-descriptor-and-change-diff-registries-proof/support_export.json"
        )
    ))
    .map_err(M5HistoricalSnapshotDescriptorChangeDiffRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5HistoricalSnapshotDescriptorChangeDiffRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5HistoricalSnapshotDescriptorChangeDiffRegistriesPacket,
    violations: &mut Vec<M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation>,
) {
    let refs: BTreeSet<&str> = packet
        ...
        .collect();
    for required in [
        M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_SCHEMA_REF,
        M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_CHANGE_DIFF_REGISTRIES_DOC_REF,
        M5_HISTORICAL_REFERENCE_MATRIX_SCHEMA_REF,
        M5_HISTORICAL_REFERENCE_MATRIX_DOC_REF,
        M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_DOMAIN_SCHEMA_REF,
        M5_DESCRIPTOR_CHANGE_DIFF_DOMAIN_SCHEMA_REF,
    ] {
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations
            .push(M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(
                M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation::RegistryRowIncomplete,
            );
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation::MandatoryAnatomyMissing);
        }
        ...
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_HISTORICAL_SNAPSHOT_DESCRIPTOR_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_DESCRIPTOR_CHANGE_DIFF_DOMAIN_SCHEMA_REF)
        {
            violations.push(
                M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation::DomainSchemaRefMissing,
            );
        }
        if row.historical_snapshot_descriptor_entries.is_empty()
            || row.descriptor_change_diff_entries.is_empty()
        {
            violations
                .push(M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(
                M5HistoricalSnapshotDescriptorChangeDiffRegistriesViolation::DishonestExample,
            );
        }
...
fn validate_governance_review(
    ...
    for ok in [
        review.historical_snapshot_descriptor_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_historical_snapshot_descriptor_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.descriptor_change_diff_keeps_proof_visible_and_binds_support_language,
        ...
        review.historical_snapshot_descriptor_or_downgrade_drift_caught_before_release,
...
fn validate_acceptance_criteria(
    ...
    let historical_snapshot_descriptors = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.historical_snapshot_descriptor_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.descriptor_change_diff_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean historical_snapshot_descriptor entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean historical_snapshot_descriptor entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = historical_snapshot_descriptors()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = historical_snapshot_descriptors()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5HistoricalSnapshotDescriptorKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5HistoricalSnapshotDescriptorSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = historical_snapshot_descriptors().any(|ex| {
        ex.degrade_reason
            == Some(
                M5HistoricalSnapshotDescriptorEntryDegradeReason::CohortDescriptorObjectIncomplete,
            )
    });
    let no_clean_incomplete = !historical_snapshot_descriptors()
        .any(|ex| ex.is_clean() && !ex.historical_snapshot_descriptor_object_complete);
    if !(journeys_covered
    ...
    // example degrades, an unbound example degrades, at least one clean bounded historical_snapshot_descriptor entry is present, and
    // no clean historical_snapshot_descriptor entry is unbounded or unbound.
    let widen_fold_degrades = historical_snapshot_descriptors().any(|ex| {
        ex.degrade_reason
            == Some(
                M5HistoricalSnapshotDescriptorEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = historical_snapshot_descriptors().any(|ex| {
        ex.degrade_reason
            == Some(M5HistoricalSnapshotDescriptorEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_historical_snapshot_descriptor = historical_snapshot_descriptors()
        .any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound =
        !historical_snapshot_descriptors().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded = !historical_snapshot_descriptors()
        .any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_historical_snapshot_descriptor
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5DescriptorChangeDiffScope::CANONICAL_SCOPES
        .iter()
    ...
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_descriptor_change_diff);
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-historical_snapshot_descriptor registry
...
pub const IMPLEMENTED_LINES: [M5HistoricalReferenceObject; 5] = M5HistoricalReferenceObject::ALL;
