//! Implemented M5 linked-change-panel and linked-change-relation registries.
//!
//! The frozen [change-intent lifecycle matrix][matrix] names Aureline's governed change-intent object classes and
//! locks their controlled vocabulary. This module makes the `linked_change_panel` class operable so the work-item
//! detail, review detail, Git / worktree, linked-change, provider-handoff, help / docs, and support / export surfaces
//! inherit one canonical linked-change descriptor rather than a hand-authored parallel prose that has to be kept
//! consistent: it turns the *linked-change-panel* grammar (one reusable panel per tracked work item that shows the
//! active engineering artifacts tied to the item — the branch / worktree state, the hosted review state, the
//! validation summary, the AI run / evidence refs, the incident / docs links, and the relation-source class each
//! artifact carries) and the *linked-change-relation* grammar (the relation state a linked change sits in — linked by
//! provider, linked locally, suggested by Aureline, a stale relation, a broken relation, or queued for publish) into
//! registry resolvers that produce export-safe, honest projections. Every tracked item then resolves to one typed
//! linked-change panel — the artifacts it publishes, their relation source, and their freshness, all attributable to
//! the work item so a locally recorded or Aureline-suggested link never reads as a provider-authoritative one — and to
//! one linked-change-relation object — the resolved link identity, the affected relation field, the
//! previous-versus-current relation-state reference, the binding state, and the active relation-source reason (linked
//! by provider, linked locally, suggested by Aureline, stale or broken relation, or queued for publish) — that the
//! work-item, review, Git / worktree, provider-handoff, and support surfaces can inspect without manual reconstruction,
//! so a stale or broken relation stays visible and actionable as a typed relation instead of collapsing into missing
//! context, and a panel that cannot bind an artifact to its relation source degrades honestly instead of flattening
//! the four relation sources into one generic badge.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable linked-change panel per tracked work item, carrying every linked-artifact field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound panel entry
//!   unless it names a canonical registry token, a classified [artifact selector][M5LinkedChangePanelKind],
//!   a disclosure role, covers every [resolution form][M5LinkedChangePanelResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every panel field (branch / worktree
//!   state, hosted review state, validation summary, AI run / evidence refs, incident / docs links, and the
//!   relation-source class), keeps its relation lineage attributable to the tracked item, and keeps a
//!   provider-facing field bound to its relation-source state; otherwise it degrades.
//! * **Keep relation source and freshness explicit rather than a hand-authored parallel prose staying consistent.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a panel entry whose
//!   relation-source / freshness join is not preserved (a locally linked or suggested relation presenting itself as
//!   provider-authoritative without one) so it degrades to
//!   [`M5LinkedChangePanelEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a provider-facing relation field whose language outruns its relation source degrades the same
//!   way — the structured blocker reason a locally-linked-as-provider attempt must surface.
//! * **Turn a stale relation, a broken relation, or a queued-for-publish link into a visible, typed relation event
//!   instead of missing context.** [`resolve_review_pack_result_entry`] names a classified
//!   [relation binding][M5LinkedChangeRelationScope]
//!   (linked-source binding, relation-freshness binding, or relation-target binding), requires the full
//!   link-identity / affected-relation-field / previous-versus-current-relation-state / binding / active-reason
//!   relation object, covers every resolution form, and degrades to
//!   [`M5LinkedChangeRelationEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the relation would present a stale link as current, hide the broken relation, or let a relation gap
//!   masquerade as full coverage, so a linked-change relation can never read as trustworthy when it has quietly
//!   dropped the reason the link could not claim provider-authoritative coverage.
//!
...
//! review detail, Git / worktree, linked-change panel, provider handoff, and support / export surfaces
//! can never fork their own linked-change meaning. Raw secret values and private endpoints stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_linked_change_panel_registries,
    seeded_m5_linked_change_panel_registries_review_pack_record_beta_narrowed,
    seeded_m5_linked_change_panel_registries_review_pack_result_preview_narrowed,
    M5_LINKED_CHANGE_PANEL_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5LinkedChangePanelRegistriesPacket`].
pub const M5_LINKED_CHANGE_PANEL_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_linked_change_panel_and_relation_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_LINKED_CHANGE_PANEL_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_LINKED_CHANGE_PANEL_REGISTRIES_SCHEMA_REF: &str =
    "schemas/teamwork/m5-linked-change-panel-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_LINKED_CHANGE_PANEL_REGISTRIES_DOC_REF: &str =
    "docs/team-workflows/m5_linked_change_panel_registries.md";
...
pub const M5_LINKED_CHANGE_PANEL_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-linked-change-panel-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_LINKED_CHANGE_PANEL_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-linked-change-panel-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_LINKED_CHANGE_PANEL_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-linked-change-panel-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_LINKED_CHANGE_PANEL_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/teamwork/m5-linked-change-panel-registries";
...
pub type M5LinkedChangePanelRegistriesConsumerSurface = M5ChangeIntentConsumerSurface;
...
pub enum M5LinkedChangePanelResolutionForm {
...
impl M5LinkedChangePanelResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LinkedChangePanelKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LinkedChangeRelationScope {
...
impl M5LinkedChangeRelationScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LinkedChangePanelSurfaceContext {
...
impl M5LinkedChangePanelSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LinkedChangePanelAnatomyPart {
...
impl M5LinkedChangePanelAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LinkedChangePanelNextAction {
...
impl M5LinkedChangePanelNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LinkedChangePanelExportField {
...
impl M5LinkedChangePanelExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LinkedChangePanelEntryDegradeReason {
...
impl M5LinkedChangePanelEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5LinkedChangePanelNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5LinkedChangePanelNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5LinkedChangePanelNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5LinkedChangePanelNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5LinkedChangePanelNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5LinkedChangeRelationEntryDegradeReason {
...
impl M5LinkedChangeRelationEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5LinkedChangePanelNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5LinkedChangePanelNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5LinkedChangePanelNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5LinkedChangePanelNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5LinkedChangePanelEntryResolutionInput {
    ...
    pub report_section: M5LinkedChangePanelKind,
    /// The render / surface context.
    pub surface_context: M5LinkedChangePanelSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5LinkedChangePanelResolutionForm>,
...
pub struct M5ResolvedLinkedChangePanelEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5LinkedChangePanelEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5LinkedChangePanelNextAction,
...
impl M5ResolvedLinkedChangePanelEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5LinkedChangeRelationEntryResolutionInput {
    ...
    pub comparison_scope: M5LinkedChangeRelationScope,
    /// The render / surface context.
    pub surface_context: M5LinkedChangePanelSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5LinkedChangePanelResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedLinkedChangeRelationEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5LinkedChangeRelationEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5LinkedChangePanelNextAction,
...
impl M5ResolvedLinkedChangeRelationEntry {
...
pub enum M5LinkedChangePanelResolutionError {
...
impl M5LinkedChangePanelResolutionError {
...
impl fmt::Display for M5LinkedChangePanelResolutionError {
...
impl Error for M5LinkedChangePanelResolutionError {}

fn form_tokens(forms: &[M5LinkedChangePanelResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5LinkedChangePanelResolutionForm]) -> bool {
    let present: BTreeSet<M5LinkedChangePanelResolutionForm> = forms.iter().copied().collect();
    M5LinkedChangePanelResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5LinkedChangePanelKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5LinkedChangeRelationScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedLinkedChangePanelEntry, M5LinkedChangePanelResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5LinkedChangePanelResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5LinkedChangePanelResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5LinkedChangePanelEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5LinkedChangePanelEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5LinkedChangePanelEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5LinkedChangePanelEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5LinkedChangePanelEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5LinkedChangePanelEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5LinkedChangePanelEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(
            M5LinkedChangePanelEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    } else if !input.proof_fresh {
        Some(M5LinkedChangePanelEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5LinkedChangePanelNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedLinkedChangePanelEntry {
...
pub fn resolve_review_pack_result_entry(
...
) -> Result<M5ResolvedLinkedChangeRelationEntry, M5LinkedChangePanelResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5LinkedChangePanelResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5LinkedChangeRelationEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5LinkedChangeRelationEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5LinkedChangeRelationEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5LinkedChangeRelationEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5LinkedChangeRelationEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5LinkedChangeRelationEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5LinkedChangePanelNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedLinkedChangeRelationEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LinkedChangePanelRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5LinkedChangePanelRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5LinkedChangePanelAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5LinkedChangePanelExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5ChangeIntentDowngradeTrigger>,
    /// Resolved line-review_pack_record-registry examples.
    pub review_pack_record_entries: Vec<M5ResolvedLinkedChangePanelEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedLinkedChangeRelationEntry>,
...
impl M5LinkedChangePanelRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5LinkedChangePanelAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5LinkedChangePanelAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5LinkedChangePanelExportField> =
            self.export_fields.iter().copied().collect();
        M5LinkedChangePanelExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedLinkedChangePanelEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedLinkedChangeRelationEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LinkedChangePanelRegistriesVocabularySet {
...
impl M5LinkedChangePanelRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ChangeIntentRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5LinkedChangePanelResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5LinkedChangePanelKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5LinkedChangeRelationScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5LinkedChangePanelSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5LinkedChangePanelEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5LinkedChangeRelationEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5LinkedChangePanelAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5LinkedChangePanelNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5LinkedChangePanelExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LinkedChangePanelRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LinkedChangePanelRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LinkedChangePanelRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LinkedChangePanelRegistriesReleasePosture {
...
/// Constructor input for [`M5LinkedChangePanelRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5LinkedChangePanelRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5LinkedChangePanelRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5LinkedChangePanelRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5LinkedChangePanelRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5LinkedChangePanelRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5LinkedChangePanelRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5LinkedChangePanelRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5LinkedChangePanelRegistriesPacket {
    /// Record kind; must equal [`M5_LINKED_CHANGE_PANEL_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_LINKED_CHANGE_PANEL_REGISTRIES_SCHEMA_VERSION`].
...
impl M5LinkedChangePanelRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5LinkedChangePanelRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_LINKED_CHANGE_PANEL_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_LINKED_CHANGE_PANEL_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5LinkedChangePanelRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_LINKED_CHANGE_PANEL_REGISTRIES_RECORD_KIND {
            violations.push(M5LinkedChangePanelRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_LINKED_CHANGE_PANEL_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5LinkedChangePanelRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5LinkedChangePanelRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5LinkedChangePanelRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5LinkedChangePanelRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Linked-Change-Panel and Linked-Change-Relation Registries\n\n");
...
#[derive(Debug)]
pub enum M5LinkedChangePanelRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5LinkedChangePanelRegistriesViolation>),
}

impl fmt::Display for M5LinkedChangePanelRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5LinkedChangePanelRegistriesArtifactError {}

/// Validation failures emitted by [`M5LinkedChangePanelRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5LinkedChangePanelRegistriesViolation {
...
impl M5LinkedChangePanelRegistriesViolation {
...
pub fn current_stable_m5_linked_change_panel_registries_export(
) -> Result<M5LinkedChangePanelRegistriesPacket, M5LinkedChangePanelRegistriesArtifactError> {
    let packet: M5LinkedChangePanelRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../artifacts/release/m5-linked-change-panel-registries-proof/support_export.json"
    )))
    .map_err(M5LinkedChangePanelRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5LinkedChangePanelRegistriesArtifactError::Validation(
            violations,
        ))
    }
}
...
fn validate_source_contracts(
    packet: &M5LinkedChangePanelRegistriesPacket,
    violations: &mut Vec<M5LinkedChangePanelRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(M5LinkedChangePanelRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5LinkedChangePanelRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(M5LinkedChangePanelRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(M5LinkedChangePanelRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(M5LinkedChangePanelRegistriesViolation::MandatoryExportFieldMissing);
        }
        ...
        if !refs.contains(M5_CHANGE_INTENT_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_LINKED_CHANGE_PANEL_DOMAIN_SCHEMA_REF)
        {
            violations.push(M5LinkedChangePanelRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5LinkedChangePanelRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5LinkedChangePanelRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5LinkedChangePanelRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(M5LinkedChangePanelRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(M5LinkedChangePanelRegistriesViolation::ConsumerProjectionIncomplete);
            return;
        }
    }
}
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5LinkedChangePanelRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5LinkedChangePanelRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5LinkedChangePanelKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5LinkedChangePanelSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations
            .push(M5LinkedChangePanelRegistriesViolation::CohortDescriptorResolutionNotProven);
    }
    ...
    {
        violations.push(
            M5LinkedChangePanelRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5LinkedChangeRelationScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(M5LinkedChangePanelRegistriesViolation::CohortEvidenceIntegrityNotProven);
    }
}
