//! Tests for the M05-1283 review-pack surface certification capstone.

use super::*;

fn packet() -> ReviewPackProfileCertificationPacket {
    seeded_m5_review_pack_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, REVIEW_PACK_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, REVIEW_PACK_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5ReviewPackCertifiedProfile::ALL {
...
fn every_frozen_family_is_certified_on_some_profile() {
    ...
    for family in M5ReviewPackObject::ALL {
        assert!(
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            ReviewPackCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, ReviewPackAxisCertificationState::Certified);
...
fn a_narrowed_profile_never_keeps_a_trusted_claim() {
    for row in &packet().rows {
        if row.is_claim_narrowed() {
            assert!(
                !row.certified_claim.asserts_certified_review_pack_truth(),
...
fn all_eight_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5ReviewPackCertClaim> = p.rows.iter().map(|r| r.certified_claim).collect();
    for claim in M5ReviewPackCertClaim::ALL {
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5ReviewPackCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5ReviewPackCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = ReviewPackCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), ReviewPackCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in ReviewPackCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == ReviewPackCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    // The localization axis is present, distinct, and exercised by the undisclosed-AI-pack-binding profile as its
    // binding narrowed axis.
    let p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5ReviewPackCertifiedProfile::UndisclosedAiPackBindingProfile)
        .expect("undisclosed-ai-pack-binding-profile row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(ReviewPackCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(ReviewPackCertificationAxis::Localization)
    );
}
...
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    ...
    for row in &packet().rows {
        let outcome = row
            .axis(ReviewPackCertificationAxis::HighContrast)
            .expect("high-contrast axis present");
        assert_eq!(outcome.state, ReviewPackAxisCertificationState::Certified);
    }
}
...
fn the_six_yellow_rows_bind_six_distinct_axes_and_triggers() {
    ...
    // per-lane concern (stale pack / owner provenance / unevaluated check / local parity / AI pack binding /
    // template attribution).
    let p = packet();
    let axes: BTreeSet<ReviewPackCertificationAxis> = p
        .rows
        .iter()
        .filter_map(|r| r.claim_auto_narrow.as_ref().map(|n| n.binding_axis))
        .collect();
    assert_eq!(axes.len(), 6, "yellow rows must bind six distinct axes");
    let triggers: std::collections::HashSet<M5ReviewPackDowngradeTrigger> = p
        .rows
        .iter()
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5ReviewPackCertifiedProfile::CertifiedReviewPackLane)
        .expect("certified-review-pack-lane row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ReviewPackCertificationAxis::Visual {
            outcome.state = ReviewPackAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("pack label silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, ReviewPackProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, ReviewPackCertificationViolation::ProfileBlocked { .. })));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5ReviewPackCertifiedProfile::ReviewableReviewPackRecordStructure
        })
        .expect("reviewable-review-pack-record-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails
        .lets_a_local_parity_estimate_masquerade_as_provider_authoritative = true;
    assert_eq!(row.derive_status(), ReviewPackProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ReviewPackCertificationViolation::GuardrailViolated { .. }
    )));
}
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5ReviewPackCertClaim::CertifiedReviewPackTruth;
    row.certified_claim = M5ReviewPackCertClaim::CertifiedReviewPackTruth;
    assert_eq!(row.derive_status(), ReviewPackProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ReviewPackCertificationAxis::DegradedState {
            *outcome = ReviewPackAxisOutcome {
                axis: ReviewPackCertificationAxis::DegradedState,
                ...
                downgrade_trigger: Some(M5ReviewPackDowngradeTrigger::PackVersionOrDigestDropped),
            };
        }
    }
    // Claim stays ReviewableReviewPackRecord == certified, no claim_auto_narrow.
    assert_eq!(row.derive_status(), ReviewPackProfileClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == ReviewPackCertificationAxis::CliExport {
            outcome.state = ReviewPackAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger =
                Some(M5ReviewPackDowngradeTrigger::PackVersionOrDigestDropped);
        }
    }
    assert_eq!(row.derive_status(), ReviewPackProfileClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(ReviewPackClaimAutoNarrow {
        binding_axis: ReviewPackCertificationAxis::DegradedState,
        from_claim: M5ReviewPackCertClaim::CertifiedReviewPackTruth,
        to_claim: M5ReviewPackCertClaim::CertifiedReviewPackTruth,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5ReviewPackCertClaim::PackVersionDigestUnverifiedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-review-pack-record-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableReviewPackRecord; certify a stronger certified operating line.
    row.certified_claim = M5ReviewPackCertClaim::CertifiedReviewPackTruth;
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5ReviewPackCertifiedProfile::StalePackVersionDigestProfile)
        .expect("stale-pack-version-digest-profile row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = ReviewPackCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), ReviewPackProfileClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = ReviewPackCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), ReviewPackProfileClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == ReviewPackProfileClaimStatus::Yellow)
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(ReviewPackCertificationAxis::Visual);
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.profile != M5ReviewPackCertifiedProfile::LocalOnlyParityProfile);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ReviewPackCertificationViolation::ProfileCoverageIncomplete
    )));
}
...
fn missing_family_coverage_is_rejected() {
    ...
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5ReviewPackObject::OwnershipSignal);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ReviewPackCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = ReviewPackProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ReviewPackCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ReviewPackCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ReviewPackCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != ReviewPackCertificationAxis::ReviewPackTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        ReviewPackCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, ReviewPackCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        ReviewPackCertificationViolation::RawReviewPackMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: ReviewPackProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk = current_m5_review_pack_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-review-pack-surface-certification/support_export.json"
    ));
    let packet_from_fixture: ReviewPackProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-review-pack-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_REVIEW_PACK_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures from the
// seed builder. Off by default so a normal `cargo test` never mutates the tree.
#[test]
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_REVIEW_PACK_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_review_pack_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/review/m5-review-pack-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/review/m5-review-pack-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/review/m5-review-pack-surface-certification");
