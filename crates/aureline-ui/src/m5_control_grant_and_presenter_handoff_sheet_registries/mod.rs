//! Implemented M5 control-grant and presenter-handoff-sheet registries.
//!
//! The frozen [collaboration-control component matrix][matrix] names Aureline's governed sensitive-collaboration
//! object classes and locks their controlled vocabulary. This module makes the explicit control-grant sheet
//! operable — as a canonical record — and adds the presenter-handoff sheet that names whether write control is
//! unavailable, requestable, granted to a single driver, or expired, so the desktop shared-terminal / debug surface,
//! the companion-follow flow, and the support / export and help / docs surfaces inherit one canonical control-grant
//! descriptor rather than a hand-authored parallel prose that has to be kept consistent: it turns the
//! *control-grant* grammar (one durable control-grant record per sensitive session — one typed field per
//! record section: the requester, issuer, and accepter identities, the granted scope and target context, the
//! time-box and expiry, the revoke path, and the single-active-driver binding kept distinct from ordinary presence
//! and follow state — each bound to one session / target scope) and the *presenter-handoff* grammar (the sheet a
//! moderator reads before assuming any authority, and the fresh visible authority event any change to an
//! already-active session must raise — a request, a grant to a single driver, a deny, a revoke, an expiry, or a
//! presenter handoff along the handoff chain that must not silently transfer shell / debugger control) into registry
//! resolvers that produce export-safe, honest projections. Every sensitive session then resolves to one typed
//! control-grant sheet — its requester / issuer / accepter, its granted scope and target context, its time-box and
//! revoke path, all attributable to the session so presence never reads as terminal / debug control — and to one
//! presenter-handoff object — the resolved session identity, the disclosed scope and target, the available
//! authority, and the active grant / expiry gate — that the shared terminal / debug, companion, and support surfaces
//! can inspect without manual reconstruction, so no two participants ever hold mutating control of the same surface
//! and the control-grant history stays visible and exportable, and a control grant that cannot bind a field to its
//! session / target scope degrades honestly instead of letting presence read as control-capable when it is not.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable control-grant sheet per session, carrying every record field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound record entry
//!   unless it names a canonical registry token, a classified [scope selector][M5ControlGrantRecordKind],
//!   a disclosure role, covers every [resolution form][M5ControlGrantRecordResolutionForm] (the
//!   canonical object, the accessible summary, and the audit record), publishes every record field (granted scope
//!   and target context, requester / issuer / accepter, time-box and revoke path, and the single-active-driver
//!   binding bound to the session identity), keeps its grant lineage attributable to the session, and
//!   keeps a single-active-driver field bound to its authority state; otherwise it degrades.
//! * **Keep the presenter-handoff sheet honest so a moderator handoff never transfers write control rather than a
//!   hand-authored parallel prose staying consistent.** [`line_preserves_rollback_and_diagnostics_before_widening`]
//!   rejects a control grant whose single-active-driver binding is not preserved (a session presenting itself as
//!   control-capable from presence alone) so it degrades to
//!   [`M5ControlGrantRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a presenter handoff whose language outruns its authority state degrades the same
//!   way — the structured blocker reason a control-from-presence attempt must surface.
//! * **Turn a request, a grant, a revoke, an expiry, or a presenter handoff into a visible, typed authority event
//!   instead of a silent upgrade.** [`resolve_review_pack_result_entry`] names a classified
//!   [authority binding][M5ControlGrantResultScope]
//!   (request binding, grant-to-single-driver binding, or expiry binding), requires the full
//!   session-identity / affected-authority-field / previous-versus-current-authority-state / binding / active-reason
//!   authority object, covers every resolution form, and degrades to
//!   [`M5ControlGrantResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the change would present an expired grant as current, hide a second driver on a sensitive surface, or let a
//!   presenter handoff masquerade as a write-control transfer, so a session change can never read as trustworthy when
...
//! can never fork their own control-grant meaning. Raw secrets, raw command text, and clipboard contents stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_control_grant_and_presenter_handoff_sheet_registries,
    seeded_m5_control_grant_and_presenter_handoff_sheet_registries_review_pack_record_beta_narrowed,
    seeded_m5_control_grant_and_presenter_handoff_sheet_registries_review_pack_result_preview_narrowed,
    M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_PACKET_ID,
};
...
use crate::m5_shared_terminal_debug_control_grant_presenter_consent_retention_and_session_restore_view_matrix::{
    ...
    M5_CONTROL_GRANT_DOMAIN_SCHEMA_REF, M5_COLLABORATION_CONTROL_MATRIX_DOC_REF,
    M5_COLLABORATION_CONTROL_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5ControlGrantRecordAndResultRegistriesPacket`].
pub const M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_control_grant_and_presenter_handoff_sheet_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_SCHEMA_REF: &str =
...
pub const M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_DOC_REF: &str =
    "docs/collaboration/m5_control_grant_and_presenter_handoff_sheet_registries.md";

/// Repo-relative path of the presenter-handoff-sheet domain schema this lane binds its B object to (the
/// machine-readable presenter / moderator token, its holder, and its handoff chain, whose authority state the
/// sheet projects — unavailable, requestable, granted to a single driver, or expired — kept distinct from shell /
/// debugger write control). Minted by the frozen collaboration-control matrix; reused here so the presenter-handoff
/// sheet and the control-grant flow never fork their authority vocabulary.
pub const M5_PRESENTER_HANDOFF_SHEET_DOMAIN_SCHEMA_REF: &str =
    "schemas/ui/m5-presenter-handoff-sheet.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-control-grant-and-presenter-handoff-sheet-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-control-grant-and-presenter-handoff-sheet-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-control-grant-and-presenter-handoff-sheet-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/collaboration/m5-control-grant-and-presenter-handoff-sheet-registries";
...
pub type M5ControlGrantRecordAndResultRegistriesConsumerSurface =
...
pub enum M5ControlGrantRecordResolutionForm {
...
impl M5ControlGrantRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ControlGrantRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ControlGrantResultScope {
...
impl M5ControlGrantResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ControlGrantRecordSurfaceContext {
...
impl M5ControlGrantRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ControlGrantRecordAnatomyPart {
...
impl M5ControlGrantRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ControlGrantRecordNextAction {
...
impl M5ControlGrantRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ControlGrantRecordExportField {
...
impl M5ControlGrantRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ControlGrantRecordEntryDegradeReason {
...
impl M5ControlGrantRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5ControlGrantRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5ControlGrantRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5ControlGrantRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5ControlGrantRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5ControlGrantRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ControlGrantResultEntryDegradeReason {
...
impl M5ControlGrantResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5ControlGrantRecordNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5ControlGrantRecordNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5ControlGrantRecordNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5ControlGrantRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5ControlGrantRecordEntryResolutionInput {
    ...
    pub report_section: M5ControlGrantRecordKind,
    /// The render / surface context.
    pub surface_context: M5ControlGrantRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ControlGrantRecordResolutionForm>,
...
pub struct M5ResolvedControlGrantRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ControlGrantRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ControlGrantRecordNextAction,
...
impl M5ResolvedControlGrantRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ControlGrantResultEntryResolutionInput {
    ...
    pub comparison_scope: M5ControlGrantResultScope,
    /// The render / surface context.
    pub surface_context: M5ControlGrantRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ControlGrantRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedControlGrantResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ControlGrantResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ControlGrantRecordNextAction,
...
impl M5ResolvedControlGrantResultEntry {
...
pub enum M5ControlGrantRecordResolutionError {
...
impl M5ControlGrantRecordResolutionError {
...
impl fmt::Display for M5ControlGrantRecordResolutionError {
...
impl Error for M5ControlGrantRecordResolutionError {}

fn form_tokens(forms: &[M5ControlGrantRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5ControlGrantRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5ControlGrantRecordResolutionForm> = forms.iter().copied().collect();
    M5ControlGrantRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5ControlGrantRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5ControlGrantResultScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedControlGrantRecordEntry, M5ControlGrantRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ControlGrantRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5ControlGrantRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ControlGrantRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ControlGrantRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5ControlGrantRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5ControlGrantRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5ControlGrantRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5ControlGrantRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5ControlGrantRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5ControlGrantRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5ControlGrantRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5ControlGrantRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedControlGrantRecordEntry {
...
pub fn resolve_review_pack_result_entry(
    input: M5ControlGrantResultEntryResolutionInput,
) -> Result<M5ResolvedControlGrantResultEntry, M5ControlGrantRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ControlGrantRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ControlGrantResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ControlGrantResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5ControlGrantResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5ControlGrantResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5ControlGrantResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5ControlGrantResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5ControlGrantRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedControlGrantResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ControlGrantRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5ControlGrantRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5ControlGrantRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5ControlGrantRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedControlGrantRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedControlGrantResultEntry>,
...
impl M5ControlGrantRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5ControlGrantRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5ControlGrantRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5ControlGrantRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5ControlGrantRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedControlGrantRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedControlGrantResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ControlGrantRecordAndResultRegistriesVocabularySet {
...
impl M5ControlGrantRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5CollaborationControlRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5ControlGrantRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5ControlGrantRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ControlGrantResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5ControlGrantRecordSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5ControlGrantRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5ControlGrantResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5ControlGrantRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5ControlGrantRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5ControlGrantRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ControlGrantRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ControlGrantRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ControlGrantRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ControlGrantRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5ControlGrantRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ControlGrantRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5ControlGrantRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5ControlGrantRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5ControlGrantRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5ControlGrantRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5ControlGrantRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5ControlGrantRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ControlGrantRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_SCHEMA_VERSION`].
...
impl M5ControlGrantRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5ControlGrantRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_RECORD_KIND
                .to_owned(),
            schema_version: M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5ControlGrantRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_RECORD_KIND {
            violations.push(M5ControlGrantRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version
            != M5_CONTROL_GRANT_AND_PRESENTER_HANDOFF_SHEET_REGISTRIES_SCHEMA_VERSION
        {
            violations.push(M5ControlGrantRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5ControlGrantRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5ControlGrantRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5ControlGrantRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Control-Grant and Presenter-Handoff-Sheet Registries\n\n");
...
#[derive(Debug)]
pub enum M5ControlGrantRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5ControlGrantRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5ControlGrantRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5ControlGrantRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5ControlGrantRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5ControlGrantRecordAndResultRegistriesViolation {
...
impl M5ControlGrantRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_control_grant_and_presenter_handoff_sheet_registries_export() -> Result<
    M5ControlGrantRecordAndResultRegistriesPacket,
    M5ControlGrantRecordAndResultRegistriesArtifactError,
> {
    let packet: M5ControlGrantRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-control-grant-and-presenter-handoff-sheet-registries-proof/support_export.json"
        )
    ))
    .map_err(M5ControlGrantRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5ControlGrantRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5ControlGrantRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5ControlGrantRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations
                .push(M5ControlGrantRecordAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5ControlGrantRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations
                .push(M5ControlGrantRecordAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5ControlGrantRecordAndResultRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5ControlGrantRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_CONTROL_GRANT_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_PRESENTER_HANDOFF_SHEET_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5ControlGrantRecordAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5ControlGrantRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5ControlGrantRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5ControlGrantRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5ControlGrantRecordAndResultRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5ControlGrantRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5ControlGrantRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5ControlGrantRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5ControlGrantRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5ControlGrantRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5ControlGrantRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5ControlGrantRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ControlGrantResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(
            M5ControlGrantRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
