//! Tests for the M05-1273 AI-review-assist surface certification capstone.

use super::*;

fn packet() -> AiReviewProfileCertificationPacket {
    seeded_m5_ai_review_assist_surface_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, AI_REVIEW_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, AI_REVIEW_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_profile_is_certified_exactly_once() {
    ...
    for profile in M5AiReviewCertifiedProfile::ALL {
...
fn every_frozen_family_is_certified_on_some_profile() {
    ...
    for family in M5AiReviewAssistObject::ALL {
        assert!(
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            AiReviewCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, AiReviewAxisCertificationState::Certified);
...
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, AI_REVIEW_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(
...
fn a_narrowed_profile_never_keeps_a_trusted_claim() {
    for row in &packet().rows {
        if row.is_claim_narrowed() {
            assert!(
                !row.certified_claim.asserts_certified_ai_review_truth(),
...
fn all_six_claim_tiers_appear_as_certified_claims() {
    let p = packet();
    let tiers: BTreeSet<M5AiReviewCertClaim> = p.rows.iter().map(|r| r.certified_claim).collect();
    for claim in M5AiReviewCertClaim::ALL {
...
fn profile_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5AiReviewCertifiedProfile::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5AiReviewCertifiedProfile::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = AiReviewCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), AiReviewCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in AiReviewCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == AiReviewCertificationAxis::CliExport
        );
...
fn localization_axis_is_a_distinct_reach_axis() {
    // The B151 localization axis is present, distinct, and exercised by the
    // unverified-publish-target profile as its binding narrowed axis.
    let p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5AiReviewCertifiedProfile::UnverifiedPublishTargetProfile)
        .expect("unverified-publish-target-profile row exists");
    let row = &p.rows[idx];
    let outcome = row
        .axis(AiReviewCertificationAxis::Localization)
    ...
    assert_eq!(
        ...
        Some(AiReviewCertificationAxis::Localization)
    );
}
...
fn high_contrast_axis_is_present_and_certified_on_every_row() {
    ...
    for row in &packet().rows {
        let outcome = row
            .axis(AiReviewCertificationAxis::HighContrast)
            .expect("high-contrast axis present");
        assert_eq!(outcome.state, AiReviewAxisCertificationState::Certified);
    }
}
...
fn the_four_yellow_rows_bind_four_distinct_axes_and_triggers() {
    ...
    // per-lane concern (provider freshness / diff-scope drift / publish-target availability / finding lifecycle).
    let p = packet();
    let axes: BTreeSet<AiReviewCertificationAxis> = p
        .rows
        .iter()
    ...
    let triggers: std::collections::HashSet<M5AiReviewAssistDowngradeTrigger> = p
        .rows
        .iter()
...
fn undisclosed_drift_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5AiReviewCertifiedProfile::CertifiedAiReviewLane)
        .expect("certified-ai-review-lane row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == AiReviewCertificationAxis::Visual {
            outcome.state = AiReviewAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("snapshot label silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, AiReviewProfileClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, AiReviewCertificationViolation::ProfileBlocked { .. })));
}
...
fn guardrail_breach_blocks_the_profile() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.profile == M5AiReviewCertifiedProfile::ReviewableAiReviewRecordStructure)
        .expect("reviewable-snapshot-record-structure row exists");
    let row = &mut p.rows[idx];
    row.guardrails
        .lets_ai_review_results_publish_or_merge_implicitly = true;
    assert_eq!(row.derive_status(), AiReviewProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, AiReviewCertificationViolation::GuardrailViolated { .. })));
}
...
fn non_live_profile_claiming_trusted_surface_blocks() {
    ...
    row.claimed_claim = M5AiReviewCertClaim::CertifiedAiReviewTruth;
    row.certified_claim = M5AiReviewCertClaim::CertifiedAiReviewTruth;
    assert_eq!(row.derive_status(), AiReviewProfileClaimStatus::Red);
    row.derived_status = row.derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == AiReviewCertificationAxis::DegradedState {
            *outcome = AiReviewAxisOutcome {
                axis: AiReviewCertificationAxis::DegradedState,
    ...
    // Claim stays ReviewableAiReviewRecord == certified, no claim_auto_narrow.
    assert_eq!(row.derive_status(), AiReviewProfileClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_profile() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == AiReviewCertificationAxis::CliExport {
            outcome.state = AiReviewAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this profile".to_owned());
            outcome.downgrade_trigger =
                Some(M5AiReviewAssistDowngradeTrigger::StaleFindingShownAsCurrent);
        }
    }
    assert_eq!(row.derive_status(), AiReviewProfileClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(AiReviewClaimAutoNarrow {
        binding_axis: AiReviewCertificationAxis::DegradedState,
        from_claim: M5AiReviewCertClaim::CertifiedAiReviewTruth,
        to_claim: M5AiReviewCertClaim::CertifiedAiReviewTruth,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5AiReviewCertClaim::ProviderFreshnessDisclosedProjection;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        ...
        .expect("reviewable-snapshot-record-structure row exists");
    let row = &mut p.rows[idx];
    // claimed is ReviewableAiReviewRecord; certify a stronger certified operating line.
    row.certified_claim = M5AiReviewCertClaim::CertifiedAiReviewTruth;
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        AiReviewCertificationViolation::CertifiedClaimExceedsClaim { .. }
    )));
}
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        ...
        .iter()
        .position(|r| {
            r.profile == M5AiReviewCertifiedProfile::DisclosedProviderFreshnessPartialProfile
        })
        .expect("disclosed-imported-offline-partial-profile row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = AiReviewCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), AiReviewProfileClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = AiReviewCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), AiReviewProfileClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == AiReviewProfileClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "outdated".to_owned();
    }
    assert_eq!(row.derive_status(), AiReviewProfileClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(AiReviewCertificationAxis::Visual);
...
fn missing_profile_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.profile != M5AiReviewCertifiedProfile::UnverifiedPublishTargetProfile);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, AiReviewCertificationViolation::ProfileCoverageIncomplete)));
}
...
fn missing_family_coverage_is_rejected() {
    ...
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5AiReviewAssistObject::ReviewScopeSelector);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, AiReviewCertificationViolation::FamilyCoverageIncomplete)));
}

#[test]
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = AiReviewProfileClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        AiReviewCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        AiReviewCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, AiReviewCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, AiReviewCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != AiReviewCertificationAxis::AiReviewTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        AiReviewCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, AiReviewCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        AiReviewCertificationViolation::RawAiReviewMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: AiReviewProfileCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_ai_review_assist_surface_certification_export().expect("export is valid");
    assert_eq!(
...
fn checked_fixtures_mirror_the_release_artifacts() {
    let fixture_export = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-ai-review-assist-surface-certification/support_export.json"
    ));
    let packet_from_fixture: AiReviewProfileCertificationPacket =
    ...
    let fixture_csv = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/review/m5-ai-review-assist-surface-certification/matrix.csv"
    ));
...
//
// Set `GEN_AI_REVIEW_ASSIST_CERT_ARTIFACTS=1` to (re)write the checked-in release artifacts and fixtures
...
fn regenerate_checked_artifacts_when_requested() {
    if std::env::var("GEN_AI_REVIEW_ASSIST_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_ai_review_assist_surface_certification_packet();
    assert!(
    ...
    let release_dir = repo.join("artifacts/review/m5-ai-review-assist-surface-certification");
    ...
    fs::write(
        repo.join("artifacts/review/m5-ai-review-assist-surface-certification.md"),
        &md,
    )
    .expect("write release report");

    let fixture_dir = repo.join("fixtures/review/m5-ai-review-assist-surface-certification");
