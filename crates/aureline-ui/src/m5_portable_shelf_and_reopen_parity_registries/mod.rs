//! Implemented M5 portable-shelf and reopen-parity registries.
//!
//! The frozen [change-object / patch-stack / landing matrix][matrix] names Aureline's governed change-orchestration
//! object classes and locks their controlled vocabulary. This module makes the matrix's `portable_shelf`
//! object class operable — as a portable shelf / bundle form for a change object or landing candidate and the
//! reopen-parity posture that keeps it honest when reopened — so the Git, patch-stack / queue, review,
//! provider-landing, help / docs, and support / export surfaces inherit one canonical portable-shelf descriptor
//! rather than a hand-authored parallel prose that has to be kept consistent: it turns the *portable shelf / bundle*
//! grammar (one durable shelf per change object or landing candidate — one typed field per section: the bundle ID,
//! the diff refs, the evidence refs, the review-pack version, the redaction profile, and the import / reopen status,
//! keeping the exported-versus-imported-versus-reopened distinction visible so stacked work survives browser handoff,
//! offline follow-up, support escalation, incident bridge, and review export without one specific code host) and the
//! *reopen parity* grammar (the browser-companion / support / incident / offline handoff parity kept separate so an
//! imported shelf stays blocked from reading as a live provider object for background agents and broad automation
//! unless the user explicitly reopens and confirms it, naming the handoff channel, the reopen state, the reopened
//! truth posture — local-only, provider-linked, stale, or redacted — the reopening actor, and whether the diff /
//! evidence identity is preserved before reopen) into registry resolvers that produce export-safe, honest
//! projections. Every change object or landing candidate then resolves to one typed portable shelf — the bundle ID
//! and diff / evidence refs it discloses, the review-pack version, the redaction profile, and the import / reopen
//! status, all attributable to the shelf so a user can export and later reopen it without losing diff / evidence
//! identity — and to one reopen-parity object — the handoff channel it travelled, the reopened truth it discloses,
//! and the diff / evidence identity it preserves before reopen — that the Git, patch-stack, provider-landing, and
//! support surfaces can inspect without manual reconstruction, so a shelf's bundle, diff / evidence identity, and
//! reopen state stay visible metadata, no member is silently reordered, and nothing lands from an overclaimed
//! imported shelf; a portable shelf or reopen parity that cannot bind a field to its real change object degrades
//! honestly instead of letting an imported shelf overclaim current hosted truth.
//!
...
//!   unless it names a canonical registry token, a classified [scope selector][M5PortableShelfRecordKind],
//!   a disclosure role, covers every [resolution form][M5PortableShelfRecordResolutionForm] (the
...
//!   [`M5PortableShelfRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
...
//!   [evaluator binding][M5PortableShelfResultScope]
...
//!   [`M5PortableShelfResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
...
pub use seed::{
    seeded_m5_portable_shelf_and_reopen_parity_registries,
    seeded_m5_portable_shelf_and_reopen_parity_registries_review_pack_record_beta_narrowed,
    seeded_m5_portable_shelf_and_reopen_parity_registries_review_pack_result_preview_narrowed,
    M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5PortableShelfRecordAndResultRegistriesPacket`].
pub const M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_portable_shelf_and_reopen_parity_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_SCHEMA_REF: &str =
    "schemas/teamwork/m5-portable-shelf-and-reopen-parity-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_DOC_REF: &str =
    "docs/team-workflows/m5_portable_shelf_and_reopen_parity_registries.md";

/// Repo-relative path of the canonical portable-shelf-reopen-parity domain schema minted by this lane (the
/// reopen-parity posture that names before any reopen: the handoff channel (browser companion, support
/// escalation, incident bridge, offline follow-up, review export), the reopen state (exported, imported,
/// reopened local, reopened provider-linked, stale, redacted), the reopened truth posture (local-only,
/// provider-linked, stale provider link, redacted profile), the reopening actor, and whether the diff /
/// evidence identity is preserved — so an imported shelf stays blocked from reading as a live provider object
/// for background agents and broad automation unless the user explicitly reopens and confirms it, and nothing
/// overclaims current hosted truth).
pub const M5_PORTABLE_SHELF_REOPEN_PARITY_DOMAIN_SCHEMA_REF: &str =
    "schemas/change/m5-portable-shelf-reopen-parity.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-portable-shelf-and-reopen-parity-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-portable-shelf-and-reopen-parity-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-portable-shelf-and-reopen-parity-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/teamwork/m5-portable-shelf-and-reopen-parity-registries";
...
pub type M5PortableShelfRecordAndResultRegistriesConsumerSurface =
...
pub enum M5PortableShelfRecordResolutionForm {
...
impl M5PortableShelfRecordResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PortableShelfRecordKind {
...
impl M5PortableShelfRecordKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PortableShelfResultScope {
...
impl M5PortableShelfResultScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PortableShelfRecordSurfaceContext {
...
impl M5PortableShelfRecordSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PortableShelfRecordAnatomyPart {
...
impl M5PortableShelfRecordAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PortableShelfRecordNextAction {
...
impl M5PortableShelfRecordNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PortableShelfRecordExportField {
...
impl M5PortableShelfRecordExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PortableShelfRecordEntryDegradeReason {
...
impl M5PortableShelfRecordEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5PortableShelfRecordNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5PortableShelfRecordNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5PortableShelfRecordNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5PortableShelfRecordNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5PortableShelfRecordNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5PortableShelfResultEntryDegradeReason {
...
impl M5PortableShelfResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5PortableShelfRecordNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5PortableShelfRecordNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5PortableShelfRecordNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5PortableShelfRecordNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5PortableShelfRecordEntryResolutionInput {
    ...
    pub report_section: M5PortableShelfRecordKind,
    /// The render / surface context.
    pub surface_context: M5PortableShelfRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5PortableShelfRecordResolutionForm>,
...
pub struct M5ResolvedPortableShelfRecordEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5PortableShelfRecordEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5PortableShelfRecordNextAction,
...
impl M5ResolvedPortableShelfRecordEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5PortableShelfResultEntryResolutionInput {
    ...
    pub comparison_scope: M5PortableShelfResultScope,
    /// The render / surface context.
    pub surface_context: M5PortableShelfRecordSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5PortableShelfRecordResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedPortableShelfResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5PortableShelfResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5PortableShelfRecordNextAction,
...
impl M5ResolvedPortableShelfResultEntry {
...
pub enum M5PortableShelfRecordResolutionError {
...
impl M5PortableShelfRecordResolutionError {
...
impl fmt::Display for M5PortableShelfRecordResolutionError {
...
impl Error for M5PortableShelfRecordResolutionError {}

fn form_tokens(forms: &[M5PortableShelfRecordResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5PortableShelfRecordResolutionForm]) -> bool {
    let present: BTreeSet<M5PortableShelfRecordResolutionForm> = forms.iter().copied().collect();
    M5PortableShelfRecordResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5PortableShelfRecordKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5PortableShelfResultScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedPortableShelfRecordEntry, M5PortableShelfRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5PortableShelfRecordResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5PortableShelfRecordResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5PortableShelfRecordEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5PortableShelfRecordEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5PortableShelfRecordEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5PortableShelfRecordEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5PortableShelfRecordEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5PortableShelfRecordEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5PortableShelfRecordEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(M5PortableShelfRecordEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    } else if !input.proof_fresh {
        Some(M5PortableShelfRecordEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5PortableShelfRecordNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedPortableShelfRecordEntry {
...
pub fn resolve_review_pack_result_entry(
...
) -> Result<M5ResolvedPortableShelfResultEntry, M5PortableShelfRecordResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5PortableShelfRecordResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5PortableShelfResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5PortableShelfResultEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5PortableShelfResultEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5PortableShelfResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5PortableShelfResultEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5PortableShelfResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5PortableShelfRecordNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedPortableShelfResultEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PortableShelfRecordAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5PortableShelfRecordAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5PortableShelfRecordAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5PortableShelfRecordExportField>,
    ...
    pub review_pack_record_entries: Vec<M5ResolvedPortableShelfRecordEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedPortableShelfResultEntry>,
...
impl M5PortableShelfRecordAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5PortableShelfRecordAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5PortableShelfRecordAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5PortableShelfRecordExportField> =
            self.export_fields.iter().copied().collect();
        M5PortableShelfRecordExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedPortableShelfRecordEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedPortableShelfResultEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PortableShelfRecordAndResultRegistriesVocabularySet {
...
impl M5PortableShelfRecordAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ChangeOrchestrationRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5PortableShelfRecordResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5PortableShelfRecordKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5PortableShelfResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5PortableShelfRecordSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5PortableShelfRecordEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5PortableShelfResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5PortableShelfRecordAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5PortableShelfRecordNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5PortableShelfRecordExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PortableShelfRecordAndResultRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PortableShelfRecordAndResultRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PortableShelfRecordAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PortableShelfRecordAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5PortableShelfRecordAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5PortableShelfRecordAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5PortableShelfRecordAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5PortableShelfRecordAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5PortableShelfRecordAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5PortableShelfRecordAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5PortableShelfRecordAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5PortableShelfRecordAndResultRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5PortableShelfRecordAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION`].
...
impl M5PortableShelfRecordAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5PortableShelfRecordAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5PortableShelfRecordAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_RECORD_KIND {
            violations.push(M5PortableShelfRecordAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_PORTABLE_SHELF_RECORD_AND_RESULT_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5PortableShelfRecordAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5PortableShelfRecordAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5PortableShelfRecordAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5PortableShelfRecordAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Portable-Shelf and Reopen-Parity Registries\n\n");
...
#[derive(Debug)]
pub enum M5PortableShelfRecordAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5PortableShelfRecordAndResultRegistriesViolation>),
}

impl fmt::Display for M5PortableShelfRecordAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5PortableShelfRecordAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5PortableShelfRecordAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5PortableShelfRecordAndResultRegistriesViolation {
...
impl M5PortableShelfRecordAndResultRegistriesViolation {
...
pub fn current_stable_m5_portable_shelf_and_reopen_parity_registries_export() -> Result<
    M5PortableShelfRecordAndResultRegistriesPacket,
    M5PortableShelfRecordAndResultRegistriesArtifactError,
> {
    let packet: M5PortableShelfRecordAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/release/m5-portable-shelf-and-reopen-parity-registries-proof/support_export.json"
        )
    ))
    .map_err(M5PortableShelfRecordAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5PortableShelfRecordAndResultRegistriesArtifactError::Validation(violations))
    }
}

fn validate_source_contracts(
    packet: &M5PortableShelfRecordAndResultRegistriesPacket,
    violations: &mut Vec<M5PortableShelfRecordAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations
                .push(M5PortableShelfRecordAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5PortableShelfRecordAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations
                .push(M5PortableShelfRecordAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations
                .push(M5PortableShelfRecordAndResultRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations.push(
                M5PortableShelfRecordAndResultRegistriesViolation::MandatoryExportFieldMissing,
            );
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_PORTABLE_SHELF_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_PORTABLE_SHELF_REOPEN_PARITY_DOMAIN_SCHEMA_REF)
        {
            violations
                .push(M5PortableShelfRecordAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5PortableShelfRecordAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5PortableShelfRecordAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations
                .push(M5PortableShelfRecordAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5PortableShelfRecordAndResultRegistriesViolation::GovernanceReviewIncomplete,
            );
            return;
        }
    }
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(
                M5PortableShelfRecordAndResultRegistriesViolation::ConsumerProjectionIncomplete,
            );
            return;
        }
    }
...
fn validate_proof_freshness(
    ...
    {
        violations
            .push(M5PortableShelfRecordAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations
            .push(M5PortableShelfRecordAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5PortableShelfRecordKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5PortableShelfRecordSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations.push(
            M5PortableShelfRecordAndResultRegistriesViolation::CohortDescriptorResolutionNotProven,
        );
    }
    ...
    {
        violations.push(
            M5PortableShelfRecordAndResultRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5PortableShelfResultScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations.push(
            M5PortableShelfRecordAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven,
        );
    }
}
