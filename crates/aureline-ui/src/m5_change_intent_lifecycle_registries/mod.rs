//! Implemented M5 change-intent-lifecycle and reconcile-flow registries.
//!
//! The frozen [change-intent lifecycle matrix][matrix] names Aureline's governed change-intent object classes and
//! locks their controlled vocabulary. This module makes the change-intent lifecycle-state and reconcile-flow lane
//! operable so the work-item detail, review detail, start-work sheet, linked-change panel, provider-handoff, help /
//! docs, and support / export surfaces inherit one canonical lifecycle-state descriptor rather than a hand-authored
//! parallel prose that has to be kept consistent: it turns the *lifecycle-state* grammar (one reusable record per
//! tracked change-intent — naming its provider ownership, its local-versus-provider commit state across local draft,
//! queued publish, provider linked, stale relation, broken relation, reconcile required, and superseded, its linked
//! branch / worktree / review identity, its relation source, and its validation evidence — so a change-intent surface
//! surfaces one explicit lifecycle-state object and never keeps a stale branch / review / provider relation green
//! after drift is detected) and the *reconcile-flow* grammar (compare and reconcile when provider state diverges from
//! the local handoff packet, the linked branch / worktree, or the hosted review target — kept honest about what the
//! provider has actually committed, what stays a local draft or queued publish, and what still needs reconciliation)
//! into registry resolvers that produce export-safe, honest projections. Every tracked change-intent then resolves to
//! one typed lifecycle-state record — its provider ownership, its commit state, its linked branch / worktree / review
//! identity, its relation source, and its validation evidence, all attributable to the work item so a local draft or
//! queued publish never reads as a provider-committed update — and to one reconcile-flow object — the reconcile
//! identity, the affected relation-source field, the previous-versus-current commit-state reference, the binding
//! state, and the active reconcile reason — that the work-item, review, linked-change, provider-handoff, and support
//! surfaces can inspect without manual reconstruction, so a change-intent surface never keeps a stale relation green
//! after drift, and a record whose linked evidence cannot be bound to its commit state degrades honestly instead of
//! letting a local handoff packet masquerade as a provider-committed link. Queued-publish and reconcile flows
//! preserve the original local packet, linked evidence, and actor lineage for retry and support export.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Emit one machine-readable lifecycle-state record per tracked change-intent, carrying every lifecycle field.**
//!   [`resolve_review_pack_record_entry`] refuses to read as a clean, registry-bound lifecycle-state entry
//!   unless it names a canonical registry token, a classified [lifecycle-state selector][M5ChangeIntentLifecycleKind]
//!   (local draft, queued publish, provider linked, stale relation, broken relation, reconcile required, or
//!   superseded — never one generic badge), a disclosure role, covers every
//!   [resolution form][M5ChangeIntentLifecycleResolutionForm] (the canonical object, the accessible summary, and the
//!   audit record), publishes every record field (provider ownership, commit state, linked branch / worktree / review
//!   identity, relation source, and validation evidence), keeps its evidence lineage attributable to the tracked
//!   item, and keeps the commit authority bound to its commit state; otherwise it degrades.
//! * **Never keep a stale branch / review / provider relation green after drift is detected.**
//!   [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a lifecycle-state entry that would keep a
//!   stale or broken relation trusted while an engineering blocker remains unresolved or that drops the reconcile /
//!   export / retry path, so it degrades to
//!   [`M5ChangeIntentLifecycleEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`],
//!   and a relation whose language outruns its commit state degrades the same way — the structured invalidation
//!   reason a local-draft-shown-as-provider-committed update must surface.
//! * **Turn local-draft, queued-publish, provider-linked, stale-relation, and broken-relation divergence into a
//!   visible, typed reconcile-flow event instead of an implied provider acceptance.**
//!   [`resolve_review_pack_result_entry`] names a classified [reconcile binding][M5ChangeIntentReconcileScope]
//!   (local-packet binding, linked-branch-worktree binding, or hosted-review-target binding), requires the full
//!   reconcile-identity / affected-relation-source-field / previous-versus-current-commit-state / binding /
//!   active-reason reconcile-flow object, covers every resolution form, and degrades to
//!   [`M5ChangeIntentReconcileEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the flow would present a queued publish or local handoff packet as provider-committed, hide the stale /
//!   broken / reconcile-required state, or drop the original local packet, linked evidence, or actor lineage, so a
//!   reconcile flow can never read as accepted when it has quietly dropped the divergence the provider has not yet
//!   reconciled.
//!
...
//! review detail, start-work sheet, linked-change panel, provider handoff, and support / export surfaces
//! can never fork their own lifecycle meaning. Raw secret values and private endpoints stay outside the export boundary.
//!
...
pub use seed::{
    seeded_m5_change_intent_lifecycle_registries,
    seeded_m5_change_intent_lifecycle_registries_review_pack_record_beta_narrowed,
    seeded_m5_change_intent_lifecycle_registries_review_pack_result_preview_narrowed,
    M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_PACKET_ID,
};
...
/// Stable record-kind tag carried by [`M5ChangeIntentLifecycleRegistriesPacket`].
pub const M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_change_intent_lifecycle_and_relation_packet_registries";

/// Schema version for M5 line-review_pack_record / line-downgrade-packet registry records.
pub const M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_SCHEMA_REF: &str =
    "schemas/teamwork/m5-change-intent-lifecycle-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_DOC_REF: &str =
    "docs/team-workflows/m5_change_intent_lifecycle_registries.md";
...
pub const M5_CHANGE_INTENT_LIFECYCLE_DOMAIN_SCHEMA_REF: &str =
    "schemas/ui/m5-change-intent-lifecycle.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/release/m5-change-intent-lifecycle-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_CSV_REF: &str =
    "artifacts/release/m5-change-intent-lifecycle-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_REPORT_REF: &str =
    "artifacts/release/m5-change-intent-lifecycle-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_FIXTURE_DIR: &str =
    "fixtures/teamwork/m5-change-intent-lifecycle-registries";
...
pub type M5ChangeIntentLifecycleRegistriesConsumerSurface = M5ChangeIntentConsumerSurface;
...
pub enum M5ChangeIntentLifecycleResolutionForm {
...
impl M5ChangeIntentLifecycleResolutionForm {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentLifecycleKind {
...
impl M5ChangeIntentLifecycleKind {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentReconcileScope {
...
impl M5ChangeIntentReconcileScope {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentLifecycleSurfaceContext {
...
impl M5ChangeIntentLifecycleSurfaceContext {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentLifecycleAnatomyPart {
...
impl M5ChangeIntentLifecycleAnatomyPart {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentLifecycleNextAction {
...
impl M5ChangeIntentLifecycleNextAction {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentLifecycleExportField {
...
impl M5ChangeIntentLifecycleExportField {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentLifecycleEntryDegradeReason {
...
impl M5ChangeIntentLifecycleEntryDegradeReason {
    ...
    pub const fn next_action(self) -> M5ChangeIntentLifecycleNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5ChangeIntentLifecycleNextAction::TraceCanonicalRegistry
            }
            Self::CohortPackScopeUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5ChangeIntentLifecycleNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5ChangeIntentLifecycleNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5ChangeIntentLifecycleNextAction::ReviewBlockedOrDegraded,
        }
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ChangeIntentReconcileEntryDegradeReason {
...
impl M5ChangeIntentReconcileEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn next_action(self) -> M5ChangeIntentLifecycleNextAction {
        match self {
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5ChangeIntentLifecycleNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
pub struct M5ChangeIntentLifecycleEntryResolutionInput {
    ...
    pub report_section: M5ChangeIntentLifecycleKind,
    /// The render / surface context.
    pub surface_context: M5ChangeIntentLifecycleSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ChangeIntentLifecycleResolutionForm>,
...
pub struct M5ResolvedChangeIntentLifecycleEntry {
    /// Stable identity of the line-review_pack_record-registry entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ChangeIntentLifecycleEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ChangeIntentLifecycleNextAction,
...
impl M5ResolvedChangeIntentLifecycleEntry {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ChangeIntentReconcileEntryResolutionInput {
    ...
    pub comparison_scope: M5ChangeIntentReconcileScope,
    /// The render / surface context.
    pub surface_context: M5ChangeIntentLifecycleSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5ChangeIntentLifecycleResolutionForm>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedChangeIntentReconcileEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    pub degrade_reason: Option<M5ChangeIntentReconcileEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5ChangeIntentLifecycleNextAction,
...
impl M5ResolvedChangeIntentReconcileEntry {
...
pub enum M5ChangeIntentLifecycleResolutionError {
...
impl M5ChangeIntentLifecycleResolutionError {
...
impl fmt::Display for M5ChangeIntentLifecycleResolutionError {
...
impl Error for M5ChangeIntentLifecycleResolutionError {}

fn form_tokens(forms: &[M5ChangeIntentLifecycleResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5ChangeIntentLifecycleResolutionForm]) -> bool {
    let present: BTreeSet<M5ChangeIntentLifecycleResolutionForm> = forms.iter().copied().collect();
    M5ChangeIntentLifecycleResolutionForm::ALL
        .iter()
...
pub fn review_pack_record_object_is_complete(
    journey: M5ChangeIntentLifecycleKind,
...
pub fn review_pack_result_stays_honest(
    scope: M5ChangeIntentReconcileScope,
...
pub fn resolve_review_pack_record_entry(
...
) -> Result<M5ResolvedChangeIntentLifecycleEntry, M5ChangeIntentLifecycleResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ChangeIntentLifecycleResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5ChangeIntentLifecycleResolutionError::ForbiddenMaterial);
    }
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ChangeIntentLifecycleEntryDegradeReason::DescriptorTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ChangeIntentLifecycleEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.report_section.is_classified() {
        Some(M5ChangeIntentLifecycleEntryDegradeReason::CohortPackScopeUnclassified)
    } else if !input.bound_to_registry {
        Some(M5ChangeIntentLifecycleEntryDegradeReason::DescriptorNotBoundToRegistry)
    } else if !object_complete {
        Some(M5ChangeIntentLifecycleEntryDegradeReason::CohortDescriptorObjectIncomplete)
    } else if !preserve_ok {
        Some(M5ChangeIntentLifecycleEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof)
    } else if !all_forms {
        Some(M5ChangeIntentLifecycleEntryDegradeReason::ResolutionFormCoverageIncomplete)
    } else if support_undisclosed {
        Some(
            M5ChangeIntentLifecycleEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort,
        )
    } else if !input.proof_fresh {
        Some(M5ChangeIntentLifecycleEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        ...
        None => M5ChangeIntentLifecycleNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedChangeIntentLifecycleEntry {
...
pub fn resolve_review_pack_result_entry(
...
) -> Result<M5ResolvedChangeIntentReconcileEntry, M5ChangeIntentLifecycleResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5ChangeIntentLifecycleResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5ChangeIntentReconcileEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        Some(M5ChangeIntentReconcileEntryDegradeReason::SurfaceContextUnresolved)
    } else if !input.comparison_scope.is_classified() {
        Some(M5ChangeIntentReconcileEntryDegradeReason::EvidenceScopeUnclassified)
    } else if !provides_record {
        Some(M5ChangeIntentReconcileEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence)
    } else if !all_forms {
        Some(M5ChangeIntentReconcileEntryDegradeReason::EvidenceFormCoverageIncomplete)
    } else if !input.proof_fresh {
        Some(M5ChangeIntentReconcileEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5ChangeIntentLifecycleNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedChangeIntentReconcileEntry {
        entry_id: input.entry_id,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentLifecycleRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5ChangeIntentLifecycleRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5ChangeIntentLifecycleAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5ChangeIntentLifecycleExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5ChangeIntentDowngradeTrigger>,
    /// Resolved line-review_pack_record-registry examples.
    pub review_pack_record_entries: Vec<M5ResolvedChangeIntentLifecycleEntry>,
    /// Resolved line-downgrade-packet examples.
    pub review_pack_result_entries: Vec<M5ResolvedChangeIntentReconcileEntry>,
...
impl M5ChangeIntentLifecycleRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5ChangeIntentLifecycleAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5ChangeIntentLifecycleAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5ChangeIntentLifecycleExportField> =
            self.export_fields.iter().copied().collect();
        M5ChangeIntentLifecycleExportField::MANDATORY
            .iter()
    ...
    fn review_pack_record_is_honest(ex: &M5ResolvedChangeIntentLifecycleEntry) -> bool {
        !ex.is_clean()
    ...
    fn downgrade_is_honest(ex: &M5ResolvedChangeIntentReconcileEntry) -> bool {
        !ex.is_clean()
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentLifecycleRegistriesVocabularySet {
...
impl M5ChangeIntentLifecycleRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ChangeIntentRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5ChangeIntentLifecycleResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5ChangeIntentLifecycleKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5ChangeIntentReconcileScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5ChangeIntentLifecycleSurfaceContext::ALL, |v| v.as_str()),
            review_pack_record_degrade_reasons: tokens(
                &M5ChangeIntentLifecycleEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            review_pack_result_degrade_reasons: tokens(
                &M5ChangeIntentReconcileEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5ChangeIntentLifecycleAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5ChangeIntentLifecycleNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5ChangeIntentLifecycleExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentLifecycleRegistriesGovernanceReview {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentLifecycleRegistriesConsumerProjection {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentLifecycleRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentLifecycleRegistriesReleasePosture {
...
/// Constructor input for [`M5ChangeIntentLifecycleRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5ChangeIntentLifecycleRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5ChangeIntentLifecycleRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5ChangeIntentLifecycleRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5ChangeIntentLifecycleRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5ChangeIntentLifecycleRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5ChangeIntentLifecycleRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5ChangeIntentLifecycleRegistriesReleasePosture,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ChangeIntentLifecycleRegistriesPacket {
    /// Record kind; must equal [`M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_SCHEMA_VERSION`].
...
impl M5ChangeIntentLifecycleRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5ChangeIntentLifecycleRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5ChangeIntentLifecycleRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_RECORD_KIND {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_CHANGE_INTENT_LIFECYCLE_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(&serde_json::to_value(self).expect(
            ...
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 Change-Intent Lifecycle-State and Reconcile-Flow Registries\n\n");
...
#[derive(Debug)]
pub enum M5ChangeIntentLifecycleRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5ChangeIntentLifecycleRegistriesViolation>),
}

impl fmt::Display for M5ChangeIntentLifecycleRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl Error for M5ChangeIntentLifecycleRegistriesArtifactError {}

/// Validation failures emitted by [`M5ChangeIntentLifecycleRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5ChangeIntentLifecycleRegistriesViolation {
...
impl M5ChangeIntentLifecycleRegistriesViolation {
...
pub fn current_stable_m5_change_intent_lifecycle_registries_export(
) -> Result<M5ChangeIntentLifecycleRegistriesPacket, M5ChangeIntentLifecycleRegistriesArtifactError>
{
    let packet: M5ChangeIntentLifecycleRegistriesPacket =
        serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../artifacts/release/m5-change-intent-lifecycle-registries-proof/support_export.json"
    )))
        .map_err(M5ChangeIntentLifecycleRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5ChangeIntentLifecycleRegistriesArtifactError::Validation(
            violations,
        ))
    }
}
...
fn validate_source_contracts(
    packet: &M5ChangeIntentLifecycleRegistriesPacket,
    violations: &mut Vec<M5ChangeIntentLifecycleRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5ChangeIntentLifecycleRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations
                .push(M5ChangeIntentLifecycleRegistriesViolation::MandatoryExportFieldMissing);
        }
        ...
        if !refs.contains(M5_CHANGE_INTENT_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_CHANGE_INTENT_LIFECYCLE_DOMAIN_SCHEMA_REF)
        {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.review_pack_record_entries.is_empty() || row.review_pack_result_entries.is_empty() {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        ...
        if !ok {
            violations.push(M5ChangeIntentLifecycleRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5ChangeIntentLifecycleRegistriesViolation::ConsumerProjectionIncomplete);
            return;
        }
    }
}
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5ChangeIntentLifecycleRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5ChangeIntentLifecycleRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let clean_surfaces: BTreeSet<String> = review_pack_records()
        ...
        .collect();
    let journeys_covered = M5ChangeIntentLifecycleKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5ChangeIntentLifecycleSurfaceContext::FIRST_CONSUMERS
        .iter()
    ...
    {
        violations
            .push(M5ChangeIntentLifecycleRegistriesViolation::CohortDescriptorResolutionNotProven);
    }
    ...
    {
        violations.push(
            M5ChangeIntentLifecycleRegistriesViolation::RollbackAndDiagnosticsPreservationNotProven,
        );
    }
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5ChangeIntentReconcileScope::CANONICAL_SCOPES
        .iter()
    ...
    {
        violations
            .push(M5ChangeIntentLifecycleRegistriesViolation::CohortEvidenceIntegrityNotProven);
    }
}
