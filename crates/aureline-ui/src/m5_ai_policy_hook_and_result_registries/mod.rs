//! Implemented M5 ai-policy-hook and ai-policy-result registries.
//!
//! The frozen [review-pack evaluator matrix][matrix] names Aureline's governed review-pack object classes and
//! locks their controlled vocabulary. This module makes the matrix's `ai_policy_hook` class operable so review,
//! AI-review, provider-handoff, and support / export surfaces inherit one canonical AI-review-policy descriptor
//! rather than a hand-authored parallel prose that has to be kept consistent: it turns the *AI-policy-hook* grammar
//! (one machine-readable policy hook per AI review run — its allowed analyzers, its severity thresholds, its
//! suppression classes, and its mandatory citation requirements — each resolving through the same active review-pack
//! version and content digest and evaluator lineage as human, local, and CI review) and the *AI-policy-result*
//! grammar (the binding a result sits in — an analyzer-result-class binding, a pack-version-and-digest binding, or
//! a rerun-staleness binding) into registry resolvers that produce export-safe, honest projections. Every AI review
//! run then resolves to one typed AI-policy-hook object — the analyzers, thresholds, suppression classes, and
//! citation requirements it applies, its evaluator lineage, and the pack version / digest it resolved through, all
//! attributable to the governing pack so a suppression class, severity threshold, or citation expectation from a
//! different or stale pack revision can never be applied silently — and to one AI-policy-result object — the
//! analyzer result class (full, experimental, or policy-downgraded), the pack version and digest, and the rerun /
//! staleness relationship a prior finding holds after a pack change (current, rerun-required, or stale) — that the
//! review, AI-review, support, and provider-handoff surfaces can inspect without manual reconstruction, so an AI run
//! operating with a narrower or different capability set than the declared pack stays visibly experimental or
//! policy-downgraded instead of reading as full pack-compliant evidence, and a hook that cannot name the pack version
//! it resolved through degrades honestly instead of running AI review under an undisclosed pack version.
//!
//! Three implementation requirements drive the resolvers:
//!
//! * **Bind AI review policy hooks — allowed analyzers, severity thresholds, suppression classes, and mandatory
//!   citation requirements — to the active review-pack version so they resolve through the same evaluator lineage
//!   as human / local / CI review.** [`resolve_ai_policy_hook_entry`] refuses to read as a clean, registry-bound
//!   hook entry unless it names a canonical registry token, a classified [policy-hook facet][M5AiPolicyHookKind],
//!   a disclosure role, covers every [resolution form][M5AiPolicyHookResolutionForm] (the canonical object, the
//!   accessible summary, and the audit record), publishes every field (the analyzers, thresholds, suppression
//!   classes, and citation requirements bound to the active pack version / digest), keeps its evaluator lineage
//!   attributable to the pack, and keeps a divergent-capability run bound to its disclosure state; otherwise it
//!   degrades.
//! * **Never silently apply a suppression class, severity threshold, or citation expectation from a different or
//!   stale pack revision.** [`line_preserves_rollback_and_diagnostics_before_widening`] rejects a hook entry whose
//!   pack-version / evaluator-lineage join is not preserved (an experimental or policy-downgraded run presenting
//!   itself as a full pack-compliant review without one) so it degrades to
//!   [`M5AiPolicyHookEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof`] — the
//!   structured blocker reason a divergent-pack-version-applied-silently attempt must surface.
//! * **Keep AI review findings, publish sheets, and support / export packets pack-version aware so reruns and
//!   outdated logic stay attributable to the governing review pack.** [`resolve_ai_policy_result_entry`] names a
//!   classified [result binding][M5AiPolicyResultScope] (analyzer-result-class binding, pack-version-and-digest
//!   binding, or rerun-staleness binding), requires the full analyzer-result-class / pack-version-and-digest /
//!   rerun-staleness result object, covers every resolution form, and degrades to
//!   [`M5AiPolicyResultEntryDegradeReason::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence`]
//!   when the result would present a prior finding as current pack-compliant evidence after the pack changed, hide
//!   the experimental / policy-downgraded label, or let a narrower analyzer set masquerade as a full run, so an AI
//!   result can never read as trustworthy when it has quietly dropped the reason it is not current, full pack-compliant
//!   evidence.
//!
...
//! can never fork their own review-pack meaning. Raw prompts, secret values, and private endpoints stay outside the export boundary.
//!
...
use crate::m5_review_pack_evaluator_matrix::{
    ...
    M5ReviewPackRequiredLabel, M5ReviewPackRole, M5_AI_POLICY_HOOK_DOMAIN_SCHEMA_REF,
    M5_REVIEW_PACK_MATRIX_DOC_REF, M5_REVIEW_PACK_MATRIX_SCHEMA_REF,
};

/// Stable record-kind tag carried by [`M5AiPolicyHookAndResultRegistriesPacket`].
pub const M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_RECORD_KIND: &str =
    "implement_m5_ai_policy_hook_and_result_packet_registries";

/// Schema version for M5 line-ai_policy_hook / line-downgrade-packet registry records.
pub const M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_SCHEMA_VERSION: u32 = 1;

/// Repo-relative path of the combined registries schema.
pub const M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_SCHEMA_REF: &str =
    "schemas/review/m5-ai-policy-hook-and-result-registries.schema.json";

/// Repo-relative path of the registries doc.
pub const M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_DOC_REF: &str =
    "docs/review/m5_ai_policy_hook_and_result_registries.md";

/// Repo-relative path of the canonical AI-policy-result domain schema minted by this lane (the typed outcome an
/// AI review run inherits from the governing review pack — the analyzer result class it holds, full versus
/// experimental versus policy-downgraded, the pack version and digest the run resolved through, and the rerun /
/// staleness relationship a prior finding holds after a pack change). Declared locally so the row-level
/// domain-schema-ref check stays self-contained.
pub const M5_AI_POLICY_RESULT_DOMAIN_SCHEMA_REF: &str =
    "schemas/review/m5-ai-policy-result.schema.json";

/// Repo-relative path of the checked support-export artifact.
pub const M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_ARTIFACT_REF: &str =
    "artifacts/review/m5-ai-policy-hook-and-result-registries-proof/support_export.json";

/// Repo-relative path of the checked machine-readable registries CSV.
pub const M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_CSV_REF: &str =
    "artifacts/review/m5-ai-policy-hook-and-result-registries-proof/matrix.csv";

/// Repo-relative path of the checked Markdown design report.
pub const M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_REPORT_REF: &str =
    "artifacts/review/m5-ai-policy-hook-and-result-registries-proof/summary.md";

/// Repo-relative path of the protected fixture directory.
pub const M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_FIXTURE_DIR: &str =
...
pub type M5AiPolicyHookAndResultRegistriesConsumerSurface = M5ReviewPackConsumerSurface;

/// One of the three resolution forms every line-ai_policy_hook or line-downgrade-packet entry must hold across so
...
/// line-ai_policy_hook and line-downgrade *domains* but not the concrete form set an entry must cover.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiPolicyHookResolutionForm {
    /// The canonical resolved line-ai_policy_hook / line-downgrade-packet object.
    CanonicalObject,
...
impl M5AiPolicyHookResolutionForm {
...
/// Controlled manifest field a ai-policy-hook entry publishes for one retiring object-class identity, so every
...
pub enum M5AiPolicyHookKind {
    ...
    PolicyHookUnclassified,
}

impl M5AiPolicyHookKind {
    /// Every public-proof-ledger kind, in declaration order.
    pub const ALL: [Self; 7] = [
        Self::AllowedAnalyzer,
        Self::SeverityThreshold,
        Self::SuppressionClass,
        Self::PolicyDowngraded,
        Self::ExperimentalAnalyzer,
        Self::MandatoryCitation,
        Self::PolicyHookUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::AllowedAnalyzer => "allowed_analyzer",
            Self::SeverityThreshold => "severity_threshold",
            Self::SuppressionClass => "suppression_class",
            Self::PolicyDowngraded => "policy_downgraded",
            Self::ExperimentalAnalyzer => "experimental_analyzer",
            Self::MandatoryCitation => "mandatory_citation",
            Self::PolicyHookUnclassified => "policy_hook_unclassified",
        }
    }

    /// Whether the item is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::PolicyHookUnclassified)
    }

    /// The canonical mode for this public-proof-ledger kind.
    pub const fn canonical_report_section_mode(self) -> &'static str {
        match self {
            Self::AllowedAnalyzer => "allowed_analyzer_mode",
            Self::SeverityThreshold => "severity_threshold_mode",
            Self::SuppressionClass => "suppression_class_mode",
            Self::PolicyDowngraded => "policy_downgraded_mode",
            Self::ExperimentalAnalyzer => "experimental_analyzer_mode",
            Self::MandatoryCitation => "mandatory_citation_mode",
            Self::PolicyHookUnclassified => "",
        }
    }
    ...
    pub const fn is_public_facing_line(self) -> bool {
        matches!(self, Self::PolicyDowngraded | Self::ExperimentalAnalyzer)
    }
}

/// Controlled ai-policy-result scope a ai-policy-hook change sits in, so a changed cutoff date or
...
pub enum M5AiPolicyResultScope {
    ...
    RerunStalenessBinding,
    /// The ai-policy-result scope is unclassified, which is disallowed.
    AiPolicyResultUnclassified,
}

impl M5AiPolicyResultScope {
    /// Every comparison scope, in declaration order.
    pub const ALL: [Self; 4] = [
        Self::AnalyzerResultClassBinding,
        Self::PackVersionDigestBinding,
        Self::RerunStalenessBinding,
        Self::AiPolicyResultUnclassified,
    ];
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::AnalyzerResultClassBinding => "analyzer_result_class_binding",
            Self::PackVersionDigestBinding => "pack_version_digest_binding",
            Self::RerunStalenessBinding => "rerun_staleness_binding",
            Self::AiPolicyResultUnclassified => "ai_policy_result_unclassified",
        }
    }

    /// Whether the comparison scope is classified (never the unclassified sentinel).
    pub const fn is_classified(self) -> bool {
        !matches!(self, Self::AiPolicyResultUnclassified)
    }
}

/// Controlled render context — which claimed M5 surface renders the registry entry, so a line-ai_policy_hook or
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiPolicyHookSurfaceContext {
...
impl M5AiPolicyHookSurfaceContext {
...
/// One mandatory rendered part a line-ai_policy_hook or line-downgrade-packet entry must be able to show, so no
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiPolicyHookAnatomyPart {
    ...
    RegistryReference,
    /// The line journey the entry classifies (line-ai_policy_hook entry).
    CohortArchetype,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles the entry publishes (line-ai_policy_hook entry).
    RepoBundleToolchainAndDeploymentRows,
    /// The resolution-form coverage (canonical / accessible / audit).
    ResolutionFormCoverage,
    /// The known-limits packet and rollback target the entry preserves before widening (line-ai_policy_hook
    /// entry).
    ...
    KeyboardRoute,
    /// The plain-language meaning of the resolved line ai_policy_hook or line downgrade (both entries).
    PlainLanguageMeaning,
}

impl M5AiPolicyHookAnatomyPart {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortEvidenceFields => "ai_policy_result_fields",
...
/// line ai_policy_hook, a line-downgrade packet, or a degraded line-ai_policy_hook / line-downgrade-packet entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiPolicyHookNextAction {
    /// Expand the resolved line ai_policy_hook's or line-downgrade packet's plain-language meaning.
    ExpandCohortMeaning,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiPolicyHookExportField {
...
impl M5AiPolicyHookExportField {
...
/// Reason a line-ai_policy_hook entry degraded below a clean, registry-bound state. The degrade-first ladder
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiPolicyHookEntryDegradeReason {
    /// The canonical registry token name is unstated; a user cannot trace what the ai_policy_hook means.
    DescriptorTokenUnstated,
    /// The render / surface context cannot currently be resolved.
    SurfaceContextUnresolved,
    /// The line journey is unclassified (not in the resolved taxonomy).
    CohortPolicyHookUnclassified,
    ...
    /// The resolved line-ai_policy_hook object is incomplete: the exact repo / journey rows, bundle IDs, install
...
impl M5AiPolicyHookEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 9] = [
        Self::DescriptorTokenUnstated,
        Self::SurfaceContextUnresolved,
        Self::CohortPolicyHookUnclassified,
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::DescriptorTokenUnstated => "ai_policy_hook_token_unstated",
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::CohortPolicyHookUnclassified => "line_policy_hook_unclassified",
            Self::DescriptorNotBoundToRegistry => "ai_policy_hook_not_bound_to_registry",
            Self::CohortDescriptorObjectIncomplete => "ai_policy_hook_object_incomplete",
            Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                "ai_policy_hook_lets_line_widen_without_rollback_or_runs_support_ahead_of_proof"
            }
    ...
    pub const fn next_action(self) -> M5AiPolicyHookNextAction {
        match self {
            Self::DescriptorTokenUnstated | Self::DescriptorNotBoundToRegistry => {
                M5AiPolicyHookNextAction::TraceCanonicalRegistry
            }
            Self::CohortPolicyHookUnclassified
            | Self::CohortDescriptorObjectIncomplete
            | Self::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof => {
                M5AiPolicyHookNextAction::InspectArchetypeOrScope
            }
            Self::ResolutionFormCoverageIncomplete => {
                M5AiPolicyHookNextAction::CompleteResolutionFormCoverage
            }
            Self::SurfaceContextUnresolved
            | Self::RollbackOrDiagnosticsNotPreservedForPublicCohort
            | Self::ProofStale => M5AiPolicyHookNextAction::ReviewBlockedOrDegraded,
        }
    }
    ...
    pub const fn downgrade_trigger(self) -> M5ReviewPackDowngradeTrigger {
        match self {
            ...
            Self::CohortPolicyHookUnclassified | Self::CohortDescriptorObjectIncomplete => {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5AiPolicyResultEntryDegradeReason {
...
impl M5AiPolicyResultEntryDegradeReason {
    /// Every degrade reason, in declaration order.
    pub const ALL: [Self; 6] = [
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::SurfaceContextUnresolved => "surface_context_unresolved",
            Self::EvidenceScopeUnclassified => "comparison_ai_policy_result_unclassified",
            Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                "ai_policy_result_runs_support_ahead_of_proof_or_drops_ai_policy_result"
            }
    ...
    pub const fn next_action(self) -> M5AiPolicyHookNextAction {
        match self {
            Self::EvidenceTokenUnstated => M5AiPolicyHookNextAction::TraceCanonicalRegistry,
            Self::EvidenceScopeUnclassified
            | Self::CohortEvidenceRunsSupportAheadOfProofOrDropsCohortEvidence => {
                M5AiPolicyHookNextAction::InspectArchetypeOrScope
            }
            ...
            Self::SurfaceContextUnresolved | Self::ProofStale => {
                M5AiPolicyHookNextAction::ReviewBlockedOrDegraded
            }
        }
    }
...
/// Input to [`resolve_ai_policy_hook_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5AiPolicyHookEntryResolutionInput {
    /// Stable identity of the line-ai_policy_hook-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this ai_policy_hook binds to (e.g. `launch.line.public-preview`); empty means
    /// unstated.
    pub line_binding_id: String,
    /// The canonical registry token name (e.g. `line.ai_policy_hook.experimental_analyzer`); empty means unstated.
    ...
    pub report_section: M5AiPolicyHookKind,
    /// The render / surface context.
    pub surface_context: M5AiPolicyHookSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5AiPolicyHookResolutionForm>,
    ...
    /// True when the behavior traces to the line-ai_policy_hook registry (never a hand-copied constant).
...
/// Resolved, export-safe line-ai_policy_hook-registry projection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedAiPolicyHookEntry {
    /// Stable identity of the line-ai_policy_hook-registry entry.
    pub entry_id: String,
    /// The stable line-binding ID this ai_policy_hook binds to.
    pub line_binding_id: String,
    ...
    /// Whether the resolved line-ai_policy_hook object publishes every required field.
    pub ai_policy_hook_object_complete: bool,
    /// Whether the entry traces to the line-ai_policy_hook registry.
    pub bound_to_registry: bool,
    ...
    pub degrade_reason: Option<M5AiPolicyHookEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5AiPolicyHookNextAction,
    /// Whether the ai_policy_hook resolves to one typed object across every claimed line (clean entry naming every
    /// fact).
    pub ai_policy_hook_resolves_across_lines: bool,
}

impl M5ResolvedAiPolicyHookEntry {
    /// Whether this line-ai_policy_hook entry reads as a clean, registry-bound state.
    pub fn is_clean(&self) -> bool {
        self.degrade_reason.is_none()
    }
}

/// Input to [`resolve_ai_policy_result_entry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5AiPolicyResultEntryResolutionInput {
    ...
    pub comparison_scope: M5AiPolicyResultScope,
    /// The render / surface context.
    pub surface_context: M5AiPolicyHookSurfaceContext,
    /// The resolution forms this entry holds across (must cover canonical / accessible / audit).
    pub resolution_form_coverage: Vec<M5AiPolicyHookResolutionForm>,
    ...
    pub keeps_ai_policy_result_visible: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5ResolvedAiPolicyResultEntry {
    /// Stable identity of the line-downgrade-packet entry.
    pub entry_id: String,
    ...
    /// gap flagged).
    pub ai_policy_result_stays_honest: bool,
    ...
    pub provides_complete_ai_policy_result: bool,
    /// Degrade reason, if the entry could not read as a clean, safe state.
    pub degrade_reason: Option<M5AiPolicyResultEntryDegradeReason>,
    /// Next safe action offered.
    pub next_action: M5AiPolicyHookNextAction,
...
impl M5ResolvedAiPolicyResultEntry {
...
pub enum M5AiPolicyHookResolutionError {
    /// The line-ai_policy_hook-entry id was empty.
...
impl M5AiPolicyHookResolutionError {
    /// Stable token used in tests and exports.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::EmptyCohortDescriptorEntryId => "empty_ai_policy_hook_entry_id",
            Self::EmptyCohortEvidencePacketEntryId => "empty_ai_policy_result_entry_id",
            Self::ForbiddenMaterial => "forbidden_material",
        }
    }
}

impl fmt::Display for M5AiPolicyHookResolutionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "m5 line-ai_policy_hook / line-downgrade-packet registry resolution error: {}",
            self.as_str()
        )
    }
}

impl Error for M5AiPolicyHookResolutionError {}

fn form_tokens(forms: &[M5AiPolicyHookResolutionForm]) -> Vec<String> {
    forms.iter().map(|f| f.as_str().to_owned()).collect()
}

fn covers_all_resolution_forms(forms: &[M5AiPolicyHookResolutionForm]) -> bool {
    let present: BTreeSet<M5AiPolicyHookResolutionForm> = forms.iter().copied().collect();
    M5AiPolicyHookResolutionForm::ALL
        .iter()
        .all(|form| present.contains(form))
}

/// Whether the resolved line-ai_policy_hook object publishes every required field: classified line journey,
...
pub fn ai_policy_hook_object_is_complete(
    journey: M5AiPolicyHookKind,
...
/// Whether the line ai_policy_hook keeps a line from widening without preserving its rollback and diagnostics
...
pub fn ai_policy_result_stays_honest(
    scope: M5AiPolicyResultScope,
    comparison_is_truthful: bool,
    keeps_ai_policy_result_visible: bool,
    ...
    scope.is_classified()
        ...
        && keeps_ai_policy_result_visible
        && (!support_language_present || support_language_bound_to_proof)
        && (!known_limits_gap_present || known_limits_gap_flagged)
}

/// Resolves a line-ai_policy_hook-registry entry so it stays bound to the line-ai_policy_hook registry: the entry
/// names its canonical token, semantic role, and line journey, covers all three resolution forms, publishes
/// a complete ai_policy_hook object (exact repo / journey rows, bundle IDs, install topology, toolchain envelope,
...
/// line proof.
pub fn resolve_ai_policy_hook_entry(
    input: M5AiPolicyHookEntryResolutionInput,
) -> Result<M5ResolvedAiPolicyHookEntry, M5AiPolicyHookResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5AiPolicyHookResolutionError::EmptyCohortDescriptorEntryId);
    }
    ...
    {
        return Err(M5AiPolicyHookResolutionError::ForbiddenMaterial);
    }

    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let object_complete = ai_policy_hook_object_is_complete(
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        ...
        Some(M5AiPolicyHookEntryDegradeReason::RollbackOrDiagnosticsNotPreservedForPublicCohort)
    ...
    let next_action = match degrade_reason {
        ...
        None => M5AiPolicyHookNextAction::ExpandCohortMeaning,
    };

    Ok(M5ResolvedAiPolicyHookEntry {
        ...
        ai_policy_hook_object_complete: object_complete,
        ...
        degrade_reason,
        next_action,
        ai_policy_hook_resolves_across_lines: degrade_reason.is_none(),
    })
}
...
/// as covered.
pub fn resolve_ai_policy_result_entry(
    input: M5AiPolicyResultEntryResolutionInput,
) -> Result<M5ResolvedAiPolicyResultEntry, M5AiPolicyHookResolutionError> {
    if input.entry_id.trim().is_empty() {
        return Err(M5AiPolicyHookResolutionError::EmptyCohortEvidencePacketEntryId);
    }
    if string_is_forbidden(&input.entry_id)
    ...
    let all_forms = covers_all_resolution_forms(&input.resolution_form_coverage);
    let record_stays_honest = ai_policy_result_stays_honest(
        input.comparison_scope,
        input.comparison_is_truthful,
        input.keeps_ai_policy_result_visible,
    ...
    let degrade_reason = if input.token_name.trim().is_empty() {
        Some(M5AiPolicyResultEntryDegradeReason::EvidenceTokenUnstated)
    } else if !input.surface_context.is_resolved() {
        ...
        Some(M5AiPolicyResultEntryDegradeReason::ProofStale)
    } else {
        None
    };
    ...
    let next_action = match degrade_reason {
        Some(reason) => reason.next_action(),
        None => M5AiPolicyHookNextAction::TraceCanonicalRegistry,
    };

    Ok(M5ResolvedAiPolicyResultEntry {
        entry_id: input.entry_id,
        ...
        keeps_ai_policy_result_visible: input.keeps_ai_policy_result_visible,
        ...
        ai_policy_result_stays_honest: record_stays_honest,
        provides_complete_ai_policy_result: provides_record,
        degrade_reason,
        next_action,
...
/// One registry row: one consumer surface bound to the resolved line-ai_policy_hook and line-downgrade-packet
/// entries it must project honestly.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiPolicyHookAndResultRegistriesRow {
    /// Consumer surface this row projects onto.
    pub consumer_surface: M5AiPolicyHookAndResultRegistriesConsumerSurface,
    ...
    pub anatomy_parts: Vec<M5AiPolicyHookAnatomyPart>,
    /// Export fields exposed (must include the mandatory five).
    pub export_fields: Vec<M5AiPolicyHookExportField>,
    /// Downgrade triggers that apply to this row.
    pub downgrade_triggers: Vec<M5ReviewPackDowngradeTrigger>,
    /// Resolved line-ai_policy_hook-registry examples.
    pub ai_policy_hook_entries: Vec<M5ResolvedAiPolicyHookEntry>,
    /// Resolved line-downgrade-packet examples.
    pub ai_policy_result_entries: Vec<M5ResolvedAiPolicyResultEntry>,
    /// Proof packet refs that keep this row current.
    pub required_proof_packet_refs: Vec<String>,
    /// Source contract refs consumed by this row (must include both the line-ai_policy_hook and
    ...
    pub collapses_distinct_ai_policy_result_classes_into_one_lane: bool,
}

impl M5AiPolicyHookAndResultRegistriesRow {
    fn declares_mandatory_anatomy(&self) -> bool {
        let present: BTreeSet<M5AiPolicyHookAnatomyPart> =
            self.anatomy_parts.iter().copied().collect();
        M5AiPolicyHookAnatomyPart::MANDATORY
            .iter()
            .all(|part| present.contains(part))
    }

    fn declares_mandatory_export_fields(&self) -> bool {
        let present: BTreeSet<M5AiPolicyHookExportField> =
            self.export_fields.iter().copied().collect();
        M5AiPolicyHookExportField::MANDATORY
            .iter()
    ...
    fn honours_invariants(&self) -> bool {
        !self.widens_a_line_without_current_rollback_and_diagnostics_downgrade
            ...
            && !self.collapses_distinct_ai_policy_result_classes_into_one_lane
    }

    /// True when a clean line-ai_policy_hook entry preserves registry-bound truth: it traces to the registry,
    /// keeps a classified line journey, publishes a complete ai_policy_hook object, preserves its rollback and
    /// diagnostics posture, covers all three resolution forms, and keeps a public-facing line's support
    /// language matched to proof.
    fn ai_policy_hook_is_honest(ex: &M5ResolvedAiPolicyHookEntry) -> bool {
        !ex.is_clean()
            || (ex.bound_to_registry
                && ex.report_section_is_classified
                && ex.ai_policy_hook_object_complete
    ...
    fn downgrade_is_honest(ex: &M5ResolvedAiPolicyResultEntry) -> bool {
        !ex.is_clean()
            || (ex.comparison_scope_is_classified
                && ex.provides_complete_ai_policy_result
                && ex.ai_policy_result_stays_honest
                && ex.covers_all_resolution_forms)
    }

    /// True when every resolved example on this row is honest.
    fn examples_are_honest(&self) -> bool {
        self.ai_policy_hook_entries
            .iter()
            .all(Self::ai_policy_hook_is_honest)
            && self
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiPolicyHookAndResultRegistriesVocabularySet {
    ...
    /// Cohort-ai_policy_hook-entry degrade-reason tokens.
    pub ai_policy_hook_degrade_reasons: Vec<String>,
    /// Cohort-downgrade-packet-entry degrade-reason tokens.
    pub ai_policy_result_degrade_reasons: Vec<String>,
    /// Anatomy-part tokens.
...
impl M5AiPolicyHookAndResultRegistriesVocabularySet {
    /// Builds the canonical vocabulary set from the typed `ALL` arrays.
    pub fn canonical() -> Self {
        Self {
            semantic_roles: tokens(&M5ReviewPackRole::ALL, |v| v.as_str()),
            resolution_forms: tokens(&M5AiPolicyHookResolutionForm::ALL, |v| v.as_str()),
            report_section_kinds: tokens(&M5AiPolicyHookKind::ALL, |v| v.as_str()),
            comparison_scopes: tokens(&M5AiPolicyResultScope::ALL, |v| v.as_str()),
            surface_contexts: tokens(&M5AiPolicyHookSurfaceContext::ALL, |v| v.as_str()),
            ai_policy_hook_degrade_reasons: tokens(&M5AiPolicyHookEntryDegradeReason::ALL, |v| {
                v.as_str()
            }),
            ai_policy_result_degrade_reasons: tokens(
                &M5AiPolicyResultEntryDegradeReason::ALL,
                |v| v.as_str(),
            ),
            anatomy_parts: tokens(&M5AiPolicyHookAnatomyPart::ALL, |v| v.as_str()),
            next_actions: tokens(&M5AiPolicyHookNextAction::ALL, |v| v.as_str()),
            export_fields: tokens(&M5AiPolicyHookExportField::ALL, |v| v.as_str()),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiPolicyHookAndResultRegistriesGovernanceReview {
    /// The ai_policy_hook registry names a canonical token, semantic role, and line journey for every entry.
    pub ai_policy_hook_registry_names_token_role_and_journey: bool,
    /// Every claimed line resolves to one typed line-ai_policy_hook object from the shared registry, not
    /// per-entry reconstruction.
    pub line_resolves_to_typed_ai_policy_hook_from_shared_registry: bool,
    /// The exact repo / journey rows, bundle IDs, install topology, toolchain envelope, and deployment
    /// profiles are published for every resolved ai_policy_hook.
    ...
    pub ai_policy_result_keeps_proof_visible_and_binds_support_language: bool,
    ...
    /// Every line-ai_policy_hook and line-downgrade-packet entry covers the canonical / accessible / audit
    /// resolution forms.
    pub every_entry_covers_all_resolution_forms: bool,
    /// Cohort-ai_policy_hook and line-downgrade-packet behavior stay bound to the shared registries rather than
    ...
    pub ai_policy_hook_or_downgrade_drift_caught_before_release: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiPolicyHookAndResultRegistriesConsumerProjection {
    /// Shiproom and release center consume the shared line-ai_policy_hook registry.
    ...
    /// Behavior traces back to the canonical line-ai_policy_hook and line-downgrade-packet domain contracts.
    pub behavior_traces_to_domain_contracts: bool,
    /// Support / export reads a single canonical line-ai_policy_hook / line-downgrade-packet registry source.
    pub support_export_reads_single_registry_source: bool,
}

/// Proof freshness block.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiPolicyHookAndResultRegistriesProofFreshness {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiPolicyHookAndResultRegistriesReleasePosture {
...
/// Constructor input for [`M5AiPolicyHookAndResultRegistriesPacket::new`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M5AiPolicyHookAndResultRegistriesPacketInput {
    /// Stable packet id.
    pub packet_id: String,
    /// Human-readable registries label.
    pub registries_label: String,
    /// Registry rows.
    pub registry_rows: Vec<M5AiPolicyHookAndResultRegistriesRow>,
    /// Frozen controlled-vocabulary set.
    pub vocabulary_set: M5AiPolicyHookAndResultRegistriesVocabularySet,
    /// Governance-review block.
    pub governance_review: M5AiPolicyHookAndResultRegistriesGovernanceReview,
    /// Consumer projection block.
    pub consumer_projection: M5AiPolicyHookAndResultRegistriesConsumerProjection,
    /// Proof freshness block.
    pub proof_freshness: M5AiPolicyHookAndResultRegistriesProofFreshness,
    /// Release and support parity posture.
    pub release_posture: M5AiPolicyHookAndResultRegistriesReleasePosture,
...
/// Export-safe M5 line-ai_policy_hook and line-downgrade-packet registries packet.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct M5AiPolicyHookAndResultRegistriesPacket {
    /// Record kind; must equal [`M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_RECORD_KIND`].
    pub record_kind: String,
    /// Schema version; must equal [`M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_SCHEMA_VERSION`].
...
impl M5AiPolicyHookAndResultRegistriesPacket {
    /// Builds a registries packet from stable-lane input.
    pub fn new(input: M5AiPolicyHookAndResultRegistriesPacketInput) -> Self {
        Self {
            record_kind: M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_RECORD_KIND.to_owned(),
            schema_version: M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_SCHEMA_VERSION,
    ...
    pub fn validate(&self) -> Vec<M5AiPolicyHookAndResultRegistriesViolation> {
        let mut violations = Vec::new();

        if self.record_kind != M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_RECORD_KIND {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::WrongRecordKind);
        }
        if self.schema_version != M5_AI_POLICY_HOOK_AND_RESULT_REGISTRIES_SCHEMA_VERSION {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::WrongSchemaVersion);
        }
        ...
        {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::MissingIdentity);
        }

        validate_source_contracts(self, &mut violations);
        if !self.vocabulary_set.matches_canonical() {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::VocabularySetDrift);
        }
        ...
        if json_contains_forbidden_material(
            &serde_json::to_value(self).expect(
                "m5 line-ai_policy_hook / line-downgrade-packet registries packet serializes",
            ),
        ) {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::RawMaterialInExport);
        }
    ...
    pub fn export_safe_json(&self) -> String {
        serde_json::to_string_pretty(self)
            .expect("m5 line-ai_policy_hook / line-downgrade-packet registries packet serializes")
    }
    ...
    pub fn render_matrix_csv(&self) -> String {
        ...
        out.push_str(
            "consumer_surface,qualification,owner,ai_policy_hook_entries,ai_policy_result_entries,degrade_reasons,downgrade_triggers\n",
        );
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                ...
                row.ai_policy_hook_entries.len(),
                row.ai_policy_result_entries.len(),
                degrades.join("|"),
    ...
    pub fn render_markdown_summary(&self) -> String {
        let mut out = String::new();
        out.push_str("# M5 AI-Policy-Hook and AI-Policy-Result Registries\n\n");
        ...
        for row in &self.registry_rows {
            ...
            out.push_str(&format!(
                "  - AI-policy-hook entries: {} / AI-policy-result entries: {}\n",
                row.ai_policy_hook_entries.len(),
                row.ai_policy_result_entries.len()
            ));
        }
    ...
    /// registry-bound line-ai_policy_hook entries are listed.
    pub fn render_ai_policy_hook_table(&self) -> String {
        let mut out = String::new();
        out.push_str(
        ...
        out.push_str("| --- | --- | --- | --- | --- | --- | --- |\n");
        for row in &self.registry_rows {
            for ex in &row.ai_policy_hook_entries {
                if !ex.is_clean() {
                    continue;
                }
                out.push_str(&format!(
...
#[derive(Debug)]
pub enum M5AiPolicyHookAndResultRegistriesArtifactError {
    /// Support export failed to parse.
    SupportExport(serde_json::Error),
    /// Support export failed validation.
    Validation(Vec<M5AiPolicyHookAndResultRegistriesViolation>),
}

impl fmt::Display for M5AiPolicyHookAndResultRegistriesArtifactError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SupportExport(error) => {
                write!(
                    formatter,
                    "m5 line-ai_policy_hook / line-downgrade-packet registries export parse failed: {error}"
                )
            }
            Self::Validation(violations) => {
                ...
                write!(
                    formatter,
                    "m5 line-ai_policy_hook / line-downgrade-packet registries export failed validation: {tokens}"
                )
            }
        }
    }
...
impl Error for M5AiPolicyHookAndResultRegistriesArtifactError {}

/// Validation failures emitted by [`M5AiPolicyHookAndResultRegistriesPacket::validate`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum M5AiPolicyHookAndResultRegistriesViolation {
    ...
    /// A registry row does not point at both the line-ai_policy_hook and line-downgrade-packet domain schemas.
    ...
    /// Cohort-ai_policy_hook-resolution is not proven: clean ai_policy_hook entries do not cover the canonical line
    /// journeys or the first release-center / shiproom / executive-steering / program-governance / support
    /// surfaces, no object-incomplete example degrades, or a clean ai_policy_hook entry published an incomplete
    /// object.
    ...
    /// example degrade, no clean bounded ai_policy_hook entry is present, or a clean ai_policy_hook entry is unbounded
    /// or unbound.
...
impl M5AiPolicyHookAndResultRegistriesViolation {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::CohortDescriptorResolutionNotProven => "ai_policy_hook_resolution_not_proven",
            Self::RollbackAndDiagnosticsPreservationNotProven => {
                "rollback_and_diagnostics_preservation_not_proven"
            }
            Self::CohortEvidenceIntegrityNotProven => "ai_policy_result_integrity_not_proven",
...
pub fn current_stable_m5_ai_policy_hook_and_result_registries_export(
) -> Result<M5AiPolicyHookAndResultRegistriesPacket, M5AiPolicyHookAndResultRegistriesArtifactError>
{
    let packet: M5AiPolicyHookAndResultRegistriesPacket = serde_json::from_str(include_str!(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../artifacts/review/m5-ai-policy-hook-and-result-registries-proof/support_export.json"
        )
    ))
    .map_err(M5AiPolicyHookAndResultRegistriesArtifactError::SupportExport)?;
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(packet)
    } else {
        Err(M5AiPolicyHookAndResultRegistriesArtifactError::Validation(
            violations,
        ))
    }
}
...
fn validate_source_contracts(
    ...
    violations: &mut Vec<M5AiPolicyHookAndResultRegistriesViolation>,
) {
    ...
    for required in [
        ...
        if !refs.contains(required) {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::MissingSourceContracts);
            return;
        }
    }
}
...
fn validate_registry_rows(
    ...
    if packet.registry_rows.is_empty() {
        violations.push(M5AiPolicyHookAndResultRegistriesViolation::NoRegistryRows);
        return;
    }
    for row in &packet.registry_rows {
        ...
        {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::RegistryRowIncomplete);
        }
        if !row.declares_mandatory_anatomy() {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::MandatoryAnatomyMissing);
        }
        if !row.declares_mandatory_export_fields() {
            violations
                .push(M5AiPolicyHookAndResultRegistriesViolation::MandatoryExportFieldMissing);
        }
        let refs: BTreeSet<&str> = row
            ...
            .map(String::as_str)
            .collect();
        if !refs.contains(M5_AI_POLICY_HOOK_DOMAIN_SCHEMA_REF)
            || !refs.contains(M5_AI_POLICY_RESULT_DOMAIN_SCHEMA_REF)
        {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::DomainSchemaRefMissing);
        }
        if row.ai_policy_hook_entries.is_empty() || row.ai_policy_result_entries.is_empty() {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::ExamplesMissing);
        }
        if !row.examples_are_honest() {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::DishonestExample);
        }
        if !row.honours_invariants() {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::RowInvariantViolated);
        }
    }
}
...
fn validate_governance_review(
    ...
    for ok in [
        review.ai_policy_hook_registry_names_token_role_and_journey,
        review.line_resolves_to_typed_ai_policy_hook_from_shared_registry,
        review.repo_bundle_toolchain_and_deployment_rows_published,
        review.lines_cannot_widen_without_rollback_and_diagnostics,
        review.ai_policy_result_keeps_proof_visible_and_binds_support_language,
        ...
        review.ai_policy_hook_or_downgrade_drift_caught_before_release,
        review.every_row_declares_mandatory_anatomy,
        review.reuses_frozen_matrix_vocabulary,
    ] {
        if !ok {
            violations.push(M5AiPolicyHookAndResultRegistriesViolation::GovernanceReviewIncomplete);
            return;
        }
    }
}
...
fn validate_consumer_projection(
    ...
    for ok in [
        ...
        if !ok {
            violations
                .push(M5AiPolicyHookAndResultRegistriesViolation::ConsumerProjectionIncomplete);
            return;
        }
    }
}
...
fn validate_proof_freshness(
    ...
    {
        violations.push(M5AiPolicyHookAndResultRegistriesViolation::ProofFreshnessIncomplete);
    }
}
...
fn validate_release_posture(
    ...
    {
        violations.push(M5AiPolicyHookAndResultRegistriesViolation::ReleasePostureIncomplete);
    }
}
...
fn validate_acceptance_criteria(
    ...
    let ai_policy_hooks = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.ai_policy_hook_entries.iter())
    };
    let downgrade = || {
        packet
            .registry_rows
            .iter()
            .flat_map(|row| row.ai_policy_result_entries.iter())
    };

    // AC1: every active line can be inspected by exact rows, bundles, toolchains, and deployment profiles.
    // Clean ai_policy_hook entries cover the canonical line journeys and the first release-center / shiproom /
    // executive-steering / program-governance / support surfaces, an object-incomplete example degrades, and no
    // clean ai_policy_hook entry published an incomplete object.
    let clean_journeys: BTreeSet<String> = ai_policy_hooks()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.report_section.clone())
        .collect();
    let clean_surfaces: BTreeSet<String> = ai_policy_hooks()
        .filter(|ex| ex.is_clean())
        .map(|ex| ex.surface_context.clone())
        .collect();
    let journeys_covered = M5AiPolicyHookKind::CANONICAL_JOURNEYS
        .iter()
        .all(|k| clean_journeys.contains(k.as_str()));
    let first_surfaces_covered = M5AiPolicyHookSurfaceContext::FIRST_CONSUMERS
        .iter()
        .all(|s| clean_surfaces.contains(s.as_str()));
    let object_incomplete_degrades = ai_policy_hooks().any(|ex| {
        ex.degrade_reason
            == Some(M5AiPolicyHookEntryDegradeReason::CohortDescriptorObjectIncomplete)
    });
    let no_clean_incomplete =
        !ai_policy_hooks().any(|ex| ex.is_clean() && !ex.ai_policy_hook_object_complete);
    if !(journeys_covered
    ...
    {
        violations
            .push(M5AiPolicyHookAndResultRegistriesViolation::CohortDescriptorResolutionNotProven);
    }

    // AC2: line packets preserve rollback and diagnostics posture before widening. A widen-without-rollback
    // example degrades, an unbound example degrades, at least one clean bounded ai_policy_hook entry is present, and
    // no clean ai_policy_hook entry is unbounded or unbound.
    let widen_fold_degrades = ai_policy_hooks().any(|ex| {
        ex.degrade_reason
            == Some(
                M5AiPolicyHookEntryDegradeReason::DescriptorLetsCohortWidenWithoutRollbackOrRunsSupportAheadOfProof,
            )
    });
    let unbound_degrades = ai_policy_hooks().any(|ex| {
        ex.degrade_reason == Some(M5AiPolicyHookEntryDegradeReason::DescriptorNotBoundToRegistry)
    });
    let bounded_clean_ai_policy_hook =
        ai_policy_hooks().any(|ex| ex.is_clean() && ex.rollback_and_diagnostics_bounded);
    let no_clean_unbound = !ai_policy_hooks().any(|ex| ex.is_clean() && !ex.bound_to_registry);
    let no_clean_unbounded =
        !ai_policy_hooks().any(|ex| ex.is_clean() && !ex.rollback_and_diagnostics_bounded);
    if !(widen_fold_degrades
        && unbound_degrades
        && bounded_clean_ai_policy_hook
        && no_clean_unbound
        && no_clean_unbounded)
    {
        violations.push(
    ...
    let clean_comparison_scopes: BTreeSet<String> = downgrade()
        ...
        .collect();
    let comparison_scopes_covered = M5AiPolicyResultScope::CANONICAL_SCOPES
        .iter()
    ...
    let no_clean_missing_downgrade =
        !downgrade().any(|ex| ex.is_clean() && !ex.provides_complete_ai_policy_result);
    ...
    {
        violations
            .push(M5AiPolicyHookAndResultRegistriesViolation::CohortEvidenceIntegrityNotProven);
    }
}
...
/// The launch-bearing lines this lane implements, for downstream reference: the line-ai_policy_hook registry
