...
fn clean_review_pack_record_input() -> M5ReadyForReviewHandoffEntryResolutionInput {
    M5ReadyForReviewHandoffEntryResolutionInput {
        ...
        report_section: M5ReadyForReviewHandoffKind::ChangedFilesScope,
        surface_context: M5ReadyForReviewHandoffSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5ReadyForReviewHandoffResolutionForm::ALL.to_vec(),
...
fn clean_downgrade_input() -> M5ReadyForReviewRelationEntryResolutionInput {
    M5ReadyForReviewRelationEntryResolutionInput {
        ...
        semantic_role: M5ChangeIntentRole::ProviderOwnershipDisclosure,
        comparison_scope: M5ReadyForReviewRelationScope::EvaluatedScopeBinding,
        surface_context: M5ReadyForReviewHandoffSurfaceContext::ShiproomSurface,
        resolution_form_coverage: M5ReadyForReviewHandoffResolutionForm::ALL.to_vec(),
...
fn seeded_registries_validates() {
    let packet = seeded_m5_ready_for_review_handoff_registries();
...
fn review_pack_record_clean_names_meaning_and_is_bound() {
    ...
    assert_eq!(
        ...
        M5ReadyForReviewHandoffNextAction::ExpandCohortMeaning
    );
}
...
fn review_pack_record_unbound_and_unclassified_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5ReadyForReviewHandoffKind::PackScopeUnclassified;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_object_incomplete_and_widen_fold_and_form_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.resolution_form_coverage = vec![M5ReadyForReviewHandoffResolutionForm::CanonicalObject];
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_public_facing_and_surface_and_proof_degrade() {
    ...
    let mut input = clean_review_pack_record_input();
    input.report_section = M5ReadyForReviewHandoffKind::FullTreeScope;
    ...
    let mut input = clean_review_pack_record_input();
    input.surface_context = M5ReadyForReviewHandoffSurfaceContext::ContextUnknown;
    assert_eq!(
        resolve_review_pack_record_entry(input)
            .unwrap()
            .degrade_reason,
...
fn review_pack_record_empty_id_and_forbidden_material_error() {
    ...
    assert_eq!(
        ...
        M5ReadyForReviewHandoffResolutionError::EmptyCohortDescriptorEntryId
    );
    ...
    assert_eq!(
        resolve_review_pack_record_entry(input).unwrap_err(),
        M5ReadyForReviewHandoffResolutionError::ForbiddenMaterial
    );
}
...
fn line_preserves_rollback_and_diagnostics_rejects_unpreserved() {
    ...
    assert!(!line_preserves_rollback_and_diagnostics_before_widening(
        M5ReadyForReviewHandoffKind::PackScopeUnclassified,
        true,
        false,
        true
    ));
...
fn downgrade_support_ahead_and_unclassified_degrade() {
    ...
    let mut input = clean_downgrade_input();
    input.comparison_scope = M5ReadyForReviewRelationScope::ChangeIntentResultUnclassified;
    assert_eq!(
...
fn downgrade_form_and_surface_and_id_and_material() {
    ...
    assert_eq!(
        ...
        M5ReadyForReviewHandoffResolutionError::EmptyCohortEvidencePacketEntryId
    );
...
fn vocabulary_set_is_canonical() {
    assert!(seeded_m5_ready_for_review_handoff_registries()
        .vocabulary_set
        .matches_canonical());
}
...
fn vocabulary_set_drift_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::VocabularySetDrift));
}
...
fn missing_source_contracts_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::MissingSourceContracts));
}
...
fn domain_schema_ref_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::DomainSchemaRefMissing));

    let mut packet = seeded_m5_ready_for_review_handoff_registries();
    packet.registry_rows[0]
        .source_contract_refs
        .retain(|r| r != M5_READY_FOR_REVIEW_HANDOFF_DOMAIN_SCHEMA_REF);
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::DomainSchemaRefMissing));
}
...
fn mandatory_anatomy_missing_fails() {
    ...
    packet.registry_rows[0]
        .anatomy_parts
        .retain(|p| *p != M5ReadyForReviewHandoffAnatomyPart::Identity);
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::MandatoryAnatomyMissing));
}
...
fn mandatory_export_field_missing_fails() {
    ...
    packet.registry_rows[0]
        .export_fields
        .retain(|f| *f != M5ReadyForReviewHandoffExportField::CohortArchetypes);
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::MandatoryExportFieldMissing));
}
...
fn examples_missing_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::ExamplesMissing));
}
...
fn dishonest_clean_example_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::DishonestExample));
}
...
fn row_invariant_violation_fails() {
    for mutate in 0u8..4 {
        ...
        assert!(packet
            .validate()
            .contains(&M5ReadyForReviewHandoffRegistriesViolation::RowInvariantViolated));
    }
}
...
fn review_pack_record_not_proven_when_incomplete_example_removed() {
    ...
    assert!(packet.validate().contains(
        &M5ReadyForReviewHandoffRegistriesViolation::CohortDescriptorResolutionNotProven
    ));
}
...
fn review_pack_result_integrity_not_proven_when_support_ahead_example_removed() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::CohortEvidenceIntegrityNotProven));
}
...
fn governance_review_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::GovernanceReviewIncomplete));
}
...
fn consumer_projection_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::ConsumerProjectionIncomplete));
}
...
fn proof_freshness_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::ProofFreshnessIncomplete));
}
...
fn release_posture_incomplete_fails() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::ReleasePostureIncomplete));
}
...
fn injected_raw_material_is_rejected() {
    ...
    assert!(packet
        .validate()
        .contains(&M5ReadyForReviewHandoffRegistriesViolation::RawMaterialInExport));
}

#[test]
fn export_carries_no_forbidden_raw_material() {
    let json = seeded_m5_ready_for_review_handoff_registries().export_safe_json();
...
fn checked_support_export_validates_and_matches_seed() {
    let from_disk = current_stable_m5_ready_for_review_handoff_registries_export().expect(
...
fn narrowed_variants_validate_and_keep_rows_visible() {
    let beta = seeded_m5_ready_for_review_handoff_registries_review_pack_record_beta_narrowed();
...
fn checked_narrowed_fixtures_validate_and_match_seed_builders() {
    let beta: M5ReadyForReviewHandoffRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-ready-for-review-handoff-registries/ready_for_review_handoff_beta_narrowed.json"
    )))
    ...
    let preview: M5ReadyForReviewHandoffRegistriesPacket = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../fixtures/teamwork/m5-ready-for-review-handoff-registries/ready_for_review_relation_preview_narrowed.json"
    )))
