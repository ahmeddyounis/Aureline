//! Protected fixture checks for the M5 lifecycle-object certification capstone.
//!
//! The integration test replays the JSON fixtures under
//! `fixtures/state/m5-lifecycle-object-certification/` through the Rust types and asserts the
...
//! `artifacts/lifecycle/m5-lifecycle-object-certification.md`, the published packet under
//! `artifacts/release/m5-lifecycle-object-certification-proof/packet.json`, the published dashboard
//! under `artifacts/release/m5-lifecycle-object-certification-proof/dashboard.json`, and the
//! published CSV under `artifacts/release/m5-lifecycle-object-certification-proof/matrix.csv` are
//! asserted bit-for-bit equal to the rendering, so the headless emitter remains the only
//! mint-from-truth path.

use std::path::{Path, PathBuf};

use aureline_shell::m5_lifecycle_object_certification::{
    seeded_m5_lifecycle_object_certification_packet,
    validate_m5_lifecycle_object_certification_packet, LifecycleObjectDashboard,
    LifecycleObjectPacket, LifecycleObjectStatus, LifecycleObjectSupportExport,
    M5_LIFECYCLE_OBJECT_CERTIFICATION_PACKET_RECORD_KIND,
    M5_LIFECYCLE_OBJECT_CERTIFICATION_PUBLISHED_REPORT_REF,
    M5_LIFECYCLE_OBJECT_CERTIFICATION_SHARED_CONTRACT_REF, REQUIRED_OBJECT_FAMILIES,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../fixtures/state/m5-lifecycle-object-certification")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    ...
    assert_eq!(
        ...
        M5_LIFECYCLE_OBJECT_CERTIFICATION_SHARED_CONTRACT_REF
    );
    assert_eq!(
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: LifecycleObjectPacket = load_json("packet.json");
    let on_disk: LifecycleObjectDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn fixture_compact_lines_match_seed() {
    ...
    assert_eq!(
        ...
        "fixture compact.txt diverged from seeded compact lines -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_lifecycle_object_certification -- compact`",
    );
}
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/lifecycle/m5-lifecycle-object-certification.md"),
    )
    .expect("published m5-lifecycle-object-certification.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    assert_eq!(
        ...
        "published packet diverged from seed -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_lifecycle_object_certification -- packet`",
    );
}
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root()
            .join("artifacts/release/m5-lifecycle-object-certification-proof/dashboard.json"),
    )
    ...
    assert_eq!(
        ...
        "published dashboard diverged from seeded projection -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_lifecycle_object_certification -- dashboard`",
    );
}
...
fn published_csv_matches_seeded_rendering() {
    ...
    assert_eq!(
        ...
        "published CSV diverged from seeded rendering -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_lifecycle_object_certification -- csv`",
    );
}
...
fn published_doc_links_artifacts_and_quotes_object_families() {
    let body = std::fs::read_to_string(
        repo_root().join("docs/lifecycle/m5_lifecycle_object_certification_contract.md"),
    )
