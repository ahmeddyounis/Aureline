//! Protected fixture checks for the M5 ambient-instrumentation stability capstone.
//!
//! The integration test replays the JSON fixtures under
//! `fixtures/ui/m5-ambient-instrumentation-stability/` through the Rust types and asserts
//! the contract invariants. The packet, dashboard, and support-export fixtures are also
//! asserted bit-for-bit equal to the records minted by the seed, and the published markdown
//! report under `artifacts/shell/m5-ambient-instrumentation-stability.md`, the published
//! packet under `artifacts/release/m5-ambient-instrumentation-stability-proof/packet.json`,
//! the published dashboard under
//! `artifacts/release/m5-ambient-instrumentation-stability-proof/dashboard.json`, and the
//! published CSV under
//! `artifacts/release/m5-ambient-instrumentation-stability-proof/matrix.csv` are asserted
...
use aureline_shell::m5_ambient_instrumentation_stability::{
    ...
    validate_m5_ambient_instrumentation_stability_packet, AmbientStabilityDashboard,
    AmbientStabilityPacket, AmbientStabilityStatus, AmbientStabilitySupportExport,
    M5AmbientStabilityProfile, M5_AMBIENT_INSTRUMENTATION_STABILITY_PACKET_RECORD_KIND,
    M5_AMBIENT_INSTRUMENTATION_STABILITY_PUBLISHED_REPORT_REF,
    M5_AMBIENT_INSTRUMENTATION_STABILITY_SHARED_CONTRACT_REF,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../fixtures/ui/m5-ambient-instrumentation-stability")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    let on_disk: AmbientStabilityPacket = load_json("packet.json");
    let seeded = seeded_m5_ambient_instrumentation_stability_packet();
    assert_eq!(
        on_disk, seeded,
    ...
    assert_eq!(
        seeded.record_kind,
        M5_AMBIENT_INSTRUMENTATION_STABILITY_PACKET_RECORD_KIND
    );
    assert_eq!(
...
fn fixture_packet_passes_validation_and_is_clean() {
    let packet: AmbientStabilityPacket = load_json("packet.json");
...
#[test]
fn fixture_every_row_keeps_complete_classes_behaviors_and_labels() {
    let packet: AmbientStabilityPacket = load_json("packet.json");
    for row in &packet.rows {
        assert!(
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: AmbientStabilityPacket = load_json("packet.json");
    let on_disk: AmbientStabilityDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn fixture_support_export_quotes_packet_and_waiver_refs() {
    let packet: AmbientStabilityPacket = load_json("packet.json");
    let export: AmbientStabilitySupportExport = load_json("support_export.json");
    let expected = AmbientStabilitySupportExport::from_packet(
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/shell/m5-ambient-instrumentation-stability.md"),
    )
    .expect("published m5-ambient-instrumentation-stability.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root()
            .join("artifacts/release/m5-ambient-instrumentation-stability-proof/packet.json"),
    )
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root()
            .join("artifacts/release/m5-ambient-instrumentation-stability-proof/dashboard.json"),
    )
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-ambient-instrumentation-stability-proof/matrix.csv"),
    )
...
fn published_doc_links_artifacts_and_quotes_profiles() {
    let body = std::fs::read_to_string(
        repo_root().join("docs/shell/m5_ambient_instrumentation_stability_contract.md"),
    )
    .expect("published ambient-instrumentation-stability contract must exist");
    assert!(body.contains("artifacts/shell/m5-ambient-instrumentation-stability.md"));
    assert!(
        body.contains("artifacts/release/m5-ambient-instrumentation-stability-proof/packet.json")
    );
    assert!(body
        .contains("artifacts/release/m5-ambient-instrumentation-stability-proof/dashboard.json"));
    assert!(body.contains("fixtures/ui/m5-ambient-instrumentation-stability/packet.json"));
    assert!(body.contains("schemas/shell/m5-ambient-instrumentation-stability.schema.json"));
    for profile in M5AmbientStabilityProfile::ALL {
        assert!(
