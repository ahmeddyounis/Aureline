//! Protected fixture checks for the M5 advisory-component release-proof capstone.
//!
//! The integration test replays the JSON fixtures under
//! `fixtures/security/m5-advisory-component-release-proof/` through the Rust types and asserts the
...
//! `artifacts/security/m5-advisory-component-release-proof.md`, the published packet under
//! `artifacts/release/m5-advisory-component-release-proof/packet.json`, the published dashboard under
//! `artifacts/release/m5-advisory-component-release-proof/dashboard.json`, and the published CSV under
//! `artifacts/release/m5-advisory-component-release-proof/matrix.csv` are asserted bit-for-bit equal
//! to the rendering, so the headless emitter remains the only mint-from-truth path.

use std::path::{Path, PathBuf};

use aureline_shell::m5_advisory_component_release_proof::{
    seeded_m5_advisory_component_release_proof_packet,
    validate_m5_advisory_component_release_proof_packet, AdvisoryReleaseProofDashboard,
    AdvisoryReleaseProofPacket, AdvisoryReleaseProofStatus, AdvisoryReleaseProofSupportExport,
    M5AdvisoryComponentFamily, M5_ADVISORY_RELEASE_PROOF_PACKET_RECORD_KIND,
    M5_ADVISORY_RELEASE_PROOF_PUBLISHED_REPORT_REF, M5_ADVISORY_RELEASE_PROOF_SHARED_CONTRACT_REF,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../fixtures/security/m5-advisory-component-release-proof")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    let on_disk: AdvisoryReleaseProofPacket = load_json("packet.json");
    let seeded = seeded_m5_advisory_component_release_proof_packet();
    assert_eq!(
        on_disk, seeded,
    ...
    assert_eq!(
        seeded.record_kind,
        M5_ADVISORY_RELEASE_PROOF_PACKET_RECORD_KIND
    );
    assert_eq!(
        seeded.shared_contract_ref,
        M5_ADVISORY_RELEASE_PROOF_SHARED_CONTRACT_REF
    );
    assert_eq!(
        seeded.published_report_ref,
        M5_ADVISORY_RELEASE_PROOF_PUBLISHED_REPORT_REF
    );
}

#[test]
fn fixture_packet_passes_validation_and_is_clean() {
    let packet: AdvisoryReleaseProofPacket = load_json("packet.json");
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: AdvisoryReleaseProofPacket = load_json("packet.json");
    let on_disk: AdvisoryReleaseProofDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn fixture_support_export_quotes_packet_and_waiver_refs() {
    let packet: AdvisoryReleaseProofPacket = load_json("packet.json");
    let export: AdvisoryReleaseProofSupportExport = load_json("support_export.json");
    let expected = AdvisoryReleaseProofSupportExport::from_packet(
...
fn fixture_compact_lines_match_seed() {
    ...
    assert_eq!(
        ...
        "fixture compact.txt diverged from seeded compact lines -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_advisory_component_release_proof -- compact`",
    );
}
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/security/m5-advisory-component-release-proof.md"),
    )
    .expect("published m5-advisory-component-release-proof.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-advisory-component-release-proof/packet.json"),
    )
    ...
    assert_eq!(
        ...
        "published packet diverged from seed -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_advisory_component_release_proof -- packet`",
    );
}
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-advisory-component-release-proof/dashboard.json"),
    )
    ...
    assert_eq!(
        ...
        "published dashboard diverged from seeded projection -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_advisory_component_release_proof -- dashboard`",
    );
}
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-advisory-component-release-proof/matrix.csv"),
    )
    .expect("published matrix.csv must exist");
    assert_eq!(
        on_disk, rendered,
        "published CSV diverged from seeded rendering -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_advisory_component_release_proof -- csv`",
    );
}
...
fn published_doc_links_artifacts_and_quotes_families() {
    let body = std::fs::read_to_string(
        repo_root().join("docs/security/m5_advisory_component_release_proof_contract.md"),
    )
    .expect("published advisory-component-release-proof contract must exist");
    assert!(body.contains("artifacts/security/m5-advisory-component-release-proof.md"));
    assert!(body.contains("artifacts/release/m5-advisory-component-release-proof/packet.json"));
    assert!(body.contains("artifacts/release/m5-advisory-component-release-proof/dashboard.json"));
    assert!(body.contains("fixtures/security/m5-advisory-component-release-proof/packet.json"));
    assert!(body.contains("schemas/security/m5-advisory-component-release-proof.schema.json"));
    for family in M5AdvisoryComponentFamily::ALL {
        assert!(
