//! Protected fixture checks for the M5 shell-primitive release-proof capstone.
//!
//! The integration test replays the JSON fixtures under
//! `fixtures/ui/m5-shell-primitive-release-proof/` through the Rust types and asserts the contract
...
//! `artifacts/shell/m5-shell-primitive-release-proof.md`, the published packet under
//! `artifacts/release/m5-shell-primitive-release-proof-proof/packet.json`, the published dashboard
//! under `artifacts/release/m5-shell-primitive-release-proof-proof/dashboard.json`, and the
//! published CSV under `artifacts/release/m5-shell-primitive-release-proof-proof/matrix.csv` are
//! asserted bit-for-bit equal to the rendering, so the headless emitter remains the only
//! mint-from-truth path.

use std::path::{Path, PathBuf};

use aureline_shell::m5_shell_primitive_release_proof::{
    seeded_m5_shell_primitive_release_proof_packet,
    validate_m5_shell_primitive_release_proof_packet, M5ShellPrimitiveFamily,
    ShellPrimitiveReleaseDashboard, ShellPrimitiveReleasePacket, ShellPrimitiveReleaseStatus,
    ShellPrimitiveReleaseSupportExport, M5_SHELL_PRIMITIVE_RELEASE_PROOF_PACKET_RECORD_KIND,
    M5_SHELL_PRIMITIVE_RELEASE_PROOF_PUBLISHED_REPORT_REF,
    M5_SHELL_PRIMITIVE_RELEASE_PROOF_SHARED_CONTRACT_REF,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/ui/m5-shell-primitive-release-proof")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    let on_disk: ShellPrimitiveReleasePacket = load_json("packet.json");
    let seeded = seeded_m5_shell_primitive_release_proof_packet();
    assert_eq!(
        on_disk, seeded,
    ...
    assert_eq!(
        seeded.record_kind,
        M5_SHELL_PRIMITIVE_RELEASE_PROOF_PACKET_RECORD_KIND
    );
    assert_eq!(
...
fn fixture_packet_passes_validation_and_is_clean() {
    let packet: ShellPrimitiveReleasePacket = load_json("packet.json");
...
#[test]
fn fixture_packet_covers_every_family_and_pillar() {
    let packet: ShellPrimitiveReleasePacket = load_json("packet.json");
    ...
    assert_eq!(packet.covered_truth_pillars.len(), 4);
}
...
#[test]
fn fixture_every_row_keeps_complete_routes_labels_profiles_and_invariant() {
    let packet: ShellPrimitiveReleasePacket = load_json("packet.json");
    for row in &packet.rows {
        assert!(
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: ShellPrimitiveReleasePacket = load_json("packet.json");
    let on_disk: ShellPrimitiveReleaseDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn fixture_support_export_quotes_packet_and_waiver_refs() {
    let packet: ShellPrimitiveReleasePacket = load_json("packet.json");
    let export: ShellPrimitiveReleaseSupportExport = load_json("support_export.json");
    let expected = ShellPrimitiveReleaseSupportExport::from_packet(
...
fn fixture_compact_lines_match_seed() {
    ...
    assert_eq!(
        ...
        "fixture compact.txt diverged from seeded compact lines -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_shell_primitive_release_proof -- compact`",
    );
}
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/shell/m5-shell-primitive-release-proof.md"),
    )
    .expect("published m5-shell-primitive-release-proof.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-shell-primitive-release-proof-proof/packet.json"),
    )
    ...
    assert_eq!(
        ...
        "published packet diverged from seed -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_shell_primitive_release_proof -- packet`",
    );
}
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-shell-primitive-release-proof-proof/dashboard.json"),
    )
    ...
    assert_eq!(
        ...
        "published dashboard diverged from seeded projection -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_shell_primitive_release_proof -- dashboard`",
    );
}
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-shell-primitive-release-proof-proof/matrix.csv"),
    )
    .expect("published matrix.csv must exist");
    assert_eq!(
        on_disk, rendered,
        "published CSV diverged from seeded rendering -- regenerate with \
         `cargo run -q -p aureline-shell --bin aureline_shell_m5_shell_primitive_release_proof -- csv`",
    );
}
...
fn published_doc_links_artifacts_and_quotes_families() {
    let body = std::fs::read_to_string(
        repo_root().join("docs/shell/m5_shell_primitive_release_proof_contract.md"),
    )
    .expect("published shell-primitive-release-proof contract must exist");
    assert!(body.contains("artifacts/shell/m5-shell-primitive-release-proof.md"));
    assert!(body.contains("artifacts/release/m5-shell-primitive-release-proof-proof/packet.json"));
    assert!(
        body.contains("artifacts/release/m5-shell-primitive-release-proof-proof/dashboard.json")
    );
    assert!(body.contains("fixtures/ui/m5-shell-primitive-release-proof/packet.json"));
    assert!(body.contains("schemas/shell/m5-shell-primitive-release-proof.schema.json"));
    for family in M5ShellPrimitiveFamily::ALL {
        assert!(
