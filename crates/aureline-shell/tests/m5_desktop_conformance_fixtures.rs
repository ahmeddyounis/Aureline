//! Protected fixture checks for the M5 desktop-qualification audit.
//!
//! The integration test replays every JSON fixture under
//! `fixtures/platform/m5_depth_surfaces/` through the Rust types and asserts
//! the contract invariants. The report fixture is also asserted bit-for-bit
//! equal to the audit minted by `seeded_m5_desktop_qualification_audit`, and
//! the markdown artifact under
//! `artifacts/ux/m5/desktop-conformance/m5_desktop_conformance_audit.md` is
...
use aureline_shell::m5_desktop_conformance::{
    ...
    M5DesktopQualificationReport, M5DesktopQualificationStatus, M5DesktopRow,
    M5DesktopSupportExport, M5_DESKTOP_PUBLISHED_REPORT_REF, M5_DESKTOP_REPORT_RECORD_KIND,
    M5_DESKTOP_SHARED_CONTRACT_REF,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/platform/m5_depth_surfaces")
}

fn artifacts_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../artifacts/ux/m5/desktop-conformance")
}
...
fn fixture_report_is_bit_for_bit_equal_to_seed() {
    let on_disk: M5DesktopQualificationReport = load_json("report.json");
    let seeded = seeded_m5_desktop_qualification_audit();
    assert_eq!(on_disk, seeded, "fixture report diverged from seeded audit");
    assert_eq!(seeded.record_kind, M5_DESKTOP_REPORT_RECORD_KIND);
    assert_eq!(seeded.shared_contract_ref, M5_DESKTOP_SHARED_CONTRACT_REF);
    assert_eq!(seeded.published_report_ref, M5_DESKTOP_PUBLISHED_REPORT_REF);
}

#[test]
fn fixture_report_passes_validation() {
    let report: M5DesktopQualificationReport = load_json("report.json");
    validate_m5_desktop_qualification(&report).expect("fixture report must validate");
...
fn fixture_support_export_quotes_report_and_case_ids() {
    let report: M5DesktopQualificationReport = load_json("report.json");
    let export: M5DesktopSupportExport = load_json("support_export.json");
    let expected =
        M5DesktopSupportExport::from_report(export.support_export_id.clone(), report.clone());
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(artifacts_root().join("m5_desktop_conformance_audit.md"))
        .expect("published m5_desktop_conformance_audit.md must exist");
    assert_eq!(
        on_disk, rendered,
        "published m5_desktop_conformance_audit.md diverged from seeded rendering -- regenerate with \
...
fn published_doc_links_every_row_and_artifact() {
    let doc_path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../docs/m5/desktop-and-handoff-parity.md");
    let body = std::fs::read_to_string(&doc_path)
        .expect("published desktop-and-handoff-parity doc must exist");
    for required in M5DesktopRow::required_rows() {
        assert!(
    ...
    assert!(body.contains("artifacts/ux/m5/desktop-conformance/m5_desktop_conformance_audit.md"));
    assert!(body.contains("fixtures/platform/m5_depth_surfaces/report.json"));
    assert!(body.contains("schemas/platform/m5-surface-desktop-qualification.schema.json"));
    assert!(body.contains("tools/ci/m5/desktop_conformance_check.py"));
}
