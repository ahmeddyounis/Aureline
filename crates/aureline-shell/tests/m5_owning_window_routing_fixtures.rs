//! Protected fixture checks for the M5 owning-window routing capstone.
//!
//! The integration test replays the JSON fixtures under
//! `fixtures/ui/m5-owning-window-routing/` through the Rust types and asserts the contract
...
//! report under `artifacts/shell/m5-owning-window-routing.md`, the published packet under
//! `artifacts/release/m5-owning-window-routing-proof/packet.json`, the published dashboard
//! under `artifacts/release/m5-owning-window-routing-proof/dashboard.json`, and the
//! published CSV under `artifacts/release/m5-owning-window-routing-proof/matrix.csv` are
...
use aureline_shell::m5_owning_window_routing::{
    ...
    RoutingContinuityDashboard, RoutingContinuityPacket, RoutingContinuityStatus,
    RoutingContinuitySupportExport, M5_OWNING_WINDOW_ROUTING_PACKET_RECORD_KIND,
    M5_OWNING_WINDOW_ROUTING_PUBLISHED_REPORT_REF, M5_OWNING_WINDOW_ROUTING_SHARED_CONTRACT_REF,
    REQUIRED_FAMILIES,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/ui/m5-owning-window-routing")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    let on_disk: RoutingContinuityPacket = load_json("packet.json");
    let seeded = seeded_m5_owning_window_routing_packet();
    assert_eq!(
        on_disk, seeded,
    ...
    assert_eq!(
        seeded.record_kind,
        M5_OWNING_WINDOW_ROUTING_PACKET_RECORD_KIND
    );
    assert_eq!(
        seeded.shared_contract_ref,
        M5_OWNING_WINDOW_ROUTING_SHARED_CONTRACT_REF
    );
    assert_eq!(
        seeded.published_report_ref,
        M5_OWNING_WINDOW_ROUTING_PUBLISHED_REPORT_REF
    );
}

#[test]
fn fixture_packet_passes_validation_and_is_clean() {
    let packet: RoutingContinuityPacket = load_json("packet.json");
    validate_m5_owning_window_routing_packet(&packet).expect("fixture packet must validate");
...
#[test]
fn fixture_every_row_declares_routing_and_plans() {
    let packet: RoutingContinuityPacket = load_json("packet.json");
    for row in &packet.rows {
        assert!(
            row.routing_expectations_complete(),
            "row {} does not declare the full routing contract",
            row.family.as_str()
        );
        assert!(
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: RoutingContinuityPacket = load_json("packet.json");
    let on_disk: RoutingContinuityDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn fixture_support_export_quotes_packet_and_waiver_refs() {
    let packet: RoutingContinuityPacket = load_json("packet.json");
    let export: RoutingContinuitySupportExport = load_json("support_export.json");
    let expected = RoutingContinuitySupportExport::from_packet(
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk =
        std::fs::read_to_string(repo_root().join("artifacts/shell/m5-owning-window-routing.md"))
            .expect("published m5-owning-window-routing.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-owning-window-routing-proof/packet.json"),
    )
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-owning-window-routing-proof/dashboard.json"),
    )
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-owning-window-routing-proof/matrix.csv"),
    )
...
fn published_doc_links_artifacts_and_quotes_families() {
    ...
    assert!(body.contains("fixtures/ui/m5-owning-window-routing/packet.json"));
    assert!(body.contains("schemas/shell/m5-owning-window-routing.schema.json"));
    for family in REQUIRED_FAMILIES {
        assert!(
