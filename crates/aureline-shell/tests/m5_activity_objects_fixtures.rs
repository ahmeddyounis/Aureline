//! Protected fixture checks for the M5 durable activity-object audit.
//!
//! The integration test replays every JSON fixture under
//! `fixtures/ux/m5/activity-center/` through the Rust types and asserts the
//! contract invariants. The report fixture is also asserted bit-for-bit equal
//! to the audit minted by `seeded_m5_activity_objects_audit`, and the markdown
//! artifact under
...
use aureline_shell::m5_activity_objects::{
    seeded_m5_activity_objects_audit, validate_m5_activity_objects, M5ActivityObjectReport,
    M5ActivitySupportExport, M5DurableGuarantee, M5DurableStatus, M5_ACTIVITY_PUBLISHED_REPORT_REF,
    M5_ACTIVITY_REPORT_RECORD_KIND, M5_ACTIVITY_SHARED_CONTRACT_REF,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/ux/m5/activity-center")
}

fn artifacts_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../artifacts/ux/m5/durable-attention-packets")
}
...
fn fixture_report_is_bit_for_bit_equal_to_seed() {
    let on_disk: M5ActivityObjectReport = load_json("report.json");
    let seeded = seeded_m5_activity_objects_audit();
    assert_eq!(on_disk, seeded, "fixture report diverged from seeded audit");
    assert_eq!(seeded.record_kind, M5_ACTIVITY_REPORT_RECORD_KIND);
    assert_eq!(seeded.shared_contract_ref, M5_ACTIVITY_SHARED_CONTRACT_REF);
    assert_eq!(
        seeded.published_report_ref,
        M5_ACTIVITY_PUBLISHED_REPORT_REF
    );
}

#[test]
fn fixture_report_passes_validation() {
    let report: M5ActivityObjectReport = load_json("report.json");
    validate_m5_activity_objects(&report).expect("fixture report must validate");
...
fn fixture_report_qualifies_every_required_guarantee() {
    ...
    for required in M5DurableGuarantee::required_guarantees() {
...
fn fixture_support_export_quotes_report_and_case_ids() {
    let report: M5ActivityObjectReport = load_json("report.json");
    let export: M5ActivitySupportExport = load_json("support_export.json");
    let expected =
        M5ActivitySupportExport::from_report(export.support_export_id.clone(), report.clone());
    assert_eq!(export, expected);
    assert!(export.case_ids.contains(&report.report_id));
    for family in &report.rows {
        assert!(
            export.case_ids.contains(&family.descriptor.family_id),
            "support export must quote family id {}",
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(artifacts_root().join("m5_activity_objects_audit.md"))
        .expect("published m5_activity_objects_audit.md must exist");
    assert_eq!(
        on_disk, rendered,
        "published m5_activity_objects_audit.md diverged from seeded rendering -- regenerate with \
...
fn published_doc_links_every_guarantee_and_artifact() {
    let doc_path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../docs/m5/durable-progress-and-reopen.md");
    let body = std::fs::read_to_string(&doc_path)
        .expect("published durable-progress-and-reopen doc must exist");
    for required in M5DurableGuarantee::required_guarantees() {
        assert!(
    ...
    assert!(body.contains("artifacts/ux/m5/durable-attention-packets/m5_activity_objects_audit.md"));
    assert!(body.contains("fixtures/ux/m5/activity-center/report.json"));
    assert!(body.contains("schemas/ux/m5-activity-object.schema.json"));
    assert!(body.contains("tools/ci/m5/activity_objects_check.py"));
}
