//! Protected fixture checks for the M5 appearance-qualification audit.
//!
//! The integration test replays every JSON fixture under
//! `fixtures/ux/m5/dark-light-hc-density-reduced-motion/` through the Rust
//! types and asserts the contract invariants. The report fixture is also
//! asserted bit-for-bit equal to the audit minted by
//! `seeded_m5_appearance_qualification_audit`, and the markdown artifact
//! under `artifacts/ux/m5/appearance-parity/m5_appearance_parity_audit.md`
...
use aureline_shell::m5_appearance_parity::{
    ...
    M5AppearanceQualificationReport, M5AppearanceRow, M5AppearanceSupportExport,
    M5QualificationStatus, M5_APPEARANCE_PUBLISHED_REPORT_REF, M5_APPEARANCE_REPORT_RECORD_KIND,
    M5_APPEARANCE_SHARED_CONTRACT_REF,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../fixtures/ux/m5/dark-light-hc-density-reduced-motion")
}

fn artifacts_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../artifacts/ux/m5/appearance-parity")
}
...
fn fixture_report_is_bit_for_bit_equal_to_seed() {
    let on_disk: M5AppearanceQualificationReport = load_json("report.json");
    let seeded = seeded_m5_appearance_qualification_audit();
    assert_eq!(on_disk, seeded, "fixture report diverged from seeded audit");
    assert_eq!(seeded.record_kind, M5_APPEARANCE_REPORT_RECORD_KIND);
    assert_eq!(
    ...
    assert_eq!(
        ...
        M5_APPEARANCE_PUBLISHED_REPORT_REF
    );
}

#[test]
fn fixture_report_passes_validation() {
    let report: M5AppearanceQualificationReport = load_json("report.json");
    validate_m5_appearance_qualification(&report).expect("fixture report must validate");
...
#[test]
fn fixture_appearance_anchor_index_is_complete_and_non_empty() {
    let report: M5AppearanceQualificationReport = load_json("report.json");
    assert_eq!(report.appearance_anchor_index.len(), report.rows.len());
    for entry in &report.appearance_anchor_index {
        assert!(
            !entry.appearance_anchor_ref.trim().is_empty(),
            "appearance anchor for {} must be non-empty",
            entry.surface_id
        );
    }
}
...
fn fixture_support_export_quotes_report_and_case_ids() {
    let report: M5AppearanceQualificationReport = load_json("report.json");
    let export: M5AppearanceSupportExport = load_json("support_export.json");
    let expected =
        M5AppearanceSupportExport::from_report(export.support_export_id.clone(), report.clone());
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(artifacts_root().join("m5_appearance_parity_audit.md"))
        .expect("published m5_appearance_parity_audit.md must exist");
    assert_eq!(
        on_disk, rendered,
        "published m5_appearance_parity_audit.md diverged from seeded rendering -- regenerate with \
...
fn published_doc_links_every_row_and_artifact() {
    let doc_path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../docs/m5/appearance-and-density-parity.md");
    let body = std::fs::read_to_string(&doc_path)
        .expect("published appearance-and-density-parity doc must exist");
    for required in M5AppearanceRow::required_rows() {
        assert!(
    ...
    assert!(body.contains("artifacts/ux/m5/appearance-parity/m5_appearance_parity_audit.md"));
    assert!(body.contains("fixtures/ux/m5/dark-light-hc-density-reduced-motion/report.json"));
    assert!(body.contains("schemas/ux/m5-appearance-qualification.schema.json"));
    assert!(body.contains("tools/ci/m5/appearance_parity_check.py"));
}
