//! Protected fixture checks for the M5 window-lifecycle-safety capstone.
//!
//! The integration test replays the JSON fixtures under
//! `fixtures/ui/m5-window-lifecycle-safety/` through the Rust types and asserts the contract
//! invariants. The packet, dashboard, and support-export fixtures are also asserted
//! bit-for-bit equal to the records minted by the seed, and the published markdown report
//! under `artifacts/shell/m5-window-lifecycle-safety.md`, the published packet under
//! `artifacts/release/m5-window-lifecycle-safety-proof/packet.json`, the published dashboard
//! under `artifacts/release/m5-window-lifecycle-safety-proof/dashboard.json`, and the
//! published CSV under `artifacts/release/m5-window-lifecycle-safety-proof/matrix.csv` are
...
fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/ui/m5-window-lifecycle-safety")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    let on_disk: WindowLifecyclePacket = load_json("packet.json");
    let seeded = seeded_m5_window_lifecycle_safety_packet();
    assert_eq!(
        on_disk, seeded,
    ...
    assert_eq!(
        seeded.record_kind,
        M5_WINDOW_LIFECYCLE_SAFETY_PACKET_RECORD_KIND
    );
    assert_eq!(
        seeded.shared_contract_ref,
        M5_WINDOW_LIFECYCLE_SAFETY_SHARED_CONTRACT_REF
    );
    assert_eq!(
...
fn fixture_packet_passes_validation_and_is_clean() {
    ...
    validate_m5_window_lifecycle_safety_packet(&packet).expect("fixture packet must validate");
...
#[test]
fn fixture_every_row_declares_resources_and_plans() {
    let packet: WindowLifecyclePacket = load_json("packet.json");
    for row in &packet.rows {
        assert!(
            row.protected_resources_complete(),
            "row {} does not declare the full protected-resource set",
            row.family.as_str()
        );
        assert!(
        ...
        assert!(
            row.plans_disclose_before_drop() && row.plans_keyboard_reachable(),
            "row {} has an invalid per-verb drag plan",
            row.family.as_str()
        );
    }
}
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: WindowLifecyclePacket = load_json("packet.json");
    let on_disk: WindowLifecycleDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk =
        std::fs::read_to_string(repo_root().join("artifacts/shell/m5-window-lifecycle-safety.md"))
            .expect("published m5-window-lifecycle-safety.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-window-lifecycle-safety-proof/packet.json"),
    )
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-window-lifecycle-safety-proof/dashboard.json"),
    )
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-window-lifecycle-safety-proof/matrix.csv"),
    )
...
fn published_doc_links_artifacts_and_quotes_families() {
    let body = std::fs::read_to_string(
        repo_root().join("docs/shell/m5_window_lifecycle_safety_contract.md"),
    )
    .expect("published window-lifecycle-safety contract must exist");
    assert!(body.contains("artifacts/shell/m5-window-lifecycle-safety.md"));
    assert!(body.contains("artifacts/release/m5-window-lifecycle-safety-proof/packet.json"));
    assert!(body.contains("artifacts/release/m5-window-lifecycle-safety-proof/dashboard.json"));
    assert!(body.contains("fixtures/ui/m5-window-lifecycle-safety/packet.json"));
    assert!(body.contains("schemas/shell/m5-window-lifecycle-safety.schema.json"));
    for family in REQUIRED_FAMILIES {
        assert!(
