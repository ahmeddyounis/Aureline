//! Protected fixture checks for the M5 discoverability-access-parity certification.
//!
//! The integration test replays the JSON fixtures under `fixtures/commands/m5-discoverability-access-parity/`
...
//! markdown report under `artifacts/commands/m5-discoverability-access-parity.md`, the published packet
//! under `artifacts/release/m5-discoverability-access-parity-proof/packet.json`, the published dashboard
//! under `artifacts/release/m5-discoverability-access-parity-proof/dashboard.json`, and the published CSV
//! under `artifacts/release/m5-discoverability-access-parity-proof/matrix.csv` are asserted bit-for-bit
...
use aureline_shell::m5_discoverability_access_parity::{
    ...
    AccessParityDashboard, AccessParityPacket, AccessParityStatus, AccessParitySupportExport,
    M5_ACCESS_PARITY_PACKET_RECORD_KIND, M5_ACCESS_PARITY_PUBLISHED_REPORT_REF,
    M5_ACCESS_PARITY_SHARED_CONTRACT_REF, REQUIRED_SURFACE_FAMILIES,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../fixtures/commands/m5-discoverability-access-parity")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    let on_disk: AccessParityPacket = load_json("packet.json");
    let seeded = seeded_m5_discoverability_access_parity_packet();
    assert_eq!(
        on_disk, seeded,
        "fixture packet diverged from seeded packet"
    );
    assert_eq!(seeded.record_kind, M5_ACCESS_PARITY_PACKET_RECORD_KIND);
    assert_eq!(
        seeded.shared_contract_ref,
        M5_ACCESS_PARITY_SHARED_CONTRACT_REF
    );
    assert_eq!(
        seeded.published_report_ref,
        M5_ACCESS_PARITY_PUBLISHED_REPORT_REF
    );
}

#[test]
fn fixture_packet_passes_validation_and_is_clean() {
    let packet: AccessParityPacket = load_json("packet.json");
    validate_m5_access_parity_packet(&packet).expect("fixture packet must validate");
...
#[test]
fn fixture_every_row_certifies_channels_fields_profiles_and_consumer_surfaces() {
    let packet: AccessParityPacket = load_json("packet.json");
    for row in &packet.rows {
        assert!(
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: AccessParityPacket = load_json("packet.json");
    let on_disk: AccessParityDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn fixture_support_export_quotes_packet_and_waiver_refs() {
    let packet: AccessParityPacket = load_json("packet.json");
    let export: AccessParitySupportExport = load_json("support_export.json");
    let expected =
        AccessParitySupportExport::from_packet(export.support_export_id.clone(), packet.clone());
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/commands/m5-discoverability-access-parity.md"),
    )
    .expect("published m5-discoverability-access-parity.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-discoverability-access-parity-proof/packet.json"),
    )
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-discoverability-access-parity-proof/dashboard.json"),
    )
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-discoverability-access-parity-proof/matrix.csv"),
    )
...
fn published_doc_links_artifacts_and_quotes_families() {
    let body = std::fs::read_to_string(
        repo_root().join("docs/commands/m5_discoverability_access_parity_contract.md"),
    )
    .expect("published access-parity contract must exist");
    assert!(body.contains("artifacts/commands/m5-discoverability-access-parity.md"));
    assert!(body.contains("artifacts/release/m5-discoverability-access-parity-proof/packet.json"));
    assert!(
        body.contains("artifacts/release/m5-discoverability-access-parity-proof/dashboard.json")
    );
    assert!(body.contains("fixtures/commands/m5-discoverability-access-parity/packet.json"));
    assert!(body.contains("schemas/commands/m5-discoverability-access-parity.schema.json"));
    for family in REQUIRED_SURFACE_FAMILIES {
        assert!(
