//! Protected fixture checks for the M5 command-surface parity certification.
//!
//! The integration test replays the JSON fixtures under
//! `fixtures/commands/m5-command-surface-parity/` through the Rust types and asserts the contract
...
//! `artifacts/commands/m5-command-surface-parity.md`, the published packet under
//! `artifacts/release/m5-command-surface-parity-proof/packet.json`, the published dashboard under
//! `artifacts/release/m5-command-surface-parity-proof/dashboard.json`, and the published CSV under
//! `artifacts/release/m5-command-surface-parity-proof/matrix.csv` are asserted bit-for-bit equal to the
...
use aureline_shell::m5_command_surface_parity::{
    ...
    CommandSurfaceParityDashboard, CommandSurfaceParityPacket, CommandSurfaceParityStatus,
    CommandSurfaceParitySupportExport, M5_COMMAND_SURFACE_PARITY_PACKET_RECORD_KIND,
    M5_COMMAND_SURFACE_PARITY_PUBLISHED_REPORT_REF, M5_COMMAND_SURFACE_PARITY_SHARED_CONTRACT_REF,
    REQUIRED_SURFACE_FAMILIES,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/commands/m5-command-surface-parity")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    let on_disk: CommandSurfaceParityPacket = load_json("packet.json");
    let seeded = seeded_m5_command_surface_parity_packet();
    assert_eq!(
        on_disk, seeded,
    ...
    assert_eq!(
        seeded.record_kind,
        M5_COMMAND_SURFACE_PARITY_PACKET_RECORD_KIND
    );
    assert_eq!(
        seeded.shared_contract_ref,
        M5_COMMAND_SURFACE_PARITY_SHARED_CONTRACT_REF
    );
    assert_eq!(
        seeded.published_report_ref,
        M5_COMMAND_SURFACE_PARITY_PUBLISHED_REPORT_REF
    );
}

#[test]
fn fixture_packet_passes_validation_and_is_clean() {
    let packet: CommandSurfaceParityPacket = load_json("packet.json");
    validate_m5_command_surface_parity_packet(&packet).expect("fixture packet must validate");
...
#[test]
fn fixture_every_row_certifies_all_parity_surfaces_modes_and_consumer_surfaces() {
    let packet: CommandSurfaceParityPacket = load_json("packet.json");
    for row in &packet.rows {
        assert!(
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: CommandSurfaceParityPacket = load_json("packet.json");
    let on_disk: CommandSurfaceParityDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn fixture_support_export_quotes_packet_and_waiver_refs() {
    let packet: CommandSurfaceParityPacket = load_json("packet.json");
    let export: CommandSurfaceParitySupportExport = load_json("support_export.json");
    let expected = CommandSurfaceParitySupportExport::from_packet(
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/commands/m5-command-surface-parity.md"),
    )
    .expect("published m5-command-surface-parity.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-command-surface-parity-proof/packet.json"),
    )
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-command-surface-parity-proof/dashboard.json"),
    )
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-command-surface-parity-proof/matrix.csv"),
    )
...
fn published_doc_links_artifacts_and_quotes_families() {
    let body = std::fs::read_to_string(
        repo_root().join("docs/commands/m5_command_surface_parity_contract.md"),
    )
    .expect("published command-surface-parity contract must exist");
    assert!(body.contains("artifacts/commands/m5-command-surface-parity.md"));
    assert!(body.contains("artifacts/release/m5-command-surface-parity-proof/packet.json"));
    assert!(body.contains("artifacts/release/m5-command-surface-parity-proof/dashboard.json"));
    assert!(body.contains("fixtures/commands/m5-command-surface-parity/packet.json"));
    assert!(body.contains("schemas/commands/m5-command-surface-parity.schema.json"));
    for family in REQUIRED_SURFACE_FAMILIES {
        assert!(
