//! Protected fixture checks for the M5 lifecycle-vocabulary parity capstone.
//!
//! The integration test replays the JSON fixtures under
//! `fixtures/state/m5-lifecycle-vocabulary-parity/` through the Rust types and asserts the contract
...
//! `artifacts/lifecycle/m5-lifecycle-vocabulary-parity.md`, the published packet under
//! `artifacts/release/m5-lifecycle-vocabulary-parity-proof/packet.json`, the published dashboard
//! under `artifacts/release/m5-lifecycle-vocabulary-parity-proof/dashboard.json`, and the published
//! CSV under `artifacts/release/m5-lifecycle-vocabulary-parity-proof/matrix.csv` are asserted
...
use aureline_shell::m5_lifecycle_vocabulary_parity::{
    ...
    VocabularyParityDashboard, VocabularyParityPacket, VocabularyParityStatus,
    VocabularyParitySupportExport, M5_LIFECYCLE_VOCABULARY_PARITY_PACKET_RECORD_KIND,
    M5_LIFECYCLE_VOCABULARY_PARITY_PUBLISHED_REPORT_REF,
    M5_LIFECYCLE_VOCABULARY_PARITY_SHARED_CONTRACT_REF, REQUIRED_STATES,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../fixtures/state/m5-lifecycle-vocabulary-parity")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    let on_disk: VocabularyParityPacket = load_json("packet.json");
    let seeded = seeded_m5_lifecycle_vocabulary_parity_packet();
    assert_eq!(
        on_disk, seeded,
    ...
    assert_eq!(
        seeded.record_kind,
        M5_LIFECYCLE_VOCABULARY_PARITY_PACKET_RECORD_KIND
    );
    assert_eq!(
        seeded.shared_contract_ref,
        M5_LIFECYCLE_VOCABULARY_PARITY_SHARED_CONTRACT_REF
    );
    assert_eq!(
...
fn fixture_packet_passes_validation_and_is_clean() {
    let packet: VocabularyParityPacket = load_json("packet.json");
    validate_m5_lifecycle_vocabulary_parity_packet(&packet).expect("fixture packet must validate");
...
#[test]
fn fixture_packet_covers_every_term() {
    let packet: VocabularyParityPacket = load_json("packet.json");
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: VocabularyParityPacket = load_json("packet.json");
    let on_disk: VocabularyParityDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn fixture_support_export_quotes_packet_and_waiver_refs() {
    let packet: VocabularyParityPacket = load_json("packet.json");
    let export: VocabularyParitySupportExport = load_json("support_export.json");
    let expected = VocabularyParitySupportExport::from_packet(
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/lifecycle/m5-lifecycle-vocabulary-parity.md"),
    )
    .expect("published m5-lifecycle-vocabulary-parity.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-lifecycle-vocabulary-parity-proof/packet.json"),
    )
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-lifecycle-vocabulary-parity-proof/dashboard.json"),
    )
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-lifecycle-vocabulary-parity-proof/matrix.csv"),
    )
...
#[test]
fn published_doc_links_artifacts_and_quotes_terms() {
    let body = std::fs::read_to_string(
        repo_root().join("docs/lifecycle/m5_lifecycle_vocabulary_parity_contract.md"),
    )
    .expect("published lifecycle-vocabulary-parity contract must exist");
    assert!(body.contains("artifacts/lifecycle/m5-lifecycle-vocabulary-parity.md"));
    assert!(body.contains("artifacts/release/m5-lifecycle-vocabulary-parity-proof/packet.json"));
    assert!(body.contains("artifacts/release/m5-lifecycle-vocabulary-parity-proof/dashboard.json"));
    assert!(body.contains("fixtures/state/m5-lifecycle-vocabulary-parity/packet.json"));
    assert!(body.contains("schemas/lifecycle/m5-lifecycle-vocabulary-parity.schema.json"));
    for state in REQUIRED_STATES {
        assert!(
            body.contains(state.as_str()),
            "doc must name controlled term {}",
            state.as_str()
        );
    }
}
