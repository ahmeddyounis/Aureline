//! Protected fixture checks for the M5 accessibility parity capstone.
//!
//! The integration test replays the JSON fixtures under
//! `fixtures/ui/m5-accessibility-parity/` through the Rust types and asserts the contract
...
//! `artifacts/shell/m5-accessibility-parity.md`, the published packet under
//! `artifacts/release/m5-accessibility-parity-proof/packet.json`, the published dashboard under
//! `artifacts/release/m5-accessibility-parity-proof/dashboard.json`, and the published CSV under
//! `artifacts/release/m5-accessibility-parity-proof/matrix.csv` are asserted bit-for-bit equal
...
use aureline_shell::m5_accessibility_parity::{
    ...
    AccessibilityParityDashboard, AccessibilityParityPacket, AccessibilityParityStatus,
    AccessibilityParitySupportExport, M5AccessibilityCondition,
    M5_ACCESSIBILITY_PARITY_PACKET_RECORD_KIND, M5_ACCESSIBILITY_PARITY_PUBLISHED_REPORT_REF,
    M5_ACCESSIBILITY_PARITY_SHARED_CONTRACT_REF,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/ui/m5-accessibility-parity")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    let on_disk: AccessibilityParityPacket = load_json("packet.json");
    let seeded = seeded_m5_accessibility_parity_packet();
    assert_eq!(
        on_disk, seeded,
    ...
    assert_eq!(
        seeded.record_kind,
        M5_ACCESSIBILITY_PARITY_PACKET_RECORD_KIND
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        M5_ACCESSIBILITY_PARITY_PUBLISHED_REPORT_REF
    );
}

#[test]
fn fixture_packet_passes_validation_and_is_clean() {
    let packet: AccessibilityParityPacket = load_json("packet.json");
    validate_m5_accessibility_parity_packet(&packet).expect("fixture packet must validate");
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: AccessibilityParityPacket = load_json("packet.json");
    let on_disk: AccessibilityParityDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn fixture_support_export_quotes_packet_and_waiver_refs() {
    let packet: AccessibilityParityPacket = load_json("packet.json");
    let export: AccessibilityParitySupportExport = load_json("support_export.json");
    let expected = AccessibilityParitySupportExport::from_packet(
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk =
        std::fs::read_to_string(repo_root().join("artifacts/shell/m5-accessibility-parity.md"))
            .expect("published m5-accessibility-parity.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-accessibility-parity-proof/packet.json"),
    )
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-accessibility-parity-proof/dashboard.json"),
    )
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-accessibility-parity-proof/matrix.csv"),
    )
...
fn published_doc_links_artifacts_and_quotes_conditions() {
    let body =
        std::fs::read_to_string(repo_root().join("docs/shell/m5_accessibility_parity_contract.md"))
            .expect("published accessibility-parity contract must exist");
    assert!(body.contains("artifacts/shell/m5-accessibility-parity.md"));
    assert!(body.contains("artifacts/release/m5-accessibility-parity-proof/packet.json"));
    assert!(body.contains("artifacts/release/m5-accessibility-parity-proof/dashboard.json"));
    assert!(body.contains("fixtures/ui/m5-accessibility-parity/packet.json"));
    assert!(body.contains("schemas/shell/m5-accessibility-parity.schema.json"));
    for condition in M5AccessibilityCondition::ALL {
        assert!(
