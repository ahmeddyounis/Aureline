//! Protected fixture checks for the M5 shell-zone occupancy capstone.
//!
//! The integration test replays the JSON fixtures under
//! `fixtures/ui/m5-shell-occupancy/` through the Rust types and asserts the contract
...
//! report under `artifacts/shell/m5-shell-zone-occupancy.md`, the published packet
//! under `artifacts/release/m5-shell-occupancy-proof/packet.json`, the published
//! dashboard under `artifacts/release/m5-shell-occupancy-proof/dashboard.json`, and
//! the published CSV under `artifacts/release/m5-shell-occupancy-proof/matrix.csv`
//! are asserted bit-for-bit equal to the rendering, so the headless emitter remains
//! the only mint-from-truth path.

use std::path::{Path, PathBuf};

use aureline_shell::m5_shell_zone_occupancy::{
    seeded_m5_shell_occupancy_packet, validate_m5_shell_occupancy_packet, ShellOccupancyDashboard,
    ShellOccupancyPacket, ShellOccupancyStatus, ShellOccupancySupportExport,
    M5_SHELL_OCCUPANCY_PACKET_RECORD_KIND, M5_SHELL_OCCUPANCY_PUBLISHED_REPORT_REF,
    M5_SHELL_OCCUPANCY_SHARED_CONTRACT_REF, REQUIRED_FAMILIES,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/ui/m5-shell-occupancy")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    ...
    assert_eq!(
        ...
        M5_SHELL_OCCUPANCY_SHARED_CONTRACT_REF
    );
    assert_eq!(
        seeded.published_report_ref,
        M5_SHELL_OCCUPANCY_PUBLISHED_REPORT_REF
    );
}
...
fn fixture_packet_passes_validation_and_is_clean() {
    ...
    validate_m5_shell_occupancy_packet(&packet).expect("fixture packet must validate");
...
#[test]
fn fixture_every_row_occupies_a_registered_slot() {
    let packet: ShellOccupancyPacket = load_json("packet.json");
    for row in &packet.rows {
        assert!(
...
fn fixture_support_export_quotes_packet_and_waiver_refs() {
    let packet: ShellOccupancyPacket = load_json("packet.json");
    let export: ShellOccupancySupportExport = load_json("support_export.json");
    let expected =
        ShellOccupancySupportExport::from_packet(export.support_export_id.clone(), packet.clone());
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk =
        std::fs::read_to_string(repo_root().join("artifacts/shell/m5-shell-zone-occupancy.md"))
            .expect("published m5-shell-zone-occupancy.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-shell-occupancy-proof/packet.json"),
    )
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-shell-occupancy-proof/dashboard.json"),
    )
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-shell-occupancy-proof/matrix.csv"),
    )
...
fn published_doc_links_artifacts_and_quotes_families() {
    let body =
        std::fs::read_to_string(repo_root().join("docs/shell/m5_shell_zone_occupancy_contract.md"))
            .expect("published shell-zone-occupancy contract must exist");
    assert!(body.contains("artifacts/shell/m5-shell-zone-occupancy.md"));
    assert!(body.contains("artifacts/release/m5-shell-occupancy-proof/packet.json"));
    assert!(body.contains("artifacts/release/m5-shell-occupancy-proof/dashboard.json"));
    assert!(body.contains("fixtures/ui/m5-shell-occupancy/packet.json"));
    assert!(body.contains("schemas/shell/m5-shell-zone-occupancy.schema.json"));
    for family in REQUIRED_FAMILIES {
        assert!(
