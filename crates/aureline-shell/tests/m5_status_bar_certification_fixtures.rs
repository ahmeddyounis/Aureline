//! Protected fixture checks for the M5 status-bar certification capstone.
//!
//! The integration test replays the JSON fixtures under
//! `fixtures/ui/m5-status-bar-certification/` through the Rust types and asserts the
...
//! markdown report under `artifacts/shell/m5-status-bar-certification.md`, the
//! published packet under
//! `artifacts/release/m5-status-bar-certification-proof/packet.json`, the published
//! dashboard under
//! `artifacts/release/m5-status-bar-certification-proof/dashboard.json`, and the
//! published CSV under
//! `artifacts/release/m5-status-bar-certification-proof/matrix.csv` are asserted
...
use aureline_shell::m5_status_bar_certification::{
    ...
    M5StatusContext, StatusBarCertificationDashboard, StatusBarCertificationPacket,
    StatusBarCertificationStatus, StatusBarCertificationSupportExport,
    M5_STATUS_BAR_CERTIFICATION_PACKET_RECORD_KIND,
    M5_STATUS_BAR_CERTIFICATION_PUBLISHED_REPORT_REF,
    M5_STATUS_BAR_CERTIFICATION_SHARED_CONTRACT_REF,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/ui/m5-status-bar-certification")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    let on_disk: StatusBarCertificationPacket = load_json("packet.json");
    let seeded = seeded_m5_status_bar_certification_packet();
    assert_eq!(
        on_disk, seeded,
...
fn fixture_packet_passes_validation_and_is_clean() {
    let packet: StatusBarCertificationPacket = load_json("packet.json");
    validate_m5_status_bar_certification_packet(&packet).expect("fixture packet must validate");
...
#[test]
fn fixture_every_row_keeps_priority_order_and_reach_routes() {
    let packet: StatusBarCertificationPacket = load_json("packet.json");
    for row in &packet.rows {
        assert!(
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: StatusBarCertificationPacket = load_json("packet.json");
    let on_disk: StatusBarCertificationDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn fixture_support_export_quotes_packet_and_waiver_refs() {
    let packet: StatusBarCertificationPacket = load_json("packet.json");
    let export: StatusBarCertificationSupportExport = load_json("support_export.json");
    let expected = StatusBarCertificationSupportExport::from_packet(
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk =
        std::fs::read_to_string(repo_root().join("artifacts/shell/m5-status-bar-certification.md"))
            .expect("published m5-status-bar-certification.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-status-bar-certification-proof/packet.json"),
    )
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-status-bar-certification-proof/dashboard.json"),
    )
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-status-bar-certification-proof/matrix.csv"),
    )
...
fn published_doc_links_artifacts_and_quotes_contexts() {
    let body = std::fs::read_to_string(
        repo_root().join("docs/shell/m5_status_bar_certification_contract.md"),
    )
    .expect("published status-bar-certification contract must exist");
    assert!(body.contains("artifacts/shell/m5-status-bar-certification.md"));
    assert!(body.contains("artifacts/release/m5-status-bar-certification-proof/packet.json"));
    assert!(body.contains("artifacts/release/m5-status-bar-certification-proof/dashboard.json"));
    assert!(body.contains("fixtures/ui/m5-status-bar-certification/packet.json"));
    assert!(body.contains("schemas/shell/m5-status-bar-certification.schema.json"));
    for context in M5StatusContext::ALL {
        assert!(
