...
//! `fixtures/state/m5-lifecycle-release-proof/` through the Rust types and asserts the
...
//! `artifacts/lifecycle/m5-lifecycle-release-proof.md`, the published packet under
//! `artifacts/release/m5-lifecycle-release-proof-proof/packet.json`, the published dashboard
//! under `artifacts/release/m5-lifecycle-release-proof-proof/dashboard.json`, and the
//! published CSV under `artifacts/release/m5-lifecycle-release-proof-proof/matrix.csv` are
...
use aureline_shell::m5_lifecycle_release_proof::{
    ...
    LifecycleReleaseProofDashboard, LifecycleReleaseProofPacket, LifecycleReleaseProofStatus,
    LifecycleReleaseProofSupportExport, M5_LIFECYCLE_RELEASE_PROOF_PACKET_RECORD_KIND,
    M5_LIFECYCLE_RELEASE_PROOF_PUBLISHED_REPORT_REF,
    M5_LIFECYCLE_RELEASE_PROOF_SHARED_CONTRACT_REF, REQUIRED_OBJECT_FAMILIES,
};

fn fixtures_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/state/m5-lifecycle-release-proof")
}
...
fn fixture_packet_is_bit_for_bit_equal_to_seed() {
    let on_disk: LifecycleReleaseProofPacket = load_json("packet.json");
    let seeded = seeded_m5_lifecycle_release_proof_packet();
    assert_eq!(
        on_disk, seeded,
    ...
    assert_eq!(
        seeded.record_kind,
        M5_LIFECYCLE_RELEASE_PROOF_PACKET_RECORD_KIND
    );
    assert_eq!(
        seeded.shared_contract_ref,
        M5_LIFECYCLE_RELEASE_PROOF_SHARED_CONTRACT_REF
    );
    assert_eq!(
...
fn fixture_packet_passes_validation_and_is_clean() {
    let packet: LifecycleReleaseProofPacket = load_json("packet.json");
    validate_m5_lifecycle_release_proof_packet(&packet).expect("fixture packet must validate");
...
#[test]
fn fixture_every_row_certifies_all_profiles_pillars_and_surfaces() {
    let packet: LifecycleReleaseProofPacket = load_json("packet.json");
    for row in &packet.rows {
        assert!(
...
fn fixture_dashboard_matches_packet_projection() {
    let packet: LifecycleReleaseProofPacket = load_json("packet.json");
    let on_disk: LifecycleReleaseProofDashboard = load_json("dashboard.json");
    assert_eq!(
        on_disk,
        packet.dashboard(),
...
fn fixture_support_export_quotes_packet_and_waiver_refs() {
    let packet: LifecycleReleaseProofPacket = load_json("packet.json");
    let export: LifecycleReleaseProofSupportExport = load_json("support_export.json");
    let expected = LifecycleReleaseProofSupportExport::from_packet(
...
fn published_report_md_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/lifecycle/m5-lifecycle-release-proof.md"),
    )
    .expect("published m5-lifecycle-release-proof.md must exist");
    assert_eq!(
        on_disk, rendered,
...
fn published_packet_json_matches_seed() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-lifecycle-release-proof-proof/packet.json"),
    )
...
fn published_dashboard_json_matches_seeded_projection() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-lifecycle-release-proof-proof/dashboard.json"),
    )
...
fn published_csv_matches_seeded_rendering() {
    ...
    let on_disk = std::fs::read_to_string(
        repo_root().join("artifacts/release/m5-lifecycle-release-proof-proof/matrix.csv"),
    )
...
fn published_doc_links_artifacts_and_quotes_families() {
    let body = std::fs::read_to_string(
        repo_root().join("docs/lifecycle/m5_lifecycle_release_proof_contract.md"),
    )
    .expect("published telemetry-conformance contract must exist");
    assert!(body.contains("artifacts/lifecycle/m5-lifecycle-release-proof.md"));
    assert!(body.contains("artifacts/release/m5-lifecycle-release-proof-proof/packet.json"));
    assert!(body.contains("artifacts/release/m5-lifecycle-release-proof-proof/dashboard.json"));
    assert!(body.contains("fixtures/state/m5-lifecycle-release-proof/packet.json"));
    assert!(body.contains("schemas/lifecycle/m5-lifecycle-release-proof.schema.json"));
    for family in REQUIRED_OBJECT_FAMILIES {
        assert!(
