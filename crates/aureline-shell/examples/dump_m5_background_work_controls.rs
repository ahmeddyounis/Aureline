//! Headless emitter for the M5 background-work row / banner controls packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-background-work-row-banner-controls-proof/`, its matrix CSV, the
...
use aureline_shell::implement_the_m5_background_work_row_and_background_work_banner_affected_work_class_state_what_still_works_resume_condition_and_override_primitive::{
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("report") => {
            print!(
                "{}",
                seeded_m5_background_work_controls().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!(
                "{}",
                seeded_m5_background_work_controls().render_matrix_csv()
            );
        }
        Some("fixture-activity-center-beta-narrowed") => {
            let packet = seeded_m5_background_work_controls_activity_center_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-background-work-preview-narrowed") => {
            let packet = seeded_m5_background_work_controls_background_work_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("validate") => {
...
fn assert_valid(packet: &M5BackgroundWorkControlsPacket) -> Result<(), Box<dyn std::error::Error>> {
