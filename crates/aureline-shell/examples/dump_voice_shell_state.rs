//! Conformance dump for the voice shell-state packet.
//!
...
//! checked-in fixtures under `fixtures/voice/mode-and-mic-state/`.

use std::path::PathBuf;

use aureline_shell::voice_shell_state::{
    seeded_voice_shell_state_packet, write_fixtures, VOICE_SHELL_STATE_FIXTURES_DIR_REF,
};
    ...
    match which.as_str() {
        ...
        "write" => {
            let dir = std::env::args()
                .nth(2)
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from(VOICE_SHELL_STATE_FIXTURES_DIR_REF));
            write_fixtures(&dir, &packet).expect("write fixtures");
            eprintln!("wrote voice shell-state fixtures to {}", dir.display());
        }
