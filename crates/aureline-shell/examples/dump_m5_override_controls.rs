//! Headless emitter for the M5 override sheet / policy-note controls packet.
//!
//! The example is the only mint-from-truth path for the support export checked in under
//! `artifacts/release/m5-override-sheet-policy-note-controls-proof/`, its matrix CSV, the
//! Markdown summary, and the narrowed fixtures under
//! `fixtures/ui/m5-override-sheet-policy-note-controls/`.
//!
//! ```text
//! cargo run -p aureline-shell --example dump_m5_override_controls -- support-export
//! cargo run -p aureline-shell --example dump_m5_override_controls -- report
//! cargo run -p aureline-shell --example dump_m5_override_controls -- csv
//! cargo run -p aureline-shell --example dump_m5_override_controls -- fixture-override-settings-beta-narrowed
//! cargo run -p aureline-shell --example dump_m5_override_controls -- fixture-activity-center-preview-narrowed
//! cargo run -p aureline-shell --example dump_m5_override_controls -- validate
//! ```

use aureline_shell::implement_the_m5_per_workspace_override_sheet_and_override_policy_note_row_current_mode_ceilings_expected_effect_reset_path_and_blocked_by_policy_primitive::{
    seeded_m5_override_controls, seeded_m5_override_controls_activity_center_preview_narrowed,
    seeded_m5_override_controls_override_settings_beta_narrowed, M5OverrideControlsPacket,
};

fn main() {
    if let Err(err) = run() {
        eprintln!("{err}");
        std::process::exit(2);
    }
}

fn run() -> Result<(), Box<dyn std::error::Error>> {
    let args: Vec<String> = std::env::args().skip(1).collect();
    match args.first().map(String::as_str) {
        Some("support-export") | None => {
            let packet = seeded_m5_override_controls();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("report") => {
            print!(
                "{}",
                seeded_m5_override_controls().render_markdown_summary()
            );
        }
        Some("csv") => {
            print!("{}", seeded_m5_override_controls().render_matrix_csv());
        }
        Some("fixture-override-settings-beta-narrowed") => {
            let packet = seeded_m5_override_controls_override_settings_beta_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("fixture-activity-center-preview-narrowed") => {
            let packet = seeded_m5_override_controls_activity_center_preview_narrowed();
            assert_valid(&packet)?;
            println!("{}", packet.export_safe_json());
        }
        Some("validate") => {
            for packet in [
                seeded_m5_override_controls(),
                seeded_m5_override_controls_override_settings_beta_narrowed(),
                seeded_m5_override_controls_activity_center_preview_narrowed(),
            ] {
                assert_valid(&packet)?;
            }
            println!("ok");
        }
        Some(other) => {
            return Err(format!("unknown subcommand: {other}").into());
        }
    }
    Ok(())
}

fn assert_valid(packet: &M5OverrideControlsPacket) -> Result<(), Box<dyn std::error::Error>> {
    let violations = packet.validate();
    if violations.is_empty() {
        Ok(())
    } else {
        let tokens: Vec<&str> = violations.iter().map(|v| v.as_str()).collect();
        Err(format!("controls packet failed validation: {}", tokens.join(",")).into())
    }
}
