//! Conformance dump for the M5 macro-replay review packet.
//!
...
use aureline_shell::ship_macro_replay_review_run_capable_or_cross_file_macro_downgrades_and_recipe_promotion::*;
...
fn main() {
    ...
    match which.as_str() {
        "summary" => {
            let packet = seeded_macro_replay_review_packet();
            assert!(packet.validate().is_empty(), "packet must validate");
            print!("{}", packet.render_markdown_summary());
        }
        "fixture" => {
            let packet = fixture_macro_replay_review_packet();
