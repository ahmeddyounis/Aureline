//! Tests for the M05-1067 adaptive-efficiency component surface certification
//! capstone.

use super::*;

fn packet() -> EfficiencySurfaceCertificationPacket {
    seeded_m5_efficiency_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, EFFICIENCY_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, EFFICIENCY_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5EfficiencyCertifiedSurface::ALL {
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, EfficiencyAxisCertificationState::Certified);
...
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5EfficiencyCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5EfficiencyCertifiedSurface::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = EfficiencyCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), EfficiencyCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in EfficiencyCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == EfficiencyCertificationAxis::CliExport
        );
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5EfficiencyCertifiedSurface::ShellStatusBar)
        .expect("shell-status-bar row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EfficiencyCertificationAxis::Visual {
            outcome.state = EfficiencyAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("source of change silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, EfficiencySurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, EfficiencyCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5EfficiencyCertifiedSurface::ActivityCenter)
        .expect("activity-center row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EfficiencyCertificationAxis::DegradedState {
            *outcome = EfficiencyAxisOutcome {
                axis: EfficiencyCertificationAxis::DegradedState,
                state: EfficiencyAxisCertificationState::DisclosedNarrowed,
                parity_note: "the adaptation reading is lagging".to_owned(),
                narrowing_reason: Some("the live proof is partial for this surface".to_owned()),
                downgrade_trigger: Some(M5EfficiencyDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays FullTruth == certified FullTruth, no claim_auto_narrow.
    assert_eq!(row.derive_status(), EfficiencySurfaceClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EfficiencyCertificationAxis::CliExport {
            outcome.state = EfficiencyAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5EfficiencyDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), EfficiencySurfaceClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(EfficiencyClaimAutoNarrow {
        binding_axis: EfficiencyCertificationAxis::DegradedState,
        from_claim: M5EfficiencyAccessClaim::FullTruth,
        to_claim: M5EfficiencyAccessClaim::FullTruth,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5EfficiencyAccessClaim::Degraded;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5EfficiencyCertifiedSurface::SupportExport)
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    row.certified_claim = M5EfficiencyAccessClaim::FullTruth; // claimed is ResolvedTruth
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5EfficiencyCertifiedSurface::WorkContentCanvas)
        .expect("work-content-canvas row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = EfficiencyCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), EfficiencySurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EfficiencyCertificationAxis::CliExport {
            outcome.state = EfficiencyAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the canvas view".to_owned());
            outcome.downgrade_trigger = Some(M5EfficiencyDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = EfficiencyCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), EfficiencySurfaceClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == EfficiencySurfaceClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "battery saver".to_owned();
    }
    assert_eq!(row.derive_status(), EfficiencySurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(EfficiencyCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5EfficiencyDowngradeTrigger::ProofStale,
    );
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5EfficiencyCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        EfficiencyCertificationViolation::SurfaceCoverageIncomplete
    )));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the stale-result-continuity-note family from every row that carries it;
    // coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5EfficiencyComponentFamily::StaleResultContinuityNote);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        EfficiencyCertificationViolation::FamilyCoverageIncomplete
    )));
}
...
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = EfficiencySurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        EfficiencyCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, EfficiencyCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, EfficiencyCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != EfficiencyCertificationAxis::EfficiencyTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        EfficiencyCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, EfficiencyCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        EfficiencyCertificationViolation::RawEfficiencyMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: EfficiencySurfaceCertificationPacket =
...
fn generate_artifacts() {
    if std::env::var("GEN_EFFICIENCY_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_efficiency_component_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-efficiency-component-certification-proof");
    ...
    let fixtures =
        Path::new(manifest).join("../../fixtures/ui/m5-efficiency-component-certification");
    ...
    fs::write(
        ...
        "# M5 adaptive-efficiency component surface certification fixtures\n\n\
         Mirror of `artifacts/release/m5-efficiency-component-certification-proof/`.\n\
         Regenerate with `GEN_EFFICIENCY_CERT_ARTIFACTS=1 cargo test -p aureline-shell \
         certify_power_state...::tests::generate_artifacts`.\n",
    )
