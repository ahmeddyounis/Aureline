...
pub fn invocation_preview_sheet_lines(record: &CommandInvocationPreviewSheetRecord) -> Vec<String> {
    ...
    lines.push(format!(
        ...
        record
            ...
            .as_deref()
            .unwrap_or("<none>"),
        record.invocation_session.context_snapshot.workspace_trust_state,
        record
            .invocation_session
            .context_snapshot
    ...
    lines.push(format!(
        "basis_snapshot_ref: {}",
        record.invocation_session.context_snapshot.basis_snapshot_ref
    ));

    if let Some(preview_ref) = record.invocation_session.preview_posture.preview_record_ref.as_ref()
    {
...
mod tests {
        ...
        for entry in std::fs::read_dir(&root).expect("fixture directory must exist") {
            ...
            let session = CommandInvocationSession {
                ...
                context_snapshot: InvocationContextSnapshot {
                    ...
                    execution_context_id: command.descriptor.policy_context.execution_context_id.clone(),
                ...
                approval_posture: ApprovalPostureBlock {
                    approval_posture_class_declared: command.descriptor.approval_posture_class.clone(),
    ...
    fn record_packet_argument_map(
        ...
        command
            ...
            .typed_arguments
            .iter()
            .map(|slot| aureline_commands::invocation::ArgumentProvenanceEntry {
