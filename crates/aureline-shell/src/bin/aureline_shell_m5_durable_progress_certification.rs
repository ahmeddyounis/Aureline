//! Headless emitter for the M5 durable progress-indicator & job-row actor, phase,
//! action, and history parity certification proof.
//!
//! The bin is the only mint-from-truth path for the published packet, dashboard,
//! support export, and CSV checked in under
//! `artifacts/release/m5-durable-progress-certification-proof/` and the markdown report
//! under `artifacts/shell/m5-durable-progress-certification.md`, plus the protected
//! fixtures under `fixtures/ui/m5-durable-progress-certification/`. Shell / activity
//! center / release automation, release-center, docs/help, and support exports resolve the
//! same certification rows — green/yellow/red status, active waivers, and the exact
//! certification causes — through this proof rather than restating durable-presence,
//! attribution, grouped-history, or export posture by hand.
//!
//! Subcommands:
//!
//! ```sh
...
//! cargo run -q -p aureline-shell --bin aureline_shell_m5_durable_progress_certification -- support-export
...
use aureline_shell::m5_durable_progress_certification::{
    ...
    validate_m5_durable_progress_certification_packet, DurableProgressCertificationPacket,
    DurableProgressCertificationSupportExport, M5_DURABLE_PROGRESS_CERTIFICATION_SUPPORT_EXPORT_ID,
};
...
fn assert_valid(
    ...
    match validate_m5_durable_progress_certification_packet(packet) {
        Ok(()) => Ok(()),
        Err(errors) => {
