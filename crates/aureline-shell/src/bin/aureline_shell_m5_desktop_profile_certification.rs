//! Headless emitter for the M5 desktop-profile certification proof.
//!
//! The bin is the only mint-from-truth path for the published packet, dashboard, support
//! export, and CSV checked in under
//! `artifacts/release/m5-desktop-profile-certification-proof/` and the markdown report under
//! `artifacts/shell/m5-desktop-profile-certification.md`, plus the protected fixtures under
//! `fixtures/ui/m5-desktop-profile-certification/`. Shell / windowing / layout / status
//! automation, release-center, docs/help, and support exports resolve the same certification
//! rows — green/yellow/red status, active waivers, and the exact profile causes — through this
//! proof rather than restating shell-zone, adaptive-layout, multi-window, or owning-window
//! routing posture by hand.
//!
//! Subcommands:
//!
//! ```sh
...
//! cargo run -q -p aureline-shell --bin aureline_shell_m5_desktop_profile_certification -- support-export
...
use aureline_shell::m5_desktop_profile_certification::{
    ...
    validate_m5_desktop_profile_certification_packet, DesktopProfilePacket,
    DesktopProfileSupportExport, M5_DESKTOP_PROFILE_CERTIFICATION_SUPPORT_EXPORT_ID,
};
...
fn assert_valid(packet: &DesktopProfilePacket) -> Result<(), Box<dyn std::error::Error>> {
    match validate_m5_desktop_profile_certification_packet(packet) {
        Ok(()) => Ok(()),
        Err(errors) => {
