//! Headless emitter for the stable learnability / glossary / contextual
//! docs-help disclosure corpus.
//!
//! Mints the on-disk fixtures under
//! `fixtures/ux/m4/promote-learnability-glossary-and-contextual-docs-help-guidance/`
//! so the switching row, docs/help browser, command palette, diagnostics
//! packet, support export, Help/About, and reviewer-facing report all quote the
...
//!   --bin aureline_shell_learnability_glossary_stable -- scenario <scenario_id>
//!
//! # Print every scenario, separated by --- markers.
//! cargo run -q -p aureline-shell \
//!   --bin aureline_shell_learnability_glossary_stable -- all
//!
//! # Plaintext truth block (per-scenario, suitable for support export).
//! cargo run -q -p aureline-shell \
//!   --bin aureline_shell_learnability_glossary_stable -- plaintext
//!
//! # Stable corpus index — scenario id, ecosystem, claim class, why-now.
//! cargo run -q -p aureline-shell \
...
use aureline_shell::learnability_glossary_stable::{
    learnability_disclosure_corpus, LearnabilityDisclosureScenario,
};
...
fn print_all() {
    let scenarios = learnability_disclosure_corpus();
...
fn print_scenario(id: &str) -> Result<(), Box<dyn std::error::Error>> {
    let scenario = learnability_disclosure_corpus()
        .into_iter()
...
fn print_plaintext() {
    for (idx, scenario) in learnability_disclosure_corpus().iter().enumerate() {
        if idx > 0 {
            println!();
        }
...
fn print_index() {
    println!(
        "{:<46} {:<18} {:<10} {:<14} chips why_now_grounded",
...
fn render_json(scenario: &LearnabilityDisclosureScenario) -> String {
