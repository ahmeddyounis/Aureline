//! Headless emitter for the M5 discoverability-affordance-parity certification.
//!
//! The bin is the only mint-from-truth path for the published packet, dashboard, support export, and CSV
//! checked in under `artifacts/release/m5-discoverability-affordance-parity-proof/` and the markdown report
//! under `artifacts/commands/m5-discoverability-affordance-parity.md`, plus the protected fixtures under
//! `fixtures/commands/m5-discoverability-affordance-parity/`. Product UI, CLI, help, Support Center, and the
//! button / tooltip / onboarding / AI / voice / companion tooling resolve the same parity rows —
//! green/yellow/red status, active waivers, and the exact conformance causes — through this proof rather
//! than restating label-reuse, side-effect-truth, authority-reach, or origin-export posture by hand.
//!
//! Subcommands:
//!
//! ```sh
...
//! cargo run -q -p aureline-shell --bin aureline_shell_m5_discoverability_affordance_parity -- support-export
...
use aureline_shell::m5_discoverability_affordance_parity::{
    ...
    AffordanceParityPacket, AffordanceParitySupportExport, M5_AFFORDANCE_PARITY_SUPPORT_EXPORT_ID,
};
...
fn assert_valid(packet: &AffordanceParityPacket) -> Result<(), Box<dyn std::error::Error>> {
    match validate_m5_affordance_parity_packet(packet) {
        Ok(()) => Ok(()),
        Err(errors) => {
