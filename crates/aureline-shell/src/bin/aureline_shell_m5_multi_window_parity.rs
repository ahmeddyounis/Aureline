//! Headless emitter for the M5 multi-window truth-parity proof.
//!
//! The bin is the only mint-from-truth path for the published packet, dashboard,
//! support export, and CSV checked in under
//! `artifacts/release/m5-multi-window-parity-proof/` and the markdown report under
//! `artifacts/shell/m5-multi-window-parity.md`, plus the protected fixtures under
//! `fixtures/ui/m5-multi-window-parity/`. Shell / windowing / layout / release
//! automation, release-center, docs/help, and support exports resolve the same parity
//! rows — green/yellow/red status, active waivers, and the exact parity causes —
//! through this proof rather than restating continuity, layout, routing, or recovery
//! posture by hand.
//!
//! Subcommands:
//!
...
//! cargo run -q -p aureline-shell --bin aureline_shell_m5_multi_window_parity -- support-export
...
use aureline_shell::m5_multi_window_parity::{
    ...
    MultiWindowParityPacket, MultiWindowParitySupportExport,
...
fn assert_valid(packet: &MultiWindowParityPacket) -> Result<(), Box<dyn std::error::Error>> {
    match validate_m5_multi_window_parity_packet(packet) {
        Ok(()) => Ok(()),
        Err(errors) => {
