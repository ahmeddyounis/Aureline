//! Headless emitter for the stable interaction-parity corpus.
//!
//! Mints the on-disk fixtures under
//! `fixtures/ux/m4/harden-focus-selection-keyboard-parity-and-collection-state/`
//! so the shell collection surface, the keyboard-help reference, the CLI
//! inspector, Help/About, and the diagnostics support export all quote the same
//! record renders as the in-code corpus. The fixture-replay test in
//! `crates/aureline-shell/tests/interaction_parity_stable_fixtures.rs` enforces
...
//!   --bin aureline_shell_interaction_parity_stable -- scenario <scenario_id>
//!
//! # Print every scenario, separated by --- markers.
//! cargo run -q -p aureline-shell \
//!   --bin aureline_shell_interaction_parity_stable -- all
//!
//! # Plaintext truth block (per-scenario, suitable for support export).
//! cargo run -q -p aureline-shell \
//!   --bin aureline_shell_interaction_parity_stable -- plaintext
//!
//! # Stable corpus index — scenario id, surface class, claim, marker.
//! cargo run -q -p aureline-shell \
//!   --bin aureline_shell_interaction_parity_stable -- index
//!
...
use aureline_shell::interaction_integrity_stable::{
    interaction_parity_corpus, InteractionParityScenario,
};
...
fn print_all() {
    let scenarios = interaction_parity_corpus();
...
fn print_scenario(id: &str) -> Result<(), Box<dyn std::error::Error>> {
    let scenario = interaction_parity_corpus()
        .into_iter()
...
fn print_plaintext() {
    for (idx, scenario) in interaction_parity_corpus().iter().enumerate() {
        if idx > 0 {
            println!();
        }
...
fn render_json(scenario: &InteractionParityScenario) -> String {
