//! Headless emitter for the M5 min-width-guard (editor minimum / compare fallback /
//! no unusable narrow pane) proof.
//!
//! The bin is the only mint-from-truth path for the published packet, dashboard,
//! support export, and CSV checked in under
//! `artifacts/release/m5-min-width-guards-proof/` and the markdown report under
//! `artifacts/shell/m5-min-width-guards.md`, plus the protected fixtures under
//! `fixtures/ui/m5-min-width-guards/`. Shell / windowing / layout / release automation,
//! release-center, docs/help, and support exports resolve the same guard rows —
//! green/yellow/red status, active waivers, and the exact guard causes — through this
//! proof rather than restating min-size, fallback, or status posture by hand.
//!
//! Subcommands:
//!
//! ```sh
...
//! cargo run -q -p aureline-shell --bin aureline_shell_m5_min_width_guards -- support-export
...
use aureline_shell::m5_min_width_guards::{
    seeded_m5_min_width_guards_packet, validate_m5_min_width_guards_packet, MinWidthGuardPacket,
    MinWidthGuardSupportExport, M5_MIN_WIDTH_GUARDS_SUPPORT_EXPORT_ID,
};
...
fn assert_valid(packet: &MinWidthGuardPacket) -> Result<(), Box<dyn std::error::Error>> {
    match validate_m5_min_width_guards_packet(packet) {
        Ok(()) => Ok(()),
        Err(errors) => {
