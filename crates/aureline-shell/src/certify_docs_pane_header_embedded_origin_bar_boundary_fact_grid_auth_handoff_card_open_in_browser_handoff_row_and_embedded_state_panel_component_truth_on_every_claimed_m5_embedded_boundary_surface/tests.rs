//! Tests for the M05-1075 embedded-boundary component surface certification
//! capstone.

use super::*;

fn packet() -> EmbeddedSurfaceCertificationPacket {
    seeded_m5_embedded_boundary_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, EMBEDDED_BOUNDARY_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, EMBEDDED_BOUNDARY_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5EmbeddedCertifiedSurface::ALL {
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(
            ...
            EmbeddedCertificationAxis::ALL.len()
        );
    }
...
fn cli_export_axis_is_certified_on_every_row() {
    ...
    for row in &p.rows {
        let export = row
            ...
            .expect("cli axis");
        assert_eq!(export.state, EmbeddedAxisCertificationState::Certified);
...
#[test]
fn a_stale_offline_or_blocked_surface_never_keeps_a_fresh_first_party_claim() {
    // Every yellow row narrows away from FullTruth (the only claim that asserts live,
    // current, fresh first-party-local truth). No surface keeps FullTruth while degraded.
    for row in &packet().rows {
...
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5EmbeddedCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5EmbeddedCertifiedSurface::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = EmbeddedCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), EmbeddedCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in EmbeddedCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == EmbeddedCertificationAxis::CliExport
        );
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5EmbeddedCertifiedSurface::DocsHelpPane)
        .expect("docs-help-pane row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EmbeddedCertificationAxis::Visual {
            outcome.state = EmbeddedAxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("owner/origin silently dropped".to_owned());
            outcome.downgrade_trigger = None;
        }
    }
    row.derived_status = row.derive_status();
    assert_eq!(row.derived_status, EmbeddedSurfaceClaimStatus::Red);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, EmbeddedCertificationViolation::SurfaceBlocked { .. })));
}
...
fn degraded_axis_without_claim_narrowing_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5EmbeddedCertifiedSurface::MarketplacePane)
        .expect("marketplace-pane row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EmbeddedCertificationAxis::DegradedState {
            *outcome = EmbeddedAxisOutcome {
                axis: EmbeddedCertificationAxis::DegradedState,
                state: EmbeddedAxisCertificationState::DisclosedNarrowed,
                parity_note: "the origin reading is lagging".to_owned(),
                narrowing_reason: Some("the live proof is partial for this surface".to_owned()),
                downgrade_trigger: Some(M5EmbeddedDowngradeTrigger::ProofStale),
            };
        }
    }
    // Claim stays ResolvedTruth == certified ResolvedTruth, no claim_auto_narrow.
    assert_eq!(row.derive_status(), EmbeddedSurfaceClaimStatus::Red);
}
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EmbeddedCertificationAxis::CliExport {
            outcome.state = EmbeddedAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity not current for this surface".to_owned());
            outcome.downgrade_trigger = Some(M5EmbeddedDowngradeTrigger::ProofStale);
        }
    }
    assert_eq!(row.derive_status(), EmbeddedSurfaceClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(EmbeddedClaimAutoNarrow {
        binding_axis: EmbeddedCertificationAxis::DegradedState,
        from_claim: M5EmbeddedAccessClaim::FullTruth,
        to_claim: M5EmbeddedAccessClaim::FullTruth,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5EmbeddedAccessClaim::Degraded;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5EmbeddedCertifiedSurface::SupportExport)
        .expect("support-export row exists");
    let row = &mut p.rows[idx];
    row.certified_claim = M5EmbeddedAccessClaim::FullTruth; // claimed is ResolvedTruth
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        EmbeddedCertificationViolation::CertifiedClaimExceedsClaim { .. }
    )));
}
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5EmbeddedCertifiedSurface::EmbeddedWebview)
        .expect("embedded-webview row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = EmbeddedCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), EmbeddedSurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == EmbeddedCertificationAxis::CliExport {
            outcome.state = EmbeddedAxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the webview".to_owned());
            outcome.downgrade_trigger = Some(M5EmbeddedDowngradeTrigger::ProofStale);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = EmbeddedCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), EmbeddedSurfaceClaimStatus::Red);
}
...
fn generic_narrow_label_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.derived_status == EmbeddedSurfaceClaimStatus::Yellow)
        .expect("a yellow row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.visible_label = "provider blocked".to_owned();
    }
    assert_eq!(row.derive_status(), EmbeddedSurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(EmbeddedCertificationAxis::Visual);
...
fn disclosed_axis_missing_trigger_is_malformed() {
    let mut o = seed_narrowed(
        ...
        M5EmbeddedDowngradeTrigger::ProofStale,
    );
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5EmbeddedCertifiedSurface::SupportExport);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, EmbeddedCertificationViolation::SurfaceCoverageIncomplete)));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Strip the boundary-fact-grid family from every row that carries it;
    // coverage must fail.
    let mut p = packet();
    for row in &mut p.rows {
        row.consumed_families
            .retain(|f| *f != M5EmbeddedBoundaryComponentFamily::BoundaryFactGrid);
        row.derived_status = row.derive_status();
    }
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, EmbeddedCertificationViolation::FamilyCoverageIncomplete)));
}

#[test]
fn stale_derived_status_is_rejected() {
    let mut p = packet();
    p.rows[0].derived_status = EmbeddedSurfaceClaimStatus::Yellow; // it is really Green
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        EmbeddedCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        EmbeddedCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, EmbeddedCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, EmbeddedCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != EmbeddedCertificationAxis::BoundaryTruth);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        EmbeddedCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, EmbeddedCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        EmbeddedCertificationViolation::RawBoundaryMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: EmbeddedSurfaceCertificationPacket =
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_embedded_boundary_component_certification_export().expect("export is valid");
    assert_eq!(
...
fn generate_artifacts() {
    if std::env::var("GEN_EMBEDDED_BOUNDARY_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_embedded_boundary_component_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-embedded-boundary-component-certification-proof");
    ...
    let fixtures =
        Path::new(manifest).join("../../fixtures/ui/m5-embedded-boundary-component-certification");
    ...
    fs::write(
        ...
        "# M5 embedded-boundary component surface certification fixtures\n\n\
         Mirror of `artifacts/release/m5-embedded-boundary-component-certification-proof/`.\n\
         Regenerate with `GEN_EMBEDDED_BOUNDARY_CERT_ARTIFACTS=1 cargo test -p aureline-shell \
         certify_docs_pane_header...::tests::generate_artifacts`.\n",
    )
