//! Inline unit tests for the M5 command-documentation certification.
...
fn seeded_packet_covers_every_surface_family_and_is_clean() {
    ...
    validate_m5_command_documentation_packet(&packet).expect("seeded packet must validate clean");
...
#[test]
fn every_row_certifies_all_doc_fields_parity_cards_and_derivation_anchors() {
    let packet = seeded_m5_command_documentation_packet();
    for row in &packet.rows {
        assert!(
        ...
        assert_eq!(row.certified_doc_fields.len(), REQUIRED_DOC_FIELDS.len());
        assert_eq!(
            row.certified_parity_cards.len(),
            REQUIRED_PARITY_CARDS.len()
        );
        assert_eq!(
            row.certified_derivation_anchors.len(),
            REQUIRED_DERIVATION_ANCHORS.len()
        );
    }
}
...
fn disclosed_surface_paraphrase_carries_an_active_waiver() {
    let packet = seeded_m5_command_documentation_packet();
    let context = packet.row(M5CommandSurfaceFamily::ContextMenu).unwrap();
    assert!(matches!(
        context.cross_surface_naming,
        CrossSurfaceNamingState::DisclosedSurfaceParaphrase
    ));
    assert!(context.requires_waiver());
    assert!(context.has_active_waiver());
    assert_eq!(packet.active_waivers.len(), 1);
    assert!(packet.active_waivers[0].is_active_at(&packet.generated_at));
}

#[test]
fn disclosed_reduced_doc_detail_keeps_command_bar_yellow_without_a_waiver() {
    let packet = seeded_m5_command_documentation_packet();
    let bar = packet.row(M5CommandSurfaceFamily::CommandBar).unwrap();
    assert!(matches!(
        bar.documentation_record,
        DocumentationRecordState::DisclosedReducedDocDetail
    ));
    assert_eq!(bar.derived_status, CommandDocStatus::Yellow);
    assert!(bar.cross_surface_naming.is_full());
    assert!(!bar.requires_waiver());
}

#[test]
fn record_mismatch_blocks_the_menu_item() {
    let packet = seeded_m5_command_documentation_packet_menu_item_record_mismatch_blocked();
    let row = packet.row(M5CommandSurfaceFamily::MenuItem).unwrap();
    assert_eq!(row.derived_status, CommandDocStatus::Red);
    assert!(!packet.report_clean);
    assert!(!packet.all_rows_publishable);
    ...
    assert!(packet
        .blocking_findings
        .iter()
        .any(|finding| matches!(finding, CommandDocFinding::DocumentationRecordBroken { .. })));
    assert!(row.conformance_causes.iter().any(|cause| matches!(
        cause.trigger,
        M5DiscoverabilityDowngradeTrigger::LifecycleOrDeprecationHidden
    )));
    assert!(validate_m5_command_documentation_packet(&packet).is_err());
}

#[test]
fn naming_drift_blocks_the_context_menu() {
    let packet = seeded_m5_command_documentation_packet_context_menu_naming_drift_blocked();
    ...
    assert!(packet
        .blocking_findings
        .iter()
        .any(|finding| matches!(finding, CommandDocFinding::CrossSurfaceNamingBroken { .. })));
    assert!(row.conformance_causes.iter().any(|cause| matches!(
        cause.trigger,
...
#[test]
fn stale_example_blocks_the_documentation_surface() {
    let packet =
    ...
    assert!(packet
        .blocking_findings
        .iter()
        .any(|finding| matches!(finding, CommandDocFinding::ExampleFreshnessBroken { .. })));
    assert!(row
        .conformance_causes
        .iter()
...
fn absent_capture_blocks_the_import_bridge_row() {
    let packet = seeded_m5_command_documentation_packet_import_bridge_capture_absent_blocked();
    ...
    assert!(packet
        .blocking_findings
        .iter()
        .any(|finding| matches!(finding, CommandDocFinding::DocExportBroken { .. })));
    assert!(row
        .conformance_causes
        .iter()
        .any(|cause| matches!(cause.trigger, M5DiscoverabilityDowngradeTrigger::ProofStale)));
...
fn headless_parity_loss_blocks_the_explainer() {
    let packet = seeded_m5_command_documentation_packet_explainer_headless_parity_lost_blocked();
    let row = packet
    ...
    assert!(packet
        .blocking_findings
        .iter()
        .any(|finding| matches!(finding, CommandDocFinding::HeadlessParityLost { .. })));
    assert!(row.conformance_causes.iter().any(|cause| matches!(
        cause.trigger,
...
#[test]
fn incomplete_doc_field_set_blocks() {
    let mut packet = seeded_m5_command_documentation_packet();
    let row = packet
        .rows
        .iter_mut()
        .find(|row| row.surface_family == M5CommandSurfaceFamily::MenuItem)
        .unwrap();
    row.certified_doc_fields.pop();
    assert!(!row.doc_fields_complete());
    assert_eq!(row.recompute_status(), CommandDocStatus::Red);
    let findings = row.compute_findings(&packet.generated_at);
    assert!(findings
        .iter()
        .any(|finding| matches!(finding, CommandDocFinding::DocFieldsIncomplete { .. })));
}

#[test]
fn incomplete_parity_card_set_blocks() {
    let mut packet = seeded_m5_command_documentation_packet();
    let row = packet
        .rows
        .iter_mut()
        .find(|row| row.surface_family == M5CommandSurfaceFamily::MenuItem)
        .unwrap();
    row.certified_parity_cards.pop();
    assert!(!row.parity_cards_complete());
    assert_eq!(row.recompute_status(), CommandDocStatus::Red);
    let findings = row.compute_findings(&packet.generated_at);
    assert!(findings
        .iter()
        .any(|finding| matches!(finding, CommandDocFinding::ParityCardsIncomplete { .. })));
}

#[test]
fn incomplete_derivation_anchor_set_blocks() {
    let mut packet = seeded_m5_command_documentation_packet();
    let row = packet
        .rows
        .iter_mut()
        .find(|row| row.surface_family == M5CommandSurfaceFamily::MenuItem)
        .unwrap();
    row.certified_derivation_anchors.pop();
    assert!(!row.derivation_anchors_complete());
    assert_eq!(row.recompute_status(), CommandDocStatus::Red);
    let findings = row.compute_findings(&packet.generated_at);
    assert!(findings.iter().any(|finding| matches!(
        finding,
        CommandDocFinding::DerivationAnchorsIncomplete { .. }
    )));
}
...
fn incomplete_consumer_surface_certification_blocks() {
    ...
    assert!(findings.iter().any(|finding| matches!(
        finding,
        CommandDocFinding::ConsumerSurfacesIncomplete { .. }
    )));
}
...
fn dashboard_projects_every_row_and_counts() {
    ...
    let context = dashboard
        .rows
        .iter()
        .find(|row| row.surface_family == M5CommandSurfaceFamily::ContextMenu)
        .unwrap();
    assert_eq!(context.status, CommandDocStatus::Yellow);
    assert!(context.has_active_waiver);
    assert!(matches!(
        context.cross_surface_naming,
        CrossSurfaceNamingState::DisclosedSurfaceParaphrase
...
fn support_export_quotes_packet_matrix_and_waiver_refs() {
    let packet = seeded_m5_command_documentation_packet();
    let export =
        CommandDocSupportExport::from_packet(M5_COMMAND_DOC_SUPPORT_EXPORT_ID, packet.clone());
...
fn markdown_and_csv_name_every_surface_family() {
    ...
    assert!(markdown.contains("m5_command_documentation_fixtures"));
    assert!(markdown.contains("waiver:command-doc-surface-paraphrase:0001"));
}
