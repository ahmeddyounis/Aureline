//! Inline unit tests for the M5 keybinding resolver inspection certification.

use super::*;

#[test]
fn seeded_packet_covers_every_surface_family_and_is_clean() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    validate_m5_keybinding_resolver_inspectors_packet(&packet)
        .expect("seeded packet must validate clean");

    assert_eq!(packet.rows.len(), REQUIRED_SURFACE_FAMILIES.len());
    for family in REQUIRED_SURFACE_FAMILIES {
        assert!(
            packet.row(family).is_some(),
            "missing row for {}",
            family.as_str()
        );
    }
    assert!(packet.report_clean);
    assert!(packet.blocking_findings.is_empty());
    assert_eq!(packet.red_row_count, 0);
    assert!(packet.all_rows_publishable);
}

#[test]
fn seeded_packet_has_six_green_and_four_yellow_rows() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    assert_eq!(packet.green_row_count, 6);
    assert_eq!(packet.yellow_row_count, 4);
    assert_eq!(packet.red_row_count, 0);

    for family in [
        M5CommandSurfaceFamily::KeybindingResolverLayer,
        M5CommandSurfaceFamily::ImportBridgeRow,
        M5CommandSurfaceFamily::LeaderSequenceHelp,
        M5CommandSurfaceFamily::CommandDocumentationSurface,
    ] {
        assert_eq!(
            packet.row(family).unwrap().derived_status,
            ResolverInspectorStatus::Yellow,
            "{} should auto-narrow to yellow",
            family.as_str()
        );
    }
}

#[test]
fn every_row_status_is_the_derived_value() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    for row in &packet.rows {
        assert_eq!(
            row.derived_status,
            row.recompute_status(),
            "row {} declares a stale status",
            row.surface_family.as_str()
        );
        assert_eq!(row.conformance_causes, row.recompute_causes());
    }
}

#[test]
fn every_row_winner_and_shadowed_are_derived() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    for row in &packet.rows {
        assert_eq!(row.winning_source_class, row.recompute_winner());
        assert_eq!(row.shadowed_source_classes, row.recompute_shadowed());
        // The winner is the highest-precedence source among the declared classes.
        if let Some(winner) = row.winning_source_class {
            let max_rank = row
                .shortcut_source_classes
                .iter()
                .map(|source| source.precedence_rank())
                .max()
                .unwrap();
            assert_eq!(winner.precedence_rank(), max_rank);
            assert!(!row.shadowed_source_classes.contains(&winner));
        } else {
            assert!(row.shortcut_source_classes.is_empty());
            assert!(row.shadowed_source_classes.is_empty());
        }
    }
}

#[test]
fn every_row_certifies_all_inspector_fields_bridge_outcomes_and_migration_actions() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    for row in &packet.rows {
        assert!(
            row.inspector_fields_complete(),
            "row {} does not reveal all seven inspector fields",
            row.surface_family.as_str()
        );
        assert!(
            row.bridge_outcomes_complete(),
            "row {} does not render all six controlled bridge outcomes",
            row.surface_family.as_str()
        );
        assert!(
            row.migration_actions_complete(),
            "row {} does not offer all three migration actions",
            row.surface_family.as_str()
        );
        assert_eq!(
            row.certified_inspector_fields.len(),
            REQUIRED_INSPECTOR_FIELDS.len()
        );
        assert_eq!(
            row.certified_bridge_outcomes.len(),
            REQUIRED_BRIDGE_OUTCOMES.len()
        );
        assert_eq!(
            row.certified_migration_actions.len(),
            REQUIRED_MIGRATION_ACTIONS.len()
        );
    }
}

#[test]
fn every_row_certifies_all_declared_consumer_surfaces() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    for row in &packet.rows {
        assert!(
            row.consumer_surfaces_complete(),
            "row {} does not certify all declared consumer surfaces",
            row.surface_family.as_str()
        );
        assert!(row.headless_parity_preserved);
    }
}

#[test]
fn every_binding_is_pulled_from_the_frozen_matrix() {
    use crate::freeze_the_m5_menu_keybinding_resolver_and_command_documentation_matrix::seeded_m5_discoverability_matrix;

    let matrix = seeded_m5_discoverability_matrix();
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    for row in &packet.rows {
        let surface = matrix
            .surface_rows
            .iter()
            .find(|surface| surface.surface_family == row.surface_family)
            .expect("driving surface family is frozen by the matrix");
        assert_eq!(row.qualification, surface.qualification);
        assert_eq!(row.owner_role, surface.owner_role);
        assert_eq!(
            row.canonical_command_binding,
            surface.canonical_command_binding
        );
        assert_eq!(row.required_labels, surface.required_labels);
        assert_eq!(row.shortcut_source_classes, surface.shortcut_source_classes);
        assert_eq!(row.conflict_reasons, surface.conflict_reasons);
        assert_eq!(
            row.import_translation_states,
            surface.import_translation_states
        );
        assert_eq!(row.stale_target_states, surface.stale_target_states);
        assert_eq!(row.unavailable_reasons, surface.unavailable_reasons);
        assert_eq!(row.feature_families, surface.feature_families);
        assert_eq!(row.required_consumer_surfaces, surface.consumer_surfaces);
        assert_eq!(
            row.applicable_downgrade_triggers,
            surface.downgrade_triggers
        );
        assert!(!row
            .canonical_command_binding
            .command_id_field
            .trim()
            .is_empty());
    }
}

#[test]
fn narrowed_rows_disclose_a_reason() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    for row in &packet.rows {
        if !matches!(row.derived_status, ResolverInspectorStatus::Green) {
            assert!(
                row.narrowing_reason
                    .as_deref()
                    .map(|reason| !reason.trim().is_empty())
                    .unwrap_or(false),
                "narrowed row {} hides its reason",
                row.surface_family.as_str()
            );
        }
    }
}

#[test]
fn disclosed_reduced_sequence_hint_carries_an_active_waiver() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    let leader = packet
        .row(M5CommandSurfaceFamily::LeaderSequenceHelp)
        .unwrap();
    assert!(matches!(
        leader.leader_sequence_inspection,
        LeaderSequenceInspectionState::DisclosedReducedSequenceHint
    ));
    assert!(leader.requires_waiver());
    assert!(leader.has_active_waiver());
    assert_eq!(packet.active_waivers.len(), 1);
    assert!(packet.active_waivers[0].is_active_at(&packet.generated_at));
}

#[test]
fn disclosed_reduced_inspector_detail_keeps_resolver_layer_yellow_without_a_waiver() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    let layer = packet
        .row(M5CommandSurfaceFamily::KeybindingResolverLayer)
        .unwrap();
    assert!(matches!(
        layer.resolver_inspection,
        ResolverInspectionState::DisclosedReducedInspectorDetail
    ));
    assert_eq!(layer.derived_status, ResolverInspectorStatus::Yellow);
    assert!(layer.leader_sequence_inspection.is_full());
    assert!(!layer.requires_waiver());
}

#[test]
fn hidden_binding_blocks_the_resolver_layer() {
    let packet =
        seeded_m5_keybinding_resolver_inspectors_packet_resolver_layer_hidden_binding_blocked();
    let row = packet
        .row(M5CommandSurfaceFamily::KeybindingResolverLayer)
        .unwrap();
    assert_eq!(row.derived_status, ResolverInspectorStatus::Red);
    assert!(!packet.report_clean);
    assert!(!packet.all_rows_publishable);
    assert!(packet.red_row_count >= 1);
    assert!(packet.blocking_findings.iter().any(|finding| matches!(
        finding,
        ResolverInspectorFinding::ResolverInspectionBroken { .. }
    )));
    assert!(row.conformance_causes.iter().any(|cause| matches!(
        cause.trigger,
        M5DiscoverabilityDowngradeTrigger::ConflictWinnerAmbiguous
    )));
    assert!(validate_m5_keybinding_resolver_inspectors_packet(&packet).is_err());
}

#[test]
fn generic_imported_wording_blocks_the_import_bridge_row() {
    let packet =
        seeded_m5_keybinding_resolver_inspectors_packet_import_bridge_generic_wording_blocked();
    let row = packet.row(M5CommandSurfaceFamily::ImportBridgeRow).unwrap();
    assert_eq!(row.derived_status, ResolverInspectorStatus::Red);
    assert!(!packet.report_clean);
    assert!(packet.blocking_findings.iter().any(|finding| matches!(
        finding,
        ResolverInspectorFinding::BridgeOutcomeBroken { .. }
    )));
    assert!(row.conformance_causes.iter().any(|cause| matches!(
        cause.trigger,
        M5DiscoverabilityDowngradeTrigger::ImportTranslationUntruthful
    )));
    assert!(validate_m5_keybinding_resolver_inspectors_packet(&packet).is_err());
}

#[test]
fn hidden_knowledge_blocks_the_leader_overlay() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet_leader_hidden_knowledge_blocked();
    let row = packet
        .row(M5CommandSurfaceFamily::LeaderSequenceHelp)
        .unwrap();
    assert_eq!(row.derived_status, ResolverInspectorStatus::Red);
    assert!(!row.has_active_waiver());
    assert!(!packet.report_clean);
    assert!(packet.blocking_findings.iter().any(|finding| matches!(
        finding,
        ResolverInspectorFinding::LeaderSequenceInspectionBroken { .. }
    )));
    assert!(row.conformance_causes.iter().any(|cause| matches!(
        cause.trigger,
        M5DiscoverabilityDowngradeTrigger::ConflictWinnerAmbiguous
    )));
    assert!(validate_m5_keybinding_resolver_inspectors_packet(&packet).is_err());
}

#[test]
fn absent_capture_blocks_the_documentation_surface() {
    let packet =
        seeded_m5_keybinding_resolver_inspectors_packet_documentation_export_absent_blocked();
    let row = packet
        .row(M5CommandSurfaceFamily::CommandDocumentationSurface)
        .unwrap();
    assert_eq!(row.derived_status, ResolverInspectorStatus::Red);
    assert!(!packet.report_clean);
    assert!(packet.blocking_findings.iter().any(|finding| matches!(
        finding,
        ResolverInspectorFinding::ResolverExportBroken { .. }
    )));
    assert!(row
        .conformance_causes
        .iter()
        .any(|cause| matches!(cause.trigger, M5DiscoverabilityDowngradeTrigger::ProofStale)));
    assert!(validate_m5_keybinding_resolver_inspectors_packet(&packet).is_err());
}

#[test]
fn headless_parity_loss_blocks_the_conflict_sheet() {
    let packet =
        seeded_m5_keybinding_resolver_inspectors_packet_conflict_sheet_headless_parity_lost_blocked(
        );
    let row = packet
        .row(M5CommandSurfaceFamily::ConflictReviewSheet)
        .unwrap();
    assert_eq!(row.derived_status, ResolverInspectorStatus::Red);
    assert!(!row.headless_parity_preserved);
    assert!(!packet.report_clean);
    assert!(packet
        .blocking_findings
        .iter()
        .any(|finding| matches!(finding, ResolverInspectorFinding::HeadlessParityLost { .. })));
    assert!(row.conformance_causes.iter().any(|cause| matches!(
        cause.trigger,
        M5DiscoverabilityDowngradeTrigger::ParitySurfaceDropped
    )));
    assert!(validate_m5_keybinding_resolver_inspectors_packet(&packet).is_err());
}

#[test]
fn incomplete_inspector_field_set_blocks() {
    let mut packet = seeded_m5_keybinding_resolver_inspectors_packet();
    let row = packet
        .rows
        .iter_mut()
        .find(|row| row.surface_family == M5CommandSurfaceFamily::MenuItem)
        .unwrap();
    row.certified_inspector_fields.pop();
    assert!(!row.inspector_fields_complete());
    assert_eq!(row.recompute_status(), ResolverInspectorStatus::Red);
    let findings = row.compute_findings(&packet.generated_at);
    assert!(findings.iter().any(|finding| matches!(
        finding,
        ResolverInspectorFinding::InspectorFieldsIncomplete { .. }
    )));
}

#[test]
fn incomplete_bridge_outcome_set_blocks() {
    let mut packet = seeded_m5_keybinding_resolver_inspectors_packet();
    let row = packet
        .rows
        .iter_mut()
        .find(|row| row.surface_family == M5CommandSurfaceFamily::MenuItem)
        .unwrap();
    row.certified_bridge_outcomes.pop();
    assert!(!row.bridge_outcomes_complete());
    assert_eq!(row.recompute_status(), ResolverInspectorStatus::Red);
    let findings = row.compute_findings(&packet.generated_at);
    assert!(findings.iter().any(|finding| matches!(
        finding,
        ResolverInspectorFinding::BridgeOutcomesIncomplete { .. }
    )));
}

#[test]
fn incomplete_migration_action_set_blocks() {
    let mut packet = seeded_m5_keybinding_resolver_inspectors_packet();
    let row = packet
        .rows
        .iter_mut()
        .find(|row| row.surface_family == M5CommandSurfaceFamily::MenuItem)
        .unwrap();
    row.certified_migration_actions.pop();
    assert!(!row.migration_actions_complete());
    assert_eq!(row.recompute_status(), ResolverInspectorStatus::Red);
    let findings = row.compute_findings(&packet.generated_at);
    assert!(findings.iter().any(|finding| matches!(
        finding,
        ResolverInspectorFinding::MigrationActionsIncomplete { .. }
    )));
}

#[test]
fn incomplete_consumer_surface_certification_blocks() {
    let mut packet = seeded_m5_keybinding_resolver_inspectors_packet();
    let row = packet
        .rows
        .iter_mut()
        .find(|row| row.surface_family == M5CommandSurfaceFamily::MenuItem)
        .unwrap();
    row.evaluated_consumer_surfaces.pop();
    assert!(!row.consumer_surfaces_complete());
    assert_eq!(row.recompute_status(), ResolverInspectorStatus::Red);
    let findings = row.compute_findings(&packet.generated_at);
    assert!(findings.iter().any(|finding| matches!(
        finding,
        ResolverInspectorFinding::ConsumerSurfacesIncomplete { .. }
    )));
}

#[test]
fn stale_resolution_is_flagged() {
    let mut packet = seeded_m5_keybinding_resolver_inspectors_packet();
    let row = packet
        .rows
        .iter_mut()
        .find(|row| row.surface_family == M5CommandSurfaceFamily::MenuItem)
        .unwrap();
    // Corrupt the declared winner so it no longer matches the derived resolution.
    row.winning_source_class = Some(M5ShortcutSourceClass::PlatformDefault);
    let findings = row.compute_findings(&packet.generated_at);
    assert!(findings
        .iter()
        .any(|finding| matches!(finding, ResolverInspectorFinding::ResolutionStale { .. })));
}

#[test]
fn dashboard_projects_every_row_and_counts() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    let dashboard = packet.dashboard();
    assert_eq!(dashboard.rows.len(), packet.rows.len());
    assert_eq!(dashboard.green_row_count, packet.green_row_count);
    assert_eq!(dashboard.yellow_row_count, packet.yellow_row_count);
    assert_eq!(dashboard.red_row_count, packet.red_row_count);
    assert_eq!(dashboard.all_rows_publishable, packet.all_rows_publishable);
    assert_eq!(dashboard.source_packet_ref, packet.packet_id);
    assert!(!dashboard.command_automation_refs.is_empty());

    let leader = dashboard
        .rows
        .iter()
        .find(|row| row.surface_family == M5CommandSurfaceFamily::LeaderSequenceHelp)
        .unwrap();
    assert_eq!(leader.status, ResolverInspectorStatus::Yellow);
    assert!(leader.has_active_waiver);
    assert!(matches!(
        leader.leader_sequence_inspection,
        LeaderSequenceInspectionState::DisclosedReducedSequenceHint
    ));
}

#[test]
fn support_export_quotes_packet_matrix_and_waiver_refs() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    let export = ResolverInspectorSupportExport::from_packet(
        M5_RESOLVER_INSPECTORS_SUPPORT_EXPORT_ID,
        packet.clone(),
    );
    assert!(export.case_ids.contains(&packet.packet_id));
    assert!(export.case_ids.contains(&packet.matrix_packet_ref));
    assert!(export.case_ids.contains(&packet.build_identity_ref));
    for row in &packet.rows {
        assert!(export
            .case_ids
            .contains(&row.surface_family.as_str().to_owned()));
    }
    for waiver in &packet.active_waivers {
        assert!(export.case_ids.contains(&waiver.waiver_id));
    }
    assert_eq!(export.dashboard, packet.dashboard());
}

#[test]
fn markdown_and_csv_name_every_surface_family() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    let markdown = packet.render_markdown();
    let csv = packet.render_matrix_csv();
    for family in REQUIRED_SURFACE_FAMILIES {
        assert!(
            csv.contains(family.as_str()),
            "csv omits {}",
            family.as_str()
        );
    }
    assert!(markdown.contains("m5_keybinding_resolver_inspectors_fixtures"));
    assert!(markdown.contains("waiver:leader-sequence-reduced-hint:0001"));
}

#[test]
fn export_carries_no_forbidden_material() {
    let packet = seeded_m5_keybinding_resolver_inspectors_packet();
    let json = packet.export_safe_json().to_lowercase();
    assert!(!json.contains("api_key"));
    assert!(!json.contains("password"));
    assert!(!json.contains("bearer "));
    assert!(!json.contains("://"));
}

#[test]
fn expired_waiver_is_flagged() {
    let waiver = ResolverInspectorWaiver {
        waiver_id: "waiver:expired:0001".to_owned(),
        surface_family: M5CommandSurfaceFamily::LeaderSequenceHelp,
        reason: "test".to_owned(),
        owner_role: "owner".to_owned(),
        expires_at: "2026-01-01T00:00:00Z".to_owned(),
    };
    assert!(!waiver.is_active_at("2026-06-30T00:00:00Z"));
    assert!(waiver.is_active_at("2025-01-01T00:00:00Z"));
}

#[test]
fn bridge_outcome_migration_incomplete_classification() {
    assert!(M5BridgeOutcomeState::Partial.migration_incomplete());
    assert!(M5BridgeOutcomeState::Shimmed.migration_incomplete());
    assert!(M5BridgeOutcomeState::Unsupported.migration_incomplete());
    assert!(!M5BridgeOutcomeState::Exact.migration_incomplete());
    assert!(!M5BridgeOutcomeState::Translated.migration_incomplete());
    assert!(!M5BridgeOutcomeState::AliasOnly.migration_incomplete());
}
