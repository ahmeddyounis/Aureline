//! Tests for the M05-859 runtime-boundary component surface certification
//! capstone.

use super::*;

fn packet() -> RuntimeSurfaceCertificationPacket {
    seeded_m5_runtime_boundary_component_certification_packet()
}
...
fn packet_record_kind_and_schema_version_are_stamped() {
    let p = packet();
    assert_eq!(p.record_kind, RUNTIME_CERT_RECORD_KIND);
    assert_eq!(p.schema_version, RUNTIME_CERT_SCHEMA_VERSION);
}
...
fn every_claimed_surface_is_certified_exactly_once() {
    ...
    for surface in M5RuntimeCertifiedSurface::ALL {
...
#[test]
fn packet_has_six_green_and_four_yellow_and_no_red() {
    let p = packet();
    assert_eq!(p.summary.green_row_count, 6);
...
fn every_row_scores_every_axis_exactly_once() {
    ...
    for row in &p.rows {
        ...
        assert_eq!(row.axis_outcomes.len(), RuntimeCertificationAxis::ALL.len());
    }
...
fn every_row_cites_the_one_canonical_bundle() {
    let p = packet();
    assert_eq!(p.canonical_bundle_ref, RUNTIME_CERT_CANONICAL_BUNDLE_REF);
    for row in &p.rows {
        assert_eq!(row.canonical_bundle_ref, RUNTIME_CERT_CANONICAL_BUNDLE_REF);
    }
...
fn surface_tokens_are_unique() {
    let tokens: BTreeSet<&str> = M5RuntimeCertifiedSurface::ALL
        .iter()
        .map(|s| s.as_str())
        .collect();
    assert_eq!(tokens.len(), M5RuntimeCertifiedSurface::ALL.len());
}

#[test]
fn axis_tokens_are_distinct() {
    let axes: BTreeSet<&str> = RuntimeCertificationAxis::ALL
        .iter()
        .map(|a| a.as_str())
        .collect();
    assert_eq!(axes.len(), RuntimeCertificationAxis::ALL.len());
}

#[test]
fn only_cli_export_axis_is_always_on() {
    for axis in RuntimeCertificationAxis::ALL {
        assert_eq!(
            axis.is_always_on(),
            axis == RuntimeCertificationAxis::CliExport
        );
...
fn undisclosed_drift_blocks_the_surface() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5RuntimeCertifiedSurface::Terminal)
        .expect("terminal row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RuntimeCertificationAxis::Visual {
            outcome.state = AxisCertificationState::UndisclosedDrift;
            outcome.narrowing_reason = Some("host boundary silently stale".to_owned());
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, RuntimeCertificationViolation::SurfaceBlocked { .. })));
}

#[test]
fn degraded_axis_without_claim_narrowing_blocks() {
    // A disclosed-narrowed axis but the claim stays Live => hidden overclaim.
    let mut p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5RuntimeCertifiedSurface::NotebookConsole)
        .expect("notebook row exists");
    let row = &mut p.rows[idx];
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RuntimeCertificationAxis::DegradedState {
            *outcome = RuntimeAxisOutcome {
                axis: RuntimeCertificationAxis::DegradedState,
                state: AxisCertificationState::DisclosedNarrowed,
                parity_note: "kernel liveness lagging".to_owned(),
                narrowing_reason: Some("kernel liveness is partial for this notebook".to_owned()),
    ...
    // Claim stays Live == certified Live, no claim_auto_narrow.
...
fn cli_export_drop_blocks_the_surface() {
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RuntimeCertificationAxis::CliExport {
            ...
            outcome.downgrade_trigger =
                Some(M5RuntimeBoundaryDowngradeTrigger::AuditTruthLostOffPrimarySurface);
        }
    }
    assert_eq!(row.derive_status(), SurfaceClaimStatus::Red);
}
...
fn spurious_claim_auto_narrow_without_claim_reduction_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.claim_auto_narrow = Some(RuntimeClaimAutoNarrow {
        binding_axis: RuntimeCertificationAxis::DegradedState,
        from_claim: M5RuntimeSupportClaim::Live,
        to_claim: M5RuntimeSupportClaim::Live,
...
fn claim_narrowed_without_disclosure_blocks() {
    let mut p = packet();
    let row = &mut p.rows[0];
    row.certified_claim = M5RuntimeSupportClaim::Degraded;
...
fn certified_claim_above_claim_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5RuntimeCertifiedSurface::Export)
        .expect("export row exists");
    let row = &mut p.rows[idx];
    row.certified_claim = M5RuntimeSupportClaim::Live; // claimed is Restored
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        RuntimeCertificationViolation::CertifiedClaimExceedsClaim { .. }
    )));
}
...
fn claim_auto_narrow_bound_to_wrong_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5RuntimeCertifiedSurface::Support)
        .expect("support row exists");
    let row = &mut p.rows[idx];
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = RuntimeCertificationAxis::Visual;
    }
    assert_eq!(row.derive_status(), SurfaceClaimStatus::Red);
}
...
fn claim_auto_narrow_bound_to_always_on_axis_blocks() {
    ...
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5RuntimeCertifiedSurface::Debug)
        .expect("debug row exists");
    let row = &mut p.rows[idx];
    ...
    for outcome in &mut row.axis_outcomes {
        if outcome.axis == RuntimeCertificationAxis::CliExport {
            outcome.state = AxisCertificationState::DisclosedNarrowed;
            outcome.narrowing_reason =
                Some("export parity is not current for the debug view".to_owned());
            outcome.downgrade_trigger =
                Some(M5RuntimeBoundaryDowngradeTrigger::AuditTruthLostOffPrimarySurface);
        }
    }
    if let Some(narrow) = row.claim_auto_narrow.as_mut() {
        narrow.binding_axis = RuntimeCertificationAxis::CliExport;
    }
    assert_eq!(row.derive_status(), SurfaceClaimStatus::Red);
}
...
fn certified_axis_carrying_a_reason_is_malformed() {
    let mut o = seed_certified(RuntimeCertificationAxis::Visual);
...
fn missing_surface_is_rejected() {
    let mut p = packet();
    p.rows
        .retain(|r| r.surface != M5RuntimeCertifiedSurface::Support);
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, RuntimeCertificationViolation::SurfaceCoverageIncomplete)));
}

#[test]
fn missing_family_coverage_is_rejected() {
    // Drop the presence-avatar-stack family entirely by neutralizing the
    // collaboration row's families; coverage must fail.
    let mut p = packet();
    let idx = p
        .rows
        .iter()
        .position(|r| r.surface == M5RuntimeCertifiedSurface::Collaboration)
        .expect("collaboration row exists");
    p.rows[idx].consumed_families = vec![M5RuntimeBoundaryComponentFamily::RemoteTargetPill];
    p.rows[idx].derived_status = p.rows[idx].derive_status();
    p.summary = p.computed_summary();
    assert!(!p.all_families_covered());
    let violations = p.validate();
    assert!(violations
        .iter()
        .any(|v| matches!(v, RuntimeCertificationViolation::FamilyCoverageIncomplete)));
}
...
fn stale_derived_status_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        RuntimeCertificationViolation::StatusDerivationStale { .. }
    )));
}
...
fn wrong_canonical_bundle_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        RuntimeCertificationViolation::RowMissingCanonicalBundle { .. }
    )));
}
...
fn packet_level_wrong_bundle_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, RuntimeCertificationViolation::WrongCanonicalBundle)));
}
...
fn duplicate_row_id_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, RuntimeCertificationViolation::DuplicateId { .. })));
}
...
fn axis_coverage_gap_is_rejected() {
    ...
    p.rows[0]
        .axis_outcomes
        .retain(|o| o.axis != RuntimeCertificationAxis::RestoreNoRerun);
    p.rows[0].derived_status = p.rows[0].derive_status();
    p.summary = p.computed_summary();
    let violations = p.validate();
    assert!(violations.iter().any(|v| matches!(
        v,
        RuntimeCertificationViolation::AxisCoverageIncomplete { .. }
    )));
}
...
fn summary_mismatch_is_rejected() {
    ...
    assert!(violations
        .iter()
        .any(|v| matches!(v, RuntimeCertificationViolation::SummaryMismatch)));
}
...
fn forbidden_material_in_export_is_rejected() {
    ...
    assert!(violations.iter().any(|v| matches!(
        v,
        RuntimeCertificationViolation::RawBoundaryMaterialInExport
    )));
}
...
fn export_json_round_trips() {
    ...
    let back: RuntimeSurfaceCertificationPacket = serde_json::from_str(&json).expect("round trips");
    assert_eq!(p, back);
...
fn checked_in_export_matches_seeded_builder() {
    let on_disk =
        current_m5_runtime_boundary_component_certification_export().expect("export is valid");
    assert_eq!(
...
fn generate_artifacts() {
    if std::env::var("GEN_RUNTIME_CERT_ARTIFACTS").is_err() {
        return;
    }
    use std::fs;
    use std::path::Path;

    let packet = seeded_m5_runtime_boundary_component_certification_packet();
    ...
    let art = Path::new(manifest)
        .join("../../artifacts/release/m5-runtime-boundary-component-certification-proof");
    ...
    let fixtures =
        Path::new(manifest).join("../../fixtures/ui/m5-runtime-boundary-component-certification");
    ...
    fs::write(
        ...
        "# M5 runtime-boundary component surface certification fixtures\n\n\
         Mirror of `artifacts/release/m5-runtime-boundary-component-certification-proof/`.\n\
         Regenerate with `GEN_RUNTIME_CERT_ARTIFACTS=1 cargo test -p aureline-shell \
         m5_runtime_boundary_component_certification::tests::generate_artifacts`.\n",
    )
