# react_expert_workflow_pack_truth_packet fixture corpus

Fixture corpus for the M4 stable React Expert workflow pack truth packet
(`schemas/language/react_expert_workflow_pack_truth.schema.json`).

Each fixture is a `ReactExpertWorkflowPackTruthPacketInput` with an
...
`ReactExpertWorkflowPackTruthPacket::materialize` agrees.

Cases:

- `baseline_stable.json` — The React Expert workflow pack carries a
  pack-qualification row at `expert_grade` plus every certified
  workflow-loop row (create, open, run, test, debug, rename, review),
  a framework-migration row, and one disclosed known-limit row that
  narrows below expert grade with a disclosure ref. Every row binds
  support, known limit, downgrade automation, and evidence classes;
...
- `expert_grade_with_unbound_evidence_blocks_stable.json` — The pack
  ...
  fixture-repo, migration, or archetype evidence backs the row.
...
- `narrowed_row_missing_disclosure_ref_blocks_stable.json` — A known
  limit row narrows below expert grade but drops its disclosure ref;
