# vue_advanced_workflow_pack_truth_packet fixture corpus

Fixture corpus for the M4 stable Vue Advanced workflow pack truth
packet
(`schemas/language/vue_advanced_workflow_pack_truth.schema.json`).

Each fixture is a `VueAdvancedWorkflowPackTruthPacketInput` with an
...
`VueAdvancedWorkflowPackTruthPacket::materialize` agrees.

Cases:

- `baseline_stable.json` — The Vue Advanced workflow pack carries a
  pack-qualification row at `expert_grade` plus every certified
  workflow-loop row (create, open, run, test, debug, rename, review),
  a framework-migration row certifying the Vue 2 Options API → Vue 3
  Composition API migration, a project-model row certifying the Vue
  single-file-component `<template>`/`<script setup>`/`<style>` split
  and Composition API vs Options API, and a build-tool parity row
  certifying the Vite, Webpack, and Vue CLI dev/build/test/preview
  surfaces. Every row binds support, known limit, downgrade
...
- `expert_grade_with_unbound_evidence_blocks_stable.json` — The pack
  ...
  fixture-repo, migration, archetype, project-model, or build-tool
  parity evidence backs the row.
...
- `narrowed_row_missing_disclosure_ref_blocks_stable.json` — A
  ...
  `project_model_subset_only` but drops its disclosure ref;
