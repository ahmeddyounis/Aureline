...
| [`template_gallery_admin_policy_narrowed.json`](./template_gallery_admin_policy_narrowed.json) | `template_and_prebuild_gallery_record` + `starter_policy_notice_record` | Organization admin policy narrows the gallery; in-place `admin_policy_notice` names `availability_narrowing_class = policy_narrowed_admin` and cites `policy_narrowing_class_refs = [admin_policy]`; bypass paths remain at equal weight. | §3.8, §3.14, §10, §13.9 |
...
| [`template_gallery_offline_no_bundle.json`](./template_gallery_offline_no_bundle.json) | `template_and_prebuild_gallery_record` + `starter_policy_notice_record` | Offline startup with no signed bundle and no usable mirror cache; `mirror_or_airgap_policy_notice` names `availability_narrowing_class = offline_no_bundle` and cites `resolution_hook_refs = [refresh_mirror, switch_to_live_origin, set_up_later]`; all four offline bypass paths render. | §3.6, §3.8 (offline_no_bundle), §10, §13.10 |
| [`template_gallery_target_runtime_unavailable.json`](./template_gallery_target_runtime_unavailable.json) | `template_and_prebuild_gallery_record` + `starter_policy_notice_record` | Gallery with a visible-but-disabled managed-cloud row; per-row `target_runtime_unavailable_notice` names `availability_narrowing_class = target_runtime_unavailable` and offers `reconnect_required` / `continue_in_restricted_mode` / `set_up_later`. | §3.4, §3.8 (target_runtime_unavailable), §3.14, §10, §13.11 |
| [`template_gallery_toolchain_os_incompatible.json`](./template_gallery_toolchain_os_incompatible.json) | `template_and_prebuild_gallery_record` with `template_health_row_record`s | Template gallery inline health strip surfaces `os_runtime_compatibility → fail (live)` plus `toolchain_compatibility → warn (cached, older)` so cards do not over-claim compatibility; bypass remains unblocked. | §3.10–§3.11, §9, §11.11, §13.12 |
