...
| `importer_review_table.json` | Shared typed fixture | Outcome-grouped review table proving `imported`, `mapped`, `skipped`, `manual_review`, `bridge_required`, and `unsupported` rows across settings, shortcuts, extensions, tasks, and workspace metadata remain visible after apply/export. It also preserves the shortcut-change digest, bridge inspectors, compatibility report links, issue-template export, partial-apply summary, and restore summary after apply. |
