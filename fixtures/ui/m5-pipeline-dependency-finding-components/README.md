...
| `dependency_row.json` | `schemas/ui/m5-dependency-row.schema.json` | A transitive dependency row preserves package name, ecosystem, manifest scope, version delta, lockfile impact, advisory count, visible limited update state, license action, and changelog action. |
| `manifest_diff_card.json` | `schemas/ui/m5-manifest-diff-card.schema.json` | A staged manifest diff previews scripts/hooks and peer/runtime constraint changes while naming checkpoint, rollback, apply boundary, and companion inspect-only parity. |
| `security_finding_card.json` | `schemas/ui/m5-security-finding-card.schema.json` | A critical no-fix-yet package finding keeps finding class, severity, confidence, freshness, affected scope, fix availability, exception-expired suppression label, exposure, remediation path, local validation, docs/help path, and audit actions visible. |
