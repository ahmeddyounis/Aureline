...
| [`state_map_row_imported_profile_state.json`](./state_map_row_imported_profile_state.json) | `state_map_row_record` | `imported_profile_state` / `user_authored_durable_truth` / `local_only` / reviewed portable projection required | §6 State map rows, §7 Export manifest rules |
