...
| Surface certification JSON (`include_str!`) | `artifacts/release/m5-deployment-continuity-component-proof/certification.json` |
| Surface certification CSV | `artifacts/release/m5-deployment-continuity-component-proof/certification.csv` |
| Surface certification report | `artifacts/release/m5-deployment-continuity-component-proof/certification.md` |
...
`checked_support_export_matches_builder` test keeps them byte-aligned. The surface
certification is generated by
`seeded_deployment_continuity_surface_certification()` and kept aligned by
`checked_surface_certification_matches_builder`.

## Individual component schemas

The matrix governs the family as a whole; the following stable UI contracts govern the
payload shape of each reusable primitive so downstream consumers can validate a single
card, row, sheet, or strip without copying the full matrix schema.

| Family | Schema |
| --- | --- |
| `install_profile_card` | `schemas/ui/m5-install-profile-card.schema.json` |
| `side_by_side_import_sheet` | `schemas/ui/m5-side-by-side-import-sheet.schema.json` |
| `rollout_ring_row` | `schemas/ui/m5-rollout-ring-row.schema.json` |
| `deployment_summary_card` | `schemas/ui/m5-deployment-summary-card.schema.json` |
| `residual_dependency_row` | `schemas/ui/m5-residual-dependency-row.schema.json` |
| `control_plane_data_plane_status_strip` | `schemas/ui/m5-control-plane-data-plane-status-strip.schema.json` |
| `mirror_offline_artifact_row` | `schemas/ui/m5-mirror-offline-artifact-row.schema.json` |
| `mode_change_review_sheet` | `schemas/ui/m5-mode-change-review-sheet.schema.json` |
| `channel_association_review_row` | `schemas/ui/m5-channel-association-review-row.schema.json` |
...
The release/support proof packet certifies the shared family across every claimed M5
deployment surface: `local_only`, `managed`, `self_hosted`, `mirrored`, `sovereign`,
`air_gapped`, `side_by_side`, `portable`, and `fleet_rollout`.

Each surface row carries four required drill families (`functional`, `accessibility`,
`export`, and `degradation`) plus compatibility notes for channel ownership, handler
precedence, mirror/offline freshness, and control-plane/data-plane continuity. A row with
stale mirror truth, residual dependency, unavailable state root, paused rollout, or
control-plane impairment must publish a narrowed effective label (`degraded_narrowed` or
`local_safe_only`) rather than inherit the `full_truth` label from a healthier surface.
