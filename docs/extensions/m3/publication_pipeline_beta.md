...
links the canonical lifecycle/deprecation packet, promotes it through
the governed registry channels, and preserves the previous installable
artifact for rollback. It does not add marketplace ranking,
recommendations, vanity listing fields, or new permission classes.
...
| Compatibility metadata | compatibility report ref, bridge matrix ref, bridge matrix row ref, lifecycle metadata ref, deprecation packet template ref, host contract family refs, capability world refs, target platforms, support class, bridge state |
...
marketplace and SDK docs use, and the same lifecycle metadata row that
defines support-window and deprecation behavior.
