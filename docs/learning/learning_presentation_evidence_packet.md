...
| Guided surfaces, glossary packs, architecture maps, and tour steps must use the shared knowledge-pack and learning-tour matrix.           | `/docs/docs/knowledge_pack_and_learning_tour_matrix.md` plus §"Surface coverage" (re-exports `guided_surface_kind`); §"Cached-pack continuity" (re-exports `freshness_class`); §"Tour-citation audit" (re-exports `waypoint_anchor_kind`). |
