# M5 remote-cache-integrity-finding and cache-bypass-drill registries

This lane is the remote-cache-poisoning-detection and cache-bypass-drill implement lane over the frozen
...
- **Canonical Rust module:**
  `crates/aureline-ui/src/m5_remote_cache_integrity_and_cache_bypass_drill_registries` (the authoritative
  validator).
- **Combined schema:**
...
- **Domain schemas:** every row points at
  [`schemas/release/m5-remote-cache-integrity-finding.schema.json`](../../schemas/release/m5-remote-cache-integrity-finding.schema.json)
  and
  [`schemas/release/m5-cache-bypass-drill.schema.json`](../../schemas/release/m5-cache-bypass-drill.schema.json)
  as its canonical domain contracts.
- **Checked proof:**
  `artifacts/release/m5-remote-cache-integrity-and-cache-bypass-drill-registries-proof/`
  (`support_export.json`, `matrix.csv`, `summary.md`).
- **Narrowed fixtures:** `fixtures/release/m5-remote-cache-integrity-and-cache-bypass-drill-registries/`
  (`hermetic_rebuild_beta_narrowed.json`, `artifact_diff_preview_narrowed.json`).

## Two registries

1. **Clean-room-rebuild lane** (`resolve_remote_cache_integrity_finding_entry`) — publishes one typed
...
2. **Artifact-diff packet** (`resolve_cache_bypass_drill_entry`) — keeps the deterministic artifact-diff packet
...
`remote_cache_integrity_finding_object_is_complete` rejects an object missing any field,
...
object degrades to `remote_cache_integrity_finding_object_incomplete`, and a divergent or omitted-family diff degrades to
...
```text
cargo run -p aureline-ui --example dump_m5_remote_cache_integrity_and_cache_bypass_drill_registries -- support-export
cargo run -p aureline-ui --example dump_m5_remote_cache_integrity_and_cache_bypass_drill_registries -- csv
cargo run -p aureline-ui --example dump_m5_remote_cache_integrity_and_cache_bypass_drill_registries -- report
cargo run -p aureline-ui --example dump_m5_remote_cache_integrity_and_cache_bypass_drill_registries -- remote-cache-integrity-finding-table
cargo run -p aureline-ui --example dump_m5_remote_cache_integrity_and_cache_bypass_drill_registries -- fixture-hermetic-rebuild-beta-narrowed
cargo run -p aureline-ui --example dump_m5_remote_cache_integrity_and_cache_bypass_drill_registries -- fixture-artifact-diff-preview-narrowed
```
