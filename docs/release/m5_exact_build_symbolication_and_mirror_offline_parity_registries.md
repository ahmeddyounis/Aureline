# M5 exact-build-symbolication and mirror-offline-parity registries

This lane is the exact-build-symbolication and mirror/offline-publication-parity implement lane over the frozen
[M5 build-lane-trust matrix](./m5_build_lane_trust_contract.md). It binds crash packets, symbol / source-map
manifests, support bundles, and symbolication reports to one exact-build descriptor so a support bundle can
explain exact versus approximate symbolication against one build identity, and it proves mirrored / offline
publication preserves the same build identity and freshness vocabulary as default publication so a mismatched
build ID or an omitted family is a blocker rather than a warning. The registry field and token names below are
inherited verbatim from the shared two-registry lane shape. It turns the *clean-room-rebuild-lane* grammar
...
- **Canonical Rust module:**
  `crates/aureline-ui/src/m5_exact_build_symbolication_and_mirror_offline_parity_registries` (the authoritative
  validator).
- **Combined schema:**
...
- **Domain schemas:** every row points at
  [`schemas/release/m5-exact-build-symbolication.schema.json`](../../schemas/release/m5-exact-build-symbolication.schema.json)
  and
  [`schemas/release/m5-mirror-offline-parity.schema.json`](../../schemas/release/m5-mirror-offline-parity.schema.json)
  as its canonical domain contracts.
- **Checked proof:**
  `artifacts/release/m5-exact-build-symbolication-and-mirror-offline-parity-registries-proof/`
  (`support_export.json`, `matrix.csv`, `summary.md`).
- **Narrowed fixtures:** `fixtures/release/m5-exact-build-symbolication-and-mirror-offline-parity-registries/`
  (`hermetic_rebuild_beta_narrowed.json`, `artifact_diff_preview_narrowed.json`).

## Two registries

1. **Clean-room-rebuild lane** (`resolve_exact_build_symbolication_entry`) — publishes one typed
...
2. **Artifact-diff packet** (`resolve_mirror_offline_parity_entry`) — keeps the deterministic artifact-diff packet
...
`exact_build_symbolication_object_is_complete` rejects an object missing any field,
...
object degrades to `exact_build_symbolication_object_incomplete`, and a divergent or omitted-family diff degrades to
