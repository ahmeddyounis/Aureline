...
- [`/docs/docs/reviewed_pack_and_late_copy_policy.md`](../docs/docs/reviewed_pack_and_late_copy_policy.md)
  ...
  — reviewed-pack binding and late-copy override contract for
  release-bearing docs/help, migration, support, and public-proof copy.
...
| `docs_help_truth` | Docs-pack applicability, Help/About version match, destination-route truth, reviewed-pack binding, and release-facing docs truth | `docs/docs/docs_pack_manifest_contract.md`, `docs/docs/help_about_service_health_routes.md`, `docs/docs/reviewed_pack_and_late_copy_policy.md`, `schemas/docs/docs_pack_manifest.schema.json`, `schemas/docs/help_status_badge.schema.json`, `schemas/docs/destination_descriptor.schema.json`, `schemas/docs/late_copy_change_packet.schema.json` | Required whenever the release claims docs/help applicability, ships a docs pack, or publishes a Help/About/service-health/support route | Raw docs bodies and raw URLs may stay outside the packet; manifest refs, descriptor refs, reviewed-pack refs, and version-match state may not |
...
- `advisory_or_revocation_notice` when an active advisory, emergency
  action, revocation, or forced-disable state touches the release.
...
7. protected-path ids plus the protected-path, latency-budget, and
   evidence-linkage revisions when the packet is making path-level
   claims rather than component-only claims;
8. the task script or success criterion the reader is meant to trust;
9. docs/help applicability via the docs-pack revision and
   `version_match_state`;
10. known limits, exclusions, and any waiver/advisory that narrows the
   claim; and
11. competitor settings whenever a public head-to-head comparison is
    made.
...
| Claim manifest | `schemas/governance/claim_manifest.schema.json`, `artifacts/governance/claim_manifest_seed.yaml`, `artifacts/governance/public_truth_parity_matrix.yaml`, `docs/governance/claim_manifest_contract.md`, `docs/docs/reviewed_pack_and_late_copy_policy.md` | cross-channel projections over `docs_help_truth`, `benchmark_public_proof`, `known_limit_and_disclosure_note`, `promotion_evidence`, and compatibility rows | Owns canonical public claim copy, downgrade routing, and channel bindings; downstream surfaces quote the same `claim_row_id` instead of re-authoring equivalent claims |
| Docs/help truth | `docs/docs/docs_pack_manifest_contract.md`, `docs/docs/help_about_service_health_routes.md`, `docs/docs/reviewed_pack_and_late_copy_policy.md`, `schemas/docs/docs_pack_manifest.schema.json`, `schemas/docs/help_status_badge.schema.json`, `schemas/docs/destination_descriptor.schema.json`, `schemas/docs/late_copy_change_packet.schema.json`, `docs/build/exact_build_identity_model.md` | `docs_help_truth` | Owns docs applicability, version-match truth, destination-route truth, and the reviewed-pack / late-copy binding used when post-freeze copy changes must remain auditable; release notes and benchmark packets quote it |
| Benchmark claims | `docs/benchmarks/benchmark_publication_pack_template.md`, `docs/benchmarks/benchmark_lab_run_results.md`, `docs/benchmarks/public_comparison_rules.md`, `schemas/benchmarks/run_result.schema.json`, `artifacts/bench/protected_metrics.yaml`, `artifacts/bench/fitness_function_catalog.yaml`, `fixtures/benchmarks/corpus_manifest.yaml`, `artifacts/perf/protected_path_ledger.yaml`, `artifacts/perf/latency_budget_ledger.yaml`, `artifacts/perf/evidence_linkage_seed.yaml` | `benchmark_public_proof` | Owns public benchmark caveats, comparability, protected-metrics revisioning, protected-path ids, and competitor-configuration disclosure |
| Support/export packets | `artifacts/support/`, `docs/state/profile_and_state_map.md`, `docs/build/exact_build_identity_model.md`, `docs/security/severity_matrix.md`, `docs/docs/docs_pack_manifest_contract.md`, `docs/docs/reviewed_pack_and_late_copy_policy.md`, `schemas/docs/late_copy_change_packet.schema.json` | support-export projections over `build_identity`, `docs_help_truth`, `advisory_or_revocation_notice`, and redaction-bound state/export records | Support/export surfaces quote upstream refs, reviewed-pack ids, and redaction classes; they do not invent a release-only or benchmark-only dialect |
