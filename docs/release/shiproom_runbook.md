...
| `object_issue_handoff` | preserve the exact object a user reported from, its current state vocabulary, destination route, and related support/incident/recovery refs | `object_stable_id`, `object_kind_class`, `source_surface_class`, `support_route_ref`, `exact_build_identity_ref`, plus `source_destination_descriptor_ref` / `target_destination_descriptor_ref` or recovery/support refs where applicable |
