...
The cross-tool schema family is three files. The eventual settings
...
`control_authority`). The vocabulary doc lists each token; the JSON
Schemas are the authoritative enum set and bind together by the shared
