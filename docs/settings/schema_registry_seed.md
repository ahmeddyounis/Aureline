...
| [`/schemas/settings/effective_control_stack_row.schema.json`](../../schemas/settings/effective_control_stack_row.schema.json) | `effective_control_stack_row` / `effective_control_stack_matrix` | Cross-surface effective control-stack matrix packet and row. Captures source, value/branch projection, lifecycle label, last refresh, expiry, offline fallback, explain-why handle, schema refs, and export-safe evidence refs so settings UI, CLI inspect, diagnostics, support export, policy explainers, and experiment/kill-switch surfaces share one decision trace rather than recomputing narratives. See [`docs/settings/effective_control_stack_contract.md`](./effective_control_stack_contract.md). |
