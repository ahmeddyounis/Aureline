# Stabilize the Django Advanced workflow pack with manage.py targets, test/debug, and template awareness — stable contract

Status: Stable lane proof for the Django Advanced workflow pack.

This document is the reviewer-facing contract for the stable Django
Advanced workflow pack truth packet. The packet is the single source
of truth that the editor framework pack panel, workflow companion,
...
The packet pins the Django Advanced workflow pack across three
intertwined truths:

1. The **app workflow loops** — create, open, run, test, debug,
   rename, and review on Django archetype repos (Django 4.2 LTS
   project archetype and Django 5.x project archetype).
2. The **Django project model** — every row that crosses the Django
   project structure (`settings.py` configuration, `urls.py` URL
   conf, app boundary inside `INSTALLED_APPS`, `manage.py` and
   `wsgi.py`/`asgi.py` entry points, plus
   `requirements*.txt`/`pyproject.toml` dependency closure) binds a
   dedicated `project_model_row` and a disclosure ref so the workflow
   pack never confuses one project model for another.
3. The **`manage.py` target parity and template awareness** —
   `manage.py` target parity (`runserver`, `test`, `makemigrations`,
   `migrate`, `shell`, `createsuperuser`, `collectstatic`, custom
   management commands plus the `pytest`/`pytest-django` test runner
   surface and the `python -m pdb` /
   `manage.py runserver --noreload` debug entry points) is bound by a
   dedicated `manage_py_target_parity_row`, and Django template
   awareness (Django Template Language tags `{% %}`, variable
   interpolation `{{ }}`, `{% url %}`/`{% block %}`/`{% extends %}`/
   `{% include %}`/`{% csrf_token %}`, custom template tags / filters,
   and the editor's awareness of `app/templates/` vs project
   `TEMPLATES['DIRS']` resolution) is bound by a dedicated
   `template_awareness_row` so run/test/debug and review steps that
   depend on the template DSL surface the gap rather than over-claim.
...
1. The **workflow pack class** — currently
   `django_advanced_workflow_pack`. Every certified packet MUST carry
   at least one row for each required pack.
2. The **workflow-pack row class** — one of `pack_qualification`,
   `workflow_loop`, `framework_migration_row`, `archetype_repo_row`,
   `project_model_row`, `manage_py_target_parity_row`,
   `template_awareness_row`, `design_partner_row`, `unsupported_gap`,
   `known_limit`, or `downgrade_automation`. A `workflow_loop` row
   MUST bind a real workflow-loop step; no other row class is
   permitted to bind one.
...
5. The **evidence class** — one of `archetype_repo_evidence`,
   ...
   `template_awareness_evidence`, `docs_disclosure_evidence`, or
...
6. The **known-limit class** — one of `none_declared`,
   ...
   `project_model_subset_only`, `manage_py_target_subset_only`,
   `template_awareness_subset_only`, `unsupported_runtime_target`,
...
those booleans to false. The packet never admits raw Django source
bodies (views, models, templates, settings), environment variable
values, `.env` secrets, ambient credentials, `SECRET_KEY` material,
database credentials, or provider payloads. `manage.py` target parity
and template awareness rows bind only the *surface* (target names,
script entry points, template-tag shapes, template-dir resolution
edges) — never the secret values themselves.
...
- Reviewer artifact: [`artifacts/language/m4/stabilize-the-django-advanced-workflow-pack-with-manage.md`](../../../artifacts/language/m4/stabilize-the-django-advanced-workflow-pack-with-manage.md)
- Checked-in packet: [`artifacts/language/m4/django_advanced_workflow_pack_truth_packet.json`](../../../artifacts/language/m4/django_advanced_workflow_pack_truth_packet.json)
...
- Replay tests: [`crates/aureline-language/tests/django_advanced_workflow_pack_truth_packet.rs`](../../../crates/aureline-language/tests/django_advanced_workflow_pack_truth_packet.rs)
