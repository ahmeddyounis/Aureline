# Vue Advanced workflow pack truth packet — reviewer artifact

This is the reviewer-facing artifact for the M4 stable Vue Advanced
...
loop coverage, Vue SFC project-model evidence (Composition API /
Options API), Vite / Webpack / Vue CLI build-tool parity evidence,
...
and is replayed by
`crates/aureline-language/tests/vue_advanced_workflow_pack_truth_packet.rs`.

## Stable claim

For the governed workflow pack class (`vue_advanced_workflow_pack`)
the packet binds:
...
- at least one `framework_migration_row` certifying the Vue 2 Options
  API → Vue 3 Composition API migration archetype,
- at least one `project_model_row` certifying the Vue single-file
  component `<template>`, `<script setup>`/`<script>`, and `<style>`
  block boundary plus Composition API vs Options API project model,
- at least one `build_tool_parity_row` certifying the Vite, Webpack,
  and Vue CLI dev/build/test/preview surfaces (including Vitest, Vue
  Test Utils, and Vue Devtools debug surfaces),
...
- a closed `evidence_class` (archetype-repo, framework-migration,
  ...
  project-model, build-tool parity, or docs-disclosure),
- a closed `known_limit_class` (framework subset, language subset,
  archetype subset, migration subset, project-model subset,
  build-tool parity subset, unsupported runtime target, beta
...
- a closed `downgrade_automation_class` (auto-narrow on missing
  ...
  unproven project model / unproven build-tool parity, auto-demote on
  low confidence, auto-block on missing evidence, manual-only, or
  `none`),
...
- Schema: `schemas/language/vue_advanced_workflow_pack_truth.schema.json`
- Checked-in packet:
...
- Rust contract:
  `crates/aureline-language/src/vue_advanced_workflow_pack_truth_packet/mod.rs`
- Replay tests:
  `crates/aureline-language/tests/vue_advanced_workflow_pack_truth_packet.rs`
- Reviewer doc:
  `docs/languages/m4/stabilize-the-vue-advanced-workflow-pack-with-project.md`
...
- Reviewer doc: [`docs/languages/m4/stabilize-the-vue-advanced-workflow-pack-with-project.md`](../../../docs/languages/m4/stabilize-the-vue-advanced-workflow-pack-with-project.md)
