...
| Workload | Concrete jobs | Queue lane | Budget domains | Collapse / staleness | Checkpoint | Restore fidelity | No hidden rerun | Session continuity | Transcript export default | Terminal boundary | Clipboard posture | Downgrade rule |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Notebook session | `notebook.cell_execution` | `interactive_background` | `knowledge_refresh_budget` | `workspace_slice_target` / `restart_after_supersede` / `refresh_on_resume` | `item_boundary` | `compatible_restore` | `explicit_rerun_only` | `transcript_restored` / `rerun_required` / `authority_current` | `redacted_transcript_default` | `local` | `local_direct` | `auto_narrow_on_queue_metadata_stale` |
| Data query console | `data.request_collection_run` | `foreground` | `foreground_task_budget` | `session_surface_target` / `replace_superseded` / `refresh_on_resume` | `none_declared` | `exact_restore` | `metadata_only_resume` | `live_session_continuity` / `none_required` / `authority_current` | `reviewed_full_transcript_opt_in` | `remote` | `bracketed_paste_review` | `auto_narrow_on_restore_fidelity_stale` |
| Pipeline run | `pipeline.log_pull`, `pipeline.artifact_pull` | `provider_overlay` | `provider_overlay_budget` | `provider_route_target` / replace-or-coalesce / refresh-or-requeue | `explicit_phase_boundary` | `evidence_only` | `blocked_until_manual_review` | `session_ended` / `rerun_required` / `authority_expired` | `metadata_only_default` | `managed` | `metadata_only_export` | `auto_narrow_on_retry_budget_exhausted` |
| Preview route | `preview.dev_server`, `preview.route_refresh` | `maintenance` | `maintenance_budget`, `knowledge_refresh_budget` | `workspace_slice_target` / restart-or-coalesce / refresh-or-drop | `time_boundary` | `placeholder_only` | `reconnect_review_required` | `session_ended` / `reconnect_available` / `authority_moved` | `redacted_transcript_default` | `container` | `remote_bridge_review` | `auto_narrow_on_terminal_boundary_stale` |
| Profiler capture | `profiler.capture` | `maintenance` | `maintenance_budget` | `session_surface_target` / `serialize_exact_duplicates` / `drop_if_stale` | `explicit_phase_boundary` | `evidence_only` | `blocked_until_manual_review` | `not_applicable` | `not_applicable` | `not_applicable` | `not_applicable` | `auto_narrow_on_missing_checkpoint_proof` |
...
| Companion handoff | `companion.handoff_package` | `upload_replication` | `replication_budget`, `provider_overlay_budget` | `handoff_subject` / `replace_superseded` / `refresh_on_resume` | `resumable_chunk_boundary` | `compatible_restore` | `reauthorize_before_resume` | `transcript_restored` / `reauthorization_required` / `authority_expired` | `metadata_only_default` | `remote` | `metadata_only_export` | `auto_narrow_on_restore_fidelity_stale` |
| Incident workspace | `incident.recovery_workspace_refresh` | `provider_overlay` | `provider_overlay_budget` | `session_surface_target` / `coalesce_stale_duplicates` / `refresh_on_resume` | `explicit_phase_boundary` | `transcript_only` | `transcript_preserved_no_rerun` | `transcript_restored` / `reconnect_available` / `authority_current` | `reviewed_full_transcript_opt_in` | `shared_control` | `shared_control_grant_required` | `auto_narrow_on_terminal_boundary_stale` |
| Infrastructure session | `infrastructure.overlay_probe` | `foreground` | `foreground_task_budget`, `provider_overlay_budget` | `session_surface_target` / `replace_superseded` / `refresh_on_resume` | `item_boundary` | `transcript_only` | `reconnect_review_required` | `session_ended` / `reconnect_available` / `policy_blocked` | `metadata_only_default` | `policy_blocked` | `policy_denied_safe_alternative` | `auto_block_on_missing_evidence` |
...
- Separate session-continuity rows now pin one honest continuity label, one
  resume requirement, and one authority status for every terminal-backed M5
  workload so `transcript restored`, `session ended`, `reconnect available`,
  `rerun required`, and `reauthorization required` stay explicit after restore.
- Separate transcript-export rows now pin bounded scrollback, redaction
  defaults, role/state labeling, prompt-boundary-marker preservation, and the
  rule that support bundles do not include raw transcript bodies by default.
- Shared-control incident rows now preserve `read_only_viewer`, `follower`, and
  `write_capable_participant` roles plus the matching request/activate/revoke
  and follow audit events in this same packet family.
